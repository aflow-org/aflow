...
namespace aflowlib {
  ...
  // class constructor
  EntryLoader::EntryLoader(std::ostream& oss) : xStream(oss) {
    init();
  }

  EntryLoader::EntryLoader(std::ofstream& FileMESSAGE, std::ostream& oss) : xStream(FileMESSAGE, oss) {
    init();
  }
  ...
  EntryLoader::EntryLoader(const EntryLoader& b) : xStream(*b.getOFStream(), *b.getOSS()) {
    copy(b);
  } // copy PUBLIC
  ...
  void EntryLoader::init() {
    ...
    m_entries_layered_map = std::make_shared<std::map<short, std::map<std::string, std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>>>>();
  ...
  void EntryLoader::free() {
    *this = {};
  } // calling the default constructor to ensure a clean instance of EntryLoader

  /// @brief create a copy (private)
  void EntryLoader::copy(const EntryLoader& b) {  // copy PRIVATE
    ...
    m_entries_layered_map = std::make_shared<std::map<short, std::map<std::string, std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>>>>(*b.m_entries_layered_map);
  }
  ...
  void EntryLoader::loadAUID(std::string AUID) {
    ...
    switch (m_current_source) {
      case Source::SQLITE: {
        const std::string where = "auid='\"" + AUID + "\"'";
        loadSqliteWhere(where);
        break;
      }

      case Source::AFLUX: {
        const std::map<std::string, std::string> matchbook{
            {   "*",               ""},
            {"auid", "'" + AUID + "'"}
        };
    ...
    switch (m_current_source) {
      ...
      case Source::RESTAPI_RAW: {
        ...
        for (std::vector<std::string>::const_iterator AUID_single = clean_AUID.begin(); AUID_single != clean_AUID.end(); AUID_single++) {
          ...
          for (uint part = 6; part <= 20; part += 2) {
            rest_query += AUID_single->substr(part, 2) + "/";
          }
          rest_query += "WEB/";
      ...
      case Source::FILESYSTEM_RAW: {
        ...
        for (std::vector<std::string>::const_iterator AUID_single = clean_AUID.begin(); AUID_single != clean_AUID.end(); AUID_single++) {
          ...
          for (uint part = 6; part <= 20; part += 2) {
            file_path += AUID_single->substr(part, 2) + "/";
          }
    ...
    switch (m_current_source) {
      ...
      case Source::RESTAPI_RAW: {
        ...
        for (std::vector<std::string>::const_iterator AURL_single = clean_AURL.begin(); AURL_single != clean_AURL.end(); AURL_single++) {
          const std::string rest_query = std::regex_replace(AURL_single->substr(28), aurostd::regex_aurl2file, "$1_" + m_sqlite_collection + "/") + "/";
          queries.push_back(rest_query);
        }
        loadRestAPIQueries(queries);
        break;
  ...
  void EntryLoader::loadAlloy(const std::string& alloy, bool recursive) {
    ...
    if (alloy.find(", ") != std::string::npos) {
      aurostd::string2tokens(alloy, alloy_elements, ", ");
    } else if (alloy.find(',') != std::string::npos) {
      aurostd::string2tokens(alloy, alloy_elements, ",");
    } else {
      alloy_elements = aurostd::getElements(alloy);
    }
  ...
  void EntryLoader::loadAlloy(const std::vector<std::string>& alloy, bool recursive) {
    ...
    for (std::vector<std::string>::const_iterator element = alloy.begin(); element != alloy.end(); element++) {
      if (xelement::xelement::isElement(*element) == 0) {
        ...
        alloy_clean.emplace_back(*element);
      }
    }
      ...
      for (size_t i = 1; i <= alloy_clean.size(); i++) {
        ...
        while (xc.increment()) {
          ...
          for (size_t idx = 0; idx < choice.size(); ++idx) {
            if (choice[idx]) {
              alloy_tmp.push_back(alloy_clean[idx]);
            }
          }
    ...
    for (std::vector<std::vector<std::string>>::iterator a = alloy_sub_list.begin(); a != alloy_sub_list.end(); a++) {
      ...
      for (std::vector<std::string>::iterator e = a->begin(); e != a->end(); e++) {
        alloy_tmp += *e;
      }
...
//    std::vector <std::string> raw_lines = getRawSqliteWhere(where);
    const vector<vector<string>> content = m_sqlite_db_ptr->getRowsMultiTables(where);
    const vector<string> keys = m_sqlite_db_ptr->getColumnNames("auid_00");

    m_logger_message << content.size() << " entries found in DB for " << where;
    outDebug(__AFLOW_FUNC__);

    const size_t start_size = m_entries_flat->size();
  ...
  /// @brief load entries from a custom AFLUX query
  ...
  void EntryLoader::loadFiles(const std::vector<std::string>& files) {
    const size_t start_size = m_entries_flat->size();
    const Source last_source = m_current_source;
  ...
  void EntryLoader::loadFolders(const std::vector<std::string>& folders) {
    ...
    while ((node = fts_read(tree))) {
      ...
      if ((node->fts_info & FTS_D)) {
        const std::string base_path = node->fts_path;
        const std::string full_path = base_path + "/" + m_filesystem_outfile;
  ...
  void EntryLoader::loadText(const std::vector<std::string>& raw_data_lines, const std::string& source) {
    for (std::vector<std::string>::const_iterator line = raw_data_lines.begin(); line != raw_data_lines.end(); line++) {
      const std::shared_ptr<aflowlib::_aflowlib_entry> entry = std::make_shared<aflowlib::_aflowlib_entry>();
      ...
      if (!entry->auid.empty() && (std::find(m_auid_list.begin(), m_auid_list.end(), entry->auid) == m_auid_list.end())) {
        ...
        if (m_xstructure_original) {
          addXstructure(*entry, true);
        }
        if (m_xstructure_relaxed) {
          addXstructure(*entry);
        }
      }
    }
  ...
  void EntryLoader::loadVector(const std::vector<std::string>& keys, const std::vector<std::vector<std::string>>& content, const std::string& source) {
    ...
    for (std::vector<std::string>::const_iterator key = keys.begin(); key != keys.end(); key++) {
      hash_list.emplace_back(aurostd::crc64(*key));
    }
    for (std::vector<std::vector<std::string>>::const_iterator row = content.begin(); row != content.end(); row++) {
      ...
      //NHA 20250926
      //check !entry->vcomposition.empty() prevents loading of entries where critical information is missing. This most likely means that the entry is a POCC parent.
      //If prototype contains POCC substring but not ARUN substring, it is a POCC parent
      if (!entry->auid.empty() && (std::find(m_auid_list.begin(), m_auid_list.end(), entry->auid) == m_auid_list.end())) {
        if (entry->vcomposition.empty() && aurostd::substring2bool(entry->prototype, "POCC") && !aurostd::substring2bool(entry->prototype, "ARUN")) {
          entry->pocc_parent = true;
          continue;
        }
        if (entry->vcomposition.empty()) {
          m_logger_message << "AFLOW entry has empty vcomposition: " << entry->auid;
          outError(__AFLOW_FUNC__, __LINE__);
          continue;
        }
        if (aurostd::substring2bool(entry->prototype, "POCC") && !aurostd::substring2bool(entry->prototype, "ARUN")) {
          continue;
        }
        m_entries_flat->push_back(entry);
        (*m_entries_layered_map)[entry->nspecies][entry->species].push_back(entry);
  ...
  bool EntryLoader::setSource(EntryLoader::Source new_source) {
    if (new_source == m_current_source) {
      return true;
    }
  ...
  std::vector<std::string> EntryLoader::getRawSqliteWhere(const std::string& where) const {
    if (m_sqlite_db_ptr != nullptr) {
      return m_sqlite_db_ptr->getEntrySet(where, aflow_ft);
    } else {
      return {};
    }
  }
  ...
  bool EntryLoader::loadXstructureFile(const aflowlib::_aflowlib_entry& entry, xstructure& new_structure, std::vector<std::string> possible_files) {
    const std::string base_url = m_restapi_server + m_restapi_path + entry.aurl.substr(28) + "/";
    std::string base_folder;
    const bool direct_fs = entry.el_source_type.substr(0, 10) == "FILESYSTEM"; // catch FILESYSTEM and FILESYSTEM_RAW
    if (direct_fs) {
    ...
    if (possible_files.empty()) {
      possible_files = m_xstructure_final_file_name;
    }
    ...
    for (std::vector<std::string>::const_iterator file_name = possible_files.begin(); file_name != possible_files.end(); file_name++) {
      if (aurostd::CompressFileWithinList(available_files, *file_name, selected_file)) {
        if (m_filesystem_available || direct_fs) {
          aurostd::compressfile2string(base_folder + selected_file, poscar);
        } else {
          poscar = getRawRestAPIQuery(base_url + selected_file, true);
        }
  ...
  bool EntryLoader::loadXstructureAflowIn(const aflowlib::_aflowlib_entry& entry, xstructure& new_structure, const int index) {
    ...
    if (m_filesystem_available || direct_fs) {
      ...
      const std::ifstream open_file(base_folder + "aflow.in");
    ...
    const bool poscar_loaded = KBIN::ExtractPOSCARStringStreamFromAFLOWIN(aflowin_content, poscar, index);
  ...
  void EntryLoader::getEntriesViewTwoLayer(vector<vector<std::shared_ptr<aflowlib::_aflowlib_entry>>>& result) const {
    for (std::map<short, std::map<std::string, std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>>>::iterator layer1 = m_entries_layered_map->begin(); layer1 != m_entries_layered_map->end(); layer1++) {
      ...
      for (std::map<std::string, std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>>::iterator layer2 = layer1->second.begin(); layer2 != layer1->second.end(); layer2++) {
        for (std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>::iterator entry = layer2->second.begin(); entry != layer2->second.end(); entry++) {
          collected_entries.push_back(*entry);
        }
      }
  ...
  void EntryLoader::getEntriesViewThreeLayer(std::vector<std::vector<std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>>>& result) const {
    for (std::map<short, std::map<std::string, std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>>>::iterator layer1 = m_entries_layered_map->begin(); layer1 != m_entries_layered_map->end(); layer1++) {
      ...
      for (std::map<std::string, std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>>::iterator layer2 = layer1->second.begin(); layer2 != layer1->second.end(); layer2++) {
        ...
        for (std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>::iterator entry = layer2->second.begin(); entry != layer2->second.end(); entry++) {
          collected_entries_l2.push_back(*entry);
        }
  ...
  ///       the loaded entries after the EntryLoader class goes out of scope
  void EntryLoader::getEntriesViewMap(std::map<short, std::map<std::string, std::vector<std::shared_ptr<aflowlib::_aflowlib_entry>>>>& result) const {
  ...
  void EntryLoader::selectSource() {
    ...
    if (m_current_source == Source::NONE || m_current_source == Source::FAILED) {
      ...
      if (EntryLoader::setSource(Source::FILESYSTEM)) {
        ...
        if (m_current_source == Source::FILESYSTEM) {
          m_logger_message << " Source::FILESYSTEM";
        } else if (m_current_source == Source::FILESYSTEM_RAW) {
          m_logger_message << " Source::FILESYSTEM_RAW";
        }
        outInfo(__AFLOW_FUNC__);
        return;
      }
      if (EntryLoader::setSource(Source::RESTAPI)) {
        m_out_super_silent = false;
        m_logger_message << "Automatically selected";
        if (m_current_source == Source::RESTAPI) {
          m_logger_message << " Source::RESTAPI";
        } else if (m_current_source == Source::RESTAPI_RAW) {
          m_logger_message << " Source::RESTAPI_RAW";
        }
        outInfo(__AFLOW_FUNC__);
        return;
      }
  ...
  bool EntryLoader::getCommandlineSource() {
    ...
    if (raw_source == "aflux") {
      return EntryLoader::setSource(Source::AFLUX);
    } else if (raw_source == "sqlite") {
      return EntryLoader::setSource(Source::SQLITE);
    } else if (raw_source == "file" || raw_source == "filesystem") {
      return EntryLoader::setSource(Source::FILESYSTEM);
    } else if (raw_source == "rest" || raw_source == "restapi") {
      return EntryLoader::setSource(Source::RESTAPI);
    } else {
      return false;
    }
  }
  ...
  void EntryLoader::listRestAPI(std::string url, std::vector<std::string>& result, bool directories) {
    result.clear();
    if (directories) {
      url += m_restapi_listing_dirs;
    } else {
      url += m_restapi_listing_files;
    }
    std::string output;
    if (200 == aurostd::httpGetStatus(url, output)) {
      const aurostd::JSON::object jo = aurostd::JSON::parse(output);
      result = jo;
  ...
  void EntryLoader::loadAlloySearchFSR(const std::vector<std::string>& alloy_list, uint lib_max) {
    ...
    size_t scanned = 0;
    size_t found = 0;
    const size_t check_idx = m_filesystem_path.size();
    struct stat file_stats{};
          ...
          if (node->fts_level == 2) {
            if (find(alloy_list.begin(), alloy_list.end(), extractAlloy(node->fts_name, 'I')) == alloy_list.end()) {
              ...
              if (stat(full_path.c_str(), &file_stats) == 0) {
                found += 1;
                found_entries.emplace_back(full_path);
                if (base_path.find("POCC") == std::string::npos) {
                  fts_set(tree, node, FTS_SKIP);
                }
              }
            }
    ...
    if (m_sqlite_alloy_db_ptr != nullptr) {
      ...
      for (std::vector<std::string>::const_iterator AUID = known_AUID_list.begin(); AUID != known_AUID_list.end(); AUID++) {
        if (std::find(m_auid_list.begin(), m_auid_list.end(), *AUID) == m_auid_list.end()) {
          missing_AUID.emplace_back(*AUID);
        }
      }
  ...
  void EntryLoader::loadAlloySearchRR(const std::vector<std::string>& alloy_list, uint lib_max) {
    ...
    while (!libx_crawl_list.empty()) {
      scanned += 1;
      const std::string url = libx_crawl_list.back();
  ...
  bool EntryLoader::cleanAUID(std::string& AUID) {
    if (AUID.find('\"') != std::string::npos) {
      aurostd::StringSubstInPlace(AUID, "\"", "");
    }
    if (AUID.substr(0, 5) == "auid:") {
      AUID = "aflow:" + AUID.substr(5);
    } else if (AUID.substr(0, 6) != "aflow:") {
      AUID = "aflow:" + AUID;
    }
    if (aurostd::_ishex(AUID.substr(6)) && AUID.size() == 22) {
      return true;
    } else {
      return false;
    }
  ...
  bool EntryLoader::cleanAURL(std::string& AURL) {
    if (AURL.substr(0, 5) == "aurl:") {
      AURL = AURL.substr(5);
    }
    if (AURL.substr(0, 28) == "aflowlib.duke.edu:AFLOWDATA/") {
      return true;
    }
  ...
  std::string EntryLoader::extractAlloy(std::string name, char lib_type) const {
    ...
    std::vector<std::string> element_match(std::sregex_token_iterator(name.begin(), name.end(), aurostd::regex_elements), std::sregex_token_iterator());
  ...
  void EntryLoader::outError(const std::string& function_name, int line_number) {
    if (!m_out_super_silent) {
      pflow::logger((std::string) __FILE__ + ":" + std::to_string(line_number), function_name, m_logger_message, *p_FileMESSAGE, *p_oss, _LOGGER_ERROR_);
    }
    aurostd::StringstreamClean(m_logger_message);
  }
