...
bool GetSphereFromFourPoints(xvector<double>& orig, double& radius, const xvector<double>& v1, const xvector<double>& v2, const xvector<double>& v3, const xvector<double>& v4) {
  ...
  A[1][4] = 0;
  A[1][5] = 1.0;
  A[2][1] = v1[1] * v1[1] + v1[2] * v1[2] + v1[3] * v1[3];
  A[2][2] = v1[1];
  A[2][3] = v1[2];
  A[2][4] = v1[3];
  A[2][5] = 1;
  A[3][1] = v2[1] * v2[1] + v2[2] * v2[2] + v2[3] * v2[3];
  A[3][2] = v2[1];
  A[3][3] = v2[2];
  A[3][4] = v2[3];
  A[3][5] = 1;
  A[4][1] = v3[1] * v3[1] + v3[2] * v3[2] + v3[3] * v3[3];
  A[4][2] = v3[1];
  A[4][3] = v3[2];
  A[4][4] = v3[3];
  A[4][5] = 1;
  A[5][1] = v4[1] * v4[1] + v4[2] * v4[2] + v4[3] * v4[3];
  A[5][2] = v4[1];
  A[5][3] = v4[2];
  A[5][4] = v4[3];
  A[5][5] = 1;

  const double M11 = minordeterminant(A, 1, 1);
  if (std::abs(M11) < eps) {
    return false;
  }
  const double M12 = minordeterminant(A, 1, 2);
  const double M13 = minordeterminant(A, 1, 3);
  const double M14 = minordeterminant(A, 1, 4);
  const double M15 = minordeterminant(A, 1, 5);
  ...
  const double d1 = modulus(v1 - orig);
  if (d1 < radius - eps || d1 > radius + eps) {
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Sphere Error [d1]", _RUNTIME_ERROR_);
  } // CO20200624
  const double d2 = modulus(v2 - orig);
  if (d2 < radius - eps || d2 > radius + eps) {
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Sphere Error [d2]", _RUNTIME_ERROR_);
  } // CO20200624
  const double d3 = modulus(v3 - orig);
  if (d3 < radius - eps || d3 > radius + eps) {
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Sphere Error [d3]", _RUNTIME_ERROR_);
  } // CO20200624
  const double d4 = modulus(v4 - orig);
  if (d4 < radius - eps || d4 > radius + eps) {
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Sphere Error [d4]", _RUNTIME_ERROR_);
  } // CO20200624
  return true;
}
...
bool GetCircumCircleeFromThreePoints(xvector<double>& orig, double& radius, const xvector<double>& v1, const xvector<double>& v2, const xvector<double>& v3) {
  ...
  if (std::abs(modulus(v1 - orig) - modulus(v2 - orig)) < eps && std::abs(modulus(v2 - orig) - modulus(v3 - orig)) < eps) {
    return true;
  }
...
bool FPositionInsideCell(const xvector<double>& r) {
  if (r[1] >= 0.0 && r[1] < 1.0 && r[2] >= 0.0 && r[2] < 1.0 && r[3] >= 0.0 && r[3] < 1.0) {
    return true;
  } else {
    return false;
  }
...
bool EmptySphere(const deque<_atom>& grid_atoms, const xvector<double>& origin_cpos, const double& radius) {
  const double eps = _EPS_;
  bool empty = true;
  //  for(size_t iat=0;iat<grid_atoms.size()&&empty==true;iat++)
  for (size_t iat = 0; iat < grid_atoms.size(); iat++) {
    //    if(abs(grid_atoms[iat].cpos[1]-1.728644884961154)<0.01)
    //     //   0.000000000099805   2.261436255651232
    //     cerr << grid_atoms[iat].cpos << "      " << grid_atoms[iat].fpos << endl;
    if (modulus(grid_atoms[iat].cpos - origin_cpos) < radius - 5 * eps) {
      empty = false;
    }
  }
  return empty;
...
bool EmptySphere(const xstructure& str, const xvector<double>& origin_cpos, const double& radius) {
  if (!str.grid_atoms_calculated) {
    throw aurostd::xerror(__AFLOW_FILE__, "EmptySphere():", "str.grid_atoms must be calculated", _INPUT_MISSING_);
  } // CO20200624
...
uint CoordinationPoint(const deque<_atom>& atoms, deque<_atom>& ratoms, const xvector<double>& point, const double& rmin, const double& rmax) {
  ...
  for (size_t iat = 0; iat < atoms.size(); iat++) {
    dist = modulus(atoms[iat].cpos - point);
    if (dist >= rmin - eps && dist <= rmax + eps) {
      out++;
      ratoms.push_back(atoms[iat]);
    }
  }
  return out;
}
...
uint CoordinationPoint(const xstructure& str, deque<_atom>& ratoms, const xvector<double>& point, const double& rmin, const double& rmax) {
  if (!str.grid_atoms_calculated) {
    throw aurostd::xerror(__AFLOW_FILE__, "CoordinationPoint():", "str.grid_atoms must be calculated", _INPUT_MISSING_);
  } // CO20200624
...
bool AddCageToCages(const xstructure& str,
                    ...
                    vector<acage>& cages,
                    vector<acage>& cagesX,
                    const bool& oss1write,
                    ostream& oss1,
                    const bool& oss2write,
                    ostream& oss2,
                    int ithread) {
  acage cage;
  const double eps = _EPS_;
  bool found = false;
  const string species = string_species;
  for (size_t ig = 0; ig < cages.size() && !found; ig++) {
  ...
  if (found == false) {
    ...
    const int ig = cagesX.size() - 1;
    // if(oss1write) oss1 << ig << " ";
    if (oss1write) {
      oss1 << "  ";
      for (int i = 1; i <= 3; i++) {
        if (cagesX.at(ig).origin_fpos[i] >= 0.0) {
          oss1 << " ";
        }
        oss1 << " " << cagesX.at(ig).origin_fpos[i];
      }
    }
    if (oss2write) {
      oss2 << "  ";
      for (int i = 1; i <= 3; i++) {
        ...
        oss2 << " " << cagesX.at(ig).origin_fpos[i];
      }
    }
    // if(oss1write) oss1 << "    i=" << ig;
    oss1.precision(_oss_short_precision_aflow_defects_);
    oss2.precision(_oss_short_precision_aflow_defects_);
    if (oss1write) {
      oss1 << "  " << aurostd::PaddedPRE(species, 2);
    }
    if (oss2write) {
      oss2 << "  " << aurostd::PaddedPRE(species, 2);
    }
    if (oss1write) {
      oss1 << "   " << cagesX.at(ig).radius << " ";
    }
    if (oss2write) {
      oss2 << "   " << cagesX.at(ig).radius << " ";
    }
    oss1.precision(_oss_precision_aflow_defects_);
    oss2.precision(_oss_precision_aflow_defects_);
    if (oss1write) {
      oss1 << "  -";// << (int) cagesX.at(ig).cages_irrtype;
    }
    if (oss2write) {
      oss2 << "  -";// << (int) cagesX.at(ig).cages_irrtype;
    }
    if (oss1write) {
      oss1 << "  " << (int) cagesX.at(ig).coordination_position;
    }
    if (oss2write) {
      oss2 << "  " << (int) cagesX.at(ig).coordination_position;
    }
    if (oss1write) {
      oss1 << "  " << cagesX.at(ig).cages_position;
    }
    if (oss2write) {
      oss2 << "  " << cagesX.at(ig).cages_position;
    }
    if (oss1write) {
      oss1 << "  [";
      for (size_t iat = 0; iat < cagesX.at(ig).atoms.size(); iat++) {
        oss1 << str.species.at(cagesX.at(ig).atoms[iat].type) << "(" << cagesX.at(ig).atoms[iat].basis << ")" << (iat < cagesX.at(ig).atoms.size() - 1 ? "," : "");
      }
      oss1 << "]";
    }
    if (oss2write) {
      oss2 << "  [";
      for (size_t iat = 0; iat < cagesX.at(ig).atoms.size(); iat++) {
        oss2 << str.species.at(cagesX.at(ig).atoms[iat].type) << "(" << cagesX.at(ig).atoms[iat].basis << ")" << (iat < cagesX.at(ig).atoms.size() - 1 ? "," : "");
      }
      oss2 << "]";
    }
    ...
    if (ithread >= 0) {
      if (oss1write) {
        oss1 << "  ithread=" << ithread;
      }
    }
    if (ithread >= 0) {
      if (oss2write) {
        oss2 << "  ithread=" << ithread;
      }
    }
    if (oss1write) {
      oss1 << endl;
...
uint GetCages4(const xstructure& str, const double& roughness, vector<acage>& cages, vector<acage>& cages4, const bool& oss1write, ostream& oss1, const bool& oss2write, ostream& oss2) {
  xvector<double> v1(3);
  xvector<double> v2(3);
  xvector<double> v3(3);
  xvector<double> v4(3);
  xvector<double> origin_cpos(3);
  xvector<double> origin_fpos(3);
  double radius;

  cages4.clear();
  bool found;
  const uint gridsize = str.grid_atoms.size();
...
void* _threaded_GetCages4(void* ptr) {
  ...
  // for(size_t iat1=0+(pparams->ITHREAD);iat1<(*pparams->pstr).grid_atoms.size();iat1+=(pparams->THREADS_MAX))
  for (size_t iat1 = 0; iat1 < (*pparams->pstr).atoms.size(); iat1++) {
    v1 = (*pparams->pstr).grid_atoms[iat1].cpos;
    // for(size_t iat2=iat1+1;iat2<(*pparams->pstr).grid_atoms.size();iat2++)
    for (size_t iat2 = iat1 + 1 + (pparams->ITHREAD); iat2 < (*pparams->pstr).grid_atoms.size(); iat2 += (pparams->THREADS_MAX)) {
      v2 = (*pparams->pstr).grid_atoms[iat2].cpos;
      for (size_t iat3 = iat2 + 1; iat3 < (*pparams->pstr).grid_atoms.size(); iat3++) {
        v3 = (*pparams->pstr).grid_atoms[iat3].cpos;
        for (size_t iat4 = iat3 + 1; iat4 < (*pparams->pstr).grid_atoms.size(); iat4++) {
          ...
          if (found) {
            ...
            if (FPositionInsideCell(origin_fpos)) {
              if (EmptySphere((*pparams->pstr), origin_cpos, radius)) {
                pthread_mutex_lock(&mutex_DEFECTS);
                AddCageToCages((*pparams->pstr), origin_cpos, origin_fpos, radius, 4, (*pparams->proughness), (*pparams->pcages), (*pparams->pcages4), (*pparams->poss1write), (*pparams->poss1),
                               (*pparams->poss2write), (*pparams->poss2), (pparams->ITHREAD));
                pthread_mutex_unlock(&mutex_DEFECTS);
              }
            }
          }
...
uint GetCages3(const xstructure& str, const double& roughness, vector<acage>& cages, vector<acage>& cages3, const bool& oss1write, ostream& oss1, const bool& oss2write, ostream& oss2) {
  xvector<double> v1(3);
  xvector<double> v2(3);
  xvector<double> v3(3);
  xvector<double> origin_cpos(3);
  ...
  cages3.clear();
  bool found;
  // for(size_t iat1=0;iat1<str.grid_atoms.size();iat1++)
  for (size_t iat1 = 0; iat1 < str.atoms.size(); iat1++) {
    v1 = str.grid_atoms[iat1].cpos;
    for (size_t iat2 = iat1 + 1; iat2 < str.grid_atoms.size(); iat2++) {
      v2 = str.grid_atoms[iat2].cpos;
      for (size_t iat3 = iat2 + 1; iat3 < str.grid_atoms.size(); iat3++) {
      ...
      for (size_t iat3 = iat2 + 1; iat3 < (*pparams->pstr).grid_atoms.size(); iat3++) {
        ...
        if (found) {
          ...
          if (FPositionInsideCell(origin_fpos)) {
            if (EmptySphere((*pparams->pstr), origin_cpos, radius)) {
              pthread_mutex_lock(&mutex_DEFECTS);
              AddCageToCages((*pparams->pstr), origin_cpos, origin_fpos, radius, 3, (*pparams->proughness), (*pparams->pcages), (*pparams->pcages3), (*pparams->poss1write), (*pparams->poss1),
                             (*pparams->poss2write), (*pparams->poss2), (pparams->ITHREAD));
              pthread_mutex_unlock(&mutex_DEFECTS);
            }
          }
...
uint GetCages2(const xstructure& str, const double& roughness, vector<acage>& cages, vector<acage>& cages2, const bool& oss1write, ostream& oss1, const bool& oss2write, ostream& oss2) {
  xvector<double> v1(3);
  xvector<double> v2(3);
  xvector<double> origin_cpos(3);
  xvector<double> origin_fpos(3);
...
void* _threaded_GetCages2(void* ptr) {
  ...
  // pthread_mutex_unlock( &mutex_DEFECTS );
  // for(size_t iat1=0;iat1<(*pparams->pstr).grid_atoms.size();iat1++)
  for (size_t iat1 = 0 + (pparams->ITHREAD); iat1 < (*pparams->pstr).grid_atoms.size(); iat1 += (pparams->THREADS_MAX)) {
    v1 = (*pparams->pstr).grid_atoms[iat1].cpos;
    for (size_t iat2 = iat1 + 1; iat2 < (*pparams->pstr).grid_atoms.size(); iat2++) {
      v2 = (*pparams->pstr).grid_atoms[iat2].cpos;
      ...
      if (found) {
        ...
        if (FPositionInsideCell(origin_fpos)) {
          if (EmptySphere((*pparams->pstr), origin_cpos, radius)) {
            pthread_mutex_lock(&mutex_DEFECTS);
            AddCageToCages((*pparams->pstr), origin_cpos, origin_fpos, radius, 2, (*pparams->proughness), (*pparams->pcages), (*pparams->pcages2), (*pparams->poss1write), (*pparams->poss1),
                           (*pparams->poss2write), (*pparams->poss2), (pparams->ITHREAD));
            pthread_mutex_unlock(&mutex_DEFECTS);
          }
        }
...
bool GetCages(const xstructure& _str, _aflags& aflags, vector<acage>& cagesirreducible, vector<acage>& cagesreducible, vector<acage>& cages4, vector<acage>& cages3, vector<acage>& cages2, double _roughness, bool FFFflag, bool osswrite, ostream& oss) {
  ...
  const xvector<double> v1(3);
  const xvector<double> v2(3);
  const xvector<double> v3(3);
  const xvector<double> v4(3);              // vectors for symmetry search
  const xvector<double> spherecenter_cpos(3);
  const xvector<double> spherecenter_fpos(3);
  double radius;
  const xvector<double> rrr(3);
  const xvector<double> origin_cpos(3);
  const xvector<double> origin_fpos(3);

  const double eps = _EPS_;
  double roughness = _roughness;
  double cagesirreducible_rmax;
  double cagesirreducible_rmin;
  double cagesreducible_rmax;
  double cagesreducible_rmin;
  double cage2_rmax;
  double cage2_rmin;
  double cage3_rmax;
  double cage3_rmin;
  ...
  string FileNameCAGE;
  if (FFFflag) {
    FileNameCAGE = DEFAULT_AFLOW_ICAGES_OUT;
  } else {
    FileNameCAGE = "/dev/null";
  }
  ...
  radius = aurostd::max(modulus(a1), modulus(a2), modulus(a3));
  xvector<int> dims(3);
  ...
  const string banner = "-------------------------------------------------------------------------------------------";
  //  oss << "------------------------------------------------------------------------------------" << endl;
  const bool PFSWRITE = FFFflag;  // ME20210521
  ofstream FileDevNull("/dev/null");
  aflags.QUIET = true;
  const bool OSSWRITE = false;// if(!OSSWRITE) PFSWRITE=false;
  oss << banner << endl; // ---------------------------------
  FFF << banner << endl; // ---------------------------------
  ...
  if (!AFLOW_PTHREADS::FLAG) {
    oss << "NO PTHREADS" << endl;
  }
  if (!AFLOW_PTHREADS::FLAG) {
    FFF << "NO PTHREADS" << endl;
  }
  if (AFLOW_PTHREADS::FLAG) {
    oss << "THREADS - AFLOW_PTHREADS::MAX_PTHREADS=" << AFLOW_PTHREADS::MAX_PTHREADS << endl;
  }
  if (AFLOW_PTHREADS::FLAG) {
    FFF << "THREADS - AFLOW_PTHREADS::MAX_PTHREADS=" << AFLOW_PTHREADS::MAX_PTHREADS << endl;
  }
  oss << banner << endl; // ---------------------------------
  FFF << banner << endl; // ---------------------------------
  ...
  if (OSSWRITE) {
    oss << "CAGES CALCULATION - Stefano Curtarolo" << endl;
  }
  if (OSSWRITE) {
    FFF << "CAGES CALCULATION - Stefano Curtarolo" << endl;
  }
  if (roughness < 0.0) {
    roughness = _CAGEROUGHNESS_;
  }
  ...
  const bool go3 = true;
  const bool go2 = true;
  const string caption = "        x(fract)          y(fract)          z(fract)       S   radius(A)   T  C  P   coordination";
  ...
  if (go4) {
    ...
    if (AFLOW_PTHREADS::FLAG) {
      ...
      for (int ithread = 0; ithread < AFLOW_PTHREADS::MAX_PTHREADS; ithread++) {
        AFLOW_PTHREADS::viret[ithread] = pthread_create(&(AFLOW_PTHREADS::vpthread[ithread]), nullptr, _threaded_GetCages4, (void*) &params[ithread]);
        //  aurostd::Sleep(10);
      }
      for (int ithread = 0; ithread < AFLOW_PTHREADS::MAX_PTHREADS; ithread++) {
  ...
  if (go3) {
    ...
    if (AFLOW_PTHREADS::FLAG) {
      ...
      for (int ithread = 0; ithread < AFLOW_PTHREADS::MAX_PTHREADS; ithread++) {
        AFLOW_PTHREADS::viret[ithread] = pthread_create(&(AFLOW_PTHREADS::vpthread[ithread]), nullptr, _threaded_GetCages3, (void*) &params[ithread]);
        //  aurostd::Sleep(10);
      }
      for (int ithread = 0; ithread < AFLOW_PTHREADS::MAX_PTHREADS; ithread++) {
        pthread_join((AFLOW_PTHREADS::vpthread[ithread]), nullptr);
  ...
  if (go2) {
    ...
    if (AFLOW_PTHREADS::FLAG) {
      ...
      for (int ithread = 0; ithread < AFLOW_PTHREADS::MAX_PTHREADS; ithread++) {
        AFLOW_PTHREADS::viret[ithread] = pthread_create(&(AFLOW_PTHREADS::vpthread[ithread]), nullptr, _threaded_GetCages2, (void*) &params[ithread]);
        //  aurostd::Sleep(10);
      }
      for (int ithread = 0; ithread < AFLOW_PTHREADS::MAX_PTHREADS; ithread++) {
        pthread_join((AFLOW_PTHREADS::vpthread[ithread]), nullptr);
  ...
  for (size_t i = 0; i < cagesreducible.size(); i++) {
    ...
    cage_found = false;
    for (size_t ii = 0; ii < cagesirreducible.size() && !cage_found; ii++) {
      if (std::abs(cagesreducible[i].radius - cagesirreducible[ii].radius) < eps) { // must have the same radius, at least
        iatom.cpos = cagesirreducible[ii].origin_cpos;
        iatom.fpos = C2F(str.lattice, iatom.cpos);
        // 	for(size_t fg=0;fg<str.fgroup.size()&&!cage_found;fg++)  // FGROUP
        // 	  tatom=SYM::ApplyAtom(iatom,str.fgroup[fg],str,true);  // FGROUP
        // 	  cage_found=(modulus(ratom.fpos-tatom.fpos)<eps);    // FGROUP
        for (size_t sg = 0; sg < str.sgroup.size() && !cage_found; sg++) {  // SGROUP
    ...
    if (cage_found == false) {                                     // new irreducible operation, generate and save it
      ...
      oss << "  ";
      for (int i = 1; i <= 3; i++) {
        if (cagesirreducible.at(ig).origin_fpos[i] >= 0.0) {
          oss << " ";
        }
        oss << " " << cagesirreducible.at(ig).origin_fpos[i];
      }
      FFF << "  ";
      for (int i = 1; i <= 3; i++) {
        if (cagesirreducible.at(ig).origin_fpos[i] >= 0.0) {
        ...
        FFF << " " << cagesirreducible.at(ig).origin_fpos[i];
      }
      // oss << "    i=" << ig;
      oss.precision(_oss_short_precision_aflow_defects_);
      oss << "  " << aurostd::PaddedPRE(species, 2);
      oss << "   " << cagesirreducible.at(ig).radius << " ";
      oss << "  " << cagesirreducible.at(ig).cages_irrtype;
      oss << "  " << (int) cagesirreducible.at(ig).coordination_position;
      oss << "  " << cagesirreducible.at(ig).cages_position;
      {
        oss << "  [";
        for (size_t iat = 0; iat < cagesirreducible.at(ig).atoms.size(); iat++) {
          oss << str.species.at(cagesirreducible.at(ig).atoms[iat].type) << "(" << cagesirreducible.at(ig).atoms[iat].basis << ")" << (iat < cagesirreducible.at(ig).atoms.size() - 1 ? "," : "");
        }
        oss << "]";
      }
      oss.precision(_oss_precision_aflow_defects_);
      ...
      FFF << "  " << cagesirreducible.at(ig).cages_irrtype;
      FFF << "  " << (int) cagesirreducible.at(ig).coordination_position;
      FFF << "  " << cagesirreducible.at(ig).cages_position;
      {
        FFF << "  [";
        for (size_t iat = 0; iat < cagesirreducible.at(ig).atoms.size(); iat++) {
          FFF << str.species.at(cagesirreducible.at(ig).atoms[iat].type) << "(" << cagesirreducible.at(ig).atoms[iat].basis << ")" << (iat < cagesirreducible.at(ig).atoms.size() - 1 ? "," : "");
        }
        FFF << "]";
      }
      FFF.precision(_oss_precision_aflow_defects_);
  ...
  oss << caption << endl;
  FFF << caption << endl;
  for (size_t ig = 0; ig < cagesirreducible.size(); ig++) {
    oss << "  ";
    for (int i = 1; i <= 3; i++) {
      if (cagesirreducible[ig].origin_fpos[i] >= 0.0) {
        oss << " ";
      }
      oss << " " << cagesirreducible[ig].origin_fpos[i];
    }
    FFF << "  ";
    for (int i = 1; i <= 3; i++) {
      if (cagesirreducible[ig].origin_fpos[i] >= 0.0) {
      ...
      FFF << " " << cagesirreducible[ig].origin_fpos[i];
    }
    // oss << "    i=" << ig;
    oss.precision(_oss_short_precision_aflow_defects_);
    oss << "  " << aurostd::PaddedPRE(species, 2);
    oss << "   " << cagesirreducible[ig].radius << " ";
    oss << "  " << cagesirreducible[ig].cages_irrtype;
    oss << "  " << (int) cagesirreducible[ig].coordination_position;
    oss << "  " << cagesirreducible[ig].cages_position;
    {
      oss << "  [";
      for (size_t iat = 0; iat < cagesirreducible[ig].atoms.size(); iat++) {
        oss << str.species.at(cagesirreducible[ig].atoms[iat].type) << "(" << cagesirreducible[ig].atoms[iat].basis << ")" << (iat < cagesirreducible[ig].atoms.size() - 1 ? "," : "");
      }
      oss << "]";
    }
    oss.precision(_oss_precision_aflow_defects_);
    ...
    FFF.precision(_oss_short_precision_aflow_defects_);
    FFF << "  " << aurostd::PaddedPRE(species, 2);
    FFF << "   " << cagesirreducible[ig].radius << " ";
    FFF << "  " << cagesirreducible[ig].cages_irrtype;
    FFF << "  " << (int) cagesirreducible[ig].coordination_position;
    FFF << "  " << cagesirreducible[ig].cages_position;
    {
      FFF << "  [";
      for (size_t iat = 0; iat < cagesirreducible[ig].atoms.size(); iat++) {
        FFF << str.species.at(cagesirreducible[ig].atoms[iat].type) << "(" << cagesirreducible[ig].atoms[iat].basis << ")" << (iat < cagesirreducible[ig].atoms.size() - 1 ? "," : "");
      }
      FFF << "]";
    }
    FFF.precision(_oss_precision_aflow_defects_);
    FFF << endl;
    //   oss << " ("; for(int i=1;i<=3;i++) {if(cagesirreducible[ig].origin_cpos[i]>=0.0) oss << " "; oss << " " << cagesirreducible[ig].origin_cpos[i];};oss << ")c" << endl;
    //    FFF << " ("; for(int i=1;i<=3;i++) {if(cagesirreducible[ig].origin_cpos[i]>=0.0) FFF << " "; FFF << " " << cagesirreducible[ig].origin_cpos[i];};FFF << ")c" << endl;

    for (size_t jg = 0; jg < cagesreducible.size(); jg++) {
      if (std::abs(cagesreducible[jg].cages_irrtype - cagesirreducible.at(ig).cages_irrtype) < eps) {
        oss.precision(_oss_short_precision_aflow_defects_);
        oss << "      equiv ";
        for (int i = 1; i <= 3; i++) {
          if (cagesreducible[jg].origin_fpos[i] >= 0.0) {
            oss << " ";
          }
          oss << " " << cagesreducible[jg].origin_fpos[i];
        }
        oss.precision(_oss_precision_aflow_defects_);
        oss << "   type=" << cagesreducible[jg].cages_irrtype << " ";
        oss << endl;
        FFF.precision(_oss_short_precision_aflow_defects_);
        FFF << "      equiv ";
        for (int i = 1; i <= 3; i++) {
          ...
          FFF << " " << cagesreducible[jg].origin_fpos[i];
        }
        FFF.precision(_oss_precision_aflow_defects_);
        FFF << "   type=" << cagesreducible[jg].cages_irrtype << " ";
        FFF << endl;
      }
    }
  }
  ...
  if (cagesreducible_rmax) {
    ;
  } // phony
  if (cagesreducible_rmin) {
    ;
  } // phony
  if (cagesirreducible_rmax) {
    ;
  } // phony
  if (cagesirreducible_rmin) {
    ;
  } // phony
  ...
  for (size_t ig = 0; ig < cagesirreducible.size(); ig++) {
    xstructure straus(str);
    straus.write_inequivalent_flag = false;
    stringstream aus;
    aus << " R=" << cagesirreducible[ig].radius << " T=" << cagesirreducible[ig].cages_irrtype;
    aus << " C=" << (int) cagesirreducible[ig].coordination_position << " P=" << cagesirreducible[ig].cages_position;
    {
      aus << " [";
      for (size_t iat = 0; iat < cagesirreducible[ig].atoms.size(); iat++) {
        aus << str.species.at(cagesirreducible[ig].atoms[iat].type) << "(" << cagesirreducible[ig].atoms[iat].basis << ")" << (iat < cagesirreducible[ig].atoms.size() - 1 ? "," : "");
      }
      aus << "]";
    }
    ...
    _atom atom;
    atom = str.atoms.at(0);
    atom.fpos = cagesirreducible[ig].origin_fpos;
    atom.cpos = cagesirreducible[ig].origin_cpos;
    atom.type = str.species.size();       // to force an extra species
    atom.name = "H";
    atom.cleanname = "H";   // atom.name=string_species;atom.cleanname=string_species;
    // ADD atom, put alphabetic
    straus.AddAtom(atom, false); // CO20230319 - add by type
    straus.SpeciesPutAlphabetic();
    // make structure
    vstr.push_back(straus);
    vtitle.push_back("cages_T" + aurostd::utype2string(cagesirreducible[ig].cages_irrtype));
  }

  aflowin << AFLOWIN_SEPARATION_LINE << endl;
  aflowin << "[AFLOW] CAGES IN AFLOW.IN FORMAT" << endl;
  aflowin << AFLOWIN_SEPARATION_LINE << endl;
  for (size_t ivstr = 0; ivstr < vstr.size(); ivstr++) {
  ...
  //  cout << aflowin.str();
  if (FFFflag) {
    aurostd::stringstream2file(aflowin, _AFLOWIN_ + ".part");  // ME20210521
  }
