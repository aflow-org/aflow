...
namespace pflow {
  void XelementPrint(const string& options, ostream& oss) {
    const bool LDEBUG = false;//(false || XHOST.DEBUG);
    if (LDEBUG) {
    ...
    if (LDEBUG) {
      cerr << "velement.size()=" << velement.size() << endl;
    }
    ...
    // move on
    const string species = tokens.at(0);
    uint Z = 0; // some defaults
    xelement::xelement xel;
    // try with number
    if (!tokens.empty()) {
      if (aurostd::string2utype<uint>(species) > 0) {
        Z = aurostd::string2utype<uint>(species);
      }
    }
    if (Z > 103) {
    ...
    if (Z == 0) {
      for (uint i = 1; i <= 103; i++) {
        xel.populate(i);
        if (aurostd::toupper(species) == aurostd::toupper(xel.symbol)) {
          Z = i;
        }
      }
    }
    ...
    if (Z == 0) {
      for (uint i = 1; i <= 103; i++) {
        xel.populate(i);
        if (aurostd::toupper(species) == aurostd::toupper(xel.name)) {
          Z = i;
        }
      }
    }
    ...
    if (Z > 0) {
      ...
      if (tokens.size() >= 2) {
        ...
        vector<string> vs;
        const uint len = 52;
        for (size_t i = 1; i < tokens.size(); i++) { // HE20220301 fixing bug that disabled "ALL" from working
          c = aurostd::toupper(tokens[i]);
          vs.clear();
          if (c == "ALL" || c == aurostd::toupper("name")) {
            units = xel.getUnits("name");
            vs.push_back(aurostd::PaddedPOST("name=" + xel.name, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("symbol")) {
            units = xel.getUnits("symbol");
            vs.push_back(aurostd::PaddedPOST("symbol=" + xel.symbol, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("Z")) {
            units = xel.getUnits("Z");
            vs.push_back(aurostd::PaddedPOST("Z=" + aurostd::utype2string(xel.Z), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("period")) {
            units = xel.getUnits("period");
            vs.push_back(aurostd::PaddedPOST("period=" + aurostd::utype2string(xel.period), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("group")) {
            units = xel.getUnits("group");
            vs.push_back(aurostd::PaddedPOST("group=" + aurostd::utype2string(xel.group), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("series")) {
            units = xel.getUnits("series");
            vs.push_back(aurostd::PaddedPOST("series=" + xel.series, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("block")) {
            units = xel.getUnits("block");
            vs.push_back(aurostd::PaddedPOST("block=" + xel.block, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          //
          if (c == "ALL" || c == aurostd::toupper("mass")) {
            units = xel.getUnits("mass");
            vs.push_back(aurostd::PaddedPOST("mass=" + aurostd::utype2string(xel.mass, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("volume_molar")) {
            units = xel.getUnits("volume_molar");
            vs.push_back(aurostd::PaddedPOST("volume_molar=" + aurostd::utype2string(xel.volume_molar, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("volume")) {
            units = xel.getUnits("volume");
            vs.push_back(aurostd::PaddedPOST("volume=" + aurostd::utype2string(xel.volume, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("area_molar_Miedema")) {
            units = xel.getUnits("area_molar_Miedema");
            vs.push_back(aurostd::PaddedPOST("area_molar_Miedema=" + aurostd::utype2string(xel.area_molar_Miedema, _DOUBLE_WRITE_PRECISION_), len) + " // (" + units + ") (V_m^{2/3})");
          }
          //
          if (c == "ALL" || c == aurostd::toupper("valence_std")) {
            units = xel.getUnits("valence_std");
            vs.push_back(aurostd::PaddedPOST("valence_std=" + aurostd::utype2string(xel.valence_std, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("valence_iupac")) {
            units = xel.getUnits("valence_iupac");
            vs.push_back(aurostd::PaddedPOST("valence_iupac=" + aurostd::utype2string(xel.valence_iupac, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("valence_PT")) {
            units = xel.getUnits("valence_PT");
            vs.push_back(aurostd::PaddedPOST("valence_PT=" + aurostd::utype2string(xel.valence_PT, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("valence_s")) {
            units = xel.getUnits("valence_s");
            vs.push_back(aurostd::PaddedPOST("valence_s=" + aurostd::utype2string(xel.valence_s, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }  // CO20201111
          if (c == "ALL" || c == aurostd::toupper("valence_p")) {
            units = xel.getUnits("valence_p");
            vs.push_back(aurostd::PaddedPOST("valence_p=" + aurostd::utype2string(xel.valence_p, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }  // CO20201111
          if (c == "ALL" || c == aurostd::toupper("valence_d")) {
            units = xel.getUnits("valence_d");
            vs.push_back(aurostd::PaddedPOST("valence_d=" + aurostd::utype2string(xel.valence_d, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }  // CO20201111
          if (c == "ALL" || c == aurostd::toupper("valence_f")) {
            units = xel.getUnits("valence_f");
            vs.push_back(aurostd::PaddedPOST("valence_f=" + aurostd::utype2string(xel.valence_f, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }  // CO20201111
          if (c == "ALL" || c == aurostd::toupper("density_PT")) {
            units = xel.getUnits("density_PT");
            vs.push_back(aurostd::PaddedPOST("density_PT=" + aurostd::utype2string(xel.density_PT, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("crystal")) {
            units = xel.getUnits("crystal");
            vs.push_back(aurostd::PaddedPOST("crystal=" + xel.crystal, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("crystal_structure_PT")) {
            units = xel.getUnits("crystal_structure_PT");
            vs.push_back(aurostd::PaddedPOST("crystal_structure_PT=" + xel.crystal_structure_PT, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("spacegroup")) {
            units = xel.getUnits("spacegroup");
            vs.push_back(aurostd::PaddedPOST("spacegroup=" + xel.spacegroup, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("spacegroup_number")) {
            units = xel.getUnits("spacegroup_number");
            vs.push_back(aurostd::PaddedPOST("spacegroup_number=" + aurostd::utype2string(xel.spacegroup_number), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("variance_parameter_mass")) {
            units = xel.getUnits("variance_parameter_mass");
            vs.push_back(aurostd::PaddedPOST("variance_parameter_mass=" + aurostd::utype2string(xel.variance_parameter_mass, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("lattice_constants")) {
            units = xel.getUnits("lattice_constants");
            vs.push_back(aurostd::PaddedPOST("lattice_constants=" + aurostd::utype2string(xel.lattice_constants[1], _DOUBLE_WRITE_PRECISION_) + "," +
                                                 aurostd::utype2string(xel.lattice_constants[2], _DOUBLE_WRITE_PRECISION_) + "," + aurostd::utype2string(xel.lattice_constants[3], _DOUBLE_WRITE_PRECISION_),
                                             len) +
                         (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("lattice_angles")) {
            units = xel.getUnits("lattice_angles");
            vs.push_back(aurostd::PaddedPOST("lattice_angles=" + aurostd::utype2string(xel.lattice_angles[1], _DOUBLE_WRITE_PRECISION_) + "," +
                                                 aurostd::utype2string(xel.lattice_angles[2], _DOUBLE_WRITE_PRECISION_) + "," + aurostd::utype2string(xel.lattice_angles[3], _DOUBLE_WRITE_PRECISION_),
                                             len) +
                         (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("phase")) {
            units = xel.getUnits("phase");
            vs.push_back(aurostd::PaddedPOST("phase=" + xel.phase, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("radius_Saxena")) {
            units = xel.getUnits("radius_Saxena");
            vs.push_back(aurostd::PaddedPOST("radius_Saxena=" + aurostd::utype2string(xel.radius_Saxena, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("radius_PT")) {
            units = xel.getUnits("radius_PT");
            vs.push_back(aurostd::PaddedPOST("radius_PT=" + aurostd::utype2string(xel.radius_PT, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("radius_covalent_PT")) {
            units = xel.getUnits("radius_covalent_PT");
            vs.push_back(aurostd::PaddedPOST("radius_covalent_PT=" + aurostd::utype2string(xel.radius_covalent_PT, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("radius_covalent")) {
            units = xel.getUnits("radius_covalent");
            vs.push_back(aurostd::PaddedPOST("radius_covalent=" + aurostd::utype2string(xel.radius_covalent, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("radius_VanDerWaals_PT")) {
            units = xel.getUnits("radius_VanDerWaals_PT");
            vs.push_back(aurostd::PaddedPOST("radius_VanDerWaals_PT=" + aurostd::utype2string(xel.radius_VanDerWaals_PT, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("radii_Ghosh08")) {
            units = xel.getUnits("radii_Ghosh08");
            vs.push_back(aurostd::PaddedPOST("radii_Ghosh08=" + aurostd::utype2string(xel.radii_Ghosh08, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("radii_Slatter")) {
            units = xel.getUnits("radii_Slatter");
            vs.push_back(aurostd::PaddedPOST("radii_Slatter=" + aurostd::utype2string(xel.radii_Slatter, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("radii_Pyykko")) {
            units = xel.getUnits("radii_Pyykko");
            vs.push_back(aurostd::PaddedPOST("radii_Pyykko=" + aurostd::utype2string(xel.radii_Pyykko, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          //
          if (c == "ALL" || c == aurostd::toupper("conductivity_electrical")) {
            units = xel.getUnits("conductivity_electrical");
            vs.push_back(aurostd::PaddedPOST("conductivity_electrical=" + aurostd::utype2string(xel.conductivity_electrical, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("electronegativity_Pauling")) {
            units = xel.getUnits("electronegativity_Pauling");
            vs.push_back(aurostd::PaddedPOST("electronegativity_Pauling=" + aurostd::utype2string(xel.electronegativity_Pauling, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("hardness_chemical_Ghosh")) {
            units = xel.getUnits("hardness_chemical_Ghosh");
            vs.push_back(aurostd::PaddedPOST("hardness_chemical_Ghosh=" + aurostd::utype2string(xel.hardness_chemical_Ghosh, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("electronegativity_Pearson")) {
            units = xel.getUnits("electronegativity_Pearson");
            vs.push_back(aurostd::PaddedPOST("electronegativity_Pearson=" + aurostd::utype2string(xel.electronegativity_Pearson, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("electronegativity_Ghosh")) {
            units = xel.getUnits("electronegativity_Ghosh");
            vs.push_back(aurostd::PaddedPOST("electronegativity_Ghosh=" + aurostd::utype2string(xel.electronegativity_Ghosh, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("electronegativity_Allen")) {
            units = xel.getUnits("electronegativity_Allen");
            vs.push_back(aurostd::PaddedPOST("electronegativity_Allen=" + aurostd::utype2string(xel.electronegativity_Allen, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          } // CO20200731
          if (c == "ALL" || c == aurostd::toupper("oxidation_states")) {
            units = xel.getUnits("oxidation_states");
            vs.push_back(aurostd::PaddedPOST("oxidation_states=" + aurostd::joinWDelimiter(aurostd::vecDouble2vecString(xel.oxidation_states, _DOUBLE_WRITE_PRECISION_), ","), len) +
                         (units.empty() ? "" : " // (" + units + ")"));
          } // CO20200731
          if (c == "ALL" || c == aurostd::toupper("oxidation_states_preferred")) {
            units = xel.getUnits("oxidation_states_preferred");
            vs.push_back(aurostd::PaddedPOST("oxidation_states_preferred=" + aurostd::joinWDelimiter(aurostd::vecDouble2vecString(xel.oxidation_states_preferred, _DOUBLE_WRITE_PRECISION_), ","), len) +
                         (units.empty() ? "" : " // (" + units + ")"));
          } // CO20200731
          if (c == "ALL" || c == aurostd::toupper("electron_affinity_PT")) {
            units = xel.getUnits("electron_affinity_PT");
            vs.push_back(aurostd::PaddedPOST("electron_affinity_PT=" + aurostd::utype2string(xel.electron_affinity_PT, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("energies_ionization")) {
            units = xel.getUnits("energies_ionization");
            vs.push_back(aurostd::PaddedPOST("energies_ionization=" + aurostd::joinWDelimiter(aurostd::vecDouble2vecString(xel.energies_ionization, _DOUBLE_WRITE_PRECISION_), ","), len) +
                         (units.empty() ? "" : " // (" + units + ")"));
          } // CO20201111
          if (c == "ALL" || c == aurostd::toupper("work_function_Miedema")) {
            units = xel.getUnits("work_function_Miedema");
            vs.push_back(aurostd::PaddedPOST("work_function_Miedema=" + aurostd::utype2string(xel.work_function_Miedema, _DOUBLE_WRITE_PRECISION_), len) + " // (" + units + ") (phi^{star})");
          }
          if (c == "ALL" || c == aurostd::toupper("density_line_electron_WS_Miedema")) {
            units = xel.getUnits("density_line_electron_WS_Miedema");
            vs.push_back(aurostd::PaddedPOST("density_line_electron_WS_Miedema=" + aurostd::utype2string(xel.density_line_electron_WS_Miedema, _DOUBLE_WRITE_PRECISION_), len) + " // (" + units +
                         ") (n_{ws}^{1/3})");
          }
          if (c == "ALL" || c == aurostd::toupper("energy_surface_0K_Miedema")) {
            units = xel.getUnits("energy_surface_0K_Miedema");
            vs.push_back(aurostd::PaddedPOST("energy_surface_0K_Miedema=" + aurostd::utype2string(xel.energy_surface_0K_Miedema, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          //
          if (c == "ALL" || c == aurostd::toupper("chemical_scale_Pettifor")) {
            units = xel.getUnits("chemical_scale_Pettifor");
            vs.push_back(aurostd::PaddedPOST("chemical_scale_Pettifor=" + aurostd::utype2string(xel.chemical_scale_Pettifor, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("Mendeleev_number")) {
            units = xel.getUnits("Mendeleev_number");
            vs.push_back(aurostd::PaddedPOST("Mendeleev_number=" + aurostd::utype2string(xel.Mendeleev_number, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }   // CO20201111
          //
          if (c == "ALL" || c == aurostd::toupper("temperature_boiling")) {
            units = xel.getUnits("temperature_boiling");
            vs.push_back(aurostd::PaddedPOST("temperature_boiling=" + aurostd::utype2string(xel.temperature_boiling, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("temperature_melting")) {
            units = xel.getUnits("temperature_melting");
            vs.push_back(aurostd::PaddedPOST("temperature_melting=" + aurostd::utype2string(xel.temperature_melting, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("enthalpy_fusion")) {
            units = xel.getUnits("enthalpy_fusion");
            vs.push_back(aurostd::PaddedPOST("enthalpy_fusion=" + aurostd::utype2string(xel.enthalpy_fusion, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }  // CO20201111
          if (c == "ALL" || c == aurostd::toupper("enthalpy_vaporization")) {
            units = xel.getUnits("enthalpy_vaporization");
            vs.push_back(aurostd::PaddedPOST("enthalpy_vaporization=" + aurostd::utype2string(xel.enthalpy_vaporization, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("enthalpy_atomization_WE")) {
            units = xel.getUnits("enthalpy_atomization_WE");
            vs.push_back(aurostd::PaddedPOST("enthalpy_atomization_WE=" + aurostd::utype2string(xel.enthalpy_atomization_WE, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }  // CO20201111
          if (c == "ALL" || c == aurostd::toupper("energy_cohesive")) {
            units = xel.getUnits("energy_cohesive");
            vs.push_back(aurostd::PaddedPOST("energy_cohesive=" + aurostd::utype2string(xel.energy_cohesive, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }  // CO20201111
          if (c == "ALL" || c == aurostd::toupper("specific_heat_PT")) {
            units = xel.getUnits("specific_heat_PT");
            vs.push_back(aurostd::PaddedPOST("specific_heat_PT=" + aurostd::utype2string(xel.specific_heat_PT, _DOUBLE_WRITE_PRECISION_), len) + " // (" + units + "))");
          }
          if (c == "ALL" || c == aurostd::toupper("critical_pressure")) {
            units = xel.getUnits("critical_pressure");
            vs.push_back(aurostd::PaddedPOST("critical_pressure=" + aurostd::utype2string(xel.critical_pressure, _DOUBLE_WRITE_PRECISION_), len) + " // (" + units + ") ");
          }
          if (c == "ALL" || c == aurostd::toupper("critical_temperature_PT")) {
            units = xel.getUnits("critical_temperature_PT");
            vs.push_back(aurostd::PaddedPOST("critical_temperature_PT=" + aurostd::utype2string(xel.critical_temperature_PT, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("thermal_expansion")) {
            units = xel.getUnits("thermal_expansion");
            vs.push_back(aurostd::PaddedPOST("thermal_expansion=" + aurostd::utype2string(xel.thermal_expansion, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("conductivity_thermal")) {
            units = xel.getUnits("conductivity_thermal");
            vs.push_back(aurostd::PaddedPOST("conductivity_thermal=" + aurostd::utype2string(xel.conductivity_thermal, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          //
          if (c == "ALL" || c == aurostd::toupper("hardness_mechanical_Brinell")) {
            units = xel.getUnits("hardness_mechanical_Brinell");
            vs.push_back(aurostd::PaddedPOST("hardness_mechanical_Brinell=" + aurostd::utype2string(xel.hardness_mechanical_Brinell, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("hardness_mechanical_Mohs")) {
            units = xel.getUnits("hardness_mechanical_Mohs");
            vs.push_back(aurostd::PaddedPOST("hardness_mechanical_Mohs=" + aurostd::utype2string(xel.hardness_mechanical_Mohs, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("hardness_mechanical_Vickers")) {
            units = xel.getUnits("hardness_mechanical_Vickers");
            vs.push_back(aurostd::PaddedPOST("hardness_mechanical_Vickers=" + aurostd::utype2string(xel.hardness_mechanical_Vickers, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("hardness_chemical_Pearson")) {
            units = xel.getUnits("hardness_chemical_Pearson");
            vs.push_back(aurostd::PaddedPOST("hardness_chemical_Pearson=" + aurostd::utype2string(xel.hardness_chemical_Pearson, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("hardness_chemical_Putz")) {
            units = xel.getUnits("hardness_chemical_Putz");
            vs.push_back(aurostd::PaddedPOST("hardness_chemical_Putz=" + aurostd::utype2string(xel.hardness_chemical_Putz, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("hardness_chemical_RB")) {
            units = xel.getUnits("hardness_chemical_RB");
            vs.push_back(aurostd::PaddedPOST("hardness_chemical_RB=" + aurostd::utype2string(xel.hardness_chemical_RB, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("modulus_shear")) {
            units = xel.getUnits("modulus_shear");
            vs.push_back(aurostd::PaddedPOST("modulus_shear=" + aurostd::utype2string(xel.modulus_shear, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("modulus_Young")) {
            units = xel.getUnits("modulus_Young");
            vs.push_back(aurostd::PaddedPOST("modulus_Young=" + aurostd::utype2string(xel.modulus_Young, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("modulus_bulk")) {
            units = xel.getUnits("modulus_bulk");
            vs.push_back(aurostd::PaddedPOST("modulus_bulk=" + aurostd::utype2string(xel.modulus_bulk, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("Poisson_ratio_PT")) {
            units = xel.getUnits("Poisson_ratio_PT");
            vs.push_back(aurostd::PaddedPOST("Poisson_ratio_PT=" + aurostd::utype2string(xel.Poisson_ratio_PT, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("modulus_bulk_x_volume_molar_Miedema")) {
            units = xel.getUnits("modulus_bulk_x_volume_molar_Miedema");
            vs.push_back(aurostd::PaddedPOST("modulus_bulk_x_volume_molar_Miedema=" + aurostd::utype2string(xel.modulus_bulk_x_volume_molar_Miedema, _DOUBLE_WRITE_PRECISION_), len) +
                         (units.empty() ? "" : " // (" + units + ")"));
          }
          //
          if (c == "ALL" || c == aurostd::toupper("magnetic_type_PT")) {
            units = xel.getUnits("magnetic_type_PT");
            vs.push_back(aurostd::PaddedPOST("magnetic_type_PT=" + xel.magnetic_type_PT, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("susceptibility_magnetic_mass")) {
            units = xel.getUnits("susceptibility_magnetic_mass");
            vs.push_back(aurostd::PaddedPOST("susceptibility_magnetic_mass=" + aurostd::utype2string(xel.susceptibility_magnetic_mass, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("susceptibility_magnetic_volume")) {
            units = xel.getUnits("susceptibility_magnetic_volume");
            vs.push_back(aurostd::PaddedPOST("susceptibility_magnetic_volume=" + aurostd::utype2string(xel.susceptibility_magnetic_volume, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("susceptibility_magnetic_molar")) {
            units = xel.getUnits("susceptibility_magnetic_molar");
            vs.push_back(aurostd::PaddedPOST("susceptibility_magnetic_molar=" + aurostd::utype2string(xel.susceptibility_magnetic_molar, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("temperature_Curie")) {
            units = xel.getUnits("temperature_Curie");
            vs.push_back(aurostd::PaddedPOST("temperature_Curie=" + aurostd::utype2string(xel.temperature_Curie, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          //
          if (c == "ALL" || c == aurostd::toupper("refractive_index")) {
            units = xel.getUnits("refractive_index");
            vs.push_back(aurostd::PaddedPOST("refractive_index=" + aurostd::utype2string(xel.refractive_index, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("color_PT")) {
            units = xel.getUnits("color_PT");
            vs.push_back(aurostd::PaddedPOST("color_PT=" + xel.color_PT, len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          //
          if (c == "ALL" || c == aurostd::toupper("HHIP")) {
            units = xel.getUnits("HHIP");
            vs.push_back(aurostd::PaddedPOST("HHIP=" + aurostd::utype2string(xel.HHIP, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("HHIR")) {
            units = xel.getUnits("HHIR");
            vs.push_back(aurostd::PaddedPOST("HHIR=" + aurostd::utype2string(xel.HHIR, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          }
          if (c == "ALL" || c == aurostd::toupper("xray_scatt")) {
            units = xel.getUnits("xray_scatt");
            vs.push_back(aurostd::PaddedPOST("xray_scatt=" + aurostd::utype2string(xel.xray_scatt, _DOUBLE_WRITE_PRECISION_), len) + (units.empty() ? "" : " // (" + units + ")"));
          } // CO20201111 //shift+1
...
namespace xelement {
  ...
  void xelement::free() { // CO20200520
    ...
    lattice_constants[1] = NNN;
    lattice_constants[2] = NNN;
    lattice_constants[3] = NNN;
    lattice_angles[1] = NNN;
    lattice_angles[2] = NNN;
    lattice_angles[3] = NNN;
    phase = "nnn";
    radius_Saxena = NNN;
    radius_PT = NNN;
    ...
    oxidation_states.clear();
    oxidation_states.push_back(NNN);  // RF+SK20200410
    oxidation_states_preferred.clear();
    oxidation_states_preferred.push_back(NNN);  // RF+SK20200410
    electron_affinity_PT = NNN;
    energies_ionization.clear();
    energies_ionization.push_back(NNN);  // CO20201111
  ...
  void xelement::copy(const xelement& b) {  // copy PRIVATE  //CO20200520
    ...
    oxidation_states.clear();
    for (size_t i = 0; i < b.oxidation_states.size(); i++) {
      oxidation_states.push_back(b.oxidation_states[i]);
    } // RF+SK20200410  //CO20201111
    oxidation_states_preferred.clear();
    for (size_t i = 0; i < b.oxidation_states_preferred.size(); i++) {
      oxidation_states_preferred.push_back(b.oxidation_states_preferred[i]);
    } // RF+SK20200410  //CO20201111
    electron_affinity_PT = b.electron_affinity_PT;
    energies_ionization.clear();
    for (size_t i = 0; i < b.energies_ionization.size(); i++) {
      energies_ionization.push_back(b.energies_ionization[i]);
    } // CO20201111
  ...
  xelement::xelement(const string& element, int oxidation_state) {
    free();
    populate(element, oxidation_state);
  } // CO20200520
  xelement::xelement(uint ZZ, int oxidation_state) {
    free();
    populate(ZZ, oxidation_state);
  } // CO20200520
  xelement::xelement(const xelement& b) {
    copy(b);
  } // CO20210201

  string xelement::getPropertyStringVector(const string& property, const string& delim, uint ncols) const { // CO20201111
    const string c = aurostd::toupper(property);
    if (ncols == AUROSTD_MAX_UINT) {
      if (c == aurostd::toupper("lattice_constants")) {
        return aurostd::joinWDelimiter(aurostd::xvecDouble2vecString(lattice_constants, _DOUBLE_WRITE_PRECISION_), delim);
      }
      if (c == aurostd::toupper("lattice_angles")) {
        return aurostd::joinWDelimiter(aurostd::xvecDouble2vecString(lattice_angles, _DOUBLE_WRITE_PRECISION_), delim);
      }
      if (c == aurostd::toupper("oxidation_states")) {
        return aurostd::joinWDelimiter(aurostd::vecDouble2vecString(oxidation_states, _DOUBLE_WRITE_PRECISION_), delim); // CO20201111
      }
      if (c == aurostd::toupper("oxidation_states_preferred")) {
        return aurostd::joinWDelimiter(aurostd::vecDouble2vecString(oxidation_states_preferred, _DOUBLE_WRITE_PRECISION_), delim); // CO20200731
      }
      if (c == aurostd::toupper("energies_ionization")) {
        return aurostd::joinWDelimiter(aurostd::vecDouble2vecString(energies_ionization, _DOUBLE_WRITE_PRECISION_), delim); // CO20201111
      }
    } else {
      ...
      if (c == aurostd::toupper("lattice_constants")) {
        for (int i = lattice_constants.lrows; i <= (lattice_constants.lrows + (int) ncols) && i <= lattice_constants.urows; i++) {
          vitems.push_back(lattice_constants[i]);
        }
      } else if (c == aurostd::toupper("lattice_angles")) {
        for (int i = lattice_angles.lrows; i <= (lattice_angles.lrows + (int) ncols) && i <= lattice_angles.urows; i++) {
          vitems.push_back(lattice_angles[i]);
        }
      } else if (c == aurostd::toupper("oxidation_states")) {
        for (size_t i = 0; i < (ncols) && i < oxidation_states.size(); i++) {
          vitems.push_back(oxidation_states[i]);
        }
      } else if (c == aurostd::toupper("oxidation_states_preferred")) {
        for (size_t i = 0; i < (ncols) && i < oxidation_states_preferred.size(); i++) {
          vitems.push_back(oxidation_states_preferred[i]);
        }
      } else if (c == aurostd::toupper("energies_ionization")) {
        for (size_t i = 0; i < (ncols) && i < energies_ionization.size(); i++) {
          vitems.push_back(energies_ionization[i]);
        }
      }
  ...
  string xelement::getPropertyString(const string& property, const string& delim, uint ncols) const { // CO20201111
    const string c = aurostd::toupper(property);
    if (c == aurostd::toupper("name")) {
      return name;
    }
    if (c == aurostd::toupper("symbol")) {
      return symbol;
    }
    if (c == aurostd::toupper("Z")) {
      return aurostd::utype2string(Z);
    }
    if (c == aurostd::toupper("period")) {
      return aurostd::utype2string(period);
    }
    if (c == aurostd::toupper("group")) {
      return aurostd::utype2string(group);
    }
    if (c == aurostd::toupper("series")) {
      return series;
    }
    if (c == aurostd::toupper("block")) {
      return block;
    }
    //
    if (c == aurostd::toupper("mass")) {
      return aurostd::utype2string(mass, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("volume_molar")) {
      return aurostd::utype2string(volume_molar, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("volume")) {
      return aurostd::utype2string(volume, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("area_molar_Miedema")) {
      return aurostd::utype2string(area_molar_Miedema, _DOUBLE_WRITE_PRECISION_);
    }
    //
    if (c == aurostd::toupper("valence_std")) {
      return aurostd::utype2string(valence_std, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("valence_iupac")) {
      return aurostd::utype2string(valence_iupac, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("valence_PT")) {
      return aurostd::utype2string(valence_PT, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("valence_s")) {
      return aurostd::utype2string(valence_s, _DOUBLE_WRITE_PRECISION_); // CO20201111
    }
    if (c == aurostd::toupper("valence_p")) {
      return aurostd::utype2string(valence_p, _DOUBLE_WRITE_PRECISION_); // CO20201111
    }
    if (c == aurostd::toupper("valence_d")) {
      return aurostd::utype2string(valence_d, _DOUBLE_WRITE_PRECISION_); // CO20201111
    }
    if (c == aurostd::toupper("valence_f")) {
      return aurostd::utype2string(valence_f, _DOUBLE_WRITE_PRECISION_); // CO20201111
    }
    if (c == aurostd::toupper("density_PT")) {
      return aurostd::utype2string(density_PT, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("crystal")) {
      return crystal;
    }
    if (c == aurostd::toupper("crystal_structure_PT")) {
      return crystal_structure_PT;
    }
    if (c == aurostd::toupper("spacegroup")) {
      return spacegroup;
    }
    if (c == aurostd::toupper("spacegroup_number")) {
      return aurostd::utype2string(spacegroup_number);
    }
    if (c == aurostd::toupper("variance_parameter_mass")) {
      return aurostd::utype2string(variance_parameter_mass, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("lattice_constants")) {
      return getPropertyStringVector(property, delim, ncols);
    }
    if (c == aurostd::toupper("lattice_angles")) {
      return getPropertyStringVector(property, delim, ncols);
    }
    if (c == aurostd::toupper("phase")) {
      return phase;
    }
    if (c == aurostd::toupper("radius_Saxena")) {
      return aurostd::utype2string(radius_Saxena, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("radius_PT")) {
      return aurostd::utype2string(radius_PT, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("radius_covalent_PT")) {
      return aurostd::utype2string(radius_covalent_PT, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("radius_covalent")) {
      return aurostd::utype2string(radius_covalent, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("radius_VanDerWaals_PT")) {
      return aurostd::utype2string(radius_VanDerWaals_PT, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("radii_Ghosh08")) {
      return aurostd::utype2string(radii_Ghosh08, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("radii_Slatter")) {
      return aurostd::utype2string(radii_Slatter, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("radii_Pyykko")) {
      return aurostd::utype2string(radii_Pyykko, _DOUBLE_WRITE_PRECISION_);
    }
    //
    if (c == aurostd::toupper("conductivity_electrical")) {
      return aurostd::utype2string(conductivity_electrical, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("electronegativity_Pauling")) {
      return aurostd::utype2string(electronegativity_Pauling, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("hardness_chemical_Ghosh")) {
      return aurostd::utype2string(hardness_chemical_Ghosh, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("electronegativity_Pearson")) {
      return aurostd::utype2string(electronegativity_Pearson, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("electronegativity_Ghosh")) {
      return aurostd::utype2string(electronegativity_Ghosh, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("electronegativity_Allen")) {
      return aurostd::utype2string(electronegativity_Allen, _DOUBLE_WRITE_PRECISION_); // CO20200731
    }
    if (c == aurostd::toupper("oxidation_states")) {
      return getPropertyStringVector(property, delim, ncols);
    }
    ...
    if (c == aurostd::toupper("electron_affinity_PT")) {
      return aurostd::utype2string(electron_affinity_PT, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("energies_ionization")) {
      return getPropertyStringVector(property, delim, ncols);
    }
    if (c == aurostd::toupper("work_function_Miedema")) {
      return aurostd::utype2string(work_function_Miedema, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("density_line_electron_WS_Miedema")) {
      return aurostd::utype2string(density_line_electron_WS_Miedema, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("energy_surface_0K_Miedema")) {
      return aurostd::utype2string(energy_surface_0K_Miedema, _DOUBLE_WRITE_PRECISION_);
    }
    //
    if (c == aurostd::toupper("chemical_scale_Pettifor")) {
      return aurostd::utype2string(chemical_scale_Pettifor, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("Mendeleev_number")) {
      return aurostd::utype2string(Mendeleev_number, _DOUBLE_WRITE_PRECISION_); // CO20201111
    }
    //
    if (c == aurostd::toupper("temperature_boiling")) {
      return aurostd::utype2string(temperature_boiling, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("temperature_melting")) {
      return aurostd::utype2string(temperature_melting, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("enthalpy_fusion")) {
      return aurostd::utype2string(enthalpy_fusion, _DOUBLE_WRITE_PRECISION_); // CO20201111
    }
    if (c == aurostd::toupper("enthalpy_vaporization")) {
      return aurostd::utype2string(enthalpy_vaporization, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("enthalpy_atomization_WE")) {
      return aurostd::utype2string(enthalpy_atomization_WE, _DOUBLE_WRITE_PRECISION_); // CO20201111
    }
    if (c == aurostd::toupper("energy_cohesive")) {
      return aurostd::utype2string(energy_cohesive, _DOUBLE_WRITE_PRECISION_); // CO20201111
    }
    if (c == aurostd::toupper("specific_heat_PT")) {
      return aurostd::utype2string(specific_heat_PT, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("critical_pressure")) {
      return aurostd::utype2string(critical_pressure, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("critical_temperature_PT")) {
      return aurostd::utype2string(critical_temperature_PT, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("thermal_expansion")) {
      return aurostd::utype2string(thermal_expansion, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("conductivity_thermal")) {
      return aurostd::utype2string(conductivity_thermal, _DOUBLE_WRITE_PRECISION_);
    }
    //
    if (c == aurostd::toupper("hardness_mechanical_Brinell")) {
      return aurostd::utype2string(hardness_mechanical_Brinell, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("hardness_mechanical_Mohs")) {
      return aurostd::utype2string(hardness_mechanical_Mohs, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("hardness_mechanical_Vickers")) {
      return aurostd::utype2string(hardness_mechanical_Vickers, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("hardness_chemical_Pearson")) {
      return aurostd::utype2string(hardness_chemical_Pearson, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("hardness_chemical_Putz")) {
      return aurostd::utype2string(hardness_chemical_Putz, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("hardness_chemical_RB")) {
      return aurostd::utype2string(hardness_chemical_RB, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("modulus_shear")) {
      return aurostd::utype2string(modulus_shear, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("modulus_Young")) {
      return aurostd::utype2string(modulus_Young, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("modulus_bulk")) {
      return aurostd::utype2string(modulus_bulk, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("Poisson_ratio_PT")) {
      return aurostd::utype2string(Poisson_ratio_PT, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("modulus_bulk_x_volume_molar_Miedema")) {
      return aurostd::utype2string(modulus_bulk_x_volume_molar_Miedema, _DOUBLE_WRITE_PRECISION_);
    }
    //
    if (c == aurostd::toupper("magnetic_type_PT")) {
      return magnetic_type_PT;
    }
    if (c == aurostd::toupper("susceptibility_magnetic_mass")) {
      return aurostd::utype2string(susceptibility_magnetic_mass, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("susceptibility_magnetic_volume")) {
      return aurostd::utype2string(susceptibility_magnetic_volume, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("susceptibility_magnetic_molar")) {
      return aurostd::utype2string(susceptibility_magnetic_molar, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("temperature_Curie")) {
      return aurostd::utype2string(temperature_Curie, _DOUBLE_WRITE_PRECISION_);
    }
    //
    if (c == aurostd::toupper("refractive_index")) {
      return aurostd::utype2string(refractive_index, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("color_PT")) {
      return color_PT;
    }
    //
    if (c == aurostd::toupper("HHIP")) {
      return aurostd::utype2string(HHIP, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("HHIR")) {
      return aurostd::utype2string(HHIR, _DOUBLE_WRITE_PRECISION_);
    }
    if (c == aurostd::toupper("xray_scatt")) {
      return aurostd::utype2string(xray_scatt, _DOUBLE_WRITE_PRECISION_);
    }
  ...
  double xelement::getPropertyDouble(const string& property, int index) const {
    ...
    if (c == aurostd::toupper("valence_iupac")) {
      return valence_iupac;
    }
    if (c == aurostd::toupper("valence_PT")) {
      return valence_PT;
    }
    if (c == aurostd::toupper("valence_s")) {
      return valence_s; // CO20201111
    }
    if (c == aurostd::toupper("valence_p")) {
      return valence_p; // CO20201111
    }
    if (c == aurostd::toupper("valence_d")) {
      return valence_d; // CO20201111
    }
    if (c == aurostd::toupper("valence_f")) {
      return valence_f; // CO20201111
    }
    if (c == aurostd::toupper("density_PT")) {
      return density_PT;
    }
    ...
    if (c == aurostd::toupper("spacegroup_number")) {
      return spacegroup_number; // UINT
    }
    if (c == aurostd::toupper("variance_parameter_mass")) {
      return variance_parameter_mass;
    }
    if (c == aurostd::toupper("lattice_constants")) { // for xvectors, index must be between 0 and rows-1
      if (index < 0 || index >= lattice_constants.rows) {
        return NNN;
      } // out of bounds
    ...
    if (c == aurostd::toupper("lattice_angles")) { // for xvectors, index must be between 0 and rows-1
      if (index < 0 || index >= lattice_angles.rows) {
        return NNN;
      } // out of bounds
    ...
    if (c == aurostd::toupper("radius_Saxena")) {
      return radius_Saxena;
    }
    if (c == aurostd::toupper("radius_PT")) {
      return radius_PT;
    }
    if (c == aurostd::toupper("radius_covalent_PT")) {
      return radius_covalent_PT;
    }
    if (c == aurostd::toupper("radius_covalent")) {
      return radius_covalent;
    }
    if (c == aurostd::toupper("radius_VanDerWaals_PT")) {
      return radius_VanDerWaals_PT;
    }
    if (c == aurostd::toupper("radii_Ghosh08")) {
      return radii_Ghosh08;
    }
    if (c == aurostd::toupper("radii_Slatter")) {
      return radii_Slatter;
    }
    if (c == aurostd::toupper("radii_Pyykko")) {
      return radii_Pyykko;
    }
    //
    if (c == aurostd::toupper("conductivity_electrical")) {
      return conductivity_electrical;
    }
    if (c == aurostd::toupper("electronegativity_Pauling")) {
      return electronegativity_Pauling;
    }
    if (c == aurostd::toupper("hardness_chemical_Ghosh")) {
      return hardness_chemical_Ghosh;
    }
    if (c == aurostd::toupper("electronegativity_Pearson")) {
      return electronegativity_Pearson;
    }
    if (c == aurostd::toupper("electronegativity_Ghosh")) {
      return electronegativity_Ghosh;
    }
    if (c == aurostd::toupper("electronegativity_Allen")) {
      return electronegativity_Allen; // CO20200731
    }
    if (c == aurostd::toupper("oxidation_states")) {
      if (index < 0 || index >= (int) oxidation_states.size()) {
        return NNN;
      } // out of bounds
      return oxidation_states[index];
    }
    if (c == aurostd::toupper("oxidation_states_preferred")) {
      if (index < 0 || index >= (int) oxidation_states_preferred.size()) {
        return NNN;
      } // out of bounds
      return oxidation_states_preferred[index];
    }
    if (c == aurostd::toupper("electron_affinity_PT")) {
      return electron_affinity_PT;
    }
    if (c == aurostd::toupper("energies_ionization")) {
      if (index < 0 || index >= (int) energies_ionization.size()) {
        return NNN;
      } // out of bounds
      return energies_ionization[index];
    }
    if (c == aurostd::toupper("work_function_Miedema")) {
      return work_function_Miedema;
    }
    if (c == aurostd::toupper("density_line_electron_WS_Miedema")) {
      return density_line_electron_WS_Miedema;
    }
    if (c == aurostd::toupper("energy_surface_0K_Miedema")) {
      return energy_surface_0K_Miedema;
    }
    //
    if (c == aurostd::toupper("chemical_scale_Pettifor")) {
      return chemical_scale_Pettifor;
    }
    if (c == aurostd::toupper("Mendeleev_number")) {
      return Mendeleev_number; // CO20201111
    }
    //
    if (c == aurostd::toupper("temperature_boiling")) {
      return temperature_boiling;
    }
    if (c == aurostd::toupper("temperature_melting")) {
      return temperature_melting;
    }
    if (c == aurostd::toupper("enthalpy_fusion")) {
      return enthalpy_fusion; // CO20201111
    }
    if (c == aurostd::toupper("enthalpy_vaporization")) {
      return enthalpy_vaporization;
    }
    if (c == aurostd::toupper("enthalpy_atomization_WE")) {
      return enthalpy_atomization_WE; // CO20201111
    }
    if (c == aurostd::toupper("energy_cohesive")) {
      return energy_cohesive; // CO20201111
    }
    if (c == aurostd::toupper("specific_heat_PT")) {
      return specific_heat_PT;
    }
    if (c == aurostd::toupper("critical_pressure")) {
      return critical_pressure;
    }
    if (c == aurostd::toupper("critical_temperature_PT")) {
      return critical_temperature_PT;
    }
    if (c == aurostd::toupper("thermal_expansion")) {
      return thermal_expansion;
    }
    if (c == aurostd::toupper("conductivity_thermal")) {
      return conductivity_thermal;
    }
    //
    if (c == aurostd::toupper("hardness_mechanical_Brinell")) {
      return hardness_mechanical_Brinell;
    }
    if (c == aurostd::toupper("hardness_mechanical_Mohs")) {
      return hardness_mechanical_Mohs;
    }
    if (c == aurostd::toupper("hardness_mechanical_Vickers")) {
      return hardness_mechanical_Vickers;
    }
    if (c == aurostd::toupper("hardness_chemical_Pearson")) {
      return hardness_chemical_Pearson;
    }
    if (c == aurostd::toupper("hardness_chemical_Putz")) {
      return hardness_chemical_Putz;
    }
    if (c == aurostd::toupper("hardness_chemical_RB")) {
      return hardness_chemical_RB;
    }
    if (c == aurostd::toupper("modulus_shear")) {
      return modulus_shear;
    }
    if (c == aurostd::toupper("modulus_Young")) {
      return modulus_Young;
    }
    if (c == aurostd::toupper("modulus_bulk")) {
      return modulus_bulk;
    }
    if (c == aurostd::toupper("Poisson_ratio_PT")) {
      return Poisson_ratio_PT;
    }
    if (c == aurostd::toupper("modulus_bulk_x_volume_molar_Miedema")) {
      return modulus_bulk_x_volume_molar_Miedema;
    }
    //
    // if(c==aurostd::toupper("magnetic_type_PT")) return magnetic_type_PT;
    if (c == aurostd::toupper("susceptibility_magnetic_mass")) {
      return susceptibility_magnetic_mass;
    }
    if (c == aurostd::toupper("susceptibility_magnetic_volume")) {
      return susceptibility_magnetic_volume;
    }
    if (c == aurostd::toupper("susceptibility_magnetic_molar")) {
      return susceptibility_magnetic_molar;
    }
    if (c == aurostd::toupper("temperature_Curie")) {
      return temperature_Curie;
    }
    //
    if (c == aurostd::toupper("refractive_index")) {
  ...
  const vector<double>& xelement::getPropertyVectorDouble(const string& property) const {
    ...
    if (c == aurostd::toupper("oxidation_states_preferred")) {
      return oxidation_states_preferred;
    }
    // if(c==aurostd::toupper("electron_affinity_PT")) return electron_affinity_PT;
    if (c == aurostd::toupper("energies_ionization")) {
      return energies_ionization;
    }
    // if(c==aurostd::toupper("work_function_Miedema")) return work_function_Miedema;
    // if(c==aurostd::toupper("density_line_electron_WS_Miedema")) return density_line_electron_WS_Miedema;
    // if(c==aurostd::toupper("energy_surface_0K_Miedema")) return energy_surface_0K_Miedema;
  ...
  string xelement::getUnits(const string& property) const {
    ...
    if (c == aurostd::toupper("volume_molar")) {
      return units_volume_molar;
    }
    if (c == aurostd::toupper("volume")) {
      return units_volume;
    }
    if (c == aurostd::toupper("area_molar_Miedema")) {
      return units_area_molar_Miedema;
    }
    //
    if (c == aurostd::toupper("valence_std")) {
      return units_valence_std;
    }
    if (c == aurostd::toupper("valence_iupac")) {
      return units_valence_iupac;
    }
    if (c == aurostd::toupper("valence_PT")) {
      return units_valence_PT;
    }
    if (c == aurostd::toupper("valence_s")) {
      return units_valence_s; // CO20201111
    }
    if (c == aurostd::toupper("valence_p")) {
      return units_valence_p; // CO20201111
    }
    if (c == aurostd::toupper("valence_d")) {
      return units_valence_d; // CO20201111
    }
    if (c == aurostd::toupper("valence_f")) {
      return units_valence_f; // CO20201111
    }
    if (c == aurostd::toupper("density_PT")) {
      return units_density_PT;
    }
    if (c == aurostd::toupper("crystal")) {
      return units_crystal;
    }
    if (c == aurostd::toupper("crystal_structure_PT")) {
      return units_crystal_structure_PT;
    }
    if (c == aurostd::toupper("spacegroup")) {
      return units_spacegroup;
    }
    if (c == aurostd::toupper("spacegroup_number")) {
      return units_spacegroup_number;
    }
    if (c == aurostd::toupper("variance_parameter_mass")) {
      return units_variance_parameter_mass;
    }
    if (c == aurostd::toupper("lattice_constants")) {
      return units_lattice_constants;
    }
    if (c == aurostd::toupper("lattice_angles")) {
      return units_lattice_angles;
    }
    if (c == aurostd::toupper("phase")) {
      return units_phase;
    }
    if (c == aurostd::toupper("radius_Saxena")) {
      return units_radius_Saxena;
    }
    if (c == aurostd::toupper("radius_PT")) {
      return units_radius_PT;
    }
    if (c == aurostd::toupper("radius_covalent_PT")) {
      return units_radius_covalent_PT;
    }
    if (c == aurostd::toupper("radius_covalent")) {
      return units_radius_covalent;
    }
    if (c == aurostd::toupper("radius_VanDerWaals_PT")) {
      return units_radius_VanDerWaals_PT;
    }
    if (c == aurostd::toupper("radii_Ghosh08")) {
      return units_radii_Ghosh08;
    }
    if (c == aurostd::toupper("radii_Slatter")) {
      return units_radii_Slatter;
    }
    if (c == aurostd::toupper("radii_Pyykko")) {
      return units_radii_Pyykko;
    }
    //
    if (c == aurostd::toupper("conductivity_electrical")) {
      return units_conductivity_electrical;
    }
    if (c == aurostd::toupper("electronegativity_Pauling")) {
      return units_electronegativity_Pauling;
    }
    if (c == aurostd::toupper("hardness_chemical_Ghosh")) {
      return units_hardness_chemical_Ghosh;
    }
    if (c == aurostd::toupper("electronegativity_Pearson")) {
      return units_electronegativity_Pearson;
    }
    if (c == aurostd::toupper("electronegativity_Ghosh")) {
      return units_electronegativity_Ghosh;
    }
    if (c == aurostd::toupper("electronegativity_Allen")) {
      return units_electronegativity_Allen; // CO20200731
    }
    if (c == aurostd::toupper("oxidation_states")) {
      return units_oxidation_states;
    }
    if (c == aurostd::toupper("oxidation_states_preferred")) {
      return units_oxidation_states_preferred;
    }
    if (c == aurostd::toupper("electron_affinity_PT")) {
      return units_electron_affinity_PT;
    }
    if (c == aurostd::toupper("energies_ionization")) {
      return units_energies_ionization;
    }
    if (c == aurostd::toupper("work_function_Miedema")) {
      return units_work_function_Miedema;
    }
    if (c == aurostd::toupper("density_line_electron_WS_Miedema")) {
      return units_density_line_electron_WS_Miedema;
    }
    if (c == aurostd::toupper("energy_surface_0K_Miedema")) {
      return units_energy_surface_0K_Miedema;
    }
    //
    if (c == aurostd::toupper("chemical_scale_Pettifor")) {
      return units_chemical_scale_Pettifor;
    }
    if (c == aurostd::toupper("Mendeleev_number")) {
      return units_Mendeleev_number; // CO20201111
    }
    //
    if (c == aurostd::toupper("temperature_boiling")) {
      return units_temperature_boiling;
    }
    if (c == aurostd::toupper("temperature_melting")) {
      return units_temperature_melting;
    }
    if (c == aurostd::toupper("enthalpy_fusion")) {
      return units_enthalpy_fusion; // CO20201111
    }
    if (c == aurostd::toupper("enthalpy_vaporization")) {
      return units_enthalpy_vaporization;
    }
    if (c == aurostd::toupper("enthalpy_atomization_WE")) {
      return units_enthalpy_atomization_WE; // CO20201111
    }
    if (c == aurostd::toupper("energy_cohesive")) {
      return units_energy_cohesive; // CO20201111
    }
    if (c == aurostd::toupper("specific_heat_PT")) {
      return units_specific_heat_PT;
    }
    if (c == aurostd::toupper("critical_pressure")) {
      return units_critical_pressure;
    }
    if (c == aurostd::toupper("critical_temperature_PT")) {
      return units_critical_temperature_PT;
    }
    if (c == aurostd::toupper("thermal_expansion")) {
      return units_thermal_expansion;
    }
    if (c == aurostd::toupper("conductivity_thermal")) {
      return units_conductivity_thermal;
    }
    //
    if (c == aurostd::toupper("hardness_mechanical_Brinell")) {
      return units_hardness_mechanical_Brinell;
    }
    if (c == aurostd::toupper("hardness_mechanical_Mohs")) {
      return units_hardness_mechanical_Mohs;
    }
    if (c == aurostd::toupper("hardness_mechanical_Vickers")) {
      return units_hardness_mechanical_Vickers;
    }
    if (c == aurostd::toupper("hardness_chemical_Pearson")) {
      return units_hardness_chemical_Pearson;
    }
    if (c == aurostd::toupper("hardness_chemical_Putz")) {
      return units_hardness_chemical_Putz;
    }
    if (c == aurostd::toupper("hardness_chemical_RB")) {
      return units_hardness_chemical_RB;
    }
    if (c == aurostd::toupper("modulus_shear")) {
      return units_modulus_shear;
    }
    if (c == aurostd::toupper("modulus_Young")) {
      return units_modulus_Young;
    }
    if (c == aurostd::toupper("modulus_bulk")) {
      return units_modulus_bulk;
    }
    if (c == aurostd::toupper("Poisson_ratio_PT")) {
      return units_Poisson_ratio_PT;
    }
    if (c == aurostd::toupper("modulus_bulk_x_volume_molar_Miedema")) {
      return units_modulus_bulk_x_volume_molar_Miedema;
    }
    //
    if (c == aurostd::toupper("magnetic_type_PT")) {
      return units_magnetic_type_PT;
    }
    if (c == aurostd::toupper("susceptibility_magnetic_mass")) {
      return units_susceptibility_magnetic_mass;
    }
    if (c == aurostd::toupper("susceptibility_magnetic_volume")) {
      return units_susceptibility_magnetic_volume;
    }
    if (c == aurostd::toupper("susceptibility_magnetic_molar")) {
      return units_susceptibility_magnetic_molar;
    }
    if (c == aurostd::toupper("temperature_Curie")) {
      return units_temperature_Curie;
    }
    //
    if (c == aurostd::toupper("refractive_index")) {
      return units_refractive_index;
    }
    if (c == aurostd::toupper("color_PT")) {
      return units_color_PT;
    }
    //
    if (c == aurostd::toupper("HHIP")) {
      return units_HHIP;
  ...
  void xelement::convertUnits(const string& property, const string& units_new) {
    ...
    int k = 0;
    if (c == "ALL") {
      aurostd::string2tokens(aurostd::toupper(_AFLOW_XELEMENT_PROPERTIES_ALL_), vproperties, ",");
    } else {
      vproperties.push_back(c);
    }
    string units_old;
    ...
    for (i = 0; i < vproperties.size(); i++) {
      ...
      double* dptr = nullptr;
      vector<double>* dvptr = nullptr;
      xvector<double>* dxvptr = nullptr;
      string* sptr = nullptr;
      if (vproperties[i] == aurostd::toupper("name")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("symbol")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("Z")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("period")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("group")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("series")) {
        continue;
      };
      if (vproperties[i] == aurostd::toupper("block")) {
        continue;
      };
      //
      if (vproperties[i] == aurostd::toupper("mass")) {
        dptr = &mass;
        sptr = &units_mass;
      }
      if (vproperties[i] == aurostd::toupper("volume_molar")) {
        dptr = &volume_molar;
        sptr = &units_volume_molar;
      }
      if (vproperties[i] == aurostd::toupper("volume")) {
        dptr = &volume;
        sptr = &units_volume;
      }
      if (vproperties[i] == aurostd::toupper("area_molar_Miedema")) {
        dptr = &area_molar_Miedema;
        sptr = &units_area_molar_Miedema;
      }
      //
      if (vproperties[i] == aurostd::toupper("valence_std")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("valence_iupac")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("valence_PT")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("valence_s")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("valence_p")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("valence_d")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("valence_f")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("density_PT")) {
        dptr = &density_PT;
        sptr = &units_density_PT;
      }
      if (vproperties[i] == aurostd::toupper("crystal")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("crystal_structure_PT")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("spacegroup")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("spacegroup_number")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("variance_parameter_mass")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("lattice_constants")) {
        dxvptr = &lattice_constants;
        sptr = &units_lattice_constants;
      }
      if (vproperties[i] == aurostd::toupper("lattice_angles")) {
        dxvptr = &lattice_angles;
        sptr = &units_lattice_angles;
      }
      if (vproperties[i] == aurostd::toupper("phase")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("radius_Saxena")) {
        dptr = &radius_Saxena;
        sptr = &units_radius_Saxena;
      }
      if (vproperties[i] == aurostd::toupper("radius_PT")) {
        dptr = &radius_PT;
        sptr = &units_radius_PT;
      }
      if (vproperties[i] == aurostd::toupper("radius_covalent_PT")) {
        dptr = &radius_covalent_PT;
        sptr = &units_radius_covalent_PT;
      }
      if (vproperties[i] == aurostd::toupper("radius_covalent")) {
        dptr = &radius_covalent;
        sptr = &units_radius_covalent;
      }
      if (vproperties[i] == aurostd::toupper("radius_VanDerWaals_PT")) {
        dptr = &radius_VanDerWaals_PT;
        sptr = &units_radius_VanDerWaals_PT;
      }
      if (vproperties[i] == aurostd::toupper("radii_Ghosh08")) {
        dptr = &radii_Ghosh08;
        sptr = &units_radii_Ghosh08;
      }
      if (vproperties[i] == aurostd::toupper("radii_Slatter")) {
        dptr = &radii_Slatter;
        sptr = &units_radii_Slatter;
      }
      if (vproperties[i] == aurostd::toupper("radii_Pyykko")) {
        dptr = &radii_Pyykko;
        sptr = &units_radii_Pyykko;
      }
      //
      if (vproperties[i] == aurostd::toupper("conductivity_electrical")) {
        dptr = &conductivity_electrical;
        sptr = &units_conductivity_electrical;
      }
      if (vproperties[i] == aurostd::toupper("electronegativity_Pauling")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("hardness_chemical_Ghosh")) {
        dptr = &hardness_chemical_Ghosh;
        sptr = &units_hardness_chemical_Ghosh;
      }
      if (vproperties[i] == aurostd::toupper("electronegativity_Pearson")) {
        dptr = &electronegativity_Pearson;
        sptr = &units_electronegativity_Pearson;
      }
      if (vproperties[i] == aurostd::toupper("electronegativity_Ghosh")) {
        dptr = &electronegativity_Ghosh;
        sptr = &units_electronegativity_Ghosh;
      }
      if (vproperties[i] == aurostd::toupper("electronegativity_Allen")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("oxidation_states")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("oxidation_states_preferred")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("electron_affinity_PT")) {
        dptr = &electron_affinity_PT;
        sptr = &units_electron_affinity_PT;
      }
      if (vproperties[i] == aurostd::toupper("energies_ionization")) {
        dvptr = &energies_ionization;
        sptr = &units_energies_ionization;
      }
      if (vproperties[i] == aurostd::toupper("work_function_Miedema")) {
        dptr = &work_function_Miedema;
        sptr = &units_work_function_Miedema;
      }
      if (vproperties[i] == aurostd::toupper("density_line_electron_WS_Miedema")) {
        dptr = &density_line_electron_WS_Miedema;
        sptr = &units_density_line_electron_WS_Miedema;
      }
      if (vproperties[i] == aurostd::toupper("energy_surface_0K_Miedema")) {
        dptr = &energy_surface_0K_Miedema;
        sptr = &units_energy_surface_0K_Miedema;
      }
      //
      if (vproperties[i] == aurostd::toupper("chemical_scale_Pettifor")) {
        continue; // CO20201111
      }
      if (vproperties[i] == aurostd::toupper("Mendeleev_number")) {
        continue; // CO20201111
      }
      //
      if (vproperties[i] == aurostd::toupper("temperature_boiling")) {
        dptr = &temperature_boiling;
        sptr = &units_temperature_boiling;
      }
      if (vproperties[i] == aurostd::toupper("temperature_melting")) {
        dptr = &temperature_melting;
        sptr = &units_temperature_melting;
      }
      if (vproperties[i] == aurostd::toupper("enthalpy_fusion")) {
        dptr = &enthalpy_fusion;
        sptr = &units_enthalpy_fusion;
      } // CO20201111
      if (vproperties[i] == aurostd::toupper("enthalpy_vaporization")) {
        dptr = &enthalpy_vaporization;
        sptr = &units_enthalpy_vaporization;
      }
      if (vproperties[i] == aurostd::toupper("enthalpy_atomization_WE")) {
        dptr = &enthalpy_atomization_WE;
        sptr = &units_enthalpy_atomization_WE;
      } // CO20201111
      if (vproperties[i] == aurostd::toupper("energy_cohesive")) {
        dptr = &energy_cohesive;
        sptr = &units_energy_cohesive;
      } // CO20201111
      if (vproperties[i] == aurostd::toupper("specific_heat_PT")) {
        dptr = &specific_heat_PT;
        sptr = &units_specific_heat_PT;
      }
      if (vproperties[i] == aurostd::toupper("critical_pressure")) {
        dptr = &critical_pressure;
        sptr = &units_critical_pressure;
      }
      if (vproperties[i] == aurostd::toupper("critical_temperature_PT")) {
        dptr = &critical_temperature_PT;
        sptr = &units_critical_temperature_PT;
      }
      if (vproperties[i] == aurostd::toupper("thermal_expansion")) {
        dptr = &thermal_expansion;
        sptr = &units_thermal_expansion;
      }
      if (vproperties[i] == aurostd::toupper("conductivity_thermal")) {
        dptr = &conductivity_thermal;
        sptr = &units_conductivity_thermal;
      }
      //
      if (vproperties[i] == aurostd::toupper("hardness_mechanical_Brinell")) {
        dptr = &hardness_mechanical_Brinell;
        sptr = &units_hardness_mechanical_Brinell;
      }
      if (vproperties[i] == aurostd::toupper("hardness_mechanical_Mohs")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("hardness_mechanical_Vickers")) {
        dptr = &hardness_mechanical_Vickers;
        sptr = &units_hardness_mechanical_Vickers;
      }
      if (vproperties[i] == aurostd::toupper("hardness_chemical_Pearson")) {
        dptr = &hardness_chemical_Pearson;
        sptr = &units_hardness_chemical_Pearson;
      }
      if (vproperties[i] == aurostd::toupper("hardness_chemical_Putz")) {
        dptr = &hardness_chemical_Putz;
        sptr = &units_hardness_chemical_Putz;
      }
      if (vproperties[i] == aurostd::toupper("hardness_chemical_RB")) {
        dptr = &hardness_chemical_RB;
        sptr = &units_hardness_chemical_RB;
      }
      if (vproperties[i] == aurostd::toupper("modulus_shear")) {
        dptr = &modulus_shear;
        sptr = &units_modulus_shear;
      }
      if (vproperties[i] == aurostd::toupper("modulus_Young")) {
        dptr = &modulus_Young;
        sptr = &units_modulus_Young;
      }
      if (vproperties[i] == aurostd::toupper("modulus_bulk")) {
        dptr = &modulus_bulk;
        sptr = &units_modulus_bulk;
      }
      if (vproperties[i] == aurostd::toupper("Poisson_ratio_PT")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("modulus_bulk_x_volume_molar_Miedema")) {
        dptr = &modulus_bulk_x_volume_molar_Miedema;
        sptr = &units_modulus_bulk_x_volume_molar_Miedema;
      }
      //
      if (vproperties[i] == aurostd::toupper("magnetic_type_PT")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("susceptibility_magnetic_mass")) {
        dptr = &susceptibility_magnetic_mass;
        sptr = &units_susceptibility_magnetic_mass;
      }
      if (vproperties[i] == aurostd::toupper("susceptibility_magnetic_volume")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("susceptibility_magnetic_molar")) {
        dptr = &susceptibility_magnetic_molar;
        sptr = &units_susceptibility_magnetic_molar;
      }
      if (vproperties[i] == aurostd::toupper("temperature_Curie")) {
        dptr = &temperature_Curie;
        sptr = &units_temperature_Curie;
      }
      //
      if (vproperties[i] == aurostd::toupper("refractive_index")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("color_PT")) {
        continue;
      }
      //
      if (vproperties[i] == aurostd::toupper("HHIP")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("HHIR")) {
        continue;
      }
      if (vproperties[i] == aurostd::toupper("xray_scatt")) {
        dptr = &xray_scatt;
        sptr = &units_xray_scatt;
      }

      if (dptr == nullptr && dvptr == nullptr && dxvptr == nullptr) {
      ...
      if (units_new == "SI") {
        ...
        else if (units_old == "m^3/mol") {
          converted = true;
        } // done
        else if (units_old == "m^3/K") {
          converted = true;
        } // done
        else if (units_old == "e-") {
          converted = true;
        } // done
        else if (units_old == "rad") {
          converted = true;
        } // done
        else if (units_old == "S/m") {
          converted = true;
        } // done
        else if (units_old == "V") {
          converted = true;
        } // done
        else if (units_old == "J/(kg K)") {
          converted = true;
        } // done
        else if (units_old == "K") {
          converted = true;
        } // done
        else if (units_old == "K^{-1}") {
          converted = true;
        } // done
        else if (units_old == "W/(m K)") {
          converted = true;
        } // done
        //
        else if (units_old == "A^3") {
          dold = (*dptr);
          if (dold != NNN && dold != AUROSTD_NAN) {
            (*dptr) *= std::pow(1e-10, 3.0);
          }
          (*sptr) = "m^3";
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " converted " << dold << " (" << units_old << ") to " << (*dptr) << " (" << (*sptr) << ")" << endl;
          }
          converted = true;
        } else if (units_old == "cm^2") {
          dold = (*dptr);
          if (dold != NNN && dold != AUROSTD_NAN) {
            (*dptr) *= std::pow(1e-2, 2.0);
          }
          (*sptr) = "m^2";
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " converted " << dold << " (" << units_old << ") to " << (*dptr) << " (" << (*sptr) << ")" << endl;
          ...
          dold = (*dptr);
          if (dold != NNN && dold != AUROSTD_NAN) {
            (*dptr) *= 1e-3 * std::pow(1.0 / 1e-2, 3.0);
          }
          (*sptr) = "kg/m^3";
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " converted " << dold << " (" << units_old << ") to " << (*dptr) << " (" << (*sptr) << ")" << endl;
          ...
          if (dptr != nullptr) {
            ...
            for (k = (*dxvptr).lrows; k <= (*dxvptr).urows; k++) {
              ...
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " converted " << dold << " (" << units_old << ") to " << (*dxvptr)[k] << " (" << (*sptr) << ")" << endl;
              }
            }
            converted = true;
          } else {
          ...
          if (dptr != nullptr) {
            ...
            for (j = 0; j < (*dvptr).size(); j++) {
              ...
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " converted " << dold << " (" << units_old << ") to " << (*dvptr)[j] << " (" << (*sptr) << ")" << endl;
              }
            }
            converted = true;
          } else {
          ...
          dold = (*dptr);
          if (dold != NNN && dold != AUROSTD_NAN) {
            (*dptr) *= 1e-2 * std::pow((1.0 / bohr2angstrom) * 1e10, 3.0);
          }
          (*sptr) = "e-/m^3";
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " converted " << dold << " (" << units_old << ") to " << (*dptr) << " (" << (*sptr) << ")" << endl;
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " units_new=(" << (*sptr) << ")" << endl;
      }
  ...
  uint xelement::isElement(const string& element) { // CO20200520
    ...
    if (Z == 0) {
      for (uint i = 1; i <= 103 && Z == 0; i++) { // CO20200520
        if (aurostd::toupper(element) == aurostd::toupper(xelement(i).symbol)) {
          Z = i;
        }
      }
    }
    ...
    if (Z == 0) {
      for (uint i = 1; i <= 103 && Z == 0; i++) { // CO20200520
        if (aurostd::toupper(element) == aurostd::toupper(xelement(i).name)) {
          Z = i;
        }
      }
    }
  ...
  void xelement::populate(const string& element, int oxidation_state) { // CO20200520
    ...
    // DEFAULT
    verbose = false;
    const uint Z = isElement(element);
    if (Z != 0) {
      (*this) = xelement(Z, oxidation_state);
      loadDefaultUnits();
      return;
    } // CO20200520
  ...
  void xelement::populate(uint ZZ, int oxidation_state) { // CO20200520
    ...
    // Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen Hydrogen
      ...
      lattice_constants[1] = 470;
      lattice_constants[2] = 470;
      lattice_constants[3] = 340;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Gas";
      radius_Saxena = 0.046;
      radius_PT = 53;
      ...
      energies_ionization.clear();
      energies_ionization.push_back(1312); // CO20201111
    ...
    // Helium Helium Helium Helium Helium Helium Helium Helium Helium Helium Helium Helium Helium Helium Helium Helium Helium
      ...
      lattice_constants[1] = 424.2;
      lattice_constants[2] = 424.2;
      lattice_constants[3] = 424.2;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Gas";
      radius_Saxena = NNN;
      radius_PT = 31;
      ...
      energies_ionization.clear();
      energies_ionization.push_back(2372.3);
      energies_ionization.push_back(5250.5); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium Lithium
      ...
      lattice_constants[1] = 351;
      lattice_constants[2] = 351;
      lattice_constants[3] = 351;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(520.2);
      energies_ionization.push_back(7298.1);
      energies_ionization.push_back(11815); // CO20201111
    ...
    // Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium Beryllium
      ...
      lattice_constants[1] = 228.58;
      lattice_constants[2] = 228.58;
      lattice_constants[3] = 358.43;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(899.5);
      energies_ionization.push_back(1757.1);
      energies_ionization.push_back(14848.7);
      energies_ionization.push_back(21006.6); // CO20201111
    ...
    // Boron Boron Boron Boron Boron Boron Boron Boron Boron Boron Boron Boron Boron Boron Boron Boron Boron
      ...
      lattice_constants[1] = 506;
      lattice_constants[2] = 506;
      lattice_constants[3] = 506;
      lattice_angles[1] = 1.01334;
      lattice_angles[2] = 1.01334;
      lattice_angles[3] = 1.01334;
      phase = "Solid";
      radius_Saxena = 0.097;
      radius_PT = 87;
      ...
      energies_ionization.clear();
      energies_ionization.push_back(800.6);
      energies_ionization.push_back(2427.1);
      energies_ionization.push_back(3659.7);
      energies_ionization.push_back(25025.8);
      energies_ionization.push_back(32826.7); // CO20201111
    ...
    // Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon Carbon
      ...
      lattice_constants[1] = 246.4;
      lattice_constants[2] = 246.4;
      lattice_constants[3] = 671.1;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(1086.5);
      energies_ionization.push_back(2352.6);
      energies_ionization.push_back(4620.5);
      energies_ionization.push_back(6222.7);
      energies_ionization.push_back(37831);
      energies_ionization.push_back(47277); // CO20201111
    ...
    // Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen Nitrogen
      ...
      lattice_constants[1] = 386.1;
      lattice_constants[2] = 386.1;
      lattice_constants[3] = 626.5;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Gas";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(1402.3);
      energies_ionization.push_back(2856);
      energies_ionization.push_back(4578.1);
      energies_ionization.push_back(7475);
      energies_ionization.push_back(9444.9);
      energies_ionization.push_back(53266.6);
      energies_ionization.push_back(64360); // CO20201111
    ...
    // Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen Oxygen
      ...
      lattice_constants[1] = 540.3;
      lattice_constants[2] = 342.9;
      lattice_constants[3] = 508.6;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = 2.313085;
      lattice_angles[3] = PI / 2;
      phase = "Gas";
      radius_Saxena = 0.060;
      radius_PT = 48;
      ...
      oxidation_states.clear();
      oxidation_states.push_back(-0.5);
      oxidation_states.push_back(-1);
      oxidation_states.push_back(-2); // RF+SK20200410
      oxidation_states_preferred.clear();
      oxidation_states_preferred.push_back(-2); // RF+SK20200410
      ...
      energies_ionization.clear();
      energies_ionization.push_back(1313.9);
      energies_ionization.push_back(3388.3);
      energies_ionization.push_back(5300.5);
      energies_ionization.push_back(7469.2);
      energies_ionization.push_back(10989.5);
      energies_ionization.push_back(13326.5);
      energies_ionization.push_back(71330);
      energies_ionization.push_back(84078); // CO20201111
    ...
    // Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine Fluorine
      ...
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 550;
      lattice_constants[2] = 328;
      lattice_constants[3] = 728;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Gas";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(1681);
      energies_ionization.push_back(3374.2);
      energies_ionization.push_back(6050.4);
      energies_ionization.push_back(8407.7);
      energies_ionization.push_back(11022.7);
      energies_ionization.push_back(15164.1);
      energies_ionization.push_back(17868);
      energies_ionization.push_back(92038.1);
      energies_ionization.push_back(106434.3); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Neon Neon Neon Neon Neon Neon Neon Neon Neon Neon Neon Neon Neon Neon Neon Neon Neon
      ...
      lattice_constants[1] = 442.9;
      lattice_constants[2] = 442.9;
      lattice_constants[3] = 442.9;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Gas";
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(2080.7);
      energies_ionization.push_back(3952.3);
      energies_ionization.push_back(6122);
      energies_ionization.push_back(9371);
      energies_ionization.push_back(12177);
      energies_ionization.push_back(15238);
      energies_ionization.push_back(19999);
      energies_ionization.push_back(23069.5);
      energies_ionization.push_back(115379.5);
      energies_ionization.push_back(131432); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium Sodium
      ...
      spacegroup_number = 229;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 429.06;
      lattice_constants[2] = 429.06;
      lattice_constants[3] = 429.06;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(495.8);
      energies_ionization.push_back(4562);
      energies_ionization.push_back(6910.3);
      energies_ionization.push_back(9543);
      energies_ionization.push_back(13354);
      energies_ionization.push_back(16613);
      energies_ionization.push_back(20117);
      energies_ionization.push_back(25496);
      energies_ionization.push_back(28932);
      energies_ionization.push_back(141362); // CO20201111
    ...
    // Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium Magnesium
      ...
      lattice_constants[1] = 320.94;
      lattice_constants[2] = 320.94;
      lattice_constants[3] = 521.08;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(737.7);
      energies_ionization.push_back(1450.7);
      energies_ionization.push_back(7732.7);
      energies_ionization.push_back(10542.5);
      energies_ionization.push_back(13630);
      energies_ionization.push_back(18020);
      energies_ionization.push_back(21711);
      energies_ionization.push_back(25661);
      energies_ionization.push_back(31653);
      energies_ionization.push_back(35458); // CO20201111
    ...
    // Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium Aluminium
      ...
      spacegroup_number = 225;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 404.95;
      lattice_constants[2] = 404.95;
      lattice_constants[3] = 404.95;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(577.5);
      energies_ionization.push_back(1816.7);
      energies_ionization.push_back(2744.8);
      energies_ionization.push_back(11577);
      energies_ionization.push_back(14842);
      energies_ionization.push_back(18379);
      energies_ionization.push_back(23326);
      energies_ionization.push_back(27465);
      energies_ionization.push_back(31853);
      energies_ionization.push_back(38473); // CO20201111
      work_function_Miedema = 4.20;
    ...
    // Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon Silicon
      ...
      lattice_constants[1] = 543.09;
      lattice_constants[2] = 543.09;
      lattice_constants[3] = 543.09;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(786.5);
      energies_ionization.push_back(1577.1);
      energies_ionization.push_back(3231.6);
      energies_ionization.push_back(4355.5);
      energies_ionization.push_back(16091);
      energies_ionization.push_back(19805);
      energies_ionization.push_back(23780);
      energies_ionization.push_back(29287);
      energies_ionization.push_back(33878);
      energies_ionization.push_back(38726); // CO20201111
    ...
    // Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus Phosphorus
      ...
      spacegroup_number = 2;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 1145;
      lattice_constants[2] = 550.3;
      lattice_constants[3] = 1126.1;
      lattice_angles[1] = 1.25384;
      lattice_angles[2] = 1.57725;
      lattice_angles[3] = 1.24896;
      phase = "Solid";
      radius_Saxena = 0.109;
      radius_PT = 98;
      ...
      energies_ionization.clear();
      energies_ionization.push_back(1011.8);
      energies_ionization.push_back(1907);
      energies_ionization.push_back(2914.1);
      energies_ionization.push_back(4963.6);
      energies_ionization.push_back(6273.9);
      energies_ionization.push_back(21267);
      energies_ionization.push_back(25431);
      energies_ionization.push_back(29872);
      energies_ionization.push_back(35905);
      energies_ionization.push_back(40950); // CO20201111
    ...
    // Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur Sulfur
      ...
      lattice_constants[1] = 1043.7;
      lattice_constants[2] = 1284.5;
      lattice_constants[3] = 2436.9;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(999.6);
      energies_ionization.push_back(2252);
      energies_ionization.push_back(3357);
      energies_ionization.push_back(4556);
      energies_ionization.push_back(7004.3);
      energies_ionization.push_back(8495.8);
      energies_ionization.push_back(27107);
      energies_ionization.push_back(31719);
      energies_ionization.push_back(36621);
      energies_ionization.push_back(43177); // CO20201111
    ...
    // Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine Chlorine
      ...
      lattice_constants[1] = 622.35;
      lattice_constants[2] = 445.61;
      lattice_constants[3] = 817.85;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Gas";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(1251.2);
      energies_ionization.push_back(2298);
      energies_ionization.push_back(3822);
      energies_ionization.push_back(5158.6);
      energies_ionization.push_back(6542);
      energies_ionization.push_back(9362);
      energies_ionization.push_back(11018);
      energies_ionization.push_back(33604);
      energies_ionization.push_back(38600);
      energies_ionization.push_back(43961); // CO20201111
    ...
    // Argon Argon Argon Argon Argon Argon Argon Argon Argon Argon Argon Argon Argon Argon Argon Argon Argon
      ...
      lattice_constants[1] = 525.6;
      lattice_constants[2] = 525.6;
      lattice_constants[3] = 525.6;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Gas";
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(1520.6);
      energies_ionization.push_back(2665.8);
      energies_ionization.push_back(3931);
      energies_ionization.push_back(5771);
      energies_ionization.push_back(7238);
      energies_ionization.push_back(8781);
      energies_ionization.push_back(11995);
      energies_ionization.push_back(13842);
      energies_ionization.push_back(40760);
      energies_ionization.push_back(46186); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium Potassium
      ...
      lattice_constants[1] = 532.8;
      lattice_constants[2] = 532.8;
      lattice_constants[3] = 532.8;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(418.8);
      energies_ionization.push_back(3052);
      energies_ionization.push_back(4420);
      energies_ionization.push_back(5877);
      energies_ionization.push_back(7975);
      energies_ionization.push_back(9590);
      energies_ionization.push_back(11343);
      energies_ionization.push_back(14944);
      energies_ionization.push_back(16963.7);
      energies_ionization.push_back(48610); // CO20201111
    ...
    // Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium Calcium
      ...
      lattice_constants[1] = 558.84;
      lattice_constants[2] = 558.84;
      lattice_constants[3] = 558.84;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(589.8);
      energies_ionization.push_back(1145.4);
      energies_ionization.push_back(4912.4);
      energies_ionization.push_back(6491);
      energies_ionization.push_back(8153);
      energies_ionization.push_back(10496);
      energies_ionization.push_back(12270);
      energies_ionization.push_back(14206);
      energies_ionization.push_back(18191);
      energies_ionization.push_back(20385);
      energies_ionization.push_back(57110); // CO20201111
    ...
    // Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium Scandium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 330.9;
      lattice_constants[2] = 330.9;
      lattice_constants[3] = 527.33;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(633.1);
      energies_ionization.push_back(1235);
      energies_ionization.push_back(2388.6);
      energies_ionization.push_back(7090.6);
      energies_ionization.push_back(8843);
      energies_ionization.push_back(10679);
      energies_ionization.push_back(13310);
      energies_ionization.push_back(15250);
      energies_ionization.push_back(17370);
      energies_ionization.push_back(21726);
      energies_ionization.push_back(24102);
      energies_ionization.push_back(66320);
      energies_ionization.push_back(73010);
      energies_ionization.push_back(80160);
      energies_ionization.push_back(89490);
      energies_ionization.push_back(97400);
      energies_ionization.push_back(105600);
      energies_ionization.push_back(117000);
      energies_ionization.push_back(124270);
      energies_ionization.push_back(547530);
      energies_ionization.push_back(582163); // CO20201111
    ...
    // Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium Titanium
      ...
      lattice_constants[1] = 295.08;
      lattice_constants[2] = 295.08;
      lattice_constants[3] = 468.55;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(658.8);
      energies_ionization.push_back(1309.8);
      energies_ionization.push_back(2652.5);
      energies_ionization.push_back(4174.6);
      energies_ionization.push_back(9581);
      energies_ionization.push_back(11533);
      energies_ionization.push_back(13590);
      energies_ionization.push_back(16440);
      energies_ionization.push_back(18530);
      energies_ionization.push_back(20833);
      energies_ionization.push_back(25575);
      energies_ionization.push_back(28125);
      energies_ionization.push_back(76015);
      energies_ionization.push_back(83280);
      energies_ionization.push_back(90880);
      energies_ionization.push_back(100700);
      energies_ionization.push_back(109100);
      energies_ionization.push_back(117800);
      energies_ionization.push_back(129900);
      energies_ionization.push_back(137530);
      energies_ionization.push_back(602930); // CO20201111
    ...
    // Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium Vanadium
      ...
      lattice_constants[1] = 303;
      lattice_constants[2] = 303;
      lattice_constants[3] = 303;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      oxidation_states.push_back(3);
      oxidation_states.push_back(2);
      oxidation_states.push_back(0); // RF+SK20200410
      oxidation_states_preferred.clear();
      oxidation_states_preferred.push_back(5); // RF+SK20200410
      electron_affinity_PT = 50.6;
      energies_ionization.clear();
      energies_ionization.push_back(650.9);
      energies_ionization.push_back(1414);
      energies_ionization.push_back(2830);
      energies_ionization.push_back(4507);
      energies_ionization.push_back(6298.7);
      energies_ionization.push_back(12363);
      energies_ionization.push_back(14530);
      energies_ionization.push_back(16730);
      energies_ionization.push_back(19860);
      energies_ionization.push_back(22240);
      energies_ionization.push_back(24670);
      energies_ionization.push_back(29730);
      energies_ionization.push_back(32446);
      energies_ionization.push_back(86450);
      energies_ionization.push_back(94170);
      energies_ionization.push_back(102300);
      energies_ionization.push_back(112700);
      energies_ionization.push_back(121600);
      energies_ionization.push_back(130700);
      energies_ionization.push_back(143400);
      energies_ionization.push_back(151440); // CO20201111
    ...
    // Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium Chromium
      ...
      lattice_constants[1] = 291;
      lattice_constants[2] = 291;
      lattice_constants[3] = 291;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      oxidation_states_preferred.push_back(6); // RF+SK20200410 //Cr+3 most preferred oxidation number
      electron_affinity_PT = 64.3;
      energies_ionization.clear();
      energies_ionization.push_back(652.9);
      energies_ionization.push_back(1590.6);
      energies_ionization.push_back(2987);
      energies_ionization.push_back(4743);
      energies_ionization.push_back(6702);
      energies_ionization.push_back(8744.9);
      energies_ionization.push_back(15455);
      energies_ionization.push_back(17820);
      energies_ionization.push_back(20190);
      energies_ionization.push_back(23580);
      energies_ionization.push_back(26130);
      energies_ionization.push_back(28750);
      energies_ionization.push_back(34230);
      energies_ionization.push_back(37066);
      energies_ionization.push_back(97510);
      energies_ionization.push_back(105800);
      energies_ionization.push_back(114300);
      energies_ionization.push_back(125300);
      energies_ionization.push_back(134700);
      energies_ionization.push_back(144300);
      energies_ionization.push_back(157700); // CO20201111
    ...
    // Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese Manganese
      ...
      lattice_constants[1] = 891.25;
      lattice_constants[2] = 891.25;
      lattice_constants[3] = 891.25;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      oxidation_states.push_back(3);
      oxidation_states.push_back(2);
      oxidation_states.push_back(0);
      oxidation_states.push_back(-1); // RF+SK20200410
      oxidation_states_preferred.clear();
      oxidation_states_preferred.push_back(2); // RF+SK20200410
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(717.3);
      energies_ionization.push_back(1509);
      energies_ionization.push_back(3248);
      energies_ionization.push_back(4940);
      energies_ionization.push_back(6990);
      energies_ionization.push_back(9220);
      energies_ionization.push_back(11500);
      energies_ionization.push_back(18770);
      energies_ionization.push_back(21400);
      energies_ionization.push_back(23960);
      energies_ionization.push_back(27590);
      energies_ionization.push_back(30330);
      energies_ionization.push_back(33150);
      energies_ionization.push_back(38880);
      energies_ionization.push_back(41987);
      energies_ionization.push_back(109480);
      energies_ionization.push_back(118100);
      energies_ionization.push_back(127100);
      energies_ionization.push_back(138600);
      energies_ionization.push_back(148500);
      energies_ionization.push_back(158600); // CO20201111
    ...
    // Iron Iron Iron Iron Iron Iron Iron Iron Iron Iron Iron Iron Iron Iron Iron Iron Iron
      ...
      lattice_constants[1] = 286.65;
      lattice_constants[2] = 286.65;
      lattice_constants[3] = 286.65;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      } // CO20201111 - http://www.knowledgedoor.com/2/elements_handbook/pauling_electronegativity.html
      else {
        electronegativity_Pauling = 1.83;
      } // oxidation_state==2 (DEFAULT)  //CO20201111
      ...
      oxidation_states_preferred.clear();
      oxidation_states_preferred.push_back(3);
      oxidation_states_preferred.push_back(2); // RF+SK20200410 //Fe+3 most preferred oxidation number
      electron_affinity_PT = 15.7;
      energies_ionization.clear();
      energies_ionization.push_back(762.5);
      energies_ionization.push_back(1561.9);
      energies_ionization.push_back(2957);
      energies_ionization.push_back(5290);
      energies_ionization.push_back(7240);
      energies_ionization.push_back(9560);
      energies_ionization.push_back(12060);
      energies_ionization.push_back(14580);
      energies_ionization.push_back(22540);
      energies_ionization.push_back(25290);
      energies_ionization.push_back(28000);
      energies_ionization.push_back(31920);
      energies_ionization.push_back(34830);
      energies_ionization.push_back(37840);
      energies_ionization.push_back(44100);
      energies_ionization.push_back(47206);
      energies_ionization.push_back(122200);
      energies_ionization.push_back(131000);
      energies_ionization.push_back(140500);
      energies_ionization.push_back(152600);
      energies_ionization.push_back(163000); // CO20201111
    ...
    // Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt Cobalt
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 250.71;
      lattice_constants[2] = 250.71;
      lattice_constants[3] = 406.95;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(760.4);
      energies_ionization.push_back(1648);
      energies_ionization.push_back(3232);
      energies_ionization.push_back(4950);
      energies_ionization.push_back(7670);
      energies_ionization.push_back(9840);
      energies_ionization.push_back(12440);
      energies_ionization.push_back(15230);
      energies_ionization.push_back(17959);
      energies_ionization.push_back(26570);
      energies_ionization.push_back(29400);
      energies_ionization.push_back(32400);
      energies_ionization.push_back(36600);
      energies_ionization.push_back(39700);
      energies_ionization.push_back(42800);
      energies_ionization.push_back(49396);
      energies_ionization.push_back(52737);
      energies_ionization.push_back(134810);
      energies_ionization.push_back(145170);
      energies_ionization.push_back(154700);
      energies_ionization.push_back(167400); // CO20201111
    ...
    // Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel Nickel
      ...
      lattice_constants[1] = 352.4;
      lattice_constants[2] = 352.4;
      lattice_constants[3] = 352.4;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(737.1);
      energies_ionization.push_back(1753);
      energies_ionization.push_back(3395);
      energies_ionization.push_back(5300);
      energies_ionization.push_back(7339);
      energies_ionization.push_back(10400);
      energies_ionization.push_back(12800);
      energies_ionization.push_back(15600);
      energies_ionization.push_back(18600);
      energies_ionization.push_back(21670);
      energies_ionization.push_back(30970);
      energies_ionization.push_back(34000);
      energies_ionization.push_back(37100);
      energies_ionization.push_back(41500);
      energies_ionization.push_back(44800);
      energies_ionization.push_back(48100);
      energies_ionization.push_back(55101);
      energies_ionization.push_back(58570);
      energies_ionization.push_back(148700);
      energies_ionization.push_back(159000);
      energies_ionization.push_back(169400); // CO20201111
    ...
    // Copper Copper Copper Copper Copper Copper Copper Copper Copper Copper Copper Copper Copper Copper Copper Copper Copper
      ...
      lattice_constants[1] = 361.49;
      lattice_constants[2] = 361.49;
      lattice_constants[3] = 361.49;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      if (oxidation_state == 2) {
        ...
        electronegativity_Pauling = 1.90;
      } // oxidation_state==1 (DEFAULT) //CO20201111
      ...
      oxidation_states_preferred.push_back(1); // RF+SK20200410 //Cu+2 most preferred oxidation number
      electron_affinity_PT = 118.4;
      energies_ionization.clear();
      energies_ionization.push_back(745.5);
      energies_ionization.push_back(1957.9);
      energies_ionization.push_back(3555);
      energies_ionization.push_back(5536);
      energies_ionization.push_back(7700);
      energies_ionization.push_back(9900);
      energies_ionization.push_back(13400);
      energies_ionization.push_back(16000);
      energies_ionization.push_back(19200);
      energies_ionization.push_back(22400);
      energies_ionization.push_back(25600);
      energies_ionization.push_back(35600);
      energies_ionization.push_back(38700);
      energies_ionization.push_back(42000);
      energies_ionization.push_back(46700);
      energies_ionization.push_back(50200);
      energies_ionization.push_back(53700);
      energies_ionization.push_back(61100);
      energies_ionization.push_back(64702);
      energies_ionization.push_back(163700);
      energies_ionization.push_back(174100); // CO20201111
    ...
    // Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc Zinc
      ...
      lattice_constants[1] = 266.49;
      lattice_constants[2] = 266.49;
      lattice_constants[3] = 494.68;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(906.4);
      energies_ionization.push_back(1733.3);
      energies_ionization.push_back(3833);
      energies_ionization.push_back(5731);
      energies_ionization.push_back(7970);
      energies_ionization.push_back(10400);
      energies_ionization.push_back(12900);
      energies_ionization.push_back(16800);
      energies_ionization.push_back(19600);
      energies_ionization.push_back(23000);
      energies_ionization.push_back(26400);
      energies_ionization.push_back(29990);
      energies_ionization.push_back(40490);
      energies_ionization.push_back(43800);
      energies_ionization.push_back(47300);
      energies_ionization.push_back(52300);
      energies_ionization.push_back(55900);
      energies_ionization.push_back(59700);
      energies_ionization.push_back(67300);
      energies_ionization.push_back(71200);
      energies_ionization.push_back(179100); // CO20201111
    ...
    // Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium Gallium
      ...
      lattice_constants[1] = 451.97;
      lattice_constants[2] = 766.33;
      lattice_constants[3] = 452.6;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(578.8);
      energies_ionization.push_back(1979.3);
      energies_ionization.push_back(2963);
      energies_ionization.push_back(6180); // CO20201111
      work_function_Miedema = 4.10;
    ...
    // Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium Germanium
      ...
      lattice_constants[1] = 565.75;
      lattice_constants[2] = 565.75;
      lattice_constants[3] = 565.75;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(762);
      energies_ionization.push_back(1537.5);
      energies_ionization.push_back(3302.1);
      energies_ionization.push_back(4411);
      energies_ionization.push_back(9020); // CO20201111
      work_function_Miedema = 4.55;
    ...
    // Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic Arsenic
      ...
      spacegroup_number = 166;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 375.98;
      lattice_constants[2] = 375.98;
      lattice_constants[3] = 1054.75;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(947);
      energies_ionization.push_back(1798);
      energies_ionization.push_back(2735);
      energies_ionization.push_back(4837);
      energies_ionization.push_back(6043);
      energies_ionization.push_back(12310); // CO20201111
    ...
    // Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium Selenium
      ...
      lattice_constants[1] = 905.4;
      lattice_constants[2] = 908.3;
      lattice_constants[3] = 1160.1;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = 1.58493;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      radius_Saxena = 0.116;
      radius_PT = 103;
      ...
      energies_ionization.clear();
      energies_ionization.push_back(941);
      energies_ionization.push_back(2045);
      energies_ionization.push_back(2973.7);
      energies_ionization.push_back(4144);
      energies_ionization.push_back(6590);
      energies_ionization.push_back(7880);
      energies_ionization.push_back(14990); // CO20201111
    ...
    // Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine Bromine
      ...
      lattice_constants[1] = 672.65;
      lattice_constants[2] = 464.51;
      lattice_constants[3] = 870.23;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Liquid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(1139.9);
      energies_ionization.push_back(2103);
      energies_ionization.push_back(3470);
      energies_ionization.push_back(4560);
      energies_ionization.push_back(5760);
      energies_ionization.push_back(8550);
      energies_ionization.push_back(9940);
      energies_ionization.push_back(18600); // CO20201111
      work_function_Miedema = 5.20;
    ...
    // Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton Krypton
      ...
      lattice_constants[1] = 570.6;
      lattice_constants[2] = 570.6;
      lattice_constants[3] = 570.6;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Gas";
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(1350.8);
      energies_ionization.push_back(2350.4);
      energies_ionization.push_back(3565);
      energies_ionization.push_back(5070);
      energies_ionization.push_back(6240);
      energies_ionization.push_back(7570);
      energies_ionization.push_back(10710);
      energies_ionization.push_back(12138);
      energies_ionization.push_back(22274);
      energies_ionization.push_back(25880);
      energies_ionization.push_back(29700);
      energies_ionization.push_back(33800);
      energies_ionization.push_back(37700);
      energies_ionization.push_back(43100);
      energies_ionization.push_back(47500);
      energies_ionization.push_back(52200);
      energies_ionization.push_back(57100);
      energies_ionization.push_back(61800);
      energies_ionization.push_back(75800);
      energies_ionization.push_back(80400);
    ...
    // Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium Rubidium
      ...
      lattice_constants[1] = 558.5;
      lattice_constants[2] = 558.5;
      lattice_constants[3] = 558.5;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(403);
      energies_ionization.push_back(2633);
      energies_ionization.push_back(3860);
      energies_ionization.push_back(5080);
      energies_ionization.push_back(6850);
      energies_ionization.push_back(8140);
      energies_ionization.push_back(9570);
      energies_ionization.push_back(13120);
      energies_ionization.push_back(14500);
      energies_ionization.push_back(26740); // CO20201111
    ...
    // Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium Strontium
      ...
      lattice_constants[1] = 608.49;
      lattice_constants[2] = 608.49;
      lattice_constants[3] = 608.49;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(549.5);
      energies_ionization.push_back(1064.2);
      energies_ionization.push_back(4138);
      energies_ionization.push_back(5500);
      energies_ionization.push_back(6910);
      energies_ionization.push_back(8760);
      energies_ionization.push_back(10230);
      energies_ionization.push_back(11800);
      energies_ionization.push_back(15600);
      energies_ionization.push_back(17100); // CO20201111
    ...
    // Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium Yttrium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 364.74;
      lattice_constants[2] = 364.74;
      lattice_constants[3] = 573.06;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(600);
      energies_ionization.push_back(1180);
      energies_ionization.push_back(1980);
      energies_ionization.push_back(5847);
      energies_ionization.push_back(7430);
      energies_ionization.push_back(8970);
      energies_ionization.push_back(11190);
      energies_ionization.push_back(12450);
      energies_ionization.push_back(14110);
      energies_ionization.push_back(18400); // CO20201111
    ...
    // Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium Zirconium
      ...
      lattice_constants[1] = 323.2;
      lattice_constants[2] = 323.2;
      lattice_constants[3] = 514.7;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(640.1);
      energies_ionization.push_back(1270);
      energies_ionization.push_back(2218);
      energies_ionization.push_back(3313);
      energies_ionization.push_back(7752);
      energies_ionization.push_back(9500); // CO20201111
    ...
    // Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium Niobium
      ...
      spacegroup_number = 229;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 330.04;
      lattice_constants[2] = 330.04;
      lattice_constants[3] = 330.04;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(652.1);
      energies_ionization.push_back(1380);
      energies_ionization.push_back(2416);
      energies_ionization.push_back(3700);
      energies_ionization.push_back(4877);
      energies_ionization.push_back(9847);
      energies_ionization.push_back(12100); // CO20201111
    ...
    // Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum Molybdenum
      ...
      lattice_constants[1] = 314.7;
      lattice_constants[2] = 314.7;
      lattice_constants[3] = 314.7;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      if (oxidation_state == 6) {
        electronegativity_Pauling = 2.35;
      } // CO20201111 - http://www.knowledgedoor.com/2/elements_handbook/pauling_electronegativity.html
      else if (oxidation_state == 5) {
        electronegativity_Pauling = 2.27;
      } // CO20201111
      else if (oxidation_state == 4) {
        electronegativity_Pauling = 2.24;
      } // CO20201111
      else if (oxidation_state == 3) {
        electronegativity_Pauling = 2.19;
      } // CO20201111
      else {
      ...
      energies_ionization.clear();
      energies_ionization.push_back(684.3);
      energies_ionization.push_back(1560);
      energies_ionization.push_back(2618);
      energies_ionization.push_back(4480);
      energies_ionization.push_back(5257);
      energies_ionization.push_back(6640.8);
      energies_ionization.push_back(12125);
      energies_ionization.push_back(13860);
      energies_ionization.push_back(15835);
      energies_ionization.push_back(17980);
      energies_ionization.push_back(20190);
      energies_ionization.push_back(22219);
      energies_ionization.push_back(26930);
      energies_ionization.push_back(29196);
      energies_ionization.push_back(52490);
      energies_ionization.push_back(55000);
      energies_ionization.push_back(61400);
      energies_ionization.push_back(67700);
      energies_ionization.push_back(74000);
      energies_ionization.push_back(80400);
    ...
    // Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium Technetium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 273.5;
      lattice_constants[2] = 273.5;
      lattice_constants[3] = 438.8;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(702);
      energies_ionization.push_back(1470);
      energies_ionization.push_back(2850); // CO20201111
    ...
    // Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium Ruthenium
      ...
      lattice_constants[1] = 270.59;
      lattice_constants[2] = 270.59;
      lattice_constants[3] = 428.15;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(710.2);
      energies_ionization.push_back(1620);
      energies_ionization.push_back(2747); // CO20201111
    ...
    // Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium Rhodium
      ...
      lattice_constants[1] = 380.34;
      lattice_constants[2] = 380.34;
      lattice_constants[3] = 380.34;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(719.7);
      energies_ionization.push_back(1740);
      energies_ionization.push_back(2997); // CO20201111
      work_function_Miedema = 5.40;
    ...
    // Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium Palladium
      ...
      lattice_constants[1] = 389.07;
      lattice_constants[2] = 389.07;
      lattice_constants[3] = 389.07;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(804.4);
      energies_ionization.push_back(1870);
      energies_ionization.push_back(3177); // CO20201111
    ...
    // Silver Silver Silver Silver Silver Silver Silver Silver Silver Silver Silver Silver Silver Silver Silver Silver Silver
      ...
      lattice_constants[1] = 408.53;
      lattice_constants[2] = 408.53;
      lattice_constants[3] = 408.53;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(731);
      energies_ionization.push_back(2070);
      energies_ionization.push_back(3361); // CO20201111
      work_function_Miedema = 4.45;
      density_line_electron_WS_Miedema = 1.39;
    ...
    // Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium Cadmium
      ...
      lattice_constants[1] = 297.94;
      lattice_constants[2] = 297.94;
      lattice_constants[3] = 561.86;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(867.8);
      energies_ionization.push_back(1631.4);
      energies_ionization.push_back(3616); // CO20201111
    ...
    // Indium Indium Indium Indium Indium Indium Indium Indium Indium Indium Indium Indium Indium Indium Indium Indium Indium
      ...
      lattice_constants[1] = 325.23;
      lattice_constants[2] = 325.23;
      lattice_constants[3] = 494.61;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      electron_affinity_PT = 28.9;
      energies_ionization.clear();
      energies_ionization.push_back(558.3);
      energies_ionization.push_back(1820.7);
      energies_ionization.push_back(2704);
      energies_ionization.push_back(5210); // CO20201111
    ...
    // Tin Tin Tin Tin Tin Tin Tin Tin Tin Tin Tin Tin Tin Tin Tin Tin Tin
      ...
      lattice_constants[1] = 583.18;
      lattice_constants[2] = 583.18;
      lattice_constants[3] = 318.19;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      if (oxidation_state == 2) {
        electronegativity_Pauling = 1.80;
      } // CO20201111 - http://www.knowledgedoor.com/2/elements_handbook/pauling_electronegativity.html
      else {
        electronegativity_Pauling = 1.96;
      } // oxidation_state==4 (DEFAULT)  //CO20201111
      ...
      energies_ionization.clear();
      energies_ionization.push_back(708.6);
      energies_ionization.push_back(1411.8);
      energies_ionization.push_back(2943);
      energies_ionization.push_back(3930.3);
      energies_ionization.push_back(7456); // CO20201111
    ...
    // Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony Antimony
      ...
      lattice_constants[1] = 430.7;
      lattice_constants[2] = 430.7;
      lattice_constants[3] = 1127.3;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(834);
      energies_ionization.push_back(1594.9);
      energies_ionization.push_back(2440);
      energies_ionization.push_back(4260);
      energies_ionization.push_back(5400);
      energies_ionization.push_back(10400); // CO20201111
    ...
    // Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium Tellurium
      ...
      lattice_constants[1] = 445.72;
      lattice_constants[2] = 445.72;
      lattice_constants[3] = 592.9;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(869.3);
      energies_ionization.push_back(1790);
      energies_ionization.push_back(2698);
      energies_ionization.push_back(3610);
      energies_ionization.push_back(5668);
      energies_ionization.push_back(6820);
      energies_ionization.push_back(13200); // CO20201111
    ...
    // Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine Iodine
      ...
      spacegroup_number = 64;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 718.02;
      lattice_constants[2] = 471.02;
      lattice_constants[3] = 981.03;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(1008.4);
      energies_ionization.push_back(1845.9);
      energies_ionization.push_back(3180); // CO20201111
    ...
    // Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon Xenon
      ...
      lattice_constants[1] = 620.23;
      lattice_constants[2] = 620.23;
      lattice_constants[3] = 620.23;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Gas";
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(1170.4);
      energies_ionization.push_back(2046.4);
      energies_ionization.push_back(3099.4); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium Cesium
      ...
      spacegroup_number = 229;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 614.1;
      lattice_constants[2] = 614.1;
      lattice_constants[3] = 614.1;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(375.7);
      energies_ionization.push_back(2234.3);
      energies_ionization.push_back(3400); // CO20201111
    ...
    // Barium Barium Barium Barium Barium Barium Barium Barium Barium Barium Barium Barium Barium Barium Barium Barium Barium
      ...
      lattice_constants[1] = 502.8;
      lattice_constants[2] = 502.8;
      lattice_constants[3] = 502.8;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(502.9);
      energies_ionization.push_back(965.2);
      energies_ionization.push_back(3600); // CO20201111
    ...
    // Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium Lanthanium
      ...
      lattice_constants[1] = 377.2;
      lattice_constants[2] = 377.2;
      lattice_constants[3] = 1214.4;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(538.1);
      energies_ionization.push_back(1067);
      energies_ionization.push_back(1850.3);
      energies_ionization.push_back(4819);
      energies_ionization.push_back(5940); // CO20201111
    ...
    // Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium Cerium
      ...
      lattice_constants[1] = 362;
      lattice_constants[2] = 362;
      lattice_constants[3] = 599;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(534.4);
      energies_ionization.push_back(1050);
      energies_ionization.push_back(1949);
      energies_ionization.push_back(3547);
      energies_ionization.push_back(6325);
      energies_ionization.push_back(7490); // CO20201111
    ...
    // Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium Praseodymium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 367.25;
      lattice_constants[2] = 367.25;
      lattice_constants[3] = 1183.54;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(527);
      energies_ionization.push_back(1020);
      energies_ionization.push_back(2086);
      energies_ionization.push_back(3761);
      energies_ionization.push_back(5551); // CO20201111
    ...
    // Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium Neodymium
      ...
      lattice_constants[1] = 365.8;
      lattice_constants[2] = 365.8;
      lattice_constants[3] = 1179.9;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(533.1);
      energies_ionization.push_back(1040);
      energies_ionization.push_back(2130);
      energies_ionization.push_back(3900); // CO20201111
      work_function_Miedema = 3.19;
      density_line_electron_WS_Miedema = 1.20;
      energy_surface_0K_Miedema = NNN;
    ...
    // Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium Promethium
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(540);
      energies_ionization.push_back(1050);
      energies_ionization.push_back(2150);
      energies_ionization.push_back(3970); // CO20201111
      work_function_Miedema = 3.19;
      density_line_electron_WS_Miedema = 1.21;
      energy_surface_0K_Miedema = NNN;
    ...
    // Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium Samarium
      ...
      lattice_constants[1] = 362.1;
      lattice_constants[2] = 362.1;
      lattice_constants[3] = 2625;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(544.5);
      energies_ionization.push_back(1070);
      energies_ionization.push_back(2260);
      energies_ionization.push_back(3990); // CO20201111
      work_function_Miedema = 3.20;
      density_line_electron_WS_Miedema = 1.21;
      energy_surface_0K_Miedema = NNN;
    ...
    // Europium Europium Europium Europium Europium Europium Europium Europium Europium Europium Europium Europium Europium Europium Europium Europium Europium
      ...
      lattice_constants[1] = 458.1;
      lattice_constants[2] = 458.1;
      lattice_constants[3] = 458.1;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(547.1);
      energies_ionization.push_back(1085);
      energies_ionization.push_back(2404);
      energies_ionization.push_back(4120); // CO20201111
      work_function_Miedema = 3.20;
      density_line_electron_WS_Miedema = 1.21;
      energy_surface_0K_Miedema = NNN;
    ...
    // Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium Gadolinium
      ...
      lattice_constants[1] = 363.6;
      lattice_constants[2] = 363.6;
      lattice_constants[3] = 578.26;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(593.4);
      energies_ionization.push_back(1170);
      energies_ionization.push_back(1990);
      energies_ionization.push_back(4250); // CO20201111
      work_function_Miedema = 3.20;
      density_line_electron_WS_Miedema = 1.21;
      energy_surface_0K_Miedema = NNN;
    ...
    // Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium Terbium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 360.1;
      lattice_constants[2] = 360.1;
      lattice_constants[3] = 569.36;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(565.8);
      energies_ionization.push_back(1110);
      energies_ionization.push_back(2114);
      energies_ionization.push_back(3839); // CO20201111
    ...
    // Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium Dysprosium
      ...
      lattice_constants[1] = 359.3;
      lattice_constants[2] = 359.3;
      lattice_constants[3] = 565.37;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(573);
      energies_ionization.push_back(1130);
      energies_ionization.push_back(2200);
      energies_ionization.push_back(3990); // CO20201111
      work_function_Miedema = 3.21;
      density_line_electron_WS_Miedema = 1.22;
      energy_surface_0K_Miedema = NNN;
    ...
    // Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium Holmium
      ...
      lattice_constants[1] = 357.73;
      lattice_constants[2] = 357.73;
      lattice_constants[3] = 561.58;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(581);
      energies_ionization.push_back(1140);
      energies_ionization.push_back(2204);
      energies_ionization.push_back(4100); // CO20201111
    ...
    // Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium Erbium
      ...
      lattice_constants[1] = 355.88;
      lattice_constants[2] = 355.88;
      lattice_constants[3] = 558.74;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(589.3);
      energies_ionization.push_back(1150);
      energies_ionization.push_back(2194);
      energies_ionization.push_back(4120); // CO20201111
      work_function_Miedema = 3.22;
    ...
    // Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium Thulium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 353.75;
      lattice_constants[2] = 353.75;
      lattice_constants[3] = 555.46;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(596.7);
      energies_ionization.push_back(1160);
      energies_ionization.push_back(2285);
      energies_ionization.push_back(4120); // CO20201111
      work_function_Miedema = 3.22;
      density_line_electron_WS_Miedema = 1.23;
      energy_surface_0K_Miedema = NNN;
    ...
    // Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium Ytterbium
      ...
      lattice_constants[1] = 548.47;
      lattice_constants[2] = 548.47;
      lattice_constants[3] = 548.47;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(603.4);
      energies_ionization.push_back(1174.8);
      energies_ionization.push_back(2417);
      energies_ionization.push_back(4203); // CO20201111
      work_function_Miedema = 3.22;
      density_line_electron_WS_Miedema = 1.23;
      energy_surface_0K_Miedema = NNN;
    ...
    // Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium Lutetium
      ...
      lattice_constants[1] = 350.31;
      lattice_constants[2] = 350.31;
      lattice_constants[3] = 555.09;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 50;
      energies_ionization.clear();
      energies_ionization.push_back(523.5);
      energies_ionization.push_back(1340);
      energies_ionization.push_back(2022.3);
      energies_ionization.push_back(4370);
      energies_ionization.push_back(6445); // CO20201111
      work_function_Miedema = 3.22;
      density_line_electron_WS_Miedema = 1.24;
      energy_surface_0K_Miedema = NNN;
    ...
    // Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium Hafnium
      ...
      lattice_constants[1] = 319.64;
      lattice_constants[2] = 319.64;
      lattice_constants[3] = 505.11;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(658.5);
      energies_ionization.push_back(1440);
      energies_ionization.push_back(2250);
      energies_ionization.push_back(3216); // CO20201111
    ...
    // Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum Tantalum
      ...
      lattice_constants[1] = 330.13;
      lattice_constants[2] = 330.13;
      lattice_constants[3] = 330.13;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(761);
      energies_ionization.push_back(1500); // CO20201111
      work_function_Miedema = 4.05;
    ...
    // Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten Tungsten
      ...
      lattice_constants[1] = 316.52;
      lattice_constants[2] = 316.52;
      lattice_constants[3] = 316.52;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(770);
      energies_ionization.push_back(1700); // CO20201111
      work_function_Miedema = 4.80;
      density_line_electron_WS_Miedema = 1.81;
    ...
    // Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium Rhenium
      ...
      lattice_constants[1] = 276.1;
      lattice_constants[2] = 276.1;
      lattice_constants[3] = 445.6;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(760);
      energies_ionization.push_back(1260);
      energies_ionization.push_back(2510);
      energies_ionization.push_back(3640); // CO20201111
      work_function_Miedema = 5.40;
    ...
    // Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium Osmium
      ...
      lattice_constants[1] = 273.44;
      lattice_constants[2] = 273.44;
      lattice_constants[3] = 431.73;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(840);
      energies_ionization.push_back(1600); // CO20201111
      work_function_Miedema = 5.40;
    ...
    // Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium Iridium
      ...
      lattice_constants[1] = 383.9;
      lattice_constants[2] = 383.9;
      lattice_constants[3] = 383.9;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(880);
      energies_ionization.push_back(1600); // CO20201111
    ...
    // Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum Platinum
      ...
      lattice_constants[1] = 392.42;
      lattice_constants[2] = 392.42;
      lattice_constants[3] = 392.42;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(870);
      energies_ionization.push_back(1791); // CO20201111
    ...
    // Gold Gold Gold Gold Gold Gold Gold Gold Gold Gold Gold Gold Gold Gold Gold Gold Gold
      ...
      lattice_constants[1] = 407.82;
      lattice_constants[2] = 407.82;
      lattice_constants[3] = 407.82;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(890.1);
      energies_ionization.push_back(1980); // CO20201111
    ...
    // Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury Mercury
      ...
      lattice_constants[1] = 300.5;
      lattice_constants[2] = 300.5;
      lattice_constants[3] = 300.5;
      lattice_angles[1] = 1.23081;
      lattice_angles[2] = 1.23081;
      lattice_angles[3] = 1.23081;
      phase = "Liquid";
      radius_Saxena = 0.150;
      radius_PT = 171;
      radius_covalent = 1.32;
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(1007.1);
      energies_ionization.push_back(1810);
      energies_ionization.push_back(3300); // CO20201111
      work_function_Miedema = 4.20;
      density_line_electron_WS_Miedema = 1.24;
    ...
    // Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium Thallium
      ...
      lattice_constants[1] = 345.66;
      lattice_constants[2] = 345.66;
      lattice_constants[3] = 552.48;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(589.4);
      energies_ionization.push_back(1971);
      energies_ionization.push_back(2878); // CO20201111
      work_function_Miedema = 3.90;
    ...
    // Lead Lead Lead Lead Lead Lead Lead Lead Lead Lead Lead Lead Lead Lead Lead Lead Lead
      ...
      lattice_constants[1] = 495.08;
      lattice_constants[2] = 495.08;
      lattice_constants[3] = 495.08;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(715.6);
      energies_ionization.push_back(1450.5);
      energies_ionization.push_back(3081.5);
      energies_ionization.push_back(4083);
      energies_ionization.push_back(6640); // CO20201111
      work_function_Miedema = 4.10;
    ...
    // Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth Bismuth
      ...
      spacegroup_number = 12;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 667.4;
      lattice_constants[2] = 611.7;
      lattice_constants[3] = 330.4;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = 1.925622;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      radius_Saxena = 0.182;
      radius_PT = 143;
      ...
      energies_ionization.clear();
      energies_ionization.push_back(703);
      energies_ionization.push_back(1610);
      energies_ionization.push_back(2466);
      energies_ionization.push_back(4370);
      energies_ionization.push_back(5400);
      energies_ionization.push_back(8520); // CO20201111
      work_function_Miedema = 4.15;
    ...
    // Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium Polonium
      ...
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 335.9;
      lattice_constants[2] = 335.9;
      lattice_constants[3] = 335.9;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      energies_ionization.clear();
      energies_ionization.push_back(812.1); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine Astatine
      ...
      energies_ionization.clear();
      energies_ionization.push_back(920); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Radon Radon Radon Radon Radon Radon Radon Radon Radon Radon Radon Radon Radon Radon Radon Radon Radon
      ...
      electron_affinity_PT = 0;
      energies_ionization.clear();
      energies_ionization.push_back(1037); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Francium Francium Francium Francium Francium Francium Francium Francium Francium Francium Francium Francium Francium Francium Francium Francium Francium
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(380); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Radium Radium Radium Radium Radium Radium Radium Radium Radium Radium Radium Radium Radium Radium Radium Radium Radium
      ...
      spacegroup_number = 229;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 514.8;
      lattice_constants[2] = 514.8;
      lattice_constants[3] = 514.8;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(509.3);
      energies_ionization.push_back(979); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium Actinium
      ...
      spacegroup_number = 225;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 567;
      lattice_constants[2] = 567;
      lattice_constants[3] = 567;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(499);
      energies_ionization.push_back(1170); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium Thorium
      ...
      spacegroup_number = 225;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 508.42;
      lattice_constants[2] = 508.42;
      lattice_constants[3] = 508.42;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(587);
      energies_ionization.push_back(1110);
      energies_ionization.push_back(1930);
      energies_ionization.push_back(2780); // CO20201111
    ...
    // Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium Protoactinium
      ...
      spacegroup_number = 139;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 392.5;
      lattice_constants[2] = 392.5;
      lattice_constants[3] = 323.8;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(568); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium Uranium
      ...
      lattice_constants[1] = 285.37;
      lattice_constants[2] = 586.95;
      lattice_constants[3] = 495.48;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(597.6);
      energies_ionization.push_back(1420); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium Neptunium
      ...
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 666.3;
      lattice_constants[2] = 472.3;
      lattice_constants[3] = 488.7;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(604.5); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium Plutonium
      ...
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 618.3;
      lattice_constants[2] = 482.2;
      lattice_constants[3] = 1096.3;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = 1.776571;
      lattice_angles[3] = PI / 2;
      phase = "Solid";
      radius_Saxena = NNN;
      radius_PT = NNN;
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(584.7); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Americium Americium Americium Americium Americium Americium Americium Americium Americium Americium Americium Americium Americium Americium Americium Americium Americium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 346.81;
      lattice_constants[2] = 346.81;
      lattice_constants[3] = 1124.1;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(578); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Curium Curium Curium Curium Curium Curium Curium Curium Curium Curium Curium Curium Curium Curium Curium Curium Curium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 349.6;
      lattice_constants[2] = 349.6;
      lattice_constants[3] = 1133.1;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(581); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium Berkelium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 341.6;
      lattice_constants[2] = 341.6;
      lattice_constants[3] = 1106.9;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(601); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Californium Californium Californium Californium Californium Californium Californium Californium Californium Californium Californium Californium Californium Californium Californium Californium Californium
      ...
      spacegroup_number = 194;
      variance_parameter_mass = 0.0;
      lattice_constants[1] = 338;
      lattice_constants[2] = 338;
      lattice_constants[3] = 1102.5;
      lattice_angles[1] = PI / 2;
      lattice_angles[2] = PI / 2;
      lattice_angles[3] = 2 * PI / 3;
      phase = "Solid";
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(608); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium Einsteinium
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(619); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium Fermium
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(627); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium Mendelevium
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(635); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
    ...
    // Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium Nobelium
      ...
      electron_affinity_PT = NNN;
      energies_ionization.clear();
      energies_ionization.push_back(642); // CO20201111
      work_function_Miedema = NNN;
      density_line_electron_WS_Miedema = NNN;
      energy_surface_0K_Miedema = NNN;
