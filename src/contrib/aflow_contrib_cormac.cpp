  ...
  void DEBYE(const string& options) {
    ...
    if (aurostd::FileExist(thermofilename)) {
      if (thermofilename != "THERMO") {
        aurostd::DecompressFile(thermofilename);
      }
      ...
      aurostd::RemoveFile("THERMO");
      aurostd::DecompressFile("THERMO.bz2");
      infilethermo.open("THERMO");
      getline(infilethermo, firstline);
      while (infilethermo.peek() != EOF) {
        getline(infilethermo, line);
      ...
      aurostd::RemoveFile("THERMO");
      aurostd::DecompressFile("THERMO.gz");
      infilethermo.open("THERMO");
      getline(infilethermo, firstline);
      while (infilethermo.peek() != EOF) {
        getline(infilethermo, line);
      ...
      aurostd::RemoveFile("THERMO");
      aurostd::DecompressFile("THERMO.xz");
      infilethermo.open("THERMO");
      getline(infilethermo, firstline);
      while (infilethermo.peek() != EOF) {
        getline(infilethermo, line);
    ...
    if (aurostd::FileExist(_AFLOWIN_)) {
      ...
      if (!POSCAR.str().empty()) {
        ...
        natoms = 0;
        for (int i = 0; i < xstr.num_each_type.size(); i++) {
          natoms = natoms + xstr.num_each_type[i];
        }
        natomcalcd = true;
        sysname = xstr.title;
      }
    ...
    const string debyedatafile = "debye_data.dat";
    ofstream fdebin;
    ...
    const string gnuplotscript = "GNUPLOT_debye.gp";
    ofstream fin;
    ...
    if (RemoveGnuplotScriptct) {
      aurostd::RemoveFile(debyedatafile);
      aurostd::RemoveFile(gnuplotscript); // Delete the gnuplot script
    }
    aurostd::RemoveFile("debye_temperature.eps");

    if (thermofilename == "THERMO.bz2") {
      aurostd::RemoveFile("THERMO.bz2");
      aurostd::CompressFile("THERMO", aurostd::compression_type::BZ2);
    }
    if (thermofilename == "THERMO.gz") {
      aurostd::RemoveFile("THERMO.gz");
      aurostd::CompressFile("THERMO", aurostd::compression_type::GZ);
    }
    if (thermofilename == "THERMO.xz") {
      aurostd::RemoveFile("THERMO.xz");
      aurostd::CompressFile("THERMO", aurostd::compression_type::XZ);
    }
    // END
    if (LDEBUG) {
