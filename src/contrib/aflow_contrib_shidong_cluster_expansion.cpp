...
int cecluster::GetNNNum(vector<double> NN_distance) {
  ...
  const vector<vector<int>> str;
  const int atom_num = 2;
  double dist;
  double dist_max = -1.0;

  if (structure.atoms.size() == 1) {
    return 1; // only one atom in the cluster
  }

  const int total_num = structure.atoms.size();
  const int str_size = CombinationNr(atom_num, total_num);
  ...
  for (int i = 0; i < str_size; i++) {
    ...
    dist = modulus(structure.atoms.at(atom_config.at(0)).cpos - structure.atoms.at(atom_config.at(1)).cpos);
    dist_max = std::max(dist, dist_max); // get the largest distance in the cluster
  }

  for (size_t i = 0; i < NN_distance.size(); i++) {
    if (is_equal(dist_max, NN_distance[i])) {
      return i;
    }
  }
...
double cecluster::ClusterDistance() {
  ...
  double dist;
  const vector<vector<int>> str; // all possible combinations of two atoms
  int total_num;
  int num;
  ...
  if (total_num == 1) {
    ...
    const int str_size = CombinationNr(num, total_num);
    dist = 0;
    vector<int> atom_config;
    for (int i = 0; i < str_size; i++) {
      if (i == 0) {
      ...
      dist = dist + modulus(structure.atoms.at(atom_config.at(0)).cpos - structure.atoms.at(atom_config.at(1)).cpos);
    }
  }
...
vector<int> AllCombination42(int num, int total_num, vector<int>& str_in) {
  ...
  if (int(str_in.size()) != num) {
    throw aurostd::xerror(__AFLOW_FILE__, "AllCombination42():", "input str must have a size equal to " + aurostd::utype2string(num), _INPUT_ILLEGAL_); // CO20200624
  }
  ...
  long int count = 0;
  int j;
  const int next = 2; // only find the next combination
...
bool comparison_cluster(const cecluster& cluster1, const cecluster& cluster2) {
  ...
  const int size1 = cluster1.structure.atoms.size();
  const int size2 = cluster2.structure.atoms.size();
  ...
  if (size1 != size2) {
    ...
    if (cluster1.pair_name != cluster2.pair_name) {
      ...
      for (int i = 0; i < size1; i++) {
        ...
        shell1 = std::max(shell1, atom1.shell);
        shell2 = std::max(shell2, atom2.shell);
      }
...
bool is_equal(const double f1, const double f2) {
  return (std::abs(f1 - f2) < _EQUAL_DOUBLE);
}
bool is_equal(const _atom& atom1, const _atom& atom2) {
  return (std::abs(modulus(atom1.cpos - atom2.cpos)) < _EQUAL_ATOM);
}
bool is_equal_fpos(const _atom& atom1, const _atom& atom2) {
  return (std::abs(modulus(atom1.fpos - atom2.fpos)) < _EQUAL_ATOM);
}
bool is_equal(const cecluster& cluster1, const cecluster& cluster2) {
  ...
  if (cluster1.structure.atoms.size() != cluster2.structure.atoms.size()) {
    return false;
  }

  for (size_t i = 0; i < cluster1.structure.atoms.size(); i++) {
    atom_tmp = cluster1.structure.atoms[i];
    flag_atom = false;
    for (size_t j = 0; j < cluster2.structure.atoms.size(); j++) {
      flag_atom = flag_atom || is_equal(atom_tmp, cluster2.structure.atoms[j]);
    }
...
bool is_equal_crystal(const cecluster& cluster1, const cecluster& cluster2) {
  ...
  for (size_t i = 0; i < cluster1.structure.atoms.size(); i++) {
    atom_ref = cluster1.structure.atoms[i];
    cluster_tmp.structure.atoms.clear();
    for (size_t j = 0; j < cluster1.structure.atoms.size(); j++) {
      atom_tmp = cluster1.structure.atoms[j];
      atom_tmp.cpos = atom_tmp.cpos - atom_ref.cpos;
      cluster_tmp.structure.atoms.push_back(atom_tmp);
    }

    if (is_equal(cluster_tmp, cluster2)) {
      return true;
    }
  }
...
ostream& operator<<(ostream& os, const ceallclusters& cestr) {
  ...
  for (size_t i = 0; i < cestr.NN_distance.size(); i++) {
    // cerr.setf(ios_base::fixed);
    // cerr.precision(4);
    os << setw(6) << i << setw(16) << cestr.NN_distance[i] << setw(10) << cestr.NN_shell_num.at(i) << endl;
  }

  return os;
}

int ceallclusters::SetCluster(const int& SiteNum_low, const int& SiteNum_up, const int& NNNum_low, const int& NNNum_up) {
  clock_t start;
  clock_t end;
  double diff;
...
int ceallclusters::GetNearestNeighbor(int NNNum) {
  ...
  _atom atom_tmp;
  const _atom atom_orig = structure.atoms.at(0);
  const int grid_num = sqrt(2.0 * double(NNNum));

  // more than one atoms in the unit cells
  NN_distance_all.clear();
  NN_distance.clear();
  for (size_t i = 0; i < structure.atoms.size(); i++) {
    NN_distance_all.push_back(modulus(structure.atoms[i].cpos - atom_orig.cpos));
  }
  ...
  //    cout.precision(10);
  for (size_t i = 0; i < structure.grid_atoms.size(); i++) {
    NN_distance_all.push_back(modulus(structure.grid_atoms[i].cpos - atom_orig.cpos));
  }
  ...
  for (size_t i = 0; i < NN_distance_all.size(); i++) { // delete duplicate items
    if (!is_equal(NN_distance_all[i], NN_distance_all.at(j))) {
      NN_distance.push_back(NN_distance_all[i]);
      j = i;
    }
  }
  ...
  for (size_t i = 0; i < NN_distance.size(); i++) {
    ...
    for (size_t j = 0; j < structure.grid_atoms.size(); j++) {
      if (is_equal(modulus(structure.grid_atoms[j].cpos - atom_orig.cpos), NN_distance[i])) {
        atom_tmp = structure.grid_atoms[j];
        atom_tmp.shell = i;
...
int ceallclusters::GenerateAllCluster(int SiteNum_low, int SiteNum_up, int NNNum_low, int NNNum_up) {
  ...
  if (SiteNum_up == 1) {
    ...
    for (site_index = SiteNum_low; site_index < SiteNum_up + 1; site_index++) {
      ...
      const int num_config = site_index - 1;
      const int total_num = atom_list.size() - 1;
      const int str_size = CombinationNr(num_config, total_num);
      ...
      for (int i = 0; i < str_size; i++) {
        ...
        cluster_tmp.structure.atoms.clear();
        cluster_tmp.structure.indices.clear();
        for (size_t j = 0; j < atom_config.size(); j++) {
          // cout << atom_config.at(j) << " ";

          cluster_tmp.structure.atoms.push_back(atom_list.at(atom_config.at(j) + 1));
          cluster_tmp.structure.indices.push_back(atom_config[j] + 1);
        }
...
int ceallclusters::GetRepresentCluster() {
  ...
  for (size_t i = 0; i < all_cluster.size(); i++) {
    for (size_t j = 0; j < all_cluster.at(i).size(); j++) {
      all_cluster.at(i)[j].equivalent_num = rep_cluster.at(i).equivalent_num;
      all_cluster.at(i)[j].dist = rep_cluster.at(i).dist;
      all_cluster.at(i)[j].pair_name.push_back(' ');
      all_cluster.at(i)[j].pair_name.append(aurostd::utype2string(i + 1));
      all_cluster.at(i)[j].index = i + 1;
    }
...
void ceallclusters::PrintRepCluster() const {
  ...
  for (size_t i = 0; i < rep_cluster.size(); i++) {
    cerr << "Cluster Name (" << rep_cluster[i].pair_name << ")\n";
    cerr << "Equivalent clusters " << rep_cluster[i].equivalent_num << "\n";
    cerr << "Site_num " << rep_cluster[i].site_num << " " << "NNNnum " << rep_cluster[i].NNNum << " " << "dist " << rep_cluster[i].dist << " " << "index " << rep_cluster[i].index << endl;
    for (size_t j = 0; j < rep_cluster[i].structure.atoms.size(); j++) {
      cerr << setw(6) << rep_cluster[i].structure.atoms[j].cpos(1) << " " << setw(6) << rep_cluster[i].structure.atoms[j].cpos(2) << " " << setw(6) << rep_cluster[i].structure.atoms[j].cpos(3) << "\n";
    }

    // output the indices of atom in atom_list
    cerr << "Indices of these atoms in atom_lists are: \n";
    for (size_t j = 0; j < rep_cluster[i].structure.indices.size(); j++) {
      cerr << setw(6) << rep_cluster[i].structure.indices[j];
    }
    cerr << "\n";
  }
}
...
void ceallclusters::PrintAllCluster() const {
  ...
  count = 0;
  for (size_t k = 0; k < all_cluster.size(); k++) {
    for (size_t i = 0; i < all_cluster[k].size(); i++) {
      cerr << "Cluster Name (" << all_cluster[k][i].pair_name << ")\n";
      cerr << "Equivalent clusters " << all_cluster[k][i].equivalent_num << "\n";
      cerr << "Site_num " << all_cluster[k][i].site_num << " " << "NNNnum " << all_cluster[k][i].NNNum << " " << "dist " << all_cluster[k][i].dist << " " << "index " << all_cluster[k][i].index << endl;
      for (size_t j = 0; j < all_cluster[k][i].structure.atoms.size(); j++) {
        cerr << setw(6) << all_cluster[k][i].structure.atoms[j].cpos(1) << " " << setw(6) << all_cluster[k][i].structure.atoms[j].cpos(2) << " " << setw(6) << all_cluster[k][i].structure.atoms[j].cpos(3) << "\n";
      }

      // output the indices of atom in atom_list
      cerr << "Indices of these atoms in atom_lists are: \n";
      for (size_t j = 0; j < all_cluster[k][i].structure.indices.size(); j++) {
        cerr << setw(6) << all_cluster[k][i].structure.indices[j];
      }
      cerr << "\n";
...
int ceallclusters::EquivalentCluster(const vector<_sym_op> pgroup) {
  ...
  bool flag_eq;
  vector<cecluster> rep_cluster_tmp;
  vector<cecluster> cluster_eq_list_tmp;
  vector<vector<cecluster>> all_cluster_tmp;
  ...
  if (pgroup.empty()) {
    throw aurostd::xerror(__AFLOW_FILE__, "ceallclusters::EquivalentCluster():", "no symmetric operator", _RUNTIME_ERROR_); // CO20200624
  }

  const int lrow = 1;
  const int urow = 3; // number of coordinate elements
  ...
  while (!rep_cluster.empty()) {
    ...
    for (size_t i = 0; i < pgroup.size(); i++) {
      ...
      for (size_t j = 1; j < rep_cluster.size(); j++) {
        flag_eq = false;
        cluster2 = rep_cluster[j];

        if (cluster1.structure.atoms.size() < cluster2.structure.atoms.size() || (cluster1.pair_name != cluster2.pair_name)) {
          if (j == 1) {
            flag = true;
          }
          break;
    ...
    rep_cluster_tmp.back().equivalent_num = cluster_eq_list_tmp.size();
...
void ceallclusters::GetClusterDistance() {
  ...
  for (size_t i = 0; i < rep_cluster.size(); i++) {
    dist = 0.0;
    if (rep_cluster[i].pair_name == label_current && (i != rep_cluster.size() - 1)) {
      counter_same_old_label++;

      dist = rep_cluster[i].ClusterDistance();
      flag = false;
      for (size_t k = 0; k < distance.size(); k++) {
        flag = flag || is_equal(dist, distance[k]);
      }
      if (!flag) {
...
string ceallclusters::GetNamebyCluster(cecluster& cluster_in) const {
  ...
  for (size_t i = 0; i < all_cluster.size(); i++) {
    for (size_t j = 0; j < all_cluster[i].size(); j++) {
      cluster_cmp = all_cluster[i][j];
      if (cluster_cmp.structure.atoms.size() != cluster_in.structure.atoms.size()) {
        flag = false;
        break;
      } else {
...
bool ceallclusters::ReadIn(string& filename) {
  ...
  if (!fin) { // cannot open file
    throw aurostd::xerror(__AFLOW_FILE__, "ceallclusters::ReadIn():", "file " + filename + " cannot be opened", _FILE_CORRUPT_); // CO20200624
  }
  ...
  for (int i = 0; i < rep_cluster_num; i++) {
    ...
    for (int i1 = 0; i1 < cluster1.site_num; i1++) {
      fin >> atom1.cpos(1) >> atom1.cpos(2) >> atom1.cpos(3) >> atom_index;
  ...
  for (size_t i = 0; i < rep_cluster.size(); i++) {
    string pair_name;
    pair_name = rep_cluster[i].pair_name;
    while (pair_name.at(pair_name.size() - 1) != ' ') {
      pair_name.erase(pair_name.end() - 1);
    }
    pair_name.append(aurostd::utype2string(i + 1));
    rep_cluster[i].pair_name = pair_name;
    rep_cluster[i].index = i + 1;

    for (size_t j = 0; j < all_cluster[i].size(); j++) {
      all_cluster[i][j].pair_name = pair_name;
      all_cluster[i][j].index = i + 1;
    }
  }
...
bool ceallclusters::WriteFile(string& filename, string& stat) {
  ...
  fout.precision(6);
  for (size_t i = 0; i < rep_cluster.size(); i++) {
    fout << rep_cluster[i].pair_name << endl;
    fout << rep_cluster[i].equivalent_num << endl;
    fout << setw(8) << rep_cluster[i].site_num << setw(8) << rep_cluster[i].NNNum << setw(8) << rep_cluster[i].dist << setw(8) << rep_cluster[i].index << endl;
    for (int i1 = 0; i1 < rep_cluster[i].site_num; i1++) {
      fout << setw(16) << rep_cluster[i].structure.atoms.at(i1).cpos[1] << setw(16) << rep_cluster[i].structure.atoms.at(i1).cpos[2] << setw(16) << rep_cluster[i].structure.atoms.at(i1).cpos[3] << setw(16)
           << rep_cluster[i].structure.indices.at(i1) << endl;
    }

    for (int j = 0; j < rep_cluster[i].equivalent_num; j++) {
      fout << all_cluster.at(i).at(j).pair_name << endl;
      fout << all_cluster.at(i).at(j).equivalent_num << endl;
      fout << setw(8) << all_cluster.at(i).at(j).site_num << setw(8) << all_cluster.at(i).at(j).NNNum << setw(8) << all_cluster.at(i).at(j).dist << setw(8) << all_cluster.at(i).at(j).index << endl;
      for (int i1 = 0; i1 < all_cluster.at(i).at(j).site_num; i1++) {
        fout << setw(16) << all_cluster.at(i).at(j).structure.atoms.at(i1).cpos(1) << setw(16) << all_cluster.at(i).at(j).structure.atoms.at(i1).cpos(2) << setw(16)
             << all_cluster.at(i).at(j).structure.atoms.at(i1).cpos(3) << setw(16) << all_cluster.at(i).at(j).structure.indices.at(i1) << endl;
      }
    }
  }
  ...
  for (size_t i = 0; i < NN_distance.size(); i++) {
    fout << setw(8) << NN_distance[i] << setw(8) << NN_shell_num.at(i) << endl;
  }

  fout << atom_list.size() << endl;
  for (size_t i = 0; i < atom_list.size(); i++) {
    fout << setw(16) << atom_list[i].cpos(1) << setw(16) << atom_list[i].cpos(2) << setw(16) << atom_list[i].cpos(3) << endl;
  }
...
bool comparison_cluster_list(const vector<cecluster>& cluster_list_1, const vector<cecluster>& cluster_list_2) {
  // comparison of two clusters lists grouped by the same pair_name
  // "smaller" cluster list has "small" pair_name
  // sorted by descending order
  // sorted then by atom position

  const int size1 = cluster_list_1.size();
  const int size2 = cluster_list_2.size();
...
ceECIcluster::ceECIcluster() {
  ...
  str_cluster = nullptr; // not assign any memory
}
...
//     for(size_t i=0; i<ECI_cluster.size(); i++) {
...
void ceECIcluster::GetECICluster() {
  ...
  for (size_t i = 0; i < str_cluster->rep_cluster.size(); i++) {
...
void ceECIcluster::PrintOutECICluster() {
  ...
  //// output the name of fit cluster
  // cerr << "The ECI fit clusters are" << endl;
  // for(size_t i=0; i<allECI_cluster.size(); i++) {
...
void ceECIcluster::PrintECI(ostream& os) {
  const ios_base::fmtflags old_stat = os.setf(ios_base::fixed, ios_base::floatfield);
  os.precision(6);

  os << "ECIs " << ECI_cluster.size() << endl;

  os << setw(12) << "0 0 0 0" << setw(12) << ECIValue().at(0) << endl;
  for (size_t i = 0; i < ECI_cluster.size(); i++) {
    const int index = ECI_cluster[i];
    os << setw(12) << str_cluster->rep_cluster.at(index).pair_name << setw(12) << ECI.at(i + 1) << endl;
  }
...
void ceECIcluster::WriteFile(ostream& os) {
  ...
  const ios_base::fmtflags old_stat = os.setf(ios_base::fixed, ios_base::floatfield);
  os.precision(12);
  const int width = 12 + 6;

  os << ECI.size() << endl;

  // ECI values
  for (size_t i = 0; i < ECI.size(); i++) {
    os << setw(width) << ECI[i] << endl;
  }
  ...
  for (size_t i = 0; i < ECI_cluster.size(); i++) {
    ...
    const int atom_num = str_cluster->rep_cluster.at(index).structure.atoms.size();

    os << atom_num << endl;

    for (int i1 = 0; i1 < atom_num; i1++) {
      os << setw(width) << str_cluster->rep_cluster.at(index).structure.atoms.at(i1).cpos[1] << setw(width) << str_cluster->rep_cluster.at(index).structure.atoms.at(i1).cpos[2] << setw(width)
         << str_cluster->rep_cluster.at(index).structure.atoms.at(i1).cpos[3] << endl;
    }
  }
...
void ceECIcluster::ReadIn(istream& os) {
  ...
  if (str_cluster == nullptr) {
    throw aurostd::xerror(__AFLOW_FILE__, "ceECIcluster::ReadIn():", "clusters must be supplied", _FILE_CORRUPT_); // CO20200624
  }
  ...
  for (int i = 0; i < ECI_cluster_num; i++) {
    ...
    for (size_t j1 = 0; j1 < str_cluster->rep_cluster.size(); j1++) {
...
void ceECIcluster::DeleteZeroECI() {
  ...
  for (size_t i = 1; i < ECI.size(); i++) {
    if (std::abs(ECI[i]) > _ECI_ZERO) {
      ECI_tmp.push_back(ECI[i]);
...
cestructure::cestructure() : structure() {
  ...
  str_cluster = nullptr; // void the pointer
}

cestructure::cestructure(string& str_name, double stoich_b_in, double fit_quantity) : structure() {
  name = str_name;
...
ostream& operator<<(ostream& os, const cestructure& cestr) {
  ...
  os << setw(6) << cestr.name << setw(12) << cestr.stoich_b << setw(12) << cestr.energy_in;
...
void cestructure::SetUp(const ceallclusters& cluster1, ceECIcluster& cluster2, istream& corfilein) {
  // set up everthing
  SetStrCluster(cluster1);
  SetAllECICluster(cluster2);
...
vector<double> cestructure::GetCorrelation(vector<int>& index_list) {
  ...
  for (size_t i1 = 0; i1 < structure.atoms.size(); i1++) {
    for (uint l1 = 1; l1 < 4; l1++) {
      if (str_tmp.atoms.at(i1).fpos[l1] < 0) {
        str_tmp.atoms.at(i1).fpos[l1] += std::abs(int(str_tmp.atoms.at(i1).fpos[l1])) + 1.0;
      }
    }
  ...
  for (size_t i = 0; i < str_tmp.atoms.size(); i++) {
    const vector<int> vec_tmp;
  ...
  double g12;
  double g13;
  for (size_t i1 = 0; i1 < atom_list.size(); i1++) {
    _atom atom_tmp;

    // str_tmp.FixLattices();

    for (size_t l2 = 0; l2 < str_tmp.atoms.size(); l2++) {
      atom_tmp.cpos = atom_list.at(i1).cpos - str_tmp.atoms[l2].cpos;

      g11 = std::abs(str_tmp.klattice[1][1] * atom_tmp.cpos[1] + str_tmp.klattice[1][2] * atom_tmp.cpos[2] + str_tmp.klattice[1][3] * atom_tmp.cpos[3]) / (2.0 * _pi) + _EQUAL_DOUBLE * 0.1;
      g12 = std::abs(str_tmp.klattice[2][1] * atom_tmp.cpos[1] + str_tmp.klattice[2][2] * atom_tmp.cpos[2] + str_tmp.klattice[2][3] * atom_tmp.cpos[3]) / (2.0 * _pi) + _EQUAL_DOUBLE * 0.1;
      g13 = std::abs(str_tmp.klattice[3][1] * atom_tmp.cpos[1] + str_tmp.klattice[3][2] * atom_tmp.cpos[2] + str_tmp.klattice[3][3] * atom_tmp.cpos[3]) / (2.0 * _pi) + _EQUAL_DOUBLE * 0.1;

      if ((std::abs(g11 - int(g11)) < _EQUAL_DOUBLE) && (std::abs(g12 - int(g12)) < _EQUAL_DOUBLE) && (std::abs(g13 - int(g13)) < _EQUAL_DOUBLE)) {
  ...
  // cerr << "atom config\n";
  // for(size_t l1 = 0; l1<equiv_atoms.size(); l1++) {
  //     cerr << structure.atoms.at(l1).name << " : " ;
  //     for(size_t l2 = 0; l2<equiv_atoms.at(l1).size(); l2++) {
  ...
  for (size_t i = 0; i < structure.atoms.size(); i++) {
    ...
    for (size_t i1 = 0; i1 < structure.atoms.size(); i1++) {
      for (uint l1 = 1; l1 < 4; l1++) {
        str_tmp.atoms.at(i1).cpos[l1] = structure.atoms[i1].cpos[l1] - structure.atoms[i].cpos[l1];
      }
    ...
    for (size_t i1 = 0; i1 < atoms_tmp.size(); i1++) {
      for (size_t i2 = 0; i2 < str_tmp.atoms.size(); i2++) {
        if (is_equal(atoms_tmp[i1], str_tmp.atoms[i2])) {
          atoms_tmp[i1].name = str_tmp.atoms[i2].name;
          break;
        }
      }
    }
    ...
    // for(size_t i1=0; i1 < atoms_org.size(); i1++) {
    //     str_tmp.atoms.at(i1).cpos = F2C(str_tmp.lattice, str_tmp.atoms.at(i1).fpos);
    //     cerr << str_tmp.atoms.at(i1) << " " << str_tmp.atoms.at(i1).name << endl;
    // }

    for (size_t i1 = 0; i1 < equiv_atoms.size(); i1++) {
      for (size_t i2 = 0; i2 < equiv_atoms[i1].size(); i2++) {
        atom_list.at(equiv_atoms[i1][i2]).name = str_tmp.atoms.at(i1).name;
      }
    }
    ...
    configuration.clear();
    for (size_t i1 = 0; i1 < atom_list.size(); i1++) {
      const char ch = atom_list[i1].name.at(0);
      switch (ch) {
        case 'A':
        case 'a': configuration.push_back(_A_ATOM); break;
        case 'B':
        case 'b': configuration.push_back(_B_ATOM); break;
      }
    }
    ...
    // for(size_t i1=0; i1<configuration.size(); i1++) {
    //     cerr << setw(4)
    //         << configuration.at(i1) << " ";
    // }
    // cerr << endl;
    ...
    for (size_t j = 0; j < index_list.size(); j++) {
      for (size_t j1 = 0; j1 < str_cluster->all_cluster.at(j).size(); j1++) {
        cecluster cluster1 = str_cluster->all_cluster.at(j).at(j1);
        int cor_tmp = 1;
        for (size_t j2 = 0; j2 < cluster1.structure.indices.size(); j2++) {
          const int ind = cluster1.structure.indices.at(j2);
...
void cestructure::GetECICorrelation() {
  ...
  for (size_t i = 0; i < ECI_cluster.size(); i++) {
    for (size_t j = 0; j < allECI_cluster.size(); j++) {
      if (ECI_cluster[i] == allECI_cluster[j]) {
        // ******************************************************
...
void cestructure::GetAllECICorrelation(istream& filein) {
  ...
  allECI_equivalent_num.push_back(1); // "0 0 0 0" cluster
  for (size_t i = 0; i < allECI_cluster.size(); i++) {
    const int index = allECI_cluster[i];
    allECI_equivalent_num.push_back(str_cluster->rep_cluster.at(index).equivalent_num);
  }
...
void cestructure::PrintOutCorrelation() {
  ...
  for (size_t i = 0; i < ECI_correlation.size(); i++) {
    if (i == 0) {
      cerr << setw(12) << "0 0 0 0";
    } else {
      const int index = ECI_cluster.at(i - 1);
      cerr << setw(12) << str_cluster->rep_cluster.at(index).pair_name;
    }
    cerr.setf(ios_base::fixed, ios_base::floatfield);
    cerr.precision(6);
    cerr << setw(12) << "eq cluster " << setw(4) << ECI_equivalent_num.at(i) << setw(6) << " cor " << setw(10) << ECI_correlation[i] << endl;
  }
...
void cestructure::WriteFile(ostream& os) {
  ...
  for (size_t i = 0; i < ECI_correlation.size(); i++) {
    if (i == 0) {
      ...
      const int index = ECI_cluster.at(i - 1);
      os << setw(12) << str_cluster->rep_cluster.at(index).pair_name;
    }
    os.setf(ios_base::fixed, ios_base::floatfield);
    os.precision(8);
    os << setw(12) << ECI_correlation[i] << endl;
  }
  ...
  while (fin >> str_name >> cor_num) {
    if ((str_name != name) || (cor_num != int(allECI_cluster.size()) + 1)) {
      ...
      for (int i = 0; i < cor_num; i++) {
        ...
        fin >> site_num >> NNNum >> dist >> index >> cor;
      }
      continue;
    }

    allECI_correlation.clear();
    for (size_t i = 0; i < allECI_cluster.size() + 1; i++) {
...
void cestructure::GetEnergy() {
  ...
  for (size_t j = 0; j < ECI_correlation.size(); j++) {
    // cerr << "j = " << j << " " << ECI.at(j)
    //     << " " << ECI_correlation.at(j)
    //     << " " << ECI_equivalent_num.at(j) << endl;
    energy += ECI.at(j) * ECI_correlation[j] * ECI_equivalent_num.at(j);
  }
}
...
void cestructure::PrintOutComparison(ostream& os) {
  ...
  os << setw(6) << name << setw(12) << stoich_b << setw(12) << energy << setw(12) << energy_in << setw(12) << de << endl;
...
ostream& operator<<(ostream& os, const ceralloy& cestr) {
  ...
  os << setw(6) << cestr.name << setw(12) << cestr.stoich_b << setw(12) << cestr.energy_in << setw(12) << cestr.energy;
...
void ceralloy::GetAllECICorrelation() {
  ...
  for (size_t i = 0; i < allECI_cluster.size(); i++) {
    const int index = allECI_cluster[i];
    const int site_num = str_cluster->rep_cluster.at(index).site_num;
    const double tmp = (1.0 - stoich_b) * _A_ATOM + stoich_b * _B_ATOM;
    const double cor = pow(tmp, site_num);
...
void cesubcluster::SetUp(vector<int> base_cluster, ceallclusters& cluster1) {
  ...
  for (size_t i = 1; i < base_cluster.size(); i++) {
    AddBaseCluster(base_cluster[i]);
  }
  GetOverlapCluster();
}
...
void cesubcluster::SetBaseCluster(int base_cluster_nr) {
  if (!base_cluster.empty()) {
    throw aurostd::xerror(__AFLOW_FILE__, "cesubcluster::SetBaseCluster():", "already has a base", _INPUT_ILLEGAL_); // CO20200624
  } else {
...
void cesubcluster::SetBaseCluster(vector<int> base_cluster_nr) {
  ...
  //} else {
  //    throw aurostd::xerror(__AFLOW_FILE__,"cesubcluster::SetBaseCluster():","must get all clusters of a structure first",_INPUT_ILLEGAL_); //CO20200624
  //}
}
...
void cesubcluster::AddBaseCluster(vector<int> base_cluster_nr) {
  for (size_t i = 0; i < base_cluster_nr.size(); i++) {
    AddBaseCluster(base_cluster_nr[i]);
  }
}
...
vector<Baker_str> cesubcluster::GetSubCluster(int base_cluster) {
  ...
  const int total_atom_num = cluster1.structure.atoms.size();
  ...
  const int dist = cluster1.dist;
  int site_num = cluster1.site_num;
  if (site_num) {};
  int NNNum = cluster1.NNNum;
  if (NNNum) {};
  int dist_tmp;
  int index_tmp = cluster1.index;
  if (index_tmp) {};
  ...
  for (int i = total_atom_num - 1; i >= 0; i--) {
    const int str_size = CombinationNr(i + 1, total_atom_num);
    ...
    for (int j = 0; j < str_size; j++) {
      ...
      bool flag_1;
      bool flag_2;
      for (size_t k = 0; k < str_cluster->all_cluster.size(); k++) {
        for (size_t k1 = 0; k1 < str_cluster->all_cluster.at(k).size(); k1++) {
          ...
          flag_1 = (str_cluster->all_cluster.at(k).at(k1).site_num == total_atom_num) && (str_cluster->all_cluster.at(k).at(k1).NNNum == cluster1.NNNum) && (dist_tmp == dist);
          ...
          if (flag_1 || flag_2) {
            ...
            for (size_t k2 = 0; k2 < sub_cluster.size(); k2++) {
              if (str_cluster->rep_cluster.at(sub_cluster[k2].index - 1).pair_name == cluster_name) {
                // if found add the number by 1
                if (is_equal(str_cluster->all_cluster.at(k).at(k1), cluster2)) {
                  sub_cluster[k2].n_num++;
                }
                flag = true;
                break; // stop k2 loop
              }
            ...
            if (flag == false) {
              ...
              const int m_tmp = str_cluster->rep_cluster.at(k).equivalent_num / str_cluster->rep_cluster.at(k).structure.atoms.size();
  ...
  for (size_t i = 0; i < sub_cluster.size(); i++) {
    cerr << sub_cluster[i].index
         << " "
      //<< str_cluster->rep_cluster.at(sub_cluster[i]).pair_name
      //<< " "
         << sub_cluster[i].m_num << " " << sub_cluster[i].n_num << " " << endl;
  }
...
void cesubcluster::GetAllSubCluster() {
  for (size_t i = 0; i < base_cluster.size(); i++) {
    sub_cluster.push_back(GetSubCluster(base_cluster[i]));
  }
}
...
vector<Baker_str> cesubcluster::GetOverlapCluster(int index_in) {
  ...
  _atom atom1;
  _atom atom2;
  deque<_atom> atom_list;
  const deque<_atom> atom_list_old;
  cecluster cluster1;
  cecluster cluster2;
  cecluster cluster_overlap;
  ...
  for (size_t i = 0; i < str_cluster->all_cluster.at(index - 1).size(); i++) {
    cluster2 = str_cluster->all_cluster.at(index - 1).at(i);

    atom_list.clear();
    for (size_t j = 0; j < cluster1.structure.atoms.size(); j++) {
      atom1 = cluster1.structure.atoms[j];
      for (size_t k = 0; k < cluster2.structure.atoms.size(); k++) {
        atom2 = cluster2.structure.atoms[k];
    ...
    const int index_tmp = str_cluster->GetIndexbyCluster(cluster_overlap);

    bool flag = false;
    for (size_t j = 0; j < subcluster_list_out.size(); j++) {
      if (index_tmp == subcluster_list_out[j].index) {
        // already stored
        flag = true;
        break;
      }
    ...
    if (!flag) { // not stored
      for (size_t j = 0; j < sub_cluster.at(index_in - 1).size(); j++) {
  ...
  for (size_t i = 0; i < subcluster_list_out.size(); i++) {
    cerr << subcluster_list_out[i].index << " " << subcluster_list_out[i].m_num << " " << subcluster_list_out[i].n_num << endl;
  }

  return subcluster_list_out;
}

vector<Baker_str> cesubcluster::GetOverlapCluster(const vector<Baker_str>& subcluster_list1, const vector<Baker_str>& subcluster_list2) {
  ...
  const int size1 = subcluster_list1.size();
  const int size2 = subcluster_list2.size();
  ...
  for (int i = 0; i < size2; i++) {
    ...
    for (size_t j = 0; j < subcluster_list_new1.size(); j++) {
      subcluster1 = subcluster_list_new1.at(j);
      if ((subcluster2.n_num != 0) && (subcluster1.index == subcluster2.index)) {
  ...
  bool flag;
  flag = false;
  for (size_t i = 1; i < subcluster_list_new2.size(); i++) {
    if (subcluster_list_new2[i].index == subcluster_list1.at(size1 - 1).index) {
      flag = true;
    }
  }
  if (!flag) {
  ...
  flag = false;
  for (size_t i = 1; i < subcluster_list_new2.size(); i++) {
    if (subcluster_list_new2[i].index == subcluster_list2.at(size2 - 1).index) {
      flag = true;
    }
  }
  if (!flag) {
  ...
  // cerr << "Overlap\n";
  // for(size_t i=0; i<subcluster_list_new2.size(); i++) {
...
void cesubcluster::GetOverlapCluster() {
  ...
  vector<Baker_str> overlap_list;
  vector<Baker_str> overlap_list_out;
  const vector<vector<Baker_str>> subcluster_list_all;
  ...
  for (int i = 0; i < size; i++) {
    ...
    overlap_list = GetOverlapCluster(sub_cluster.at(config.at(0)), sub_cluster.at(config.at(1)));
    overlap_list_out = MergeTwoSubCluster(overlap_list, overlap_list_out);
  }
  ...
  for (size_t i = 0; i < overlap_cluster.size(); i++) {
    cerr << "n_num " << overlap_cluster[i].n_num << endl;
    cerr << "m_num " << overlap_cluster[i].m_num << endl;
    cerr << "index " << overlap_cluster[i].index << endl;
    cerr << endl;
  }
  cerr << endl;
}

vector<Baker_str> cesubcluster::MergeTwoSubCluster(const vector<Baker_str>& subcluster_list1, const vector<Baker_str>& subcluster_list2) {
  ...
  for (size_t i = 0; i < subcluster_list2.size(); i++) {
    subcluster_tmp = subcluster_list2[i];
    flag = false;
    for (size_t j = 0; j < subcluster_list_new.size(); j++) {
      if (is_equal(subcluster_tmp, subcluster_list_new[j])) {
        flag = true;
        break;
      }
    }
  ...
  // cerr << "After Merge\n";
  // for(size_t i=0; i<subcluster_list_new.size(); i++) {
...
ceSL::ceSL(string& str_name) : cestructure(str_name, 0.0, 0.0) {
  ...
  if (!(base == "fcc" || base == "bcc" || base == "hcp")) {
    throw aurostd::xerror(__AFLOW_FILE__, "ceSL::ceSL():", "structure must be one of fcc/bcc/hcp", _INPUT_ILLEGAL_); // CO20200624
  }
}

ceSL::ceSL(string& str_name, double stoich_b, double fit_quantity) : cestructure(str_name, stoich_b, fit_quantity) {
  for (size_t i = 0; i < name.size(); i++) {
    base.push_back(tolower(name[i]));
  }
...
ceSL::ceSL(string& str_name, xstructure& xstr) : cestructure(xstr) {
  ...
  for (size_t i = 0; i < structure.atoms.size(); i++) {
    atom1 = structure.atoms[i];
    flag = true;

    for (size_t j = 0; j < xstr.fgroup.size() && flag; j++) {
      atom2.fpos = xstr.fgroup[j].Uf * atom1.fpos + xstr.fgroup[j].ftau;
      for (size_t k = 0; k < xstr.atoms.size() && flag; k++) {
        if (is_equal_fpos(atom2, xstr.atoms[k])) {
          if (xstr.atoms[k].name == xstr.species.at(0)) {
            structure.atoms.at(i).name = "A";
          } else if (xstr.atoms[k].name == xstr.species.at(1)) {
...
ostream& operator<<(ostream& os, const ceSL& cestr) {
  ...
  os << setw(18) << cestr.name << setw(12) << cestr.stoich_b << setw(12) << cestr.energy;
...
void ceSL::SetUp(string& SLname) {
  if (NameToStructure(SLname)) {
    ...
    const int cell_nr_in = cell_nr;
    GetStructure(N_mat, cell_nr_in);
    SetConfig(config);
  } else {
    throw aurostd::xerror(__AFLOW_FILE__, "ceSL::SetUp():", "cannot find superlattice with name " + SLname, _INPUT_MISSING_); // CO20200624
  }
}
...
void ceSL::GetStructure(xmatrix<int> Nmat_in, int cell_nr_in) {
  ...
  xstructure str_base;
  xstructure str_SL; // get the structure of structure_type (fcc/bcc/hcp)
  str_base.coord_flag = _COORDS_CARTESIAN_;
  ...
  const int atom_nr = cell_nr_in * str_base.atoms.size();
  cell_nr = cell_nr_in;
  ...
  for (int i1 = -Nmax; i1 < Nmax + 1; i1++) {
    for (int i2 = -Nmax; i2 < Nmax + 1; i2++) {
      for (int i3 = -Nmax; i3 < Nmax + 1; i3++) {
        ...
        flag = flag && (1.0 - std::abs(x1) > _EQUAL_DOUBLE) && (1.0 - std::abs(x2) > _EQUAL_DOUBLE) && (1.0 - std::abs(x3) > _EQUAL_DOUBLE);

        if (flag) {
          for (size_t k = 0; k < str_base.atoms.size(); k++) {
            for (int l1 = 1; l1 < 4; l1++) {
              atom_tmp.cpos[l1] = x1 * str_SL.lattice[1][l1] + x2 * str_SL.lattice[2][l1] + x3 * str_SL.lattice[3][l1];
              atom_tmp.name = "A";
            }
            ...
            for (uint l1 = 1; l1 < 4; l1++) {
              if (atom_tmp.fpos[l1] < 0) {
                atom_tmp.fpos[l1] += std::abs(int(atom_tmp.fpos[l1])) + 1.0;
              }
            }
            ...
            for (size_t l1 = 0; l1 < str_SL.atoms.size(); l1++) {
              // no duplicated atoms in base atoms

              if (is_equal(atom_tmp, str_SL.atoms[l1])) {
                flag_nodup = false;
                break;
              }
            }
...
void ceSL::SetConfig(vector<int>& atom_config) {
  ...
  for (size_t k = 0; k < structure.atoms.size(); k++) {
    structure.atoms[k].name = "A";
  }

  for (size_t k = 0; k < atom_config.size(); k++) {
    structure.atoms.at(atom_config[k]).name = "B";
  }

  const int total_atom = structure.atoms.size();
...
void ceSL::StructureToName() {
  ...
  for (size_t i = 0; i < config.size(); i++) {
    sum += int(pow(2.0, double(config[i])));
  }

  const string prefix = base + "_bSL";
  name.clear();
  name = prefix + name;
  for (size_t i = 0; i < N.size(); i++) {
    for (size_t j = 0; j < N.at(0).size(); j++) {
      name = name + aurostd::utype2string(N[i].at(j));
      name.push_back('_');
    }
  }
...
bool ceSL::NameToStructure(string& SLname) {
  const string prefix = "SL";
  bool flag_fmt = true;
  const uint name_size = SLname.size();
  size_t SL_pos;
  SL_pos = SLname.find("SL");
  if ((uint(SL_pos) > name_size) && (SLname.find("0x") > name_size) && (SLname.find("_") > name_size)) {
    flag_fmt = false;
  }
  ...
  for (int j = 0; j < 3; j++) {
    ...
    for (int j2 = 0; j2 < 3; j2++) {
      num.clear();
      for (size_t i = 0; i < name_tmp.size(); i++) {
        if (name_tmp[i] != '_') {
          num.push_back(name_tmp[i]);
        } else {
  ...
  for (int i = name_tmp.size() - 1; i >= 0; i--) {
    ...
    switch (ch) {
      ...
      case 'F':
      case 'f': n = 15; break;
      default: n = aurostd::string2utype<int>(num);
    }
...
void ceSL::PrintStructure(ostream& os) {
  os << name << endl;
  os << "trans_matrix = ";
  for (size_t i1 = 0; i1 < N.size(); i1++) {
    for (size_t i2 = 0; i2 < N.at(0).size(); i2++) {
      os << setw(4) << N[i1].at(i2);
    }
    os << " | ";
  }
  os << endl;
  ...
  os.precision(14);
  for (int i = 1; i <= 3; i++) {
    os << setw(19) << structure.lattice[i][1] << setw(19) << structure.lattice[i][2] << setw(19) << structure.lattice[i][3] << endl;
  }
  os << setw(4) << structure.num_each_type.at(0) << setw(4) << structure.num_each_type.at(1) << endl;
  ...
  for (size_t i = 0; i < structure.atoms.size(); i++) {
    os << setw(19) << structure.atoms[i].fpos[1] << setw(19) << structure.atoms[i].fpos[2] << setw(19) << structure.atoms[i].fpos[3] << setw(4) << structure.atoms[i].name << endl;
  }
...
void ceSL::PrintStructure(ostream& os, vector<_ceatom> atom_species) {
  ...
  for (size_t i = 0; i < structure.atoms.size(); i++) {
    string name;
    if (structure.atoms[i].name == "A") {
      name = atom_species.at(0).name;
    } else if (structure.atoms[i].name == "B") {
      name = atom_species.at(1).name;
    }
    os << setw(19) << structure.atoms[i].fpos[1] << setw(19) << structure.atoms[i].fpos[2] << setw(19) << structure.atoms[i].fpos[3] << setw(4) << name << endl;
  }
  os.precision(_DEFAULT_PRECISION);
  os.setf(old_stat, ios_base::floatfield);
}
...
double ceSL::IsSQS(int site_num, int NNNum) {
  ...
  for (size_t i = 0; i < str_cluster->rep_cluster.size(); i++) {
    ...
    if (cluster_iter.site_num <= site_num && cluster_iter.site_num > 1) {
  ...
  const int size_tmp = SQScompare_cluster_list.size();
  for (size_t i = 0; i < SQScompare_cluster_list.size(); i++) {
    // whether correlation is needed to be calculated
    const int index = SQScompare_cluster_list[i].index;
    iter = find(allECI_cluster.begin(), allECI_cluster.end(), index);
    if (iter < allECI_cluster.end()) {
      // correlation has already been calculated
      const int pos = iter - allECI_cluster.begin();
      if (pos < int(allECI_correlation.size())) {
        SQScompare_cluster_list[i].correlation = allECI_correlation.at(pos);
      } else {
    ...
    const double tmp = (1.0 - stoich_b) * _A_ATOM + stoich_b * _B_ATOM;
    const double cor = pow(tmp, SQScompare_cluster_list[i].site_num);
    SQScompare_cluster_list[i].correlation_random_structure = cor;
    ...
    // SQScompare_cluster_list[i].weight =
    //    (i==0) ? pow(size_tmp, 3): pow((size_tmp - i -1), 2);
    SQScompare_cluster_list[i].weight = pow(10.0, (size_tmp - i));
  }

  vector<double> cor_list;
  if (!cal_cor_list.empty()) {
    cor_list = GetCorrelation(index_list);
    for (size_t i = 0; i < cal_cor_list.size(); i++) {
      const int cluster_index = cal_cor_list[i];
      SQScompare_cluster_list.at(cluster_index).correlation = cor_list.at(i);
    }
  }
  ...
  for (size_t i = 0; i < SQScompare_cluster_list.size(); i++) {
    const double cor = SQScompare_cluster_list[i].correlation;
    const double cor_random = SQScompare_cluster_list[i].correlation_random_structure;
    const double weight = SQScompare_cluster_list[i].weight;
...
void ceSL::OutputSQS(ostream& oss) {
  oss << setw(20) << "Strucutre " << setw(12) << name << endl;
  oss << setw(20) << "concentration" << setw(20) << stoich_b << endl;
  oss << setw(20) << "atom number" << setw(20) << cell_nr << endl;
  oss << "-------------------------------------------------------------\n";
  oss << setw(10) << "cluster" << setw(30) << "          correlation" << endl;

  oss << "         " << setw(20) << "CE::SQS" << setw(20) << "random" << endl;
  oss << "-------------------------------------------------------------\n";
  for (size_t i = 0; i < SQScompare_cluster_list.size(); i++) {
    oss << setw(10) << SQScompare_cluster_list[i].pair_name << setw(20) << SQScompare_cluster_list[i].correlation << setw(20) << SQScompare_cluster_list[i].correlation_random_structure << endl;
  }
}
...
bool ceSL::IsPrimitiveCell() {
  ...
  for (size_t i = 0; i < structure.atoms.size(); i++) {
    atom1 = structure.atoms[i]; // check whether atoms.at(j) is a translation
    ...
    for (size_t j = 0; j < structure.atoms.size(); j++) {
      ...
      for (uint l1 = 1; l1 < 4; l1++) {
        if (atom4.fpos[l1] < 0) {
          atom4.fpos[l1] += std::abs(int(atom4.fpos[l1])) + 1.0;
        }
      }
      ...
      for (size_t l1 = 0; l1 < structure.atoms.size(); l1++) {
        atom3 = structure.atoms[l1];
