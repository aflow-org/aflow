...
#include "extern/SYMBOLICCPLUSPLUS/symbolic_main.h"
#include "extern/SYMBOLICCPLUSPLUS/symmatrix.h"
...
void substituteVariableWithParameterDesignation(SymbolicWyckoffSite& Wyckoff_symbolic) {
  ...
  for (size_t i = 0; i < Wyckoff_symbolic.equations.size(); i++) {
...
namespace symbolic {
  Symbolic string2symbolic(const string& str) {
    ...
    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_SYMBOLIC_);
    ...
    Symbolic out;
    for (size_t i = 0; i < sdouble_temp.size(); i++) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " sdouble_temp: (dbl) " << sdouble_temp[i].dbl << " (chr) " << sdouble_temp[i].chr << endl;
      }
      // ---------------------------------------------------------------------------
      // if contains char
      if (sdouble_temp[i].chr != '\0') {
        stringstream ss_char;
        ss_char << sdouble_temp[i].chr;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " symbolic expression: out=" << out << endl;
    }
    return out;
  }
} // namespace symbolic
...
namespace symbolic {
  bool isEqual(const Symbolic& a, const Symbolic& b) {
    ...
    const bool VERBOSE = false; // VERBOSE INSTEAD OF LDEBUG SINCE A NESTED FUNCTION

    const Symbolic diff = a - b;
...
namespace symbolic {
  bool isEqualVector(const Symbolic& a_vec, const Symbolic& b_vec) {
    ...
    if (a_vec_info != typeid(SymbolicMatrix).name() || b_vec_info != typeid(SymbolicMatrix).name()) {
      stringstream message;
      message << "One or both of the inputs are not a SymbolicMatrix (i.e., typeids are different):"
              << " a_vec (input) id: " << a_vec_info << " b_vec (input) id: " << b_vec_info << " SymbolicMatrix id: " << typeid(SymbolicMatrix).name();
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_);
    }

    const bool VERBOSE = false; // VERBOSE INSTEAD OF LDEBUG SINCE FUNCITON IS NESTED
...
namespace symbolic {
  vector<vector<string>> matrix2VectorVectorString(const Symbolic& lattice) {
    ...
    if (lattice_info != typeid(SymbolicMatrix).name()) {
      stringstream message;
      message << "The input is not a SymbolicMatrix (i.e., typeids are different): lattice (input) id: " << lattice_info << " SymbolicMatrix id: " << typeid(SymbolicMatrix).name();
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_);
    }
    ...
    for (uint i = 0; i < 3; i++) {
      vector<string> row;
      for (uint j = 0; j < 3; j++) {
        stringstream ss_component;
        ss_component << lattice.row(i)(j);
...
namespace symbolic {
  Symbolic BringInCell(const Symbolic& vec_in, double tolerance, double upper_bound, double lower_bound) {
    ...
    const Symbolic constant = Symbolic(1); // 1 is a great constant

    for (uint i = 0; i < 3; i++) {
      // ---------------------------------------------------------------------------
      // bring in cell based on constant term
      while (double(vec_out(i).coeff(constant, 1)) - upper_bound >= -tolerance) {
        vec_out(i) -= 1;
      }
      while (double(vec_out(i).coeff(constant, 1) - lower_bound) < -tolerance) {
        vec_out(i) += 1;
      }
    }
    return vec_out;
...
namespace anrl {
  symbolic::Symbolic SymbolicANRLPrimitiveLattices(const string& lattice_and_centering, const char& space_group_letter) {
    ...
    const symbolic::Symbolic a("a");
    const symbolic::Symbolic b("b");
    const symbolic::Symbolic c("c");
    const symbolic::Symbolic cx("cx");
    const symbolic::Symbolic cy("cy");
    const symbolic::Symbolic cz("cz");
    const symbolic::Symbolic beta("beta");
    const symbolic::Symbolic gamma("gamma");
// These are added to fix the handling of square roots in hexagonal/rhombohedral lattices
// MJM20240222 from DX20210106 changes in version 3.13_merge_AZ_DH
    const symbolic::Symbolic half = symbolic::Symbolic(1) / 2;
    const symbolic::Symbolic sqrt3 = sqrt(symbolic::Symbolic(3));
    const symbolic::Symbolic third = symbolic::Symbolic(1) / 3;
    const symbolic::Symbolic one_sym = symbolic::Symbolic(1);

    // ---------------------------------------------------------------------------
    // triclinic (aP)
    if (lattice_and_centering == "aP") {
      lattice = ((a, _SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_), (b * cos(gamma), b * sin(gamma), _SYMBOLIC_ZERO_), (cx, cy, cz));
    }

    // ---------------------------------------------------------------------------
    // simple monoclinic (mP)
    if (lattice_and_centering == "mP") {
      lattice = ((a, _SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, b, _SYMBOLIC_ZERO_), (c * cos(beta), _SYMBOLIC_ZERO_, c * sin(beta)));
    }

    // ---------------------------------------------------------------------------
    // base-centered monoclinic (mC)
    if (lattice_and_centering == "mC") {
      lattice = ((half * a, -half * b, _SYMBOLIC_ZERO_), // DX20210106 - need parentheses and floats
                 (half * a, half * b, _SYMBOLIC_ZERO_), (c * cos(beta), _SYMBOLIC_ZERO_, c * sin(beta)));
    }

    // ---------------------------------------------------------------------------
    // simple orthorhombic (oP)
    if (lattice_and_centering == "oP") {
      lattice = ((a, _SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, b, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, c));
    }
    ...
    if (lattice_and_centering == "oC") {
      if (space_group_letter == 'A') {
        lattice = ((a, _SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, half * b, -half * c), (_SYMBOLIC_ZERO_, half * b, half * c));
      }
      if (space_group_letter == 'C') {
        lattice = ((half * a, -half * b, _SYMBOLIC_ZERO_), (half * a, half * b, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, c));
      }
    }
    ...
    if (lattice_and_centering == "oI") {
      lattice = ((-half * a, half * b, half * c), (half * a, -half * b, half * c), (half * a, half * b, -half * c));
    }

    // ---------------------------------------------------------------------------
    // face-centered orthorhombic (oF)
    if (lattice_and_centering == "oF") {
      lattice = ((_SYMBOLIC_ZERO_, half * b, half * c), (half * a, _SYMBOLIC_ZERO_, half * c), (half * a, half * b, _SYMBOLIC_ZERO_));
    }

    // ---------------------------------------------------------------------------
    // simple tetragonal (tP)
    if (lattice_and_centering == "tP") {
      lattice = ((a, _SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, a, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, c));
    }

    // ---------------------------------------------------------------------------
    // body-centered tetragonal (tI)
    if (lattice_and_centering == "tI") {
      lattice = ((-half * a, half * a, half * c), (half * a, -half * a, half * c), (half * a, half * a, -half * c));
    }

    // ---------------------------------------------------------------------------
    // hexagonal/trigonal (hP)
    if (lattice_and_centering == "hP") {
    // Symbolic changes originally MJM20240222/DX20210106 -- MJM20250521
    //  lattice = ((half*a, -(sqrt(3.0)/2.0)*a, _SYMBOLIC_ZERO_),
    //      (half*a, (sqrt(3.0)/2.0)*a, _SYMBOLIC_ZERO_),
      lattice = ((half * a, -(sqrt3 * half) * a, _SYMBOLIC_ZERO_), (half * a, (sqrt3 * half) * a, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, c));
    }

    // ---------------------------------------------------------------------------
    // rhombohedral (hR)
    if (lattice_and_centering == "hR") {
      // lattice = ((half*a, -(1.0/(2.0*sqrt(3.0)))*a, (1.0/3.0)*c),
      //    (_SYMBOLIC_ZERO_, (1.0/sqrt(3.0))*a, (1.0/3.0)*c),
      //    (-half*a, -(1.0/(2.0*sqrt(3.0)))*a, (1.0/3.0)*c));
      lattice = ((half * a, -(half / sqrt3) * a, (third) *c), (_SYMBOLIC_ZERO_, (one_sym / sqrt3) * a, (third) *c), (-half * a, -(half / sqrt3) * a, (third) *c));
    }

    // ---------------------------------------------------------------------------
    // simple cubic (cP)
    if (lattice_and_centering == "cP") {
      lattice = ((a, _SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, a, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, a));
    }

    // ---------------------------------------------------------------------------
    // body-centered cubic (cI)
    if (lattice_and_centering == "cI") {
      lattice = ((-half * a, half * a, half * a), (half * a, -half * a, half * a), (half * a, half * a, -half * a));
    }

    // ---------------------------------------------------------------------------
    // face-centered cubic (cF)
    if (lattice_and_centering == "cF") {
      lattice = ((_SYMBOLIC_ZERO_, half * a, half * a), (half * a, _SYMBOLIC_ZERO_, half * a), (half * a, half * a, _SYMBOLIC_ZERO_));
    }
...
namespace anrl {
  vector<symbolic::Symbolic> equations2SymbolicEquations(const vector<vector<string>>& equations) {
    ...
    if (LDEBUG) {
      for (size_t i = 0; i < symbolic_equations.size(); i++) {
...
namespace anrl {
  symbolic::Symbolic cartesian2lattice(const symbolic::Symbolic& lattice, const symbolic::Symbolic& cartesian_coordinate) {
    ...
    const symbolic::Symbolic volume = lattice.row(0) | ((lattice.row(1) % (lattice.row(2))).transpose()); // | is dot product, % is cross product
    if (LDEBUG) {
    ...
    const symbolic::Symbolic b1 = (lattice.row(1) % (lattice.row(2))) / volume;
    const symbolic::Symbolic b2 = (lattice.row(2) % (lattice.row(0))) / volume;
    const symbolic::Symbolic b3 = (lattice.row(0) % (lattice.row(1))) / volume;
...
namespace anrl {
  symbolic::Symbolic getXYZ2LatticeTransformation(const string& lattice_and_centering) {
    ...
    // ---------------------------------------------------------------------------
    if (lattice_and_centering == "aP") {
      xyz2lattice = ((a, b * cos(gamma), cx), (_SYMBOLIC_ZERO_, b * sin(gamma), cy), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, cz));
    }

    else if (lattice_and_centering == "mP" || lattice_and_centering == "mC") {
      xyz2lattice = ((a, _SYMBOLIC_ZERO_, c * cos(beta)), (_SYMBOLIC_ZERO_, b, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, c * sin(beta)));
    } else if (lattice_and_centering == "oP" || lattice_and_centering == "oC" || lattice_and_centering == "oI" || lattice_and_centering == "oF") {
      xyz2lattice = ((a, _SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, b, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, c));
    } else if (lattice_and_centering == "tP" || lattice_and_centering == "tI") {
      xyz2lattice = ((a, _SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, a, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, c));
      ...
      xyz2lattice = ((a, _SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, a, _SYMBOLIC_ZERO_), (_SYMBOLIC_ZERO_, _SYMBOLIC_ZERO_, a));
    }
...
namespace anrl {
  vector<symbolic::Symbolic> getEquationsForCenteredLattices(const string& lattice_and_centering, const symbolic::Symbolic& lattice, const vector<symbolic::Symbolic>& conventional_equations) {
    ...
    const symbolic::Symbolic xyz2lattice = getXYZ2LatticeTransformation(lattice_and_centering);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " symbolic transformation from xyz to lattice: " << xyz2lattice << endl;
    }

    // ---------------------------------------------------------------------------
    // transform symbolic coordinates
    for (size_t i = 0; i < conventional_equations.size(); i++) {
      const symbolic::Symbolic vec = xyz2lattice * conventional_equations[i];
      lattice_equations.emplace_back(cartesian2lattice(lattice, vec).simplify());
    }
    ...
    for (size_t i = 0; i < lattice_equations.size(); i++) {
    ...
    for (size_t i = 0; i < lattice_equations.size(); i++) {
      ...
      for (size_t j = 0; j < primitive_lattice_equations.size(); j++) {
        if (symbolic::isEqualVector(primitive_lattice_equations[j], lattice_equations[i])) {
          is_unique_equation = false;
          break;
        }
      }
      if (is_unique_equation) {
        primitive_lattice_equations.push_back(lattice_equations[i]);
      }
    }
...
namespace anrl {
  vector<symbolic::Symbolic> convertEquations2FractionalEquations(const string& lattice_and_centering, const symbolic::Symbolic& lattice, const vector<symbolic::Symbolic> conventional_equations) {
    ...
    if (lattice_and_centering == "mC" || lattice_and_centering == "oC" || lattice_and_centering == "oI" || lattice_and_centering == "oF" || lattice_and_centering == "tI" || lattice_and_centering == "cI" ||
...
namespace anrl {
  void addSymbolicEquation2Atoms(const vector<symbolic::Symbolic>& equations, deque<_atom>& atoms, bool isfpos) {
    ...
    for (size_t i = 0; i < atoms.size(); i++) {
      ...
      for (uint j = 0; j < 3; j++) {
        stringstream ss_pos;
        ss_pos << equations[i].row(j);
        coordinate.push_back(ss_pos.str());
      }
      if (isfpos) {
        atoms[i].fpos_equation = coordinate;
      } else {
        atoms[i].cpos_equation = coordinate;
      }
    }
