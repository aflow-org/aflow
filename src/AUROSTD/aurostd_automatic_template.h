...
#define AST_UTYPE_NUM_COMPLEX \
  int, unsigned int, long int, long unsigned int, long long int, long long unsigned int, float, double, long double, aurostd::xcomplex<float>, aurostd::xcomplex<double>, aurostd::xcomplex<long double>
...
/// @endcode
#define AST_GEN_1(type_selection) AST_EVAL(AST_INVOKE(AST_MAP, AST_TEMPLATE, type_selection))
...
/// @brief autogenerate setters/getters for conversion between class members and json objects
/// @param accessor the setter or getter macro to use as a mapping
/// @param members the macro defining the list of members to expand on
/// @note this simply just maps the accessor macro over the elements of the members macro
#define AST_JSON_ACCESSOR(accessor, ...) AST_EVAL(AST_INVOKE(AST_MAP, accessor, __VA_ARGS__))

#define JSON_GETTER(var) {#var, this->var},
/// @note This assumes the json object is named @c jo
#define JSON_SETTER(var) this->var = static_cast<decltype(var)>(jo[#var]);

/// Expands the getter: {#var, this->var} on each of members
#define AST_JSON_GETTER(...) AST_JSON_ACCESSOR(JSON_GETTER, __VA_ARGS__)
/// Expands the setter: this->var = static_cast<decltype(var)>(jo[#var]);
/// @note This assumes the json object is named @c jo
#define AST_JSON_SETTER(...) AST_JSON_ACCESSOR(JSON_SETTER, __VA_ARGS__)
