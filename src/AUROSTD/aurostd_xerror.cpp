...
#define _AFLOW_NUM_ERR_TYPES_ 7  // Number of error types
  ...
  bool xerror::codeValid() const {
    if (error_code < 0) {
      return false;
    }
    if (error_type > _AFLOW_NUM_ERR_TYPES_) {
      return false;
    }
    if (error_type == 0 && error_number == 0) {
      return false;  // "Error code" 0 is reserved
    }
    if (error_number > 0) {  // an error value of 0 is the generic error, so it's always valid
      if (errors[error_type][error_number - 1].empty()) {
        return false;
      }
    }
    return true;
  ...
  std::string xerror::buildMessageString() {
    ...
    msgstr << error_string() << std::endl; //<< " - ";  // error type  //CO20181226
    if (error_code == 2) {
      //   msgstr << xerror_PID;  //SC20200508
  ...
  std::string xerror::error_string() const {
