...
namespace aurostd {  // namespace aurostd
  ...
  xmatrix<utype>::xmatrix(int nrh, int nch, int nrl, int ncl) : msize(0) {
    ...
    cerr << "M -> constructor:"
         << "  lrows=" << lrows << ", urows=" << urows << ", lcols=" << lcols << ", ucols=" << ucols << ", rows=" << rows << ", cols=" << cols << endl;
#endif
    if (msize > 0) {
      corpus = new utype*[rows + XXEND]();  // HE20220613 initialize corpus memory
      if (!corpus) {
        throw aurostd::xerror(__AFLOW_FILE__, "aurostd::xmatrix<utype>::xmatrix():", "allocation failure 1 (int,int,int,int)", _ALLOC_ERROR_);
      }
      corpus += -lrows + XXEND;
      corpus[lrows] = new utype[rows * cols + XXEND]();  // HE20220613 initialize corpus memory
      if (!corpus[lrows]) {
        throw aurostd::xerror(__AFLOW_FILE__, "aurostd::xmatrix<utype>::xmatrix():", "allocation failure 2 (int,int,int,int)", _ALLOC_ERROR_);
      }
      corpus[lrows] += -lcols + XXEND;
      for (int i = lrows + 1; i <= urows; i++) {
        corpus[i] = corpus[i - 1] + cols;
      }  // this propagates previous line to all lrows
      clear();
    }
#ifdef _XMATH_DEBUG_CONSTRUCTORS
    cerr << "issquare=" << issquare << ", isfloat=" << isfloat << ", iscomplex=" << iscomplex << ", sizeof=" << size << ", msize=" << msize << endl;
#endif
  }
#define AST_TEMPLATE(utype) template xmatrix<utype>::xmatrix(int, int, int, int);
...
#define AST_TEMPLATE(utype) template xmatrix<utype>::xmatrix(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_BOOL)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template xmatrix<utype>::xmatrix(const xvector<utype>&);
  AST_GEN_1(AST_UTYPE_BOOL)
#undef AST_TEMPLATE
  ...
  template <class utype> void xmatrix<utype>::init() {  // HE20220613 initialize all members of xmatrix
    rows = 0;
    lrows = 0;
    urows = 0;
    cols = 0;
...
namespace aurostd {  // namespace aurostd
  ...
  xmatrix<utype>::xmatrix(int vrows, int vcols, utype* a) : msize(0) {  // a starts from 0..
    ...
    if (msize > 0) {
      corpus = new utype*[rows + XXEND]();  // HE20220613 initialize corpus memory
      if (!corpus) {
        throw aurostd::xerror(__AFLOW_FILE__, "aurostd::xmatrix<utype>::xmatrix():", "allocation failure 1 (int,int,utype*)", _ALLOC_ERROR_);
      }
      corpus += -lrows + XXEND;
      corpus[lrows] = new utype[rows * cols + XXEND]();  // HE20220613 initialize corpus memory
      if (!corpus[lrows]) {
        throw aurostd::xerror(__AFLOW_FILE__, "aurostd::xmatrix<utype>::xmatrix():", "allocation failure 2 (int,int,utype*)", _ALLOC_ERROR_);
      }
      corpus[lrows] += -lcols + XXEND;
      int i = 0;
      int j = 0;
      ...
      for (i = lrows; i <= urows; i++) {
        for (j = lcols; j <= ucols; j++) {
          corpus[i][j] = a[(i - 1) * ucols + (j - 1)]; // a.corpus[i][j];  // LIKE FORTRAN
        }
      }
        //      delete [] a;
    }
...
#define AST_TEMPLATE(utype) template xmatrix<utype>::~xmatrix();
  AST_GEN_1(AST_UTYPE_BOOL)
#undef AST_TEMPLATE
} // namespace aurostd
// ----------------------------------------------------------------------------
...
namespace aurostd { // namespace aurostd
  template <class utype> void xmatrix<utype>::free() { // CO20190808
    if (msize > 0) {
      delete[] (corpus[lrows] + lcols - XXEND);
      delete[] (corpus + lrows - XXEND);
    }
    lrows = urows = lcols = ucols = 0;
    refresh();
  }
} // namespace aurostd

namespace aurostd {  // namespace aurostd
  template <class utype> void xmatrix<utype>::copy(const xmatrix<utype>& b) { // CO20190808
    if (lrows != b.lrows || urows != b.urows || lcols != b.lrows || ucols != b.ucols ||
        msize != b.msize) { // if dims(this)!=dims(a) => build a new xmatrix !!!  //CO20190808 - VERY IMPORTANT that we not only check lrows/urows/lcols/ucols, but msize, as xmatrix could just have been initialized (msize==0)
      free();
      lrows = b.lrows;
      urows = b.urows;
      rows = b.rows;
      ...
      if (msize > 0) {
        corpus = new utype*[rows + XXEND](); // HE20220613 initialize corpus memory
        if (!corpus) {
          throw aurostd::xerror(__AFLOW_FILE__, "aurostd::xmatrix<utype>::copy():", "allocation failure 1 in COPY", _ALLOC_ERROR_);
        }
        corpus += -lrows + XXEND;
        corpus[lrows] = new utype[rows * cols + XXEND](); // HE20220613 initialize corpus memory
        if (!corpus[lrows]) {
          throw aurostd::xerror(__AFLOW_FILE__, "aurostd::xmatrix<utype>::copy():", "allocation failure 2 in COPY", _ALLOC_ERROR_);
        }
        corpus[lrows] += -lcols + XXEND;
        for (int i = lrows + 1; i <= urows; i++) {
          corpus[i] = corpus[i - 1] + cols;
...
#define AST_TEMPLATE(utype) template xmatrix<utype>& xmatrix<utype>::operator=(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_BOOL)
#undef AST_TEMPLATE
...
namespace aurostd { // namespace aurostd
  template <class utype> void xmatrix<utype>::refresh() { // CO20190808
  ...
  template <class utype> void xmatrix<utype>::shift(int new_lrows, int new_lcols) {
    if (new_lrows == lrows && new_lcols == lcols) {
      return;
    }
    const int row_shift = lrows - new_lrows;
    const int col_shift = lcols - new_lcols;
    corpus += row_shift;
    lrows = new_lrows;
    lcols = new_lcols;
  ...
  /// \param new_lrows new lower bound for rows
  /// @note modifies the pointers but not the underlying data field
  template <class utype> void xmatrix<utype>::shiftrow(int new_lrows) {
  ...
  /// \param new_lcols new lower bound for columns
  /// @note modifies the pointers but not the underlying data field
  template <class utype> void xmatrix<utype>::shiftcol(int new_lcols) {
  ...
  /// @note modifies the pointers but not the underlying data field
  template <class utype> void xmatrix<utype>::reshape(uint new_rows, uint new_cols) {
    ...
    const int new_urows = lrows + new_rows - 1;
    const int new_ucols = lcols + new_cols - 1;
  ...
  /// @mod{HE,20220915,created function}
  /// @note modifies the pointers but not the underlying data field
  template <class utype> void xmatrix<utype>::reshape(int new_urows, int new_ucols, int new_lrows, int new_lcols) {
    const int new_rows = new_urows - new_lrows + 1;
    const int new_cols = new_ucols - new_lcols + 1;
    if (new_rows < 1 || new_cols < 1) {
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "New dimensions cannot be less than one", _VALUE_ILLEGAL_);
    }
    ...
    if ((lrows == new_lrows) && (lcols == new_lcols) && (urows == new_urows) && (ucols == new_ucols)) {
      return;
    }
...
#define AST_TEMPLATE(utype) template xvector<utype> xmatrix<utype>::operator()(int) const;
  AST_GEN_1(AST_UTYPE_BOOL)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xvector<utype> xmatrix<utype>::getcol(int) const;
  AST_GEN_1(AST_UTYPE_BOOL)
#undef AST_TEMPLATE
} // namespace aurostd

namespace aurostd { // namespace aurostd
  template <class utype> xvector<utype> xmatrix<utype>::getdiag(int k, int _lrows) const { // CO20191210
    // first get length
    int _rows = 0;
    int i = 0;
    int j = 0;
...
namespace aurostd { // namespace aurostd
  template <class utype> void xmatrix<utype>::getxvecInPlace(xvector<utype>& xv_out, int lrow, int urow, int lcol, int ucol, int lrows_out) const {
    ...
    if (lrow < lrows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "lrow<lrows", _INDEX_BOUNDS_);
    }
    if (urow > urows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "urow>urows", _INDEX_BOUNDS_);
    }
    if (lcol < lcols) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "lcol<lcols", _INDEX_BOUNDS_);
    }
    if (ucol > ucols) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "ucol>ucols", _INDEX_BOUNDS_);
    }
    if (lcol > ucol) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "lcol>ucol", _INDEX_BOUNDS_);
    }
    if (lrow > urow) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "lrow>urow", _INDEX_BOUNDS_);
    }
    ...
    const int size_out = (ucol - lcol + 1) * (urow - lrow + 1);
    const int urows_out = size_out + lrows_out - 1;
    if (!(xv_out.rows == size_out && xv_out.lrows == lrows_out)) { // check if necessary to create new object
      const xvector<utype> xv(urows_out, lrows_out);
      xv_out = xv;
    }
...
namespace aurostd { // namespace aurostd
  ...
  /// @xlink{aurostd::getxmatInPlace()}
  template <class utype> xvector<utype> xmatrix<utype>::getxvec(int lrow, int urow, int lcol, int ucol, int lrows_out) const {
...
namespace aurostd { // namespace aurostd
  ...
  /// @xlink{aurostd::getxvecInPlace()}
  /// @xlink{aurostd::getxmatInPlace()}
  template <class utype> xvector<utype> xmatrix<utype>::getxvec() const {
...
namespace aurostd { // namespace aurostd
  ...
  void xmatrix<utype>::getxmatInPlace(xmatrix<utype>& mat_out, int lrow, int urow, int lcol, int ucol, int lrows_out, int lcols_out) const { // lrow, lcol references corpus, lrows_out references output //CO20191110
    // AZ20220627 START
    if (lrow < lrows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "lrow<lrows", _INDEX_BOUNDS_);
    }
    ...
    // AZ20220627 END
    const int rows_out = (urow - lrow) + 1;
    const int cols_out = (ucol - lcol) + 1;
    ...
    if (!(mat_out.rows == rows_out && mat_out.cols == cols_out && mat_out.lrows == lrows_out && mat_out.lcols == lcols_out)) { // check if necessary to create new object
      const xmatrix<utype> mat(rows_out, cols_out, lrows_out, lcols_out);
      mat_out = mat;
    }
    for (int i = lrow; i <= urow; i++) {
      for (int j = lcol; j <= ucol; j++) {
        mat_out[(i - lrow) + mat_out.lrows][(j - lcol) + mat_out.lcols] = corpus[i][j];
      }
    }
  }

  template <class utype>
  xmatrix<utype> xmatrix<utype>::getxmat(int lrow, int urow, int lcol, int ucol, int lrows_out, int lcols_out) const { // lrow, lcol references corpus, lrows_out references output  //CO20191110
    xmatrix<utype> xmat;
...
// CO20190808
namespace aurostd { // namespace aurostd
  template <class utype> void xmatrix<utype>::setmat(const xmatrix<utype>& mat, int lrow, int lcol) { // these are the starting lrow, lcol, end is dictated by size of mat //CO20191110
#ifdef _XMATRIX_CHECK_BOUNDARIES_
    bool const LDEBUG = (false || XHOST.DEBUG);
    const int urow = lrow + mat.rows - 1; // ending row
    const int ucol = lcol + mat.cols - 1; // ending col
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " urow=" << urow << endl;
      cerr << __AFLOW_FUNC__ << " ucol=" << ucol << endl;
    }
    if (lrow < lrows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "lrow<lrows", _VALUE_ILLEGAL_);
    }
    if (urow > urows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "urow>urows", _VALUE_ILLEGAL_);
    }
    if (lcol < lcols) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "lcol<lcols", _VALUE_ILLEGAL_);
    }
    if (ucol > ucols) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "ucol>ucols", _VALUE_ILLEGAL_);
    }
#endif
    for (int i = mat.lrows; i <= mat.urows; i++) {
      for (int j = mat.lcols; j <= mat.ucols; j++) {
        corpus[lrow + i - mat.lrows][lcol + j - mat.lcols] = mat[i][j];
      }
    }
  }

  template <class utype> void xmatrix<utype>::setmat(const xvector<utype>& xv, int icol, bool col) { // replace icol (col==true) or row (col==false) //CO20191110
    int lrow = 1;
    int lcol = 1;
    if (col == true) {
...
#endif
    if (col == true) {
      for (int i = xv.lrows; i <= xv.urows; i++) {
        corpus[lrow + i - xv.lrows][icol] = xv[i];
      }
    } else {
      for (int i = xv.lrows; i <= xv.urows; i++) {
        corpus[icol][lcol + i - xv.lrows] = xv[i];
      }
    }
  }
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  template <class utype> xmatrix<utype>& xmatrix<utype>::operator+=(const xmatrix<utype>& r) {
#ifdef _XMATH_DEBUG_OPERATORS
    printf("M -> operator +=: ");
    ...
    if (this->rows != r.rows || this->cols != r.cols) {
      const string message = "(this->rows!=r.rows||this->cols!=r.cols)";
  ...
  template <class utype> xmatrix<utype>& xmatrix<utype>::operator+=(const std::initializer_list<std::initializer_list<utype>> ll) { // HE20220616
    const int ll_rows = ll.size();
    const int ll_cols = ll.begin()->size();
    if (this->rows != ll_rows || this->cols != ll_cols) {
      const string message = "shape miss-match";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_MISMATCH_);
    }
    ...
    for (il2i l = ll.begin(); l < ll.end(); l++) {
      if (ll_cols != (int) l->size()) {
        const string message = "column size size mismatch ";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_MISMATCH_);
      }
      for (ili entry = l->begin(); entry < l->end(); entry++) {
...
// -------------------------------------------------- operator xmatrix *= xmatrix
namespace aurostd { // namespace aurostd
  template <class utype> xmatrix<utype>& xmatrix<utype>::operator*=(const xmatrix<utype>& b) {
#ifdef _XMATH_DEBUG_OPERATORS
    printf("M -> operator *=: ");
    printf("this->lrows=%i, this->urows=%i, ", this->lrows, this->urows);
    printf("this->lcols=%i, this->ucols=%i\n", this->lcols, this->ucols);
...
namespace aurostd { // namespace aurostd
  template <class utype> xmatrix<utype>& xmatrix<utype>::operator*=(utype r) { // CO20191110
#ifdef _XMATH_DEBUG_OPERATORS
    printf("M -> operator *=: ");
    printf("this->lrows=%i, this->urows=%i, ", this->lrows, this->urows);
    printf("this->lcols=%i, this->ucols=%i\n", this->lcols, this->ucols);
...
namespace aurostd { // namespace aurostd
  ...
  template <class utype> xmatrix<utype>& xmatrix<utype>::operator/=(const xmatrix<utype>& a) { // CO20191201 - right matrix division
#ifdef _XMATH_DEBUG_OPERATORS
    printf("M -> operator *=: ");
    printf("this->lrows=%i, this->urows=%i, ", this->lrows, this->urows);
    printf("this->lcols=%i, this->ucols=%i\n", this->lcols, this->ucols);
...
#define AST_TEMPLATE(utype) template xmatrix<utype> operator+(const xmatrix<utype>&);
...
// --------------------------------------------------- operator xmatrix + xmatrix
namespace aurostd { // namespace aurostd
  template <class utype> xmatrix<utype> operator+(const xmatrix<utype>& a, const xmatrix<utype>& b) {
#ifdef _XMATH_DEBUG_OPERATORS
    ...
    int lc = 1;
    int uc = 1;
    if ((a.lrows == b.lrows) && (a.lcols == b.lcols)) {
      lr = a.lrows;
      ur = a.urows;
      lc = a.lcols;
      uc = a.ucols;
...
#define AST_TEMPLATE(utype) template xmatrix<utype> operator+(const xmatrix<utype>&, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
namespace aurostd { // namespace aurostd
  template <class utype> xmatrix<utype> operator*(const xmatrix<utype>& a, const xmatrix<utype>& b) {
    ...
    for (i = c.lrows, ii = a.lrows; i <= c.urows; i++, ii++) {
      ...
      ai = a[ii];
      // for(k=a.lcols,kk=b.lcols;k<=a.ucols;k++,kk++)
      for (k = a.lcols, kk = b.lrows; k <= a.ucols; k++, kk++) { // CO20200106 - patching for auto-indenting
        bk = b[kk];
        aik = ai[k];
...
#undef AST_TEMPLATE
  ...
  template <class utype> xmatrix<xcomplex<utype>> operator*(const xmatrix<utype>& a, const xmatrix<xcomplex<utype>>& b) {
#ifdef _XMATH_DEBUG_OPERATORS
    printf("M -> operator *: a.lrows=%i, a.urows=%i, a.lcols=%i, a.ucols=%i\n", a.lrows, a.urows, a.lcols, a.ucols);
    printf("M -> operator *: b.lrows=%i, b.urows=%i, b.lcols=%i, b.ucols=%i\n", b.lrows, b.urows, b.lcols, b.ucols);
#endif
...
#define AST_TEMPLATE(utype) template xmatrix<xcomplex<utype>> operator*(const xmatrix<utype>&, const xmatrix<xcomplex<utype>>&);
...
#define AST_TEMPLATE(utype) template xvector<utype> operator*(const xmatrix<utype>&, const xvector<utype>&);
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xvector<utype> operator*(const xvector<utype>&, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd {
  template <class utype> xvector<xcomplex<utype>> operator*(const xmatrix<utype>& a, const xvector<xcomplex<utype>>& b) {
    if (a.cols != b.rows) {
      stringstream message;
      message << "xmatrix * xvector: Matrix and vector have different dimensions.";
      message << " a.cols = " << a.cols << ", b.rows = " << b.rows;
...
#define AST_TEMPLATE(utype) template xvector<xcomplex<utype>> operator*(const xmatrix<utype>&, const xvector<xcomplex<utype>>&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> operator+(const utype, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> operator+(const xmatrix<utype>&, const utype);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<xcomplex<utype>> operator*(const utype, const xmatrix<xcomplex<utype>>&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE

  template <class utype> xmatrix<xcomplex<utype>> operator*(const xmatrix<xcomplex<utype>>& a, utype s) {
    return s * a;
  }
#define AST_TEMPLATE(utype) template xmatrix<xcomplex<utype>> operator*(const xmatrix<xcomplex<utype>>&, const utype);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
...
#undef AST_TEMPLATE
  ...
  template <class utype>
  xmatrix<utype> // operator xmatrix / xmatrix
...
#define AST_TEMPLATE(utype) template bool __identical(const xmatrix<utype>&, const xmatrix<utype>&, const utype&, const char&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype>
  bool // is xmatrix == xmatrix ?
  identical(const xmatrix<utype>& a, const xmatrix<utype>& b, const double& _tol_, const char& _mode_) {
    // ME20190814 BEGIN
    if ((a.rows != b.rows) || (a.cols != b.cols)) {
      return false;
    }
    ...
    if (a.isfloat) {
      if (_mode_ == 1) { // relative tolerance
        for (int i = a.lrows, ii = b.lrows; i <= a.urows; i++, ii++) {
          for (int j = a.lcols, jj = b.lcols; j <= a.ucols; j++, jj++) {
            if ((abs(a[i][j] - b[ii][jj]) / (abs(a[i][j]) / 2.0 + abs(b[ii][jj]) / 2.0 + _tol_)) >= _tol_) {
              return false;
            }
          }
        }
...
#define AST_TEMPLATE(utype) template bool identical(const xmatrix<utype>&, const xmatrix<utype>&, const double&, const char&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix == xmatrix ?
  identical(const xmatrix<utype>& a, const xmatrix<utype>& b, const double& _tol_) {
    return (bool) identical(a, b, _tol_, (char) 0); // relative
  }
#define AST_TEMPLATE(utype) template bool identical(const xmatrix<utype>&, const xmatrix<utype>&, const double&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix == xmatrix ?
  rel_identical(const xmatrix<utype>& a, const xmatrix<utype>& b, const double& _tol_) {
    return (bool) identical(a, b, _tol_, (char) 1); // relative
  }
#define AST_TEMPLATE(utype) template bool rel_identical(const xmatrix<utype>&, const xmatrix<utype>&, const double&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix == xmatrix ?
  abs_identical(const xmatrix<utype>& a, const xmatrix<utype>& b, const double& _tol_) {
    return (bool) identical(a, b, _tol_, (char) 0); // absolute
  }
#define AST_TEMPLATE(utype) template bool abs_identical(const xmatrix<utype>&, const xmatrix<utype>&, const double&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  identical(const xmatrix<utype>& a, const xmatrix<utype>& b) {
    return (bool) identical(a, b, (double) _AUROSTD_XMATRIX_TOLERANCE_IDENTITY_, (char) 0);
  }
#define AST_TEMPLATE(utype) template bool identical(const xmatrix<utype>&, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template bool operator==(const xmatrix<utype>&, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix != xmatrix ?
  isdifferent(const xmatrix<utype>& a, const xmatrix<utype>& b, const double& _tol_) {
    return (bool) !identical(a, b, _tol_, (char) 0);
  }
#define AST_TEMPLATE(utype) template bool isdifferent(const xmatrix<utype>&, const xmatrix<utype>&, const double&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template bool isdifferent(const xmatrix<utype>&, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix == xmatrix ?
  isequal(const xmatrix<utype>& a, const xmatrix<utype>& b, const double& _tol_) {
    return (bool) identical(a, b, _tol_, (char) 0);
  }
#define AST_TEMPLATE(utype) template bool isequal(const xmatrix<utype>&, const xmatrix<utype>&, const double&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template bool isequal(const xmatrix<utype>&, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#undef AST_TEMPLATE
  ...
  //  namespace aurostd
  template <class utype> bool identical(const xmatrix<xcomplex<utype>>& a, const xmatrix<xcomplex<utype>>& b, const double& _tol_, const char& _mode_) {
    if ((a.rows != b.rows) || (a.cols != b.cols)) {
      return false;
    }
    if (_mode_ == 1) { // relative tolerance
      for (int i = a.lrows, ii = b.lrows; i <= a.urows; i++, ii++) {
        for (int j = a.lcols, jj = b.lcols; i <= a.ucols; j++, jj++) {
          if ((abs(a[i][j].re - b[ii][jj].re) / abs(a[i][j].re / 2.0 + abs(b[ii][jj].re) / 2.0 + _tol_)) >= _tol_) {
            return false;
          }
          if ((abs(a[i][j].im - b[ii][jj].im) / abs(a[i][j].im / 2.0 + abs(b[ii][jj].im) / 2.0 + _tol_)) >= _tol_) {
            return false;
          }
        }
      }
      ...
      for (int i = a.lrows, ii = b.lrows; i <= a.urows; i++, ii++) {
        for (int j = a.lcols, jj = b.lcols; j <= a.ucols; j++, jj++) {
          if (isdifferent(a, b, _tol_)) {
            return false;
          }
        }
      }
    } else { // unknown mode
      const string message = "Unknown mode " + utype2string<char>(_mode_) + ".";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ILLEGAL_);
    }
    return true;
  }
#define AST_TEMPLATE(utype) template bool identical(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&, const double&, const char&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix == xmatrix ?
  identical(const xmatrix<xcomplex<utype>>& a, const xmatrix<xcomplex<utype>>& b, const double& _tol_) {
    return (bool) identical(a, b, _tol_, (char) 0); // relative
  }
#define AST_TEMPLATE(utype) template bool identical(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&, const double&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix == xmatrix ?
  rel_identical(const xmatrix<xcomplex<utype>>& a, const xmatrix<xcomplex<utype>>& b, const double& _tol_) {
    return (bool) identical(a, b, _tol_, (char) 1); // relative
  }
#define AST_TEMPLATE(utype) template bool rel_identical(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&, const double&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix == xmatrix ?
  abs_identical(const xmatrix<xcomplex<utype>>& a, const xmatrix<xcomplex<utype>>& b, const double& _tol_) {
    return (bool) identical(a, b, _tol_, (char) 0); // absolute
  }
#define AST_TEMPLATE(utype) template bool abs_identical(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&, const double&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template bool identical(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template bool operator==(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix != xmatrix ?
  isdifferent(const xmatrix<xcomplex<utype>>& a, const xmatrix<xcomplex<utype>>& b, const double& _tol_) {
    return (bool) !identical(a, b, _tol_, (char) 0);
  }
#define AST_TEMPLATE(utype) template bool isdifferent(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&, const double&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template bool isdifferent(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
  ...
  template <class utype>
  bool // is xmatrix == xmatrix ?
  isequal(const xmatrix<xcomplex<utype>>& a, const xmatrix<xcomplex<utype>>& b, const double& _tol_) {
    return (bool) identical(a, b, _tol_, (char) 0);
  }
#define AST_TEMPLATE(utype) template bool isequal(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&, const double&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template bool isequal(const xmatrix<xcomplex<utype>>&, const xmatrix<xcomplex<utype>>&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
...
#undef AST_TEMPLATE
  ...
  // namespace aurostd
  template <class utype> bool isinteger(const xmatrix<utype>& a, const utype& tol) {
    if (a.isfloat || a.iscomplex) {
      for (int i = a.lrows; i <= a.urows; i++) {
        for (int j = a.lcols; j <= a.ucols; j++) {
          if (isinteger(a[i][j], tol) == false) {
            return false;
          }
        }
      }
...
#define AST_TEMPLATE(utype)                                     \
  template bool isinteger(const xmatrix<utype>&, const utype&); \
  ;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  // namespace aurostd
  // CO START
  template <class utype> bool isidentity(const xmatrix<utype>& a) {
    // ME20200725 - changed exit to return false
    if (a.rows != a.cols) {
      return false;
    }
    for (int i = a.lrows; i <= a.urows; i++) {
      for (int j = a.lcols; j <= a.ucols; j++) {
        if (i - a.lrows + 1 != j - a.lcols + 1) { // i != j
          if (aurostd::abs(a[i][j]) > _AUROSTD_XMATRIX_TOLERANCE_IDENTITY_) {
            return false;
          }
        } else {
          if (aurostd::abs(1.0 - a[i][j]) > _AUROSTD_XMATRIX_TOLERANCE_IDENTITY_) {
            return false;
          }
        }
      }
...
#define AST_TEMPLATE(utype) template bool isidentity(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  // CO START

  // namespace aurostd
  template <class utype> bool isdiagonal(const xmatrix<utype>& a, const utype& _eps_) { // DX20171025
    for (int i = a.lrows; i <= a.urows; i++) {
      for (int j = a.lcols; j <= a.ucols; j++) {
        if (i - a.lrows + 1 != j - a.lcols + 1) { // i != j
          if (aurostd::abs(a[i][j]) > _eps_) {
            return false;
          }
        }
      }
...
#define AST_TEMPLATE(utype) template bool isdiagonal(const xmatrix<utype>&, const utype&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  // namespace aurostd
  template <class utype> bool issymmetric(const xmatrix<utype>& a) {
    // ME20200725 - changed exit to return false
    if (a.rows != a.cols) {
      return false;
    }
    for (int i = a.lrows; i <= a.urows; i++) {
      for (int j = a.lcols; j <= a.ucols; j++) {
        if (aurostd::abs(a[i][j] - a[j][i]) > _AUROSTD_XMATRIX_TOLERANCE_IDENTITY_) {
          return false;
        }
      }
    }
...
#define AST_TEMPLATE(utype) template bool issymmetric(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  // namespace aurostd
  template <class utype> bool isantisymmetric(const xmatrix<utype>& a) {
    // ME20200725 - changed exit to return false
    if (a.rows != a.cols) {
      return false;
    }
...
#define AST_TEMPLATE(utype) template bool isantisymmetric(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  // namespace aurostd
  template <class utype> bool ishermitian(const xmatrix<utype>& a) {
    // ME20200725 - changed exit to return false
    if (a.rows != a.cols) {
      return false;
    }
    for (int i = a.lrows; i <= a.urows; i++) {
      for (int j = a.lcols; j <= a.ucols; j++) {
        if (aurostd::abs(a[i][j] - aurostd::conj(a[j][i])) > _AUROSTD_XMATRIX_TOLERANCE_IDENTITY_) {
          return false;
        }
      }
    }
...
#define AST_TEMPLATE(utype) template bool ishermitian(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  // namespace aurostd
  template <class utype> bool isantihermitian(const xmatrix<utype>& a) {
    // ME20200725 - changed exit to return false
    if (a.rows != a.cols) {
      return false;
    }
...
#define AST_TEMPLATE(utype) template bool isantihermitian(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
namespace aurostd {
  ...
  template <class utype> xmatrix<utype> reshape(const xvector<utype>& vec, int rows, int cols) {
    xmatrix<utype> mat = reshape(vec);
    mat.reshape(rows, cols);
    return mat;
  }
#define AST_TEMPLATE(utype) template xmatrix<utype> reshape(const xvector<utype>&, int, int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template xmatrix<utype> reshape(const xvector<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  /// @mod{ME,20190617,created function}
  /// @mod{HE,20220915,using general stacking function}
  template <class utype> xmatrix<utype> reshape_rows(const xvector<utype>& vec) {
    xmatrix<utype> mat = reshape(vec);
    mat.reshape(1);
    return mat;
  }
#define AST_TEMPLATE(utype) template xmatrix<utype> reshape_rows(const xvector<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  /// // 4.5 4.5 4.5
  /// @endcode
  template <class utype> xmatrix<utype> vstack(const vector<xvector<utype>>& vec_list) {
    if (vec_list.empty()) {
      return xmatrix<utype>();
    }
    const int rows = vec_list[0].rows;
    const int cols = (int) vec_list.size();
    for (const xvector<utype>& vec : vec_list) {
      if (rows != vec.rows) {
        stringstream message;
...
#define AST_TEMPLATE(utype) template xmatrix<utype> vstack(const vector<xvector<utype>>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  /// // 1.5 3.0 4.5
  /// @endcode
  template <class utype> xmatrix<utype> hstack(const vector<xvector<utype>>& vec_list) {
    if (vec_list.empty()) {
      return xmatrix<utype>();
    }
    const int cols = vec_list[0].rows;
    const int rows = (int) vec_list.size();
    for (const xvector<utype>& vec : vec_list) {
      if (cols != vec.rows) {
        stringstream message;
        message << "vectors must have the same dimensions " << rows << " != " << vec.rows;
...
#define AST_TEMPLATE(utype) template xmatrix<utype> hstack(const vector<xvector<utype>>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  ...
  template <class utype> xmatrix<utype> eye(int nrh, int nch, int nrl, int ncl) __xprototype { // CO20190520
...
#define AST_TEMPLATE(utype) template xmatrix<utype> eye(int, int, int, int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template xmatrix<utype> ones_xm(int, int, int, int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template xmatrix<long double> xlongdouble(const xmatrix<utype>&) __xprototype;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<double> xdouble(const xmatrix<utype>&) __xprototype;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<float> xfloat(const xmatrix<utype>&) __xprototype;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<long int> xlongint(const xmatrix<utype>&) __xprototype;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<int> xint(const xmatrix<utype>&) __xprototype;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<char> xchar(const xmatrix<utype>&) __xprototype;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd

namespace aurostd { // conversion to vector<vector<utype> >
  template <class utype> vector<vector<utype>> xmatrix2vectorvector(const xmatrix<utype>& xmat) __xprototype {
    const int isize = xmat.rows;
    const int jsize = xmat.cols;
    // vecconst tor<vector<utype> > mat; vector<utype> v; mat=vector<vector<utype> > (m,v); // by hand
...
#define AST_TEMPLATE(utype) template vector<vector<utype>> xmatrix2vectorvector(const xmatrix<utype>&) __xprototype;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> vectorvector2xmatrix(const vector<vector<utype>>&, int, int) __xprototype;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template xmatrix<utype> vectorvector2xmatrix(const vector<vector<string>>&, int, int) __xprototype;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<double> xmatrixutype2double(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
  ...
  template <class utype> xmatrix<utype> xmatrixdouble2utype(const xmatrix<double>& a, bool check_int) { // CO20191201
    ...
    if (check_int) {
      for (i = a.lrows; i <= a.urows; i++) {
        for (j = a.lcols; j <= a.ucols; j++) {
          if (!isinteger(a[i][j])) {
            throw aurostd::xerror(__AFLOW_FILE__, "aurostd::xmatrixdouble2utype():", "non-integer found [" + aurostd::utype2string(a[i][j]) + "]", _INPUT_ILLEGAL_);
          }
        }
      }
    }
...
#define AST_TEMPLATE(utype) template xmatrix<utype> xmatrixdouble2utype(const xmatrix<double>&, bool);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template void reset(xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template void xmatrix<utype>::clear(void);
  AST_GEN_1(AST_UTYPE_BOOL)
#undef AST_TEMPLATE
  template <class utype> // function clear xmatrix<>
...
#define AST_TEMPLATE(utype) template void clear(xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template void set(xmatrix<utype>&, const utype&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
namespace aurostd {
  // ensure the compiler enticipates all types the xmatrix class should work with (avoids linker issues)
  // should also be initiaized with xcomplex, but not all xmatrix functions can be run with it
  // for now a subset of functions are specificaly initialized for xcomplex
#define AST_TEMPLATE(utype) template class xmatrix<utype>;
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  ...
  utype det(const xmatrix<utype>& a) {
    ...
    if (!a.issquare) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "a must be square", _INPUT_ILLEGAL_);
    }
    ...
    if (size == 2) {
      return (utype) a[1][1] * a[2][2] - a[1][2] * a[2][1];
    }
    if (size == 3) {
      return (utype) (a[1][1] * a[2][2] * a[3][3] + a[1][2] * a[2][3] * a[3][1] + a[1][3] * a[2][1] * a[3][2] - a[1][3] * a[2][2] * a[3][1] - a[1][2] * a[2][1] * a[3][3] - a[1][1] * a[2][3] * a[3][2]);
    }
    if (size == 4) {
...
#define AST_TEMPLATE(utype) template utype det(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template utype determinant(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
// CO20180515
namespace aurostd { // namespace aurostd
  template <class utype> utype CMdet(const xmatrix<utype>& B) { // Cayley-Menger Determinant for simplex content
    const bool LDEBUG = (false || XHOST.DEBUG);
    if (!B.issquare) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " B_hat=" << endl;
      cerr << B_hat << endl;
      cerr << __AFLOW_FUNC__ << " det(B_hat)=" << detB_hat << endl;
    }
    return detB_hat;
  }
#define AST_TEMPLATE(utype) template utype CMdet(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
// CO20190324
namespace aurostd { // namespace aurostd
  template <class utype> xmatrix<utype> getRotationMatrix3D(const xvector<utype>& a, const xvector<utype>& b) {
    const bool LDEBUG = (false || XHOST.DEBUG);
    // tests of stupidity
    if (a.rows != 3) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "a.rows!=3", _INPUT_ILLEGAL_);
    }
    if (b.rows != 3) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "b.rows!=3", _INPUT_ILLEGAL_);
    }
    if (LDEBUG) {
    ...
    // trivial cases
    if (aurostd::isequal(a, b, (utype) _AUROSTD_ZERO_TOL_)) {
      return R;
    }
    if (aurostd::isequal(a, -b, (utype) _AUROSTD_ZERO_TOL_)) {
      R(1, 1) = R(2, 2) = R(3, 3) = -1;
    ...
    vx[2][3] = -v[1];
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vx=" << endl;
      cerr << vx << endl;
    }
    if (!aurostd::isequal(s, (utype) 0, (utype) _AUROSTD_ZERO_TOL_)) {
      R += vx + vx * vx * ((utype) 1 - c) / (s * s);
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " R=" << endl;
      cerr << R << endl;
    }
    return R;
  }
#define AST_TEMPLATE(utype) template xmatrix<utype> getRotationMatrix3D(const xvector<utype>&, const xvector<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
namespace aurostd { // namespace aurostd
  template <class utype>
  void // minor submatrix - IN PLACE (faster for big routines)
  submatrixInPlace(const xmatrix<utype>& a, xmatrix<utype>& b, int irow, int jcol) {
    if (irow >= a.lrows && irow <= a.urows && jcol >= a.lcols && jcol <= a.ucols) {
      if (b.lrows != a.lrows || b.lcols != a.lcols || b.urows != a.urows - 1 || b.ucols != b.ucols - 1) { // CO20191201
        const xmatrix<utype> c(a.urows - 1, a.ucols - 1, a.lrows, a.lcols);
        b = c;
      }
      for (int i = a.lrows; i <= a.urows; i++) {
        for (int j = a.lcols; j <= a.ucols; j++) {
    ...
    if ((irow < a.lrows || irow > a.urows) && jcol >= a.lcols && jcol <= a.ucols) {
      if (b.lrows != a.lrows || b.lcols != a.lcols || b.urows != a.urows || b.ucols != b.ucols - 1) { // CO20191201
        const xmatrix<utype> c(a.urows, a.ucols - 1, a.lrows, a.lcols);
        b = c;
      }
      for (int i = a.lrows; i <= a.urows; i++) {
        for (int j = a.lcols; j <= a.ucols; j++) {
    ...
    if (irow >= a.lrows && irow <= a.urows && (jcol < a.lcols || jcol > a.ucols)) {
      if (b.lrows != a.lrows || b.lcols != a.lcols || b.urows != a.urows - 1 || b.ucols != b.ucols) { // CO20191201
        const xmatrix<utype> c(a.urows - 1, a.ucols, a.lrows, a.lcols);
        b = c;
      }
      for (int i = a.lrows; i <= a.urows; i++) {
        for (int j = a.lcols; j <= a.ucols; j++) {
...
#define AST_TEMPLATE(utype) template void submatrixInPlace(const xmatrix<utype>&, xmatrix<utype>&, int, int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype>
  xmatrix<utype> // minor submatrix - IN PLACE
...
#define AST_TEMPLATE(utype) template xmatrix<utype> submatrix(const xmatrix<utype>&, int, int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
namespace aurostd { // namespace aurostd
  template <class utype>
  utype // minor submatrix
  minordet(const xmatrix<utype>& a, const int& irow, const int& jcol) {
    return det(submatrix(a, irow, jcol));
  }
#define AST_TEMPLATE(utype) template utype minordet(const xmatrix<utype>&, const int&, const int&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template utype minordeterminant(const xmatrix<utype>&, const int&, const int&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
namespace aurostd { // namespace aurostd
  ...
  void adjointInPlace(const xmatrix<utype>& a, xmatrix<utype>& b) { // CO20191201
    ...
    if (!a.issquare) {
      throw aurostd::xerror(__AFLOW_FILE__, "aurostd::inverseByAdjoint()", "a must be square", _INPUT_ILLEGAL_);
    }
    b = a;
...
#define AST_TEMPLATE(utype) template void adjointInPlace(const xmatrix<utype>&, xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template xmatrix<utype> adjoint(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> // function inverse xmatrix<>
  xmatrix<utype> inverseByAdjoint(const xmatrix<utype>& a) {
    return (utype) 1.0 / det(a) * adjoint(a);
  } // CO20191201
#define AST_TEMPLATE(utype) template xmatrix<utype> inverseByAdjoint(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  xmatrix<utype> inverse(const xmatrix<utype>& a) {
    ...
    utype adet = det(a);
    if (adet == (utype) 0) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "singular matrix", _INPUT_ILLEGAL_);
    }
    if (size == 1) {
      b[1][1] = (utype) 1 / a[1][1];
      return b;
    ...
    if (size == 3) {
      b(1, 1) = (+a(2, 2) * a(3, 3) - a(2, 3) * a(3, 2)) / adet;
      b(1, 2) = (-a(1, 2) * a(3, 3) + a(1, 3) * a(3, 2)) / adet;
      b(1, 3) = (+a(1, 2) * a(2, 3) - a(1, 3) * a(2, 2)) / adet;
      b(2, 1) = (-a(2, 1) * a(3, 3) + a(2, 3) * a(3, 1)) / adet;
      b(2, 2) = (+a(1, 1) * a(3, 3) - a(1, 3) * a(3, 1)) / adet;
      b(2, 3) = (-a(1, 1) * a(2, 3) + a(1, 3) * a(2, 1)) / adet;
      b(3, 1) = (+a(2, 1) * a(3, 2) - a(2, 2) * a(3, 1)) / adet;
      b(3, 2) = (-a(1, 1) * a(3, 2) + a(1, 2) * a(3, 1)) / adet;
      b(3, 3) = (+a(1, 1) * a(2, 2) - a(1, 2) * a(2, 1)) / adet;
      return b;
    }
    if (size == 4) {
      b(1, 1) = (+a(2, 2) * (a(3, 3) * a(4, 4) - a(3, 4) * a(4, 3)) - a(2, 3) * (a(3, 2) * a(4, 4) - a(3, 4) * a(4, 2)) + a(2, 4) * (a(3, 2) * a(4, 3) - a(3, 3) * a(4, 2))) / adet;
      b(2, 1) = (-a(2, 1) * (a(3, 3) * a(4, 4) - a(3, 4) * a(4, 3)) + a(2, 3) * (a(3, 1) * a(4, 4) - a(3, 4) * a(4, 1)) - a(2, 4) * (a(3, 1) * a(4, 3) - a(3, 3) * a(4, 1))) / adet;
      b(3, 1) = (+a(2, 1) * (a(3, 2) * a(4, 4) - a(3, 4) * a(4, 2)) - a(2, 2) * (a(3, 1) * a(4, 4) - a(3, 4) * a(4, 1)) + a(2, 4) * (a(3, 1) * a(4, 2) - a(3, 2) * a(4, 1))) / adet;
      b(4, 1) = (-a(2, 1) * (a(3, 2) * a(4, 3) - a(3, 3) * a(4, 2)) + a(2, 2) * (a(3, 1) * a(4, 3) - a(3, 3) * a(4, 1)) - a(2, 3) * (a(3, 1) * a(4, 2) - a(3, 2) * a(4, 1))) / adet;
      b(1, 2) = (-a(1, 2) * (a(3, 3) * a(4, 4) - a(3, 4) * a(4, 3)) + a(1, 3) * (a(3, 2) * a(4, 4) - a(3, 4) * a(4, 2)) - a(1, 4) * (a(3, 2) * a(4, 3) - a(3, 3) * a(4, 2))) / adet;
      b(2, 2) = (+a(1, 1) * (a(3, 3) * a(4, 4) - a(3, 4) * a(4, 3)) - a(1, 3) * (a(3, 1) * a(4, 4) - a(3, 4) * a(4, 1)) + a(1, 4) * (a(3, 1) * a(4, 3) - a(3, 3) * a(4, 1))) / adet;
      b(3, 2) = (-a(1, 1) * (a(3, 2) * a(4, 4) - a(3, 4) * a(4, 2)) + a(1, 2) * (a(3, 1) * a(4, 4) - a(3, 4) * a(4, 1)) - a(1, 4) * (a(3, 1) * a(4, 2) - a(3, 2) * a(4, 1))) / adet;
      b(4, 2) = (+a(1, 1) * (a(3, 2) * a(4, 3) - a(3, 3) * a(4, 2)) - a(1, 2) * (a(3, 1) * a(4, 3) - a(3, 3) * a(4, 1)) + a(1, 3) * (a(3, 1) * a(4, 2) - a(3, 2) * a(4, 1))) / adet;
      b(1, 3) = (+a(1, 2) * (a(2, 3) * a(4, 4) - a(2, 4) * a(4, 3)) - a(1, 3) * (a(2, 2) * a(4, 4) - a(2, 4) * a(4, 2)) + a(1, 4) * (a(2, 2) * a(4, 3) - a(2, 3) * a(4, 2))) / adet;
      b(2, 3) = (-a(1, 1) * (a(2, 3) * a(4, 4) - a(2, 4) * a(4, 3)) + a(1, 3) * (a(2, 1) * a(4, 4) - a(2, 4) * a(4, 1)) - a(1, 4) * (a(2, 1) * a(4, 3) - a(2, 3) * a(4, 1))) / adet;
      b(3, 3) = (+a(1, 1) * (a(2, 2) * a(4, 4) - a(2, 4) * a(4, 2)) - a(1, 2) * (a(2, 1) * a(4, 4) - a(2, 4) * a(4, 1)) + a(1, 4) * (a(2, 1) * a(4, 2) - a(2, 2) * a(4, 1))) / adet;
      b(4, 3) = (-a(1, 1) * (a(2, 2) * a(4, 3) - a(2, 3) * a(4, 2)) + a(1, 2) * (a(2, 1) * a(4, 3) - a(2, 3) * a(4, 1)) - a(1, 3) * (a(2, 1) * a(4, 2) - a(2, 2) * a(4, 1))) / adet;
      b(1, 4) = (-a(1, 2) * (a(2, 3) * a(3, 4) - a(2, 4) * a(3, 3)) + a(1, 3) * (a(2, 2) * a(3, 4) - a(2, 4) * a(3, 2)) - a(1, 4) * (a(2, 2) * a(3, 3) - a(2, 3) * a(3, 2))) / adet;
      b(2, 4) = (+a(1, 1) * (a(2, 3) * a(3, 4) - a(2, 4) * a(3, 3)) - a(1, 3) * (a(2, 1) * a(3, 4) - a(2, 4) * a(3, 1)) + a(1, 4) * (a(2, 1) * a(3, 3) - a(2, 3) * a(3, 1))) / adet;
      b(3, 4) = (-a(1, 1) * (a(2, 2) * a(3, 4) - a(2, 4) * a(3, 2)) + a(1, 2) * (a(2, 1) * a(3, 4) - a(2, 4) * a(3, 1)) - a(1, 4) * (a(2, 1) * a(3, 2) - a(2, 2) * a(3, 1))) / adet;
      b(4, 4) = (+a(1, 1) * (a(2, 2) * a(3, 3) - a(2, 3) * a(3, 2)) - a(1, 2) * (a(2, 1) * a(3, 3) - a(2, 3) * a(3, 1)) + a(1, 3) * (a(2, 1) * a(3, 2) - a(2, 2) * a(3, 1))) / adet;
      return b;
    }
    ...
    const xmatrix<utype> id = aurostd::identity(a);
...
#define AST_TEMPLATE(utype) template xmatrix<utype> inverse(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  // For example, future implementation: https://arxiv.org/pdf/2208.01239.pdf
  template <class utype> // function inverse xmatrix<>
  xmatrix<aurostd::xcomplex<utype>> inverse(const xmatrix<aurostd::xcomplex<utype>>& a) {
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Not implemented for xcomplex", _RUNTIME_ERROR_);
  }
#define AST_TEMPLATE(utype) template xmatrix<aurostd::xcomplex<utype>> inverse(const xmatrix<aurostd::xcomplex<utype>>&);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
...
// ----------------------------------------------------------------------------
namespace aurostd { // namespace aurostd
  template <class utype> bool isNonInvertible(const xmatrix<utype>& a) { // RETURN ERROR if non invertible  //CO20191201
...
#define AST_TEMPLATE(utype) template bool isNonInvertible(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> inverseByQR(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  ...
  template <class utype> void LUPDecomposition(const xmatrix<utype>& A, xmatrix<double>& LU, xmatrix<double>& P, utype tol) { // SD20220426
    if (!A.issquare) {
      const string message = "Matrix needs to be square for LU decomposition";
    ...
    for (int i = LU.lrows; i <= LU.urows; i++) {
      ...
      if (maxA < (double) tol) {
        const string message = "Matrix is degenerate";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_);
      }
      if (imax != i) {
        // pivoting P
...
#define AST_TEMPLATE(utype) template void LUPDecomposition(const xmatrix<utype>&, xmatrix<double>&, xmatrix<double>&, utype);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  /// @see
  /// @xlink{https://en.wikipedia.org/wiki/LU_decomposition}
  template <class utype> void LUPDecomposition(const xmatrix<utype>& A, xmatrix<double>& L, xmatrix<double>& U, xmatrix<double>& P, utype tol) { // SD20220426
    xmatrix<double> LU;
...
#define AST_TEMPLATE(utype) template void LUPDecomposition(const xmatrix<utype>&, xmatrix<double>&, xmatrix<double>&, xmatrix<double>&, utype);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template xmatrix<utype> inverseByLUP(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> roundoff(const xmatrix<utype>&, utype);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> roundoff(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template utype modulus(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template utype modulussquare(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template utype modulus2(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template utype sum(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xvector<utype> sum_column(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xvector<utype> mean_column(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xvector<utype> sum_row(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xvector<utype> mean_row(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template utype min(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template utype min(const xmatrix<utype>&, int&, int&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template utype max(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template utype max(const xmatrix<utype>&, int&, int&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  template <class utype>
  utype // DX20170115 - double to utype (needed for xcomplex)                                          // trace
...
#endif
    if (!a.issquare) {
      const string message = "Trace is only defined for square matrices";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
...
#define AST_TEMPLATE(utype) template utype trace(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd {
  ...
  template <class utype> xmatrix<utype> HadamardProduct(const xmatrix<utype>& A, const xmatrix<utype>& B) { // SD20220422
    if ((A.rows != B.rows) || (A.cols != B.cols)) {
      stringstream message;
...
#define AST_TEMPLATE(utype) template xmatrix<utype> HadamardProduct(const xmatrix<utype>&, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
// ----------------------------------------------------------------------------
namespace aurostd {
  template <class utype> xmatrix<utype> KroneckerProduct(const xmatrix<utype>& A, const xmatrix<utype>& B) { // ME20180614 - Kronecker product
    int rows;
    int cols;
    int r;
    int c;
...
#define AST_TEMPLATE(utype) template xmatrix<utype> KroneckerProduct(const xmatrix<utype>&, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> identity(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
// ME20200123
namespace aurostd {
  template <class utype> xmatrix<utype> identity(const utype& _type, int ubounds, int lbounds) {
    return eye<utype>(ubounds, ubounds, lbounds, lbounds);
    if (_type) {
      ;
    } // To suppress compiler warnings
  }
#define AST_TEMPLATE(utype) template xmatrix<utype> identity(const utype&, int, int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> conj(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
// ----------------------------------------------------------------------------
namespace aurostd { // namespace aurostd
  template <class utype> void traspSquareInPlace(xmatrix<utype>& a, bool conjugate) { // CO20191201 - only designed for square matrices
    if (!a.issquare) {
      throw aurostd::xerror(__AFLOW_FILE__, "aurostd::traspSquareInPlace():", "this function is designed only for square matrices", _VALUE_ILLEGAL_);
    }
    int i = 0;
    int j = 0;
    utype aij = (utype) 0;
...
#define AST_TEMPLATE(utype) template void traspSquareInPlace(xmatrix<utype>&, bool);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> void traspInPlace(const xmatrix<utype>& a, xmatrix<utype>& b, bool conjugate) { // CO20191201
    ...
    if (b.lrows != a.lcols || b.lcols != a.lrows || b.urows != a.ucols || b.ucols != a.urows) {
      const xmatrix<utype> c(a.ucols, a.urows, a.lcols, a.lrows);
      b = c;
    }
    int i = 0;
    int j = 0;
    if (a.iscomplex && conjugate) { // CO20191201
      for (i = a.lrows; i <= a.urows; i++) {
        for (j = a.lcols; j <= a.ucols; j++) {
          b[j][i] = conj(a[i][j]);
        } // cannot do j=i+1 since b is unpopulated
      }
      return;
    } else {
      for (i = a.lrows; i <= a.urows; i++) {
...
#define AST_TEMPLATE(utype) template void traspInPlace(const xmatrix<utype>&, xmatrix<utype>&, bool);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#endif
    if (a.issquare) {
      traspSquareInPlace(a, conjugate);
      return;
    } // this works because we modify a[i][j] and a[j][i] at once
...
#define AST_TEMPLATE(utype) template void traspInPlace(xmatrix<utype>&, bool);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype>
  xmatrix<utype> // trasp xmatrix
  trasp(const xmatrix<utype>& a, bool conjugate) { // CO20191201
    xmatrix<utype> b(a.ucols, a.urows, a.lcols, a.lrows);
    traspInPlace(a, b, conjugate);
    return b;
  }
#define AST_TEMPLATE(utype) template xmatrix<utype> trasp(const xmatrix<utype>&, bool);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> trasp(const xvector<utype>&, bool);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  ...
  swap_cols(xmatrix<utype>& a, const int& i, const int& j) { // swap_columns
    if (i < a.lcols || i > a.ucols) {
      return; // nothing to do, out of boundaries
    }
    if (j < a.lcols || j > a.ucols) {
      return; // nothing to do, out of boundaries
    }
    if (i == j) {
      return; // nothing to do, no swap
    }
    utype temp;
...
#define AST_TEMPLATE(utype) template void swap_cols(xmatrix<utype>&, const int&, const int&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template void swap_columns(xmatrix<utype>&, const int&, const int&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  swap_rows(xmatrix<utype>& a, const int& i, const int& j) { // swap_rows
    if (i < a.lrows || i > a.urows) {
      return; // nothing to do, out of boundaries
    }
    if (j < a.lrows || j > a.urows) {
      return; // nothing to do, out of boundaries
    }
    if (i == j) {
      return; // nothing to do, no swap
...
#define AST_TEMPLATE(utype) template void swap_rows(xmatrix<utype>&, const int&, const int&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> sign(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> nint(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> floor(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  ...
  // #define AST_TEMPLATE(utype) template xmatrix<utype> trunc(const xmatrix<utype>&);
  //     AST_GEN_1(AST_UTYPE_NUM)
  // #undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> round(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> ceil(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> mabs(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template xmatrix<utype> abs(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  template <class utype>
  xmatrix<utype> // exp xmatrix
  exp_old(const xmatrix<utype>& a) {
    if (!a.issquare) {
      const string message = "exp only defined for square matrices.";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    xmatrix<utype> out(a.urows, a.ucols, a.lrows, a.lcols);
    xmatrix<utype> an(a.urows, a.ucols, a.lrows, a.lcols);
...
#define AST_TEMPLATE(utype) template xmatrix<utype> exp_old(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  ...
  exp(const xmatrix<utype>& a) {
    ...
    for (int n = 0; !convergence; n++) {
      ...
      // if(abs(trace(an)/trace(out))<_exponential_convergence)
      if (n > 30) {
        if (abs(trace(an)) < _exponential_convergence) {
          convergence = true;
        }
      }
      if (n > 100) {
...
#define AST_TEMPLATE(utype) template xmatrix<utype> exp(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE
} // namespace aurostd
  ...
  sin(const xmatrix<utype>& a) {
...
#endif
    if (!a.issquare) {
      const string message = "sin only defined for square matrices.";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    xmatrix<utype> out(a.urows, a.ucols, a.lrows, a.lcols);
    xmatrix<utype> an(a.urows, a.ucols, a.lrows, a.lcols);
    ...
    for (int n = 0; !convergence; n++) {
      if (n == 0) {
        ...
        an = (a * a * an) / utype(-1.0 * (2.0 * n + 1.0) * (2.0 * n));
      }
      out = out + an;
      //    if(abs(trace(an)/trace(out))<_exponential_convergence)
      if (n > 30) {
...
#define AST_TEMPLATE(utype) template xmatrix<utype> sin(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#endif
    if (!a.issquare) {
      const string message = "cos only defined for square matrices.";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    xmatrix<utype> out(a.urows, a.ucols, a.lrows, a.lcols);
    xmatrix<utype> an(a.urows, a.ucols, a.lrows, a.lcols);
...
#define AST_TEMPLATE(utype) template xmatrix<utype> cos(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
  ...
  sinh(const xmatrix<utype>& a) {
...
#endif
    if (!a.issquare) {
      const string message = "sinh only defined for square matrices.";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    xmatrix<utype> out(a.urows, a.ucols, a.lrows, a.lcols);
    xmatrix<utype> an(a.urows, a.ucols, a.lrows, a.lcols);
    ...
    for (int n = 0; !convergence; n++) {
      if (n == 0) {
        ...
        an = (a * a * an) / utype((2.0 * n + 1.0) * (2.0 * n));
      }
      out = out + an;
      // if(abs(trace(an)/trace(out))<_exponential_convergence)
      if (n > 30) {
...
#define AST_TEMPLATE(utype) template xmatrix<utype> sinh(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#endif
    if (!a.issquare) {
      const string message = "cosh only defined for square matrices.";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    xmatrix<utype> out(a.urows, a.ucols, a.lrows, a.lcols);
    xmatrix<utype> an(a.urows, a.ucols, a.lrows, a.lcols);
...
#define AST_TEMPLATE(utype) template xmatrix<utype> cosh(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template void GaussJordan(xmatrix<utype>&, xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // least square stuff aurostd adaptation of nrecipes    // 1 August 2014
  ...
  template <class utype> void gaussj(xmatrix<utype>& a, int n, xmatrix<utype>& b, int m) { // with indices
    ...
    int k;
    int l;
    int ll; // default definitions to avoid compilation errors
    utype big;
    utype dum;
    utype pivinv;
    utype temp;
...
#define AST_TEMPLATE(utype) template void gaussj(xmatrix<utype>&, int, xmatrix<utype>&, int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
// ----------------------------------------------------------------------------
namespace aurostd { // least square stuff aurostd adaptation of nrecipes    // 1 August 2014
  template <class utype> void lfit(xvector<utype> x, xvector<utype> y, xvector<utype> sig, xvector<utype>& a, xvector<int> ia, xmatrix<utype>& covar, utype& chisq, void (*funcs)(utype, xvector<utype>&)) {
    ...
    aurostd::xvector<utype> afunc(1, ma); // NOLINT // passed to a func pointer as ref, no const
    for (j = 1; j <= (int) ma; j++) {
      if (ia[j]) {
        mfit++;
      }
...
#define AST_TEMPLATE(utype) template void lfit(xvector<utype>, xvector<utype>, xvector<utype>, xvector<utype>&, xvector<int>, xmatrix<utype>&, utype&, void (*)(utype, xvector<utype>&));
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
namespace aurostd { // least square stuff aurostd adaptation of nrecipes    // 1 August 2014
  template <class utype> void covsrt(xmatrix<utype>& covar, xvector<int> ia, int mfit) {
    ...
    const int ma = covar.rows;
    if (covar.cols != covar.rows) {
      const string message = "covar.cols!=covar.rows";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_MISMATCH_);
    }
...
#define AST_TEMPLATE(utype) template void covsrt(xmatrix<utype>&, xvector<int>, int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  void GCD(const xmatrix<int>& ma, const xmatrix<int>& mb, xmatrix<int>& mgcd) { // CO20191201
    if (ma.rows == 0 || ma.cols == 0) {
      throw aurostd::xerror(__AFLOW_FILE__, "aurostd::GCD():", "ma.rows==0 || ma.cols==0", _INPUT_NUMBER_);
    }
    xmatrix<int> mx(ma.urows, ma.ucols, ma.lrows, ma.lcols);
    xmatrix<int> my(ma.urows, ma.ucols, ma.lrows, ma.lcols);
  ...
  void GCD(const xmatrix<int>& ma, const xmatrix<int>& mb, xmatrix<int>& mgcd, xmatrix<int>& mx, xmatrix<int>& my) { // CO20191219
    ...
    // ma vs. mb
    if (ma.lrows != mb.lrows) {
      throw aurostd::xerror(__AFLOW_FILE__, "aurostd::GCD():", "ma.lrows!=mb.lrows", _INDEX_MISMATCH_);
    }
    if (ma.urows != mb.urows) {
      throw aurostd::xerror(__AFLOW_FILE__, "aurostd::GCD():", "ma.urows!=mb.urows", _INDEX_MISMATCH_);
    }
    if (ma.lcols != mb.lcols) {
      throw aurostd::xerror(__AFLOW_FILE__, "aurostd::GCD():", "ma.lcols!=mb.lcols", _INDEX_MISMATCH_);
    }
    if (ma.ucols != mb.ucols) {
      throw aurostd::xerror(__AFLOW_FILE__, "aurostd::GCD():", "ma.ucols!=mb.ucols", _INDEX_MISMATCH_);
    }
    // ma vs. mgcd
    if (ma.lrows != mgcd.lrows || ma.urows != mgcd.urows || ma.lcols != mgcd.lcols || ma.ucols != mgcd.ucols) {
      const xmatrix<int> mgcd_tmp(ma);
      mgcd = mgcd_tmp;
    }
    // ma vs. mx
    if (ma.lrows != mx.lrows || ma.urows != mx.urows || ma.lcols != mx.lcols || ma.ucols != mx.ucols) {
      const xmatrix<int> mx_tmp(ma);
      mx = mx_tmp;
    }
    // ma vs. my
    if (ma.lrows != my.lrows || ma.urows != my.urows || ma.lcols != my.lcols || ma.ucols != my.ucols) {
      const xmatrix<int> my_tmp(ma);
      my = my_tmp;
    }
...
// DX20201125
namespace aurostd {
  template <class utype> void polarDecomposition(const xmatrix<utype>& transformation_matrix, xmatrix<utype>& rotation, xmatrix<utype>& deformation, bool check_orthogonal_rotation) {
    ...
    const uint dimension = transformation_matrix.rows;

    // ---------------------------------------------------------------------------
    // square the matrix
    const xmatrix<utype> T_squared = trasp(transformation_matrix) * transformation_matrix;

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " T^2: " << T_squared << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " diag_matrix: " << diag_matrix << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " deformation matrix (U): " << deformation << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " rotation matrix (R): " << rotation << endl;
    }

    // ---------------------------------------------------------------------------
    // verify conditions of an orthogonal matrix for rotation
    if (check_orthogonal_rotation) {
      const xmatrix<utype> identity_matrix = rotation * trasp(rotation);
      if (LDEBUG) {
        // R^T==R^-1
...
#define AST_TEMPLATE(utype) template void polarDecomposition(const xmatrix<utype>&, xmatrix<utype>&, xmatrix<utype>&, bool);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd {
  ...
  template <class utype> void equilibrateMatrix(const xmatrix<utype>& A_orig, xmatrix<utype>& A, xmatrix<utype>& R, xmatrix<utype>& C, uint niter, utype tol) {
    ...
    A = A_block;
    xmatrix<utype> D = aurostd::eye<utype>(mn);
    xmatrix<utype> D_tmp;
    utype d;
    bool flag_conv = false;
    uint iter = 0;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " A_orig=" << A_orig << endl;
    }
    while (iter < niter && flag_conv != true) {
      ...
      for (int i = 1; i <= mn; i++) {
        ...
        D = D * D_tmp;
        A = D_tmp * A * D_tmp;
        if (flag_conv && (aurostd::abs(1.0 - d) > tol)) {
          flag_conv = false;
        }
      }
      iter++;
...
#define AST_TEMPLATE(utype) template void equilibrateMatrix(const xmatrix<utype>&, xmatrix<utype>&, xmatrix<utype>&, xmatrix<utype>&, uint, utype);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd { // namespace aurostd
  ...
  std::ostream& operator<<(std::ostream& buf, const xmatrix<utype>& x) {
    ...
    // cerr << iobuf2 << endl;
    if (done == false) {
      const string message = "no data type available for user type";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
...
#define AST_TEMPLATE(utype) template std::ostream& operator<<(std::ostream&, const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template utype orthogonality_defect(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  // 3-540-22156-5
  template <class utype> bool gaussian_reduce_two_vectors(xvector<utype>& B, xvector<utype>& C, utype eps) {
    xvector<utype> temp(3);
    for (int it = 0;; it++) { // dont touch this
      int SwapCnt = 0; // GH Counter for the number of times B and C are swapped
      if (it > 100) {
        cerr << "gaussian_reduce_two_vectors failed to converge" << endl;
        return false;
      }
...
#define AST_TEMPLATE(utype) template bool gaussian_reduce_two_vectors(xvector<utype>&, xvector<utype>&, utype);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype> void reduce_A_in_ABC(xvector<utype>& A, xvector<utype>& B, xvector<utype>& C, utype eps) {
    ...
    if (aurostd::isNonInvertible(ABC)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "A,B,C vectors in reduce_A_in_ABC are co-planar", _VALUE_RANGE_);
    } // CO20191201
    ...
    i1(2) = i(2);
    i1(3) = i(3);
    dist(1) = aurostd::modulus(T - ABC * i1);
    i2(1) = i(1);
    i2(2) = i(2) + 1;
    i2(3) = i(3);
    dist(2) = aurostd::modulus(T - ABC * i2);
    i3(1) = i(1);
    i3(2) = i(2);
    i3(3) = i(3) + 1;
    dist(3) = aurostd::modulus(T - ABC * i3);
    i4(1) = i(1);
    i4(2) = i(2) + 1;
    i4(3) = i(3) + 1;
    dist(4) = aurostd::modulus(T - ABC * i4);
    idx = 0;
    idx = aurostd::mini(dist);
    ...
    if (idx == 0) {
      const string message = "Case failed in reduce_A_in_ABC";
...
#define AST_TEMPLATE(utype) template void reduce_A_in_ABC(xvector<utype>&, xvector<utype>&, xvector<utype>&, utype);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  //  number theory, 2004, vol. 3076, pp. 338-357 ISBN 3-540-22156-5
  template <class utype> utype reduce_to_shortest_basis(const xmatrix<utype>& IN, xmatrix<utype>& OUT, utype eps, bool VERBOSE) {
    xvector<utype> A(3);
    xvector<utype> B(3);
    xvector<utype> C(3);
    ...
    utype odnew;
    int ii = 0;
    const int iimax = 10000;
    // IN has const colum-vectors
    for (int i = 1; i <= 3; i++) {
    ...
    if (VERBOSE) {
      cout << "aurostd::reduce_to_shortest_basis: Before reduction, the orthogonality defect of the basis was " << odnew << endl;
    }
    bool goexit = false;
    while (goexit == false) {
      od = odnew;
    ...
    if (aurostd::isNonInvertible(OUT)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "OUT matrix is singular in reduce_to_shortest_basis", _VALUE_RANGE_);
    } // CO20191201
    ...
    if (sum(abs(check * IN - nint(check * IN))) > eps) {
      const string message = "Reduced lattice vectors in reduce_to_shortest_basis changed the original lattice";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    if (VERBOSE) {
      cout << "aurostd::reduce_to_shortest_basis: After reduction, the orthogonality defect of the basis is " << orthogonality_defect(OUT) << endl;
    }
    ...
    if (aurostd::det(OUT) < eps) {
      ...
      for (int i = 1; i <= 3; i++) {
        ...
        OUT(i, 1) = OUT(i, 2);
        OUT(i, 2) = temp;
      } // swap 1st with 2nd vector
    }
    // OUT has colum-vectors
    return orthogonality_defect(OUT);
  }
#define AST_TEMPLATE(utype) template utype reduce_to_shortest_basis(const xmatrix<utype>&, xmatrix<utype>&, utype, bool);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> xmatrix<utype> reduce_to_shortest_basis(const xmatrix<utype>& IN, utype eps, bool VERBOSE) {
    xmatrix<utype> newbasis(3, 3);
    reduce_to_shortest_basis(IN, newbasis, eps, VERBOSE);
    return newbasis;
  }
#define AST_TEMPLATE(utype) template xmatrix<utype> reduce_to_shortest_basis(const xmatrix<utype>&, utype, bool);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> xmatrix<utype> reduce_to_shortest_basis(const xmatrix<utype>& IN) {
    return reduce_to_shortest_basis(IN, (utype) _DEFAULT_EPS_BASIS_REDUCE_, false);
  }
#define AST_TEMPLATE(utype) template xmatrix<utype> reduce_to_shortest_basis(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define ROTATE(a, i, j, k, l)        \
  ...
  template <class utype> int jacobi(const xmatrix<utype>& ain, xvector<utype>& d, xmatrix<utype>& v) {
    ...
    for (i = 1; i <= 50; i++) {
      ...
      for (ip = 1; ip <= n - 1; ip++) {
        for (iq = ip + 1; iq <= n; iq++) {
          ...
          if (i > 4 && (utype) (aurostd::abs(d[ip]) + g) == (utype) aurostd::abs(d[ip]) && (utype) (aurostd::abs(d[iq]) + g) == (utype) aurostd::abs(d[iq])) {
            a[ip][iq] = 0.0;
...
#define AST_TEMPLATE(utype) template int jacobi(const xmatrix<utype>&, xvector<utype>&, xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
#undef ROTATE
  ...
  template <class utype> xvector<utype> jacobiHermitian(xmatrix<xcomplex<utype>>& A, char _sort_) {
    xmatrix<xcomplex<utype>> U(A.rows, A.cols);
    return jacobiHermitian(A, U, _sort_);
  }
#define AST_TEMPLATE(utype) template xvector<utype> jacobiHermitian(xmatrix<xcomplex<utype>>&, char);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE

  template <class utype> xvector<utype> jacobiHermitian(xmatrix<xcomplex<utype>>& A, xmatrix<xcomplex<utype>>& U, char _sort_) {
    // Matrices have to be square
    if (!A.issquare) {
      const string message = "Input matrix is not square.";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }

    // Reshape eigenvector matrix if needed
    if ((U.rows != A.rows) || (U.cols != A.cols)) {
      const xmatrix<xcomplex<utype>> U_new(A.urows, A.ucols, A.lrows, A.lcols);
      U = U_new;
    }
    ...
    for (nSweep = 1; nSweep <= max_sweeps; nSweep++) {
      ...
      for (q = 2; q <= n; q++) {
        for (p = 1; p <= q - 1; p++) {
          sum = magsqr(A[p][q]);
          if ((nSweep > threshold_sweep) && (sum < 0.5 * eps * std::max<utype>(ev[2][p] * ev[2][p], ev[2][q] * ev[2][q]))) {
            A[p][q] = 0;
    ...
    if (nSweep > max_sweeps) {
      const string message = "Number of sweeps exceeded maximum number of sweeps.";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
...
#define AST_TEMPLATE(utype) template xvector<utype> jacobiHermitian(xmatrix<xcomplex<utype>>&, xmatrix<xcomplex<utype>>&, char);
  AST_GEN_1(AST_UTYPE_FLOAT)
#undef AST_TEMPLATE
} // namespace aurostd
...
// ****************************************************
namespace aurostd {
  template <class utype> void eigsrt(xvector<utype>& d, xmatrix<utype>& v) {
...
#define AST_TEMPLATE(utype) template void eigsrt(xvector<utype>&, xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
// CO20171129
namespace aurostd {
  template <class utype> void QRDecomposition_HouseHolder(const xmatrix<utype>& mat_orig, xmatrix<utype>& Q, xmatrix<utype>& R, double tol) { // CO20191110
...
#define AST_TEMPLATE(utype) template void QRDecomposition_HouseHolder(const xmatrix<utype>&, xmatrix<utype>&, xmatrix<utype>&, double);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> void QRDecomposition_HouseHolder_MW(const xmatrix<utype>& mat_orig, xmatrix<utype>& Q, xmatrix<utype>& R, double tol) { // CO20191110
    // mat is mxn, m>=n
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " mat_orig=" << endl;
      cerr << mat_orig << endl;
    }
    if (mat_orig.rows < mat_orig.cols) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m<n, please flip the matrix", _VALUE_ERROR_);
    }
    ...
    for (int k = R.lcols; k <= R.ucols; k++) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " step k=" << k << endl;
      }
      xmatrix<utype> x(R.urows, R.lcols, R.lrows, R.lcols);
      x.setmat(R.getxmat(k, R.urows, k, k), k, R.lcols); // x(k:m,1)=R(k:m,k);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " x=" << endl;
        cerr << x << endl;
      }
      xmatrix<utype> v(x); //+x first
      v[k][v.lcols] = x[k][x.lcols] + aurostd::modulus(x);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " v(unnormalized)=" << endl;
        cerr << v << endl;
      }
      vModulus = aurostd::modulus(v);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " ||v||=" << vModulus << endl;
      }
      if (!iszero(vModulus, tol)) { // prevents division by 0
        v /= vModulus;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " v(  normalized)=" << endl;
          cerr << v << endl;
        }
        const xmatrix<utype> u = (utype) 2.0 * trasp(R) * v;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " u=" << endl;
          cerr << u << endl;
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " R( pre)=" << endl;
          cerr << R << endl;
        }
        R -= v * trasp(u); // product HR
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " R(post)=" << endl;
          cerr << R << endl;
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " Q( pre)=" << endl;
          cerr << Q << endl;
        }
        Q -= (utype) 2.0 * Q * v * trasp(v); // product QR
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " Q(post)=" << endl;
          cerr << Q << endl;
        }
      }
    }
    ...
    if (!aurostd::isequal(mat_orig, Q * R, tol)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "QR decomposition failed (A!=Q*R)", _RUNTIME_ERROR_);
    }
    if (!aurostd::isequal(trasp(Q) * Q, eye<utype>(Q.urows, Q.ucols, Q.lrows, Q.lcols), tol)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "QR decomposition failed (Q not orthonormal)", _RUNTIME_ERROR_);
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " END" << endl;
    }
  }
#define AST_TEMPLATE(utype) template void QRDecomposition_HouseHolder_MW(const xmatrix<utype>&, xmatrix<utype>&, xmatrix<utype>&, double);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> void QRDecomposition_HouseHolder_TB(const xmatrix<utype>& mat_orig, xmatrix<utype>& Q, xmatrix<utype>& R, double tol) { // CO20191110
    // mat is mxn, m>=n
    ...
    int k = 0;
    xmatrix<utype> x;
    xmatrix<utype> A; // since x and A changes size with each loop, let getxmatInPlace() handle it internally
    ...
    for (k = R.lcols; k <= R.ucols; k++) {
      ...
      if (!iszero(vModulus, tol)) { // prevents division by 0
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " A( pre)=" << endl;
          cerr << A << endl;
        }
        A -= (utype) 2.0 * v * trasp(v) * A;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " A(post)=" << endl;
          cerr << A << endl;
        }
    ...
    for (k = R.lcols; k <= R.urows; k++) { // calculate Q*e1,Q*e2...
      ...
      for (i = R.lrows; i <= R.urows; i++) {
        ek[i][ek.lcols] = (i == k) ? (utype) 1 : (utype) 0;
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " ek( pre)=" << endl;
        cerr << ek << endl;
      }
      for (i = R.ucols; i >= R.lcols; i--) {
        ek.getxmatInPlace(x, i, R.urows, R.lcols, R.lcols);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " x( pre)=" << endl;
          cerr << x << endl;
        }
        x -= (utype) 2.0 * V[i - R.lcols] * trasp(V[i - R.lcols]) * x;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " x(post)=" << endl;
          cerr << x << endl;
        }
        ek.setmat(x, i, R.lcols);
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " ek(post)=" << endl;
        cerr << ek << endl;
      }
...
#define AST_TEMPLATE(utype) template void QRDecomposition_HouseHolder_TB(const xmatrix<utype>&, xmatrix<utype>&, xmatrix<utype>&, double);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype> void getEHermite(utype a, utype b, xmatrix<utype>& ehermite) { // CO+YL20191201
    ...
    GCD(a, b, gcd, x, y);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " gcd(" << a << "," << b << ")=" << gcd << ", x=" << x << ", y=" << y << endl;
    }
    // ehermite is 2x2
    if (ehermite.rows != 2 || ehermite.cols != 2) {
      const xmatrix<utype> ehermite_tmp(2, 2);
      ehermite = ehermite_tmp;
    }
    if (gcd) {
...
#define AST_TEMPLATE(utype) template void getEHermite(utype, utype, xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> void getSmithNormalForm(const xmatrix<utype>& A_in, xmatrix<utype>& U_out, xmatrix<utype>& V_out, xmatrix<utype>& S_out, double tol) { // CO+YL20191201
    // implementation is inspired by that found here: http://pydoc.net/GBpy/0.1.1/GBpy.tools/
    // original license: GNU-GPL Style.
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " A=" << endl;
      cerr << A_in << endl;
    }

    xmatrix<double> S = aurostd::xmatrixutype2double(A_in);
    S.shift(1, 1); // algorithm depends on lrows==lcols==1
    ...
    for (j = S.lcols; j <= min_mn; j++) {
      ...
      for (i = j + 1; i <= m; i++) {
        if (!iszero(S[i][j], tol)) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " getEHermite(S[j=" << j << "][j=" << j << "]=" << S[j][j] << ",S[i=" << i << "][j=" << j << "]=" << S[i][j] << ")=" << endl;
            cerr << E << endl;
          }

          // Apply the transform to S
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " S(pre)=" << endl;
            cerr << S << endl;
          }
          // build S([j i],:)
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " S([j=" << j << " i=" << i << "],:)=" << endl;
            cerr << twoXn_in << endl;
          }
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " S(post)=" << endl;
            cerr << S << endl;
          }

          // Apply the transform to U
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " U(pre)=" << endl;
            cerr << U << endl;
          }
          // build U(:,[j i])
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " U(:,[j=" << j << " i=" << i << "])=" << endl;
            cerr << mXtwo_in << endl;
          }
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " U(post)=" << endl;
            cerr << U << endl;
          }
        }
      }
      ...
      for (i = j + 2; i <= n; i++) {
        if (!iszero(S[j][i], tol)) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " getEHermite(S[j=" << j << "][j+1=" << j + 1 << "]=" << S[j][j + 1] << ",S[j=" << j << "][i=" << i << "]=" << S[j][i] << ")=" << endl;
            cerr << E << endl;
          }
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " S(:,[j+1=" << j + 1 << " i=" << i << "])=" << endl;
            cerr << mXtwo_in << endl;
          }
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " V(pre)=" << endl;
            cerr << V << endl;
          }
          // build V(:,[j+1 i])
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " V(:,[j+1=" << j + 1 << " i=" << i << "])=" << endl;
            cerr << nXtwo_in << endl;
          }
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " V(post)=" << endl;
            cerr << V << endl;
          }
        }
      }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " S is now upper bidiagonal, eliminating superdiagonal non-zeros" << endl;
    }
    ...
    while (!iszero(D, tol)) {
      ...
      if (std::signbit(S[k][k])) {
        for (i = S.lcols; i <= S.ucols; i++) {
          S[k][i] = -S[k][i];
        }
        for (i = U.lrows; i <= U.urows; i++) {
          U[i][k] = -U[i][k];
        }
      }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " S(:,[k=" << k << " k+1=" << k + 1 << "])=" << endl;
        cerr << mXtwo_in << endl;
      }
      mXtwo_out = mXtwo_in * trasp(E);
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " V(:,[k=" << k << " k+1=" << k + 1 << "])=" << endl;
        cerr << nXtwo_in << endl;
      }
      nXtwo_out = nXtwo_in / E;
      ...
      if (!iszero(S[k][k + 1], tol)) {
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " getEHermite(S[k=" << k << "][k=" << k << "]=" << S[k][k] << ",S[k=" << k << "][k+1=" << k + 1 << "]=" << S[k][k + 1] << ")=" << endl;
          cerr << E << endl;
        }
        ...
        for (j = S.lcols; j <= min_mn; j++) {
          if (j + 1 <= m) {
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " getEHermite(S[j=" << j << "][j=" << j << "]=" << S[j][j] << ",S[j+1=" << j + 1 << "][j=" << j << "]=" << S[j + 1][j] << ")=" << endl;
              cerr << E << endl;
            }
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " S([j=" << j << " j+1=" << j + 1 << "],:)=" << endl;
              cerr << twoXn_in << endl;
            }
            twoXn_out = E * twoXn_in; // 2x2 x 2x3 = 2x3
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " U(:,[j=" << j << " j+1=" << j + 1 << "])=" << endl;
              cerr << mXtwo_in << endl;
            }
            mXtwo_out = mXtwo_in / E;
          ...
          if (j + 2 <= n) {
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " getEHermite(S[j=" << j << "][j+1=" << j + 1 << "]=" << S[j][j + 1] << ",S[j=" << j << "][j+2=" << j + 2 << "]=" << S[j][j + 2] << ")=" << endl;
              cerr << E << endl;
            }
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " S(:,[j+1=" << j + 1 << " j+2=" << j + 2 << "])=" << endl;
              cerr << mXtwo_in << endl;
            }
            mXtwo_out = mXtwo_in * trasp(E);
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " V(:,[j+1=" << j + 1 << " j+2=" << j + 2 << "])=" << endl;
              cerr << nXtwo_in << endl;
            }
            nXtwo_out = nXtwo_in / E;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " S is now diagonal, make it non-negative" << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " squeezing factors to lower right to enforce divisibility condition" << endl;
    }
    ...
    for (i = S.lcols; i <= min_mn; i++) {
      for (j = i + 1; j <= min_mn; j++) {
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " E=" << endl;
          cerr << E << endl;
          cerr << __AFLOW_FUNC__ << " F=" << endl;
          cerr << F << endl;
        }
        ...
        // build S([i j],[i j])
        twoXtwo_in[twoXtwo_in.lrows][twoXtwo_in.lcols] = S[i][i];
        twoXtwo_in[twoXtwo_in.lrows][twoXtwo_in.ucols] = S[i][j]; // urows=lrows+1, ucols=lcols+1
        twoXtwo_in[twoXtwo_in.urows][twoXtwo_in.lcols] = S[j][i];
        twoXtwo_in[twoXtwo_in.urows][twoXtwo_in.ucols] = S[j][j]; // urows=lrows+1, ucols=lcols+1
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " S([i=" << i << " j=" << j << "],[i=" << i << " j=" << j << "])=" << endl;
          cerr << twoXtwo_in << endl;
        }
        twoXtwo_out = E * twoXtwo_in * trasp(F);
        // store twoXtwo_out into S([i j],[i j])
        S[i][i] = twoXtwo_out[twoXtwo_out.lrows][twoXtwo_out.lcols];
        S[i][j] = twoXtwo_out[twoXtwo_out.lrows][twoXtwo_out.ucols]; // urows=lrows+1, ucols=lcols+1
        S[j][i] = twoXtwo_out[twoXtwo_out.urows][twoXtwo_out.lcols];
        S[j][j] = twoXtwo_out[twoXtwo_out.urows][twoXtwo_out.ucols]; // urows=lrows+1, ucols=lcols+1
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " S(post)=" << endl;
          cerr << S << endl;
        }
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " U(:,[i=" << i << " j=" << j << "])=" << endl;
          cerr << mXtwo_in << endl;
        }
        mXtwo_out = mXtwo_in / E;
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " V(:,[i=" << i << " j=" << j << "])=" << endl;
          cerr << nXtwo_in << endl;
        }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " converting to xmatrix<int>" << endl;
    }
    ...
    if (!aurostd::isequal(A_in, U_out * S_out * trasp(V_out), (utype) tol)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "SNF decomposition failed", _RUNTIME_ERROR_);
    }

    // operations below here for Matlab-like output

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " transposing V to match Matlab" << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " inverting V and U to match Matlab" << endl;
    }
    V_out = inverse(V_out);
    U_out = inverse(U_out); // to be identical to matlab's smithForm we need V -> inv(V) U-> inv(U)
    // if results differ slightly from matlab, check _GCD() and enable matlab implementation for gcd(1,1)
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " U=" << endl;
      cerr << U_out << endl;
    ...
    if (!aurostd::isequal(S_out, U_out * A_in * V_out, (utype) tol)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "SNF decomposition failed AFTER Matlab transformations", _RUNTIME_ERROR_);
    }
...
#define AST_TEMPLATE(utype) template void getSmithNormalForm(const xmatrix<utype>&, xmatrix<utype>&, xmatrix<utype>&, xmatrix<utype>&, double);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
namespace aurostd {
  ...
  template <class utype> utype NR_SQR(utype a) {
    if (a == (utype) 0.0) {
      return 0.0;
    } else {
      return a * a;
...
#define AST_TEMPLATE(utype) template utype NR_SQR(utype);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> utype pythag(utype a, utype b) {
    utype absa;
    utype absb;
    absa = aurostd::abs(a);
    absb = aurostd::abs(b);
...
#define AST_TEMPLATE(utype) template utype pythag(utype, utype);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

#define NR_SIGN(a, b) ((b) >= 0.0 ? aurostd::abs(a) : -aurostd::abs(a))
  template <class utype> void tqli(xvector<utype>& d, xvector<utype>& e, xmatrix<utype>& z) {
    ...
    for (l = 1; l <= n; l++) {
      ...
      do {
        for (m = l; m <= n - 1; m++) {
          ...
          if ((utype) (aurostd::abs(e[m]) + dd) == dd) {
            break;
          }
        }
        if (m != l) {
...
#define AST_TEMPLATE(utype) template void tqli(xvector<utype>&, xvector<utype>&, xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
#undef NR_SIGN
} // namespace aurostd
...
namespace aurostd {
#define RADIX 2.0
  template <class utype> void balanc(xmatrix<utype>& a) {
...
#define AST_TEMPLATE(utype) template void balanc(xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
#undef RADIX
...
#define AST_TEMPLATE(utype) template void elmhes(xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
#undef ELMHES_SWAP
} // namespace aurostd
...
#define NR_SIGN(a, b) ((b) >= 0.0 ? aurostd::abs(a) : -aurostd::abs(a))
  ...
  template <class utype> void hqr(xmatrix<utype>& a, xvector<utype>& wr, xvector<utype>& wi) {
    int nn;
    int m;
    int l;
    int k;
    ...
    while (nn >= 1) {
      ...
      do {
        for (l = nn; l >= 2; l--) {
          ...
          if ((utype) (aurostd::abs(a[l][l - 1]) + s) == s) {
            break;
          }
        }
        x = a[nn][nn];
        if (l == nn) {
          ...
          if (l == (nn - 1)) {
            ...
            if (its == 30) {
              const string message = "Too many iterations in hqr";
              throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
            }
            if (its == 10 || its == 20) {
              t += x;
            ...
            for (m = (nn - 2); m >= l; m--) {
              ...
              v = aurostd::abs(p) * (aurostd::abs(a[m - 1][m - 1]) + aurostd::abs(z) + aurostd::abs(a[m + 1][m + 1]));
              if ((utype) (u + v) == v) {
                break;
              }
            }
...
#define AST_TEMPLATE(utype) template void hqr(xmatrix<utype>&, xvector<utype>&, xvector<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
#undef NR_SIGN
} // namespace aurostd
...
namespace aurostd {
  ...
  template <class utype> void eigen(const xmatrix<utype>& ain, xvector<utype>& wr, xvector<utype>& wi) {
    xmatrix<utype> a(ain);
    balanc(a);
    elmhes(a);
    hqr(a, wr, wi);
  }
#define AST_TEMPLATE(utype) template void eigen(const xmatrix<utype>&, xvector<utype>&, xvector<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define AST_TEMPLATE(utype) template utype l1_norm(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> double frobenius_norm(const xmatrix<utype>& m) {
    double norm = 0;
...
#define AST_TEMPLATE(utype) template double frobenius_norm(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> double l2_norm(const xmatrix<utype>& m) {
    xvector<utype> wr(m.rows);
    xvector<utype> wi(m.rows);
    eigen(m, wr, wi);
    eigen((trasp(m) * m), wr, wi);
    return sqrt(max(wr));
  }
#define AST_TEMPLATE(utype) template double l2_norm(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template utype linf_norm(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
} // namespace aurostd
...
#define cematrix_EQUAL_DOUBLE 1.0e-9 // two doubles are equal if difference is smaller than it
  ...
  void cematrix::LeastSquare(xvector<double>& y_vec, xvector<double>& y_sigma) { // function
    if (nrow != y_vec.rows) {
      const string message = "No match of ranks of b and A. Input two matrices A (m x n) and b (m x 1)";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_MISMATCH_);
    }
    // SVDcmp(A);
  ...
  void cematrix::LeastSquare(xvector<double>& y_vec) {
    ...
    for (int i = y_sigma.lrows; i <= y_sigma.urows; i++) {
      y_sigma[i] = 1.0;
    }
  ...
  double cematrix::Pythag2(double a, double b) { // calculate (a^2+ b^2)^(1/2)
    ...
    const double aabs = abs(a);
    const double babs = abs(b);
    double val_min;
    double val_max;
  ...
  xmatrix<double> cematrix::InverseMatrix() {
    ...
    if (DetW < cematrix_EQUAL_DOUBLE) {
      const string message = "Singular Matrix. No Inversion.";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    ...
    const bool verbose = (LDEBUG || isequal(Iden, Iden_tmp)); // CO20190327
  ...
  void cematrix::SVDFit(xvector<double>& y, xvector<double>& y_sigma) {
    ...
    const xvector<double> y_cal(1, nrow);
    A = M;
    if (A.rows != y.rows) {
      const string message = "Ranks of x vector and y vector do not match!";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_MISMATCH_);
    }
...
namespace aurostd {
  ...
  template <class utype> matrix<utype>::matrix() {
    free();
  } // default
#define AST_TEMPLATE(utype) template matrix<utype>::matrix(void);
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE
#define AST_TEMPLATE(utype) template matrix<matrix<utype>>::matrix(void);
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE

  template <class utype> matrix<utype>::matrix(const int m) { // specifying rows
    free(); // CO20200404 pflow::matrix()->aurostd::matrix()
    const std::vector<utype> v;
    mat = std::vector<std::vector<utype>>(m, v);
  }
#define AST_TEMPLATE(utype) template matrix<utype>::matrix(const int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> matrix<utype>::matrix(const int m, const int n) { // specifying rows and columns
    free(); // CO20200404 pflow::matrix()->aurostd::matrix()
    const std::vector<utype> v(n);
    mat = std::vector<std::vector<utype>>(m, v);
  }
#define AST_TEMPLATE(utype) template matrix<utype>::matrix(const int, const int);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> matrix<utype>::matrix(const int m, const std::vector<utype>& inutypevec) { // specifying rows as vectors.
    free(); // CO20200404 pflow::matrix()->aurostd::matrix()
    mat = std::vector<std::vector<utype>>(m, inutypevec);
  }
#define AST_TEMPLATE(utype) template matrix<utype>::matrix(const int, const std::vector<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> matrix<utype>::matrix(const int m, const int n, const utype& inutype) { // specifying rows and columns and initial values
    free(); // CO20200404 pflow::matrix()->aurostd::matrix()
    const std::vector<utype> v(n);
    mat = std::vector<std::vector<utype>>(m, v);
...
#define AST_TEMPLATE(utype) template matrix<utype>::matrix(const int, const int, const utype&);
  AST_GEN_1(AST_UTYPE_NUM_STDCOMPLEX)
#undef AST_TEMPLATE
#define AST_TEMPLATE(utype) template matrix<matrix<utype>>::matrix(const int, const int, const matrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM_STDCOMPLEX)
#undef AST_TEMPLATE

  // CO20200404 START - patching matrix for nietzsche
  template <class utype> matrix<utype>::matrix(const matrix& b) {
    copy(b);
  }
#define AST_TEMPLATE(utype) template matrix<utype>::matrix(const matrix&);
  AST_GEN_1(AST_UTYPE_NUM_STDCOMPLEX)
#undef AST_TEMPLATE
#define AST_TEMPLATE(utype) template matrix<matrix<std::complex<utype>>>::matrix(const matrix&);
  AST_GEN_1(AST_UTYPE_NUM_STDCOMPLEX)
#undef AST_TEMPLATE
...
#undef AST_TEMPLATE
  ...
  template <class utype> void matrix<utype>::clear() {
    const matrix a;
    copy(a);
  } // clear PUBLIC
#define AST_TEMPLATE(utype) template void matrix<utype>::clear();
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE

  template <class utype> const matrix<utype>& matrix<utype>::operator=(const matrix& other) {
    if (this != &other) {
      copy(other);
    }
    return *this;
  }
#define AST_TEMPLATE(utype) template const matrix<utype>& matrix<utype>::operator=(const matrix&);
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE
#define AST_TEMPLATE(utype) template const matrix<matrix<utype>>& matrix<matrix<utype>>::operator=(const matrix&);
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE

  template <class utype> void matrix<utype>::free() {
    for (size_t i = 0; i < mat.size(); i++) {
      mat[i].clear();
    }
    mat.clear();
  }
#define AST_TEMPLATE(utype) template void matrix<utype>::free();
  AST_GEN_1(AST_UTYPE_NUM_COMPLEX)
#undef AST_TEMPLATE

  template <class utype> void matrix<utype>::copy(const matrix& b) {
    free();
    for (size_t i = 0; i < b.mat.size(); i++) {
      mat.push_back(std::vector<utype>(0));
      for (size_t j = 0; j < b.mat[i].size(); j++) {
        mat[i].push_back(b.mat[i][j]);
      }
    }
  }
#define AST_TEMPLATE(utype) template void matrix<utype>::copy(const matrix&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  // accessors
  template <class utype> void matrix<utype>::print() {
...
#define AST_TEMPLATE(utype) template void matrix<utype>::print(void);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype> matrix<utype> matrix<utype>::transpose() const {
    matrix<utype> tmat;
...
#define AST_TEMPLATE(utype) template matrix<utype> matrix<utype>::transpose(void) const;
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype> void matrix<utype>::vecvec2mat(const std::vector<std::vector<utype>>& inVV) {
...
#define AST_TEMPLATE(utype) template void matrix<utype>::vecvec2mat(const std::vector<std::vector<utype>>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
  ...
  template <class utype> void matrix<utype>::vec2mat(const std::vector<utype>& inV) {
...
#define AST_TEMPLATE(utype) template void matrix<utype>::vec2mat(const std::vector<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE

  template <class utype> matrix<utype> xmatrix2matrix(const xmatrix<utype>& _xmatrix) {
    const int isize = _xmatrix.rows;
    const int jsize = _xmatrix.cols;
...
#define AST_TEMPLATE(utype) template matrix<utype> xmatrix2matrix(const xmatrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
...
#define AST_TEMPLATE(utype) template xmatrix<utype> matrix2xmatrix(const matrix<utype>&);
  AST_GEN_1(AST_UTYPE_NUM)
#undef AST_TEMPLATE
