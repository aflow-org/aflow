...
#ifndef _AUROSTD_HASH_H_
#define _AUROSTD_HASH_H_
...
namespace aurostd {
  ...
  // functions using fast openSSL implementation  //HE20240908
  std::string file2hash(const std::filesystem::path& file_path, const std::string& hash_type = "sha256");
  std::string string2hash(const std::string& input, const std::string& hash_type = "sha256");
  // helper for these hash functions
  std::string binary2hex(const unsigned char* value, uint data_length);

  // legacy hash functions - to be deprecated
  unsigned int getFileCheckSum(const std::string&, const std::string&);  // ME20190219
  unsigned int getFletcher32(unsigned short*, size_t);  // ME20190219

  // crc64 is used mainly for AUID callculation
  // crc64_tab is in the header to enable constexpr crc64 functions //HE20220404
  ...
  static const std::string human_alphanum_choices = "0123456789ABCDEFGHJKLMNPQRSTUVWXYZ"; ///< basis for the human readable hash, drops O and I to avoid mix-ups

  uint64_t crc64(uint64_t crc, const unsigned char* s, uint64_t l);
  uint64_t crc64(uint64_t crc, const std::string& s);
  uint64_t crc64(const std::string& s);  // HE20220404 partner function to aurostd::ctcrc64 starting at crc 0
  std::string crc2string(uint64_t crc);
  std::string crc2human(uint64_t crc, uint width = 0); // HE20230221
  std::string crc2human(const std::string& input, uint width = 0); // HE20230221
  std::string file2auid(const std::string& file); // SC20200326

  // constexpr variant of aurostd::crc64 that the compiler can precalculate // HE20220404
  // for compiler optimized string based switch/case
  constexpr uint64_t ctcrc64_impl(const char* p, const size_t len, const uint64_t crc) {
    return len ? ctcrc64_impl(p + 1, len - 1, (crc >> 8) ^ crc64_tab[(uint8_t) crc ^ *p]) : crc;
  }

  constexpr uint64_t ctcrc64(const char* data, const size_t length) {
