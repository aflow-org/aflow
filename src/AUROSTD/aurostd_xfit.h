...
namespace aurostd {
  ...
  utype polynomialFindExtremum(const xvector<utype>& p,
                               const utype xmin,
                               const utype xmax,
...
namespace aurostd {
  ...
  bool findZeroNewtonRaphson(const xvector<double>& x0,
                             const std::vector<std::function<double(xvector<double>)>>& vf,
                             const std::vector<std::vector<std::function<double(xvector<double>)>>>& jac,
                             xvector<double>& x,
                             const uint niter = 100,
                             const double tol = (double) AUROSTD_IDENTITY_TOL); // SD20220616 //CO20230313 - utype clang warnings
  bool findZeroDeflation(const xvector<double>& x0,
                         const std::vector<std::function<double(xvector<double>)>>& vf,
                         const std::vector<std::vector<std::function<double(xvector<double>)>>>& jac,
                         xmatrix<double>& mx,
                         const uint niter = 100,
                         const double tol = (double) AUROSTD_IDENTITY_TOL); // SD20220619  //CO20230313 - utype clang warnings
} // namespace aurostd
...
//********************************************************************************
namespace aurostd {
  bool checkDerivatives(const xvector<double>& x, const std::function<double(xvector<double>)>& f, const std::vector<std::function<double(xvector<double>)>>& df, const double tol = (double) AUROSTD_IDENTITY_TOL); // SD20220622 //CO20230313 - utype clang warnings
  std::vector<std::vector<std::function<double(xvector<double>)>>> calcNumericalJacobian(const std::vector<std::function<double(xvector<double>)>>& vf, const xvector<double>& _dx); // SD20220624
} // namespace aurostd
...
namespace aurostd {
  ...
  public:
    ...
    NonlinearFit(xvector<double>& x, xvector<double>& y, xvector<double>& guess, double foo(const double x, const xvector<double>& p, xvector<double>& dydp), double tol = 1e-6, double tau = 1e-12, int max_iter = 1000);
    ~NonlinearFit();
