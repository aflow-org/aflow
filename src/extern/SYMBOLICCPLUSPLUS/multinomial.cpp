...
#ifndef SYMBOLIC_CPLUSPLUS_MULTINOMIAL_DEFINE //DX20200824 - added for proper compilation
#define SYMBOLIC_CPLUSPLUS_MULTINOMIAL_DEFINE //DX20200824 - added for proper compilation
...
namespace symbolic { //DX20200825
  ...
  {u.push_back(std::pair<T,int>(one(T()),1));}
  ...
  template <class T>
    Multinomial<T> Multinomial<T>::operator-() const
    {
      ...
      typename std::list<std::pair<T,int> >::iterator i = p2.u.begin();
      typename std::list<std::pair<T,int> >::iterator j = p2.m.begin();
  ...
  template <class T>
    std::list <std::pair<T,int> > //DX20200825 - add std namespace to list
    merge(const std::list <std::pair<T,int> > &l1,const std::list <std::pair<T,int> > &l2) //DX20200825 - add std namespace to list
    {
      std::list <std::pair<T,int> > l; //DX20200825 - add std namespace to list
      typename std::list<std::pair<T,int> >::const_iterator i = l1.begin();
      typename std::list<std::pair<T,int> >::const_iterator j = l2.begin();
  ...
  template <class T>
    std::list <std::pair<T,int> > //DX20200825 - add std namespace to list
    distribute(const std::list <std::pair<T,int> > &l1,const std::list <std::pair<T,int> > &l2) //DX20200825 - add std namespace to list
    {
      std::list <std::pair<T,int> > l; //DX20200825 - add std namespace to list
      typename std::list<std::pair<T,int> >::const_iterator i;
      typename std::list<std::pair<T,int> >::const_iterator j;

      for(i=l1.begin();i!=l1.end();i++)
      {
        std::list <std::pair<T,int> > t; //DX20200825 - add std namespace to list
  ...
  template <class T>
    ...
    {
      Multinomial<T> p2;
      typename std::list<std::pair<T,int> >::const_iterator i = u.begin();
      typename std::list<std::pair<Multinomial<T>,int> >::const_iterator j = m.begin();
  ...
  template <class T>
    void Multinomial<T>::remove_zeros(void)
    {
      ...
      {
        typename std::list<std::pair<Multinomial<T>,int> >::iterator i, j;
        ...
        const Multinomial<T> &y) const
    {
      ...
      if(x.type==univariate && y.type==univariate)
      {
        ...
        if(x.variable< y.variable)
        {
          ...
          typename std::list<std::pair<T,int> >::const_iterator i = y.u.begin();
          for(;i!=y.u.end();i++)
          {
        ...
        else 
        {
          ...
          typename std::list<std::pair<T,int> >::const_iterator i = x.u.begin();
          for(;i!=x.u.end();i++)
          {
