...
namespace symbolic { // DX20200625
  template <class T> class Multinomial {
  public:
    Multinomial() : type(number), n(zero(T())) {}
    Multinomial(T);
    Multinomial(std::string x);
    Multinomial(const Multinomial<T>& p) : variable(p.variable), type(p.type), n(p.n), u(p.u), m(p.m) {}
  ...
  protected:
    void remove_zeros();
    std::pair<Multinomial<T>, Multinomial<T>> reconcile(const Multinomial<T>&, const Multinomial<T>&) const;
    [[nodiscard]] std::vector<std::string> toarray() const;
    std::string variable;
  ...
  template <class T> Multinomial<T> operator+(const T&, const Multinomial<T>&);
  template <class T> Multinomial<T> operator-(const T&, const Multinomial<T>&);
  template <class T> Multinomial<T> operator*(const T&, const Multinomial<T>&);
  template <class T> int operator==(T, const Multinomial<T>&);
  template <class T> int operator!=(T, const Multinomial<T>&);
