...
const std::vector<std::string> LATEX_DEFAULT_COLORS = {"blue",   "green", "red",    "brown",    "cyan", "lime",      "magenta", "olive", "orange", "pink",
                                                       "purple", "teal",  "violet", "darkgray", "gray", "lightgray", "black",   "white", "yellow"}; // putting favorites near beginning, not so good ones in the back
const std::vector<std::string> LATEX_DVIPS_COLORS = {"Apricot",     "Aquamarine",    "Bittersweet", "Black",          "Blue",           "BlueGreen", "BlueViolet",  "BrickRed",    "Brown",       "BurntOrange",
                                                     "CadetBlue",   "CarnationPink", "Cerulean",    "CornflowerBlue", "Cyan",           "Dandelion", "DarkOrchid",  "Emerald",     "ForestGreen", "Fuchsia",
                                                     "Goldenrod",   "Gray",          "GreenGreen",  "Yellow",         "JungleGreen",    "Lavender",  "LimeGreen",   "Magenta",     "Mahogany",    "Maroon",
                                                     "Melon",       "MidnightBlue",  "Mulberry",    "NavyBlue",       "OliveGreen",     "Orange",    "OrangeRed",   "Orchid",      "Peach",       "Periwinkle",
                                                     "PineGreen",   "Plum",          "ProcessBlue", "Purple",         "RawSienna",      "Red",       "RedOrange",   "RedViolet",   "Rhodamine",   "RoyalBlue",
                                                     "RoyalPurple", "RubineRed",     "Salmon",      "SeaGreen",       "Sepia",          "SkyBlue",   "SpringGreen", "Tan",         "TealBlue",    "Thistle",
                                                     "Turquoise",   "Violet",        "VioletRed",   "White",          "WildStrawberry", "Yellow",    "YellowGreen", "YellowOrange"}; // contains defaults and more
const std::vector<std::string> LATEX_COLORS_TO_AVOID = {"black", "white", "yellow", "darkgray", "gray", "lightgray"};
...
namespace chull {
  bool convexHull(const aurostd::xoption& _vpflow) {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    ostream& oss = cout;
    ...
    const string directory = getPath(vpflow, FileMESSAGE, oss);
    ...
    if (vpflow.flag("CHULL::SCREEN_ONLY")) {
      if (!(vpflow.flag("CHULL::TEXT_DOC") || vpflow.flag("CHULL::JSON_DOC"))) {
        vpflow.flag("CHULL::JSON_DOC", true);
      }
      if (vpflow.flag("CHULL::TEXT_DOC") && vpflow.flag("CHULL::JSON_DOC")) {
        vpflow.flag("CHULL::TEXT_DOC", false);
    ...
    const string usage_usage = "aflow --convex_hull=|--chull --alloy=MnPdPt[,AlCuZn,...] [--np=1] [chull_options] [--destination=[DIRECTORY]]";
    vector<string> usage_options;
    usage_options.push_back(usage_usage);
    usage_options.emplace_back(" ");
    usage_options.emplace_back("chull_options:");
    usage_options.emplace_back(" ");
    usage_options.emplace_back("GENERAL OPTIONS:");
    usage_options.emplace_back("--usage");
    usage_options.emplace_back("--print=|--p=|--output=|--o=latex|pdf|png|json|text|jupyter|jupyter2|jupyter3");
    usage_options.emplace_back("--keep=tex|--keep_tex|--keeptex|--tex");
    usage_options.emplace_back("--keep=log|--keep_log|--keeplog|--log");
    usage_options.emplace_back("--keep=tex,log");
    usage_options.emplace_back(" ");
    usage_options.emplace_back("LOADING OPTIONS:");
    usage_options.emplace_back("--load_library=|--loadlibrary=|--ll=icsd|lib1|lib2|lib3");
    usage_options.emplace_back("--load_API|--load_api|--loadapi|--lapi|--api");
    usage_options.emplace_back("--load_entries_entry_output|--loadentriesentryoutput|--leo");
    usage_options.emplace_back("--neglect=|--ban=aflow:bb0d45ab555bc208);aflow:fb9eaa58604ce774");
    usage_options.emplace_back("--see_neglect|--seeneglect|--sn");
    usage_options.emplace_back("--remove_extreme_points=|--removeextremepoints=|--remove_extrema=|--removeextrema=|--rep=-1000");
    usage_options.emplace_back("--entropic_temperature|--entropictemperature|--entroptemp");
    usage_options.emplace_back(" ");
    usage_options.emplace_back("ANALYSIS OPTIONS:");
    usage_options.emplace_back("--distance_to_hull=|--distancetohull=|--distance2hull=|--dist2hull=|--d2h=aflow:bb0d45ab555bc208,aflow:fb9eaa58604ce774");
    usage_options.emplace_back("--stability_criterion=|--stabilitycriterion=|--stable_criterion=|--scriterion=|--sc=aflow:bb0d45ab555bc208,aflow:fb9eaa58604ce774");
    usage_options.emplace_back(
    ...
    usage_options.emplace_back("--hull_formation_enthalpy=|--hull_enthalpy=|--hull_energy=0.25,0.25");
    usage_options.emplace_back("--skip_structure_comparison|--skipstructruecomparison|--skipstructcomp|--ssc");
    usage_options.emplace_back("--skip_stability_criterion_analysis|--skipstabilitycriterionanalysis|--skip_scriterion|--skipscriterion|--sscriterion");
    usage_options.emplace_back(
        "--skip_n_plus_1_enthalpy_gain_analysis|--skip_n_plus_1_energy_gain_analysis|--skipnplus1enthalpygainanalysis|--skipnplus1energygainanalysis|--skip_nplus1|--skipnplus1|--snp1|--snpo");
    usage_options.emplace_back("--include_skewed_hulls|--include_skewed|--ish");
    usage_options.emplace_back("--include_unreliable_hulls|--include_unreliable|--iuh");
    usage_options.emplace_back("--include_outliers|--io");
    usage_options.emplace_back("--strict_outlier_analysis|--soa");
    usage_options.emplace_back("--include_ill_converged|--iic");
    usage_options.emplace_back("--force");
    usage_options.emplace_back(" ");
    usage_options.emplace_back("LATEX/PDF/PNG OPTIONS:");
    usage_options.emplace_back("--image_only|--imageonly|--image");
    usage_options.emplace_back("--no_document|--nodocument|--no_doc|--nodoc|--full_page_image|--fullpageimage");
    usage_options.emplace_back("--document_only|--documentonly|--doc_only|--doconly|--doc");
    usage_options.emplace_back("--latex_output|--latexoutput");
    usage_options.emplace_back("--latex_interactive|--latexinteractive");
    usage_options.emplace_back("--light_contrast|--lightcontrast|--lc");
    usage_options.emplace_back("--large_font|--largefont|--large|--lf");
    usage_options.emplace_back("--png_resolution=|--pngresolution=|--pngr=300");
    usage_options.emplace_back("--plot_iso_max_latent_heat|--plot_isomax|--iso_max|--isomax");
    usage_options.emplace_back(" ");
    // output usage
    if (vpflow.flag("CHULL::USAGE")) {
      if (!vpflow.flag("CHULL::SCREEN_ONLY")) {
        init::MessageOption("--usage", "CHULL()", usage_options);
      }
      if (vpflow.flag("CHULL::SCREEN_ONLY") && vpflow.flag("CHULL::JSON_DOC")) {
        oss << "{}";
      } // so JSON-reader doesn't bomb
      return true;
    }
    ...
    if (!vpflow.flag("PFLOW::LOAD_LIBRARY")) {
      vpflow.push_attached("PFLOW::LOAD_LIBRARY", "all");
    }
    aurostd::string2tokens(vpflow.getattachedscheme("PFLOW::LOAD_LIBRARY"), vlibraries, ",");
    // special case, all
    if ((vlibraries.size() == 1) && (vlibraries[0] == "all")) {
      pflow::defaultLoadEntriesFlags(vpflow, FileMESSAGE, oss, std::string("A"), false, true);
    } else {
      bool found = false;
      for (size_t i = 0, fl_size_i = vlibraries.size(); i < fl_size_i; i++) {
        found = false;
        for (uint lib = 1; lib <= _AFLOW_LIB_MAX_ && !found; lib++) {
          const string LIB = aurostd::utype2string(lib);
    ...
    if (vpflow.flag("CHULL::OUTPUT")) {
      ...
      for (size_t i = 0, fl_size_i = out_forms.size(); i < fl_size_i; i++) {
        if (!((out_forms[i][0] == 'A' || out_forms[i][0] == 'a') || (out_forms[i][0] == 'F' || out_forms[i][0] == 'f') || (out_forms[i][0] == 'T' || out_forms[i][0] == 't') ||
              (out_forms[i][0] == 'J' || out_forms[i][0] == 'j') || (out_forms[i][0] == 'W' || out_forms[i][0] == 'w') ||
    ...
    if (inputs.empty()) {
      ...
      if (!vpflow.flag("CHULL::SCREEN_ONLY")) {
        init::ErrorOption("--alloy=" + vpflow.getattachedscheme("PFLOW::ALLOY"), "PFLOW()", usage_options);
      }
      if (vpflow.flag("CHULL::SCREEN_ONLY") && vpflow.flag("CHULL::JSON_DOC")) {
        oss << "{}";
      } // so JSON-reader doesn't bomb
    ...
    aurostd::StringSubstInPlace(inputs, "-", "");
    ; // CO20200531 - removing '-' from web
    //
    ...
    if (aurostd::substring2bool(inputs, ":")) { // enumeration mode
      ...
      vector<int> vsizes;
      for (size_t i = 0, fl_size_i = tokens_colon.size(); i < fl_size_i; i++) {
        vcomponents.emplace_back(0);
        if (tokens_colon[i] == "LIB2") {
          tokens_colon[i] = SPECIE_RAW_LIB2;
          verbose_elimination = false;
        }
        if (tokens_colon[i] == "LIB2U") {
          tokens_colon[i] = SPECIE_RAW_LIB2U;
          verbose_elimination = false;
        }
        if (tokens_colon[i] == "LIB3") {
          tokens_colon[i] = SPECIE_RAW_LIB3;
          verbose_elimination = false;
        }
        aurostd::string2tokens(tokens_colon[i], tokens_comma, ",");
        for (size_t j = 0, fl_size_j = tokens_comma.size(); j < fl_size_j; j++) {
          vcomponents[i].push_back(tokens_comma[j]);
        }
      ...
      while (xc.increment()) {
        ...
        if (verbose_elimination) {
          velements = aurostd::getElements(aurostd::joinWDelimiter(velements, ""), pp_string, FileMESSAGE, true, true, false, oss);
        } // fast way to eliminate pseudopotential information  //clean and sort, do not keep_pp  //CO20190712
        std::sort(velements.begin(), velements.end());
        original_input = aurostd::joinWDelimiter(velements, "");
        std::sort(velements.begin(), velements.end());
        velements.erase(std::unique(velements.begin(), velements.end()), velements.end()); // MnMnPd is same as MnPd
        if (velements.size() == nary) {
          vinputs.push_back(aurostd::joinWDelimiter(velements, ""));
        } // ignore MnPd if requested ternaries
        else {
          if (verbose_elimination) {
      ...
      if (inputs == "LIB3HEUSLERS" && aurostd::FileExist("/common/LIB3/LIB")) {
        ...
        inputs = aurostd::execute2string(R"(find /common/LIB3/LIB -maxdepth 2 -name 'T000*' | grep 'T0001\|T0002\|T0003' | xargs dirname | sed 's/\/common\/LIB3\/LIB\///' | sort | uniq | tr -s '\r\n' ',' | sed -e 's/,$/\n/')");
        //[CO20221112 - too slow]velements.clear();
        //[CO20221112 - too slow]for(size_t i=0;i<vlib3.size();i++){
      ...
      for (size_t i = 0, fl_size_i = tokens_comma.size(); i < fl_size_i; i++) {
    ...
    if (verbose_elimination) {
      vector<string> _vinputs = vinputs;
      vinputs.clear();
      if (!_vinputs.empty()) {
        vinputs.push_back(_vinputs[0]);
      }
      for (size_t i = 1, fl_size_i = _vinputs.size(); i < fl_size_i; i++) { // VERBOSE
      ...
      vinputs.erase(std::unique(vinputs.begin(), vinputs.end()), vinputs.end());
    }
    ...
    const uint ntasks = vinputs.size();
    bool Krun = true, KrunSingle = true;
#ifdef AFLOW_MULTITHREADS_ENABLE
    int const ncpus = KBIN::get_NCPUS();
    if (ncpus > 1) {
      xthread::xThread xt(oss, ncpus, 1);
      vector<uint> vKrun(ntasks, 1); // c++ doesn't like vector<bool>
      std::function<void(uint, const vector<string>&, const aurostd::xoption&, const _aflags&, vector<uint>&, ostream&)> fn = [&](uint i, const vector<string>& vinputs, const aurostd::xoption& xoptions,
                                                                                                                                  const _aflags& aflags, vector<uint>& vKrun, ostream& oss) {
        vKrun[i] = (convexHull(vinputs[i], xoptions, aflags, oss, true) ? 1 : 0);
      };
      // create dumb oss for threaded environment
      const ofstream devnull("/dev/null"); // NULL
      ...
      for (uint i = 0; i < ntasks; i++) {
        ...
        if (Krun) {
          logger_type = _LOGGER_COMPLETE_;
          message << vinputs[i] << " completed successfully";
        } else {
          logger_type = _LOGGER_ERROR_;
          message << vinputs[i] << " did not complete successfully, run serially";
        }
  ...
  bool convexHull(const string& input, const aurostd::xoption& _vpflow, const _aflags& aflags, ostream& oss, bool silence_flag_check) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " BEGIN with input=" << input << endl;
    }
    ...
    const vector<string> velements = aurostd::getElements(input, pp_string, FileMESSAGE, true, true, false, oss); // clean and sort, do not keep_pp  //CO20190712
    if (velements.empty()) {
    ...
    if (vpflow.flag("CHULL::LOG")) {
      ...
      const string log_destination = directory + log_name; // no output before banner //CO20180220
    ...
    chull::flagCheck(vpflow, velements, FileMESSAGE, oss, silence_flag_check); // spit out all flag options
    ...
    //       for(size_t ia=0,fl_size_ia=vauid.size();ia<fl_size_ia;ia++) {
    ...
    if (vpflow.flag("CHULL::WRITE_JUPYTER2") || vpflow.flag("CHULL::WRITE_JUPYTER3")) {
      string aflow_chull_jupyter_json = aurostd::EmbData::get_content("aflow_chull_jupyter.json", "SCRIPTS");
      const string aflow_chull_jupyter_subdir = "AFLOW_CHULL_JUPYTER";
      const string jupyter_directory = directory + aflow_chull_jupyter_subdir;
      aurostd::DirectoryMake(jupyter_directory);
      aurostd::StringSubstInPlace(aflow_chull_jupyter_json, "AFLOW_ALLOY_INSERT_HERE", alloy);
      const string ver = (vpflow.flag("CHULL::WRITE_JUPYTER3")) ? "3" : "2";
      const string full_ver = (vpflow.flag("CHULL::WRITE_JUPYTER3")) ? "3" : "2";
      aurostd::StringSubstInPlace(aflow_chull_jupyter_json, "<ver>", ver);
      aurostd::StringSubstInPlace(aflow_chull_jupyter_json, "<full ver>", full_ver);

      // write needed files
      stringstream output;
      output << aflow_chull_jupyter_json;
      aurostd::stringstream2file(output, jupyter_directory + '/' + "notebook.ipynb");
      aurostd::EmbData::save_to_file("aflow_chull_jupyter_plotter.py", "SCRIPTS", jupyter_directory + '/' + "aflow_chull_plotter.py");
      aurostd::EmbData::save_to_file("aflow_chull_python.py", "SCRIPTS", jupyter_directory + '/' + "aflow_chull.py");
      aurostd::EmbData::save_to_file("aflow_chull_jupyter_requirements.txt", "SCRIPTS", jupyter_directory + '/' + "requirements.txt");
    ...
    if (vpflow.flag("CHULL::DIST2HULL") || vpflow.flag("CHULL::STABILITY_CRITERION") || vpflow.flag("CHULL::N+1_ENTHALPY_GAIN") || vpflow.flag("CHULL::HULL_FORMATION_ENTHALPY")) {
      ...
      message << "Skipping thermodynamic properties extraction";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    }
    ...
    const ConvexHull hull(vpflow, velements, FileMESSAGE, oss);
    ...
    const uint dimension = hull.getDim();
    if (!dimension) {
    ...
    if (vpflow.flag("CHULL::DIST2HULL")) {
      ...
      try {
        vdist2hull = hull.getDistancesToHull(vauid);
      ...
      // for (size_t ia = 0; ia < vauid.size(); ia++) {
      //   if(!vpflow.flag("CHULL::ENTROPIC_TEMPERATURE")) {vdist2hull[ia]=abs(vdist2hull[ia]);}
      // }

      for (size_t ia = 0, fl_size_ia = vauid.size(); ia < fl_size_ia; ia++) {
        if (!vpflow.flag("CHULL::ENTROPIC_TEMPERATURE")) {
          message << vauid[ia] << " dist2hull = " << chull::convertUnits(vdist2hull[ia], _m_) << " (meV/atom)";
        } else {
          message << vauid[ia] << " dist2hull = " << vdist2hull[ia] << " (K)";
        }
    ...
    if (vpflow.flag("CHULL::STABILITY_CRITERION")) { // CO20210201 - chull-web SS plotter //&&(!vpflow.flag("CHULL::WEB_DOC"))
      ...
      // proceed otherwise at your own risk
      try {
        vscriterion = hull.getStabilityCriterion(vauid);
      } catch (aurostd::xerror& err) {
        pflow::logger(err.whereFileName(), err.whereFunction(), err.what(), aflags, FileMESSAGE, oss, _LOGGER_ERROR_);
        if (vpflow.flag("CHULL::LOG")) {
          FileMESSAGE.close();
    ...
    if (vpflow.flag("CHULL::N+1_ENTHALPY_GAIN")) {
      ...
      // proceed otherwise at your own risk
      try {
        vn1egain = hull.getNPlus1EnthalpyGain(vauid);
      } catch (aurostd::xerror& err) {
        pflow::logger(err.whereFileName(), err.whereFunction(), err.what(), aflags, FileMESSAGE, oss, _LOGGER_ERROR_);
        if (vpflow.flag("CHULL::LOG")) {
          FileMESSAGE.close();
      ...
      for (size_t ia = 0, fl_size_ia = vauid.size(); ia < fl_size_ia; ia++) {
        if (!vpflow.flag("CHULL::ENTROPIC_TEMPERATURE")) {
          message << vauid[ia] << " n+1_enthalpy_gain = " << chull::convertUnits(vn1egain[ia], _m_) << " (meV/atom)";
        } else {
          message << vauid[ia] << " n+1_enthalpy_gain = " << vn1egain[ia] << " (K)";
        }
    ...
    if (vpflow.flag("CHULL::HULL_FORMATION_ENTHALPY")) {
      ...
      const xvector<double> coords(dimension);
      aurostd::string2tokens<double>(vpflow.getattachedscheme("CHULL::HULL_FORMATION_ENTHALPY"), _coords, ",");
      for (size_t j = 0, fl_size_j = _coords.size(); j < fl_size_j && j < dimension; j++) {
        coords[j + coords.lrows] = _coords[j];
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " coords=" << coords << endl;
      }
      ...
      // proceed otherwise at your own risk
      try {
        const ChullPoint cp(coords, FileMESSAGE, oss, hull.m_has_stoich_coords, hull.m_formation_energy_hull, false); // not a real point
        dist2hull = hull.getDistanceToHull(cp, false, true); // do not redo, get signed distance (this is energy)
        const bool should_be_positive = !hull.m_lower_hull;
        const bool correct_sign_vertical_distance = chull::correctSignVerticalDistance(dist2hull, should_be_positive);
        if (LDEBUG) {
        ...
        if (!correct_sign_vertical_distance) {
          dist2hull *= -1.0;
        }
      } catch (aurostd::xerror& err) {
        pflow::logger(err.whereFileName(), err.whereFunction(), err.what(), aflags, FileMESSAGE, oss, _LOGGER_ERROR_);
      ...
      if (vpflow.flag("CHULL::SCREEN_ONLY")) {
        const uint precision = COEF_PRECISION;
        const double roundoff_tol = 5.0 * pow(10, -((int) precision) - 1);
        ...
        message << " = ";
        if (!vpflow.flag("CHULL::ENTROPIC_TEMPERATURE")) {
          message << chull::convertUnits(dist2hull, _m_) << " (meV/atom)";
        } else {
          message << dist2hull << " (K)";
        }
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_COMPLETE_);
    ...
    if (vpflow.flag("CHULL::WEB_DOC")) {
      if (!hull.write(chull_web_ft)) {
        if (vpflow.flag("CHULL::LOG")) {
          FileMESSAGE.close();
        }
        return false; /*Krun=false;*continue;*/
      }
    } // web-specific json doc
    if (vpflow.flag("CHULL::LATEX_DOC") || vpflow.flag("CHULL::PNG_IMAGE")) {
      if (!hull.write(latex_ft)) {
        if (vpflow.flag("CHULL::LOG")) {
          FileMESSAGE.close();
        }
        return false; /*Krun=false;*continue;*/
      }
    } // latex doc, keep last as it will trip on useless plots
...
namespace chull {
  ...
  string getPath(string _path, ofstream& FileMESSAGE, ostream& oss, bool silent) { // main function
    stringstream message;
    const string pwd = getPath(false);
    const string home = XHOST.home; // aurostd::execute2string(XHOST.command("echo") + " $HOME");
    string path;
...
namespace chull {
  ...
  void flagCheck(aurostd::xoption& vpflow, const vector<string>& velements, ofstream& FileMESSAGE, ostream& oss, bool silent) { // main function
    ...
    for (size_t lib = 1, fl_size_lib = velements.size(); lib <= fl_size_lib && lib <= _AFLOW_LIB_MAX_; lib++) {
...
//     for(size_t i=0,fl_size_i=vauid.size();i<fl_size_i;i++){
...
//       found=false;
//       for(size_t j=0,fl_size_j=eq_gstates.size();j<fl_size_j;j++){
...
//     for(size_t i=0,fl_size_i=points.size();i<fl_size_i;i++){
//       if(points[i].m_is_artificial){continue;}  //they will be added again
//       found=false;
//       for(size_t j=0,fl_size_j=points_to_neglect.size();j<fl_size_j&&!found;j++){
...
//     uint i_nary,i_alloy;
//     for(size_t i=0,fl_size_i=points_to_neglect.size();i<fl_size_i;i++) {
...
namespace chull {
  ...
  double H_f_atom(const ChullPoint& point, char units) {
    if (point.m_has_entry) {
      return H_f_atom(point.m_entry, units);
    }
    if (point.m_formation_energy_coord) {
      return convertUnits(point.getLastCoord(), units);
    }
  ...
  double H_f_atom(const aflowlib::_aflowlib_entry& entry, char units) {
    const double d_tmp = entry.enthalpyFormationAtom(0); // entry.enthalpy_formation_atom - ADDING CCE @ 0K
  ...
  double T_S(const aflowlib::_aflowlib_entry& entry) {
    const double d_tmp = entry.entropic_temperature;
    if (d_tmp == AUROSTD_NAN) {
      return AUROSTD_NAN;
    }
    return d_tmp;
  ...
  double EFA(const aflowlib::_aflowlib_entry& entry, char units) {
    const double d_tmp = entry.entropy_forming_ability;
    if (d_tmp == AUROSTD_NAN) {
      return AUROSTD_NAN;
    }
    return convertUnits(d_tmp, units);
  ...
  double isoMaxLatentHeat(const aflowlib::_aflowlib_entry& entry, double x, char units) {
    if (T_S(entry) == AUROSTD_NAN) {
      return AUROSTD_NAN;
    }
    if (x <= 0 || x >= 1.0) {
      return AUROSTD_NAN;
    } // protect log()
    const double d_tmp = ((double) KBOLTZEV) * T_S(entry) * (x * log(x) + (1.0 - x) * log(1.0 - x));
    return convertUnits(d_tmp, units);
  }
  ...
  bool subspaceBelongs(const xvector<int>& space, const xvector<int>& subspace) {
    ...
    if (subspace.rows != space.rows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between space and subspace");
    }
    if (sum(subspace) > sum(space)) {
      return false;
    } // cannot be of higher dimension
    for (int i = subspace.lrows; i <= subspace.urows; i++) {
      if (subspace[i] == 1 && space[i] == 0) {
        return false;
      }
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " is relevant!" << endl;
    }
    return true;
  }

  bool correctSignVerticalDistance(double dist_2_hull, bool should_be_positive) {
    if (should_be_positive && aurostd::notPositive(dist_2_hull, true, ZERO_TOL)) {
      return false;
    }
    if (!should_be_positive && aurostd::notNegative(dist_2_hull, true, ZERO_TOL)) {
      return false;
    }
    return true;
  }
  ...
  xvector<double> getTruncatedCoords(const xvector<double>& coords, const xvector<int>& elements_present) {
    if (coords.rows != elements_present.rows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Reduction invalid, coords mismatch");
    }
    const uint h_dim = sum(elements_present);
    const xvector<double> red_coords(coords.lrows, coords.lrows + h_dim - 1);
    vector<uint> relevant_indices = getRelevantIndices(elements_present);
    for (size_t i = 0, fl_size_i = relevant_indices.size(); i < fl_size_i; i++) {
      red_coords[i + red_coords.lrows] = coords[relevant_indices[i]];
    }
    return red_coords;
  }
  ...
  vector<uint> getRelevantIndices(const xvector<int>& elements_present) {
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = relevant_indices.size(); i < fl_size_i; i++) {
        cerr << relevant_indices[i] << (i != relevant_indices.size() - 1 ? "," : "");
      }
  ...
  bool coordsIdentical(const xvector<double>& coords1, const xvector<double>& coords2) {
    return identical(coords1, coords2, ZERO_TOL);
  }
} // namespace chull
...
namespace chull {
  ...
  ChullPointLight::ChullPointLight(const ChullPointLight& b) : xStream(*b.getOFStream(), *b.getOSS()) {
    copy(b);
  } // copy PUBLIC  //upcasting is allowed, works for ChullPointLight and ChullPoint

  ChullPointLight::~ChullPointLight() {
    xStream::free();
    free();
  }
  ...
  bool ChullPointLight::operator<(const ChullPointLight& other) const {
    ...
    if (m_coords.lrows != other.m_coords.lrows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_coords.lrows!=other.m_coords.lrows");
    }
    if (m_coords.rows != other.m_coords.rows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_coords.rows!=other.m_coords.rows");
    }
    for (int i = m_coords.lrows; i <= m_coords.urows; i++) {
      if (m_coords[i] != other.m_coords[i]) {
        return (m_coords[i] < other.m_coords[i]);
      }
    }
    return false;
  }

  void ChullPointLight::clear() {
    free();
  } // clear PUBLIC
  ...
  double ChullPointLight::getLastCoord() const {
    return m_coords[m_coords.urows];
  }
  ...
  // constructor
  //--------------------------------------------------------------------------------
  ChullPoint::ChullPoint(ostream& oss, bool has_stoich_coords, bool formation_energy_coord, bool is_artificial) : ChullPointLight(oss) {
    initialize(has_stoich_coords, formation_energy_coord, is_artificial);
  }
  ChullPoint::ChullPoint(const xvector<double>& coord, ostream& oss, bool has_stoich_coords, bool formation_energy_coord, bool is_artificial) : ChullPointLight(oss) {
    initialize(coord, has_stoich_coords, formation_energy_coord, is_artificial);
  }
  ChullPoint::ChullPoint(const vector<string>& velements, const aflowlib::_aflowlib_entry& entry, ostream& oss, bool formation_energy_coord) : ChullPointLight(oss) {
    initialize(velements, entry, formation_energy_coord);
  }
  ChullPoint::ChullPoint(ofstream& FileMESSAGE, ostream& oss, bool has_stoich_coords, bool formation_energy_coord, bool is_artificial) : ChullPointLight(FileMESSAGE, oss) {
    initialize(has_stoich_coords, formation_energy_coord, is_artificial);
  }
  ChullPoint::ChullPoint(const xvector<double>& coord, ofstream& FileMESSAGE, ostream& oss, bool has_stoich_coords, bool formation_energy_coord, bool is_artificial) : ChullPointLight(FileMESSAGE, oss) {
    initialize(coord, has_stoich_coords, formation_energy_coord, is_artificial);
  }
  ChullPoint::ChullPoint(const vector<string>& velements, const aflowlib::_aflowlib_entry& entry, ofstream& FileMESSAGE, ostream& oss, bool formation_energy_coord) : ChullPointLight(FileMESSAGE, oss) {
    initialize(velements, entry, formation_energy_coord);
  }
  ChullPoint::ChullPoint(const ChullPoint& b) : xStream(*b.getOFStream(), *b.getOSS()), ChullPointLight(b) {
    copy(b);
  } // copy PUBLIC
  ...
  void ChullPoint::clear() {
    free();
  } // clear PUBLIC
  void ChullPoint::free() {
    ChullPointLight::free();
    m_entry.clear();
    if (m_entry.vsg.empty()) {
      m_entry.vsg.push_back(NOSG);
    }
    if (m_entry.vsg2.empty()) {
      m_entry.vsg2.push_back(NOSG);
    } // hack so it doesn't break with front(),back(),[0]
    s_coords.clear();
    c_coords.clear();
  ...
  bool ChullPoint::isWithinHalfHull(bool lower_hull) const {
  ...
  xvector<double> ChullPoint::getStoichiometricCoords() const {
    ...
    if (m_coords.rows == 1) { // trivial unary hull
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " m_coords.rows==1: generating null xvector" << endl;
      }
      if (!isUnary()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Found non-unary point on unary hull", _INPUT_UNKNOWN_);
      }
      const xvector<double> r_coords = aurostd::null_xv<double>(); // get null vector //(m_coords.lrows,m_coords.lrows);  //just return vector length 1 with 0 inside
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " r_coords=" << r_coords << endl;
      }
      return r_coords;
    }
    const xvector<double> r_coords(m_coords.lrows, m_coords.urows - 1);
    for (int i = m_coords.lrows; i <= m_coords.urows - 1; i++) {
      r_coords[i] = m_coords[i];
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " r_coords=" << r_coords << endl;
    }
  ...
  xvector<double> ChullPoint::getTruncatedReducedCoords(const xvector<int>& elements_present, vector_reduction_type vred) const {
    if (vred == frac_vrt || vred == no_vrt) {
      return getTruncatedSCoords(elements_present);
    } else if (vred == gcd_vrt) {
      return getTruncatedCCoords(elements_present, true);
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown reduce mode", _INPUT_UNKNOWN_);
    }
  ...
  xvector<double> ChullPoint::getTruncatedSCoords(const xvector<int>& elements_present) const {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (!m_has_stoich_coords) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Non-stoich coordinates");
    }
    const xvector<double> coords = getTruncatedCoords(s_coords, elements_present);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " truncated stoich for " << s_coords << " is " << coords << " (elements_present=" << elements_present << ")" << endl;
    }
    return coords;
  }
  ...
  xvector<double> ChullPoint::getTruncatedCCoords(const xvector<int>& elements_present, bool reduce) const {
    ...
    const xvector<double> coords = getTruncatedCoords(c_coords, elements_present);
    xvector<double> final_coords = coords; // DX20191125
    if (reduce) {
      aurostd::reduceByGCD(coords, final_coords);
    } // DX20191125 - new form of function
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " truncated " << (reduce ? "reduced" : "") << " comp for " << c_coords << " is " << final_coords << " (elements_present=" << elements_present << ")" << endl;
    } // DX20191125 - changed coords to final_coords
  ...
  xvector<double> ChullPoint::getReducedCCoords() const {
    ...
    aurostd::reduceByGCD(c_coords, coords, ZERO_TOL); // DX20191125 - new form of function
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " reduced comp for " << c_coords << " is " << coords << endl;
    }
    return coords;
  }

  uint ChullPoint::loadXstructures(bool relaxed_only) { // load relaxed only
    if (m_entry.prototype.find("POCC") != string::npos) {
      return false;
    } // POCC entries have no composition
    if (!pflow::loadXstructures(m_entry, *p_FileMESSAGE, *p_oss, relaxed_only)) {
      return false;
    }
  ...
  bool ChullPoint::getMostRelaxedXstructure(xstructure& xstr) const { // this is const!
    ...
    if (pflow::loadXstructureLibEntry(m_entry, xstr)) {
      return true;
    }
    aflowlib::_aflowlib_entry entry;
    entry.auid = m_entry.auid;
    entry.aurl = m_entry.aurl;
    entry.vspecies = m_entry.vspecies; // fast copy  //CO20221110 - pass vspecies here to populate xstructure
    if (!pflow::loadXstructures(entry, *p_FileMESSAGE, *p_oss, true)) {
      return false;
    }
  ...
  uint ChullPoint::getDim() const {
    return m_coords.rows;
  }
  bool ChullPoint::isUnary() const {
    return m_i_nary == 0;
  }
  double ChullPoint::getFormationEnthalpy() const {
    return H_f_atom(*this, _std_);
  } // m_entry.enthalpy_formation_atom;
  double ChullPoint::getEntropicTemperature() const {
    return T_S(*this);
  } // m_entry.entropic_temperature;
  double ChullPoint::getEntropyFormingAbility() const {
    return EFA(*this, _std_);
  } // m_entry.entropic_temperature;
  const vector<string>& ChullPoint::getVSG() const {
    return m_entry.vsg2;
  }
  const string& ChullPoint::getSG() const {
    return getVSG().back();
  } // tight tolerance fine!  //vsg === LOOSE //vsg2 === TIGHT // doesn't make sense
  double ChullPoint::getDist2Hull(char units) const {
    if (m_dist_2_hull == AUROSTD_MAX_DOUBLE) {
      return AUROSTD_MAX_DOUBLE;
    }
    if (m_formation_energy_coord) {
      return convertUnits(m_dist_2_hull, units);
    } else {
      return m_dist_2_hull;
    } // no unit conversions coded yet here
  }
  double ChullPoint::getStabilityCriterion(char units) const {
    if (m_stability_criterion == AUROSTD_MAX_DOUBLE) {
      return AUROSTD_MAX_DOUBLE;
    }
    if (m_formation_energy_coord) {
      return convertUnits(m_stability_criterion, units);
    } else {
      return m_stability_criterion;
    } // no unit conversions coded yet here
  }
  double ChullPoint::getRelativeStabilityCriterion() const {
    ...
    if (aurostd::identical(getLastCoord(), 0.0, ZERO_TOL)) {
      return 0.0;
    }
    if (LDEBUG) {
    ...
    return std::abs(m_stability_criterion / getLastCoord()); // abs() because they are generally opposite signs //delivers as decimal, show as percentage
  }
  double ChullPoint::getNPlus1EnthalpyGain(char units) const {
    if (m_n_plus_1_enthalpy_gain == AUROSTD_MAX_DOUBLE) {
      return AUROSTD_MAX_DOUBLE;
    }
    if (m_formation_energy_coord) {
      return convertUnits(m_n_plus_1_enthalpy_gain, units);
    } else {
      return m_n_plus_1_enthalpy_gain;
    } // no unit conversions coded yet here
  }
  double ChullPoint::getEntropyStabilizationCoefficient(char units) const {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (getDist2Hull() == AUROSTD_MAX_DOUBLE) {
      return AUROSTD_MAX_DOUBLE;
    }
    if (getEntropyFormingAbility() == AUROSTD_NAN) {
      return AUROSTD_MAX_DOUBLE;
    }
    if (aurostd::zeroWithinTol(getEntropyFormingAbility(), ZERO_TOL)) {
      return 0.0;
  ...
  bool ChullPoint::entryIdentical(const aflowlib::_aflowlib_entry& other) const {
    return (m_entry.compound == other.compound) && (m_entry.prototype == other.prototype) && // believe it or not, compound + prototype is PROBABLY enough, but let's be sure
  ...
  void ChullPoint::addEntry(const aflowlib::_aflowlib_entry& entry) {
    m_entry = entry;
    if (CORRECT_BAD_DATABASE) {
      m_entry.correctBadDatabase(*p_FileMESSAGE, true, *p_oss);
    } // verbose //carried over from apennsy (SC)
  }
  ...
  void ChullPoint::setGenCoords(const xvector<double>& coord, bool formation_energy_coord) {
    ...
    m_formation_energy_coord = formation_energy_coord;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_within_half_hull=" << isWithinHalfHull(m_formation_energy_coord) << endl;
    } // m_formation_energy_coord===lower_hull
  }
  ...
  void ChullPoint::setGenCoords(const vector<string>& velements, const aflowlib::_aflowlib_entry& entry, bool formation_energy_coord) {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (entry.vcomposition.empty() && entry.vstoichiometry.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No vcomposition or vstoichiometry found for entry.auid=" + entry.auid, _RUNTIME_ERROR_);
    }
    const xvector<double> coord(velements.size());
    bool found = false; // not necessary, we already check in entryValid()
    double c_sum = 0.0;
    if (!entry.vcomposition.empty()) {
      for (size_t i = 0, fl_size_i = entry.vcomposition.size(); i < fl_size_i; i++) {
        c_sum += entry.vcomposition[i];
      } // derive stoich exactly!
      if (c_sum == 0) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "c_sum==0 (entry.auid=" + entry.auid + ",entry.aurl=" + entry.aurl + ")", _RUNTIME_ERROR_);
      }
      for (size_t i = 0, fl_size_i = velements.size(); i < fl_size_i - 1; i++) {
        found = false;
        for (size_t j = 0, fl_size_j = entry.vspecies.size(); j < fl_size_j && !found; j++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " converting stoich[aurl=" << entry.aurl << "]=" << aurostd::joinWDelimiter(aurostd::vecDouble2vecString(entry.vstoichiometry), ",") << " to fractions" << endl;
      }
      for (size_t i = 0, fl_size_i = entry.vstoichiometry.size(); i < fl_size_i; i++) { // derive stoich exactly!
        aurostd::double2fraction(entry.vstoichiometry[i], numerator, denominator,
                                 DEFAULT_POCC_STOICH_TOL); // ZERO_TOL*10 is preferred, but due to buy in GetStoich(), use DEFAULT_POCC_STOICH_TOL instead (for now) //ZERO_TOL=1e-8, make slightly looser than what's written to aflowlib.out
      ...
      if (!aurostd::identical(c_sum, 1.0, ZERO_TOL)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "c_sum!=1 (entry.auid=" + entry.auid + ",entry.aurl=" + entry.aurl + ")", _RUNTIME_ERROR_);
      }
      for (size_t i = 0, fl_size_i = velements.size(); i < fl_size_i - 1; i++) {
        found = false;
        for (size_t j = 0, fl_size_j = entry.vspecies.size(); j < fl_size_j && !found; j++) {
    ...
    if (formation_energy_coord) {
      coord[coord.urows] = H_f_atom(entry);
    } // entry.enthalpy_formation_atom
    else {
      coord[coord.urows] = entry.entropic_temperature;
    } // entropic temperature is positive for stable compounds (we want upper half convex-hull)
    setGenCoords(coord, formation_energy_coord);
  }
  ...
  vector<uint> ChullPoint::getRelevantIndices(const xvector<int>& elements_present) const {
    ...
    if (elements_present.rows != s_coords.rows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between point and elements_present");
    }
    for (int i = elements_present.lrows; i <= elements_present.urows; i++) {
  ...
  void ChullPoint::setStoichCoords() {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (!m_has_stoich_coords) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Non-stoich coordinates: aurl=" + m_entry.aurl);
    }
    double c_sum = 0.0; // concentration sum
    const xvector<double> stoich(m_coords.urows, m_coords.lrows);
    const xvector<int> elements_present(m_coords.urows, m_coords.lrows);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_coords=" << m_coords << endl;
    }
    for (int j = m_coords.lrows; j <= m_coords.urows - 1; j++) {
      if (std::signbit(m_coords[j])) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Negative stoich coordinate found: aurl=" + m_entry.aurl);
      } // no negative numbers in stoich coordinates, only energy
      stoich[j] = m_coords[j];
      if (aurostd::nonZeroWithinTol(m_coords[j], ZERO_TOL)) {
        elements_present[j] = 1;
      }
      c_sum += m_coords[j];
    }
    ...
    if (std::signbit(stoich[stoich.urows])) {
      ...
      if (aurostd::zeroWithinTol(stoich[stoich.urows], ZERO_TOL)) {
    ...
    if (aurostd::nonZeroWithinTol(stoich[stoich.urows], ZERO_TOL)) {
      elements_present[elements_present.urows] = 1;
    } // check if nary++
    s_coords = stoich;
    c_coords = s_coords;
    if (m_has_entry) {
      c_sum = 0.0;
      for (size_t i = 0, fl_size_i = m_entry.vcomposition.size(); i < fl_size_i; i++) {
        c_sum += m_entry.vcomposition[i];
      } // essentially UNDO s_coords calculation
      c_coords *= c_sum;
    }
  ...
  void ChullPoint::setHullCoords() {
    setHullCoords(m_coords);
  } // default to m_coords
  void ChullPoint::setHullCoords(const xvector<double>& coords) {
    h_coords = coords;
  }
  void ChullPoint::setHullCoords(const xvector<int>& elements_present) {
    ...
    const xvector<double> coords = chull::getTruncatedCoords(m_coords, elements_present);
    // vector<uint> relevant_indices=getRelevantIndices(elements_present);
    // for(size_t i=0,fl_size_i=relevant_indices.size();i<fl_size_i;i++){coords[i+coords.lrows]=m_coords[relevant_indices[i]];}
    coords[coords.urows] = getLastCoord(); // overwrite last coord appropriately
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " setting h_coords=" << coords << endl;
    }
  ...
  void ChullPoint::reduceCoords(const xvector<int>& elements_present) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_coords(pre )=" << m_coords << endl;
    }
    const xvector<double> coords = chull::getTruncatedCoords(m_coords, elements_present);
    coords[coords.urows] = getLastCoord(); // overwrite last coord appropriately
    initializeCoords(coords, m_formation_energy_coord);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_coords(post)=" << m_coords << endl;
    }
    if (m_has_stoich_coords) {
      setStoichCoords();
    }
  ...
  void ChullPoint::cleanPointForHullCalc() {
    h_coords.clear();
  }
...
namespace chull {
  ...
  FacetPoint::FacetPoint(const ChullPointLight& point, uint index) {
    initialize(point, index);
  } // need BOTH point and index, otherwise, just use point/index independently
  FacetPoint::FacetPoint(const FacetPoint& b) {
    copy(b);
  } // copy PUBLIC
  FacetPoint::~FacetPoint() {
    free();
  ...
  bool FacetPoint::operator<(const FacetPoint& other) const {
    return ch_index < other.ch_index;
  }
  void FacetPoint::clear() {
    const FacetPoint a;
    copy(a);
  } // clear PUBLIC
...
namespace chull {
  ...
  bool sortThermoPoints::operator()(const ChullPointLight& ci, const ChullPointLight& cj) const { // upcasting is allowed, works for ChullPointLight and ChullPoint
    if (!(ci.m_initialized && cj.m_initialized)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Points not initialized");
    }
    ...
    // }
    if (ci.m_coords.rows != cj.m_coords.rows) {
  ...
  //[CO20221111 - VERY SLOW]}
  _aflowlib_entry_LIB2sorting::_aflowlib_entry_LIB2sorting(aflowlib::_aflowlib_entry& entry, uint index, vector<string>& velements_chull, ofstream& FileMESSAGE, ostream& oss) :
  ...
  _aflowlib_entry_LIB2sorting::~_aflowlib_entry_LIB2sorting() {
    xStream::free();
    m_species_AURL.clear();
  }
  bool _aflowlib_entry_LIB2sorting::operator<(const _aflowlib_entry_LIB2sorting& other) const {
    ...
    if (m_catalog == "LIB2" && m_nspecies == 1 && other.m_nspecies == 1) {
      ...
      for (k = 0; k < m_species_AURL.size() && i_within_hull; k++) {
        if (!aurostd::WithinList(m_velements_chull, m_species_AURL[k])) {
          i_within_hull = false;
        }
      }
      bool j_within_hull = true;
      for (k = 0; k < other.m_species_AURL.size() && j_within_hull; k++) {
        if (!aurostd::WithinList(m_velements_chull, other.m_species_AURL[k])) {
          j_within_hull = false;
        }
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " i_within_hull=" << i_within_hull << " j_within_hull=" << j_within_hull << endl;
      }
      if (i_within_hull != j_within_hull) {
        return i_within_hull;
      }
      for (k = 0; k < m_species_AURL.size(); k++) {
...
namespace chull {
  ...
  // constructor
  //--------------------------------------------------------------------------------
  ChullFacet::ChullFacet(ostream& oss) : xStream(oss), m_initialized(false) {
    free();
  }
  ChullFacet::ChullFacet(ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    free();
  }
  ChullFacet::ChullFacet(const ChullFacet& b) : xStream(*b.getOFStream(), *b.getOSS()) {
    copy(b);
  } // copy PUBLIC
  ...
  bool ChullFacet::operator<(const ChullFacet& other) const {
    if (m_vertices.size() != other.m_vertices.size()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between facet points");
    }

    // simply sort by m_point (indices)
    for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
      if (!(m_vertices[i].m_initialized && other.m_vertices[i].m_initialized)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized facetpoint");
      }
      if (m_vertices[i].ch_index != other.m_vertices[i].ch_index) {
        return m_vertices[i].ch_index < other.m_vertices[i].ch_index;
      }
    }
    return false;
  }

  void ChullFacet::clear() {
    const ChullFacet a;
    copy(a);
  } // clear PRIVATE
  ...
  void ChullFacet::copy(const ChullFacet& b) { // copy PRIVATE
    ...
    m_initialized = b.m_initialized;
    m_vertices.clear();
    for (size_t i = 0, fl_size_i = b.m_vertices.size(); i < fl_size_i; i++) {
      m_vertices.push_back(b.m_vertices[i]);
    }
    m_dim = b.m_dim;
    m_has_stoich_coords = b.m_has_stoich_coords;
    m_formation_energy_coord = b.m_formation_energy_coord;
    ...
    m_directive_vectors.clear();
    for (size_t i = 0, fl_size_i = b.m_directive_vectors.size(); i < fl_size_i; i++) {
      m_directive_vectors.push_back(b.m_directive_vectors[i]);
    }
    m_normal = b.m_normal;
    m_offset = b.m_offset;
    ...
    f_outside_set.clear();
    for (size_t i = 0, fl_size_i = b.f_outside_set.size(); i < fl_size_i; i++) {
      f_outside_set.push_back(b.f_outside_set[i]);
    }
    f_furthest_point = b.f_furthest_point;
    m_ridges.clear();
    for (size_t i = 0, fl_size_i = b.m_ridges.size(); i < fl_size_i; i++) {
      m_ridges.push_back(b.m_ridges[i]);
    }
    f_neighbors.clear();
    for (size_t i = 0, fl_size_i = b.f_neighbors.size(); i < fl_size_i; i++) {
      f_neighbors.push_back(b.f_neighbors[i]);
    }
  }
  ...
  bool ChullFacet::shareRidge(const ChullFacet& other) const {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    bool match;
    for (size_t i = 0, fl_size_i = m_ridges.size(); i < fl_size_i; i++) {
      const vector<uint>& ridge_indices1 = m_ridges[i].getCHIndices();
      for (size_t j = 0, fl_size_j = other.m_ridges.size(); j < fl_size_j; j++) {
        const vector<uint>& ridge_indices2 = other.m_ridges[j].getCHIndices();
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " comparing ";
          for (size_t ri = 0, fl_size_ri = ridge_indices1.size(); ri < fl_size_ri; ri++) {
            cerr << ridge_indices1[ri] << " ";
          }
          cerr << " vs. ";
          for (size_t rj = 0, fl_size_rj = ridge_indices1.size(); rj < fl_size_rj; rj++) {
            cerr << ridge_indices2[rj] << " ";
          }
        }
        match = (ridge_indices1 == ridge_indices2);
        if (LDEBUG) {
          cerr << (match ? "MATCH" : "NO") << endl;
        }
        if (match) {
          return true;
        }
  ...
  bool ChullFacet::isPointOnFacet(uint i_point) const {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (m_vertices.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Facet has no vertices");
    }
    // if(!m_initialized){throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"Uninitialized facet");}
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " checking if point[" << i_point << "] is on this facet" << endl;
    }
    for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " m_vertices[i].ch_index==" << m_vertices[i].ch_index << " ?= i_point==" << i_point << endl;
      }
      if (m_vertices[i].ch_index == i_point) {
        return true;
      }
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " point[" << i_point << "] is not on this facet" << endl;
    }
    return false;
  }

  bool ChullFacet::isPointOutside(const FacetPoint& f_point) const {
    if (!f_point.m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized facetpoint");
    }
    if (isPointOnFacet(f_point)) {
      return false;
    }
  ...
  bool ChullFacet::isPointOutside(const ChullPointLight& point) const {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (!m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Facet not initialized");
    }
    const double dist_point = getSignedPointPlaneDistance(point);
    ...
    if (aurostd::zeroWithinTol(dist_point, ZERO_TOL)) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " point appears to be right on top of facet" << endl;
      }
      return false;
    }
    is_outside = (std::signbit(dist_point));
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " sign of distance indicates point is " << (is_outside ? "OUTSIDE" : "INSIDE") << " hull" << endl;
    }
  ...
  double ChullFacet::getSignedPointPlaneDistance(const ChullPointLight& point) const {
    return getSignedPointPlaneDistance(point.h_coords);
  }
  double ChullFacet::getSignedPointPlaneDistance(const xvector<double>& point) const {
    if (!m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Facet not initialized");
    }
    if (point.rows != m_vertices[0].ch_point.h_coords.rows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between point and facet");
    }
    const xvector<double> diff = point - m_vertices[0].ch_point.h_coords;
  ...
  double ChullFacet::getSignedVerticalDistanceToZero(const xvector<double>& point) const {
    ...
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (!m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized facet");
    }
    ...
    for (int i = point.lrows; i <= point.urows - 1; i++) {
      dist -= m_normal[i] * point[i];
    } // note that we don't go to rows (because we assume it is zero)
  ...
  vector<uint> ChullFacet::getCHIndices() const {
    if (m_vertices.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No vertices found");
    }
    vector<uint> vi;
    for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
      vi.push_back(m_vertices[i].ch_index);
    }
    return vi;
  }
  ...
  void ChullFacet::addVertex(const ChullPointLight& point, uint index) {
    const FacetPoint fp(point, index);
    return addVertex(fp);
  } // no need for full copy
  ...
  void ChullFacet::initialize(const xvector<double>& ref, uint h_dim, bool check_validity) {
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
        cerr << "    " << m_vertices[i].ch_point.h_coords << endl;
      }
    }
    m_dim = h_dim;
  ...
  bool ChullFacet::hasValidPoints(string& error) {
    ...
    error.clear();
    if (m_vertices.empty()) {
      error = "Facet has no defining points";
      return false;
    }
    for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
      if (!m_vertices[i].m_initialized) {
        error = "Uninitialized facetpoint";
      }
    }
    if ((uint) m_vertices[0].ch_point.h_coords.rows != m_vertices.size()) {
      ...
      error = "Dimension mismatch among points and coordinates";
      return false;
    }
    for (size_t i = 1, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
      if (LDEBUG) {
      ...
      if (m_vertices[i].ch_point.h_coords.rows != m_vertices[0].ch_point.h_coords.rows) {
        error = "Dimension mismatch among facet points";
        return false;
      }
    }
    for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
      for (size_t j = i + 1, fl_size_j = m_vertices.size(); j < fl_size_j; j++) {
        if (m_vertices[i].ch_index == m_vertices[j].ch_index) {
          error = "Facet points are degenerate";
          return false;
        }
      }
    }
    if (m_vertices.size() != (uint) m_vertices[0].ch_point.h_coords.rows) {
      error = "Facet has wrong number of defining points given dimension";
      return false;
    }
    return true;
  }
  ...
  void ChullFacet::setContent() {
    ...
    string error;
    if (!hasValidPoints(error)) {
      return;
    }
    m_content = 0.0; // if we don't set later, it's because it's really zero
    const xmatrix<double> B(m_vertices.size(), m_vertices.size(), 1, 1); // for determinant
    for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
      for (size_t k = 0, fl_size_k = m_vertices.size(); k < fl_size_k; k++) {
        if (LDEBUG) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " B=" << endl;
      cerr << B << endl;
    }
    const double CMdetB = aurostd::CMdet(B);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_has_stoich_coords=" << std::boolalpha << m_has_stoich_coords << ", m_formation_energy_coord=" << std::boolalpha << m_formation_energy_coord << endl;
    }
    ...
    double tol = ZERO_TOL;
    if ((m_dim < 4) && m_has_stoich_coords && m_formation_energy_coord) {
      tol = ZERO_COEF_TOL;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " CMdet(B)=" << CMdetB << endl;
    }
    ...
    if (aurostd::zeroWithinTol(CMdetB, tol)) {
      return;
    } // error="CMdet(B) is zero, shows hyper-collinearity";
    const double j = m_vertices.size() - 1; // two vertices == line segment == 1-simplex
    const double coef = pow(-1.0, j + 1.0) / (pow(2.0, j) * pow(aurostd::factorial(j), 2));
    if (LDEBUG) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " content=" << m_content << endl;
    }
    if (aurostd::zeroWithinTol(m_content, tol)) {
      return;
    } // error="simplex content is zero, shows hyper-collinearity";
    m_is_hypercollinear = false;
  }
  ...
  void ChullFacet::setDirectiveVectors(bool check_validity) { // perhaps we already checked...
    m_directive_vectors.clear();
    string error;
    if (check_validity && !hasValidPoints(error)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, error);
    }
  ...
  bool ChullFacet::pointsMatchDirectiveVectors(string& error) {
    error.clear();
    // now we can define
    if (m_directive_vectors.size() != m_vertices.size() - 1) {
      error = "Dimension mismatch among directive vectors and points";
      return false;
    }
    if ((uint) m_directive_vectors[0].rows != m_vertices.size()) {
      error = "Dimension mismatch among directive vectors coordinates and points";
      return false;
    }
    return true;
  }
  ...
  bool ChullFacet::hasValidDirectiveVectors(string& error) {
    error.clear();
    if (m_directive_vectors.empty()) {
      error = "No directive vectors found";
      return false;
    }
    for (size_t i = 1, fl_size_i = m_directive_vectors.size(); i < fl_size_i; i++) {
      if (m_directive_vectors[i].rows != m_directive_vectors[0].rows) {
        error = "Dimension mismatch among directive vectors coordinates";
        return false;
      }
    }
    for (size_t i = 0, fl_size_i = m_directive_vectors.size(); i < fl_size_i; i++) {
      if (aurostd::zeroWithinTol(modulus(m_directive_vectors[i]), ZERO_TOL)) {
        error = "Ill-defined directive vectors";
        return false;
      }
    }
    return true;
  ...
  bool ChullFacet::hasCollinearVectors(bool check_validity) { // perhaps we already checked...
    string error;
    if (check_validity && !pointsMatchDirectiveVectors(error)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, error);
    }
    if (check_validity && !hasValidDirectiveVectors(error)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, error);
    }
    for (size_t i = 0, fl_size_i = m_directive_vectors.size(); i < (fl_size_i - 1); i++) {
      for (size_t j = i + 1, fl_size_j = m_directive_vectors.size(); j < fl_size_j; j++) {
        if (aurostd::isCollinear(m_directive_vectors[i], m_directive_vectors[j], ZERO_TOL)) {
          return true;
        }
      }
    }
  ...
  bool ChullFacet::isValid(string& error) {
    ...
    if (!pointsMatchDirectiveVectors(error)) {
      return false;
    }
    if (!hasValidDirectiveVectors(error)) {
      return false;
    } // we need to preserve neighbors, so do not kill these pseudo-facets
    if (false && hasCollinearVectors(false)) { // we need to preserve neighbors, so do not kill these pseudo-facets
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " shows collinarity" << endl;
      }
      error = "Directive vectors are collinear";
      return false;
    }
    return true;
  }
  ...
  void ChullFacet::setNormal(bool check_validity) {
    ...
    m_normal.clear();
    string error;
    if (check_validity && !isValid(error)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, error);
    }
    if (LDEBUG) {
      for (size_t i = 0, fl_size_i = m_directive_vectors.size(); i < fl_size_i; i++) {
    ...
    if (m_directive_vectors.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No directive vectors calculated");
    }
    m_normal = aurostd::getGeneralNormal(m_directive_vectors);
    if (aurostd::zeroWithinTol(aurostd::modulus(m_normal), ZERO_TOL)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid normal calculated");
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " normal=" << m_normal << endl;
    }
  ...
  void ChullFacet::setOffset() {
    if (m_vertices.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Facet has not been defined");
    }
    const xvector<double>& plane_point = m_vertices[0].ch_point.h_coords;
    if (m_normal.rows != plane_point.rows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between normal and point");
    }
  ...
  void ChullFacet::setCentroid() {
    ...
    for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
      points.push_back(m_vertices[i].ch_point.h_coords);
    }
    m_facet_centroid = aurostd::getCentroid(points);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " centroid: " << m_facet_centroid << endl;
    }
  }
  ...
  void ChullFacet::setVertical() {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " looking if vertical hull: normal=" << m_normal << endl;
    }
    const double tol = (m_has_stoich_coords && m_formation_energy_coord ? ZERO_COEF_TOL : ZERO_TOL); // 1e-4 if stoich coords
  ...
  void ChullFacet::setArtificial() { // in half hulls, this finds the facet of all artificial points
    for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
      if (!m_vertices[i].ch_point.m_is_artificial) {
        m_is_artificial = false;
        return;
      }
    }
  ...
  void ChullFacet::alignNormalInward() {
    ...
    const double dist = scalar_product(m_normal, m_hull_reference) + m_offset; // point-plane distance formula without normalization
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_normal=" << m_normal << endl;
      cerr << __AFLOW_FUNC__ << " h_reference=" << m_hull_reference << endl;
      cerr << __AFLOW_FUNC__ << " dist=" << dist << endl;
    }
    const bool negate = std::signbit(dist);
    if (negate) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " NEGATING" << endl;
      }
      m_normal = -m_normal;
      m_offset = -m_offset; // flips sign of offset
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " aligned normal=" << m_normal << endl;
    }
  }
  ...
  void ChullFacet::setFurthestPoint() {
    ...
    if (f_outside_set.empty()) {
      return;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " getting furthest point for facet with normal " << m_normal << " (is_artificial=" << m_is_artificial << ")" << endl;
    }
    double dist = AUROSTD_MAX_DOUBLE;
    double max_dist = 0;
    for (size_t i = 0, fl_size_i = f_outside_set.size(); i < fl_size_i; i++) {
      dist = std::abs(getSignedPointPlaneDistance(f_outside_set[i].ch_point.h_coords)); // we only care about magnitude here
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " point=" << f_outside_set[i].ch_point.h_coords << " is " << dist << " from facet" << endl;
      }
      if (dist > max_dist) {
        max_dist = dist;
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = m_vertices.size(); i < fl_size_i; i++) {
        cerr << "       point[" << m_vertices[i].ch_index << "]=" << m_vertices[i].ch_point.h_coords << endl;
      }
  ...
  void ChullFacet::setRidges() {
    ...
    for (size_t fl_size_i = m_vertices.size(), i = (fl_size_i - 1); i < fl_size_i; i--) { // wrap around to HUGE number
      m_ridges.emplace_back(*p_FileMESSAGE, *p_oss); // CO20180305
      for (size_t j = 0, fl_size_j = m_vertices.size(); j < fl_size_j; j++) {
        if (i != j) {
          m_ridges.back().addVertex(m_vertices[j]);
        }
      }
      if (m_ridges.back().m_vertices.size() != m_dim - 1) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Ridge vertex count and facet dimension mismatch");
      }
    ...
    if (LDEBUG) {
      for (size_t i = 0, fl_size_i = m_ridges.size(); i < fl_size_i; i++) {
        ...
        for (size_t j = 0, fl_size_j = m_ridges[i].m_vertices.size(); j < fl_size_j; j++) {
...
namespace chull {
  ...
  CoordGroup::CoordGroup(const xvector<double>& coord, bool has_stoich_coords) {
    initialize(coord, has_stoich_coords);
  }
  CoordGroup::CoordGroup(const CoordGroup& b) {
    copy(b);
  } // copy PUBLIC
  CoordGroup::~CoordGroup() {
    free();
  ...
  bool CoordGroup::operator<(const CoordGroup& other) const {
    ...
    } // return (m_coords.rows<other.m_coords.rows);
    for (int i = m_coords.lrows; i <= m_coords.urows; i++) {
      if (m_coords(i) != other.m_coords(i)) {
        return (m_coords(i) < other.m_coords(i));
      }
  ...
  void CoordGroup::copy(const CoordGroup& b) {
    ...
    m_coords = b.m_coords;
    m_points.clear();
    for (size_t i = 0, fl_size_i = b.m_points.size(); i < fl_size_i; i++) {
      m_points.push_back(b.m_points[i]);
    }
    m_has_stoich_coords = b.m_has_stoich_coords;
    ...
    m_candidate_hull_points.clear();
    for (size_t i = 0, fl_size_i = b.m_candidate_hull_points.size(); i < fl_size_i; i++) {
      m_candidate_hull_points.push_back(b.m_candidate_hull_points[i]);
    }
    m_i_nary = b.m_i_nary;
    m_i_alloy = b.m_i_alloy;
    ...
    m_decomp_phases.clear();
    for (size_t i = 0, fl_size_i = b.m_decomp_phases.size(); i < fl_size_i; i++) {
      m_decomp_phases.push_back(b.m_decomp_phases[i]);
    }
    m_decomp_coefs = b.m_decomp_coefs;
    for (size_t i = 0, fl_size_i = m_equilibrium_phases.size(); i < fl_size_i; i++) {
      m_equilibrium_phases.clear();
    }
    m_equilibrium_phases.clear();
    for (size_t i = 0; i < b.m_equilibrium_phases.size(); i++) {
      m_equilibrium_phases.push_back(b.m_equilibrium_phases[i]);
    }
    m_calculated_equivalent_g_states = b.m_calculated_equivalent_g_states;
    m_equivalent_g_states.clear();
    for (size_t i = 0, fl_size_i = b.m_equivalent_g_states.size(); i < fl_size_i; i++) {
      m_equivalent_g_states.push_back(b.m_equivalent_g_states[i]);
    }
    m_sym_equivalent_g_states.clear();
    for (size_t i = 0, fl_size_i = b.m_sym_equivalent_g_states.size(); i < fl_size_i; i++) {
      m_sym_equivalent_g_states.push_back(b.m_sym_equivalent_g_states[i]);
    }
    m_stability_criterion = b.m_stability_criterion;
    m_n_plus_1_enthalpy_gain = b.m_n_plus_1_enthalpy_gain;
  ...
  xvector<int> CoordGroup::getElementsPresent() const {
    ...
    for (int i = m_coords.lrows; i <= m_coords.urows; i++) {
      if (std::signbit(m_coords[i]) || m_coords[i] > 1.0) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " hid_dim=" << hid_dim << endl;
    }
    if (std::signbit(hid_dim) || hid_dim > 1.0) {
      if (aurostd::zeroWithinTol(hid_dim, ZERO_TOL)) {
        hid_dim = 0.0;
      } // only zero out for the last coord, as it's derived from the subtraction of the others
      else {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coord(" + aurostd::utype2string(m_coords.rows) + ") is outside of [0,1] range of a generalized stoichiometry coordinate");
      }
    }

    const xvector<int> elements_present(m_coords.lrows, m_coords.urows + 1);
    for (int i = m_coords.lrows; i <= m_coords.urows; i++) {
      if (aurostd::nonZeroWithinTol(m_coords[i], ZERO_TOL)) {
        elements_present[i] = 1;
      }
    }
    if (aurostd::nonZeroWithinTol(hid_dim, ZERO_TOL)) {
      elements_present[elements_present.urows] = 1;
    }
...
namespace chull {
  ...
  Alloy::Alloy(const xvector<int>& elements_present) {
    initialize(elements_present);
  }
  Alloy::Alloy(const Alloy& b) {
    copy(b);
  }
  Alloy::~Alloy() {
    free();
  ...
  bool Alloy::operator<(const Alloy& other) const {
    // safety, so it doesn't break, but it's outside scope of function
    if (m_elements_present.rows != other.m_elements_present.rows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between alloys");
    } // return (m_elements_present.rows<other.m_elements_present.rows);
    for (int i = m_elements_present.lrows; i <= m_elements_present.urows; i++) {
      if (m_elements_present[i] != other.m_elements_present[i]) {
        return (m_elements_present[i] < other.m_elements_present[i]);
      }
    }
    return false;
  }
  ...
  void Alloy::copy(const Alloy& b) {
    ...
    m_dim = b.m_dim;
    m_coord_groups.clear();
    for (size_t i = 0, fl_size_i = b.m_coord_groups.size(); i < fl_size_i; i++) {
      m_coord_groups.push_back(b.m_coord_groups[i]);
    }
    m_facets.clear();
    for (size_t i = 0, fl_size_i = b.m_facets.size(); i < fl_size_i; i++) {
      m_facets.push_back(b.m_facets[i]);
    }
  }
  ...
  bool Alloy::belongs2Hull(const xvector<int>& elements_present_hull) const {
    return subspaceBelongs(elements_present_hull, m_elements_present);
  }
} // namespace chull
...
namespace chull {
  ...
  bool Nary::operator<(const Nary& other) const {
    return (nary < other.nary);
  }
  ...
  void Nary::copy(const Nary& b) {
    ...
    nary = b.nary;
    m_alloys.clear();
    for (size_t i = 0, fl_size_i = b.m_alloys.size(); i < fl_size_i; i++) {
      m_alloys.push_back(b.m_alloys[i]);
    }
  }
...
namespace chull {
  ...
  // constructor
  //--------------------------------------------------------------------------------
  ConvexHull::ConvexHull(ostream& oss) : xStream(oss), m_initialized(false) {
    initialize();
  }
  ConvexHull::ConvexHull(const string& alloy, ostream& oss) : xStream(oss), m_initialized(false) {
    initialize(alloy);
  }
  ConvexHull::ConvexHull(const vector<string>& velements, ostream& oss) : xStream(oss), m_initialized(false) {
    initialize(velements);
  }
  ConvexHull::ConvexHull(const vector<string>& velements, const vector<vector<vector<aflowlib::_aflowlib_entry>>>& entries, ostream& oss) : xStream(oss), m_initialized(false) {
    initialize(velements, entries);
  }
  ConvexHull::ConvexHull(const vector<xvector<double>>& vcoords, ostream& oss, bool has_stoich_coords, bool formation_energy_hull, bool add_artificial_unaries) : xStream(oss), m_initialized(false) {
    initialize(vcoords, has_stoich_coords, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const vector<ChullPoint>& vpoints, ostream& oss, bool formation_energy_hull, bool add_artificial_unaries) : xStream(oss), m_initialized(false) {
    initialize(vpoints, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const vector<ChullPoint>& vpoints, const vector<string>& velements, ostream& oss, bool formation_energy_hull, bool add_artificial_unaries) : xStream(oss), m_initialized(false) {
    initialize(vpoints, velements, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize();
  }
  ConvexHull::ConvexHull(const string& alloy, ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(alloy);
  }
  ConvexHull::ConvexHull(const vector<string>& velements, ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(velements);
  }
  ConvexHull::ConvexHull(const vector<string>& velements, const vector<vector<vector<aflowlib::_aflowlib_entry>>>& entries, ofstream& FileMESSAGE, ostream& oss) :
      xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(velements, entries);
  }
  ConvexHull::ConvexHull(const vector<xvector<double>>& vcoords, ofstream& FileMESSAGE, ostream& oss, bool has_stoich_coords, bool formation_energy_hull, bool add_artificial_unaries) :
      xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vcoords, has_stoich_coords, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const vector<ChullPoint>& vpoints, ofstream& FileMESSAGE, ostream& oss, bool formation_energy_hull, bool add_artificial_unaries) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vpoints, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const vector<ChullPoint>& vpoints, const vector<string>& velements, ofstream& FileMESSAGE, ostream& oss, bool formation_energy_hull, bool add_artificial_unaries) :
      xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vpoints, velements, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, ostream& oss) : xStream(oss), m_initialized(false) {
    initialize(vpflow);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const string& alloy, ostream& oss) : xStream(oss), m_initialized(false) {
    initialize(vpflow, alloy);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<string>& velements, ostream& oss) : xStream(oss), m_initialized(false) {
    initialize(vpflow, velements);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<string>& velements, const vector<vector<vector<aflowlib::_aflowlib_entry>>>& entries, ostream& oss) : xStream(oss), m_initialized(false) {
    initialize(vpflow, velements, entries);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<xvector<double>>& vcoords, ostream& oss, bool has_stoich_coords, bool formation_energy_hull, bool add_artificial_unaries) :
      xStream(oss), m_initialized(false) {
    initialize(vpflow, vcoords, has_stoich_coords, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<ChullPoint>& vpoints, ostream& oss, bool formation_energy_hull, bool add_artificial_unaries) : xStream(oss), m_initialized(false) {
    initialize(vpflow, vpoints, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<ChullPoint>& vpoints, const vector<string>& velements, ostream& oss, bool formation_energy_hull, bool add_artificial_unaries) :
      xStream(oss), m_initialized(false) {
    initialize(vpflow, vpoints, velements, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vpflow);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const string& alloy, ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vpflow, alloy);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<string>& velements, ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vpflow, velements);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<string>& velements, const vector<vector<vector<aflowlib::_aflowlib_entry>>>& entries, ofstream& FileMESSAGE, ostream& oss) :
      xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vpflow, velements, entries);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<xvector<double>>& vcoords, ofstream& FileMESSAGE, ostream& oss, bool has_stoich_coords, bool formation_energy_hull, bool add_artificial_unaries) :
      xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vpflow, vcoords, has_stoich_coords, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<ChullPoint>& vpoints, ofstream& FileMESSAGE, ostream& oss, bool formation_energy_hull, bool add_artificial_unaries) :
      xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vpflow, vpoints, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const aurostd::xoption& vpflow, const vector<ChullPoint>& vpoints, const vector<string>& velements, ofstream& FileMESSAGE, ostream& oss, bool formation_energy_hull, bool add_artificial_unaries) :
      xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(vpflow, vpoints, velements, formation_energy_hull, add_artificial_unaries);
  }
  ConvexHull::ConvexHull(const ConvexHull& b) : xStream(*b.getOFStream(), *b.getOSS()) {
    copy(b);
  }
  ...
  void ConvexHull::free() {
    ...
    if (m_allowed_dft_types.empty()) {
      m_allowed_dft_types.emplace_back("PAW_PBE");
    }
    cleanHull();
  }
  ...
  void ConvexHull::copy(const ConvexHull& b) { // copy PRIVATE
    ...
    m_initialized = b.m_initialized;
    m_velements.clear();
    for (size_t i = 0, fl_size_i = b.m_velements.size(); i < fl_size_i; i++) {
      m_velements.push_back(b.m_velements[i]);
    }
    m_icsd_entries.clear();
    for (size_t i = 0, fl_size_i = b.m_icsd_entries.size(); i < fl_size_i; i++) {
      m_icsd_entries.push_back(b.m_icsd_entries[i]);
    }
    m_points.clear();
    for (size_t i = 0, fl_size_i = b.m_points.size(); i < fl_size_i; i++) {
      m_points.push_back(b.m_points[i]);
    ...
    m_naries.clear();
    for (size_t i = 0, fl_size_i = b.m_naries.size(); i < fl_size_i; i++) {
      m_naries.push_back(b.m_naries[i]);
    }
    m_coord_groups.clear();
    for (size_t i = 0, fl_size_i = b.m_coord_groups.size(); i < fl_size_i; i++) {
      m_coord_groups.push_back(b.m_coord_groups[i]);
    ...
    m_i_facets.clear();
    for (size_t i = 0, fl_size_i = b.m_i_facets.size(); i < fl_size_i; i++) {
      m_i_facets.push_back(b.m_i_facets[i]);
    }
    ...
    m_allowed_dft_types.clear();
    for (size_t i = 0, fl_size_i = b.m_allowed_dft_types.size(); i < fl_size_i; i++) {
      m_allowed_dft_types.push_back(b.m_allowed_dft_types[i]);
    }
    h_dim = b.h_dim;
    m_elements_present = b.m_elements_present;
    h_points.clear();
    for (size_t i = 0, fl_size_i = b.h_points.size(); i < fl_size_i; i++) {
      h_points.push_back(b.h_points[i]);
    }
    h_centroid = b.h_centroid;
    h_reference = b.h_reference;
    h_facets.clear();
    for (size_t i = 0, fl_size_i = b.h_facets.size(); i < fl_size_i; i++) {
      h_facets.push_back(b.h_facets[i]);
    }
    h_visible_facets.clear();
    for (size_t i = 0, fl_size_i = b.h_visible_facets.size(); i < fl_size_i; i++) {
      h_visible_facets.push_back(b.h_visible_facets[i]);
    }
    h_horizon_ridges.clear();
    for (size_t i = 0, fl_size_i = b.h_horizon_ridges.size(); i < fl_size_i; i++) {
      h_horizon_ridges.push_back(b.h_horizon_ridges[i]);
    }
  }
  ...
  uint ConvexHull::getEntriesCount(bool only_within_half_hull) const {
    ...
    for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
      if (!m_coord_groups[i_coord_group].m_initialized) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup[" + aurostd::utype2string(i_coord_group) + "] is not initialized");
      }
      for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
        ...
        if (only_within_half_hull && m_half_hull) {
          if (m_points[i_point].isWithinHalfHull(m_lower_hull)) {
            count++;
          }
        } else {
          count++;
  ...
  uint ConvexHull::getEntriesCount(uint i_nary, bool only_within_half_hull) const {
    ...
    uint count = 0;
    if (i_nary > m_naries.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within m_naries");
    }
    if (!m_naries[i_nary].m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized nary");
    }
    for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
      count += getEntriesCount(i_nary, i_alloy, only_within_half_hull);
    }
    return count;
  }
  ...
  uint ConvexHull::getEntriesCount(uint i_nary, uint i_alloy, bool only_within_half_hull) const {
    ...
    if (i_nary > m_naries.size() - 1 || i_alloy > m_naries[i_nary].m_alloys.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within m_naries");
    }
    if (!m_naries[i_nary].m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized nary");
    }
    if (!m_naries[i_nary].m_alloys[i_alloy].m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized alloy");
    }
    uint i_point = AUROSTD_MAX_UINT;
    uint count = 0;
    uint i_coord_group = AUROSTD_MAX_UINT;
    for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups.size(); i < fl_size_i; i++) {
      i_coord_group = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups[i];
      if (!m_coord_groups[i_coord_group].m_initialized) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup[" + aurostd::utype2string(i_coord_group) + "] is not initialized");
      }
      for (size_t j = 0, fl_size_j = m_coord_groups[i_coord_group].m_points.size(); j < fl_size_j; j++) {
  ...
  vector<vector<uint>> ConvexHull::getHullSizes(bool only_within_half_hull) const {
    ...
    for (size_t i_nary1 = 0, fl_size_i_nary1 = m_naries.size(); i_nary1 < fl_size_i_nary1; i_nary1++) {
      counts.emplace_back(0);
      for (size_t i_alloy1 = 0, fl_size_i_alloy1 = m_naries[i_nary1].m_alloys.size(); i_alloy1 < fl_size_i_alloy1; i_alloy1++) {
    ...
    for (size_t i_nary1 = 0, fl_size_i_nary1 = m_naries.size(); i_nary1 < fl_size_i_nary1; i_nary1++) {
      for (size_t i_alloy1 = 0, fl_size_i_alloy1 = m_naries[i_nary1].m_alloys.size(); i_alloy1 < fl_size_i_alloy1; i_alloy1++) {
        ...
        for (uint i_nary2 = 0; i_nary2 <= i_nary1; i_nary2++) { // up to i_nary1
          for (size_t i_alloy2 = 0, fl_size_i_alloy2 = m_naries[i_nary2].m_alloys.size(); i_alloy2 < fl_size_i_alloy2; i_alloy2++) {
    ...
    for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
      for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups.size(); i < fl_size_i; i++) {
        ...
        if (!m_coord_groups[i_coord_group].m_is_on_hull) {
          continue;
        } // if it's not on the hull, definitely won't contain gstates
        for (size_t j = 0, fl_size_j = m_coord_groups[i_coord_group].m_points.size(); j < fl_size_j; j++) {
          i_point = m_coord_groups[i_coord_group].m_points[j];
          if (!m_points[i_point].m_initialized) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(i_point) + "] is not initialized");
  ...
  vector<uint> ConvexHull::getHullPoints(bool sort_stoich_ascending) const { // pure hull-members, not equivalent ones too
    ...
    for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
      ...
      for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
        ...
        if (m_points[i_point].m_is_on_hull) {
          hull_points.push_back(i_point);
        }
      }
    }
  ...
  vector<uint> ConvexHull::getGStates(bool include_unaries, bool sort_stoich_ascending) const { // pure g_states, not equivalent ones too
    ...
    for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
      ...
      if (m_coord_groups[i_coord_group].m_is_on_hull) {
        if (!isViablePoint(m_coord_groups[i_coord_group].m_hull_member)) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No hull member set for m_coord_group[" + aurostd::utype2string(i_coord_group) + "]");
        }
        i_point = artificialMap(m_coord_groups[i_coord_group].m_hull_member);
        if (!isViableGState(i_point)) {
          continue;
        } // could be legitimately missing g-state unary
        if (!m_points[i_point].m_initialized) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(i_point) + "] is not initialized");
        }
        if (!m_points[i_point].m_is_g_state) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Mis-identified ground-state structure");
        }
        if (!(!include_unaries && m_points[i_point].isUnary())) {
          g_states.push_back(i_point);
        }
      }
    }
  ...
  uint ConvexHull::getUnaryGState(uint i_alloy) const {
    const uint i_nary = 0;
    if (i_alloy > m_naries[i_nary].m_alloys.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within alloys");
    }
    uint g_state = AUROSTD_MAX_UINT;
    if (m_naries[i_nary].m_alloys[i_alloy].m_coord_groups.size() != 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unexpected count of coordgroups for unaries, should only be 1");
    }
    const uint i_coord_group = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups[0];
    uint i_point = AUROSTD_MAX_UINT;
    for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
      i_point = m_coord_groups[i_coord_group].m_points[i];
      if (!m_points[i_point].m_initialized) {
  ...
  bool ConvexHull::isViablePoint(uint i_point) const {
    return i_point < m_points.size();
  }
  bool ConvexHull::isViableGState(uint g_state) const {
    return isViablePoint(g_state) && m_points[g_state].m_has_entry;
  }
  ...
  bool ConvexHull::findPoint(const string& auid, uint& i_point) const {
    ...
    for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
      for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
        ...
        if (!m_points[_i_point].m_has_entry) {
          continue;
        }
        if (m_points[_i_point].m_entry.auid == auid) {
          i_point = _i_point;
          return true;
        }
      }
  ...
  bool ConvexHull::findPoint(const xvector<double>& coords, uint& i_point) const {
    ...
    for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
      for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
        _i_point = m_coord_groups[i_coord_group].m_points[i];
        if (aurostd::identical(m_points[_i_point].m_coords, coords, ZERO_TOL)) {
          i_point = _i_point;
          return true;
        }
      }
  ...
  bool ConvexHull::getNariesIndex(uint i_point, uint& i_nary, uint& i_alloy, uint& i_coord_group, bool redo) const {
    if (i_point > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
    }
  ...
  bool ConvexHull::getNariesIndex(const ChullPoint& point, uint& i_nary, uint& i_alloy, uint& i_coord_group, bool redo) const {
    if (!point.m_has_stoich_coords) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point does not have stoich coordinates");
    }
  ...
  bool ConvexHull::getCoordGroupIndex(const ChullPoint& point, uint& i_coord_group, bool redo) const {
    if (!point.m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized chullpoint");
    }
    if (!redo) {
      i_coord_group = point.m_i_coord_group;
      if (i_coord_group < m_coord_groups.size()) {
        return true;
      }
    }
  ...
  bool ConvexHull::getCoordGroupIndex(const xvector<double>& r_coords, uint& i_coord_group) const {
    ...
    for (size_t i = 0, fl_size_i = m_coord_groups.size(); i < fl_size_i; i++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " m_coord_groups[i=" << i << "].m_coords=" << m_coord_groups[i].m_coords << " != " << "r_coords=" << r_coords << endl;
      }
    }
    if (LDEBUG) {
  ...
  bool ConvexHull::getAlloyIndex(const ChullPoint& point, uint& i_nary, uint& i_alloy, bool redo) const {
    ...
    if (!redo) {
      ...
      if (i_nary < m_naries.size() && i_alloy < m_naries[i_nary].m_alloys.size()) {
        return true;
      }
    }
  ...
  bool ConvexHull::getAlloyIndex(const CoordGroup& cg, uint& i_nary, uint& i_alloy, bool redo) const {
    ...
    if (!cg.m_has_stoich_coords) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup does not have stoich coordinates");
    }
    if (cg.m_points.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup has no points");
    }
    if (!redo) {
      i_nary = cg.m_i_nary;
  ...
  bool ConvexHull::getAlloyIndex(const xvector<int>& elements_present, uint& i_nary, uint& i_alloy) const {
    ...
    for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys.size(); i < fl_size_i; i++) {
      if (!m_naries[i_nary].m_alloys[i].m_initialized) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized alloy");
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " m_naries[i_nary=" << i_nary << "].m_alloys[i=" << i << "].m_elements_present=" << m_naries[i_nary].m_alloys[i].m_elements_present << endl;
      }
  ...
  uint ConvexHull::artificialMap(uint i_point) const {
    ...
    if (!m_add_artificial_unaries) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid artificial point, not sure how to handle mapping");
    }

    const ChullPoint& art_point = m_points[i_point];
    uint i_coord_group = AUROSTD_MAX_UINT;
    if (!getCoordGroupIndex(art_point, i_coord_group)) {
  ...
  bool ConvexHull::write(filetype ftype) const {
    bool written = false;
    try {
      if (ftype == chull_apool_ft) {
        writeAPool();
        written = true;
      } else if (ftype == json_ft || ftype == txt_ft) {
        writeText(ftype);
        written = true;
      } else if (ftype == latex_ft) {
        writeLatex();
        written = true;
      } else if (ftype == chull_web_ft) {
        writeWebApp();
        written = true;
      }
    } catch (aurostd::xerror& err) {
  ...
  void ConvexHull::setDefaultCFlags() {
    pflow::defaultLoadEntriesFlags(m_cflags, *p_FileMESSAGE, *p_oss, std::string("A"), false, true);
  }
  void ConvexHull::setCFlags(const aurostd::xoption& vpflow) {
    m_cflags = vpflow;
  }
  void ConvexHull::setDirectory() {
    m_aflags.Directory = getPath(m_cflags, *p_FileMESSAGE, *p_oss);
  }
  // MOVED TO xStream
  ...
  bool ConvexHull::entryValid(const aflowlib::_aflowlib_entry& entry, string& reason, char& LOGGER_TYPE, bool ignore_bad_database) const {
    ...
    bool found = false;
    for (size_t j = 0, fl_size_j = entry.vspecies.size(); j < fl_size_j; j++) {
      found = false;
      for (size_t i = 0, fl_size_i = m_velements.size(); i < fl_size_i && !found; i++) {
        if (m_velements[i] == entry.vspecies[j]) {
          found = true;
        }
      }
      if (!found) {
    ...
    if (ignore_bad_database && entry.ignoreBadDatabase(reason)) {
      return false;
    }
  ...
  void ConvexHull::addArtificialUnaries(uint dim) {
    ...
    if (dim > m_dim - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid dimension requested");
    }
    const xvector<double> dummy(m_dim);
    if (dim != m_dim - 1) {
      dummy[dim + dummy.lrows] = 1;
    }
    m_points.emplace_back(dummy, *p_FileMESSAGE, *p_oss, m_has_stoich_coords, m_formation_energy_hull, true);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " new artificial point=" << m_points.back().m_coords << endl;
    }
  }

  void ConvexHull::addArtificialUnaries() {
    for (uint i = 0; i < m_dim; i++) {
      addArtificialUnaries(i);
    }
  }
  ...
  void ConvexHull::loadPoints(const string& alloy) {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " initializing alloy, compound=" << alloy << endl;
    }
    const vector<string> velements = aurostd::getElements(alloy, pp_string, *p_FileMESSAGE, true, true, false, *p_oss); // clean and sort, do not keep_pp  //CO20190712
  ...
  void ConvexHull::loadPoints(const vector<string>& _velements) {
    ...
    vector<string> velements;
    for (size_t i = 0, fl_size_i = _velements.size(); i < fl_size_i; i++) {
      velements.push_back(_velements[i]);
    }
    std::sort(velements.begin(), velements.end()); // safe

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " initializing velements, velements=" << aurostd::joinWDelimiter(velements, ",") << endl;
    }
    ...
    if (sortLIB2) {
      ...
      for (i_alloy = 0; i_alloy < entries[i_nary].size(); i_alloy++) {
        ...
        for (i_entry = 0; i_entry < entries[i_nary][i_alloy].size(); i_entry++) {
          vaes.emplace_back(entries[i_nary][i_alloy][i_entry], i_entry, velements, *p_FileMESSAGE, *p_oss);
        }
  ...
  void ConvexHull::loadPoints(const vector<string>& _velements, const vector<vector<vector<aflowlib::_aflowlib_entry>>>& entries) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " initializing velements WITH entries, velements=" << aurostd::joinWDelimiter(velements, ",") << endl;
    }
    ...
    for (size_t i = 0, fl_size_i = entries.size(); i < fl_size_i; i++) {
      for (size_t j = 0, fl_size_j = entries[i].size(); j < fl_size_j; j++) {
        for (size_t k = 0, fl_size_k = entries[i][j].size(); k < fl_size_k; k++) {
          ...
          points.push_back(cp);
          // save icsd entries
          if (aurostd::substring2bool(entries[i][j][k].prototype, "_ICSD_")) {
            m_icsd_entries.push_back(points.size() - 1);
          }
        }
      }
    }
  ...
  void ConvexHull::loadPoints(const vector<xvector<double>>& vcoords, bool has_stoich_coords, bool formation_energy_hull, bool add_artificial_unaries) {
    ...
    vector<ChullPoint> points;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " initializing vcoords (has_stoich_coords==" << has_stoich_coords << "), count=" << vcoords.size() << endl;
    }
    ChullPoint cp;
    for (size_t i = 0, fl_size_i = vcoords.size(); i < fl_size_i; i++) {
  ...
  void ConvexHull::loadPoints(const vector<ChullPoint>& vpoints, const vector<string>& velements, bool formation_energy_hull, bool add_artificial_unaries) {
    ...
    if (!velements.empty()) {
      m_velements.clear();
      for (size_t i = 0, fl_size_i = velements.size(); i < fl_size_i; i++) {
        m_velements.push_back(velements[i]);
      }
      std::sort(m_velements.begin(), m_velements.end()); // safe
    }

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " initializing chullpoints, count=" << vpoints.size() << endl;
    }
    ...
    } // this MUST be true: chull cannot find facets for 1D hulls
    // test of stupidity
    for (size_t i = 0, fl_size_i = m_points.size(); i < fl_size_i; i++) {
      if (!m_points[i].m_initialized) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized point");
      }
      if (m_points[i].getDim() != m_dim) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch among points");
      }
    ...
    if (m_half_hull && m_has_stoich_coords && m_add_artificial_unaries) {
      m_thermo_hull = true;
    }
    if (LDEBUG) {
  ...
  void ConvexHull::calculateOutlierThreshold(const vector<double>& _energies, double& upper_threshold, double& lower_threshold) {
    const xvector<double> energies = aurostd::vector2xvector<double>(_energies);
  ...
  void ConvexHull::calculateOutlierThreshold(const xvector<double>& energies, double& upper_threshold, double& lower_threshold) {
    ...
    const bool iqr_method = true; // unfortunately, MAD is normal distribution dependent, NOT our case here
    ...
    if ((uint) energies.rows < iqr_count_threshold) { // ALWAYS not enough points to do statistics (need 3 quartiles)
      ...
      } // don't want to see these errors, they are expected
      else if (m_cflags.flag("CHULL::STRICT_OUTLIER_ANALYSIS") && (!m_cflags.flag("FORCE"))) {
        message << " (results may not be reliable). Terminating hull analysis.";
    ...
    if (LDEBUG) {
      xvector<double> temp_eng = energies;
      sort(temp_eng);
      cerr << "lastCoords(): " << temp_eng << endl;
    }
    double q1;
    double q2;
    double q3;
  ...
  vector<uint> ConvexHull::calculateOutliers(const vector<uint>& points_to_consider) {
    ...
    if (show_warnings) {
      LOGGER_TYPE = _LOGGER_WARNING_;
    } // show warning if we do not remove!
  ...
  vector<uint> ConvexHull::getOutliers(const xvector<int>& elements_present) {
    ...
    vector<uint> points_to_consider;
    uint i_point = AUROSTD_MAX_UINT;
    const char LOGGER_TYPE = _LOGGER_OPTION_;
    bool silent = false;
    const bool see_neglect = m_cflags.flag("CHULL::SEE_NEGLECT");
    ...
    if (m_half_hull) { // we only care about points above/below hull
      for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
        ...
        if (m_coord_groups[i_coord_group].getElementsPresent() != elements_present) {
          continue;
        }
        for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
          i_point = m_coord_groups[i_coord_group].m_points[i];
          ...
          if (m_points[i_point].isWithinHalfHull(m_lower_hull)) {
            points_to_consider.push_back(i_point);
          } //! std::signbit(m_points[i_point].getLastCoord())) //positive
          else {
            silent = (!see_neglect && LOGGER_TYPE == _LOGGER_OPTION_);
            message << "Neglecting ";
            if (m_points[i_point].m_has_entry) {
              message << "[auid=" << m_points[i_point].m_entry.auid << ",aurl=" << m_points[i_point].m_entry.aurl << "] ";
            } else {
    ...
    const bool check_count = (m_half_hull && sum(elements_present) == 2);
    if (check_count) {
      const uint binaries_half_hull_threshold = DEFAULT_CHULL_OUTLIER_ANALYSIS_COUNT_THRESHOLD_BINARIES;
  ...
  vector<uint> ConvexHull::findArtificialPoints(uint i_coord_group) {
    if (i_coord_group > m_coord_groups.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within coordgroups");
    }
    if (!m_coord_groups[i_coord_group].m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized coordgroup");
    }
    ...
    for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
      ...
      if (m_points[i_point].m_is_artificial) {
        artificial_points.push_back(i_point);
      }
    }
  ...
  uint ConvexHull::findArtificialUnary(uint i_coord_group) {
    if (!m_has_stoich_coords) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No unaries to be found (coordinates are not stoichiometric)");
    }
    if (i_coord_group > m_coord_groups.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within coordgroups");
    }
    ...
    if (m_coord_groups[i_coord_group].m_points.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup[" + aurostd::utype2string(i_coord_group) + "] is empty");
    }
    if (!m_points[m_coord_groups[i_coord_group].m_points[0]].isUnary()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup[" + aurostd::utype2string(i_coord_group) + "] is not unary");
    }

    vector<uint> artificial_points = findArtificialPoints(i_coord_group);
    if (artificial_points.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Missing artificial points");
    }
    if (artificial_points.size() != 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Too many artificial points: " + aurostd::utype2string(artificial_points.size()));
    }
  ...
  void ConvexHull::organizeHullPoints(uint i_coord_group) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " finding min/max energy point(s) for coord_group[" << i_coord_group << "]" << endl;
    }

    m_coord_groups[i_coord_group].m_candidate_hull_points.clear();
    const uint p_size = m_coord_groups[i_coord_group].m_points.size();
    if (p_size == 0) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup has no points");
    }
    ...
    if (m_coord_groups[i_coord_group].m_has_artificial_unary) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " looking for artificial point in coord_group[" << i_coord_group << "]" << endl;
      }
  ...
  void ConvexHull::organizeHullPoints() {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (m_coord_groups.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Points have not been structured correctly");
    }
    if (LDEBUG) {
    ...
    for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
      organizeHullPoints(i_coord_group);
    }
  }
  ...
  void ConvexHull::initializeNaries() {
    ...
    stringstream message;
    // clear
    for (size_t i_nary = 0, fl_size_i_nary = m_naries.size(); i_nary < fl_size_i_nary; i_nary++) {
      for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
    ...
    if (m_naries.empty()) {
      message << "m_naries.size()=0, xcombos may be broken";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message);
    }
    if (m_naries[0].m_alloys.empty()) {
      message << "m_naries[0].m_alloys.size()==0, xcombos may be broken";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message);
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_naries.size()=" << m_naries.size() << endl;
    }
  }
  ...
  void ConvexHull::structurePoints() {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " gathering points to neglect" << endl;
    }

    bool fhsc_requested = m_cflags.flag("CHULL::CALCULATE_FAKE_HULL_STABILITY_CRITERION"); // CO20210315
    const bool perform_structure_comparison = (true && (!m_cflags.flag("CHULL::SKIP_STRUCTURE_COMPARISON"))); //(1&&!(m_cflags.flag("CHULL::SKIP_STRUCTURE_COMPARISON")||(!m_cflags.flag("CHULL::MULTI_OUTPUT")&&m_cflags.flag("CHULL::LATEX_DOC")&&m_cflags.flag("CHULL::IMAGE_ONLY"))));
    uint i_point_sc = AUROSTD_MAX_UINT;
    xvector<double> r_coords_sc_input;
    const string auid_sc = m_cflags.getattachedscheme("CHULL::CALCULATE_FAKE_HULL_STABILITY_CRITERION");
    if (auid_sc.empty()) {
      fhsc_requested = false;
    }
    if (fhsc_requested) {
      for (size_t i = 0, fl_size_i = m_points.size(); i < fl_size_i && i_point_sc == AUROSTD_MAX_UINT; i++) {
    ...
    if (i_point_sc == AUROSTD_MAX_UINT) {
      fhsc_requested = false;
    }

    bool remove_requested = m_cflags.flag("CHULL::NEGLECT");
    const bool see_neglect = m_cflags.flag("CHULL::SEE_NEGLECT");
    const bool remove_submodular = true; // remove AEL-AGL, APL, etc.
    const bool fhn1eg_requested = m_cflags.flag("CHULL::CALCULATE_FAKE_HULL_N+1_ENTHALPY_GAIN"); // SK20200327
    const bool remove_invalid = true;
    const bool remove_duplicate_entries = true; // we remove duplicate entries from the database, but in general, keep input of user constant
    bool remove_extreme = m_cflags.flag("CHULL::REMOVE_EXTREMA");
    const bool perform_outliers_analysis = DEFAULT_CHULL_PERFORM_OUTLIER_ANALYSIS;
    const bool keep_outliers = (!perform_outliers_analysis || m_cflags.flag("CHULL::INCLUDE_OUTLIERS"));
    const bool remove_outliers = !keep_outliers;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " remove_outliers=" << remove_outliers << endl;
    }

    vector<string> points_neglect;
    double extrema_val = 0.0;
    if (remove_requested) {
      aurostd::string2tokens(m_cflags.getattachedscheme("CHULL::NEGLECT"), points_neglect, ",");
    }
    if (points_neglect.empty()) {
    ...
    if (remove_requested) {
      removing_messages.emplace_back("undesired");
    }
    if (remove_invalid) {
      removing_messages.emplace_back("erroneous");
    }
    if (remove_duplicate_entries) {
      removing_messages.emplace_back("duplicate");
    }
    if (remove_extreme) {
      removing_messages.emplace_back("extreme");
    }
    if (remove_outliers) {
      removing_messages.emplace_back("outlier");
    }
    if (fhn1eg_requested) {
      removing_messages.push_back(aurostd::utype2string(m_velements.size()) + "D (N+1 enthalpy gain)");
    } // SK20200327
    if (fhsc_requested) {
      removing_messages.push_back("auid=" + auid_sc + " and equivalent (stability criterion)");
    } // SK20200327
    if (!removing_messages.empty()) {
    ...
    if (m_cflags.flag("CHULL::INCLUDE_PAW_GGA") && (!aurostd::WithinList(m_allowed_dft_types, "PAW_GGA"))) {
      m_allowed_dft_types.emplace_back("PAW_GGA");
    }

    // ignore bad database
    const bool ignore_bad_database = (DEFAULT_CHULL_IGNORE_KNOWN_ILL_CONVERGED && !m_cflags.flag("CHULL::INCLUDE_ILL_CONVERGED"));
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " ignore_bad_database=" << ignore_bad_database << endl;
    }

    // organize into coordgroups
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " organizing into coordgroups" << endl;
    }
    vector<uint> unique_entries;
    string invalid_reason;
    string canonical_auid;
    char LOGGER_TYPE = _LOGGER_OPTION_;
    bool silent = false;
    ...
    for (size_t i = 0, fl_size_i = m_points.size(); i < fl_size_i; i++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " point[" << i << "]: m_coords=" << point.m_coords << ", r_coords=" << r_coords << ", compound=\"" << point.m_entry.compound << "\", dim=" << point.m_i_nary + 1 << endl;
      }
    ...
    for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
      ...
      if (!points_to_remove.empty()) {
        ...
        for (size_t i = 0, fl_size_i = points_to_remove.size(); i < fl_size_i; i++) {
          ...
          if (m_points[i_point].m_has_entry) {
            message << "auid=" << m_points[i_point].m_entry.auid;
          } else {
            message << "m_coords=" << m_points[i_point].m_coords;
          }
          pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_OPTION_);
          m_coord_groups[i_coord_group].m_points.erase(m_coord_groups[i_coord_group].m_points.begin() + points_to_remove[i]);
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " after outlier removal, m_coord_groups[" << i_coord_group << "].m_points.size() = " << m_coord_groups[i_coord_group].m_points.size() << endl;
        }
      }
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " done organizing into coordgroups" << endl;
    }
    ...
    if (m_has_stoich_coords) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " stoich_coords found, also sorting into n-aries and alloys" << endl;
      }
      ...
      for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " m_coord_groups[i_coord_group=" << i_coord_group << "].m_points.size()=" << m_coord_groups[i_coord_group].m_points.size() << endl;
        }
        if (m_coord_groups[i_coord_group].m_points.empty()) {
          continue;
        }
        // we already filled bins, if we cannot find alloy system, then it's a bust
        if (!getAlloyIndex(m_coord_groups[i_coord_group], i_nary, i_alloy, true)) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot get alloys index");
        } // redo as we never done before
      ...
      const bool only_within_half_hull = false; // m_half_hull; //these counts should reflect total entries
      ...
      for (size_t i_nary = 0, fl_size_i_nary = m_naries.size(); i_nary < fl_size_i_nary; i_nary++) {
        for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
          ...
          if (!m_velements.empty()) {
            message << " " << aurostd::joinWDelimiter(alloyToElements(i_nary, i_alloy), "-");
          }
        ...
        for (size_t i_nary = 0, fl_size_i_nary = m_naries.size(); i_nary < fl_size_i_nary; i_nary++) {
          ...
          for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
            ...
            for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups.size(); i < fl_size_i; i++) {
              ...
              for (size_t j = 0, fl_size_j = m_coord_groups[i_coord_group_sort].m_points.size(); j < fl_size_j; j++) {
                cerr << __AFLOW_FUNC__ << " compound=";
                cerr << (m_points[m_coord_groups[i_coord_group_sort].m_points[j]].m_is_artificial ? string("ARTIFICIAL") : m_points[m_coord_groups[i_coord_group_sort].m_points[j]].m_entry.compound) << " ";
  ...
  vector<string> ConvexHull::alloyToElements(const ChullPoint& point) const {
    return alloyToElements(point.m_i_nary, point.m_i_alloy);
  }
  vector<string> ConvexHull::alloyToElements(uint i_nary, uint i_alloy) const {
    ...
    if ((uint) elements_present.rows != m_velements.size()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between elements present and composition");
    }
    vector<string> vout;
    for (int i = elements_present.lrows; i <= elements_present.urows; i++) {
      if (elements_present[i] == 1) {
        vout.push_back(m_velements[i - elements_present.lrows]);
      }
    }
    return vout;
  }
  ...
  void ConvexHull::checkStructurePoints() {
    ...
    const bool binary_statistics_check = m_naries.size() > 1; // unary hull //true;

    if (m_has_stoich_coords) {
      // UNARIES - START
      // tests of stupidity
      if (m_naries.size() != m_dim) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Missing n-aries");
      } // these are populated by default if done correctly
      uint i_nary = 0;
      if (m_naries[i_nary].m_alloys.size() != m_dim) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Missing unary alloys");
      } // these are populated by default if done correctly

      // check for ground-states (non-artificial points)
      bool found_real = false;
      bool found_artificial = false;
      uint i_coord_group = 0;
      uint i_point = 0;
      uint i_point_real = 0;
      for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
        if (m_naries[i_nary].m_alloys[i_alloy].m_coord_groups.size() != 1) {
        ...
        for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i && !(found_real && found_artificial); i++) {
          i_point = m_coord_groups[i_coord_group].m_points[i];
          if (!m_points[i_point].m_initialized) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(i_point) + "] is not initialized");
          }
          if (m_points[i_point].m_is_artificial) {
            ...
            if (!found_real) {
              i_point_real = i_point;
            } // grab first i_point that's real
            found_real = true;
          }
        }
        if (m_add_artificial_unaries && !found_artificial) {
          message << "Missing artificial points for";
          if (i_alloy < m_velements.size()) {
            message << " " << m_velements[i_alloy];
          }
          message << " [i_nary=" << i_nary << ",i_alloy=" << i_alloy << "]";
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message.str());
          message.str("");
        }
        if (found_real) {
          if (m_thermo_hull) {
            if (std::abs(m_points[i_point_real].getLastCoord()) >= ENERGY_TOL) {
              ...
              else if (m_cflags.flag("FORCE") || m_cflags.flag("CHULL::INCLUDE_SKEWED_HULLS")) {
                pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
              } else {
      ...
      // BINARIES - START
      const uint count_threshold_binaries_total = DEFAULT_CHULL_OUTLIER_ANALYSIS_COUNT_THRESHOLD_BINARIES;
      if (binary_statistics_check) {
        ...
        for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
          ...
          if (count < count_threshold_binaries_total) {
            ...
            if (!m_velements.empty()) {
              hull << aurostd::joinWDelimiter(alloyToElements(i_nary, i_alloy), "-") << " ";
            }
            ...
            else if (m_cflags.flag("FORCE") || m_cflags.flag("CHULL::INCLUDE_UNRELIABLE_HULLS")) {
              pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
            } else {
              message << ". Override with --force (results may not be reliable).";
              throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message);
  ...
  void ConvexHull::addPointToFacet(ChullFacet& facet, uint i_point) {
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = facet.m_vertices.size(); i < fl_size_i; i++) {
        cerr << facet.m_vertices[i].ch_index << "  h_coords=" << facet.m_vertices[i].ch_point.h_coords << endl;
      }
    }
  }

  void ConvexHull::initializeFacet(ChullFacet& facet, bool check_validity) {
    facet.initialize(h_reference, h_dim, check_validity);
  }

  uint ConvexHull::getExtremePoint(uint dim) {
    const vector<FacetPoint> points_to_avoid;
  ...
  uint ConvexHull::getExtremePoint(uint dim, const vector<FacetPoint>& points_to_avoid) {
    ...
    stringstream message;
    if (h_points.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No starting points provided");
    }
    uint i_point = AUROSTD_MAX_UINT;
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = h_points.size(); i < fl_size_i; i++) {
        cerr << h_points[i] << "  h_coords=" << m_points[h_points[i]].h_coords << endl;
      }
      cerr << __AFLOW_FUNC__ << " points_to_avoid:" << endl;
      for (size_t j = 0, fl_size_j = points_to_avoid.size(); j < fl_size_j; j++) {
    ...
    for (size_t i = 0, fl_size_i = h_points.size(); i < fl_size_i; i++) {
      ...
      avoid = false;
      for (size_t j = 0, fl_size_j = points_to_avoid.size(); j < fl_size_j && !avoid; j++) {
      ...
      // HE20220412 END
      if (std::abs(m_points[h_points[i]].h_coords[h_coords_index]) > extreme) {
        i_point = h_points[i];
        extreme = std::abs(m_points[h_points[i]].h_coords[h_coords_index]);
      }
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " i_point=" << i_point << endl;
    }
    return i_point;
  }
  ...
  void ConvexHull::setCentroid() {
    ...
    vector<xvector<double>> points;
    for (size_t i = 0, fl_size_i = h_points.size(); i < fl_size_i; i++) {
      points.push_back(m_points[h_points[i]].h_coords);
    }
    h_centroid = h_reference = aurostd::getCentroid(points); // fix h_reference later
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " centroid: " << h_centroid << endl;
    }
  }
  ...
  vector<FacetPoint> ConvexHull::getInitialExtremePoints() {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " start" << endl;
    }
    ...
    if (h_points.size() < h_dim) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Not enough points to build an initial facet");
    }
    for (uint i = 0; i < h_dim; i++) {
      addPointToFacet(facet, getExtremePoint(i, facet.m_vertices));
    }
    if (!facet.isValid(error)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "An initial facet cannot be created per initial extreme points, invalid points input: " + error);
    }
    initializeFacet(facet, false); // already validated
    // now get furthest point from this facet, initialize outside set first
    FacetPoint f_point;
    for (size_t i = 0, fl_size_i = h_points.size(); i < fl_size_i; i++) {
      if (facet.isPointOnFacet(h_points[i])) {
        continue;
      }
      f_point.initialize(m_points[h_points[i]], h_points[i]);
      facet.f_outside_set.push_back(f_point);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " adding to facet[" << 0 << "] outside set: " << h_points[i] << endl;
      }
    }
    if (!facet.f_outside_set.empty()) { // in special case where there are only 2 points in 2D, we simply return facet and not simplex
  ...
  void ConvexHull::setNeighbors() {
    ...
    if (LDEBUG) {
      vector<uint> indices;
      for (size_t i = 0, fl_size_i = h_facets.size(); i < fl_size_i; i++) {
        indices = h_facets[i].getCHIndices();
        cerr << __AFLOW_FUNC__ << " facet[" << i << "] has indices: ";
        for (size_t j = 0, fl_size_j = indices.size(); j < fl_size_j; j++) {
          cerr << indices[j];
          if (j != indices.size() - 1) {
            cerr << ",";
          }
    ...
    for (size_t i_facet = 0, fl_size_i_facet = h_facets.size(); i_facet < fl_size_i_facet; i_facet++) {
      h_facets[i_facet].f_neighbors.clear();
    }
    for (size_t i = 0, fl_size_i = h_facets.size(); i < fl_size_i; i++) {
      for (size_t j = i + 1, fl_size_j = h_facets.size(); j < fl_size_j; j++) {
      ...
      if (h_facets.size() > 1 && h_facets[i].f_neighbors.size() != h_dim) {
        ...
        if (LDEBUG) {
          ...
          for (size_t j = 0, fl_size_j = h_facets[i].m_vertices.size(); j < fl_size_j; j++) {
            cerr << h_facets[i].m_vertices[j].ch_point.h_coords << (j != h_facets[i].m_vertices.size() - 1 ? ", " : "");
          }
          cerr << "]: ";
          for (size_t j = 0, fl_size_j = h_facets[i].f_neighbors.size(); j < fl_size_j; j++) {
    ...
    for (size_t i = 0, fl_size_i = h_facets.size(); i < fl_size_i; i++) {
      std::sort(h_facets[i].f_neighbors.begin(), h_facets[i].f_neighbors.end());
    }
  ...
  void ConvexHull::createInitializeSimplex() {
    ...
    vector<xvector<double>> points;
    for (size_t i = 0, fl_size_i = initial_extreme_points.size(); i < fl_size_i; i++) {
      points.push_back(initial_extreme_points[i].ch_point.h_coords);
    }
    h_reference = aurostd::getCentroid(points);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " new h_reference=" << h_reference << endl;
    }

    // new first facet of endpoints

    if (initial_extreme_points.size() == h_dim + 1) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " building initial simplex" << endl;
      }
    } else if (initial_extreme_points.size() == h_dim) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " not enough points to build a simplex, will settle for a facet instead" << endl;
      }
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Not enough points to construct initial simplex/facet");
    }
    ...
    while (xc.increment()) {
      ...
      if (LDEBUG) {
        ...
        for (size_t i = 0, fl_size_i = indices.size(); i < fl_size_i; i++) {
          cerr << indices[i] << (i != indices.size() - 1 ? "," : "");
        }
        cerr << endl;
      }
      h_facets.emplace_back(*p_FileMESSAGE, *p_oss); // CO20180305
      for (size_t i = 0, fl_size_i = indices.size(); i < fl_size_i; i++) {
        if (indices[i] == 1) {
          h_facets.back().addVertex(initial_extreme_points[i]);
        }
      }
      if (LDEBUG) {
      ...
      if (!h_facets.back().isValid(error)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "An initial facet cannot be created, invalid points input: " + error);
      }
      initializeFacet(h_facets.back(), false); // already validated
    }
    if (initial_extreme_points.size() == h_dim + 1) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " initial simplex built" << endl;
      }
    } else if (initial_extreme_points.size() == h_dim) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " initial (single) facet built" << endl;
      }
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Not enough points to construct initial simplex/facet");
    }
    ...
    for (size_t i = 0, fl_size_i = h_points.size(); i < fl_size_i; i++) {
      associated = false;
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " associating point[" << h_points[i] << "] to a facet's outside set" << endl;
      }
      f_point.initialize(m_points[h_points[i]], h_points[i]);
      for (size_t i_facet = 0, fl_size_i_facet = h_facets.size(); i_facet < fl_size_i_facet && !associated; i_facet++) {
        if (h_facets[i_facet].isPointOnFacet(f_point)) {
          associated = true;
        } // skip the obvious
        if (!associated && h_facets[i_facet].isPointOutside(f_point)) {
          ...
          associated = true;
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " associating point[" << h_points[i] << "] with facet[" << i_facet << "].f_outside_set" << endl;
          }
        }
      }
      if (!associated) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " NOT associating point[" << h_points[i] << "] with any facet outside set" << endl;
        }
      }
    }
    // get furthest point
    for (size_t i_facet = 0, fl_size_i_facet = h_facets.size(); i_facet < fl_size_i_facet; i_facet++) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " facet[" << i_facet << "] has " << h_facets[i_facet].f_outside_set.size() << " outside points" << endl;
      }
  ...
  void ConvexHull::setVisibleFacets(uint i_facet) {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (i_facet > h_facets.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index with h_facets");
    }
    // clean
    for (size_t i = 0, fl_size_i = h_facets.size(); i < fl_size_i; i++) {
      h_facets[i].f_visited = false;
    }
    h_visible_facets.clear();
    ...
    uint i_visible;
    uint i_neigh;
    //[CO20190409 - size() must be recalculated for EVERY loop]for(size_t i=0,fl_size_i=h_visible_facets.size();i<fl_size_i;i++)
    for (size_t i = 0; i < h_visible_facets.size(); i++) { // CO20200106 - patching for auto-indenting
      i_visible = h_visible_facets[i];
      for (size_t j = 0, fl_size_j = h_facets[i_visible].f_neighbors.size(); j < fl_size_j; j++) {
        i_neigh = h_facets[i_visible].f_neighbors[j];
        if (h_facets[i_neigh].f_visited) {
          continue;
        }
        h_facets[i_neigh].f_visited = true;
        if (h_facets[i_neigh].isPointOutside(h_facets[i_facet].f_furthest_point)) {
          h_visible_facets.push_back(i_neigh);
        }
      }
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = h_visible_facets.size(); i < fl_size_i; i++) {
        cerr << h_visible_facets[i] << " ";
      }
      cerr << endl;
    }
  }
  ...
  void ConvexHull::setHorizonRidges() {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (h_visible_facets.empty()) {
      return;
    }
    h_horizon_ridges.clear();
    bool match;
    for (size_t i1 = 0, fl_size_i1 = h_visible_facets.size(); i1 < fl_size_i1; i1++) {
      for (size_t j1 = 0, fl_size_j1 = h_facets[h_visible_facets[i1]].m_ridges.size(); j1 < fl_size_j1; j1++) {
        match = false;
        for (size_t i2 = 0, fl_size_i2 = h_visible_facets.size(); i2 < fl_size_i2 && !match; i2++) {
          if (i1 == i2) {
            continue;
          }
          for (size_t j2 = 0, fl_size_j2 = h_facets[h_visible_facets[i2]].m_ridges.size(); j2 < fl_size_j2 && !match; j2++) {
        ...
        if (!match) {
          h_horizon_ridges.push_back(h_facets[h_visible_facets[i1]].m_ridges[j1]);
        }
      }
    }
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = h_horizon_ridges.size(); i < fl_size_i; i++) {
        for (size_t j = 0, fl_size_j = h_horizon_ridges[i].m_vertices.size(); j < fl_size_j; j++) {
  ...
  uint ConvexHull::createNewFacets(FacetPoint furthest_point) {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (furthest_point.ch_index > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index with furthest_point");
    }
    const uint old_facet_count = h_facets.size();
    string error; // dummy so we don't recreate every time
    for (size_t i = 0, fl_size_i = h_horizon_ridges.size(); i < fl_size_i; i++) {
      // check obvious
      if (h_horizon_ridges[i].isPointOnFacet(furthest_point)) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " skipping new facet: duplicate point" << endl;
        }
        continue;
      }
      h_horizon_ridges[i].addVertex(furthest_point);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " furthest point for new facet: " << furthest_point.ch_index << " " << m_points[furthest_point.ch_index].h_coords << endl;
      }
      if (!h_horizon_ridges[i].isValid(error)) { // corner case, happens when creating new facets on edges or corners of hull
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " skipping new facet: " << error << endl;
        }
        continue;
      }
      ...
      if (LDEBUG) {
        ...
        for (size_t j = 0, fl_size_j = h_facets.back().m_vertices.size(); j < fl_size_j; j++) {
          cerr << m_points[h_facets.back().m_vertices[j].ch_index].h_coords << " | ";
        }
        cerr << endl;
      }
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " " << h_facets.size() - old_facet_count << " new facets" << endl;
    }
  ...
  void ConvexHull::updateOutsideSet(uint new_facet_count) { // they are at the end of the list
    ...
    for (size_t i = 0, fl_size_i = h_visible_facets.size(); i < fl_size_i; i++) {
      i_visible = h_visible_facets[i];
      for (size_t j = 0, fl_size_j = h_facets[i_visible].f_outside_set.size(); j < fl_size_j; j++) {
        const FacetPoint& f_point = h_facets[i_visible].f_outside_set[j];
        associated = false;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " re-associating point[" << f_point.ch_index << "] with a new facet" << endl;
        }
        for (size_t fl_size_i_facet = h_facets.size(), i_facet = fl_size_i_facet - new_facet_count; i_facet < fl_size_i_facet && !associated; i_facet++) {
          if (h_facets[i_facet].isPointOnFacet(f_point)) {
            associated = true;
          } // skip the obvious
          if (!associated && h_facets[i_facet].isPointOutside(f_point)) {
            ...
            associated = true;
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " associating point[" << f_point.ch_index << "] with facet[" << i_facet << "].f_outside_set" << endl;
            }
          }
        }
      }
    }
    for (size_t fl_size_i_facet = h_facets.size(), i_facet = fl_size_i_facet - new_facet_count; i_facet < fl_size_i_facet; i_facet++) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " NEW facet[" << i_facet << "] has " << h_facets[i_facet].f_outside_set.size() << " outside points" << endl;
      }
      h_facets[i_facet].setFurthestPoint();
    }
  }
  ...
  void ConvexHull::deleteVisibleFacets() {
    ...
    for (size_t i = 0, fl_size_i = h_visible_facets.size(); i < fl_size_i; i++) {
      h_facets.erase(h_facets.begin() + h_visible_facets[i]);
    }
  }
  ...
  void ConvexHull::removeDuplicateHullPoints() {
    ...
    for (size_t i = 0, fl_size_i = h_points.size(); i < fl_size_i; i++) {
      for (size_t j = i + 1, fl_size_j = h_points.size(); j < fl_size_j; j++) {
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = indices_2_remove.size(); i < fl_size_i; i++) {
        cerr << indices_2_remove[i] << " ";
      }
      cerr << endl;
    }
    std::sort(indices_2_remove.rbegin(), indices_2_remove.rend()); // descending
    for (size_t i = 0, fl_size_i = indices_2_remove.size(); i < fl_size_i; i++) {
      h_points.erase(h_points.begin() + indices_2_remove[i]);
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " done" << endl;
    }
  ...
  void ConvexHull::calculateFacets() {
    ...
    if (h_points.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No points found on hull");
    }
    h_dim = m_points[h_points[0]].h_coords.rows;

    for (size_t i = 1, fl_size_i = h_points.size(); i < fl_size_i; i++) {
      if ((uint) m_points[h_points[i]].h_coords.rows != h_dim) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid initialization of hull coordinates");
      }
    }

    if (LDEBUG) {
      for (size_t i = 0, fl_size_i = h_points.size(); i < fl_size_i; i++) {
        cerr << __AFLOW_FUNC__ << " m_coords=" << m_points[h_points[i]].m_coords << "; h_coords=" << m_points[h_points[i]].h_coords
             << endl; //<< "; relevant " << m_points[h_points[i]].isRelevantPoint(elements_present) << endl;
      }
    }
    ...
    //[CO20190409 - size() must be recalculated for EVERY loop]for(size_t i_facet=0,fl_size_i_facet=h_facets.size();i_facet<fl_size_i_facet;i_facet++)
    for (size_t i_facet = 0; i_facet < h_facets.size(); i_facet++) { // CO20200106 - patching for auto-indenting
      if (h_facets[i_facet].f_outside_set.empty()) {
        continue;
      } // the only way we exit this loop is if we continue for ALL facets
      ...
      if (new_facet_count) {
        ...
        for (size_t fl_size_i_facet = h_facets.size(), i_facet = fl_size_i_facet - new_facet_count; i_facet < fl_size_i_facet; i_facet++) {
          h_facets[i_facet].setFurthestPoint();
        } // get furthest points for new facets
    ...
    for (size_t i = 0, fl_size_i = h_facets.size(); i < fl_size_i; i++) {
      ...
      std::sort(h_facets[i].m_vertices.begin(), h_facets[i].m_vertices.end(), sortThermoPoints(sort_stoich_ascending, m_sort_energy_ascending));
    }
    ...
    if (LDEBUG) {
      ...
      for (size_t i_facet = 0, fl_size_i_facet = h_facets.size(); i_facet < fl_size_i_facet; i_facet++) {
        ...
        for (size_t i = 0, fl_size_i = h_facets[i_facet].m_vertices.size(); i < fl_size_i; i++) {
        ...
        cerr << "normal " << h_facets[i_facet].m_normal << " | angles " << aurostd::getGeneralAngles(h_facets[i_facet].m_normal, ZERO_TOL)
             << endl; //<< " | x " << cos(h_facets[i_facet].m_angle) << " | y " << sin(h_facets[i_facet].m_angle)  << endl;
      }
    }
  }
  ...
  const xvector<int>& ConvexHull::getElementsPresent(uint i_point) const {
    if (i_point > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, "ConvexHull::getElementsPresent():", "Invalid index within points");
    }
  ...
  xvector<int> ConvexHull::getElementsPresent(const vector<uint>& vcpoints) const {
    ...
    if (vcpoints.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No points provided");
    }
    xvector<int> elements_present = getElementsPresent(vcpoints[0]);
    if (vcpoints.size() == 1) {
      return elements_present;
    }
    for (size_t i = 1, fl_size_i = vcpoints.size(); i < fl_size_i; i++) {
      const uint i_point = vcpoints[i];
  ...
  void ConvexHull::setElementsPresent(uint i_nary, uint i_alloy) {
    m_elements_present = getElementsPresent(i_nary, i_alloy);
  }
  ...
  void ConvexHull::addRelevantUnariesToHullCalculation(xvector<int>& elements_present) {
    const uint i_nary = 0;
    if ((uint) elements_present.rows != m_naries[i_nary].m_alloys.size()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unary alloy is missing from m_naries");
    }
    uint i_coord_group;
    uint i_point;
    for (int i_alloy = elements_present.lrows; i_alloy <= elements_present.urows; i_alloy++) {
      if (elements_present[i_alloy] == 1) {
        if (m_naries[i_nary].m_alloys[i_alloy - elements_present.lrows].m_coord_groups.size() != 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unexpected count of coordgroups for unaries, should only be 1");
        }
        ...
        for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_candidate_hull_points.size(); i < fl_size_i; i++) {
    ...
    for (uint i_nary = 1; i_nary < i_nary_max; i_nary++) {
      ...
      for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
        ...
        if (m_naries[i_nary].m_alloys[i_alloy].belongs2Hull(m_elements_present)) {
          for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys[i_alloy].m_facets.size(); i < fl_size_i; i++) {
            i_facet = m_naries[i_nary].m_alloys[i_alloy].m_facets[i];
            if (i_facet > m_facets.size() - 1) {
              throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within m_facets");
            }
            const ChullFacet& facet = m_facets[i_facet];
            for (size_t j = 0, fl_size_j = facet.m_vertices.size(); j < fl_size_j; j++) {
  ...
  void ConvexHull::preparePointsForHullCalculation(uint i_nary, uint i_alloy) {
    ...
    for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups.size(); i < fl_size_i; i++) {
      ...
      for (size_t j = 0, fl_size_j = m_coord_groups[i_coord_group].m_candidate_hull_points.size(); j < fl_size_j; j++) {
  ...
  uint ConvexHull::getNearestFacetVertically(const vector<uint>& i_facets, const ChullPoint& point) const {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_coords=" << point.m_coords << ", h_coords=" << point.h_coords << endl;
    }
  ...
  uint ConvexHull::getNearestFacetVertically(const vector<uint>& i_facets, const xvector<double>& point) const {
    ...
    double vdist;
    double dist = AUROSTD_MAX_DOUBLE;
    for (size_t i = 0, fl_size_i = i_facets.size(); i < fl_size_i; i++) {
      i_facet = i_facets[i];
      if (i_facet > m_facets.size() - 1) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within m_facets");
      }
      const ChullFacet& facet = m_facets[i_facet];
      if (!facet.m_initialized) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Facet not initialized");
      }
      if (facet.m_is_hypercollinear) {
        continue;
      }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " looking at facet[" << i_facet << "]" << endl;
      }
      vdist = std::abs(facet.getSignedVerticalDistanceToZero(point)); // abs, //(repetita iuvant)
      if (vdist < dist) {
        i_facet_min = i_facet;
        dist = vdist;
      }
    ...
    if (dist == AUROSTD_MAX_DOUBLE) {
      if (i_facet_artificial > m_facets.size() - 1) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No nearest facet found");
      }
  ...
  const vector<uint>& ConvexHull::getRelevantFacets(uint i_nary, uint i_alloy) const {
    ...
    if (i_nary == 0) { // unaries are special case
      ...
      for (size_t fl_size_i_nary = m_naries.size(), i_nary = fl_size_i_nary - 1; i_nary < fl_size_i_nary; i_nary--) {
        for (size_t fl_size_i_alloy = m_naries[i_nary].m_alloys.size(), i_alloy = fl_size_i_alloy - 1; i_alloy <= fl_size_i_alloy; i_alloy--) {
          if (!m_naries[i_nary].m_alloys[i_alloy].m_facets.empty()) {
            return m_naries[i_nary].m_alloys[i_alloy].m_facets;
          }
        }
      }
    } else {
  ...
  void ConvexHull::setHullMembers() {
    return setHullMembers(m_i_facets);
  }
  ...
  void ConvexHull::setHullMembers(const vector<uint>& i_facets) {
    if (i_facets.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Hull has yet to be calculated");
    }
    uint i_point = AUROSTD_MAX_UINT;
    uint g_state = AUROSTD_MAX_UINT;
    uint i_coord_group = AUROSTD_MAX_UINT;
    ...
    for (size_t i = 0, fl_size_i = i_facets.size(); i < fl_size_i; i++) {
      ...
      for (size_t j = 0, fl_size_j = m_facets[i_facet].m_vertices.size(); j < fl_size_j; j++) {
        i_point = m_facets[i_facet].m_vertices[j].ch_index;
        if (!getCoordGroupIndex(i_point, i_coord_group)) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within coordgroups");
        }
        // hull member == yes
        ...
        if (m_thermo_hull) {
          m_coord_groups[i_coord_group].m_hull_member = i_point;
        } // very safe, only one hull-member per coordgroup
        // g-state == if not artificial point
        g_state = artificialMap(i_point);
        if (!m_points[g_state].m_is_artificial) {
          m_points[g_state].m_is_g_state = true;
        }
      }
    }
  }
  ...
  void ConvexHull::setNearestFacet(uint i_nary, uint i_alloy, uint i_coord_group) {
    ...
    const uint i_point = m_coord_groups[i_coord_group].m_points[0];
    if (!m_points[i_point].m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(i_point) + "] is not initialized");
    }
    ...
    const xvector<int>& elements_present = m_naries[i_nary].m_alloys[i_alloy].m_elements_present;
    point.setHullCoords(elements_present);
    const vector<uint>& i_facets = getRelevantFacets(i_nary, i_alloy); // m_naries[i_nary].m_alloys[i_alloy].m_facets;
    const uint i_facet = getNearestFacetVertically(i_facets, point);
  ...
  double ConvexHull::getSignedVerticalDistanceWithinCoordGroup(uint i_coord_group, const ChullPoint& point) const {
    ...
    const xvector<double> r_coords = point.getStoichiometricCoords();
    if (!identical(m_coord_groups[i_coord_group].m_coords, r_coords, ZERO_TOL)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point does not belong to coordgroup");
    }

    const uint ref_point = artificialMap(m_coord_groups[i_coord_group].m_points[0]); // don't need to set distance for artificial point, re-scale to this point
    if (!isViablePoint(ref_point)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup contains invalid point");
    }
    double ref_dist = 0.0;
    ...
    if (!m_coord_groups[i_coord_group].m_is_on_hull) {
      const uint i_facet = m_coord_groups[i_coord_group].m_nearest_facet;
      if (i_facet > m_facets.size() - 1) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within m_facets");
      }
    ...
    const double delta = (point.getLastCoord() - m_points[ref_point].getLastCoord());
    const double dist = ref_dist - delta;
    if (LDEBUG) {
  ...
  double ConvexHull::getDistanceToHull(uint i_point, bool redo, bool get_signed_distance) const {
    if (i_point > (m_points.size() - 1)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
    } // HE20210629 make comparison more precise
  ...
  double ConvexHull::getDistanceToHull(const ChullPoint& point, bool redo, bool get_signed_distance) const {
    ...
    if (!redo) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " attempting to find coordgroup for fast distance calculation" << endl;
      }
      uint i_coord_group = AUROSTD_MAX_UINT;
      ...
      if (getCoordGroupIndex(point, i_coord_group, true)) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " found coordgroup[" << i_coord_group << "] (coords=" << m_coord_groups[i_coord_group].m_coords << ")" << endl;
        }
        dist = getSignedVerticalDistanceWithinCoordGroup(i_coord_group, point);
        if (get_signed_distance == false) {
          dist = std::abs(dist);
        } // CO20180828 //CO20190808
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " dist=" << dist << endl;
        }
        return dist;
    ...
    if (m_has_stoich_coords) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " stoich_coords detected, trying to find relevant facets" << endl;
      }
      uint i_nary;
      uint i_alloy;
      if (!getAlloyIndex(point, i_nary, i_alloy)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot get alloys index");
      }
      if (i_nary == 0) { // unaries are special cases, they RARELY work with normal getSignedVerticalDistance()
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " found unary, need to use coordgroup to find distance (rarely works with normal getSignedVerticalDistance()" << endl;
        }
        uint i_coord_group = AUROSTD_MAX_UINT;
        if (!getCoordGroupIndex(point, i_coord_group, true)) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot find unary coordgroup");
        }
        dist = getSignedVerticalDistanceWithinCoordGroup(i_coord_group, point);
        if (get_signed_distance == false) {
          dist = std::abs(dist);
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " searching through ALL facets for closest vertical distance" << endl;
      }
    }
  ...
  vector<double> ConvexHull::getDistancesToHull(const vector<string>& vauid, bool redo) const {
    ...
    for (size_t i = 0, fl_size_i = vauid.size(); i < fl_size_i; i++) {
      ...
      if (!findPoint(auid, i_point)) {
        message << "Specified auid not found on hull (auid=" << auid << ")";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message);
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " grabbing distance to hull for i_point=" << i_point << endl;
      }
  ...
  bool ConvexHull::hullDistanceExtractionRequired() const {
    if (thermoPropertiesExtractionRequired()) {
      return true;
    } // dependency
    if (!m_cflags.flag("CHULL::MULTI_OUTPUT") && (m_cflags.flag("CHULL::LATEX_DOC") || m_cflags.flag("CHULL::PNG_IMAGE"))) {
      const aurostd::xoption lflags = resolvePlotLabelSettings();
      const bool meta_labels = lflags.flag("CHULL::PLOT_META_LABELS");
      const bool labels_off_hull = lflags.flag("CHULL::PLOT_LABELS_OFF_HULL");
      if (meta_labels && labels_off_hull) {
        return true;
      } // need distances to hull here
      const bool no_doc = m_cflags.flag("CHULL::NO_DOC");
      if (no_doc) {
        return false;
      }
      const bool image_only = m_cflags.flag("CHULL::IMAGE_ONLY");
      if (image_only) {
        return false;
      }
    }
  ...
  bool ConvexHull::thermoPropertiesExtractionRequired() const {
    if (thermoPostProcessingExtractionRequired()) {
      return true;
    } // dependency
    if (!m_cflags.flag("CHULL::MULTI_OUTPUT") && (m_cflags.flag("CHULL::LATEX_DOC") || m_cflags.flag("CHULL::PNG_IMAGE"))) {
      const aurostd::xoption lflags = resolvePlotLabelSettings();
      const bool icsd_labels = lflags.flag("CHULL::PLOT_ICSD_LABELS");
      if (icsd_labels) {
        return true;
      }
      const bool no_doc = m_cflags.flag("CHULL::NO_DOC");
  ...
  void ConvexHull::setDistancesToHull(uint i_nary, uint i_alloy) {
    ...
    const bool perform_hull_distance_calculation = (true && hullDistanceExtractionRequired() && (!m_cflags.flag("CHULL::SKIP_HULL_DISTANCE_CALCULATION")) &&
    ...
    if (!perform_hull_distance_calculation) {
      return;
    }
    ...
    for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups.size(); i < fl_size_i; i++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " looking at i_coord_group=" << i_coord_group << endl;
      }
      if (!m_coord_groups[i_coord_group].m_is_on_hull) {
        setNearestFacet(i_nary, i_alloy, i_coord_group);
      } // only off-hull need nearest facets
  ...
  void ConvexHull::setDistancesToHull(uint i_nary, uint i_alloy, uint i_coord_group) {
    ...
    if (m_coord_groups[i_coord_group].m_nearest_facet > m_facets.size() - 1) {
      setNearestFacet(i_nary, i_alloy, i_coord_group);
    }
    if (m_coord_groups[i_coord_group].m_points.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup[" + aurostd::utype2string(i_coord_group) + "] has no points");
    ...
    const bool check_dist_2_hull = false;
    if (!m_coord_groups[i_coord_group].m_is_on_hull) { // off-hull, this is the reason it works for unaries, unaries coordgroup are declared ON-HULL, so dist==0 automatically
      ...
      if (m_half_hull) { // tests of stupidity
        ...
        // sign of distance:
        // independent of lower/upper hull:  above hull is negative, below hull is positive
        const bool should_be_positive = !m_lower_hull;
        const bool correct_sign_vertical_distance = chull::correctSignVerticalDistance(dist_2_hull, should_be_positive);
        if (m_lower_hull && !correct_sign_vertical_distance) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "(lower half hull) found point BELOW hull (entry=" + point.m_entry.auid + ",dist2Hull=" + aurostd::utype2string(dist_2_hull, 4) + ")");
        }
        if (!m_lower_hull && !correct_sign_vertical_distance) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "(upper half hull) found point ABOVE hull (entry=" + point.m_entry.auid + ",dist2Hull=" + aurostd::utype2string(dist_2_hull, 4) + ")");
        }
      }
    }
    ...
    point.m_dist_2_hull = m_coord_groups[i_coord_group].m_nearest_distance = std::abs(dist_2_hull);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " |dist|=" << point.m_dist_2_hull << endl;
    }
    uint j_point;
    for (size_t j = 0, fl_size_j = m_coord_groups[i_coord_group].m_points.size(); j < fl_size_j; j++) {
      ...
      if (i_point != j_point) {
        m_points[j_point].m_dist_2_hull += std::abs(point.getLastCoord() - m_points[j_point].getLastCoord());
  ...
  vector<uint> ConvexHull::extractDecompositionPhases(const ChullFacet& facet) const {
    ...
    if (decomp_phases.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No decomposition phases found");
    }
    std::sort(decomp_phases.begin(), decomp_phases.end(), sortCHullPoints(m_points, false, true)); // stoich sorting should be descending, energy sorting default okay here
  ...
  vector<uint> ConvexHull::getDecompositionPhases(const ChullPoint& point) const {
    ...
    if (point.m_is_on_hull) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No decomposition coefficients for hull members");
    }
    uint i_facet = AUROSTD_MAX_UINT;
    // polymorph possibility, other reductions in dimensionality are prescribed with the coefficients
    uint i_coord_group = AUROSTD_MAX_UINT;
    const bool found_coord_group = getCoordGroupIndex(point, i_coord_group);
    ...
    if (!found_coord_group || (i_facet > m_facets.size() - 1)) {
  ...
  void ConvexHull::setDecompositionPhases(uint i_nary, uint i_alloy, uint i_coord_group) {
    ...
    if (m_coord_groups[i_coord_group].m_is_on_hull) {
      if (!isViablePoint(m_coord_groups[i_coord_group].m_hull_member)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No hull member set for m_coord_group[" + aurostd::utype2string(i_coord_group) + "]", _RUNTIME_ERROR_);
      }
      vector<uint> dcomp_phases;
      dcomp_phases.push_back(m_coord_groups[i_coord_group].m_hull_member);
  ...
  xvector<double> ConvexHull::getDecompositionCoefficients(const ChullPoint& point, vector_reduction_type vred) const {
    ...
    if (found_coord_group) { // composition has already been considered by hull, might be g-state
      if (m_coord_groups[i_coord_group].m_is_on_hull) {
        const xvector<double> dcomp_coefs(2);
        dcomp_coefs[dcomp_coefs.lrows] = dcomp_coefs[dcomp_coefs.lrows + 1] = 1.0;
        return dcomp_coefs;
      }
      if (!m_coord_groups[i_coord_group].m_decomp_phases.empty()) {
  ...
  xvector<double> ConvexHull::getDecompositionCoefficients(const ChullPoint& point, const vector<uint>& decomp_phases, vector_reduction_type vred) const {
    ...
    if (decomp_phases.size() == 1) { // probably polymorph
      const uint i_point_other = decomp_phases[0];
      if (!isViablePoint(i_point_other)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Index to decomposition phase not found");
      }
      const ChullPoint& point_other = m_points[i_point_other];
      uint i_coord_group = AUROSTD_MAX_UINT;
      uint i_coord_group_other = AUROSTD_MAX_UINT;
      const bool found_coord_group = getCoordGroupIndex(point, i_coord_group);
      const bool found_coord_group_other = getCoordGroupIndex(point_other, i_coord_group_other);
    ...
    // rhs
    for (size_t i = 0, fl_size_i = decomp_phases.size(); i < fl_size_i; i++) {
      if (decomp_phases[i] > m_points.size() - 1) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
      }
      if (!m_points[decomp_phases[i]].m_initialized) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized point");
      }
      rhs.push_back(m_points[decomp_phases[i]].getTruncatedReducedCoords(m_elements_present, vred));
    }
    const xvector<double> coef = balanceChemicalEquation(lhs, rhs, true, ZERO_TOL); // normalize
  ...
  void ConvexHull::setDecompositionCoefficients(uint i_nary, uint i_alloy, uint i_coord_group) {
    ...
    const vector<uint>& decomp_phases = m_coord_groups[i_coord_group].m_decomp_phases;
  ...
  vector<uint> ConvexHull::getAdjacentFacets(uint hull_member, bool ignore_hypercollinear, bool ignore_vertical, bool ignore_artificial) const {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (hull_member > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No hull member has been identified");
    }
    vector<uint> adjacent_i_facets;
    if (!m_points[hull_member].m_is_on_hull) {
    ...
    const uint i_nary = m_points[hull_member].m_i_nary;
    const uint i_alloy = m_points[hull_member].m_i_alloy;
    if (i_nary > m_naries.size() - 1 || i_alloy > m_naries[i_nary].m_alloys.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within m_naries");
    }
    if (!m_naries[i_nary].m_initialized) {
    ...
    for (size_t i = 0, fl_size_i = i_facets.size(); i < fl_size_i; i++) {
      ...
      if (ignore_hypercollinear && m_facets[i_facet].m_is_hypercollinear) {
        continue;
      } // really (d-1) facets
      if (ignore_vertical && m_facets[i_facet].m_is_vertical) {
        continue;
      } // really (d-1) facets
      if (ignore_artificial && m_facets[i_facet].m_is_artificial) {
        continue;
      } // contains all unaries
      if (m_facets[i_facet].isPointOnFacet(hull_member)) {
        adjacent_i_facets.push_back(i_facet);
      }
    }
    if (adjacent_i_facets.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No adjacent facets found");
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " stop" << endl;
    }
    return adjacent_i_facets;
  }
  ...
  vector<vector<uint>> ConvexHull::getEquilibriumPhases(uint hull_member) const {
    ...
    if (!point.m_is_on_hull) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No equilibrium phases for non hull members");
    }
    ...
    for (size_t i = 0, fl_size_i = adjacent_i_facets.size(); i < fl_size_i; i++) {
      adjacent_facets.push_back(m_facets[adjacent_i_facets[i]]);
    }

    // sort
    for (size_t i_facet = 0, fl_size_i_facet = adjacent_facets.size(); i_facet < fl_size_i_facet; i_facet++) {
      std::sort(adjacent_facets[i_facet].m_vertices.begin(), adjacent_facets[i_facet].m_vertices.end(), sortThermoPoints(false, true)); // stoich sorting should be descending, energy sorting default okay here
    }
    std::sort(adjacent_facets.begin(), adjacent_facets.end(), sortFacetsByPoints(m_points, false, false, false, false)); // not auto sort, sort everything descending

    uint i_point = AUROSTD_MAX_UINT;
    for (size_t i_facet = 0, fl_size_i_facet = adjacent_facets.size(); i_facet < fl_size_i_facet; i_facet++) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " found new relevant facet[" << i_facet << "] with points: ";
      }
      equilibrium_phases.emplace_back(0);
      for (size_t i = 0, fl_size_i = adjacent_facets[i_facet].m_vertices.size(); i < fl_size_i; i++) {
        i_point = adjacent_facets[i_facet].m_vertices[i].ch_index;
        equilibrium_phases.back().push_back(i_point);
        if (LDEBUG) {
          cerr << m_points[i_point].h_coords << " [" << i_point << "]" << (i != adjacent_facets[i_facet].m_vertices.size() - 1 ? ", " : "");
        }
      }
      if (LDEBUG) {
        cerr << endl;
  ...
  void ConvexHull::setEquilibriumPhases(uint i_nary, uint i_alloy, uint i_coord_group) {
    ...
    const uint hull_member = m_coord_groups[i_coord_group].m_hull_member;
    if (hull_member > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No hull member has been identified");
    }
    if (!m_points[hull_member].m_is_on_hull) {
    ...
    if (m_points[hull_member].isUnary()) {
      return;
    } // others are in equilibrium with it, must be context of mixture!

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " looking at hull-member[" << hull_member << "]=" << m_points[hull_member].h_coords << endl;
    }
  ...
  bool ConvexHull::phasesEquivalent(uint i_point1, uint i_point2, bool perform_structure_comparison) const {
    const bool LDEBUG = (false || _DEBUG_CHULL_ || XHOST.DEBUG);
    if (i_point1 > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
    }
    if (i_point2 > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
    }
    if (!m_points[i_point1].m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(i_point1) + "] is not initialized");
    }
    if (!m_points[i_point2].m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(i_point2) + "] is not initialized");
    }

    if (i_point1 == i_point2) {
      return true;
    } // it is equivalent to self, so we only do checks if necessary

    // return false early for points without an entry
    if (!m_points[i_point1].m_has_entry) {
      return false;
    }
    if (!m_points[i_point2].m_has_entry) {
      return false;
    }

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " comparing [auid=" << m_points[i_point1].m_entry.auid << "] and [auid=" << m_points[i_point2].m_entry.auid << "]" << endl;
    }
    ...
    if (energiesDiffer(i_point1, i_point2, false)) {
      return false;
    } // first filter by those with wildly different energies, not strict
    if (spacegroupsDiffer(i_point1, i_point2, false)) {
      return false;
    } // first filter by those with wildly different spacegroups, not strict
    if (!perform_structure_comparison) {
      return false;
    } // only return true if you can compare structures
    if (!structuresEquivalent(i_point1, i_point2)) {
      return false;
    }
  ...
  bool ConvexHull::energiesDiffer(uint i_point1, uint i_point2, bool strict) const {
    ...
    if (!m_points[i_point1].m_has_entry) {
      return strict;
    } // if strict, then return that they do differ (true)
    if (!m_points[i_point2].m_has_entry) {
      return strict;
    } // if strict, then return that they do differ (true)
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << energy1 << " vs. " << energy2 << endl;
    }
    if (energy1 >= AUROSTD_NAN || energy2 >= AUROSTD_NAN) {
      return strict;
    } // if strict, then return that they do differ (true)

    const bool differs = aurostd::isdifferent(energy1, energy2, ENERGY_TOL);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " energies do " << (differs ? "" : "NOT ") << "differ" << endl;
    }
    return differs;
  }
  ...
  bool ConvexHull::spacegroupsDiffer(uint i_point1, uint i_point2, bool strict) const {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << sg1 << " vs. " << sg2 << endl;
    }
    if (sg1 == NOSG || sg2 == NOSG) {
      return strict;
    } // if strict, then return that they do differ (true)

    const bool differs = (sg1 != sg2);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " spacegroups do " << (differs ? "" : "NOT ") << "differ" << endl;
    }
    return differs;
  }
  ...
  bool ConvexHull::structuresEquivalent(uint i_point1, uint i_point2) const {
    ...
    xstructure a;
    xstructure b;
    if (!m_points[i_point1].getMostRelaxedXstructure(a)) {
      return false;
    }
    if (!m_points[i_point2].getMostRelaxedXstructure(b)) {
      return false;
    }
    ...
    const bool are_equivalent = (compare::structuresMatch(a, b, true, false, false) ||
                                 compare::structuresMatch(b, a, true, false, false)); // match species and use fast match, but not scale volume, two structures with different volumes (pressures) are different! //DX20180123 - added fast_match = true //DX20190318 - not fast_match but optimized_match=false //CO20221112 - bit OR short circuit, there are some structures where compare_materials is input-specific
    if (LDEBUG) {
  ...
  vector<uint> ConvexHull::getEquivalentEntries(uint i_point) const {
    ...
    for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
      ...
      if (!phasesEquivalent(i_point, j_point, perform_structure_comparison)) {
        continue;
      }
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = equivalent_entries.size(); i < fl_size_i; i++) {
        cerr << equivalent_entries[i] << (i != equivalent_entries.size() - 1 ? ", " : "");
      }
      cerr << endl;
    }
  ...
  bool ConvexHull::isICSD(uint i_point, uint& i_point_icsd) const {
    ...
    const string point_icsd_number = getICSDNumber(i_point, true);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " looking for icsd_number=" << point_icsd_number << endl;
    }
    if (point_icsd_number.empty()) {
      return false;
    } // no possible match
    if (m_icsd_entries.empty()) {
      return false;
    } // no possible match
    string entry_icsd_number;
    for (size_t i = 0, fl_size_i = m_icsd_entries.size(); i < fl_size_i; i++) {
      if (i_point == m_icsd_entries[i]) {
        i_point_icsd = m_icsd_entries[i];
        return true;
      } // instant match
      entry_icsd_number = getICSDNumber(m_icsd_entries[i], true);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " comparing with entry_icsd_number=" << entry_icsd_number << endl;
      }
      if (entry_icsd_number.empty()) {
        continue;
      } // this is weird/bad, but let's not exit the whole program
  ...
  void ConvexHull::setEquivalentGStates(uint i_nary, uint i_alloy, uint i_coord_group) {
    ...
    const uint g_state = m_coord_groups[i_coord_group].m_ref_state;
    if (g_state > m_points.size() - 1) {
      return;
    }
    if (!m_points[g_state].m_has_entry) {
      return;
    } // throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"No entry (structure) found"); //only point in coordgroup

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " looking at g-state[" << g_state << "]=" << m_points[g_state].h_coords << endl;
    }
    m_coord_groups[i_coord_group].m_equivalent_g_states = getEquivalentEntries(g_state);
    if (m_coord_groups[i_coord_group].m_equivalent_g_states.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No equivalent states found (not even self)");
    }
    m_coord_groups[i_coord_group].m_calculated_equivalent_g_states = true;
    uint i_point = AUROSTD_MAX_UINT;
    uint i_point_icsd = AUROSTD_MAX_UINT;
    bool found_icsd = false;
    bool _found_icsd = false; // whether icsd exists among g_states
    ...
    for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_equivalent_g_states.size(); i < fl_size_i; i++) {
  ...
  void ConvexHull::setEquivalentStates(uint i_nary, uint i_alloy, uint i_coord_group) {
    ...
    double dist = 0.0;
    if (m_cflags.flag("CHULL::DIST_FOR_EQUIVALENCE_ANALYSIS")) {
      dist = aurostd::string2utype<double>(m_cflags.getattachedscheme("CHULL::DIST_FOR_EQUIVALENCE_ANALYSIS"));
    }
    const bool ND_only = m_cflags.flag("CHULL::DIST_FOR_EQUIVALENCE_ANALYSIS_ND_ONLY");
    if ((!(ND_only && i_nary != (m_dim - 1))) && aurostd::nonZeroWithinTol(dist, ZERO_TOL)) {
      ...
      if (!m_velements.empty()) {
        message << " " << aurostd::joinWDelimiter(alloyToElements(i_nary, i_alloy), "-") << " hull";
      }
      ...
      for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
        ...
        if (aurostd::WithinList(vpoints2check, i_point)) {
          continue;
        }
        if (!m_points[i_point].m_initialized) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(i_point) + "] is not initialized");
        }
        if (m_points[i_point].getDist2Hull(_m_) <= dist) {
          vpoints2check.push_back(i_point);
        }
      }
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vpoints2check=" << aurostd::joinWDelimiter(vpoints2check, ",") << endl;
    }

    if (vpoints2check.empty()) {
      return;
    }

    uint j_point = AUROSTD_MAX_UINT;
    uint j_point_icsd = AUROSTD_MAX_UINT;
    //
    bool found_icsd = false;
    bool _found_icsd = false; // whether icsd exists among equivalent entries
    uint i_canonical_icsd = AUROSTD_MAX_UINT; // index of canonical icsd
    uint canonical_icsd_number = AUROSTD_MAX_UINT; // lowest number
    uint icsd_number = AUROSTD_MAX_UINT;

    for (size_t i = 0, fl_size_i = vpoints2check.size(); i < fl_size_i; i++) {
      i_point = vpoints2check[i];
      if (i_point > m_points.size() - 1) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
      }
      ChullPoint& point_i = m_points[i_point];
      if (!point_i.m_has_entry) {
        continue;
      } // throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"No entry (structure) found"); //only point in coordgroup
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " looking at i_point[" << i_point << "]=" << m_points[i_point].h_coords << endl;
      }
      if (point_i.m_calculated_equivalent_entries == true) {
        continue;
      } // speed up
      point_i.m_equivalent_entries = getEquivalentEntries(i_point);
      if (point_i.m_equivalent_entries.empty()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No equivalent states found (not even self)");
      }
      ...
      for (size_t j = 0, fl_size_j = point_i.m_equivalent_entries.size(); j < fl_size_j; j++) {
        j_point = point_i.m_equivalent_entries[j];
        if (j_point > m_points.size() - 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
        }
        if (!m_points[j_point].m_initialized) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(j_point) + "] is not initialized");
        }
  ...
  vector<uint> ConvexHull::getSymEquivalentGStates(uint g_state) const {
    ...
    if (!point.m_is_g_state) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No equivalent g-states for non ground-state structures");
    }
    uint i_coord_group = AUROSTD_MAX_UINT;
    if (!getCoordGroupIndex(point, i_coord_group)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within coordgroups");
    ...
    for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
      ...
      if (g_state != i_point) {
        if (spacegroupsDiffer(g_state, i_point, true)) {
          continue;
        }
      }
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0, fl_size_i = sym_equivalent_g_states.size(); i < fl_size_i; i++) {
        cerr << sym_equivalent_g_states[i] << (i != sym_equivalent_g_states.size() - 1 ? ", " : "");
      }
      cerr << endl;
    }
  ...
  void ConvexHull::setSymEquivalentGStates(uint i_nary, uint i_alloy, uint i_coord_group) {
    ...
    uint i_point = AUROSTD_MAX_UINT;
    for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_sym_equivalent_g_states.size(); i < fl_size_i; i++) {
  ...
  void ConvexHull::extractThermodynamicProperties(uint i_nary, uint i_alloy) {
    ...
    const bool perform_thermo_properties_extraction =
        (true && thermoPropertiesExtractionRequired() && (!m_cflags.flag("CHULL::SKIP_HULL_DISTANCE_CALCULATION")) && (!m_cflags.flag("CHULL::SKIP_THERMO_PROPERTIES_EXTRACTION")) &&
    ...
    if (!perform_thermo_properties_extraction) {
      return;
    }
    if (!m_thermo_hull) {
  ...
  void ConvexHull::thermodynamicsPostProcessing() {
    ...
    const bool perform_thermo_post_processing = (true && thermoPostProcessingExtractionRequired() && (!m_cflags.flag("CHULL::SKIP_HULL_DISTANCE_CALCULATION")) &&
                                                 (!m_cflags.flag("CHULL::SKIP_THERMO_PROPERTIES_EXTRACTION")) && (!m_cflags.flag("CHULL::SKIP_THERMO_POST_PROCESSING")) &&
                                                 true); //(1&&!(m_cflags.flag("CHULL::SKIP_THERMO_PROPERTIES_EXTRACTION")||(!m_cflags.flag("CHULL::MULTI_OUTPUT")&&m_cflags.flag("CHULL::LATEX_DOC")&&m_cflags.flag("CHULL::IMAGE_ONLY"))));
    if (!perform_thermo_post_processing) {
      return;
    }
    ...
    const bool perform_stability_criterion = (true && (!m_cflags.flag("CHULL::SKIP_STABILITY_CRITERION_ANALYSIS"))); //(1&&!(m_cflags.flag("CHULL::SKIP_STABILITY_CRITERION_ANALYSIS")||(!m_cflags.flag("CHULL::MULTI_OUTPUT")&&m_cflags.flag("CHULL::LATEX_DOC")&&m_cflags.flag("CHULL::IMAGE_ONLY"))));
    if (perform_stability_criterion) {
      setStabilityCriterion();
    }

    const bool perform_n_plus_1_enthalpy_gain = (true && (!m_cflags.flag("CHULL::SKIP_N+1_ENTHALPY_GAIN_ANALYSIS"))); //(1&&!(m_cflags.flag("CHULL::SKIP_N+1_ENTHALPY_GAIN_ANALYSIS")||(!m_cflags.flag("CHULL::MULTI_OUTPUT")&&m_cflags.flag("CHULL::LATEX_DOC")&&m_cflags.flag("CHULL::IMAGE_ONLY"))));
    if (perform_n_plus_1_enthalpy_gain) {
      setNPlus1EnthalpyGain();
    }
  ...
  void ConvexHull::calculate() {
    ...
    if (m_has_stoich_coords) {
      if (m_naries.empty()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Points have yet to be structured");
      }
      const bool calc_highest_hull_only = m_cflags.flag("CHULL::CALCULATE_HIGHEST_DIMENSION_ONLY");
      ...
      for (size_t i_nary = i_nary_start, fl_size_i_nary = m_naries.size(); i_nary < fl_size_i_nary; i_nary++) {
        for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
          ...
          if (m_thermo_hull) {
            extractThermodynamicProperties(i_nary, i_alloy);
          }
        }
      }
      ...
      for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[0].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
        setDistancesToHull(0, i_alloy);
      } // do this always
      if (m_thermo_hull) { // very safe, not sure how these algorithms perform outside of this domain, already know we have m_has_stoich_coords
        ...
        // pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
        for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[0].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
          extractThermodynamicProperties(0, i_alloy);
        } // knows to skip facet storage //setOnHullProperties(0,i_alloy);
      }
      // print counts of g_states by dimension
      uint gstate_count = 0;
      uint total_gstate_count = 0;
      for (size_t i_nary = 0, fl_size_i_nary = m_naries.size(); i_nary < fl_size_i_nary; i_nary++) {
        gstate_count = getGStateCount(i_nary);
        message << "Found " << gstate_count << " " << pflow::arity_string(i_nary + 1, false, false) << " ";
        if (m_thermo_hull) {
          message << "ground-state phases";
        } else {
          message << "points on the hull";
        }
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
  ...
  double ConvexHull::getStabilityCriterion(const string& cauid) const {
    ...
    vector<string> vcauid;
    vcauid.push_back(cauid);
  ...
  double ConvexHull::getStabilityCriterion(uint cpoint) const {
    ...
    if (cpoint > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
    }
    vector<uint> vcpoints;
    vcpoints.push_back(cpoint);
  ...
  vector<double> ConvexHull::getStabilityCriterion(const vector<string>& vcauid) const {
    ...
    for (size_t i = 0, fl_size_i = vcauid.size(); i < fl_size_i; i++) {
  ...
  vector<double> ConvexHull::getStabilityCriterion(const vector<uint>& vcpoints) const {
    ...
    // set fake_hull
    const xvector<int> elements_present_points = getElementsPresent(vcpoints);
    const xvector<int> elements_present_hull = elements_present_points;
    if (sum(elements_present_hull) == 1) {
      // pick ANY other index to be 1 as well
      if (elements_present_hull[elements_present_hull.lrows] == 0) {
        elements_present_hull[elements_present_hull.lrows] = 1;
      } else {
        if (elements_present_hull.rows < 2) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "elements_present.rows<2", _INPUT_ILLEGAL_);
        } // we cannot have unary hulls as input (facets issue)
        if (elements_present_hull[elements_present_hull.lrows + 1] == 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "elements_present mismatch for unary hull", _INDEX_MISMATCH_);
        }
        elements_present_hull[elements_present_hull.lrows + 1] = 1;
      }
    }
    if (sum(elements_present_hull) == 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Could not fix sum(elements_present_hull)==1", _RUNTIME_ERROR_);
    }
    ...
    ChullPoint point;
    uint i_point = AUROSTD_MAX_UINT;
    for (size_t i = 0, fl_size_i = vcpoints.size(); i < fl_size_i; i++) {
      i_point = vcpoints[i];
      ...
      // }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " m.coords=" << point.m_coords << endl;
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " h.coords=" << point.h_coords << endl;
      }
  ...
  void ConvexHull::getFakeHull(const vector<uint>& vcpoints, ConvexHull& fake_hull) const {
    ...
    const xvector<int> elements_present = aurostd::ones_xv<int>(m_points[0].m_coords.lrows, m_points[0].m_coords.lrows + (int) getDim() - 1);
  ...
  void ConvexHull::getFakeHull(const vector<uint>& vcpoints, const xvector<int>& elements_present_hull, const xvector<int>& elements_present_points, ConvexHull& fake_hull) const {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " initial points_to_neglect.size()=" << vcpoints.size() << endl;
    }
    ...
    for (size_t i = 0, fl_size_i = vcpoints.size(); i < fl_size_i; i++) {
      ...
      if (!getCoordGroupIndex(point, i_coord_group)) {
        message << "Coordgroup index not set (input[" << i << "])";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message);
      }
      if (!m_coord_groups[i_coord_group].m_initialized) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized coordgroup");
      ...
      if (m_coord_groups[i_coord_group].m_points.empty()) {
        message << "No points found within coordgroup (input[" << i << "])";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message);
      }
      ...
      if (m_coord_groups[i_coord_group].m_is_on_hull) {
        ...
        if (!m_points[g_state].m_is_g_state) {
          message << "Coordgroup reference is not a ground-state structure (input[" << i << "])";
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message);
        }
        // const vector<uint>& eq_gstates=m_coord_groups[i_coord_group].m_equivalent_g_states;
        if (m_coord_groups[i_coord_group].m_calculated_equivalent_g_states) {
          eq_gstates = m_coord_groups[i_coord_group].m_equivalent_g_states;
        } else {
          eq_gstates = getEquivalentEntries(g_state);
        }
        if (eq_gstates.empty()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No equivalent states found (not even self)");
        }
        // for(size_t j=0,fl_size_j=eq_gstates.size();j<fl_size_j;j++){points_to_neglect.push_back(m_points[eq_gstates[j]]);}
        for (size_t j = 0, fl_size_j = eq_gstates.size(); j < fl_size_j; j++) {
          points_to_neglect.push_back(eq_gstates[j]);
        }
      }
    }

    std::sort(points_to_neglect.begin(), points_to_neglect.end());
    points_to_neglect.erase(std::unique(points_to_neglect.begin(), points_to_neglect.end()), points_to_neglect.end()); // first remove duplicate indices

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " points_to_neglect.size()=" << points_to_neglect.size() << endl;
      cerr << __AFLOW_FUNC__ << " points_to_neglect: ";
      for (size_t i = 0, fl_size_i = points_to_neglect.size(); i < fl_size_i; i++) {
        cerr << points_to_neglect[i] << " ";
      }
      cerr << endl;
    }
    ...
    if (sum(elements_present_points) > sum(elements_present_hull)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "elements_present_hull must belong to bigger subspace than elements_present_points", _INPUT_ILLEGAL_);
    }
    ...
    for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " m_coord_groups[i_coord_group=" << i_coord_group << "].getElementsPresent()=" << m_coord_groups[i_coord_group].getElementsPresent() << endl;
      }
      if (!subspaceBelongs(elements_present_points, m_coord_groups[i_coord_group].getElementsPresent())) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " Ignoring irrelevant subspace of m_coord_groups[i_coord_group=" << i_coord_group << "] (m_coord_groups[i_coord_group=" << i_coord_group
               << "].getElementsPresent()=" << m_coord_groups[i_coord_group].getElementsPresent() << ")" << endl;
        }
        continue;
      }
      for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
        ...
        if (aurostd::WithinList(points_to_neglect, i_point)) {
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " Removing point from fake hull (i_point=" << i_point << ",auid=" << m_points[i_point].m_entry.auid << ")" << endl;
          }
          continue;
        }
        const ChullPoint& point = m_points[i_point];
        if (point.m_is_artificial) {
          continue;
        ...
        // if(found){continue;}
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " including point[i_point=" << i_point << ",auid=" << m_points[i_point].m_entry.auid << "].compound=" << point.m_entry.compound << endl;
        }
        new_points.push_back(point);
        new_points.back().reduceCoords(elements_present_hull);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " point[" << i_point << "].m_coords=" << new_points.back().m_coords << endl;
        }
        new_points.back().cleanPointForHullTransfer();
      }
    }
    if (new_points.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No points found for pseudo convex hull");
    }
    ...
    if (!m_velements.empty()) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " relevant_indices=" << aurostd::joinWDelimiter(relevant_indices, ",") << endl;
      }
      for (size_t i = 0, fl_size_i = relevant_indices.size(); i < fl_size_i; i++) {
        velements.push_back(m_velements[relevant_indices[i] - 1]);
      } //-1 because xvector->vecto
      if (LDEBUG) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Creating new (pseudo) hull without relevant g-states" << endl;
    }
    ...
    const bool see_sub_output = false; // true;
    std::streambuf* buf_fh;
    if (see_sub_output) {
      buf_fh = std::cout.rdbuf();
    } // to cout
    else {
      buf_fh = devnull.rdbuf();
    } // to devnull
    ...
    if (!fake_hull.m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Could not create pseudo convex hull");
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " New (pseudo) hull created" << endl;
    }
  }
  ...
  vector<double> ConvexHull::getNPlus1EnthalpyGain(const vector<string>& vcauid) const {
    ConvexHull fake_hull;
    return getNPlus1EnthalpyGain(vcauid, fake_hull, false);
  }
  ...
  vector<double> ConvexHull::getNPlus1EnthalpyGain(const vector<uint>& vcpoints) const {
    ConvexHull fake_hull;
    return getNPlus1EnthalpyGain(vcpoints, fake_hull, false);
  }
  vector<double> ConvexHull::getNPlus1EnthalpyGain(const vector<uint>& vcpoints, ConvexHull& fake_hull, bool hull_set) const {
    ...
    for (size_t i = 0, fl_size_i = vcpoints.size(); i < fl_size_i; i++) {
      vnp1eg.push_back(getNPlus1EnthalpyGain(vcpoints[i], fake_hull, hull_set));
    }
    return vnp1eg;
  }
  double ConvexHull::getNPlus1EnthalpyGain(uint i_point) const {
    ConvexHull fake_hull;
    return getNPlus1EnthalpyGain(i_point, fake_hull, false);
  }
  double ConvexHull::getNPlus1EnthalpyGain(uint i_point, ConvexHull& fake_hull, bool hull_set) const {
    ...
    if (point.m_i_nary == 0) {
      return AUROSTD_MAX_DOUBLE;
    } // N+1(unary)=formation_enthalpy (distance from 0 point) - TECHNICALLY true (allowed in polytope theory), but our definition for N+1 enthalpy gain really starts for binaries (Delta H[{N|1,...,N-1}]), can NOT have 1|0
    const bool force_calc_binaries = false; // set true for debugging
    if (point.m_i_nary == 0 || (point.m_i_nary == 1 && force_calc_binaries == false)) { // N+1(unary)=formation_enthalpy (distance from 0 point), N+1(binary)=formation_enthalpy (distance from 0 tieline)
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " found " << (point.m_i_nary == 0 ? "unary" : "binary") << ", returning abs(H_f_atom)" << endl;
      } // enthalpy_formation_atom
      if (point.m_has_entry && H_f_atom(point.m_entry) != AUROSTD_NAN) {
        return std::abs(H_f_atom(point.m_entry));
      } // point.m_entry.enthalpy_formation_atom
      return AUROSTD_MAX_DOUBLE; // 0; //return null
    }

    // set fake_hull
    const xvector<int> elements_present_points = point.m_elements_present;
    const xvector<int> elements_present_hull = elements_present_points;
    if (sum(elements_present_hull) == 1) {
      // pick ANY other index to be 1 as well
      if (elements_present_hull[elements_present_hull.lrows] == 0) {
    ...
    if (hull_set == false) {
      uint j_nary = AUROSTD_MAX_UINT;
      uint j_alloy = AUROSTD_MAX_UINT;
      if (!getAlloyIndex(point, j_nary, j_alloy)) {
        message << "Alloy index not set (auid=" << point.m_entry.auid << ")";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message);
      }
      uint i_coord_group = AUROSTD_MAX_UINT;
      vector<uint> vcpoints;
      for (size_t fl_size_i_nary = m_naries.size(), i_nary = (fl_size_i_nary - 1); i_nary < fl_size_i_nary && i_nary >= j_nary; i_nary--) { // go backwards!
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " removing i_nary=" << i_nary << endl;
        }
        for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
          for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups.size(); i < fl_size_i; i++) {
            i_coord_group = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups[i];
  ...
  void ConvexHull::setStabilityCriterion() {
    ...
    double scriterion;
    vector<uint> eq_gstates;
    for (size_t i = 0, fl_size_i = g_states.size(); i < fl_size_i; i++) {
      i_point = g_states[i];
      if (i_point > m_points.size() - 1) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
      }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " looking at point[i_point=" << i_point << "].m_coords=" << m_points[i_point].m_coords << endl;
      }
      ...
      if (!m_coord_groups[i_coord_group].m_is_on_hull) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Coordgroup is not on the hull");
      }
      ...
      if (m_coord_groups[i_coord_group].m_calculated_equivalent_g_states) {
        ...
        eq_gstates = getEquivalentEntries(i_point);
      }
      if (eq_gstates.empty()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No equivalent states found (not even self)");
      }
      for (size_t j = 0, fl_size_j = eq_gstates.size(); j < fl_size_j; j++) {
        if (eq_gstates[j] > m_points.size() - 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
        }
        if (!m_points[eq_gstates[j]].m_initialized) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(eq_gstates[j]) + "] is not initialized");
        }
        ...
        if (std::signbit(m_points[i_point].m_stability_criterion) != std::signbit(scriterion)) {
          scriterion = 0;
        } // define these to be 0, AlFe hull look at Fe
        m_points[eq_gstates[j]].m_stability_criterion = scriterion;
      }
    }
  }

  void ConvexHull::setNPlus1EnthalpyGain(uint i_point) {
    ConvexHull fake_hull;
    return setNPlus1EnthalpyGain(i_point, fake_hull, false);
  }
  void ConvexHull::setNPlus1EnthalpyGain(uint i_point, ConvexHull& fake_hull, bool hull_set) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " looking at point[i_point=" << i_point << "].m_coords=" << m_points[i_point].m_coords << ", compound=" << m_points[i_point].m_entry.compound << endl;
    }
    ...
    if (m_points[i_point].m_i_nary == 1) {
      if (m_points[i_point].m_has_entry && H_f_atom(m_points[i_point].m_entry) != AUROSTD_NAN) { // m_points[i_point].m_entry.enthalpy_formation_atom
        if (!aurostd::identical(aurostd::abs(np1egain), aurostd::abs(H_f_atom(m_points[i_point].m_entry)), ZERO_MEV_TOL)) { // m_points[i_point].m_entry.enthalpy_formation_atom
          message << "std::abs(np1egain) != abs(H_f_atom(m_points[i_point].m_entry)) [ " << std::abs(np1egain) << " != " << std::abs(H_f_atom(m_points[i_point].m_entry)) << " ], please check"; // m_points[i_point].m_entry.enthalpy_formation_atom
    ...
    for (size_t j = 0, fl_size_j = eq_gstates.size(); j < fl_size_j; j++) {
      ...
      // unaries could be really close to 0
      // don't set if it becomes positive
      if (std::signbit(m_points[i_point].m_n_plus_1_enthalpy_gain) != std::signbit(np1egain)) {
        np1egain = 0;
      } // define these to be 0, AlFe hull look at Fe
  ...
  void ConvexHull::setNPlus1EnthalpyGain() {
    ...
    for (i = 0, fl_size_i = g_states.size(); i < fl_size_i; i++) {
      ...
      if (!found) {
        ...
        v_g_states.emplace_back(0);
        v_g_states.back().push_back(g_states[i]);
      }
    }
    if (v_elements_present.size() != v_g_states.size()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "v_elements_present.size()!=v_g_states.size()", _INDEX_MISMATCH_);
    }
  ...
  string ConvexHull::getICSDNumber(const aflowlib::_aflowlib_entry& entry, bool remove_suffix) const {
    ...
    if (!aurostd::substring2bool(proto, "ICSD_")) {
      return "";
    }
    ...
    string icsd_number;
    if (tokens.size() == 2) {
      icsd_number = tokens[1];
    } // aurostd::substring2bool(proto,"ICSD_")
    else if (tokens.size() == 3 && aurostd::substring2bool(proto, "_ICSD_")) {
      icsd_number = tokens[2];
    }
    if (icsd_number.empty()) {
      return icsd_number;
    }
    if (remove_suffix) {
      aurostd::string2tokens(icsd_number, tokens, ".");
      if (!tokens.empty()) {
        icsd_number = tokens[0];
      }
      aurostd::string2tokens(icsd_number, tokens, ":");
      if (!tokens.empty()) {
        icsd_number = tokens[0];
  ...
  string ConvexHull::prettyPrintPrototype(const aflowlib::_aflowlib_entry& entry, bool double_back_slash, bool icsd_label_skim) const { // main function
    ...
    const bool fix_icsd_labels = true;
    const string proto = entry.prototype;
    if (fix_icsd_labels && aurostd::substring2bool(proto, "ICSD_")) {
      const vector<string> tokens;
      const string string_to_fix = getICSDNumber(entry, icsd_label_skim == true); // remove suffix if skim label
      // aurostd::string2tokens(proto,tokens,"_");
  ...
  string ConvexHull::getPlotHeaderPDF(char function_mode, const string& column_header, bool display_color_gradient) const {
    ...
    addplot_output_ss.str("");

    const uint dimension = getDim();
    const bool reverse_axes = DEFAULT_CHULL_LATEX_REVERSE_AXIS;
    ...
    const string dark_border_points_command = tmp_dark_border_points_command.str();

    // first do a check that the function_mode is not out of scope
    if (dimension == 2) {
      if (function_mode != ADDPLOT_MODE_HULL_POINTS && function_mode != ADDPLOT_MODE_OFF_HULL_POINTS && function_mode != ADDPLOT_MODE_HULL_FACETS) {
      ...
      if (m_velements.size() != 2) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Input dimension mode (2D) and elements.size() mismatch");
      }
    } else if (dimension == 3) {
      if (function_mode != ADDPLOT_MODE_HULL_POINTS && function_mode != ADDPLOT_MODE_OFF_HULL_POINTS && function_mode != ADDPLOT_MODE_HULL_FACETS && function_mode != ADDPLOT_MODE_HULL_FACETSDROP_SHADOWS &&
      ...
      if (m_velements.size() != 3) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Input dimension mode (3D) and elements.size() mismatch");
      }
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Input dimension mode out of scope");
    }

    // also check column_header
    bool found = false;
    for (size_t i = 0, fl_size_i = m_velements.size(); i < fl_size_i; i++) {
      found = (found || (column_header == m_velements[i]));
    }
    ...
    if (!found) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Input column_header mode out of scope (" + column_header + ")");
    }

    // main addplot function
    if (dimension == 2) {
      addplot_output_ss << "\\addplot+[" << endl;
    } else { // dimension==3
      if (function_mode == ADDPLOT_MODE_HEAT_MAPS) {
        addplot_output_ss << "\\addplot3[" << endl;
      } else {
        addplot_output_ss << "\\addplot3+[" << endl;
      }
    }

    if (function_mode == ADDPLOT_MODE_HULL_FACETS || function_mode == ADDPLOT_MODE_HULL_FACETSDROP_SHADOWS) {
      ...
      if (dimension == 2) {
        if (reverse_axes) {
          addplot_output_ss << "[x=" << m_velements[0];
        } else {
      ...
      if (function_mode == ADDPLOT_MODE_HEAT_MAPS) {
        ...
        if (function_mode == ADDPLOT_MODE_HULL_POINTS) {
          ...
          if (dimension == 2) {
            addplot_output_ss << "mark size=4," << endl;
          } else { // dimension==3
      ...
      if (display_color_gradient || function_mode == ADDPLOT_MODE_HEAT_MAPS) {
        ...
        if (function_mode == ADDPLOT_MODE_OFF_HULL_POINTS) {
        ...
        if (function_mode == ADDPLOT_MODE_HULL_POINTS) {
          addplot_output_ss << dark_border_points_command << "," << endl;
        }
        addplot_output_ss << "visualization depends on={\\thisrow{" << column_header << "} \\as \\" << column_header << "}," << endl; // defines visualization dependency
      } else {
        if (function_mode == ADDPLOT_MODE_HULL_POINTS) {
          addplot_output_ss << "mark options={draw=black,fill=blue,solid}," << endl;
        } else {
          addplot_output_ss << ",draw=red}," << endl;
    ...
    addplot_output_ss << "{" << endl;
    for (size_t i = 0, fl_size_i = m_velements.size(); i < fl_size_i; i++) {
      addplot_output_ss << aurostd::PaddedPOST(m_velements[i], 30);
    }
  ...
  string ConvexHull::getPlotPointContentPDF(const ChullPoint& point, bool zero_end_point, bool zero_dist_2_hull) const { // true,false
    ...
    if (PRINT_DIST2HULL_COL_TEX) {
      // dist_2_hull, row 7
      if (zero_dist_2_hull) {
        addplot_output_ss << aurostd::PaddedPOST(0, 30);
      } // aurostd::PaddedPOST(aurostd::utype2string(0.0,CHULL_PRECISION,false,ROUNDOFF_TOL,FIXED_STREAM), 30);
      else {
        addplot_output_ss << aurostd::PaddedPOST(aurostd::utype2string(point.getDist2Hull(_m_), CHULL_PRECISION), 30);
      }
    }
    // end line
    addplot_output_ss << endl;
  ...
  string ConvexHull::getNodeCoordPosition(const aflowlib::_aflowlib_entry& entry, const xvector<double>& coord) const {
    ...
    if (dimension == 2) {
      if (coord.rows != 2) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Input dimension mode (2D) and coordinate size mismatch");
      }
    } else if (dimension == 3) {
      if (coord.rows != 3) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Input dimension mode (3D) and coordinate size mismatch");
      }
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Input dimension mode out of scope");
    }
    ...
    if (dimension == 2) {
      ...
      if (!reverse_axes) {
        output << aurostd::utype2string(1.0 - coord[coord.lrows], FULL_PRECISION);
      } else {
        output << aurostd::utype2string(coord[coord.lrows], FULL_PRECISION);
      }
      output << ",";
      if (m_formation_energy_hull) {
  ...
  string ConvexHull::nodeCreator(stringstream& option, stringstream& position, stringstream& content) const {
    const string _option = option.str();
    const string _position = position.str();
    const string _content = content.str();
    option.str("");
    position.str("");
    content.str("");
  ...
  bool ConvexHull::unwantedFacetLine(uint vi, uint vj, vector<vector<uint>>& facet_lines, bool check_border) const { // bool check_border = true;
    ...
    if (vi > m_points.size() - 1 || vj > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
    }
    if (check_border) {
      ...
      const bool endpointA = ci.isUnary();
      const bool endpointB = cj.isUnary();
      const aflowlib::_aflowlib_entry& entryA = ci.m_entry;
      const aflowlib::_aflowlib_entry& entryB = cj.m_entry;
      // unary to unary
      if (endpointA && endpointB) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " unary to unary" << endl;
        }
        return true;
      }
      ...
      if (endpointA && entryB.vspecies.size() == 2 && scalar_product(coordAR, coordBR) >= ZERO_TOL) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " unary to binary" << endl;
        }
        return true;
      }
      ...
      if (entryA.vspecies.size() == 2 && endpointB && scalar_product(coordAR, coordBR) >= ZERO_TOL) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " binary to unary" << endl;
        }
        return true;
      }
      ...
      if (entryA.vspecies.size() == 2 && entryA.vspecies == entryB.vspecies) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " binary to binary" << endl;
        }
        return true;
      }
    }
    ...
    for (size_t i = 0, fl_size_i = facet_lines.size(); i < fl_size_i; i++) {
    ...
    facet_lines.emplace_back(0);
    facet_lines.back().push_back(vi);
    facet_lines.back().push_back(vj);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " saving facet lines: " << vi << "," << vj << endl;
    }
    return false;
  }

  string ConvexHull::getPointsPropertyHeaderList(filetype ftype) const {
    const bool m_formation_energy_hull = !m_cflags.flag("CHULL::ENTROPIC_TEMPERATURE"); // energy vs. entropic_temperature hull
    ...
    string headers;
    if (!(ftype == latex_ft && !compounds_column_report)) {
      headers += (!headers.empty() ? string(",") : string("")) + "compound";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "compound_reduced";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "compound_reduced_latex";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "compound_fractional";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "compound_fractional_latex";
    }
    headers += (!headers.empty() ? string(",") : string("")) + "prototype";
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "prototype_latex";
    }
    headers += (!headers.empty() ? string(",") : string("")) + "auid";
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "aurl";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "url_entry_page";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "nspecies";
    }
    headers += (!headers.empty() ? string(",") : string("")) + "space_group_orig";
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "space_group_orig_latex";
    }
    headers += (!headers.empty() ? string(",") : string("")) + "space_group_relax";
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "space_group_relax_latex";
    }
    ...
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "ground_state";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "auid_equivalent_structures";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "ground_state_icsd";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "auid_icsd";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "prototype_icsd";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "compound_phases_equilibrium";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "auid_phases_equilibrium";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "compound_phases_decomposition";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "auid_phases_decomposition";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "coefficient_phases_decomposition";
    }
    if (m_formation_energy_hull) {
      headers += (!headers.empty() ? string(",") : string("")) + "distance_hull_enthalpy_formation_atom";
    } else {
      headers += (!headers.empty() ? string(",") : string("")) + "distance_hull_entropic_temperature";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "stability_criterion";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "stability_criterion_relative";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "N+1_enthalpy_gain";
    }
    if (ftype == txt_ft || ftype == json_ft) {
      headers += (!headers.empty() ? string(",") : string("")) + "entropy_stabilization_coefficient";
    }

    return headers;
  }

  string ConvexHull::getDelta(bool helvetica_font) const {
    return (helvetica_font ? "\\Updelta" : "\\Delta");
  }
  ...
  string ConvexHull::getSnapshotTableHeader(string headers, bool designate_HEADER) const {
    ...
    const bool helvetica_font = DEFAULT_CHULL_LATEX_HELVETICA_FONT;

    vector<uint> valignments_headertable_uint;
    uint total_alignment = 0;
    for (size_t i = 0, fl_size_i = vheaders.size(); i < fl_size_i; i++) {
      if (vheaders[i] == "compound") {
        vlabels.emplace_back("compound");
        vpaddings.push_back(80);
        valignments_headertable_uint.push_back(3);
        total_alignment += valignments_headertable_uint.back();
      } else if (vheaders[i] == "prototype") {
        vlabels.emplace_back("prototype");
        vpaddings.push_back(80);
        valignments_headertable_uint.push_back(3);
        total_alignment += valignments_headertable_uint.back();
      } else if (vheaders[i] == "auid") {
        vlabels.emplace_back("auid");
        vpaddings.push_back(80);
        valignments_headertable_uint.push_back(3);
        total_alignment += valignments_headertable_uint.back();
      } else if (vheaders[i] == "space_group_orig") {
        vlabels.emplace_back("original space group");
        vpaddings.push_back(30);
        valignments_headertable_uint.push_back(2);
        total_alignment += valignments_headertable_uint.back();
      } // SG original
      else if (vheaders[i] == "space_group_relax") {
        vlabels.emplace_back("relaxed space group");
        vpaddings.push_back(30);
        valignments_headertable_uint.push_back(2);
        total_alignment += valignments_headertable_uint.back();
      } // SG relax
      else if (vheaders[i] == "spin_atom") {
        vlabels.emplace_back(R"(spin $\left(\mu_{\mathrm{B}}\mathrm{/atom}\right)$)");
        vpaddings.push_back(30);
        valignments_headertable_uint.push_back(2);
        total_alignment += valignments_headertable_uint.back();
      } else if (vheaders[i] == "enthalpy_formation_atom") {
        vlabels.emplace_back("$H_{\\mathrm{f}}$ (meV/atom)");
        vpaddings.push_back(30);
        valignments_headertable_uint.push_back(2);
        total_alignment += valignments_headertable_uint.back();
      } else if (vheaders[i] == "entropic_temperature") {
        vlabels.emplace_back("$T_{\\mathrm{S}}$ (K)");
        vpaddings.push_back(30);
        valignments_headertable_uint.push_back(2);
        total_alignment += valignments_headertable_uint.back();
      } else if (vheaders[i] == "distance_hull_enthalpy_formation_atom") {
        vlabels.push_back("$" + getDelta(helvetica_font) + " H_{\\mathrm{hull}}$ (meV/atom)");
        vpaddings.push_back(30);
        valignments_headertable_uint.push_back(2);
        total_alignment += valignments_headertable_uint.back();
      } else if (vheaders[i] == "distance_hull_entropic_temperature") {
        vlabels.push_back("$" + getDelta(helvetica_font) + " T_{\\mathrm{S}}$ (K)");
        vpaddings.push_back(30);
        valignments_headertable_uint.push_back(2);
        total_alignment += valignments_headertable_uint.back();
      } else {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown property");
      }
    }

    vector<string> vlabels_styled;
    for (size_t i = 0, fl_size_i = vlabels.size(); i < fl_size_i; i++) {
      vlabels_styled.push_back(aurostd::PaddedPOST("{\\footnotesize " + vlabels[i] + "}", vpaddings[i]));
    } // used to be small before auid

    const double page_width = (LATEX_WIDTH_LETTER_STD - (LATEX_LEFT_MARGIN_LETTER_STD + LATEX_RIGHT_MARGIN_LETTER_STD));
    const double penalty_tabcolsep = 2 * LATEX_TABCOLSEP_STD * LATEX_PT2INCH; // there are two colsep for reach column, then convert to inches
    const double penalty_arrayrulewidth = (valignments_headertable_uint.size() + 1) * LATEX_ARRAYRULEWIDTH_STD * LATEX_PT2INCH /
                                          (valignments_headertable_uint.size()); // there are columns+1 separators, then convert to inches, then convert to per column penalty

    vector<string> valignments_headertable_string;
    double page_width_fraction = 0.0;
    for (size_t i = 0, fl_size_i = valignments_headertable_uint.size(); i < fl_size_i; i++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " " << valignments_headertable_string.back() << endl;
      }
    }
  ...
  bool ConvexHull::addInternalHyperlinks(bool internal_links_graph2report, bool internal_links_withinreport) const {
    const int links_setting = DEFAULT_CHULL_LATEX_LINKS;
    ...
    if ((!internal_links_graph2report) && (!internal_links_withinreport)) {
      no_internal_links = true;
    }
  ...
  double ConvexHull::getRoundToValue(double point_range) const {
    ...
    if (order_of_mag < 1) {
      order_of_mag = 1;
    } // we need to work with integers since units are meV/atom, anything less is really just noise
    const double round_to_value = pow(10, order_of_mag - 1) * 2.5; // round to nearest 2.5, 25, 250
  ...
  double ConvexHull::getYTickDistance(double y_range, int approx_num_ticks, double round_to_value) const {
    ...
    int exponent = 0;
    while (delta_pow < round_to_value) {
      exponent += 1;
      delta_pow = delta * pow(10, exponent);
    }
    delta = delta_pow;
    ...
    const double y_tick_distance = y_tick_distance_int * pow(10, -exponent);
    if (LDEBUG) {
  ...
  vector<string> ConvexHull::grabAcceptableLatexColors(bool replace_pranab_standard, bool allow_dvips_colors, uint count) const {
    const vector<string> banned_colors;
  ...
  vector<string> ConvexHull::grabAcceptableLatexColors(const vector<string>& banned_colors, bool replace_pranab_standard, bool allow_dvips_colors, uint count) const {
    vector<string> _latex_colors;
    vector<string> latex_colors;
    string color;
    string lower_color;
    uint loop = 0;
    while (true) {
      _latex_colors = LATEX_DEFAULT_COLORS;
      for (size_t i = 0, fl_size_i = _latex_colors.size(); i < fl_size_i; i++) {
        color = _latex_colors[i];
        lower_color = aurostd::tolower(color);
        if (aurostd::WithinList(banned_colors, lower_color)) {
          continue;
        }
        if (replace_pranab_standard && (lower_color == "green" || lower_color == "red")) {
          color = "pranab_" + lower_color;
        } // now in pranab standard!
        latex_colors.push_back(color);
        if (latex_colors.size() == count) {
          return latex_colors;
        }
      }
      if (allow_dvips_colors && (loop++ % 2 == 0)) {
        _latex_colors = LATEX_DVIPS_COLORS;
      } else {
        _latex_colors = LATEX_DEFAULT_COLORS;
      }
    }
  ...
  aurostd::xoption ConvexHull::resolvePlotLabelSettings() const {
    ...
    const string label_options = DEFAULT_CHULL_LATEX_LABEL_NAME;
    const bool plot_labels = (!label_options.empty()); // overarching flag, mostly for getting options
    if (plot_labels) {
      ...
      for (size_t i = 0, fl_size_i = vlabelstring.size(); i < fl_size_i; i++) {
        if (vlabelstring[i][0] == 'B' || vlabelstring[i][0] == 'b') { // both
          ...
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Incorrect input for plot labels \"" + vlabelstring[i] + "\"");
        }
      }
    }
    if (no_labels) {
  ...
  void ConvexHull::writeLatex() const {
    ...
    if (!aurostd::IsCommandAvailable("pdflatex")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "\"pdflatex\" needs to be in your path");
    }
    if (m_cflags.flag("CHULL::PNG_IMAGE")) {
      if (!aurostd::IsCommandAvailable("convert")) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "\"convert\" needs to be in your path");
      }
    }
    ...
    const bool draw_all_facet_lines = false; // only draw uniques, keep false
    const bool include_equilibrium_phases = true; // keep true

    // USER INPUTS
    bool doc_only = m_cflags.flag("CHULL::DOC_ONLY");
    const bool no_doc = m_cflags.flag("CHULL::NO_DOC");
    const bool image_only = m_cflags.flag("CHULL::IMAGE_ONLY");
    const bool light_contrast = m_cflags.flag("CHULL::LIGHT_CONTRAST");
    const bool large_font = m_cflags.flag("CHULL::LARGE_FONT");
    const bool keep_tex = m_cflags.flag("CHULL::KEEP_TEX");
    const bool show_latex_output = m_cflags.flag("CHULL::LATEX_OUTPUT");
    const bool latex_interactive_mode = m_cflags.flag("CHULL::LATEX_INTERACTIVE");
    ...
    if (print_logo_2 && print_aflow_webaddress_logo) {
      print_aflow_webaddress_logo = false;
    }
    const int banner_setting = DEFAULT_CHULL_LATEX_BANNER;
    const bool no_banner = (banner_setting == 0);
    bool small_banner = (banner_setting == 2 || image_only);
    if (no_banner && small_banner) {
      small_banner = false;
    }
    if (small_banner && (image_only || no_doc)) {
      print_aflow_logo_full = print_logo_2 = false;
    }
    const bool compounds_column_report = DEFAULT_CHULL_LATEX_COMPOUNDS_COLUMN;
    const bool stoich_header_report = DEFAULT_CHULL_LATEX_STOICH_HEADER;
    const bool plot_unaries = DEFAULT_CHULL_LATEX_PLOT_UNARIES;
    const string filter_scheme = DEFAULT_CHULL_LATEX_FILTER_SCHEME;
    ...
    const double filter_cutoff = DEFAULT_CHULL_LATEX_FILTER_VALUE;

    const int plot_off_hull_setting = DEFAULT_CHULL_LATEX_PLOT_OFF_HULL; // does not include unstable
    bool plot_off_hull;
    if (plot_off_hull_setting == -1) {
      plot_off_hull = (getDim() == 2 ? true : false);
    } else {
      plot_off_hull = (plot_off_hull_setting == 0 ? false : true);
    }
    if (filter_by_z || filter_by_distance) {
      plot_off_hull = true;
    }
    ...
    const bool hull_drop_shadow = DEFAULT_CHULL_LATEX_FACET_LINE_DROP_SHADOW; // gus paper

    const string ternary_label_color_setting = DEFAULT_CHULL_LATEX_TERNARY_LABEL_COLOR;
    string ternary_label_color = (ternary_label_color_setting.empty() ? "white" : ternary_label_color_setting);
    if (hull_drop_shadow && ternary_label_color == "white") {
      ternary_label_color = "yellow";
    } // white doesn't work here
    if (!show_heat_map && (ternary_label_color == "white" || ternary_label_color == "yellow")) {
      ternary_label_color = "black";
    } // white/yellow doesn't work here

    const string color_map_setting = DEFAULT_CHULL_LATEX_COLOR_MAP;
    string color_map = (color_map_setting.empty() ? "rgb(0pt)=(0,0,1); rgb(63pt)=(1,0.644,0)" : color_map_setting);

    // labels options
    const aurostd::xoption lflags = resolvePlotLabelSettings();
    const bool plot_labels = lflags.flag("CHULL::PLOT_LABELS");
    const bool labels_off_hull = lflags.flag("CHULL::PLOT_LABELS_OFF_HULL");
    const bool meta_labels = lflags.flag("CHULL::PLOT_META_LABELS");
    const bool compound_labels = lflags.flag("CHULL::PLOT_COMPOUND_LABELS");
    const bool prototype_labels = lflags.flag("CHULL::PLOT_PROTOTYPE_LABELS");
    const bool icsd_labels = lflags.flag("CHULL::PLOT_ICSD_LABELS");
    const bool no_labels = lflags.flag("CHULL::PLOT_NO_LABELS");
    ...
    //[MOVED to resolvePlotLabelSettings()]  for(size_t i=0,fl_size_i=vlabelstring.size();i<fl_size_i;i++) {
    ...
    const int plot_reduced_composition_setting = DEFAULT_CHULL_LATEX_PLOT_REDUCED_COMPOSITION;
    bool plot_reduced_composition;
    if (plot_reduced_composition_setting == -1) {
      plot_reduced_composition = (!(getDim() == 2 && compound_labels && plot_labels));
    } else {
      plot_reduced_composition = (plot_reduced_composition_setting == 0 ? false : true);
    }
    if (no_labels) {
    ...
    const int bold_labels_setting = DEFAULT_CHULL_LATEX_BOLD_LABELS;
    const bool bold_labels = (bold_labels_setting == 1);
    const bool bold_labels_ternaries = (bold_labels || bold_labels_setting == -1);

    // override with user inputs
    if (light_contrast) {
      color_map = "rgb(0pt)=(0.035,0.270,0.809); rgb(63pt)=(1,0.644,0)";
    }
    if (font_size.empty()) {
      if (large_font) {
        if (helvetica_font) {
        ...
        if (image_only) {
          ...
          font_size = "large";
        } // safely, I can do large
      }
    }

    const double opacity_watermark = 0.3;
    const string watermark_x_shift = "2.5in";
    ...
    stringstream doc_header_TEX_ss;
    stringstream _doc_header_TEX_ss; // latex header commands
    stringstream tikzpic_settings_TEX_ss;
    stringstream _tikzpic_settings_TEX_ss; // settings in tikzpic
    ...
    stringstream report_data_ss;
    stringstream _report_data_ss;
    ...
    string main_TEX_file;
    string main_PDF_file;
    string main_PNG_file;
    string main_file;
    string main_output_file;
    string input;
    string input_hyphened;
    ...
    if (image_only) {
      main_TEX_file = main_file + "_IMAGEONLY.tex";
    } else {
      main_TEX_file = main_file + ".tex";
    }
    main_PDF_file = main_file + ".pdf";
    main_PNG_file = main_file + ".png";
    const string aflow_logo_full_file = "aflow_logo_full.pdf";
    const string aflow_logo_skinny_file = "aflow_logo_skinny.pdf";
    const string logo_file_2 = "logo2.png";
    string DEFAULT_PLOT_COLUMN_HEADER;
    if (m_formation_energy_hull) {
      DEFAULT_PLOT_COLUMN_HEADER = "H_f_meVatom";
    } else {
      DEFAULT_PLOT_COLUMN_HEADER = "T_S";
    }
    ...
    vector<vector<uint>> facet_lines;
    vector<vector<uint>> facet_lines_dropshadow;
    double min_point = 0.0;
    double max_point = 0.0;
    double point_range = 0.0; // saves min/max energy value to determine if we can
    // have a colorbar
    double z_filter_cutoff = 0.0;
    double dist_filter_cutoff = 0.0;
    string output_name;
    ...
    const uint count_total_entries = getEntriesCount(false);
    uint i_point = AUROSTD_MAX_UINT;
    uint i_coord_group = AUROSTD_MAX_UINT;
    bool added_header;
    ...
    const string MARGIN_PICTURE = "includeheadfoot,headheight=16pt,margin=0.5in"; // do not make margin any smaller, will screw up fancyheaders elsewhere
    const string MARGIN_REPORT = "includeheadfoot,headheight=" + (print_aflow_logo_full ? string("70") : string("50")) +
                                 "pt,headsep=0.1in,top=0.5in,bottom=0.75in,left=" + aurostd::utype2string(LATEX_LEFT_MARGIN_LETTER_STD) + "in,right=" + aurostd::utype2string(LATEX_RIGHT_MARGIN_LETTER_STD) +
                                 "in,footskip=0.5in";

    const string headers = getPointsPropertyHeaderList(latex_ft);
    vector<string> vheaders;
    aurostd::string2tokens(headers, vheaders, ",");
    ...
    //[CO20190226 - TABU IS BROKEN IN TeX Live 2019]for(size_t i=0,fl_size_i=vheaders.size();i<fl_size_i;i++){
    ...
    for (size_t i = 0, fl_size_i = vheaders.size(); i < fl_size_i; i++) {
      if (vheaders[i] == "compound") {
        valignments_entrytable_uint.push_back(3);
        total_alignment_entrytable += valignments_entrytable_uint.back();
      } else if (vheaders[i] == "prototype") {
        valignments_entrytable_uint.push_back(3);
        total_alignment_entrytable += valignments_entrytable_uint.back();
      } else if (vheaders[i] == "auid") {
        ...
        valignments_entrytable_uint.push_back(2);
        total_alignment_entrytable += valignments_entrytable_uint.back();
      } else if (vheaders[i] == "space_group_relax") {
        valignments_entrytable_uint.push_back(2);
        total_alignment_entrytable += valignments_entrytable_uint.back();
    ...
    const double page_width = (LATEX_WIDTH_LETTER_STD - (LATEX_LEFT_MARGIN_LETTER_STD + LATEX_RIGHT_MARGIN_LETTER_STD));
    const double penalty_tabcolsep = 2 * LATEX_TABCOLSEP_STD * LATEX_PT2INCH; // there are two colsep for reach column, then convert to inches
    const double penalty_arrayrulewidth = (valignments_entrytable_uint.size() + 1) * LATEX_ARRAYRULEWIDTH_STD * LATEX_PT2INCH /
                                          (valignments_entrytable_uint.size()); // there are columns+1 separators, then convert to inches, then convert to per column penalty
    ...
    for (size_t i = 0, fl_size_i = vheaders.size(); i < fl_size_i; i++) {
      ...
      if (vheaders[i] == "compound") {
        valignments_entrytable_string.push_back("L{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      } else if (vheaders[i] == "prototype") {
        valignments_entrytable_string.push_back("L{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      } else if (vheaders[i] == "auid") {
        valignments_entrytable_string.push_back("L{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
        ...
        valignments_entrytable_string.push_back("R{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      } else if (vheaders[i] == "enthalpy_formation_atom") {
        valignments_entrytable_string.push_back("R{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      } else if (vheaders[i] == "entropic_temperature") {
        valignments_entrytable_string.push_back("R{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " " << valignments_entrytable_string.back() << endl;
      }
    }

    // for compound name / ground state thermo properties (2 columns)
    const uint n_cols_compoundname_thermoprops = 2;
    ...
    for (uint i = 0; i < n_cols_compoundname_thermoprops; i++) {
      if (i == 0) {
        valignments_compoundname_thermopropstable_uint.push_back(3);
        total_alignment_compoundname_thermoprops += valignments_compoundname_thermopropstable_uint.back();
      }
      if (i == 1) {
        valignments_compoundname_thermopropstable_uint.push_back(5);
        total_alignment_compoundname_thermoprops += valignments_compoundname_thermopropstable_uint.back();
      }
    }
    ...
    for (size_t i = 0, fl_size_i = valignments_compoundname_thermopropstable_uint.size(); i < fl_size_i; i++) {
      ...
      if (i == 0) {
        valignments_compoundname_thermopropstable_string.push_back("L{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      }
      if (i == 1) {
        valignments_compoundname_thermopropstable_string.push_back("R{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " " << valignments_compoundname_thermopropstable_string.back() << endl;
      }
    }

    // for 1 column tables (left align)
    const uint n_cols_onecol_left = 1;
    vector<uint> valignments_onecol_lefttable_uint;
    uint total_alignment_onecol_left = 0;
    for (uint i = 0; i < n_cols_onecol_left; i++) {
      if (i == 0) {
        valignments_onecol_lefttable_uint.push_back(1);
        total_alignment_onecol_left += valignments_onecol_lefttable_uint.back();
      }
    }

    vector<string> valignments_onecol_lefttable_string;
    page_width_fraction = 0.0;
    for (size_t i = 0, fl_size_i = valignments_onecol_lefttable_uint.size(); i < fl_size_i; i++) {
      page_width_fraction = page_width * (double) valignments_onecol_lefttable_uint[i] / (double) total_alignment_onecol_left;
      if (i == 0) {
        valignments_onecol_lefttable_string.push_back("L{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " " << valignments_onecol_lefttable_string.back() << endl;
      }
    }

    // for equilibrium/decomposition reactions
    const uint n_cols_reaction = 2;
    vector<uint> valignments_reactiontable_uint;
    uint total_alignment_reaction = 0;
    for (uint i = 0; i < n_cols_reaction; i++) {
      if (i == 0) {
        valignments_reactiontable_uint.push_back(1);
        total_alignment_reaction += valignments_reactiontable_uint.back();
      }
      if (i == 1) {
        valignments_reactiontable_uint.push_back(3);
        total_alignment_reaction += valignments_reactiontable_uint.back();
      }
    }
    ...
    page_width_fraction = 0.0;
    for (size_t i = 0, fl_size_i = valignments_reactiontable_uint.size(); i < fl_size_i; i++) {
      page_width_fraction = page_width * (double) valignments_reactiontable_uint[i] / (double) total_alignment_reaction;
      if (i == 0) {
        valignments_reactiontable_string.push_back("L{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      }
      if (i == 1) {
        valignments_reactiontable_string.push_back("R{" + aurostd::utype2string(page_width_fraction - penalty_tabcolsep - penalty_arrayrulewidth) + "in}");
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " " << valignments_reactiontable_string.back() << endl;
      }
    }
    ...
    if (plot_iso_max_latent_heat) {
      ...
      for (size_t i = 0, fl_size_i = g_states.size(); i < fl_size_i; i++) {
        ...
        if (iso_max < iso_max_latent_heat_min) {
          iso_max_latent_heat_min = iso_max;
        }
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " iso-max-latent-lines min=" << iso_max_latent_heat_min << endl;
      }
    }
    ...
    if (!doc_only && (dimension == 2 || dimension == 3)) {
      ...
      if (filter_by_z || !plot_unstable) {
        ...
        if (filter_by_z) {
          ...
          const bool plot_unstable_old = plot_unstable;
          if (m_formation_energy_hull) {
            if (z_filter_cutoff > 0.0) {
              plot_unstable = true;
            }
          } else {
            if (z_filter_cutoff < 0.0) {
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " z_filter_cutoff=" << z_filter_cutoff << endl;
        }
      }
      if (filter_by_distance) {
        dist_filter_cutoff = filter_cutoff;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " DIST dist_filter_cutoff=" << dist_filter_cutoff << endl;
        }
      ...
      for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
        point_added = false;
        for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i && !(dimension == 3 && point_added); i++) { // we only add one point per coordgroup for 3D
          i_point = m_coord_groups[i_coord_group].m_points[i];
          if (!m_points[i_point].m_initialized) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Point[" + aurostd::utype2string(i_point) + "] is not initialized");
          }
          ...
          if (!plot_unaries && point.isUnary()) {
            continue;
          }
          if (!plot_off_hull && !point.m_is_g_state) {
            continue;
          } // if point.isUnary() and it gets here, only pass g-state
          if (!point.m_is_g_state) {
            if (filter_by_z || !plot_unstable) {
              if (m_formation_energy_hull) {
          ...
          if (point.getLastCoord() < min_point) {
            min_point = point.getLastCoord();
          }
          if (point.getLastCoord() > max_point) {
            max_point = point.getLastCoord();
          }
      ...
      if (!plot_points_count) {
        ...
        plot_unstable = true;
        if (m_formation_energy_hull) {
          z_filter_cutoff = 1000.0;
        } // 1000 meV
        else {
          z_filter_cutoff = 1.0 / (((double) KBOLTZEV) * (0.5 * log(0.5) + (1.0 - 0.5) * log(1.0 - 0.5)));
        } // 1.0 eV / (kB * 2 * 0.5 log(0.5))
      ...
      for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
        ...
        for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i && !(dimension == 3 && point_added); i++) { // we only add one point per coordgroup for 3D
          ...
          if (point.isUnary()) {
            ...
            plot_points_count_no_end_points++;
          }
          plot_points_count++;
      ...
      if (!plot_points_count) { // CO20200106 - patching for auto-indenting
        doc_only = true;
    ...
    // links options
    const int links_setting = DEFAULT_CHULL_LATEX_LINKS;
    const bool kill_all_links = (links_setting == 0); // no links whatsoever
    const bool external_links = addExternalHyperlinks(); // weblinks
    ...
    const bool internal_links = addInternalHyperlinks(internal_links_graph2report, internal_links_withinreport); // no jumping
    if (!internal_links) {
      internal_links_graph2report = internal_links_withinreport = false;
    }
    ...
    if (helvetica_font) {
      ...
      doc_header_TEX_ss << R"(\renewcommand\familydefault{\sfdefault})" << " \%HEADER" << endl;
    ...
    if (!no_doc && !image_only) {
      ...
      doc_header_TEX_ss << "\\vbadness=10000" << " \%HEADER"
                        << endl; // patches vbadness warnings (hline issue) with longtable - https://tex.stackexchange.com/questions/71096/underfull-vbox-in-each-longtable-can-it-be-fixed-or-must-it-be-ignored
    ...
    if (true || !kill_all_links) { // need for paper reference (CHULL_JOURNAL_LATEX)
    ...
    if (!image_only) {
      ...
      _doc_header_TEX_ss << R"(\setlength{\TPHorizModule}{\paperwidth}\setlength{\TPVertModule}{\paperheight})" << " \%HEADER" << endl;
      ...
      if (!(small_banner || no_banner)) {
        ...
        if (print_aflow_logo_full || print_aflow_text_logo) {
          ...
          if (!print_aflow_logo_full) {
            _doc_header_TEX_ss << "\\shortstack[l]{";
          }
          if (external_links) {
            _doc_header_TEX_ss << "\\href{http://aflow.org}";
          }
          if (print_aflow_logo_full) {
            _doc_header_TEX_ss << "{\\includegraphics[scale=0.2]{" << aflow_logo_full_file << "}}";
          } else {
        ...
        if (print_logo_2 || print_aflow_webaddress_logo) {
          ...
          if (print_logo_2) {
            if (external_links) {
              _doc_header_TEX_ss << "\\href{http://www.nomad-coe.eu/}";
            }
            ...
            _doc_header_TEX_ss << "{" << label_image_font_size << R"({\fontfamily{\sfdefault}\selectfont{aflow.org}}})"; //\\fontfamily{\\phv}\\selectfont
          }
          _doc_header_TEX_ss << "}}\\end{textblock}" << " \%HEADER" << endl;
        }
        _doc_header_TEX_ss << "}" << " \%HEADER" << endl;
        ...
        _doc_header_TEX_ss << "\\begin{textblock}{0.05}[0.5,1.0](0.925,0.95){\\rotatebox{90}{" << general_image_font_size << " " << string("\\printCountTotalEntries\\") << " entries}}\\end{textblock}"
                           << " \%HEADER" << endl; // string("\\printCountHullEntries") //hull count
    ...
    if (!no_doc && !image_only) {
      ...
      const double tableheaderone_colsize = (LATEX_WIDTH_LETTER_STD - (LATEX_LEFT_MARGIN_LETTER_STD + LATEX_RIGHT_MARGIN_LETTER_STD)) / (print_logo_2 ? 3.0 : 2.0);
      _doc_header_TEX_ss << "\\noindent\\begin{tabular}{";
      _doc_header_TEX_ss << "@{}L{" << tableheaderone_colsize << "in}";
      if (print_logo_2) {
        _doc_header_TEX_ss << "@{}C{" << tableheaderone_colsize << "in}";
      }
      ...
      if (external_links) {
        _doc_header_TEX_ss << "\\href{" << AFLOW_WEB << "}";
      }
      if (print_aflow_logo_full) {
        _doc_header_TEX_ss << R"({\raisebox{-.5\height}{\includegraphics[scale=0.25]{)" << aflow_logo_full_file << "}}}";
      } else {
        _doc_header_TEX_ss << "{\\LARGE AFLOW V" << string(AFLOW_VERSION) << "}";
        _doc_header_TEX_ss << "\\\\";
        _doc_header_TEX_ss << "{\\large C. Oses and S. Curtarolo}";
      ...
      if (print_logo_2) {
        if (external_links) {
          _doc_header_TEX_ss << "\\href{" << NOMAD_WEB << "}";
        }
        _doc_header_TEX_ss << R"({\raisebox{-.5\height}{\includegraphics[scale=0.125]{)" << logo_file_2 << "}}}";
        _doc_header_TEX_ss << " & ";
      }
      ...
      _doc_header_TEX_ss << "\\fancyhf{}" << " \%HEADER" << endl;
      _doc_header_TEX_ss << R"(\fancyhead[C]{\usebox{\snapshotTableHeaderOne}})" << " \%HEADER" << endl;
      ...
      _doc_header_TEX_ss << "\\fancyhf{}" << " \%HEADER" << endl;
      _doc_header_TEX_ss << R"(\fancyhead[C]{\usebox{\snapshotTableHeaderTwo}})" << " \%HEADER" << endl;
      _doc_header_TEX_ss << "\\fancyfoot[C]{\\thepage}" << " \%HEADER" << endl;
      _doc_header_TEX_ss << "\\renewcommand{\\headrulewidth}{0pt}     \% size of header line" << " \%HEADER" << endl;
      _doc_header_TEX_ss << "\\renewcommand{\\footrulewidth}{0pt}     \% size of header line" << " \%HEADER" << endl;
      _doc_header_TEX_ss << "}" << " \%HEADER" << endl;
    ...
    if (!image_only) {
      _doc_header_TEX_ss << "\\newAlloyFormatted{" << input_hyphened << "}{" << count_total_entries << "}" << endl;
    } //{" << count_total_entries << "}" << endl;
    ...
    if (!doc_only && (dimension == 2 || dimension == 3)) {
      ...
      doc_header_TEX_ss << R"(\AtBeginEnvironment{tikzpicture}{\tracinglostchars=0\relax})" << " \%HEADER" << endl; // CO20221111 - avoids printing dumb warnings
      ...
      if (image_only) {
        doc_header_TEX_ss << "\\beginpgfgraphicnamed{aflow_" << input << "_hull}" << " \%HEADER" << endl;
      }
      if (!image_only) {
      ...
      if (m_formation_energy_hull) {
        ...
        if (plot_iso_max_latent_heat) { // min_point might change with iso_max
          const double old_min_point = min_point;
          min_point = min(min_point, iso_max_latent_heat_min);
          if (std::abs(old_min_point - min_point) > std::abs(max_point - old_min_point) / (double) approx_num_ticks) { // is it bigger than ~tic distance?
      ...
      const bool set_y_range_exactly = true;
      ...
      double y_range = std::abs(ymax - ymin);
      const double round_to_value = getRoundToValue(point_range);
      const double extra_padding = (point_range) *extra_padding_multiplier; // to avoid label clashing with axis, 20% should be enough
      // int extra_padding=aurostd::roundDouble((point_range)*0.2, (int)round_to_value, false);  //round down to keep tight, unless you get 0, need more padding  //CO20190724 - explicit double->int conversion
      // (floor is fine) if(extra_padding==0){extra_padding=aurostd::roundDouble((point_range)*0.2, (int)round_to_value, true);}  //CO20190724 - explicit double->int conversion (floor is fine)

      // adding conversion between meV/atom to kJ/mol
      bool showkJmolaxis = m_formation_energy_hull; // default
      const double kJmol_threshold = 1.0 / ((double) meVatom2kJmol); // should give about 10, 1 kJ/mol -> 10 meV/atom
      if (false && showkJmolaxis && y_range < kJmol_threshold) {
        showkJmolaxis = false;
      } // we need integer y tick distance, don't show if it's not at least as big as ~10meV/atom, we should now be able to handle fractional y_dist

      if (showkJmolaxis) {
        y_tick_distance_right /= 10.0;
      } // default value, store AFTER setting showkJmolaxis
      ...
      if (!plot_points_count || (dimension == 3 && !show_heat_map && plot_points_count < 2) || (std::abs(point_range) < ZERO_TOL)) {
        include_color_bar = false;
        show_heat_map = false;
      ...
      if ((!plot_points_count) || (std::abs(point_range) < ZERO_RANGE_TOL)) {
        ...
        if (!m_formation_energy_hull && !(plot_unaries && unary_g_state_unstable) && (!plot_unstable || std::abs(min_point) < ZERO_TOL)) {
          ymin = 0;
        } else {
          ymin = min_point;
          if (dimension == 2) { // only do rounding for dim==2, dim==3 we need EXACT values or colorbar gets screwed up
            if (m_formation_energy_hull) {
              ymin -= extra_padding;
            } // CO20180227 - we need to avoid labels here
        ...
        // we might have hull endpoints higher than 0
        if (m_formation_energy_hull && !(plot_unaries && unary_g_state_unstable) && (!plot_unstable || std::abs(max_point) < ZERO_TOL)) {
          ymax = 0;
        } else {
          ymax = max_point;
          if (dimension == 2) { // only do rounding for dim==2, dim==3 we need EXACT values or colorbar gets screwed up
        ...
        if (showkJmolaxis) {
          const double round_to_value_right = getRoundToValue(meVatom2kJmol * point_range);
      ...
      if (dimension == 3) {
        image_width_proportion = 1.2;
      } // approx 20cm given 17cm width
      ...
      if (image_only) {
        unary_distance_axis_3D = 0.35;
      } // CO FIX

      uint g_state;
      if (dimension == 2) {
        uint top_axis_point;
        uint bottom_axis_point;
        if (!reverse_axes) {
          top_axis_point = 0;
          bottom_axis_point = 1;
        } else {
        ...
        { // CO20200106 - patching for auto-indenting
          ...
          pseudo_preliminary_axes_TEX_ss << "}," << endl;
          if (set_y_range_exactly) {
            pseudo_preliminary_axes_TEX_ss << "ytick distance={" << y_tick_distance_right << "}," << endl;
          }
        ...
        pseudo_preliminary_axes_TEX_ss << "}," << endl;
        if (set_y_range_exactly) {
          pseudo_preliminary_axes_TEX_ss << "ytick distance={" << y_tick_distance << "}," << endl;
        }
        pseudo_preliminary_axes_TEX_ss << "scaled y ticks=false," << endl;
      } else { // dimension==3
        ...
        if (showkJmolaxis) {
          if (display_color_gradient) {
            if (include_color_bar) {
              ...
              pseudo_preliminary_axes_TEX_ss << "}," << endl;
              pseudo_preliminary_axes_TEX_ss << "ylabel style={font=" << label_image_font_size << ",at={(axis cs:0,1,0)},anchor=north east,below=" << unary_distance_axis_3D
                                             << "cm,left=" << unary_distance_axis_3D << "cm,opacity=0}, \%we need labels for proper padding, but avoid bolder labels with opacity==0" << endl;
              // Xlabel
              pseudo_preliminary_axes_TEX_ss << "xlabel={";
              ...
              pseudo_preliminary_axes_TEX_ss << "}," << endl;
              pseudo_preliminary_axes_TEX_ss << "xlabel style={font=" << label_image_font_size << ",at={(axis cs:1,0,0)},anchor=south,above=" << sqrt(2.0 * unary_distance_axis_3D * unary_distance_axis_3D)
                                             << "cm,opacity=0}, \%we need labels for proper padding, but avoid bolder labels with opacity==0" << endl;
              // Zlabel
              ...
              // }
              pseudo_preliminary_axes_TEX_ss << "}," << endl;
              pseudo_preliminary_axes_TEX_ss << "zlabel style={font=" << label_image_font_size << ",at={(axis cs:0,0,1)},anchor=north west,below=" << unary_distance_axis_3D
                                             << "cm,right=" << unary_distance_axis_3D << "cm,opacity=0},  \%we need labels for proper padding, but avoid bolder labels with opacity==0" << endl;
              pseudo_preliminary_axes_TEX_ss << "xmin=0," << endl;
              pseudo_preliminary_axes_TEX_ss << "xmax=1," << endl;
              ...
              pseudo_preliminary_axes_TEX_ss << "yticklabel shift=6pt," << endl;
              const double round_to_value_right = getRoundToValue(meVatom2kJmol * point_range);
              const double y_tick_distance_right = getYTickDistance(meVatom2kJmol * y_range, approx_num_ticks, round_to_value_right);
              if (set_y_range_exactly) {
                pseudo_preliminary_axes_TEX_ss << "ytick distance={" << y_tick_distance_right << "}," << endl;
              }
              ...
              added_header = false;
              if (show_heat_map) {
                for (size_t i = 0, fl_size_i = m_i_facets.size(); i < fl_size_i; i++) {
                  ...
                  if (facet.m_is_vertical) {
                    if (LDEBUG) {
                      cerr << __AFLOW_FUNC__ << " NOT plane: " << std::abs(facet.m_normal[facet.m_normal.urows]) << endl;
                    }
                  } else {
                    if (LDEBUG) {
                      cerr << __AFLOW_FUNC__ << " plane: " << std::abs(facet.m_normal[facet.m_normal.urows]) << endl;
                    }
                    num_horizontal_planes++;
                  }
                  chull_points = facet.getCHIndices();
                  added_header = false;
                  for (size_t j = 0, fl_size_j = chull_points.size(); j < fl_size_j; j++) {
              ...
              // now hullPoints
              added_header = false;
              for (size_t i = 0, fl_size_i = hull_points.size(); i < fl_size_i; i++) {
              ...
              if (plot_off_hull) {
                added_header = false;
                for (size_t i = 0, fl_size_i = plot_points.size(); i < fl_size_i; i++) {
                  const ChullPoint& point = m_points[plot_points[i]];
                  if (point.m_is_g_state) {
                    continue;
                  } // we already plotted these points
                  if (!added_header) {
        ...
        pseudo_preliminary_axes_TEX_ss << "{" << m_velements[0] << "}";
        pseudo_preliminary_axes_TEX_ss << "}," << endl;
        pseudo_preliminary_axes_TEX_ss << "ylabel style={font=" << label_image_font_size << ",at={(axis cs:0,1,0)},anchor=north east,below=" << unary_distance_axis_3D << "cm,left=" << unary_distance_axis_3D
                                       << "cm}," << endl;
        // Xlabel
        pseudo_preliminary_axes_TEX_ss << "xlabel={";
        g_state = getUnaryGState(1);
        ...
        pseudo_preliminary_axes_TEX_ss << "{" << m_velements[2] << "}";
        pseudo_preliminary_axes_TEX_ss << "}," << endl;
        pseudo_preliminary_axes_TEX_ss << "zlabel style={font=" << label_image_font_size << ",at={(axis cs:0,0,1)},anchor=north west,below=" << unary_distance_axis_3D << "cm,right=" << unary_distance_axis_3D
                                       << "cm}," << endl;
      }
      ...
      if (display_color_gradient) {
        if (include_color_bar) {
          ...
          if (showkJmolaxis) {
            tikzpic_settings_TEX_ss << "axis y line*=right," << endl;
          }
          // ytick={18,20,25,...,45}
          ...
          if (set_y_range_exactly) {
            tikzpic_settings_TEX_ss << "ytick distance={" << y_tick_distance << "}," << endl;
          }
      ...
      if (dimension == 2) {
        ...
        if (draw_all_facet_lines) {
          ...
          num_horizontal_planes = 0;
          for (size_t ii = 0, fl_size_ii = facet_lines.size(); ii < fl_size_ii; ii++) {
            facet_lines[ii].clear();
          }
          facet_lines.clear();
          for (size_t ii = 0, fl_size_ii = facet_lines_dropshadow.size(); ii < fl_size_ii; ii++) {
            facet_lines_dropshadow[ii].clear();
          }
          facet_lines_dropshadow.clear();
          for (size_t i = 0, fl_size_i = m_i_facets.size(); i < fl_size_i; i++) {
            const ChullFacet& facet = m_facets[m_i_facets[i]];
            if (facet.m_is_artificial) {
              continue;
            ...
            chull_points.push_back(chull_points[0]); // that way we get full facet
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " looking for all unwanted facets" << endl;
            }
            for (size_t l = 0, fl_size_l = chull_points.size(); l < fl_size_l - 1; l++) {
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " looking at point l=" << l << " and l=" << l + 1 << endl;
              }
              if (!unwantedFacetLine(chull_points[l], chull_points[l + 1], facet_lines, true)) {
                if (LDEBUG) {
                  cerr << __AFLOW_FUNC__ << " plotting point l=" << l << " and l=" << l + 1 << endl;
                }
                // convex hull facets (lines)
                convex_hull_facets_TEX_ss << getPlotHeaderPDF(ADDPLOT_MODE_HULL_FACETS, DEFAULT_PLOT_COLUMN_HEADER, display_color_gradient);
                for (uint j = 0; j < 2; j++) {
              ...
              if (hull_drop_shadow) {
                if (!unwantedFacetLine(chull_points[l], chull_points[l + 1], facet_lines_dropshadow, false)) {
                  if (LDEBUG) {
                    cerr << __AFLOW_FUNC__ << " plotting drop shadow l=" << l << " and l=" << l + 1 << endl;
                  }
                  // thick white line for contrast
                  convex_hull_facets_drop_shadow_TEX_ss << getPlotHeaderPDF(ADDPLOT_MODE_HULL_FACETSDROP_SHADOWS, DEFAULT_PLOT_COLUMN_HEADER, display_color_gradient);
                  for (uint j = 0; j < 2; j++) {
        ...
        if (hull_drop_shadow) {
          tikzpic_TEX_ss << convex_hull_facets_drop_shadow_TEX_ss.str();
        }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " start plotting off hull points" << endl;
      }
      if (plot_off_hull || !no_labels || internal_links_graph2report || no_doc) {
        ...
        for (size_t i = 0, fl_size_i = plot_points.size(); i < fl_size_i; i++) {
          ...
          if (internal_links_graph2report || (no_doc && external_links)) {
            ...
            node_content_ss << "\\tiny";
            node_content_ss << "{";
            if (no_doc && external_links) {
              node_content_ss << "\\href{" << ENTRY_PAGE_URL_PREFIX << entry.auid << "}";
            } else {
              // uint i_coord_group;
              if (getCoordGroupIndex(point, i_coord_group)) {
                const uint ref_state = m_coord_groups[i_coord_group].m_ref_state;
          ...
          if (!no_labels && !(!labels_off_hull && !point.m_is_g_state) && !(point.isUnary() && !(meta_labels || prototype_labels))) {
            ...
            if (meta_labels) {
              const uint precision_tmp = 0;
              const double tmp_roundoff_tol = 5.0 * pow(10, -((int) precision_tmp) - 1);
              // no node option
              ...
              // get node content
              if (dimension == 3 && entry.vspecies.size() == 3) {
                node_content_ss << "\\color{" << ternary_label_color << "}";
              }
              ...
              if (dimension == 2 && !plot_labels && prototype_labels && rotate_labels) {
                if (point.m_is_g_state && !point.isUnary()) {
                  ...
                  if (bold_labels /* && !helvetica_font*/) {
                    node_content_ss << "\\textbf{";
                  }
                  if (!m_formation_energy_hull) {
                    node_content_ss << "~~";
                  } // pre - 1
                  ...
                  if (m_formation_energy_hull) {
                    node_content_ss << "~~";
                  } // post - 2: only works if we put two?
                  if (bold_labels /* && !helvetica_font*/) {
                    node_content_ss << "}";
                  }
                  node_content_ss << "}";
                ...
                // get node option
                if (point.isUnary()) {
                  if (entry.vspecies[0] == m_velements[0]) {
                    node_option_ss << "anchor=east";
                  } else {
                    node_option_ss << "anchor=west";
                  }
                } else {
                  node_option_ss << "anchor=" << (m_formation_energy_hull ? "south,above=0.1cm" : "north,below=0.1cm");
                }
                // get node position
                node_position_ss << getNodeCoordPosition(point);
                // get node content
                ...
                output_name = prettyPrintPrototype(point, false, false); // only one backslash needed
                if (point.isUnary() && entry.vspecies[0] == m_velements[1]) {
                  node_content_ss << "~~";
                }
                node_content_ss << output_name;
                if (point.isUnary() && entry.vspecies[0] == m_velements[0]) {
                  node_content_ss << "~~";
                }
                // enclose brackets
                node_content_ss << "}";
                ...
                if (rotate_labels) {
                  if (dimension == 2) {
                    ...
                    if (entry.vspecies.size() == 2) {
                      ...
                      if (!show_heat_map) {
                        node_option_ss << "anchor=south,above=0.1cm";
                      }
                ...
                } // these are sort of "special" font settings meant to reduce clutter, no need to add to aflowrc
                else {
                  node_content_ss << "\\" << font_size << "{";
                } // already gets wrapped with brackets in nodeCreator(), just leave alone so you mitigate spaces
                if (bold_labels || (entry.vspecies.size() == 3 && bold_labels_ternaries) /* && !helvetica_font*/) {
                  node_content_ss << "\\textbf{";
                }
                ...
                if (rotate_labels) {
                  if (dimension == 2) {
                    if (!point.isUnary()) { // handle unaries later
                      if (!m_formation_energy_hull && point.m_is_g_state) {
                        node_content_ss << "~";
                      }
                    }
                  } else { // dimension==3
                ...
                if (dimension == 2) {
                  if (point.isUnary()) { // unaries
                    if (entry.vspecies[0] == m_velements[1]) {
                      node_content_ss << "~";
                    }
                  }
                }
                ...
                if (icsd_labels) { // icsd_labels mutually exclusive with compound/prototype
                  // uint i_coord_group;
                  if (getCoordGroupIndex(point, i_coord_group)) {
                    const uint canonical_icsd = m_coord_groups[i_coord_group].m_i_icsd_g_state;
        ...
        if (plot_iso_max_latent_heat) {
          ...
          if (latex_colors.size() != g_states.size()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Issue grabbing latex colors");
          }
          for (size_t gi = 0, fl_size_gi = g_states.size(); gi < fl_size_gi; gi++) {
            color = latex_colors[gi];
            // add curves
            tikzpic_TEX_ss << "\\addplot[domain=0.001:0.999,smooth,variable=\\x," << color << "] {";
            g_point = g_states[gi];
            tikzpic_TEX_ss << ((double) KBOLTZEV) << "*" << 1.0e3 << "*" << T_S(m_points[g_point]) << R"(*(\x*ln(\x)+(1-\x)*ln(1-\x))};)" << endl;
            // add labels
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " start plotting logos" << endl;
      }
      ...
      if (dimension == 2) {
        tikzpic_TEX_ss << "\\end{axis}" << endl;
      } else {
        tikzpic_TEX_ss << "\\end{ternaryaxis}" << endl;
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " ending axis" << endl;
      }
      ...
      if (image_only) {
        tikzpic_TEX_ss << "\\endpgfgraphicnamed" << endl;
      } else {
    ...
    if (!no_doc && !image_only) {
      ...
      const vector<vector<uint>> equilibrium_phases;
      vector<vector<ChullPoint>> equilibrium_phases_CP;
      const vector<ChullPoint> dummyDCP;
      vector<string> equilibrium_phases_vs;
      vector<string> _equilibrium_phases_vs;
      const vector<uint> decomposition_phases;
      ...
      const bool add_vspace = true; // ONLY once, first TERNARY is too close to column headers
      string pdftable_font_sizes;
      vector<string> chpoint_properties;
      bool print_scriterion = true;
      bool print_np1 = true;
      stringstream scriterion_data_ss;
      stringstream np1_data_ss;
      // uint num_cols_scriterion=2;
      // uint num_cols_np1=3;
      uint precision_tmp = 0;
      double tmp_roundoff_tol;
      if (compounds_column_report) {
        pdftable_font_sizes = "\\fontsize{4}{6}\\selectfont";
      } else {
        pdftable_font_sizes = "\\fontsize{5}{7}\\selectfont";
      }
      ...
      for (size_t fl_size_i_nary = m_naries.size(), i_nary = (fl_size_i_nary - 1); i_nary < fl_size_i_nary; i_nary--) { // go backwards!
          ...
          for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys[i_alloy].m_coord_groups.size(); i < fl_size_i; i++) {
            ...
            for (size_t j = 0, fl_size_j = m_coord_groups[i_coord_group].m_points.size(); j < fl_size_j; j++) {
              ...
              if (!counter) {
                ...
                if (stoich_header_report) {
                  ...
                  _report_data_ss << prettyPrintCompound(entry, gcd_vrt, true, latex_ft);
                }
                ...
                if (m_coord_groups[i_coord_group].m_is_on_hull) {
                  ...
                  if (include_equilibrium_phases && !equilibrium_phases.empty()) {
                    for (size_t k = 0, fl_size_k = equilibrium_phases.size(); k < fl_size_k; k++) {
                      for (size_t l = 0, fl_size_l = equilibrium_phases[k].size(); l < fl_size_l; l++) {
                      ...
                      if (m_coord_groups[i_coord_group].getDim() > 6) {
                        equilibrium_phases_vs.push_back(aurostd::joinWDelimiter(_equilibrium_phases_vs, " -- "));
                      } // that way, we don't run off the line
                      else {
                        equilibrium_phases_vs.push_back(aurostd::joinWDelimiter(_equilibrium_phases_vs, "--"));
                      }
                  ...
                  if (!decomposition_phases.empty() && scalar_product(decomposition_coefficients, decomposition_coefficients) >= ZERO_TOL) {
                    ...
                    for (size_t k = 0, fl_size_k = decomposition_phases.size(); k < fl_size_k; k++) {
                      if (std::abs(decomposition_coefficients[decomposition_coefficients.lrows + k + 1]) < ZERO_TOL) {
                        continue;
                      }
                      ...
                      misc_ss << aurostd::utype2string(std::abs(decomposition_coefficients[decomposition_coefficients.lrows + k + 1]), COEF_PRECISION) << "~";
                      misc_ss << output_name;
              ...
              // }
              if (point.isGState()) {
                report_data_ss << aurostd::PaddedPOST("\\rowcolor{green!85!blue} ", 30);
              } // red!25
              else if (point.m_is_sym_equivalent_g_state) {
                report_data_ss << aurostd::PaddedPOST("\\rowcolor{orange!85} ", 30);
              } // odd should be white
              else if (counter % 2) {
                report_data_ss << aurostd::PaddedPOST("\\rowcolor{white} ", 30);
              } // odd should be white
              else {
                report_data_ss << aurostd::PaddedPOST("\\rowcolor{gray!25} ", 30);
              }
              chpoint_properties.clear();
              for (size_t i = 0, fl_size_i = vheaders.size(); i < fl_size_i; i++) {
                chpoint_properties.push_back(grabCHPointProperty(point, vheaders[i], latex_ft));
              }
            ...
            if (!report_data_ss.str().empty()) {
              ...
              if (m_coord_groups[i_coord_group].m_is_on_hull) {
                ...
                if (m_coord_groups[i_coord_group].m_i_nary >= 1 &&
                    m_coord_groups[i_coord_group].m_n_plus_1_enthalpy_gain < AUROSTD_NAN) { // print binaries anyway... //print only for ternaries and up, binaries is trivial formation enthalpy (save space)
                  print_np1 = true;
                  precision_tmp = 0;
                  tmp_roundoff_tol = 5.0 * pow(10, -((int) precision_tmp) - 1);
                  np1_data_ss << "$" << getDelta(helvetica_font) << " H[" << m_coord_groups[i_coord_group].m_i_nary + 1 << "|"; // this delta is okay, should be italicized
                  if (m_coord_groups[i_coord_group].m_i_nary == 0) {
                    np1_data_ss << "0";
                  } // not allowed anymore, but we can print the value
                  else if (m_coord_groups[i_coord_group].m_i_nary == 1) {
                    np1_data_ss << "1";
                  } else if (m_coord_groups[i_coord_group].m_i_nary == 2) {
                    np1_data_ss << "\\{1,2\\}";
                  } else if (m_coord_groups[i_coord_group].m_i_nary == 3) {
                    np1_data_ss << "\\{1,2,3\\}";
                  } else if (m_coord_groups[i_coord_group].m_i_nary > 3) {
                    np1_data_ss << "\\{1,\\cdots," << m_coord_groups[i_coord_group].m_i_nary << "\\}";
                  }
                  np1_data_ss << "]=";
              ...
              // compound name
              main_TEX_ss << "\\begin{longtable}{"
                          << aurostd::joinWDelimiter((m_coord_groups[i_coord_group].m_is_on_hull ? valignments_compoundname_thermopropstable_string : valignments_onecol_lefttable_string), "") << "}" << endl;
              if (internal_links && isViablePoint(m_coord_groups[i_coord_group].m_ref_state)) {
                const uint ref_state = m_coord_groups[i_coord_group].m_ref_state;
                main_TEX_ss << "\\multicolumn{" << (m_coord_groups[i_coord_group].m_is_on_hull ? n_cols_compoundname_thermoprops : n_cols_onecol_left)
                            << "}{l}{\\phantomsection\\label{" + input + "_" + m_points[ref_state].m_entry.auid + "}} \\\\[0.1cm]" << endl;
              } else {
                main_TEX_ss << "\\multicolumn{" << (m_coord_groups[i_coord_group].m_is_on_hull ? n_cols_compoundname_thermoprops : n_cols_onecol_left) << "}{l}{} \\\\[0.1cm]" << endl;
    ...
    const string PWD = getPath();
    const string path = getPath(m_cflags, *p_FileMESSAGE, *p_oss);
    string destination = path + main_PDF_file;
    main_output_file = main_PDF_file;
    if (m_cflags.flag("CHULL::PNG_IMAGE") && !m_cflags.flag("CHULL::LATEX_DOC")) {
      destination = path + main_PNG_file;
      main_output_file = main_PNG_file;
    }
    const string LATEX_dir = aurostd::TmpDirectoryCreate("chullLATEX");
    ...
    // watermark
    if (!doc_only) {
      aurostd::EmbData::save_to_file("AFLOW_LOGO_SKINNY.pdf", "IMAGES", LATEX_dir + "/" + aflow_logo_skinny_file);
    ...
    if (print_aflow_logo_full) {
      aurostd::EmbData::save_to_file("AFLOW_LOGO_FULL.pdf", "IMAGES", LATEX_dir + "/" + aflow_logo_full_file);
    ...
    if (print_logo_2) {
      aurostd::EmbData::save_to_file("NOMAD_LOGO.png", "IMAGES", LATEX_dir + "/" + logo_file_2);
    ...
    if (show_latex_output) {
      if (latex_interactive_mode) {
        for (uint i = 0; i < num_compile; i++) {
          aurostd::execute(command.str());
        }
      } // will not save output, allows you to interact with LaTEX
      else {
        for (uint i = 0; i < num_compile; i++) {
          *p_oss << aurostd::execute2string(command.str()) << endl;
        }
      } // saves output
    } else {
      for (uint i = 0; i < num_compile; i++) {
    ...
    if (!aurostd::FileExist(LATEX_dir + "/" + main_PDF_file)) {
      ...
      if (!doc_only) {
        files_2_move.push_back(LATEX_dir + "/" + aflow_logo_skinny_file);
      }
      if (print_aflow_logo_full) {
        files_2_move.push_back(LATEX_dir + "/" + aflow_logo_full_file);
      }
      if (print_logo_2) {
        files_2_move.push_back(LATEX_dir + "/" + logo_file_2);
      }
    ...
    if (m_cflags.flag("CHULL::PNG_IMAGE")) {
      command.str("");
      clean_command.str("");
      const uint default_resolution = DEFAULT_CHULL_PNG_RESOLUTION;
      uint resolution = default_resolution;
      const string resolution_input = m_cflags.getattachedscheme("CHULL::PNG_RESOLUTION");
      if (!resolution_input.empty()) {
        if (!aurostd::isfloat(resolution_input)) {
          ...
          resolution = aurostd::string2utype<int>(resolution_input);
        }
        if (resolution == 0) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " executing: " << clean_command.str() << endl;
      }
      const string convert_output = aurostd::execute2string(command.str());
    ...
    if (m_cflags.flag("CHULL::LATEX_DOC")) {
      files_2_move.push_back(LATEX_dir + "/" + main_PDF_file);
    }
    if (m_cflags.flag("CHULL::PNG_IMAGE")) {
      files_2_move.push_back(LATEX_dir + "/" + main_PNG_file);
    }
    if (keep_tex) {
      files_2_move.push_back(LATEX_dir + "/" + main_TEX_file);
      if (!doc_only) {
    ...
    aurostd::RemoveDirectory(LATEX_dir);
#endif
    if (!aurostd::FileExist(destination)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unable to write " + main_output_file + " to " + path);
    }
  ...
  string ConvexHull::getJSONHeader() const {
    ...
    misc_ss << R"("alloy":")" << aurostd::joinWDelimiter(m_velements, "") << "\"";
    vout.push_back(misc_ss.str());
    misc_ss.str("");

    misc_ss << R"("aflow_version":"aflow)" << string(AFLOW_VERSION) << "\"";
    vout.push_back(misc_ss.str());
    misc_ss.str("");

    misc_ss << R"("aflow_website":")" << AFLOW_WEB << "\"";
    vout.push_back(misc_ss.str());
    misc_ss.str("");

    misc_ss << R"("date":")" << aurostd::get_datetime_formatted("-", false) << "\""; // CO20180819 - we don't need minute/second precision, date is fine //previously datetime
    vout.push_back(misc_ss.str());
    misc_ss.str("");
  ...
  string ConvexHull::grabCHPointProperty(const ChullPoint& point, const string& property, filetype ftype) const {
    ...
    const uint precision = COEF_PRECISION;
    uint precision_tmp = COEF_PRECISION;
    ...
    const string string_wrapper_math_mode = "$"; // latex only
    bool compounds_column_report = false;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " starting property=" << property << endl;
    }
    bool latex_property = false;
    bool math_mode = false;
    if (property == "compound" || property == "compound_latex" || property == "compound_reduced" || property == "compound_reduced_latex" || property == "compound_fractional" ||
        property == "compound_fractional_latex") {
      ...
      if (property == "compound_reduced" || property == "compound_reduced_latex") {
        vred = gcd_vrt;
      }
      if (property == "compound_fractional" || property == "compound_fractional_latex") {
        vred = frac_vrt;
      }
      ...
      if ((ftype == latex_ft || latex_property) && math_mode && !value.empty()) {
        value = aurostd::wrapString(value, string_wrapper_math_mode);
      }
      ...
      if (ftype == latex_ft || latex_property) {
        value = prettyPrintPrototype(point, ftype == json_ft, false);
      } // JSON needs double backslash
      else {
        value = point.m_entry.prototype;
      }
      value = aurostd::wrapString(value, string_wrapper);
      padding = 100; // latex only
    } else if (property == "auid") {
      if (ftype == latex_ft) {
        value = "\\texttt{" + aurostd::wrapString(point.m_entry.auid, string_wrapper) + "}";
      } else {
        value = aurostd::wrapString(point.m_entry.auid, string_wrapper);
      }
    } else if (property == "aurl") {
      value = aurostd::wrapString(point.m_entry.aurl, string_wrapper);
    } else if (property == "url_entry_page") {
      value = aurostd::wrapString(ENTRY_PAGE_URL_PREFIX + point.m_entry.auid, string_wrapper);
      ...
      if (ftype == latex_ft || latex_property) {
        sg = aurostd::fixStringLatex(sg, ftype == json_ft, true);
      }
      value = sg;
      if ((ftype == latex_ft || latex_property) && math_mode && !value.empty()) {
        value = aurostd::wrapString(value, string_wrapper_math_mode);
    ...
    } else if (property == "ground_state") {
      value = (point.isGState() ? "true" : "false");
    } else if (property == "auid_equivalent_structures") {
      if (!(ftype == txt_ft || ftype == json_ft)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No latex rule defined for " + property);
      }
      if (!point.m_equivalent_entries.empty()) {
        const vector<uint>& equivalent_g_states = point.m_equivalent_entries;
        vector<string> auids;
        uint i_point = AUROSTD_MAX_UINT;
        for (size_t i = 0, fl_size_i = equivalent_g_states.size(); i < fl_size_i; i++) {
      ...
      if (point.m_found_icsd) {
        const uint i_point = point.m_i_icsd;
        if (i_point > m_points.size() - 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index within points");
        }
      ...
      if (point.isGState() && !point.isUnary()) { // unaries are always gstates, but do NOT have any mixture context
        ...
        if (!m_cflags.flag("CHULL::CALCULATE_HIGHEST_DIMENSION_ONLY")) {
          if (m_coord_groups[i_coord_group].m_equilibrium_phases.empty()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Equilibrium phases not set");
          }
        }
        ...
        if (!equilibrium_phases.empty()) {
          for (size_t i = 0, fl_size_i = equilibrium_phases.size(); i < fl_size_i; i++) {
            if (equilibrium_phases[i].empty()) {
              compounds.push_back(null_value);
              continue;
            }
            _compounds.clear();
            for (size_t j = 0, fl_size_j = equilibrium_phases[i].size(); j < fl_size_j; j++) {
              ...
              if (m_points[i_point].m_has_entry) {
                ...
                _compounds.push_back(null_value);
              }
            }
      ...
      if (GET_DECOMPOSITION_POLYMORPHS || !point.isGState()) {
        ...
        if (GET_DECOMPOSITION_POLYMORPHS || !m_coord_groups[i_coord_group].m_is_on_hull) {
          if (!m_coord_groups[i_coord_group].m_decomp_phases.empty()) {
            vector<string> compounds;
            uint i_point = AUROSTD_MAX_UINT;
            for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_decomp_phases.size(); i < fl_size_i; i++) {
        ...
        if (GET_DECOMPOSITION_POLYMORPHS || !m_coord_groups[i_coord_group].m_is_on_hull) {
          if (!m_coord_groups[i_coord_group].m_decomp_phases.empty()) {
            ...
            for (int i = m_coord_groups[i_coord_group].m_decomp_coefs.lrows; i <= m_coord_groups[i_coord_group].m_decomp_coefs.urows; i++) {
              if (aurostd::nonZeroWithinTol(m_coord_groups[i_coord_group].m_decomp_coefs[i], ZERO_TOL)) {
                nonzero_coefs.push_back(m_coord_groups[i_coord_group].m_decomp_coefs[i]);
              }
            }
            precision_tmp = precision;
            tmp_roundoff_tol = 5.0 * pow(10, -((int) precision_tmp) - 1);
      ...
      if (i_nary != AUROSTD_MAX_UINT) {
        i_nary += 1;
      } // m_i_nary is just index, increment by count to get nspecies
      else {
        i_nary = 0;
      } // set to something reasonable
      if (i_nary == 0) { // potential other ways of defining nspecies
        if (point.m_has_entry) {
          i_nary = point.m_entry.nspecies;
        }
      }
      ...
      if (m_cflags.flag("CHULL::SKIP_STABILITY_CRITERION_ANALYSIS") || point.getStabilityCriterion(_m_) >= AUROSTD_NAN) {
        value = null_value;
      } else {
        if (point.isGState()) {
      ...
      if (m_cflags.flag("CHULL::SKIP_STABILITY_CRITERION_ANALYSIS") || point.getRelativeStabilityCriterion() >= AUROSTD_NAN) {
        value = null_value;
      } else {
        if (point.isGState()) {
      ...
      if (m_cflags.flag("CHULL::SKIP_N+1_ENTHALPY_GAIN_ANALYSIS") || point.getNPlus1EnthalpyGain(_m_) >= AUROSTD_NAN) {
        value = null_value;
      } else {
        if (point.isGState()) {
    ...
    if (property == "position_vertices") {
      ...
      for (size_t i = 0, fl_size_i = facet.m_vertices.size(); i < fl_size_i; i++) {
        if (facet.m_vertices[i].ch_point.m_has_entry) {
          ...
          const uint i_nary = facet.m_vertices[i].ch_point.m_i_nary;
          const uint i_alloy = facet.m_vertices[i].ch_point.m_i_alloy;
          const string elements = aurostd::joinWDelimiter(alloyToElements(i_nary, i_alloy), ""); // unary, so "" delimiter doesn't play a role
          compounds.push_back(aurostd::wrapString("artificial:" + elements, string_wrapper));
        } else {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown print setting");
        }
      }
      value = aurostd::wrapString(aurostd::joinWDelimiter(compounds, ","), list_prefix, list_suffix);
      ...
      value = aurostd::utype2string(facet.m_content, precision, true, roundoff_tol, FIXED_STREAM);
    } else if (property == "normal") {
      value = "[" + aurostd::joinWDelimiter(aurostd::xvecDouble2vecString(facet.m_normal, precision, true, roundoff_tol, FIXED_STREAM), vector_delimiter) + "]";
    } else if (property == "offset") {
      value = aurostd::utype2string(facet.m_offset, precision, true, roundoff_tol, FIXED_STREAM);
    } else if (property == "centroid") {
      value = "[" + aurostd::joinWDelimiter(aurostd::xvecDouble2vecString(facet.m_facet_centroid, precision, true, roundoff_tol, FIXED_STREAM), vector_delimiter) + "]";
    } else if (property == "is_hypercollinear") {
      value = (facet.m_is_hypercollinear ? "true" : "false");
    } else if (property == "is_vertical") {
      value = (facet.m_is_vertical ? "true" : "false");
    } else if (property == "is_artificial") {
      value = (facet.m_is_artificial ? "true" : "false");
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown property");
    }
    if (value.empty()) {
  ...
  vector<vector<string>> ConvexHull::getPointsData(const string& properties_str, vector<string>& headers, filetype ftype) const {
    ...
    for (size_t i_coord_group = 0, fl_size_i_coord_group = m_coord_groups.size(); i_coord_group < fl_size_i_coord_group; i_coord_group++) {
      ...
      for (size_t i = 0, fl_size_i = m_coord_groups[i_coord_group].m_points.size(); i < fl_size_i; i++) {
        ...
        ventries.emplace_back(0);
        for (size_t j = 0, fl_size_j = vproperties.size(); j < fl_size_j; j++) {
    ...
    if (ftype == json_ft) {
      ...
      for (size_t i = 0, fl_size_i = vproperties.size(); i < fl_size_i; i++) {
  ...
  vector<vector<vector<vector<string>>>> ConvexHull::getFacetsData(const string& facet_properties_str, vector<string>& headers, filetype ftype) const {
    ...
    if (m_naries.size() < 2) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No naries (larger than 1) found");
    }
    ...
    uint i_facet;
    for (size_t i_nary = 1, fl_size_i_nary = m_naries.size(); i_nary < fl_size_i_nary; i_nary++) {
      if (!m_naries[i_nary].m_initialized) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized nary[" + aurostd::utype2string(i_nary) + "]");
      }
      if (m_naries[i_nary].m_alloys.empty()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No alloys found in nary[" + aurostd::utype2string(i_nary) + "]");
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " looking at i_nary=" << i_nary << endl;
      }
      ventries.emplace_back(0);
      for (size_t i_alloy = 0, fl_size_i_alloy = m_naries[i_nary].m_alloys.size(); i_alloy < fl_size_i_alloy; i_alloy++) {
        if (!m_naries[i_nary].m_alloys[i_alloy].m_initialized) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized nary[i_nary=" + aurostd::utype2string(i_nary) + ",i_alloy=" + aurostd::utype2string(i_alloy) + "]");
        }
        if (!m_cflags.flag("CHULL::CALCULATE_HIGHEST_DIMENSION_ONLY")) {
          if (m_naries[i_nary].m_alloys[i_alloy].m_facets.empty()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No facets found in nary[i_nary=" + aurostd::utype2string(i_nary) + ",i_alloy=" + aurostd::utype2string(i_alloy) + "]");
          }
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " looking at i_nary=" << i_nary << ",i_alloy=" << i_alloy << endl;
        }
        ventries.back().emplace_back(0);
        for (size_t i = 0, fl_size_i = m_naries[i_nary].m_alloys[i_alloy].m_facets.size(); i < fl_size_i; i++) {
          i_facet = m_naries[i_nary].m_alloys[i_alloy].m_facets[i];
          const ChullFacet& facet = m_facets[i_facet];
          if (!facet.m_initialized) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__,
                                  "Facet[i_nary=" + aurostd::utype2string(i_nary) + ",i_alloy=" + aurostd::utype2string(i_alloy) + ",i_facet=" + aurostd::utype2string(i_facet) + "] is not initialized");
          }
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " looking at i_nary=" << i_nary << ",i_alloy=" << i_alloy << ",i_facet=" << i_facet << endl;
          }
          ventries.back().back().emplace_back(0);
          for (size_t j = 0, fl_size_j = vproperties.size(); j < fl_size_j; j++) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " created ventries" << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " created headers" << endl;
    }
    return ventries;
  }
  ...
  void ConvexHull::getPlainTextColumnSizes(const vector<string>& headers, const vector<vector<string>>& ventries, vector<uint>& sizes) const {
    for (size_t i = 0, fl_size_i = ventries.size(); i < fl_size_i; i++) {
      if (headers.size() != ventries[i].size()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between headers and ventries");
      }
    }
    if (sizes.empty()) { // initialize
      sizes.resize(headers.size());
      for (size_t i = 0, fl_size_i = headers.size(); i < fl_size_i; i++) {
        sizes[i] = headers[i].size();
      }
    }
    if (sizes.size() != headers.size()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between sizes and headers");
    }
    uint p_size;
    for (size_t i = 0, fl_size_i = ventries.size(); i < fl_size_i; i++) {
      for (size_t j = 0, fl_size_j = ventries[i].size(); j < fl_size_j; j++) {
  ...
  void ConvexHull::getPlainTextColumnSizesPoints(const vector<string>& headers, const vector<vector<string>>& ventries, vector<uint>& sizes) const {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " sizes=" << sizes.size() << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " determined sizes" << endl;
    }
  }
  ...
  string ConvexHull::getPlainTextTable(const vector<string>& headers, const vector<vector<string>>& ventries, const vector<uint>& sizes) const {
    stringstream out_ss;
    for (size_t i = 0, fl_size_i = headers.size(); i < fl_size_i; i++) {
      out_ss << aurostd::PaddedPOST(headers[i], sizes[i] + 5);
    }
    out_ss << endl;
    for (size_t i = 0, fl_size_i = ventries.size(); i < fl_size_i; i++) {
      for (size_t j = 0, fl_size_j = ventries[i].size(); j < fl_size_j; j++) {
  ...
  string ConvexHull::getJSONTable(const vector<string>& headers, const vector<vector<string>>& ventries) const {
    ...
    for (size_t i = 0, fl_size_i = ventries.size(); i < fl_size_i; i++) {
      for (size_t j = 0, fl_size_j = ventries[i].size(); j < fl_size_j; j++) {
        ...
        _vout.push_back(misc_ss.str());
        misc_ss.str("");
      }
      vout.push_back(aurostd::wrapString(aurostd::joinWDelimiter(_vout, ","), "{", "}"));
      _vout.clear();
    }
    return aurostd::wrapString(aurostd::joinWDelimiter(vout, ","), "[", "]");
  }
  ...
  void ConvexHull::writeText(filetype ftype) const {
    ...
    if (!(ftype == txt_ft || ftype == json_ft)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown mode");
    }
    ...
    if (ftype == txt_ft) {
      message << "Starting plain text generator";
    } else {
      message << "Starting JSON generator";
    }
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);

    const string properties_str_points = getPointsPropertyHeaderList(ftype);

    const bool terse_output = false;
    string facet_properties_str = "position_vertices";
    if (!terse_output) {
      facet_properties_str += ",compound_vertices";
    }
    ...
    const vector<vector<string>> ventries_points = getPointsData(properties_str_points, headers_points, ftype);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " got points data" << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " got facets data" << endl;
    }

    // HEADER
    if (ftype == json_ft) {
      vout.push_back("\"hull_data\":" + getJSONHeader());
    } else {
      main_text_ss << getPlainTextHeader();
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " created doc header" << endl;
    }

    // points data
    if (ftype == json_ft) {
      vout.push_back("\"points_data\":" + getJSONTable(headers_points, ventries_points));
    } else {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " added points data" << endl;
    }
    ...
    if (ftype == json_ft) {
      ...
      misc_ss.str("");
      vector<string> _vout;
      for (size_t i = 0, fl_size_i = ventries_facets.size(); i < fl_size_i; i++) {
        for (size_t j = 0, fl_size_j = ventries_facets[i].size(); j < fl_size_j; j++) {
          misc_ss << "\"" << i + 2 << "-nary:";
          if (!m_velements.empty()) {
            misc_ss << aurostd::joinWDelimiter(alloyToElements(i + 1, j), "-");
          } else {
            misc_ss << j + 1;
          }
      ...
      for (size_t i = 0, fl_size_i = ventries_facets.size(); i < fl_size_i; i++) {
        for (size_t j = 0, fl_size_j = ventries_facets[i].size(); j < fl_size_j; j++) {
          ...
          if (!m_velements.empty()) {
            main_text_ss << "(" << aurostd::joinWDelimiter(alloyToElements(i + 1, j), "-") << ")";
          } else {
            main_text_ss << "(" << j + 1 << ")";
          }
          main_text_ss << endl;
          main_text_ss << getPlainTextTable(headers_facets, ventries_facets[i][j], column_sizes);
          if (!(i == ventries_facets.size() - 1 && j == ventries_facets[i].size() - 1)) {
            main_text_ss << endl;
          }
        }
        if (i != ventries_facets.size() - 1) {
          main_text_ss << endl;
        }
      }
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " added facets data" << endl;
    }

    if (ftype == json_ft) {
      main_text_ss << aurostd::wrapString(aurostd::joinWDelimiter(vout, ","), "{", "}");
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " now writting to file" << endl;
    }
    const string path = getPath(m_cflags, *p_FileMESSAGE, *p_oss);
    const string destination = path + file_name;
    aurostd::stringstream2file(main_text_ss, destination);
    if (!aurostd::FileExist(destination)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unable to write " + file_name + " to " + path);
    }
  ...
  void ConvexHull::writeWebApp() const {
    ...
    // SK20200331 START
    const bool fhsc_requested = m_cflags.flag("CHULL::CALCULATE_FAKE_HULL_STABILITY_CRITERION"); // only neglect feature via web
    const bool fhn1eg_requested = m_cflags.flag("CHULL::CALCULATE_FAKE_HULL_N+1_ENTHALPY_GAIN");
    ...
    if (fhsc_requested) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " main_JSON_file=" << main_JSON_file << endl;
      }
    }
    ...
    for (size_t i = 0, fl_size_i = m_coord_groups.size(); i < fl_size_i; i++) {
      ...
      for (size_t j = 0, fl_size_j = m_coord_groups[i].m_points.size(); j < fl_size_j; j++) {
        //////////////////////////////////////////////////////////////////////////
        // START Entries filter
        //////////////////////////////////////////////////////////////////////////
        ...
        if (!stoich_groups_set) {
          ...
          for (int j = point.m_elements_present.lrows; j <= point.m_elements_present.urows; j++) {
            if (point.m_elements_present[j] == 1) {
              stoich_comp_JSON_ss << "{";
              stoich_comp_JSON_ss << R"("element":")" << m_velements[j - point.m_elements_present.lrows] << "\",";
          ...
          stoich_data_JSON_ss << R"("id":")" << id_data_JSON_ss.str() << "\",";
          id_data_JSON_ss.str("");
          // get is_hull
          ...
          if (!decomposition_phases.empty() && scalar_product(decomposition_coefficients, decomposition_coefficients) >= ZERO_TOL) {
            ...
            for (size_t k = 0, fl_size_k = decomposition_phases.size(); k < fl_size_k; k++) {
              ...
              if (!dc_phase.m_has_entry) {
                // we need to adjust for missing unaries
                // look for coords
                decomp_data_JSON_ss << R"("entry":")" << AFLOW_HULL_ENDPOINT_STRING << ":" << aurostd::joinWDelimiter(alloyToElements(dc_phase), "") << "\","; // unary, so "" delimiter doesn't play a role
              } else {
                const aflowlib::_aflowlib_entry& equation_entry = dc_phase.m_entry;
                decomp_data_JSON_ss << R"("entry":")" << equation_entry.auid << "\",";
              }
        ...
        if (!point.m_has_entry) {
          ...
          if (point.m_is_g_state) {
            hull_points_data_JSON_ss << data_helper_ss.str();
          }
          ...
          //
          distances_data_JSON_ss << ",";
          distances_data_JSON_ss << R"("compound": ")" << entry.compound << "\"";
          distances_data_JSON_ss << ",";
        ...
        if (point.m_is_g_state) {
          hull_points_data_JSON_ss << ",";
          hull_points_data_JSON_ss << R"("compound": ")" << entry.compound << "\"";
          hull_points_data_JSON_ss << ",";
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vertices=" << aurostd::joinWDelimiter(vertices, ",") << endl;
    }

    vector<uint> v_ch_indices;
    vector<uint> v_vertices_indices;
    bool found = false;
    for (size_t i = 0, fl_size_i = m_facets.size(); i < fl_size_i; i++) {
      // const xvector<double> normal = m_facets[i].m_normal;
      if (!(/*m_facets[i].m_is_hypercollinear||*/ m_facets[i].m_is_vertical || m_facets[i].m_is_artificial) &&
          m_facets[i].m_dim == m_dim) { // keep is_hypercollinear, might create small gaps in visualization otherwise //CO20190423 - EG needs three-dimensional only for 3D
        v_ch_indices = m_facets[i].getCHIndices();
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " v_ch_indices=" << aurostd::joinWDelimiter(v_ch_indices, ",") << endl;
        }
        v_vertices_indices.clear();
        for (size_t j = 0, fl_size_j = v_ch_indices.size(); j < fl_size_j; j++) {
          found = false;
          for (size_t k = 0, fl_size_k = vertices.size(); k < fl_size_k && !found; k++) {
          ...
          if (!found) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "ChullPoint[" + aurostd::utype2string(v_ch_indices[j]) + "] could not be found in getHullPoints()", _INDEX_ILLEGAL_);
          }
        }
    ...
    for (size_t i = 0, fl_size_i = vertices.size(); i < fl_size_i; i++) {
      ...
      if (!point.m_has_entry) {
        ...
        vertices_data_JSON_ss << "\"" << entry.auid << "\"";
      }

      vertices_data_JSON_ss << ",";
      vertices_data_JSON_ss << R"("compound": ")" << entry.compound << "\",";
    ...
    main_JSON_ss << "{";
    main_JSON_ss << R"("name":")" << input << "\","; // input_hyphened
    ...
    const string path = getPath(m_cflags, *p_FileMESSAGE, *p_oss);
    const string destination = path + main_JSON_file;
    aurostd::stringstream2file(main_JSON_ss, destination);
    if (!aurostd::FileExist(destination)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Could not write " + main_JSON_file + " to " + path);
    }
  ...
  bool ConvexHull::sortWithinCoordGroup::operator()(uint i, uint j) {
    ...
    if (ci.m_has_entry && cj.m_has_entry) {
      ...
      return (energy_sort || ((ci.getLastCoord() == cj.getLastCoord()) && (ci.m_entry.prototype < cj.m_entry.prototype)) ||
              ((ci.getLastCoord() == cj.getLastCoord()) && (ci.m_entry.prototype == cj.m_entry.prototype) && (ci.m_entry.compound < cj.m_entry.compound)) ||
              ((ci.getLastCoord() == cj.getLastCoord()) && (ci.m_entry.prototype == cj.m_entry.prototype) && (ci.m_entry.compound == cj.m_entry.compound) &&
  ...
  bool ConvexHull::sortCHullPoints::operator()(uint i, uint j) const {
    if (i > m_points.size() - 1 || j > m_points.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Invalid index for m_points");
    }
    const ChullPoint& ci = m_points[i];
    const ChullPoint& cj = m_points[j];
    const sortThermoPoints stp(m_sort_stoich_ascending, m_sort_energy_ascending);
    return stp.operator()(ci, cj);
  }

  bool ConvexHull::sortFacetsByPoints::operator()(const ChullFacet& fi, const ChullFacet& fj) const {
    if (fi.m_vertices.size() != fj.m_vertices.size()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch among facets");
    } // return m_ascending_order ? fi.m_vertices.size()<rj.m_vertices.size() : fi.m_vertices.size()>rj.m_vertices.size();
    if (fi.m_normal.rows != fj.m_normal.rows) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Dimension mismatch between facet normals");
    }
    if (!(fi.m_initialized && fj.m_initialized)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Uninitialized facet");
    } // ensure we have inward pointing normal and angle

    if (m_auto_sort_energy) {
      if (fi.m_in_lower_hemisphere != fj.m_in_lower_hemisphere) {
        return fi.m_in_lower_hemisphere > fj.m_in_lower_hemisphere;
      } // upper hemisphere last
      if (fi.m_is_hypercollinear != fj.m_is_hypercollinear) {
        return fi.m_is_hypercollinear > fj.m_is_hypercollinear;
      } // hypercollinearity last within hemisphere
      if (fi.m_is_vertical != fj.m_is_vertical) {
        return fi.m_is_vertical > fj.m_is_vertical;
      } // vertical above hypercollinearity
    }

    for (size_t i = 0, fl_size_i = fi.m_vertices.size(); i < fl_size_i; i++) {
      if (fi.m_vertices[i].ch_index != fj.m_vertices[i].ch_index) {
        ...
        if (m_auto_sort_stoich) {
          sort_stoich_ascending = !fi.m_in_lower_hemisphere;
        } // left to right in lower hemisphere
        if (m_auto_sort_energy) {
          sort_energy_ascending = fi.m_in_lower_hemisphere;
        } // bottom to top in lower hemisphere
        const sortThermoPoints stp(sort_stoich_ascending, sort_energy_ascending);
        return stp.operator()(ci, cj);
      }
    }
