...
xmatrix<double> GetAllDistances(bool& samplingOK, xstructure str, xstructure superStr, int cellNumber) {
  ...
  xvector<double> b;
  xvector<double> c;
  xvector<double> d; // Lattice vectors and distance vector
  double radius;
  //////////////////
  ...
  //////////////////
  const uint k = (uint) cellNumber;
  const uint p = (uint) cellNumber / 2;
  ...
  for (size_t i = 1; i <= str.atoms.size(); i++) {
    out = 0;
    for (size_t j = 1; j <= superStr.atoms.size(); j++) {
...
void FindGap(xvector<double>& maxGaps, xvector<double>& gapPositions, xmatrix<double> distances, xstructure str, xstructure superStr, double new_tol) {
  /*
  ...
  for (size_t i = 1; i <= str.atoms.size(); i++) { // Must be done for each individual atom
    ...
    for (size_t j = 2; j <= superStr.atoms.size(); j++) {
      if ((maxGaps(i) - new_tol) <= (individualDistances(j) - individualDistances(j - 1))) {
        if ((individualDistances(j - 1) < gapPositions(i) || maxGaps(i) < (individualDistances(j) - individualDistances(j - 1) - new_tol)) &&
...
void FindNewGap(uint i, xvector<double>& maxGaps, xvector<double>& gapPositions, xmatrix<double> distances, xstructure superStr, double new_tol) {
  const double cutoff = gapPositions(i);
  xvector<double> individualDistances(superStr.atoms.size(), 1);
...
void BuildAE(deque<deque<int>>& environment, xvector<double> gapPositions, xstructure str, xstructure superStr, int cellNumber, double new_tol) {
  xvector<double> d;
  const uint k = (uint) cellNumber;
  const uint p = (uint) cellNumber / 2;
...
void BuildNewAE(int i, deque<deque<int>>& environment, xvector<double> gapPositions, xstructure superStr, int cellNumber, double new_tol) {
  xvector<double> d;
  const uint k = (uint) cellNumber;
  const uint p = (uint) cellNumber / 2;
...
void DetermineDistinctAEs(vector<vector<xmatrix<int>>> VatomicEnvironments, vector<vector<vector<int>>>& distinctAE, vector<vector<vector<int>>>& AEcoefs, vector<int>& str_remove) {
  xmatrix<int> str;
  const uint num_species = VatomicEnvironments[0].size();

  for (size_t i = 0; i < VatomicEnvironments.size(); i++) {
          ...
          for (int j = 0; j < str(1, 2); j++) {
            for (size_t l = 0; l < distinctAE[ns].size(); l++) {
              ...
              if (similarVertices == str(2, j + 1)) {
                ...
                c_temp.push_back(0);
                for (size_t n = 0; n < AEcoefs[ns].size(); n++) {
...
namespace pflow {
  ...
  deque<deque<int>> CalcAtomicEnvironment(xstructure& str) {
    ...
    const double new_tol = str.sym_eps;
    ...
    for (size_t i = 1; i <= str.atoms.size(); i++) { // Just a check
      if (str.atoms[i - 1].type != superStr.atoms[(p * (k * k + k + 1) + (i - 1) * k * k * k)].type) {
        cout << "WARNING!!! Mapping unsuccessful!" << endl;
      }
    }
    ...
    xvector<double> maxGaps(str.atoms.size(), 1);
    xvector<double> gapPositions(str.atoms.size(), 1);
    FindGap(maxGaps, gapPositions, distances, str, superStr, new_tol);

    deque<deque<int>> environment;
    const deque<deque<int>> dummyIDD;
    const deque<int> dummyID;
    ...
    const xvector<double> atomicDistances(1, maxAE);
    xvector<double> d;
    xvector<double> v1;
    xvector<double> v2;
    xvector<double> v3;
    ...
    while (sumRecalcAE != 0) {
      for (size_t i = 1; i <= str.atoms.size(); i++) {// Finds connectivity and raises flag in case of maximum gap rule failure
        if (recalcAE[i] == 1) {
          recalcAE[i] = 0;

          for (size_t j = 1; j <= environment[i - 1].size(); j++) {
            for (size_t l = 1; l <= environment[i - 1].size(); l++) {
              d = superStr.atoms[environment[i - 1][j - 1] - 1].cpos - superStr.atoms[environment[i - 1][l - 1] - 1].cpos;
              if (modulus(d) < dist && j != l) {
                dist = modulus(d);
              }
            }
            for (size_t l = 1; l <= environment[i - 1].size(); l++) { // Build connectivity matrix
              d = superStr.atoms[environment[i - 1][j - 1] - 1].cpos - superStr.atoms[environment[i - 1][l - 1] - 1].cpos;
          ...
          for (size_t j = 1; j <= environment[i - 1].size(); j++) { // Symmetrizes connectivity matrix
            for (size_t l = 1; l <= environment[i - 1].size(); l++) {
          ...
          for (size_t j = 1; j <= environment[i - 1].size(); j++) { // Flags undercoordinated atoms (only the ones with less than 3 neighbors)
            neighCount = 0;
            for (size_t l = 1; l <= environment[i - 1].size(); l++) {
          ...
          for (size_t j = 1; j <= environment[i - 1].size(); j++) {
            for (size_t l = 1; l <= environment[i - 1].size(); l++) { // Checks if there are atoms over polyhedra surfaces
    ...
    for (size_t i = 1; i <= str.atoms.size(); i++) { // Categorizes each AE
      ...
      for (size_t j = 1; j <= environment[i - 1].size(); j++) {
        for (size_t l = 1; l <= environment[i - 1].size(); l++) {
          if (aEConnectivity[i - 1][j - 1][l - 1] == 1) { // j linked to l
            ...
            for (size_t m = 1; m <= environment[i - 1].size(); m++) {
            ...
            for (size_t m = 1; m <= environment[i - 1].size(); m++) {
              for (size_t n = 1; n <= environment[i - 1].size(); n++) {
                if (aEConnectivity[i - 1][l - 1][m - 1] == 1 && aEConnectivity[i - 1][m - 1][n - 1] == 1 && aEConnectivity[i - 1][n - 1][j - 1] == 1 && aEConnectivity[i - 1][m - 1][j - 1] != 1 &&
                    aEConnectivity[i - 1][n - 1][l - 1] != 1 && j != m && l != n) { // Rectangle (j->l->m->n->j)
                  for (size_t o = 1; o <= environment[i - 1].size(); o++) {
                    if (aEConnectivity[i - 1][l - 1][o - 1] == 1 && aEConnectivity[i - 1][m - 1][o - 1] == 1 && aEConnectivity[i - 1][n - 1][o - 1] == 1 &&
                        aEConnectivity[i - 1][j - 1][o - 1] == 1) { // The tetragon is the base of a pyramid
                      pyramid = true;
                    }
                  }
                  if (!pyramid) {
    ...
    for (size_t i = 1; i <= str.atoms.size(); i++) { // Identifies and groups vertices
      if (!environment[i - 1].empty()) {
        for (size_t j = 1; j <= (environment[i - 1].size() - 1); j++) {
          for (size_t l = 1; l <= (environment[i - 1].size() - j); l++) {
      ...
      for (size_t j = 1; j <= environment[i - 1].size(); j++) { // Counts the #occurrences of each vertex type
        for (size_t l = 1; l <= (vertType[i - 1].size() / 3); l++) {
  ...
  xmatrix<int> GetAtomicEnvironment(xstructure& str, int ns, deque<deque<int>> vertType) {
    ...
    for (int i = (1 + i_inc); i <= (str.num_each_type.at(ns) + i_inc - 1); i++) {
      for (size_t j = 1; j <= (str.atoms.size() - i); j++) {
        if (vertType[i - 1].size() == vertType[i + j - 1].size()) {// AE may be similar
          for (size_t l = 1; l <= (vertType[i - 1].size() / 3); l++) {
            for (size_t m = 1; m <= (vertType[i + j - 1].size() / 3); m++) {
    ...
    for (uint i = 1; i < distinctAE + 1; i++) {
      undercoord = false;
      for (size_t j = 0; j < (systemAE[i].size() / 3); j++) {
      ...
      if (undercoord == false) {
        ...
        // for(size_t j=0;j<systemAE[i].size();j++){
        //  cout << systemAE[i][j] << " ";
        // }
        // cout << endl;
      }
      ...
      for (int i = (1 + i_inc); i <= (str.num_each_type.at(ns) + i_inc); i++) { // Counts AE occurrences
        for (uint j = 1; j <= AEcount; j++) {
          if (vertType[i - 1].size() == (AEkeep[j].size() - 1)) {// AE may be similar
            for (size_t l = 1; l <= (vertType[i - 1].size() / 3); l++) {
              for (size_t m = 1; m <= (AEkeep[j].size() / 3); m++) {
                if (vertType[i - 1][1 + (l - 1) * 3] == AEkeep[j][1 + (m - 1) * 3] && vertType[i - 1][(l - 1) * 3] == AEkeep[j][(m - 1) * 3] && vertType[i - 1][2 + (l - 1) * 3] == AEkeep[j][2 + (m - 1) * 3]) {
                  aux = aux + 3;
                }
              }
            }
    ...
    for (uint i = 1; i <= AEcount; i++) {
      ...
      for (size_t j = 1; j <= (AEkeep[i].size() - 1); j++) {
  ...
  void ComputeGFA(vector<double> Egs,
                  vector<vector<double>> VDecomp_coefs,
                  ...
                  vector<nhull::Entry> vcpt,
                  vector<vector<double>> VStoichE,
                  vector<string> species) {
    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_GFA_); // CO20190424
    if (LDEBUG) { // CO20190424
      for (size_t i = 0; i < Egs.size(); i++) {
        cerr << __AFLOW_FUNC__ << " Egs[i=" << i << "]=" << Egs[i] << endl;
      } // CO20190424
    } // CO20190424
    ...
    while (xc.increment()) {
      ...
      for (size_t i = 0; i < ti.size() - 1; i++) {
        for (size_t j = i + 1; j < ti.size(); j++) {
          for (uint x = 0; x < dimension - 1; x++) {
            if (std::abs(VStoichE.at(ti[i]).at(x) - VStoichE.at(ti[j]).at(x)) > 0.000001) {
              break;
    ...
    for (size_t X = 0; X < vcpt.size(); X++) {
      vector<double> gstoich;
      double last = 1;
      for (int i = vcpt[X].m_coord.lrows; i <= vcpt[X].m_coord.urows - 1; i++) {  // CO20190424
        gstoich.push_back(vcpt[X].m_coord[i]);
        last = last - vcpt[X].m_coord[i];
      }
      gstoich.push_back(last);
      cout << "stoichiometry " << X << ": ";
      for (size_t i = 0; i < gstoich.size(); i++) {
        cout << gstoich[i] << " ";
      }
      ...
      for (uint ns = 0; ns < dimension; ns++) {
        ...
        const xvector<double> ps_temp(distinctAE_size[ns], 1);
        VpseudoGS_AEcoefs.push_back(ps_temp);
      }
      xvector<double> pscg_temp(entries_size, 1);
      vector<double> weight_avgDP;
      vector<double> VEavg;
      for (size_t p = 0; p < VDecomp_phases[X].size(); p++) {
        if (VDecomp_phases[X][p] > entries_size - 1) {
          cerr << "ERROR in convex hull calculation: decomposition phase is not an entry." << endl;
        }
        pscg_temp[VDecomp_phases[X][p] + 1] = VDecomp_coefs[X][p];
      }
      for (uint ns = 0; ns < dimension; ns++) {
        xvector<double> psg_temp = aurostd::operator*(pscg_temp, xAEcoefs[ns]);
        const double modulus = aurostd::modulus(psg_temp);
        ...
        if (modulus == 0) {
          ...
          for (uint a = 1; a < distinctAE_size[ns] + 1; a++) {
            ...
            if (VpseudoGS_AEcoefs[ns][a] > 1.0000001) {
              cout << "ERROR: a=" << a << ", VpseudoGS_AEcoefs[ns][a]=" << VpseudoGS_AEcoefs[ns][a] << endl;
            }
          }
        }
      }
      const vector<int> stoich_indices;

      for (size_t l = 0; l < indices.size(); l++) {
        bool may_balance = true;
        vector<vector<int>> side_l;
        ...
        if (may_balance == true) {
          ...
          if (stop == false) {
            ...
            xvector<double> compositionL(dimension); //-1,0);  //CO20190424
            for (uint x = 0; x < dimension; x++) {
            ...
            bool balanced = true;
            try {
              stoich_coeffs = balanceChemicalEquation(lhs, rhs, true, 0.001);
            } catch (aurostd::xerror& vre) {
              balanced = false;
              const string check = vre.what();
            ...
            if (balanced == true) {
              ...
              for (uint ns = 0; ns < dimension; ns++) {
                ...
                if (gstoich[ns] > 0.0000001) {
                  ...
                  const double modulus = aurostd::modulus(ps_temp);
                  if (modulus < 0.0000001) {
                    ...
                    for (uint a = 1; a < distinctAE_size[ns] + 1; a++) {
                      ...
                      if (ps_temp[a] > 1.0000001) {
                        cout << "ERROR: l=" << l << ", ns=" << ns << ", a=" << a << ", ps_temp[a]=" << ps_temp[a] << endl;
                      }
                    }
                  }

                  double dotProduct_temp = aurostd::scalar_product(ps_temp, VpseudoGS_AEcoefs[ns]) * gstoich[ns];
                  if (std::abs(dotProduct_temp) < 0.0000001) {
                    dotProduct_temp = 0;
                  }
              ...
              if (dotProduct > 1.0000001 || dotProduct < -0.0000001) {
                cout << "ERROR: dotProduct=" << dotProduct << endl;
              }
              if (std::abs(dotProduct) < 0.0000001) {
                dotProduct = 0;
              }
              ...
              const double gfa_tmp = exp(-std::abs(El_ref - Egs[X]) / kBT_ROOM) * weight * (1.0 - dotProduct); // DX20210122 - create variable
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " exp(-abs(El_ref-Egs[X])/kBT)*weight*(1.0-dotProduct): " << gfa_tmp << endl;
              }

              gfa[X] += gfa_tmp;
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " gfa[X]: " << gfa[X] << endl;
              }
              sampling[X] = sampling[X] + weight;

              weight_avgDP.push_back(weight);
              if (weight_avgDP.back() < -0.0000001) {
                cout << "ERROR: weight_avgDP.back() = " << weight_avgDP.back() << ", weight = " << weight << endl;
              }
      ...
      if (gfa[X] > 0.0000001) {
        ...
        for (uint ns = 0; ns < dimension; ns++) {
          ...
          double TDP_ns = 0;
          if (gstoich[ns] > 0.0000001) {
            if (!Vpseudo_AEcoefs[ns].empty()) {
              vector<xvector<double>> distinctPS;
              vector<double> dPS_coefs;
              vector<double> dPS_weights;
              distinctPS.push_back(Vpseudo_AEcoefs[ns][0]);
              dPS_coefs.push_back(0);
              dPS_weights.push_back(weight_avgDP[0]);
              cout << "species: " << species[ns] << ", number of pseudostructures: " << Vpseudo_AEcoefs[ns].size() << endl;
              for (size_t l = 0; l < Vpseudo_AEcoefs[ns].size(); l++) {
                bool equal_pseudo = false;
                for (size_t m = 0; m < distinctPS.size(); m++) {
                  if (std::abs(weight_avgDP[l] - dPS_weights[m]) < 0.0000001) {
                    for (uint a = 1; a < distinctAE_size[ns] + 1; a++) {
                      if (std::abs(Vpseudo_AEcoefs[ns][l][a] - distinctPS[m][a]) > 0.0000001) {
                        break;
                  ...
                  if (equal_pseudo == true) {
                    ...
                    if (dPS_weights.back() < -0.0000001) {
                      cout << "ERROR: l=" << l << ", weight_avgDP[l] = " << weight_avgDP[l] << ", dPS_weights.back() = " << dPS_weights.back() << endl;
                    }
                  }
                }
              }
              ...
              for (size_t d1 = 0; d1 < distinctPS.size() - 1; d1++) {
                for (size_t d2 = d1 + 1; d2 < distinctPS.size(); d2++) {
                  double DP = 0.0;
                  const double d12 = dPS_coefs[d1] * dPS_coefs[d2] * dPS_weights[d1] * dPS_weights[d2];
                  if (dPS_coefs[d1] < 0) {
                    cout << "ERROR: dPS_coefs[d1] = " << dPS_coefs[d1] << endl;
                  } else if (dPS_coefs[d2] < 0) {
                    cout << "ERROR: dPS_coefs[d2] = " << dPS_coefs[d2] << endl;
                  } else if (dPS_weights[d1] < 0) {
                    cout << "ERROR: dPS_weights[d1] = " << dPS_weights[d1] << endl;
                  } else if (dPS_weights[d2] < 0) {
                    cout << "ERROR: dPS_weights[d2] = " << dPS_weights[d2] << endl;
                  }
                  for (uint a = 1; a < distinctAE_size[ns] + 1; a++) {
              ...
              for (size_t d = 0; d < distinctPS.size(); d++) {
    ...
    for (size_t ns = 0; ns < gstoich_max.size(); ns++) {
      cout << gstoich_max[ns] << " ";
    }
    cout << endl;
  }
  ...
  void CalculateGFA(aurostd::xoption& vpflow, const string& alloy, const string& AE_file_read, double fe_cut) {
    ...
    vector<double> sampling;
    vector<double> gfa;
    vector<double> tEForm;
    vector<double> EForm;
    vector<double> Egs;
    vector<string> tAUID;
    vector<string> AUID;
    vector<vector<double>> stoichiometries;
    vector<vector<double>> VDecomp_coefs;
    vector<vector<double>> VStoichiometry;
    vector<vector<double>> VStoichE;
    vector<vector<uint>> VDecomp_phases;
    vector<vector<string>> VSpecies;
    vector<vector<xstructure>> tVStructure;
    vector<vector<xstructure>> tVStructure2;
    vector<vector<xstructure>> VStructure;
    vector<vector<xmatrix<int>>> VatomicEnvironments;
    vector<aflowlib::_aflowlib_entry> LIB_entries;
    //nhull::ConvexHull the_hull;
    vector<nhull::Entry> Vpoint;
    vector<nhull::Entry> vcpt;
    ostream& oss = cout;
    vector<string> species = aurostd::getElements(alloy, pp_string, true, true, false, oss);  // clean and sort, do not keep_pp
    const string input = aurostd::joinWDelimiter(species, ""); // getAlphabeticString(alloy); //CO20190712
    vector<nhull::CoordGroup> VCoordGroup;

    const uint dimension = species.size();
    ...
    for (size_t i = 0; i < LIB_entries.size(); i++) {
      bool equal_E = false;
      for (size_t j = 0; j < LIB_entries.size(); j++) {
        if (j != i && LIB_entries[i].vstoichiometry[0] == LIB_entries[j].vstoichiometry[0] && LIB_entries[i].vspecies[0] == LIB_entries[j].vspecies[0] && std::abs(tEForm[j] - tEForm[i]) < 0.01 &&
            (LIB_entries[i].lattice_system_relax.empty() || LIB_entries[j].lattice_system_relax.empty() || LIB_entries[i].lattice_system_relax == LIB_entries[j].lattice_system_relax)) {
    ...
    for (uint i = 0; i < entries_size; i++) {
      if (VSpecies[i].size() < species.size()) {
        ...
        for (size_t j = 0; j < species.size(); j++) {
          for (size_t k = 0; k < VSpecies[i].size(); k++) {
    ...
    for (uint i = 1; i < entries_size; i++) {
      bool GotIt = false;
      for (size_t j = 0; j < stoichiometries.size(); j++) {
        for (size_t k = 0; k < VStoichiometry[i].size(); k++) {
    ...
    for (uint i = 0; i < entries_size; i++) {
      VStoichE.push_back(VStoichiometry[i]);
      VStoichE[i].push_back(EForm[i]);
      VStoichE[i].push_back(i);
    }
    sort(VStoichE.begin(), VStoichE.end());

    for (size_t i = 0; i < VStoichE.size(); i++) {
      for (size_t j = 0; j < tAUID.size(); j++) {
    ...
    for (uint i = 0; i < entries_size; i++) {
      ...
      if (AE_file_read != "none") {
        ...
        for (size_t ns = 0; ns < species.size(); ns++) {
          Vtokens.clear();
          line_count = 0;
          vector<string> tokens;
          for (size_t j = 0; j < AE_file_read_vstr.size(); j++) {
            line_count++;
            tokens.clear();
            const string AE_line(AE_file_read_vstr[j]);
            stringstream AE_line_sstr(AE_line);
            while (AE_line_sstr >> buffer) {
              tokens.push_back(buffer);
            }
            if (tokens[0] == "AUID" && tokens[2] == AUID[i] && tokens[5] == species[ns]) {
              count = 0;
              for (size_t k = j + 1; k < AE_file_read_vstr.size(); k++) {
                tokens.clear();
                const string AE_line2(AE_file_read_vstr[k]);
                stringstream AE_line2_sstr(AE_line2);
                while (AE_line2_sstr >> buffer) {
                  tokens.push_back(buffer);
                }
                if (tokens[0] == "AUID") {
                  break;
          ...
          if (line_count != AE_file_read_vstr.size()) {
            const int AEmc = Vtokens[0].size();
            xmatrix<int> ae_tmp(1, 1, count, AEmc);
            for (uint k = 0; k < count; k++) {
              for (size_t l = 0; l < Vtokens[k].size(); l++) {
                const uint m = k + 1;
                const uint n = l + 1;
        ...
        if (line_count == AE_file_read_vstr.size()) {
          ...
          str.ReScale(1.0);
          const deque<deque<int>> vertType = CalcAtomicEnvironment(str);
          int nae = 0;
          for (size_t ns = 0; ns < species.size(); ns++) {
            if (VStoichE[i][ns] > 0.0000001) {
              ...
              const xmatrix<int> environmentSet(1, 1, 1, 1);
      ...
      for (size_t ns = 0; ns < species.size(); ns++) {
        AEData << "AUID = " << AUID[i] << " atom = " << species[ns] << endl << VatomicEnvironments[i][ns] << endl;
      }
    }
    ...
    vector<vector<vector<int>>> distinctAE;
    vector<vector<vector<int>>> AEcoefs;
    ...
    for (size_t ns = 0; ns < distinctAE.size(); ns++) {
      cout << "species: " << species[ns] << ", number of distinct atomic environments: " << distinctAE[ns].size() << endl;
    }

    deque<int> sremove;
    for (size_t i = 0; i < str_remove.size(); i++) {
      if (str_remove[i] == 1) {
        sremove.push_front(i);
      }
    }
    ...
    for (size_t ns = 0; ns < species.size(); ns++) {
      ...
      const xmatrix<int> xAEcoefsI = aurostd::vectorvector2xmatrix(AEcoefs[ns]);
    ...
    ofstream EntryData;
    const string Entryfilename = "GFA_entries.dat";
    EntryData.open(Entryfilename.c_str());

    EntryData << endl
              << LIB_entries.size() << " entries loaded. " << endl
              << entries_less_dup << " entries remain after removing duplicates." << endl
              << entries_size << " entries have formation enthalpy < " << fe_cut << " eV." << endl;

    entries_size = VStoichE.size();

    EntryData << entries_size << " entries remain after atomic environments routine." << endl << endl << "stoichiometries and formation enthalpies for all structures used in the GFA calculation:" << endl;

    for (uint i = 0; i < entries_size; i++) {
      for (size_t j = 0; j < VStoichE[i].size() - 1; j++) {
    ...
    if (dimension == 2) {
      ...
      for (double i = 0; i < 101; i++) {
        ...
        const nhull::Entry cpt(vst, cout, true, true, true);
        vcpt.push_back(cpt);
      }
    } else if (dimension == 3) {
    ...
    for (size_t i = 0; i < VStoichE.size(); i++) {
      ...
      const nhull::Entry cpt(vst, cout, true, true, false);
    ...
    XHOST.QUIET = true;
    nhull::ConvexHull the_hull(vpflow, tVpoint, cout, true, true);
    //XHOST.QUIET = quiet;

    VCoordGroup = the_hull.getCoordGroups();
    Vpoint = the_hull.getPoints();

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " [1]" << endl;
    } // CO20190424

    for (size_t i = 0; i < stoichiometries.size(); i++) {
      bool equal = false;
      for (size_t j = 0; j < vcpt.size(); j++) {
        for (int k = vcpt[j].m_coord.lrows; k <= vcpt[j].m_coord.urows - 1; k++) {  // CO20190424
          if (vcpt[j].m_coord[k] != stoichiometries[i][k - vcpt[j].m_coord.lrows]) {  // CO20190424
            break;
          } else if ((k - vcpt[j].m_coord.lrows) == (int) dimension - 2) {  // CO20190424
            equal = true;
          }
        }
        if (equal == true) {
    ...
    VDecomp_coefs.resize(vcpt.size());
    VDecomp_phases.resize(vcpt.size());
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " [2.0]" << endl;
    } // CO20190424
    for (size_t i = 0; i < vcpt.size(); i++) {
      bool on_hull = false;
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " [2.1]" << endl;
      } // CO20190424
      for (size_t j = 0; j < VCoordGroup.size(); j++) {
        for (int k = vcpt[i].m_coord.lrows; k <= vcpt[i].m_coord.urows - 1; k++) {  // CO20190424
          if (std::abs(vcpt[i].m_coord[k] - VCoordGroup[j].m_coord[k]) > 0.00001) {
            break;
          } else if ((k - vcpt[i].m_coord.lrows) == (int) dimension - 2 && VCoordGroup[j].m_is_on_hull) {  // CO20190424
            on_hull = true;
            VDecomp_coefs[i].push_back(1.0);
            VDecomp_phases[i].push_back(VCoordGroup[j].m_ref_state);
      ...
      if (on_hull == false) {
        ...
        vector<xvector<double>> rhs;
        double others = 0;
        xvector<double> compositionL(VDecomp_phases[i].size()); //-1,0);  //CO20190424

        for (size_t j = 0; j < VDecomp_phases[i].size(); j++) {
          if (Vpoint[VDecomp_phases[i][j]].m_is_artificial) {
            VDecomp_phases[i][j] = the_hull.getCoordGroups().at(Vpoint.at(VDecomp_phases[i][j]).m_i_coord_group).m_ref_state;
          }
        }
        int k = compositionL.lrows; // CO20190424
        for (int j = vcpt[i].m_coord.lrows; j <= vcpt[i].m_coord.urows - 1; j++) {  // CO20190424
          if (vcpt[i].m_coord[j] != 0) {
            compositionL[k] = vcpt[i].m_coord[j];
            others = others + vcpt[i].m_coord[j];
            k++;
          }
        }
        if (std::abs(1 - others) > 0.00001) {
          compositionL[compositionL.urows] = 1 - others;
        } // CO20190424
        lhs.push_back(compositionL);

        for (size_t j = 0; j < VDecomp_phases[i].size(); j++) {
          others = 0; /*k=compositionR.lrows;*/
          xvector<double> compositionR(VDecomp_phases[i].size()); //-1,0); //CO20190424
          k = compositionR.lrows; // CO20190424
          for (int l = vcpt[i].m_coord.lrows; l <= vcpt[i].m_coord.urows - 1; l++) {  // CO20190424
            if (vcpt[i].m_coord[l] != 0) {
              compositionR[k] = Vpoint.at(VDecomp_phases[i][j]).m_coord[l];
              others = others + Vpoint.at(VDecomp_phases[i][j]).m_coord[l];
              k++;
            }
          }
          if (std::abs(1 - others) > 0.00001) {
            compositionR[compositionR.urows] = 1 - others;
          }  // CO20190424
          rhs.push_back(compositionR);
        }

        xvector<double> decomp_coeffs = balanceChemicalEquation(lhs, rhs, true, 0.001);
        for (size_t j = 1; j < VDecomp_phases[i].size() + 1; j++) {
    ...
    ComputeGFA(Egs, VDecomp_coefs, sampling, gfa, VDecomp_phases, xAEcoefs, dimension, entries_size, distinctAE_size, vcpt, VStoichE, species);
    ...
    for (size_t i = 0; i < vcpt.size(); i++) {
      double last = 1;
      for (int j = vcpt[i].m_coord.lrows; j <= vcpt[i].m_coord.urows - 1; j++) {  // CO20190424
        GFAData << vcpt[i].m_coord[j] << "  ";
        last = last - vcpt[i].m_coord[j];
      }
