...
namespace compare {
  string compareAtomDecorations(istream& input, const aurostd::xoption& vpflow) {
    ...
    if (vpflow.flag("COMPARE::USAGE")) {
      const string usage = "aflow --compare_atom_decorations|--unique_atom_decorations [GENERAL_COMPARISON_OPTIONS] < file";
      const string options_function_string = "unique_atom_decorations_options: [--print_misfit]";

      vector<string> options;
      vector<string> options_general;
      vector<string> options_function;
    ...
    const uint ncpus_default = 1; // initialize with one cpu; default
    XtalFinderCalculator xtal_finder(DEFAULT_XTALFINDER_MISFIT_MATCH, DEFAULT_XTALFINDER_MISFIT_FAMILY, FileMESSAGE, ncpus_default, oss);
    ...
    if (vpflow.flag("COMPARE_PERMUTATION::PRINT")) {
      print_misfit = true;
    }
    ...
    const vector<string> unique_permutations = xtal_finder.getUniquePermutations(xstr, comparison_options, results_ss, xtal_finder.num_proc, print_misfit);
...
vector<string> XtalFinderCalculator::getUniquePermutations(xstructure& xstr, aurostd::xoption& comparison_options, stringstream& results_ss, uint num_proc, bool print_misfit) {
  ...
  if (!print_misfit) {
    if (!compare::arePermutationsComparableViaComposition(xstr)) { // DX20190624 - put into function
      deque<int> reduced_stoichiometry;
      aurostd::reduceByGCD(xstr.num_each_type, reduced_stoichiometry); // DX20191125
      deque<uint> reduced_stoichiometry_uint;
      for (size_t i = 0; i < reduced_stoichiometry.size(); i++) {
        reduced_stoichiometry_uint.push_back((uint) reduced_stoichiometry[i]);
      } // DX20191125
      ...
      if (format == json_ft) { // DX20190506
        ...
        for (size_t j = 0; j < unique_permutations.size(); j++) {
          vcontent.push_back("[\"" + unique_permutations[j] + "\"]");
        } // DX20210517 - fixed printing error
  ...
  if (format == txt_ft) { // DX20190506
    ...
    for (size_t j = 0; j < structure.atom_decorations_equivalent.size(); j++) {
  ...
  // DX20190506 START
    ...
    for (size_t j = 0; j < structure.atom_decorations_equivalent.size(); j++) {
      ...
      vcontent_json.push_back(sstmp.str());
      sstmp.str("");
    }
...
namespace compare {
  ...
  string compareInputStructures(const aurostd::xoption& vpflow, ostream& logstream) { // DX20190425 - changed name, more general
    ...
    // ---------------------------------------------------------------------------
    // instantiate XtalFinder calculator
    XtalFinderCalculator xtal_finder(DEFAULT_XTALFINDER_MISFIT_MATCH, DEFAULT_XTALFINDER_MISFIT_FAMILY, FileMESSAGE, ncpus_default, logstream);
    ...
    const bool magnetic_comparison = vpflow.flag("COMPARE::MAGNETIC");
    vector<string> magmoms_for_systems;
    if (magnetic_comparison) {
      const string magnetic_info = vpflow.getattachedscheme("COMPARE::MAGNETIC");
    ...
    if (file_list.size() == 2 && !prototypes_final.empty()) {
      ...
      if (prototypes_final[0].mapping_info_duplicate.size() == 1) {
        const double misfit_final = prototypes_final[0].mapping_info_duplicate[0].misfit;
        ...
        if (print) {
          oss << xtal_finder.printStructureMappingResults(prototypes_final[0].mapping_info_duplicate[0], prototypes_final[0].structure_representative->structure, prototypes_final[0].structures_duplicate[0]->structure);
        }
      } else {
        message << "UNMATCHABLE" << endl;
        if (XHOST.QUIET) {
    ...
    // write results to files //DX20201229 - consolidated into functions
      ...
      if (write_json) {
        xtal_finder.writeComparisonOutputFile(results_json, directory, json_ft, compare_input_xf, same_species);
      }
      if (write_txt) {
        xtal_finder.writeComparisonOutputFile(results_txt, directory, txt_ft, compare_input_xf, same_species);
      }
    }
...
namespace compare {
  string isopointalPrototypes(istream& input, const aurostd::xoption& vpflow) {
    ...
    if (vpflow.flag("ISOPOINTAL_PROTOTYPES::USAGE")) {
      const string usage = "aflow --get_isopointal_prototypes|--isopointal_prototypes|--get_same_symmetry_prototypes < file";
      const string options_function_string = "options: [--catalog=aflow|htqc|all]";
      vector<string> options;
      vector<string> options_function;
      aurostd::string2tokens(options_function_string, options_function, " ");
      options.push_back(usage);
    ...
    const std::set<string> isopointal_prototypes = compare::getIsopointalPrototypes(xstr, catalog);

    // ---------------------------------------------------------------------------
    // print format //DX20210208
    bool write_txt = XHOST.vflag_control.flag("PRINT_MODE::TXT");
    const bool write_json = XHOST.vflag_control.flag("PRINT_MODE::JSON");
    ...
    if (write_txt) {
      if (isopointal_prototypes.empty()) {
        ss_output << "no isopointal prototypes in AFLOW";
      } else {
        ss_output << aurostd::joinWDelimiter(isopointal_prototypes, ",");
      }
    }
    if (write_json) {
      if (!ss_output.str().empty()) {
        ss_output << endl;
      } // if printing both text and json, add a newline
      const aurostd::JSON::object jo(aurostd::JSON::object_types::DICTIONARY);
      ss_output << jo.toString();
    }
...
// ***************************************************************************
namespace compare {
  std::set<string> getIsopointalPrototypes(xstructure& xstr, const string& catalog) {
    // ---------------------------------------------------------------------------
    // stoichiometry
    const vector<uint> stoichiometry = xstr.GetReducedComposition(true);
    ...
    std::set<string> vuid = aflowlib::GetPrototypesBySymmetry(stoichiometry, xstr.space_group_ITC, grouped_Wyckoff_positions, SG_SETTING_ANRL, catalog);
...
namespace compare {
  vector<string> getMatchingPrototypes(xstructure& xstr, const string& catalog) {
    const aurostd::xoption vpflow_input;
...
namespace compare {
  vector<string> getMatchingPrototypes(xstructure& xstr, const aurostd::xoption& vpflow_input, const string& catalog) { // DX20210421 - added vpflow variant
    ...
    // ---------------------------------------------------------------------------
    // quiet output
    const bool original_quiet = XHOST.QUIET;
    XHOST.QUIET = true;
    ...
    } // DX20210421 - protect against no matching entries

    // ---------------------------------------------------------------------------
    // sort, put best matches first
    vector<double> misfit_matched;
    uint placement_index = prototypes[0].structures_duplicate.size();
    for (size_t i = 0; i < prototypes[0].structures_duplicate.size(); i++) {
      for (size_t j = 0; j < misfit_matched.size(); j++) {
        if (prototypes[0].mapping_info_duplicate[i].misfit < misfit_matched[j]) {
          placement_index = 0;
        }
      }
...
namespace compare {
  string printMatchingPrototypes(istream& input, const aurostd::xoption& vpflow) {
    ...
    // FLAG: usage
    if (vpflow.flag("COMPARE::USAGE")) {
      const string usage = "aflow --compare2prototypes|--compare2protos [GENERAL_COMPARISON_OPTIONS] [COMPARE2PROTOTYPES_OPTIONS] < file";
      const string options_function_string = "compare2protos_options: [--catalog=aflow|htqc|all]";
...
string XtalFinderCalculator::printMatchingPrototypes(xstructure& xstr, const aurostd::xoption& vpflow) {
  const vector<StructurePrototype> prototypes = compare2prototypes(xstr, vpflow);

  // ---------------------------------------------------------------------------
  // print results (use XHOST flags since only checking once)
  const bool same_species = false; // default for prototypes
  if (XHOST.vflag_control.flag("PRINT_MODE::JSON")) {
    return printResults(prototypes, same_species, json_ft);
...
// ***************************************************************************
vector<StructurePrototype> XtalFinderCalculator::compare2prototypes(const xstructure& xstrIN, const aurostd::xoption& vpflow) {
  const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_COMPARE_);
  stringstream message;
  const bool quiet = false;

  const xstructure xstr = xstrIN; // DX20200226 - copy
  ...
  // ---------------------------------------------------------------------------
  // same spaces
  const bool same_species = false; // compare2prototype: by definition, want to compare backbone structure, i.e., ignore species
  ...
  if (LDEBUG) {
    ...
    for (size_t i = 0; i < structure_containers[0].grouped_Wyckoff_positions.size(); i++) {
  ...
  const uint space_group_num = structure_containers[0].space_group;
  const vector<GroupedWyckoffPosition> grouped_Wyckoff_positions = structure_containers[0].grouped_Wyckoff_positions;
  ...
  if (comparison_options.flag("COMPARISON_OPTIONS::IGNORE_SYMMETRY")) {
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    vuid = aflowlib::GetPrototypesByStoichiometry(stoichiometry, catalog);
  }
  // find prototypes based on stoichiometry and space group only
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    const vector<GroupedWyckoffPosition> empty_Wyckoff_positions;
    vuid = aflowlib::GetPrototypesBySymmetry(stoichiometry, space_group_num, empty_Wyckoff_positions, SG_SETTING_ANRL, catalog);
  }
  // find prototypes based on stoichiometry, space group, and Wyckoff positions only (recommended/default)
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    vuid = aflowlib::GetPrototypesBySymmetry(stoichiometry, space_group_num, grouped_Wyckoff_positions, SG_SETTING_ANRL, catalog);
  }

  if (!vuid.empty()) {
    message << "Potential compatible prototypes: " << vuid.size() << " (" << aurostd::joinWDelimiter(vuid, ",") << ").";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
  } else {
  ...
  const uint nthreads_original = AFLOW_PTHREADS::MAX_PTHREADS;
  AFLOW_PTHREADS::MAX_PTHREADS = 1;

  addAFLOWPrototypes2container(vuid);
  ...
  if (prototypes_final.empty()) {
    return prototypes_final;
  } // DX20190314 originally : return oss.str()
  ...
  if (comparison_options.flag("COMPARISON_OPTIONS::CALCULATE_UNIQUE_PERMUTATIONS")) {
    ...
    for (size_t i = 0; i < prototypes_final.size(); i++) {
      ...
      if (!prototypes_final[i].structure_representative->is_structure_generated) {
        if (!compare::generateStructure(prototypes_final[i].structure_representative->name, prototypes_final[i].structure_representative->source, prototypes_final[i].structure_representative->relaxation_step,
                                        prototypes_final[i].structure_representative->structure, *p_oss)) { // DX20200429
      ...
      //   for(size_t d=0;d<final_permutations[j].structures_duplicate.size();d++){ tmp_permutations.push_back(final_permutations[j].structures_duplicate[d]->name); } //push back equivalent permutations
...
namespace compare {
  // load input structure
  bool isMatchingStructureInDatabase(const xstructure& xstrIN, const aurostd::xoption& vpflow, ostream& logstream) {
    ofstream FileMESSAGE;
    return isMatchingStructureInDatabase(xstrIN, vpflow, FileMESSAGE, logstream);
  }
  bool isMatchingStructureInDatabase(const xstructure& xstrIN, const aurostd::xoption& vpflow, ofstream& FileMESSAGE, ostream& logstream) {
    ...
    return (!prototypes_final[0].structures_duplicate.empty());
  }
} // namespace compare
...
namespace compare {
  // load input structure
  vector<matching_structure> matchingStructuresInDatabase(const xstructure& xstrIN, const aurostd::xoption& vpflow, ostream& logstream) {
    ofstream FileMESSAGE;
    return matchingStructuresInDatabase(xstrIN, vpflow, FileMESSAGE, logstream);
  }
  vector<matching_structure> matchingStructuresInDatabase(const xstructure& xstrIN, const aurostd::xoption& vpflow, ofstream& FileMESSAGE, ostream& logstream) {
    ...
    if (prototypes_final[0].structures_duplicate.empty()) { // DX20210122 - "==" not "!="
      return matched_database_structures;
    }

    // ---------------------------------------------------------------------------
    // return equivalent structures to input
    for (size_t i = 0; i < prototypes_final[0].structures_duplicate.size(); i++) {
...
// ***************************************************************************
// load input structure
vector<StructurePrototype> XtalFinderCalculator::compare2database(const xstructure& xstrIN, const aurostd::xoption& vpflow) {
  const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_COMPARE_);
  stringstream message;
  ...
  if (vpflow.flag("COMPARE2DATABASE::RELAXATION_STEP")) {
    if (aurostd::tolower(vpflow.getattachedscheme("COMPARE2DATABASE::RELAXATION_STEP")).find("orig") != std::string::npos || vpflow.getattachedscheme("COMPARE2DATABASE::RELAXATION_STEP") == "0") {
      relaxation_step = _COMPARE_DATABASE_GEOMETRY_ORIGINAL_;
               ...
               aurostd::tolower(vpflow.getattachedscheme("COMPARE2DATABASE::RELAXATION_STEP")).find("middle_relax") != std::string::npos || vpflow.getattachedscheme("COMPARE2DATABASE::RELAXATION_STEP") == "1") {
  ...
  if (!property_list.empty() && relaxation_step != _COMPARE_DATABASE_GEOMETRY_MOST_RELAXED_) {
    string relaxation_name;
    if (relaxation_step == _COMPARE_DATABASE_GEOMETRY_ORIGINAL_) {
      relaxation_name = "original";
    } else if (relaxation_step == _COMPARE_DATABASE_GEOMETRY_RELAX1_) {
      relaxation_name = "relax1";
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unexpected relaxation step input: " + aurostd::utype2string<uint>(relaxation_step) + ".", _INPUT_ERROR_);
    }
  ...
  string schema_unit;
  string schema_type;
  for (size_t p = 0; p < property_list.size(); p++) {
  ...
  if (vpflow.flag("COMPARE2DATABASE::CATALOG")) {
    const string catalog = aurostd::toupper(vpflow.getattachedscheme("COMPARE2DATABASE::CATALOG")); // DX20210615 - older versions of aflux require uppercase; safety
    if (catalog != "ALL") {
  ...
  const bool ignore_symmetry = comparison_options.flag("COMPARISON_OPTIONS::IGNORE_SYMMETRY");
  string sg_matchbook;
  if (!ignore_symmetry) {
    if (!isSymmetryCalculated(structure_containers[0])) {
      calculateSymmetries(1);
    } // 1: one structure -> one processor
  ...
  if ((sg_matchbook.find("spacegroup_relax") == string::npos) && !aurostd::WithinList(property_list, "spacegroup_relax")) {
    vmatchbook.emplace_back("$spacegroup_relax");
  }
  if (!aurostd::WithinList(property_list, "Pearson_symbol_relax")) {
    vmatchbook.emplace_back("$Pearson_symbol_relax");
  }
  ...
  vmatchbook.emplace_back("format(aflow)");
  // ME20220420 - Use paged requests to avoid timeouts
  vmatchbook.emplace_back("");
  uint page = 1;
  const string page_size_str = vpflow.getattachedscheme("COMPARE::PAGE_SIZE");
  const uint page_size = page_size_str.empty() ? DEFAULT_COMPARE_AFLUX_PAGE_SIZE : aurostd::string2utype<uint>(page_size_str);
  string response;
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " AFLUX response:" << endl << response << endl;
  }
  ...
  if (LDEBUG) {
    for (size_t i = 0; i < properties_response.size(); i++) {
      for (size_t j = 0; j < properties_response[i].size(); j++) {
  ...
  for (size_t i = 0; i < auids.size(); i++) {
    ...
    const vector<string> species = aurostd::getElements(compounds[i], vcomposition);
    if (LDEBUG) {
    ...
    if (j != vcomposition.size()) {
      continue;
    }
    vector<uint> tmp_reduced_stoich;
    aurostd::reduceByGCD(tmp_stoich, tmp_reduced_stoich); // DX20191125
    if (!same_species) {
    ...
    if (compare::sameStoichiometry(structure_containers[0].stoichiometry, tmp_reduced_stoich)) {
      ...
      for (size_t l = 0; l < properties_response[i].size(); l++) {
        bool property_requested = false;
        for (size_t m = 0; m < property_list.size(); m++) {
          if (properties_response[i][l].first == property_list[m]) {
...
namespace compare {
  ...
  string printCompare2Database(istream& input, const aurostd::xoption& vpflow, ostream& logstream) {
    ...
    // FLAG: usage
    if (vpflow.flag("COMPARE::USAGE")) {
      const string usage = "aflow --compare2database [GENERAL_COMPARISON_OPTIONS] [COMPARE2DATABASE_OPTIONS] < file";
      const string options_function_string = "compare2database_options: [--catalog=lib1|lib2|lib3|lib4|lib6|lib7|icsd] [--properties=enthalpy_atom,natoms,...] [--relaxation_step=original|relax1|most_relaxed]";
...
namespace compare {
  string printCompare2Database(const xstructure& xstrIN, const aurostd::xoption& vpflow, ofstream& FileMESSAGE, ostream& logstream) {
    ...
    // write results to files //DX20201229 - consolidated into functions
      ...
      const string directory = aurostd::getPWD();
      if (write_json) {
        xtal_finder_database.writeComparisonOutputFile(results_json, directory, json_ft, compare2database_xf, same_species);
      }
      if (write_txt) {
        xtal_finder_database.writeComparisonOutputFile(results_txt, directory, txt_ft, compare2database_xf, same_species);
      }
    }
...
namespace compare {
  ...
  string compareDatabaseEntries(const aurostd::xoption& vpflow, ofstream& FileMESSAGE, ostream& logstream) { // DX20191125 - added ofstream and ostream
    ...
    const string directory = aurostd::getPWD(); // DX20201229
    stringstream message;
    ...
    // FLAG: usage
    if (vpflow.flag("COMPARE::USAGE")) {
      const string usage = "aflow --compare_database_entries [GENERAL_COMPARISON_OPTIONS] [COMPARE_DATABASE_ENTRIES_OPTIONS] < file";
      const string options_function_string =
          "compare_database_entries_options: [--alloy=AgAlMn...] [--nspecies=3] [--catalog=lib1|lib2|lib3|lib4|lib6|lib7|icsd] [--properties=enthalpy_atom,natoms,...] "
          "[--relaxation_step=original|relax1|most_relaxed] [--space_group=225,186,227,...] [--stoichiometry=1,2,3,...]";
    ...
    if (catalog.empty() || catalog == "icsd" || catalog == "all") { // DX20191108 - needs to be outside of loop
    ...
    if (vpflow.flag("COMPARE_DATABASE_ENTRIES::ARITY")) {
      ...
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, FileMESSAGE, logstream, _LOGGER_MESSAGE_);
      if (arity != 0) {
        vmatchbook.push_back("nspecies(" + aurostd::utype2string<uint>(arity) + ")");
      }
      // ME20220419 - Add MUTE operator to reduce response size
      if (!aurostd::WithinList(property_list, "nspecies")) {
        vmatchbook.back() = "$" + vmatchbook.back();
    ...
    vector<uint> stoichiometry_reduced;
    vector<uint> stoichiometry_input;
    if (vpflow.flag("COMPARE_DATABASE_ENTRIES::STOICHIOMETRY")) {
      ...
      for (size_t i = 0; i < stoichiometry_vstring.size(); i++) {
        stoichiometry_input.push_back(aurostd::string2utype<uint>(stoichiometry_vstring[i]));
      }
      ...
      if (!same_species) {
        std::sort(stoichiometry_reduced.begin(), stoichiometry_reduced.end());
      }
    ...
    if (vpflow.flag("COMPARE_DATABASE_ENTRIES::ALLOY")) {
      const string alloy_string = vpflow.getattachedscheme("COMPARE_DATABASE_ENTRIES::ALLOY");
      // split by comma
    ...
    const string sg_matchbook = xtal_finder.getSpaceGroupMatchbookFromOptions(vpflow, relaxation_step);
    ...
    for (size_t i = 0; i < auids.size(); i++) {
      ...
      // note: do not include in AFLUX matchbook, we would need to specify a range of compatible stoichs (could be expensive)
      // instead: filter on stoichiometry after recieving AFLUX response
      if (stoichiometry_reduced.empty() || compare::sameStoichiometry(stoichiometry_reduced, tmp_reduced_stoich)) {
        // ---------------------------------------------------------------------------
        // load structures from aflowlib_entry
        try {
          aflowlib::_aflowlib_entry entry;
    ...
    const vector<StructurePrototype> prototypes_final = xtal_finder.compareMultipleStructures(xtal_finder.num_proc, same_species, directory, comparison_options); // DX20200103 - condensed booleans to xoptions
    ...
    // write results to files //DX20201229 - consolidated into functions
      ...
      if (write_txt) {
        xtal_finder.writeComparisonOutputFile(results_txt, directory, txt_ft, compare_database_entries_xf, same_species);
      }
      if (write_json) {
        xtal_finder.writeComparisonOutputFile(results_json, directory, json_ft, compare_database_entries_xf, same_species);
      }
    }
...
// ***************************************************************************
vector<StructurePrototype> XtalFinderCalculator::compareStructuresFromStructureList(const vector<string>& filenames, vector<string>& magmoms_for_systems, uint num_proc, bool same_species, const aurostd::xoption& comparison_options) {
  // ---------------------------------------------------------------------------
  ...
  loadStructuresFromStructureList(filenames, magmoms_for_systems, same_species);
...
vector<StructurePrototype> XtalFinderCalculator::compareStructuresFromDirectory(const string& directory, vector<string>& magmoms_for_systems, uint num_proc, bool same_species, const aurostd::xoption& comparison_options) {
  ...
  loadStructuresFromDirectory(directory, magmoms_for_systems, same_species);
...
// ***************************************************************************
vector<StructurePrototype> XtalFinderCalculator::compareStructuresFromFile(const string& filename, vector<string>& magmoms_for_systems, uint num_proc, bool same_species, const aurostd::xoption& comparison_options) {
  // ---------------------------------------------------------------------------
  // directory to write results
  const string directory = aurostd::getPWD();
  ...
  loadStructuresFromFile(filename, magmoms_for_systems, same_species);
...
// ***************************************************************************
vector<StructurePrototype> XtalFinderCalculator::compareStructuresFromString(const string& structures_string, vector<string>& magmoms_for_systems, uint num_proc, bool same_species, const aurostd::xoption& comparison_options) {
  // ---------------------------------------------------------------------------
  // directory to write results
  const string directory = aurostd::getPWD();
  ...
  loadStructuresFromStringstream(structures, magmoms_for_systems, same_species);
...
namespace compare {
  vector<aflowlib::_aflowlib_entry> getUniqueEntries(vector<aflowlib::_aflowlib_entry>& entries, ostream& oss, ofstream& FileMESSAGE, uint num_proc, bool same_species, bool scale_volume, bool optimize_match) {
    // ---------------------------------------------------------------------------
    // instantiate XtalFinder calculator
    XtalFinderCalculator xtal_finder(DEFAULT_XTALFINDER_MISFIT_MATCH, DEFAULT_XTALFINDER_MISFIT_FAMILY, FileMESSAGE, num_proc, oss);

    // ---------------------------------------------------------------------------
    // load structures from aflowlib entries
    const vector<string> magmoms_for_systems; // DX20201111 - not included for now
    xtal_finder.loadStructuresFromAflowlibEntries(entries, magmoms_for_systems, same_species);
    ...
    // ---------------------------------------------------------------------------
    // compare structures returns vector<StructureProtoype> of unique/duplicate info
    vector<StructurePrototype> grouped_structures = xtal_finder.compareMultipleStructures(xtal_finder.num_proc, same_species, directory, comparison_options);

    // ---------------------------------------------------------------------------
    // filter out duplicate aflowlib entries
    vector<aflowlib::_aflowlib_entry> entries_unique;
    for (size_t i = 0; i < grouped_structures.size(); i++) {
...
vector<vector<uint>> XtalFinderCalculator::groupSimilarXstructures(const vector<xstructure>& vxstrs, aurostd::xoption& comparison_options, bool same_species) {
  // Compares a vector of xstructures and groups the indices of the
  // xstructures by structural similarity

  const uint nstrs = vxstrs.size();

  stringstream structure_name;
  const string source = "input";
  const uint relaxation_step = 0; // unknowable from input
  const string directory = aurostd::getPWD();
  ...
  // ---------------------------------------------------------------------------
  // compare structures returns vector<StructureProtoype> of unique/duplicate info
  vector<StructurePrototype> grouped_structures = compareMultipleStructures(num_proc, same_species, directory,
  ...
  for (size_t i = 0; i < grouped_structures.size(); i++) {
    ...
    for (size_t j = 0; j < grouped_structures[i].structures_duplicate.size(); j++) {
...
vector<vector<uint>> XtalFinderCalculator::groupSimilarXstructures(const vector<xstructure>& vxstrs, bool same_species) {
  aurostd::xoption comparison_options = compare::loadDefaultComparisonOptions();
...
// ***************************************************************************
namespace compare {
  bool structuresMatch(const xstructure& xstr1, const xstructure& xstr2, bool same_species, uint num_proc) {
...
namespace compare {
  bool structuresMatch(const xstructure& xstr1, const xstructure& xstr2, bool same_species, bool scale_volume, bool optimize_match, uint num_proc) {
    double misfit_final = AUROSTD_MAX_DOUBLE;
...
// ***************************************************************************
namespace compare {
  double getMisfitBetweenStructures(const xstructure& xstr1, const xstructure& xstr2, bool same_species, uint num_proc) {
    double misfit_final = AUROSTD_MAX_DOUBLE;
    const bool scale_volume = true; // default is true
    const bool optimize_match = false; // default is false
...
// ***************************************************************************
namespace compare {
  bool aflowCompareStructure(const xstructure& xstr1, const xstructure& xstr2, bool same_species, bool scale_volume, bool optimize_match, double& misfit_final, uint num_proc) {
