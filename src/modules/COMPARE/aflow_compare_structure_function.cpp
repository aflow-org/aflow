...
// ***************************************************************************
void XtalFinderCalculator::compareStructures(structure_container& str_rep, structure_container& str_matched, structure_mapping_info& match_info, bool same_species, bool scale_volume, bool optimize_match) {
  ...
  if (str_rep.structure.dist_nn_min == AUROSTD_NAN) {
    if (!str_rep.nearest_neighbor_distances.empty()) {
      str_rep.structure.dist_nn_min = aurostd::min(str_rep.nearest_neighbor_distances);
    } else {
      str_rep.structure.dist_nn_min = SYM::minimumDistance(str_rep.structure);
    }
  }
  if (str_matched.structure.dist_nn_min == AUROSTD_NAN) {
    if (!str_matched.nearest_neighbor_distances.empty()) {
      str_matched.structure.dist_nn_min = aurostd::min(str_matched.nearest_neighbor_distances);
    } else {
      str_matched.structure.dist_nn_min = SYM::minimumDistance(str_matched.structure);
    }
  }

  if (compare::matchableSpecies(str_rep.structure, str_matched.structure, same_species)) {
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Searching for new representation of test structure ..." << endl;
    }
...
// ***************************************************************************
vector<StructurePrototype> XtalFinderCalculator::compareMultipleStructures(uint num_proc, bool same_species, const string& directory) {
  // ---------------------------------------------------------------------------
...
vector<StructurePrototype> XtalFinderCalculator::compareMultipleStructures(uint num_proc, bool same_species, const string& directory, const aurostd::xoption& comparison_options) {
  const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_COMPARE_);
  ...
  if (comparison_options.flag("COMPARISON_OPTIONS::PRIMITIVIZE") || comparison_options.flag("COMPARISON_OPTIONS::MINKOWSKI") || comparison_options.flag("COMPARISON_OPTIONS::NIGGLI")) {
  ...
  for (size_t i = 0; i < structure_containers.size(); i++) {
    all_symmetries_calculated = (all_symmetries_calculated && isSymmetryCalculated(structure_containers[i]));
  } // DX20200810
  ...
  for (size_t i = 0; i < structure_containers.size(); i++) {
    all_environments_calculated = (all_environments_calculated && isLFAEnvironmentCalculated(structure_containers[i]));
  } // DX20200810
  ...
  for (size_t i = 0; i < structure_containers.size(); i++) {
    all_neighbors_calculated = (all_neighbors_calculated && (!structure_containers[i].nearest_neighbor_distances.empty()));
  } // DX20200925 - gcc-10 warnings
  ...
  if (!same_species && comparison_options.flag("COMPARISON_OPTIONS::CALCULATE_UNIQUE_PERMUTATIONS")) {
    ...
    for (size_t i = 0; i < prototypes_final.size(); i++) {
  ...
  const bool store_checkpoint = false;
  if (store_checkpoint) {
    const string results_json = printResults(prototypes_final, same_species, json_ft);
    writeComparisonOutputFile(results_json, directory, json_ft, compare_input_xf, same_species);
    const string results_txt = printResults(prototypes_final, same_species, txt_ft);
...
// ***************************************************************************
void XtalFinderCalculator::getOptions(const aurostd::xoption& vpflow, aurostd::xoption& comparison_options) {
  ...
  if (misfit_match > misfit_family) {
    message << "Matching misfit threshold must be less than the same family threshold:"
            << " misfit match threshold: " << misfit_match << " misfit family threshold: " << misfit_family;
...
// ***************************************************************************
string XtalFinderCalculator::getSpaceGroupMatchbookFromOptions(const aurostd::xoption& vpflow,
  ...
  if (vpflow.flag("COMPARE_DATABASE_ENTRIES::SPACE_GROUP")) {
    const string space_group_input = vpflow.getattachedscheme("COMPARE_DATABASE_ENTRIES::SPACE_GROUP");
    ...
    uint sg_tmp = 0;
    for (size_t i = 0; i < vspace_group_strings.size(); i++) {
...
namespace compare {
  structure_container initializeStructureContainer(const xstructure& structure, bool same_species) {
    ...
    if (!str_rep.structure.atoms.empty() && str_rep.structure.atoms[0].name.empty()) {
      str_rep.structure.DecorateWithFakeElements();
    } // GetElements does not change xstructure //SD20220221
    ...
    if (str_rep.structure.species.empty()) {
      deque<string> deque_species;
      for (size_t j = 0; j < str_rep.elements.size(); j++) {
        deque_species.push_back(str_rep.elements[j]);
      }
    ...
    const vector<double> zero_vec_double;
...
void StructurePrototype::free() {
  ...
  structure_representative = nullptr; // DX20201204
...
ostream& operator<<(ostream& oss, const StructurePrototype& StructurePrototype) {
  ...
  if (StructurePrototype.iomode == JSON_MODE) {
    ...
    if (!StructurePrototype.atom_decorations_equivalent.empty()) { // DX20190425 - only print if calculated
      sscontent_json << "\"atom_decorations_equivalent\":[";
      tmp_vstring.clear();
      for (size_t i = 0; i < StructurePrototype.atom_decorations_equivalent.size(); i++) {
    ...
    tmp_vstring.clear();
    for (size_t i = 0; i < StructurePrototype.grouped_Wyckoff_positions.size(); i++) {
      stringstream ss_tmp;
    ...
    if (!StructurePrototype.aflow_label.empty()) {
      // aflow_label
      sscontent_json << R"("aflow_prototype_label":")" << StructurePrototype.aflow_label << "\"" << eendl;
      vcontent_json.push_back(sscontent_json.str());
      sscontent_json.str("");
    ...
    if (!StructurePrototype.matching_aflow_prototypes.empty()) {
    ...
    tmp_vstring.clear();
    for (size_t i = 0; i < StructurePrototype.environments_LFA.size(); i++) {
      stringstream ss_tmp;
    ...
    if (!StructurePrototype.property_names.empty()) { // DX20190425 - only print if requested
      // property_names
...
// ***************************************************************************
uint XtalFinderCalculator::numberOfDuplicates(const StructurePrototype& prototype) const {
  // Count the number of duplicate structures

  uint number_of_duplicates = 0;
  for (size_t i = 0; i < prototype.mapping_info_duplicate.size(); i++) {
    if (prototype.mapping_info_duplicate[i].misfit <= misfit_match && (prototype.mapping_info_duplicate[i].misfit + 1.0) > 1e-3) {
...
// ***************************************************************************
string StructurePrototype::printPropertiesOfStructure(structure_container* str_pointer) const {
  ...
  if (!str_pointer->properties.empty()) {
    for (uint i = 0; i < str_pointer->properties.size(); i++) {
      if (str_pointer->properties_types[i] == "number") {
        json[str_pointer->properties_names[i]] = str_pointer->properties[i];
        ...
        json[str_pointer->properties_names[i]] = tokens;
      } else {
        json[str_pointer->properties_names[i]] = str_pointer->properties[i];
      }  // when all else fails: treat as string
    }
  }
  const string output = json.toString();
...
// ***************************************************************************
string StructurePrototype::printStructureTransformationInformation(const structure_mapping_info& misfit_info) const {
  ...
  json["basis_transformation"] = misfit_info.basis_transformation;
  // rotation
  json["rotation"] = misfit_info.rotation;
  // origin_shift
  json["origin_shift"] = misfit_info.origin_shift;
  // atom map
  json["atom_map"] = misfit_info.atom_map;
  // basis map
  json["basis_map"] = misfit_info.basis_map;
  const string output = json.toString();
  return output.substr(1, output.size() - 2);
}
...
string StructurePrototype::printRepresentativeStructure() const {
  ...
  if (!structure_representative->properties.empty()) {
...
string StructurePrototype::printMatchedStructures(matched_structure_type_xtalfinder mode) const {
  ...
  stringstream sscontent_json;
  vector<string> vcontent_json;
  vector<string> vstructures;

  const bool print_transformation = true;

  // print duplicate structure information
  if (mode == duplicate_structure_xf) {
    for (size_t j = 0; j < structures_duplicate.size(); j++) {
      ...
      vcontent_json.push_back(sscontent_json.str());
      sscontent_json.str("");
      if (!structures_duplicate[j]->properties.empty()) {
  ...
  // print same family structure information
    ...
    for (size_t j = 0; j < structures_family.size(); j++) {
      ...
      vcontent_json.push_back(sscontent_json.str());
      sscontent_json.str("");
      if (!structures_family[j]->properties.empty()) {
...
// ***************************************************************************
uint StructurePrototype::numberOfComparisons() const {
...
// ***************************************************************************
bool StructurePrototype::isSymmetryCalculated() const {
...
// ***************************************************************************
bool StructurePrototype::isLFAEnvironmentCalculated() const {
  // Check if the LFA environment is calculated for the prototype

  return (!environments_LFA.empty());
}

// ***************************************************************************
// XtalFinderCalculator::isLFAEnvironmentCalculated()
// ***************************************************************************
bool XtalFinderCalculator::isLFAEnvironmentCalculated(structure_container& structure) {
  // Check if the LFA environment is calculated for a particular
  // structure in the container

  return (!structure.environments_LFA.empty());
}

// ***************************************************************************
// XtalFinderCalculator::areNearestNeighborsCalculated()
// ***************************************************************************
bool XtalFinderCalculator::areNearestNeighborsCalculated(structure_container& structure) {
  // Check if the nearest neighbor distances have been calculated for
  // a particular structure in the container

  return (!structure.nearest_neighbor_distances.empty());
}
...
bool StructurePrototype::calculateSymmetry() {
  ...
  if (!structure_representative->structure.sym_eps_calculated || structure_representative->structure.sym_eps == AUROSTD_NAN) {
    structure_representative->structure.sym_eps = SYM::defaultTolerance(structure_representative->structure);
  } // DX20210421
...
void XtalFinderCalculator::calculateSymmetry(structure_container& str_rep) {
  ...
  bool no_scan = false; // DX20191230
  // use sym_eps if given, otherwise use default
  if (!str_rep.structure.sym_eps_calculated || str_rep.structure.sym_eps == AUROSTD_NAN) {
    str_rep.structure.sym_eps = SYM::defaultTolerance(str_rep.structure);
  } // DX20210421
...
// ***************************************************************************
void XtalFinderCalculator::addStructure2container(const xstructure& xstr, const string& structure_name, const string& source, uint relaxation_step, bool same_species) {
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " loaded " << structure_name << " structure." << endl;
  }
...
// ***************************************************************************
void XtalFinderCalculator::removeStructureFromContainerByName(const string& structure_name) {
...
// ***************************************************************************
void XtalFinderCalculator::setStructureAsRepresentative(StructurePrototype& structure_tmp, uint container_index) {
  setStructureAsRepresentative(structure_tmp, &structure_containers[container_index]);
}

void XtalFinderCalculator::setStructureAsRepresentative(StructurePrototype& structure_tmp, structure_container* str_pointer) {
...
// index input
void XtalFinderCalculator::addStructure2duplicatesList(StructurePrototype& structure_tmp, uint container_index) {
  addStructure2duplicatesList(structure_tmp, &structure_containers[container_index]);
}

// structure_container pointer input
void XtalFinderCalculator::addStructure2duplicatesList(StructurePrototype& structure_tmp, structure_container* str_pointer) {
  // check pointer is not null
  if (str_pointer == nullptr) {
    stringstream message;
    message << "Input is a null pointer.";
  ...
  // initialize
  const structure_mapping_info temp_misfit_info = compare::initialize_misfit_struct();
  structure_tmp.mapping_info_duplicate.push_back(temp_misfit_info);

  // update properties
  if (!structure_tmp.structures_duplicate.back()->properties.empty()) {
    for (size_t i = 0; i < structure_tmp.structures_duplicate.back()->properties_names.size(); i++) {
...
void XtalFinderCalculator::addStructure2sameFamilyList(StructurePrototype& structure_tmp, uint index) {
  // Index corresponds to element in StructurePrototype object.

  // check pointer is not null
  if (structure_tmp.structures_duplicate[index] == nullptr) {
    stringstream message;
...
void XtalFinderCalculator::addStructure2sameFamilyList(StructurePrototype& structure_tmp, structure_container* str_pointer) {
...
void StructurePrototype::copyDuplicate(const StructurePrototype& b, uint index,
  ...
  if (!copy_misfit) {
    const structure_mapping_info temp_misfit_info = compare::initialize_misfit_struct(); // DX20191217
...
ostream& operator<<(ostream& oss, const XtalFinderCalculator& xtalfinder) {
  ...
  for (size_t i = 0; i < xtalfinder.structure_containers.size(); i++) {
    oss << "  " << xtalfinder.structure_containers[i].name << " (compound=" << xtalfinder.structure_containers[i].compound << ", SG=" << xtalfinder.structure_containers[i].space_group << ")" << endl;
  }
  return oss;
}
...
// ***************************************************************************
void XtalFinderCalculator::loadStructuresFromDirectory(const string& directory, const vector<string>& magmoms_for_systems, bool same_species) {
  ...
  const string source = "file"; // all structures from file in the directory
  const uint relaxation_step = 0; // assume unrelaxed; impossible to know from this input-type
  ...
  for (size_t i = 0; i < vfiles.size(); i++) {
    if (LDEBUG) {
      cerr << "compare:: " << i << "/" << vfiles.size() << " " << vfiles[i] << endl;
    }
    if (vfiles[i].find("comparison_output.json") != std::string::npos || vfiles[i].find("comparison_output.out") != std::string::npos ||
        vfiles[i].find("comparison_no_duplicate_compounds_output.json") != std::string::npos || vfiles[i].find("comparison_no_duplicate_compounds_output.out") != std::string::npos ||
        vfiles[i].find("duplicate_compounds_output.json") != std::string::npos || vfiles[i].find("duplicate_compounds_output.out") != std::string::npos || vfiles[i].find("nohup.out") != std::string::npos) {
      ...
      xstructure xstr;
      if (!compare::generateStructure(structure_name, source, 0, xstr, *p_oss)) {
        continue;
      }
      ...
      if (magmoms_for_systems.size() == vfiles.size()) {
        try {
          pflow::ProcessAndAddSpinToXstructure(xstr, magmoms_for_systems[i]);
        } // DX20190801
        catch (aurostd::xerror& excpt) {
          message << "Magnetic information could not be loaded (" << magmoms_for_systems[i] << ".";
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ERROR_);
        } // DX20190801
      }
...
// ***************************************************************************
void XtalFinderCalculator::loadStructuresFromAflowlibEntries(const vector<aflowlib::_aflowlib_entry>& entries, const vector<string>& magmoms_for_systems, bool same_species) {
  ...
  string structure_name;
  const string source = "aflowlib";
  const uint relaxation_step = 2; // input assumed to be most relaxed

  // ---------------------------------------------------------------------------
  // loop through aflowlib entries
  for (size_t i = 0; i < entries.size(); i++) {
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Loading entry " << i << ": " << entries[i].auid << endl;
    }

    // load structure (if possible with try-catch)
    xstructure xstr_tmp;
    try {
      xstr_tmp = entries[i].vstr.back();
    } // back() is the most relaxed structure
    catch (aurostd::xerror& excpt) {
    ...
    if (magmoms_for_systems.size() == entries.size()) {
      try {
        pflow::ProcessAndAddSpinToXstructure(xstr_tmp, magmoms_for_systems[i]);
      } catch (aurostd::xerror& excpt) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Successfully loaded entry " << i << ": " << entries[i].auid << endl;
    }
  }
}
...
// ***************************************************************************
void XtalFinderCalculator::loadStructuresFromFile(const string& filename, const vector<string>& magmoms_for_systems, bool same_species) {
  ...
  aurostd::compressfile2stringstream(filename, input_file);
  loadStructuresFromStringstream(input_file, magmoms_for_systems, same_species);
}

// ME20210206 - Added stringstream variant
void XtalFinderCalculator::loadStructuresFromStringstream(stringstream& input_stream, const vector<string>& magmoms_for_systems, bool same_species) {
  const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_COMPARE_);
  stringstream message;
  ...
  const string START = _VASP_POSCAR_MODE_EXPLICIT_START_;
  const string STOP = _VASP_POSCAR_MODE_EXPLICIT_STOP_;
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " structure_count=" << structure_count << endl;
  }

  const uint relaxation_step = 0;
  for (size_t i = 0; i < structure_lines.size(); i++) {
    if (LDEBUG) {
      cerr << "compare:: loading " << i << "/" << structure_lines.size() << endl;
    }

    stringstream designation;
    designation << "file structure # " << i + 1 << "/" << start_string.size();  // CO20211118 - otherwise last structure is 81/82
    xstructure xstr;
...
//  ***************************************************************************
void XtalFinderCalculator::loadStructuresFromStructureList(const vector<string>& filenames, const vector<string>& magmoms_for_systems, bool same_species) {
  ...
  for (size_t i = 0; i < filenames.size(); i++) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << ": loaded structure " << i << endl;
    }
  }
}
  ...
  void generateStructures(vector<StructurePrototype>& structures, ostream& oss, uint start_index, uint end_index) {
    ...
    for (uint i = start_index; i < end_index; i++) {
      if (!structures[i].structure_representative->is_structure_generated) {
        structures[i].structure_representative->is_structure_generated = generateStructure(structures[i].structure_representative->name, structures[i].structure_representative->source,
                                                                                           structures[i].structure_representative->relaxation_step, // DX20200429
                                                                                           structures[i].structure_representative->structure, oss);
      }
    }
  }
} // namespace compare
...
// ***************************************************************************
namespace compare {
  bool generateStructure(const string& structure_name, const string& structure_source, uint relaxation_step, xstructure& structure,
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " generating structure: " << structure_name << " from " << structure_source << endl;
    }
    ...
    // load from AURL
      ...
      const uint relaxation_step = _COMPARE_DATABASE_GEOMETRY_MOST_RELAXED_; // default
      bool load_most_relaxed_structure_only = true;
      if (relaxation_step != _COMPARE_DATABASE_GEOMETRY_MOST_RELAXED_) {
        load_most_relaxed_structure_only = false;
      }
      aflowlib::_aflowlib_entry entry;
      entry.aurl = structure_name;
      vector<string> structure_files;
      // DX ORIG B4 20191105 - if(!pflow::loadXstructures(entry,FileMESSAGE,oss,true,structure_name,true)){ cerr << __AFLOW_FUNC__ << "WARNING::Could not load structure via aurl..." << endl; return false;} //DX20190326
      if (!pflow::loadXstructures(entry, structure_files, FileMESSAGE, oss, load_most_relaxed_structure_only)) {
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "Could not load structure (aurl=" + entry.aurl + ") ... skipping...", FileMESSAGE, oss, _LOGGER_WARNING_);
        return false;
      }
      ...
      if (load_most_relaxed_structure_only && entry.vstr.size() == 1) {
        ...
        if (entry.vstr.size() == 3 && structure_files.size() == 3) {
          if (relaxation_step == _COMPARE_DATABASE_GEOMETRY_ORIGINAL_ && (structure_files[0] == "POSCAR.orig" || structure_files[0] == "POSCAR.relax1")) {
            structure_index = 0;
            found_structure = true;
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " loaded original structure: " << structure_files[0] << endl;
            }
          } else if (relaxation_step == _COMPARE_DATABASE_GEOMETRY_RELAX1_ && (structure_files[1] == "POSCAR.relax2" || structure_files[1] == "CONTCAR.relax1")) {
            structure_index = 1;
            found_structure = true;
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " loaded relax1 structure: " << structure_files[1] << endl;
            }
          }
        }
      }
      ...
      if (found_structure) {
        ...
        cerr << __AFLOW_FUNC__ << "::WARNING: More structures loaded than anticipated." << endl;
        return false;
      }
    }
    // ---------------------------------------------------------------------------
    // load from file
      ...
      stringstream sss;
      aurostd::compressfile2stringstream(structure_name, sss);
      try {
        structure.clear(); // DX20210129
        structure.initialize(sss); // DX20210129
      } catch (aurostd::xerror& excpt) {
        cerr << "Could not load structure " << structure_name << "...skipping structure";
        return false;
      } // DX20190718
    }
    // ---------------------------------------------------------------------------
    // load from file containing list of structures (like aflow.in or pocc)
      ...
      const string structure_designation = aurostd::RemoveWhiteSpaces(tokens[1]);
      uint structure_number = 0;
      uint number_of_structures = 0;
      tokens.clear();
      aurostd::string2tokens(structure_designation, tokens, "/");
      ...
      for (size_t i = 0; i < lines.size(); i++) {
        if (lines[i].find(_VASP_POSCAR_MODE_EXPLICIT_START_) != std::string::npos) {
          ...
          const xstructure xstr(geometry);
          structure = xstr;
          break;
        }
      }
    ...
    // load permutation
      ...
      const string tmp_source = structure_source;
      stringstream sss;
      sss << aurostd::StringSubst(tmp_source, "permutation of: ", "");
      structure.clear(); // DX20210129
      structure.initialize(sss); // DX20210129
      deque<string> species;
      for (size_t j = 0; j < structure_name.size(); j++) {
        stringstream ss_site;
        ss_site << structure_name[j];
        species.push_back(ss_site.str());
      }
    ...
    // load-type not accounted for
      ...
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, FileMESSAGE, _LOGGER_WARNING_); // DX20200103 - cerr to logger
      return false;
    }
...
namespace compare {
  string findICSDName(const string& name) {
    ...
    if (!ICSD_substring_found) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " WARNING: Could not find ICSD substring in name. Representative prototype will not necessarily be the minimum ICSD number." << endl;
...
namespace compare {
  string findMinimumICSDEntry(const vector<string>& ICSD_entries) {
    ...
    string min_ICSD;
    int min_num = 1e9;
    for (size_t i = 0; i < ICSD_entries.size(); i++) {
      if (ICSD_entries[i].empty()) {
        continue;
      } // DX20191108 - if not an ICSD, skip
      vector<string> tokens;
...
// ***************************************************************************
void XtalFinderCalculator::compareAtomDecorations(StructurePrototype& structure, uint num_proc, bool optimize_match) {
...
void XtalFinderCalculator::compareAtomDecorations(StructurePrototype& structure, uint num_proc, aurostd::xoption& permutation_options) {
  string misfit_result;
  return compareAtomDecorations(structure, misfit_result, num_proc, permutation_options);
}

void XtalFinderCalculator::compareAtomDecorations(StructurePrototype& structure, string& misfit_results, uint num_proc, aurostd::xoption& permutation_options) {
  ...
  const bool same_species = true; // permutation comparisons must compare the same species
  const bool quiet = true; // true

  vector<StructurePrototype> final_permutations;

  // ---------------------------------------------------------------------------
  // get stoichiometry
  const vector<uint> stoichiometry = structure.structure_representative->structure.GetReducedComposition();

  // ---------------------------------------------------------------------------
  // calculate symmetry (if not already calculated)
  const bool ignore_symmetry = permutation_options.flag("COMPARISON_OPTIONS::IGNORE_SYMMETRY");
  ...
  if (!compare::arePermutationsComparableViaComposition(stoichiometry) || (!ignore_symmetry && !compare::arePermutationsComparableViaSymmetry(structure.structure_representative->grouped_Wyckoff_positions))) {
    ...
    for (size_t j = 0; j < unique_permutations.size(); j++) {
      vector<string> permutations_tmp;
      permutations_tmp.push_back(unique_permutations[j]);
  ...
  for (uint mode = 0; mode < 2; mode++) {
    ...
    permutation_comparisons = groupStructurePrototypes(same_species, permutation_options.flag("COMPARISON_OPTIONS::IGNORE_SYMMETRY"), permutation_options.flag("COMPARISON_OPTIONS::IGNORE_WYCKOFF"),
    ...
    if (VERBOSE) {
      for (size_t i = 0; i < permutation_comparisons.size(); i++) {
        cerr << "Initial permutation groupings: " << permutation_comparisons[i] << endl;
      }
    }

    // ---------------------------------------------------------------------------
    // compare permutations
    final_permutations = runComparisonScheme(permutation_comparisons, same_species, num_proc, permutation_options,
                                             quiet); // DX20200103 - condensed booleans to xoptions
    ...
    if (!compare::checkNumberOfGroupings(final_permutations, decoration_names.size())) {
      if (!quiet || LDEBUG) {
        ...
        if (LDEBUG) {
          for (size_t i = 0; i < final_permutations.size(); i++) {
            message << final_permutations[i] << endl;
          }
        }
        // minimal output
        else {
          for (size_t i = 0; i < final_permutations.size(); i++) { // DX20191218 - new misfit struct version
            message << final_permutations[i].structure_representative->name << " = ";
            vector<string> duplicate_info;
            for (size_t j = 0; j < final_permutations[i].structures_duplicate.size(); j++) {
              stringstream ss_tmp;
              ss_tmp << final_permutations[i].structures_duplicate[j]->name << " (misfit_duplicate=" << final_permutations[i].mapping_info_duplicate[j].misfit << ")";
      ...
      final_permutations = checkForBetterMatches(final_permutations, num_proc, true, same_species, permutation_options,
                                                 quiet); // DX20200103 - condensed booleans to xoptions
  ...
  if (VERBOSE) {
    for (size_t i = 0; i < final_permutations.size(); i++) {
      cerr << "Final permutation groupings: " << final_permutations[i] << endl;
    }
  }

  // ---------------------------------------------------------------------------
  // store results
  for (size_t j = 0; j < final_permutations.size(); j++) {
    vector<string> permutations_tmp;
    permutations_tmp.push_back(final_permutations[j].structure_representative->name); // push back representative permutation
    for (size_t d = 0; d < final_permutations[j].structures_duplicate.size(); d++) {
      permutations_tmp.push_back(final_permutations[j].structures_duplicate[d]->name);
    } // push back equivalent permutations
    structure.atom_decorations_equivalent.push_back(permutations_tmp);
  }
...
// ***************************************************************************
void XtalFinderCalculator::generateAtomPermutedStructures(structure_container& structure) {
  // Generate all atom permuted variants of a given structure.
  // Moved Heap's algorithm into aurostd::xcombos.

  deque<string> species;
  const bool is_symmetry_calculated = isSymmetryCalculated(structure);

  // ---------------------------------------------------------------------------
  // get permuted species strings via Heap's algorithm:
  // swap lowest position index first (left-most)
  const vector<string> species_permuted = getSpeciesPermutedStrings(structure.stoichiometry);

  vector<string> names = pflow::getFakeElements(structure.stoichiometry.size()); // DX20200728 - now in pflow
  const vector<uint> indices = structure.stoichiometry;
  const uint num_elements = structure.stoichiometry.size();
  ...
  while (indices_combos.increment()) {
    all_indices.push_back(indices_combos.getCombo());
  }
  ...
  for (size_t i = 0; i < all_indices.size(); i++) {
    ...
    species.clear();
    for (size_t j = 0; j < all_indices[i].size(); j++) {
    ...
    const string system_name = aurostd::joinWDelimiter(species, "");
    stringstream ss_str;
    ss_str << "permutation of: " << xstr_tmp; // DX20190730
    ...
    if (is_symmetry_calculated) {
      ...
      for (size_t j = 0; j < grouped_Wyckoff_positions.size(); j++) {
...
// ***************************************************************************
// deque input
vector<string> XtalFinderCalculator::getSpeciesPermutedStrings(const deque<uint>& stoichiometry) {
  const vector<uint> stoichiometry_vstring = aurostd::deque2vector(stoichiometry);
  return getSpeciesPermutedStrings(stoichiometry_vstring);
}

// vector input
vector<string> XtalFinderCalculator::getSpeciesPermutedStrings(const vector<uint>& stoichiometry) {
  vector<string> species;
  vector<string> species_permuted;

  vector<string> names = pflow::getFakeElements(stoichiometry.size()); // DX20200728 - now in pflow
  const vector<uint> indices = stoichiometry;
  const uint num_elements = stoichiometry.size();
...
namespace compare {
  bool arePermutationsComparableViaComposition(const xstructure& xstr) {
    ...
    for (size_t i = 0; i < xstr.num_each_type.size(); i++) {
      for (size_t j = i + 1; j < xstr.num_each_type.size(); j++) {
        if (xstr.num_each_type[i] == xstr.num_each_type[j]) {
          matchable_sites = true;
          break;
        }
      }
      if (matchable_sites) {
...
namespace compare {
  bool arePermutationsComparableViaComposition(const vector<uint>& composition, bool reduce_composition) {
    ...
    if (reduce_composition) {
      aurostd::reduceByGCD(composition, tmp_composition);
    } // DX20191125
    else {
      tmp_composition = composition;
    } // assuming it is already reduced

    // ---------------------------------------------------------------------------
    // check if stoichiometries occur more than once
    bool matchable_sites = false;
    for (size_t i = 0; i < tmp_composition.size(); i++) {
      for (size_t j = i + 1; j < tmp_composition.size(); j++) {
        if (tmp_composition[i] == tmp_composition[j]) {
          matchable_sites = true;
          break;
        }
      }
...
namespace compare {
  bool arePermutationsComparableViaSymmetry(const vector<GroupedWyckoffPosition>& grouped_Wyckoff_positions) {
    ...
    for (size_t i = 0; i < sorted_temp_grouped_Wyckoffs.size(); i++) {
      const string set_1_site_symmetry = aurostd::joinWDelimiter(sorted_temp_grouped_Wyckoffs[i].site_symmetries, ",");
      const string set_1_multiplicity = aurostd::joinWDelimiter(sorted_temp_grouped_Wyckoffs[i].multiplicities, ",");
      for (size_t j = i + 1; j < sorted_temp_grouped_Wyckoffs.size(); j++) {
        const string set_2_site_symmetry = aurostd::joinWDelimiter(sorted_temp_grouped_Wyckoffs[j].site_symmetries, ",");
        const string set_2_multiplicity = aurostd::joinWDelimiter(sorted_temp_grouped_Wyckoffs[j].multiplicities, ",");
...
// ***************************************************************************
void XtalFinderCalculator::addAFLOWPrototypes2container(const std::set<string>& vuid) {
  ...
  for (const string& uid : vuid) {
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Storing AFLOW prototype information for " << uid << endl;
    }
...
// ***************************************************************************
void XtalFinderCalculator::addDatabaseEntry2container(aflowlib::_aflowlib_entry& entry, const vector<string>& species, uint relaxation_step, bool same_species) {
  ...
  if (relaxation_step == _COMPARE_DATABASE_GEOMETRY_MOST_RELAXED_) {
    load_most_relaxed_structure_only = true;
  }
  ...
  if (!load_most_relaxed_structure_only) {
    if (entry.vstr.size() == 3 && structure_files.size() == 3) {
      ...
      } else if (relaxation_step == _COMPARE_DATABASE_GEOMETRY_MOST_RELAXED_ && (structure_files[2] == "POSCAR.relax2" || structure_files[2] == "CONTCAR.relax1")) {
        structure_index = 2;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " loaded most relaxed structure: " << structure_files[2] << endl;
        }
      } else {
  ...
  const deque<string> deque_species = aurostd::vector2deque(species);
  entry.vstr[structure_index].SetSpecies(deque_species);
  const string str_path = entry.getPathAURL(*p_FileMESSAGE, *p_oss, false);
...
// ***************************************************************************
bool XtalFinderCalculator::splitComparisonIntoThreads(vector<StructurePrototype>& comparison_schemes, uint num_proc, vector<std::pair<uint, uint>>& start_indices, vector<std::pair<uint, uint>>& end_indices) {
  ...
  const bool safety_check = false; // safety check if split incorrectly

  uint number_of_comparisons = 0;
  for (size_t i = 0; i < comparison_schemes.size(); i++) {
    number_of_comparisons += comparison_schemes[i].numberOfComparisons();
  }
  // cerr << "# of comparisons: " << number_of_comparisons << endl;

  if (number_of_comparisons == 0) {
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Number of comparisons is zero, no need to split into threads." << endl;
    }
    return true;
  }

  uint num_per_thread = number_of_comparisons / num_proc;
  const uint residual = number_of_comparisons % num_proc;
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Number of comparisons per thread: " << num_per_thread << endl;
  }
  ...
  uint count = 0;
  uint thread_count = 0;
  std::pair<uint, uint> tmp_start;
  std::pair<uint, uint> tmp_end;
  std::pair<uint, uint> indices;
  for (size_t i = 0; i < comparison_schemes.size(); i++) {
    // DEBUG cerr << "splitting comparison indices: i: " << i << "/" << comparison_schemes.size() << endl;
    for (size_t j = 0; j < comparison_schemes[i].structures_duplicate.size(); j++) {
  ...
  if (count) {
    ...
    tmp_end.first = comparison_schemes.size() - 1;
    tmp_end.second = comparison_schemes[tmp_end.first].structures_duplicate.size();
    end_indices.push_back(tmp_end);
  }
  ...
  if (safety_check) {
    ...
    for (uint n = 0; n < num_of_threads; n++) {
      const uint i_min = start_indices[n].first;
      const uint i_max = end_indices[n].first;
      uint j_min = 0;
      uint j_max = 0;
      for (size_t i = 0; i < comparison_schemes.size(); i++) {
        // to loop properly
        if (i == i_min) {
          ...
          if (i == i_max) {
            j_max = end_indices[n].second;
          } else {
            j_max = comparison_schemes[i].structures_duplicate.size();
          } //-1 since in loop: j<=j_max
        } else if (i == i_max) {
          j_min = 0;
          j_max = end_indices[n].second;
        } else {
        ...
        for (size_t j = 0; j < comparison_schemes[i].structures_duplicate.size(); j++) {
          if (i >= i_min && j >= j_min && i <= i_max && j < j_max) {
            recovered += 1;
          }
        }
      }
...
// ***************************************************************************
void XtalFinderCalculator::performStructureConversions(uint i, const vector<bool>& calculate_primitive_vec, const vector<bool>& calculate_Minkowski_vec, const vector<bool>& calculate_Niggli_vec) {
...
// ***************************************************************************
void XtalFinderCalculator::convertStructures(const aurostd::xoption& comparison_options, uint num_proc) {
  ...
  const uint number_of_structures = structure_containers.size();
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Number of threads=" << num_proc << endl;
  }

  xthread::xThread xt(num_proc);
  std::function<void(uint, const vector<bool>&, const vector<bool>&, const vector<bool>&)> fn = std::bind(&XtalFinderCalculator::performStructureConversions, this, _1, _2, _3, _4);
  ...
  // NONTHREADS - START
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Non-threaded version. Number of threads=" << num_proc << endl;
  }

  // ---------------------------------------------------------------------------
  // perform the relevant structure conversions
  for (uint i = 0; i < number_of_structures; i++) {
    performStructureConversions(i, calculate_primitive_vec, calculate_Minkowski_vec, calculate_Niggli_vec);
  }
#endif
...
void XtalFinderCalculator::getPrimitiveStructures(uint start_index, uint end_index) {
  ...
  if (end_index > structure_containers.size()) {
    end_index = structure_containers.size();
  }
...
void XtalFinderCalculator::calculateSpaceGroups(uint start_index, uint end_index, uint setting) { // DX20191230 - added setting
  ...
  for (uint i = start_index; i < end_index; i++) { // DX20191107 - switching convention <= vs <
    // use sym_eps if given, otherwise use default
    if (!structure_containers[i].structure.sym_eps_calculated || structure_containers[i].structure.sym_eps == AUROSTD_NAN) {
      structure_containers[i].structure.sym_eps = SYM::defaultTolerance(structure_containers[i].structure);
    } // DX20210421
...
void XtalFinderCalculator::calculateSymmetries(uint num_proc) {
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Non-threaded version: " << num_proc << endl;
  }
...
void XtalFinderCalculator::setSymmetryPlaceholders() {
  ...
  for (size_t i = 0; i < structure_containers.size(); i++) {
    ...
    const vector<GroupedWyckoffPosition> vGWyckoffPos_tmp;
...
namespace compare {
  void groupWyckoffPositions(const vector<wyckoffsite_ITC>& wyckoff_sites_ITC, vector<GroupedWyckoffPosition>& grouped_positions) {
    uint type_count = 0; // DX20190425 - add type indicator

    for (size_t i = 0; i < wyckoff_sites_ITC.size(); i++) {
      const uint multiplicity = wyckoff_sites_ITC[i].multiplicity; // DX20191031
      const string letter = wyckoff_sites_ITC[i].letter; // DX20191031
      const string site_symmetry = wyckoff_sites_ITC[i].site_symmetry; // DX20191031
...
// ***************************************************************************
namespace compare {
  void groupWyckoffPositionsFromGroupedString(uint space_group_number, uint setting, const vector<vector<string>>& grouped_Wyckoff_string, vector<GroupedWyckoffPosition>& grouped_positions) {
    ...
    for (size_t i = 0; i < grouped_Wyckoff_string.size(); i++) {
      GroupedWyckoffPosition tmp;
      ...
      for (size_t j = 0; j < grouped_Wyckoff_string[i].size(); j++) {
...
namespace compare {
  string printWyckoffString(const vector<GroupedWyckoffPosition>& grouped_positions, bool alphabetize) {
    ...
    for (size_t i = 0; i < grouped_positions.size(); i++) {
      ...
      for (size_t j = 0; j < grouped_positions[i].letters.size(); j++) {
...
namespace compare {
  vector<GroupedWyckoffPosition> sortSiteSymmetryOfGroupedWyckoffPositions(const vector<GroupedWyckoffPosition>& grouped_Wyckoffs) {
    ...
    for (size_t i = 0; i < grouped_Wyckoffs.size(); i++) {
      sorted_site_symmetry_Wyckoff_positions.push_back(grouped_Wyckoffs[i]);
      for (size_t j = 0; j < grouped_Wyckoffs[i].site_symmetries.size(); j++) {
...
namespace compare {
  bool matchableWyckoffPositions(const vector<GroupedWyckoffPosition>& grouped_Wyckoffs_str1, const vector<GroupedWyckoffPosition>& grouped_Wyckoffs_str2, bool same_species) {
    ...
    if (grouped_Wyckoffs_str1.size() != grouped_Wyckoffs_str2.size()) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " # of Wyckoff positions does not match (" << grouped_Wyckoffs_str1.size() << " vs " << grouped_Wyckoffs_str2.size() << endl;
      }
      return false;
    }
    ...
    if (LDEBUG) {
      for (size_t i = 0; i < grouped_Wyckoffs_str1_sorted.size(); i++) {
        cerr << "str1: " << grouped_Wyckoffs_str1_sorted[i] << endl;
      }
      for (size_t i = 0; i < grouped_Wyckoffs_str2_sorted.size(); i++) {
        cerr << "str2: " << grouped_Wyckoffs_str2_sorted[i] << endl;
      }
    }
    ...
    bool match_single_str1 = false;
    uint match_count = 0;
    for (size_t i = 0; i < grouped_Wyckoffs_str1_sorted.size(); i++) {
      match_set_str1 = false;
      for (size_t j = 0; j < grouped_Wyckoffs_str2_sorted.size(); j++) {
        // ---------------------------------------------------------------------------
        // check if species in str2 have already been matched
        if (matched_species_str2[j]) {
          continue;
        }
        // ---------------------------------------------------------------------------
        ...
        if (((same_species && grouped_Wyckoffs_str1_sorted[i].element == grouped_Wyckoffs_str2_sorted[j].element) || !same_species) && // check species requirement
          ...
          match_count = 0;
          Wyckoff_subset_matches.clear();
          Wyckoff_subset_matches.resize(grouped_Wyckoffs_str2_sorted[j].multiplicities.size(), false);
          for (size_t m = 0; m < grouped_Wyckoffs_str1_sorted[i].multiplicities.size(); m++) {
            match_single_str1 = false;
            for (size_t n = 0; n < grouped_Wyckoffs_str2_sorted[j].multiplicities.size(); n++) {
              if (grouped_Wyckoffs_str1_sorted[i].multiplicities[m] == grouped_Wyckoffs_str2_sorted[j].multiplicities[n] &&
                  grouped_Wyckoffs_str1_sorted[i].site_symmetries[m] == grouped_Wyckoffs_str2_sorted[j].site_symmetries[n] && !Wyckoff_subset_matches[n]) {
                match_count++;
            ...
            if (!match_single_str1) {
              break;
            } // if no matches for any Wyckoff positions in str1, then we cannot match to this Wyckoff set
          }
          // ---------------------------------------------------------------------------
    ...
    for (size_t i = 0; i < matched_species_str2.size(); i++) {
      if (!matched_species_str2[i]) {
        return false;
      }
    }
...
namespace compare {
  vector<vector<string>> convertANRLWyckoffString2GroupedPositions(const string& label) {
    ...
    // DEBUG for(size_t i=0;i<anrl_grouped_Wyckoff_letters.size();i++){
...
namespace compare {
  vector<vector<string>> convertWyckoffString2GroupedPositions(const string& Wyckoff_letter_string) {
    ...
    for (size_t i = 0; i < token.size(); i++) {
      Wyckoff_set.push_back(token[i]);
    }
    for (size_t i = 0; i < Wyckoff_set.size(); i++) {
      aurostd::string2tokens(Wyckoff_set[i], token, ",");
      vector<string> Wyckoff_letters_for_species;
      for (size_t j = 0; j < token.size(); j++) {
        Wyckoff_letters_for_species.push_back(token[j]);
      }
...
namespace compare {
  bool matchableWyckoffPositionSet(const vector<vector<vector<string>>>& grouped_possible_Wyckoff_letters, const vector<vector<string>>& grouped_Wyckoff_letters) {
    ...
    const bool all_Wyckoffs_matched = false;
    ...
    vector<int> number_of_letters_1;
    vector<int> number_of_letters_2;
    for (size_t i = 0; i < grouped_possible_Wyckoff_letters.size(); i++) {
    ...
    for (size_t i = 0; i < number_of_letters_1.size(); i++) {
      if (number_of_letters_1[i] != number_of_letters_2[i]) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " Number of Wyckoff letters does not match between structures." << endl;
        }
        return false;
      }
    }

    const vector<std::pair<uint, uint>> matchable_indices;
    uint number_of_matched_sets = 0;
    for (size_t m = 0; m < grouped_Wyckoff_letters.size(); m++) {
      ...
      for (size_t i = 0; i < grouped_possible_Wyckoff_letters.size(); i++) {
        if (grouped_possible_Wyckoff_letters[i].size() == grouped_Wyckoff_letters[m].size()) {
          ...
          for (size_t j = 0; j < grouped_possible_Wyckoff_letters[i].size(); j++) {
            bool Wyckoff_matched = false;
            for (size_t k = 0; k < grouped_possible_Wyckoff_letters[i][j].size(); k++) {
              for (size_t n = 0; n < grouped_Wyckoff_letters[m].size(); n++) {
                bool index_used = false;
                for (size_t t = 0; t < index_matched.size(); t++) {
                  if (n == index_matched[t]) {
                    index_used = true;
                  }
                }
                if (index_used) {
...
namespace compare {
  bool structuresCompatible(const structure_container& structure1,
    ...
    if (((ignore_symmetry && ignore_Wyckoff) || (!ignore_symmetry && ignore_Wyckoff && structure1.Pearson == structure2.Pearson && matchableSpaceGroups(structure1.space_group, structure2.space_group)) ||
         (!ignore_symmetry && !ignore_Wyckoff && structure1.Pearson == structure2.Pearson && matchableSpaceGroups(structure1.space_group, structure2.space_group) &&
...
// ***************************************************************************
vector<StructurePrototype> XtalFinderCalculator::groupStructurePrototypes(bool same_species,
                                                                          bool ignore_symmetry,
                                                                          bool ignore_Wyckoff,
                                                                          bool ignore_environment,
                                                                          bool ignore_environment_angles, // DX20200320
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Number of structures to group: " << structure_containers.size() << endl;
  }
  ...
  for (size_t i = 0; i < structure_containers.size(); i++) {
    ...
    for (size_t j = 0; j < comparison_schemes.size(); j++) {
      // bool same_material_stoich=false;

      if (compare::structuresCompatible(structure_containers[i], *comparison_schemes[j].structure_representative, same_species, ignore_symmetry, ignore_Wyckoff, ignore_environment, ignore_environment_angles,
                                        duplicates_removed)) { // DX20190829 - added duplicates_removed //DX20200320 - added environment angles

        if (!same_species) {
          for (size_t e = 0; e < structure_containers[i].elements.size(); e++) {
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Prepared comparison sets: " << endl;
...
// ***************************************************************************
void XtalFinderCalculator::findDuplicateCompounds(uint num_proc, bool remove_duplicates, const string& directory, const aurostd::xoption& comparison_options) {
  ...
  // ---------------------------------------------------------------------------
  // prepare options
  const bool tmp_same_species = true;
  ...
  pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
  vector<StructurePrototype> unique_compounds = compareMultipleStructures(num_proc, tmp_same_species, directory, remove_duplicates_options);
  ...
  for (size_t i = 0; i < unique_compounds.size(); i++) {
  ...
  if (XHOST.vflag_control.flag("PRINT_MODE::JSON")) {
    const string results_json = printResults(unique_compounds, tmp_same_species, json_ft);
    ...
    const string results_txt = printResults(unique_compounds, tmp_same_species, txt_ft);
  ...
  if (remove_duplicates) {
    ...
    for (size_t i = 0; i < unique_compounds.size(); i++) {
      for (size_t j = 0; j < unique_compounds[i].structures_duplicate.size(); j++) {
        duplicates_name.push_back(unique_compounds[i].structures_duplicate[j]->name);
      }
    }
    for (size_t i = 0; i < duplicates_name.size(); i++) {
...
// ***************************************************************************
vector<StructurePrototype> XtalFinderCalculator::checkForBetterMatches(vector<StructurePrototype>& prototype_schemes, uint num_proc, bool check_for_better_matches, bool same_species, const aurostd::xoption& comparison_options, bool quiet) {
  ...
  const double misfit_min = 0.01; // used for check_for_better_matches : this is quite strict; if too expensive, make more loose
  const double misfit_max = misfit_match; // used for check_for_better_matches : otherwise we will compare same family structures which have already been moved (if using !clean_unmatched)

  // ---------------------------------------------------------------------------
  // check which structures could potentially match to others based on misfit
  vector<StructurePrototype> comparison_groups;

  for (size_t i = 0; i < prototype_schemes.size(); i++) {
    for (size_t j = 0; j < prototype_schemes[i].structures_duplicate.size(); j++) {
      if ((check_for_better_matches && prototype_schemes[i].mapping_info_duplicate[j].misfit > misfit_min && prototype_schemes[i].mapping_info_duplicate[j].misfit < misfit_max) || // find better match
          (!check_for_better_matches && (prototype_schemes[i].mapping_info_duplicate[j].misfit > misfit_max || aurostd::isequal(prototype_schemes[i].mapping_info_duplicate[j].misfit, 1.0, 1e-6) ||
                                         aurostd::isequal(prototype_schemes[i].mapping_info_duplicate[j].misfit, AUROSTD_MAX_DOUBLE, 1e-6)))) { // find a match //DX20191218
        StructurePrototype str_proto_tmp;
        ...
        uint start_index = 0;
        if (check_for_better_matches) {
          start_index = i + 1;
        }
        for (size_t k = start_index; k < prototype_schemes.size(); k++) {
          if (i != k) { // don't perform the same comparison again //DX20200414
            if (compare::structuresCompatible(*prototype_schemes[i].structure_representative, // compares StructurePrototype objects
                                              *prototype_schemes[k].structure_representative, same_species, check_better_matches_options.flag("COMPARISON_OPTIONS::IGNORE_SYMMETRY"),
                                              check_better_matches_options.flag("COMPARISON_OPTIONS::IGNORE_WYCKOFF"), check_better_matches_options.flag("COMPARISON_OPTIONS::IGNORE_ENVIRONMENT_ANALYSIS"),
  ...
  // ---------------------------------------------------------------------------
  // compare structures
  vector<StructurePrototype> other_matches_schemes = runComparisonScheme(comparison_groups, same_species, num_proc, check_better_matches_options,
                                                                         quiet); // DX20200103 - condensed booleans to xoptions

  // ---------------------------------------------------------------------------
  // check if there are any better matches and reorganize if necessary
  // the original match is stored in the first position
  for (size_t i = 0; i < other_matches_schemes.size(); i++) {
    ...
    uint min_index = 0;
    for (size_t j = 1; j < other_matches_schemes[i].mapping_info_duplicate.size(); j++) { // DX20191218
    ...
    if (min_index != 0) {
      for (size_t j = 0; j < prototype_schemes.size(); j++) {
        ...
        if (prototype_schemes[j].structure_representative->name == other_matches_schemes[i].structures_duplicate[0]->name) { // DX20201223 - always remove the old match
          for (size_t k = 0; k < prototype_schemes[j].structures_duplicate.size(); k++) {
  ...
  if (LDEBUG) {
    for (size_t i = 0; i < prototype_schemes.size(); i++) {
      cerr << __AFLOW_FUNC__ << " prototype_schemes[i]: " << prototype_schemes[i] << endl;
    }
  }
...
void XtalFinderCalculator::representativePrototypeForICSDRunsNEW(vector<StructurePrototype>& comparison_schemes) {
  ...
  for (size_t i = 0; i < comparison_schemes.size(); i++) {
    if (!comparison_schemes[i].structures_duplicate.empty()) {
      ...
      const string min_ICSD_entry = compare::findMinimumICSDEntry(ICSD_entries);
...
void XtalFinderCalculator::runComparisonThreads(
  ...
  const uint i_min = start_indices.first;
  const uint i_max = end_indices.first;
  uint j_min = 0;
  uint j_max = 0;

  // to prevent nested multi-processes
  const uint num_proc_orig = num_proc;
  num_proc = 1;
  ...
  for (uint i = i_min; i <= i_max; i++) {
    ...
    for (uint j = j_min; j < j_max; j++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Loaded representative structure = " << structure_rep_tmp.name << endl;
      }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Loaded duplicate structure = " << structure_dup_tmp.name << endl;
      }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Comparing " << structure_rep_tmp.name << " and " << structure_dup_tmp.name << endl;
      }
      compareStructures(structure_rep_tmp, structure_dup_tmp, final_misfit_info, same_species, scale_volume,
                        optimize_match); // DX20200103 - condensed booleans to xoptions

      // ---------------------------------------------------------------------------
      // store the figure of misfit
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Finished comparing " << structure_rep_tmp.name << " and " << structure_dup_tmp.name << endl;
        cerr << __AFLOW_FUNC__ << " Comparison complete, misfit = " << final_misfit_info.misfit << "." << endl;
      }
...
// ***************************************************************************
void XtalFinderCalculator::runComparisons(vector<StructurePrototype>& comparison_schemes, bool same_species, bool scale_volume, bool optimize_match) {
    ...
    for (size_t j = 0; j < comparison_schemes[i].structures_duplicate.size(); j++) {
      if (j == 0) {
        ...
        if (!comparison_schemes[i].structure_representative->is_structure_generated) {
          if (!compare::generateStructure(comparison_schemes[i].structure_representative->name, comparison_schemes[i].structure_representative->source,
                                          comparison_schemes[i].structure_representative->relaxation_step, comparison_schemes[i].structure_representative->structure, *p_oss)) {
      ...
      if (!comparison_schemes[i].structures_duplicate[j]->is_structure_generated) {
        if (!compare::generateStructure(comparison_schemes[i].structures_duplicate[j]->name, comparison_schemes[i].structures_duplicate[j]->source,
                                        comparison_schemes[i].structures_duplicate[j]->relaxation_step, comparison_schemes[i].structures_duplicate[j]->structure, *p_oss)) { // DX20200429
      ...
      // ---------------------------------------------------------------------------
      // call the main comparison function
      compareStructures(*comparison_schemes[i].structure_representative, *comparison_schemes[i].structures_duplicate[j], final_misfit_info, same_species, scale_volume,
                        optimize_match); // DX20200103 - condensed booleans to xoptions
...
// ***************************************************************************
vector<StructurePrototype> XtalFinderCalculator::runComparisonScheme(vector<StructurePrototype>& comparison_schemes, bool same_species, uint num_proc, const aurostd::xoption& comparison_options, bool quiet) {
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Number of comparison sets: " << comparison_schemes.size() << endl;
    cerr << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl;
    cerr << printResults(comparison_schemes, same_species, txt_ft) << endl;
  }
  ...
  std::function<void(uint, vector<StructurePrototype>&, const vector<std::pair<uint, uint>>&, const vector<std::pair<uint, uint>>&, bool, bool, bool)> fn =
      std::bind(&XtalFinderCalculator::runComparisonThreads, this, _1, _2, _3, _4, _5, _6, _7);
  xt.run(num_comparison_threads, fn, comparison_schemes, start_indices, end_indices, same_species, scale_volume, optimize_match);
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Non-threaded version." << endl;
  }
  runComparisons(comparison_schemes, same_species, scale_volume, optimize_match);
  ...
  if (comparison_options.flag("COMPARISON_OPTIONS::PRINT_MATCHES_TO_INPUT_ONLY") && !comparison_schemes.empty()) {
    appendStructurePrototypes(comparison_schemes, prototypes_final, comparison_options.flag("COMPARISON_OPTIONS::CLEAN_UNMATCHED"), quiet);
  ...
  if (!comparison_options.flag("COMPARISON_OPTIONS::IGNORE_ENVIRONMENT_ANALYSIS") || comparison_options.flag("COMPARISON_OPTIONS::CHECK_OTHER_GROUPINGS")) {
    ...
    comparison_schemes = checkForBetterMatches(comparison_schemes, num_proc, false, same_species, check_better_matches_options,
                                               quiet); // DX20200103 - condensed booleans to xoptions
  }
  ...
  if (num_mismatches == 0 && !comparison_options.flag("COMPARISON_OPTIONS::SINGLE_COMPARISON_ROUND")) {
    appendStructurePrototypes(comparison_schemes, prototypes_final, comparison_options.flag("COMPARISON_OPTIONS::CLEAN_UNMATCHED"),
                              quiet); // DX20200103
  }
  ...
  while (num_mismatches != 0) {
    ...
    if (comparison_options.flag("COMPARISON_OPTIONS::SINGLE_COMPARISON_ROUND")) {
      return prototypes_final;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number_of_comparisons: " << number_of_comparisons << endl;
    }
    ...
    if (number_of_comparisons > 0) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << ": Number of comparisons is not zero... " << number_of_comparisons << endl;
      }
#ifdef AFLOW_MULTITHREADS_ENABLE
      num_comparison_threads = aurostd::min(num_proc, number_of_comparisons);
      ...
      runComparisonThreads(0, comparison_schemes, start_indices, end_indices, same_species, scale_volume, optimize_match);
      // SINGLE THREAD - END
#endif
    }
  ...
  appendStructurePrototypes(comparison_schemes, prototypes_final, comparison_options.flag("COMPARE_STRUCTURE::CLEAN_UNMATCHED"),
                            quiet); // DX20200103
...
namespace compare {
  bool checkNumberOfGroupings(const vector<StructurePrototype>& comparison_schemes, uint number) {
    ...
    for (size_t i = 0; i < divisor_pairs.size(); i++) {
...
// ***************************************************************************
void XtalFinderCalculator::makeRepresentativeEvenPermutation(vector<StructurePrototype>& comparison_schemes, const vector<string>& name_order) {
  ...
  uint min_even_duplicate_permutation_num = AUROSTD_MAX_UINT;
  ...
  for (size_t i = 0; i < comparison_schemes.size(); i++) {
    ...
    for (size_t p = 0; p < comparison_schemes[i].structures_duplicate.size(); p++) {
      for (size_t j = 0; j < name_order.size(); j++) {
...
// ***************************************************************************
uint XtalFinderCalculator::numberOfMismatches(const vector<StructurePrototype>& comparison_schemes) const {
  // Count the number of comparisons that have a misfit greater than
  // misfit_match (default: 0.1) i.e., not a match

  int num_mismatches = 0;
  for (size_t i = 0; i < comparison_schemes.size(); i++) {
    for (size_t j = 0; j < comparison_schemes[i].mapping_info_duplicate.size(); j++) {
...
// ***************************************************************************
void XtalFinderCalculator::appendStructurePrototypes(vector<StructurePrototype>& comparison_schemes,
                                                     vector<StructurePrototype>& prototypes_final,
  ...
  const uint number_of_comparisons = comparison_schemes.size();
    ...
    for (size_t j = 0; j < comparison_schemes[i].mapping_info_duplicate.size(); j++) {
      if (comparison_schemes[i].mapping_info_duplicate[j].misfit > misfit_match) {
        ...
        if (first_mismatch) {
          ...
          if (clean_unmatched) {
            comparison_schemes[i].removeNonDuplicate(j);
            j--;
          } // DX20190504 - put in if-statement
...
// ***************************************************************************
void XtalFinderCalculator::combinePrototypesOfDifferentSymmetry(vector<StructurePrototype>& prototypes_final, bool same_species, uint num_proc) {
  ...
  const uint nprototypes = prototypes_final.size();
  ...
  for (uint i = 0; i < nprototypes; i++) {
    ...
    for (size_t j = i; j < prototypes_final.size(); j++) {
      if (
          // If same_species==true
          (same_species && compare::matchableSpecies(prototypes_final[i].structure_representative->structure, prototypes_final[j].structure_representative->structure, same_species) &&
           prototypes_final[i].stoichiometry == prototypes_final[j].stoichiometry && prototypes_final[i].Pearson == prototypes_final[j].Pearson &&
           !compare::matchableSpaceGroups(prototypes_final[i].space_group, prototypes_final[j].space_group)) ||
          // If same_species==false
          (!same_species && prototypes_final[i].stoichiometry == prototypes_final[j].stoichiometry && prototypes_final[i].Pearson == prototypes_final[j].Pearson &&
           !compare::matchableSpaceGroups(prototypes_final[i].space_group, prototypes_final[j].space_group))) {
    ...
    if (min_misfit != AUROSTD_MAX_DOUBLE) {
      ...
      for (size_t j = 0; j < prototypes_final[other_ind].structures_duplicate.size(); j++) {
...
// ***************************************************************************
string XtalFinderCalculator::printResults(const vector<StructurePrototype>& prototypes_final, bool same_species, filetype format) const {
  ...
  // text format
    ...
    for (size_t j = 0; j < prototypes_final.size(); j++) {
      ...
      if (same_species) {
        ss_out << prototypes_final[j].structure_representative->compound;
      } else {
        ss_out << aurostd::joinWDelimiter(prototypes_final[j].stoichiometry, ":");
      }
      ...
      if (same_species) {
        const uint number_of_duplicates = numberOfDuplicates(prototypes_final[j]); // DX20190506 - made function
      ...
      if (!prototypes_final[j].aflow_label.empty()) {
      ...
      if (!prototypes_final[j].matching_aflow_prototypes.empty()) {
      ...
      if (!prototypes_final[j].atom_decorations_equivalent.empty()) {
        vector<string> unique_decorations;
        for (size_t d = 0; d < prototypes_final[j].atom_decorations_equivalent.size(); d++) {
          unique_decorations.push_back(prototypes_final[j].atom_decorations_equivalent[d][0]);
        }
      ...
      if (!prototypes_final[j].structure_representative->properties.empty()) {
        ss_out << "  " << setw(structure_spacing) << std::left << "structure";
        ss_out << setw(misfit_spacing) << std::right << "misfit";
        for (size_t l = 0; l < prototypes_final[j].structure_representative->properties_names.size(); l++) {
          if (!prototypes_final[j].structure_representative->properties_units[l].empty()) {
            ss_out << setw(property_spacing) << std::right << prototypes_final[j].structure_representative->properties_names[l] + "(" + prototypes_final[j].structure_representative->properties_units[l] + ")";
          } else {
            ss_out << setw(property_spacing) << std::right << prototypes_final[j].structure_representative->properties_names[l];
          }
        }
        ss_out << endl;
      ...
      if (!prototypes_final[j].structure_representative->properties.empty()) {
        ...
        for (size_t l = 0; l < prototypes_final[j].structure_representative->properties.size(); l++) {
      ...
      if (prototypes_final[j].structure_representative->properties.empty()) {
        for (size_t l = 0; l < prototypes_final[j].property_names.size(); l++) {
          if (!prototypes_final[j].property_units[l].empty()) {
            ss_out << setw(property_spacing) << std::right << prototypes_final[j].property_names[l] + "(" + prototypes_final[j].property_units[l] + ")";
          } else {
            ss_out << setw(property_spacing) << std::right << prototypes_final[j].property_names[l];
          }
        }
      }
      ss_out << endl;
...
// ***************************************************************************
string XtalFinderCalculator::printStructureMappingResults(const structure_mapping_info& misfit_info, const xstructure& xstr_reference, const xstructure& xstr_mapped, const string& mode) {
  ...
  if (aurostd::toupper(mode) == "TEXT" || aurostd::toupper(mode) == "TXT") {
    ...
    if (!DEFAULT_XTALFINDER_SUPERCELL_METHOD) {
      ...
      xstr_transformed.TransformStructure(misfit_info.basis_transformation, misfit_info.rotation, misfit_info.origin_shift,
...
namespace compare {
  bool matchableSpecies(const xstructure& xstr1, const xstructure& xstr2, const bool& same_species) {
    ...
    if (xstr1.species.size() == xstr2.species.size()) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " NUMBER OF TYPES OF ATOMIC SPECIES IS NOT THE SAME." << endl;
...
// ***************************************************************************
namespace compare {
  bool sameSpecies(const xstructure& xstr1, const xstructure& xstr2, bool display) {
    // Determine if the structures have the same types and counts of species

    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_COMPARE_);

    const bool VERBOSE = (display && LDEBUG); // DX20191125
    ...
    if (xstr1.num_each_type.size() != xstr2.num_each_type.size()) {
      ...
      if (VERBOSE) { // DX20190702 - condense if-statements
        cerr << __AFLOW_FUNC__ << ": Number of element types are not the same."
             << " xstr 1: " << xstr1.num_each_type.size() << " and xstr2: " << xstr2.num_each_type.size() << endl;
      }
    ...
    if (xstr1_num_each_type != xstr2_num_each_type) {
      if (VERBOSE) {
        cerr << __AFLOW_FUNC__ << " Number of each type of element are incompatible." << endl;
      }
      return false;
    }
    if (VERBOSE) {
      cerr << __AFLOW_FUNC__ << " Number of each type of element are compatible; proceeding." << endl;
    }
...
namespace compare {
  void rescaleStructure(xstructure& xstr1, xstructure& xstr2) {
    ...
    if (std::abs(xstr1.scale - xstr2.scale) > 0.001) {
      xstr1.ReScale(1.0);
      xstr2.ReScale(1.0);
      xstr1.FixLattices();
      xstr2.FixLattices();
...
namespace compare {
  void atomicNumberDensity(const xstructure& xstr1, xstructure& xstr2, double& rescale_factor) {
...
namespace compare {
  vector<string> sortSpeciesByFrequency(const xstructure& xstr) {
    ...
    for (size_t i = 0; i < xstr.num_each_type.size(); i++) {
      ...
      tmp_pair.first = xstr.species[i];
      tmp_pair.second = xstr.num_each_type[i];
    ...
    for (size_t i = 0; i < species_and_counts.size(); i++) {
      reordered_species.push_back(species_and_counts[i].first);
    }
...
namespace compare {
  vector<uint> atomIndicesSortedByFrequency(const xstructure& xstr) {
    ...
    for (size_t i = 0; i < species_str.size(); i++) {
      for (size_t j = 0; j < xstr.atoms.size(); j++) {
...
// ***************************************************************************
namespace compare {
  bool similarLatticeParameters(const xvector<double> d1, const xvector<double> d2) {
    ...
    // relative
    if (relative) {
      const double tol_length = 0.3;
      const double tol_angle = 0.3;

      if (std::abs(d1(1) - d2(1)) < tol_length * std::abs(d1(1)) && std::abs(d1(2) - d2(2)) < tol_length * std::abs(d1(2)) && std::abs(d1(3) - d2(3)) < tol_length * std::abs(d1(3)) &&
          std::abs(d1(4) - d2(4)) < tol_angle * std::abs(d1(4)) && std::abs(d1(5) - d2(5)) < tol_angle * std::abs(d1(5)) && std::abs(d1(6) - d2(6)) < tol_angle * std::abs(d1(6))) {
        return true;
      } else {
        return false;
      }
    ...
    // absolute
      ...
      const double tol_length = 1.0;
      const double tol_angle = 5.0; // 1 Angstrom; 5 degrees

      if (std::abs(d1(1) - d2(1)) < tol_length && std::abs(d1(2) - d2(2)) < tol_length && std::abs(d1(3) - d2(3)) < tol_length && std::abs(d1(4) - d2(4)) < tol_angle && std::abs(d1(5) - d2(5)) < tol_angle &&
          std::abs(d1(6) - d2(6)) < tol_angle) {
        return true;
      } else {
        return false;
      }
...
// ***************************************************************************
bool XtalFinderCalculator::findMatch(const xstructure& xstr1,
  ...
  const double _SAFE_MATCH_CUTOFF_ = minimum_interatomic_distance / DEFAULT_XTALFINDER_SAFE_ATOM_MATCH_SCALING; // DX20200623

  // sets the size of the mapping info
  const uint natoms1 = xstr1.atoms.size();
  const uint natoms2 = xstr2.atoms.size();
  compare::resizeMappingInfo(mapping_info, natoms1);

  // magnetic info
  const bool is_non_collinear = xstr1.atoms[0].noncoll_spin_is_given; // DX20191213
  const bool is_collinear = xstr1.atoms[0].spin_is_given; // DX20191213

  // to store mapping info to check
  vector<uint> mapped_indices_1;
  vector<uint> mapped_indices_2;
  vector<string> mapped_names_1;
  vector<string> mapped_names_2;

  const xmatrix<double> lattice = xstr2.lattice;
  ...
  double tmp = AUROSTD_MAX_DOUBLE;
  double dist = AUROSTD_MAX_DOUBLE;
  double match_dist = AUROSTD_MAX_DOUBLE;
  ...
  for (j = 0; j < natoms1; j++) {
            ...
            for (uint p = 0; p < l3_size; p++) {
              ...
              if (tmp < dist) {
                ...
                if (dist < _SAFE_MATCH_CUTOFF_) {
                  break;
                } // only check if we lower, not every time
              }
    ...
    if (VERBOSE) {
      cerr << __AFLOW_FUNC__ << " Mappings information (iterative): " << endl;
    ...
    if (!compare::consistentAtomMappingType(xstr1.atoms[x1], xstr2.atoms[mapping_info.atom_map[x1]], x1, mapping_info.atom_map[x1], same_species, is_collinear, is_non_collinear)) {
      return false;
    }

    // ---------------------------------------------------------------------------
    // check if indices used more than once (i.e., many-to-one mapping)
    if (!compare::consistentAtomMappingIndex(x1, mapping_info.atom_map[x1], mapped_indices_1, mapped_indices_2)) {
      return false;
    }

    // ---------------------------------------------------------------------------
    // check if sets of mapped atoms are consistent (no cross-matching of types)
    if (!compare::consistentAtomSetMappings(xstr1.atoms[x1].name, xstr2.atoms[mapping_info.atom_map[x1]].name, mapped_names_1, mapped_names_2)) {
      // cerr << "x1: " << x1 << endl;
  ...
  vector<xvector<double>> new_mapping_vectors = compare::minimizeMappingDistances(mapping_info.vectors_mapped, origin_shift_test);

  vector<double> new_mapping_distances(new_mapping_vectors.size());
  for (size_t i = 0; i < new_mapping_vectors.size(); i++) {
    new_mapping_distances[i] = aurostd::modulus(new_mapping_vectors[i]);
  } // DX20200922 - assign with [i], no longer dynamic

  if (LDEBUG) {
    for (size_t i = 0; i < new_mapping_vectors.size(); i++) {
      cerr << __AFLOW_FUNC__ << " minimum distance: " << mapping_info.distances_mapped[i] << " (before) --> " << new_mapping_distances[i] << " (after)" << endl;
    }
    cerr << __AFLOW_FUNC__ << " sum(distances_orig)=" << aurostd::sum(mapping_info.distances_mapped) << " sum(distances_new)=" << aurostd::sum(new_mapping_distances) << endl;
  }
  ...
  if ((aurostd::sum(new_mapping_distances) - aurostd::sum(mapping_info.distances_mapped)) < _ZERO_TOL_) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " the minimization method did not reduce the mapping distances; use the original mapping distances." << endl;
    }
  }
  return true;
}
...
// ***************************************************************************
namespace compare {
  vector<xvector<double>> minimizeMappingDistances(const vector<xvector<double>>& distance_vectors) {
...
namespace compare {
  vector<xvector<double>> minimizeMappingDistances(const vector<xvector<double>>& distance_vectors, xvector<double>& origin_shift) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " origin_shift " << origin_shift << " mod=" << aurostd::modulus(origin_shift) << endl;
    }

    // ---------------------------------------------------------------------------
    // subtract off the residuals (rigid shift)
    vector<xvector<double>> new_distance_vectors(distance_vectors.size());
    for (size_t i = 0; i < new_distance_vectors.size(); i++) {
...
namespace compare {
  void cellDiagonal(const xmatrix<double>& lattice, vector<double>& diag_sum, vector<double>& diag_diff, const double& scale) {
    ...
    diag_sum.push_back(std::abs(distance(origin, (lattice(1) + lattice(2)) * scale)));
    diag_sum.push_back(std::abs(distance(origin, (lattice(1) + lattice(3)) * scale)));
    diag_sum.push_back(std::abs(distance(origin, (lattice(2) + lattice(3)) * scale)));
    diag_diff.push_back(std::abs(distance(lattice(1), lattice(2)) * scale));
    diag_diff.push_back(std::abs(distance(lattice(1), lattice(3)) * scale));
    diag_diff.push_back(std::abs(distance(lattice(2), lattice(3)) * scale));
  }
} // namespace compare
...
namespace compare {
  double latticeDeviation(const vector<double>& diag_sum1, const vector<double>& diag_sum2, const vector<double>& diag_diff1, const vector<double>& diag_diff2) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      dev.push_back((std::abs(diag_sum2[i] - diag_sum1[i]) + std::abs(diag_diff2[i] - diag_diff1[i])) / (diag_sum1[i] + diag_diff1[i])); // DX20191112 - should be compared to the reference, (1) not (2)?
    }
...
namespace compare {
  vector<AtomEnvironment> computeLFAEnvironment(const xstructure& xstr, bool unique_only) {
    ...
    for (size_t i = 0; i < LFAs.size(); i++) {
      ...
      if (unique_only) {
        for (size_t j = 0; j < environments_LFA.size(); j++) {
          bool duplicate = false;
          for (size_t k = 0; k < all_environments_LFA.size(); k++) {
...
namespace compare {
  bool compatibleEnvironmentSets(const vector<AtomEnvironment>& env_set1,
    ...
    if (env_set1.empty() || env_set2.empty()) {
      return true;
    }
    ...
    // if same species
    if (same_species) {
      for (size_t i = 0; i < env_set1.size(); i++) {
        bool matched_set = false;
        vector<vector<string>> matched_species;
        for (size_t j = 0; j < env_set2.size(); j++) {
...
namespace compare {
  bool compatibleEnvironments(const AtomEnvironment& env_1,
                              ...
                              bool same_species,
                              bool ignore_environment_angles, // DX20200320 - added environment angles
                              bool exact_match) {
    vector<vector<string>> matched_species;
...
namespace compare {
  bool compatibleEnvironments(const AtomEnvironment& env_1,
    ...
    const double _TOL_EXACT_MATCH_ = 0.01; // hundredth of an Angstrom, perhaps put in header?
    const double _TOL_LOOSE_MATCH_ = 0.2; // ten percent, perhaps put in header? //DX20190724 - changed from 0.25 to 0.1 //DX20200421 - changed to 0.2 with new relative matching
    const double max_distance_env1 = aurostd::max(env_1.distances_neighbor); // normalize distances for relative comparisons (needed for volume scaling) //DX20200421
    const double max_distance_env2 = aurostd::max(env_2.distances_neighbor); // normalize distances for relative comparisons (needed for volume scaling) //DX20200421

    // ---------------------------------------------------------------------------
    // check for element for center first (fast)
    if (same_species) {
      if (env_1.element_center != env_2.element_center) {
        return false;
      }
    }
    ...
    for (size_t i = 0; i < env_1.elements_neighbor.size(); i++) {
      bool match_found = false;
      vector<string> species;
      for (size_t j = 0; j < env_2.elements_neighbor.size(); j++) {
        // ---------------------------------------------------------------------------
        // check same species, if applicable
        if (same_species && env_1.elements_neighbor[i] != env_2.elements_neighbor[j]) {
          continue;
        }
        ...
        if (exact_match && env_1.coordinations_neighbor[i] != env_2.coordinations_neighbor[j]) {
          continue;
        } // DX20210514 - if exact match, then this is needed

        // ---------------------------------------------------------------------------
        // exact match
        if (exact_match && aurostd::abs(env_1.distances_neighbor[i] - env_2.distances_neighbor[j]) < _TOL_EXACT_MATCH_) {
          match_found = true;
          species.push_back(env_2.elements_neighbor[j]);
        }
    ...
    if (!ignore_environment_angles && same_species) { // same species for now
      ...
      for (size_t i = 0; i < angles_sets_1.size(); i++) {
        // ---------------------------------------------------------------------------
        ...
        if (angles_sets_1[i].size() <= angles_sets_2[i].size()) {
          for (size_t j = 0; j < angles_sets_1[i].size(); j++) {
            bool matched = false;
            for (size_t k = 0; k < angles_sets_2[i].size(); k++) {
              ...
              if (VERBOSE) {
                cerr << __AFLOW_FUNC__ << " angles dont match: " << angles_sets_1[i][j] << " vs " << angles_sets_2[i][k] << " | diff=" << angles_sets_1[i][j] - angles_sets_2[i][k] << endl;
              }
            }
            if (!matched) {
              matched_species.clear();
              return false;
            }
          }
        }
        ...
        // case 2) if set 1 > set 2
          ...
          for (size_t j = 0; j < angles_sets_2[i].size(); j++) {
            bool matched = false;
            for (size_t k = 0; k < angles_sets_1[i].size(); k++) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " environments are compatible." << endl;
    }
...
namespace compare {
  vector<vector<double>> getAnglesBetweenMixedSpeciesEnvironments(const vector<vector<xvector<double>>>& coordinates_neighbor) {
    ...
    for (size_t j = 1; j < coordinates_neighbor.size(); j++) {
      vector<double> angles;
      for (size_t i = 0; i < coordinates_neighbor[0].size(); i++) {
        for (size_t k = 0; k < coordinates_neighbor[j].size(); k++) {
...
namespace compare {
  bool compatibleNearestNeighborTypesEnvironments(const vector<vector<double>>& nn_lfa_with_types_1, const vector<vector<double>>& nn_lfa_with_types_2, int type_match) {
    ...
    if (VERBOSE) {
      for (size_t i = 0; i < nn_lfa_with_types_1.size(); i++) {
        cerr << "1 " << i << ": " << aurostd::joinWDelimiter(aurostd::vecDouble2vecString(nn_lfa_with_types_1[i]), ",") << endl;
      }
      for (size_t i = 0; i < nn_lfa_with_types_2.size(); i++) {
    ...
    if (type_match == 2) {
      if (nn_lfa_with_types_1.size() == nn_lfa_with_types_2.size()) {
        for (size_t i = 0; i < nn_lfa_with_types_1.size(); i++) {
          bool matched_set = false;
          for (size_t j = 0; j < nn_lfa_with_types_2.size(); j++) {
            bool matched = true;
            for (size_t ii = 0; ii < nn_lfa_with_types_1[i].size(); ii++) {
...
// ***************************************************************************
namespace compare {
  void coordinateDeviation(structure_mapping_info& mapping_info, const vector<double>& nn_xstr1, const vector<double>& nn_xstr2) {
    ...
    double dd = 0.0;
    double nn1 = 0.0;
    double nn2 = 0.0; // dd=delta distance, nn=nearest neighbor
    int fail1 = 0;
    int fail2 = 0;
    for (j = 0; j < mapping_info.atom_map.size(); j++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " xstr1 index: " << j << endl;
        cerr << __AFLOW_FUNC__ << " xstr2 index: " << mapping_info.atom_map[j] << endl;
        cerr << __AFLOW_FUNC__ << " nn1: " << nn1 << endl;
        cerr << __AFLOW_FUNC__ << " nn2: " << nn2 << endl;
        cerr << __AFLOW_FUNC__ << " dd: " << dd << endl;
      }
      if (dd <= 0.5 * nn1) {
        fail1 = 0;
      } else {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " cumulative num: " << num << endl;
      cerr << __AFLOW_FUNC__ << " cumulative den: " << den << endl;
    }

    if (den == 0) {
      mapping_info.coordinate_displacement = 1;
    } else {
      mapping_info.coordinate_displacement = num / den;
    }
...
// ***************************************************************************
namespace compare {
  void magneticDeviation(const xstructure& xstr1, const xstructure& xstr2, structure_mapping_info& mapping_info) {
    // BETA functionality: determine magnetic deviation between magnetic
    // structures

    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_COMPARE_);

    const double _NON_COLLINEAR_ANGLE_DEGREE_TOL_ = 10.0;
    double magmom_num = 0.0;
    double magmom_den = 0.0;
    uint mag_fail_1 = 0;
    uint mag_fail_2 = 0;
    const bool is_non_collinear = xstr1.atoms[0].noncoll_spin_is_given;

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " is_non_collinear = " << is_non_collinear << endl;
    }

    for (size_t j = 0; j < mapping_info.atom_map.size(); j++) {
      // ---------------------------------------------------------------------------
      // collinear
      if (!is_non_collinear) {
        const double magmom_diff = aurostd::abs(xstr1.atoms[j].spin - xstr2.atoms[mapping_info.atom_map[j]].spin);
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " matching xstr1 (" << j << ") mag=" << xstr1.atoms[j].spin << " to xstr2 (" << mapping_info.atom_map[j] << ") mag=" << xstr2.atoms[mapping_info.atom_map[j]].spin << endl;
          cerr << __AFLOW_FUNC__ << " magmom_diff: " << magmom_diff << endl;
          cerr << __AFLOW_FUNC__ << " spin structure 1 (signbit): " << std::signbit(xstr1.atoms[j].spin) << endl;
          cerr << __AFLOW_FUNC__ << " spin structure 2 (signbit): " << std::signbit(xstr2.atoms[mapping_info.atom_map[j]].spin) << endl;
        }
      ...
      // non-collinear
        ...
        const double magmom_diff = aurostd::modulus(xstr1.atoms[j].noncoll_spin - xstr2.atoms[mapping_info.atom_map[j]].noncoll_spin);
        const double angle_between_non_collinear_spins = rad2deg * aurostd::angle(xstr1.atoms[j].noncoll_spin, xstr2.atoms[mapping_info.atom_map[j]].noncoll_spin);
        if (magmom_diff > _ZERO_TOL_) { // to account for -0.0 vs 0.0
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " matching xstr1 (" << j << ") mag=" << xstr1.atoms[mapping_info.atom_map[j]].noncoll_spin << " to xstr2 (" << mapping_info.atom_map[j]
               << ") mag=" << xstr2.atoms[mapping_info.atom_map[j]].noncoll_spin << endl;
          cerr << __AFLOW_FUNC__ << " magmom_diff: " << magmom_diff << endl;
          cerr << __AFLOW_FUNC__ << " angle between two structures " << angle_between_non_collinear_spins << endl;
        }
      }
    }
...
namespace compare {
  double computeMisfitMagnetic(const structure_mapping_info& mapping_info) {
    return computeMisfitMagnetic(mapping_info.lattice_deviation, mapping_info.coordinate_displacement, mapping_info.failure, mapping_info.magnetic_displacement, mapping_info.magnetic_failure);
  }
} // namespace compare

namespace compare {
  double computeMisfitMagnetic(double dev, double dis, double fail, double mag_dis, double mag_fail) {
...
// ***************************************************************************
string XtalFinderCalculator::printAtomMappings(const structure_mapping_info& misfit_info, const xstructure& xstr1, const xstructure& xstr2) {
  // Print the atom mapping information: mapping indices, types, distances
  // and vectors

  stringstream output;
  stringstream index_map;
  stringstream element_map;

  output << std::setiosflags(std::ios::fixed | std::ios::left);

  // header line
  output << std::setw(15) << "Indices" << std::setw(15) << "Types" << std::setw(25) << "Distances (Angst.)"
         << "Mapping vector (Cart.)" << endl;

  // content
  for (size_t i = 0; i < misfit_info.atom_map.size(); i++) {
    index_map.str("");
    element_map.str("");
    ...
    output << std::setw(15) << index_map.str() << std::setw(15) << element_map.str() << std::setw(25) << aurostd::roundoff(misfit_info.distances_mapped[i], AUROSTD_ROUNDOFF_TOL)
...
namespace compare {
  string printAtomMappings(const structure_mapping_info& misfit_info) {
    ...
    // header line
    output << std::setw(15) << "Indices" << std::setw(25) << "Distances (Angst.)"
           << "Mapping vector (Cart.)" << endl;
    ...
    for (size_t i = 0; i < misfit_info.atom_map.size(); i++) {
      ...
      output << std::setw(15) << index_map.str() << std::setw(25) << misfit_info.distances_mapped[i] << misfit_info.vectors_mapped[i] << endl;
    }
    return output.str();
  }
} // namespace compare
...
// ***************************************************************************
string XtalFinderCalculator::printUnmatchedAtoms(const structure_mapping_info& misfit_info, const xstructure& xstr1, const xstructure& xstr2) {
  stringstream output;
...
namespace compare {
  xstructure GetLFASupercell(const xstructure& xstr, const xvector<int>& dims, const string& lfa_name) {
    ...
    for (size_t a = 0; a < xstr_LFA_only.atoms.size(); a++) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Number of LFAs in supercell: " << xstr_LFA_supercell.atoms.size() << endl;
    }
...
// ***************************************************************************
void XtalFinderCalculator::latticeSearch(structure_container& xstr_rep,
  ...
  if (scale_volume) {
    compare::atomicNumberDensity(xstr1, xstr2, match_info.rescale_factor);
  }

  // ensure cleared
  // xstr_match.misfit_info = compare::initialize_misfit_struct();
  const bool magnetic_analysis = (xstr1.atoms[0].spin_is_given || xstr1.atoms[0].noncoll_spin_is_given);
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " lattice dims : " << dims << endl;
  }

  // ---------------------------------------------------------------------------
  // peform supercell expansion on LFA atoms in structure2
  const xstructure xstr_LFA_supercell = compare::GetLFASupercell(xstr2, dims, lfa_str2);

  // ---------------------------------------------------------------------------
  // find possible translation vectors
  vector<xvector<double>> translation_vectors;
  findSimilarTranslationVectors(xstr1.lattice, xstr_LFA_supercell, xstr2, translation_vectors);
  ...
  buildSimilarLattices(translation_vectors, xstr1.lattice, lattices, latt_devs, optimize_match,
                       scale_volume); // DX20200422

  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Number of lattices to compare: " << lattices.size() << endl;
  }
  ...
  if (!lattices.empty()) {
    ...
    if (xstr_rep.nearest_neighbor_distances.empty()) { // use xstr_rep so we only calculate once
    ...
    if (DEFAULT_XTALFINDER_SUPERCELL_METHOD) { // default = false
      ...
      const xmatrix<double> f2c = xstr1.scale * trasp(xstr1.lattice); // DX+ME20210113 - calculate outside, fast

      for (size_t iat = 0; iat < xstr1.atoms.size(); iat++) {
    ...
    const uint number_of_structures = vstrs_matched.size();
    xthread::xThread xt(num_proc);
    std::function<bool(const uint, const uint, const xstructure&, const vector<double>&, const xstructure&, const string&, vector<xmatrix<double>>&, vector<structure_mapping_info>&, bool, bool)> search_atom_mappings =
    ...
    // ---------------------------------------------------------------------------
    // test origin shifts
    for (size_t y = 0; y < LFA_str2.size(); y++) {
      for (size_t x = 0; x < LFA_str1.size(); x++) {
        lfa_str1 = LFA_str1[x];
        lfa_str2 = LFA_str2[y];
        if (same_species && lfa_str1 != lfa_str2) {
          continue;
        }

        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " LFA (structure 1): " << lfa_str1 << endl;
          cerr << __AFLOW_FUNC__ << " LFA (structure 2): " << lfa_str2 << endl;
        }
        ...
        for (size_t i = 0; i < xstr1.atoms.size(); i++) {
          if (xstr1.atoms[i].name == lfa_str1) {
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " Searching for possible matching structures [THREADED VERSION]" << endl;
            }
            xt.runPredistributed(number_of_structures, search_atom_mappings, xstr1, all_nn1, xstr2, lfa_str2, lattices, vstrs_matched, same_species, optimize_match);
#else
            // ---------------------------------------------------------------------------
            // non-threaded
            // uint n=0;
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " Searching for possible matching structures [NON-THREADED VERSION]" << endl;
            }
            searchAtomMappings(start_index, end_index, xstr1, all_nn1,
                               xstr2, // DX20201208
                               lfa_str2, lattices, vstrs_matched, same_species, optimize_match);
#endif

            // ---------------------------------------------------------------------------
            // collect misfits and matching structure representations
            for (size_t p = 0; p < vstrs_matched.size(); p++) {
            ...
            if (match_info.misfit < misfit_match && !optimize_match) { // DX20220406 - 0.1 to misfit_match (tunable)
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " Found match (misfit = " << match_info.misfit << ")! Terminating search early." << endl;
              }
              return;
            }
            ...
            if (!optimize_match && aurostd::isequal(match_info.misfit, AUROSTD_MAX_DOUBLE)) {
              if (same_species) {
                test_one_origin_only = true;
              } // DX20190809 - need type match here; otherwise we may miss structure-type matches
              else {
                break;
              } // DX20201217 - move on to next LFA
            }
            if (test_one_origin_only) {
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " No mapping found. Searched only one origin. Terminating search early." << endl;
              }
              return;
            }
          }
        ...
        if (test_one_lfa_only) {
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " No match found. Searched only one LFA set. Terminating search early." << endl;
          }
          return;
        }
      }
...
namespace compare {
  xstructure supercell2newRepresentation(const xstructure& xstr_supercell, const xmatrix<double>& lattice) {
    ...
    for (size_t iat = 0; iat < proto.atoms.size(); iat++) {
    ...
    const xvector<double> abc_angles = Getabc_angles(proto.lattice, DEGREES);
    const xmatrix<double> clattice = GetClat(abc_angles);
    ...
    for (size_t j = 0; j < new_basis_2.size(); j++) {
      bool duplicate_lattice_point = false;
      for (size_t a = 0; a < new_basis.size(); a++) {
...
// ***************************************************************************
bool XtalFinderCalculator::searchAtomMappings(uint start_index,
                                              uint end_index,
  ...
  const bool calculate_magnetic_misfit = (_CALCULATE_MAGNETIC_MISFIT_ && ((xstr1.atoms[0].spin_is_given && xstr2.atoms[0].spin_is_given) || (xstr1.atoms[0].noncoll_spin_is_given && xstr2.atoms[0].noncoll_spin_is_given)));

  // ---------------------------------------------------------------------------
  // sort atom index by species frequency (speed increase)
  const vector<uint> atom_index_xstr1 = compare::atomIndicesSortedByFrequency(xstr1);
  ...
  for (uint p = start_index; p < end_index; p++) {
    ...
    // 2 = transformation method (default)
      ...
      compare::getLatticeTransformation(xstr2_tmp.lattice, xstr1.lattice, lattices[p], vstrs_matched[p].basis_transformation, vstrs_matched[p].rotation);

      // ---------------------------------------------------------------------------
      // now transform xstr2 into its new representation
      try {
        xstr2_tmp.TransformStructure(vstrs_matched[p].basis_transformation, vstrs_matched[p].rotation);
    ...
    const double minimum_interatomic_distance = aurostd::min(xstr1.dist_nn_min, xstr2_tmp.dist_nn_min); // DX20200622

    if (compare::sameSpecies(xstr2_tmp, xstr1, false)) {
      // ---------------------------------------------------------------------------
      // sort atom index by species frequency (speed increase)
      // this will remain the same regardless of origin shifts below
      const vector<uint> atom_index_xstr2 = compare::atomIndicesSortedByFrequency(xstr2_tmp);

      bool all_nn_calculated = false;
      xvector<double> shift_xstr2;
      for (size_t iat = 0; iat < xstr2_tmp.atoms.size(); iat++) {
        if (xstr2_tmp.atoms[iat].name == lfa) {
          ...
          // ---------------------------------------------------------------------------
          // find matched atoms
          if (findMatch(xstr1, xstr2_tmp, atom_index_xstr1, atom_index_xstr2, minimum_interatomic_distance, vstrs_matched[p], same_species)) {
            if (VERBOSE) {
              for (size_t m = 0; m < vstrs_matched[p].atom_map.size(); m++) {
            ...
            if (calculate_magnetic_misfit) {
              ...
              if (LDEBUG) {
                cerr << "with spin: mis,latt_dev,cd,f,mag_dis,mag_fail: " << vstrs_matched[p].magnetic_misfit << ", " << vstrs_matched[p].lattice_deviation << ", " << vstrs_matched[p].coordinate_displacement
                     << ", " << vstrs_matched[p].failure << ", " << vstrs_matched[p].magnetic_displacement << ", " << vstrs_matched[p].magnetic_failure << endl;
              }
            } else {
          ...
          // could not map atoms with this origin choice
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " Could not match atom positions. Try new origin choice." << endl;
            }
    ...
    // transformation does not yield commensurate atom counts
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Structure transformation does not yield commensurate atom counts:"
             << " ref=" << aurostd::joinWDelimiter(xstr1.num_each_type, ",") << " vs test=" << aurostd::joinWDelimiter(xstr2_tmp.num_each_type, ",") << endl;
      }
    }
  }
  return true;
...
// ***************************************************************************
namespace compare {
  bool consistentAtomMappingType(const _atom& atom1, const _atom& atom2, uint index_x1, uint index_x2, bool same_species, bool is_collinear, bool is_non_collinear) {
    const bool VERBOSE = false;
    ...
    if (same_species && (atom1.name != atom2.name)) {
      if (VERBOSE) {
        ...
        cerr << __AFLOW_FUNC__ << " WARNING: Matching species are not the same type, throwing out match (same species comparison)" << endl;
      }
      return false;
    }
    ...
    if (!_CALCULATE_MAGNETIC_MISFIT_) {
      ...
      if (is_collinear && (aurostd::abs(atom1.spin - atom2.spin) > _SPIN_TOL_)) {
        if (VERBOSE) {
          cerr << __AFLOW_FUNC__ << " WARNING: Matching atoms do not have the same collinear spin, throwing out match" << endl;
        }
        return false;
      }
      // check non_collinear spin
      else if (is_non_collinear && (aurostd::abs(atom1.noncoll_spin(1) - atom2.noncoll_spin(1)) > _SPIN_TOL_ || aurostd::abs(atom1.noncoll_spin(2) - atom2.noncoll_spin(2)) > _SPIN_TOL_ ||
                                    aurostd::abs(atom1.noncoll_spin(3) - atom2.noncoll_spin(3)) > _SPIN_TOL_)) {
        if (VERBOSE) {
          cerr << __AFLOW_FUNC__ << " WARNING: Matching atoms do not have the same non-collinear spin, throwing out match" << endl;
        }
        return false;
      }
    }
...
// ***************************************************************************
namespace compare {
  bool consistentAtomMappingIndex(uint index1, uint index2, const vector<uint>& index1_list, const vector<uint>& index2_list) {
    const bool VERBOSE = false;
    ...
    if (aurostd::WithinList(index1_list, index1)) {
      if (VERBOSE) {
        cerr << "WARNING: STRUCTURE 1: index " << index1 << " has already been mapped (stored indices: " << aurostd::joinWDelimiter(index1_list, ",") << endl;
      }
      return false;
    }
    // ---------------------------------------------------------------------------
    // check if index in structure 2 has been mapped to already
      ...
      if (VERBOSE) {
        cerr << __AFLOW_FUNC__ << " WARNING: STRUCTURE 2: index " << index2 << " has already been mapped (stored indices: " << aurostd::joinWDelimiter(index2_list, ",") << endl;
      }
      return false;
    }
    return true;
...
// ***************************************************************************
namespace compare {
  bool consistentAtomSetMappings(const string& atom1_name, const string& atom2_name, const vector<string>& vatoms1_name, const vector<string>& vatoms2_name) {
    const bool VERBOSE = false;

    for (size_t i = 0; i < vatoms1_name.size(); i++) {
      if (atom1_name == vatoms1_name[i] && atom2_name != vatoms2_name[i]) {
        if (VERBOSE) {
          cerr << __AFLOW_FUNC__ << " WARNING: Matching one type of atom to more than one type: " << atom1_name << " == " << atom2_name << " | " << vatoms1_name[i] << " == " << vatoms2_name[i] << endl;
        }
        return false;
      }
    }
...
// ***************************************************************************
void XtalFinderCalculator::findSimilarTranslationVectors(const xmatrix<double>& q1, const xstructure& xstr_LFA_supercell, const xstructure& xstr, vector<xvector<double>>& lattice_vecs) {
  ...
  const bool relative_tolerance = true;
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Lattice parameters: " << aurostd::modulus(q1(1)) << ", " << aurostd::modulus(q1(2)) << ", " << aurostd::modulus(q1(3)) << endl;
    cerr << __AFLOW_FUNC__ << " Modulus search range for lattice vector a: " << min_q1_a << " - " << max_q1_a << endl;
    cerr << __AFLOW_FUNC__ << " Modulus search range for lattice vector b: " << min_q1_b << " - " << max_q1_b << endl;
    cerr << __AFLOW_FUNC__ << " Modulus search range for lattice vector c: " << min_q1_c << " - " << max_q1_c << endl;
  }
  ...
  const uint natoms_lfa_supercell = xstr_LFA_supercell.atoms.size(); // DX20201221 - reduce cost in loop
  for (uint i = 0; i < natoms_lfa_supercell; i++) {
    tmp_vec = xstr_LFA_supercell.atoms[i].cpos;
    tmp_mod = aurostd::modulus(tmp_vec);
    if ((tmp_mod <= max_q1_a && tmp_mod >= min_q1_a) || (tmp_mod <= max_q1_b && tmp_mod >= min_q1_b) || (tmp_mod <= max_q1_c && tmp_mod >= min_q1_c)) {
      lattice_vecs.push_back(tmp_vec);
    }
  }

  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Number of potential lattice vectors: " << lattice_vecs.size() << endl;
  }
  ...
  if (aurostd::min(xstr.num_each_type) != 1) {
    ...
    for (size_t i = 0; i < lattice_vecs.size(); i++) {
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Number of lattice vectors (preserves periodicity): " << lattice_vecs.size() << endl;
    for (size_t i = 0; i < lattice_vecs.size(); i++) {
      cerr << __AFLOW_FUNC__ << " lattice vector " << i << ": " << lattice_vecs[i] << " (" << aurostd::modulus(lattice_vecs[i]) << ")" << endl;
    }
  }
}
...
// ***************************************************************************
bool XtalFinderCalculator::buildSimilarLattices(const vector<xvector<double>>& translation_vectors, const xmatrix<double>& q1, vector<xmatrix<double>>& lattices, vector<double>& latt_devs, bool optimize_match, bool scale_volume) const {
  const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_COMPARE_);
  const bool VERBOSE = false;

  // ---------------------------------------------------------------------------
  // sort via smallest misfit for speed up
  const bool sort_via_lattice_deviation = true; // DX20190626 - speed increase
  const bool relative_tolerance = false; // DX20190703
  ...
  double tol_vol = 0.1;
  const double abs_det_q1 = std::abs(det(q1)); // volume //DX20201130
  if (scale_volume) {
    tol_vol = (1.0 / 3.0);
  }
  const double det_tol = tol_vol * abs_det_q1;
  ...
  if (LDEBUG) {
    if (relative_tolerance) {
      cerr << __AFLOW_FUNC__ << " Tolerance for a (Angstroms): " << tol_a << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for b (Angstroms): " << tol_b << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for c (Angstroms): " << tol_c << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for alpha (degrees): " << abc_angles_q1[4] * 0.3 << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for beta (degrees): " << abc_angles_q1[5] * 0.3 << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for gamma (degrees): " << abc_angles_q1[6] * 0.3 << endl;
    } else {
      cerr << __AFLOW_FUNC__ << " Tolerance for a (Angstroms): " << tol_a << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for b (Angstroms): " << tol_b << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for c (Angstroms): " << tol_c << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for alpha (degrees): " << 5 << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for beta (degrees): " << 5 << endl;
      cerr << __AFLOW_FUNC__ << " Tolerance for gamma (degrees): " << 5 << endl;
    }
  }
  ...
  for (uint i = 0; i < n_translations; i++) {
    ...
    if (std::abs(translations_mod[i] - abc_angles_q1[1]) < tol_a) { // check a
      for (uint j = 0; j < n_translations; j++) {
        if (j != i) {
          // ---------------------------------------------------------------------------
          // check lattice vector length: b
          if (std::abs(translations_mod[j] - abc_angles_q1[2]) < tol_b) { // check b
            for (uint k = 0; k < n_translations; k++) {
              if (k != i && k != j) {
                // ---------------------------------------------------------------------------
                // check lattice vector length: c
                if (std::abs(translations_mod[k] - abc_angles_q1[3]) < tol_c) { // check c
                  tmp_lattice = SYM::xvec2xmat(translation_vectors[i], translation_vectors[j], translation_vectors[k]);
                  // ---------------------------------------------------------------------------
                  // check determinant
                  if (std::abs(abs_det_q1 - std::abs(det(tmp_lattice))) < det_tol) { // check determinant/volume
                    const xvector<double> abc_angles_q2 = Getabc_angles(tmp_lattice, DEGREES);
                    // ---------------------------------------------------------------------------
                    // check angles
                    ...
                    if (compare::similarLatticeParameters(abc_angles_q1, abc_angles_q2)) {
                      const double tmp_latt_dev = compare::checkLatticeDeviation(abs_det_q1, tmp_lattice, D1, F1);
                      // ---------------------------------------------------------------------------
                      ...
                      if ((!optimize_match && tmp_latt_dev <= misfit_match) ||
                          (optimize_match && tmp_latt_dev <= misfit_family)) { // fast match doesn't care about finding same family information //DX20190318 - removed unique since it doesn't exist yet
                        // ---------------------------------------------------------------------------
                        ...
                        for (size_t t = 0; t < lattices.size(); t++) {
  ...
  if (VERBOSE) {
    ...
    for (size_t i = 0; i < lattices.size(); i++) {
      // DX20201130 cerr << __AFLOW_FUNC__ << endl << " lattice: " << endl << lattices[i] << endl << " clattice: " << clattices[i] << endl << " volume: " << det(lattices[i]) << endl << " lattice deviation: " << endl << latt_devs[i] << endl;
      cerr << __AFLOW_FUNC__ << endl << " lattice: " << endl << lattices[i] << endl << " volume: " << det(lattices[i]) << endl << " lattice deviation: " << endl << latt_devs[i] << endl;
      const xvector<double> abc_angles_q2 = Getabc_angles(lattices[i], DEGREES);
...
namespace compare {
  void getLatticeTransformations(const xmatrix<double>& lattice_original,
    ...
    for (size_t i = 0; i < candidate_lattices.size(); i++) {
      getLatticeTransformation(lattice_original, lattice_ideal, candidate_lattices[i], basis_transformation, rotation);
...
// ***************************************************************************
namespace compare {
  void getLatticeTransformation(const xmatrix<double>& lattice_original, const xmatrix<double>& lattice_ideal, const xmatrix<double>& candidate_lattice, xmatrix<double>& basis_transformation, xmatrix<double>& rotation) {
    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_COMPARE_);
    ...
    const xmatrix<double> metric_tensor_original = MetricTensor(lattice_original);
    const xmatrix<double> metric_tensor_ideal = MetricTensor(lattice_ideal);
    ...
    const xmatrix<double> metric_tensor_candidate = MetricTensor(candidate_lattice);
    const xmatrix<double> basis_transformation_tmp = GetBasisTransformation(lattice_original, candidate_lattice);
    const double theta = acos((aurostd::trace(basis_transformation_tmp) - 1.0) / 2.0);

    if (!aurostd::identical(metric_tensor_original, metric_tensor_candidate) || aurostd::det(basis_transformation_tmp) < 0.0 ||
        !aurostd::isidentity(basis_transformation_tmp * trasp(basis_transformation_tmp)) || aurostd::isequal(theta, PI, 1e-2)) {
      // ---------------------------------------------------------------------------
      ...
      const xmatrix<double> lattice_new = basis_transformation * lattice_original;

      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " lattice new: " << lattice_new << endl;
        cerr << __AFLOW_FUNC__ << " lattice ideal: " << lattice_ideal << endl;
        cerr << __AFLOW_FUNC__ << " basis transformation: " << basis_transformation << endl;
        cerr << __AFLOW_FUNC__ << " det(basis transformation): " << aurostd::det(basis_transformation) << endl;

        // ---------------------------------------------------------------------------
        // calculate the metric tensors, they should be equal after the basis
        // transformation
        const xmatrix<double> metric_tensor_new = MetricTensor(lattice_new);
        const xmatrix<double> metric_tensor_ideal = MetricTensor(lattice_ideal);
        cerr << __AFLOW_FUNC__ << " metric_tensor_new: " << metric_tensor_new << endl;
        cerr << __AFLOW_FUNC__ << " metric_tensor_ideal: " << metric_tensor_ideal << endl;
      }

      // ---------------------------------------------------------------------------
      // then rotate to the ideal lattice
      const xmatrix<double> rotation_tmp = trasp(GetRotation(lattice_new, lattice_ideal)); // use trasp for AFLOW convention
    ...
    // if the metric tensors ARE equal: simple rotation between lattices
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " rotation only (no basis transformation)!" << endl;
      }
      ...
      // we can differentiate this with a polar decomposition T=R*U
      // T: original matrix, R: pure rotation, U: deformation matrix
      const xmatrix<double> rotation_tmp = trasp(GetRotation(lattice_original, lattice_ideal)); // use trasp for AFLOW convention
      aurostd::polarDecomposition(rotation_tmp, rotation, deformation);

      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " rotation: " << rotation << endl;
      }
    }
  }
} // namespace compare
...
// ***************************************************************************
namespace compare {
  vector<xstructure> getTransformedStructures(const xstructure& xstr, const vector<xmatrix<double>>& basis_transformations, const vector<xmatrix<double>>& rotations) {
    // bool LDEBUG=(false || XHOST.DEBUG || _DEBUG_COMPARE_);

    vector<xstructure> vxstrs_transformed;
    const xstructure xstr_transformed_tmp;

    for (size_t i = 0; i < basis_transformations.size(); i++) {
...
// ***************************************************************************
void XtalFinderCalculator::calculatePrototypeDesignations(vector<StructurePrototype>& prototypes, uint num_proc) {
  const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_COMPARE_);
  stringstream message;
  ...
  xthread::xThread xt(num_proc);
  std::function<void(vector<StructurePrototype>::iterator&)> fn = std::bind(&XtalFinderCalculator::getPrototypeDesignations, this, _1);
...
void XtalFinderCalculator::getPrototypeDesignations(vector<StructurePrototype>::iterator& proto) {
...
// ***************************************************************************
void XtalFinderCalculator::getPrototypeDesignationsPreDistributed(vector<StructurePrototype>& prototypes, uint start_index, uint end_index) {
  // If end index is greater than prototypes.size(), then
  // determine prototoype designation for all structures
  if (end_index > prototypes.size()) {
    end_index = prototypes.size();
  }
...
// ***************************************************************************
void XtalFinderCalculator::calculateMatchingAFLOWPrototypes(vector<StructurePrototype>& prototypes, uint num_proc) {
  // bool LDEBUG=(false || XHOST.DEBUG || _DEBUG_COMPARE_);
  stringstream message;
  ...
  const bool quiet_orig = XHOST.QUIET;
  XHOST.QUIET = true;

  const uint number_of_structures = prototypes.size();
#ifdef AFLOW_MULTITHREADS_ENABLE
  // split task into threads
  xthread::xThread xt(num_proc);
  std::function<void(uint, vector<StructurePrototype>&, const aurostd::xoption&)> fn = std::bind(&XtalFinderCalculator::getMatchingAFLOWPrototypes, this, _1, _2, _3);
...
// ***************************************************************************
void XtalFinderCalculator::getMatchingAFLOWPrototypes(uint i, vector<StructurePrototype>& prototypes, const aurostd::xoption& vpflow_protos) {
  ...
  if (!matching_protos.empty()) {
    for (size_t j = 0; j < matching_protos[0].structures_duplicate.size(); j++) {
...
// ******************************************************************************
void XtalFinderCalculator::writeComparisonOutputFile(const string& output, const string& directory, filetype format, output_file_xtalfinder comparison_mode, bool same_species) {
  ...
  if (same_species) {
    ...
    switch (comparison_mode) {
      case (compare_input_xf): file_prefix = DEFAULT_XTALFINDER_FILE_MATERIAL; break;
      case (duplicate_compounds_xf): file_prefix = DEFAULT_XTALFINDER_FILE_DUPLICATE; break;
    ...
    switch (comparison_mode) {
      case (compare_input_xf): file_prefix = DEFAULT_XTALFINDER_FILE_STRUCTURE; break;
      case (duplicate_compounds_xf): break;
      case (compare2database_xf):
  ...
  // information
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
    message << output;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_RAW_);
  }
}
...
namespace compare {
  void writePythonScript(ostream& oss) {
    ...
    const string xtalfinder_python_subdir = "AFLOW_XTALFINDER_PYTHON";
    const string python_dir = directory + "/" + xtalfinder_python_subdir;
    ...
    aurostd::EmbData::save_to_file("aflow_xtalfinder_python.py", "SCRIPTS", python_dir + "/" + "aflow_xtalfinder_python.py");
  }
} // namespace compare
