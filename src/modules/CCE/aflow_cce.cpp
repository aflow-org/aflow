...
namespace cce {
  ...
  void run(aurostd::xoption& flags, ostream& oss) { // CO20201105
    if (aurostd::toupper(flags.getattachedscheme("CCE_CORRECTION::PRINT")) == "PYTHON") {
      const string directory = aurostd::getPWD(); // CO20201126 //"."; //can change later with a flag input
      const string aflow_cce_python_subdir = "AFLOW_CCE_PYTHON";
      const string python_directory = directory + '/' + aflow_cce_python_subdir;
      ...
      aurostd::EmbData::save_to_file("aflow_cce_python.py", "SCRIPTS", python_directory + '/' + "aflow_cce_python.py");
      // print CCE citation
      oss << print_citation();
    } else {
      if (flags.flag("CCE_CORRECTION")) {
        cce::print_corrections(flags, oss);
      }
    }
  }
  void run(aurostd::xoption& flags, std::istream& ist, ostream& oss) {  // CO20201105
    if (aurostd::toupper(flags.getattachedscheme("CCE_CORRECTION::PRINT")) == "PYTHON") {
      ...
      if (flags.flag("CCE_CORRECTION::POSCAR2CCE")) {
        cce::print_corrections(flags, ist, oss);
      }
      if (flags.flag("CCE_CORRECTION::GET_CCE_CORRECTION")) {
        cce::print_corrections(flags, ist, oss);
      }
      if (flags.flag("CCE_CORRECTION::GET_OXIDATION_NUMBERS")) {
        cce::print_oxidation_numbers(flags, ist, oss);
      }
      if (flags.flag("CCE_CORRECTION::GET_CATION_COORDINATION_NUMBERS")) {
        cce::print_cation_coordination_numbers(flags, ist, oss);
      }
    }
  }
  // CO20201105 END
  ...
  void print_corrections(xstructure& structure, aurostd::xoption& flags, ostream& oss) {  // CO20201105
    ...
    /********************************************************/
    get_dft_form_enthalpies_functionals(flags.getattachedscheme("CCE_CORRECTION::ENTHALPIES_FORMATION_DFT"), flags.getattachedscheme("CCE_CORRECTION::FUNCTIONALS"),
                                        cce_vars); // provide precalc. DFT formation enthalpies & corresponding functionals
  ...
  void print_corrections(xstructure& structure, aurostd::xoption& flags, aurostd::xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    if (cce_flags.flag("CORRECTABLE")) {
      ...
      const uint num_funcs = cce_vars.vfunctionals.size();
      const uint num_temps = cce_vars.vtemperatures.size();
      for (uint k = 0, ksize = num_funcs; k < ksize; k++) { // looping over and checking of vfunctionals is necessary to ensure correct correspondence between given formation enthalpies [k] and corrections with respect to the functional
        if (cce_vars.vfunctionals[k] != "exp") {
          if (!cce_vars.enthalpies_dft.empty()) { // only when precalculated DFT formation enthalpies are provided, calculate corrected values
            for (uint l = 0; l < num_temps; l++) {
              cce_vars.enthalpy_formation_cell_cce[num_temps * k + l] =
                  cce_vars.enthalpies_dft[k] - cce_vars.cce_correction[num_temps * k + l]; // num_temps*k since to each functional belong num_temps corrections (currently for 298.15 and 0K)
            }
          }
  ...
  void print_cation_coordination_numbers(aurostd::xoption& flags, std::istream& ist, ostream& oss) {
    ...
    // read structure
    const xstructure structure = read_structure(ist);
    ...
    if (cce_flags.flag("MULTI_ANION_SYSTEM")) {
      for (size_t k = 0, ksize = cce_vars.multi_anion_species.size(); k < ksize; k++) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " getting neighbors for multi anion species " << k << " (" << cce_vars.multi_anion_species[k] << ")" << endl;
        }
  ...
  void print_oxidation_numbers(aurostd::xoption& flags, std::istream& ist, ostream& oss) {
    ...
    // initialise flags and variables
    const aurostd::xoption cce_flags = init_flags();
    CCE_Variables cce_vars = init_variables(structure);
  ...
  vector<double> calculate_corrections(const string& directory_path) {
    ...
    const xstructure structure = read_structure(poscar); // AFLOW seems to automatically unzip and rezip zipped files so that only the file name without zipping extension needs to be given
    ...
    const string functional = get_functional_from_aflow_in_outcar(structure, aflowin_file, outcar_file);
    if (functional.empty()) {
  ...
  vector<double> calculate_corrections(const xstructure& structure, string functional, const string& directory_path, ostream& oss) {
    ofstream FileMESSAGE;
    return calculate_corrections(structure, functional, FileMESSAGE, directory_path, oss);
  } // functional needed as input when determining oxidation numbers from electronegativities
  ...
  void CCE_core(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, vector<vector<uint>>& multi_anion_num_neighbors, const string& directory_path) {
    ...
    cce_vars.vtemperatures = CCE_vtemperatures;
    const uint num_temps = cce_vars.vtemperatures.size();
    ...
    if (cce_flags.flag("CORRECTABLE")) {
      ...
      if (cce_flags.flag("MULTI_ANION_SYSTEM")) {
        for (size_t k = 0, ksize = cce_vars.multi_anion_species.size(); k < ksize; k++) {
          if (LDEBUG) {
            cerr << endl;
            cerr << __AFLOW_FUNC__ << " getting corrections for multi anion species " << k << " (" << cce_vars.multi_anion_species[k] << ")" << endl;
          }
    ...
    if (cce_flags.flag("CORRECTABLE")) {
      ...
      for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) {
        if ((structure.atoms[i].cleanname != cce_vars.anion_species) && (cce_vars.multi_anion_atoms[i] != 1)) { // exclude main anion species and multi anion atoms detected previously
          if (cce_vars.num_neighbors[i] > 0) { // are there actually bonds between the cation and the (main) anion species
            ...
            for (uint k = 0; k < num_funcs; k++) {
              if (cce_vars.vfunctionals[k] != "exp") {
                for (uint l = 0; l < num_temps; l++) {
                  cce_vars.cce_correction[num_temps * k + l] +=
                      (cce_vars.num_neighbors[i] * cce_vars.corrections_atom[num_temps * k + l][i]); // num_temps*k since to each functional belong num_temps corrections (currently for 298.15 and 0K)
                }
      ...
      if (cce_flags.flag("MULTI_ANION_SYSTEM")) {
        for (size_t k = 0, ksize = cce_vars.multi_anion_species.size(); k < ksize; k++) {
          if (LDEBUG) {
            cerr << endl;
            cerr << __AFLOW_FUNC__ << " adding corrections for multi anion species " << k << " (" << cce_vars.multi_anion_species[k] << ")" << endl;
          }
          for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) {
            if ((structure.atoms[i].cleanname != cce_vars.anion_species) && (cce_vars.multi_anion_atoms[i] != 1)) { // exclude main anion species and multi anion atoms detected previously
  ...
  void get_dft_form_enthalpies_functionals(const string& enthalpies_dft_input_str, const string& functionals_input_str, CCE_Variables& cce_vars) {
    ...
    if (functionals_input_str.empty() && cce_vars.enthalpies_dft.size() == 1) {
      ...
      cce_vars.vfunctionals.emplace_back("PBE");
    ...
    if (LDEBUG) {
      const bool roff = false;
      cerr << __AFLOW_FUNC__ << " INPUT DFT FORMATION ENTHALPIES & FUNCTIONALS:" << endl;
      cerr << __AFLOW_FUNC__ << "  input dft formation enthalpies=" << aurostd::joinWDelimiter(aurostd::vecDouble2vecString(cce_vars.enthalpies_dft, 6, roff), ",") << " (assumed to be in eV/cell)" << endl;
      cerr << __AFLOW_FUNC__ << "  input functionals=" << functionals_input_str << endl;
    }
    ...
    for (uint k = 0, ksize = num_funcs; k < ksize; k++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " cce_vars.vfunctionals[" << k << "]: " << cce_vars.vfunctionals[k] << endl;
      }
    ...
    if (functionals_input_str.empty() && enthalpies_dft_input_str.empty()) {
      for (size_t k = 0, ksize = CCE_vdefault_output_functionals.size(); k < ksize; k++) {
        if (get_offset(CCE_vdefault_output_functionals[k]) == -1) {
          message << " Unknown functional " << cce_vars.vfunctionals[k] << ". Please choose PBE, LDA, or SCAN.";
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_);
        }
        cce_vars.vfunctionals.push_back(CCE_vdefault_output_functionals[k]);
        cce_vars.offset.push_back(get_offset(CCE_vdefault_output_functionals[k]));
      }
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " PBE: " << aurostd::WithinList(cce_vars.vfunctionals, "PBE") << endl;
      cerr << __AFLOW_FUNC__ << " LDA: " << aurostd::WithinList(cce_vars.vfunctionals, "LDA") << endl;
      cerr << __AFLOW_FUNC__ << " SCAN: " << aurostd::WithinList(cce_vars.vfunctionals, "SCAN") << endl;
      cerr << __AFLOW_FUNC__ << " PBE+U:ICSD: " << aurostd::WithinList(cce_vars.vfunctionals, "PBE+U:ICSD") << endl;
      cerr << __AFLOW_FUNC__ << " exp: " << aurostd::WithinList(cce_vars.vfunctionals, "exp") << endl;
      cerr << endl;
    }
  }
  ...
  int get_offset(const string& functional) {
    if (functional == "PBE") {
      return 0;
    } else if (functional == "LDA") {
      return 2;
    } else if (functional == "SCAN") {
      return 4;
    } else if (functional == "PBE+U:ICSD") {
      return 6;
    } else if (functional == "exp") {
      return 8;
    } else {
      return -1;
    }
  ...
  string get_functional_from_aflow_in_outcar(const xstructure& structure, string& aflowin_file, string& outcar_file) {
    stringstream message;
    string functional;
    const string aflowIn = aurostd::RemoveComments(aurostd::file2string(aflowin_file));
    vector<string> vlines = aurostd::string2vectorstring(aflowIn);
    string line_a;
    bool pbe = false;
    const bool ldau = false;
    ...
    if (!pbe && !lda) {
      if (aurostd::FileExist(outcar_file) || aurostd::CompressFileExist(outcar_file)) {
        xOUTCAR outcar;
        outcar.GetPropertiesFile(outcar_file);
        uint pbe_count = 0;
        uint lda_count = 0;
        for (size_t i = 0; i < outcar.species_pp_type.size(); i++) {
          if (outcar.species_pp_type[i].find("PAW_PBE") != string::npos) {
            pbe_count += 1;
          } else if (outcar.species_pp_type[i].find("PAW_LDA") != string::npos) {
            lda_count += 1;
          }
        }
    ...
    if (!vflags.KBIN_VASP_LDAU_PARAMETERS.empty() && vflags.KBIN_VASP_FORCE_OPTION_LDAU2.isentry && !vflags.KBIN_VASP_FORCE_OPTION_LDAU_ADIABATIC.isentry && !vflags.KBIN_VASP_FORCE_OPTION_LDAU_CUTOFF.isentry) {
      ...
      // get species
      const string species_part = ldau_params_vector[0];
      ...
      // get Us
      const string Us_part = ldau_params_vector[2];
      const vector<string> Us_string_vector;
      ...
      for (size_t k = 0; k < species_vector.size(); k++) {
      ...
      for (size_t k = 0; k < species_vector.size(); k++) {
        if (Us_vector[k] != standard_ICSD_Us_vector[k]) {
          message << " BAD NEWS: For this DFT+U calculation with Dudarev's method the provided U value of " << Us_vector[k] << " eV for " << species_vector[k] << " does not match the standard value of "
                  << standard_ICSD_Us_vector[k] << " eV. There are no corrections for this case.";
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_);
        }
      }
      pbe_u_icsd = true;
    ...
    if ((vflags.KBIN_VASP_FORCE_OPTION_LDAU1.isentry || vflags.KBIN_VASP_FORCE_OPTION_LDAU2.isentry) && !pbe_u_icsd) {
      message << " BAD NEWS: It seems you are providing an aflow.in for a DFT+U calculation with different parameters than for the AFLOW ICSD database (Dudarev's approach, LDAU2=ON). There are no corrections "
                 "for this case.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_);
    }
  ...
  string determine_anion_species(const xstructure& structure, CCE_Variables& cce_vars) {
    ...
    stringstream message;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " ANION SPECIES FROM ALLEN ELECTRONEGATIVITIES:" << endl;
    }
    const _atom atom;
    uint z = 0;
    ...
    if (LDEBUG) {
      ...
      cerr << __AFLOW_FUNC__ << " anion species: " << cce_vars.anion_species << endl;
      cerr << __AFLOW_FUNC__ << " anion charge: " << cce_vars.standard_anion_charge << endl;
      cerr << endl;
    }
  ...
  vector<uint> check_for_multi_anion_system(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, double tolerance) {
    ...
    stringstream message;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " CHECKING FOR MULTI-ANION SYSTEM:" << endl;
    }
    for (size_t i = 0; i < structure.atoms.size(); i++) { // initialize elements of vector to 0
      cce_vars.multi_anion_atoms[i] = 0;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " structure=" << endl;
      cerr << structure << endl;
    }

    // CO20200914 - START
    if (cce_vars.xstr_neighbors.atoms.empty() || cce_vars.i_neighbors.size() != structure.atoms.size() || cce_vars.i_neighbors.size() != cce_vars.distances.size()) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " NEIGHBORS ANALYSIS START" << endl;
      }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " NEIGHBORS ANALYSIS STOP" << endl;
      }
    }
    // CO20200914 - END
    ...
    double cutoff = 0.0;
    double electronegativity_atom = 0.0;
    double electronegativity_neighbor = 0.0;
    xelement::xelement atom_element;
    xelement::xelement neigh_element;
    for (i = 0, isize = cce_vars.i_neighbors.size(); i < isize; i++) { // same size as structure.atoms.size(); number of atoms in the structure (not determined by cutoff (or cutoffs_max))
      ...
      for (j = 0, jsize = cce_vars.i_neighbors[i].size(); j < jsize; j++) { // number of nearest neighbors within cutoff of atom i; number of neighbors of each atom i determined by cutoffs_max
        if (cce_vars.distances[i][j] > cutoff) {
          break;
        } // we only need first nearest neighbors
        ...
        if (cce_vars.distances[i][j] <= cutoff) { // distance must be larger than DEFAULT_CCE_SELF_DIST_TOL since GetStrNeighData includes also structure.atoms[i] itself as neighbor having distance zero to itself
          if (atom.cleanname == cce_vars.anion_species) {
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " electronegativity of atom " << i << ": " << electronegativity_atom << endl;
              cerr << __AFLOW_FUNC__ << " electronegativity of neighbor " << j << ": " << electronegativity_neighbor << endl;
            }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " multi_anion_count for atom " << i << " (" << structure.atoms[i].cleanname << "): " << multi_anion_count << endl;
        cerr << __AFLOW_FUNC__ << " neighbors_count for atom " << i << " (" << structure.atoms[i].cleanname << "): " << neighbors_count << endl;
      }
      if ((multi_anion_count == neighbors_count) && (structure.atoms[i].cleanname != cce_vars.anion_species)) { // anion_species should not be detected again as multi_anion_species
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " Atom " << i << " (" << structure.atoms[i].cleanname << ") has been detected as a multi-anion atom." << endl;
        }
        // set multi anion species
        uint multi_anion_species_count = 0;
        for (size_t k = 0, ksize = cce_vars.multi_anion_species.size(); k < ksize; k++) { // have to make sure that no anion species is added twice
        ...
        if (multi_anion_species_count == cce_vars.multi_anion_species.size()) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " New multi anion species: " << structure.atoms[i].cleanname << endl;
          }
          if (structure.atoms[i].cleanname == "O") { // check whether one of the multi anion species is O for which per/superoxide test needs to be made
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " Oxygen was detected as multi anion species, i.e. system needs to be tested for (su-)peroxide ions." << endl;
            }
          }
        }
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " Oxidation state for atom " << i << " (" << structure.atoms[i].cleanname << ") has been set to: " << cce_vars.oxidation_states[i] << endl;
        }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number of multi anion species (1-total_number_anion_species since the main anion species with largest electronegativity is NOT counted as multi anion species): "
           << cce_vars.multi_anion_species.size() << endl;
      for (size_t k = 0, ksize = cce_vars.multi_anion_species.size(); k < ksize; k++) {
        cerr << __AFLOW_FUNC__ << " multi anion species " << k << ": " << cce_vars.multi_anion_species[k] << endl;
      }
      cerr << endl;
    }
  ...
  // get_num_neighbors////////////////////////////////////////////////////////
  //  trying to get the number of neighbors for each atom within respective species selective cutoff
  vector<uint> get_num_neighbors(const xstructure& structure,
                                 const string& anion_species,
                                 xoption& cce_flags,
                                 CCE_Variables& cce_vars,
                                 double tolerance) { // anion_species here cannot be taken from cce_vars since function is also used to determine multi anion num_neighbors for which anion_species is the respective multi_anion_species
    const bool LDEBUG = (false || XHOST.DEBUG || CCE_DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " STRUCTURAL ANALYSIS:" << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " structure=" << endl;
      cerr << structure << endl;
    ...
    for (i = 0, isize = cce_vars.i_neighbors.size(); i < isize; i++) { // same size as structure.atoms.size(); number of atoms in the structure (not determined by cutoff (or cutoffs_max))
          ...
          if (!anion_species.empty()) { // variable called anion type since function was developed for CCE for polar materials but it can be used to check for any atom type and only include those as neighbors
            ...
            if (atom.cleanname == anion_species) {
              ...
              if (!cce_flags.flag("MULTI_ANION_SYSTEM")) {
                ...
                other_neighbors << "Not all nearest neighbors of " << structure.atoms[i].cleanname << " (ATOM[" << i << "]) within the distance tolerance of " << tolerance << " Ang. are " << anion_species
                                << ", there is also " << atom.cleanname << endl;
              }
            }
          } else {
            neighbors_count += 1;
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " number of " << anion_species << " nearest neighbors within " << tolerance << " Ang. tolerance of " << structure.atoms[i].cleanname << " (ATOM[" << i
             << "]): " << num_neighbors[i] << endl;
        cerr << endl;
      }
    }
  ...
  vector<double> get_dist_cutoffs(const xstructure& structure) {
    ...
    for (int i = 1, isize = distsij.rows + 1; i < isize; i++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " nearest neighbor distance for species " << i << " is " << near_neigh_dist[i - 1] << " Ang." << endl;
      }
  ...
  void check_per_super_oxides(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars) {
    ...
    stringstream message;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " CHECKING FOR (SU-)PEROXIDES:" << endl;
    }
    uint perox_count = 0;
    uint superox_count = 0;
    for (size_t i = 0; i < structure.atoms.size(); i++) { // initialize elements of vectors to 0
    ...
    for (i = 0, isize = cce_vars.i_neighbors.size(); i < isize; i++) { // same size as structure.atoms.size(); number of atoms in the structure (not determined by cutoff (or cutoffs_max))
      if (structure.atoms[i].cleanname == "O") { // identify per- and superoxides by O-O bond length
        for (j = 0, jsize = cce_vars.i_neighbors[i].size(); j < jsize; j++) { // number of nearest neighbors within cutoff of atom i; number of neighbors of each atom i determined by the cutoffs_max
          if (cce_vars.distances[i][j] > cce_vars.distances[i][0]) {
            break;
          } // we only need first nearest neighbors
          const _atom& atom = cce_vars.xstr_neighbors.grid_atoms[cce_vars.i_neighbors[i][j]];
          if (atom.cleanname == "O") {
            if (cce_vars.distances[i][j] <= DEFAULT_CCE_O2_MOLECULE_LOWER_CUTOFF) { // distance must be larger than DEFAULT_CCE_SELF_DIST_TOL to savely exclude the anion itself having distance zero to itself; if O-O bond is shorter than in O2 molecule (approx. 1.21 Ang) the result of the structural relaxation is most likely wrong
              ...
              message << " THE DETERMINED OXYGEN-OXYGEN BOND LENGTH IS ABOUT THE SAME AS IN THE O2 MOLECULE, I.E. THE STRUCTURE SEEMS TO INCLUDE MOLECULAR OXYGEN FOR WHICH NO CCE CORRECTION IS AVAILABLE! THE "
                         "O-O BOND LENGTH IS: "
                      << cce_vars.distances[i][j] << " Ang.";
              throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_);
              ...
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " WARNING: This should be a peroxide; the O-O bond length is: " << cce_vars.distances[i][j] << " Ang." << endl;
              }
              perox_count += 1;
  ...
  vector<double> get_oxidation_states_from_electronegativities(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    stringstream message;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " DETERMINATION OF OXIDATION NUMBERS FROM PREFERRED/ALL KNOWN OXIDATION STATES, ALLEN ELECTRONEGATIVITIES, AND STRUCTURE:" << endl;
    }
    // deal with anion charge in cell first
    set_anion_oxidation_states(structure, cce_vars);
    // treat cations
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " CATION PART:" << endl;
    }
    ...
    const uint num_cation_species = structure.species.size() - 1;
    ...
    const uint natoms = structure.atoms.size();
    cce_vars.cations_map.resize(num_cation_species);
    uint i = 0;
    for (uint at = 0; at < natoms; at++) {
      for (i = 0; i < num_cation_species; i++) {
        if (structure.atoms[at].cleanname == cce_vars.species_electronegativity_sorted[i]) {
          break; // if it finds that the atom belongs to the ith species sorted by electronegativities, break to insert it at the proper place for the ith species into cation_map
        }
      }
      if (i < num_cation_species) {
        cce_vars.cations_map[i].push_back(at);
      }
    }
  ...
  void set_anion_oxidation_states(const xstructure& structure, CCE_Variables& cce_vars) {
    const bool LDEBUG = (false || XHOST.DEBUG || CCE_DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << __AFLOW_FUNC__ << " ANION PART:" << endl;
    }
    double total_anion_charge = 0;
    for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) { // loop over all atoms in structure
      if ((structure.atoms[i].cleanname == cce_vars.anion_species) || (cce_vars.multi_anion_atoms[i] == 1)) {
        ...
        if (LDEBUG) {
          if (cce_vars.multi_anion_atoms[i] == 1) { // for multi anion atoms oxidation states have been assigned previously
            cerr << __AFLOW_FUNC__ << " anion oxidation number for multi-anion ATOM[" << i << "] (" << structure.atoms[i].cleanname << ") has been assigned previously to: " << cce_vars.oxidation_states[i] << endl;
          } else {
            cerr << __AFLOW_FUNC__ << " anion oxidation number for ATOM[" << i << "] (" << structure.atoms[i].cleanname << "): " << cce_vars.oxidation_states[i] << endl;
          }
        }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Total anion charge in cell: " << total_anion_charge << endl;
    }
  }
  ...
  void sort_species_by_electronegativity(const xstructure& structure, CCE_Variables& cce_vars) {
    ...
    for (size_t j = 0, jsize = structure.species.size(); j < jsize; j++) { // loop over all species
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " species_electronegativity_sorted[" << j << "]: " << cce_vars.species_electronegativity_sorted[j] << endl;
        cerr << __AFLOW_FUNC__ << " electronegativities_sorted[" << j << "]: " << electronegativities_sorted[j] << endl;
      }
    }
  }
  ...
  void try_preferred_oxidation_states(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars) {
    const bool LDEBUG = (false || XHOST.DEBUG || CCE_DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Trying preferred oxidation numbers:" << endl;
    }
  ...
  void treat_SbO2_special_case(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number of Sb ions= " << amount_Sb << endl;
      cerr << __AFLOW_FUNC__ << " number of O ions= " << amount_O << endl;
    }
    ...
    if (aurostd::isequal(Sb_O_ratio, 0.5)) {
      ...
      if ((structure.atoms.size() % 6) == 0) {
        const uint num_formula_units_in_cell = structure.atoms.size() / 6; // 6 for Sb2O4
        if (LDEBUG) {
  ...
  void treat_Pb3O4_special_case(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number of Pb ions= " << amount_Pb << endl;
      cerr << __AFLOW_FUNC__ << " number of O ions= " << amount_O << endl;
    }
    ...
    if (aurostd::isequal(Pb_O_ratio, 0.75)) {
      ...
      if ((structure.atoms.size() % 7) == 0) {
        const uint num_formula_units_in_cell = structure.atoms.size() / 7; // 7 for Pb3O4
        if (LDEBUG) {
          cerr << "number of formula units in cell: " << num_formula_units_in_cell << endl;
        }
        for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) { // loop over all atoms in structure
  ...
  void treat_Ti_O_Magneli_phase_special_case(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Ti-O system, Magneli for Ti_nO_(2n-1), i.e. Ti-O ratio= " << 3.0 / 5 << ", " << 4.0 / 7 << ", " << 5.0 / 9 << ", " << 6.0 / 11 << ", " << 7.0 / 13 << ", " << 8.0 / 15 << ", "
           << 9.0 / 17 << ", " << 10.0 / 19 << "..." << endl;
    }
    double amount_O = 0;
    double amount_Ti = 0;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number of O ions= " << amount_O << endl;
      cerr << __AFLOW_FUNC__ << " number of Ti ions= " << amount_Ti << endl;
    }
    ...
    if (magneli == false) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Not a Magneli composition." << endl;
      }
    }
    if (magneli) {
      for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) { // loop over all atoms in structure
  ...
  void treat_Fe3O4_special_case(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number of Fe ions= " << amount_Fe << endl;
      cerr << __AFLOW_FUNC__ << " number of O ions= " << amount_O << endl;
    }
    ...
    if (aurostd::isequal(Fe_O_ratio, 0.75)) {
      ...
      if ((structure.atoms.size() % 7) == 0) {
        const uint num_formula_units_in_cell = structure.atoms.size() / 7; // 7 for Fe3O4
        if (LDEBUG) {
          cerr << "number of formula units in cell: " << num_formula_units_in_cell << endl;
        }
        for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) { // loop over all atoms in structure
  ...
  void treat_X3O4_special_case(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, const string& cation_species, ostream& oss) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number of " << cation_species << " ions = " << amount_cation_species << endl;
      cerr << __AFLOW_FUNC__ << " number of O ions= " << amount_O << endl;
    }
    ...
    if (aurostd::isequal(cation_species_O_ratio, 0.75)) {
      ...
      if ((structure.atoms.size() % 7) == 0) {
        const uint num_formula_units_in_cell = structure.atoms.size() / 7; // 7 for Mn3O4 and Co3O4
        if (LDEBUG) {
          cerr << "number of formula units in cell: " << num_formula_units_in_cell << endl;
        }
        for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) { // loop over all atoms in structure
  ...
  void treat_alkali_sesquioxide_special_case(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " This is a binary alkali metal oxide, checking whether it is an alkali metal sesquioxide..." << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number of O ions= " << amount_O << endl;
      cerr << __AFLOW_FUNC__ << " number of alkali (" << alkali_metal << ") ions= " << amount_alkali << endl;
    }
    ...
    if (aurostd::isequal(O_alkali_ratio, 1.5)) {
      ...
      if ((structure.atoms.size() % 5) == 0) {
        const uint num_formula_units_in_cell = structure.atoms.size() / 5; // 5 for alkali_metal2O3
        if (LDEBUG) {
          cerr << "number of formula units in cell: " << num_formula_units_in_cell << endl;
        }
        for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) { // loop over all atoms in structure
  ...
  void treat_MnMoO4_special_case(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number of Mn ions= " << amount_Mn << endl;
      cerr << __AFLOW_FUNC__ << " number of Mo ions= " << amount_Mo << endl;
      cerr << __AFLOW_FUNC__ << " number of O ions= " << amount_O << endl;
    }
  ...
  void treat_Ca2Fe2O5_CaFe2O4_LDA_special_case(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " number of Ca ions= " << amount_Ca << endl;
      cerr << __AFLOW_FUNC__ << " number of Fe ions= " << amount_Fe << endl;
      cerr << __AFLOW_FUNC__ << " number of O ions= " << amount_O << endl;
    }
  ...
  void treat_FeTiO3_LDA_special_case(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, ostream& oss) {
    ...
    if (amount_Ti != 0 && amount_O != 0) {
      if (aurostd::isequal(amount_Fe / amount_Ti, 1.0) && aurostd::isequal(amount_Fe / amount_O, 1.0 / 3) && aurostd::isequal(amount_Ti / amount_O, 1.0 / 3)) {
        stringstream message;
        message << " FeTiO3 special treatment for LDA since oxidation numbers for Ti, which should be Ti+4, are not correctly determined from Bader charges and using other fixing would also change Fe "
                   "oxidation numbers!!!"
                << endl;
        if (!cce_flags.flag("UNIT_TEST")) {
          ostream& oss = cout;
  ...
  void try_all_oxidation_states(const xstructure& structure, CCE_Variables& cce_vars) {
    const bool LDEBUG = (false || XHOST.DEBUG || CCE_DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Trying all known oxidation numbers:" << endl;
    }
    // use ME's implementation of my algorithm to determine oxidation_numbers
  ...
  void determine_cation_oxidation_states(const xstructure& structure, CCE_Variables& cce_vars, const vector<vector<double>>& possible_ox_states) {
    const bool LDEBUG = (false || XHOST.DEBUG || CCE_DEBUG);
    const uint num_cation_species = cce_vars.cations_map.size(); // the number of cation species
    const uint natoms = cce_vars.oxidation_states.size();

    // Initialize
    for (uint i = 0; i < num_cation_species; i++) { // loop over all cations
      for (size_t j = 0; j < cce_vars.cations_map[i].size(); j++) { // loop over atoms of the ith cation type that is given by the second index of cation_map
        if (cce_vars.multi_anion_atoms[cce_vars.cations_map[i][j]] != 1) { // exclude atoms that have been identified as multi anion atoms previously
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " Updating oxidation number for atom " << cce_vars.cations_map[i][j] << " (" << structure.atoms[cce_vars.cations_map[i][j]].cleanname << ") to " << possible_ox_states[i][0] << endl;
          }
    ...
    if (LDEBUG) {
      for (size_t n = 0, nsize = structure.atoms.size(); n < nsize; n++) {
        cerr << __AFLOW_FUNC__ << " chosen oxidation state for atom " << n << " (" << structure.atoms[n].cleanname << "): " << cce_vars.oxidation_states[n] << endl;
      }
    }
    // check
    double total_ox = 0.0;
    for (uint at = 0; at < natoms; at++) {
      total_ox += cce_vars.oxidation_states[at];
    }
    ...
    if (iszero) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Oxidation numbers successfully determined since oxidation sum is " << total_ox << endl;
      }
    } else {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " No successful determination of oxidation numbers since oxidation sum is " << total_ox << endl;
      }
    }
    ...
    if (!iszero) { // maybe the oxidation numbers from the initialization work already, then nothing else needs to be done
      ...
      for (uint j = 0; j < num_cation_species; j++) {
        if (possible_ox_states[j].size() > maxsize) {
          maxsize = possible_ox_states[j].size();
        }
      }
      ...
      while (!iszero && (i < maxsize)) {
        for (uint j = num_cation_species; j > 0; j--) { // have to loop downward since species are sorted ascending by electronegativity and oxidation number should be changed for less electronegative species first
          ...
          if (i < num_ox_states) { // the index i looping over all possible ox states for species j must be smaller than the maximum number of pref./all ox states for this species
            for (size_t k = 0; k < cce_vars.cations_map[j - 1].size(); k++) {
              if (cce_vars.multi_anion_atoms[cce_vars.cations_map[j - 1][k]] != 1) { // exclude atoms that have been identified as multi anion atoms previously
                if (LDEBUG) {
                  cerr << __AFLOW_FUNC__ << " Updating oxidation number for atom " << cce_vars.cations_map[j - 1][k] << "(" << structure.atoms[cce_vars.cations_map[j - 1][k]].cleanname << ") to "
                       << possible_ox_states[j - 1][i] << endl;
                }
  ...
  double get_oxidation_states_sum(CCE_Variables& cce_vars) {
    ...
    for (size_t k = 0, ksize = cce_vars.oxidation_states.size(); k < ksize; k++) {
  ...
  vector<double> get_oxidation_states_from_Bader(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, const string& directory_path, ostream& oss) {
    ...
    stringstream message;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " DETERMINATION OF OXIDATION NUMBERS FROM BADER CHARGES:" << endl;
    }
    if (cce_vars.anion_species == "O") {
      ...
      if (aurostd::FileExist(directory_path + "/" + _AFLOWIN_)) {
        ...
        if (cce_flags.flag("CORRECTABLE")) {
          ...
          const string Bader_file = directory_path + "/" + system_name + "_abader.out";
          if (aurostd::FileExist(Bader_file) || aurostd::CompressFileExist(Bader_file)) {
  ...
  void get_system_name_functional_from_aflow_in(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, string& system_name, string& functional, const string& directory_path) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " functional determined from aflow.in: " << functional << endl;
      cerr << __AFLOW_FUNC__ << " PBE: " << aurostd::WithinList(cce_vars.vfunctionals, "PBE") << endl;
      cerr << __AFLOW_FUNC__ << " LDA: " << aurostd::WithinList(cce_vars.vfunctionals, "LDA") << endl;
      cerr << __AFLOW_FUNC__ << " SCAN: " << aurostd::WithinList(cce_vars.vfunctionals, "SCAN") << endl;
      cerr << __AFLOW_FUNC__ << " PBE+U:ICSD: " << aurostd::WithinList(cce_vars.vfunctionals, "PBE+U:ICSD") << endl;
  ...
  vector<double> get_Bader_charges_from_Bader_file(const xstructure& structure, CCE_Variables& cce_vars, const string& Bader_file) {
    ...
    aurostd::compressfile2vectorstring(Bader_file, vlines);
    ...
    for (size_t k = 0, ksize = structure.atoms.size(); k < ksize; k++) { // there should always be as many Bader charges as atoms in the structure
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Bader_charges[" << k << "]: " << cce_vars.Bader_charges[k] << endl;
      }
    }
  ...
  vector<double> Bader_charges_to_oxidation_states(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, string& functional, ostream& oss) {
    ...
    for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) { // loop over all atoms in structure
      if (structure.atoms[i].cleanname != cce_vars.anion_species) {
        ...
        if (get_Bader_templates(structure.atoms[i].cleanname).empty()) {
          cce_flags.flag("CORRECTABLE", false);
          error1 = true;
          ...
          oss << "VERY BAD NEWS: There is no correction for " << structure.atoms[i].cleanname << " (ATOM[" << i << "])" << " coordinated by " << cce_vars.anion_species
              << " since this species was not included in the set for deducing corrections!" << endl;
          oss << endl;
          if (species_missing_corrections.empty()) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " Bader templates: " << Bader_templ_line << endl;
          }
          ...
          const uint num_ox_states = aurostd::string2utype<uint>(Bader_tokens[0]);
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " number of oxidation states for which Bader charges are available: " << num_ox_states << endl;
          }
          double Bader_template = 0; // Bader cation charge from the binary oxide from which the correction was deduced
          const double Bader_tolerance = 0.26; // tolerance with which the Bader charge of each atom is allowed to deviate from any of the template values to still assign the correction (and oxidation number)
          ...
          const double Bader_deviation_0 = Bader_deviation; // initial Bader deviation for later check whether any corrections were found, i. e. Bader deviation was changed
          ...
          for (uint n = 0; n < num_ox_states; n++) { // loop over all oxidation number for which Bader charges are available and read the Bader charges from the respective positions
            ...
            const int offset = get_offset(functional); // define also local offset variable since offset must correspond to functional detected from aflow.in
            for (size_t j = 0; j < vfunctional_aflow_in.size(); j++) {
              Bader_template = aurostd::string2utype<double>(Bader_tokens[offset / 2 + 2 + (CCE_num_functionals_Bader + 1) * n]);
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " Bader_template " << vfunctional_aflow_in[j] << ": " << Bader_template << endl;
              }
            }
            if (std::abs(Bader_template - cce_vars.Bader_charges[i]) < Bader_tolerance &&
                std::abs(Bader_template - cce_vars.Bader_charges[i]) <
                    Bader_deviation) { // must be compared to Bader_deviation to load correction for Bader charge closest to template and not to be overloaded by other corrections for later tested Bader charges (oxidation states)
              Bader_deviation = std::abs(Bader_template - cce_vars.Bader_charges[i]);
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " Bader_deviation: " << Bader_deviation << endl;
              }
              cce_vars.oxidation_states[i] = aurostd::string2utype<double>(
                  Bader_tokens[1 + (CCE_num_functionals_Bader + 1) * n]); // (CCE_num_functionals_Bader+1)*n because oxidation numbers in Bader_templ_line are separated by corrections for four different functionals (PBE, LDA, SCAN, and PBE+U:ICSD) and there is 1 formal oxidation number in front of the Bader charges
        ...
        cce_vars.oxidation_states[i] = cce_vars.standard_anion_charge; // oxidation numbers for O are assumed to be -2 (other anions accordingly) and are corrected below if it is a per- or superoxide O atom as identified from the structural analysis in other functions
        if (cce_vars.num_perox_bonds > 0) {
          for (size_t j = 0, jsize = structure.atoms.size(); j < jsize; j++) {
    ...
    if (error1) { // errors can only be thrown after loop over atoms is complete since the output should indicate all species for which corrections might be missing/cannot be identified
      message << " VERY BAD NEWS: The formation enthalpy of this system is not correctable since there are no corrections for " << aurostd::joinWDelimiter(species_missing_corrections, ", ")
              << " coordinated by " << cce_vars.anion_species << "!" << endl;
  ...
  void get_corrections(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, const string& considered_anion_species, const vector<uint>& num_neighbors, vector<vector<double>>& corrections_atom, ostream& oss) {
    ...
    bool error = false;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << "  ASSIGNMENT OF CORRECTIONS:" << endl;
    }
    ...
    for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) { // loop over all atoms in structure
      if ((structure.atoms[i].cleanname != cce_vars.anion_species) && (cce_vars.multi_anion_atoms[i] != 1)) { // exclude main anion species and multi anion atoms detected previously
        ...
        const string cor_identifier = structure.atoms[i].cleanname + "_+" + aurostd::utype2string<double>(cce_vars.oxidation_states[i]) + "_" + considered_anion_species;
        if (num_neighbors[i] > 0) { // are there actually bonds between the cation and the anion species under consideration (to load only needed corrections in multi-anion systems)
          if (get_corrections_line(considered_anion_species, cor_identifier).empty()) { // the considered anion species can be the main anion species or a multi anion species
            cce_flags.flag("CORRECTABLE", false);
            error = true;
            ...
            const string info_missing_corrections = structure.atoms[i].cleanname + " in oxidation state " + "+" + aurostd::utype2string<double>(cce_vars.oxidation_states[i]) + " coordinated by " + considered_anion_species;
            oss << "BAD NEWS: No correction available for " << structure.atoms[i].cleanname << " (atom " << i + 1 << ")" << " in oxidation state +" << cce_vars.oxidation_states[i] << " when coordinated by "
                << considered_anion_species << "." << endl; // i+1 to convert to human based counting from 1
            if (missing_corrections.empty()) {
            ...
            for (uint k = 0; k < 12; k++) { // larger than ox. num. +12 should not occur
              if (!get_corrections_line(considered_anion_species, structure.atoms[i].cleanname + "_+" + aurostd::utype2string<uint>(k) + "_" + considered_anion_species).empty()) {
                ox_nums_count += 1;
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " corrections line: " << corrections_line << endl;
            }
            if (corrections_line.find("*") != string::npos) {
              message << " The correction for " << structure.atoms[i].cleanname << " (atom " << i + 1 << ")" << " in oxidation state +" << cce_vars.oxidation_states[i] << " when coordinated by "
                      << considered_anion_species << " might be less accurate since it was obtained from less well validated experimental data as the other corrections!";
            ...
            if (corrections_line.find("^") != string::npos) {
              message << " The correction for " << structure.atoms[i].cleanname << " (atom " << i + 1 << ")" << " in oxidation state +" << cce_vars.oxidation_states[i] << " when coordinated by "
                      << considered_anion_species << " is only approximate since in this case the explicit oxidation state dependence was lifted!";
              pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, oss, _LOGGER_WARNING_, XHOST.vflag_control.flag("PRINT_MODE::JSON")); // CO20210623 - silent with json output
            }
    ...
    for (uint k = 0; k < num_funcs; k++) {
      if (cce_vars.vfunctionals[k] != "exp") {
        for (uint l = 0; l < num_temps; l++) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " correction for " << structure.atoms[i].cleanname << " (atom[" << i << "]) for " << cce_vars.vtemperatures[l]
                 << "K: " << corrections_atom[num_temps * k + l][i] << " eV/bond" << endl;
          }
        }
      } else {
        corrections_atom[num_temps * k][i] = aurostd::string2utype<double>(corrections_tokens[cce_vars.offset[k] + 2]);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " correction for " << structure.atoms[i].cleanname << " (atom[" << i << "]) for 298.15K: " << corrections_atom[num_temps * k][i] << " eV/bond" << endl;
        }
      }
    }
  }
  ...
  void check_get_per_super_ox_corrections(CCE_Variables& cce_vars) {
    ...
    if (cce_vars.num_perox_bonds > 0) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " corrections line peroxides: " << corrections_line << endl;
      }
      vector<string> corrections_tokens;
      aurostd::string2tokens(corrections_line, corrections_tokens, " "); // seems to automatically reduce the number of multiple spaces in a row to one so that e.g. corrections_tokens[1] is not a space but the oxidation number
      for (uint k = 0; k < num_funcs; k++) {
        if (cce_vars.vfunctionals[k] != "exp") {
          for (uint l = 0; l < num_temps; l++) {
            cce_vars.perox_correction[num_temps * k + l] =
                aurostd::string2utype<double>(corrections_tokens[cce_vars.offset[k] + l + 2]); // num_temps*k since to each functional belong num_temps corrections (currently for 298.15 and 0K)
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " peroxide correction for " << cce_vars.vtemperatures[l] << "K: " << cce_vars.perox_correction[num_temps * k + l] << " eV/bond" << endl;
            }
          }
        } else {
          cce_vars.perox_correction[num_temps * k] = aurostd::string2utype<double>(corrections_tokens[cce_vars.offset[k] + 2]);
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " peroxide correction for 298.15K: " << cce_vars.perox_correction[num_temps * k] << " eV/bond" << endl;
          }
        }
      }
    }
    if (cce_vars.num_superox_bonds > 0) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " corrections line superoxides: " << corrections_line << endl;
      }
      vector<string> corrections_tokens;
      aurostd::string2tokens(corrections_line, corrections_tokens, " "); // seems to automatically reduce the number of multiple spaces in a row to one so that e.g. corrections_tokens[1] is not a space but the oxidation number
      for (uint k = 0; k < num_funcs; k++) {
        if (cce_vars.vfunctionals[k] != "exp") {
          for (uint l = 0; l < num_temps; l++) {
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " superoxide correction for " << cce_vars.vtemperatures[l] << "K: " << cce_vars.superox_correction[num_temps * k + l] << " eV/bond" << endl;
            }
          }
        } else {
          cce_vars.superox_correction[num_temps * k] = aurostd::string2utype<double>(corrections_tokens[cce_vars.offset[k] + 2]);
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " superoxide correction for 298.15K: " << cce_vars.superox_correction[num_temps * k] << " eV/bond" << endl;
          }
        }
      }
    }
      ...
      for (uint k = 0; k < num_funcs; k++) {
        if (cce_vars.vfunctionals[k] != "exp") {
          for (uint l = 0; l < num_temps; l++) {
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " peroxide correction for " << cce_vars.vtemperatures[l] << "K per cell: " << std::setprecision(3) << std::fixed
                   << (cce_vars.num_perox_bonds * cce_vars.perox_correction[num_temps * k + l]) << " eV" << endl;
              cerr << endl;
            }
          }
        } else {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " peroxide correction for 298.15K per cell: " << std::setprecision(3) << std::fixed
                 << (cce_vars.num_perox_bonds * cce_vars.perox_correction[num_temps * k]) << " eV" << endl;
          }
        }
      }
    }
    if (cce_vars.num_superox_bonds > 0) {
      ...
      for (uint k = 0; k < num_funcs; k++) {
        if (cce_vars.vfunctionals[k] != "exp") {
          for (uint l = 0; l < num_temps; l++) {
            cce_vars.cce_correction[num_temps * k + l] +=
                (cce_vars.num_superox_bonds * cce_vars.superox_correction[num_temps * k + l]); // num_temps*k since to each functional belong num_temps corrections (currently for 298.15 and 0K)
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " superoxide correction for " << cce_vars.vtemperatures[l] << "K per cell: " << std::setprecision(3) << std::fixed
                   << (cce_vars.num_superox_bonds * cce_vars.superox_correction[num_temps * k + l]) << " eV" << endl;
              cerr << endl;
            }
          }
        } else {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " superoxide correction for 298.15K per cell: " << std::setprecision(3) << std::fixed
                 << (cce_vars.num_superox_bonds * cce_vars.superox_correction[num_temps * k]) << " eV" << endl;
          }
        }
      }
    }
  ...
  void apply_pbe_u_icsd_shifts(const xstructure& structure, CCE_Variables& cce_vars, ostream& oss) {
    ...
    for (uint k = 0; k < num_funcs; k++) {
      if (cce_vars.vfunctionals[k] == "PBE+U:ICSD") { // for PBE+U:ICSD additional ref. enthalpy shifts must be applied since corrections have been fitted to ref. enthalpies calculated with U but internal AFLOW ref. enthalpies are from plain PBE
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << cce_vars.vfunctionals[k] << " applying ref. enthalpy shifts for PBE+U:ICSD." << endl;
        }
        for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) {
          for (uint l = 0; l < num_temps; l++) {
            const double ref_enthalpy_shift = get_ref_enthalpy_shift_pbe_u_icsd(structure.atoms[i].cleanname);
  ...
  string print_JSON_cation_coordination_numbers(const xstructure& structure, xoption& cce_flags, const CCE_Variables& cce_vars, vector<vector<uint>>& multi_anion_num_neighbors) {
    ...
    const uint nspecies = structure.species.size();
    ...
    for (uint i = 0; i < nspecies; i++) {
      ...
      for (int k = 0; k < structure.num_each_type[i]; k++) {
        if ((structure.atoms[l].cleanname != cce_vars.anion_species) && (cce_vars.multi_anion_atoms[l] != 1)) { // exclude main anion species and multi anion atoms detected previously
          ...
          if (cce_flags.flag("MULTI_ANION_SYSTEM")) {
            for (size_t j = 0, jsize = cce_vars.multi_anion_species.size(); j < jsize; j++) {
              if (multi_anion_num_neighbors[j][l] > 0) { // are there actually bonds between the cation and the multi anion species
                if (cation_info.empty()) { // if no neighbors from main anion species, append basic cation info first
                  cation_info = "\"" + structure.species[i] + "\":";
                }
      ...
      if (!atoms_neighbors_vector.empty()) {
  ...
  string print_JSON_ox_nums(const xstructure& structure, const CCE_Variables& cce_vars) {
    ...
    for (uint i = 0; i < nspecies; i++) {
      ...
      for (int k = 0; k < structure.num_each_type[i]; k++) {
        ...
        if (k < structure.num_each_type[i] - 1) {
          json << ",";
        }
        l++;
      }
  ...
  string print_JSON_corrections(const xstructure& structure, const CCE_Variables& cce_vars) {
    stringstream json;
    const bool print_Hf = (!cce_vars.enthalpies_dft.empty());
    const uint nfuncs = cce_vars.vfunctionals.size();
    const uint num_temps = cce_vars.vtemperatures.size();
    const uint natoms = structure.atoms.size();
  ...
  string print_output_cation_coordination_numbers(const xstructure& structure, xoption& cce_flags, CCE_Variables& cce_vars, vector<vector<uint>>& multi_anion_num_neighbors) {
    ...
    for (size_t i = 0, isize = structure.atoms.size(); i < isize; i++) {
      if ((structure.atoms[i].cleanname != cce_vars.anion_species) && (cce_vars.multi_anion_atoms[i] != 1)) { // exclude main anion species and multi anion atoms detected previously
        if (cce_vars.num_neighbors[i] > 0) { // are there actually bonds between the cation and the (main) anion species
          output << std::setw(4) << std::right << i + 1 << std::setw(13) << std::left << "   " + structure.atoms[i].cleanname << std::setw(8) << cce_vars.anion_species << std::setw(13) << std::right
                 << cce_vars.num_neighbors[i] << endl; // i+1: convert to 1-based counting
        }
        if (cce_flags.flag("MULTI_ANION_SYSTEM")) {
          for (size_t k = 0, ksize = cce_vars.multi_anion_species.size(); k < ksize; k++) {
            if (multi_anion_num_neighbors[k][i] > 0) { // are there actually bonds between the cation and the multi anion species
              output << std::setw(4) << std::right << i + 1 << std::setw(13) << std::left << "   " + structure.atoms[i].cleanname << std::setw(8) << cce_vars.multi_anion_species[k] << std::setw(13)
                     << std::right << multi_anion_num_neighbors[k][i] << endl; // i+1: convert to 1-based counting
            }
          }
        }
      }
  ...
  string print_output_corrections(const xstructure& structure, CCE_Variables& cce_vars, const vector<double>& enthalpy_formation_cell_cce) {
    ...
    for (uint k = 0; k < num_funcs; k++) {
      if (cce_vars.vfunctionals[k] != "exp") {
        for (uint l = 0; l < num_temps; l++) {
          output << std::showpos << std::setprecision(3) << std::fixed << std::setw(10) << std::left << cce_vars.vfunctionals[k] << std::setw(14) << std::right << cce_vars.vtemperatures[l] << std::setw(13)
                 << cce_vars.cce_correction[num_temps * k + l] << std::setw(13) << cce_vars.cce_correction[num_temps * k + l] / structure.atoms.size() << endl;
        }
      }
    }
    output << endl;
    ...
    for (uint k = 0; k < num_funcs; k++) {
      if (cce_vars.vfunctionals[k] == "exp" && cce_vars.enthalpies_dft.empty()) { // second condition for that if precalc. form. enthalpies are given and asking for exp., exp. result is not written twice
        ...
        output << std::showpos << std::setprecision(3) << std::fixed << std::setw(10) << std::left << "CCE@exp" << std::setw(14) << std::right << "298.15" << std::setw(17)
               << enthalpy_formation_cell_cce[num_temps * k] << std::setw(17) << enthalpy_formation_cell_cce[num_temps * k] / structure.atoms.size() << endl;
    ...
    if (!cce_vars.enthalpies_dft.empty()) {
      output << "CCE FORMATION ENTHALPIES:" << endl;
      output << std::setw(10) << "functional" << std::setw(14) << "temperature" << std::setw(17) << "form. enthalpy" << std::setw(17) << "form. enthalpy" << endl;
      output << std::setw(24) << "(K)" << std::setw(17) << "(eV/cell)" << std::setw(17) << "(eV/atom)" << endl;
      const uint num_funcs = cce_vars.vfunctionals.size();
      for (uint k = 0; k < num_funcs; k++) {
        if (cce_vars.vfunctionals[k] != "exp") {
          for (uint l = 0; l < num_temps; l++) {
            output << std::showpos << std::setprecision(3) << std::fixed << std::setw(10) << std::left << cce_vars.vfunctionals[k] << std::setw(14) << std::right << cce_vars.vtemperatures[l] << std::setw(17)
                   << enthalpy_formation_cell_cce[num_temps * k + l] << std::setw(17) << enthalpy_formation_cell_cce[num_temps * k + l] / structure.atoms.size() << endl;
          }
  ...
  string print_test_output(CCE_Variables& cce_vars, const vector<double>& enthalpy_formation_cell_cce) {
    ...
    for (uint k = 0; k < num_funcs; k++) {
      if (cce_vars.vfunctionals[k] != "exp") {
        for (uint l = 0; l < num_temps; l++) {
          output << std::setprecision(3) << std::fixed << cce_vars.cce_correction[num_temps * k + l] << (k < num_funcs - 1 || l == 0 || !cce_vars.enthalpies_dft.empty() ? "," : "");
        }
      }
    }
    // exp part
  ...
  string get_corrections_line(const string& considered_anion_species, const string& cor_identifier) {
    if (considered_anion_species == "O") {
      return get_corrections_line_O(cor_identifier);
    }
    if (considered_anion_species == "N") {
      return get_corrections_line_N(cor_identifier);
    } else {
      return "";
    }
  }
  ...
  string get_corrections_line_O(const string& cor_identifier) {
    ...
    // OXIDES
    if (cor_identifier == "Ag_+1_O") {
      return "Ag_+1_Ag2O             +1         -0.00825           -0.00700      -0.05400           -0.05375      -0.06525            -0.06450       -0.06150                    -0.06000               -0.08050 "
             "           4                 AgO/A2B_cP6_224_b_a.AB              2           1                -0.107   ";
    } else if (cor_identifier == "Al_+3_O") {
      return "Al_+3_Al2O3            +3          0.18692            0.17783      -0.00733           -0.01683      -0.01242            -0.02217        0.18667                     0.17758               -1.44725 "
             "           12                AlO/A2B3_hR10_167_c_e-001.AB        2           3                -3.473   ";
    } else if (cor_identifier == "As_+5_O") {
      return "As_+5_As2O5            +5          0.20220            0.19190      -0.06360           -0.07520       0.02120             0.00920        0.19770                     0.18760               -0.95360 "
             "           10                AsO/A2B5_oP28_19_2a_5a-001.AB       2           5                -1.362   ";
    } else if (cor_identifier == "B_+3_O") {
      return "B_+3_B2O3              +3          0.19517            0.18250      -0.06933           -0.08350      -0.04717            -0.06117        0.19317                     0.18067               -2.19983 "
             "           6                 B_hO/A2B3_hP15_144_2a_3a-001.AB     2           3                -2.64    ";
    } else if (cor_identifier == "Ba_+2_O") {
      return "Ba_+2_BaO              +2          0.11833            0.11650       0.00983            0.00750       0.00417             0.00200        0.11817                     0.11633               -0.94683 "
             "           6                 Ba_svO/AB_cF8_225_a_b.AB            1           1                -2.841   ";
    } else if (cor_identifier == "Be_+2_O") {
      return "Be_+2_BeO              +2          0.19525            0.18750       0.00825            0.00000       0.00600            -0.00225        0.19475                     0.18700               -1.57900 "
             "           4                 Be_svO/AB_hP4_186_b_b-001.AB        1           1                -3.158   ";
    } else if (cor_identifier == "Bi_+3_O") {
      return "Bi_+3_Bi2O3            +3         -0.02580           -0.02760      -0.17520           -0.17780      -0.03560            -0.03790       -0.02830                    -0.03010               -0.59150 "
             "           10                Bi_dO/A2B3_mP20_14_2e_3e-001.AB     2           3                -1.183   ";
    } else if (cor_identifier == "Ca_+2_O") {
      return "Ca_+2_CaO              +2          0.10567            0.10017      -0.03317           -0.03950      -0.02217            -0.02800        0.10567                     0.10017               -1.09667 "
             "           6                 Ca_svO/AB_cF8_225_a_b.AB            1           1                -3.29    ";
    } else if (cor_identifier == "Cd_+2_O") {
      return "Cd_+2_CdO              +2          0.10417            0.10133       0.01617            0.01300       0.00533             0.00233        0.07417                     0.07117               -0.44633 "
             "           6                 CdO/AB_cF8_225_a_b.AB               1           1                -1.339   ";
    } else if (cor_identifier == "Co_+2_O") {
      return "Co_+2_CoO              +2          0.23967            0.23733       0.16617            0.16450       0.12300             0.11933        0.00300                    -0.00017               -0.41067 "
             "           6                 CoO/AB_cF8_225_a_b.AB               1           1                -1.232   ";
    } else if (cor_identifier == "Cr_+3_O") {
      return "Cr_+3_Cr2O3            +3          0.15283            0.14733       0.04542            0.03908      -0.01892            -0.02467       -0.18417                    -0.18958               -0.98000 "
             "           12                Cr_pvO/A2B3_hR10_167_c_e-001.AB     2           3                -2.352   ";
    } else if (cor_identifier == "Cr_+6_O") {
      return "Cr_+6_CrO3             +6         -0.13225           -0.14425      -0.30525           -0.32100      -0.28125            -0.29675       -0.06775                    -0.07950               -1.52100 "
             "           4                 Cr_pvO/AB3_oC16_40_b_a2b-001.AB     1           3                -1.521   ";
    } else if (cor_identifier == "Cs_+1_O") {
      return "Cs_+1_Cs2O             +1          0.10083            0.10083      -0.05667           -0.05833      -0.00500            -0.00600        0.10117                     0.10117               -0.59767 "
             "           6                 Cs_svO/A2B_hR3_166_c_a-001.AB       2           1                -1.195   ";
    } else if (cor_identifier == "Cu_+1_O") {
      return "Cu_+1_Cu2O_NJ          +1          0.13100            0.12950       0.03400            0.03175       0.06350             0.06175        0.06800                     0.06625               -0.44225 "
             "           4                 Cu_pvO/A2B_cP6_224_b_a.AB           2           1                -0.59    ";
    } else if (cor_identifier == "Cu_+2_O") {
      return "Cu_+2_CuO_NJ           +2          0.10150            0.09725      -0.02575           -0.03075       0.00675             0.00200        0.06525                     0.06075               -0.40425 "
             "           4                 Cu_pvO/AB_mC8_15_a_e-001.AB         1           1                -0.808   ";
    } else if (cor_identifier == "Fe_+2_O") {
      return "Fe_+2_FeO_NJ           +2          0.17633            0.17283       0.13100            0.12867       0.01883             0.01433       -0.00900                    -0.01300               -0.47000 "
             "           6                 Fe_pvO/AB_cF8_225_a_b.AB            1           1                -1.41    ";
    } else if (cor_identifier == "Fe_+3_O") {
      return "Fe_+3_Fe2O3            +3          0.16325            0.15858       0.01300            0.00550      -0.06550            -0.07175       -0.03417                    -0.04517               -0.71117 "
             "           12                Fe_pvO/A2B3_hR10_167_c_e-001.AB     2           3                -1.707   ";
    } else if (cor_identifier == "Ga_+3_O") {
      return "Ga_+3_Ga2O3            +3          0.20090            0.19250       0.00680           -0.00220       0.03860             0.02890        0.16220                     0.15370               -1.12880 "
             "           10                Ga_hO/A2B3_mC20_12_2i_3i-001.AB     2           3                -2.258   ";
    } else if (cor_identifier == "Ge_+4_O") {
      return "Ge_+4_GeO2             +4          0.19917            0.18950      -0.03567           -0.04617       0.03967             0.02900        0.19883                     0.18917               -1.00183 "
             "           6                 Ge_hO/A2B_tP6_136_f_a-001.BA        1           2                -2.004   ";
    } else if (cor_identifier == "Hf_+4_O") {
      return "Hf_+4_HfO2_Ba          +4          0.16171            0.15657      -0.02957           -0.03529      -0.02686            -0.03257        0.16286                     0.15786               -1.69486 "
             "           7                 Hf_pvO/A2B_mP12_14_2e_e-001.BA      1           2                -3.955   ";
    } else if (cor_identifier == "Hg_+2_O") {
      return "Hg_+2_HgO              +2          0.17000            0.15250      -0.06500           -0.08650       0.05800             0.03800        0.16050                     0.14300               -0.47050 "
             "           2                 HgO/AB_oP8_62_c_c-006.AB            1           1                -0.47    ";
    } else if (cor_identifier == "In_+3_O") {
      return "In_+3_In2O3            +3          0.13525            0.13025      -0.01667           -0.02217      -0.01300            -0.01858        0.09750                     0.09258               -0.79967 "
             "           12                In_dO/A2B3_cI80_199_a2b_2c-001.AB   2           3                -1.919   ";
    } else if (cor_identifier == "Ir_+4_O") {
      return "Ir_+4_IrO2_Ba          +4          0.02017            0.01567      -0.18133           -0.18683       0.01800             0.01350       -0.04283                    -0.04717               -0.41917 "
             "           6                 IrO/A2B_tP6_136_f_a-001.BA          1           2                -0.838   ";
    } else if (cor_identifier == "K_+1_O") {
      return "K_+1_K2O               +1          0.08300            0.08025      -0.02625           -0.03013      -0.00350            -0.00712        0.08262                     0.07987               -0.47050 "
             "           8                 K_svO/AB2_cF12_225_a_c.BA           2           1                -1.255   ";
    } else if (cor_identifier == "Li_+1_O") {
      return "Li_+1_Li2O             +1          0.07663            0.07038      -0.01538           -0.02225      -0.01175            -0.01863        0.08400                     0.07775               -0.77463 "
             "           8                 Li_svO/AB2_cF12_225_a_c.BA          2           1                -2.066   ";
    } else if (cor_identifier == "Mg_+2_O") {
      return "Mg_+2_MgO              +2          0.13350            0.12717       0.00250           -0.00417      -0.00233            -0.00900        0.13183                     0.12550               -1.03917 "
             "           6                 Mg_pvO/AB_cF8_225_a_b.AB            1           1                -3.118   ";
    } else if (cor_identifier == "Mn_+2_O") {
      return "Mn_+2_MnO              +2          0.25133            0.25133       0.27000            0.26933      -0.03250            -0.03400       -0.01650                    -0.02033               -0.66483 "
             "           6                 Mn_pvO/AB_cF8_225_a_b.AB            1           1                -1.994   ";
    } else if (cor_identifier == "Mn_+4_O") {
      return "Mn_+4_MnO2             +4          0.06000            0.05233      -0.09433           -0.10300      -0.15817            -0.16667        0.11017                     0.10233               -0.89983 "
             "           6                 Mn_pvO/A2B_tP6_136_f_a-001.BA       1           2                -1.8     ";
    } else if (cor_identifier == "Mo_+4_O") {
      return "Mo_+4_MoO2             +4          0.02917            0.02150      -0.18433           -0.19267      -0.10533            -0.11367        0.06400                     0.05650               -1.01550 "
             "           6                 Mo_pvO/A2B_mP12_14_2e_e-002.BA      1           2                -2.031   ";
    } else if (cor_identifier == "Mo_+6_O") {
      return "Mo_+6_MoO3             +6         -0.04700           -0.06025      -0.34100           -0.35750      -0.25575            -0.27175        0.07800                     0.06375               -1.93075 "
             "           4                 Mo_pvO/AB3_oP16_62_c_3c-001.AB      1           3                -1.931   ";
    } else if (cor_identifier == "Na_+1_O") {
      return "Na_+1_Na2O_NJ          +1          0.08225            0.07762      -0.00425           -0.00962      -0.01125            -0.01675        0.08200                     0.07737               -0.54150 "
             "           8                 Na_svO/AB2_cF12_225_a_c.BA          2           1                -1.444   ";
    } else if (cor_identifier == "Nb_+2_O") {
      return "Nb_+2_NbO              +2          0.05925            0.05325      -0.12625           -0.13275      -0.08450            -0.09100        0.03975                     0.03375               -1.08750 "
             "           4                 Nb_svO/AB_cP6_221_c_d.AB            1           1                -2.175   ";
    } else if (cor_identifier == "Ni_+2_O") {
      return "Ni_+2_NiO              +2          0.25583            0.25367       0.15517            0.15117       0.19800             0.19550       -0.07683                    -0.08017               -0.41400 "
             "           6                 Ni_pvO/AB_cF8_225_a_b.AB            1           1                -1.242   ";
    } else if (cor_identifier == "Os_+4_O") {
      return "Os_+4_OsO2             +4          0.06167            0.05700      -0.14433           -0.14983       0.00117            -0.00400       -0.01267                    -0.01717               -0.50883 "
             "           6                 OOs_pv/A2B_tP6_136_f_a-001.AB       1           2                -1.018   ";
    } else if (cor_identifier == "Os_+8_O") {
      return "Os_+8_OsO4             +8         -0.22250           -0.22950      -0.37925           -0.39200      -0.27725            -0.28800       -0.22450                    -0.23175               -1.02000 "
             "           4                 OOs_pv/A4B_mC20_15_2f_e-001.AB      1           4                -0.816   ";
    } else if (cor_identifier == "Pb_+2_O") {
      return "Pb_+2_PbO              +2          0.00275            0.00325      -0.10875           -0.10925      -0.05475            -0.05500       -0.00075                    -0.00025               -0.56850 "
             "           4                 OPb_d/AB_tP4_129_a_c-001.AB         1           1                -1.137   ";
    } else if (cor_identifier == "Pb_+4_O") {
      return "Pb_+4_PbO2             +4          0.05683            0.05450      -0.12500           -0.12850      -0.02500            -0.02833        0.04683                     0.04467               -0.47417 "
             "           6                 OPb_d/A2B_tP6_136_f_a-001.AB        1           2                -0.948   ";
    } else if (cor_identifier == "Pd_+2_O") {
      return "Pd_+2_PdO              +2          0.05775            0.05475      -0.07925           -0.08300      -0.02450            -0.02800       -0.02650                    -0.02950               -0.29925 "
             "           4                 OPd_pv/AB_tP4_131_c_e-002.BA        1           1                -0.599   ";
    } else if (cor_identifier == "Rb_+1_O") {
      return "Rb_+1_Rb2O             +1          0.09500            0.09400      -0.02150           -0.02350       0.00488             0.00313        0.09450                     0.09350               -0.43900 "
             "           8                 ORb_sv/AB2_cF12_225_a_c.AB          2           1                -1.171   ";
    } else if (cor_identifier == "Re_+4_O") {
      return "Re_+4_ReO2_Ba          +4          0.08967            0.08450      -0.12417           -0.13017       0.02117             0.01550        0.05067                     0.04650               -0.77550 "
             "           6                 ORe_pv/A2B_mP12_14_2e_e-002.AB      1           2                -1.551   ";
    } else if (cor_identifier == "Re_+6_O") {
      return "Re_+6_ReO3_Ba          +6         -0.07267           -0.08033      -0.30400           -0.31250      -0.15967            -0.16817       -0.08233                    -0.08967               -1.01767 "
             "           6                 ORe_pv/A3B_cP4_221_d_a.AB           1           3                -1.526   ";
    } else if (cor_identifier == "Rh_+3_O") {
      return "Rh_+3_Rh2O3            +3          0.01408            0.00650      -0.13633           -0.14150      -0.05833            -0.06308       -0.11975                    -0.12783               -0.30717 "
             "           12                ORh_pv/A2B3_hR10_167_c_e-001.BA     2           3                -0.737   ";
    } else if (cor_identifier == "Ru_+4_O") {
      return "Ru_+4_RuO2             +4         -0.00483           -0.01150      -0.20567           -0.21333      -0.11183            -0.11917       -0.05283                    -0.05800               -0.52683 "
             "           6                 ORu_pv/A2B_tP6_136_f_a-001.AB       1           2                -1.054   ";
    } else if (cor_identifier == "Sb_+3_O") {
      return "Sb_+3_Sb2O3            +3          0.12067            0.11533      -0.11350           -0.12117      -0.01983            -0.02667        0.11733                     0.11217               -1.23700 "
             "           6                 OSb/A3B2_cF80_227_f_e-001.AB        2           3                -1.484   ";
    } else if (cor_identifier == "Sb_+5_O") {
      return "Sb_+5_Sb2O5_Ba         +5          0.10517            0.09700      -0.13233           -0.14175      -0.05725            -0.06692        0.09825                     0.09008               -0.83942 "
             "           12                OSb/A2B5_mC28_15_f_e2f-002.BA       2           5                -1.439   ";
    } else if (cor_identifier == "Sc_+3_O") {
      return "Sc_+3_Sc2O3            +3          0.16175            0.15408      -0.00833           -0.01658      -0.02567            -0.03383        0.16608                     0.15867               -1.64817 "
             "           12                OSc_sv/A2B3_cI80_206_ad_e-001.BA    2           3                -3.956   ";
    } else if (cor_identifier == "Se_+4_O") {
      return "Se_+4_SeO2             +4          0.07500            0.06567      -0.22767           -0.23967      -0.09633            -0.10833        0.07300                     0.06367               -0.77767 "
             "           3                 OSe/A2B_tP24_135_gh_h-001.AB        1           2                -0.778   ";
    } else if (cor_identifier == "Si_+4_O") {
      return "Si_+4_SiO2(al-quartz)  +4          0.25300            0.23800      -0.02325           -0.03900      -0.02075            -0.03675        0.25275                     0.23775               -2.36025 "
             "           4                 OSi/A2B_hP9_152_c_a-001.AB          1           2                -3.147   ";
    } else if (cor_identifier == "Sn_+2_O") {
      return "Sn_+2_SnO_Ba           +2          0.06700            0.06500      -0.06375           -0.06650      -0.01300            -0.01575        0.12025                     0.11825               -0.74050 "
             "           4                 OSn/AB_tP4_129_a_c-001.AB           1           1                -1.481   ";
    } else if (cor_identifier == "Sn_+4_O") {
      return "Sn_+4_SnO2_Ba          +4          0.15050            0.14333      -0.04600           -0.05400      -0.01533            -0.02367        0.28650                     0.28000               -1.00333 "
             "           6                 OSn/A2B_tP6_136_f_a-001.AB          1           2                -2.007   ";
    } else if (cor_identifier == "Sr_+2_O") {
      return "Sr_+2_SrO              +2          0.10817            0.10483      -0.01917           -0.02317      -0.01550            -0.01933        0.10767                     0.10433               -1.02267 "
             "           6                 OSr_sv/AB_cF8_225_a_b.AB            1           1                -3.068   ";
    } else if (cor_identifier == "Te_+4_O") {
      return "Te_+4_TeO2_Ba          +4          0.06300            0.05575      -0.20325           -0.21225      -0.08850            -0.09700        0.06275                     0.05575               -0.83800 "
             "           4                 OTe/A2B_tP12_92_b_a-002.AB          1           2                -1.117   ";
    } else if (cor_identifier == "Ti_+2_O") {
      return "Ti_+2_TiO              +2          0.11313            0.10667      -0.06667           -0.07375      -0.02646            -0.03312        0.10479                     0.09958               -1.17188 "
             "           4.8               OTi_sv/AB_mC20_12_b2i_c2i-001.AB    1           1                -2.812   ";
    } else if (cor_identifier == "Ti_+3_O") {
      return "Ti_+3_Ti2O3            +3          0.09800            0.09050      -0.08550           -0.09358      -0.06875            -0.07667        0.07933                     0.07233               -1.31358 "
             "           12                OTi_sv/A2B3_hR10_167_c_e-001.BA     2           3                -3.153   ";
    } else if (cor_identifier == "Ti_+4_O") {
      return "Ti_+4_TiO2(rutile)     +4          0.10717            0.09717      -0.09650           -0.10717      -0.12367            -0.13450        0.05217                     0.04233               -1.63067 "
             "           6                 OTi_sv/A2B_tP6_136_f_a-001.AB       1           2                -3.261   ";
    } else if (cor_identifier == "Tl_+1_O") {
      return "Tl_+1_Tl2O             +1         -0.00650           -0.00533      -0.06650           -0.06600      -0.06117            -0.06050       -0.00783                    -0.00667               -0.28917 "
             "           6                 OTl_d/AB2_hR6_166_c_2c-001.AB       2           1                -0.578   ";
    } else if (cor_identifier == "Tl_+3_O") {
      return "Tl_+3_Tl2O3            +3          0.05358            0.05175      -0.09358           -0.09617      -0.01400            -0.01658        0.04492                     0.04300               -0.33717 "
             "           12                OTl_d/A2B3_cI80_206_ad_e-001.BA     2           3                -0.809   ";
    } else if (cor_identifier == "V_+2_O") {
      return "V_+2_VO                +2          0.26367            0.26200       0.12033            0.11517       0.15683             0.15467        0.24633                     0.24167               -0.74583 "
             "           6                 OV_sv/AB_cF8_225_a_b.AB             1           1                -2.237   ";
    } else if (cor_identifier == "V_+3_O") {
      return "V_+3_V2O3              +3          0.09825            0.09183      -0.06608           -0.07342      -0.05350            -0.06000       -0.04833                    -0.05458               -1.05267 "
             "           12                OV_sv/A2B3_hR10_167_c_e-002.BA      2           3                -2.526   ";
    } else if (cor_identifier == "V_+4_O") {
      return "V_+4_VO2               +4          0.04667            0.03750      -0.14967           -0.15983      -0.15367            -0.16367       -0.02633                    -0.03600               -1.23300 "
             "           6                 OV_sv/A2B_mP12_14_2e_e-003.AB       1           2                -2.466   ";
    } else if (cor_identifier == "V_+5_O") {
      return "V_+5_V2O5              +5         -0.00820           -0.01890      -0.21180           -0.22480      -0.21790            -0.23070        0.00730                    -0.00370               -1.60670 "
             "           10                OV_sv/A5B2_oP14_59_a2e_e-001.AB     2           5                -2.295   ";
    } else if (cor_identifier == "W_+4_O") {
      return "W_+4_WO2               +4          0.05667            0.05117      -0.15867           -0.16483      -0.04833            -0.05433        0.05417                     0.04883               -1.01833 "
             "           6                 OW_pv/A2B_mP12_14_2e_e-002.AB       1           2                -2.037   ";
    } else if (cor_identifier == "W_+6_O") {
      return "W_+6_WO3               +6          0.00500           -0.00250      -0.20783           -0.21650      -0.13617            -0.14483        0.00867                     0.00117               -1.45567 "
             "           6                 OW_pv/A3B_mP16_14_3e_e-001.AB       1           3                -2.183   ";
    } else if (cor_identifier == "Y_+3_O") {
      return "Y_+3_Y2O3              +3          0.13583            0.13042      -0.02192           -0.02800      -0.04825            -0.05425        0.15800                     0.15250               -1.64533 "
             "           12                OY_sv/A2B3_cI80_206_ad_e-001.BA     2           3                -3.949   ";
    } else if (cor_identifier == "Zn_+2_O") {
      return "Zn_+2_ZnO              +2          0.18525            0.18025       0.04225            0.03675       0.04550             0.03975        0.00300                    -0.00250               -0.90825 "
             "           4                 OZn/AB_hP4_186_b_b-001.AB           1           1                -1.817   ";
    } else if (cor_identifier == "Zr_+4_O") {
      return "Zr_+4_ZrO2_NJ          +4          0.13929            0.13200      -0.04514           -0.05300      -0.06314            -0.07086        0.18086                     0.17371               -1.62486 "
             "           7                 OZr_sv/A2B_mP12_14_2e_e-001.AB      1           2                -3.791   ";
    }
    // corrections for per- and superoxides
    else if (cor_identifier == "O2_-2_O") {
      return "O2_-2_Li2O2            -1         -0.09300           -0.08556      -0.11600           -0.11100       0.24000             0.24756       -0.12700                    -0.12000                2.72556 "
             "           1                 -                                   2           2                0.0      ";
    } else if (cor_identifier == "O2_-1_O") {
      return "O2_-1_KO2              -0.5       -0.54500           -0.54350      -0.26900           -0.26970      -0.04900            -0.04680       -0.54320                    -0.54170                1.75600 "
             "           1                 -                                   1           2                0.0      ";
    } else {
      return "";
    }
  ...
  string get_corrections_line_N(const string& cor_identifier) {
    ...
    // However when I compare the values with my calculated LDA, PBE and SCAN data, the -1.10 eV/formula unit fits better to the overall fact that LDA and SCAN overestimate the formation enthalpy (too negative
    // value) and PBE does the opposite. With the newer value of -1.62 eV/formula unit this would not be the case. Therefore I took the old tabulated value for deducing the corrections (January 07 2020).
    //                                           cation species            ox. state  PBE (for 298.15K)  PBE (for 0K)  LDA (for 298.15K)  LDA (for 0K)  SCAN (for 298.15K)  SCAN (for 0K)  PBE(+U)_ICSD (for
    //                                           298.15K) PBE(+U)_ICSD (for 0K)  Exp. (for 298.15K)  M-X bonds       Protos                               n cations   n anions        H_f^exp (for 298.15K)
    //                                                                                (eV/bond)          (eV/bond)     (eV/bond)          (eV/bond)     (eV/bond)           (eV/bond)      (eV/bond) (eV/bond)
    //                                                                                (eV/bond) per f. u.                                            per f. u.   per f. u.       (eV/bond) TABLE IV in supp.
    // NITRIDES
    if (cor_identifier == "Al_+3_N") {
      return "Al_+3_AlN              +3          0.11875            0.10850      -0.04575           -0.05575      -0.03850            -0.04875        0.11800                     0.10825               -0.82500 "
             "           4                AlN/AB_hP4_186_b_b-001.AB            1            1                0.0    ";
    } else if (cor_identifier == "B_+3_N") {
      return "B_+3_BN_NJ             +3          0.00100           -0.00867      -0.15933           -0.17000      -0.11000            -0.12067        0.00167                    -0.00767               -0.86700 "
             "           3                B_hN/AB_hP4_194_c_d-001.AB           1            1                0.0    ";
    } else if (cor_identifier == "Ca_+2_N") {
      return "Ca_+2_Ca3N2_Ba         +2          0.03967            0.03400      -0.09225           -0.09792      -0.04392            -0.04942        0.03958                     0.03442               -0.37225 "
             "           12               Ca_svN/A2B3_cI80_206_ad_e-001.BA     3            2                0.0    ";
    } else if (cor_identifier == "Li_+1_N") {
      return "Li_+1_Li3N             +1          0.01663            0.00912      -0.06850           -0.07562      -0.03588            -0.04338        0.02725                     0.02013               -0.21350 "
             "           8                Li_svN/A3B_hP4_191_bc_a-001.AB       3            1                0.0    ";
    } else if (cor_identifier == "Zn_+2_N") {
      return "Zn_+2_Zn3N2            +2          0.06733            0.06225      -0.03492           -0.03908       0.00417            -0.00025       -0.06792                    -0.07242               -0.01950 "
             "           12               NZn/A2B3_cI80_206_ad_e-001.AB        3            2                0.0    ";
    } else if (cor_identifier == "Be_+2_N") {
      return "Be_+2_Be3N2            +2          0.05908            0.05292      -0.06500           -0.07133      -0.03433            -0.04075        0.05875                     0.05258               -0.50917 "
             "           12               Be_svN/A2B3_cI80_206_ad_e-001.BA     3            2                0.0    ";
    } else if (cor_identifier == "Cr_+3_N") {
      return "Cr_+3_CrN              +3          0.08417            0.08167      -0.02100           -0.02583      -0.01867            -0.02233       -0.22883                    -0.23283               -0.20250 "
             "           6                Cr_pvN/AB_cF8_225_a_b.AB             1            1                0.0    ";
    } else if (cor_identifier == "Ga_+3_N") {
      return "Ga_+3_GaN_!!!          +3          0.04725            0.03950      -0.11725           -0.12550      -0.04200            -0.05075        0.01550                     0.00750               -0.28400 "
             "           4                Ga_hN/AB_hP4_186_b_b-001.AB          1            1                0.0    ";
    } else if (cor_identifier == "Hf_+3_N") {
      return "Hf_+3_HfN              +3          0.05983            0.05633      -0.09283           -0.09683      -0.01433            -0.01817        0.05933                     0.05583               -0.64533 "
             "           6                Hf_pvN/AB_cF8_225_a_b.AB             1            1                0.0    ";
    } else if (cor_identifier == "In_+3_N") {
      return "In_+3_InN_Ba           +3          0.08825            0.08350      -0.05700           -0.06225      -0.01125            -0.01650        0.04800                     0.04325               -0.04450 "
             "           4                In_dN/AB_hP4_186_b_b-001.AB          1            1                0.0    ";
    } else if (cor_identifier == "La_+3_N") {
      return "La_+3_LaN_Ba           +3          0.08300            0.07983      -0.00100           -0.00433      -0.01600            -0.01950        0.13183                     0.12883               -0.52400 "
             "           6                LaN/AB_cF8_225_a_b.AB                1            1                0.0    ";
    } else if (cor_identifier == "Mg_+2_N") {
      return "Mg_+2_Mg3N2            +2          0.08300            0.07642      -0.03475           -0.04167      -0.01242            -0.01950        0.08083                     0.07425               -0.39858 "
             "           12               Mg_pvN/A2B3_cI80_206_ad_e-001.BA     3            2                0.0    ";
    } else if (cor_identifier == "Nb_+3_N") {
      return "Nb_+3_NbN              +3          0.07467            0.06967      -0.06550           -0.07083       0.02667             0.02150        0.07400                     0.06900               -0.40833 "
             "           6                NNb_sv/AB_cF8_225_a_b.AB             1            1                0.0    ";
    } else if (cor_identifier == "Sc_+3_N") {
      return "Sc_+3_ScN              +3         -0.09850           -0.10500      -0.23150           -0.23833      -0.20667            -0.21350       -0.07883                    -0.08517               -0.54200 "
             "           6                NSc_sv/AB_cF8_225_a_b.AB             1            1                0.0    ";
    } else if (cor_identifier == "Si_+4_N") {
      return "Si_+4_Si3N4            +4          0.00000           -0.01300      -0.22408           -0.23733      -0.15183            -0.16508       -0.00058                    -0.01358               -0.64325 "
             "           12               NSi/A4B3_hP14_173_bc_c-001.AB        3            4                0.0    ";
    } else if (cor_identifier == "Ta_+3_N") {
      return "Ta_+3_TaN              +3          0.15283            0.14933       0.00617            0.00233       0.13100             0.12733        0.15650                     0.15317               -0.43583 "
             "           6                NTa_pv/AB_cF8_225_a_b.AB             1            1                0.0    ";
    } else if (cor_identifier == "Ti_+3_N") {
      return "Ti_+3_TiN              +3          0.00367           -0.00283      -0.14550           -0.15233      -0.08733            -0.09400        0.03083                     0.02467               -0.58400 "
             "           6                NTi_sv/AB_cF8_225_a_b.AB             1            1                0.0    ";
    } else if (cor_identifier == "V_+3_N") {
      return "V_+3_VN                +3          0.04500            0.03917      -0.09483           -0.10100      -0.01683            -0.02250        0.03283                     0.03100               -0.37650 "
             "           6                NV_sv/AB_cF8_225_a_b.AB              1            1                0.0    ";
    } else if (cor_identifier == "Y_+3_N") {
      return "Y_+3_YN                +3         -0.06283           -0.06750      -0.18267           -0.18783      -0.16950            -0.17450       -0.04933                    -0.05400               -0.51683 "
             "           6                NY_sv/AB_cF8_225_a_b.AB              1            1                0.0    ";
    } else if (cor_identifier == "Zr_+3_N") {
      return "Zr_+3_ZrN_NJ           +3          0.04250            0.03733      -0.09933           -0.10500      -0.04067            -0.04617        0.06783                     0.06267               -0.63100 "
             "           6                NZr_sv/AB_cF8_225_a_b.AB             1            1                0.0    ";
    }
    ...
    else if (cor_identifier == "Na_+1_N") {
      return "Na_+1_Na3N_Vajenine07* +1          0.03850            0.03400      -0.04367           -0.04883      -0.01150            -0.01683        0.03850                     0.03400                0.11050 "
             "           6                -                                    3            1                0.663  ";
    } else if (cor_identifier == "W_+6_N") {
      return "W_+6_Na3WN3_Elder93*   +6         -0.02325           -0.03050      -0.08401           -0.10626      -0.02850            -0.03251       -0.00025                    -0.00750               -1.26100 "
             "           4                -                                    1            3                -3.718 ";
    } else if (cor_identifier == "Mo_+5_N") {
      return "Mo_+5_LiMoN2_Elder93*  +5          0.28370            0.28038       0.15583            0.15129       0.24888             0.24488        0.28625                     0.28304               -0.45333 "
             "           6                -                                    1            2                -4.001 ";
    }
    ...
    else if (cor_identifier == "W_+4_N") {
      return "W_+4_Na3WN3_Elder93*^  +4         -0.02325           -0.03050      -0.08401           -0.10626      -0.02850            -0.03251       -0.00025                    -0.00750               -1.26100 "
             "           4                -                                    1            3                -3.718 ";
    } else if (cor_identifier == "Mo_+4_N") {
      return "Mo_+4_LiMoN2_Elder93*^ +4          0.28370            0.28038       0.15583            0.15129       0.24888             0.24488        0.28625                     0.28304               -0.45333 "
             "           6                -                                    1            2                -4.001 ";
    } else {
      return "";
    }
  ...
  string get_Bader_templates(const string& element) {
    ...
    if (element == "Ag") {
      return "1   +1   0.4794   0.4659   0.4992   0.4513";
    }
    if (element == "Al") {
      return "1   +3   2.4800   2.4533   2.5188   2.4706";
    }
    if (element == "As") {
      return "1   +5   2.5884   2.6167   2.7515   2.5858";
    }
    if (element == "B") {
      return "1   +3   2.3417   2.3087   2.4076   2.3261";
    }
    if (element == "Ba") {
      return "1   +2   1.4375   1.4110   1.4636   1.4534";
    }
    if (element == "Be") {
      return "1   +2   1.7043   1.6823   1.7276   1.7159";
    }
    if (element == "Bi") {
      return "1   +3   1.7693   1.7505   1.7962   1.7707";
    }
    if (element == "Ca") {
      return "1   +2   1.4772   1.4152   1.5117   1.4828";
    }
    if (element == "Cd") {
      return "1   +2   1.1479   1.0778   1.2298   1.1791";
    }
    if (element == "Co") {
      return "1   +2   1.1974   1.1351   1.2440   1.2665";
    }
    if (element == "Cr") {
      return "2   +3   1.7009   1.6240   1.7560   1.7671   +6   2.0198   1.9856   2.1249   2.0498";
    }
    if (element == "Cs") {
      return "1   +1   0.6818   0.6747   0.7132   0.6845";
    }
    if (element == "Cu") {
      return "2   +1   0.5287   0.5236   0.5464   0.5105   +2   0.9651   0.9614   1.0033   1.0437";
    }
    if (element == "Fe") {
      return "2   +2   1.2937   1.2688   1.3440   1.3784   +3   1.6233   1.3824   1.7882   1.8411";
    }
    if (element == "Ga") {
      return "1   +3   1.8610   1.8334   1.9507   1.8600";
    }
    if (element == "Ge") {
      return "1   +4   2.3971   2.3777   2.5471   2.4253";
    }
    if (element == "Hf") {
      return "1   +4   2.5659   2.4994   2.6054   2.5587";
    }
    if (element == "Hg") {
      return "1   +2   0.8813   0.8887   0.9201   0.8884";
    }
    if (element == "In") {
      return "1   +3   1.8191   1.7843   1.9155   1.8147";
    }
    if (element == "Ir") {
      return "1   +4   1.6523   1.6448   1.6469   1.6083";
    }
    if (element == "K") {
      return "1   +1   0.7458   0.7101   0.7660   0.7265";
    }
    if (element == "Li") {
      return "1   +1   0.8162   0.7974   0.8223   0.8372";
    }
    if (element == "Mg") {
      return "1   +2   1.6973   1.6729   1.7147   1.6821";
    }
    if (element == "Mn") {
      return "2   +2   1.3286   1.2956   1.4272   1.3775   +4   1.8460   1.7701   1.9347   1.8942";
    }
    if (element == "Mo") {
      return "2   +4   2.0898   2.0799   2.1917   2.0983   +6   2.6270   2.6499   2.7532   2.5918";
    }
    if (element == "Na") {
      return "1   +1   0.7868   0.7676   0.8073   0.7888";
    }
    if (element == "Nb") {
      return "1   +2   1.3151   1.3781   1.3329   1.2806";
    }
    if (element == "Ni") {
      return "1   +2   1.1075   1.0731   1.1578   1.2148";
    }
    if (element == "Os") {
      return "2   +4   1.8518   1.8489   1.9032   1.8615   +8   2.6008   2.5884   2.6683   2.5591";
    }
    if (element == "Pb") {
      return "2   +2   1.1513   1.1301   1.1698   1.1493   +4   2.0173   2.0324   2.1245   2.0469";
    }
    if (element == "Pd") {
      return "1   +2   0.8312   0.8326   0.8630   0.8242";
    }
    if (element == "Rb") {
      return "1   +1   0.7401   0.7124   0.7634   0.7192";
    }
    if (element == "Re") {
      return "2   +4   2.0264   2.0190   2.0771   2.0627   +6   2.9344   2.9106   3.0224   2.8782";
    }
    if (element == "Rh") {
      return "1   +3   1.2622   1.2465   1.3048   1.2867";
    }
    if (element == "Ru") {
      return "1   +4   1.7022   1.6972   1.7991   1.8402";
    }
    if (element == "Sb") {
      return "2   +3   1.7754   1.7743   1.8537   1.7742   +5   2.7273   2.7153   2.8873   2.7311";
    }
    if (element == "Sc") {
      return "1   +3   2.0251   1.9722   2.0868   2.1215";
    }
    if (element == "Se") {
      return "1   +4   1.9132   1.9535   2.0425   1.8766";
    }
    if (element == "Si") {
      return "1   +4   3.2043   3.1655   3.2758   3.1716";
    }
    if (element == "Sn") {
      return "2   +2   1.1884   1.1704   1.2280   1.1716   +4   2.2780   2.2608   2.4336   2.2097";
    }
    if (element == "Sr") {
      return "1   +2   1.4895   1.4274   1.5335   1.4767";
    }
    if (element == "Te") {
      return "1   +4   2.2551   2.2661   2.3516   2.2452";
    }
    if (element == "Ti") {
      return "3   +2   1.2335   1.2183   1.2637   1.2658   +3   1.9228   1.8736   1.9955   2.0175   +4   2.2327   2.1844   2.3356   2.3956";
    }
    if (element == "Tl") {
      return "2   +1   0.5628   0.5488   0.5779   0.5640   +3   1.4739   1.4718   1.5568   1.4781";
    }
    if (element == "V") {
      return "4   +2   1.4271   1.4437   1.4306   1.5312   +3   1.7739   1.7286   1.8327   1.8783   +4   2.0728   2.0077   2.1737   2.1641   +5   2.1742   2.1430   2.2781   2.1982";
    }
    if (element == "W") {
      return "2   +4   2.2270   2.1974   2.2933   2.2501   +6   2.9727   2.9284   3.0812   2.9838";
    }
    if (element == "Y") {
      return "1   +3   2.1451   2.1027   2.2107   2.1473";
    }
    if (element == "Zn") {
      return "1   +2   1.2134   1.2091   1.2764   1.1739";
    }
    if (element == "Zr") {
      return "1   +4   2.5731   2.5281   2.6697   2.5711";
    }
    // corrections for per- (Li2O2) and superoxides (KO2)
    if (element == "O") {
      return "2   -1   -0.8505  -0.8275  -0.8534  -0.8765  -0.5 -0.4334  -0.4157  -0.4338  -0.4313";
    } else {
      return "";
    }
  }
  ...
  double get_ref_enthalpy_shift_pbe_u_icsd(const string& element) {
    if (element == "Sc") {
      return 1.1610235;
    } // AUID="4b48722b5ae8f9e9"
    if (element == "Ti") {
      return 3.12868285;
    } // AUID="9cdd346558c528c6"
    if (element == "V") {
      return 2.7806973;
    } // AUID="94da8dda3e65c762"
    if (element == "Cr") {
      return 4.0654442;
    } // AUID="f020a26862c0a532"
    if (element == "Mn") {
      return 2.5458603448275854;
    } // AUID="99be850476e2dfb3"
    if (element == "Fe") {
      return 3.4640268;
    } // AUID="15e65f5e50047695"
    if (element == "Co") {
      return 4.0265057;
    } // AUID="9c4653e42c9f870b"
    if (element == "Ni") {
      return 3.7335372;
    } // AUID="ead949d4e6fa93d1"
    if (element == "Cu") {
      return 1.887473;
    } // AUID="62263cf2c885649c"
    if (element == "Zn") {
      return 0.80815757;
    } // AUID="d4ad14ef9da00329"
    if (element == "Ga") {
      return 0.486032725;
    } // AUID="567d70c3d85e2c7f"
    if (element == "Nb") {
      return 1.8050886;
    } // AUID="141a7bea5919e0b3"
    if (element == "Mo") {
      return 2.7869557;
    } // AUID="444a5f723c4c0576"
    if (element == "Tc") {
      return 3.3680085;
    } // AUID="c33964edb4f74ebf"
    if (element == "Ru") {
      return 3.613056;
    } // AUID="a3d039d8d88a4a86"
    if (element == "Rh") {
      return 3.436254;
    } // AUID="b1547f3221c0c660"
    if (element == "Pd") {
      return 2.7531925;
    } // AUID="4af3f66c14a34cee"
    if (element == "Ag") {
      return 2.40546596;
    } // AUID="eca65d7d992efb48"
    if (element == "Cd") {
      return 0.47503727;
    } // AUID="3388a058d6fd344d"
    if (element == "In") {
      return 0.1350879;
    } // AUID="5ce1eee07a5df3a7"
    if (element == "Sn") {
      return 0.1359635;
    } // AUID="37c5d78c5699ee73"
    if (element == "Ta") {
      return 1.772861;
    } // AUID="926ce9924f7a5fd2"
    if (element == "W") {
      return 2.388796;
    } // AUID="cad81c4b63d2d96e"
    if (element == "Re") {
      return 2.957064;
    } // AUID="d72276b27490b853"
    if (element == "Os") {
      return 3.1945335;
    } // AUID="216ecda62b9b32bf"
    if (element == "Ir") {
      return 2.8837526;
    } // AUID="9c41e654d31db16a"
    if (element == "Pt") {
      return 2.4902537;
    } // AUID="59081d68fabd053c"
    if (element == "La") {
      return 0.139627;
    } // AUID="8353806ee117d7fd"
    if (element == "Ce") {
      return AUROSTD_NAN;
    } // AUID="6e846dc10a52b11b"; no groundstate_energy in aflow_xpseudopotentials_data.cpp yet
    if (element == "Pr") {
      return AUROSTD_NAN;
    } // AUID="82dbd5ee06192a4c"; no groundstate_energy in aflow_xpseudopotentials_data.cpp yet
    if (element == "Nd") {
      return AUROSTD_NAN;
    } // AUID="81811d7c33a85a8b"; no groundstate_energy in aflow_xpseudopotentials_data.cpp yet
    if (element == "Sm") {
      return AUROSTD_NAN;
    } // AUID="e892f33edd3901b8"; no groundstate_energy in aflow_xpseudopotentials_data.cpp yet
    if (element == "Eu") {
      return 0.3926978;
    } // AUID="9b801fd8fd52e116"
    if (element == "Gd") {
      return AUROSTD_NAN;
    } // AUID="c0f01c59eb971d30"; no groundstate_energy in aflow_xpseudopotentials_data.cpp yet
    if (element == "Dy") {
      return 0.00023385;
    } // AUID="0373199190a8abb7"; calc. with and without U gives basically same energy
    if (element == "Tm") {
      return AUROSTD_NAN;
    } // AUID="766b44cce10b0315"; no groundstate_energy in aflow_xpseudopotentials_data.cpp yet
    if (element == "Yb") {
      return 0.5301314;
    } // AUID="de1c22a384a8d945"
    if (element == "Lu") {
      return AUROSTD_NAN;
    } // AUID="98b3a5d589b252b5"; no groundstate_energy in aflow_xpseudopotentials_data.cpp yet
    if (element == "Th") {
      return AUROSTD_NAN;
    } // AUID="78f7995fc479457d"; no groundstate_energy in aflow_xpseudopotentials_data.cpp yet
    if (element == "U") {
      return AUROSTD_NAN;
    } // AUID="2f0fba94372190f4"; no groundstate_energy in aflow_xpseudopotentials_data.cpp yet
    else {
      return -1;
    }
  }
