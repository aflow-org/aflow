...
namespace AGL_functions {
  ...
  uint polynomial_fit_weight_ave(uint& imin, vector<double>& polynomialcoeffs, vector<double>& polynomialerrors, vector<double>& xdata_to_fit, vector<double>& ydata_to_fit, _AGL_data& AGL_data, ofstream& FileMESSAGE) {
    ...
    vector<int> npolycoeffworkfit(AGL_data.maxfit);
    vector<int> ndatafit(AGL_data.maxfit);
    // Service variables:
    int npolycoeffworkmax;
    int ifit;
    double weight_norm;
    double weight_rms;
    double weight_rms_gaussian;
    double rmsmin;
    double deriv_prod;
    double plnvp;
    double plnvm;
    const int ndatapoints = ydata_to_fit.size();
    ...
    const int max_poly_coeffs = ndatapoints - nlimit - 1;
    ...
    while ((ndataiterfit >= ndatamin) && ((first_entry_tofit < imin) && (last_entry_tofit > imin))) {
      ...
      npolycoeffworkmax = std::min(ndataiterfit - nlimit - 1, AGL_data.maxpolycoeffs);
      for (npolycoeffwork = npolycoeffworkmin; npolycoeffwork <= npolycoeffworkmax; npolycoeffwork++) {
        ...
        aglerror = AGL_functions::polynom_eval(xdata_to_fit[imin - 1], polycoeffwork, plnvm, 1);
        ...
        // Save parameters obtained from fitting procedure
          ...
          npolynomialcoeffs = std::max(npolynomialcoeffs, npolycoeffwork);
          npolycoeffworkfit[nfit] = npolycoeffwork;
          ndatafit[nfit] = ndataiterfit;
          rmsmin = std::min(rmsmin, AGL_data.rms * npolycoeffwork / ndataiterfit);
        }
      }
    ...
    for (ifit = 1; ifit <= nfit; ifit++) {
      ...
      ndataiterfit = ndatafit[ifit];
      npolycoeffwork = npolycoeffworkfit[ifit];
      polycoeffwork.resize(npolycoeffwork + 1);
      ...
      for (int i = 0; i <= npolycoeffwork; i++) {
        polynomialcoeffs[i] = polynomialcoeffs[i] + weight_rms_gaussian * polycoeffwork[i];
        polynomialerrors.at(2 * i) = polynomialerrors.at(2 * i) + weight_rms_gaussian * polycoeffwork[i] * polycoeffwork[i];
        for (int j = 0; j < i; j++) {
          polynomialerrors.at(i + j) = polynomialerrors.at(i + j) + 2.0 * weight_rms_gaussian * polycoeffwork[j] * polycoeffwork[i];
        }
      }
    ...
    for (int i = 0; i <= npolynomialcoeffs; i++) {
      polynomialcoeffs[i] = polynomialcoeffs[i] / weight_norm;
      polynomialerrors[i] = polynomialerrors[i] / weight_norm;
    }
...
namespace AGL_functions {
  ...
  uint polynomial_fit_weight_ave_debug(uint& imin, vector<double>& polynomialcoeffs, vector<double>& polynomialerrors, vector<double>& xdata_to_fit, vector<double>& ydata_to_fit, _AGL_data& AGL_data, ofstream& FileMESSAGE) {
    ...
    const string ofilefittname = AGL_data.dirpathname + "/AGL_Plot_Fitting_coefficients.dat";
    stringstream ofilefittss;
    const string ofileplotxname = AGL_data.dirpathname + "/AGL_Polynomial_plot_x.dat";
    stringstream ofileplotxss;
    const string ofileplotvname = AGL_data.dirpathname + "/AGL_Polynomial_plot_v.dat";
    stringstream ofileplotvss;
    const string ofileplotfitname = AGL_data.dirpathname + "/AGL_Polynomial_plot_fit.dat";
    stringstream ofileplotfitss;
    const string ofileplotafitname = AGL_data.dirpathname + "/AGL_Polynomial_plot_all_fits.dat";
    stringstream ofileplotafitss;
    const string ofilermsname = AGL_data.dirpathname + "/AGL_Polynomial_plot_rms.dat";
    stringstream ofilermsss;

    for (size_t i = 0; i < xdata_to_fit.size(); i++) {
      ofileplotxss << xdata_to_fit[i] << "\t" << ydata_to_fit[i] << endl;
    }

    for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
      ofileplotvss << AGL_data.volumeinput[i] << "\t" << AGL_data.energyinput[i] << endl;
    }
    ...
    while ((ndataiterfit >= ndatamin) && ((first_entry_tofit < imin) && (last_entry_tofit > imin))) {
      ...
      for (npolycoeffwork = npolycoeffworkmin; npolycoeffwork <= npolycoeffworkmax; npolycoeffwork++) {
        ...
        for (size_t ij = 0; ij < polycoeffwork.size(); ij++) {
          aus << polycoeffwork[ij] << "\t";
        }
        aus << endl;
        aus << _AGLSTR_MESSAGE_ + "polynomial_fit_weight_ave_debug: nfit = " << nfit << endl;
        aus << _AGLSTR_MESSAGE_ + "polynomial_fit_weight_ave_debug: first_entry_tofit = " << first_entry_tofit << ", last_entry_tofit = " << last_entry_tofit << endl;
        ...
        for (int ij = 0; ij < npolycoeffwork; ij++) {
          ofilefittss << "polycoeffwork[" << ij << "] = " << polycoeffwork[ij] << endl;
        }
        if (pferr != 0) {
          return 2;
        }
    ...
    for (ifit = 1; ifit <= nfit; ifit++) {
      allfits[ifit - 1].resize(ydata_to_fit.size());
      ndataiterfit = ndatafit[ifit];
      ...
      for (int ij = 0; ij < npolycoeffwork; ij++) {
        ...
        for (size_t i = 0; i < ydata_to_fit.size(); i++) {
          aglerror = AGL_functions::polynom_eval(xdata_to_fit[i], polynomialcoeffs, plnvf, 0);
          allfits[ifit - 1][i] = plnvf;
        }
      }
      if (pferr != 0) {
        return 2;
    ...
    for (int ij = 0; ij < npolynomialcoeffs; ij++) {
      ofilefittss << "polynomialcoeffs[" << ij << "] = " << polynomialcoeffs[ij] << endl;
    }
    ...
    for (size_t i = 0; i < xdata_to_fit.size(); i++) {
      aglerror = AGL_functions::polynom_eval(xdata_to_fit[i], polynomialcoeffs, plnvf, 0);
      ofileplotfitss << xdata_to_fit[i] << "\t" << plnvf << endl;
    }

    for (size_t i = 0; i < xdata_to_fit.size(); i++) {
      ofileplotafitss << xdata_to_fit[i];
      for (ifit = 1; ifit <= nfit; ifit++) {
        ofileplotafitss << "\t" << allfits[ifit - 1][i];
      }
    ...
    ofilefittss << endl;
    if (!aurostd::stringstream2file(ofilefittss, ofilefittname, aurostd::compression_type::None, "WRITE")) {
      aurostd::StringstreamClean(aus);
    ...
    if (!aurostd::stringstream2file(ofileplotxss, ofileplotxname, aurostd::compression_type::None, "WRITE")) {
      aurostd::StringstreamClean(aus);
    ...
    if (!aurostd::stringstream2file(ofileplotvss, ofileplotvname, aurostd::compression_type::None, "WRITE")) {
      aurostd::StringstreamClean(aus);
    ...
    if (!aurostd::stringstream2file(ofileplotfitss, ofileplotfitname, aurostd::compression_type::None, "WRITE")) {
      aurostd::StringstreamClean(aus);
    ...
    if (!aurostd::stringstream2file(ofileplotafitss, ofileplotafitname, aurostd::compression_type::None, "WRITE")) {
      aurostd::StringstreamClean(aus);
    ...
    if (!aurostd::stringstream2file(ofilermsss, ofilermsname, aurostd::compression_type::None, "WRITE")) {
      aurostd::StringstreamClean(aus);
...
namespace AGL_functions {
  ...
  uint bracket_minimum(uint& imin, vector<double>& ydata_to_fit, ofstream& FileMESSAGE) {
    ...
    istep = 0;
    if (ydata_to_fit[imin] < ydata_to_fit.at(imin + 1) && ydata_to_fit[imin] < ydata_to_fit[imin - 1]) {
      return 0;
    } else if (aurostd::abs(ydata_to_fit[imin] - ydata_to_fit.at(imin + 1)) < tol && aurostd::abs(ydata_to_fit[imin] - ydata_to_fit[imin - 1]) < tol) {
      aurostd::StringstreamClean(aus);
      aus << _AGLSTR_WARNING_ + "bracket_minimum: Flat function, impossible to search" << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      return 2;
    } else if (ydata_to_fit[imin] >= ydata_to_fit.at(imin + 1) && ydata_to_fit[imin] <= ydata_to_fit[imin - 1]) {
      istep = 1;
    } else if (ydata_to_fit[imin] <= ydata_to_fit.at(imin + 1) && ydata_to_fit[imin] >= ydata_to_fit[imin - 1]) {
      istep = -1;
    } else {
      ...
      aus << _AGLSTR_WARNING_ + "bracket_minimum: imin = " << imin << endl;
      aus << _AGLSTR_WARNING_ + "bracket_minimum: ydata_to_fit[imin - 1] = " << ydata_to_fit[imin - 1] << endl;
      aus << _AGLSTR_WARNING_ + "bracket_minimum: ydata_to_fit[imin] = " << ydata_to_fit[imin] << endl;
    ...
    while (imin > 0 && imin < (ydata_to_fit.size() - 1)) {
      if (ydata_to_fit[imin] > ydata_to_fit.at(imin + istep)) {
        imin = imin + istep;
      }
...
namespace AGL_functions {
  ...
  uint bracket_minimum_global(uint& imin, vector<double>& ydata_to_fit, ofstream& FileMESSAGE) {
    ...
    imin = 0;
    funcmin = ydata_to_fit[imin];
    for (size_t istep = 1; istep < ydata_to_fit.size(); istep++) {
      if (ydata_to_fit[istep] < funcmin) {
        imin = istep;
        funcmin = ydata_to_fit[imin];
      }
    }
...
namespace AGL_functions {
  ...
  uint polynom_bracket_minimum(uint& imin, vector<double>& x, vector<double>& polynomialcoeffs, ofstream& FileMESSAGE) {
    ...
    for (size_t istep = 0; istep < x.size(); istep++) {
      AGL_functions::polynom_eval(x[istep], polynomialcoeffs, plnv, 0);
      y[istep] = plnv;
    }

    imin = 0;
    funcmin = y[imin];
    for (size_t istep = 1; istep < x.size(); istep++) {
      if (y[istep] < funcmin) {
        imin = istep;
        funcmin = y[imin];
      }
...
namespace AGL_functions {
  ...
  uint autocorrect_polynom_minimum(uint& itry, vector<double>& xvalues, vector<double>& polynomialcoeffs, double& xmin, ofstream& FileMESSAGE) {
    ...
    pmerr = AGL_functions::polynom_minimum(xvalues[itry], xvalues[lowerbound], xvalues[upperbound], polynomialcoeffs, xmin, FileMESSAGE);
...
namespace AGL_functions {
  ...
  uint polynom_fit(uint& first_entry_tofit, uint& last_entry_tofit, vector<double>& xdata, vector<double>& ydata, vector<double>& weight, double& rms, int& npolycoeffwork, vector<double>& polycoeffwork, bool gxmdebug, ofstream& FileMESSAGE) {
    ...
    for (uint k = first_entry_tofit; k <= last_entry_tofit; k++) {
      weightnorm = weightnorm + weight[k];
    }
    weightnorm = 1.0 / weightnorm;
    cmatrix.resize(npolycoeffwork + 1);
    for (int i = 0; i < npolycoeffwork + 1; i++) {
      cmatrix[i].resize(npolycoeffwork + 2);
    }
    // Construct the linear system matrix cmatrix:
    for (int j = 0; j <= npolycoeffwork; j++) {
      cmatrix[j].at(npolycoeffwork + 1) = 0.0;
      if (j > 0) {
        for (uint k = first_entry_tofit; k <= last_entry_tofit; k++) {
          cmatrix[j].at(npolycoeffwork + 1) = cmatrix[j].at(npolycoeffwork + 1) + weight[k] * ydata[k] * (pow(xdata[k], j));
        }
      } else {
        for (uint k = first_entry_tofit; k <= last_entry_tofit; k++) {
          cmatrix[j].at(npolycoeffwork + 1) = cmatrix[j].at(npolycoeffwork + 1) + weight[k] * ydata[k];
        }
      }
      cmatrix[j].at(npolycoeffwork + 1) = weightnorm * cmatrix[j].at(npolycoeffwork + 1);
      for (int i = j; i <= npolycoeffwork; i++) {
        cmatrix[i][j] = 0.0;
        ij = i + j;
        if (ij > 0) {
          for (uint k = first_entry_tofit; k <= last_entry_tofit; k++) {
            cmatrix[i][j] = cmatrix[i][j] + weight[k] * pow(xdata[k], ij);
          }
          cmatrix[i][j] = weightnorm * cmatrix[i][j];
        } else {
          cmatrix[i][j] = 1.0;
        }
    ...
    variance = 0.0;
    for (uint k = first_entry_tofit; k <= last_entry_tofit; k++) {
      aglerror = AGL_functions::polynom_eval(xdata[k], polycoeffwork, plnv, 0);
      variance = variance + weight[k] * pow((ydata[k] - plnv), 2);
    }
...
namespace AGL_functions {
  uint gaussxm(vector<vector<double>>& cmatrix, int& n, vector<double>& xdata, bool gxmdebug, ofstream& FileMESSAGE) {
    ...
    aurostd::xmatrix<double> cxm(n, n);
    aurostd::xmatrix<double> bxm(n, 1);
    ostringstream aus;

    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        cxm[i + 1][j + 1] = cmatrix[i][j];
        if (LDEBUG && gxmdebug) {
          aurostd::StringstreamClean(aus);
...
namespace AGL_functions {
  ...
  uint polynom_eval(double& xval, vector<double>& polynomialcoeffs, double& yval, uint norder) {
    ...
    for (size_t i = (polynomialcoeffs.size() - 1); i > norder; i--) {
      next_coeff = polynomialcoeffs[i - 1];
...
namespace AGL_functions {
  ...
  uint debyeint(double& yval, double& Debye_int_val, _AGL_data& AGL_data, ofstream& FileMESSAGE) {
    ...
    vector<double> weight;
    double debye_integral;
    double debye_int_prev;
    double diff_int_abs;
    double sumdebyeint;
    int i;
    int nl;
    ...
    if (yval <= 250) {
      ...
      for (nl = 5; nl <= maxnl; nl = nl + 5) {
        ...
        sumdebyeint = 0.0;
        for (i = 0; i < nl; i++) {
          sumdebyeint = sumdebyeint + weight[i] * fdebyeint(xpoints[i]);
        }
