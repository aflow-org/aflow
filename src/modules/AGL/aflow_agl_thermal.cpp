...
namespace AGL_functions {
  ...
  uint self_consistent_debye(double& Temperature,
                             vector<double>& polynomialcoeffs,
                             vector<double>& polynomialerror,
                             vector<double>& xdata_to_fit,
                             vector<double>& ydata_to_fit,
                             uint& imin,
                             bool firsttime,
    ...
    double theta0 = 0.0;
    double theta = 0.0;
    double DebyeIntegral = 0.0;
    double DebyeIntegralerror = 0.0;
    double UIntEnergy = 0.0;
    double Cvt = 0.0;
    double F_Helmholtz = 0.0;
    ...
    double d2Fdx2 = 0.0;
    double plnv = 0.0;
    double pressure_F = 0.0;
    double bulkmodT = 0.0;
    double mie_gruneisen = 0.0;
    double bulkmodstV = 0.0;
    ...
    if (firsttime) {
      for (size_t i = 0; i < xdata_to_fit.size(); i++) {
        aglerror = polynom_eval(xdata_to_fit[i], polynomialcoeffs, plnv, 1);
        press_stat[i] = -xdata_to_fit[i] * plnv / (3.0 * AGL_data.volumeinput[i]);
      }
      return aglerror;
    }
    ...
    while ((!converged || (iloops < mloops)) && (iloops < AGL_data.maxloops)) {
      ...
      for (size_t i = 0; i < ydata_to_fit.size(); i++) {
        thermal_properties(AGL_data.tdebye[i], Temperature, DebyeIntegral, DebyeIntegralerror, UIntEnergy, Cvt, F_Helmholtz, S_Entropy, AGL_data, FileMESSAGE);
        ydata_to_fit[i] = AGL_data.energyinput[i] + F_Helmholtz;
      }
      ...
      for (size_t i = 0; i < xdata_to_fit.size(); i++) {
        aglerror = AGL_functions::thermal_properties(AGL_data.tdebye[i], Temperature, DebyeIntegral, DebyeIntegralerror, UIntEnergy, Cvt, F_Helmholtz, S_Entropy, AGL_data, FileMESSAGE);
        aglerror = AGL_functions::polynom_eval(xdata_to_fit[i], polynomialcoeffs, dFdx, 1);
        aglerror = AGL_functions::polynom_eval(xdata_to_fit[i], polynomialcoeffs, d2Fdx2, 2);
        pressure_F = -xdata_to_fit[i] * dFdx / (3.0 * AGL_data.volumeinput[i]);
        bulkmodT = -xdata_to_fit[i] / (9.0 * AGL_data.volumeinput[i]) * (2.0 * dFdx - xdata_to_fit[i] * d2Fdx2);
        mie_gruneisen = (pressure_F - press_stat[i]) * AGL_data.volumeinput[i] / UIntEnergy;
        bulkmodstV = AGL_data.volumeinput[i] * bulkmodT + Temperature * mie_gruneisen * mie_gruneisen * Cvt;
        if (bulkmodstV < 0.0) {
          if (i <= imin) {
            aurostd::StringstreamClean(aus);
          ...
          theta = (hbar_eV / KBOLTZEV) * pow((6.0 * pi * pi * AGL_data.natoms / AGL_data.volumeinput[i]), THIRD) * AGL_data.poissonratiofunction * sqrt((bulkmodstV / AGL_data.cellmass) * eV_Ang3_to_amu_Ang_s);
          delta_theta = theta - AGL_data.tdebye[i];
          if (i > 0) {
        ...
        if ((AGL_data.volumeinput[i] - AGL_data.volumeinput[imin] < AGL_data.volumeinput[imin] * 0.1) && (i >= 3 && i >= (AGL_data.volumeinput.size() / 10))) {
...
namespace AGL_functions {
  ...
  uint debye_polynom_fit(vector<double>& polynomialcoeffs, _AGL_data& AGL_data, ofstream& FileMESSAGE) {
    ...
    vector<int> npolycoeffworkfit(AGL_data.maxfit + 1);
    vector<int> ndatafit(AGL_data.maxfit + 1);
    // Service variables
    ...
    for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
      xdata[i] = log(AGL_data.volumeinput[i]);
      ydata[i] = log(AGL_data.tdebye[i]);
      weight[i] = 1.0;
    }
    if (LDEBUG) {
      aurostd::StringstreamClean(aus);
    ...
    ndatamin = std::max(ndataiterfit - 2 * ndel, npolycoeffworkmin + 1);
    rmsmin = 1e33;
    ...
    while (ndataiterfit >= ndatamin) {
      ...
      npolycoeffworkmax = std::min(ndataiterfit - limit - 1, AGL_data.maxpolycoeffs);
      for (npolycoeffwork = npolycoeffworkmin; npolycoeffwork <= npolycoeffworkmax; npolycoeffwork++) {
        ...
        // Save fit parameters
          ...
          rmsmin = std::min(rmsmin, rms * npolycoeffwork / ndataiterfit);
        }
      }
  ...
  uint thermal_properties(double& ThetaD, double& Temperature, double& DebyeIntegral, double& DebyeIntegralerror, double& internalenergy, double& Cv, double& helmholtzenergy, double& entropy, _AGL_data& AGL_data, ofstream& FileMESSAGE) {
    ...
    if (yval <= 250) {
      ...
      while ((nl <= maxnl) && (DebyeIntegralerror >= eps)) {
        ...
        sumdebyeint = 0.0;
        for (i = 0; i < nl; i++) {
          sumdebyeint = sumdebyeint + weight[i] * debye_function(xpoints[i]);
        }
...
namespace AGL_functions {
  ...
  uint thermalconductD(double& acousticthetaD, vector<double>& temperature, double& kappaD, vector<double>& kappaT, double& gammaD, double& voleqD, double& avmass) {
    ...
    const double hbar = PLANCKSCONSTANT_hbar;
    ...
    for (size_t i = 0; i < temperature.size(); i++) {
      if (aurostd::abs(temperature[i]) < tol) {
        kappaT[i] = 0.0;
      } else {
        kappaT[i] = kappaD * acousticthetaD / temperature[i];
      }
    }
    return 0;
  }
...
namespace AGL_functions {
  ...
  uint thermalconductDvol(double& acousticthetaD, vector<double>& temperature, vector<double>& kappaTV, double& gammaD, vector<vector<double>>& volumeq, double& avmass) {
    ...
    kth = KBOLTZ * acousticthetaD / hbar;
    kths = kth * kth;
    const double gammafunc = ((0.849 * 3 * pow(4, THIRD)) / (20 * pow(PI, 3) * (1.0 - (0.514 / gammaD) + (0.228 / (gammaD * gammaD)))));
    kappaTV.resize(temperature.size());
    for (size_t i = 0; i < temperature.size(); i++) {
      if (aurostd::abs(temperature[i]) < tol) {
        kappaTV[i] = 0.0;
      } else {
        vDcr = (pow(volumeq[i][0], THIRD)) * 1e-10;
        kappaTV[i] = gammafunc * kths * KBOLTZ * vDcr * avmass * acousticthetaD / (hbar * gammaD * gammaD * temperature[i]);
      }
    }
    return 0;
  }
