...
namespace AGL_functions {
  ...
  uint gibbsrun(_AGL_data& AGL_data, ofstream& FileMESSAGE) {
    ...
    double gammaalphaT;
    double Bstatic;
    double DebyeIntegral;
    double DebyeIntegralerror;
    double UIntEnergyVib;
    double HelmholtzEnergy;
    double Entropy;
    double Cvt;
    double UIntEnergy;
    double Emin;
    ...
    double Vmin_Ang;
    double Vmin_Bohr;
    double volume_0pressure;
    double gibbsenergykjmol_0pressure;
    double gibbsenergyeV_0pressure;
    double binp_bcnt;
    double energyvalue;
    double energyerror;
    double tdebyemin;
    double xmin;
    double plnv;
    double IntEnergyVibkjmol;
    double CvVibkjmol;
    double CvVibUnitkB;
    double HelmholtzVibkjmol;
    double EntropyVibkjmol;
    double EdftUintVib;
    double IntEnergyVibmeV;
    double HelmholtzVibmeV;
    double EntropyVibmeV;
    double EntropyVibUnitkB;
    double S_Entropy;
    double Pbeta;
    double Cpkjmol;
    double CpUnitkB;
    ...
    vector<double> xconfigurationvector;
    vector<double> gibbsenergydatapoints;
    vector<double> helmholtzenergydatapoints;
    vector<double> energypolynomialcoeffs;
    vector<double> gibbsenergypolynomialcoeffs;
    vector<double> helmholtzenergypolynomialcoeffs;
    vector<double> tdebyepolynomialcoeffs;
    vector<double> energypolynomialerror;
    vector<double> helmholtzpolynomialerror;
    vector<double> gibbspolynomialerror;
    vector<double> gibbsenergykjmol;
    vector<double> gibbsenergyeV;
    vector<double> relative_error;
    vector<double> press_stat;
    vector<double> F_Helmholtzvinp;
    vector<double> bcnt_K_sp;
    vector<double> bcnt_P_sp;
    vector<double> bcnt_V_sp;
    vector<double> bcnt_beta_sp;
    vector<double> volumetemperature0pressure;
    vector<double> gibbsenergytemperature0pressurekjmol;
    vector<double> dbulkmodulusdpt0pressure;
    vector<double> d2bulkmodulusdp2t0pressure;
    vector<double> theta;
    vector<double> Cv;
    const int pmerr = 0;
    ostringstream aus;
    ...
    const double poissonratiofunc1 = (2.0 / 3.0) * ((1.0 + AGL_data.poissonratio) / (1.0 - 2.0 * AGL_data.poissonratio));
    const double poissonratiofunc2 = (1.0 / 3.0) * ((1.0 + AGL_data.poissonratio) / (1.0 - AGL_data.poissonratio));
    const double poissonratiofunc3 = 2.0 * sqrt(pow(poissonratiofunc1, 3)) + sqrt(pow(poissonratiofunc2, 3));
    ...
    if (AGL_data.fittype == 5) {
      ...
      const string evdataname = AGL_data.dirpathname + "/evdata.in";
      ...
      for (uint i = 0; i < ndatapoints; i++) {
        evtest >> volume[i] >> energy[i];
        AGL_data.volumeinput[i] = volume[i];
        AGL_data.energyinput[i] = energy[i];
      }
      aurostd::StringstreamClean(aus);
      aus << _AGLSTR_MESSAGE_ << "Number of data points = " << ndatapoints << endl;
      for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
        aus << _AGLSTR_MESSAGE_ << "(V, E) = " << AGL_data.volumeinput[i] << "\t" << AGL_data.energyinput[i] << endl;
      }
    ...
    const double vtref = AGL_data.volumeinput[itmin];

    xconfigurationvector.resize(AGL_data.volumeinput.size());
    for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
      xconfigurationvector[i] = pow(AGL_data.volumeinput[i] / vtref, THIRD);
    }
    ...
    uint evj = 1;
    while (((AGL_data.energyinput[evj] - AGL_data.energyinput[evj - 1]) / (AGL_data.volumeinput[evj] - AGL_data.volumeinput[evj - 1]) >=
            (AGL_data.energyinput[evj] - AGL_data.energyinput.at(evj + 1)) / (AGL_data.volumeinput[evj] - AGL_data.volumeinput.at(evj + 1))) &&
           evj < AGL_data.volumeinput.size() - 2) {
      evj = evj + 1;
    }
    aurostd::StringstreamClean(aus);
    ...
    AGL_data.energyinput[0] = AGL_data.energyinput[evj - 1];
    AGL_data.volumeinput[0] = AGL_data.volumeinput[evj - 1];
    AGL_data.energyinput[1] = AGL_data.energyinput[evj];
    AGL_data.volumeinput[1] = AGL_data.volumeinput[evj];
    AGL_data.energyinput[2] = AGL_data.energyinput.at(evj + 1);
    AGL_data.volumeinput[2] = AGL_data.volumeinput.at(evj + 1);
    if (AGL_data.i_debye == 1) {
      AGL_data.tdebye[0] = AGL_data.tdebye[evj - 1];
      AGL_data.tdebye[1] = AGL_data.tdebye[evj];
      AGL_data.tdebye[2] = AGL_data.tdebye.at(evj + 1);
    }
    evj = evj + 1;
    uint jtmax = 2;
    ...
    for (size_t i = evj + 1; i < AGL_data.volumeinput.size(); i++) {
      if (LDEBUG) {
        aurostd::StringstreamClean(aus);
      ...
      if (AGL_data.fittype == 0) {
        if ((AGL_data.energyinput[evj] - AGL_data.energyinput[evj - 1]) / (AGL_data.volumeinput[evj] - AGL_data.volumeinput[evj - 1]) <
            (AGL_data.energyinput[evj] - AGL_data.energyinput[i]) / (AGL_data.volumeinput[evj] - AGL_data.volumeinput[i])) {
          evj = evj + 1;
          jtmax = i;
          AGL_data.energyinput[evj] = AGL_data.energyinput[i];
          AGL_data.volumeinput[evj] = AGL_data.volumeinput[i];
          if (AGL_data.i_debye == 1) {
            AGL_data.tdebye[evj] = AGL_data.tdebye[i];
          }
        }
      } else if (AGL_data.fittype == 1 || AGL_data.fittype == 3) {
        if (i < itmin) {
          if ((AGL_data.energyinput[i] < AGL_data.energyinput[evj]) && (AGL_data.energyinput[i] > AGL_data.energyinput.at(i + 1))) {
            evj = evj + 1;
            jtmax = i;
            AGL_data.energyinput[evj] = AGL_data.energyinput[i];
            AGL_data.volumeinput[evj] = AGL_data.volumeinput[i];
    ...
    if (jtmax < itmin && (AGL_data.fittype == 0)) {
      ...
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if ((AGL_data.energyinput[itmin] - AGL_data.energyinput[itmin - 1]) / (AGL_data.volumeinput[itmin] - AGL_data.volumeinput[itmin - 1]) <
          (AGL_data.energyinput[itmin] - AGL_data.energyinput.at(itmin + 1)) / (AGL_data.volumeinput[itmin] - AGL_data.volumeinput.at(itmin + 1))) {
        AGL_data.energyinput[evj] = AGL_data.energyinput[itmin];
        AGL_data.volumeinput[evj] = AGL_data.volumeinput[itmin];
        for (size_t i = itmin + 1; i < AGL_data.energyinput.size() - 1; i++) {
          if ((AGL_data.energyinput[evj] - AGL_data.energyinput[evj - 1]) / (AGL_data.volumeinput[evj] - AGL_data.volumeinput[evj - 1]) <
              (AGL_data.energyinput[evj] - AGL_data.energyinput[i]) / (AGL_data.volumeinput[evj] - AGL_data.volumeinput[i])) {
            evj = evj + 1;
            AGL_data.energyinput[evj] = AGL_data.energyinput[i];
    ...
    for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
      aus << _AGLSTR_MESSAGE_ << "V = " << AGL_data.volumeinput[i] << "; E = " << AGL_data.energyinput[i] << endl;
    }
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (AGL_data.fittype == 3 || AGL_data.fittype == 4) {
      imin = 0;
      energyreference = AGL_data.energyinput[0];
      for (size_t i = 0; i < AGL_data.energyinput.size(); i++) {
        if (AGL_data.energyinput[i] < energyreference) {
          energyreference = AGL_data.energyinput[i];
          imin = i;
        }
      }
      if (!LDEBUG) {
      ...
      for (size_t i = 0; i < AGL_data.energyinput.size(); i++) {
        aglerror = AGL_functions::polynom_eval(xconfigurationvector[i], energypolynomialcoeffs, plnv, 0);
        if (aglerror != 0) {
          aurostd::StringstreamClean(aus);
        ...
        AGL_data.energyinput[i] = plnv;
        AGL_data.volumeinput[i] = (pow(xconfigurationvector[i], 3.0)) * vtref;
      }
    }

    // Search for the minimum energy point of the accepted data points
    volumereference = AGL_data.volumeinput[0];
    energyreference = AGL_data.energyinput[0];
    imin = 0;
    for (size_t i = 0; i < AGL_data.energyinput.size(); i++) {
      if (AGL_data.energyinput[i] < energyreference) {
        volumereference = AGL_data.volumeinput[i];
        energyreference = AGL_data.energyinput[i];
        imin = i;
      }
    }
    ...
    xconfigurationvector.resize(AGL_data.volumeinput.size());
    for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
      xconfigurationvector[i] = pow(AGL_data.volumeinput[i] / volumereference, THIRD);
    }
    aurostd::StringstreamClean(aus);
    ...
    if (LDEBUG) {
      const string polyerepname = AGL_data.dirpathname + "/AGL_Plot_adjusted_energyinput.dat";
      stringstream polyerepss;
      for (size_t i = 0; i < AGL_data.energyinput.size(); i++) {
        polyerepss << xconfigurationvector[i] << "\t" << AGL_data.energyinput[i] << endl;
      }
      if (!aurostd::stringstream2file(polyerepss, polyerepname, aurostd::compression_type::None, "WRITE")) {
        aurostd::StringstreamClean(aus);
    ...
    if (LDEBUG) {
      ...
      const string einppolyfitname = AGL_data.dirpathname + "/AGL_Plot_fitted_energyinput.dat";
      ...
      if (AGL_data.volumeinput.size() != xconfigurationvector.size()) {
        ...
        for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
          aglerror = AGL_functions::polynom_eval(xconfigurationvector[i], energypolynomialcoeffs, plnv, 0);
          einppolyfitss << AGL_data.volumeinput[i] << "\t" << plnv << endl;
        }
      }
      if (!aurostd::stringstream2file(einppolyfitss, einppolyfitname, aurostd::compression_type::None, "WRITE")) {
        aurostd::StringstreamClean(aus);
    ...
    for (size_t i = 0; i < gibbsenergypolynomialcoeffs.size(); i++) {
      gibbsenergypolynomialcoeffs[i] = energypolynomialcoeffs[i];
    }
    ...
    for (size_t k = 0; k < AGL_data.pressure_external.size(); k++) {
      ...
      if ((gibbsenergydatapoints.size() != AGL_data.energyinput.size()) || (gibbsenergydatapoints.size() != AGL_data.volumeinput.size())) {
        ...
        for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
          gibbsenergydatapoints[i] = AGL_data.energyinput[i] + AGL_data.volumeinput[i] * (AGL_data.pressure_external[k] / eV_Ang3_to_GPa);
        }
      }
      if (AGL_data.fittype == 0) {
      ...
      if (aglerror != 0) {
        ...
        if (AGL_data.run_all_pressure_temperature) {
          aurostd::StringstreamClean(aus);
          aus << _AGLSTR_WARNING_ + "Cannot find minimum of E + pV at p = " << AGL_data.pressure_external[k] << endl;
          ...
          if (k == 0) {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_ERROR_ + "Static: p = " << AGL_data.pressure_external[k] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                << ", gibbsenergydatapoints.size() = " << gibbsenergydatapoints.size() << endl;
            aus << _AGLSTR_ERROR_ + "gibbsenergydatapoints = ";
            for (size_t i = 0; i < gibbsenergydatapoints.size(); i++) {
              aus << gibbsenergydatapoints[i] << "\t";
            }
            aus << endl;
            aus << _AGLSTR_ERROR_ + "Cannot find minimum for pressure = " << AGL_data.pressure_external[k] << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            return 2;
          } else {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_WARNING_ + "Static: " << AGL_data.pressure_external[k] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                << ", gibbsenergydatapoints.size() = " << gibbsenergydatapoints.size() << endl;
            ...
            aus << endl;
            aus << _AGLSTR_WARNING_ + "Cannot find minimum of E + pV at p = " << AGL_data.pressure_external[k] << endl;
            aus << _AGLSTR_WARNING_ + "Resetting maximum pressure value to p = " << AGL_data.pressure_external[k - 1] << endl;
      ...
      gibbsenergypolynomialcoeffs[3] = energypolynomialcoeffs[3] + (AGL_data.pressure_external[k] / eV_Ang3_to_GPa) * volumereference;
      ...
      if (aglerror == 0) {
        ...
        if (AGL_data.run_all_pressure_temperature) {
          ...
          if (k == 0) {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_ERROR_ + "Cannot find minimum of polynomial fit for E + pV at p = " << AGL_data.pressure_external[k] << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            return 2;
          } else {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_WARNING_ + "Cannot find minimum of polynomial fit for E + pV at p = " << AGL_data.pressure_external[k] << endl;
            aus << _AGLSTR_WARNING_ + "Resetting maximum pressure value to p = " << AGL_data.pressure_external[k - 1] << endl;
            aus << _AGLSTR_WARNING_ + "Resetting number of pressure points to " << k << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            AGL_data.pressure_external.resize(k);
      ...
      if (AGL_data.run_all_pressure_temperature) {
        AGL_data.voleqmin[k] = pow(xmin, 3.0) * volumereference;
        AGL_data.VolumeStaticPressure[k] = pow(xmin, 3.0) * volumereference;
        AGL_data.StaticPressure[k] = AGL_data.pressure_external[k];
        aglerror = AGL_functions::polynom_eval(xmin, gibbsenergypolynomialcoeffs, plnv, 0);
        gibbsenergyeV[k] = plnv;
        gibbsenergykjmol[k] = plnv * eV2kjmol;
        // Save static Gibbs free energy (i.e. enthalpy: H = E + pV)
        AGL_pressure_enthalpy.pressure_external = AGL_data.pressure_external[k];
        ...
        AGL_data.bulkmodulus[k] = plnv * eV_Ang3_to_GPa * xmin * xmin / (9.0 * AGL_data.voleqmin[k]);
        ...
        relative_error[k] = energyerror / std::max(aurostd::abs(energyvalue), aurostd::abs(energyvalue) + (energyerror / 2.0));
        if (aglerror != 0) {
          aurostd::StringstreamClean(aus);
          aus << _AGLSTR_WARNING_ + "Error evaluating polynomials for pressure = " << AGL_data.pressure_external[k] << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          continue;
        }
      } else {
        ...
        AGL_data.StaticPressure.push_back(AGL_data.pressure_external[k]);
        aglerror = AGL_functions::polynom_eval(xmin, gibbsenergypolynomialcoeffs, plnv, 0);
        ...
        aglerror = AGL_functions::polynom_eval(xmin, gibbsenergypolynomialcoeffs, plnv, 2);
        AGL_data.bulkmodulus.push_back(plnv * eV_Ang3_to_GPa * xmin * xmin / (9.0 * AGL_data.voleqmin[k]));
        aglerror = AGL_functions::polynom_eval(xmin, energypolynomialcoeffs, energyvalue, 0);
        aglerror = AGL_functions::polynom_eval(xmin, energypolynomialerror, plnv, 0);
        energyerror = sqrt(aurostd::abs(plnv - energyvalue * energyvalue));
        relative_error.push_back(energyerror / std::max(aurostd::abs(energyvalue), aurostd::abs(energyvalue) + energyerror / 2.0));
        if (aglerror != 0) {
          aurostd::StringstreamClean(aus);
    ...
    volume_0pressure = AGL_data.voleqmin[0];
    gibbsenergykjmol_0pressure = gibbsenergykjmol[0];
    binp_bcnt = AGL_data.bulkmodulus[0];
    if (AGL_data.i_eqn_of_state >= 0) {
      ...
      AGL_data.outfiless << " ----------------------------------------------------------------------------------------- " << endl;
      for (size_t k = 0; k < AGL_data.pressure_external.size(); k++) {
        AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.pressure_external[k] << "        " << setw(10) << gibbsenergykjmol[k] << "      " << setw(10)
                           << AGL_data.voleqmin[k] * pow(angstrom2bohr, 3.0) << "      " << setw(6) << setprecision(5) << AGL_data.voleqmin[k] / volume_0pressure << "    " << setw(12) << setprecision(2)
                           << AGL_data.bulkmodulus[k] << "   " << setw(15) << setprecision(6) << relative_error[k] << endl;
      }
    } else if (AGL_data.i_debye == -1) {
      for (size_t k = 0; k < AGL_data.pressure_external.size(); k++) {
        AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.pressure_external[k] << "\t" << setw(6) << 0.0 << "\t" << setw(10) << AGL_data.voleqmin[k] * pow(angstrom2bohr, 3.0) << "\t"
                           << setw(11) << setprecision(5) << gibbsenergykjmol[k] << "\t" << setw(8) << setprecision(2) << AGL_data.bulkmodulus[k] << endl;
      }
    } else {
      for (size_t k = 0; k < AGL_data.pressure_external.size(); k++) {
        AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.pressure_external[k] << "\t" << setw(6) << "static" << "\t" << setw(10) << AGL_data.voleqmin[k] * pow(angstrom2bohr, 3.0)
                           << "\t" << setw(11) << setprecision(5) << gibbsenergykjmol[k] << "\t" << setw(8) << setprecision(2) << AGL_data.bulkmodulus[k] << endl;
      }
    }
    ...
    if (AGL_data.run_all_pressure_temperature) {
      ...
      for (size_t i = 0; i < xconfigurationvector.size(); i++) {
        ...
        const double dEdx2_xd2Edx2 = xconfigurationvector[i] * d2Edx2 - 2.0 * dEdx;
      ...
      // Vinet equation of state evaluation of thermal properties
        ...
        gibbsenergyeV_0pressure = gibbsenergyeV[0];
    ...
    if (AGL_data.i_debye == 1) {
      ...
      if (AGL_data.volumeinput.size() != AGL_data.d2EnergydVolume2_static.size()) {
        ...
        tdebyemin = (hbar_eV / KBOLTZEV) * pow(6 * pi * pi * AGL_data.natoms * AGL_data.volumeinput[imin] * AGL_data.volumeinput[imin], THIRD) * AGL_data.poissonratiofunction *
                    sqrt((aurostd::abs(AGL_data.d2EnergydVolume2_static[imin]) / AGL_data.cellmass) * eV_Ang3_to_amu_Ang_s);
      }
    } else {
      if (AGL_data.i_eqn_of_state >= 0) {
    ...
    if (LDEBUG) {
      ...
      double tf1;
      double tf2;
      const string ofiletf0name = AGL_data.dirpathname + "/AGL_Poly_Ex0.dat";
      stringstream ofiletf0ss;
      const string ofiletf1name = AGL_data.dirpathname + "/AGL_Poly_Ex1.dat";
      stringstream ofiletf1ss;
      const string ofiletf2name = AGL_data.dirpathname + "/AGL_Poly_Ex2.dat";
      stringstream ofiletf2ss;
      for (size_t i = 0; i < xconfigurationvector.size(); i++) {
        aglerror = AGL_functions::polynom_eval(xconfigurationvector[i], energypolynomialcoeffs, tf0, 0);
        aglerror = AGL_functions::polynom_eval(xconfigurationvector[i], energypolynomialcoeffs, tf1, 1);
        aglerror = AGL_functions::polynom_eval(xconfigurationvector[i], energypolynomialcoeffs, tf2, 2);
        ofiletf0ss << xconfigurationvector[i] << "\t" << tf0 << endl;
        ofiletf1ss << xconfigurationvector[i] << "\t" << tf1 << endl;
        ofiletf2ss << xconfigurationvector[i] << "\t" << tf2 << endl;
      }
      if (aglerror != 0) {
        aurostd::StringstreamClean(aus);
        aus << _AGLSTR_ERROR_ + "Failed to evaluate polynomial" << endl;
      ...
      if (!aurostd::stringstream2file(ofiletf0ss, ofiletf0name, aurostd::compression_type::None, "WRITE")) {
        aurostd::StringstreamClean(aus);
    ...
    for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
      if (AGL_data.i_debye != 1 && (AGL_data.d2EnergydVolume2_static[i] <= 0.0 && (AGL_data.fittype < 2 || AGL_data.fittype == 3))) {
        if (i > itry) {
          aurostd::StringstreamClean(aus);
          aus << _AGLSTR_WARNING_ + "Warning! convex function at i = " << i << endl;
          aus << _AGLSTR_WARNING_ + "Warning! d2EnergydVolume2_static = " << AGL_data.d2EnergydVolume2_static[i] << endl;
        ...
        if (i != ij) {
          ...
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          AGL_data.volumeinput[ij] = AGL_data.volumeinput[i];
          AGL_data.energyinput[ij] = AGL_data.energyinput[i];
          xconfigurationvector[ij] = xconfigurationvector[i];
          gibbsenergydatapoints[ij] = gibbsenergydatapoints[i];
          AGL_data.IntEnergStatic[ij] = AGL_data.IntEnergStatic[i];
          F_Helmholtzvinp[ij] = F_Helmholtzvinp[ij];
          AGL_data.d2EnergydVolume2_static[ij] = AGL_data.d2EnergydVolume2_static[i];
          if (i == imin) {
            imin = ij;
            aurostd::StringstreamClean(aus);
        ...
        // tdebyevol is the Debye temperature for the structure with volume AGL_data.volumeinput[ij]
        if (AGL_data.volumeinput.size() != AGL_data.d2EnergydVolume2_static.size()) {
          aurostd::StringstreamClean(aus);
          aus << _AGLSTR_ERROR_ + "Vectors are different size" << endl;
          aus << _AGLSTR_ERROR_ + "AGL_data.volumeinput.size() = " << AGL_data.volumeinput.size() << endl;
          ...
          tdebyevol = (hbar_eV / KBOLTZEV) * pow(6 * pi * pi * AGL_data.natoms * AGL_data.volumeinput[ij] * AGL_data.volumeinput[ij], THIRD) * AGL_data.poissonratiofunction *
                      sqrt((aurostd::abs(AGL_data.d2EnergydVolume2_static[ij]) / AGL_data.cellmass) * eV_Ang3_to_amu_Ang_s);
          if (LDEBUG) {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_MESSAGE_ + "tdebyevol = " << tdebyevol << endl;
            aus << _AGLSTR_MESSAGE_ + "AGL_data.d2EnergydVolume2_static[ij] = " << AGL_data.d2EnergydVolume2_static[ij] << endl;
            aus << _AGLSTR_MESSAGE_ + "AGL_data.volumeinput[ij] = " << AGL_data.volumeinput[ij] << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          }
        }
        if (AGL_data.i_debye == 3) {
          ...
          if (AGL_data.i_eqn_of_state >= 0) {
            AGL_data.outfiless << setw(12) << setprecision(6) << std::fixed << (AGL_data.volumeinput[ij] * pow(angstrom2bohr, 3.0)) << "    " << setw(12) << AGL_data.tdebye[ij] << "    " << setw(12) << tdebyevol << endl;
          }
        } else if (AGL_data.i_debye == 1) {
          if (AGL_data.i_eqn_of_state >= 0) {
            // OBSOETE AGL_data.outfiless << setw(12) << setprecision(6) << fixed << AGL_data.volumeinput[ij] << "    " << setw(12) << AGL_data.tdebye[ij] << endl;
            AGL_data.outfiless << setw(12) << setprecision(6) << std::fixed << (AGL_data.volumeinput[ij] * pow(angstrom2bohr, 3.0)) << "    " << setw(12) << AGL_data.tdebye[ij] << endl;
          }
        } else {
    ...
    if (LDEBUG) {
      const string ofiled2EnergydVolume2_staticname = AGL_data.dirpathname + "/AGL_Poly_d2EnergydVolume2_static.dat";
      ...
      for (size_t i = 0; i < xconfigurationvector.size(); i++) {
        ofiled2EnergydVolume2_staticss << xconfigurationvector[i] << "\t" << AGL_data.d2EnergydVolume2_static[i] << endl;
      }
      if (!aurostd::stringstream2file(ofiled2EnergydVolume2_staticss, ofiled2EnergydVolume2_staticname, aurostd::compression_type::None, "WRITE")) {
        aurostd::StringstreamClean(aus);
      ...
      const string ofiletdebiname = AGL_data.dirpathname + "/AGL_Plot_tdebye_initial.dat";
      stringstream ofiletdebiss;
      for (size_t i = 0; i < xconfigurationvector.size(); i++) {
        ofiletdebiss << xconfigurationvector[i] << "\t" << AGL_data.tdebye[i] << endl;
      }
      if (!aurostd::stringstream2file(ofiletdebiss, ofiletdebiname, aurostd::compression_type::None, "WRITE")) {
        aurostd::StringstreamClean(aus);
    ...
    for (size_t j = 0; j < AGL_data.temperature_external.size(); j++) {
      aurostd::StringstreamClean(aus);
      if (LIB2RAW_FIX_THIS_LATER) {  // to avoid too much garbage in lib2raw
        aus << _AGLSTR_MESSAGE_ << "Temperature = " << AGL_data.temperature_external[j] << "K" << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      AGL_data.max_temperature = AGL_data.temperature_external[j];
      ...
      if (AGL_data.i_debye == 2) {
        ...
        aglerror = AGL_functions::self_consistent_debye(AGL_data.temperature_external[j], helmholtzenergypolynomialcoeffs, helmholtzpolynomialerror, xconfigurationvector, helmholtzenergydatapoints, imin, false,
                                                        press_stat, AGL_data, FileMESSAGE);
        ...
        if (aglerror != 0) {
          ...
          if (AGL_data.run_all_pressure_temperature) {
            ...
            if (j == 0) {
              aurostd::StringstreamClean(aus);
              aus << _AGLSTR_ERROR_ + "Self-consistent polynomial fit: T = " << AGL_data.temperature_external[j] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                  << ", helmholtzenergydatapoints.size() = " << helmholtzenergydatapoints.size() << endl;
              aus << _AGLSTR_ERROR_ + "helmholtzenergydatapoints = ";
              for (size_t i = 0; i < helmholtzenergydatapoints.size(); i++) {
                aus << helmholtzenergydatapoints[i] << "\t";
              }
              aus << endl;
              aus << _AGLSTR_ERROR_ + "Cannot find minimum for temperature = " << AGL_data.temperature_external[j] << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
              return 2;
            } else {
              aurostd::StringstreamClean(aus);
              aus << _AGLSTR_WARNING_ + "Self-consistent polynomial fit: T = " << AGL_data.temperature_external[j] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                  << ", helmholtzenergydatapoints.size() = " << helmholtzenergydatapoints.size() << endl;
              ...
              aus << endl;
              aus << _AGLSTR_WARNING_ + "Cannot find minimum of E + F at T = " << AGL_data.temperature_external[j] << endl;
              aus << _AGLSTR_WARNING_ + "Resetting maximum value of T to T = " << AGL_data.temperature_external[j] << endl;
        ...
        if (aglerror != 0) {
          ...
          if (AGL_data.run_all_pressure_temperature) {
            ...
            if (j == 0) {
              aurostd::StringstreamClean(aus);
              aus << _AGLSTR_ERROR_ + "No polynomial fit found with a minimum within bounds of input data" << endl;
              aus << _AGLSTR_ERROR_ + "Debye polynomial fit: T = " << AGL_data.temperature_external[j] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                  << ", helmholtzenergydatapoints.size() = " << helmholtzenergydatapoints.size() << endl;
              aus << _AGLSTR_ERROR_ + "Cannot find minimum for temperature = " << AGL_data.temperature_external[j] << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
              if (aglerror == 2) {
              ...
              aus << _AGLSTR_WARNING_ + "Debye polynomial fit: T = " << AGL_data.temperature_external[j] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                  << ", helmholtzenergydatapoints.size() = " << helmholtzenergydatapoints.size() << endl;
              aus << _AGLSTR_WARNING_ + "Resetting maximum value of T to T = " << AGL_data.temperature_external[j] << endl;
              aus << _AGLSTR_WARNING_ + "Resetting number of temperature points to " << j << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      ...
      if (LDEBUG && AGL_data.fittype == 5) {
        const string ofiletdebname = AGL_data.dirpathname + "/AGL_Plot_tdebye.dat";
        ...
        for (size_t i = 0; i < xconfigurationvector.size(); i++) {
          ofiletdebss << xconfigurationvector[i] << "\t" << AGL_data.tdebye[i] << endl;
        }
        if (!aurostd::stringstream2file(ofiletdebss, ofiletdebname, aurostd::compression_type::None, "WRITE")) {
          aurostd::StringstreamClean(aus);
      ...
      if ((helmholtzenergydatapoints.size() != AGL_data.energyinput.size()) || (helmholtzenergydatapoints.size() != F_Helmholtzvinp.size()) || (helmholtzenergydatapoints.size() != AGL_data.tdebye.size())) {
        ...
        for (size_t i = 0; i < helmholtzenergydatapoints.size(); i++) {
          aglerror = AGL_functions::thermal_properties(AGL_data.tdebye[i], AGL_data.temperature_external[j], DebyeIntegral, DebyeIntegralerror, UIntEnergy, Cvt, F_Helmholtzvinp[i], S_Entropy, AGL_data, FileMESSAGE);
          helmholtzenergydatapoints[i] = AGL_data.energyinput[i] + F_Helmholtzvinp[i];
        }
      }
      if (LDEBUG && AGL_data.fittype == 5) {
        const string ofilehelmname = AGL_data.dirpathname + "/AGL_Plot_Helmholtz_energy.dat";
        ...
        for (size_t i = 0; i < xconfigurationvector.size(); i++) {
          ofilehelmss << xconfigurationvector[i] << "\t" << F_Helmholtzvinp[i] << endl;
        }
        if (!aurostd::stringstream2file(ofilehelmss, ofilehelmname, aurostd::compression_type::None, "WRITE")) {
          aurostd::StringstreamClean(aus);
        ...
        aurostd::StringstreamClean(aus);
        aus << _AGLSTR_WARNING_ + "Polynomial fit: T = " << AGL_data.temperature_external[j] << ", helmholtzenergydatapoints.size() = " << helmholtzenergydatapoints.size() << endl;
        aus << _AGLSTR_WARNING_ + "helmholtzenergydatapoints = ";
        for (size_t i = 0; i < helmholtzenergydatapoints.size(); i++) {
          aus << helmholtzenergydatapoints[i] << "\t";
        }
        ...
        for (size_t i = 0; i < F_Helmholtzvinp.size(); i++) {
          aus << F_Helmholtzvinp[i] << "\t";
        }
        aus << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      ...
      if (aglerror != 0) {
        ...
        if (AGL_data.run_all_pressure_temperature) {
          ...
          if (j == 0) {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_ERROR_ + "Bracket minimum: T = " << AGL_data.temperature_external[j] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                << ", helmholtzenergydatapoints.size() = " << helmholtzenergydatapoints.size() << endl;
            aus << _AGLSTR_ERROR_ + "helmholtzenergydatapoints = ";
            for (size_t i = 0; i < helmholtzenergydatapoints.size(); i++) {
              aus << helmholtzenergydatapoints[i] << "\t";
            ...
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_WARNING_ + "Bracket minimum: T = " << AGL_data.temperature_external[j] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                << ", helmholtzenergydatapoints.size() = " << helmholtzenergydatapoints.size() << endl;
      ...
      if (aglerror != 0) {
        ...
        if (AGL_data.run_all_pressure_temperature) {
          aurostd::StringstreamClean(aus);
          aus << _AGLSTR_WARNING_ + "Problem fitting polynomial to E + F at T = " << AGL_data.temperature_external[j] << endl;
          aus << _AGLSTR_WARNING_ + "Skipping this temperature point" << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          minpolyerror = aglerror;
          aglerror = 0;
          ...
          if (j == 0) {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_ERROR_ + "No polynomial fit found with a minimum within bounds of input data" << endl;
            aus << _AGLSTR_ERROR_ + "Polynomial fit:  T = " << AGL_data.temperature_external[j] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                << ", helmholtzenergydatapoints.size() = " << helmholtzenergydatapoints.size() << endl;
            aus << _AGLSTR_ERROR_ + "Cannot find minimum for temperature = " << AGL_data.temperature_external[j] << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            if (aglerror == 2) {
            ...
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_WARNING_ + "No polynomial fit found with a minimum within bounds of input data" << endl;
            aus << _AGLSTR_WARNING_ + "Polynomial fit:  T = " << AGL_data.temperature_external[j] << "\t" << ", itry = " << itry << "\t" << ", imin = " << imin << "\t"
                << ", helmholtzenergydatapoints.size() = " << helmholtzenergydatapoints.size() << endl;
            aus << _AGLSTR_WARNING_ + "Resetting maximum value of T to T = " << AGL_data.temperature_external[j - 1] << endl;
            aus << _AGLSTR_WARNING_ + "Resetting number of temperature points to " << j << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            AGL_data.temperature_external.resize(j);
            if (AGL_data.savedatapressure) {
      ...
      for (size_t i = 0; i < helmholtzenergypolynomialcoeffs.size(); i++) {
        gibbsenergypolynomialcoeffs[i] = helmholtzenergypolynomialcoeffs[i];
      }
      for (size_t k = 0; k < AGL_data.pressure_external.size(); k++) {
        aurostd::StringstreamClean(aus);
        if (LIB2RAW_FIX_THIS_LATER) {  // to avoid too much garbage in lib2raw
          aus << _AGLSTR_MESSAGE_ << "Pressure = " << AGL_data.pressure_external[k] << "GPa" << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        // Calculate the value of the Gibbs free energy E + F + pV for each volume AGL_data.volumeinput[i] and store in gibbsenergydatapoints
        ...
        if ((gibbsenergydatapoints.size() != AGL_data.energyinput.size()) || (gibbsenergydatapoints.size() != F_Helmholtzvinp.size()) || (helmholtzenergydatapoints.size() != AGL_data.volumeinput.size())) {
          ...
          for (size_t i = 0; i < AGL_data.volumeinput.size(); i++) {
            gibbsenergydatapoints[i] = AGL_data.energyinput[i] + F_Helmholtzvinp[i] + (AGL_data.volumeinput[i] * AGL_data.pressure_external[k] / eV_Ang3_to_GPa);
          }
        }
        if (AGL_data.fittype == 0) {
          if (minpolyerror > 0) {
        ...
        if (aglerror != 0) {
          ...
          if (AGL_data.run_all_pressure_temperature) {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_WARNING_ + "Cannot find minimum of E + pV + F at T = " << AGL_data.temperature_external[j] << ", p = " << AGL_data.pressure_external[k] << endl;
            ...
            if (k == 0) {
              if (j == 0) {
                aurostd::StringstreamClean(aus);
                aus << _AGLSTR_ERROR_ + "gmin, T = " << AGL_data.temperature_external[j] << ", P = " << AGL_data.pressure_external[k] << ", trial point = " << itry << ", minimum point = " << imin
                    << ", total points = " << gibbsenergydatapoints.size() << endl;
                aus << _AGLSTR_ERROR_ + "gibbsenergydatapoints = ";
                for (size_t i = 0; i < gibbsenergydatapoints.size(); i++) {
                  aus << gibbsenergydatapoints[i] << "\t";
                }
                aus << endl;
                aus << _AGLSTR_ERROR_ + "Cannot find minimum for pressure = " << AGL_data.pressure_external[k] << " and temperature = " << AGL_data.temperature_external[j] << endl;
                aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
                return 2;
              } else {
                aurostd::StringstreamClean(aus);
                aus << _AGLSTR_WARNING_ + "gmin, T = " << AGL_data.temperature_external[j] << ", P = " << AGL_data.pressure_external[k] << ", trial point = " << itry << ", minimum point = " << imin
                    << ", total points = " << gibbsenergydatapoints.size() << endl;
                aus << _AGLSTR_WARNING_ + "gibbsenergydatapoints = ";
                for (size_t i = 0; i < gibbsenergydatapoints.size(); i++) {
                  aus << gibbsenergydatapoints[i] << "\t";
              ...
              aus << endl;
              aus << _AGLSTR_WARNING_ + "Cannot find minimum of E + pV + F at T = " << AGL_data.temperature_external[j] << ", p = " << AGL_data.pressure_external[k] << endl;
              aus << _AGLSTR_WARNING_ + "Resetting maximum value of p to p = " << AGL_data.pressure_external[k - 1] << endl;
              aus << _AGLSTR_WARNING_ + "Resetting number of pressure points to " << k << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
              AGL_data.pressure_external.resize(k);
              AGL_data.d2EnergydVolume2_dynamic.resize(AGL_data.pressure_external.size());
        ...
        if (LDEBUG) {
          ...
          aus << _AGLSTR_MESSAGE_ << "bracket_minimum: var(itry) = " << xconfigurationvector[itry] << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        ...
        if (AGL_data.run_all_pressure_temperature) {
          ...
          if (aglerror != 0) {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_WARNING_ + "Cannot fit polynomial to E + pV + F at T = " << AGL_data.temperature_external[j] << ", p = " << AGL_data.pressure_external[k] << endl;
            aus << _AGLSTR_WARNING_ + "Skipping this pressure and temperature point" << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            continue;
          }
        } else {
          gibbsenergypolynomialcoeffs[3] = helmholtzenergypolynomialcoeffs[3] + (AGL_data.pressure_external[k] / eV_Ang3_to_GPa) * volumereference;
        }
        aglerror = AGL_functions::autocorrect_polynom_minimum(itry, xconfigurationvector, gibbsenergypolynomialcoeffs, xmin, FileMESSAGE);
        ...
        if (pmerr == 0) {
          ...
          if (AGL_data.run_all_pressure_temperature) {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_WARNING_ + "Cannot find minimum of polynomial fit for E + pV + F at T = " << AGL_data.temperature_external[j] << ", p = " << AGL_data.pressure_external[k] << endl;
            aus << _AGLSTR_WARNING_ + "Skipping this pressure and temperature point" << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            continue;
          } else {
            if (k == 0) {
              if (j == 0) {
                aurostd::StringstreamClean(aus);
                aus << _AGLSTR_ERROR_ + "Cannot find minimum of polynomial fit for E + pV + F for p = " << AGL_data.pressure_external[k] << " and T = " << AGL_data.temperature_external[j] << endl;
                aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
                return 2;
              } else {
                aurostd::StringstreamClean(aus);
                aus << _AGLSTR_WARNING_ + "Cannot find minimum of polynomial fit for E + pV + F at T = " << AGL_data.temperature_external[j] << ", p = " << AGL_data.pressure_external[k] << endl;
                aus << _AGLSTR_WARNING_ + "Resetting maximum temperature value to T = " << AGL_data.temperature_external[j - 1] << endl;
                aus << _AGLSTR_WARNING_ + "Resetting number of temperature points to " << j << endl;
                aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
                AGL_data.temperature_external.resize(j);
                if (AGL_data.savedatapressure) {
              ...
              aurostd::StringstreamClean(aus);
              aus << _AGLSTR_WARNING_ + "Cannot find minimum of polynomial fit for E + F + pV at T = " << AGL_data.temperature_external[j] << ", p = " << AGL_data.pressure_external[k] << endl;
              aus << _AGLSTR_WARNING_ + "Resetting maximum pressure value to p = " << AGL_data.pressure_external[k - 1] << endl;
              aus << _AGLSTR_WARNING_ + "Resetting number of pressure points to " << k << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
              AGL_data.pressure_external.resize(k);
        ...
        if (AGL_data.run_all_pressure_temperature) {
          ...
          AGL_pressure_temperature_energy.pressure_external = AGL_data.pressure_external[k];
          AGL_pressure_temperature_energy.temperature_external = AGL_data.temperature_external[j];
        } else {
          AGL_data.GibbsFreeEnergyPressureeV[j].push_back(plnv);
        }
        ...
        if (AGL_data.run_all_pressure_temperature) {
          AGL_pressure_temperature_energy.volume_equilibrium = AGL_data.voleqmin[k];
          aglerror = AGL_functions::numerical_eos_run_all_pT(volumereference, energypolynomialcoeffs, helmholtzenergypolynomialcoeffs, AGL_pressure_temperature_energy.volume_equilibrium, bulkmodpres,
                                                             pres_static, d2EnergydVolume2_dynamic_pres, Gruneisen_pres);
          if (aglerror != 0) {
            aurostd::StringstreamClean(aus);
          ...
          if (d2EnergydVolume2_dynamic_pres <= 0.0) {
            aurostd::StringstreamClean(aus);
            aus << _AGLSTR_WARNING_ + "d^2 E / d V^2 is inconsistent at T = " << AGL_data.temperature_external[j] << ", p = " << AGL_data.pressure_external[k] << endl;
            aus << _AGLSTR_WARNING_ + "Skipping this temperature and pressure point" << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            continue;
          }
          theta_pres = (hbar_eV / KBOLTZEV) * pow(6 * pi * pi * AGL_data.natoms * AGL_pressure_temperature_energy.volume_equilibrium * AGL_pressure_temperature_energy.volume_equilibrium, THIRD) *
                       AGL_data.poissonratiofunction * sqrt((d2EnergydVolume2_dynamic_pres / AGL_data.cellmass) * eV_Ang3_to_amu_Ang_s);
          AGL_functions::thermal_properties(theta_pres, AGL_data.temperature_external[j], DebyeIntegral, DebyeIntegralerror, UIntEnergyVib, Cv_pres, HelmholtzEnergy, Entropy, AGL_data, FileMESSAGE);
          ...
          AGL_data.xminsav[j].push_back(xmin);
          AGL_data.VolumeEquilibrium[j].push_back(AGL_data.voleqmin[k]);
        }
      }
      ...
      if (AGL_data.run_all_pressure_temperature) {
        ...
        if (AGL_data.i_eqn_of_state < 0) {
          if ((AGL_data.pressure_external.size() != AGL_data.voleqmin.size()) || (AGL_data.pressure_external.size() != gibbsenergykjmol.size()) || (AGL_data.pressure_external.size() != AGL_data.bulkmodulus.size())) {
            ...
            for (size_t k = 0; k < AGL_data.pressure_external.size(); k++) {
              AGL_data.outfiless << setw(8) << setprecision(2) << AGL_data.pressure_external[k] << "\t" << setw(6) << AGL_data.temperature_external[j] << "\t" << setw(10) << AGL_data.voleqmin[k] << "\t"
                                 << setw(11) << setprecision(5) << gibbsenergykjmol[k] << "\t" << setw(8) << setprecision(2) << AGL_data.bulkmodulus[k] << endl;
            }
          }
        } else {
          ...
          binp_bcnt = AGL_data.bulkmodulus[0];
          AGL_data.outfiless << endl;
          AGL_data.outfiless << "Temperature:  T = " << AGL_data.temperature_external[j] << "K" << endl;
          ...
          for (size_t k = 0; k < AGL_data.pressure_external.size(); k++) {
            AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.pressure_external[k] << "        " << setw(10) << gibbsenergykjmol[k] << "      " << setw(10)
                               << AGL_data.voleqmin[k] * pow(angstrom2bohr, 3.0) << "      " << setw(8) << setprecision(6) << AGL_data.voleqmin[k] / volume_0pressure << "        " << setw(8) << setprecision(2)
                               << AGL_data.bulkmodulus[k] << "        " << setw(9) << setprecision(6) << relative_error[k] << endl;
          }
          ...
          volumetemperature0pressure.push_back(AGL_data.voleqmin[0]);
          gibbsenergytemperature0pressurekjmol.push_back(gibbsenergykjmol[0]);
          ...
          AGL_data.GibbsFreeEnergy0pressurekjmol.push_back(gibbsenergykjmol[0]);
          AGL_data.GibbsFreeEnergy0pressureeV.push_back(gibbsenergyeV[0]);
          ...
          for (size_t k = 0; k < AGL_data.pressure_external.size(); k++) {
            if (AGL_data.i_debye == 1 || AGL_data.i_debye == 2) {
              ...
              logvol = log(AGL_data.voleqmin[k]);
              ...
              aglerror = AGL_functions::thermal_properties(theta[k], AGL_data.temperature_external[j], DebyeIntegral, DebyeIntegralerror, UIntEnergyVib, Cv[k], HelmholtzEnergy, Entropy, AGL_data, FileMESSAGE);
              IntEnergyVibkjmol = UIntEnergyVib * eV2kjmol;
              IntEnergyVibmeV = UIntEnergyVib * 1000;
              CvVibkjmol = Cv[k] * eV2kjmol * 1000;
              ...
              // Evaluates polynomial energypolynomialcoeffs to get DFT energy at equilibrium V
              AGL_functions::polynom_eval(AGL_data.xminsav[j][k], energypolynomialcoeffs, energyvalue, 0);
              EdftUintVib = energyvalue + UIntEnergyVib;
              AGL_data.EnergyDFT_UIntVib[j].push_back(EdftUintVib);
            } else {
              if (k == 0) {
                aglerror = AGL_functions::debye_polynom_fit(tdebyepolynomialcoeffs, AGL_data, FileMESSAGE);
                if (aglerror != 0) {
              ...
              if (AGL_data.d2EnergydVolume2_dynamic[k] <= 0.0) {
                aurostd::StringstreamClean(aus);
                aus << _AGLSTR_WARNING_ + "inconsistent derivative, v[k] = " << AGL_data.voleqmin[k] << ", p[k] = " << AGL_data.pressure_external[k] << endl;
                aus << _AGLSTR_WARNING_ + "k = " << k << ", T = " << AGL_data.temperature_external[j] << ", d2EnergydVolume2_dynamic[k] = " << AGL_data.d2EnergydVolume2_dynamic[k] << endl;
                aus << _AGLSTR_WARNING_ + "making a Gruneisen interpolation" << endl;
                aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
                ilow = 0;
                while (AGL_data.voleqmin[k] >= AGL_data.volumeinput[ilow] && ilow < (AGL_data.volumeinput.size() - 1)) {
                  ilow = ilow + 1;
                }
                if ((ilow == (AGL_data.volumeinput.size() - 1) && AGL_data.voleqmin[k] >= AGL_data.volumeinput[ilow]) || ilow == 0) {
                  if (k == 0) {
                    aurostd::StringstreamClean(aus);
                    aus << _AGLSTR_ERROR_ + "inconsistent volume, v[k] = " << AGL_data.voleqmin[k] << ", p[k] = " << AGL_data.pressure_external[k] << endl;
                    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
                    return 5;
                  } else {
                    aurostd::StringstreamClean(aus);
                    aus << _AGLSTR_WARNING_ + "Inconsistent volume, v[k] = " << AGL_data.voleqmin[k] << ", p[k] = " << AGL_data.pressure_external[k] << ", k = " << k << endl;
                    aus << _AGLSTR_WARNING_ + "Resetting maximum pressure value to p = " << AGL_data.pressure_external[k - 1] << endl;
                    aus << _AGLSTR_WARNING_ + "Resetting number of pressure points to " << k << endl;
                    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
                    AGL_data.pressure_external.resize(k);
                ...
                ilow = ilow - 1;
                AGL_data.d2EnergydVolume2_dynamic[k] = AGL_data.d2EnergydVolume2_static[ilow] *
                                                       pow(AGL_data.voleqmin[k] / AGL_data.volumeinput[ilow], log(AGL_data.d2EnergydVolume2_static.at(ilow + 1) / AGL_data.d2EnergydVolume2_static[ilow]) /
                                                                                                                  log(AGL_data.volumeinput.at(ilow + 1) / AGL_data.volumeinput[ilow]));
              }
              // Isotropic Debye model properties
              theta[k] = (hbar_eV / KBOLTZEV) * pow(6 * pi * pi * AGL_data.natoms * AGL_data.voleqmin[k] * AGL_data.voleqmin[k], THIRD) * AGL_data.poissonratiofunction *
                         sqrt((AGL_data.d2EnergydVolume2_dynamic[k] / AGL_data.cellmass) * eV_Ang3_to_amu_Ang_s);
              AGL_functions::thermal_properties(theta[k], AGL_data.temperature_external[j], DebyeIntegral, DebyeIntegralerror, UIntEnergyVib, Cv[k], HelmholtzEnergy, Entropy, AGL_data, FileMESSAGE);
              IntEnergyVibkjmol = UIntEnergyVib * eV2kjmol;
              IntEnergyVibmeV = UIntEnergyVib * 1000;
              CvVibkjmol = Cv[k] * eV2kjmol * 1000;
              CvVibUnitkB = CvVibkjmol * kj2unit;
            ...
            AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.pressure_external[k] << "        " << setw(11) << setprecision(5) << IntEnergyVibkjmol << "     " << setw(12)
                               << setprecision(6) << CvVibkjmol << "    " << setw(10) << setprecision(4) << HelmholtzVibkjmol << "      " << setw(11) << setprecision(6) << EntropyVibkjmol << "     " << setw(9)
                               << setprecision(2) << theta[k] << "       " << setw(6) << setprecision(3) << AGL_data.gamma_G[k] << endl;
            if (k == 0) {
              ...
              AGL_data.DebyeTemperature0pressure.push_back(theta[k]);
              AGL_data.GruneisenParameter0pressure.push_back(AGL_data.gamma_G[k]);
            ...
            if (AGL_data.savedatapressure) {
              ...
              AGL_data.CvkjmolPressure[j].push_back(CvVibkjmol);
              AGL_data.CvunitkBpressure[j].push_back(CvVibUnitkB);
              AGL_data.InternalEnergyPressurekjmol[j].push_back(IntEnergyVibkjmol);
              AGL_data.InternalEnergyPressuremeV[j].push_back(IntEnergyVibmeV);
              AGL_data.EntropyPressurekjmol[j].push_back(EntropyVibkjmol);
              AGL_data.EntropyPressuremeV[j].push_back(EntropyVibmeV);
              AGL_data.EntropyunitkBpressure[j].push_back(EntropyVibUnitkB);
              AGL_data.DebyeTemperaturePressure[j].push_back(theta[k]);
              AGL_data.GruneisenParameterPressure[j].push_back(AGL_data.gamma_G[k]);
              AGL_data.HelmholtzEnergyPressurekjmol[j].push_back(HelmholtzVibkjmol);
              AGL_data.HelmholtzEnergyPressuremeV[j].push_back(HelmholtzVibmeV);
            }
          }
          ...
          AGL_data.outfiless << " --------------------------------------------------------------------------- " << endl;
          for (size_t k = 0; k < AGL_data.pressure_external.size(); k++) {
            Pbeta = Cv[k] * AGL_data.gamma_G[k] / AGL_data.voleqmin[k] * eV_Ang3_to_GPa;
            AGL_data.alpha[k] = Pbeta / AGL_data.bulkmodulus[k];
            gammaalphaT = 1.0 + AGL_data.gamma_G[k] * AGL_data.alpha[k] * AGL_data.temperature_external[j];
            Cpkjmol = Cv[k] * gammaalphaT * eV2kjmol * 1000;
            CpUnitkB = Cpkjmol * kj2unit;
            Bstatic = AGL_data.bulkmodulus[k] * gammaalphaT;
            AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.pressure_external[k] << "        " << setw(17) << setprecision(8) << AGL_data.alpha[k] * 1e5 << "  " << setw(13)
                               << setprecision(9) << Pbeta << "   " << setw(8) << setprecision(2) << Bstatic << "    " << setw(12) << setprecision(6) << Cpkjmol << endl;
            if (k == 0) {
              AGL_data.ThermalExpansion0pressure.push_back(AGL_data.alpha[k]);
          ...
          AGL_data.outfiletss << setw(8) << setprecision(2) << std::fixed << AGL_data.temperature_external[j] << setprecision(8) << setw(15) << AGL_data.InternalEnergy0pressuremeV[j] << " " << setw(15)
                              << AGL_data.HelmholtzEnergy0pressuremeV[j] << " " << setw(15) << AGL_data.Entropy0pressureunitkB[j] << " " << setw(15) << AGL_data.CvunitkB0pressure[j] << " " << setw(15)
                              << AGL_data.DebyeTemperature0pressure[j] << " " << setw(23) << AGL_data.GruneisenParameter0pressure[j] << endl;
        }
      }
    ...
    if (AGL_data.run_all_pressure_temperature) {
      ...
      if (AGL_data.i_eqn_of_state >= 0) {
        ...
        AGL_data.outfiless << " -------------------------------------------------------------------------------------------- " << endl;
        for (size_t j = 0; j < AGL_data.temperature_external.size(); j++) {
          AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.temperature_external[j] << "        " << setw(10) << setprecision(4) << (volumetemperature0pressure[j] * pow(angstrom2bohr, 3.0))
                             << "      " << setw(10) << setprecision(4) << gibbsenergytemperature0pressurekjmol[j] << "      " << setw(10) << setprecision(4) << AGL_data.InternalEnergy0pressurekjmol[j]
                             << "      " << setw(11) << setprecision(5) << AGL_data.Entropy0pressurekjmol[j] << "     " << setw(12) << setprecision(6) << AGL_data.Cvkjmol0pressure[j] << endl;
        }
        AGL_data.outfiless << endl;
        ...
        AGL_data.outfiless << " ------------------------------------------------------------------------------------------------- " << endl;
        for (size_t j = 0; j < AGL_data.temperature_external.size(); j++) {
          AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.temperature_external[j] << "        " << setw(8) << setprecision(2) << AGL_data.bulkmodulusisothermal_0pressure[j]
                             << "        " << setw(8) << setprecision(5) << dbulkmodulusdpt0pressure[j] << "        " << setw(12) << setprecision(6) << d2bulkmodulusdp2t0pressure[j] << "    " << setw(8)
                             << setprecision(2) << AGL_data.bulkmodulusstatic_0pressure[j] << "        " << setw(17) << setprecision(5) << AGL_data.ThermalExpansion0pressure[j] * 100000.0 << endl;
        }
        if (AGL_data.i_eqn_of_state == 5 || AGL_data.i_eqn_of_state == 6) {
          AGL_data.outfiless << endl;
          ...
          AGL_data.outfiless << " ------------------------------------------------------------------------ " << endl;
          for (size_t j = 0; j < AGL_data.temperature_external.size(); j++) {
            AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.temperature_external[j] << "\t" << setw(8) << setprecision(6) << bcnt_K_sp[j] << "\t" << setw(9) << setprecision(2)
                               << -bcnt_P_sp[j] << "\t" << setw(12) << setprecision(6) << bcnt_V_sp[j] << "\t" << setw(8) << setprecision(5) << bcnt_beta_sp[j] << endl;
          }
        }
        AGL_data.outfiless << endl;
        ...
        AGL_data.outfiless << " -------------------------------------- " << endl;
        for (size_t j = 0; j < AGL_data.temperature_external.size(); j++) {
          AGL_data.outfiless << setw(8) << setprecision(2) << std::fixed << AGL_data.temperature_external[j] << "        " << setw(9) << setprecision(2) << AGL_data.DebyeTemperature0pressure[j] << "       "
                             << setw(6) << setprecision(2) << AGL_data.GruneisenParameter0pressure[j] << endl;
        }
      }
    }
