...
namespace AGL_functions {
  ...
  uint runHugoniot(const std::vector<double>& pressures_external,
    ...
    double initial_temperature_external = 0.0;
    double initial_energyDFT_UIntVib_kJg = 0.0;
    double initial_pressure_external = 0.0;

    // CHANGES BY PA20171224
    if (0 != findBestInitialConditions(pressures_external, temperatures_external, mass_densities_gcm3, energies_pT_kJg, desired_initial_pressure_external, desired_initial_temperature_external,
                                       initial_mass_density_gcm3, initial_temperature_external, initial_energyDFT_UIntVib_kJg, initial_pressure_external, FileMESSAGE)) {
      return 1;
    }
    // END CHANGES BY PA20171224
    ...
    for (size_t i = 0; i < pressures_external.size(); ++i) {
      const double pressure_external = pressures_external[i];
      // Create the input vectors
      const std::vector<double>& energiesDFT_UIntVib_kJg = energies_pT_kJg[i];
      std::vector<double> tmp_mass_densities_gcm3;
      for (size_t j = 0; j < temperatures_external.size(); ++j) {
        tmp_mass_densities_gcm3.push_back(mass_densities_gcm3[j][i]);
      }
      // END CHANGES BY PA20171224

      // Now calculate the Hugoniot!
      double hugoniot_density = 0.0;
      double hugoniot_temperature = 0.0;
      double hugoniot_energy = 0.0;
      // CHANGES BY PA20170603
      const uint ret = calculateHugoniotDataPoint(initial_mass_density_gcm3, initial_energyDFT_UIntVib_kJg, initial_pressure_external, tmp_mass_densities_gcm3, temperatures_external, energiesDFT_UIntVib_kJg,
                                                  pressure_external, hugoniot_density, hugoniot_temperature, hugoniot_energy, hugoniotextrapolate, FileMESSAGE);
      if (ret == 1) {
        aurostd::StringstreamClean(aus);
...
namespace AGL_functions {
  ...
  uint runHugoniotAllTemperaturesAndPressures(const std::vector<AGL_pressure_temperature_energies>& AGL_pressure_temperature_energy_list,
    ...
    const double pressureTol = 1.e-5;
    ...
    for (size_t i = 0; i < AGL_pressure_temperature_energy_list.size(); ++i) {
      ...
      const double energy_kJg = (AGL_pressure_temperature_energy_list[i].E_DFT_internal_vib_energy * 1.6e-22) / cellmass_grams;
      energies_pT_kJg.push_back(energy_kJg);

      // Next get equilibrium cell volume in units of cm^3
      const double volume_cm3 = AGL_pressure_temperature_energy_list[i].volume_equilibrium * 1.e-24;
      // Calculate mass density and save for each temperature and pressure
      const double massdensity_gcm3 = cellmass_grams / volume_cm3;
      mass_densities_gcm3.push_back(massdensity_gcm3);

      // Add the pressure if it isn't already in the pressures vector
      const double pressure = AGL_pressure_temperature_energy_list[i].pressure_external;
    ...
    if (0 != findBestInitialConditionsAllTemperaturesAndPressures(AGL_pressure_temperature_energy_list, mass_densities_gcm3, energies_pT_kJg, desired_initial_pressure_external, desired_initial_temperature_external,
                                                                  initial_mass_density_gcm3, initial_temperature_external, initial_energyDFT_UIntVib_kJg, initial_pressure_external, FileMESSAGE)) {
      return 1;
    }
    ...
    // Now let's get all the data
    for (size_t i = 0; i < pressures_external.size(); ++i) {
      const double currentPressure = pressures_external[i];
      ...
      for (size_t j = 0; j < AGL_pressure_temperature_energy_list.size(); ++j) {
        const double pressure = AGL_pressure_temperature_energy_list[j].pressure_external;
        if (aurostd::abs(pressure - currentPressure) < pressureTol) {
          temperatures_external.push_back(AGL_pressure_temperature_energy_list[j].temperature_external);
          energiesDFT_UIntVib_kJg.push_back(energies_pT_kJg[j]);
          tmp_mass_densities_gcm3.push_back(mass_densities_gcm3[j]);
        }
      }
      ...
      double hugoniot_energy = 0.0;
      const uint ret = calculateHugoniotDataPoint(initial_mass_density_gcm3, initial_energyDFT_UIntVib_kJg, initial_pressure_external, tmp_mass_densities_gcm3, temperatures_external, energiesDFT_UIntVib_kJg,
                                                  currentPressure, hugoniot_density, hugoniot_temperature, hugoniot_energy, hugoniotextrapolate, FileMESSAGE);
      if (ret == 1) {
        aurostd::StringstreamClean(aus);
        aus << _AGLSTR_ERROR_ + "Error in " << __FUNCTION__ << ": calculateHugoniotDataPoint returned with an error!" << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        return ret;
      } else if (ret == 2) {
        // This means that we are extrapolating too far and should stop
...
namespace AGL_functions {
  ...
  uint calculateHugoniotDataPoint(double initial_mass_density_gcm3,
    ...
    if (mass_densities_gcm3.size() != temperatures_external.size() || mass_densities_gcm3.size() != energiesDFT_UIntVib_kJg.size()) {
      aurostd::StringstreamClean(aus);
    ...
    for (size_t i = 0; i < mass_densities_gcm3.size(); ++i) {
      if (i != 0 && temperatures_external[i] < temperatures_external.at(i - 1)) {
        aurostd::StringstreamClean(aus);
      ...
      const double val = energiesDFT_UIntVib_kJg[i] - initial_energyDFT_UIntVib_kJg - 0.5 * (pressure_external + initial_pressure_external) * (1.0 / initial_mass_density_gcm3 - 1.0 / mass_densities_gcm3[i]);
      // std::cout << "E is " << energiesDFT_UIntVib_kJg[i] << "; D is " << mass_densities_gcm3[i] << "\n";
      //  Figure out when we switch signs. The solution is between these
      //  two points.
      if ((val == 0.0 || aurostd::abs(val + lastVal) < aurostd::abs(val) + aurostd::abs(lastVal)) && i != 0) {
        ind1 = i - 1;
        ind2 = i;
        break;
      }
    ...
    if (ind1 == 0 && ind2 == 0) {
      ...
      if (hugoniotextrapolate) {
        const size_t lastInd = mass_densities_gcm3.size() - 1;
        // If the first point is closer to the solution, use the first two
        // points. Otherwise, use the last two points.
        if (aurostd::abs(energiesDFT_UIntVib_kJg[0] - initial_energyDFT_UIntVib_kJg - 0.5 * (pressure_external + initial_pressure_external) * (1.0 / initial_mass_density_gcm3 - 1.0 / mass_densities_gcm3[0])) <
            aurostd::abs(energiesDFT_UIntVib_kJg[lastInd] - initial_energyDFT_UIntVib_kJg - 0.5 * (pressure_external + initial_pressure_external) * (1.0 / initial_mass_density_gcm3 - 1.0 / mass_densities_gcm3[lastInd]))) {
          ind1 = 0;
          ind2 = 1;
        } else {
          ind1 = lastInd - 1;
    ...
    interpolateToFindHugoniotEnergyAndDensity(initial_mass_density_gcm3, initial_energyDFT_UIntVib_kJg, initial_pressure_external, pressure_external, energiesDFT_UIntVib_kJg[ind1],
                                              energiesDFT_UIntVib_kJg[ind2], mass_densities_gcm3[ind1], mass_densities_gcm3[ind2], hugoniot_energy, hugoniot_density);
    ...
    // be less than energiesDFT_UIntVib_kJg[ind1]. In addition, fraction may be greater than 1.0
    // if and only if the solution is greater than temperatures_external.at(lastInd - 1).
    const double fraction = (hugoniot_energy - energiesDFT_UIntVib_kJg[ind1]) / aurostd::abs(energiesDFT_UIntVib_kJg[ind2] - energiesDFT_UIntVib_kJg[ind1]);

    // The temperature should be the same fraction away from the ind1
    // temperature.
    hugoniot_temperature = aurostd::abs(temperatures_external[ind1] - temperatures_external[ind2]) * fraction + temperatures_external[ind1];
    ...
    if (hugoniot_temperature > 1.5 * temperatures_external.back()) {
      aurostd::StringstreamClean(aus);
      aus << _AGLSTR_WARNING_ + "In " << __FUNCTION__ << ", the solution was found to be greater than 1.5 times the greatest temperature value in the temperatures_external vector." << endl;
      aus << _AGLSTR_WARNING_ + "Data should beyond this temperature should be considered invalid since too great an extrapolation was used." << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      return 2;
    }
    // END CHANGES BY PA20170603
...
namespace AGL_functions {
  ...
  uint findBestInitialConditions(const std::vector<double>& pressures_external,
    ...
    int bestInitialPressureInd = -1;
    int bestInitialTemperatureInd = -1;
    double bestInitialPressure = -1.0;
    double bestInitialTemperature = -1.0;
    ...
    if (aurostd::abs(bestInitialPressure - desired_initial_pressure_external) > 1.0) {
      aurostd::StringstreamClean(aus);
      aus << _AGLSTR_WARNING_ + "Warning in " << __FUNCTION__ << ": the closest pressure to the desired initial pressure of " << desired_initial_pressure_external << " GPa was found to be "
          << bestInitialPressure << " GPa." << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }
    ...
    for (size_t i = 0; i < temperatures_external.size(); ++i) {
      ...
      // closer to the desired initial, keep it.
      const double currentTemperature = temperatures_external[i];
      // END CHANGES BY PA20171224
      if (bestInitialTemperatureInd == -1 || aurostd::abs(currentTemperature - desired_initial_temperature_external) < aurostd::abs(bestInitialTemperature - desired_initial_temperature_external)) {
    ...
    if (aurostd::abs(bestInitialTemperature - desired_initial_temperature_external) > 10.0) {
      aurostd::StringstreamClean(aus);
      aus << _AGLSTR_WARNING_ + "Warning in " << __FUNCTION__ << ": the closest temperature to the desired initial temperature of " << desired_initial_temperature_external << " K was found to be "
          << bestInitialTemperature << " K." << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }

    // Now that we have the indices, extract the data for the initial conditions.
    initial_energyDFT_UIntVib_kJg = energies_pT_kJg[bestInitialPressureInd][bestInitialTemperatureInd];
    initial_temperature_external = bestInitialTemperature;
    initial_mass_density_gcm3 = mass_densities_gcm3[bestInitialTemperatureInd][bestInitialPressureInd];
...
namespace AGL_functions {
  ...
  uint findBestInitialConditionsAllTemperaturesAndPressures(const std::vector<AGL_pressure_temperature_energies>& AGL_pressure_temperature_energy_list,
    ...
    for (size_t i = 0; i < AGL_pressure_temperature_energy_list.size(); ++i) {
      const double& currentPressure = AGL_pressure_temperature_energy_list[i].pressure_external;
      if (aurostd::abs(currentPressure - desired_initial_pressure_external) < aurostd::abs(bestInitialPressure - desired_initial_pressure_external)) {
        bestInitialPressure = currentPressure;
      }
    }
    ...
    for (size_t i = 0; i < AGL_pressure_temperature_energy_list.size(); ++i) {
      ...
      if (aurostd::abs(currentPressure - bestInitialPressure) < pressureTol) {
        const double& currentTemperature = AGL_pressure_temperature_energy_list[i].temperature_external;
        if (aurostd::abs(currentTemperature - desired_initial_temperature_external) < aurostd::abs(bestInitialTemperature - desired_initial_temperature_external)) {
          bestInitialTemperature = currentTemperature;
          bestConditionsInd = i;
        }
      }
    ...
    // Now that we have the indices, extract the data for the initial conditions.
    initial_energyDFT_UIntVib_kJg = energies_pT_kJg[bestConditionsInd];
    initial_temperature_external = bestInitialTemperature;
    initial_mass_density_gcm3 = mass_densities_gcm3[bestConditionsInd];
    initial_pressure_external = bestInitialPressure;
...
namespace AGL_functions {
  ...
  void interpolateToFindHugoniotEnergyAndDensity(
    ...
    const double slope = (density2 - density1) / (energy2 - energy1);
    const double slope_intercept = density1 - slope * energy1;
    ...
    const double cached_expression = pressure_external + initial_pressure_external + 2.0 * initial_mass_density_gcm3 * initial_energyDFT_UIntVib_kJg;
    // The following equation is ugly, but it is an analytical solution that results from the
    // interpolation.
    hugoniot_energy = (sqrt(4.0 * pow(slope_intercept, 2.0) * pow(initial_mass_density_gcm3, 2.0) + 4.0 * slope_intercept * slope * cached_expression * initial_mass_density_gcm3 +
                            slope * (slope * pow(cached_expression, 2.0) - 8.0 * (pressure_external + initial_pressure_external) * pow(initial_mass_density_gcm3, 2.0))) -
