...
namespace apl {
  ...
  void PhononCalculator::setHarmonicForceConstants(const ForceConstantCalculator& fc) {
    setHarmonicForceConstants(fc.getForceConstants(), fc.getBornEffectiveChargeTensor(), fc.getDielectricTensor(), fc.isPolarMaterial());// AS20201208
  }

  // AS20201204 BEGIN
  void PhononCalculator::setHarmonicForceConstants(const vector<vector<xmatrix<double>>>& IFC, const vector<xmatrix<double>>& bornEffectiveChargeTensor, const xmatrix<double>& dielectricTensor, bool isPolar) {
    string message;
    // check if the input IFCs have correct size
    const uint natoms = _supercell.getNumberOfAtoms();
    if (IFC.size() != natoms) {
    ...
    const uint niatoms = _supercell.getNumberOfUniqueAtoms();
    if (isPolar) {
      if (bornEffectiveChargeTensor.size() != niatoms) {
        ...
        for (uint i = 0; i < niatoms; i++) {
          if ((bornEffectiveChargeTensor[i].rows != 3) || (bornEffectiveChargeTensor[i].cols != 3)) {
  ...
  void PhononCalculator::setHarmonicForceConstants(const vector<vector<xmatrix<double>>>& IFC) {
    const vector<xmatrix<double>> bornEffectiveChargeTensor;// dummy object, material is not polar
    const xmatrix<double> dielectricTensor;// dummy object, material is not polar
  ...
  void PhononCalculator::awake() {
    ...
    if (_isPolarMaterial) {
      readBornChargesDielectricTensor(aurostd::CleanFileName(base + DEFAULT_APL_POLAR_FILE));
    }
  }

  void PhononCalculator::readHarmonicIFCs(const string& hibfile_raw) {
    string message;
    string hibfile;
    if (!aurostd::CompressFileExist(hibfile_raw, hibfile)) {
    ...
    aurostd::compressfile2vectorstring(hibfile, vlines);  // CO //ME20181226
    ...
    if (line.find("xml") == string::npos) {
      const string message = "Not an xml file.";
    ...
    while (true) {
      ...
      if (line.find("aflow_version") != string::npos) {
        break;
      }
    }
    ...
    while (true) {
      ...
      for (int k = 1; k <= 3; k++) {
        line = vlines[line_count++];  // CO
        const int t = line.find_first_of(">") + 1;
  ...
  void PhononCalculator::readBornChargesDielectricTensor(const string& hibfile_raw) {
    string message;
    string hibfile;
    if (!aurostd::CompressFileExist(hibfile_raw, hibfile)) {
      message = "Cannot find file " + hibfile + ".";
    ...
    aurostd::compressfile2vectorstring(hibfile, vlines);
  ...
  void PhononCalculator::setAnharmonicForceConstants(const AnharmonicIFCs& fc) {
    const int order = fc.getOrder();
    const int nifcs = (int) anharmonicIFCs.size();
    const vector<vector<double>> dummy_ifc;
    const vector<vector<int>> dummy_clst;
  ...
  void PhononCalculator::readAnharmonicIFCs(string filename) {
    ...
    const uint nanharm = anharmonicIFCs.size();
    ...
    while (line.find("/force_constants") == string::npos) {
      ...
      if (line.find("atoms") != string::npos) {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          for (int j = 0; j < 3; j++) {
            fc.push_back(aurostd::string2utype<double>(tokens[j]));
          }
        }
...
namespace apl {
  ...
  xvector<double> PhononCalculator::getFrequency(const xvector<double>& kpoint, const IPCFreqFlags& flags, xmatrix<xcomplex<double>>& eigenvectors) {
    return getFrequency(kpoint, kpoint, flags, eigenvectors);
  }

  xvector<double> PhononCalculator::getFrequency(const xvector<double>& kpoint, const xvector<double>& kpoint_nac, const IPCFreqFlags& flags, xmatrix<xcomplex<double>>& eigenvectors) {
    vector<xvector<double>> placeholder_gvel;
    return getFrequency(kpoint, kpoint_nac, flags, eigenvectors, placeholder_gvel, false);
  }

  xvector<double> PhononCalculator::getFrequency(const xvector<double>& kpoint, const IPCFreqFlags& flags, xmatrix<xcomplex<double>>& eigenvectors, vector<xvector<double>>& gvel, bool calc_derivative) {
  ...
  xvector<double> PhononCalculator::getFrequency(const xvector<double>& kpoint, const xvector<double>& kpoint_nac, const IPCFreqFlags& flags, xmatrix<xcomplex<double>>& eigenvectors, vector<xvector<double>>& gvel, bool calc_gvel) {
    ...
    if (calc_gvel) {
      const double conversionFactorGvel = getFrequencyConversionFactor(flags, apl::THZ | apl::OMEGA);
      const uint nbranches = getNumberOfBranches();
      gvel.clear();
      gvel.resize(nbranches, xvector<double>(3));
      xvector<xcomplex<double>> eigenvec(3);
      xvector<xcomplex<double>> eigenvec_conj(3);
  ...
  double PhononCalculator::getFrequencyConversionFactor(IPCFreqFlags inFlags, IPCFreqFlags outFlags) {
    ...
    if (inFlags & apl::RAW) {
      if (outFlags & apl::RAW) {
        ...
        const string message = "Not implemented conversion.";
  ...
  xvector<double> PhononCalculator::getEigenvalues(const xvector<double>& kpoint, const xvector<double>& kpoint_nac, xmatrix<xcomplex<double>>& eigenvectors, vector<xmatrix<xcomplex<double>>>& dDynMat, bool calc_derivative) {
  ...
  xmatrix<xcomplex<double>> PhononCalculator::getDynamicalMatrix(const xvector<double>& kpoint, const xvector<double>& kpoint_nac, vector<xmatrix<xcomplex<double>>>& dDynMat, bool calc_derivative) {
    const uint scAtomsSize = _supercell.getSupercellStructure().atoms.size();
    ...
    if (calc_derivative) {  // reset dDynMat
      dDynMat.clear();
      const xmatrix<xcomplex<double>> mat(_nBranches, _nBranches, 1, 1);
    ...
    for (uint ipc1 = 0; ipc1 < pcAtomsSize; ipc1++) {
      const uint isc1 = _supercell.pc2scMap(ipc1);

      for (uint isc2 = 0; isc2 < scAtomsSize; isc2++) {
        const uint ipc2 = _supercell.sc2pcMap(isc2);
    ...
    // Divide by masses
    for (uint i = 0; i < pcAtomsSize; i++) {
      const double mass_i = _supercell.getAtomMass(_supercell.pc2scMap(i));
      for (uint j = 0; j < pcAtomsSize; j++) {
        const double mass_j = _supercell.getAtomMass(_supercell.pc2scMap(j));
        for (int ix = 1; ix <= 3; ix++) {
          for (int iy = 1; iy <= 3; iy++) {
  ...
  xmatrix<xcomplex<double>> PhononCalculator::getNonanalyticalTermWang(const xvector<double>& _q, vector<xmatrix<xcomplex<double>>>& derivative, bool calc_derivative) {
    ...
    if (aurostd::modulus(q) > _FLOAT_TOL_) {
      ...
      // Calculation
      const double fac0 = hartree2eV * bohr2angstrom;  // from a.u. to eV/A  //ME20200206 - replaced with xscalar constants
      const double volume = det(pc.lattice);
      const double fac1 = 4.0 * PI / volume;
      const double nbCells = det(sc.lattice) / volume;

      // Precompute product of q-point with charge tensor
      vector<xvector<double>> qZ(pcIAtomsSize);
      for (uint at = 0; at < pcIAtomsSize; at++) {
        qZ[at] = q * _bornEffectiveChargeTensor[at];
      }

      const double dotprod = scalar_product(q, _dielectricTensor * q);
      const double prefactor = fac0 * fac1 / (dotprod * nbCells);
      for (uint ipc1 = 0; ipc1 < pcAtomsSize; ipc1++) {
        ...
        for (uint ipc2 = 0; ipc2 < pcAtomsSize; ipc2++) {
          ...
          for (int ix = 1; ix <= 3; ix++) {
            for (int iy = 1; iy <= 3; iy++) {
              ...
              const double borni = qZ[iat1][ix];
              const double bornj = qZ[iat2][iy];
  ...
  xmatrix<xcomplex<double>> PhononCalculator::getEwaldSumDipoleDipoleContribution(const xvector<double> qpoint, bool includeTerm1) {
    ...
    // Reciprocal Space
    const xmatrix<double> klattice = trasp(ReciprocalLattice(pc.lattice));

    // Grid
    const int n1 = (int) (sqrt(geg) / aurostd::modulus(klattice(1))) + 1;
    const int n2 = (int) (sqrt(geg) / aurostd::modulus(klattice(2))) + 1;
    const int n3 = (int) (sqrt(geg) / aurostd::modulus(klattice(3))) + 1;

    // Calculation
    const double fac0 = hartree2eV * bohr2angstrom;  // from a.u. to eV/A  //ME20200207 - replaced with xscalar constants
    const double SQRTPI = sqrt(PI);
    const double volume = det(pc.lattice);
    const double fac = 4.0 * PI / volume;
    const xcomplex<double> iONE(0.0, 1.0);
      ...
      for (int m1 = -n1; m1 <= n1; m1++) {
        for (int m2 = -n2; m2 <= n2; m2++) {
          for (int m3 = -n3; m3 <= n3; m3++) {
            const xvector<double> g = m1 * klattice(1) + m2 * klattice(2) + m3 * klattice(3) + qpoint;

            geg = scalar_product(g, _dielectricTensor * g);

            if (aurostd::abs(geg) > _FLOAT_TOL_ && geg / lambda2 / 4.0 < gmax) {
              const double fac2 = fac * exp(-geg / lambda2 / 4.0) / geg;
              ...
              for (uint ipc1 = 0; ipc1 < pcAtomsSize; ipc1++) {
                ...
                for (uint ipc2 = 0; ipc2 < pcAtomsSize; ipc2++) {
                  ...
                  const xcomplex<double> facg = fac2 * exp(iONE * scalar_product(g, sc.atoms[ipc2].cpos - sc.atoms[ipc1].cpos));
    ...
    // Term 2
    const uint scAtomsSize = sc.atoms.size();
    for (uint ipc1 = 0; ipc1 < pcAtomsSize; ipc1++) {
      const uint isc1 = _supercell.pc2scMap(ipc1);
      ...
      for (uint isc2 = 0; isc2 < scAtomsSize; isc2++) {
        ...
        const xvector<double> rc = SYM::minimizeDistanceCartesianMethod(sc.atoms[isc2].cpos, sc.atoms[isc1].cpos, sc.lattice);
        const xvector<double> rf = F2C(sc.lattice, rc);

        if (aurostd::modulus(rc) < _FLOAT_TOL_) {
          continue;
        }

        //
        const xvector<double> delta = _inverseDielectricTensor * rc;
        ...
        const double emy2dpi = 2.0 * exp(-y2) / SQRTPI;
        const double erfcdy = erfc(y) / y;
        const double c1 = ym2 * (3.0 * erfcdy * ym2 + (emy2dpi * (3.0 * ym2 + 2.0)));
        const double c2 = ym2 * (erfcdy + emy2dpi);
        for (int a = 1; a <= 3; a++) {
          for (int b = 1; b <= 3; b++) {
            H(a, b) = x(a) * x(b) * c1 - _inverseDielectricTensor(a, b) * c2;
          }
        ...
        //
        const xcomplex<double> fac = fac0 * lambda3 * _recsqrtDielectricTensorDeterminant * e;
        for (int ix = 1; ix <= 3; ix++) {
          for (int iy = 1; iy <= 3; iy++) {
            dynamicalMatrix(3 * ipc1 + ix, 3 * ipc2 + iy) -= fac * zhz(ix, iy);
          }
    ...
    const double facterm3 = fac0 * 4.0 * lambda3 * _recsqrtDielectricTensorDeterminant / (3.0 * SQRTPI);
    for (uint ipc1 = 0; ipc1 < pcAtomsSize; ipc1++) {
...
namespace apl {
  ...
  vector<vector<xvector<double>>> PhononCalculator::calculateGroupVelocitiesOnMesh(vector<vector<double>>& freqs, vector<xmatrix<xcomplex<double>>>& eigenvectors) {
    ...
    std::function<void(int, vector<vector<double>>&, vector<xmatrix<xcomplex<double>>>&, vector<vector<xvector<double>>>&)> fn = std::bind(&PhononCalculator::calculateGroupVelocitiesThread, this, _1, _2, _3, _4);
  ...
  void PhononCalculator::calculateGroupVelocitiesThread(int q, vector<vector<double>>& freqs, vector<xmatrix<xcomplex<double>>>& eigenvectors, vector<vector<xvector<double>>>& gvel) {
    xvector<double> f;
  ...
  void PhononCalculator::writeGroupVelocitiesToFile(const string& filename, const vector<vector<xvector<double>>>& gvel) {
    const vector<vector<double>> freqs;
    writeGroupVelocitiesToFile(filename, gvel, freqs);
  }
  void PhononCalculator::writeGroupVelocitiesToFile(const string& filename, const vector<vector<xvector<double>>>& gvel, const vector<vector<double>>& freqs, const string& unit) {
    if (gvel.empty()) {
    ...
    if (!freqs.empty()) {
      ...
      output << std::setiosflags(std::ios::fixed | std::ios::right);
      output << std::setw(10) << "# Q-point" << std::setw(20) << " "
