...
namespace apl {
  ...
  void PhononDispersionCalculator::initPathCoords(  // CO20180406
    ...
    if (USER_DC_INITCOORDS.empty() || USER_DC_INITLABELS.empty()) {
      const string message = "Inputs are empty.";
  ...
  void PhononDispersionCalculator::initPathLattice(const string& USER_DC_INITLATTICE, int USER_DC_NPOINTS) {
    ...
    if (lattice.empty()) {
      ...
      // CO START
      if (a.bravais_lattice_variation_type.empty()) {
        if (a.spacegroup.empty()) {
          if (a.space_group_ITC < 1 || a.space_group_ITC > 230) {
            a.space_group_ITC = a.SpaceGroup_ITC();
          } // if (a.space_group_ITC == 0) { //CO20180214 - if not set then it could be 32767 //[CO20200106 - close bracket for indenting]}
    ...
    if (!USER_DC_OWNPATH.empty()) {
      if (USER_DC_OWNPATH.find('|') != string::npos) {
        ...
        if (tokens[0].find('|') != string::npos) {
          const string message = "Cannot have | in the first path coordinate";
  ...
  void PhononDispersionCalculator::calculateInOneThread(int iqp) {
    ...
    if (_pc->isPolarMaterial() && (aurostd::modulus(_qpoints[iqp]) < 0.005)) {
      const int npts = _pb.getDensity() + 1;
      const int i = iqp / npts;
      const xvector<double> qpoint_nac = _qpoints[i * npts] - _qpoints[(i + 1) * npts - 1];
  ...
  void PhononDispersionCalculator::calc(const IPCFreqFlags frequencyFormat) {
    ...
    // Prepare storage
    _freqs.clear();
    const xvector<double> zero(_pc->getNumberOfBranches());
    for (size_t i = 0; i < _qpoints.size(); i++) {
      _freqs.push_back(zero);
    }

    const int nqps = (int) _qpoints.size();
  ...
  void PhononDispersionCalculator::writePDIS(const string& directory) {
    ...
    const string filename = aurostd::CleanFileName(directory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_PDIS_FILE); // ME20181226
    ...
    double x = 0.0;
    const double wholePathLength = _pb.getPathLength();
    std::map<double, string> labelMap;
    for (uint i = 1; i < _pb.getPointSize(); i++) {
      outfile << "# <label>     " << x << " " << setw(5) << _pb.getPointLabel(i) << std::endl;
      ...
      if (isExactQPoint(_qpoints[i], _pc->getSuperCellStructure().lattice)) {
        ...
        if (j == exactPointPositions.size()) {
          ...
          for (; iter != labelMap.end(); iter++) {
            if (aurostd::abs(iter->first - x) < _FLOAT_TOL_) {
              break;
            }
          }
          ...
          outfile << "# <exact>     " << x << " " << setw(5) << name << std::endl;
    ...
    // PN //PN20180705
    const string hskptsfile = aurostd::CleanFileName(directory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_HSKPTS_FILE); // ME20181226
  ...
  bool PhononDispersionCalculator::isExactQPoint(const xvector<double>& qpoint, const xmatrix<double>& lattice) {
    ...
    for (int i = 1; i <= 1; i++) {
      for (int j = 1; j <= 1; j++) {
        for (int k = 1; k <= 1; k++) {
          const xvector<double> L = (((double) i) * lattice(1) + ((double) j) * lattice(2) + ((double) k) * lattice(3));
          const xcomplex<double> p = exp(iONE * scalar_product(qpoint, L));
          if ((aurostd::abs(p.imag()) < _FLOAT_TOL_) && (aurostd::abs(p.real() - 1.0) < _FLOAT_TOL_)) {
            isExact = true;
            break;
          }
        }
  ...
  void PhononDispersionCalculator::writePHEIGENVAL(const string& directory) {
    ...
    const string filename = aurostd::CleanFileName(directory + "/" + DEFAULT_APL_PHEIGENVAL_FILE);
  ...
  xEIGENVAL PhononDispersionCalculator::createEIGENVAL() {
    ...
    // Data
    const double weight = 1.0 / _freqs.size();
    const double factorTHz2Raw = _pc->getFrequencyConversionFactor(apl::THZ, apl::RAW);
    const double factorRaw2meV = _pc->getFrequencyConversionFactor(apl::RAW, apl::MEV);
    const double conv = factorTHz2Raw * factorRaw2meV / 1000;
    const apl::PathBuilder::StoreEnumType store = _pb.getStore();
    ...
    for (uint q = 0; q < xeigen.number_kpoints; q++) {
      if (store == apl::PathBuilder::CARTESIAN_LATTICE) {
        xeigen.vkpoint[q] = c2f * _qpoints[q];
      } else {
        xeigen.vkpoint[q] = _qpoints[q];
      }
  ...
  void PhononDispersionCalculator::writePHKPOINTS(const string& directory) {
    ...
    const string filename = aurostd::CleanFileName(directory + "/" + DEFAULT_APL_PHKPOINTS_FILE);
...
namespace apl {
  ...
  void PathBuilder::addPoint(const string& l, const xvector<double>& p) {
    ...
    if (p.rows != _pointsVectorDimension) {
      ...
      const string message = "Wrong dimension of the point.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
  ...
  int PathBuilder::getDensity() const {
  ...
  void PathBuilder::setDensity(int n) {
    if (n < 0) {
      ...
      const string message = "The density should be >= 0.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
  ...
  void PathBuilder::buildPath() {
    ...
    if (_mode == SINGLE_POINT_MODE) {
      for (size_t i = 1; i < _points.size(); i++) {
        const xvector<double> startPoint = _points[i - 1];
        _path.push_back(startPoint);
        if (_nPointsPerSubPath != 0) {
          const xvector<double> dPoint = (_points[i] - _points[i - 1]) * (1.0 / (_nPointsPerSubPath));
          for (int j = 1; j <= (int) _nPointsPerSubPath; j++) {
            const xvector<double> p = startPoint + j * dPoint;
            _path.push_back(p);
          }
        }
      }
    ...
    if (_mode == COUPLE_POINT_MODE) {
      ...
      xvector<double> startPoint(3);
      xvector<double> endPoint(3);
      endPoint = _points[0];
      for (size_t i = 0; i < _points.size(); i += 2) {
        ...
        if (_nPointsPerSubPath != 0) {
          const xvector<double> dPoint = (endPoint - startPoint) * (1.0 / (_nPointsPerSubPath));
          for (int j = 1; j <= (int) _nPointsPerSubPath; j++) {
            const xvector<double> p = startPoint + j * dPoint;
            _path.push_back(p);
          }
  ...
  uint PathBuilder::getPointSize() {
    ...
    const string message = "Unknown mode.";
  ...
  vector<aurostd::xvector<double>> PathBuilder::getPath(ModeEnumType mode, const string& userPath) {
    ...
    for (size_t i = 0; i < tokens.size(); i++) {
      ...
      for (; j < _labels.size(); j++) {
        ...
        if (_labels[j].find(tokens[i]) != string::npos) {
          break;
        }
      }
      if (j == _labels.size()) {
        ...
        const string message = "Undefined label of the point.";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
      }
  ...
  void PathBuilder::transform(const aurostd::xmatrix<double>& m) {
    ...
    for (size_t i = 0; i < _points.size(); i++) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " IN: " << _labels[i] << ": " << _points[i] << std::endl;
      }
      _points[i] = m * _points[i];
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " OUT: " << _labels[i] << ": " << _points[i] << std::endl;
      }
    }
  }
  ...
  void PathBuilder::defineCustomPoints(const string& coords, const string& labels, const Supercell& sc, bool CARTESIAN_COORDS) { // CO20180409
    ...
    for (size_t i = 0; i < vcoords.size(); i++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " found point " << vlabels[i] << ": (" << (CARTESIAN_COORDS ? "cartesian" : "fractional") << ") " << coordinate[0] << "," << coordinate[1] << "," << coordinate[2] << std::endl;
      }
    ...
    // const xstructure& scStructure)
  {
    ...
    if (!foundBZ) {
      ...
      const string message = "The Brillouin zone was not found for this lattice.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
  ...
  xKPOINTS PathBuilder::createKPOINTS(const Supercell& sc) {
    ...
    if (_store == CARTESIAN_LATTICE) {
      const xmatrix<double> c2f = inverse(trasp(ReciprocalLattice(sc.getOriginalStructure().lattice)));
      for (size_t i = 0; i < points.size(); i++) {
        points[i] = c2f * points[i];
      }
    } else {
      const xmatrix<double> f2c = trasp(ReciprocalLattice(sc.getInputStructure().lattice));
      const xmatrix<double> c2f_orig = inverse(trasp(ReciprocalLattice(sc.getOriginalStructure().lattice)));
      for (size_t i = 0; i < points.size(); i++) {
        points[i] = c2f_orig * f2c * points[i];
      }
    }
