...
namespace apl {
  ...
  void DOSCalculator::initialize(const xoption& aplopts) {
    ...
    if ((!_projections.empty()) && aplopts.flag("DOS_FRAC")) {
      for (size_t p = 0; p < _projections.size(); p++) {
  ...
  void DOSCalculator::calculateFrequencies() {
    ...
    // CO START
    const string message = "Calculating frequencies for the phonon DOS.";
    ...
    for (size_t i = 0; i < _freqs.size(); i++) {
      for (int j = _freqs[i].lrows; j <= _freqs[i].urows; j++) {
        if ((_freqs[i][j] < 0.00) && (_freqs[i][j] > MIN_FREQ_THRESHOLD)) {
          _freqs[i][j] = 0.00;
        }
      }
    }
    ...
    for (size_t i = 0; i < _freqs.size(); i++) {
      for (int j = _freqs[i].lrows; j <= _freqs[i].urows; j++) {
        if (_freqs[i](j) > _maxFreq) {
          _maxFreq = _freqs[i](j);
        }
        if (_freqs[i](j) < _minFreq) {
          _minFreq = _freqs[i](j);
        }
      }
    }
    _maxFreq += 1.0;
    if (_minFreq < MIN_FREQ_THRESHOLD) {
      _minFreq -= 1.0;
    } else {
      _minFreq = 0.0;
    }
  ...
  void DOSCalculator::smearWithGaussian(vector<double>& dos, vector<double>& idos, double h, double sigma) {
    ...
    const int ng = (int) (6.0 * sigma / h + 1.0);
    const double fact = 1.0 / (sqrt(2.0 * M_PI) * sigma);
    vector<double> gauss;
    vector<double> igauss;
    double gnorm = 0.0;
    for (int ig = -ng; ig <= ng; ig++) {
      const double eg = ig * h;
      const double arg = eg * eg / (sigma * sigma) / 2.0;
    ...
    for (int ie = 0; ie < (int) dos.size(); ie++) {
      const double wt = dos[ie] * h;
      const double wti = idos[ie] * h;
    ...
    //
    dos.clear();
    for (size_t i = 0; i < newdos.size(); i++) {
  ...
  // ///////////////////////////////////////////////////////////////////////////
  void DOSCalculator::calc(int USER_DOS_NPOINTS, double USER_DOS_SMEAR, double fmin, double fmax, bool VERBOSE) {
    string message;
    if (!_pc_set) {
      message = "PhononCalculator pointer not set.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_INIT_);
    ...
    // ME20190624
    if (!_projections.empty()) {
      _projectedDOS.resize(_pc->getInputCellStructure().atoms.size(), vector<vector<double>>(_projections.size(), vector<double>(USER_DOS_NPOINTS)));
    }
  ...
  void DOSCalculator::calcDosRS() {
    for (size_t k = 0; k < _bins.size(); k++) {
      for (size_t i = 0; i < _freqs.size(); i++) {
        for (int j = _freqs[i].lrows; j <= _freqs[i].urows; j++) {
          if ((_freqs[i][j] > (_bins[k] - _halfStepDOS)) && (_freqs[i][j] < (_bins[k] + _halfStepDOS))) {
  ...
  void DOSCalculator::calcDosLT() {
    ...
    if (nproj > 0) {
      ...
      for (int q = 0; q < _qm.getnQPs(); q++) {
        for (uint br = 0; br < _pc->getNumberOfBranches(); br++) {
          const int ibranch = br + _freqs[0].lrows;
          for (uint p = 0; p < nproj; p++) {
            for (uint at = 0; at < natoms; at++) {
              if (aurostd::iszero(proj_norm[p])) {  // zero-vector = atom-projected DOS
                ...
                eig.im = 0.0;
                for (int i = 1; i < 4; i++) {
                  eig += proj_norm[p][i] * _eigen[q][3 * at + i][ibranch];
                }
    ...
    vector<double> f(4);
    const double max_freq = _bins.back() + _halfStepDOS;
    const double min_freq = _bins.front() - _halfStepDOS;
    ...
    for (int itet = 0; itet < _qm.getnIrredTetrahedra(); itet++) {
      const double weightVolumeTetrahedron = _qm.getWeightTetrahedron(itet) * _qm.getVolumePerTetrahedron();
      ...
      for (int ibranch = _freqs[0].lrows; ibranch <= _freqs[0].urows; ibranch++) {
        ...
        const double fmin = f[0];
        const double fmax = f[3];
        if (fmax > max_freq) {
          continue;
        }
        if (fmin < min_freq) {
        ...
        if (kstop < (int) _bins.size()) {
          kstop = _bins.size() - 1;
        }

        const double f21 = f[1] - f[0];
        const double f31 = f[2] - f[0];
        const double f32 = f[2] - f[1];
        const double f41 = f[3] - f[0];
        const double f42 = f[3] - f[1];
        const double f43 = f[3] - f[2];
        const double cc12 = 3.0 * weightVolumeTetrahedron / (f21 * f31 * f41);
        const double cc23 = weightVolumeTetrahedron / (f31 * f41);
        const double cc23a = 3.0 * f21 * cc23;
        const double cc23b = 6.0 * cc23;
        const double cc23c = -3.0 * cc23 * (f31 + f42) / (f32 * f42);
        const double cc34 = 3.0 * weightVolumeTetrahedron / (f41 * f42 * f43);

        double fbin;
        double dos;
        double part;
        const int br = ibranch - _freqs[0].lrows;
  ...
  void DOSCalculator::writePDOS(const string& directory) {
    ...
    const string filename = directory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_PDOS_FILE;
    const double factorTHz2Raw = _pc->getFrequencyConversionFactor(apl::THZ, apl::RAW);
    const double factorRaw2rcm = _pc->getFrequencyConversionFactor(apl::RAW, apl::RECIPROCAL_CM);
    ...
    for (size_t k = 0; k < _dos.size(); k++) {
      outfile << setw(15) << _bins[k] << " " << setw(15) << _bins[k] * factorTHz2Raw * factorRaw2rcm << " " << setw(15) << _bins[k] * factorTHz2Raw * factorRaw2meV << " " << setw(15) << _dos[k] << std::endl;
    }
  ...
  void DOSCalculator::writePHDOSCAR(const string& directory) {
    ...
    const string filename = aurostd::CleanFileName(directory + "/" + DEFAULT_APL_PHDOSCAR_FILE);
    message = "Writing phonon density of states into file " + filename + ".";
    pflow::logger(__AFLOW_FILE__, "APL", message, _pc->getDirectory(), *_pc->getOFStream(), *_pc->getOSS());
    stringstream doscar;
    const xDOSCAR xdos = createDOSCAR();
    doscar << xdos;
  ...
  void DOSCalculator::writePDOS(string path, string ex)  //[PN]
  {
    ...
    const string filename = DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_PDOS_FILE; // ME20181226
    ...
    // CO START
    const string file = path + "/" + filename + "." + ex; // ME20181226
