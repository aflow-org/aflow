...
namespace apl {
  ...
  void QMesh::initialize(const vector<int>& vgrid, const xstructure& xs, ofstream& mf, bool include_inversions, bool gamma_centered, ostream& oss) {
    xStream::initialize(mf, oss);
    initialize(vgrid, xs, include_inversions, gamma_centered);
  }

  void QMesh::initialize(const vector<int>& vgrid, const xstructure& xs, bool include_inversions, bool gamma_centered) {
    initialize(aurostd::vector2xvector(vgrid), xs, include_inversions, gamma_centered);
  }

  void QMesh::initialize(const xvector<int>& grid, const xstructure& xs, ofstream& mf, bool include_inversions, bool gamma_centered, ostream& oss) {
    xStream::initialize(mf, oss);
    initialize(grid, xs, include_inversions, gamma_centered);
  }

  void QMesh::initialize(const xvector<int>& grid, const xstructure& xs, bool include_inversions, bool gamma_centered) {
    const string tmp_dir = _directory;  // Do not delete the directory
    clear();
    _directory = tmp_dir;
    setGrid(grid);
  ...
  void QMesh::setupReciprocalCell(xstructure xs, bool include_inversions) {
    ...
    if ((include_inversions && !xs.pgroupk_Patterson_calculated) || (!include_inversions && !xs.pgroupk_xtal_calculated)) {
      const string message = "Calculation of the crystallographic point group of the reciprocal cell unsuccessful.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }

    if (include_inversions) {
      _recCell.pgroup = xs.pgroupk_Patterson;
    } else {
  ...
  void QMesh::makeIrreducible() {
    if (_reduced) {
      return;  // ME20190701 - don't reduce if it's already reduced
    }
    stringstream message;
    ...
    _weights.clear();
    _nIQPs = 0;
    const int nsym = (int) _recCell.pgroup.size();
...
namespace apl {
  ...
  int QMesh::getQPointIndex(xvector<double> fpos) const {
    ...
    const int p = (int) aurostd::nint((fpos[1] * 2 * _qptGrid[1] + _qptGrid[1] + 1) / 2);
    const int r = (int) aurostd::nint((fpos[2] * 2 * _qptGrid[2] + _qptGrid[2] + 1) / 2);
    const int s = (int) aurostd::nint((fpos[3] * 2 * _qptGrid[3] + _qptGrid[3] + 1) / 2);
  ...
  const vector<int>& QMesh::getLittleGroup(int iq) const {
    if (iq < _nIQPs) {
      ...
      const string message = "Little groups are only calculated for irreducible q-points.";
...
namespace apl {
  ...
  void QMesh::writeQpoints(string filename, bool cartesian) {
    ...
    if (!aurostd::FileExist(filename)) {
      const string message = "Could not write q-points to file.";
  ...
  void QMesh::writeIrredQpoints(string filename, bool cartesian) {
    ...
    aurostd::stringstream2file(output, filename);
    if (!aurostd::FileExist(filename)) {
      const string message = "Could not write irreducible q-points to file.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_ERROR_);
    }
  }
...
namespace apl {
  ...
  void QMesh::findMostCompactTetrahedra(vector<vector<xvector<int>>>& tetrahedra) const {
    int lxx = 0;
    int lyy = 0;
    ...
    for (int lx = 0; lx <= 1; lx++) {
      for (int ly = 0; ly <= 1; ly++) {
        ...
        for (int i = 0; i < 6; i++) {
          ...
          for (int j = 0; j < 4; j++) {
            for (int k = 1; k < 4; k++) {
              tet[j][k] = (double) tetrahedra[i][j][k];
            }
            if (lx == 1) {
              tet[j][1] = 1 - tet[j][1];
            }
    ...
    for (int i = 0; i < 6; i++) {
      for (int j = 0; j < 4; j++) {
        if (lxx == 1) {
          tetrahedra[i][j][1] = 1 - tetrahedra[i][j][1];
        }
        if (lyy == 1) {
          tetrahedra[i][j][2] = 1 - tetrahedra[i][j][2];
        }
      }
    }
  }
  ...
  void QMesh::makeIrreducibleTetrahedra() {
    ...
    if (_nQPs != _nIQPs) {
      ...
      for (int t = 0; t < _nTetra; t++) {
        ...
        for (int i = 0; i < 4; i++) {
          compare[i] = getIrredQPointIndex(compare[i]);
        }
        std::sort(compare.begin(), compare.end());
        for (it = 0; it < _nIrredTetra; it++) {
          for (m = 0; m < 4; m++) {
            if (compare[m] != irred[it][m]) {
              break;
            }
          }
          if (m == 4) {
