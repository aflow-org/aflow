...
namespace apl {
  ...
  void TCONDCalculator::initialize(const aurostd::xoption& opts) {
    ...
    // Set up temperatures
    const double tstart = aurostd::string2utype<double>(opts.getattachedscheme("TSTART"));
    const double tend = aurostd::string2utype<double>(opts.getattachedscheme("TEND"));
    const double tstep = aurostd::string2utype<double>(opts.getattachedscheme("TSTEP"));
    ...
    if (calc_boundary) {
      boundary_grain_size = aurostd::string2utype<double>(opts.getattachedscheme("NANO_SIZE"));
    }
...
namespace apl {
  ...
  void TCONDCalculator::calculateThermalConductivity() {
    if (!_initialized) {
      const string message = "Not initialized.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_INIT_);
    }
    ...
    if (!calc_rta_only && !_qm->littleGroupsCalculated()) {
      _qm->calculateLittleGroups();
    }
  ...
  void TCONDCalculator::writeOutputFiles(const string& directory) {
    ...
    if (calc_boundary) {
      const string filename = aurostd::CleanFileName(directory + "/" + DEFAULT_AAPL_FILE_PREFIX + DEFAULT_AAPL_BOUNDARY_FILE);
...
namespace apl {
  ...
  void TCONDCalculator::calculateGrueneisenParameters() {
    ...
    // Grueneisen parameters
    const string message = "Calculating Grueneisen parameters.";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _pc->getDirectory(), *_pc->getOFStream(), *_pc->getOSS());
  ...
  vector<vector<double>> TCONDCalculator::calculateModeGrueneisen(const vector<vector<vector<xcomplex<double>>>>& phases) {
    ...
    const uint nclusters = clusters.size();
    vector<double> invmasses(nclusters);
    for (uint c = 0; c < nclusters; c++) {
      double mass = 1.0;
      for (int i = 0; i < 2; i++) {
        mass *= scell.getAtomMass(clusters[c][i]);
      }
    ...
    const uint ncart = cart_indices.size();

    // Prepare precomputation of eigenvalue products
    const int natoms = (int) _pc->getInputCellStructure().atoms.size();
    const vector<int> atpowers(2, 1);
    vector<vector<int>> at_eigen;
    aurostd::xcombos at_combos(natoms, 2, 'E', true);
    while (at_combos.increment()) {
      at_eigen.push_back(at_combos.getCombo());
    }
    const uint nateigen = at_eigen.size();
    ...
    uint c = 0;
    uint crt = 0;
    xcomplex<double> prefactor;
    xcomplex<double> eigen;
    xcomplex<double> g_mode;
  ...
  double TCONDCalculator::calculateAverageGrueneisen(double T, const vector<vector<double>>& grueneisen_mode) {
    vector<vector<double>> occ = getOccupationNumbers(T);
    int iq = 0;
    double c;
    double c_tot = 0;
    double g_tot = 0;
    const double prefactor = 1E24 * std::pow(PLANCKSCONSTANT_hbar_THz, 2) / (KBOLTZ * std::pow(T, 2));
...
namespace apl {
  ...
  void TCONDCalculator::calculateTransitionProbabilities() {
    ...
    std::function<void(int, vector<vector<vector<vector<double>>>>&, const vector<vector<vector<xcomplex<double>>>>&)> fn = std::bind(&TCONDCalculator::calculateTransitionProbabilitiesPhonon, this, _1, _2, _3);
    ...
    if (calc_isotope) {
      ...
      const uint natoms = pcell.atoms.size();
      uint at = 0;
      for (at = 0; at < natoms; at++) {
        if (GetPearsonCoefficient(pcell.atoms[at].atomic_number) > 0) {
          break;
        }
      }
  ...
  vector<vector<vector<xcomplex<double>>>> TCONDCalculator::calculatePhases(bool conjugate) {
    ...
    const uint niatoms = pcell.atoms.size();
    ...
    for (uint iat = 0; iat < niatoms; iat++) {
      ...
      for (uint at = 0; at < natoms; at++) {
        ...
        for (int q = 0; q < nQPs; q++) {
          if (conjugate) {
            phases[iat][at][q] = exp(-iONE * scalar_product(qpts[q], min_vec));
          } else {
            phases[iat][at][q] = exp(iONE * scalar_product(qpts[q], min_vec));
          }
        }
  ...
  void TCONDCalculator::calculateTransitionProbabilitiesPhonon(int i, vector<vector<vector<vector<double>>>>& phase_space, const vector<vector<vector<xcomplex<double>>>>& phases) {
    // Prepare and precompute
    const Supercell& scell = _pc->getSupercell();
    const vector<vector<int>>& clusters = _pc->getClusters(3);
    const uint nclusters = clusters.size();
    ...
    for (uint c = 0; c < nclusters; c++) {
      double mass = 1.0;
      for (int o = 0; o < 3; o++) {
        mass *= scell.getAtomMass(clusters[c][o]);
      }
      invmasses[c] = 1 / sqrt(mass);
    }
    ...
    while (branch_combos.increment()) {
      branches.push_back(branch_combos.getCombo());
    }
    const uint nbr = branches.size();
    ...
    vector<vector<xcomplex<double>>> eigenprods(nateigen, vector<xcomplex<double>>(ncart));
    for (int j = 0; j < 2; j++) {
      atpowers[j] = aurostd::powint(natoms, 2 - j);
    }

    // Precompute the indices of -q to use for inversion symmetry
    vector<int> q_minus(nQPs);
    for (int q = 0; q < nQPs; q++) {
      q_minus[q] = _qm->getQPointIndex(-_qm->getQPoint(q).fpos);
    }

    // Initialize variables
    xcomplex<double> matrix;
    xcomplex<double> prefactor;
    xcomplex<double> eigen;
    vector<vector<double>> weights(3, vector<double>(nQPs));
    vector<vector<double>> frequencies(3, vector<double>(nQPs));
    vector<int> qpts(3);
    vector<int> proc(2);
    ...
    for (br = 0; br < nbr; br++) {
      ...
      for (j = 0; j < 3; j++) {
        getWeightsLT(freq_ref, frequencies[j], weights[j]);
      }
      ...
      for (q = 0; q < nQPs; q++) {
        ...
        if (calc) {
          ...
          for (j = 0; j < 3; j++) {
            if (freq[qpts[j]][branches[br][j]] < _FLOAT_TOL_) {
              break;
            }
          }
          calc = (j == 3);
        ...
        if (calc) {
          ...
          for (c = 0; c < nclusters; c++) {
            ...
            prefactor.im = 0.0;
            for (j = 1; j < 3; j++) {
              prefactor *= phases[iat][clusters[c][j]][qpts[j]];
            }
            e = 0;
            for (j = 0; j < 3; j++) {
              e += scell.sc2pcMap(clusters[c][j]) * atpowers[j];
            }
            for (crt = 0; crt < ncart; crt++) {
              // Perform multiplication explicitly in place instead of using xcomplex.
              // This is three times faster because constructors and destructors are not called.
          ...
          if (prod > _ZERO_TOL_) {
            for (j = 0; j < 3; j++) {
              prod /= freq[qpts[j]][branches[br][j]];
            }
            for (j = 0; j < 3; j++) {
  ...
  void TCONDCalculator::calculateTransitionProbabilitiesIsotope(int i) {
    ...
    uint at = 0;
    for (at = 0; at < natoms; at++) {
      pearson[at] = GetPearsonCoefficient(pcell.atoms[at].atomic_number);
    }

    vector<double> frequencies(nQPs);
  ...
  void TCONDCalculator::getWeightsLT(double freq_ref, const vector<double>& frequencies, vector<double>& weights) {
    ...
    const double vol = _qm->getVolumePerTetrahedron();
    const int ntet = _qm->getnTetrahedra();
    ...
    for (i = 0; i < ntet; i++) {
      ...
      if (aurostd::isequal(f1, f4)) {
        continue;
      }

      const int q1 = index[index_sort[0]];
      const int q2 = index[index_sort[1]];
      const int q3 = index[index_sort[2]];
      const int q4 = index[index_sort[3]];
      ...
      if (f3 <= freq_ref && freq_ref < f4) {
        ...
        g = (f2 - f1 + 2.0 * (freq_ref - f2) - (f4 + f3 - f2 - f1) * std::pow(freq_ref - f2, 2) / ((f3 - f2) * (f4 - f2))) / ((f3 - f1) * (f4 - f1));
...
namespace apl {
  ...
  void TCONDCalculator::getProcess(const vector<int>& process, vector<int>& qpts, vector<int>& branches, int& sign) {
    // Signs and q-points
    // Signs are stored in the q-index instead of using an extra integer.
    const xvector<double> qsum = _qm->getQPoint(qpts[0]).fpos;
    if (process[0] < 0) {
      sign = 1;
      qpts[1] = -process[0] - 1;
  ...
  xmatrix<double> TCONDCalculator::calculateThermalConductivityTensor(double T, vector<vector<double>>& rates_total, vector<vector<double>>& rates_anharm) {
    stringstream message;
    message << "Calculating thermal conductivity for " << T << " K.";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _pc->getDirectory(), *_pc->getOFStream(), *_pc->getOSS());
    // Bose-Einstein distribution
    const vector<vector<double>> occ = getOccupationNumbers(T);
    ...
    if (!calc_rta_only) {
      xmatrix<double> tcond_prev(3, 3);
      const xmatrix<double> diff(3, 3);
      int num_iter = 1;
      double norm = 0.0;
  ...
  //  process.
  double TCONDCalculator::getOccupationTerm(const vector<vector<double>>& occ, int sign, const vector<int>& qpts, const vector<int>& branches) {
    if (sign) {  // -
  ...
  vector<vector<double>> TCONDCalculator::calculateTotalRates(const vector<vector<double>>& occ, vector<vector<double>>& rates_anharm) {
...
#ifdef AFLOW_MULTITHREADS_ENABLE
    xthread::xThread xt(_pc->getNCPUs());
    std::function<void(int, const vector<vector<double>>&, vector<vector<double>>&)> fn = std::bind(&TCONDCalculator::calcAnharmRates, this, _1, _2, _3);
  ...
  void TCONDCalculator::calcAnharmRates(int i, const vector<vector<double>>& occ, vector<vector<double>>& rates) {
    ...
    int sign = -1;
    qpts[0] = _qm->getIbzqpts()[i];
    for (size_t p = 0, nprocs = processes[i].size(); p < nprocs; p++) {
  ...
  xmatrix<double> TCONDCalculator::calcTCOND(double T, const vector<vector<double>>& occ, const vector<vector<xvector<double>>>& mfd) {
    xmatrix<double> tcond(3, 3);
    const bool cumulative = calc_cumulative;
    const double prefactor = 1E24 * std::pow(PLANCKSCONSTANT_hbar_THz, 2) / (KBOLTZ * std::pow(T, 2) * nQPs * Volume(_pc->getInputCellStructure()));
    for (int q = 0; q < nQPs; q++) {
      for (int br = 0; br < nBranches; br++) {
        bool include = true;
        if (freq[q][br] < _FLOAT_TOL_) {
          ...
          const double mfpath = scalar_product(mfd[q][br], gvel[q][br]) / aurostd::modulus(gvel[q][br]);
          include = !(mfpath > boundary_grain_size);
        }
        if (include) {
          const double x = occ[q][br] * (occ[q][br] + 1) * freq[q][br];
          for (int i = 1; i < 4; i++) {
            for (int j = 1; j < 4; j++) {
  ...
  void TCONDCalculator::getMeanFreeDispFull(const vector<vector<double>>& rates, const vector<vector<double>>& occ, vector<vector<xvector<double>>>& mfd) {
    vector<vector<xvector<double>>> delta(nIQPs, vector<xvector<double>>(nBranches, xvector<double>(3)));
#ifdef AFLOW_MULTITHREADS_ENABLE
    xthread::xThread xt(_pc->getNCPUs());
    std::function<void(int, const vector<vector<double>>&, const vector<vector<xvector<double>>>&, vector<vector<xvector<double>>>&)> fn = std::bind(&TCONDCalculator::calculateDelta, this, _1, _2, _3, _4);
  ...
  void TCONDCalculator::calculateDelta(int i, const vector<vector<double>>& occ, const vector<vector<xvector<double>>>& mfd, vector<vector<xvector<double>>>& delta) {
    ...
    for (size_t p = 0, nprocs = processes[i].size(); p < nprocs; p++) {
      ...
      if (processes[i][p][0] < 0) {
        correction += mfd[qpts[1]][branches[1]];
      } else {
        correction -= mfd[qpts[1]][branches[1]];
      }
    ...
    if (calc_isotope) {
      ...
      int br1 = 0;
      int br2 = 0;
      for (size_t p = 0, nprocs = processes_iso[i].size(); p < nprocs; p++) {
    ...
    const uint nsym = little_group.size();
  ...
  void TCONDCalculator::correctMFD(const vector<vector<double>>& rates, const vector<vector<xvector<double>>>& delta, vector<vector<xvector<double>>>& mfd) {
    const vector<_sym_op>& pgroup = _qm->getReciprocalCell().pgroup;
    const xvector<double> xvec(3);
    int iq = -1;
    for (int q = 0; q < nQPs; q++) {
      ...
      for (int br = 0; br < nBranches; br++) {
        if (rates[iq][br] > 0.0) {
          mfd[q][br] = (freq[q][br] * gvel[q][br] + Uc * delta[iq][br]) / rates[iq][br];
        } else {
          mfd[q][br] = xvec;
        }
      }
...
namespace apl {
  ...
  void TCONDCalculator::writeTempIndepOutput(const string& filename, string keyword, const string& unit, const vector<vector<double>>& data) {
    if (data.empty()) {
      return;  // Nothing to write
    }
    stringstream output;
    output << AFLOWIN_SEPARATION_LINE << std::endl;
    aurostd::StringSubstInPlace(keyword, ' ', '_');
    const string key = "[AAPL_" + aurostd::toupper(keyword) + "]";
    ...
    output << std::setw(10) << "# Q-point" << std::setw(20) << " " << keyword << " (" << unit << ")" << std::endl;
  ...
  void TCONDCalculator::writeTempDepOutput(const string& filename, string keyword, const string& unit, const vector<double>& temps, const vector<vector<vector<double>>>& data) {
    if (data.empty()) {
      return; // Nothing to write
    }
    stringstream output;
  ...
  //  output file writers.
  void TCONDCalculator::writeDataBlock(stringstream& output, const vector<vector<double>>& data) {
    for (size_t q = 0; q < data.size(); q++) {
      output << std::setiosflags(std::ios::fixed | std::ios::right);
      output << std::setw(10) << q;
      for (size_t br = 0; br < data[q].size(); br++) {
  ...
  void TCONDCalculator::writePhaseSpace(const string& filename) {
    if (phase_space.empty()) {
      return;  // Nothing to write
    }
    // Calculate totals
    vector<double> ps_procs(4);
    vector<double> ps_nu(2);
  ...
  void TCONDCalculator::writeGrueneisen(const string& filename) {
    ...
    output << std::setw(8) << "# T (K)" << std::setw(23) << "Grueneisen parameter" << std::endl;
    for (size_t t = 0; t < grueneisen_avg.size(); t++) {
  ...
  void TCONDCalculator::writeThermalConductivity(const string& filename) {
    ...
    output << std::setw(8) << "# T (K)" << std::setw(75) << " "
    ...
    aurostd::stringstream2file(output, filename);
    if (!aurostd::FileExist(filename)) {
      const string message = "Could not write thermal conductivities to file.";
      throw xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_ERROR_);
    }
  }
