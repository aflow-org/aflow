...
namespace apl {
  ...
  void ClusterSet::initialize(const Supercell& supercell, int _order, int cut_shell, double cut_rad) {
    ...
    if (cut_shell > 0) {
      const double max_rad = getMaxRad(scell, cut_shell);
      if (max_rad > cut_rad) {
...
namespace apl {
  ...
  vector<vector<int>> ClusterSet::getSymmetryMap() {
    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_AAPL_CLUSTERS_);
    string message;
    ...
    if (SYM::PointGroupsIdentical(scell.pgroup_xtal, pcell.pgroup_xtal, scell.sym_eps, false)) {
      ...
      const bool skewed = SYM::isLatticeSkewed(scell.lattice, pcell.dist_nn_min, tol);
      const uint fgsize = pcell.fgroup.size();
      const uint natoms = scell.atoms.size();
      ...
      for (uint fg = 0; fg < fgsize; fg++) {
        ...
        for (uint atsc = 0; atsc < natoms; atsc++) {
          ...
          for (uint at_map = 0; at_map < natoms && !mapped; at_map++) {
            if (SYM::FPOSMatch(fpos_scaled, scell.atoms[at_map].fpos, scell.lattice, scell.f2c, skewed, scell.sym_eps)) { // DX20190619 - lattice and f2c as input
  ...
  double ClusterSet::getMaxRad(const xstructure& cell, int cut_shell) {
    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_AAPL_CLUSTERS_) || _DEBUG_AAPL_CLUSTERS_;
    if (cut_shell > 0) {
      int countshell = 0;
      double shell_rad = 0.0;
      double max_rad = 0.0;
      const uint natoms = cell.atoms.size();
      xvector<double> distances(natoms - 1, 0);
      for (size_t i = 0; i < cell.iatoms.size(); i++) {
        countshell = 0;
        const int at1 = cell.iatoms[i][0];
        for (uint at2 = 0; at2 < natoms; at2++) {
          distances[at2] = aurostd::modulus(SYM::minimizeDistanceCartesianMethod(cell.atoms[at1].cpos, cell.atoms[at2].cpos,
  ...
  void ClusterSet::buildShells() {
    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_AAPL_CLUSTERS_);
    const string message = "Building coordination shells.";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, directory, *p_FileMESSAGE, *p_oss);
    int at1sc = 0;
    vector<int> shell;
    coordination_shells.clear();
    const uint natoms_pcell = pcell.atoms.size();
    const uint natoms_scell = scell.atoms.size();
    for (uint at1 = 0; at1 < natoms_pcell; at1++) {
      shell.clear();
      at1sc = pc2scMap[at1];
      shell.push_back(at1sc);
      for (uint at2 = 0; at2 < natoms_scell; at2++) {
        const double cart_dist = aurostd::modulus(SYM::minimizeDistanceCartesianMethod(scell.atoms[at1sc].cpos, scell.atoms[at2].cpos,
      ...
      if (LDEBUG) {
        const uint shellsize = shell.size();
...
namespace apl {
  ...
  void ClusterSet::build() {
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0; i < ineq_clusters.size(); i++) {
        std::cerr << "Inequivalent cluster " << i << std::endl;
        for (size_t j = 0; j < ineq_clusters[i].size(); j++) {
          for (size_t k = 0; k < getCluster(ineq_clusters[i][j]).atoms.size(); k++) {
  ...
  vector<_cluster> ClusterSet::buildClusters() {
    ...
    vector<int> atoms_in_cluster(order);
    vector<int> clst;
    const bool repeat = true;
    for (size_t s = 0; s < coordination_shells.size(); s++) {
      ...
      while (clst_combos.increment()) {
        ...
        for (int o = 1; o < order; o++) {
          const int at = coordination_shells[s][clst[o - 1] + 1];
          for (int d = 1; d < o; d++) {
            const int atc = atoms_in_cluster[d];
            const double mdist = aurostd::modulus(SYM::minimizeDistanceCartesianMethod(scell.atoms[at].cpos, scell.atoms[atc].cpos,
                                                                                       scell.lattice)); // DX20190613
            if (mdist > cutoff) {
              append = false;
              d = o;
        ...
        if (append) {
          ...
          if (LDEBUG) {
            ...
            for (size_t c = 0; c < cluster.atoms.size(); c++) {
  ...
  // shells.
  void ClusterSet::getInequivalentClusters(vector<_cluster>& clusters, vector<vector<int>>& ineq_clst) {
    const string message = "Determining inequivalent clusters.";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, directory, *p_FileMESSAGE, *p_oss);
    int equivalent_clst = -1;
    vector<int> unique_atoms;
    vector<vector<int>> compositions;
    const uint nspecies = scell.species.size();
    ineq_clst.clear();
    for (size_t c = 0; c < clusters.size(); c++) {
      // Determine if cluster is equivalent to an inequivalent cluster
      equivalent_clst = equivalenceCluster(clusters[c].atoms, unique_atoms, compositions, ineq_clst);
      if (equivalent_clst > -1) {
        ...
        const int unique = getNumUniqueAtoms(clusters[c].atoms);
        unique_atoms.push_back(unique);
        if (nspecies > 1) {
          const vector<int> comp = getComposition(clusters[c].atoms);
  ...
  int ClusterSet::getNumUniqueAtoms(const vector<int>& atoms) const {
    int unique = 1;
  ...
  bool ClusterSet::sameComposition(const vector<int>& comp1, const vector<int>& comp2) const {
    for (size_t s = 0; s < scell.species.size(); s++) {
      if (comp1[s] != comp2[s]) {
        return false;
      }
    }
  ...
  int ClusterSet::equivalenceCluster(const vector<int>& atoms, const vector<int>& unique_atoms, const vector<vector<int>>& compositions, const vector<vector<int>>& ineq_clst) {
    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_AAPL_CLUSTERS_);
    if (LDEBUG) {
    ...
    const uint nspecies = scell.species.size();
    const int unique = getNumUniqueAtoms(atoms);
    vector<int> composition;
    if (nspecies > 1) {
      composition = getComposition(atoms);
    }
    vector<int> cluster_transl(order);
    vector<int> cluster_transformed(order);
    if (!ineq_clst.empty()) {
      int perm_index = -1;
      for (size_t fg = 0; fg < pcell.fgroup.size(); fg++) {
        for (int at = 0; at < order; at++) {
          cluster_transformed[at] = symmetry_map[fg][atoms[at]];
        }
        for (int at = 0; at < order; at++) {
          cluster_transl = translateToPcell(cluster_transformed, at);
          for (size_t ineq = 0; ineq < ineq_clst.size(); ineq++) {
  ...
  vector<int> ClusterSet::translateToPcell(const vector<int>& atoms, int at) {
    xvector<double> translation_vector;
    xvector<double> fpos_transl;
    vector<int> atoms_translated(order);
    const bool update = false;
    const int atompc = sc2pcMap[atoms[at]];
    const int atomsc = pc2scMap[atompc];
    ...
    for (size_t i = 0; i < atoms.size(); i++) {
      if ((int) i == at) {
        ...
        for (size_t j = 0; j < scell.atoms.size(); j++) {
          // No need to compare when atoms are of different type
          if (scell.atoms[j].type == scell.atoms[atoms[i]].type) {
            if (SYM::FPOSMatch(fpos_transl, scell.atoms[j].fpos, scell.lattice, scell.f2c, update, scell.sym_eps)) { // DX20190619 - lattice and f2c as input
  ...
  int ClusterSet::comparePermutations(const vector<int>& cluster_ineq, const vector<int>& cluster_compare) {
    int index = 0;
    int iper = 0;
    uint depth = 0;
    uint jump = 0;
    const uint n = cluster_ineq.size();
  ...
  bool ClusterSet::atomsMatch(const vector<int>& permutation, const vector<int>& cluster_perm, const vector<int>& cluster_orig, const int& depth) {
...
namespace apl {
  ...
  void ClusterSet::buildDistortions() {
    const string message = "Getting inequivalent distortions.";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, directory, *p_FileMESSAGE, *p_oss);
    ...
    for (size_t i = 0; i < ineq_distortions.size(); i++) {
  ...
  vector<xvector<double>> ClusterSet::getCartesianDistortionVectors() {
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0; i < dist_vecs.size(); i++) {
        for (int j = 1; j < 4; j++) {
  ...
  vector<_ineq_distortions> ClusterSet::initializeIneqDists() {
    ...
    for (size_t ineq = 0; ineq < ineq_clusters.size(); ineq++) {
      const int same = sameDistortions(getCluster(ineq_clusters[ineq][0]), ineq_dists);
      if (same == -1) {
  ...
  int ClusterSet::sameDistortions(const _cluster& clst, const vector<_ineq_distortions>& ineq_dists) const {
    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_AAPL_CLUSTERS_);
    int same = -1;
    for (size_t ineq = 0; ineq < ineq_dists.size(); ineq++) {
      same = (int) ineq;
      for (int at = 0; at < order - 1; at++) {
  ...
  vector<vector<int>> ClusterSet::getTestDistortions(const vector<int>& clusters) {
    ...
    vector<vector<int>> test_distortions;
    vector<int> distortions(order - 1);
    vector<int> distortions_base(order - 1);
    vector<int> signs;
    for (size_t d = 0; d < dists.size(); d++) {
      int dist = dists[d];
  ...
  void ClusterSet::getInequivalentDistortions(const vector<vector<int>>& dists, _ineq_distortions& ineq_dists) {
    const bool LDEBUG = (false || XHOST.DEBUG || _DEBUG_AAPL_CLUSTERS_);
    for (size_t td = 0; td < dists.size(); td++) {
      if (ineq_dists.distortions.empty()) {
      ...
      if (!allZeroDistortions(ineq_dists.atoms, dists[td])) {
        ...
        for (size_t fg = 0; fg < pcell.fgroup.size(); fg++) {
          if (allAtomsMatch(fg, ineq_dists.atoms)) {
            eq = equivalenceDistortions(pcell.fgroup[fg].Uc, dists[td], ineq_dists.distortions, ineq_dists.atoms);
            if (eq > -1) {
              appendDistortion(ineq_dists, dists[td], eq, fg);
              if (LDEBUG) {
                std::cerr << "ClusterSet::getInequivalentDistortions: Distortion ";
                for (size_t d = 0; d < dists[td].size(); d++) {
                  std::cerr << " " << dists[td][d];
                }
  ...
  void ClusterSet::appendDistortion(_ineq_distortions& ineq_dists, vector<int> dist, const int& eq, const int& fg) {
    vector<int> transformation_map;
    if (eq == -1) {
      // No equivalent distortion found - create a new inequivalent distortion
      vector<vector<int>> init_dist;
  ...
  //  to any).
  int ClusterSet::equivalenceDistortions(const xmatrix<double>& Uc, const vector<int>& dist, const vector<vector<vector<int>>>& ineq_dists, const vector<int>& atoms) {
    const uint distsize = dist.size();
    const uint natoms = atoms.size();
    ...
    for (size_t ineq = 0; ineq < ineq_dists.size(); ineq++) {
      for (uint at = 0; at < natoms; at++) {
        ...
        for (int i = 1; i < 4; i++) {
          if (std::abs(vec_dist(i)) > _ZERO_TOL_) {
            vec_dist(i) /= std::abs(vec_dist(i));
          }
          if (std::abs(vec_comp(i)) > _ZERO_TOL_) {
            vec_comp(i) /= std::abs(vec_comp(i));
          }
        }
  ...
  vector<int> ClusterSet::getTransformationMap(const int& fg, const int& atom) {
    ...
    if (aurostd::iszero(transl, _ZERO_TOL_)) {
      ...
      for (uint at = 0; at < natoms; at++) {
        ...
        const xvector<double> fpos = scell.c2f * (scell.atoms[atom_trans].cpos - transl);
        for (uint at_map = 0; at_map < natoms; at_map++) {
          if (SYM::FPOSMatch(fpos, scell.atoms[at_map].fpos, scell.lattice, scell.f2c, skewed, scell.sym_eps)) { // DX20190619 - lattice and f2c as input
  ...
  vector<_ineq_distortions> ClusterSet::getHigherOrderDistortions() {
    vector<_ineq_distortions> ineq_dists;
    if (order != 4) {
      return ineq_dists;  // Not implemented for higher order and not necessary for lower
    }
    ...
    for (size_t iat = 0; iat < pcell.iatoms.size(); iat++) {
      const int at = pc2scMap[pcell.iatoms[iat][0]];
      const vector<int> atoms(1, at);
      ineq_dists[iat].atoms = atoms;
      int cl = 0;
      for (cl = 0; cl < (int) clusters.size(); cl++) {
        int a = 0;
        for (a = 0; a < order; a++) {
          if (clusters[cl].atoms[a] != at) {
            break;
          }
        }
        if (a == order) {
      ...
      const vector<int> clusters(1, cl);
      ...
      for (size_t td = 0; td < dists.size(); td++) {
        ...
        if (eq == -1) {
          appendDistortion(ineq_dists[iat], dists[td]);
        }
      }
    }
    return ineq_dists;
...
namespace apl {
  ...
  vector<_linearCombinations> ClusterSet::getLinearCombinations() {
    ...
    for (size_t ineq = 0; ineq < ineq_clusters.size(); ineq++) {
      ...
      const vector<vector<int>> symops = getInvariantSymOps(getCluster(ineq_clusters[ineq][0]));
      const vector<vector<double>> coeff_mat = buildCoefficientMatrix(symops);
      if (!coeff_mat.empty()) {
        ...
        for (size_t r = 0; r < rref.size(); r++) {
          while ((std::abs(rref[r][r + shift]) < _ZERO_TOL_) && (r + shift < rref[r].size())) {
          ...
          vector<int> indices;
          for (size_t c = r + shift; c < rref[r].size(); c++) {
            if (std::abs(rref[r][c]) > _ZERO_TOL_) {
              allzero = false;
              if (c != r + shift) {
        ...
        for (size_t i = 0; i < lcomb.indices.size(); i++) {
          for (size_t j = 0; j < lcomb.indices[i].size(); j++) {
            for (size_t ind = 0; ind < lcomb.independent.size(); ind++) {
  ...
  vector<vector<int>> ClusterSet::getInvariantSymOps(const _cluster& ineq_clst) {
    ...
    for (size_t fg = 0; fg < pcell.fgroup.size(); fg++) {
      ...
      int at = 0;
      for (size_t iperm = 0; iperm < permutations.size(); iperm++) {
        ...
        if ((fg > 0) || (iperm > 0)) {
          ...
          for (size_t i = 0; i < cluster_transl.size(); i++) {
  ...
  vector<vector<double>> ClusterSet::buildCoefficientMatrix(const vector<vector<int>>& symops) {
    ...
    for (size_t sop = 0; sop < symops.size(); sop++) {
      fg = symops[sop][0];
      perm = symops[sop][1];
  ...
  vector<vector<double>> ClusterSet::getRREF(vector<vector<double>> mat) {
    ...
    for (uint col = 0; col < ncols; col++) {
      for (uint row = 0; row < nrows; row++) {
        if (std::abs(mat[row][col]) < _ZERO_TOL_) {
          mat[row][col] = 0.0;
        }
      }
      for (uint row = currentrow + 1; row < nrows; row++) {
        if (std::abs(mat[row][col]) - std::abs(mat[currentrow][col]) > _ZERO_TOL_) {
      ...
      if (std::abs(mat[currentrow][col]) > _ZERO_TOL_) {
...
namespace apl {
  ...
  void ClusterSet::writeClusterSetToFile(const string& filename) {
    ...
    if (order == 4) {
      output << writeHigherOrderDistortions();
    }
    output << "</cluster_set>" << std::endl;
  ...
  string ClusterSet::writeParameters() {
    ...
    parameters << tab << tab << R"(<i name="aflow_version" type="string">)" << AFLOW_VERSION << "</i>" << std::endl;
    string time = aflow_get_time_string();
    if (time[time.size() - 1] == '\n') {
      time.erase(time.size() - 1);
    }
    parameters << tab << tab << R"(<i name="date" type="string">)" << time << "</i>" << std::endl;
    parameters << tab << tab << R"(<i name="checksum" file=")" << _AFLOWIN_;
    ...
    // Structure
    parameters << tab << R"(<structure units="Angstrom" cs="fractional">)" << std::endl;
    ...
    for (size_t i = 0; i < pcell.atoms.size(); i++) {
      const int t = pcell.atoms[i].type;
  ...
  string ClusterSet::writeClusters(const vector<_cluster>& clusters) const {
    const string tab = " ";
    stringstream clst;
  ...
  string ClusterSet::writeInequivalentClusters() {
    ...
    for (size_t i = 0; i < ineq_clusters.size(); i++) {
      ...
      clusters << tab << tab << tab << "<varray name=\"clusters\">" << aurostd::joinWDelimiter(ineq_clusters[i], " ") << "</varray>" << std::endl;
  ...
  string ClusterSet::writeLinearCombinations(const _linearCombinations& lcombs) {
    ...
    for (size_t i = 0; i < lcombs.coefficients.size(); i++) {
      lc << basetab << tab << tab << "<v>";
      for (size_t j = 0; j < lcombs.coefficients[i].size(); j++) {
    ...
    for (size_t i = 0; i < lcombs.indep2depMap.size(); i++) {
      lc << basetab << tab << tab << "<v>";
      for (size_t j = 0; j < lcombs.indep2depMap[i].size(); j++) {
  ...
  string ClusterSet::writeIneqDist(const _ineq_distortions& ineq_dist) {
    ...
    idist << basetab << "<atoms>";
    for (size_t at = 0; at < ineq_dist.atoms.size(); at++) {
      idist << " " << ineq_dist.atoms[at];
    }
    idist << " </atoms>" << std::endl;

    idist << basetab << "<clusters>";
    for (size_t c = 0; c < ineq_dist.clusters.size(); c++) {
      idist << " " << ineq_dist.clusters[c];
    }
    idist << " </clusters>" << std::endl;

    idist << basetab << "<distortions>" << std::endl;
    for (size_t d = 0; d < ineq_dist.distortions.size(); d++) {
      idist << basetab << tab << "<varray ID=\"" << d << "\">" << std::endl;
      for (size_t i = 0; i < ineq_dist.distortions[d].size(); i++) {
        idist << basetab << tab << tab << "<v>";
        for (size_t j = 0; j < ineq_dist.distortions[d][i].size(); j++) {
    ...
    idist << basetab << tab << "<varray>" << std::endl;
    for (size_t r = 0; r < ineq_dist.rotations.size(); r++) {
      idist << basetab << tab << tab << "<v>";
      for (size_t i = 0; i < ineq_dist.rotations[r].size(); i++) {
    ...
    for (size_t t = 0; t < ineq_dist.transformation_maps.size(); t++) {
      idist << basetab << tab << "<varray ID=\"" << t << "\">" << std::endl;
      for (size_t i = 0; i < ineq_dist.transformation_maps[t].size(); i++) {
        idist << basetab << tab << tab << "<v>";
        for (size_t j = 0; j < ineq_dist.transformation_maps[t][i].size(); j++) {
  ...
  string ClusterSet::writeHigherOrderDistortions() {
    ...
    for (size_t i = 0; i < higher_order_ineq_distortions.size(); i++) {
      distortions << tab << tab << "<ineq_dist ID=\"" << i << "\">" << std::endl;
  ...
  void ClusterSet::readClusterSetFromFile(const string& filename) {
    ...
    if (!aurostd::CompressFileExist(filename) && !aurostd::FileExist(filename)) {
    ...
    // Start reading
    uint line_count = 0;
    const string line = vlines[line_count++];
    ...
    if (checkCompatibility(line_count, vlines)) {
      ...
      if (order == 4) {
        readHigherOrderDistortions(line_count, vlines);
      }
    } else {
  ...
  bool ClusterSet::checkCompatibility(uint& line_count, const vector<string>& vlines) {
    ...
    if (strtoul(tokens[0].c_str(), nullptr, 16) != aurostd::getFileCheckSum(directory + "/" + _AFLOWIN_, APL_CHECKSUM_ALGO)) {  // ME20190219
      ...
      while (compatible) {
        ...
        if (line.find("order") != string::npos) {
          t = line.find_first_of(">") + 1;
          aurostd::string2tokens(line.substr(t, line.find_last_of("<") - t), tokens, string(" "));
          const int ord = aurostd::string2utype<int>(tokens[0]);
          tokens.clear();
          if (ord != order) {
      ...
      while (compatible) {
        ...
        if (line.find("cutoff") != string::npos) {
          t = line.find_first_of(">") + 1;
          aurostd::string2tokens(line.substr(t, line.find_last_of("<") - t), tokens, string(" "));
          const double cut = aurostd::string2utype<double>(tokens[0]);
          tokens.clear();
          if (std::abs(cut - cutoff) > _ZERO_TOL_) {
      ...
      while (compatible) {
        ...
        if (line.find("varray name=\"positions\"") != string::npos) {
          ...
          const uint nspecies = species.size();
          if (nspecies == pcell.atoms.size()) {
            for (uint sp = 0; sp < nspecies; sp++) {
              const int type = pcell.atoms[sp].type;
              const string spec = pcell.species[type];
              const xvector<double> pos = pcell.atoms[sp].fpos;
              if (species[sp] != spec) {
  ...
  void ClusterSet::readInequivalentClusters(uint& line_count, const vector<string>& vlines) {
    string line;
    string message;
      ...
      if (line.find("ineq_cluster") != string::npos) {
        while (line.find("/ineq_cluster") == string::npos) {
          ...
          if (line.find("varray name=\"clusters\"") != string::npos) {
            ...
            const _linearCombinations lcomb = readLinearCombinations(line_count, vlines);
  ...
  vector<_cluster> ClusterSet::readClusters(uint& line_count, const vector<string>& vlines) {
    string message;
    string line;
  ...
  _linearCombinations ClusterSet::readLinearCombinations(uint& line_count, const vector<string>& vlines) {
    string message;
    string line;
  ...
  void ClusterSet::readInequivalentDistortions(uint& line_count, const vector<string>& vlines) {
    string line;
    string message;
    const uint vsize = vlines.size();
    ...
    while (line.find("/inequivalent_distortions") == string::npos) {
      ...
      if (line.find("ineq_dist") != string::npos) {
        const _ineq_distortions idist = readIneqDist(line_count, vlines);
  ...
  void ClusterSet::readHigherOrderDistortions(uint& line_count, const vector<string>& vlines) {
    string line;
    string message;
    const uint vsize = vlines.size();
