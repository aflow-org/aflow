...
namespace apl {
  ...
  bool ForceConstantCalculator::runVASPCalculations(_xinput& xInput, _aflags& _aflowFlags, _kflags& _kbinFlags, _xflags& _xFlags, string& AflowIn) {
    string message;
    if (!_initialized) {
  ...
  bool ForceConstantCalculator::run() {
    ...
    if (!outfileFoundAnywherePhonons(xInputs)) {
      return false;
    }

    // Read Born effective charges and dielectric tensor
    if (_isPolarMaterial && !calculateBornChargesDielectricTensor(xInputs.back())) {
      return false;
    }

    if (_method == "DM") {  // Direct method - see README_AFLOW_APL.TXT
      if (!calculateForceConstantsDM()) {
        return false;
      }
    } else if (_method == "LR") {  // Linear response - see README_AFLOW_APL.TXT
      if (!readForceConstantsFromVasprun(xInputs[0])) {
        return false;
      }
    } else {
      return false;
    ...
    if (!_supercell->fullBasisCalculatedAGROUP()) {
      _supercell->getFullBasisAGROUP();
    }
  ...
  void ForceConstantCalculator::symmetrizeForceConstantMatrices() {
    const bool LDEBUG = (false || _DEBUG_APL_HARM_IFCS_ || XHOST.DEBUG);
    string message;
    // Test of stupidity...
    ...
    for (uint i = 0; i < _supercell->getNumberOfAtoms(); i++) {
      ...
      for (uint j = 0; j < _supercell->getNumberOfAtoms(); j++) {
        ...
        for (size_t symOpID = 0; symOpID < agroup.size(); symOpID++) {
          ...
          try {
            ...
            const int l = _supercell->atomGoesTo(symOp, j, i, true); // JAHNATEK ORIGINAL //CO20190218
...
namespace apl {
  ...
  bool ForceConstantCalculator::runVASPCalculationsBE(_xinput& xInput, _aflags& _aflowFlags, _kflags& _kbinFlags, _xflags& _xFlags, string& _AflowIn, uint ncalcs) {
    bool stagebreak = false;
    ...
    if (xInput.getXStr().title.empty()) {
      xInput.getXStr().buildGenericTitle(true, false);
    }
    ...
    if (xInput.AFLOW_MODE_VASP) {
      ...
      const string runname = _AFLOW_APL_BORN_EPSILON_DIRECTORY_NAME_;
      xInput.setDirectory(_directory + "/" + runname);
      if (!filesExistPhonons(xInput)) {
        const string message = "Creating " + xInput.getDirectory();
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
  ...
  bool ForceConstantCalculator::calculateBornChargesDielectricTensor(const _xinput& xinpBE) {
    ...
    if (xinpBE.AFLOW_MODE_VASP) {
      const string directory = xinpBE.xvasp.Directory;
      string infilename = directory + "/OUTCAR.static";

      if (!aurostd::CompressFileExist(infilename, infilename)) {
    ...
    if (xinpBE.AFLOW_MODE_VASP) {
      readBornEffectiveChargesFromOUTCAR(xinpBE);
    } else if (xinpBE.AFLOW_MODE_AIMS) {
      readBornEffectiveChargesFromAIMSOUT();
    }
    ...
    if (xinpBE.AFLOW_MODE_VASP) {
      readDielectricTensorFromOUTCAR(xinpBE);
    }
    if (xinpBE.AFLOW_MODE_AIMS) {
      readDielectricTensorFromAIMSOUT();
    }
  ...
  //////////////////////////////////////////////////////////////////////////////
  void ForceConstantCalculator::readBornEffectiveChargesFromAIMSOUT() {
    const string message = "This functionality has not been implemented yet.";
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
  }

  void ForceConstantCalculator::readBornEffectiveChargesFromOUTCAR(const _xinput& xinp) {  // ME20190113
    string message;
    const string directory = xinp.xvasp.Directory;  // ME20190113

    // Open our file
    string infilename = directory + "/OUTCAR.static";
    vector<string> vlines;
    if (aurostd::CompressFileExist(infilename, infilename)) {
      aurostd::compressfile2vectorstring(infilename, vlines);
    }
    if (vlines.empty()) {
      message = "Cannot open OUTCAR static";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_ERROR_);
    }

    // Find
    size_t line_count = 0;
    for (size_t iline = 0; iline < vlines.size() && !line_count; iline++) {
      if (vlines[iline].find("BORN EFFECTIVE CHARGES (") != string::npos) {
        line_count = iline + 2;
      }
    }
    if (!line_count) {
      message = "No information on Born effective charges in OUTCAR";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_ERROR_);
    }
    ...
    for (size_t i = 0; i < _supercell->getInputStructure().atoms.size(); i++) {
      ...
      for (int j = 1; j <= 3; j++) {
        aurostd::string2tokens(vlines[line_count++], tokens);
  ...
  //////////////////////////////////////////////////////////////////////////////
  void ForceConstantCalculator::symmetrizeBornEffectiveChargeTensors() {
    // CO START
    //  Test of stupidity...
    stringstream message;
    if (_supercell->getEPS() == AUROSTD_NAN) {
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
    for (size_t i = 0; i < _bornEffectiveChargeTensor.size(); i++) {
      const int id = i;
      message << "Atom [" << aurostd::PaddedNumString(id, 3) << "] (" << std::setw(2) << _supercell->getInputStructure().atoms[id].cleanname << ") Born effective charge = ";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
      for (int a = 1; a <= 3; a++) {
        for (int b = 1; b <= 3; b++) {
    ...
    for (uint i = 0; i < _supercell->getNumberOfUniqueAtoms(); i++) {
      const int basedUniqueAtomID = _supercell->getUniqueAtomID(i);
    ...
    for (uint i = 0; i < _supercell->getNumberOfAtoms(); i++) {
      ...
      xmatrix<double> sum(3, 3);
      for (size_t symOpID = 0; symOpID < agroup[i].size(); symOpID++) {
    ...
    // Show charges
    for (uint i = 0; i < _supercell->getNumberOfUniqueAtoms(); i++) {
      const int id = _supercell->getUniqueAtomID(i);
      message << "Atom [" << aurostd::PaddedNumString(id, 3) << "] (" << std::setw(2) << _supercell->getSupercellStructure().atoms[id].cleanname << ") Born effective charge = ";
      for (int a = 1; a <= 3; a++) {
        for (int b = 1; b <= 3; b++) {
          message << std::fixed << std::setw(5) << std::setprecision(3) << _bornEffectiveChargeTensor[i](a, b) << " ";
        }
  ...
  //////////////////////////////////////////////////////////////////////////////
  void ForceConstantCalculator::readDielectricTensorFromAIMSOUT() {
    const string message = "This functionality has not been implemented yet.";
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
  }
  ...
  void ForceConstantCalculator::readDielectricTensorFromOUTCAR(const _xinput& xinp) {  // ME20190113
    ...
    if (vlines.empty()) {
      message = "Cannot open OUTCAR static";
      aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_ERROR_);
    }

    // Find
    size_t line_count = 0;
    for (size_t iline = 0; iline < vlines.size() && !line_count; iline++) {
      if (vlines[iline].find("MACROSCOPIC STATIC DIELECTRIC TENSOR (") != string::npos) {
        line_count = iline + 2;
      }
    }
    if (!line_count) {
...
namespace apl {
  ...
  void ForceConstantCalculator::hibernate() {
    ...
    const string base = _directory + "/" + DEFAULT_APL_FILE_PREFIX;
  ...
  void ForceConstantCalculator::writeHarmonicIFCs(const string& filename) {
    ...
    outfile << tab << tab << R"(<i name="aflow_version" type="string">)" << AFLOW_VERSION << "</i>" << std::endl;
    string time = aflow_get_time_string();
    if (time[time.size() - 1] == '\n') {
      time.erase(time.size() - 1);
    }
    outfile << tab << tab << R"(<i name="date" type="string">)" << time << "</i>" << std::endl;
    ...
    // Force constants
    outfile << tab << R"(<fcms units="eV/Angstrom^2" cs="cartesian" rows=")" << _forceConstantMatrices.size() << "\" cols=\"" << _forceConstantMatrices[0].size() << "\">" << std::endl;

    outfile << tab << tab << "<varray>" << std::endl;
    for (size_t i = 0; i < _forceConstantMatrices.size(); i++) {
      outfile << tab << tab << tab << "<varray row=\"" << i << "\">" << std::endl;
      for (size_t j = 0; j < _forceConstantMatrices[i].size(); j++) {
        outfile << tab << tab << tab << tab << "<matrix row=\"" << i << "\" col=\"" << j << "\">" << std::endl;
        for (int k = 1; k <= 3; k++) {
  ...
  void ForceConstantCalculator::writeBornChargesDielectricTensor(const string& filename) {
    ...
    outfile << tab << R"(<born units="a.u." cs="cartesian">)" << std::endl;
    outfile << tab << tab << "<varray>" << std::endl;
    for (size_t i = 0; i < _bornEffectiveChargeTensor.size(); i++) {
      const int id = _supercell->getUniqueAtomID(i);
    ...
    // Dielectric tensor
    outfile << tab << R"(<epsilon units="a.u." cs="cartesian">)" << std::endl;
  ...
  void ForceConstantCalculator::saveState(const string& filename) {
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
    stringstream out;
    const string tag = "[APL_FC_CALCULATOR]";
    out << AFLOWIN_SEPARATION_LINE << std::endl;
    out << tag << "ENGINE=" << _method << std::endl;
    if (xInputs[0].AFLOW_MODE_VASP) {
      out << tag << "AFLOW_MODE=VASP" << std::endl;
    } else if (xInputs[0].AFLOW_MODE_AIMS) {
      out << tag << "AFLOW_MODE=AIMS" << std::endl;
    }
    out << AFLOWIN_SEPARATION_LINE << std::endl;
    ...
    if (_method == "DM") {  // Direct method - see README_AFLOW_APL.TXT
      ...
      int idxRun = 0;
      for (size_t i = 0; i < _uniqueDistortions.size(); i++) {
        for (size_t j = 0; j < _uniqueDistortions[i].size(); j++) {
          out << i << " " << _uniqueDistortions[i][j] << " " << xInputs[idxRun++].xvasp.AVASP_arun_runname;
          if (vvgenerate_plus_minus[i][j]) {
            out << " " << xInputs[idxRun++].xvasp.AVASP_arun_runname;
          }
          out << std::endl;
        }
      }
      ...
      if (_calculateZeroStateForces) {
        out << tag << "ZEROSTATE_RUNNAME=" << xInputs[idxRun++].xvasp.AVASP_arun_runname << std::endl;
      }
      out << AFLOWIN_SEPARATION_LINE << std::endl;
      out << tag << "POLAR=" << _isPolarMaterial << std::endl;
      if (_isPolarMaterial) {
        out << tag << "POLAR_RUNNAME=" << xInputs[idxRun].xvasp.AVASP_arun_runname << std::endl;
      }
      out << AFLOWIN_SEPARATION_LINE << std::endl;
    } else if (_method == "LR") {  // Linear response - see README_AFLOW_APL.TXT
      out << tag << "POLAR=" << _isPolarMaterial << std::endl;
  ...
  void ForceConstantCalculator::readFromStateFile(const string& filename) {
    ...
    vector<string> vlines;
    vector<string> tokens;
    aurostd::compressfile2vectorstring(filename, vlines);
    const uint nlines = vlines.size();
    uint iline = 0;
    _xinput xInput;
    while (++iline < nlines) {
    ...
    if (_method == "DM") {
        ...
        if (aurostd::substring2bool(vlines[iline], "DISTORTION_MAGNITUDE=")) {
          ...
          while ((iline++ < nlines) && !aurostd::substring2bool(vlines[iline], "DISTORTIONS=STOP")) {
            ...
            if (idist + 1 > _uniqueDistortions.size()) {
              _uniqueDistortions.emplace_back(0);
              vvgenerate_plus_minus.emplace_back(0);
            }
            for (int i = 1; i < 4; i++) {
              distortion[i] = aurostd::string2utype<double>(tokens[i]);
            }
      ...
      int idxRun = 0;
      for (size_t i = 0; i < _uniqueDistortions.size(); i++) {
        const int idAtom = (DISTORTION_INEQUIVONLY ? _supercell->getUniqueAtomID(i) : i);
        for (size_t j = 0; j < _uniqueDistortions[i].size(); j++) {
      ...
      while (++iline < nlines) {
        if (aurostd::substring2bool(vlines[iline], "POLAR=")) {
          aurostd::string2tokens(vlines[iline], tokens, "=");
          if (tokens.size() != 2) {
            const string message = "Tag for POLAR is broken.";
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_CORRUPT_);
          }
          _isPolarMaterial = aurostd::string2utype<bool>(tokens[1]);
          if (_isPolarMaterial) {
    ...
    // Set directories
    string dir;
    for (size_t i = 0; i < xInputs.size(); i++) {
...
namespace apl {
  ...
  bool ForceConstantCalculator::runVASPCalculationsDM(_xinput& xInput, _aflags& _aflowFlags, _kflags& _kbinFlags, _xflags& _xFlags, string& _AflowIn) {
    stringstream message;
    bool stagebreak = false;
    ...
    for (size_t i = 0; i < _uniqueDistortions.size(); i++) {
      vvgenerate_plus_minus.emplace_back(0); // CO20181226
      for (size_t j = 0; j < _uniqueDistortions[i].size(); j++) {
    ...
    for (size_t i = 0; i < _uniqueDistortions.size(); i++) {
      for (size_t j = 0; j < _uniqueDistortions[i].size(); j++) {
        ...
        for (uint k = 0; k < (generate_plus_minus ? 2 : 1); k++) {
          ...
          xInputs.push_back(xInput);
          const uint idxRun = xInputs.size() - 1;
          const uint idAtom = (DISTORTION_INEQUIVONLY ? _supercell->getUniqueAtomID(i) : i); // CO20190218
          ...
          if (_supercell->scell_dim.rows == 3) {
            ...
            const xmatrix<double> scmat = aurostd::xmatrixutype2double(aurostd::reshape(_supercell->scell_dim, 3, 3));
            xstr.title += "[" + xmatDouble2String(scmat, 0) + "]";
          }
          xstr.title += " atom=" + aurostd::utype2string<int>(idAtom); // ME20190109
          const xvector<double> dist_cart = DISTORTION_MAGNITUDE * _uniqueDistortions[i][j];  // ME20190112
          ...
          if (_kbinFlags.AFLOW_MODE_AIMS) {
            const string runname = ARUN_DIRECTORY_PREFIX + "APL_" + aurostd::utype2string<int>(idxRun) + "A" + aurostd::utype2string<uint>(_supercell->getUniqueAtomID(i)) + "D" + aurostd::utype2string<uint>(j);
    ...
    if (_calculateZeroStateForces) {
      ...
      if (xInputs[idxRun].getXStr().title.empty()) {
        xInputs[idxRun].getXStr().buildGenericTitle(true, false);
      } // CO20181226
      ...
      if (_kbinFlags.AFLOW_MODE_AIMS) {
        const string runname = ARUN_DIRECTORY_PREFIX + "APL_" + aurostd::utype2string<uint>(idxRun) + "ZEROSTATE";
        xInputs[idxRun].setDirectory(_directory + "/" + runname);
        if (!filesExistPhonons(xInputs[idxRun])) {
          message << "Creating " << xInputs[idxRun].getDirectory();
          pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
  ...
  void ForceConstantCalculator::estimateUniqueDistortions(const xstructure& xstr, vector<vector<xvector<double>>>& uniqueDistortions) {
    ...
    // Clear old stuff
    for (size_t i = 0; i < uniqueDistortions.size(); i++) {
    ...
    if (xstr.agroup_calculated) {
      ...
      if (GENERATE_ONLY_XYZ) {
        ...
        for (size_t idTestVector = 0; idTestVector < testDistortions.size(); idTestVector++) {
        ...
        for (size_t idTestVector = 0; idTestVector < testDistortions.size(); idTestVector++) {
          const double len = aurostd::modulus(testDistortions[idTestVector]);
      ...
      const uint natoms = DISTORTION_INEQUIVONLY ? xstr.iatoms.size() : xstr.atoms.size();
      for (uint i = 0; i < natoms; i++) {
        const int atomID = (DISTORTION_INEQUIVONLY ? xstr.iatoms[i][0] : i); // CO20190218
        // cout << "atomID = " << atomID << std::endl; //CO20190218
        if (xstr.agroup[atomID].empty()) { // CO20190218
          message << "Site point group operations are missing.";
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
        }
        ...
        for (size_t idTestVector = 0; idTestVector < testDistortions.size(); idTestVector++) {
          ...
          testDistortion(testDistortions[idTestVector], xstr.agroup[atomID], allDistortionsOfAtom, uniqueDistortionsOfAtom, true); // CO20190114 - maximize distortion impact first
        ...
        for (size_t j = 0; j < testDistortions.size() - 1; j++) {
          for (size_t k = j + 1; k < testDistortions.size(); k++) {
            if (testVectorDim[k] > testVectorDim[j]) {
              const xvector<double> temp = testDistortions[k];
              testDistortions[k] = testDistortions[j];
              testDistortions[j] = temp;
              const int itemp = testVectorDim[k];
        ...
        for (size_t idTestVector = 0; idTestVector < testDistortions.size(); idTestVector++) {
          testDistortion(testDistortions[idTestVector], xstr.agroup[atomID], allDistortionsOfAtom, uniqueDistortionsOfAtom,
                         DISTORTION_SYMMETRIZE);  // CO20190114 - if DISTORTION_SYMMETRIZE==false, we want all 3 directions (don't integrate equivalent distortions into count)
          if (allDistortionsOfAtom.size() >= 3) {
            break;
          }
        }
    ...
    int dof = 0;
    for (size_t i = 0; i < _uniqueDistortions.size(); i++) { // CO20200106 - wrapping with guard
      dof += _uniqueDistortions[i].size();
    }
    message << "Found " << dof << " degree(s) of freedom.";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
    const uint natoms = DISTORTION_INEQUIVONLY ? _supercell->getNumberOfUniqueAtoms() : _supercell->getNumberOfAtoms();
    for (uint i = 0; i < natoms; i++) {  // CO20200212 - int->uint
      const uint id = (DISTORTION_INEQUIVONLY ? _supercell->getUniqueAtomID(i) : i); // CO20190218
      for (size_t j = 0; j < _uniqueDistortions[i].size(); j++) {
        message << "Atom [" << aurostd::PaddedNumString(id, 3) << "] (" << std::setw(2) << _supercell->getSupercellStructure().atoms[id].cleanname << ") will be displaced in direction [" << std::fixed
                << std::setw(5) << std::setprecision(3) << _uniqueDistortions[i][j](1) << "," << std::fixed << std::setw(5) << std::setprecision(3) << _uniqueDistortions[i][j](2) << "," << std::fixed
  ...
  void ForceConstantCalculator::testDistortion(const xvector<double>& distortionVector,
    ...
    for (size_t k = 0; k < allDistortionsOfAtom.size(); k++) {
    ...
    if (integrate_equivalent_distortions) { // CO20181226
      ...
      for (size_t iSymOp = 0; iSymOp < symPool.size(); iSymOp++) {
  ...
  bool ForceConstantCalculator::needMinus(uint atom_index, uint distortion_index, bool inequiv_only) { // CO20190218
    ...
    xvector<double> rotated_distortion;
...
namespace apl {
  ...
  bool ForceConstantCalculator::calculateForceConstantsDM() {
    ...
    const string filename = aurostd::CleanFileName(_directory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_DYNMAT_FILE);
  ...
  bool ForceConstantCalculator::calculateForceFields() {
    ...
    if (!outfileFoundEverywherePhonons(xInputs, _directory, *p_FileMESSAGE, *p_oss, _isPolarMaterial)) {
      return false;
    }
    ...
    for (uint i = 0; i < natoms; i++) {  // CO20200212 - int->uint
      ...
      for (size_t j = 0; j < _uniqueDistortions[i].size(); j++) {
        ...
        for (uint k = 0; k < _supercell->getNumberOfAtoms(); k++) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " force[idistortion=" << i << ",atom=" << k << ",+]=" << xInputs[idxRun].getXStr().qm_forces[k] << std::endl;
          } // CO20190218
          if (generate_plus_minus) {  // CO
            ...
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " force[idistortion=" << i << ",atom=" << k << ",-]=" << xInputs[idxRun + 1].getXStr().qm_forces[k] << std::endl;
            } // CO20190218
          }
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " force[idistortion=" << i << ",atom=" << k << ",AVG]=" << force << std::endl;
          } // CO20190218
        ...
        // Remove drift
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " drift[idistortion=" << i << ",total]=" << drift << std::endl;
        } // CO20190218
        drift(1) = drift(1) / forcefield.size();
        drift(2) = drift(2) / forcefield.size();
        drift(3) = drift(3) / forcefield.size();
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " drift[idistortion=" << i << ",AVG]=" << drift << std::endl;
        } // CO20190218
        for (size_t k = 0; k < forcefield.size(); k++) {
          forcefield[k] = forcefield[k] - drift;
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " force[idistortion=" << i << ",atom=" << k << ",-drift]=" << forcefield[k] << std::endl;
          } // CO20190218
        }
  ...
  void ForceConstantCalculator::completeForceFields() {
    ...
    for (uint i = 0; i < (DISTORTION_INEQUIVONLY ? _supercell->getNumberOfUniqueAtoms() : _supercell->getNumberOfAtoms()); i++) { // CO20190218
      ...
      if (_uniqueDistortions[i].size() != 3) {
        ...
        const int atomID = (DISTORTION_INEQUIVONLY ? _supercell->getUniqueAtomID(i) : i); // CO20190218
        ...
        const uint currentSizeDistortions = _uniqueDistortions[i].size(); // CO20190218
          ...
          for (size_t symOpID = 0; symOpID < agroup.size(); symOpID++) {
            ...
            for (uint k = 0; k < _supercell->getNumberOfAtoms(); k++) {
              try {
                // ME20191219 - atomGoesTo now uses basis_atoms_map; keep translation option in case
                //  the basis has not been calculated for some reason
                const int l = _supercell->atomComesFrom(symOp, k, atomID, true); // CO20190218
            ...
            if (aurostd::modulus(testVec) > _FLOAT_TOL_) {
              ...
              const double testVectorLength = aurostd::modulus(testVec);
              for (uint l = 0; l < _supercell->getNumberOfAtoms(); l++) {
        ...
        allDistortionsOfAtom.clear();
        for (size_t ii = 0; ii < forcePool.size(); ii++) {
          forcePool[ii].clear();
        }
        forcePool.clear();
      }
      ...
      if (_uniqueDistortions[i].size() != 3) {
        const string message = "Cannot complete force fields by symmetry.";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
      }
    }
  }
  ...
  void ForceConstantCalculator::projectToCartesianDirections() {
    ...
    for (uint i = 0; i < (DISTORTION_INEQUIVONLY ? _supercell->getNumberOfUniqueAtoms() : _supercell->getNumberOfAtoms()); i++) { // CO20190218
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " looking at displaced atom[idistortion=" << i << "]" << std::endl;
      } // CO20190218
      ...
      for (uint j = 0; j < _supercell->getNumberOfAtoms(); j++) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " looking at supercell atom[" << j << "]" << std::endl;
        } // CO20190218
        for (int k = 0; k < 3; k++) {
          for (int l = 1; l <= 3; l++) {
  ...
  void ForceConstantCalculator::buildForceConstantMatrices() {
    ...
    for (uint i = 0; i < _supercell->getNumberOfAtoms(); i++) {
      _forceConstantMatrices.emplace_back(0);
      for (uint j = 0; j < _supercell->getNumberOfAtoms(); j++) {
        _forceConstantMatrices.back().emplace_back(3, 3, 1, 1);
      }
    }
    ...
    for (uint i = 0; i < (DISTORTION_INEQUIVONLY ? _supercell->getNumberOfUniqueAtoms() : _supercell->getNumberOfAtoms()); i++) { // CO20190218
      ...
      const int basedAtomID = (DISTORTION_INEQUIVONLY ? _supercell->getUniqueAtomID(i) : i); // CO20190218

      // This is easy. We know everything. Just construct a set of matrices.
      xmatrix<double> m(3, 3, 1, 1);
      for (uint j = 0; j < _supercell->getNumberOfAtoms(); j++) {
        for (size_t k = 0; k < _uniqueDistortions[i].size(); k++) {
          const double distortionLength = aurostd::modulus(DISTORTION_MAGNITUDE * _uniqueDistortions[i][k]);
        ...
        for (uint j = 1; j < _supercell->getNumberOfEquivalentAtomsOfType(i); j++) { // CO20190218
          ...
          for (uint k = 0; k < _supercell->getNumberOfAtoms(); k++) {
            try {
              ...
              const int l = _supercell->atomGoesTo(symOp, k, _supercell->getUniqueAtomID(i, j)); // JAHNATEK ORIGINAL //CO20190218
              ...
              message << "Mapping problem " << k << " <-> ?.";
              throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Mapping failed.");
            }
          }
          //_forceConstantMatrices.push_back(row);  //JAHNATEK ORIGINAL //CO20190218
          // row.clear();  //JAHNATEK ORIGINAL //CO20190218
    ...
    for (size_t i = 0; i < _forceConstantMatrices.size(); i++) {
      for (size_t j = 0; j < _forceConstantMatrices.size(); j++) {
  ...
  void ForceConstantCalculator::writeDYNMAT(const string& filename) {
    ...
    for (uint i = 0; i < _supercell->getNumberOfUniqueAtoms(); i++) {
      for (size_t j = 0; j < _uniqueDistortions[i].size(); j++) {
        ...
        for (uint k = 0; k < _supercell->getNumberOfAtoms(); k++) {
          outfile << setw(15) << _uniqueForces[i][j][k](1) << setw(15) << _uniqueForces[i][j][k](2) << setw(15) << _uniqueForces[i][j][k](3) << std::endl;
        }
      }
    }
...
namespace apl {
  ...
  bool ForceConstantCalculator::runVASPCalculationsLR(_xinput& xInput, _aflags& _aflowFlags, _kflags& _kbinFlags, _xflags& _xFlags, string& _AflowIn) {
    bool stagebreak = false;
    ...
    if (xInput.AFLOW_MODE_AIMS) {
      const string runname = _AFLOW_APL_DFPT_DIRECTORY_NAME_;  // ME20200213
      xInput.setDirectory(_directory + "/" + runname);
      if (!filesExistPhonons(xInput)) {
        const string message = "Creating " + xInput.getDirectory();
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
  ...
  bool ForceConstantCalculator::readForceConstantsFromVasprun(_xinput& xinp) {
    ...
    const uint natoms = _supercell->getSupercellStructure().atoms.size();
    const uint nhessian = hessian.size();
    if (nhessian != 3 * natoms) {
      message << "Hessian matrix does not have the correct number of rows (has " << nhessian << ", should have " << (3 * natoms) << ")." << std::endl;
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_CORRUPT_);
    }
    uint i = 0;
    for (i = 0; i < nhessian; i++) {
      if (hessian[i].size() != 3 * natoms) {
        break;
      }
    }
    if (i != nhessian) {
