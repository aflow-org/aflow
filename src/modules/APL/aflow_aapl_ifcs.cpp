...
namespace apl {
  ...
  void AnharmonicIFCs::initialize(const Supercell& scell, int _order, const aurostd::xoption& opts) {
    ...
    const double cut_rad = aurostd::string2utype<double>(tokens[order - 3]);
    aurostd::string2tokens(opts.getattachedscheme("CUT_SHELL"), tokens, ",");
    if (tokens.size() < (uint) order - 2) {
      message = "Not enough parameters for CUT_SHELL";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_MISMATCH_);
    }
    const int cut_shell = aurostd::string2utype<int>(tokens[order - 3]);
    clst.initialize(scell, _order, cut_shell, cut_rad);
    const string clust_hib_file = directory + "/" + DEFAULT_AAPL_FILE_PREFIX + _CLUSTER_SET_FILE_[_order - 3];
    bool awakeClusterSet = aurostd::CompressFileExist(clust_hib_file);
    if (awakeClusterSet) {
      try {
...
namespace apl {
  ...
  bool AnharmonicIFCs::runVASPCalculations(_xinput& xinput, _aflags& aflags, _kflags& kflags, _xflags& xflags) {
    ...
    int nruns = 0;
    for (size_t ineq = 0; ineq < clst.ineq_distortions.size(); ineq++) {
      nruns += clst.ineq_distortions[ineq].distortions.size();
    }
    if (order == 4) {
      for (size_t ineq = 0; ineq < clst.higher_order_ineq_distortions.size(); ineq++) {
    ...
    for (size_t ineq = 0; ineq < clst.ineq_distortions.size(); ineq++) {
      ...
      for (size_t dist = 0; dist < idist.distortions.size(); dist++) {
    ...
    if (order == 4) {
      for (size_t ineq = 0; ineq < clst.higher_order_ineq_distortions.size(); ineq++) {
        ...
        for (size_t dist = 0; dist < idist.distortions.size(); dist++) {
          ...
          if ((kflags.KBIN_MPI && (kflags.KBIN_BIN.find("46") != string::npos)) || (kflags.KBIN_MPI && (kflags.KBIN_MPI_BIN.find("46") != string::npos))) {
  ...
  string AnharmonicIFCs::buildRunName(const vector<int>& distortions, const vector<int>& atoms, int run, int nruns) const {
  ...
  void AnharmonicIFCs::applyDistortions(_xinput& xinp, const vector<xvector<double>>& distortion_vectors, const vector<int>& distortions, const vector<int>& atoms, double scale) const {
    xstructure& xstr = xinp.getXStr();  // ME20190109
    for (size_t at = 0; at < atoms.size(); at++) {
      const int atsc = atoms[at];
      ...
      for (int i = 1; i < 4; i++) {
        if (std::abs(dist_cart(i)) < _ZERO_TOL_) {
          dist_cart(i) = 0.0;
        } else {
      ...
      distortion << " distortion=[" << std::setprecision(3) << dist_cart[1] << "," << std::setprecision(3) << dist_cart[2] << "," << std::setprecision(3) << dist_cart[3] << "]"; // ME20190112
...
namespace apl {
  ...
  bool AnharmonicIFCs::calculateForceConstants() {
    ...
    if (!outfileFoundEverywherePhonons(xInputs, directory, *p_FileMESSAGE, *p_oss)) {
      return false;
    }
    if (_useZeroStateForces) {
      vector<string> dirs;
      aurostd::DirectoryLS(directory, dirs);
      const uint ndir = dirs.size();
      uint d = 0;
      for (d = 0; d < ndir; d++) {
      ...
      if (d == ndir) {
        const string message = "Could not find ZEROSTATE directory.";
    ...
    const vector<vector<double>> ifcs_unsym = calculateUnsymmetrizedIFCs(clst.ineq_distortions, force_tensors);
    force_tensors.clear();
  ...
  vector<vector<int>> AnharmonicIFCs::getClusters() const {
    const uint nclusters = clst.clusters.size();
...
namespace apl {
  ...
  vector<vector<int>> AnharmonicIFCs::getCartesianIndices() const {
  ...
  vector<vector<vector<xvector<double>>>> AnharmonicIFCs::storeForces(vector<_xinput>& xInp) {
    ...
    int idxRun = 0;
    for (size_t id = 0; id < clst.ineq_distortions.size(); id++) {
      force_tensors[id] = getForces(id, idxRun, xInp);
    }
    if (order == 4) {
      addHigherOrderForces(force_tensors, idxRun, xInp);
    }
  ...
  vector<vector<xvector<double>>> AnharmonicIFCs::getForces(int id, int& idxRun, vector<_xinput>& xInp) {
    const _ineq_distortions& ineq_dists = clst.ineq_distortions[id];
    const int natoms = (int) clst.scell.atoms.size();
    ...
    int fg = 0;
    int index = 0;
    for (size_t ineq = 0; ineq < ineq_dists.distortions.size(); ineq++) {
      ...
      for (size_t i = 1; i < ineq_dists.distortions[ineq].size(); i++) {
        // For each equivalent distortion, transform the forces using symmetry
        const vector<xvector<double>> qmforces_trans(natoms, xvector<double>(3));
        index = 0;
        for (int ind = 0; ind < order - 1; ind++) {
  ...
  void AnharmonicIFCs::addHigherOrderForces(vector<vector<vector<xvector<double>>>>& force_tensor, int& idxRun, vector<_xinput>& xInp) {
    const vector<_ineq_distortions>& ineq_dists = clst.higher_order_ineq_distortions;
    const uint ndist = force_tensor[0].size();
    const uint natoms = clst.scell.atoms.size();
    const vector<xvector<double>> forces(natoms, xvector<double>(3));
    for (size_t ineq = 0; ineq < ineq_dists.size(); ineq++) {
      uint idist;
      const int at = ineq_dists[ineq].atoms[0];
      for (idist = 0; idist < clst.ineq_distortions.size(); idist++) {
        int a;
        for (a = 0; a < order - 1; a++) {
          if (clst.ineq_distortions[idist].atoms[a] != at) {
            break;
          }
        }
        if (a == order - 1) {
      ...
      for (int i = 0; i < 6; i++) {
        force_tensor[idist].push_back(forces);
      }

      for (size_t dist = 0; dist < ineq_dists[ineq].distortions.size(); dist++) {
        const vector<xvector<double>>& qmforces = xInp[idxRun].getXStr().qm_forces;
        int d = ineq_dists[ineq].distortions[dist][0][0];
        force_tensor[idist][ndist + d][at] = qmforces[at];
        int fg = 0;
        for (size_t i = 1; i < ineq_dists[ineq].distortions[dist].size(); i++) {
  ...
  int AnharmonicIFCs::getTransformedAtom(const vector<int>& symmap, const int& at) {
    for (size_t i = 0; i < symmap.size(); i++) {
      if (symmap[i] == at) {
        return i;
      }
    }
  ...
  vector<vector<double>> AnharmonicIFCs::calculateUnsymmetrizedIFCs(const vector<_ineq_distortions>& idist, const vector<vector<vector<xvector<double>>>>& forces) {
    vector<vector<double>> ifcs(clst.clusters.size(), vector<double>(cart_indices.size()));
    int at = 0;
    int cl = 0;
    int ic = 0;
    const double denom = std::pow(distortion_magnitude, order - 1);
    for (size_t f = 0; f < forces.size(); f++) {
      for (size_t c = 0; c < idist[f].clusters.size(); c++) {
  ...
  double AnharmonicIFCs::finiteDifference(const vector<vector<xvector<double>>>& forces, int at, const vector<int>& cart_ind, const vector<int>& atoms) const {
    vector<int> powers(order - 1, 1);
    ...
    for (size_t a = 0; a < atoms.size(); a++) {
      count = 1;
      while (((int) a + count < order - 1) && (atoms[a + count] == atoms[a]) && (cart_ind[a + count] == cart_ind[a])) {
        count++;
      }
    ...
    if (derivatives[0] == 3) {
      ...
      for (int i = 0; i < 5; i++) {
        if (C3[i] != 0.0) {
          diff -= C3[i] * forces[dists[i]][at][cart_ind[order - 1] + 1];
        }
      }
    } else {
      const uint nder = derivatives.size();
        ...
        if (coeff != 0.0) {
          ...
          for (uint i = 0; i < nder; i++) {
            for (int j = 0; j < derivatives[i]; j++) {
              if (index[i] == 1) {
                dist += powers[d] * (cart_ind[d] + 3 * j);
              } else {
                dist += powers[d] * (cart_ind[d] + 3 * index[i] / 2);
              }
              d++;
            }
          }
...
namespace apl {
  ...
  vector<vector<double>> AnharmonicIFCs::symmetrizeIFCs(vector<vector<double>> ifcs) {
    // Initialize tensors
    const vector<vector<int>> reduced_clusters = getReducedClusters();
    ...
    *p_oss << std::setw(20) << "Abs. max. error" << std::endl;
    do {
      ...
      max_err = -1E30;
      for (size_t i = 0; i < dev_from_zero.size(); i++) {
        for (size_t j = 0; j < dev_from_zero[i].size(); j++) {
          if (std::abs(dev_from_zero[i][j]) > max_err) {
            max_err = std::abs(dev_from_zero[i][j]);
          }
        }
      }
  ...
  vector<vector<int>> AnharmonicIFCs::getReducedClusters() {
    ...
    for (size_t c = 0; c < clst.clusters.size(); c++) {
      const vector<int>& atoms = clst.clusters[c].atoms;
      bool append = true;
      for (size_t r = 0; r < reduced_clusters.size(); r++) {
        append = false;
        for (int i = 0; i < order - 1; i++) {
  ...
  //
  //  Check the typedefs at the beginning of the file for tform and v4int
  void AnharmonicIFCs::getTensorTransformations(v4int& eq_ifcs, vector<vector<tform>>& transformations) {
    for (size_t ineq = 0; ineq < clst.ineq_clusters.size(); ineq++) {
      ...
      for (size_t c = 1; c < clst.ineq_clusters[ineq].size(); c++) {
        tform trf;
        const _cluster cluster_trans = clst.getCluster(clst.ineq_clusters[ineq][c]);
        int fg = 0;
        int perm = 0;
        int rw = 0;
        int cl = 0;
  ...
  void AnharmonicIFCs::transformIFCs(const vector<vector<tform>>& transformations, vector<vector<double>>& ifcs) {
    int clst_orig = 0;
    int clst_trans = 0;
    int cart_indices_orig = 0;
    double coeff = 0.0;
    for (size_t ineq = 0; ineq < clst.ineq_clusters.size(); ineq++) {
      ...
      for (size_t c = 1; c < clst.ineq_clusters[ineq].size(); c++) {
        ...
        for (int itrans = 0; itrans < clst.nifcs; itrans++) {
          ...
          for (size_t t = 0; t < transf.first.size(); t++) {
  ...
  void AnharmonicIFCs::calcSums(const vector<vector<int>>& reduced_clusters, const vector<vector<double>>& ifcs, vector<vector<double>>& dev_from_zero, vector<vector<double>>& abssum) {
    ...
    for (size_t i = 0; i < clst.ineq_clusters.size(); i++) {
      for (size_t j = 0; j < clst.ineq_clusters[i].size(); j++) {
        const uint r = findReducedCluster(reduced_clusters, clst.getCluster(clst.ineq_clusters[i][j]).atoms);
        for (int c = 0; c < clst.nifcs; c++) {
          dev_from_zero[r][c] += ifcs[clst.ineq_clusters[i][j]][c];
          abssum[r][c] += std::abs(ifcs[clst.ineq_clusters[i][j]][c]);
        }
      }
    }
  }
  ...
  void AnharmonicIFCs::correctIFCs(vector<vector<double>>& ifcs, const vector<vector<double>>& dev_from_zero, const vector<vector<double>>& abssum, const vector<vector<int>>& reduced_clusters, const v4int& eq_ifcs) {
    vector<int> eq;
    for (size_t ineq = 0; ineq < clst.ineq_clusters.size(); ineq++) {
      const uint nclusters = clst.ineq_clusters[ineq].size();
      const int ic = clst.ineq_clusters[ineq][0];
      // Calculate correction terms
      vector<vector<double>> correction_terms(nclusters);
      for (uint c = 0; c < nclusters; c++) {
        correction_terms[c] = getCorrectionTerms(clst.ineq_clusters[ineq][c], reduced_clusters, ifcs, dev_from_zero, abssum);
      }
      ...
      for (size_t i = 0; i < indep.size(); i++) {
        ...
        for (uint c = 0; c < nclusters; c++) {
          ...
          for (size_t dep = 0; dep < lcomb.indep2depMap[i].size(); dep++) {
  ...
  vector<double> AnharmonicIFCs::getCorrectionTerms(const int& c, const vector<vector<int>>& reduced_clusters, const vector<vector<double>>& ifcs, const vector<vector<double>>& dev_from_zero, const vector<vector<double>>& abssum) {
    vector<double> correction_terms = ifcs[c];
    vector<double> correction(clst.nifcs);
    for (int i = 0; i < clst.nifcs; i++) {
      correction[i] = std::abs(correction_terms[i]);
    }
    const uint r = findReducedCluster(reduced_clusters, clst.getCluster(c).atoms);
    for (int crt = 0; crt < clst.nifcs; crt++) {
      if (abssum[r][crt] != 0.0) {
        correction[crt] *= dev_from_zero[r][crt] / abssum[r][crt];
      } else {
        correction[crt] = 0.0;
      }
  ...
  uint AnharmonicIFCs::findReducedCluster(const vector<vector<int>>& reduced_clusters, const vector<int>& atoms) const {
    for (size_t r = 0; r < reduced_clusters.size(); r++) {
      int at = 0;
      for (at = 0; at < order - 1; at++) {
        if (reduced_clusters[r][at] != atoms[at]) {
          break;
        }
      }
      if (at == order - 1) {
...
namespace apl {
  ...
  void AnharmonicIFCs::writeIFCsToFile(const string& filename) {
    ...
    if (!aurostd::FileExist(filename)) {
      const string message = "Could not write tensor to file.";
  ...
  string AnharmonicIFCs::writeParameters() {
    ...
    parameters << tab << R"(<distortion_magnitude units="Angstrom" cs="cartesian">)";
    ...
    for (size_t i = 0; i < clst.pcell.atoms.size(); i++) {
      const int t = clst.pcell.atoms[i].type;
  ...
  string AnharmonicIFCs::writeIFCs() {
    ...
    const int extra = 5; // first digit, decimal point, minus sign + 2 spaces
    double max_ifc = -1E30;
    for (size_t i = 0; i < force_constants.size(); i++) {
      for (size_t j = 0; j < force_constants[i].size(); j++) {
        if (std::abs(force_constants[i][j]) > max_ifc) {
          max_ifc = std::abs(force_constants[i][j]);
        }
      }
    }
    ...
    for (size_t c = 0; c < clst.clusters.size(); c++) {
      ifcs << tab << tab << "<varray atoms=\"" << aurostd::joinWDelimiter(clst.clusters[c].atoms, " ") << "\">" << std::endl;
      int crt = 0;
