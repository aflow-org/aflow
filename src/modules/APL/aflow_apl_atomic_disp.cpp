...
namespace apl {
  ...
  void AtomicDisplacements::calculateEigenvectors() {
    _eigenvectors.clear();
    _frequencies.clear();
    const int nq = (int) _qpoints.size();
    if (nq == 0) {
      return;
    }
    const uint natoms = _pc->getInputCellStructure().atoms.size();
    const uint nbranches = _pc->getNumberOfBranches();
...
namespace apl {
  ...
  void AtomicDisplacements::calculateMeanSquareDisplacementMatrices() {
    const string message = "Calculating mean square displacement matrices.";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _pc->getDirectory(), *_pc->getOFStream(), *_pc->getOSS());
    _displacement_matrices.clear();
    _displacement_modes.clear();
    calculateEigenvectors();

    const uint ntemps = _temperatures.size();
    const uint natoms = _pc->getInputCellStructure().atoms.size();
    const uint nq = _qpoints.size();
    const uint nbranches = _pc->getNumberOfBranches();
    ...
    for (uint at = 0; at < natoms; at++) {
      masses[at] = AMU2KILOGRAM * _pc->getSupercell().getAtomMass(_pc->getSupercell().pc2scMap(at));
    }

    // Factor 2pi necessary because frequencies are raw
    const double prefactor = (PLANCKSCONSTANT_hbar * Hz2THz * std::pow(1e10, 2) / (2 * PI * (double) _qpoints.size()));
    for (uint q = 0; q < nq; q++) {
      for (uint br = 0; br < nbranches; br++) {
        if (_frequencies[q][br] > _FLOAT_TOL_) {
          ...
          for (uint t = 0; t < ntemps; t++) {
            const double prefactor_T = prefactor * ((0.5 + getOccupationNumber(_temperatures[t], _frequencies[q][br])) / _frequencies[q][br]);
            for (uint at = 0; at < natoms; at++) {
  ...
  void AtomicDisplacements::calculateModeDisplacements(const vector<xvector<double>>& qpts, bool coords_are_fractional) {
    if (!_pc_set) {
      const string message = "PhononCalculator pointer not set.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_INIT_);
    }
    _qpoints.clear();
    const uint nq = qpts.size();
    _qpoints.resize(nq);
    if (coords_are_fractional) {
      const xmatrix<double> f2c = trasp(ReciprocalLattice(_pc->getInputCellStructure().lattice));
      for (uint q = 0; q < nq; q++) {
        _qpoints[q].fpos = qpts[q];
        _qpoints[q].cpos = f2c * qpts[q];
      }
    } else {
      const xmatrix<double> c2f = inverse(trasp(ReciprocalLattice(_pc->getInputCellStructure().lattice)));
      for (uint q = 0; q < nq; q++) {
  ...
  void AtomicDisplacements::calculateModeDisplacements() {
    ...
    vector<double> masses(natoms);
    for (uint at = 0; at < natoms; at++) {
      masses[at] = _pc->getSupercell().getAtomMass(_pc->getSupercell().pc2scMap(at));
    }
  ...
  double AtomicDisplacements::getOccupationNumber(double T, double f) {
    if (T < _FLOAT_TOL_) {
      ...
      return (1.0 / (exp(BEfactor_h_THz * f / T) - 1));
    }
  }
...
namespace apl {
  ...
  vector<vector<xvector<double>>> AtomicDisplacements::getDisplacementVectors() const {
    ...
    const uint ntemps = _displacement_matrices.size();
    if (ntemps > 0) {
      const uint natoms = _displacement_matrices[0].size();
...
namespace apl {
  ...
  void AtomicDisplacements::writeMeanSquareDisplacementsToFile(string filename) {
    ...
    stringstream output;
    const string tag = "[APL_DISPLACEMENTS]";

    output << AFLOWIN_SEPARATION_LINE << std::endl;
    output << tag << "SYSTEM=" << _pc->_system << std::endl;
    output << tag << "START" << std::endl;
    output << "#" << std::setw(9) << "T (K)" << std::setw(15) << "Species" << std::setw(15) << "x (A^2)" << std::setw(15) << "y (A^2)" << std::setw(15) << "z (A^2)" << std::endl;
    output << std::fixed << std::showpoint;
    for (size_t t = 0; t < _temperatures.size(); t++) {
      output << std::setw(10) << std::setprecision(2) << _temperatures[t];
      for (size_t at = 0; at < disp_vec[t].size(); at++) {
        output << (at == 0 ? std::setw(15) : std::setw(25)) << _pc->getInputCellStructure().atoms[at].cleanname;
        for (int i = 1; i < 4; i++) {
          output << std::setw(15) << std::setprecision(8) << disp_vec[t][at][i];
        }
        output << std::endl;
      }
    }
  ...
  void AtomicDisplacements::writeSceneFileVsim(string filename, const xstructure& xstr_projected, const vector<vector<vector<xvector<xcomplex<double>>>>>& displacements) {
    if (!_pc_set) {
      const string message = "PhononCalculator pointer not set.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_INIT_);
    }
    ...
    stringstream output;
    // Lattice
    output << std::setw(15) << std::setprecision(8) << std::fixed << xstr_projected.lattice[1][1] << std::setw(15) << std::setprecision(8) << std::fixed << xstr_projected.lattice[2][1] << std::setw(15)
           << std::setprecision(8) << std::fixed << xstr_projected.lattice[2][2] << std::endl;
    output << std::setw(15) << std::setprecision(8) << std::fixed << xstr_projected.lattice[3][1] << std::setw(15) << std::setprecision(8) << std::fixed << xstr_projected.lattice[3][2] << std::setw(15)
           << std::setprecision(8) << std::fixed << xstr_projected.lattice[3][3] << std::endl;
    // Atoms
    const uint natoms = xstr_projected.atoms.size();
    for (uint at = 0; at < natoms; at++) {
      for (int i = 1; i < 4; i++) {
        output << std::setw(15) << std::setprecision(8) << xstr_projected.atoms[at].cpos[i];
      }
      output << std::setw(4) << xstr_projected.atoms[at].cleanname << std::endl;
    }

    for (size_t q = 0; q < displacements.size(); q++) {
      for (size_t br = 0; br < displacements[q].size(); br++) {
        output << "#metaData: qpt=[";
        for (int i = 1; i < 4; i++) {
          output << _qpoints[q].fpos[i] << ";";
        }
        output << _frequencies[q][br] << "\\" << std::endl;
        for (uint at = 0; at < natoms; at++) {
          output << "#;";
          for (int i = 1; i < 4; i++) {
            output << displacements[q][br][at][i].re << ";";
          }
          for (int i = 1; i < 4; i++) {
...
namespace apl {
  ...
  void createAtomicDisplacementSceneFile(const aurostd::xoption& vpflow, ofstream& mf, ostream& oss) {
    ...
    if (format.empty()) {
      format = aurostd::toupper(DEFAULT_APL_ADISP_SCENE_FORMAT);
    }
    const vector<string> allowed_formats{"XCRYSDEN", "V_SIM"};
    ...
    // Amplitude
    const string amplitude_str = vpflow.getattachedscheme("ADISP::AMPLITUDE");
    double amplitude = 0.0;
    if (amplitude_str.empty()) {
      amplitude = DEFAULT_APL_ADISP_AMPLITUDE;
    } else {
      amplitude = aurostd::string2utype<double>(amplitude_str);
    }
    ...
    const string nsteps_str = vpflow.getattachedscheme("ADISP::STEPS");
    int nsteps = 0;
    if (format != "V_SIM") {
      if (nsteps_str.empty()) {
        nsteps = DEFAULT_APL_ADISP_NSTEPS;
      } else {
        nsteps = aurostd::string2utype<int>(nsteps_str);
      }
      if (nsteps < 1) {
    ...
    // Number of periods
    const string nperiods_str = vpflow.getattachedscheme("ADISP::PERIODS");
    int nperiods = 0;
    if (format != "V_SIM") {
      if (nperiods_str.empty()) {
        nperiods = DEFAULT_APL_ADISP_NPERIODS;
      } else {
        nperiods = aurostd::string2utype<int>(nperiods_str);
      }
      if (nperiods < 1) {
    ...
    // Branches
    const string branches_str = vpflow.getattachedscheme("ADISP::BRANCHES");
    vector<int> branches;
    if (format != "V_SIM") {
      if (branches_str.empty()) {
        message = "No branches selected. Displacements will be calculated for all modes.";
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, directory, mf, oss);
      } else {
    ...
    // q-points
    const string qpoints_str = vpflow.getattachedscheme("ADISP::QPOINTS");
    ...
    if (qpoints_str.empty()) {
      ...
      if (tokens.size() % 3 == 0) {
        for (size_t i = 0; i < tokens.size(); i += 3) {
          for (int j = 0; j < 3; j++) {
            q[j + 1] = aurostd::string2utype<double>(tokens[i + j]);
          }
          qpoints.push_back(q);
        }
      } else {
    ...
    const string statefile = directory + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_STATE_FILE;
    ...
    if (nbr == 0) {
      ...
      const int nbranches = pc.getNumberOfBranches();
      for (int br = 0; br < nbr; br++) {
    ...
    if (format == "XCRYSDEN") {
      ...
      for (size_t q = 0; q < qpoints.size(); q++) {
        for (size_t br = 0; br < branches.size(); br++) {
      ...
      const string filename = directory + DEFAULT_APL_FILE_PREFIX + "displacements.ascii";
  ...
  vector<vector<vector<double>>> AtomicDisplacements::createDisplacementsXcrysden(const Supercell& scell, double amplitude, int q, int br, int nsteps) {
    ...
    const uint natoms = scell_str.atoms.size();
  ...
  void AtomicDisplacements::getOrientedDisplacementsVsim(xstructure& xstr_oriented, vector<vector<vector<xvector<xcomplex<double>>>>>& displacements, double amplitude) {
    ...
    const double singamma = sin(params[6]);
    const double l32 = (2 * cosalpha - 2 * cosbeta * cosgamma) / (2 * singamma);
    ...
    // Project positions
    const uint natoms = xstr_orig.atoms.size();
    ...
    const uint nq = _qpoints.size();
    const uint nbr = _pc->getNumberOfBranches();
    displacements.clear();
    displacements.resize(nq, vector<vector<xvector<xcomplex<double>>>>(nbr, vector<xvector<xcomplex<double>>>(natoms)));

    const xmatrix<double> transf = amplitude * xstr_oriented.f2c * xstr_orig.c2f;
    for (uint q = 0; q < nq; q++) {
