...
namespace KBIN {
  ...
  bool relaxStructureAPL_VASP(int start_relax,
    ...
    if (start_relax == _NUM_RELAX_) {
      return true;  // Nothing to do here
    }

    // Store original settings
    const string prec = vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme;
    const bool prec_entry = vflags.KBIN_VASP_FORCE_OPTION_PREC.isentry;
    const aurostd::xoption kbin_vasp_run = vflags.KBIN_VASP_RUN;
    const int nrelax = xvasp.NRELAX;
    ...
    const string prec_phonons = xvasp.aplopts.getattachedscheme("AFLOWIN_FLAG::APL_PREC");
    ...
    if (relax_commensurate) {
      ...
      for (int i = 1; i < 4; i++) {
        kpts_pc[i] = scell_dims[i] * kpts_sc[i];
      }
      message << "Relaxations will be performed on a " << aurostd::joinWDelimiter(kpts_pc, "x") << " k-point grid.";
      pflow::logger(__AFLOW_FILE__, "APL", message, aflags.Directory, fileMessage, oss);
      ...
      const aurostd::xoption kbin_vasp_kpoints_mode_back = vflags.KBIN_VASP_KPOINTS_MODE;
      const aurostd::xoption kbin_vasp_kpoints_file_back = vflags.KBIN_VASP_KPOINTS_FILE;
      const string kbin_vasp_kpoints_explicit_start_stop_back = vflags.KBIN_VASP_KPOINTS_EXPLICIT_START_STOP.str();
      ...
      const bool scell_verbose = aplopts.flag("SCELL::VERBOSE");
      aplopts.flag("SCELL::VERBOSE", false);
      const xvector<int> scell_dims_new = scell.determineSupercellDimensions(aplopts);
  ...
  bool runRelaxationsAPL_VASP(int start_relax, const string& AflowIn, _xvasp& xvasp, _aflags& aflags, _kflags& kflags, _vflags& vflags, ofstream& fileMessage) {
    ...
    if (Krun) {
      ...
      for (i = start_relax; Krun && i <= _NUM_RELAX_; i++) {
        aus << 11111 * i << " RELAXATION APL - " << xvasp.Directory << " - K=[" << xvasp.str.kpoints_k1 << " " << xvasp.str.kpoints_k2 << " " << xvasp.str.kpoints_k3 << "]" << " - " << kflags.KBIN_BIN << " - "
            << Message(__AFLOW_FILE__) << endl;
      ...
      if (Krun && (i == _NUM_RELAX_)) {
        aus << 11111 * i << " RELAXATION APL END - " << xvasp.Directory << " - K=[" << xvasp.str.kpoints_k1 << " " << xvasp.str.kpoints_k2 << " " << xvasp.str.kpoints_k3 << "]" << " - " << kflags.KBIN_BIN
            << " - " << Message(__AFLOW_FILE__) << endl;
      }
    }
    if (!Krun) {
    ...
    stringstream xstr;
    const string filename = aflags.Directory + "/CONTCAR." + _APL_RELAX_PREFIX_ + aurostd::utype2string<int>(_NUM_RELAX_); // ME20190308
    ...
    if (!vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("PRESERVE") && !vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.xscheme.empty()) { // ME20190109
  ...
  void VASP_RunPhonons_APL(_xvasp& xvasp, string AflowIn, _aflags& aflags, _kflags& kflags, _vflags& vflags, ofstream& FileMESSAGE, ostream& oss) {
    _xinput xinput(xvasp);
    _xflags xflags(vflags);
    RunPhonons_APL(xinput, AflowIn, aflags, kflags, xflags, FileMESSAGE, oss);
  }

  void RunPhonons_APL(_xinput& xinput, string AflowIn, _aflags& aflags, _kflags& kflags, _xflags& xflags, ofstream& FileMESSAGE, ostream& oss) {
    const bool LDEBUG = (false || XHOST.DEBUG);
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [0]" << std::endl;
    }
    // Test
    if (!(kflags.KBIN_PHONONS_CALCULATION_APL || kflags.KBIN_PHONONS_CALCULATION_QHA || kflags.KBIN_PHONONS_CALCULATION_AAPL)) {
      return; // PN20180705
    }
    ...
    if (xinput.AFLOW_MODE_VASP) {
      pflow::fixEmptyAtomNames(xinput.xvasp.str, true);
    }
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "RUNNING...", aflags, FileMESSAGE, oss);
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [1a]" << std::endl;
    }
    ...
    if (xinput.AFLOW_MODE_VASP) {
      ...
      if (!xflags.vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry && DEFAULT_VASP_FORCE_OPTION_BADER) {
        ...
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss);
      }
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [1b]" << std::endl;
    }

    /***************************** READ PARAMETERS *****************************/

    aurostd::xoption aplopts;
    const int USER_MAXSHELL = 0; // Not implemented yet
    for (size_t i = 0; i < kflags.KBIN_MODULE_OPTIONS.aplflags.size(); i++) {
    ...
    if (aplopts.flag("RELAX")) {
      ...
      if (xinput.AFLOW_MODE_VASP) {
        ...
        for (int i = 1; i <= _NUM_RELAX_; i++) {
          const string contcar = aflags.Directory + "/CONTCAR." + _APL_RELAX_PREFIX_ + aurostd::utype2string<int>(i);
          if (aurostd::CompressFileExist(contcar) || aurostd::FileExist(contcar)) { // ME20200103 - also look for uncompressed files
            START_RELAX++;
          } else {
            break;
          }
    ...
    if (aplopts.flag("DC")) {
      ...
      if (aplopts.getattachedscheme("DCPATH") == "LATTICE") {
        ...
        const string dc_initcoords_frac = aplopts.getattachedscheme("DCINITCOORDSFRAC");
        const string dc_initcoords_cart = aplopts.getattachedscheme("DCINITCOORDSCART");
    ...
    if (aplopts.flag("DOS") || aplopts.flag("TP")) { // ME20190423
      const string dosmethod = aplopts.getattachedscheme("DOSMETHOD");
      ...
      if ((USER_DOS_PROJECTIONS.empty()) || (dosmethod == "RS")) {
        message << " Projected phonon DOS will NOT be calculated.";
      } else {
        message << " Projected phonon DOS will be calculated along the " << (aplopts.flag("DOS_CART") ? "fractional" : "Cartesian") << " directions ";
        for (size_t i = 0; i < USER_DOS_PROJECTIONS.size(); i++) {
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [1c]" << std::endl;
    }
    ...
    if (kflags.KBIN_PHONONS_CALCULATION_AAPL) {
      for (size_t i = 0; i < kflags.KBIN_MODULE_OPTIONS.aaplflags.size(); i++) {
      ...
      const bool defaults = (!aaplopts.flag("CUT_RAD") && !aaplopts.flag("CUT_SHELL"));
      ...
      if (aaplopts.flag("KPPRA")) {
        message << "AAPL will use a KPPRA of " << aaplopts.getattachedscheme("KPPRA") << " for static calculations." << std::endl;
      }
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [2]" << std::endl;
    }
    ...
    if (aplopts.getattachedscheme("ENGINE") == "LR" || (aplopts.getattachedscheme("ENGINE") == string("DM") && aplopts.flag("POLAR"))) {
      if (xflags.AFLOW_MODE_VASP && !XHOST.GENERATE_AFLOWIN_ONLY) { // ME20190313 - Do not check the VASP binary for generate_aflowin_only
        try {
          ...
          const double vaspVersion = aurostd::VersionString2Double(KBIN::BIN2VASPVersion((kflags.KBIN_MPI ? kflags.KBIN_MPI_BIN : kflags.KBIN_BIN)));
          ...
          if (aurostd::isequal(vaspVersion, 0.0) == false) { // CO20210315
            ...
            message << "Failed.";
            pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_WARNING_);
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unexpected binary format.", _FILE_WRONG_FORMAT_);
          }
    ...
    if (aurostd::CompressFileExist(phposcar_file, phposcar_file)) {
    ...
    if (!aurostd::CompressFileExist(phposcar_file) && aplopts.flag("RELAX")) {
      ...
      bool Krun = true;
      if (xinput.AFLOW_MODE_VASP) {
        Krun = relaxStructureAPL_VASP(START_RELAX, AflowIn, aplopts, scell_dims, aplopts.flag("RELAX_COMMENSURATE"), xinput.xvasp, aflags, kflags, xflags.vflags, FileMESSAGE);
      }
      if (!Krun) {
    ...
    if (xflags.vflags.KBIN_VASP_FORCE_OPTION_NBANDS_EQUAL.isentry) {
      const int ncells = scell_dims[1] * scell_dims[2] * scell_dims[3];
    ...
    if (!aurostd::CompressFileExist(phposcar_file)) {
    ...
    if (kflags.KBIN_PHONONS_CALCULATION_QHA) {
      ...
      for (size_t i = 0; i < kflags.KBIN_MODULE_OPTIONS.qhaflags.size(); i++) {
        const string& key = kflags.KBIN_MODULE_OPTIONS.qhaflags[i].keyword;
        message << (kflags.KBIN_MODULE_OPTIONS.qhaflags[i].isentry ? "Setting" : "DEFAULT") << " " << _ASTROPT_ << key << "=" << kflags.KBIN_MODULE_OPTIONS.qhaflags[i].xscheme;
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss);
      ...
      // AS20200513 BEGIN
      apl::QHA qha(supercell.getInputStructureLight(), xinput, qhaopts, aplopts, FileMESSAGE, oss);
    ...
    const string hibfile = aurostd::CleanFileName(aflags.Directory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_HARMIFC_FILE);
    bool awakeHarmIFCs = (aplopts.flag("HIBERNATE") && aurostd::CompressFileExist(hibfile));
    bool apl_stagebreak = false;
    // Try to read first
    if (awakeHarmIFCs) {
      try {
        pflow::logger(__AFLOW_FILE__, "APL", "Awakening...", aflags, FileMESSAGE, oss);
        phcalc.awake();
      } catch (aurostd::xerror& e) {
        message << e.buildMessageString() << " Skipping awakening...";
        pflow::logger(__AFLOW_FILE__, "APL", message, aflags, FileMESSAGE, oss, _LOGGER_WARNING_);
        awakeHarmIFCs = false;
      }
    }
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [3b]" << std::endl;
    }
    ...
    if (kflags.KBIN_PHONONS_CALCULATION_AAPL) {
      const int max_order = (aaplopts.flag("FOURTH_ORDER") ? 4 : 3);
      for (int o = 3; o <= max_order; o++) {
        // Try and load IFCs from file
        const string ifcs_hib_file = aurostd::CleanFileName(aflags.Directory + "/" + DEFAULT_AAPL_FILE_PREFIX + _ANHARMONIC_IFCS_FILE_[o - 3]);
        bool awakeAnharmIFCs = (aplopts.flag("HIBERNATE") && aurostd::CompressFileExist(ifcs_hib_file));
        if (awakeAnharmIFCs) {
          try {
            ...
            if (o == 3) {
              ...
              message << aurostd::utype2string<int>(o) + "th";
            }
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [5a]" << std::endl;
    }

    apl::IPCFreqFlags frequencyFormat = apl::NONE;

    const string USER_FREQFORMAT = aurostd::toupper(aplopts.getattachedscheme("FREQFORMAT"));
    if (!USER_FREQFORMAT.empty()) {
      // Convert format to machine representation
      tokens.clear();
      aurostd::string2tokens(USER_FREQFORMAT, tokens, string(" |:;,"));
      for (size_t i = 0; i < tokens.size(); i++) {
        if (tokens[i] == string("OMEGA")) {
          frequencyFormat |= apl::OMEGA;
          continue;
        }
        if (tokens[i] == string("HERTZ")) {
          frequencyFormat |= apl::HERTZ;
          continue;
        } else if (tokens[i] == string("THZ")) {
          frequencyFormat |= apl::THZ;
          continue;
        } else if (tokens[i] == string("CM-1") || tokens[i] == string("RECIPROCAL_CM")) {
          frequencyFormat |= apl::RECIPROCAL_CM;
          continue;
        } else if (tokens[i] == string("MEV")) {
          frequencyFormat |= apl::MEV;
          continue;
        }
        if (tokens[i] == string("ALLOW_NEGATIVE")) {
      ...
      if (((frequencyFormat & ~apl::OMEGA) & ~apl::ALLOW_NEGATIVE) == apl::NONE) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Ambiguous frequency format.", _INPUT_AMBIGUOUS_);
      }
    } else {
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [5b]" << std::endl;
    }
    ...
    if (aplopts.flag("DC")) {
      ...
      if (aplopts.getattachedscheme("DCPATH") == "LATTICE") {
        ...
        pdisc.initPathCoords(aplopts.getattachedscheme("DCINITCOORDS"), aplopts.getattachedscheme("DCINITCOORDSLABELS"), aurostd::string2utype<int>(aplopts.getattachedscheme("DCPOINTS")), aplopts.flag("DCCORDS_CART"));
      }
      // ME20190501 Allow user to override path
      const string USER_DC_USERPATH = aplopts.getattachedscheme("DCUSERPATH");
      ...
      if (aplopts.getattachedscheme("DCPATH") == "LATTICE") {
        supercell.projectToOriginal(); // ME20200117 - reset to original
      }
    }
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [5c]" << std::endl;
    }

    if (aplopts.flag("DOS") || aplopts.flag("TP")) {
      // Calculate DOS
      if (!phcalc.getQMesh().initialized()) {
        phcalc.initialize_qmesh(USER_DOS_MESH);
      }
      ...
      if (aplopts.flag("TP")) {
        ...
        const double USER_TP_TSTART = aurostd::string2utype<double>(aplopts.getattachedscheme("TSTART"));
        const double USER_TP_TEND = aurostd::string2utype<double>(aplopts.getattachedscheme("TEND"));
        const double USER_TP_TSTEP = aurostd::string2utype<double>(aplopts.getattachedscheme("TSTEP"));
        ...
        if (aplopts.flag("GROUP_VELOCITY")) {
          const string gvelfile = aurostd::CleanFileName(aflags.Directory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_GVEL_FILE);
          message << "Writing group velocities into file " << gvelfile << ".";
          pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss);
          const vector<vector<xvector<double>>> gvel = phcalc.calculateGroupVelocitiesOnMesh();
        ...
        if (dosc.hasImaginaryFrequencies()) { // ME20200210
          ...
          for (i = 0; i < freqs.size(); i++) {
            if (freqs[i] > -_FLOAT_TOL_) {
              break;
            }
          }
          if (i > 0) {
            const double idos_percent = 100.0 * idos[i - 1] / idos.back();
            message << "There are imaginary frequencies in the phonon DOS, covering " << std::fixed << std::setprecision(1) << idos_percent << "\% of the integrated DOS. "
    ...
    if (!apl_outfile.str().empty()) {
      const string filename = aurostd::CleanFileName(aflags.Directory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_OUT_FILE);
      aurostd::stringstream2file(apl_outfile, filename);
      if (!aurostd::FileExist(filename)) {
        const string message = "Cannot open output file " + filename + ".";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_ERROR_);
      }
    }
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " DEBUG [6]" << std::endl;
    }
...
namespace apl {
  ...
  void validateParametersSupercellAPL(aurostd::xoption& aplopts) {
    ...
    if (aplopts.flag("SUPERCELL_ENTRY")) {
      vector<int> tokens;
      const string supercell_scheme = aplopts.getattachedscheme("SUPERCELL");
  ...
  void validateParametersDispersionsAPL(aurostd::xoption& aplopts) {
    ...
    if (scheme == "MANUAL") {
      ...
      const uint ncoords = tokens.size();
  ...
  void validateParametersDosAPL(aurostd::xoption& aplopts, const _aflags& aflags, ofstream& FileMESSAGE, ostream& oss) {
    ...
    // DOS Method
    const string dos_method = aurostd::toupper(aplopts.getattachedscheme("DOSMETHOD"));
    ...
    // DOS projections
    if (aplopts.flag("DOS_PROJECT")) {
      const string dos_proj_cart = aplopts.getattachedscheme("DOSPROJECTIONS_CART");
      const string dos_proj_frac = aplopts.getattachedscheme("DOSPROJECTIONS_FRAC");
      if (!dos_proj_frac.empty() || !dos_proj_cart.empty()) {
        if (!dos_proj_cart.empty() && !dos_proj_frac.empty()) {
          ...
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_AMBIGUOUS_);
        } else {
          string projscheme;
    ...
    if (aplopts.flag("TP")) {
      ...
      const double tstart = aurostd::string2utype<double>(tokens[0]);
      const double tend = aurostd::string2utype<double>(tokens[1]);
      const double tstep = aurostd::string2utype<double>(tokens[2]);
      if (tstart > tend) {
  ...
  void validateParametersAAPL(xoption& aaplopts, const _aflags& aflags, ofstream& FileMESSAGE, ostream& oss) {
    const string module = "AAPL";
    string scheme;
    string message;
    vector<string> tokens;
    const string _ASTROPT_ = "[AFLOW_AAPL]";
    ...
    const bool defaults = (!aaplopts.flag("CUT_SHELL") && !aaplopts.flag("CUT_RAD")); // flag is false when isentry was false
    const bool fourth_order = aaplopts.flag("FOURTH_ORDER");
    if (defaults || aaplopts.flag("CUT_RAD")) {
      aurostd::string2tokens(aaplopts.getattachedscheme("CUT_RAD"), tokens, " ,");
      if (tokens.empty()) {
        ...
        pflow::logger(__AFLOW_FILE__, module, message, aflags, FileMESSAGE, oss, _LOGGER_WARNING_);
      }
      if (aaplopts.flag("FOURTH_ORDER")) {
        if (tokens.size() == 1) {
    ...
    if (aaplopts.flag("CUT_RAD") != aaplopts.flag("CUT_SHELL")) {
      if (!aaplopts.flag("CUT_RAD")) {
        aaplopts.push_attached("CUT_RAD", (fourth_order ? "0.0,0.0" : "0.0"));
      }
      if (!aaplopts.flag("CUT_SHELL")) {
        aaplopts.push_attached("CUT_RAD", (fourth_order ? "0,0" : "0"));
      }
    }
    ...
    if (tokens.size() == 3) {
      ...
      const double USER_TCT_TEND = aurostd::string2utype<double>(tokens[1]);
      const double USER_TCT_TSTEP = aurostd::string2utype<double>(tokens[2]);
      ...
      if (USER_TCT_TSTART == 0) {
        ...
        pflow::logger(__AFLOW_FILE__, module, message, aflags, FileMESSAGE, oss);
  ...
  void validateParametersQHA(xoption& qhaopts, const _aflags& aflags, ofstream& FileMESSAGE, ostream& oss) {
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "Validating QHA parameters.", aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    ...
    if (dtokens.size() != 3) {
      ...
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, msg, _INPUT_NUMBER_);
    }
    ...
    // EOS_MODEL
    option = "EOS_MODEL";
    const string MODEL = aurostd::toupper(qhaopts.getattachedscheme(option));
    ...
    for (size_t i = 0; i < tokens.size(); i++) {
      ...
      if (token == "SJ") {
        qhaopts.flag("EOS_MODEL:SJ", true);
      }
      if (token == "BM2") {
        qhaopts.flag("EOS_MODEL:BM2", true);
      }
      if (token == "BM3") {
        qhaopts.flag("EOS_MODEL:BM3", true);
      }
      if (token == "BM4") {
        qhaopts.flag("EOS_MODEL:BM4", true);
      }
      if (token == "M") {
        qhaopts.flag("EOS_MODEL:M", true);
      }
    }

    // GP_DISTORTION
    const double gp_distortion = aurostd::string2utype<double>(qhaopts.getattachedscheme("GP_DISTORTION"));
    ...
    // MODE
    option = "MODE";
    const string MODE = aurostd::toupper(qhaopts.getattachedscheme(option));
    ...
    for (size_t i = 0; i < tokens.size(); i++) {
      token = aurostd::toupper(tokens[i]);
      if (token == "QHA") {
        qhaopts.flag("MODE:QHA", true);
      }
      if (token == "QHA3P") {
        qhaopts.flag("MODE:QHA3P", true);
      }
      if (token == "SCQHA") {
        qhaopts.flag("MODE:SCQHA", true);
      }
      if (token == "QHANP") {
        qhaopts.flag("MODE:QHANP", true);
      }
    }
    ...
    const int TAYLOR_EXPANSION_ORDER = aurostd::string2utype<int>(qhaopts.getattachedscheme("TAYLOR_EXPANSION_ORDER"));
...
namespace apl {
  ...
  void createAflowInPhononsAIMS(_aflags& _aflowFlags, _kflags& _kbinFlags, _xflags& _xFlags, string& _AflowIn, _xinput& xinp, ofstream& FileMESSAGE) {
    ...
    const _vflags vflags(_xFlags.vflags);
    _xaims xaims(xinp.xaims);
    _aimsflags aimsflags(_xFlags.aimsflags);

    const string directory = xinp.getDirectory();
    if (directory.empty()) {
    ...
    aurostd::Chmod(0777, directory); // CHMOD Directory 777
    ...
    const string geom_filename = xaims.Directory + "/" + AFLOWRC_DEFAULT_AIMS_EXTERNAL_GEOM;
    ...
    aurostd::Chmod(0666, geom_filename, std::filesystem::perm_options::add);

    // CO START
    const string filename = directory + string("/") + _AFLOWIN_;
    aurostd::stringstream2file(outfile, filename);
    if (!aurostd::FileExist(filename)) {
      message = "Cannot create [" + _AFLOWIN_ + "] file.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_ERROR_);
    }
    aurostd::Chmod(0666, filename, std::filesystem::perm_options::add); // CHMOD a+rw _AFLOWIN_
    // CO END
  }
...
namespace apl {
  ...
  bool outfileFoundAnywherePhonons(vector<_xinput>& xinps) {
    for (size_t idxRun = 0; idxRun < xinps.size(); idxRun++) {
      const string dir = xinps[idxRun].getDirectory();

      if (xinps[idxRun].AFLOW_MODE_VASP) {
        if (aurostd::CompressFileExist(dir + string("/vasprun.xml.static")) || aurostd::CompressFileExist(dir + string("/vasprun.xml")) || aurostd::CompressFileExist(dir + "/" + DEFAULT_AFLOW_QMVASP_OUT)) { // ME20190607
          return true;
        }
      }
      if (xinps[idxRun].AFLOW_MODE_AIMS) {
        if (aurostd::CompressFileExist(xinps[idxRun].getDirectory() + string("/aims.out"))) {
          return true;
        }
      }
    }
  ...
  bool filesExistPhonons(_xinput& xinp) {
    const string dir = xinp.getDirectory() + string("/");
    if (aurostd::FileExist(dir + _AFLOWIN_)) {
      return true; // do not OVERWRITE an aflow.in
    }
    if (xinp.AFLOW_MODE_VASP) {
      if (aurostd::CompressFileExist(dir + string("vasprun.xml.static")) || aurostd::CompressFileExist(dir + string("vasprun.xml")) || aurostd::CompressFileExist(dir + DEFAULT_AFLOW_QMVASP_OUT)) { // ME20200203 - Added qmvasp file
        return true;
      }
    }
    if (xinp.AFLOW_MODE_AIMS) {
      if (aurostd::CompressFileExist(dir + string("aims.out"))) {
        return true;
      }
    }
    return false;
  ...
  bool outfileFoundEverywherePhonons(vector<_xinput>& xinps, const string& directory, ofstream& FileMESSAGE, ostream& oss, bool contains_born) {
    const string mode = xinps[0].xvasp.AVASP_arun_mode;
    stringstream _logger;
    _logger << "Reading force files.";
    pflow::logger(__AFLOW_FILE__, mode, _logger, directory, FileMESSAGE, oss);
    ...
    for (uint idxRun = 0; idxRun < ninps; idxRun++) {
      ...
      pflow::logger(__AFLOW_FILE__, mode, _logger, xinps[idxRun].getDirectory(), FileMESSAGE, oss);
      const string directory = xinps[idxRun].getDirectory();
      ...
      if (!xinps[idxRun].getXStr().qm_calculated) {
        const string message = "The force file in " + directory + " is wrong.";
    ...
    pflow::logger(__AFLOW_FILE__, mode, _logger, directory, FileMESSAGE, oss, _LOGGER_COMPLETE_);
    return true;
  }

  bool readForcesFromDirectory(_xinput& xinp) {
    const uint natoms = xinp.getXStr().atoms.size();
    xinp.getXStr().qm_forces.clear();
    // Load data....
    if (xinp.AFLOW_MODE_VASP) {
      if (!aurostd::CompressFileExist(xinp.getDirectory() + "/" + DEFAULT_AFLOW_QMVASP_OUT) && !aurostd::CompressFileExist(xinp.getDirectory() + "/vasprun.xml.static") &&
          !aurostd::CompressFileExist(xinp.getDirectory() + "/vasprun.xml")) {
        return false;
      }
      // ME20190607 BEGIN
      //  Read forces from aflow qmvasp file - much faster
      string file = xinp.getDirectory() + "/" + DEFAULT_AFLOW_QMVASP_OUT;
      if (aurostd::CompressFileExist(file)) {
        const xQMVASP qmvasp(file);
        ...
        for (size_t i = 0; i < vasprunxml.vforces.size(); i++) {
          xinp.getXStr().qm_forces.push_back(vasprunxml.vforces[i]);
        }
      }
      if (xinp.getXStr().qm_forces.size() == natoms) {
        xinp.getXStr().qm_calculated = true;
      }
      return true;
    } else if (xinp.AFLOW_MODE_AIMS) {
      if (!aurostd::CompressFileExist(xinp.getDirectory() + string("/aims.out"))) {
        return false;
      }
      xAIMSOUT xaimsout(xinp.getDirectory() + "/aims.out");
      for (size_t i = 0; i < xaimsout.vforces.size(); i++) {
        xinp.getXStr().qm_forces.push_back(xaimsout.vforces[i]);
      }
      if (xinp.getXStr().qm_forces.size() == natoms) {
        xinp.getXStr().qm_calculated = true;
      }
  ...
  void subtractZeroStateForces(vector<_xinput>& xinps, bool contains_born) {
    ...
    const uint natoms = xinps[ninps].getXStr().atoms.size();
    for (uint idxRun = 0; idxRun < ninps; idxRun++) {
      if (xinps[idxRun].getXStr().atoms.size() != natoms) {
        const string message = "Structure and ZEROSTATE structure do not have the same number of atoms.";
  ...
  void subtractZeroStateForces(vector<_xinput>& xinps, _xinput& zerostate) {
    string message;
    const uint natoms = zerostate.getXStr().atoms.size();
...
namespace apl {
  ...
  bool APL_Get_AflowInName(string& AflowInName, const string& directory_LIB) {
    ...
    for (size_t i = 0; i < vaflowins.size(); i++) {
      ...
      if (aurostd::substring2bool(aflowin, _ASTROPT_APL_ + "CALC", true) || aurostd::substring2bool(aflowin, _ASTROPT_APL_OLD_ + "CALC", true)) {
