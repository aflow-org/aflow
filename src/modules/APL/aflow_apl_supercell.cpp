...
namespace apl {
  ...
  void Supercell::initialize(const xstructure& _xstr, bool VERBOSE) {
    ...
    const string tmp_dir = _directory;  // Do not delete directory
    clear();
    _directory = tmp_dir;
    ...
    if (LDEBUG) { // CO20190218
      const bool write_inequivalent_flag = _inStructure.write_inequivalent_flag;
  ...
  void Supercell::calculateWholeSymmetry(xstructure& xstr, bool VERBOSE) {
    ...
    { // CO20200106 - patching for auto-indenting
      const string message = "Symmetry routine failed.";
  ...
  void Supercell::reset() {
    ...
    for (size_t i = 0; i < _scStructure.num_each_type.size(); i++) {
      _scStructure.num_each_type[i] = 0;
    }

    for (size_t i = 0; i < _scStructure.iatoms.size(); i++) {
  ...
  xvector<int> Supercell::determineSupercellDimensions(const aurostd::xoption& opts) {
    ...
    xvector<int> dims(3);
    const string method = opts.getattachedscheme("SUPERCELL::METHOD");
    if (method.empty()) {
      message << "Supercell method empty.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ILLEGAL_);
    }
    const string value = opts.getattachedscheme("SUPERCELL::VALUE");
    if (value.empty()) {
    ...
    if (method == "SUPERCELL") {
      ...
      const int minatoms = aurostd::string2utype<int>(value);
      ...
      if (opts.flag("SUPERCELL::VERBOSE")) {
        message << "Radius=" << aurostd::PaddedPOST(aurostd::utype2string<double>(radius, 3), 4) << " supercell=" << dims[1] << "x" << dims[2] << "x" << dims[3] << " natoms=" << natoms;
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
      }
      ...
      dims[3] = Ni;
      if (opts.flag("SUPERCELL::VERBOSE")) {
        message << "Ni=" << Ni << " supercell=" << Ni << "x" << Ni << "x" << Ni << " natoms=" << natoms;
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
      }
    } else if (method == "SHELLS") {
      const int shells = aurostd::string2utype<int>(value);
      if (opts.flag("SUPERCELL::VERBOSE")) {
        message << "Searching for suitable cell to handle " << shells << " shells...";
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss);
      }
      const bool get_full_shells = true;
      dims = getSupercellDimensionsShell(shells, get_full_shells);
    } else {
      message << "Unknown supercell method " + method + ".";
      aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ILLEGAL_);
    }
    return dims;
  }
  ...
  void Supercell::build(aurostd::xoption& opts, bool VERBOSE) {
    ...
    const xvector<int> dims = determineSupercellDimensions(opts);
    build(dims);
  }
  ...
  void Supercell::build(const xvector<int>& dims, bool VERBOSE) {
    ...
    const bool get_full_sym = false; // GETFULLSYMBASIS;  //( GETFULLSYMBASIS || _derivative_structure ); //CO

    // Print info
    if (dims.rows == 3) {
      const string dimstring = aurostd::joinWDelimiter(dims, "x");
      ...
      const xmatrix<int> scmat = aurostd::reshape(dims, 3, 3);
      const string dimstring = aurostd::xmatDouble2String(aurostd::xmatrixutype2double(scmat), 0);
      _scStructure.info = "Supercell " + dimstring;
      if (VERBOSE) {
        message << "The supercell is going to build as [" << dimstring << "]";
    ...
    _scStructure = GetSuperCell(_inStructure, dims, _sc2pcMap, _pc2scMap, true, get_full_sym, false,
                                true); // now gets symmetries too! no need for full_basis (just a check) //CO20190409 - force_supercell_matrix==false as we might have a derivative structure, force_strict_pc2scMap==true because we want to map to true primitive cell, no equivalent atoms
  ...
  void Supercell::trimStructure(int n, const xvector<double>& a, const xvector<double>& b, const xvector<double>& c, bool constructSymmetry) {
    ...
    for (size_t ia = 0; ia < _inStructure.iatoms.size(); ia++) {
      for (size_t iia = 0; iia < _inStructure.iatoms[ia].size(); iia++) {
        ...
        for (int i = 0; i <= p; i++) {
          for (int j = 0; j <= p; j++) {
            for (int k = 0; k <= p; k++) {
              ...
              cshift = (((double) i) * _inStructure.lattice(1) + ((double) j) * _inStructure.lattice(2) + ((double) k) * _inStructure.lattice(3));
              atom.cpos = atom.cpos + cshift;
              atom.fpos = C2F(_scStructure.lattice, atom.cpos);

              // Add only atoms inside the cell
              if (atom.fpos(1) > 1.0 - _FLOAT_TOL_ || atom.fpos(2) > 1.0 - _FLOAT_TOL_ || atom.fpos(3) > 1.0 - _FLOAT_TOL_ || atom.fpos(1) < 0.0 - _FLOAT_TOL_ || atom.fpos(2) < 0.0 - _FLOAT_TOL_ ||
                  atom.fpos(3) < 0.0 - _FLOAT_TOL_) {
                continue;
              }
              ...
              if (i == 0 && j == 0 && k == 0) {
                _pc2scMap.push_back(_scStructure.atoms.size() - 1);
              }
            }
          }
        }
    ...
    if (constructSymmetry) {
      for (size_t l = 0; l < _inStructure.fgroup.size(); l++) {
        for (int i = 0; i < p; i++) {
          for (int j = 0; j < p; j++) {
            for (int k = 0; k < p; k++) {
              // Create position of new atoms...
              ...
              if (symOp.ftau(1) > 1.0 - _FLOAT_TOL_ || symOp.ftau(2) > 1.0 - _FLOAT_TOL_ || symOp.ftau(3) > 1.0 - _FLOAT_TOL_ || symOp.ftau(1) < 0.0 - _FLOAT_TOL_ || symOp.ftau(2) < 0.0 - _FLOAT_TOL_ ||
                  symOp.ftau(3) < 0.0 - _FLOAT_TOL_) {
                continue;
              }
    ...
    for (int i = 0; i < (int) _scStructure.num_each_type.size(); i++) {
      const int end = start + _scStructure.num_each_type[i];
      for (int j = start; j < end - 1; j++) {
        if (aurostd::abs(_scStructure.atoms[j].fpos(1)) < _FLOAT_TOL_) {
          _scStructure.atoms[j].fpos(1) = 0.0;
        }
        if (aurostd::abs(_scStructure.atoms[j].fpos(2)) < _FLOAT_TOL_) {
          _scStructure.atoms[j].fpos(2) = 0.0;
        }
        if (aurostd::abs(_scStructure.atoms[j].fpos(3)) < _FLOAT_TOL_) {
          _scStructure.atoms[j].fpos(3) = 0.0;
        }

        for (int k = j + 1; k < end; k++) {
          if (aurostd::abs(_scStructure.atoms[k].fpos(1)) < _FLOAT_TOL_) {
            _scStructure.atoms[k].fpos(1) = 0.0;
          }
          if (aurostd::abs(_scStructure.atoms[k].fpos(2)) < _FLOAT_TOL_) {
            _scStructure.atoms[k].fpos(2) = 0.0;
          }
          if (aurostd::abs(_scStructure.atoms[k].fpos(3)) < _FLOAT_TOL_) {
            _scStructure.atoms[k].fpos(3) = 0.0;
          }

          if (_scStructure.atoms[k].fpos(1) < _scStructure.atoms[j].fpos(1)) {
            const _atom ta = _scStructure.atoms[j];
  ...
  bool Supercell::projectToPrimitive() {
    ...
    vector<int> sc2pc;
    xstructure pcell;
    if (!_pcStructure.iatoms_calculated) {
      SYM::CalculateInequivalentAtoms(_pcStructure);
    }
    ...
    if (_inStructure_original.atoms.size() == pcell.atoms.size()) {
      const xmatrix<double> U = trasp(_inStructure_original.scale * _inStructure_original.lattice) * inverse(trasp(pcell.scale * _pcStructure.lattice));
    ...
    if (!mapped) {
      ...
      if (LDEBUG) {
        std::cerr << __AFLOW_FUNC__ << " Not mapped succesfully. Try shifting atoms." << std::endl;
      }
      xvector<double> ones(3);
      ones.set(1.0);
      for (size_t at = 0; at < pcell.atoms.size(); at++) {
    ...
    if (mapped) {
      ...
      const uint niatoms_pc = iatoms_pc.size();
      const uint niatoms_oc = iatoms_oc.size();
      pcell.iatoms.clear();
      ...
      for (uint iatpc = 0; ((iatpc < niatoms_pc) && mapped); iatpc++) {
        ...
        for (iatoc = 0; iatoc < niatoms_oc; iatoc++) {
          ...
          for (i = 0; i < iatoms_oc[iatoc].size(); i++) {
            if (SYM::FPOSMatch(fpos, _inStructure_original.atoms[iatoms_oc[iatoc][i]].fpos, _inStructure_original.lattice, _inStructure_original.f2c, _skew, _sym_eps)) {
              pcell.iatoms[iatoc] = _pcStructure.iatoms[iatpc];
              for (size_t j = 0; j < iatoms_pc[iatpc].size(); j++) {
                pcell.atoms[iatoms_pc[iatpc][j]].index_iatoms = iatoc;
              }
              break;
            }
          }
          if (i < iatoms_oc[iatoc].size()) {
            break;
          }
        }
        if (iatoc == niatoms_oc) {
          if (LDEBUG) {
            std::cerr << __AFLOW_FUNC__ << " Did not map iatoms of primitive cell (failed for " << iatpc << ")." << std::endl;
          }
          mapped = false;
        }
      }
  ...
  bool Supercell::getMaps(const xstructure& pcell, const xstructure& ocell, const xstructure& scell, vector<int>& pc2sc, vector<int>& sc2pc) const {
    const bool LDEBUG = (false || XHOST.DEBUG);
    const uint natoms_pc = pcell.atoms.size();
    const uint natoms_oc = ocell.atoms.size();
    const uint natoms_sc = scell.atoms.size();
    ...
    pc2sc.clear();
    pc2sc.resize(natoms_pc);
    sc2pc.clear();
    sc2pc.resize(natoms_sc);
    xvector<double> fpos(3);
      ...
      for (j = 0; j < natoms_oc; j++) {
        if (SYM::FPOSMatch(fpos, ocell.atoms[j].fpos, ocell.lattice, ocell.f2c, _skew, _sym_eps)) {
          ...
          for (k = 0; k < natoms_sc; k++) {
            if (aurostd::isequal(ocell.atoms[j].cpos, scell.atoms[k].cpos)) {
              break;
            }
          }
  ...
  xvector<int> Supercell::getSupercellDimensionsShell(uint cut_shell, bool get_full_shells) {
    ...
    const uint natoms_iat = _inStructure.iatoms.size();
    ...
    xvector<int> dims(3);
    xvector<int> dims_prev(3);
    xvector<double> cpos_image;
    ...
    for (; !all_shells_in_scell; radius += 0.01) {
      dims = LatticeDimensionSphere(_inStructure.lattice, radius);
      if (dims == dims_prev) {
        continue; // No need to check if the supercell dimensions have not changed
      }
      dims_prev = dims;
      ...
      for (iat = 0; iat < natoms_iat; iat++) {
        if (!shells_in_scell[iat]) { // Only check for atoms for which the cell was not big enough yet
          ...
          for (gat = 1; (gat < ngridatoms) && (countshell < cut_shell); gat++) {
            if (distances[gat] > distances[gat - 1] + _APL_SHELL_TOL_) {
              countshell++;
            }
          }
        ...
        if (!shells_in_scell[iat]) {
          break; // No need to check other atoms - it has to fit for all
        }
      }
  ...
  void Supercell::setupShellRestrictions(int MAX_NN_SHELLS) {
    const uint niatoms = _scStructure.iatoms.size();
    const uint natoms = _scStructure.atoms.size();
    const uint iat = 0;
    uint at = 0;
    ...
    for (uint iat = 0; iat < niatoms; iat++) {
      ...
      for (at = 1; (at < natoms) && (countshell < MAX_NN_SHELLS); at++) {
        if (distances[at] > distances[at - 1] + _APL_SHELL_TOL_) {
          countshell++;
        }
      }
      if (countshell < MAX_NN_SHELLS) {
        ...
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, _directory, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
        break;
      }
  ...
  bool Supercell::isConstructed() const {
  ...
  uint Supercell::getNumberOfEquivalentAtomsOfType(int i) const { // CO20190218
    ...
    if (i >= (int) _scStructure.iatoms.size()) {
      const string message = "Wrong index " + aurostd::utype2string<int>(i) + ".";
...
#ifndef __OPTIMIZE
    if (i >= (int) _scStructure.iatoms.size()) {
      const string message = "Wrong index[1] " + aurostd::utype2string<int>(i) + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_BOUNDS_);
    }

    if (j >= (int) _scStructure.iatoms[i].size()) {
      const string message = "Wrong index[2] " + aurostd::utype2string<int>(i) + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_BOUNDS_);
    }
#endif
  ...
  int Supercell::atomGoesTo(const _sym_op& symOp, int atomID, int centerID, bool translate) { // CO20190218
    ...
    if (atomID >= (int) _scStructure.atoms.size()) {
      const string message = "Wrong atomID index " + aurostd::utype2string<int>(atomID) + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_BOUNDS_);
    }

    if (centerID >= (int) _scStructure.atoms.size()) {
      const string message = "Wrong centerID index " + aurostd::utype2string<int>(centerID) + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_BOUNDS_);
    }
#endif
    ...
    if (symOp.basis_map_calculated) {
      return symOp.basis_atoms_map[atomID];
    }

    // Get the center atom center...
    if (translate && symOp.is_agroup) {
      center(centerID);
    }
    ...
    if (!SYM::ApplyAtomValidate(_scStructure.atoms[atomID], rotatedAtom, symOp, _scStructure, _skew, true, false, _sym_eps)) {
      const string message = "Illegitimate mapping.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
...
#endif
      string const message = "Mapping failed.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
  ...
  int Supercell::atomComesFrom(const _sym_op& symOp, int atomID, int centerID, bool translate) { // CO20190218
    ...
    // ME20191219 - use basis_atoms_map
    if (symOp.basis_map_calculated) {
      const int natoms = (int) _scStructure.atoms.size();
      for (int at = 0; at < natoms; at++) {
        if (symOp.basis_atoms_map[at] == atomID) {
          return at;
        }
      }
  ...
  const _sym_op& Supercell::getSymOpWhichMatchAtoms(int whichAtomID, int toAtomID, int GROUP) {
    ...
    if (whichAtomID >= (int) _scStructure.atoms.size()) {
      const string message = "Wrong atom1ID index " + aurostd::utype2string<int>(whichAtomID) + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_BOUNDS_);
    }

    if (toAtomID >= (int) _scStructure.atoms.size()) {
      const string message = "Wrong atom2ID index " + aurostd::utype2string<int>(toAtomID) + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INDEX_BOUNDS_);
    }
#endif

    vector<_sym_op>* symPool = nullptr;
    if (GROUP == _PGROUP_) {
      symPool = &_scStructure.pgroup;
    } else if (GROUP == _FGROUP_) {
      symPool = &_scStructure.fgroup;
    } else {
      const string message = "Unknown group type.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    ...
    if (iSymOp == symPool->size()) {
      ...
      for (; l < symPool->size(); l++) {
        ...
        // DX                               _scStructure, true, false, _derivative_structure);  //CO no roff
        _atom newAtom = SYM::ApplyAtom(_scStructure.atoms[whichAtomID], (*symPool)[l], _scStructure, true, false); // CO no roff
  ...
  void Supercell::center(int i) {
    ...
    for (size_t i = 0; i < _inStructure.atoms.size(); i++) {
    ...
    for (size_t i = 0; i < _scStructure.atoms.size(); i++) {
  ...
  void Supercell::center_original() {
  ...
  // ME20190715 - added const to use function with const Supercell &
  const vector<_sym_op>& Supercell::getFGROUP() const {
  ...
  // ME20191219
  bool Supercell::fullBasisCalculatedAGROUP() const {
    const uint natoms = _scStructure.atoms.size();
    for (uint at = 0; at < natoms; at++) {
      const vector<_sym_op>& agroup = getAGROUP(at);
      for (size_t symop = 0; symop < agroup.size(); symop++) {
        if (!agroup[symop].basis_map_calculated) {
          return false;
        }
      }
    }
  ...
  // ME20190715 - added const to use function with const Supercell &
  const vector<vector<_sym_op>>& Supercell::getAGROUP() const {
  ...
  // ME20190715 - added const to use function with const Supercell &
  bool Supercell::isDerivativeStructure() const {
  ...
  double Supercell::getUniqueAtomMass(int i) const {
    ...
    const double mass = GetAtomMass(_scStructure.atoms[_scStructure.iatoms[i][0]].atomic_number); // ME20190111
    if (mass == NNN) {
      const string message = "Unknown atom types.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ILLEGAL_);
    }
  ...
  void Supercell::calculatePhaseVectors() {
    const uint natoms_in = _inStructure.atoms.size();
    const uint natoms_sc = _scStructure.atoms.size();
    phase_vectors.clear();
  ...
  bool Supercell::calcShellPhaseFactor(int atomID, int centerID, const xvector<double>& qpoint, xcomplex<double>& phase, int& multiplicity, xvector<xcomplex<double>>& derivative, bool calc_derivative) {
