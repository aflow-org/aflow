...
namespace KBIN {
  ...
  uint relaxStructureAEL_VASP(const string& AflowIn, _xvasp& xvasp, _aflags& aflags, _kflags& kflags, _vflags& vflags, ofstream& FileMESSAGE) {
...
namespace AEL_functions {
  ...
  // See Computer Physics Communications 158, 57-72 (2004), Journal of Molecular Structure (Theochem) 368, 245-255 (1996), Phys. Rev. B 90, 174107 (2014) and Phys. Rev. Materials 1, 015401 (2017) for details
  uint Get_ElasticProperties_AEL_postprocess(const string& directory,
                                             ...
                                             vector<vector<double>>& ael_elastic_tensor,
                                             vector<vector<double>>& ael_compliance_tensor) {
    // Class to contain AEL input and output data
    _AEL_data AEL_data;
    ostringstream aus;
    _xvasp xvasp;
    ...
    const vector<double> elastic_tensor_row;
    string AflowInName;
    string FileLockName;
...
namespace AEL_functions {
  uint Get_PoissonRatio(_xvasp& xvasp, const string& AflowIn, _aflags& aflags, _kflags& kflags, _vflags& vflags, double& Poissonratio, bool postprocess, ofstream& FileMESSAGE) {
    ...
    // Call RunElastic_AEL to run AEL
    const uint aelerror = AEL_functions::RunElastic_AEL(xvasp, AflowIn, aflags, kflags, vflags, AEL_data, FileMESSAGE);
    if (!AEL_data.mechanically_stable) {
      aurostd::StringstreamClean(aus);
      aus << _AELSTR_WARNING_ + "Negative stiffness tensor eigenvalues indicate mechanical instability" << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
...
namespace AEL_functions {
  uint Get_ElasticTensor_xmatrix(_xvasp& xvasp, const string& AflowIn, _aflags& aflags, _kflags& kflags, _vflags& vflags, xmatrix<double> ElasticTensor, ofstream& FileMESSAGE) {
    ...
    if (aelerror == 0) {
      ...
      aus << _AELSTR_MESSAGE_ + "AEL Elastic run completed successfully!" << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      for (size_t i = 0; i < AEL_data.elastic_tensor.size(); i++) {
        for (size_t j = 0; j < AEL_data.elastic_tensor[i].size(); j++) {
          ElasticTensor[i + 1][j + 1] = AEL_data.elastic_tensor[i][j];
        }
      }
    } else if (aelerror == 8) {
      aurostd::StringstreamClean(aus);
...
namespace AEL_functions {
  ...
  uint RunElastic_AEL(_xvasp& xvasp, const string& AflowIn, _aflags& aflags, _kflags& kflags, _vflags& vflags, _AEL_data& AEL_data, ofstream& FileMESSAGE) {
    ...
    aurostd::xoption USER_NEG_STRAINS;
    USER_NEG_STRAINS.option = true;
    aurostd::xoption USER_STRAIN_SYMMETRY;
    USER_STRAIN_SYMMETRY.option = false;
    aurostd::xoption USER_CHECK_ELASTIC_SYMMETRY;
    USER_CHECK_ELASTIC_SYMMETRY.option = true;
    aurostd::xoption USER_ORIGIN_STRAIN_FIT;
    USER_ORIGIN_STRAIN_FIT.option = false;
    aurostd::xoption USER_ORIGIN_STRAIN_CALC;
    USER_ORIGIN_STRAIN_CALC.option = false;
    aurostd::xoption USER_RELAXED_STRUCT_FIT;
    USER_RELAXED_STRUCT_FIT.option = false;
    aurostd::xoption USER_VASPRUNXML_STRESS;
    USER_VASPRUNXML_STRESS.option = false;
    ...
    aurostd::xoption USER_RELAX;
    USER_RELAX.option = false;
    ...
    aurostd::xoption USER_AFLOWIN_OVERWRITE;
    USER_AFLOWIN_OVERWRITE.option = false;
    aurostd::xoption USER_AUTOSKIP_FAILED_ARUNS;
    USER_AUTOSKIP_FAILED_ARUNS.option = false;
    aurostd::xoption USER_PRESSURE_CALC;
    USER_PRESSURE_CALC.option = false;
    aurostd::xoption USER_USE_STABLE_PRESSURE;
    USER_USE_STABLE_PRESSURE.option = false;
    aurostd::xoption USER_SYMMETRIZE;
    USER_SYMMETRIZE.option = false;
    aurostd::xoption USER_VASPSYM;
    USER_VASPSYM.option = false;
    ...
    vector<double> PressureVolumes;
    vector<double> VolumeScaleFactors;
    ...
    if (USER_PRESSURE_CALC.option) {
      ...
      if (aelerror == 0) {
        ...
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        for (size_t k = 0; k < Pressure.size(); k++) {
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "Pressure = " << Pressure[k] << "GPa, Volume = " << PressureVolumes[k] << "Ang^3" << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
      } else if (aelerror == 8) {
        aurostd::StringstreamClean(aus);
    ...
    if (!USER_SKIP_FAILED_ARUNS.empty()) {
      ...
      for (size_t i = 0; i < AEL_data.failed_arun_list.size(); i++) {
        aurostd::StringstreamClean(aus);
        aus << _AELSTR_MESSAGE_ << "Failed ARUN directory = " << AEL_data.failed_arun_list[i] << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
    }
    ...
    if (USER_SPECIES_MASS.option) {
      ...
      for (size_t i = 0; i < xvasp.str.species.size(); i++) {
        AEL_data.cellmasskg = AEL_data.cellmasskg + xvasp.str.num_each_type[i] * GetAtomMass(xvasp.str.species[i]);
      }
      ...
      for (size_t i = 0; i < xvasp.str.species.size(); i++) {
        for (int j = 0; j < xvasp.str.num_each_type[i]; j++) {
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ << "Atom " << ij << " = " << xvasp.str.species[i] << endl;
          aus << _AELSTR_MESSAGE_ << "Atom " << ij << " = " << xvasp.str.species_pp[i] << endl;
          aus << _AELSTR_MESSAGE_ << "Atom " << ij << " = " << xvasp.str.species_pp_type[i] << endl;
          aus << _AELSTR_MESSAGE_ << "Atom " << ij << " = " << xvasp.str.species_pp_version[i] << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          ij++;
        }
      }
      ...
      for (size_t i = 0; i < xvasp.str.atoms.size(); i++) {
        AEL_data.cellmasskg = AEL_data.cellmasskg + GetAtomMass(xvasp.str.atoms[i].cleanname);
      }
      // Write out the names of the atoms used in the calculation
      for (size_t i = 0; i < xvasp.str.atoms.size(); i++) {
        aurostd::StringstreamClean(aus);
        aus << _AELSTR_MESSAGE_ << "Atom " << i << " = " << xvasp.str.atoms[i].cleanname << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
    }
    ...
    try {
      ...
      if (USER_NEG_STRAINS.option) {
        ...
        for (int i = nstrainstepshalf; i < (USER_NNORMAL_STRAINS - nstrainstepshalf); i++) {
          AEL_data.normal_deformations_half.push_back(AEL_data.normal_deformations[i]);
        }
        ...
        for (int i = nstrainstepshalf; i < (USER_NSHEAR_STRAINS - nstrainstepshalf); i++) {
          AEL_data.shear_deformations_half.push_back(AEL_data.shear_deformations[i]);
        }
      } else {
        aurostd::StringstreamClean(aus);
      ...
      for (size_t i = 1; i <= AEL_data.normal_strain.size(); i++) {
        for (size_t j = 0; j < AEL_data.normal_deformations.size(); j++) {
          ...
          strain_matrix[i][i] = strain_matrix[i][i] + AEL_data.normal_deformations[j];
          AEL_data.normal_strain[i - 1].push_back(strain_matrix);
        }
      }
      ...
      for (size_t i = 1; i <= AEL_data.shear_strain.size(); i++) {
        ...
        for (size_t j = 0; j < AEL_data.shear_deformations.size(); j++) {
          strain_matrix = aurostd::identity(strain_matrix);
          strain_matrix[k][l] = strain_matrix[k][l] + AEL_data.shear_deformations[j];
          AEL_data.shear_strain[i - 1].push_back(strain_matrix);
        }
      }
      ...
      if (USER_PRESSURE_CALC.option) {
        for (size_t i = 0; i < VolumeScaleFactors.size(); i++) {
          compressedstructure = initialstructure;
          compressedstructure.InflateVolume(VolumeScaleFactors[i]);
          pressurestructures.push_back(compressedstructure);
        }
        for (size_t i = 0; i < VolumeScaleFactors.size(); i++) {
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "Pressure = " << Pressure[i] << ", Volume = " << PressureVolumes[i] << endl;
          aus << _AELSTR_MESSAGE_ + "VolumeScaleFactor = " << VolumeScaleFactors[i] << ", Rescaled Volume = " << pressurestructures[i].Volume() << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
      }
      ...
      for (size_t i = 1; i <= AEL_data.normal_strain.size(); i++) {
        for (size_t j = 0; j < AEL_data.normal_deformations.size(); j++) {
          ...
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "AEL_data.normal_strain[i-1][j] = " << endl;
          aus << AEL_data.normal_strain[i - 1][j] << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          strainedstructure.lattice = strainedstructure.lattice * AEL_data.normal_strain[i - 1][j];
          vaspRuns.push_back(vaspRun);
          const uint idVaspRun = vaspRuns.size() - 1;
          vaspRuns[idVaspRun].str = strainedstructure;
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "idVaspRun = " << idVaspRun << endl;
          aus << _AELSTR_MESSAGE_ + "vaspRuns[idVaspRun].str.lattice = " << endl;
          aus << vaspRuns[idVaspRun].str.lattice << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          ...
          dirrunname.push_back(AEL_data.dirpathname + "/" + runname[idVaspRun]);
          vaspRuns[idVaspRun].str = strainedstructure;
          AEL_data.strain_matrix_list.push_back(AEL_data.normal_strain[i - 1][j]);
          vaspRuns[idVaspRun].AVASP_arun_runname = arunname[idVaspRun];
          vaspRuns[idVaspRun].AVASP_arun = true;
          vaspRuns[idVaspRun].AVASP_arun_mode = "AEL";
        }
      }
      ...
      for (size_t i = 1; i <= AEL_data.shear_strain.size(); i++) {
        for (size_t j = 0; j < AEL_data.shear_deformations.size(); j++) {
          ...
          aus << _AELSTR_MESSAGE_ + "AEL_data.shear_strain[i-1][j] = " << endl;
          aus << AEL_data.shear_strain[i - 1][j] << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          strainedstructure.lattice = strainedstructure.lattice * AEL_data.shear_strain[i - 1][j];
          aurostd::StringstreamClean(aus);
          ...
          dirrunname.push_back(AEL_data.dirpathname + "/" + runname[idVaspRun]);
          vaspRuns[idVaspRun].str = strainedstructure;
          AEL_data.strain_matrix_list.push_back(AEL_data.shear_strain[i - 1][j]);
          vaspRuns[idVaspRun].AVASP_arun_runname = arunname[idVaspRun];
          vaspRuns[idVaspRun].AVASP_arun = true;
          vaspRuns[idVaspRun].AVASP_arun_mode = "AEL";
        }
      ...
      if (USER_PRESSURE_CALC.option) {
        if (aurostd::abs(Pressure[0]) > tolzero) {
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_ERROR_ + "Calculation of elastic constants at finite pressure" << endl;
          aus << _AELSTR_ERROR_ + "First pressure value = " << Pressure[0] << " > 0.0" << endl;
        ...
        for (size_t k = 0; k < Pressure.size(); k++) {
          ...
          compressedstructure = pressurestructures[k];
          string strpressure;
          ostringstream cnvstrpressure;
          cnvstrpressure << Pressure[k];
          strpressure = cnvstrpressure.str();
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "creating normal strained structures at P = " << Pressure[k] << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          // Applies normal strain to structures
          for (size_t i = 1; i <= AEL_data.normal_strain.size(); i++) {
            for (size_t j = 0; j < AEL_data.normal_deformations.size(); j++) {
              ...
              const uint idVaspRun = vaspRunsPressure.size() - 1;
              vaspRunsPressure[idVaspRun].str = strainedstructure;
              aurostd::StringstreamClean(aus);
              aus << _AELSTR_MESSAGE_ + "idVaspRun = " << idVaspRun << endl;
              aus << _AELSTR_MESSAGE_ + "vaspRunsPressure[idVaspRun].str.lattice = " << endl;
              aus << vaspRunsPressure[idVaspRun].str.lattice << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
              // Set up name of separate directory for AFLOW run for each strained structure
              string stridVaspRun;
              ostringstream cnvidVaspRun;
              ...
              strnormindstrain = cnvnormindstrain.str();
              if (USER_DIRNAME_ARUN.option) {
                if (aurostd::abs(Pressure[k]) < tolzero) {
              ...
              dirrunnamepressure.push_back(AEL_data.dirpathname + "/" + runnamepressure[idVaspRun]);
              vaspRunsPressure[idVaspRun].str = strainedstructure;
              vaspRunsPressure[idVaspRun].AVASP_arun_runname = arunnamepressure[idVaspRun];
              vaspRunsPressure[idVaspRun].AVASP_arun = true;
              vaspRunsPressure[idVaspRun].AVASP_arun_mode = "AEL";
            }
          }

          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "creating shear strained structures at P = " << Pressure[k] << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          ...
          if (AEL_data_pressures.size() < (k + 1)) {
            ...
            AEL_data_pressures[k].applied_pressure = Pressure[k];
          }
        }
      }
      ...
      for (size_t idVaspRun = 0; idVaspRun < vaspRuns.size(); idVaspRun++) {
        ...
        aelerror = AEL_functions::aelvaspflags(vaspRuns[idVaspRun], vaspFlags, kbinFlags, dirrunname[idVaspRun], AEL_data, FileMESSAGE);
        if (aelerror != 0) {
          aurostd::StringstreamClean(aus);
        ...
        if (!avasp_pop_xvasp_success) {
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_ERROR_ + "Failed to set AFLOW flags for ARUN " << arunname[idVaspRun] << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          return 1;
        }
        ...
        if (AEL_data.relax_static || AEL_data.static_only) {
          if (aurostd::FileExist(vaspRuns[idVaspRun].Directory + "/" + _AFLOWLOCK_) || aurostd::FileExist(vaspRuns[idVaspRun].Directory + string("/OUTCAR.static")) ||
              ...
              ((XHOST.ARUN_POSTPROCESS || AEL_data.postprocess) && ((aurostd::FileExist(vaspRuns[idVaspRun].Directory + "/agl.LOCK")) || (aurostd::FileExist(vaspRuns[idVaspRun].Directory + "/ael.LOCK")) ||
                                                                    (aurostd::FileExist(vaspRuns[idVaspRun].Directory + "/LOCK")) || (aurostd::FileExist(dirrunname[idVaspRun] + "/agl.LOCK")) ||
                                                                    (aurostd::FileExist(dirrunname[idVaspRun] + "/ael.LOCK")) || (aurostd::FileExist(dirrunname[idVaspRun] + "/LOCK"))))) {
            continue;
          }
        } else {
          if (aurostd::FileExist(vaspRuns[idVaspRun].Directory + "/" + _AFLOWLOCK_) || aurostd::FileExist(vaspRuns[idVaspRun].Directory + string("/OUTCAR.relax2")) ||
              aurostd::CompressFileExist(vaspRuns[idVaspRun].Directory + string("/OUTCAR.relax2")) || aurostd::FileExist(dirrunname[idVaspRun] + "/" + _AFLOWLOCK_) ||
              aurostd::FileExist(dirrunname[idVaspRun] + string("/OUTCAR.relax2")) || aurostd::CompressFileExist(dirrunname[idVaspRun] + string("/OUTCAR.relax2")) ||
              ((XHOST.ARUN_POSTPROCESS || AEL_data.postprocess) && ((aurostd::FileExist(vaspRuns[idVaspRun].Directory + "/agl.LOCK")) || (aurostd::FileExist(vaspRuns[idVaspRun].Directory + "/ael.LOCK")) ||
                                                                    (aurostd::FileExist(vaspRuns[idVaspRun].Directory + "/LOCK")) || (aurostd::FileExist(dirrunname[idVaspRun] + "/agl.LOCK")) ||
                                                                    (aurostd::FileExist(dirrunname[idVaspRun] + "/ael.LOCK")) || (aurostd::FileExist(dirrunname[idVaspRun] + "/LOCK"))))) {
            continue;
        ...
        for (size_t ij = 0; ij < AEL_data.failed_arun_list.size(); ij++) {
          if (aurostd::substring2bool(runname[idVaspRun], AEL_data.failed_arun_list[ij], true)) {
            skipdir = true;
          }
        }
        if (skipdir) {
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "Skipping directory: " << runname[idVaspRun] << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          skipdir = false;
          continue;
        }
      ...
      if (USER_PRESSURE_CALC.option) {
        for (size_t k = 0; k < vaspRunsPressures.size(); k++) {
          // Set up AFLOW runs for strained structures
          for (size_t idVaspRun = 0; idVaspRun < vaspRunsPressures[k].size(); idVaspRun++) {
            // Assign the values of the flags provided by the user in the aflow.in file to the class containing the input data for the VASP run
            aelerror = AEL_functions::aelvaspflags(vaspRunsPressures[k][idVaspRun], vaspFlags, kbinFlags, dirrunnamepressures[k][idVaspRun], AEL_data, FileMESSAGE);
            if (aelerror != 0) {
              aurostd::StringstreamClean(aus);
              aus << _AELSTR_ERROR_ + "Failed to assign values of flags from aflow.in file" << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            ...
            // Therefore do not touch, but store this structure in the list, so that it can be used in the next part of code.
            if (AEL_data.relax_static || AEL_data.static_only) {
              if (aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + "/" + _AFLOWLOCK_) || aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + string("/OUTCAR.static")) ||
                  aurostd::CompressFileExist(vaspRunsPressures[k][idVaspRun].Directory + string("/OUTCAR.static")) || aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + "/" + _AFLOWLOCK_) ||
                  aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + string("/OUTCAR.static")) || aurostd::CompressFileExist(dirrunnamepressures[k][idVaspRun] + string("/OUTCAR.static")) ||
                  ((XHOST.ARUN_POSTPROCESS || AEL_data.postprocess) &&
                   ((aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + "/agl.LOCK")) || (aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + "/ael.LOCK")) ||
                    (aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + "/LOCK")) || (aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + "/agl.LOCK")) ||
                    (aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + "/ael.LOCK")) || (aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + "/LOCK"))))) {
                continue;
              }
            } else {
              if (aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + "/" + _AFLOWLOCK_) || aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + string("/OUTCAR.relax2")) ||
                  aurostd::CompressFileExist(vaspRunsPressures[k][idVaspRun].Directory + string("/OUTCAR.relax2")) || aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + "/" + _AFLOWLOCK_) ||
                  aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + string("/OUTCAR.relax2")) || aurostd::CompressFileExist(dirrunnamepressures[k][idVaspRun] + string("/OUTCAR.relax2")) ||
                  ((XHOST.ARUN_POSTPROCESS || AEL_data.postprocess) &&
                   ((aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + "/agl.LOCK")) || (aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + "/ael.LOCK")) ||
                    (aurostd::FileExist(vaspRunsPressures[k][idVaspRun].Directory + "/LOCK")) || (aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + "/agl.LOCK")) ||
                    (aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + "/ael.LOCK")) || (aurostd::FileExist(dirrunnamepressures[k][idVaspRun] + "/LOCK"))))) {
            ...
            // If so, then skip generation and continue to next structure
            for (size_t ij = 0; ij < AEL_data.failed_arun_list.size(); ij++) {
              if (aurostd::substring2bool(runnamepressures[k][idVaspRun], AEL_data.failed_arun_list[ij], true)) {
                skipdir = true;
              }
            }
            if (skipdir) {
              aurostd::StringstreamClean(aus);
              aus << _AELSTR_MESSAGE_ + "Skipping directory: " << runnamepressures[k][idVaspRun] << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
              skipdir = false;
              continue;
            }
      ...
      uint numskippedruns;
      for (size_t i = 1; i <= AEL_data.normal_strain.size(); i++) {
        numskippedruns = AEL_data.normal_deformations.size() - AEL_data.normal_deformations_complete[i - 1].size();
      ...
      for (size_t i = 1; i <= AEL_data.shear_strain.size(); i++) {
        numskippedruns = AEL_data.shear_deformations.size() - AEL_data.shear_deformations_complete[i - 1].size();
        if (numskippedruns > AEL_data.skiparunsmax) {
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_ERROR_ + "Number of failed runs in one independent directions = " << numskippedruns << endl;
      ...
      if (USER_WRITE_FULL_RESULTS.option) {
        ...
        // HE20240221 update to aurostd::JSON::object
        aurostd::StringstreamClean(aus);
        ...
        const string ofilestrjson = AEL_data.dirpathname + "/AEL_structures.json";
      ...
      if (AEL_data.fitrelaxedstruct) {
        ...
        vaspRuns.push_back(vaspRun);
        const uint idVaspRun = vaspRuns.size() - 1;
        vaspRuns[idVaspRun].str = initialstructure;
        string stridVaspRun;
        ostringstream cnvidVaspRun;
        cnvidVaspRun << idVaspRun;
        stridVaspRun = cnvidVaspRun.str();
      ...
      if (AEL_data.structurecalculated.empty()) {
        aurostd::StringstreamClean(aus);
        ...
        for (uint i = 0; i < AEL_data.structurecalculated.size(); i++) {
          ...
          json_loop["energy"] = AEL_data.energycalculated[i] / AEL_data.natomscell;
          json_loop["pressure"] = 0.1 * AEL_data.pressurecalculated[i];
          json_loop["stress_tensor"] = AEL_data.stresscalculated[i];
          json_loop["structure"] = xstructure2json(vaspRuns[idSuccessRun].str);
          json["AEL_energy_structures"].push_back(json_loop);
        }

        const string ofileenergstrjson = AEL_data.dirpathname + "/AEL_energy_structures.json";
        json.saveFile(ofileenergstrjson);
      }
      ...
      for (size_t i = 0; i < AEL_data.elastic_eigenvalues.size(); i++) {
        aurostd::StringstreamClean(aus);
        aus << _AELSTR_MESSAGE_ + "Elastic tensor eigenvalue " << i << " = " << AEL_data.elastic_eigenvalues[i] << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        if (AEL_data.elastic_eigenvalues[i] < 0.0) {
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_WARNING_ + "Elastic eigenvalue " << i << " = " << AEL_data.elastic_eigenvalues[i] << " is negative" << endl;
      ...
      const double bulkvoigthalfratio = AEL_data.bulkmodulus_voigt_half / AEL_data.bulkmodulus_voigt;
      aurostd::StringstreamClean(aus);
      ...
      const double bulkreusshalfratio = AEL_data.bulkmodulus_reuss_half / AEL_data.bulkmodulus_reuss;
      aurostd::StringstreamClean(aus);
      ...
      const double shearvoigthalfratio = AEL_data.shearmodulus_voigt_half / AEL_data.shearmodulus_voigt;
      aurostd::StringstreamClean(aus);
      ...
      const double shearreusshalfratio = AEL_data.shearmodulus_reuss_half / AEL_data.shearmodulus_reuss;
      aurostd::StringstreamClean(aus);
      ...
      const double bulkvrhhalfratio = AEL_data.bulkmodulus_vrh_half / AEL_data.bulkmodulus_vrh;
      aurostd::StringstreamClean(aus);
      ...
      const double shearvrhhalfratio = AEL_data.shearmodulus_vrh_half / AEL_data.shearmodulus_vrh;
      aurostd::StringstreamClean(aus);
      ...
      if (USER_PRESSURE_CALC.option) {
        for (size_t k = 0; k < vaspRunsPressures.size(); k++) {
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "extract stress tensors for Pressure = " << Pressure[k] << "GPa" << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          // Extract stress tensors from results of VASP calculations for strained structures
          aelerror = AEL_functions::extractstress(vaspRunsPressures[k], AEL_data_pressures[k], dirrunnamepressures[k], FileMESSAGE);
          if (aelerror != 0) {
            aurostd::StringstreamClean(aus);
            aus << _AELSTR_ERROR_ + "Failed to extract stress tensors" << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          ...
          // Check number of skipped runs in each direction does not exceed limit
          uint numskippedruns;
          for (size_t i = 1; i <= AEL_data_pressures[k].normal_strain.size(); i++) {
            numskippedruns = AEL_data_pressures[k].normal_deformations.size() - AEL_data_pressures[k].normal_deformations_complete[i - 1].size();
            if (numskippedruns > AEL_data_pressures[k].skiparunsmax) {
              aurostd::StringstreamClean(aus);
              aus << _AELSTR_ERROR_ + "Number of failed runs in one independent directions = " << numskippedruns << endl;
              aus << _AELSTR_ERROR_ + "Number of failed runs exceeds limit in one independent direction of " << AEL_data.skiparunsmax << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          ...
          for (size_t i = 1; i <= AEL_data_pressures[k].shear_strain.size(); i++) {
            numskippedruns = AEL_data_pressures[k].shear_deformations.size() - AEL_data_pressures[k].shear_deformations_complete[i - 1].size();
            if (numskippedruns > AEL_data_pressures[k].skiparunsmax) {
              aurostd::StringstreamClean(aus);
              aus << _AELSTR_ERROR_ + "Number of failed runs in one independent directions = " << numskippedruns << endl;
              aus << _AELSTR_ERROR_ + "Number of failed runs exceeds limit per independent directions of " << AEL_data_pressures[k].skiparunsmax << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
              return 1;
            }
          }
          ...
          // Runs elastic constants algorithm within AFLOW
          aelerror = AEL_functions::elasticityfit(AEL_data_pressures[k], FileMESSAGE);
          if (aelerror != 0) {
            aurostd::StringstreamClean(aus);
            aus << _AELSTR_ERROR_ + "Failed to calculate elastic constants" << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          ...
          // Checks that Poisson ratio is within the range -1.0 to 0.5
          // Materials with a Poisson ratio outside of this range are typically unstable
          if ((AEL_data_pressures[k].poisson_ratio < -1.0) || (AEL_data_pressures[k].poisson_ratio > 0.5)) {
            aurostd::StringstreamClean(aus);
            aus << _AELSTR_WARNING_ + "Poisson ratio = " << AEL_data.poisson_ratio << " is outside range from -1.0 to 0.5" << endl;
            ;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          ...
          // Checks that eigenvalues of elastic stiffness tensor are all greater than zero
          // Negative eigenvalues imply that the material is mechanically unstable
          for (size_t i = 0; i < AEL_data_pressures[k].elastic_eigenvalues.size(); i++) {
            aurostd::StringstreamClean(aus);
            aus << _AELSTR_MESSAGE_ + "Elastic tensor eigenvalue " << i << " = " << AEL_data_pressures[k].elastic_eigenvalues[i] << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            if (AEL_data_pressures[k].elastic_eigenvalues[i] < 0.0) {
              aurostd::StringstreamClean(aus);
              aus << _AELSTR_WARNING_ + "Elastic eigenvalue " << i << " = " << AEL_data_pressures[k].elastic_eigenvalues[i] << " is negative" << endl;
              aus << _AELSTR_WARNING_ + "Negative eigenvalue implies material is unstable" << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
              AEL_data_pressures[k].mechanically_stable = false;
            }
          }

          // Uses Bravais lattice symmetry to check symmetry and consistency of elastic constant tensor
          if (USER_CHECK_ELASTIC_SYMMETRY.option) {
            aelerror = AEL_functions::elasticitycheck(AEL_data_pressures[k], xvasp, FileMESSAGE);
            if (aelerror != 0) {
              aurostd::StringstreamClean(aus);
              aus << _AELSTR_ERROR_ + "Error checking elastic constants symmetry" << endl;
              aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          ...
          // Calculates elastic moduli using elastic constant tensor
          aelerror = AEL_functions::elasticmoduli(AEL_data_pressures[k], FileMESSAGE);
          if (aelerror != 0) {
            aurostd::StringstreamClean(aus);
            aus << _AELSTR_ERROR_ + "Failed to calculate elastic moduli" << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          ...
          // Compare bulk and shear moduli calculated using full strain set to those calculated using half strain set
          // If values differ by more than 15%, it is likely that the stress-strain curve is no longer linear
          const double bulkvoigthalfratio = AEL_data_pressures[k].bulkmodulus_voigt_half / AEL_data_pressures[k].bulkmodulus_voigt;
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "Voigt bulk modulus = " << AEL_data_pressures[k].bulkmodulus_voigt << endl;
          aus << _AELSTR_MESSAGE_ + "Voigt bulk modulus (half strain) = " << AEL_data_pressures[k].bulkmodulus_voigt_half << endl;
          aus << _AELSTR_MESSAGE_ + "Voigt bulk modulus ratio = " << bulkvoigthalfratio << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          if ((bulkvoigthalfratio < 0.85) || (bulkvoigthalfratio > 1.15)) {
            aurostd::StringstreamClean(aus);
          ...
          const double bulkreusshalfratio = AEL_data_pressures[k].bulkmodulus_reuss_half / AEL_data_pressures[k].bulkmodulus_reuss;
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "Reuss bulk modulus = " << AEL_data_pressures[k].bulkmodulus_reuss << endl;
          aus << _AELSTR_MESSAGE_ + "Reuss bulk modulus (half strain) = " << AEL_data_pressures[k].bulkmodulus_reuss_half << endl;
          aus << _AELSTR_MESSAGE_ + "Reuss bulk modulus ratio = " << bulkreusshalfratio << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          if ((bulkreusshalfratio < 0.85) || (bulkreusshalfratio > 1.15)) {
            aurostd::StringstreamClean(aus);
          ...
          const double shearvoigthalfratio = AEL_data_pressures[k].shearmodulus_voigt_half / AEL_data_pressures[k].shearmodulus_voigt;
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "Voigt shear modulus = " << AEL_data_pressures[k].shearmodulus_voigt << endl;
          aus << _AELSTR_MESSAGE_ + "Voigt shear modulus (half strain) = " << AEL_data_pressures[k].shearmodulus_voigt_half << endl;
          aus << _AELSTR_MESSAGE_ + "Voigt shear modulus ratio = " << shearvoigthalfratio << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          if ((shearvoigthalfratio < 0.85) || (shearvoigthalfratio > 1.15)) {
            aurostd::StringstreamClean(aus);
          ...
          const double shearreusshalfratio = AEL_data_pressures[k].shearmodulus_reuss_half / AEL_data_pressures[k].shearmodulus_reuss;
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "Reuss shear modulus = " << AEL_data_pressures[k].shearmodulus_reuss << endl;
          aus << _AELSTR_MESSAGE_ + "Reuss shear modulus (half strain) = " << AEL_data_pressures[k].shearmodulus_reuss_half << endl;
          aus << _AELSTR_MESSAGE_ + "Reuss shear modulus ratio = " << shearreusshalfratio << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          if ((shearreusshalfratio < 0.85) || (shearreusshalfratio > 1.15)) {
            aurostd::StringstreamClean(aus);
          ...
          const double bulkvrhhalfratio = AEL_data_pressures[k].bulkmodulus_vrh_half / AEL_data_pressures[k].bulkmodulus_vrh;
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "VRH bulk modulus = " << AEL_data_pressures[k].bulkmodulus_vrh << endl;
          aus << _AELSTR_MESSAGE_ + "VRH bulk modulus (half strain) = " << AEL_data_pressures[k].bulkmodulus_vrh_half << endl;
          aus << _AELSTR_MESSAGE_ + "VRH bulk modulus ratio = " << bulkvrhhalfratio << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          if ((bulkvrhhalfratio < 0.85) || (bulkvrhhalfratio > 1.15)) {
            aurostd::StringstreamClean(aus);
          ...
          const double shearvrhhalfratio = AEL_data_pressures[k].shearmodulus_vrh_half / AEL_data_pressures[k].shearmodulus_vrh;
          aurostd::StringstreamClean(aus);
          aus << _AELSTR_MESSAGE_ + "VRH shear modulus = " << AEL_data_pressures[k].shearmodulus_vrh << endl;
          aus << _AELSTR_MESSAGE_ + "VRH shear modulus (half strain) = " << AEL_data_pressures[k].shearmodulus_vrh_half << endl;
          aus << _AELSTR_MESSAGE_ + "VRH shear modulus ratio = " << shearvrhhalfratio << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          if ((shearvrhhalfratio < 0.85) || (shearvrhhalfratio > 1.15)) {
            aurostd::StringstreamClean(aus);
      ...
      if (USER_PRESSURE_CALC.option && USER_USE_STABLE_PRESSURE.option) {
        ...
        for (size_t k = 0; k < Pressure.size(); k++) {
          if (std::abs(Pressure[k]) > maxPressure) {
            maxPressure = Pressure[k];
          }
        }
        double lowestStablePressure = maxPressure + 1.0;
        stablePressureStructure = 0;
        for (size_t k = 0; k < AEL_data_pressures.size(); k++) {
          if (AEL_data_pressures[k].mechanically_stable) {
            stablePressureFound = true;
            aurostd::StringstreamClean(aus);
            aus << _AELSTR_MESSAGE_ + "Structure at pressure = " << AEL_data_pressures[k].applied_pressure << "GPa is elastically stable" << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
            if (std::abs(AEL_data_pressures[k].applied_pressure) < lowestStablePressure) {
              stablePressureStructure = k;
              lowestStablePressure = AEL_data_pressures[k].applied_pressure;
            }
          }
        }
        aurostd::StringstreamClean(aus);
        ...
        if (stablePressureFound) {
          AEL_data = AEL_data_pressures[stablePressureStructure];
        }
        aurostd::StringstreamClean(aus);
      ...
      for (size_t i = 0; i < AEL_data.elastic_tensor.size(); i++) {
        for (size_t j = 0; j < AEL_data.elastic_tensor.size(); j++) {
          oss << setw(15) << setprecision(6) << (AEL_data.elastic_tensor[i][j]);
        }
        oss << endl;
      }
      ...
      for (size_t i = 0; i < AEL_data.compliance_tensor.size(); i++) {
        for (size_t j = 0; j < AEL_data.compliance_tensor.size(); j++) {
          oss << setw(15) << setprecision(6) << (AEL_data.compliance_tensor[i][j]);
        }
        oss << endl;
      }
      oss << "[AEL_COMPLIANCE_TENSOR]STOP" << endl;
      oss << AFLOWIN_SEPARATION_LINE << endl;
      const string ofileafaelname = AEL_data.dirpathname + "/aflow.ael.out";
      if (!aurostd::stringstream2file(oss, ofileafaelname, aurostd::compression_type::None, "WRITE")) {
        aurostd::StringstreamClean(aus);
      ...
      const string ofileelconstname = AEL_data.dirpathname + "/AEL_Elastic_constants.out";
      if (!aurostd::stringstream2file(oss, ofileelconstname, aurostd::compression_type::None, "WRITE")) {
        aurostd::StringstreamClean(aus);
      ...
      const string ofilecomptensname = AEL_data.dirpathname + "/AEL_Compliance_tensor.out";
      if (!aurostd::stringstream2file(oss, ofilecomptensname, aurostd::compression_type::None, "WRITE")) {
        aurostd::StringstreamClean(aus);
      ...
      uint elasticindexi;
      uint elasticindexj;
      string strelasticindexi;
      string strelasticindexj;
      string elastic_constant_label;
      ostringstream cnvstrelasticindexi;
      ostringstream cnvstrelasticindexj;
      ...
      for (size_t i = 0; i < AEL_data.elastic_tensor.size(); i++) {
        for (size_t j = 0; j < AEL_data.elastic_tensor.size(); j++) {
          ...
          if ((i == 0) && (j == 0)) {
            oss << "\"" + elastic_constant_label + "\":" << AEL_data.elastic_tensor[i][j];
          } else {
            oss << ",\"" + elastic_constant_label + "\":" << AEL_data.elastic_tensor[i][j];
          }
        }
      ...
      for (size_t i = 0; i < AEL_data.compliance_tensor.size(); i++) {
        for (size_t j = 0; j < AEL_data.compliance_tensor.size(); j++) {
          ...
          if ((i == 0) && (j == 0)) {
            oss << "\"" + elastic_constant_label + "\":" << AEL_data.compliance_tensor[i][j];
          } else {
            oss << ",\"" + elastic_constant_label + "\":" << AEL_data.compliance_tensor[i][j];
          }
        }
      }
      oss << R"(},"units":"GPa"})" << endl;
      const string ofileelastictensorjson = AEL_data.dirpathname + "/AEL_elastic_tensor.json";
      if (!aurostd::stringstream2file(oss, ofileelastictensorjson, aurostd::compression_type::None, "WRITE")) {
        aurostd::StringstreamClean(aus);
      ...
      if (USER_WRITE_FULL_RESULTS.option) {
        ...
        const string ofileaellibname = AEL_data.dirpathname + "/aellib.out";
        if (!aurostd::stringstream2file(oss, ofileaellibname, aurostd::compression_type::None, "WRITE")) {
          aurostd::StringstreamClean(aus);
        ...
        for (size_t i = 0; i < AEL_data.youngsmodulus_directional.size(); i++) {
          oss << setw(15) << setprecision(6) << (AEL_data.youngsmodulus_directional[i]);
        }
        oss << endl;
        oss << "##########################" << endl;
        oss << "# Shear modulus YZ XZ XY #" << endl;
        for (size_t i = 0; i < AEL_data.shearmodulus_directional.size(); i++) {
          oss << setw(15) << setprecision(6) << (AEL_data.shearmodulus_directional[i]);
        }
        oss << endl;
        oss << "###################################" << endl;
        oss << "# Poisson ratio XY XZ YX YZ ZX ZY #" << endl;
        for (size_t i = 0; i < AEL_data.poissonratio_directional.size(); i++) {
          oss << setw(15) << setprecision(6) << (AEL_data.poissonratio_directional[i]);
        }
        oss << endl;
        const string ofileelmodirname = AEL_data.dirpathname + "/AEL_elastic_moduli_directional.out";
        if (!aurostd::stringstream2file(oss, ofileelmodirname, aurostd::compression_type::None, "WRITE")) {
          aurostd::StringstreamClean(aus);
      ...
      if (USER_PRESSURE_CALC.option) {
        ...
        for (size_t k = 0; k < vaspRunsPressures.size(); k++) {
          ...
          aus << _AELSTR_MESSAGE_ + "Pressure = " << Pressure[k] << "GPa" << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          // Writes elastic moduli values in file for REST-API
          oss << AFLOWIN_SEPARATION_LINE << endl;
          oss << "[AEL_PRESSURE]PRESSURE = " << Pressure[k] << "GPa" << endl;
          if (AEL_data_pressures[k].mechanically_stable) {
          ...
          oss << "[AEL_RESULTS]START" << endl;
          oss << "ael_poisson_ratio=" << AEL_data_pressures[k].poisson_ratio << "  " << endl;
          oss << "ael_bulk_modulus_voigt=" << AEL_data_pressures[k].bulkmodulus_voigt << "  (GPa)" << endl;
          oss << "ael_bulk_modulus_reuss=" << AEL_data_pressures[k].bulkmodulus_reuss << "  (GPa)" << endl;
          oss << "ael_shear_modulus_voigt=" << AEL_data_pressures[k].shearmodulus_voigt << "  (GPa)" << endl;
          oss << "ael_shear_modulus_reuss=" << AEL_data_pressures[k].shearmodulus_reuss << "  (GPa)" << endl;
          oss << "ael_bulk_modulus_vrh=" << AEL_data_pressures[k].bulkmodulus_vrh << "  (GPa)" << endl;
          oss << "ael_shear_modulus_vrh=" << AEL_data_pressures[k].shearmodulus_vrh << "  (GPa)" << endl;
          oss << "ael_elastic_anisotropy=" << AEL_data_pressures[k].elastic_anisotropy << "  " << endl;
          oss << "ael_speed_sound_transverse=" << AEL_data_pressures[k].speed_sound_transverse << " (m/s)" << endl;
          oss << "ael_speed_sound_longitudinal=" << AEL_data_pressures[k].speed_sound_longitudinal << " (m/s)" << endl;
          oss << "ael_speed_sound_average=" << AEL_data_pressures[k].speed_sound_average << " (m/s)" << endl;
          oss << "ael_debye_temperature=" << AEL_data_pressures[k].debye_temperature << "  " << endl;
          oss << "ael_applied_pressure=" << AEL_data_pressures[k].applied_pressure << "  (GPa)" << endl;
          oss << "ael_average_external_pressure=" << AEL_data_pressures[k].average_external_pressure << "  (GPa)" << endl;
          oss << "[AEL_RESULTS]STOP" << endl;
          oss << AFLOWIN_SEPARATION_LINE << endl;
          oss << "[AEL_STIFFNESS_TENSOR]START" << endl;
          for (size_t i = 0; i < AEL_data_pressures[k].elastic_tensor.size(); i++) {
            for (size_t j = 0; j < AEL_data_pressures[k].elastic_tensor.size(); j++) {
              oss << setw(15) << setprecision(6) << (AEL_data_pressures[k].elastic_tensor[i][j]);
            }
            oss << endl;
          }
          oss << "[AEL_STIFFNESS_TENSOR]STOP" << endl;
          oss << AFLOWIN_SEPARATION_LINE << endl;
          oss << "[AEL_COMPLIANCE_TENSOR]START" << endl;
          for (size_t i = 0; i < AEL_data_pressures[k].compliance_tensor.size(); i++) {
            for (size_t j = 0; j < AEL_data_pressures[k].compliance_tensor.size(); j++) {
              oss << setw(15) << setprecision(6) << (AEL_data_pressures[k].compliance_tensor[i][j]);
            }
            oss << endl;
          }
          oss << "[AEL_COMPLIANCE_TENSOR]STOP" << endl;
        ...
        const string ofileafaelname = AEL_data.dirpathname + "/aflow.ael_pressure.out";
        if (!aurostd::stringstream2file(oss, ofileafaelname, aurostd::compression_type::None, "WRITE")) {
          aurostd::StringstreamClean(aus);
