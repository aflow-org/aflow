...
double Murnaghan(const double x, const xvector<double> &p, xvector<double> &dydp) {
  const double Eeq = p[1];
  const double Veq = p[2];
  const double B = p[3];
  const double Bp = p[4];
  const double V = x;
  ...
  dydp[4] = (B * (-V + 2.0 * Bp * V - pow(Bp, 2) * V + pow(Bp, 2) * Veq + V * pow(Veq / V, Bp) * (1.0 - 2.0 * Bp + (Bp - 1.0) * Bp * log(Veq / V)))) / (pow(Bp - 1.0, 2) * pow(Bp, 2));
...
double BirchMurnaghan(const double x, const xvector<double> &p, xvector<double> &dydp) {
  ...
  const double kappa = pow(Veq / V, 2.0 / 3.0);

  dydp[1] = 1.0;
  dydp[2] = 3.0 * B * kappa * (kappa - 1) * (3.0 * (Bp - 6.0) * V * pow(Veq / V, 1.0 / 3.0) + Veq * (62.0 - 36.0 * kappa + 3 * Bp * (3 * kappa - 4))) / (16.0 * Veq);
...
namespace apl {
  ...
  ///
  void QHA::initialize(const xstructure &in_structure, _xinput &xinput, xoption &qha_options, xoption &apl_options, ofstream &FileMESSAGE, ostream &oss) {
    ...
    const double Volume = origStructure.GetVolume();

    // parse QHA-related aflow.in options
    isEOS = qha_options.flag("EOS");
    vector<double> eosrange(3);
    aurostd::string2tokens(qha_options.getattachedscheme("EOS_DISTORTION_RANGE"), eosrange, " :");
    ...
    if (qha_options.flag("EOS_MODEL:BM3") || qha_options.flag("EOS_MODEL:SJ") || qha_options.flag("EOS_MODEL:M")) {
    ...
    message << "EOS model:" << (qha_options.flag("EOS_MODEL:SJ") ? " SJ" : "");
    message << (qha_options.flag("EOS_MODEL:BM2") ? " BM2" : "");
    message << (qha_options.flag("EOS_MODEL:BM3") ? " BM3" : "");
    message << (qha_options.flag("EOS_MODEL:BM4") ? " BM4" : "");
    message << (qha_options.flag("EOS_MODEL:M") ? " M" : "") << "." << std::endl;
    ;
    ...
    pflow::logger(QHA_ARUN_MODE, __AFLOW_FUNC__, message, currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    ...
    gprange[0] = 1.0 - gp_distortion;
    gprange[1] = 1.0;
    gprange[2] = 1.0 + gp_distortion;
    N_GPvolumes = 3;
    for (int j = 0; j < N_GPvolumes; j++) {
      arun_runnames_apl_gp.push_back("PHONON_" + aurostd::utype2string(gprange[j], precision_format, false, FIXED_STREAM));

      // if APL state file for V=1.0 exists, reuse it and do not create PHONON_1.000 directory
      if (aurostd::isequal(gprange[j], 1.0) && aurostd::CompressFileExist(currentDirectory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_STATE_FILE)) {
        subdirectories_apl_gp.emplace_back(".");
      } else {
        subdirectories_apl_gp.push_back(ARUN_DIRECTORY_PREFIX + QHA_ARUN_MODE + '_' + arun_runnames_apl_gp.back());
      }
    ...
    if (isEOS || isQHA3P || isSCQHA || isQHANP) {
      ...
      for (double i = eosrange[0]; i <= (eosrange[1] + AUROSTD_IDENTITY_TOL); i += eosrange[2]) {
        arun_runnames_apl_eos.push_back("PHONON_" + aurostd::utype2string(i, precision_format, false, FIXED_STREAM));

        // if APL state file for V=1.0 exists, reuse it and do not create PHONON_1.000 directory
        if (aurostd::isequal(i, 1.0) && aurostd::CompressFileExist(currentDirectory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_STATE_FILE)) {
          subdirectories_apl_eos.emplace_back(".");
        } else {
          subdirectories_apl_eos.push_back(ARUN_DIRECTORY_PREFIX + QHA_ARUN_MODE + '_' + arun_runnames_apl_eos.back());
        }

        arun_runnames_static.push_back("STATIC_" + aurostd::utype2string(i, precision_format, false, FIXED_STREAM));
        dirname = ARUN_DIRECTORY_PREFIX + QHA_ARUN_MODE + '_' + arun_runnames_static.back();
    ...
    if (isQHANP) {
      ...
      for (int i = 0; i < N_QHANPvolumes; i++) {
        ...
        arun_runnames_apl_qhanp.push_back("PHONON_" + aurostd::utype2string(coef, precision_format, false, FIXED_STREAM));

        // if APL state file for V=1.0 exists, reuse it and do not create PHONON_1.000 directory
        if (aurostd::isequal(coef, 1.0) && aurostd::CompressFileExist(currentDirectory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_STATE_FILE)) {
          subdirectories_apl_qhanp.emplace_back(".");
        } else {
          subdirectories_apl_qhanp.push_back(ARUN_DIRECTORY_PREFIX + QHA_ARUN_MODE + '_' + arun_runnames_apl_qhanp.back());
        }
    ...
    if (tokens.size() != 3) {
      ...
      throw aurostd::xerror(__AFLOW_FILE__, QHA_ARUN_MODE, msg, _INPUT_NUMBER_);
    }
    const double tp_start = tokens[0];
    const double tp_end = tokens[1];
    const double tp_step = tokens[2];
  ...
  void QHA::run(_xflags &xflags, _aflags &aflags, _kflags &kflags) {
    ...
    if (!isEOS && !isGP_FD) {
      const string message = "Nothing to be done. Selector EOS=ON or GP_FINITE_DIFF=ON";
    ...
    try {
      ...
      if ((isQHA && isEOS) || isQHA3P || isSCQHA || isQHANP) {
        ...
        pflow::logger(QHA_ARUN_MODE, __AFLOW_FUNC__, msg, currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
        vector<vector<bool>> file_is_present(subdirectories_static.size(), vector<bool>(4));

        const uint n_static_calcs = checkStaticCalculations(file_is_present);
        ...
        if (n_static_calcs == subdirectories_static.size()) {
          ...
          if (n_static_calcs > 0) {
            ...
            const string message = "Waiting for required STATIC calculations.";
      ...
      if (isGP_FD || isQHA3P || isSCQHA) {
        ...
        if (gp_data_available) {
          if (isQHA || isQHA3P) {
            const double V0 = origStructure.GetVolume() / NatomsOrigCell;
            ...
            if (isQHA3P && eos_static_data_available) {
              ...
              if (qha_options.flag("EOS_MODEL:BM2")) {
                writeThermalProperties(EOS_BIRCH_MURNAGHAN2, QHA3P_CALC, currentDirectory);
                writeTphononDispersions(EOS_BIRCH_MURNAGHAN2, QHA3P_CALC, currentDirectory);
              }

              if (qha_options.flag("EOS_MODEL:BM3")) {
                writeThermalProperties(EOS_BIRCH_MURNAGHAN3, QHA3P_CALC, currentDirectory);
                writeTphononDispersions(EOS_BIRCH_MURNAGHAN3, QHA3P_CALC, currentDirectory);
              }

              if (qha_options.flag("EOS_MODEL:BM4")) {
                writeThermalProperties(EOS_BIRCH_MURNAGHAN4, QHA3P_CALC, currentDirectory);
                writeTphononDispersions(EOS_BIRCH_MURNAGHAN4, QHA3P_CALC, currentDirectory);
              }
          ...
          if (isSCQHA && eos_static_data_available) {
            ...
            if (qha_options.flag("EOS_MODEL:BM2")) {
              ...
              writeTphononDispersions(EOS_BIRCH_MURNAGHAN2, SCQHA_CALC, currentDirectory);
            }
            if (qha_options.flag("EOS_MODEL:BM3")) {
              runSCQHA(EOS_BIRCH_MURNAGHAN3, true, currentDirectory);
              writeTphononDispersions(EOS_BIRCH_MURNAGHAN3, SCQHA_CALC, currentDirectory);
            }
            if (qha_options.flag("EOS_MODEL:BM4")) {
              runSCQHA(EOS_BIRCH_MURNAGHAN4, true, currentDirectory);
              writeTphononDispersions(EOS_BIRCH_MURNAGHAN4, SCQHA_CALC, currentDirectory);
            }
            if (qha_options.flag("EOS_MODEL:M")) {
              runSCQHA(EOS_MURNAGHAN, true, currentDirectory);
              writeTphononDispersions(EOS_MURNAGHAN, SCQHA_CALC, currentDirectory);
            }
          }
        }
      }
    } catch (aurostd::xerror &e) {
      pflow::logger(e.whereFileName(), e.whereFunction(), e.what(), currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_ERROR_);
      return;
    }
  }
  ...
  ///
  void QHA::createSubdirectoriesStaticRun(const _xflags &xflags, const _aflags &aflags, const _kflags &kflags, const vector<vector<bool>> &file_is_present) {
    string msg;
    ...
    stringstream aflow;
    const string incar_explicit = xinput.xvasp.AVASP_INCAR_EXPLICIT_START_STOP.str();
    ...
    for (size_t i = 0; i < subdirectories_static.size(); i++) {
      if (includeElectronicContribution) {
        if (file_is_present[i][ST_DF_EIGENVAL] && file_is_present[i][ST_DF_IBZKPT] && file_is_present[i][ST_DF_OUTCAR]) {
          continue;
        }
      } else if (file_is_present[i][ST_DF_OUTCAR]) {
        continue;
      }

      if (aurostd::FileExist(currentDirectory + '/' + subdirectories_static[i] + '/' + _AFLOWIN_)) {
        continue;
      }
  ...
  int QHA::checkStaticCalculations(vector<vector<bool>> &file_is_present) {
    ...
    for (size_t i = 0; i < subdirectories_static.size(); i++) {
      ...
      if (aurostd::CompressFileExist(outcarfile)) {
      ...
      if (includeElectronicContribution) {
        ...
        if (aurostd::CompressFileExist(eigenvfile)) {
          file_is_present[i][ST_DF_EIGENVAL] = true;
        } else {
          all_files_are_present = false;
        }

        if (aurostd::CompressFileExist(ibzkptfile)) {
          file_is_present[i][ST_DF_IBZKPT] = true;
        } else {
          all_files_are_present = false;
        }
      }
      if (all_files_are_present) {
        count++;
      }
    }
  ...
  void QHA::printMissingStaticFiles(const vector<vector<bool>> &list, const vector<string> &subdirectories) {
    string msg;
  ...
  ///
  void QHA::createSubdirectoriesAPLRun(const _xflags &xflags, const _aflags &aflags, const _kflags &kflags, const vector<vector<bool>> &file_is_present, QHAtype qhatype) {
    string msg;
    ...
    vector<string> &subdirectories = (QHA_FD == qhatype) ? subdirectories_apl_gp : (QHA_EOS == qhatype) ? subdirectories_apl_eos : subdirectories_apl_qhanp;
    vector<string> &arun_runnames = (QHA_FD == qhatype) ? arun_runnames_apl_gp : (QHA_EOS == qhatype) ? arun_runnames_apl_eos : arun_runnames_apl_qhanp;
    vector<double> &coefVolumes = (QHA_FD == qhatype) ? coefGPVolumes : (QHA_EOS == qhatype) ? coefEOSVolumes : coefQHANPVolumes;

    // create aflow.in if there are no outputs with results in the directory.
    // The existing aflow.in file will not be overwritten.
    for (size_t i = 0; i < subdirectories.size(); i++) {
      if (file_is_present[i][PH_DF_HARMIFC]) {
        continue;
      }
  ...
  int QHA::checkAPLCalculations(vector<vector<bool>> &file_is_present, QHAtype qhatype) {
    ...
    for (size_t i = 0; i < subdirectories.size(); i++) {
      ...
      if (aurostd::CompressFileExist(harmifcfile)) {
      ...
      if (aurostd::CompressFileExist(phposcarfile)) {
  ...
  bool QHA::runAPL(_xflags &xflags, _aflags &aflags, _kflags &kflags, QHAtype qhatype) {
    ...
    switch (qhatype) {
      case (QHA_EOS): msg += "EOS"; break;
      case (QHA_FD): msg += "FD"; break;
      case (QHA_TE): msg += "TE"; break;
    }
    msg += " APL calculations exist.";
    pflow::logger(QHA_ARUN_MODE, __AFLOW_FUNC__, msg, currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);

    const vector<string> &subdirectories = (QHA_FD == qhatype) ? subdirectories_apl_gp : (QHA_EOS == qhatype) ? subdirectories_apl_eos : subdirectories_apl_qhanp;

    vector<vector<bool>> file_is_present(subdirectories.size(), vector<bool>(2));

    bool apl_data_available = false;
    const uint n_apl_calcs = checkAPLCalculations(file_is_present, qhatype);
    if (n_apl_calcs == subdirectories.size()) {
      ...
      if (n_apl_calcs > 0) {
        ...
        // finish as DONE without telling the user why no output has been produed.
        const string message = "Waiting for required PHONON calculations.";
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_NOTICE_);
      }
  ...
  ///
  bool QHA::readAPLCalculationData(const vector<string> &subdirectories, _kflags &kflags, QHAtype type) {
    ...
    for (size_t i = 0; i < subdirectories.size(); i++) {
      ...
      if (aurostd::CompressFileExist(phposcarfile, phposcarfile)) {
        phcalc.initialize_supercell(xstructure(phposcarfile, IOVASP_POSCAR), false);
      } // verbose=false: do not write files related to symmetry-analysis
      ...
      const string hibernation_file = directory + '/' + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_HARMIFC_FILE;
      if (aurostd::CompressFileExist(hibernation_file)) {
        msg = "Reading hibernation file...";
        pflow::logger(QHA_ARUN_MODE, __AFLOW_FUNC__, msg, currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
        try {
          phcalc.awake();
        } catch (aurostd::xerror &e) {
          pflow::logger(QHA_ARUN_MODE, __AFLOW_FUNC__, e.buildMessageString(), currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_ERROR_);
      ...
      // phonon dispersions
      const vector<xvector<double>> dummy_dos_projections; // do not need for QHA
      vector<int> dos_mesh(3);

      vector<string> tokens;

      // ME20200518 - Changed keywords to correspond to rest of APL
      aurostd::string2tokens(apl_options.getattachedscheme("DOSMESH"), tokens, string(" xX"));
      ...
      dosc.calc(aurostd::string2utype<double>(apl_options.getattachedscheme("DOSPOINTS")), aurostd::string2utype<double>(apl_options.getattachedscheme("DOSSMEAR")));
      ...
      if (dosc.hasImaginaryFrequencies()) {
        ...
        if (ignore_imaginary) {
          ...
          pflow::logger(QHA_ARUN_MODE, __AFLOW_FUNC__, msg, currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
        } else {
      ...
      const string USER_DC_INITLATTICE;
      // ME20200518 - Changed keyword to common APL keywords
      const int USER_DC_NPOINTS = aurostd::string2utype<int>(apl_options.getattachedscheme("DCPOINTS"));
      ...
      switch (type) {
        case (QHA_FD):
          ...
          if (i == 0) {
            omegaV_mesh = vector<vector<vector<double>>>(Nqpoints, vector<vector<double>>(Nbranches, vector<double>(N_GPvolumes)));
          }
        ...
        case (QHA_EOS):
          ...
          eos_vib_thermal_properties.emplace_back(dosc, *p_FileMESSAGE);

          // allocate memory at the first run (an entire chunk of memory is required because
          // data will be reordered)
          if (i == 0) {
            omegaV_mesh_EOS = vector<vector<vector<double>>>(Nqpoints, vector<vector<double>>(Nbranches, vector<double>(N_EOSvolumes)));
          }
        ...
        case (QHA_TE):
          ...
          // data will be reordered)
          if (i == 0) {
            omegaV_mesh_QHANP = vector<vector<vector<double>>>(Nqpoints, vector<vector<double>>(Nbranches, vector<double>(N_QHANPvolumes)));
          }
    ...
    switch (type) {
      case (QHA_EOS): msg = (std::string) "[QHA]IMAG=" + (has_negative_frequencies ? "YES" : "NO") + "\n"; break;
      case (QHA_FD):
        msg = (std::string) "[QHA3P]IMAG=" + (has_negative_frequencies ? "YES" : "NO") + "\n";
        msg = (std::string) "[SCQHA]IMAG=" + (has_negative_frequencies ? "YES" : "NO") + "\n";
        break;
      case (QHA_TE): msg = (std::string) "[QHANP]IMAG=" + (has_negative_frequencies ? "YES" : "NO") + "\n"; break;
    }

    const string filename = currentDirectory + '/' + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_IMAG_FILE;
    if (aurostd::FileExist(filename)) {
      if (!aurostd::string2file(msg, filename, aurostd::compression_type::None, "APPEND")) {
        msg = "Error writing to " + filename + " file.";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, msg, _FILE_ERROR_);
      }
    } else {
  ...
  bool QHA::readStaticCalculationsData() {
    ...
    for (size_t i = 0; i < subdirectories_static.size(); i++) {
      ...
      directory = currentDirectory + '/' + subdirectories_static[i];
      outcarfile = directory + '/' + "OUTCAR.static";
      if (!outcar.initialize(outcarfile)) {
      ...
      if (includeElectronicContribution) {
        static_eigvals.emplace_back(directory + "/EIGENVAL.static");
        static_ibzkpts.emplace_back(directory + "/IBZKPT.static");
  ...
  double QHA::calcGrueneisen(double V, xvector<double> &xomega, double &w) {
    ...
    if (freqs_are_nonzero) {
      ...
      for (int Vid = 0; Vid < N_EOSvolumes; Vid++) {
        err = std::abs(a + b * EOSvolumes[Vid] + c * pow(EOSvolumes[Vid], 2) + d * pow(EOSvolumes[Vid], 3) - xomega[Vid + 1]) / xomega[Vid + 1];
        err *= 100.0;
        if (err >= 10.0) {
  ...
  double QHA::calcGrueneisenFD(const xvector<double> &xomega) {
    ...
    if (xomega[2] > _ZERO_TOL_) {
      return -GPvolumes[1] / xomega[2] * (xomega[3] - xomega[1]) / (GPvolumes[2] - GPvolumes[0]);
    } else {
      return 0.0;
    }
  }
  ...
  ///
  double QHA::calcFreeEnergyFit(double T, double V, EOSmethod eos_method, QHAmethod qha_method, uint contrib) {
    if (T < _ZERO_TOL_) {
      T = 0.0;
    }

    xvector<double> F = calcFreeEnergy(T, qha_method, contrib);
    const xvector<double> p = fitToEOSmodel(F, eos_method);
    return evalEOSmodel(V, p, eos_method);
  }

  /// Calculates the free energy as a function of temperature.
  xvector<double> QHA::calcFreeEnergy(double T, QHAmethod qha_method, uint contrib) {
    string msg;
    ...
    if (contrib & F_ELEC) {
      if (doSommerfeldExpansion) {
        F += calcElectronicFreeEnergySommerfeld(T);
      } else {
  ...
  double QHA::calcBulkModulus(double V, const xvector<double> &parameters, EOSmethod method) {
    ...
    switch (method) {
      case (EOS_SJ): {
        ...
        const xvector<double> dEdp = aurostd::evalPolynomialDeriv(x, parameters, 2);
  ...
  double QHA::calcBprime(double V, const xvector<double> &parameters, EOSmethod method) {
    ...
    switch (method) {
      case (EOS_SJ): {
        const double x = std::pow(V, -1.0 / 3.0);
        const xvector<double> dEdp = aurostd::evalPolynomialDeriv(x, parameters, 3);
        Bp = Bprime_SJ(x, dEdp);
      } break;
      case (EOS_BIRCH_MURNAGHAN2):
      case (EOS_BIRCH_MURNAGHAN3):
  ...
  double QHA::calcInternalEnergyFit(double T, double V, EOSmethod method) {
    ...
    static const xvector<double> xvolumes = aurostd::vector2xvector(EOSvolumes);
  ...
  double QHA::calcVibFreeEnergy(double T, int id) {
    return eos_vib_thermal_properties[id].getVibrationalFreeEnergy(T, apl::eV) / NatomsOrigCell;
  }
  ...
  ///
  xvector<double> QHA::fitToEOSmodel(xvector<double> &V, xvector<double> &E, EOSmethod method) {
    string msg;
  ...
  double QHA::evalEOSmodel(double V, const xvector<double> &p, EOSmethod method) {
    ...
    switch (method) {
      case (EOS_SJ): energy = aurostd::evalPolynomial(std::pow(V, -1.0 / 3.0), p); break;
      case (EOS_BIRCH_MURNAGHAN2):
      case (EOS_BIRCH_MURNAGHAN3):
      case (EOS_BIRCH_MURNAGHAN4): energy = aurostd::evalPolynomial(std::pow(V, -2.0 / 3.0), p); break;
      case (EOS_MURNAGHAN): energy = Murnaghan(V, p, dydp); break;
      default:
        msg = "Nonexistent EOS method was passed to " + string(__func__);
        throw aurostd::xerror(__AFLOW_FILE__, QHA_ARUN_MODE, msg, _INPUT_UNKNOWN_);
        break;
  ...
  /// @param qha_method defines what kind of QHA calculation is performed.
  ///
  double QHA::getEqVolumeT(double T, EOSmethod eos_method, QHAmethod qha_method, uint contrib) {
    xvector<double> F = calcFreeEnergy(T, qha_method, contrib);
  ...
  void QHA::calcCVandGP(double T, double &CV, double &GP) {
    ...
    const uint NIrQpoints = omegaV_mesh.size();
    ...
    const double beta = 1.0 / KBOLTZEV / T;
  ...
  void QHA::calcCVandGPfit(double T, double V, double &CV, double &GP) {
    ...
    const uint NIrQpoints = omegaV_mesh_EOS.size();
  ...
  ///
  double QHA::calcThermalExpansion(double T, EOSmethod eos_method, QHAmethod qha_method, uint contrib) {
    if (!(T > 0)) {
      return 0;
    }

    const double dT = DCOEFF * T;
    return 0.5 * (getEqVolumeT(T + dT, eos_method, qha_method, contrib) - getEqVolumeT(T - dT, eos_method, qha_method, contrib)) / dT / getEqVolumeT(T, eos_method, qha_method, contrib);
  }
  ...
  /// @param eos_method defines which model is used for the EOS fit
  ///
  double QHA::calcIsochoricSpecificHeat(double T, double V, EOSmethod eos_method, QHAmethod qha_method, uint contrib) {
    const double dT = DCOEFF * T;
    double CV = 0;
    if (T > 0) {
      CV = -(calcFreeEnergyFit(T + dT, V, eos_method, qha_method, contrib) - 2 * calcFreeEnergyFit(T, V, eos_method, qha_method, contrib) + calcFreeEnergyFit(T - dT, V, eos_method, qha_method, contrib));
      CV *= T / pow(dT, 2);
    }
  ...
  ///
  double QHA::calcEntropy(double T, double V, EOSmethod eos_method, QHAmethod qha_method, uint contrib) {
    const double dT = DCOEFF * T;
    return -0.5 * (calcFreeEnergyFit(T + dT, V, eos_method, qha_method, contrib) - calcFreeEnergyFit(T - dT, V, eos_method, qha_method, contrib)) / dT;
  }
      ...
      for (uint band = 0; band < static_eigvals[id].number_bands; band++) {
        for (uint i = 0; i < static_ibzkpts[id].ntetrahedra; i++) {
          ...
          for (uint s = 0; s < static_eigvals[id].spin + 1; s++) {
            ...
            if (Ef < energy_tetrahedron[1] && energy_tetrahedron[4] < Ef) {
              DEf += 0;
            }
  ...
  double QHA::calcChemicalPotential(double T, int Vid) {
    ...
    const bool LDEBUG = (false || DEBUG_QHA || XHOST.DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " begin" << std::endl;
    }

    // Fermi energy is used as a starting guess for the value of chemical potential
    const double guess = Efermi_V[Vid];
    const double f = calcIDOS(guess, T, static_eigvals[Vid]) - Nelectrons;
    if (std::abs(f) < AUROSTD_IDENTITY_TOL) {
      return guess;
    }

    double left_end = 0.0;
    double middle = 0.0;
    double right_end = 0.0;
    double f_at_left_end = 0.0;
    double f_at_middle = 0.0;
    double f_at_right_end = 0.0;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " end" << std::endl;
    }
    return middle;
  }
  ...
  double QHA::calcElectronicFreeEnergy(double T, int Vid) {
    ...
    const double mu = calcChemicalPotential(T, Vid);
    const double mu0 = calcChemicalPotential(Tmin, Vid);
    double E = 0.0;
    double f = 0.0;
    double f0 = 0.0;
    double weight = 0.0;
    for (uint k = 0; k < eig.number_kpoints; k++) {
      ...
      for (uint s = 0; s <= eig.spin; s++) {
        for (uint b = 0; b < eig.number_bands; b++) {
          E = eig.venergy[k][b][s];
          f = aurostd::FermiDirac(E, mu, T);
          f0 = aurostd::FermiDirac(E, mu0, Tmin);
          U += E * weight * (f - f0);

          if (f > 0 && f < 1) {
            S -= (f * log(f) + (1 - f) * log(1 - f)) * weight;
          }
        }
      }
    }
  ...
  ///
  double QHA::extrapolateFrequency(double V, const xvector<double> &xomega, QHAmethod qha_method) {
    string msg;
    double result = 0.0;
    switch (qha_method) {
      ...
      case (QHA3P_CALC):
      case (SCQHA_CALC): {
        const double dwdV = (xomega[1] - xomega[3]) / (GPvolumes[0] - GPvolumes[2]);
        const double d2wdV2 = (xomega[1] + xomega[3] - 2.0 * xomega[2]) / pow(0.5 * (GPvolumes[0] - GPvolumes[2]), 2);

        const double dV = (V - GPvolumes[1]);
      ...
      case (QHANP_CALC): {
        ...
        const double V0 = QHANPvolumes[TaylorExpansionOrder];
        int order_begin = 0;
        int order_end = 0;
        double deriv = 0.0;
      ...
      case (QHA_CALC):
        ...
        throw aurostd::xerror(__AFLOW_FILE__, QHA_ARUN_MODE, msg, _INPUT_ILLEGAL_);
        break;
    }
    return result;
  }
  ...
  ///
  double QHA::extrapolateGrueneisen(double V, const xvector<double> &xomega, QHAmethod qha_method) {
    double gamma = 0.0;
    const double w = extrapolateFrequency(V, xomega, qha_method);
    ...
    if (w > 1e-6) {
      switch (qha_method) {
        ...
        case (SCQHA_CALC): {
          double dwdV = 0;
          double d2wdV2 = 0;
          const double V0 = GPvolumes[1];
          dwdV = (xomega[1] - xomega[3]) / (GPvolumes[0] - GPvolumes[2]);
          d2wdV2 = (xomega[1] + xomega[3] - 2.0 * xomega[2]) / pow(0.5 * (GPvolumes[0] - GPvolumes[2]), 2);
        ...
        default:
          ...
          throw aurostd::xerror(__AFLOW_FILE__, QHA_ARUN_MODE, message, _INPUT_ILLEGAL_);
          break;
      }
    }
  ...
  double QHA::calcVibFreeEnergyTaylorExpansion(double T, int Vid, QHAmethod qha_method) {
    ...
    for (size_t q = 0; q < omegaV.size(); q++) {
      for (int branch = 0; branch < Nbranches; branch++) {
        xomega = aurostd::vector2xvector(omegaV[q][branch]);

        w = extrapolateFrequency(EOSvolumes[Vid], xomega, qha_method) * THz2Hz * PLANCKSCONSTANTEV_h;
        fi = 0.5 * w;

        if (w > AUROSTD_IDENTITY_TOL && T > _ZERO_TOL_) {
          fi += KBOLTZEV * T * log(1 - exp(-w * beta));
        }
  ...
  double QHA::calcInternalEnergyTaylorExpansion(double T, double V, QHAmethod qha_method) {
    ...
    for (size_t q = 0; q < omegaV.size(); q++) {
      for (int branch = 0; branch < Nbranches; branch++) {
        ...
        if (w > _AUROSTD_XSCALAR_TOLERANCE_IDENTITY_ && T > _AUROSTD_XSCALAR_TOLERANCE_IDENTITY_) {
          ui += w / (exp(w * beta) - 1.0);
        }
  ...
  double QHA::calcVPgamma(double T, double V) {
    double calcVPgamma = 0.0;
    double ui = 0.0;
    double w = 0.0;
    const double beta = 1.0 / KBOLTZEV / T;
    int NQpoints = 0;
    xvector<double> xomega;
    for (size_t q = 0; q < omegaV_mesh.size(); q++) {
      for (int branch = 0; branch < Nbranches; branch++) {
        xomega = aurostd::vector2xvector(omegaV_mesh[q][branch]);
  ...
  double QHA::calcSCQHAequilibriumVolume(double T, double Vguess, xvector<double> &fit_params, EOSmethod method) {
    ...
    double Pe = 0.0;
    double VPg = 0.0; // electronic pressure and volume multiplied by phononic pressure
    double Vnew = 0.0;
    double V = Vguess;
    int iter = 0;
  ...
  double QHA::calcSCQHAequilibriumVolume(double T, EOSmethod method) {
    ...
    const double Vguess = SCQHA_INITIAL_VOLUME_FACTOR * calcEquilibriumVolume(fit_params, method);
  ...
  /// http://dx.doi.org/10.1103/PhysRevMaterials.3.073801
  /// and https://doi.org/10.1016/j.commatsci.2016.04.012
  void QHA::runSCQHA(EOSmethod method, bool all_iterations_self_consistent, const string &directory) {
    ...
    // to avoid division by zero in the self-consistent loop,
    // the initial volume is taken to be 10% bigger
    const double V0K = calcSCQHAequilibriumVolume(0, SCQHA_INITIAL_VOLUME_FACTOR * EOS_volume_at_equilibrium, fit_params, method);

    // the name of the output file depends on the EOS fit method
    stringstream file;
    const string filename = directory + '/' + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_THERMO_FILE;
    const string sc = all_iterations_self_consistent ? "SC_" : "";
    string blockname = "[SCQHA_" + sc;
    switch (method) {
      case (EOS_SJ): blockname += "SJ_THERMO]"; break;
      case (EOS_BIRCH_MURNAGHAN2): blockname += "BM2_THERMO]"; break;
      case (EOS_BIRCH_MURNAGHAN3): blockname += "BM3_THERMO]"; break;
      case (EOS_BIRCH_MURNAGHAN4): blockname += "BM4_THERMO]"; break;
      case (EOS_MURNAGHAN): blockname += "M_THERMO]"; break;
      default:
        msg = "Nonexistent EOS method was passed to " + string(__func__);
        throw aurostd::xerror(__AFLOW_FILE__, QHA_ARUN_MODE, msg, _INPUT_UNKNOWN_);
        break;
    ...
    // print header
    file << setw(5) << "# T[K]" << setw(SW) << ' ' << setw(TW) << "Veq[ev/atom]" << setw(SW) << ' ' << setw(TW) << "F(Veq)[eV/atom]" << setw(SW) << ' ' << setw(TW) << "B[GPa]" << setw(SW) << ' ' << setw(TW)
         << "beta[10^-5/K]" << setw(SW) << ' ' << setw(TW) << "Cv[kB/atom]" << setw(SW) << ' ' << setw(TW) << "Cp[kB/atom]" << setw(SW) << ' ' << setw(TW) << "gamma" << setw(SW) << ' ' << std::endl;

    double T = Temperatures[0];

    // to avoid division by zero in the self-consistent loop,
    // the initial volume is taken to be 10% bigger
    double V = calcSCQHAequilibriumVolume(T, SCQHA_INITIAL_VOLUME_FACTOR * EOS_volume_at_equilibrium, fit_params, method);

    const double dT = (Temperatures[Ntemperatures - 1] - Temperatures[0]) / (Ntemperatures - 1);
    ...
    const uint NIrQpoints = omegaV_mesh.size(); // number of irreducible q-points
    ...
    for (int Tid = 0; Tid < Ntemperatures; Tid++) {
      ...
      file << setw(5) << T << setw(SW) << ' ' << setw(TW) << V << setw(SW) << ' ' << setw(TW) << Feq << setw(SW) << ' ' << setw(TW) << B << setw(SW) << ' ' << setw(TW) << beta * 1e5 << setw(SW) << ' ' << //[10^-5/K]
          setw(TW) << CV << setw(SW) << ' ' << setw(TW) << CP << setw(SW) << ' ' << setw(TW) << GP << setw(SW) << ' ' << std::endl;
    }
    file << blockname + "STOP" << std::endl;
    file << AFLOWIN_SEPARATION_LINE << std::endl;

    if (!aurostd::stringstream2file(file, filename, aurostd::compression_type::None, "APPEND")) {
      msg = "Error writing to " + filename + "file.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, msg, _FILE_ERROR_);
    }
  }
  ...
  ///
  void QHA::writeThermalProperties(EOSmethod eos_method, QHAmethod qha_method, const string &directory) {
    ...
    string blockname = "[";
    switch (qha_method) {
      case (QHANP_CALC): blockname += "QHANP_"; break;
      case (QHA3P_CALC): blockname += "QHA3P_"; break;
      case (QHA_CALC): blockname += "QHA_"; break;
      default:
        msg = "Nonexistent QHA method was passed to " + string(__func__);
        throw aurostd::xerror(__AFLOW_FILE__, QHA_ARUN_MODE, msg, _INPUT_UNKNOWN_);
        break;
    ...
    // QHA calculation
    switch (eos_method) {
      case (EOS_SJ): blockname += "SJ"; break;
      case (EOS_BIRCH_MURNAGHAN2): blockname += "BM2"; break;
      case (EOS_BIRCH_MURNAGHAN3): blockname += "BM3"; break;
      case (EOS_BIRCH_MURNAGHAN4): blockname += "BM4"; break;
      case (EOS_MURNAGHAN): blockname += "M"; break;
      default:
        msg = "Nonexistent EOS method was passed to " + string(__func__);
        throw aurostd::xerror(__AFLOW_FILE__, QHA_ARUN_MODE, msg, _INPUT_UNKNOWN_);
        break;
    ...
    const string blockname_coeff = blockname + "_COEFF]";
    const string blockname_mesh = blockname + "_THERMO_MESH]";
    blockname += "_THERMO]";
    ...
    stringstream thermo_block;
    stringstream thermo_mesh_block;

    // write header
    thermo_block << setw(5) << "#T[K]" << setw(SW) << ' ' << setw(TW) << "V[A^3/atom]" << setw(SW) << ' ' << setw(TW) << "F(V)[eV/atom]" << setw(SW) << ' ' << setw(TW) << "B[GPa]" << setw(SW) << ' ' << setw(TW)
                 << "beta[10^-5/K]" << setw(SW) << ' ' << setw(TW) << "Cv(V)[kB/atom]" << setw(SW) << ' ' << setw(TW) << "Cp(V)[kB/atom]" << setw(SW) << ' ' << setw(TW) << "gamma" << setw(SW) << ' ' << setw(TW)
                 << "Bprime";
    // the following properties are calculated only with a regular QHA calculation
    if (qha_method == QHA_CALC) {
      thermo_mesh_block << setw(5) << "#T[K]" << setw(SWTM) << ' ' << setw(TWTM) << "gamma" << setw(SWTM) << ' ' << setw(TWTM) << "beta[10^-5/K]" << setw(SWTM) << ' ' << setw(TWTM) << "Cv[kB/atom]"
                        << setw(SWTM) << ' ' << setw(TWTM) << "Cp[kB/atom]" << setw(SWTM) << ' ' << setw(TWTM) << "gamma(V0)" << setw(SWTM) << ' ' << setw(TWTM) << "beta(V0)[10^-5/K]" << setw(SWTM) << ' '
                        << setw(TWTM) << "Cv(V0)[kB/atom]" << setw(SWTM) << ' ' << setw(TWTM) << "Cp(V0)[kB/atom]";
    }
    thermo_block << std::endl;
    thermo_mesh_block << std::endl;

    xvector<double> F(N_EOSvolumes); // free energy
    const xvector<double> xvolumes = aurostd::vector2xvector(EOSvolumes);
    aurostd::cematrix VM(aurostd::Vandermonde_matrix(xvolumes, 3));

    F = calcFreeEnergy(0, qha_method, F_VIB);

    fitToEOSmodel(F, eos_method);
    const double V0K = EOS_volume_at_equilibrium; // equilibrium volume at 0K
    const double B0K = EOS_bulk_modulus_at_equilibrium; // bulk modulus at 0K
    ...
    double CV_mesh_V0K = 0.0;
    double GP_mesh_V0K = 0.0;
    double CP_mesh_V0K = 0.0;
    double beta_mesh_V0K = 0.0;
    double CV_mesh_V = 0.0;
    double GP_mesh_V = 0.0;
    double CP_mesh_V = 0.0;
    double beta_mesh_V = 0.0;

    // if electronic contributions are requested
    double beta_elec_V0K = 0.0;
    double beta_elec_V = 0.0;
    double CV_elec_V0K = 0.0;
    double CV_elec_V = 0.0;
    ...
    try {
      for (int Tid = 0; Tid < Ntemperatures; Tid++) {
        ...
        if (qha_method == QHA_CALC) {
          if (includeElectronicContribution) {
            if (doSommerfeldExpansion) {
              ...
              CV_elec_V0K = calcIsochoricSpecificHeat(T, V0K, eos_method, qha_method, F_ELEC) / KBOLTZEV; // [kB/atom]
              CV_elec_V = calcIsochoricSpecificHeat(T, Veq, eos_method, qha_method, F_ELEC) / KBOLTZEV; // [kB/atom]
        ...
        thermo_block << setw(5) << T << setw(SW) << ' ' << std::fixed << std::setprecision(PRECISION) << setw(TW) << Veq << setw(SW) << ' ' << setw(TW) << Feq << setw(SW) << ' ' << //[eV/atom]
            setw(TW) << B << setw(SW) << ' ' << setw(TW) << beta * 1e5 << setw(SW) << ' ' << //[10^-5/K]
            setw(TW) << CV << setw(SW) << ' ' << //[kB/atom]
            setw(TW) << CP << setw(SW) << ' ' << //[kB/atom]
            setw(TW) << GP << setw(SW) << ' ' << setw(TW) << Bp;
        // the following properties are calculated only with a regular QHA calculation
        if (qha_method == QHA_CALC) {
          thermo_mesh_block.unsetf(ios_base::floatfield);
          thermo_mesh_block << setw(5) << T << setw(SWTM) << ' ' << std::fixed << std::setprecision(PRECISION) << setw(TWTM) << GP_mesh_V << setw(SWTM) << ' ' << setw(TWTM) << beta_mesh_V * 1e5 << setw(SWTM)
                            << ' ' << //[10^-5/K]
              setw(TWTM) << CV_mesh_V << setw(SWTM) << ' ' << //[kB/atom]
              setw(TWTM) << CP_mesh_V << setw(SWTM) << ' ' << //[kB/atom]
              setw(TWTM) << GP_mesh_V0K << setw(SWTM) << ' ' << setw(TWTM) << beta_mesh_V0K * 1e5 << setw(SWTM) << ' ' << //[10^-5/K]
      ...
      if (e.whatCode() == _VALUE_RANGE_) {
        pflow::logger(e.whereFileName(), e.whereFunction(), e.buildMessageString(), currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
      } else {
        throw;
      }
    }
    ...
    if (aurostd::FileExist(filename)) {
      if (!aurostd::stringstream2file(file_coeff, filename, aurostd::compression_type::None, "APPEND")) {
        msg = "Error writing to " + filename + " file.";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, msg, _FILE_ERROR_);
      }
    } else {
  ...
  void QHA::writeFVT(const string &directory) {
    ...
    stringstream file;
    const string filename = directory + '/' + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_FVT_FILE;
    file.precision(10);
    ...
    for (int Tid = 0; Tid < Ntemperatures; Tid++) {
      ...
      for (int Vid = 0; Vid < N_EOSvolumes; Vid++) {
        ...
        file << setw(TW) << EOSvolumes[Vid] << setw(SW) << ' ' << setw(TW) << Ftot << setw(SW) << ' ' << setw(TW) << calcVibFreeEnergy(T, Vid) << setw(SW) << ' ' << setw(TW) << Felec[Vid + 1] << setw(SW) << ' '
             << setw(TW) << E0_V[Vid] << std::endl;
      }
  ...
  void QHA::writeGPpath(double V, const string &directory) {
    ...
    const string filename = directory + '/' + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_GP_PATH_FILE;
  ...
  void QHA::writeAverageGPfiniteDifferences(const string &directory) {
    ...
    stringstream file;
    const string filename = directory + '/' + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_GP_AVG_FILE;
    file.precision(10);

    file << AFLOWIN_SEPARATION_LINE << std::endl;
    file << "[QHA_AVG_GP]START" << std::endl;

    file << setw(5) << "# T[K]" << setw(SW) << ' ' << setw(TW) << "gamma" << setw(SW) << ' ' << setw(TW) << "CV" << setw(SW) << ' ' << std::endl;
    ...
    for (int Tid = 0; Tid < Ntemperatures; Tid++) {
      ...
      file << setw(5) << T << setw(SW) << ' ' << setw(TW) << GP << setw(SW) << ' ' << setw(TW) << CV << setw(SW) << ' ' << std::endl;
    }
  ...
  void QHA::writeGPmeshFD(const string &directory) {
    ...
    stringstream file;
    const string filename = directory + '/' + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_GP_MESH_FILE;
    file.precision(6);
    ...
    xeigen.venergy.resize(NIrQpoints, deque<deque<double>>(xeigen.number_bands, deque<double>(2)));
  ...
  void QHA::writeFrequencies(const string &directory) {
    string msg;
    stringstream file;
    const string filename = directory + '/' + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_FREQS_FILE;

    file << AFLOWIN_SEPARATION_LINE << std::endl;
    file << "[QHA_FREQUENCIES]START" << std::endl;
    for (size_t q = 0; q < omegaV_mesh_EOS.size(); q++) {
      for (int branch = 0; branch < Nbranches; branch++) {
        for (int Vid = 0; Vid < N_EOSvolumes; Vid++) {
  ...
  void QHA::writeTphononDispersions(EOSmethod eos_method, QHAmethod qha_method, const string &directory) {
    string msg;
    double V = 0.0;
    int T = 0;
    xvector<double> xomega;
      ...
      try {
        ...
        switch (qha_method) {
          case (QHA_CALC):
            ...
            // here QHA3P and SCQHA share the same code
          case (QHA3P_CALC):
          case (SCQHA_CALC): eig = gp_ph_dispersions.front(); break;
          case (QHANP_CALC):
        ...
        // venergy.at(kpoint number).at(band number).at(spin number)
        for (size_t q = 0; q < eig.venergy.size(); q++) {
          for (int branch = 0; branch < Nbranches; branch++) {
            switch (qha_method) {
              case (QHA_CALC):
                for (int Vid = 0; Vid < N_EOSvolumes; Vid++) {
        ...
        switch (qha_method) {
          case (QHA_CALC): filename += DEFAULT_QHA_FILE_PREFIX; break;
          case (QHA3P_CALC): filename += DEFAULT_QHA3P_FILE_PREFIX; break;
          case (SCQHA_CALC): filename += DEFAULT_SCQHA_FILE_PREFIX; break;
          case (QHANP_CALC):
            // not supported, this case is handled in an earlier switch statement
            break;
        }
        ...
        // printed, and all calculated data should be saved to the file
        if (e.whatCode() == _VALUE_RANGE_) {
          pflow::logger(e.whereFileName(), e.whereFunction(), "called by " + __AFLOW_FUNC__ + " " + e.what(), currentDirectory, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
        } else {
          throw;
        }
      }
  ...
  void QHA::writeQHAresults(const string &directory) {
    ...
    try {
      ...
      const double volume = EOS_volume_at_equilibrium;
      const double free_energy = EOS_energy_at_equilibrium;
      const double bulk_modulus = EOS_bulk_modulus_at_equilibrium;
      const double bprime = EOS_Bprime_at_equilibrium;
      const double thermal_expansion = calcThermalExpansion(T, EOS_SJ, QHA_CALC, f_contrib);

      double CV = 0.0;
      double grueneisen_300K = 0.0;
      calcCVandGPfit(T, V0K, CV, grueneisen_300K);
      const double grueneisen = calcGPinfFit(V0K);

      CV = calcIsochoricSpecificHeat(T, volume, EOS_SJ, QHA_CALC, f_contrib) / KBOLTZEV; // [kB/atom]
      const double CP = CV + volume * T * bulk_modulus * pow(thermal_expansion, 2) / eV2GPa / KBOLTZEV; // [kB/atom]
      ...
      const string filename = directory + '/' + DEFAULT_QHA_FILE_PREFIX + "out";
...
namespace apl {
  ...
  bool QHA_Get_AflowInName(string &AflowInName, const string &directory_LIB) {
    ...
    string aflowin_name;
    string aflowin;
    string fullpath_aflowin_name;
    for (size_t i = 0; i < vaflowins.size(); i++) {
      aflowin_name = vaflowins[i];
      fullpath_aflowin_name = directory_LIB + '/' + aflowin_name;
      if (aurostd::CompressFileExist(fullpath_aflowin_name, stmp) && aurostd::IsCompressed(stmp)) {
      ...
      if (aurostd::substring2bool(aflowin, "[AFLOW_QHA]CALC", true) || aurostd::substring2bool(aflowin, "[VASP_QHA]CALC", true)) {
  ...
  string EOSmethod2label(EOSmethod eos_method) {
    const string function = "EOSmethod2label():";
    string msg;

    switch (eos_method) {
      case (EOS_SJ): return "SJ"; break;
      case (EOS_BIRCH_MURNAGHAN2): return "BM2"; break;
      case (EOS_BIRCH_MURNAGHAN3): return "BM3"; break;
      case (EOS_BIRCH_MURNAGHAN4): return "BM4"; break;
      case (EOS_MURNAGHAN): return "M"; break;
      default:
  ...
  string QHAmethod2label(QHAmethod qha_method) {
    const string function = "QHAmethod2label():";
    string msg;

    switch (qha_method) {
      case (QHA_CALC): return "QHA"; break;
      case (QHA3P_CALC): return "QHA3P"; break;
      case (SCQHA_CALC): return "SCQHA"; break;
      case (QHANP_CALC): return "QHANP"; break;
      default:
  ...
  bool hasImaginary(const string &filename, const string &QHA_method) {
    const string function = "hasImaginary():";
    string msg;

    vector<string> vlines;
    bool has_imaginary = false;
    if (!aurostd::compressfile2vectorstring(filename, vlines)) {
