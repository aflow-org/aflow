...
namespace pocc {
  ...
  EnsembleThermo::EnsembleThermo(const string &currentDir, vector<string> &directories, const string &fname, const string &calc_type, apl::EOSmethod eos_method, bool isFVTprovided, ofstream &FileMESSAGE, ostream &oss) {
    ...
    const bool LDEBUG = false || _DEBUG_POCC_THERMO_ || XHOST.DEBUG;
    ...
    for (size_t i = 0; i < directories.size(); i++) {
      ...
      if (!aurostd::CompressFileExist(file)) {
        ...
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, msg, directories[i], *p_FileMESSAGE, *p_oss, _LOGGER_ERROR_);
        return;
      }

      if (isFVTprovided) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << "FVT mode" << std::endl;
        }

        uint Nvolumes = 0;
        uint Ntemperatures = 0;
        ...
        readFVTdata(directories[i], fname, calc_type, Nvolumes, Ntemperatures, T, coeffs, Vmin, Vmax);
      } else {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << "COFF mode" << std::endl;
        }

        readCoeffParameters(file, Vmin, Vmax);

        const apl::EOSmethod eos_method = apl::EOS_SJ;
        if (!readCoeffData(file, calc_type + "_" + apl::EOSmethod2label(eos_method), T, coeffs)) {
          msg = "No data was extracted for " + calc_type;
          msg += " with " + apl::EOSmethod2label(eos_method) + " EOS.";
          msg += " The calculation will be stopped.";
          pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, msg, directories[i], *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
          return;
        }
      }
    ...
    for (uint i = 0; i < Nstructures - 1; i++) {
      for (int row = 1; row <= nrows; row++) {
        if (!aurostd::isequal(T_list[i][row], T_list[i + 1][row])) {
          ...
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, msg, _VALUE_ILLEGAL_);
        }
      }
    }
    T = T_list[min_id];
    ...
    const int ncols = coeffs_list[0].cols;
    for (uint i = 1; i < Nstructures; i++) {
  ...
  /// the FVT data.
  void EnsembleThermo::readFVTParameters(const string &filename, const string &blockname, uint &Nvolumes, uint &Ntemperatures) {
    string msg;

    Nvolumes = 0;
    Ntemperatures = 0;

    const string file = aurostd::compressfile2string(filename);

    const string block = "[" + blockname + "_FVT_PARAMETERS]";
    const string params_block = aurostd::substring2string(file, block + "START", block + "STOP", 1);

    vector<string> v_params_block = aurostd::string2vectorstring(params_block);
    vector<uint> tokens;
    for (size_t i = 0; i < v_params_block.size(); i++) {
  ...
  /// @param blockname defines the type of calculation that was used to generate
  /// the FVT data.
  void EnsembleThermo::readFVTdata(const string &dirname, const string &filename, const string &blockname, uint n_volumes, uint n_temperatures, xvector<double> &t, xmatrix<double> &c, double &Vmin, double &Vmax) {
    string msg;
    const string block = "[" + blockname + "_FVT]";
    string filename_compressed = dirname + "/" + filename;
    if (!(aurostd::CompressFileExist(filename_compressed, filename_compressed))) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "File " + filename_compressed + " not found", _FILE_NOT_FOUND_);
    }
    const string data_block = aurostd::substring2string(aurostd::compressfile2string(filename_compressed), block + "START", block + "STOP", 1);
    ...
    xvector<double> V(n_volumes);
    xvector<double> F(n_volumes);
    double T = 0.0;
    int id = 0;
    apl::QHA qha;
    for (uint t = 0; t < n_temperatures; t++) {
      ...
      try {
        ...
        if (e.whatCode() == _VALUE_RANGE_) {
          pflow::logger(e.whereFileName(), e.whereFunction(), e.what(), dirname, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
          break;
        } else {
          throw;
        }
  ...
  bool EnsembleThermo::readCoeffData(const string &filename, const string &blockname, xvector<double> &T, xmatrix<double> &coeffs) {
    string msg;
    const string block = "[" + blockname + "_COEFF]";
    const bool LDEBUG = false || _DEBUG_POCC_THERMO_ || XHOST.DEBUG;
    string filename_compressed;

    // extract data
    if (!(aurostd::CompressFileExist(filename, filename_compressed))) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "File " + filename + " not found", _FILE_NOT_FOUND_);
    }
    const string data = aurostd::substring2string(aurostd::compressfile2string(filename_compressed), block + "START", block + "STOP", 1);
  ...
  void EnsembleThermo::readCoeffParameters(const string &filename, double &Vmin, double &Vmax) {
    string msg;
    ...
    bool Vmin_found = false;
    bool Vmax_found = false;

    vector<double> tokens;
    const vector<string> stokens;
  ...
  double EnsembleThermo::logZ(const xvector<double> &E, const vector<int> &degeneracies, double T) {
    ...
    const double shift = max(khi);
    double sum = 0.0;
    const int gtotal = aurostd::sum(degeneracies);
  ...
  xvector<double> EnsembleThermo::calcThermalExpansionSG(const xvector<double> &volumes, double dT) {
    ...
    const int npoints = volumes.rows;
    if (npoints < 5) {
  ...
  xvector<double> EnsembleThermo::calcIsobaricSpecificHeatSG(const xvector<double> &free_energies, double dT) {
    ...
    SGmat[2][1] = -2.1428571428571428E+000;
    SGmat[2][2] = -1.1428571428571428E+000;
    SGmat[2][3] = -1.4285714285714285E-001;
    SGmat[3][1] = -2.8571428571428570E-001;
    SGmat[3][2] = -2.8571428571428570E-001;
    SGmat[3][3] = -2.8571428571428570E-001;
    SGmat[4][1] = 1.8571428571428570E+000;
    SGmat[4][2] = 8.5714285714285698E-001;
    SGmat[4][3] = -1.4285714285714285E-001;
    SGmat[5][1] = -7.1428571428571419E-001;
    SGmat[5][2] = -2.1428571428571425E-001;
    SGmat[5][3] = 2.8571428571428570E-001;

    SGmat[1][4] = -2.1428571428571425E-001;
    SGmat[1][5] = -7.1428571428571419E-001;
    SGmat[2][4] = 8.5714285714285698E-001;
    SGmat[2][5] = 1.8571428571428570E+000;
    SGmat[3][4] = -2.8571428571428570E-001;
    SGmat[3][5] = -2.8571428571428570E-001;
    SGmat[4][4] = -1.1428571428571428E+000;
    SGmat[4][5] = -2.1428571428571428E+000;
    ...
    string msg;
    const int npoints = free_energies.rows;
    if (npoints < 5) {
      msg = "Savitzky-Golay filter requires at least 5 points: only ";
      msg += aurostd::utype2string(npoints) + " were provided.";
      msg += " Make sure that the minimum range among the set of POCC structures,";
    ...
    for (int i = 1; i <= 5; i++) {
      dummy[i] = free_energies[npoints - 5 + i];
    }
    endpoints = dummy * SGmat;
    for (int i = 4; i <= 5; i++) {
      Cp[npoints - 5 + i] = endpoints[i];
    }
  ...
  void EnsembleThermo::calculateThermodynamicProperties() {
    ...
    try {
      for (int i = 1; i <= nrows; i++) {
        ...
        for (int v = volumes.lrows; v <= volumes.urows; v++) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " E: " << E << std::endl;
          }
      ...
      // printed, and all calculated data should be saved to the file
      if (e.whatCode() == _VALUE_RANGE_) {
        pflow::logger(e.whereFileName(), e.whereFunction(), e.what(), aurostd::getPWD(), *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
      } else {
        throw;
      }
    }
    ...
    const double dT = (max(T) - min(T)) / (nrows - 1);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " dT = " << dT << std::endl;
    }
  ...
  void EnsembleThermo::writeThermodynamicProperties() const {
    string msg;
    const apl::QHAmethod qha_method = apl::QHA_CALC;
    ...
    file << setw(5) << "#T[K]" << setw(SW) << ' ' << setw(TW) << "V[A^3/atom]" << setw(SW) << ' ' << setw(TW) << "F(V)[eV/atom]" << setw(SW) << ' ' << setw(TW) << "B[GPa]" << setw(SW) << ' ' << setw(TW)
         << "beta[10^-5/K]" << setw(SW) << ' ' << setw(TW) << "Cv(V)[kB/atom]" << setw(SW) << ' ' << setw(TW) << "Cp(V)[kB/atom]" << setw(SW) << ' ' << setw(TW) << "gamma(beta,B,Cv(V))" << setw(SW) << ' '
         << setw(TW) << "Bprime" << std::endl;

    for (int i = 1; i <= nrows; i++) {
      file << setw(5) << T[i] << setw(SW) << ' ' << setw(TW) << Veq[i] << setw(SW) << ' ' << setw(TW) << Feq[i] << setw(SW) << ' ' << setw(TW) << B[i] << setw(SW) << ' ' << setw(TW) << beta[i] * 1e5 << setw(SW)
           << ' ' << setw(TW) << Cv[i] << setw(SW) << ' ' << setw(TW) << Cp[i] << setw(SW) << ' ' << setw(TW) << gamma[i] << setw(SW) << ' ' << setw(TW) << Bprime[i] << std::endl;
    }
  ...
  void POccCalculator::calculateQHAProperties() {
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, msg, aurostd::getPWD(), *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    ...
    if (!dirs2ignore.empty()) {
      XHOST.vflag_control.flag("ARUNS2SKIP", true);
      XHOST.vflag_control.addattachedscheme("ARUNS2SKIP", aurostd::joinWDelimiter(dirs2ignore, ","), true);
    ...
    const string fname = DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_COEFF_FILE;
    EnsembleThermo ens(m_aflags.Directory, m_ARUN_directories, fname, apl::QHAmethod2label(qha_method), apl::EOS_SJ, false, *p_FileMESSAGE);
