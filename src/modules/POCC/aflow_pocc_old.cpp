...
#include "modules/POCC/aflow_pocc.h" //CO20180409
...
namespace pflow {
  void POCC_INPUT() {
    ...
    ofstream oss;
    string aflowin;
    const string MESSAGE = "pflow::POCC_INPUT ERROR";
...
bool POCC_GENERATE_INPUT(ofstream& FileMESSAGE, _aflags& aflags) {
  ...
  if (!pocc::POCC_Already_Calculated_Input(AflowIn, aflags.Directory)) {
    ...
    const xstructure xstr_pocc(sspoccSTRUCTURE, IOVASP_POSCAR);
...
// ***************************************************************************
template <typename T> void remove_duplicates(vector<T>& vec) {
...
str_num_data OptimizePoccValue(double dvalue, double tol) {
  ...
  for (size_t i = 0; i < vec_double.size(); i++) {
    const double num_diff = std::abs(vec_double[i] - dvalue);
    if (num_diff <= tol) {
      double_range.push_back(vec_double[i]);
    }
  }

  const vector<int> frac_tmp;
  vector<str_num_data> vec_str_num_data;
  str_num_data tmp_str_num_data;
  for (size_t i = 0; i < double_range.size(); i++) {
    const double db_tmp = double_range[i];
...
void UpdateXstr_comp_each_type(xstructure& b) {
  ...
  for (size_t i = 0; i < b.atoms.size(); i++) {
    const double tmp = b.atoms[i].partial_occupation_value;
    poccaus.push_back(tmp);
  }

  b.comp_each_type.clear();
  for (size_t i = 0; i < b.num_each_type.size(); i++) {
    b.comp_each_type.push_back(0.0);
  }

  for (size_t i = 0; i < b.atoms.size(); i++) {
    if (poccaus.at(i) < 1.0 - 1e-5) {
      b.atoms[i].partial_occupation_flag = true;
      b.atoms[i].partial_occupation_value = poccaus.at(i);
      b.comp_each_type.at(b.atoms[i].type) += b.atoms[i].partial_occupation_value;
    } else {
      b.atoms[i].partial_occupation_flag = false;
      b.atoms[i].partial_occupation_value = 1.0;
      b.comp_each_type.at(b.atoms[i].type) += b.atoms[i].partial_occupation_value;
    }
  }
}
...
void UpdateXstr(xstructure& xstr_orig, ofstream& FileMESSAGE, _aflags& aflags) {
  ...
  if (xstr_orig.partial_occupation_flag) {
    ...
    for (size_t i = 0; i < xstr_orig.atoms.size(); i++) {
      partial_value_tmp = xstr_orig.atoms[i].partial_occupation_value;
      str_num_data tmp;
      tmp = OptimizePoccValue(partial_value_tmp, epsilon);
      xstr_orig.atoms[i].partial_occupation_value = tmp.decimal;
...
// ***************************************************************************
double CalculateDistanceXstructure(xstructure& xstr, int i, int j) {
  const double d1 = xstr.atoms.at(i).cpos(1) - xstr.atoms.at(j).cpos(1);
  const double d2 = xstr.atoms.at(i).cpos(2) - xstr.atoms.at(j).cpos(2);
  const double d3 = xstr.atoms.at(i).cpos(3) - xstr.atoms.at(j).cpos(3);
  const double d = sqrt(d1 * d1 + d2 * d2 + d3 * d3);
  return d;
}
...
vector<vector<int>> CalculateLableXstructure(xstructure& xstr) {
  ...
  for (unsigned int i = 0; i < a.size(); i++) {
    ...
    if (CheckVacancyOnOnesite(xstr, i)) {
      const int NumVacancy = xstr.num_each_type.size();
...
bool CheckPartialOccupation(xstructure& xstr) {
  ...
  if (xstr.partial_occupation_flag) {
    for (unsigned int i = 0; i < xstr.atoms.size(); i++) {
      if (xstr.atoms[i].partial_occupation_flag) {
        RunFLAG = true;
      }
    }
  }
...
xstructure CleanVacancy(xstructure& xstr) {
  ...
  const int Nions = xstr.atoms.size();
  const int Ntype = xstr.num_each_type.size();
  const int NumLastType = xstr.num_each_type.at(Ntype - 1);
  ...
  if (CheckPartialOccupation(xstr)) {
    if (!CheckMultiOccupied(xstr)) {
      ...
      for (unsigned int i = 0; i < SumOccupation.size(); i++) {
        if ((1 - SumOccupation[i]) > 1E-4) {
          RunFLAG = true;
        } else if (SumOccupation[i] > 1) {
...
vector<vector<int>> CalculatecRange(xstructure& xstr) {
  ...
  const int nDFull = NormalisedNumberXstructure(xstr).size();
  ...
  if (CheckPartialOccupation(xstr)) {
    ...
    for (unsigned int i = 0; i < ConUniqueSite.size(); i++) {
      FracTmp = Decimal2Fraction(ConUniqueSite[i]);
      ConFrac = GetFraction(FracTmp);
      AllConFrac.push_back(ConFrac);
    }

    vector<int> denominatortmp;
    for (size_t i = 0; i < AllConFrac.size(); i++) {
      denominatortmp.push_back(AllConFrac[i].at(1));
    }
    const int Xstrlcm = CalculateLcmofVector(denominatortmp);

    int tmp;
    for (size_t i = 0; i < AllConFrac.size(); i++) {
      cRangeTmp.clear();
      tmp = Xstrlcm / AllConFrac[i].at(1) * AllConFrac[i].at(0);
    ...
    // int sum_num=0;
    // for (size_t i=0; i<XstrcRange.size();i++) {
    ...
    for (unsigned int i = 0; i < xstr.num_each_type.size(); i++) {
      cRangeTmp.clear();
      const int k = xstr.num_each_type[i];
      const int Nions = xstr.atoms.size();
...
vector<int> Decimal2Fraction(double a) {
  ...
  vector<int> FracTmp;
  int b;
  const double DecNum = a;
  // Debug
  double FracTmpDouble;
  double NumDiff = 1;
  if (std::abs(a - int(a)) < Tol_DecimalPart) {
    IntList.push_back(0);
    IntList.push_back(int(a));
  } else {
    while (std::abs(a) > Tol_DecimalPart && NumDiff > Tol_FracDeciDiff) {
      b = int(1.0 / a);
      if (b > MaxDenominator) {
        break;
      }
      ...
      NumDiff = std::abs(DecNum - FracTmpDouble);
    }
  }
  return IntList;
}
...
vector<int> GetFraction(vector<int> IntList) {
  ...
  vector<int> Fraction;
  const int LenghIntList = IntList.size();
  int tmp;
  int fac = 1;
  int Denom;
...
// ***************************************************************************
vector<int> double2fraction(double a) {
  const vector<int> IntList = Decimal2Fraction(a);
...
namespace pocc {
  string POSCAR2GulpInput(xstructure& xstr, vector<string> AtomSpecies) {
    ...
    int iatom = 0;
    for (size_t itype = 0; itype < xstr.num_each_type.size(); itype++) {
      for (int j = 0; j < xstr.num_each_type[itype]; j++) {
    ...
    for (size_t i = 0; i < xstr.atoms.size(); i++) {
      oss << xstr.atoms[i].name << '\t' << xstr.atoms[i].fpos(1) << '\t' << xstr.atoms[i].fpos(2) << '\t' << xstr.atoms[i].fpos(3) << endl;
    }
    oss << "#" << endl;
    oss << "species    " << xstr.num_each_type.size() << endl;

    string SpeciesTmp;
    for (size_t i = 0; i < AtomSpecies.size(); i++) {
      SpeciesTmp.clear();
      SpeciesTmp = AtomSpecies[i];
...
namespace pocc {
  double CalculateEnergyUsingGulp(xstructure& xstr, vector<string> AtomSpecies) {
    ...
    string GulpOut;
    const string GulpTmp = aurostd::TmpFileCreate("GulpTmp");
    ...
    const string RunGulp = "cat " + GulpTmp + " | gulp";
    ...
    const string anchor_word_Total_energy = "Total lattice energy";
    string line;
    vector<string> vgulp_output_line;
    vector<string> vgulp_output;
...
void PrintGulpEnergies(vector<xstructure>& vxstr) {
  ...
  for (size_t i = 0; i < etot_vxstr.size(); i++) {
    ostringstream oss;
    oss.setf(std::ios::fixed, std::ios::floatfield);
    oss.precision(12);
    oss << "No. " << i + 1 << " " << setw(15) << etot_vxstr[i] << endl;
    cout << oss.str();
  }
}
...
namespace pocc {
  vector<xstructure> SortGroupXstr(vector<xstructure> groupxstr, vector<string> AtomSpecies) {
    ...
    for (size_t i = 0; i < groupxstr.size(); i++) {
      strno_energy strno_energy_tmp;
      TotalEnergyTmp = pocc::CalculateEnergyUsingGulp(groupxstr[i], AtomSpecies);
    ...
    for (size_t i = 0; i < vec_group_strno_energy.size(); i++) {
      cerr << "No. " << setfill('0') << setw(6) << i + 1 << "   Str # " << setfill('0') << setw(6) << vec_group_strno_energy[i].number << " : " << vec_group_strno_energy[i].energy << " eV" << endl;
      number.push_back(vec_group_strno_energy[i].number);
    }
...
namespace pocc {
  vector<xstructure> SortGroupXstrUFFEnergy(vector<xstructure> groupxstr) {
    ...
    for (size_t i = 0; i < groupxstr.size(); i++) {
      strno_energy strno_energy_tmp;
      TotalEnergyTmp = pocc::CalculateUFFEnergy(groupxstr[i]);
    ...
    for (size_t i = 0; i < vec_group_strno_energy.size(); i++) {
      cout << "No. " << setfill('0') << setw(6) << i + 1 << "   Str # " << setfill('0') << setw(6) << vec_group_strno_energy[i].number << " : " << vec_group_strno_energy[i].energy << " eV" << endl;
      number.push_back(vec_group_strno_energy[i].number);
    }
...
vector<xmatrix<double>> CalculateHNF(xstructure str, int n) {
  ...
  xmatrix<double> Bi(3, 3);
  xmatrix<double> Bj(3, 3);
  xmatrix<double> R(3, 3);
  ...
  for (i = 0; i < (int) vHNF.size(); i++) {
    ...
    for (size_t istr = 0; istr < vB.size() && found == false; istr++) {                              // cycle through the unique vBs
      ...
      for (size_t pgroup = 0; pgroup < non_pocc_xstr.pgroup.size() && found == false; pgroup++) {                  // cycle through the pgroup of str
        R = trasp(non_pocc_xstr.pgroup[pgroup].Uc);
        if (LDEBUG) { // CO20180409
...
xstructure XstrSubstitute(xstructure& a, vector<int> vec_n, string b) {
  ...
  for (size_t i = 0; i < vec_n_sorted.size(); i++) {
    const int k = vec_n_sorted[i];
  ...
  const int ntype = xstr_tmp.num_each_type.size();
...
xstructure XstrSubstitute(xstructure& a, vector<int> vec_n, vector<string> b) {
  ...
  for (size_t i = 0; i < vec_num_name_new.size(); i++) {
    vec_num_sorted.push_back(vec_num_name_new[i].number);
    vec_str_sorted.push_back(vec_num_name_new[i].name);
  }

  xstructure xstr_tmp = a;
  // Firstly, we add all the atoms we need
  for (size_t i = 0; i < vec_num_sorted.size(); i++) {
    const int k = vec_num_sorted[i];
    _atom atom_replaced = xstr_tmp.atoms.at(k);
  ...
  // Clean Vacancies
  const int natoms = xstr_tmp.atoms.size();
...
xstructure NormalizeXstructure(xstructure a) {
  ...
  for (size_t i = 0; i < NormalisedNumber.size(); i++) {
    if (NormalisedNumber[i].size() >= 2) {
      for (size_t j = 1; j < NormalisedNumber[i].size(); j++) { // Keep the first one
        const int k = NormalisedNumber[i][j];
        removed_number.push_back(k);
      }
    }
  }
  vector<int> sorted_removed_number = SortMax2Min(removed_number);
  for (size_t i = 0; i < sorted_removed_number.size(); i++) {
    const int k = sorted_removed_number[i];
    b.RemoveAtom(k);
  }
  return b;
}
...
bool CheckDoubleOccupied(xstructure xstr_orig) {
  ...
  const vector<int> vec_one_site_num;
  for (size_t i = 0; i < num_norm_xstr.size(); i++) {
    if (num_norm_xstr[i].size() == 1 && CheckVacancyOnOnesite(xstr_orig, i)) {
      FLAG_double_occupied = true;
    }
    if (num_norm_xstr[i].size() == 2 && !CheckVacancyOnOnesite(xstr_orig, i)) {
      FLAG_double_occupied = true;
    }
  }
...
bool CheckTripleOccupied(xstructure xstr_orig) {
  ...
  for (size_t i = 0; i < num_norm_xstr.size(); i++) {
    if (num_norm_xstr[i].size() == 2 && CheckVacancyOnOnesite(xstr_orig, i)) {
      FLAG_triple_occupied = true;
    }
    if (num_norm_xstr[i].size() == 3 && !CheckVacancyOnOnesite(xstr_orig, i)) {
      FLAG_triple_occupied = true;
    }
  }
...
bool CheckFourfoldOccupied(xstructure xstr_orig) {
  ...
  for (size_t i = 0; i < num_norm_xstr.size(); i++) {
    if (num_norm_xstr[i].size() == 3 && CheckVacancyOnOnesite(xstr_orig, i)) {
      FLAG_fourfold_occupied = true;
    }
    if (num_norm_xstr[i].size() == 4 && !CheckVacancyOnOnesite(xstr_orig, i)) {
      FLAG_fourfold_occupied = true;
    }
  }
...
bool CheckFivefoldOccupied(xstructure xstr_orig) {
  ...
  for (size_t i = 0; i < num_norm_xstr.size(); i++) {
    if (num_norm_xstr[i].size() == 4 && CheckVacancyOnOnesite(xstr_orig, i)) {
      FLAG_fivefold_occupied = true;
    }
    if (num_norm_xstr[i].size() == 5 && !CheckVacancyOnOnesite(xstr_orig, i)) {
      FLAG_fivefold_occupied = true;
    }
  }
...
void CombineAll(const vector<vector<vector<int>>>& allVecs, size_t vecIndex, vector<vector<int>>& intSoFar, vector<vector<vector<int>>>& results) {
  ...
  if (vecIndex >= allVecs.size()) {
    ...
    for (size_t i = 0; i < intSoFar.size(); i++) {
      str_tmp.push_back(intSoFar[i]);
    }
  ...
  for (size_t i = 0; i < allVecs.at(vecIndex).size(); i++) {
    ...
    tmp.push_back(allVecs.at(vecIndex)[i]);
...
vector<string> CalculateSecondAtomicNameSupercell(xstructure xstr_orig, int n) {
  ...
  for (size_t i = 0; i < xstr_norm.atoms.size(); i++) {
    if (xstr_norm.atoms[i].partial_occupation_flag) {
      // if this is partial occupation
      if (num_norm_xstr.at(i).size() == 1) {
        // This means this site only contains vacancy
        for (size_t j = 0; j < vec_num_supercell.at(i).size(); j++) {
          AtomicNameSupercell.push_back(name_vacancy);
        }
      } else {
        // This means site site contains another element
        const int int_tmp = num_norm_xstr[i].at(1); // Read the atomic number in xstr_orig
        const string tmp_second_element_name = xstr_orig.atoms.at(int_tmp).name; // Read the name
        for (size_t j = 0; j < vec_num_supercell.at(i).size(); j++) {
      ...
      for (size_t j = 0; j < vec_num_supercell.at(i).size(); j++) {
        const string tmp_name = xstr_norm.atoms[i].name;
...
vector<vector<int>> GenerateSupercellAtomNumber(xstructure xstr_orig, int n) {
  ...
  const vector<vector<vector<int>>> vec_two_num_vacancy;

  const vector<vector<int>> num_norm_xstr = NormalisedNumberXstructure(xstr_orig); // format like 0 1 2; 3
  ...
  for (size_t i = 0; i < xstr_norm.atoms.size(); i++) {
    if (xstr_norm.atoms[i].partial_occupation_flag) {
      const int num_vacancy = hnf_double2int((1 - xstr_norm.atoms[i].partial_occupation_value) * n);
  ...
  vector<int> tmp;
  for (size_t i = 0; i < final_num_result.size(); i++) {
    tmp.clear();
    for (size_t j = 0; j < final_num_result[i].size(); j++) {
      for (size_t k = 0; k < final_num_result[i][j].size(); k++) {
        tmp.push_back(final_num_result[i][j][k]);
      }
    }
...
vector<xstructure> AssignPartialOccupation(xstructure xstr_supercell, xstructure xstr_orig, int n) {
  ...
  for (size_t i = 0; i < tmp_atom_num_supercell.size(); i++) {
    xstructure xstr_tmp = xstr_supercell;
    const vector<int> atom_number = tmp_atom_num_supercell[i];
    vector<int> atom_number_sorted = SortMax2Min(tmp_atom_num_supercell[i]);

    // Add all the needed atoms
    for (size_t i = 0; i < atom_number_sorted.size(); i++) {
      const int k = atom_number_sorted[i];
      _atom atom_replaced = xstr_tmp.atoms.at(k);
    ...
    const int LastAtom = xstr_tmp.atoms.size() - 1;
...
int CalculateSupercellSize(xstructure xstr_orig) {
  ...
  if (!xstr_orig.partial_occupation_HNF) {
    ...
    for (size_t i = 0; i < xstr_orig.atoms.size(); i++) {
      const double partial_value_tmp = xstr_orig.atoms[i].partial_occupation_value;
      str_num_data tmp;
...
xstructure AssignAlphabeticNameXstr(xstructure& xstr) {
  vector<string> names;
  names.emplace_back("As");
  names.emplace_back("Ba");
  names.emplace_back("Cl");
  names.emplace_back("Dy");
  names.emplace_back("Eu");
  names.emplace_back("Fe");
  names.emplace_back("Ge");
  names.emplace_back("He");

  xstructure xstr_new = xstr;
  int iatom = 0;
  for (size_t itype = 0; itype < xstr.num_each_type.size(); itype++) {
    const string species = string(names.at(xstr_new.atoms.at(iatom).type));
...
xstructure CleanNameXstr(xstructure& xstr) {
  xstructure xstr_new = xstr;
  for (size_t i = 0; i < xstr_new.atoms.size(); i++) {
    xstr_new.atoms[i].name_is_given = false;
  }
  return xstr_new;
}
...
vector<xstructure> AssignNameXstr(vector<xstructure>& vxstr, vector<string> names) {
  ...
  xstructure xstr_tmp;
  for (size_t i = 0; i < vxstr.size(); i++) {
    xstr_tmp = AssignNameXstr(vxstr[i], names);
...
namespace pocc {
  void HNFCELL(istream& input) {
    const xstructure xstr_ori(input, IOVASP_AUTO);
    const vector<xstructure> vxstr = Partial2Supercell(xstr_ori);
  }
} // namespace pocc
...
vector<xstructure> Partial2Supercell(xstructure xstr_ori) {
  ...
  if (xstr_ori.atoms.at(0).name_is_given) {
    str_species_ori = xstr_ori.SpeciesString();
  } else {
    str_species_ori = "As Ba Cl Dy Eu Fe Ga He In K";
  }
  ...
  const int nHNF = InitializeXstr(xstr, vxstr_species_ori, FileMESSAGE, aflags);
  ...
  for (size_t i = 0; i < vv_xstr.size(); i++) {
    for (size_t j = 0; j < vv_xstr[i].size(); j++) {
      xstructure xstr_tmp = vv_xstr[i][j];
  ...
  for (size_t i = 0; i < vxstr_sorted.size(); i++) {
    aus << AFLOWIN_SEPARATION_LINE << endl;
...
vector<xstructure> CalculateInitialSupercell(xstructure xstr, int n, ofstream& FileMESSAGE, _aflags& aflags) {
  ...
  for (int i = 0; i < (int) sHNF.size(); i++) {
    b_tmp = GetSuperCell(xstr, trasp(sHNF[i]));// when generates structure, using trasp
    b_tmp.title += "  [HNF(" + aurostd::utype2string(n) + ")=" + aurostd::utype2string(i + 1) + "/" + aurostd::utype2string(sHNF.size()) + "=";
    const string str_sHNF = " " + aurostd::utype2string(sHNF[i](1, 1)) + " " + aurostd::utype2string(sHNF[i](1, 2)) + " " + aurostd::utype2string(sHNF[i](1, 3)) + "; " + aurostd::utype2string(sHNF[i](2, 1)) +
                            " " + aurostd::utype2string(sHNF[i](2, 2)) + " " + aurostd::utype2string(sHNF[i](2, 3)) + "; " + aurostd::utype2string(sHNF[i](3, 1)) + " " + aurostd::utype2string(sHNF[i](3, 2)) +
                            " " + aurostd::utype2string(sHNF[i](3, 3)) + "]";
...
int InitializeXstr(xstructure& xstr, vector<string> vxstr_species_ori, ofstream& FileMESSAGE, _aflags& aflags) {
  ...
  const uint digits = 6;
  const uint digits1 = 4;
  const uint digits2 = 2 * digits + 11;

  if (xstr.partial_occupation_HNF) {
    return xstr.partial_occupation_HNF;
  ...
  if (xstr.partial_occupation_site_tol > 0) {
    tolerance = xstr.partial_occupation_site_tol;
  }
  ...
  vector<uint> M_iatom;
  for (size_t iatom = 0; iatom < xstr.atoms.size(); iatom++) {
  ...
  for (size_t iatom = 0; iatom < xstr.atoms.size(); iatom++) {
    if (xstr.atoms[iatom].partial_occupation_value < 1.0) {
  ...
  while (error > tolerance) {
    ...
    for (size_t iatom = 0; iatom < xstr.atoms.size(); iatom++) {
      ...
      for (int j = 0; j <= HNFi; j++) {
        eps = std::abs((double) j / HNFi - xstr.atoms[iatom].partial_occupation_value);
  ...
  for (size_t iatom = 0; iatom < xstr.atoms.size(); iatom++) {
    if (xstr.atoms[iatom].partial_occupation_value < 1.0) {
      xstr.atoms[iatom].partial_occupation_value = effective_pocc_iatom.at(iatom);
    }
  }
...
vector<vector<xstructure>> Partial2Xstr_Fivefold_Occupied(xstructure xstr, int nHNF, ofstream& FileMESSAGE, _aflags& aflags) {
  ...
  const xstructure xstr_norm = NormalizeXstructure(xstr);
  xstructure xstr_tri = xstr;
  for (size_t i = 0; i < xstr_norm.atoms.size(); i++) {
    if (NormalisedNumber.at(i).size() == 5) {
      const int k = NormalisedNumber.at(i).at(4);  // begin from 0, so the 5th atom should be at(4)
  ...
  for (size_t i = 0; i < xstr_norm.atoms.size(); i++) {
    if ((NormalisedNumber.at(i).size() == 5) || ((NormalisedNumber.at(i).size() == 4) && CheckVacancyOnOnesite(xstr, i))) { // tripled occupied
      const int k1 = NormalisedNumber.at(i).at(0);
      const int k2 = NormalisedNumber.at(i).at(1);
      const int k3 = NormalisedNumber.at(i).at(2);
      const int k4 = NormalisedNumber.at(i).at(3);
      const double sum_first_four_pocc_value = xstr.atoms.at(k1).partial_occupation_value + xstr.atoms.at(k2).partial_occupation_value + xstr.atoms.at(k3).partial_occupation_value + xstr.atoms.at(k4).partial_occupation_value;
      const int num_vacancy = hnf_double2int((1 - sum_first_four_pocc_value) * nHNF);
      for (int k = 0; k < num_vacancy; k++) {
        if (NormalisedNumber.at(i).size() == 5) { // If it contains four elements
          const int k5 = NormalisedNumber.at(i).at(4); // get the the name of the 5th element
      ...
      const float first_three_pocc_value = xstr.atoms.at(k1).partial_occupation_value + xstr.atoms.at(k2).partial_occupation_value + xstr.atoms.at(k3).partial_occupation_value;
      const int num_first_three = hnf_double2int(first_three_pocc_value * nHNF);
      vector<int> vec_num_part_new;
      for (size_t i = num_first_three; i < vec_num_part.size(); i++) {
  ...
  int total = 0;
  for (size_t k = 0; k < vec_vec_xstr.size(); k++) {  // 2-D vector
    vector<xstructure> vec_xstr = vec_vec_xstr[k];
    vector<xstructure> vec_new_xstr;
    for (size_t i = 0; i < vec_xstr.size(); i++) {
      xstructure xstr_tmp = vec_xstr[i];
      for (size_t j = 0; j < formated_final_num.size(); j++) {
        total++;
        const vector<int> vec_int = formated_final_num[j];
        const xstructure new_xstr = XstrSubstitute(xstr_tmp, vec_int, element_name);
  ...
  for (size_t i = 0; i < vec_vec_xstr_fivefold.size(); i++) {
    const vector<xstructure> vec_new_xstr = vec_vec_xstr_fivefold[i];
    const vector<xstructure> vxstr_final = RemoveEquivalentXstr(vec_new_xstr, FileMESSAGE, aflags);
...
vector<vector<xstructure>> Partial2Xstr_Fourfold_Occupied(xstructure xstr, int nHNF, ofstream& FileMESSAGE, _aflags& aflags) {
  ...
  for (size_t i = 0; i < xstr_norm.atoms.size(); i++) {
    if (NormalisedNumber.at(i).size() == 4) { // if the 4th element is vacancy, then it is fake triple one
      const int k = NormalisedNumber.at(i).at(3);
      xstr_tri.RemoveAtom(k);
    }
  }
  ...
  vector<string> element_name; // replacing original name
  for (size_t i = 0; i < xstr_norm.atoms.size(); i++) {
    if ((NormalisedNumber.at(i).size() == 4) || ((NormalisedNumber.at(i).size() == 3) && CheckVacancyOnOnesite(xstr, i))) { // tripled occupied
      const int k1 = NormalisedNumber.at(i).at(0);
      const int k2 = NormalisedNumber.at(i).at(1);
      const int k3 = NormalisedNumber.at(i).at(2);
      const double sum_first_three_pocc_value = xstr.atoms.at(k1).partial_occupation_value + xstr.atoms.at(k2).partial_occupation_value + xstr.atoms.at(k3).partial_occupation_value;
      const int num_vacancy = hnf_double2int((1 - sum_first_three_pocc_value) * nHNF);
      for (int k = 0; k < num_vacancy; k++) {
        if (NormalisedNumber.at(i).size() == 4) { // If it contains four elements
          const int k4 = NormalisedNumber.at(i).at(3); // get the the name of  4th element
      ...
      const float first_two_pocc_value = xstr.atoms.at(k1).partial_occupation_value + xstr.atoms.at(k2).partial_occupation_value;
      const int num_first_two = hnf_double2int(first_two_pocc_value * nHNF);
      vector<int> vec_num_part_new;
      for (size_t i = num_first_two; i < vec_num_part.size(); i++) {
        vec_num_part_new.push_back(vec_num_part.at(i));
      }
      vector<vector<int>> num_com_vacancy;
      num_com_vacancy.clear();
  ...
  vector<vector<xstructure>> vec_vec_xstr_final;
  for (size_t i = 0; i < vec_vec_xstr_fourfold.size(); i++) {
    const vector<xstructure> vec_new_xstr = vec_vec_xstr_fourfold[i];
    const vector<xstructure> vxstr_final = RemoveEquivalentXstr(vec_new_xstr, FileMESSAGE, aflags);
    vec_vec_xstr_final.push_back(vxstr_final);
  }
  return vec_vec_xstr_final;
...
vector<vector<xstructure>> Partial2Xstr_Triple_Occupied(xstructure xstr, int nHNF, ofstream& FileMESSAGE, _aflags& aflags) {
  ...
  for (size_t i = 0; i < xstr_norm.atoms.size(); i++) {
    if (NormalisedNumber.at(i).size() == 3) { // if the 3th element is vacancy, then it is fake triple one
      const int k = NormalisedNumber.at(i).at(2);
      xstr_tri.RemoveAtom(k);
    }
  }
  ...
  vector<string> element_name; // replacing original name
  for (size_t i = 0; i < xstr_norm.atoms.size(); i++) {
    if ((NormalisedNumber.at(i).size() == 3) || ((NormalisedNumber.at(i).size() == 2) && CheckVacancyOnOnesite(xstr, i))) { // tripled occupied
      const int k1 = NormalisedNumber.at(i).at(0);
      const int k2 = NormalisedNumber.at(i).at(1);
      const double sum_first_two_pocc_value = xstr.atoms.at(k1).partial_occupation_value + xstr.atoms.at(k2).partial_occupation_value;
      const int num_vacancy = hnf_double2int((1 - sum_first_two_pocc_value) * nHNF);
      for (int k = 0; k < num_vacancy; k++) {
      ...
      const int num_first_one = hnf_double2int(xstr.atoms.at(k1).partial_occupation_value * nHNF);
      vector<int> vec_num_part_new;
      for (size_t i = num_first_one; i < vec_num_part.size(); i++) {
        vec_num_part_new.push_back(vec_num_part.at(i));
      }
      vector<vector<int>> num_com_vacancy;
      num_com_vacancy.clear();
  ...
  vector<vector<xstructure>> vec_vec_xstr_final;
  for (size_t i = 0; i < vec_vec_xstr_triple.size(); i++) {
    const vector<xstructure> vec_new_xstr = vec_vec_xstr_triple[i];
    const vector<xstructure> vxstr_final = RemoveEquivalentXstr(vec_new_xstr, FileMESSAGE, aflags);
    vec_vec_xstr_final.push_back(vxstr_final);
  }
  return vec_vec_xstr_final;
...
vector<vector<xstructure>> Partial2Xstr(xstructure xstr, int nHNF, ofstream& FileMESSAGE, _aflags& aflags) {
  ...
  aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
  aus.str("");
  const xstructure xstr_c = NormalizeXstructure(xstr);
  ...
  vector<xstructure> vxstr_mid;
  vxstr_mid.clear();
  vector<vector<xstructure>> vv_xstr;
  vv_xstr.clear();
  for (size_t i = 0; i < vxstr_init.size(); i++) {
    const xstructure xstr_tmp = vxstr_init[i];
    vxstr_mid = AssignPartialOccupation(xstr_tmp, xstr, n);
    vv_xstr.push_back(vxstr_mid);
  }

  vector<xstructure> vxstr_mid2;
  vxstr_mid2.clear();
  vector<vector<xstructure>> vv_xstr2;
  vv_xstr2.clear();
  int total = 0;
  for (size_t i = 0; i < vv_xstr.size(); i++) {
    for (size_t j = 0; j < vv_xstr[i].size(); j++) {
      ...
      // vv_xstr[i][j].title+=str_title;
      aus << vv_xstr[i][j];
      vxstr_mid2.push_back(vv_xstr[i][j]);
  ...
  vector<vector<xstructure>> vv_xstr_final;
  vv_xstr_final.clear();
  for (size_t i = 0; i < vv_xstr2.size(); i++) {
    const vector<xstructure> vxstr_tmp = vv_xstr2[i];
    const vector<xstructure> vxstr_nq2 = RemoveEquivalentXstr(vxstr_tmp, FileMESSAGE, aflags); //
...
// ***************************************************************************
bool comparison_atom_fpos(_atom atom1, _atom atom2) {
  const double _EQUAL_DOUBLE = 1E-6;
  if (std::abs(atom1.fpos[1] - atom2.fpos[1]) > _EQUAL_DOUBLE) {
    return (atom1.fpos[1] < atom2.fpos[1]);
  } else if (std::abs(atom1.fpos[2] - atom2.fpos[2]) > _EQUAL_DOUBLE) {
    return (atom1.fpos[2] < atom2.fpos[2]);
  } else if (std::abs(atom1.fpos[3] - atom2.fpos[3]) > _EQUAL_DOUBLE) {
...
namespace pflow {
  void Sort_atom_fpos(xstructure& xstr) {
    ...
    int begin = 0;
    for (size_t i = 0; i < xstr.num_each_type.size(); i++) {
      const int j = xstr.num_each_type[i];
...
bool comparison_atom_cpos(_atom atom1, _atom atom2) {
  const double _EQUAL_DOUBLE = 1E-6;
  if (std::abs(atom1.cpos[1] - atom2.cpos[1]) > _EQUAL_DOUBLE) {
    return (atom1.cpos[1] < atom2.cpos[1]);
  } else if (std::abs(atom1.cpos[2] - atom2.cpos[2]) > _EQUAL_DOUBLE) {
    return (atom1.cpos[2] < atom2.cpos[2]);
  } else if (std::abs(atom1.cpos[3] - atom2.cpos[3]) > _EQUAL_DOUBLE) {
...
bool LatticeCompare(xstructure xstr1, xstructure xstr2) {
  ...
  tmp1 = std::abs(xstr1.a - xstr2.a) + std::abs(xstr1.b - xstr2.b) + std::abs(xstr1.c - xstr2.c);
  tmp2 = std::abs(xstr1.alpha - xstr2.alpha) + std::abs(xstr1.beta - xstr2.beta) + std::abs(xstr1.gamma - xstr2.gamma);
  ...
  if (!LatticeCompare(xstr1, xstr2)) {
    ...
    if (xstr1.atoms.size() == xstr2.atoms.size()) {
      for (size_t i = 0; i < xstr1.atoms.size(); i++) {
        ...
        double b2 = 0;
        double b3 = 0;
        b1 = xstr1.atoms[i].fpos[1] - xstr2.atoms[i].fpos[1];
        b2 = xstr1.atoms[i].fpos[2] - xstr2.atoms[i].fpos[2];
        b3 = xstr1.atoms[i].fpos[3] - xstr2.atoms[i].fpos[3];
...
vector<xstr_energy> xstructure2xstr_energy(vector<xstructure> vec_xstr) {
  ...
  for (size_t i = 0; i < vec_xstr.size(); i++) {
    ...
    xstr_energy_tmp.xstr = vec_xstr[i];
    double TotalEnergyTmp;
    if (CALCULATE_ENERGY_UFF) {
      // TotalEnergyTmp = pocc::CalculateUFFEnergy(xstr_tmp);
      TotalEnergyTmp = pocc::CalculateUFFEnergy(vec_xstr[i]);
    } else {
      // TotalEnergyTmp = pocc::CalculateEnergyUsingGulp(xstr_tmp);
      TotalEnergyTmp = pocc::CalculateEnergyUsingGulp(vec_xstr[i]);
    }
...
vector<xstructure> RemoveEquivalentXstr(vector<xstructure> vec_xstr, ofstream& FileMESSAGE, _aflags& aflags) {
  ...
  for (size_t i = 1; i < vec_xstr_energy.size(); i++) {
    uint j;
    for (j = 0; j < vec_xstr_energy_final.size(); j++) {
      if (std::abs(vec_xstr_energy[i].energy - vec_xstr_energy_final.at(j).energy) < epsilon_etot) { // if they have same energy
        break;
      }
    }
    if (j == vec_xstr_energy_final.size()) {
      vec_xstr_energy_final.push_back(vec_xstr_energy[i]);
    }
  }

  // cerr << "KY TEST" << endl;
  vector<int> vecDG;
  vecDG.clear(); // Degenracy
  for (size_t i = 0; i < vec_xstr_energy_final.size(); i++) {
    int DGi = 0;
    for (size_t j = 0; j < vec_xstr_energy.size(); j++) {
      if (std::abs(vec_xstr_energy_final[i].energy - vec_xstr_energy[j].energy) < epsilon_etot) {
        DGi++;
      }
    }
    vecDG.push_back(DGi);
  ...
  const int number = vec_xstr_energy.size() - vec_xstr_energy_final.size();
  ostringstream aus;
  ...
  for (size_t i = 0; i < vec_xstr_energy_final.size(); i++) {
    xstructure xstr_tmp = vec_xstr_energy_final[i].xstr;
    const string str_title = " DG=" + aurostd::utype2string(vecDG.at(i));
...
namespace pocc {
  bool DIFF_UFFENERGY(xstructure xstr1, xstructure xstr2) {
    ...
    const double e1 = pocc::CalculateUFFEnergy(xstr1);
    const double e2 = pocc::CalculateUFFEnergy(xstr2);
    const double e_delta = std::abs(e1 - e2);
    if (e_delta < energy_eps_tol_) {
      return true;
    } else {
      return false;
    }
...
namespace pocc {
  bool DIFF(xstructure xstr1, xstructure xstr2) {
    ...
    if (pocc::DIFF_LEVEL1(xstr1, xstr2)) {
      RUN_FLAG = true;
      return RUN_FLAG;
    }
    // Highest lelvel
    if (pocc::DIFF_LEVEL4(xstr1, xstr2)) {
      RUN_FLAG = true;
      return RUN_FLAG;
    }
    if (pocc::DIFF_LEVEL4(xstr2, xstr1)) {
      RUN_FLAG = true;
      return RUN_FLAG;
    }
    return RUN_FLAG;
...
namespace pocc {
  bool DIFF_LEVEL4(xstructure xstr1, xstructure xstr2) {
    ...
    for (size_t i = 0; i < vec_rot.size(); i++) {
      xstructure xstr2_sp_rot_tmp = vec_rot[i];
      for (size_t j = 0; j < xstr2_sp_rot_tmp.atoms.size(); j++) {
  ...
  vector<xstructure> GenerateRotatedXstr(xstructure xstr) {
    ...
    for (size_t i = 0; i < xstr.pgroup.size(); i++) {
      for (size_t j = 0; j < xstr.atoms.size(); j++) {
        xstr_tmp.atoms.at(j).fpos = xstr.pgroup[i].Uf * xstr.atoms[j].fpos;
      }
  ...
  void DIFF(string options) {
    ...
    if (LDEBUG) {
      for (size_t i = 0; i < tokens.size(); i++) {
        cerr << __AFLOW_FUNC__ << " tokens[i]=" << tokens[i] << endl;
      }
    }
    ...
    if (!aurostd::FileExist(tokens.at(0))) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "file not found: " + tokens.at(0), _FILE_CORRUPT_);
    } // CO20200624
    if (!aurostd::FileExist(tokens.at(1))) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "file not found: " + tokens.at(1), _FILE_CORRUPT_);
    } // CO20200624
    xstructure xstr1;
    xstructure xstr2;
...
void xstructure::AddAtom_POCC(const _atom& atom) {
  ...
  for (size_t iat = 0; iat < atoms.size() && FOUND_POSITION == false; iat++) {
      ...
      for (int i = -1; i <= 1 && FOUND_POSITION == false; i++) {
        for (int j = -1; j <= 1 && FOUND_POSITION == false; j++) {
          for (int k = -1; k <= 1 && FOUND_POSITION == false; k++) {
            ...
            if (aurostd::modulus(atoms[iat].fpos - (aijk + atom.fpos)) < 0.01) {
              FOUND_POSITION = true;
            }
          }
        }
  ...
  if (FOUND_POSITION == true) {
    return;
  } // found no need to add it further
  ...
  if (FOUND_SPECIES == false) {
    if (LDEBUG) {
      cerr << "AddAtom new_specie=" << atom.name << endl;
    }
    ...
    species_pp_type.emplace_back(""); // cerr << "AddAtom=" << atom.name << endl;
    species_pp_version.emplace_back(""); // cerr << "AddAtom=" << atom.name << endl;
    species_pp_ZVAL.push_back(0.0); // cerr << "AddAtom=" << atom.name << endl;
    species_pp_vLDAU.emplace_back(); // cerr << "AddAtom=" << atom.name << endl;
    ...
    if (LDEBUG) {
      cerr << "AddAtom increasing species_position " << species_position << endl;
    }
...
namespace pocc {
  void POSCAR2ENUM(xstructure& a, stringstream& oss, ofstream& FileMESSAGE, _aflags& aflags) {
    ...
    const string str_species_ori = xstr_in.SpeciesString();
    vector<string> vxstr_species_ori;
    aurostd::string2tokens(str_species_ori, vxstr_species_ori, " ");

    const int nHNF = InitializeXstr(xstr_in, vxstr_species_ori, FileMESSAGE, aflags);
    const int nMin = nHNF;
    int nMax = nHNF;
    ...
    const int nDFull = NormalisedNumber.size();
    int Ntype;
    if (CheckVacancy(xstr_in)) {
      Ntype = xstr_in.num_each_type.size() + 1;
    } else {
      Ntype = xstr_in.num_each_type.size();
    }
    ...
    for (unsigned int i = 0; i < NormalisedNumber.size(); i++) {
      const int j = NormalisedNumber[i].at(0);
    ...
    const int NumCell = cRangeFrac.at(0).at(2) / nDFull;
    if (NumCell > nMax) {
      nMax = NumCell;
    }
    if (!CheckPartialOccupation(xstr_in)) {
      nMax = 1;
    }
...
extern "C" {
#ifdef _AFLOW_GUS_POCC_
void __aflow_call_enum_MOD_aflow_pass_parameter(double* parLV,
                                                int* nDFull,
                                                double* dFull,
                                                int* rdFull,
                                                int* cdFull,
...
namespace pocc {
  vector<xstructure> MultienumGenerateXstr(xstructure& xstr, ofstream& FileMESSAGE, _aflags& aflags) {
    ...
    double parLV[3 * 3];
    parLV[0] = xstr.lattice(1, 1);
    parLV[1] = xstr.lattice(1, 2);
    parLV[2] = xstr.lattice(1, 3);
    parLV[3] = xstr.lattice(2, 1);
    parLV[4] = xstr.lattice(2, 2);
    parLV[5] = xstr.lattice(2, 3);
    parLV[6] = xstr.lattice(3, 1);
    parLV[7] = xstr.lattice(3, 2);
    parLV[8] = xstr.lattice(3, 3);
    ...
    if (VacancyFLAG) {
      Ntype = xstr.num_each_type.size() + 1;
    } // Ntype means the number of types of atoms
    else {
      Ntype = xstr.num_each_type.size();
    }
    int k = Ntype;
    ...
    for (unsigned int i = 0; i < LabelXstr.size(); i++) {
      LabelTmp.clear();
      for (unsigned int j = 0; j < LabelXstr[i].size(); j++) {
        LabelTmp.push_back(LabelXstr[i][j]);
      }
      if (LabelXstr[i].size() < MaxNum) {
        for (unsigned int k = 0; k < (MaxNum - LabelXstr[i].size()); k++) {
    ...
    for (unsigned int i = 0; i < NewLabelXstr.size(); i++) {
      for (unsigned int j = 0; j < NewLabelXstr[i].size(); j++) {
        OneDimensionLabelXstr.push_back(NewLabelXstr[i][j]);
      }
    }
    //-----------------------------------------------------------------------
    ...
    fs::current_path(tmpdir.c_str());  // CO20191112

    __aflow_call_enum_MOD_aflow_pass_parameter(parLV, &nDFull, dFull, &rdFull, &cdFull, &k, &nMin, &nMax, pLatTyp, &eps, &full, labelFull, &rlabelFull, &clabelFull, digitFull, &ndigitFull, equivalencies,
                                               &nequivalencies, &conc_check, cRange, &rcRange, &ccRange);
    ...
    fs::current_path(cur_dir_name.c_str());  // CO20191112
    return groupxstr;
  }
} // namespace pocc
#endif
...
namespace pocc {
  bool MultienumPrintSortedXstr(istream& input) {
    ...
    const vector<xstructure> groupxstr_names = AssignNameXstr(groupxstr, AtomSpecies);
    ...
    for (size_t i = 0; i < vxstr_final.size(); i++) {
      xstructure xstr_tmp = vxstr_final[i];
    ...
    stringstream ss;
    stringstream oss;
    for (size_t i = 0; i < vxstr_final_alphabetic.size(); i++) {
      ss.str("");
      ss << setfill('0') << setw(6) << (i + 1);
      oss << AFLOWIN_SEPARATION_LINE << endl;
      oss << _VASP_POSCAR_MODE_EXPLICIT_START_ << ss.str() << endl;  // CO20200624
      oss << vxstr_final_alphabetic[i];
      oss << _VASP_POSCAR_MODE_EXPLICIT_STOP_ << ss.str() << endl; // CO20200624
      oss << AFLOWIN_SEPARATION_LINE << endl;
    }
...
namespace pocc {
  void POCC_SetOptions(string options, string& directory, double& T, double& DOS_Emin, double& DOS_Emax, double& DOSSCALE) {
    ...
    directory = "./";
    DOS_Emin = DEFAULT_DOS_EMIN / 2.0;
    DOS_Emax = DEFAULT_DOS_EMAX / 2.0;
    DOSSCALE = DEFAULT_DOS_SCALE; // some defaults
    ...
    if (LDEBUG) {
      for (size_t i = 0; i < tokens.size(); i++) {
        cerr << "pocc::POCC_SetOptions: tokens[i]=" << tokens[i] << endl;
      }
    }
...
namespace pocc {
  bool POCC_Already_Calculated_Input(const string& str_AflowIn, const string& dir) {
    return aurostd::substring2bool(str_AflowIn, "[VASP_POSCAR_MODE_EXPLICIT]START.POCC") || aurostd::CompressFileExist(dir + "/" + POCC_FILE_PREFIX + POCC_ALL_SUPERCELLS_FILE);
  }
} // namespace pocc
...
namespace pocc {
  void POCC_CalculateDistribution(vector<double>& vdelta_toten, vector<double>& vprob, const double& T, const string& NameDist, const vector<int>& vDE) {
    ...
    double Fi;
    double DEi;
    for (size_t i = 0; i < vdelta_toten.size(); i++) {
      const double delta_toten = vdelta_toten[i];
      DEi = vDE.at(i);
      if (NameDist == "B") {
    ...
    const double sum_vFi = aurostd::sum<double>(vFi);
    for (size_t i = 0; i < vFi.size(); i++) {
      const double probi = vFi[i] / sum_vFi;
...
namespace pocc {
  vector<vector<double>> SpinFlipDOS(const vector<vector<double>>& vva) {
    ...
    vvb.clear();
    if (vva.at(0).size() % 2 == 0) {
      throw aurostd::xerror(__AFLOW_FILE__, "pocc::SpinFlipDOS():", "input not spin-polarized", _INPUT_ILLEGAL_);
    } // CO20200624
    for (size_t i = 0; i < vva.size(); i++) {
      vector<double> vtmp;
      vtmp.clear();
      vtmp.push_back(vva[i].at(0));
      for (size_t j = 1; j < vva.at(0).size(); j = j + 2) {
        vtmp.push_back(vva[i].at(j + 1) * (-1));   // exchange columns 1 and 2 spin_dn becomes spin_up
        vtmp.push_back(vva[i].at(j) * (-1));   // spin_up becomes spin_dn
      }
      vvb.push_back(vtmp);
    }
    return vvb;
  ...
  vector<vector<double>> SpinSplitDOS(const vector<vector<double>>& vva) {
    ...
    for (size_t i = 0; i < vva.size(); i++) {
      ...
      for (size_t j = 1; j < vva[i].size(); j++) {
        const double tmp = 0.5 * vva[i][j];
        vtmp.push_back(tmp);
        vtmp.push_back((-1) * tmp);
      }
      vvb.push_back(vtmp);
    }
    return vvb;
...
namespace pocc {
  vector<vector<double>> POCC_Formalise(const bool& SPIN_FLAG, const double& weight, const double& mag, const vector<vector<double>>& TDOS) {
    ...
    if (SPIN_FLAG) {
      ...
      } else if (TDOS.at(0).size() == 5 && std::abs(mag) < 1E-6) { // must be TOTALPDOS: E, s, p, d, f, no-spin-polarized
        vvb = pocc::SpinSplitDOS(vva);
...
void ExtracAllPOSCARSFromAflowin(vector<xstructure>& vxstr, const string& str_aflowin) {
  const bool LDEBUG = (false || XHOST.DEBUG);
  vxstr.clear();
  const string POSCAR_START_DELIMITER = _VASP_POSCAR_MODE_EXPLICIT_START_; // CO20200624
  const string POSCAR_STOP_DELIMITER = _VASP_POSCAR_MODE_EXPLICIT_STOP_; // CO20200624
  ...
  for (size_t i = 0; i < vKBIN_VASP_POSCAR_MODE_EXPLICIT_VSTRING.size(); i++) {
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vKBIN_VASP_POSCAR_MODE_EXPLICIT_VSTRING[i]=" << endl;
      cerr << vKBIN_VASP_POSCAR_MODE_EXPLICIT_VSTRING[i] << endl;
    }
    ...
    if (!POSCAR.str().empty()) {
      vxstr.emplace_back(POSCAR, IOVASP_AUTO);
    }
  }
}
...
void GetDegeneracyFromVPOSCAR(const vector<xstructure>& vxstr, vector<int>& vDE) {
  ...
  for (size_t i = 0; i < vxstr.size(); i++) {
    ...
    DEI = 1;
    for (size_t j = 0; j < vtitle.size(); j++) {
      if (!aurostd::substring2bool(vtitle[j], "DG=")) {
        continue;
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " found DG tag: \"" << vtitle[j] << "\"" << endl;
      }
      vtitle2.clear();
...
namespace pocc {
  void POCC_GENERATE_DOSDATA(
      const string& directory, const double& T, vector<vector<double>>& TDOS_ONLY, vector<vector<double>>& PDOS_ONLY, vector<double>& POCC_Efermi, double& POCC_mag, double& Egap_net, vector<double>& Egap, vector<double>& vprob) {
    ...
    string aflowin;
    const string MESSAGE = "pocc::POCC_DOS ERROR";
    aflowin = string(directory + "/" + _AFLOWIN_);
    if (!aurostd::FileExist(aflowin)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "file not found: " + aflowin, _FILE_CORRUPT_);
    } // CO20200624
    string str_AflowIn;
    aurostd::file2string(aflowin, str_AflowIn);
    const bool pocc_already_calculated_input = pocc::POCC_Already_Calculated_Input(str_AflowIn, directory);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " pocc_already_calculated_input=" << pocc_already_calculated_input << endl;
    }
    if (pocc_already_calculated_input) {
      // Fix Degeneracy, Reading all POSCARs from aflowin
      vector<xstructure> vxstr;
      if (aurostd::CompressFileExist(directory + "/" + POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE)) { // read from unique structures file
        string unique_structures_file_contents;
        aurostd::compressfile2string(directory + "/" + POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE, unique_structures_file_contents);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " unique_structures_file_contents=" << endl;
          cerr << unique_structures_file_contents << endl;
        }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " vxstr.size()=" << vxstr.size() << endl;
      }
      vector<int> vDE;
      vDE.clear();
      ...
      vector<string> vrun;
      vector<string> vdoscar_files;
      vector<string> voutcar_files;
      string command;
      string aflow_pocc_out;
      command = R"(grep "\[VASP_POSCAR_MODE_EXPLICIT\]START\." )" + aflowin;
      aflow_pocc_out = aurostd::execute2string(command);
      aurostd::StringSubstInPlace(aflow_pocc_out, _VASP_POSCAR_MODE_EXPLICIT_START_, ""); // replace string  //CO20200624
      ...
      for (size_t i = 0; i < vrun.size(); i++) {
        found_ext = false;
        for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
          outcar_file = aurostd::CleanFileName(directory + "/ARUN." + vrun[i] + "/OUTCAR.static" + XHOST.vext[iext]);
        ...
        if (!found_ext) {
          aus << "ERROR" << ": file not found " << aurostd::CleanFileName(directory + "/ARUN." + vrun[i] + "/OUTCAR.static") << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      ...
      for (size_t i = 0; i < vrun.size(); i++) {
        found_ext = false;
        for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
          doscar_file = aurostd::CleanFileName(directory + "/ARUN." + vrun[i] + "/DOSCAR.static" + XHOST.vext[iext]);
        ...
        if (!found_ext) {
          aus << "ERROR" << ": file not found " << aurostd::CleanFileName(directory + "/ARUN." + vrun[i] + "/DOSCAR.static") << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          FLAG_ALLFILES_EXIST = false;
        }
      }
      ...
      vector<int> vions;
      vector<double> vtoten_per_atom;
      vector<double> vmag;
      vector<double> vEgap_net;
      vector<vector<double>> vEgap;
      const bool POCC_SPIN_FLAG = false;  // default non-spin-polarized
      for (size_t i = 0; i < vrun.size(); i++) {
        if (LDEBUG) {
          cerr << "pocc::POCC_GENERATE_DOSDATA: vrun.at(" << i << ")=" << vrun[i] << endl;
        }
        ...
        const double EFERMI = outcar_aus.Efermi;
        ...
        string outcar_bands = voutcar_files.at(i);
        aurostd::StringSubstInPlace(outcar_bands, "static", "bands");
        ...
        if (LDEBUG || true) {
          cerr << "pocc::POCC_GENERATE_DOSDATA: vrun.at(" << i << ")=" << vrun[i] << "; DE=" << vDE[i] << "; NIONS=" << vions.at(i) << "; enthalpy_atom=" << vtoten_per_atom.at(i) << "; mag_cell=" << vmag.at(i)
               << "; Egap_net=" << vEgap_net.at(i) << endl;    // CO - should read out enthalpy/atom from STATIC, not BANDS
        }

        //	if(LDEBUG||1) {
        //	cerr << "pocc::POCC_GENERATE_DOSDATA: vrun.at(" << i << ")=" << vrun[i] << "; NIONS=" << outcar_aus.NIONS << "; enthalpy_atom=" << outcar_aus.enthalpy_atom << "; mag_cell=" << outcar_aus.mag_cell << "; Egap_net=" << outcar_aus.Egap_net << endl;   //CO
        //	}
      }
      ...
      //  for (size_t i=0; i<vmag.size();i++) {
      //  vmag_abs.push_back(abs(vmag.at(i))); //ferromagnetic alignment
      //  }

      POCC_mag = 0.0;
      for (size_t i = 0; i < vmag.size(); i++) {
        POCC_mag += std::abs(vmag[i]) * vprob.at(i); // Calculate average absolute magnetic moment
      }

      Egap_net = 0.0;
      for (size_t i = 0; i < vEgap_net.size(); i++) {
        Egap_net += vEgap_net[i] * vprob.at(i); // Calculate average Egap_net
      }

      Egap.clear();
      for (size_t j = 0; j < vEgap.at(0).size(); j++) {
        Egap.push_back(0);
        for (size_t i = 0; i < vEgap.size(); i++) {
          // Calculate average Egap
          Egap.at(j) += vEgap.at(0)[j] * vprob.at(i); // CO - otherwise, it crashes if you have Egap up AND Egap down (magnetized system)
          // Egap.at(j)+=vEgap.at(i)[j]*vprob.at(i);  //CO
        }
      }

      // Get TDOS & TOTALPDOS DATA
      vector<vector<vector<double>>> POCC_TDOS;
      vector<vector<vector<double>>> POCC_TOTALPDOS;
      for (size_t i = 0; i < vdoscar_files.size(); i++) {
        const string doscar_file = vdoscar_files[i];
        stringstream ss_doscar;
        aurostd::compressfile2stringstream(doscar_file, ss_doscar);
        const string outcar_file = voutcar_files.at(i);
        stringstream ss_outcar;
        aurostd::compressfile2stringstream(outcar_file, ss_outcar);
        double Efermi;
        ...
        // if no PDOS, exit
        // I will fix later
        if (!(estructure::GET_DOS_DATA(ss_doscar, ss_outcar, Efermi, TDOS, TOTALPDOS) && !TOTALPDOS.empty())) {
      ...
      const vector<vector<double>> POCC_TOTALPDOS_normalized = aurostd::NormalizeAndSum3DVector(POCC_TOTALPDOS, vprob); // normalize pdos
      ...
      if (LDEBUG) {
        ...
        for (size_t i = 0; i < POCC_TDOS_normalized.size(); i++) {
...
namespace pocc {
  void POCC_COMBINE_TDOS_PDOS_ONEDOS(const vector<vector<double>>& TDOS, const vector<vector<double>>& PDOS, vector<vector<double>>& DOS, vector<vector<double>>& DOS_IDOS) { // CO20190808 - one without IDOS and one with
    ...
    if (TDOS.at(0).size() == 3) { // non-spin
      for (size_t i = 0; i < TDOS.size(); i++) {
        vtmp.clear();
        vtmp.push_back(TDOS[i].at(0)); // Energy
        for (size_t j = 1; j < PDOS.at(i).size(); j++) {
          vtmp.push_back(PDOS.at(i)[j]); // s, p, d
        }
        vtmp.push_back(TDOS[i].at(1)); // TDOS
        DOS.push_back(vtmp);
        vtmp.push_back(TDOS[i].at(2)); // CO20190808 - IDOS
      ...
      for (size_t i = 0; i < TDOS.size(); i++) {
        ...
        vtmp.push_back(TDOS[i].at(0)); // Energy
        for (size_t j = 1; j < PDOS.at(i).size(); j++) {
          vtmp.push_back(PDOS.at(i)[j]); // s, p ,d, f
        }
        vtmp.push_back(TDOS[i].at(1)); // TDOS up
        vtmp.push_back(TDOS[i].at(2)); // TDOS dn
        DOS.push_back(vtmp);
        vtmp.push_back(TDOS[i].at(3));
        vtmp.push_back(TDOS[i].at(4)); // CO20190808 - IDOS up/down
...
namespace pocc {
  void POCC_GENERATE_OUTPUT(const string& directory, const double& T, const double& DOS_Emin, double& DOS_Emax, const double& DOSSCALE) {
    ...
    if (!XHOST.is_command("gnuplot")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "gnuplot is missing", _RUNTIME_INIT_);
    } // CO20200624
    if (!XHOST.is_command("convert")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "convert is missing", _RUNTIME_INIT_);
    } // CO20200624
    vector<vector<double>> TDOS_ONLY;
    vector<vector<double>> PDOS_ONLY;
    vector<vector<double>> DOS;
    vector<vector<double>> DOS_IDOS;
    vector<double> vEfermi;
    vector<double> Egap;
    vector<double> vprob;
    double mag;
    double Egap_net; // CO20190808 - DOS_IDOS
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " starting" << endl;
    }
    POCC_GENERATE_DOSDATA(directory, T, TDOS_ONLY, PDOS_ONLY, vEfermi, mag, Egap_net, Egap, vprob);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " POCC_GENERATE_DOSDATA done" << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " TDOS_ONLY.size()=" << TDOS_ONLY.size() << endl;
      for (size_t i = 0; i < TDOS_ONLY.size(); i++) {
        cerr << __AFLOW_FUNC__ << " TDOS_ONLY[i=" << i << "].size()=" << TDOS_ONLY[i].size() << endl;
      }
      cerr << __AFLOW_FUNC__ << " PDOS_ONLY.size()=" << PDOS_ONLY.size() << endl;
      for (size_t i = 0; i < PDOS_ONLY.size(); i++) {
    ...
    const double DOSMAX = aurostd::FindMaxIn2DvectorExcept1stColumn(DOS, DOS_Emin, DOS_Emax);

    // write 2D vector into files
    vector<vector<double>>* p_DOS2FILE = &DOS; // CO20190808 - write out DOS_IDOS if desired
    if (PRINT_AVG_IDOS) {
      p_DOS2FILE = &DOS_IDOS;
    } // CO20190808 - write out DOS_IDOS if desired
    const vector<vector<double>>& DOS2FILE = *p_DOS2FILE; // CO20190808 - write out DOS_IDOS if desired
    const string str_dos = aurostd::vector2string(DOS2FILE); // CO20190808 - write out DOS_IDOS if desired
    const string dosdatafile = "DOSDATA_" + aurostd::utype2string(T) + "K";
    aurostd::string2file(str_dos, dosdatafile);

    // generate gnuplot script
    const string POCC_NAME = "untitled_" + aurostd::utype2string(T) + "K";
    const string gnuplotscript = "GNUPLOT_POCC_" + POCC_NAME + "_DOS.gp";
    const string str_gnu = POCC_GENERATE_GNUPLOTSCRIPT(DOS, POCC_NAME, dosdatafile, T, DOS_Emin, DOS_Emax, DOSSCALE, DOSMAX);
...
namespace pocc {
  void POCC_BANDGAP(string options) {
    ...
    if (Egap.empty()) { // CAMILO, can Egap have size = 0 ?? this is the <xOUTCAR.Egap> with vprob
      cout << "Temperature: " << Temperature << "  Egap_net:  " << Egap_net << endl;
    }
    if (Egap.size() == 1) {
      cout << "Temperature: " << Temperature << "  Egap up:  " << Egap.at(0) << "  Egap_net:  " << Egap_net << endl;
    }
    if (Egap.size() == 2) {
      cout << "Temperature: " << Temperature << "  Egap up:  " << Egap.at(0) << "  Egap dn:  " << Egap.at(1) << "  Egap net: " << Egap_net << endl;
    }
  }
} // namespace pocc
...
namespace pocc {
  ...
  bool Bond::operator==(const Bond& other) const {
    bool FLAG = false;
    if ((SameAtom(bgn, other.end) && SameAtom(end, other.bgn)) || (SameAtom(bgn, other.bgn) && SameAtom(end, other.end))) {
      FLAG = true;
    }
    return FLAG;
  }
...
namespace pocc {
  void RemoveSameBond(vector<Bond>& Bonds_orig, vector<Bond>& Bonds_new) {
    ...
    for (size_t i = 1; i < Bonds_orig.size(); i++) {
      uint j;
      for (j = 0; j < Bonds_new.size(); j++) {
        if (Bonds_orig[i] == Bonds_new[j]) {
          break;
        }
      }
      if (j == Bonds_new.size()) {
        Bonds_new.push_back(Bonds_orig[i]);
      }
    }
  }
} // namespace pocc
...
namespace pocc {
  void SetUFFPara(_atom atomi, _atom atomj, double& R0, double& Kij, double& Xij, double& Dij) {
    const bool LDEBUG = (false || XHOST.DEBUG); // CO20180409
    // double bondorder = 1.0; //single bond
    const string atomi_name = atomi.name;
    const string atomj_name = atomj.name;
    pocc::UFFPara uffparai;
    uffparai.GetUFFParameters(atomi_name);
    pocc::UFFPara uffparaj;
    uffparaj.GetUFFParameters(atomj_name);
    const double ri = uffparai.r1;
    const double rj = uffparaj.r1; // bond length
    const double Zi = uffparai.Z1;
    const double Zj = uffparaj.Z1; // effective charge
    const double chiI = uffparai.Xi;
    const double chiJ = uffparaj.Xi; // GMP electronegativity
    const double Xi = uffparai.x1;
    const double Xj = uffparaj.x1; // nonbond distance
    const double Di = uffparai.D1;
    const double Dj = uffparaj.D1; // nonbond energy
    // From Equation 3
    // double rbo = -0.1332*(ri+rj)*log(bondorder); //zero for single bond
    // From Equation 4
    const double ren = ri * rj * (pow((sqrt(chiI) - sqrt(chiJ)), 2.0)) / (chiI * ri + chiJ * rj);
    // From Equation 2
...
namespace pocc {
  double CalculateBondEnergy(xstructure xstr, _atom atomi, _atom atomj) {
    ...
    xstr.ReScale(1.0); // Safety
    const double rij = AtomDist(atomi, atomj); // Notice here rij is r, while r0 is rij in paper
    if (LDEBUG) { // CO20180409
      cerr << "distij " << rij << endl;
    }
    const double delta = rij - r0;
...
namespace pocc {
  double CalculateNonBondEnergy(xstructure xstr, _atom atomi, _atom atomj) {
    ...
    xstr.ReScale(1.0); // Safety
    const double x = AtomDist(atomi, atomj); // x distance
    const double X6 = pow((Xij / x), 6);
    const double X12 = pow((Xij / x), 12);

    if (LDEBUG) { // CO20180409
      pocc::UFFPara uffparai;
      uffparai.GetUFFParameters(atomi.name);
      pocc::UFFPara uffparaj;
      uffparaj.GetUFFParameters(atomj.name);
...
namespace pocc {
  double CalculateUFFEnergy(xstructure xstr) {
    ...
    for (size_t i = 0; i < Bonds.size(); i++) {
      const Bond bondi = Bonds[i];
      const _atom atomi = bondi.bgn;
      const _atom atomj = bondi.end;
      if (LDEBUG) { // CO20180409
    ...
    for (size_t i = 0; i < NonBonds.size(); i++) {
      const Bond bondi = NonBonds[i];
      const _atom atomi = bondi.bgn;
      const _atom atomj = bondi.end;
...
namespace pocc {
  void ExtractBonds(const xstructure& xstr, deque<deque<_atom>>& neigh_mat_bonded, deque<deque<_atom>>& neigh_mat_nonbonded) {
    ...
    const xstructure xstr_tmp = xstr;
    ...
    for (size_t ia = 0; ia < neigh_mat.size(); ia++) {
      atom_tmp_bonded.clear();
      atom_tmp_nonbonded.clear();
      const _atom a = neigh_mat[ia].at(0);
      const _atom a1 = neigh_mat[ia].at(1);
      atom_tmp_bonded.push_back(a);
      atom_tmp_bonded.push_back(a1); // a, a1 must be bonded
      atom_tmp_nonbonded.push_back(a);

      const double dr0 = AtomDist(a, a1);
      for (size_t in = 2; in < neigh_mat[ia].size(); in++) {
        const _atom an = neigh_mat[ia][in];
        if (std::abs(AtomDist(a, an) - dr0) < 0.5) {
    ...
    if (LDEBUG) { // CO20180409
      cerr << "BOND " << endl;
      for (size_t i = 0; i < neigh_mat_bonded.size(); i++) {
        cerr << "i=" << i << " " << neigh_mat_bonded[i].size() << endl;
      }
      cerr << "NONBOND " << endl;
      for (size_t i = 0; i < neigh_mat_nonbonded.size(); i++) {
...
namespace pocc {
  void AnalyzeBonds(const xstructure& xstr, vector<Bond>& Bonds, vector<Bond>& NonBonds) {
    ...
    for (size_t i = 0; i < neigh_mat_bonded.size(); i++) {
      const _atom atomI = neigh_mat_bonded[i].at(0);
      v_xstrBonds.clear();
      for (size_t j = 1; j < neigh_mat_bonded[i].size(); j++) {
        const _atom atomJ = neigh_mat_bonded[i][j];
        Bond bondIJ;
        bondIJ.Set(xstr, atomI, atomJ);
        v_xstrBonds.push_back(bondIJ);
      }
      vv_xstrBonds.push_back(v_xstrBonds);
    }
    for (size_t i = 0; i < vv_xstrBonds.size(); i++) {
      for (size_t j = 0; j < vv_xstrBonds[i].size(); j++) {
        Bonds.push_back(vv_xstrBonds[i][j]);
      }
    }
    ...
    for (size_t i = 0; i < neigh_mat_nonbonded.size(); i++) {
      const _atom atomI = neigh_mat_nonbonded[i].at(0);
      v_xstrNonBonds.clear();
      for (size_t j = 1; j < neigh_mat_nonbonded[i].size(); j++) {
        const _atom atomJ = neigh_mat_nonbonded[i][j];
        Bond bondIJ;
        bondIJ.Set(xstr, atomI, atomJ);
    ...
    for (size_t i = 0; i < vv_xstrNonBonds.size(); i++) {
      for (size_t j = 0; j < vv_xstrNonBonds[i].size(); j++) {
        NonBonds.push_back(vv_xstrNonBonds[i][j]);
      }
    }
  }
} // namespace pocc
...
namespace pocc {
  string ReturnAtomSpecies(string atom) {
    if (atom.compare("H") == 0) {
      return "H  core";
    } else if (atom.compare("He") == 0) {
      return "He core";
    } else if (atom.compare("Li") == 0) {
      return "Li core";
    } else if (atom.compare("Be") == 0) {
      return "Be core";
    } else if (atom.compare("B") == 0) {
      return "B  core";
    } else if (atom.compare("C") == 0) {
      return "C  core";
    } else if (atom.compare("N") == 0) {
      return "N  core";
    } else if (atom.compare("O") == 0) {
      return "O  core";
    } else if (atom.compare("F") == 0) {
      return "F  core";
    } else if (atom.compare("Ne") == 0) {
      return "Ne core";
    } else if (atom.compare("Na") == 0) {
      return "Na core";
    } else if (atom.compare("Mg") == 0) {
      return "Mg core";
    } else if (atom.compare("Al") == 0) {
      return "Al core";
    } else if (atom.compare("Si") == 0) {
      return "Si core";
    } else if (atom.compare("P") == 0) {
      return "P  core";
    } else if (atom.compare("S") == 0) {
      return "S  core";
    } else if (atom.compare("Cl") == 0) {
      return "Cl core";
    } else if (atom.compare("Ar") == 0) {
      return "Ar core";
    } else if (atom.compare("K") == 0) {
      return "K  core";
    } else if (atom.compare("Ca") == 0) {
      return "Ca core";
    } else if (atom.compare("Sc") == 0) {
      return "Sc core";
    } else if (atom.compare("Ti") == 0) {
      return "Ti core";
    } else if (atom.compare("V") == 0) {
      return "V  core";
    } else if (atom.compare("Cr") == 0) {
      return "Cr core";
    } else if (atom.compare("Mn") == 0) {
      return "Mn core";
    } else if (atom.compare("Fe") == 0) {
      return "Fe core";
    } else if (atom.compare("Co") == 0) {
      return "Co core";
    } else if (atom.compare("Ni") == 0) {
      return "Ni core";
    } else if (atom.compare("Cu") == 0) {
      return "Cu core";
    } else if (atom.compare("Zn") == 0) {
      return "Zn core";
    } else if (atom.compare("Ga") == 0) {
      return "Ga core";
    } else if (atom.compare("Ge") == 0) {
      return "Ge core";
    } else if (atom.compare("As") == 0) {
      return "As core";
    } else if (atom.compare("Se") == 0) {
      return "Se core";
    } else if (atom.compare("Br") == 0) {
      return "Br core";
    } else if (atom.compare("Kr") == 0) {
      return "Kr core";
    } else if (atom.compare("Rb") == 0) {
      return "Rb core";
    } else if (atom.compare("Sr") == 0) {
      return "Sr core";
    } else if (atom.compare("Y") == 0) {
      return "Y  core";
    } else if (atom.compare("Zr") == 0) {
      return "Zr core";
    } else if (atom.compare("Nb") == 0) {
      return "Nb core";
    } else if (atom.compare("Mo") == 0) {
      return "Mo core";
    } else if (atom.compare("Tc") == 0) {
      return "Tc core";
    } else if (atom.compare("Ru") == 0) {
      return "Ru core";
    } else if (atom.compare("Rh") == 0) {
      return "Rh core";
    } else if (atom.compare("Pd") == 0) {
      return "Pd core";
    } else if (atom.compare("Ag") == 0) {
      return "Ag core";
    } else if (atom.compare("Cd") == 0) {
      return "Cd core";
    } else if (atom.compare("In") == 0) {
      return "In core";
    } else if (atom.compare("Sn") == 0) {
      return "Sn core";
    } else if (atom.compare("Sb") == 0) {
      return "Sb core";
    } else if (atom.compare("Te") == 0) {
      return "Te core";
    } else if (atom.compare("I") == 0) {
      return "I  core";
    } else if (atom.compare("Xe") == 0) {
      return "Xe core";
    } else if (atom.compare("Cs") == 0) {
      return "Cs core";
    } else if (atom.compare("Ba") == 0) {
      return "Ba core";
    } else if (atom.compare("La") == 0) {
      return "La core";
    } else if (atom.compare("Ce") == 0) {
      return "Ce core";
    } else if (atom.compare("Pr") == 0) {
      return "Pr core";
    } else if (atom.compare("Nd") == 0) {
      return "Nd core";
    } else if (atom.compare("Pm") == 0) {
      return "Pm core";
    } else if (atom.compare("Sm") == 0) {
      return "Sm core";
    } else if (atom.compare("Eu") == 0) {
      return "Eu core";
    } else if (atom.compare("Gd") == 0) {
      return "Gd core";
    } else if (atom.compare("Tb") == 0) {
      return "Tb core";
    } else if (atom.compare("Dy") == 0) {
      return "Dy core";
    } else if (atom.compare("Ho") == 0) {
      return "Ho core";
    } else if (atom.compare("Er") == 0) {
      return "Er core";
    } else if (atom.compare("Tm") == 0) {
      return "Tm core";
    } else if (atom.compare("Yb") == 0) {
      return "Yb core";
    } else if (atom.compare("Lu") == 0) {
      return "Lu core";
    } else if (atom.compare("Hf") == 0) {
      return "Hf core";
    } else if (atom.compare("Ta") == 0) {
      return "Ta core";
    } else if (atom.compare("W") == 0) {
      return "W  core";
    } else if (atom.compare("Re") == 0) {
      return "Re core";
    } else if (atom.compare("Os") == 0) {
      return "Os core";
    } else if (atom.compare("Ir") == 0) {
      return "Ir core";
    } else if (atom.compare("Pt") == 0) {
      return "Pt core";
    } else if (atom.compare("Au") == 0) {
      return "Au core";
    } else if (atom.compare("Hg") == 0) {
      return "Hg core";
    } else if (atom.compare("Tl") == 0) {
      return "Tl core";
    } else if (atom.compare("Pb") == 0) {
      return "Pb core";
    } else if (atom.compare("Bi") == 0) {
      return "Bi core";
    } else if (atom.compare("Po") == 0) {
      return "Po core";
    } else if (atom.compare("At") == 0) {
      return "At core";
    } else if (atom.compare("Rn") == 0) {
      return "Rn core";
    } else if (atom.compare("Fr") == 0) {
      return "Fr core";
    } else if (atom.compare("Ra") == 0) {
      return "Ra core";
    } else if (atom.compare("Ac") == 0) {
      return "Ac core";
    } else if (atom.compare("Th") == 0) {
      return "Th core";
    } else if (atom.compare("Pa") == 0) {
      return "Pa core";
    } else if (atom.compare("U") == 0) {
      return "U  core";
    } else if (atom.compare("Np") == 0) {
      return "Np core";
    } else if (atom.compare("Pu") == 0) {
      return "Pu core";
    } else if (atom.compare("Am") == 0) {
      return "Am core";
    } else if (atom.compare("Cm") == 0) {
      return "Cm core";
    } else if (atom.compare("Bk") == 0) {
      return "Bk core";
    } else if (atom.compare("Cf") == 0) {
      return "Cf core";
    } else if (atom.compare("Es") == 0) {
      return "Es core";
    } else if (atom.compare("Fm") == 0) {
      return "Fm core";
    } else if (atom.compare("Md") == 0) {
      return "Md core";
    } else if (atom.compare("No") == 0) {
      return "No core";
    } else if (atom.compare("Lr") == 0) {
      return "Lr core";
    } else {
      return "Unknown";
    }
...
namespace pocc {
  string ReturnAtomSpeciesPotential(string atom) {
    if (atom.compare("H") == 0) {
      return "H    0.354   180.000 2.886   0.044   12.000 0.7120 0 0.000  0.0  0.0000  4.528";
    } else if (atom.compare("He") == 0) {
      return "He   0.849   90.000  2.362   0.056   15.240 0.0972 4 0.000  0.0  0.0000  9.660";
    } else if (atom.compare("Li") == 0) {
      return "Li   1.336   180.000 2.451   0.025   12.000 1.0255 0 0.000  0.0  0.0000  3.006";
    } else if (atom.compare("Be") == 0) {
      return "Be   1.074   109.470 2.745   0.085   12.000 1.5650 3 0.000  0.0  0.0000  4.877";
    } else if (atom.compare("B") == 0) {
      return "B    0.838   109.470 4.083   0.180   12.052 1.7550 3 0.000  0.0  0.0000  4.068";
    } else if (atom.compare("C") == 0) {
      return "C    0.706   180.000 3.851   0.105   12.730 1.9120 3 2.119  0.0  0.0000  5.343";
    } else if (atom.compare("N") == 0) {
      return "N    0.656   180.000 3.660   0.069   13.407 2.5438 3 0.450  0.0 61.2230  6.899";
    } else if (atom.compare("O") == 0) {
      return "O    0.639   180.000 3.500   0.060   14.085 2.2998 3 0.018  0.0  0.0000  8.741";
    } else if (atom.compare("F") == 0) {
      return "F    0.668   180.000 3.364   0.050   14.762 1.7350 0 0.000  0.0  0.0000 10.874";
    } else if (atom.compare("Ne") == 0) {
      return "Ne   0.920   90.000  3.243   0.042   15.440 0.1944 4 0.000  0.0  0.0000 11.040";
    } else if (atom.compare("Na") == 0) {
      return "Na   1.539   180.000 2.983   0.030   12.000 1.0809 0 0.000  0.0  0.0000  2.843";
    } else if (atom.compare("Mg") == 0) {
      return "Mg   1.421   109.470 3.021   0.111   12.000 1.7866 3 0.000  0.0  0.0000  3.951";
    } else if (atom.compare("Al") == 0) {
      return "Al   1.244   109.470 4.499   0.505   11.278 1.7924 3 0.000  0.0  0.0000  3.041";
    } else if (atom.compare("Si") == 0) {
      return "Si   1.117   109.470 4.295   0.402   12.175 2.3232 3 1.225  0.0  0.0000  4.168";
    } else if (atom.compare("P") == 0) {
      return "P    1.101   93.800  4.147   0.305   13.072 2.8627 3 2.400 22.0 84.4339  5.463";
    } else if (atom.compare("S") == 0) {
      return "S    1.064   92.1000 4.035   0.274   13.969 2.7032 3 0.484  0.0  0.0000  6.928";
    } else if (atom.compare("Cl") == 0) {
      return "Cl   1.044   180.000 3.947   0.227   14.866 2.3484 0 0.000  0.0  0.0000  8.564";
    } else if (atom.compare("Ar") == 0) {
      return "Ar   1.032   90.000  3.868   0.185   15.763 0.2994 4 0.000  0.0  0.0000  9.465";
    } else if (atom.compare("K") == 0) {
      return "K    1.953   180.000 3.812   0.035   12.000 1.1645 0 0.000  0.0  0.0000  2.421";
    } else if (atom.compare("Ca") == 0) {
      return "Ca   1.761   90.000  3.399   0.238   12.000 2.1414 6 0.000  0.0  0.0000  3.231";
    } else if (atom.compare("Sc") == 0) {
      return "Sc   1.513   109.470 3.295   0.019   12.000 2.5924 3 0.000  0.0  0.0000  3.395";
    } else if (atom.compare("Ti") == 0) {
      return "Ti   1.412   109.470 3.175   0.017   12.000 2.6595 3 0.000  0.0  0.0000  3.470";
    } else if (atom.compare("V") == 0) {
      return "V    1.402   109.470 3.144   0.016   12.000 2.6789 3 0.000  0.0  0.0000  3.650";
    } else if (atom.compare("Cr") == 0) {
      return "Cr   1.345   90.000  3.023   0.015   12.000 2.4631 6 0.000  0.0  0.0000  3.415";
    } else if (atom.compare("Mn") == 0) {
      return "Mn   1.382   90.000  2.961   0.013   12.000 2.4301 6 0.000  0.0  0.0000  3.325";
    } else if (atom.compare("Fe") == 0) {
      return "Fe   1.270   109.470 2.912   0.013   12.000 2.4301 3 0.000  0.0  0.0000  3.760";
    } else if (atom.compare("Co") == 0) {
      return "Co   1.241   90.000  2.872   0.014   12.000 2.4301 6 0.000  0.0  0.0000  4.105";
    } else if (atom.compare("Ni") == 0) {
      return "Ni   1.164   90.000  2.834   0.015   12.000 2.4301 4 0.000  0.0  0.0000  4.465";
    } else if (atom.compare("Cu") == 0) {
      return "Cu   1.302   109.470 3.495   0.005   12.000 1.7565 3 0.000  0.0  0.0000  3.729";
    } else if (atom.compare("Zn") == 0) {
      return "Zn   1.193   109.470 2.763   0.124   12.000 1.3084 3 0.000  0.0  0.0000  5.106";
    } else if (atom.compare("Ga") == 0) {
      return "Ga   1.260   109.470 4.383   0.415   11.000 1.8206 3 0.000  0.0  0.0000  2.999";
    } else if (atom.compare("Ge") == 0) {
      return "Ge   1.197   109.470 4.280   0.379   12.000 2.7888 3 0.701  0.0  0.0000  4.051";
    } else if (atom.compare("As") == 0) {
      return "As   1.211   92.100  4.230   0.309   13.000 2.8640 3 1.500 22.0 86.9735  5.188";
    } else if (atom.compare("Se") == 0) {
      return "Se   1.190   90.600  4.205   0.291   14.000 2.7645 3 0.335  0.0  0.0000  6.428";
    } else if (atom.compare("Br") == 0) {
      return "Br   1.192   180.000 4.189   0.251   15.000 2.5186 0 0.000  0.0  0.0000  7.790";
    } else if (atom.compare("Kr") == 0) {
      return "Kr   1.147   90.000  4.141   0.220   16.000 0.4520 4 0.000  0.0  0.0000  8.505";
    } else if (atom.compare("Rb") == 0) {
      return "Rb   2.260   180.000 4.114   0.040   12.000 1.5922 0 0.000  0.0  0.0000  2.331";
    } else if (atom.compare("Sr") == 0) {
      return "Sr   2.052   90.000  3.641   0.235   12.000 2.4486 6 0.000  0.0  0.0000  3.024";
    } else if (atom.compare("Y") == 0) {
      return "Y    1.698   109.470 3.345   0.072   12.000 3.2573 3 0.000  0.0  0.0000  3.830";
    } else if (atom.compare("Zr") == 0) {
      return "Zr   1.564   109.470 3.124   0.069   12.000 3.6675 3 0.000  0.0  0.0000  3.400";
    } else if (atom.compare("Nb") == 0) {
      return "Nb   1.473   109.470 3.165   0.059   12.000 3.6179 3 0.000  0.0  0.0000  3.550";
    } else if (atom.compare("Mo") == 0) {
      return "Mo   1.467   90.000  3.052   0.056   12.000 3.4021 6 0.000  0.0  0.0000  3.465";
    } else if (atom.compare("Tc") == 0) {
      return "Mo   1.322   90.000  2.998   0.048   12.000 3.4021 3 0.000  0.0  0.0000  3.465";
    } else if (atom.compare("Ru") == 0) {
      return "Tc   1.478   90.000  2.963   0.056   12.000 3.4021 6 0.000  0.0  0.0000  3.290";
    } else if (atom.compare("Rh") == 0) {
      return "Ru   1.332   90.000  2.929   0.053   12.000 3.4021 6 0.000  0.0  0.0000  3.575";
    } else if (atom.compare("Pd") == 0) {
      return "Pd   1.338   90.000  2.899   0.048   12.000 3.2077 4 0.000  0.0  0.0000  4.320";
    } else if (atom.compare("Ag") == 0) {
      return "Ag   1.386   180.000 3.148   0.036   12.000 1.9557 1 0.200  0.0  0.0000  4.436";
    } else if (atom.compare("Cd") == 0) {
      return "Cd   1.403   109.470 2.848   0.228   12.000 1.6525 3 0.000  0.0  0.0000  5.034";
    } else if (atom.compare("In") == 0) {
      return "In   1.459   109.470 4.463   0.599   11.000 2.0704 3 0.000  0.0  0.0000  2.997";
    } else if (atom.compare("Sn") == 0) {
      return "Sn   1.398   109.470 4.392   0.567   12.000 2.9608 3 0.199  0.0  0.0000  3.987";
    } else if (atom.compare("Sb") == 0) {
      return "Sb   1.407   91.600  4.420   0.449   13.000 2.7042 3 1.100 22.0 87.7047  4.899";
    } else if (atom.compare("Te") == 0) {
      return "Te   1.386   90.250  4.470   0.398   14.000 2.8821 3 0.300  0.0  0.0000  5.816";
    } else if (atom.compare("I") == 0) {
      return "I    1.382   180.000 4.500   0.339   15.000 2.6537 0 0.000  0.0  0.0000  6.822";
    } else if (atom.compare("Xe") == 0) {
      return "Xe   1.267   90.000  4.404   0.332   12.000 0.5560 4 0.000  0.0  0.0000  7.595";
    } else if (atom.compare("Cs") == 0) {
      return "Cs   2.570   180.000 4.517   0.045   12.000 1.5728 0 0.000  0.0  0.0000  2.183";
    } else if (atom.compare("Ba") == 0) {
      return "Ba   2.277   90.000  3.703   0.364   12.000 2.7266 6 0.000  0.0  0.0000  2.814";
    } else if (atom.compare("La") == 0) {
      return "La   1.943   109.470 3.522   0.017   12.000 3.3049 3 0.000  0.0  0.0000  2.8355";
    } else if (atom.compare("Ce") == 0) {
      return "Ce   1.841   90.000  3.556   0.013   12.000 3.3049 6 0.000  0.0  0.0000  2.774";
    } else if (atom.compare("Pr") == 0) {
      return "Pr   1.823   90.000  3.606   0.010   12.000 3.3049 6 0.000  0.0  0.0000  2.858";
    } else if (atom.compare("Nd") == 0) {
      return "Nd   1.816   90.000  3.575   0.010   12.000 3.3049 6 0.000  0.0  0.0000  2.8685";
    } else if (atom.compare("Pm") == 0) {
      return "Pm   1.801   90.000  3.547   0.009   12.000 3.3049 6 0.000  0.0  0.0000  2.881";
    } else if (atom.compare("Sm") == 0) {
      return "Sm   1.780   90.000  3.520   0.008   12.000 3.3049 6 0.000  0.0  0.0000  2.9115";
    } else if (atom.compare("Eu") == 0) {
      return "Eu   1.771   90.000  3.493   0.008   12.000 3.3049 6 0.000  0.0  0.0000  2.8785";
    } else if (atom.compare("Gd") == 0) {
      return "Gd   1.735   90.000  3.368   0.009   12.000 3.3049 6 0.000  0.0  0.0000  3.1665";
    } else if (atom.compare("Tb") == 0) {
      return "Tb   1.732   90.000  3.451   0.007   12.000 3.3049 6 0.000  0.0  0.0000  3.018";
    } else if (atom.compare("Dy") == 0) {
      return "Dy   1.710   90.000  3.428   0.007   12.000 3.3049 6 0.000  0.0  0.0000  3.0555";
    } else if (atom.compare("Ho") == 0) {
      return "Ho   1.696   90.000  3.409   0.007   12.000 3.4157 6 0.000  0.0  0.0000  3.127";
    } else if (atom.compare("Er") == 0) {
      return "Er   1.673   90.000  3.391   0.007   12.000 3.3049 6 0.000  0.0  0.0000  3.1865";
    } else if (atom.compare("Tm") == 0) {
      return "Tm   1.660   90.000  3.374   0.006   12.000 3.3049 6 0.000  0.0  0.0000  3.2514";
    } else if (atom.compare("Yb") == 0) {
      return "Yb   1.637   90.000  3.355   0.228   12.000 2.6177 6 0.000  0.0  0.0000  3.2889";
    } else if (atom.compare("Lu") == 0) {
      return "Lu   1.671   90.000  3.640   0.041   12.000 3.2709 6 0.000  0.0  0.0000  2.9629";
    } else if (atom.compare("Hf") == 0) {
      return "Hf   1.611   109.470 3.141   0.072   12.000 3.9212 3 0.000  0.0  0.0000  3.7000";
    } else if (atom.compare("Ta") == 0) {
      return "Ta   1.511   109.470 3.170   0.081   12.000 4.0748 3 0.000  0.0  0.0000  5.1000";
    } else if (atom.compare("W") == 0) {
      return "W    1.392   90.000  3.069   0.067   12.000 3.6937 6 0.000  0.0  0.0000  4.6300";
    } else if (atom.compare("Re") == 0) {
      return "Re   1.372   90.000  2.954   0.066   12.000 3.6937 6 0.000  0.0  0.0000  3.9600";
    } else if (atom.compare("Os") == 0) {
      return "Os   1.372   90.000  3.120   0.037   12.000 3.6937 6 0.000  0.0  0.0000  5.1400";
    } else if (atom.compare("Ir") == 0) {
      return "Ir   1.371   90.000  2.840   0.073   12.000 3.7307 6 0.000  0.0  0.0000  5.0000";
    } else if (atom.compare("Pt") == 0) {
      return "Pt   1.364   90.000  2.754   0.080   12.000 3.3817 4 0.000  0.0  0.0000  4.7900";
    } else if (atom.compare("Au") == 0) {
      return "Au   1.262   90.000  3.293   0.039   12.000 2.6255 4 0.000  0.0  0.0000  4.8940";
    } else if (atom.compare("Hg") == 0) {
      return "Hg   1.340   180.000 2.705   0.385   12.000 1.7497 1 0.000  0.0  0.0000  6.2700";
    } else if (atom.compare("Tl") == 0) {
      return "Tl   1.518   120.000 4.347   0.680   11.000 2.0685 3 0.000  0.0  0.0000  3.2000";
    } else if (atom.compare("Pb") == 0) {
      return "Pb   1.459   109.470 4.297   0.663   12.000 2.8461 3 0.100  0.0  0.0000  3.9000";
    } else if (atom.compare("Bi") == 0) {
      return "Bi   1.512   90.000  4.370   0.518   13.000 2.4700 3 1.000 22.0 90.0000  4.6900";
    } else if (atom.compare("Po") == 0) {
      return "Po   1.500   90.000  4.709   0.325   14.000 2.3329 3 0.300  0.0  0.0000  4.2100";
    } else if (atom.compare("At") == 0) {
      return "At   1.545   180.000 4.750   0.284   15.000 2.2357 0 0.000  0.0  0.0000  4.7500";
    } else if (atom.compare("Rn") == 0) {
      return "Rn   1.420   90.000  4.765   0.248   16.000 0.5832 4 0.000  0.0  0.0000  5.3700";
    } else if (atom.compare("Fr") == 0) {
      return "Fr   2.880   180.000 4.900   0.050   12.000 1.8469 0 0.000  0.0  0.0000  2.0000";
    } else if (atom.compare("Ra") == 0) {
      return "Ra   2.512   90.000  3.677   0.404   12.000 2.9161 6 0.000  0.0  0.0000  2.8430";
    } else if (atom.compare("Ac") == 0) {
      return "Ac   1.983   90.000  3.478   0.033   12.000 3.8882 6 0.000  0.0  0.0000  2.8350";
    } else if (atom.compare("Th") == 0) {
      return "Th   1.721   90.000  3.396   0.026   12.000 4.2021 6 0.000  0.0  0.0000  3.1750";
    } else if (atom.compare("Pa") == 0) {
      return "Pa   1.711   90.000  3.424   0.022   12.000 3.8882 6 0.000  0.0  0.0000  2.9850";
    } else if (atom.compare("U") == 0) {
      return "U    1.684   90.000  3.395   0.022   12.000 3.8882 6 0.000  0.0  0.0000  3.3410";
    } else if (atom.compare("Np") == 0) {
      return "Np   1.666   90.000  3.424   0.019   12.000 3.8882 6 0.000  0.0  0.0000  3.5490";
    } else if (atom.compare("Pu") == 0) {
      return "Pu   1.657   90.000  3.424   0.016   12.000 3.8882 6 0.000  0.0  0.0000  3.2430";
    } else if (atom.compare("Am") == 0) {
      return "Am   1.660   90.000  3.381   0.014   12.000 3.8882 6 0.000  0.0  0.0000  2.9895";
    } else if (atom.compare("Cm") == 0) {
      return "Cm   1.801   90.000  3.326   0.013   12.000 3.8882 6 0.000  0.0  0.0000  2.8315";
    } else if (atom.compare("Bk") == 0) {
      return "Bk   1.761   90.000  3.339   0.013   12.000 3.8882 6 0.000  0.0  0.0000  3.1935";
    } else if (atom.compare("Cf") == 0) {
      return "Cf   1.750   90.000  3.313   0.013   12.000 3.8882 6 0.000  0.0  0.0000  3.1970";
    } else if (atom.compare("Es") == 0) {
      return "Es   1.724   90.000  3.299   0.012   12.000 3.8882 6 0.000  0.0  0.0000  3.3330";
    } else if (atom.compare("Fm") == 0) {
      return "Fm   1.712   90.000  3.286   0.012   12.000 3.8882 6 0.000  0.0  0.0000  3.4000";
    } else if (atom.compare("Md") == 0) {
      return "Md   1.689   90.000  3.274   0.011   12.000 3.8882 6 0.000  0.0  0.0000  3.4700";
    } else if (atom.compare("No") == 0) {
      return "No   1.679   90.000  3.248   0.011   12.000 3.8882 6 0.000  0.0  0.0000  3.4750";
    } else if (atom.compare("Lr") == 0) {
      return "Lr   1.698   90.000  3.236   0.011   12.000 3.8882 6 0.000  0.0  0.0000  3.5000";
    } else {
      return "Unknown";
    }
  }
...
namespace pocc {
  string ReturnAtomProperties(string atom) {
    if (atom.compare("H") == 0) {
      return "Hydrogen         H     1   1.007940    0.53    2.20     ";
    } else if (atom.compare("He") == 0) {
      return "Helium           He    2   4.002602    0.31    4.16     ";
    } else if (atom.compare("Li") == 0) {
      return "Lithium          Li    3   6.941000    1.67    0.98     ";
    } else if (atom.compare("Be") == 0) {
      return "Beryllium        Be    4   9.012182    1.12    1.57     ";
    } else if (atom.compare("B") == 0) {
      return "Boron            B     5   10.811000   0.87    2.04     ";
    } else if (atom.compare("C") == 0) {
      return "Carbon           C     6   12.010700   0.67    2.55     ";
    } else if (atom.compare("N") == 0) {
      return "Nitrogen         N     7   14.006700   0.56    3.04     ";
    } else if (atom.compare("O") == 0) {
      return "Oxygen           O     8   15.999400   0.48    3.44     ";
    } else if (atom.compare("F") == 0) {
      return "Fluorine         F     9   18.998403   0.42    3.98     ";
    } else if (atom.compare("Ne") == 0) {
      return "Neon             Ne    10  20.179700   0.38    4.79     ";
    } else if (atom.compare("Na") == 0) {
      return "Sodium           Na    11  22.989770   1.90    0.93     ";
    } else if (atom.compare("Mg") == 0) {
      return "Magnesium        Mg    12  24.305000   1.45    1.31     ";
    } else if (atom.compare("Al") == 0) {
      return "Aluminium        Al    13  26.981538   1.18    1.61     ";
    } else if (atom.compare("Si") == 0) {
      return "Silicon          Si    14  28.085500   1.11    1.90     ";
    } else if (atom.compare("P") == 0) {
      return "Phosphorus       P     15  30.973761   0.98    2.19     ";
    } else if (atom.compare("S") == 0) {
      return "Sulphur          S     16  32.065000   0.88    2.58     ";
    } else if (atom.compare("Cl") == 0) {
      return "Chlorine         Cl    17  35.453000   0.79    3.16     ";
    } else if (atom.compare("Ar") == 0) {
      return "Argon            Ar    18  39.948000   0.71    3.24     ";
    } else if (atom.compare("K") == 0) {
      return "Potassium        K     19  39.098300   2.43    0.82     ";
    } else if (atom.compare("Ca") == 0) {
      return "Calcium          Ca    20  40.078000   1.94    1.00     ";
    } else if (atom.compare("Sc") == 0) {
      return "Scandium         Sc    21  44.955910   1.84    1.36     ";
    } else if (atom.compare("Ti") == 0) {
      return "Titanium         Ti    22  47.867000   1.76    1.54     ";
    } else if (atom.compare("V") == 0) {
      return "Vanadium         V     23  50.941500   1.71    1.63     ";
    } else if (atom.compare("Cr") == 0) {
      return "Chromium         Cr    24  51.996100   1.66    1.66     ";
    } else if (atom.compare("Mn") == 0) {
      return "Manganese        Mn    25  54.938049   1.61    1.55     ";
    } else if (atom.compare("Fe") == 0) {
      return "Iron             Fe    26  55.845000   1.56    1.83     ";
    } else if (atom.compare("Co") == 0) {
      return "Cobalt           Co    27  58.933200   1.52    1.88     ";
    } else if (atom.compare("Ni") == 0) {
      return "Nickel           Ni    28  58.693400   1.49    1.91     ";
    } else if (atom.compare("Cu") == 0) {
      return "Copper           Cu    29  63.546000   1.45    1.90     ";
    } else if (atom.compare("Zn") == 0) {
      return "Zinc             Zn    30  65.390000   1.42    1.65     ";
    } else if (atom.compare("Ga") == 0) {
      return "Gallium          Ga    31  69.723000   1.36    1.81     ";
    } else if (atom.compare("Ge") == 0) {
      return "Germanium        Ge    32  72.640000   1.25    2.01     ";
    } else if (atom.compare("As") == 0) {
      return "Arsenic          As    33  74.921600   1.14    2.18     ";
    } else if (atom.compare("Se") == 0) {
      return "Selenium         Se    34  78.960000   1.03    2.55     ";
    } else if (atom.compare("Br") == 0) {
      return "Bromine          Br    35  79.904000   0.94    2.96     ";
    } else if (atom.compare("Kr") == 0) {
      return "Krypton          Kr    36  83.800000   0.88    3.00     ";
    } else if (atom.compare("Rb") == 0) {
      return "Rubidium         Rb    37  85.467800   2.65    0.82     ";
    } else if (atom.compare("Sr") == 0) {
      return "Strontium        Sr    38  87.620000   2.19    0.95     ";
    } else if (atom.compare("Y") == 0) {
      return "Yttrium          Y     39  88.905850   2.12    1.22     ";
    } else if (atom.compare("Zr") == 0) {
      return "Zirconium        Zr    40  91.224000   2.06    1.33     ";
    } else if (atom.compare("Nb") == 0) {
      return "Niobium          Nb    41  92.906380   1.98    1.60     ";
    } else if (atom.compare("Mo") == 0) {
      return "Molybdenum       Mo    42  95.940000   1.90    2.16     ";
    } else if (atom.compare("Tc") == 0) {
      return "Technetium       Mo    43  98.000000   1.83    1.90     ";
    } else if (atom.compare("Ru") == 0) {
      return "Ruthenium        Tc    44  101.070000  1.78    2.20     ";
    } else if (atom.compare("Rh") == 0) {
      return "Rhodium          Ru    45  102.905500  1.73    2.28     ";
    } else if (atom.compare("Pd") == 0) {
      return "Palladium        Pd    46  106.420000  1.69    2.20     ";
    } else if (atom.compare("Ag") == 0) {
      return "Silver           Ag    47  107.868200  1.65    1.93     ";
    } else if (atom.compare("Cd") == 0) {
      return "Cadmium          Cd    48  112.411000  1.61    1.69     ";
    } else if (atom.compare("In") == 0) {
      return "Indium           In    49  114.818000  1.56    1.78     ";
    } else if (atom.compare("Sn") == 0) {
      return "Tin              Sn    50  118.710000  1.45    1.96     ";
    } else if (atom.compare("Sb") == 0) {
      return "Antimony         Sb    51  121.760000  1.33    2.05     ";
    } else if (atom.compare("Te") == 0) {
      return "Tellurium        Te    52  127.600000  1.23    2.10     ";
    } else if (atom.compare("I") == 0) {
      return "Iodine           I     53  126.904470  1.15    2.66     ";
    } else if (atom.compare("Xe") == 0) {
      return "Xenon            Xe    54  131.293000  1.08    2.60     ";
    } else if (atom.compare("Cs") == 0) {
      return "Cesium           Cs    55  132.905450  2.98    0.79     ";
    } else if (atom.compare("Ba") == 0) {
      return "Barium           Ba    56  137.327000  2.53    0.89     ";
    } else if (atom.compare("La") == 0) {
      return "Lanthanium       La    57  138.905500  1.95    1.10     ";
    } else if (atom.compare("Ce") == 0) {
      return "Cerium           Ce    58  140.116000  1.85    1.12     ";
    } else if (atom.compare("Pr") == 0) {
      return "Praseodymium     Pr    59  140.907650  2.47    1.13     ";
    } else if (atom.compare("Nd") == 0) {
      return "Neodymium        Nd    60  144.240000  2.06    1.14     ";
    } else if (atom.compare("Pm") == 0) {
      return "Promethium       Pm    61  145.000000  2.05    1.07     ";
    } else if (atom.compare("Sm") == 0) {
      return "Samarium         Sm    62  150.360000  2.38    1.17     ";
    } else if (atom.compare("Eu") == 0) {
      return "Europium         Eu    63  151.964000  2.31    1.01     ";
    } else if (atom.compare("Gd") == 0) {
      return "Gadolinium       Gd    64  157.250000  2.33    1.20     ";
    } else if (atom.compare("Tb") == 0) {
      return "Terbium          Tb    65  158.925340  2.25    1.10     ";
    } else if (atom.compare("Dy") == 0) {
      return "Dysprosium       Dy    66  162.500000  2.28    1.22     ";
    } else if (atom.compare("Ho") == 0) {
      return "Holmium          Ho    67  164.930320  2.26    1.23     ";
    } else if (atom.compare("Er") == 0) {
      return "Erbium           Er    68  167.259000  2.26    1.24     ";
    } else if (atom.compare("Tm") == 0) {
      return "Thulium          Tm    69  168.934210  2.22    1.25     ";
    } else if (atom.compare("Yb") == 0) {
      return "Ytterbium        Yb    70  173.040000  2.22    1.06     ";
    } else if (atom.compare("Lu") == 0) {
      return "Lutetium         Lu    71  174.967000  2.17    1.27     ";
    } else if (atom.compare("Hf") == 0) {
      return "Hafnium          Hf    72  178.490000  2.08    1.30     ";
    } else if (atom.compare("Ta") == 0) {
      return "Tantalum         Ta    73  180.947900  2.00    1.50     ";
    } else if (atom.compare("W") == 0) {
      return "Tungsten         W     74  183.840000  1.93    2.36     ";
    } else if (atom.compare("Re") == 0) {
      return "Rhenium          Re    75  186.207000  1.88    1.90     ";
    } else if (atom.compare("Os") == 0) {
      return "Osmium           Os    76  190.230000  1.85    2.20     ";
    } else if (atom.compare("Ir") == 0) {
      return "Iridium          Ir    77  192.217000  1.80    2.20     ";
    } else if (atom.compare("Pt") == 0) {
      return "Platinum         Pt    78  195.078000  1.77    2.28     ";
    } else if (atom.compare("Au") == 0) {
      return "Gold             Au    79  196.966550  1.74    2.54     ";
    } else if (atom.compare("Hg") == 0) {
      return "Mercury          Hg    80  200.590000  1.71    2.00     ";
    } else if (atom.compare("Tl") == 0) {
      return "Thallium         Tl    81  204.383300  1.56    1.62     ";
    } else if (atom.compare("Pb") == 0) {
      return "Lead             Pb    82  207.200000  1.54    2.33     ";
    } else if (atom.compare("Bi") == 0) {
      return "Bismuth          Bi    83  208.980380  1.43    2.02     ";
    } else if (atom.compare("Po") == 0) {
      return "Polonium         Po    84  209.000000  1.35    2.00     ";
    } else if (atom.compare("At") == 0) {
      return "Astatine         At    85  210.000000  1.27    2.20     ";
    } else if (atom.compare("Rn") == 0) {
      return "Radon            Rn    86  222.000000  1.20    2.59     ";
    } else if (atom.compare("Fr") == 0) {
      return "Francium         Fr    87  223.000000  2.18    0.70     ";
    } else if (atom.compare("Ra") == 0) {
      return "Radium           Ra    88  226.000000  2.40    0.90     ";
    } else if (atom.compare("Ac") == 0) {
      return "Actinium         Ac    89  227.000000  2.20    1.10     ";
    } else if (atom.compare("Th") == 0) {
      return "Thorium          Th    90  232.038100  1.65    1.30     ";
    } else if (atom.compare("Pa") == 0) {
      return "Protoactinium    Pa    91  231.035880  1.65    1.50     ";
    } else if (atom.compare("U") == 0) {
      return "Uranium          U     92  238.028910  1.42    1.38     ";
    }
