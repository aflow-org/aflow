...
namespace pocc {
  void writeQHAdatablock(stringstream& output, vector<vector<double>> data, string block_name) {
    const string blockname = "[" + block_name + "]";

    output << AFLOWIN_SEPARATION_LINE << std::endl;
    output << blockname + "START" << std::endl;

    // print header
    output << setw(5) << "#T[K]" << setw(SW) << ' ' << setw(TW) << "V[A^3/atom]" << setw(SW) << ' ' << setw(TW) << "F(V)[eV/atom]" << setw(SW) << ' ' << setw(TW) << "B[GPa]" << setw(SW) << ' ' << setw(TW)
           << "beta[10^-5/K]" << setw(SW) << ' ' << setw(TW) << "Cv(V)[kB/atom]" << setw(SW) << ' ' << setw(TW) << "Cp(V)[kB/atom]" << setw(SW) << ' ' << setw(TW) << "gamma" << setw(SW) << ' ' << setw(TW) << "Bprime";
    output << std::endl;
    ...
    for (size_t row = 0; row < data.size(); row++) {
      ...
      output << setw(5) << data[row][0] << setw(SW) << ' ' << std::fixed << std::setprecision(PRECISION);
      // output other properties
      for (size_t col = 1; col < data[row].size(); col++) {
  ...
  void POccCalculator::calculateQHAPropertiesAVG(const vector<double>& v_temperatures) {
    ...
    const bool LDEBUG = false || _DEBUG_POCC_QHA_ || XHOST.DEBUG;

    msg = "Performing POCC+QHA post-processing step.";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, msg, m_aflags.Directory, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    ...
    for (size_t i = 0; i < m_ARUN_directories.size(); i++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " file: " << filename << endl;
      }

      if (aurostd::CompressFileExist(filename)) {
        data = aurostd::substring2string(aurostd::compressfile2string(filename), "[QHA_SJ_THERMO]START", "[QHA_SJ_THERMO]STOP", 0);

        vector<string> lines = aurostd::string2vectorstring(data);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " datasize: " << lines.size() << endl;
        }
        if (lines.size() > 1) {
          ...
          for (size_t j = 0; j < lines.size(); j++) {
            ...
            if (ncols == line.size()) {
              ...
              throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, msg, _INDEX_MISMATCH_);
            }
          }
    ...
    const uint n = pocc_qha_thermo_properties.size();
    ...
    for (uint i = 0; i < n; i++) {
      if (!pocc_qha_thermo_properties[i].empty()) {
        pocc_qha_thermo_properties[i].resize(minsize);
      } else {
        pocc_qha_thermo_properties[i] = vector<vector<double>>(minsize, vector<double>(ncols));
      }
    }
    ...
    for (uint i = 0; i < n - 1; i++) {
      for (size_t row = 0; row < minsize; row++) {
        if (!aurostd::isequal(pocc_qha_thermo_properties[i][row][0], pocc_qha_thermo_properties[i + 1][row][0])) {
    ...
    for (size_t i = 0; i < v_temperatures.size(); i++) {
      ...
      for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
        ...
        for (size_t row = 0; row < minsize; row++) {
          for (uint col = 0; col < ncols; col++) {
            averaged_qha_data[row][col] += (*it).m_probability * pocc_qha_thermo_properties[isupercell][row][col];
          }
        }
      }
      writeQHAdatablock(file, averaged_qha_data, "POCC_QHA_SJ_THERMO_T=" + aurostd::utype2string(T));
    }
    ...
    for (size_t row = 0; row < minsize; row++) {
      ...
      for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
        isupercell = std::distance(l_supercell_sets.begin(), it);
        for (uint col = 0; col < ncols; col++) {
          averaged_qha_data_T[row][col] += (*it).m_probability * pocc_qha_thermo_properties[isupercell][row][col];
        }
      }
    }
    ...
    for (size_t row = 0; row < minsize; row++) {
      ...
      for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
        isupercell = std::distance(l_supercell_sets.begin(), it);
        for (uint col = 0; col < ncols; col++) {
          averaged_qha_data_F_T[row][col] += (*it).m_probability * pocc_qha_thermo_properties[isupercell][row][col];
        }
      }
    }
    ...
    file << setw(5) << "#T[K]" << setw(SW) << ' ' << setw(TW) << "beta[10^-5/K]" << std::endl;
    for (size_t row = 0; row < minsize; row++) {
      file.unsetf(ios_base::floatfield);
      file << setw(5) << averaged_qha_data_T[row][0] << setw(SW) << ' ' << std::fixed << std::setprecision(PRECISION) << setw(TW) << beta[row + 1] * 1e5 << std::endl;
    }
    ...
    for (size_t row = 0; row < minsize; row++) {
      file.unsetf(ios_base::floatfield);
      file << setw(5) << averaged_qha_data_F_T[row][0] << setw(SW) << ' ' << std::fixed << std::setprecision(PRECISION) << setw(TW) << beta[row + 1] * 1e5 << std::endl;
    }
