...
namespace pocc {
  void POccCalculator::calculateElasticProperties(const vector<double>& v_temperatures) {
    bool ael_run_postprocess = false;
    bool ael_write_full_results = false;
    vector<double> Bvoigt;
    vector<double> Breuss;
    vector<double> Bvrh;
    vector<double> Gvoigt;
    vector<double> Greuss;
    vector<double> Gvrh;
    vector<double> Poisson_ratio;
    vector<vector<vector<double>>> elastic_tensor_list;
    vector<vector<vector<double>>> compliance_tensor_list;
    // for (size_t i = 0; i < v_temperatures.size(); i++) {
...
// ***************************************************************************
//  void POccCalculator::generateElasticProperties(vector<double>& Bvoigt, vector<double>& Breuss, vector<double>& Bvrh, vector<double>& Gvoigt, vector<double>& Greuss, vector<double>& Gvrh,vector<double>&
//  Poisson_ratio, vector<vector<vector<double> > >& elastic_tensor_list,  vector<vector<vector<double> > >& compliance_tensor_list) Runs AEL postprocessing to generate elastic properties
// ***************************************************************************
namespace pocc {
  void POccCalculator::generateElasticProperties(vector<double>& Bvoigt,
                                                 vector<double>& Breuss,
                                                 vector<double>& Bvrh,
                                                 vector<double>& Gvoigt,
                                                 vector<double>& Greuss,
                                                 ...
                                                 vector<vector<vector<double>>>& elastic_tensor_list,
                                                 vector<vector<vector<double>>>& compliance_tensor_list) {
    const bool LDEBUG = (false || _DEBUG_POCC_AEL_AGL_ || XHOST.DEBUG);
    uint aelerror = 0;
    if (LDEBUG) {
    ...
    double ael_bulk_modulus_voigt = 0.0;
    double ael_bulk_modulus_reuss = 0.0;
    double ael_bulk_modulus_vrh = 0.0;
    double ael_shear_modulus_voigt = 0.0;
    double ael_shear_modulus_reuss = 0.0;
    double ael_shear_modulus_vrh = 0.0;
    ...
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " ael_filename=[" << ael_filename << "]" << endl;
      }
      // if(!aurostd::CompressFileExist(ael_filename,ael_filename)){throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"No aflow.ael.out file found [dir="+pocc_directory_abs+"]",_FILE_NOT_FOUND_);}
      ...
      if (run_directory) {
        ...
        if (aurostd::FileExist(pocc_directory_abs + "/" + _AFLOWLOCK_)) {
          aurostd::file2file(pocc_directory_abs + "/" + _AFLOWLOCK_, pocc_directory_abs + "/" + _AFLOWLOCK_ + ".run");
        } // keep original LOCK
      ...
      if (aurostd::CompressFileExist(ael_filename, ael_filename)) {
        aurostd::compressfile2string(ael_filename, ael_file_content);
        aflow_ael_out.str(aurostd::substring2string(ael_file_content, "[AEL_RESULTS]START", "[AEL_RESULTS]STOP", 1));
        if (LDEBUG) {
        ...
        for (size_t i = 0; i < vline.size(); i++) {
          ...
          if (tokens.size() >= 2) {
            if (tokens.at(0) == "ael_poisson_ratio") {
              ael_poisson_ratio = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "ael_bulk_modulus_voigt") {
              ael_bulk_modulus_voigt = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "ael_bulk_modulus_reuss") {
              ael_bulk_modulus_reuss = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "ael_shear_modulus_voigt") {
              ael_shear_modulus_voigt = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "ael_shear_modulus_reuss") {
              ael_shear_modulus_reuss = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "ael_bulk_modulus_vrh") {
              ael_bulk_modulus_vrh = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "ael_shear_modulus_vrh") {
              ael_shear_modulus_vrh = aurostd::string2utype<double>(tokens.at(1));
            }
          }
        }
        aflow_elastic_tensor.str(aurostd::substring2string(ael_file_content, "[AEL_STIFFNESS_TENSOR]START", "[AEL_STIFFNESS_TENSOR]STOP", 1));
        if (LDEBUG) {
        ...
        for (size_t i = 0; i < vline.size(); i++) {
          ...
          if (tokens.size() >= 6) {
            ...
            for (size_t j = 0; j < tokens.size(); j++) {
              elastic_const = aurostd::string2utype<double>(tokens[j]);
              elastic_const_row.push_back(elastic_const);
            }
            elastic_tensor.push_back(elastic_const_row);
          }
        }
        aflow_compliance_tensor.str(aurostd::substring2string(ael_file_content, "[AEL_COMPLIANCE_TENSOR]START", "[AEL_COMPLIANCE_TENSOR]STOP", 1));
        if (LDEBUG) {
          // aurostd::StringstreamClean(aus);
        ...
        for (size_t i = 0; i < vline.size(); i++) {
          ...
          if (tokens.size() >= 6) {
            ...
            for (size_t j = 0; j < tokens.size(); j++) {
              compliance_const = aurostd::string2utype<double>(tokens[j]);
...
// ***************************************************************************
//  void POccCalculator::getElasticProperties(vector<double>& Bvoigt, vector<double>& Breuss, vector<double>& Bvrh, vector<double>& Gvoigt, vector<double>& Greuss, vector<double>& Gvrh,vector<double>&
//  Poisson_ratio, vector<vector<vector<double> > >& elastic_tensor_list,  vector<vector<vector<double> > >& compliance_tensor_list) Retrieves elastic properties from aflow.ael.out files
// ***************************************************************************
namespace pocc {
  void POccCalculator::getElasticProperties(vector<double>& Bvoigt,
                                            vector<double>& Breuss,
                                            vector<double>& Bvrh,
                                            vector<double>& Gvoigt,
                                            vector<double>& Greuss,
    ...
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      ...
      if (aurostd::CompressFileExist(ael_filename, ael_filename)) {
        message << "Reading " << m_ARUN_directories[isupercell] + "/aflow.ael.out";
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_); // CO20200404
        aurostd::compressfile2string(ael_filename, ael_file_content);
        aflow_ael_out.str(aurostd::substring2string(ael_file_content, "[AEL_RESULTS]START", "[AEL_RESULTS]STOP", 1));
      } else {
...
// ***************************************************************************
//   void POccCalculator::getAverageElasticProperties(const vector<double>& v_temperatures, bool& ael_write_full_results, vector<double>& Bvoigt, vector<double>& Breuss, vector<double>& Bvrh, vector<double>&
//   Gvoigt, vector<double>& Greuss, vector<double>& Gvrh,vector<double>& Poisson_ratio, vector<vector<vector<double> > >& elastic_tensor_list,  vector<vector<vector<double> > >& compliance_tensor_list)
// Calculates thermal average of elastic properties for all POCC configurations
// Writes results to aflow.pocc_ael.out
// ***************************************************************************
namespace pocc {
  void POccCalculator::getAverageElasticProperties(const vector<double>& v_temperatures,
    ...
    double T = 0.0;
    vector<double> Yvoigt;
    vector<double> Yreuss;
    vector<double> Yvrh;
    vector<double> Hv_Teter_voigt;
    vector<double> Hv_Teter_reuss;
    vector<double> Hv_Teter_vrh;
    vector<double> Hv_Chen_voigt;
    vector<double> Hv_Chen_reuss;
    vector<double> Hv_Chen_vrh;
    vector<double> Hv_Tian_voigt;
    vector<double> Hv_Tian_reuss;
    vector<double> Hv_Tian_vrh;
    vector<double> Pugh_ratio;
    vector<double> Cauchy_pressure;
    vector<vector<double>> elastic_tensor_ave;
    vector<vector<double>> compliance_tensor_ave;
    vector<double> elastic_tensor_row;
    vector<double> compliance_tensor_row;
    double Bvoigt_ave = 0.0;
    double Breuss_ave = 0.0;
    double Bvrh_ave = 0.0;
    double Gvoigt_ave = 0.0;
    double Greuss_ave = 0.0;
    double Gvrh_ave = 0.0;
    double Hv_Teter_voigt_val = 0.0;
    double Hv_Teter_reuss_val = 0.0;
    double Hv_Teter_vrh_val = 0.0;
    double Hv_Chen_voigt_val = 0.0;
    double Hv_Chen_reuss_val = 0.0;
    double Hv_Chen_vrh_val = 0.0;
    double Hv_Tian_voigt_val = 0.0;
    double Hv_Tian_reuss_val = 0.0;
    double Hv_Tian_vrh_val = 0.0;
    double Hv_Teter_voigt_ave = 0.0;
    double Hv_Teter_reuss_ave = 0.0;
    double Hv_Teter_vrh_ave = 0.0;
    double Hv_Chen_voigt_ave = 0.0;
    double Hv_Chen_reuss_ave = 0.0;
    double Hv_Chen_vrh_ave = 0.0;
    double Hv_Tian_voigt_ave = 0.0;
    double Hv_Tian_reuss_ave = 0.0;
    double Hv_Tian_vrh_ave = 0.0;
    double Yvoigt_ave = 0.0;
    double Yreuss_ave = 0.0;
    double Yvrh_ave = 0.0;
    double Yvoigt_val = 0.0;
    double Yreuss_val = 0.0;
    double Yvrh_val = 0.0;
    double Bvoigt_max = 0.0;
    double Breuss_max = 0.0;
    double Bvrh_max = 0.0;
    double Gvoigt_max = 0.0;
    double Greuss_max = 0.0;
    double Gvrh_max = 0.0;
    double Yvoigt_max = 0.0;
    double Yreuss_max = 0.0;
    double Yvrh_max = 0.0;
    double Bvoigt_min = 0.0;
    double Breuss_min = 0.0;
    double Bvrh_min = 0.0;
    double Gvoigt_min = 0.0;
    double Greuss_min = 0.0;
    double Gvrh_min = 0.0;
    double Yvoigt_min = 0.0;
    double Yreuss_min = 0.0;
    double Yvrh_min = 0.0;
    double Hv_Teter_voigt_max = 0.0;
    double Hv_Teter_reuss_max = 0.0;
    double Hv_Teter_vrh_max = 0.0;
    double Hv_Chen_voigt_max = 0.0;
    double Hv_Chen_reuss_max = 0.0;
    double Hv_Chen_vrh_max = 0.0;
    double Hv_Tian_voigt_max = 0.0;
    double Hv_Tian_reuss_max = 0.0;
    double Hv_Tian_vrh_max = 0.0;
    double Hv_Teter_voigt_min = 0.0;
    double Hv_Teter_reuss_min = 0.0;
    double Hv_Teter_vrh_min = 0.0;
    double Hv_Chen_voigt_min = 0.0;
    double Hv_Chen_reuss_min = 0.0;
    double Hv_Chen_vrh_min = 0.0;
    double Hv_Tian_voigt_min = 0.0;
    double Hv_Tian_reuss_min = 0.0;
    double Hv_Tian_vrh_min = 0.0;
    double Poisson_ratio_ave = 0.0;
    double Poisson_ratio_min = 0.0;
    double Poisson_ratio_max = 0.0;
    double Pugh_val = 0.0;
    double Pugh_ratio_ave = 0.0;
    double Pugh_ratio_min = 0.0;
    double Pugh_ratio_max = 0.0;
    double Cauchy_ave = 0.0;
    double Cauchy_val = 0.0;
    double Cauchy_min = 0.0;
    double Cauchy_max = 0.0;
    const xmatrix<double> elastic_const(6, 6);
    const xmatrix<double> elastic_const_inv(6, 6);

    double kval = 0.0;
    double kvalsq = 0.0;
    stringstream ossmain;
    stringstream ossfull;

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " v_temperatures=" << aurostd::joinWDelimiter(aurostd::vecDouble2vecString(v_temperatures, 5), ",") << endl;
    }  // CO20200404
    ...
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      ...
      // Cauchy pressure
      Cauchy_val = ((elastic_tensor_list.at(isupercell).at(0).at(1) + elastic_tensor_list.at(isupercell).at(0).at(2) + elastic_tensor_list.at(isupercell).at(1).at(2)) / 3.0) -
                   ((elastic_tensor_list.at(isupercell).at(3).at(3) + elastic_tensor_list.at(isupercell).at(4).at(4) + elastic_tensor_list.at(isupercell).at(5).at(5)) / 3.0);
    ...
    for (size_t j = 0; j < elastic_tensor_list.at(0).size(); j++) {
      elastic_tensor_row.clear();
      for (size_t k = 0; k < elastic_tensor_list.at(0)[j].size(); k++) {
        elastic_tensor_row.push_back(0.0);
      }
      elastic_tensor_ave.push_back(elastic_tensor_row);
    }

    for (size_t j = 0; j < compliance_tensor_list.at(0).size(); j++) {
      compliance_tensor_row.clear();
      for (size_t k = 0; k < compliance_tensor_list.at(0)[j].size(); k++) {
        compliance_tensor_row.push_back(0.0);
      }
      compliance_tensor_ave.push_back(compliance_tensor_row);
    }

    for (size_t jt = 0; jt < v_temperatures.size(); jt++) {
      T = v_temperatures[jt];
      message << "Averaging elastic properties at T=" << T << "K";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_); // CO20200404 //CO20200502 - no getTemperatureString(T) needed here
      ...
      Hv_Tian_vrh_ave = 0.0;
      for (size_t j = 0; j < elastic_tensor_ave.size(); j++) {
        for (size_t k = 0; k < elastic_tensor_ave[j].size(); k++) {
          elastic_tensor_ave[j][k] = 0.0;
        }
      }

      for (size_t j = 0; j < compliance_tensor_ave.size(); j++) {
        for (size_t k = 0; k < compliance_tensor_ave[j].size(); k++) {
          compliance_tensor_ave[j][k] = 0.0;
        }
      }
      ...
      for (size_t j = 0; j < elastic_tensor_ave.size(); j++) {
        for (size_t k = 0; k < elastic_tensor_ave[j].size(); k++) {
          ossmain << setw(15) << setprecision(6) << (elastic_tensor_ave[j][k]);
        }
        ossmain << endl;
      }
      ...
      for (size_t j = 0; j < compliance_tensor_ave.size(); j++) {
        for (size_t k = 0; k < compliance_tensor_ave[j].size(); k++) {
          ossmain << setw(15) << setprecision(6) << (compliance_tensor_ave[j][k]);
        }
        ossmain << endl;
      }
      ...
      if (ael_write_full_results) {
        ...
        for (size_t j = 0; j < elastic_tensor_ave.size(); j++) {
          for (size_t k = 0; k < elastic_tensor_ave[j].size(); k++) {
            ossfull << setw(15) << setprecision(6) << (elastic_tensor_ave[j][k]);
          }
          ossfull << endl;
        }
        ...
        for (size_t j = 0; j < compliance_tensor_ave.size(); j++) {
          for (size_t k = 0; k < compliance_tensor_ave[j].size(); k++) {
            ossfull << setw(15) << setprecision(6) << (compliance_tensor_ave[j][k]);
          }
          ossfull << endl;
        }
    ...
    const string ofileafpoccaelname = m_aflags.Directory + "/aflow.pocc_ael.out";
    aurostd::stringstream2file(ossmain, ofileafpoccaelname, aurostd::compression_type::None, "WRITE");
    if (ael_write_full_results) {
      ossfull << AFLOWIN_SEPARATION_LINE << endl;
      ossfull << "[POCC_AEL_RESULTS]START_TEMPERATURE=" << (*this).getTemperatureString(T) << "_K" << endl;
      ...
      ossfull << "# Config" << "  " << aurostd::PaddedPRE("Bvoigt(GPa)", 2, " ") << "  " << aurostd::PaddedPRE("Breuss(GPa)", 2, " ") << "  " << aurostd::PaddedPRE("Bvrh(GPa)", 2, " ") << "  "
              << aurostd::PaddedPRE("Gvoigt(GPa)", 2, " ") << "  " << aurostd::PaddedPRE("Greuss(GPa)", 2, " ") << "  " << aurostd::PaddedPRE("Gvrh(GPa)", 2, " ") << "  "
              << aurostd::PaddedPRE("Evoigt(GPa)", 2, " ") << "  " << aurostd::PaddedPRE("Ereuss(GPa)", 2, " ") << "  " << aurostd::PaddedPRE("Evrh(GPa)", 2, " ") << "  "
              << aurostd::PaddedPRE("Poisson_ratio", 2, " ") << "  " << aurostd::PaddedPRE("Hv_Teter(GPa)", 2, " ") << "  " << aurostd::PaddedPRE("Hv_Chen(GPa)", 2, " ") << "  "
              << aurostd::PaddedPRE("Hv_Tian(GPa)", 2, " ") << "  " << aurostd::PaddedPRE("Pugh_modulus_ratio", 2, " ") << "  " << aurostd::PaddedPRE("Cauchy_pressure(GPa)", 2, " ") << endl;
      for (size_t i = 0; i < Bvoigt.size(); i++) {
        ossfull << setw(8) << fixed << i << "  " << setw(11) << setprecision(3) << Bvoigt[i] << "  " << setw(11) << setprecision(3) << Breuss.at(i) << "  " << setw(9) << setprecision(3) << Bvrh.at(i) << "  "
                << setw(11) << setprecision(3) << Gvoigt.at(i) << "  " << setw(11) << setprecision(3) << Greuss.at(i) << "  " << setw(9) << setprecision(3) << Gvrh.at(i) << "  " << setw(11) << setprecision(3)
                << Yvoigt.at(i) << "  " << setw(11) << setprecision(3) << Yreuss.at(i) << "  " << setw(9) << setprecision(3) << Yvrh.at(i) << "  " << setw(13) << setprecision(6) << Poisson_ratio.at(i) << "  "
                << setw(13) << setprecision(3) << Hv_Teter_vrh.at(i) << "  " << setw(12) << setprecision(3) << Hv_Chen_vrh.at(i) << "  " << setw(12) << setprecision(3) << Hv_Tian_vrh.at(i) << "  " << setw(18)
                << setprecision(6) << Pugh_ratio.at(i) << "  " << setw(20) << setprecision(3) << Cauchy_pressure.at(i) << endl;
      }
      ossfull << "[POCC_AEL_CONFIG_RESULTS]STOP_TEMPERATURE=" << (*this).getTemperatureString(T) << "_K" << endl;
      ossfull << AFLOWIN_SEPARATION_LINE << endl;
      const string ofileafpoccaelfullname = m_aflags.Directory + "/aflow.pocc_ael_full.out";
      aurostd::stringstream2file(ossfull, ofileafpoccaelfullname, aurostd::compression_type::None, "WRITE");
      ossfull.clear();
      ossfull.str(std::string());
    }
  }
...
namespace pocc {
  void POccCalculator::calculateDebyeThermalProperties(const vector<double>& v_temperatures) {
    bool agl_run_postprocess;
    bool agl_write_full_results;
    vector<double> Debye_temperature;
    vector<double> Debye_acoustic;
    vector<double> Gruneisen;
    vector<double> Cv300K;
    vector<double> Cp300K;
    vector<double> Fvib300K_atom;
    vector<double> Fvib300K_cell;
    vector<double> Svib300K_atom;
    vector<double> Svib300K_cell;
    vector<double> kappa300K;
    vector<vector<double>> agl_temperatures;
    vector<vector<double>> agl_gibbs_energies_atom;
    vector<vector<double>> agl_vibrational_energies_atom;
    setAGLOptions(agl_run_postprocess, agl_write_full_results);
    if (agl_run_postprocess) {
      generateDebyeThermalProperties(Debye_temperature, Debye_acoustic, Gruneisen, Cv300K, Cp300K, Fvib300K_atom, Fvib300K_cell, Svib300K_atom, Svib300K_cell, kappa300K, agl_temperatures,
                                     agl_gibbs_energies_atom, agl_vibrational_energies_atom);

    } else {
      getDebyeThermalProperties(Debye_temperature, Debye_acoustic, Gruneisen, Cv300K, Cp300K, Fvib300K_atom, Fvib300K_cell, Svib300K_atom, Svib300K_cell, kappa300K, agl_temperatures, agl_gibbs_energies_atom,
                                agl_vibrational_energies_atom);
    }
    getAverageDebyeThermalProperties(v_temperatures, agl_write_full_results, Debye_temperature, Debye_acoustic, Gruneisen, Cv300K, Cp300K, Fvib300K_atom, Fvib300K_cell, Svib300K_atom, Svib300K_cell, kappa300K,
                                     agl_temperatures, agl_gibbs_energies_atom, agl_vibrational_energies_atom);
  }
} // namespace pocc
...
// ***************************************************************************
//  void POccCalculator::generateDebyeThermalProperties(const uint& ntemperature, const uint& stemperature, const uint& npressure, const double& spressure, vector<double>& Debye_temperature, vector<double>&
//  Debye_acoustic, vector<double>& Gruneisen, vector<double>& Cv300K, vector<double>& Cp300K, vector<double>& Fvib300K_atom, vector<double>& Fvib300K_cell, vector<double>& Svib300K_atom, vector<double>&
//  Svib300K_cell, vector<double>& kappa300K, vector<vector<double> >& agl_temperatures, vector<vector<double> >& agl_gibbs_energies_atom, vector<vector<double> >& agl_vibrational_energies_atom) Runs AGL
//  postprocessing to generate thermal properties
// ***************************************************************************
namespace pocc {
  void POccCalculator::generateDebyeThermalProperties(vector<double>& Debye_temperature,
    ...
    double agl_debye_temperature = 0.0;
    double agl_debye_acoustic = 0.0;
    double agl_gruneisen = 0.0;
    double agl_heat_capacity_Cv_300K = 0.0;
    double agl_heat_capacity_Cp_300K = 0.0;
    double agl_vibrational_free_energy_300K_cell = 0.0;
    double agl_vibrational_free_energy_300K_atom = 0.0;
    double agl_vibrational_entropy_300K_cell = 0.0;
    double agl_vibrational_entropy_300K_atom = 0.0;
    ...
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      ...
      if (aurostd::CompressFileExist(agl_filename, agl_filename)) {
        aurostd::compressfile2string(pocc_directory_abs + "/" + "aflow.agl.out", agl_file_content);
        aflow_agl_out.str(aurostd::substring2string(agl_file_content, "[AGL_RESULTS]START", "[AGL_RESULTS]STOP", 1));
        if (LDEBUG) {
        ...
        for (size_t i = 0; i < vline.size(); i++) {
          ...
          if (tokens.size() >= 2) {
            if (tokens.at(0) == "agl_debye") {
              agl_debye_temperature = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "agl_acoustic_debye") {
              agl_debye_acoustic = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "agl_gruneisen") {
              agl_gruneisen = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "agl_heat_capacity_Cv_300K") {
              agl_heat_capacity_Cv_300K = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "agl_heat_capacity_Cp_300K") {
              agl_heat_capacity_Cp_300K = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "agl_vibrational_free_energy_300K_cell") {
              agl_vibrational_free_energy_300K_cell = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "agl_vibrational_free_energy_300K_atom") {
              agl_vibrational_free_energy_300K_atom = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "agl_vibrational_entropy_300K_cell") {
              agl_vibrational_entropy_300K_cell = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "agl_vibrational_entropy_300K_atom") {
              agl_vibrational_entropy_300K_atom = aurostd::string2utype<double>(tokens.at(1));
            }
            if (tokens.at(0) == "agl_thermal_conductivity_300K") {
              agl_thermal_conductivity_300K = aurostd::string2utype<double>(tokens.at(1));
            }
          }
        }
        aflow_agl_energies_out.str(aurostd::substring2string(agl_file_content, "[AGL_ENERGIES_TEMPERATURE]START", "[AGL_ENERGIES_TEMPERATURE]STOP", 1));
        if (LDEBUG) {
...
// ***************************************************************************
//  void POccCalculator::getDebyeThermalProperties(vector<double>& Debye_temperature, vector<double>& Debye_acoustic, vector<double>& Gruneisen, vector<double>& Cv300K, vector<double>& Cp300K, vector<double>&
//  Fvib300K_atom, vector<double>& Fvib300K_cell, vector<double>& Svib300K_atom, vector<double>& Svib300K_cell, vector<double>& kappa300K, vector<vector<double> >& agl_temperatures, vector<vector<double> >&
//  agl_gibbs_energies_atom, vector<vector<double> >& agl_vibrational_energies_atom) Retrieves thermal properties from aflow.agl.out files
// ***************************************************************************
namespace pocc {
  void POccCalculator::getDebyeThermalProperties(vector<double>& Debye_temperature,
                                                 vector<double>& Debye_acoustic,
                                                 vector<double>& Gruneisen,
                                                 vector<double>& Cv300K,
                                                 vector<double>& Cp300K,
    ...
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      ...
      agl_filename = pocc_directory_abs + "/aflow.agl.out";
      if (aurostd::CompressFileExist(agl_filename, agl_filename)) {
        message << "Reading " << m_ARUN_directories[isupercell] + "/aflow.agl.out";
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_); // CO20200404
        aurostd::compressfile2string(agl_filename, agl_file_content);
        aflow_agl_out.str(aurostd::substring2string(agl_file_content, "[AGL_RESULTS]START", "[AGL_RESULTS]STOP", 1));
      } else {
...
// ***************************************************************************
//   void POccCalculator::getAverageDebyeThermalProperties(const vector<double>& v_temperatures, bool& agl_write_full_results, vector<double>& Debye_temperature, vector<double>& Debye_acoustic, vector<double>&
//   Gruneisen, vector<double>& Cv300K, vector<double>& Cp300K, vector<double>& Fvib300K_atom, vector<double>& Fvib300K_cell, vector<double>& Svib300K_atom, vector<double>& Svib300K_cell, vector<double>&
//   kappa300K, vector<vector<double> >& agl_temperatures, vector<vector<double> >& agl_gibbs_energies_atom, vector<vector<double> >& agl_vibrational_energies_atom)
// Calculates thermal average of thermal properties for all POCC configurations
// Writes results to aflow.pocc_agl.out
// ***************************************************************************
namespace pocc {
  void POccCalculator::getAverageDebyeThermalProperties(const vector<double>& v_temperatures,
                                                        bool agl_write_full_results,
                                                        vector<double>& Debye_temperature,
                                                        vector<double>& Debye_acoustic,
                                                        vector<double>& Gruneisen,
    ...
    stringstream ossmain;
    stringstream ossfull;
    vector<double> agl_gibbs_energies_atom_ave;
    vector<double> agl_vibrational_energies_atom_ave;
    vector<double> agl_temperatures_ave;
    double Debye_temperature_ave = 0.0;
    double Debye_acoustic_ave = 0.0;
    double Gruneisen_ave = 0.0;
    double Cv300K_ave = 0.0;
    double Cp300K_ave = 0.0;
    double Fvib300K_atom_ave = 0.0;
    double Fvib300K_cell_ave = 0.0;
    double Svib300K_atom_ave = 0.0;
    double Svib300K_cell_ave = 0.0;
    double kappa300K_ave = 0.0;
    double Debyemin = 0.0;
    double Debyemax = 0.0;
    double gibbs_energy_atom_ave = 0.0;
    double vibrational_energy_atom_ave = 0.0;
    double gibbs_energy_atom_ave_temp = 0.0;
    double vibrational_energy_atom_ave_temp = 0.0;
    double tdiff = 0.0;
    double tdiffmin = 0.0;
    double tmaxlow = 0.0;
    uint jtp = 0;
    uint jtmaxlow = 0;
    uint imaxlow = 0;
    const string aelaflowin = "agl_aflow.in";
    ...
    for (size_t isupercell = 1; isupercell < agl_temperatures.size(); isupercell++) {
      if (agl_temperatures[isupercell].size() < jtmaxlow) {
        jtmaxlow = agl_temperatures[isupercell].size();
        imaxlow = isupercell;
        if (jtmaxlow > 0) {
          tmaxlow = agl_temperatures[isupercell].at(jtmaxlow - 1);
        } else {
          tmaxlow = 0.0;
        }
      }
    ...
    if (tmaxlow < 1.0) {
      message << "Maximum temperature = " << tmaxlow;
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_); // CO20200404
    }
    ...
    for (size_t jt = 0; jt < v_temperatures.size(); jt++) {
      T = v_temperatures[jt];
      message << "Averaging Debye thermal properties at T=" << T << "K";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_); // CO20200404 //CO20200502 - no (*this).getTemperatureString(T) needed here
      setPOccStructureProbabilities(T);
      ...
      for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
        ...
        jtp = 0;
        for (size_t j = 0; j < agl_temperatures.at(isupercell).size(); j++) {
          tdiff = fabs(agl_temperatures.at(isupercell)[j] - T);
          if (tdiff < tdiffmin) {
            jtp = j;
            tdiffmin = tdiff;
          }
      ...
      for (size_t j = 0; j < agl_temperatures_ave.size(); j++) {
        ossmain << setw(8) << setprecision(2) << fixed << agl_temperatures_ave[j] << "  " << setw(10) << setprecision(4) << agl_gibbs_energies_atom_ave.at(j) << "  " << setw(14) << setprecision(4)
                << agl_vibrational_energies_atom_ave.at(j) << endl;
      }
      ...
      if (agl_write_full_results) {
        ...
        for (size_t j = 0; j < agl_temperatures_ave.size(); j++) {
          ossfull << setw(8) << setprecision(2) << fixed << agl_temperatures_ave[j] << "  " << setw(10) << setprecision(4) << agl_gibbs_energies_atom_ave.at(j) << "  " << setw(14) << setprecision(4)
                  << agl_vibrational_energies_atom_ave.at(j) << endl;
        }
    ...
    const string ofileafpoccaglname = m_aflags.Directory + "/aflow.pocc_agl.out";
    aurostd::stringstream2file(ossmain, ofileafpoccaglname, aurostd::compression_type::None, "WRITE");
    ossmain.clear();
    ossmain.str(std::string());
    if (agl_write_full_results) {
      ossfull << AFLOWIN_SEPARATION_LINE << endl;
      ossfull << "[POCC_AGL_CONFIG_RESULTS]START_TEMPERATURE=" << (*this).getTemperatureString(T) << "_K" << endl;
      ossfull << "# Config" << "  " << aurostd::PaddedPRE("Debye_temperature(K)", 2, " ") << "  " << aurostd::PaddedPRE("Debye_acoustic(K)", 2, " ") << "  " << aurostd::PaddedPRE("Gruneisen_parameter", 2, " ")
              << "  " << aurostd::PaddedPRE("Cv(kB/cell)", 2, " ") << "  " << aurostd::PaddedPRE("Cp(kB/cell)", 2, " ") << "  " << aurostd::PaddedPRE("Fvib/cell(meV/cell)", 2, " ") << "  "
              << aurostd::PaddedPRE("Fvib/atom(meV/atom)", 2, " ") << "  " << aurostd::PaddedPRE("Svib/cell(meV/cell*K)", 2, " ") << "  " << aurostd::PaddedPRE("Svib/atom(meV/atom*K)", 2, " ") << "  "
              << aurostd::PaddedPRE("Thermal_conductivity(W/(m*K))", 2, " ") << endl;
      for (size_t i = 0; i < Debye_temperature.size(); i++) {
        ossfull << setw(8) << fixed << i << "  " << setw(20) << setprecision(3) << Debye_temperature[i] << "  " << setw(17) << setprecision(3) << Debye_acoustic.at(i) << "  " << setw(19) << setprecision(5)
                << Gruneisen.at(i) << "  " << setw(11) << setprecision(4) << Cv300K.at(i) << "  " << setw(11) << setprecision(4) << Cp300K.at(i) << "  " << setw(19) << setprecision(4) << Fvib300K_cell.at(i)
                << "  " << setw(19) << setprecision(4) << Fvib300K_atom.at(i) << "  " << setw(21) << setprecision(5) << Svib300K_cell.at(i) << "  " << setw(21) << setprecision(6) << Svib300K_atom.at(i) << "  "
                << setw(29) << setprecision(6) << kappa300K.at(i) << endl;
      }
      ossfull << "[POCC_AGL_CONFIG_RESULTS]STOP_TEMPERATURE=" << (*this).getTemperatureString(T) << "_K" << endl;
      ossfull << AFLOWIN_SEPARATION_LINE << endl;
      const string ofileafpoccaglenergname = m_aflags.Directory + "/aflow.pocc_agl_full.out";
      aurostd::stringstream2file(ossfull, ofileafpoccaglenergname, aurostd::compression_type::None, "WRITE");
      ossfull.clear();
      ossfull.str(std::string());
    }
  }
