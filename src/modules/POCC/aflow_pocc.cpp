...
namespace pocc {
  ...
  bool poccInput() {
    ...
    FileMESSAGE.open("aflow_pocc.log");
    // directory
    string directory;
...
namespace pocc {
  void poccOld2New(ostream& oss) {
    ofstream FileMESSAGE;
    return poccOld2New(FileMESSAGE, oss);
  }
  void poccOld2New(ofstream& FileMESSAGE, ostream& oss) {
    ...
    if (!kflags.KBIN_POCC) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Not a POCC run", _INPUT_MISSING_);
    }

    if (!vflags.KBIN_VASP_POSCAR_MODE.flag("EXPLICIT_START_STOP_POINT")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No POCC ordered structures found in " + _AFLOWIN_, _INPUT_MISSING_);
    }
    ...
    message << "Writing " << POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE;
    ...
    for (iline = 0; iline < vlines_orig.size(); iline++) {
      ...
      if (line.find(_VASP_POSCAR_MODE_EXPLICIT_STOP_P_) != string::npos) {
        reading_explicit = false;
      } else if (line.find(_VASP_POSCAR_MODE_EXPLICIT_START_P_) != string::npos) {
        reading_explicit = true;
      } else {
        if (reading_explicit == false) {
          vlines.push_back(line);
        }
      }
    }
    const string search = "[AFLOW] **********";  // in case AFLOWIN_SEPARATION_LINE changes in length
    for (iline = vlines.size() - 1; iline < vlines.size(); iline--) {
      if (vlines[iline].find(search) == string::npos) {
        break;
      } // separation line not found
      if ((iline - 1) < vlines.size() && vlines[iline - 1].find(search) != string::npos) {
        vlines.pop_back();
      } // double separation line found, remove last one
    }
    string AflowIn_file_ORIG = AflowIn_file;
    aurostd::StringSubstInPlace(AflowIn_file_ORIG, ".in", ".ORIG.in");
    aurostd::file2file(AflowIn_file, AflowIn_file_ORIG);
    message << "Saving " << AflowIn_file << " as " << AflowIn_file_ORIG;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    message << "Rewriting " << AflowIn_file;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_COMPLETE_);
    stringstream aflowin_ss;
    for (size_t i = 0; i < vlines.size(); i++) {
      aflowin_ss << vlines[i] << endl;
    }
...
namespace pocc {
  void POCC_Convolution(const aurostd::xoption& vpflow) {
    ...
    // address for lib2raw
    _aflags aflags;
    aflags.Directory = ".";  // make current directory for now, we can change in the future
...
namespace pocc {
  double poccDOSCAR2temperature(const string& doscar_path) {
    ...
    const string pocc_doscar_end = "K";
    const string::size_type loc_start = vtokens.back().find(pocc_doscar_start);
    const string::size_type loc_end = vtokens.back().find(pocc_doscar_end);
    ...
    const string temperature_str = vtokens.back().substr(loc_start + (pocc_doscar_start.size()), loc_end - (loc_start + (pocc_doscar_start.size())));
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " temperature_str=" << temperature_str << endl;
    }
...
namespace KBIN {
  void VASP_RunPOCC(const string& directory, ostream& oss) {
    ofstream FileMESSAGE;
    return VASP_RunPOCC(directory, FileMESSAGE, oss);
  }
  void VASP_RunPOCC(const string& directory, ofstream& FileMESSAGE, ostream& oss) {
    ...
    const _vflags vflags = KBIN::VASP_Get_Vflags_from_AflowIN(AflowIn, aflags, kflags);

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " kflags.KBIN_PHONONS_CALCULATION_AEL=" << kflags.KBIN_PHONONS_CALCULATION_AEL << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " kflags.KBIN_PHONONS_CALCULATION_AGL=" << kflags.KBIN_PHONONS_CALCULATION_AGL << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " kflags.KBIN_PHONONS_CALCULATION_APL=" << kflags.KBIN_PHONONS_CALCULATION_APL << endl;
    }
  ...
  void VASP_RunPOCC(const _xvasp& xvasp, const string& AflowIn, const _aflags& aflags, const _kflags& kflags, const _vflags& vflags, ofstream& FileMESSAGE, ostream& oss) {
    ...
    try {
      ...
      if (!pocc::structuresGenerated(aflags.Directory)) {
        ...
        // ME2021104
        if (kflags.KBIN_PHONONS_CALCULATION_AEL || kflags.KBIN_PHONONS_CALCULATION_AGL || kflags.KBIN_PHONONS_CALCULATION_APL) {
          const string message =
              "Cannot run AFLOW modules without fully completed POCC calculations."
...
namespace pocc {
  string POCC_MINIMUM_CONFIGURATION(const aurostd::xoption& vpflow) {
    return POCC_MINIMUM_CONFIGURATION(vpflow.getattachedscheme("POCC_MINIMUM_CONFIGURATION"));
  }
  // SG20240403 - fixed so that the files that are read are full paths with directory (not just basename)
  string POCC_MINIMUM_CONFIGURATION(const string& directory) {
    ...
    std::filesystem::path path_qmvasp;
    for (const std::filesystem::directory_entry& dir_entry : std::filesystem::directory_iterator(path)) {
      if (dir_entry.is_directory()) {
        if (aurostd::substring2bool(dir_entry.path().filename().string(), "ARUN.POCC")) {
          path_qmvasp = dir_entry.path() / DEFAULT_AFLOW_QMVASP_OUT;
          if (!(std::filesystem::is_regular_file(path_qmvasp))) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, dir_entry.path().filename().string() + " does not contain " + DEFAULT_AFLOW_QMVASP_OUT, _FILE_NOT_FOUND_);
          }
          const xQMVASP qmvasp = xQMVASP(path_qmvasp.filename().string());
          if (qmvasp.H_atom_relax == AUROSTD_NAN) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, path_qmvasp.filename().string() + " does not show a relaxation run", _FILE_ERROR_);
          }
          v_qmvasp.emplace_back(qmvasp);
        }
      }
    }
    const uint v_qmvasp_size = v_qmvasp.size(); // NO MORE PUSH_BACK BELOW! Will save time for AAPL calculations
    if (v_qmvasp_size < 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No ARUN.POCC found in Directory=" + directory, _FILE_NOT_FOUND_);
    }
    ...
    if (LDEBUG) {
      for (uint i = 0; i < v_qmvasp_size - 1; i++) {
        ...
        aurostd::StringSubstInPlace(qmvasp_filename, "/" + DEFAULT_AFLOW_QMVASP_OUT, "");
...
namespace pocc {
  ...
  xstructure extractPARTCAR(const string& AflowIn) {
    ...
    const xstructure xstr_pocc(ss_pocc_structure, IOVASP_POSCAR);
    return xstr_pocc;
  }
} // namespace pocc
...
namespace pocc {
  void parsePOccHashFromXStructureTitle(const string& title, string& pocc_hash) {
    string hnf_index_str;
    string site_config_index_str;
  ...
  void parsePOccHashFromXStructureTitle(const string& title, string& pocc_hash, string& hnf_index_str, string& site_config_index_str) {
    ...
    for (size_t t = 0; t < vtokens.size(); t++) { // find pocc hash directly
      if (aurostd::substring2bool(vtokens[t], "H") && aurostd::substring2bool(vtokens[t], "C")) {
        ...
        if (vtokens2.size() != 2) {
          pocc_hash = "";
          continue;
        }  // did not find pocc_hash, keep going
        // check hnf_index
        tmp_str = vtokens2[0]; // should contain an H
        if (!aurostd::substring2bool(tmp_str, "H")) {
          pocc_hash = "";
          continue;
        }  // did not find pocc_hash, keep going
        aurostd::StringSubstInPlace(tmp_str, "H", ""); // should now be hnf_index
        hnf_index_str = tmp_str;
        if (!aurostd::isfloat(hnf_index_str)) {
          pocc_hash = "";
          continue;
        }  // did not find pocc_hash, keep going
        // now check site_config_index
        site_config_index_str = vtokens2[1];
        if (!aurostd::isfloat(site_config_index_str)) {
          pocc_hash = "";
          continue;
        }  // did not find pocc_hash, keep going
        break;
    ...
    if (pocc_hash.empty()) {  // otherwise build pocc hash from components in the title
      ...
      for (size_t t = 0; t < vtokens.size(); t++) {
        if (aurostd::substring2bool(vtokens[t], "HNF(n=")) { // HNF(n=2,#2/3)
          ...
          aurostd::StringSubstInPlace(vtokens3[0], "#", "");
          if (!aurostd::isfloat(vtokens3[0])) {
            continue;
          }   // did not find pocc_hash, keep going
        ...
        if (aurostd::substring2bool(vtokens[t], "site_config(")) { // site_config(#1/2)
          ...
          aurostd::StringSubstInPlace(vtokens2[0], "site_config(", "");
          aurostd::StringSubstInPlace(vtokens2[0], "#", "");
          if (!aurostd::isfloat(vtokens2[0])) {
            continue;
          }   // did not find pocc_hash, keep going
  ...
  unsigned long long int getDGFromXStructureTitle(const string& title) {
    ...
    if (!aurostd::substring2bool(title, "DG=")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, R"("DG=" not found in title [title=")" + title + "\"]", _FILE_CORRUPT_);
    }
    ...
    for (size_t i = 0; i < vtokens.size(); i++) {
      if (aurostd::substring2bool(vtokens[i], "DG=")) {
        ...
        if (vtokens2.size() != 2) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot parse at DG= (vtokens2.size()==" + aurostd::utype2string(vtokens2.size()) + ")", _FILE_CORRUPT_);
        }
        if (!aurostd::isfloat(vtokens2[1])) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "degeneracy is not an integer", _VALUE_ILLEGAL_);
        }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " DG=" << dg << endl;
    }
    return dg;
  }
  void parsePropertyByTag(const string& line, const string& tag, double& prop) {
    ...
    if (vtokens.size() != 2) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown " + tag + " line format", _FILE_CORRUPT_);
    }
    if (!aurostd::isfloat(vtokens[1])) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, tag + " is not a double", _VALUE_ERROR_);
    }
    prop = aurostd::string2utype<double>(vtokens[1]);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " " << tag << "=" << prop << endl;
    }
  }
  bool patchStructuresAllFile(const _aflags& aflags, string& structures_file, stringstream& structures_file_ss, ofstream& FileMESSAGE, ostream& oss) {
    ...
    if (!found_file && aurostd::CompressFileExist(aflags.Directory + "/" + POCC_FILE_PREFIX + POCC_ALL_SUPERCELLS_FILE, structures_file)) {
      aurostd::compressfile2stringstream(structures_file, structures_file_ss);
      found_file = true;
    }
    if (!found_file && aurostd::FileExist(aflags.Directory + "/" + POCC_FILE_PREFIX + POCC_ALL_SUPERCELLS_FILE, structures_file)) {
      aurostd::file2stringstream(structures_file, structures_file_ss);
      found_file = true;
    }
    if (!found_file) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, aflags.Directory + "/" + POCC_FILE_PREFIX + POCC_ALL_SUPERCELLS_FILE + " does not exist", _FILE_NOT_FOUND_);
    }
    if (aurostd::substring2bool(structures_file_ss, POSCAR_series_START_tag)) {
      return false;
    }

    stringstream message;
    message << "Patching " << POCC_FILE_PREFIX + POCC_ALL_SUPERCELLS_FILE;
  ...
  bool patchStructuresAllFile(stringstream& structures_file_ss) {
    ...
    unsigned long long int i_pscs_psc_set = 0;

    const string POccStructure_title_tag = "HNF(n="; // in case we cannot find POSCAR_START_tag, SHACHAR OLD FORMAT
    bool add_POSCAR_START_tag = false;  // SHACHAR OLD FORMAT
    bool add_l_supercell_sets_size = false; // SHACHAR OLD FORMAT
    const string structures_file_str = structures_file_ss.str();
    ...
    if (found_tag_sg == false) {
      ...
      if (aurostd::substring2bool(structures_file_str, structure_group_tag)) {
        found_tag_sg = true;
      }
    }
    // STRUCTURE_GROUP
    ...
    // not found
    if (found_tag_sg == false) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot find structure group tag", _FILE_CORRUPT_);
    }
    ...
    if (!aurostd::substring2bool(structures_file_str, POSCAR_series_START_tag)) {
      ...
      aurostd::string2vectorstring(structures_file_str, vlines); // aurostd::compressfile2vectorstring(structures_file,vlines);
      for (size_t iline = 0; iline < vlines.size(); iline++) {  // see if you can get l_supercell_sets_size from first structure_group_tag line
        if (aurostd::substring2bool(vlines[iline], structure_group_tag)) {
          if (l_supercell_sets_size == 0 && aurostd::substring2bool(vlines[iline], "/")) {
            ...
            if (vtokens2.size() != 2) {
              break;
            }  // didn't work //throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"Unknown format for line containing l_supercell_sets_size",_FILE_CORRUPT_);
            if (!aurostd::isfloat(vtokens2.back())) {
              break;
            }  // didn't work //throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,vtokens2.back()+" is not a number",_FILE_CORRUPT_);
            l_supercell_sets_size = aurostd::string2utype<int>(vtokens2.back());
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " l_supercell_sets_size=" << l_supercell_sets_size << endl;
            }
            break;
          }
        }
      }
      const string poscar_start_tag = (add_POSCAR_START_tag ? POccStructure_title_tag : POSCAR_START_tag);
      const string poscar_stop_tag = (add_POSCAR_START_tag ? "[AFLOW] **" : POSCAR_STOP_tag);
      for (size_t iline = 0; iline < vlines.size(); iline++) {
        if (aurostd::substring2bool(vlines[iline], structure_group_tag)) {
          pscs_psc_set_sizes.push_back(0);
        }
        if (aurostd::substring2bool(vlines[iline], poscar_start_tag)) {
          pscs_psc_set_sizes.back()++;
        }
      }
      if (l_supercell_sets_size == 0) {
        add_l_supercell_sets_size = true;
        l_supercell_sets_size = pscs_psc_set_sizes.size();
      } // could not find from structure_group_tag
      if (l_supercell_sets_size == 0) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot find l_supercell_sets_size", _RUNTIME_INIT_);
      }
      if (pscs_psc_set_sizes.empty()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot find pscs_psc_set_sizes", _RUNTIME_INIT_);
      }
      if ((uint) l_supercell_sets_size != pscs_psc_set_sizes.size()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__,
                              "l_supercell_sets_size[=" + aurostd::utype2string((uint) l_supercell_sets_size) + "]!=pscs_psc_set_sizes.size()[=" + aurostd::utype2string(pscs_psc_set_sizes.size()) + "]", _INDEX_MISMATCH_);
      }
      for (size_t i = 0; i < pscs_psc_set_sizes.size(); i++) {
        if (pscs_psc_set_sizes[i] == 0) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "pscs_psc_set_sizes[i=" + aurostd::utype2string(i) + "]==0", _RUNTIME_INIT_);
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " pscs_psc_set_sizes[i=" << i << "]=" << pscs_psc_set_sizes[i] << endl;
        }
      }
      i_l_supercell_sets_size = -1;
      i_pscs_psc_set = 0;
      ...
      for (size_t iline = 0; iline < vlines.size(); iline++) {
        if (aurostd::substring2bool(vlines[iline], structure_group_tag)) { // reset
          ...
          if (add_l_supercell_sets_size) {
            aurostd::StringSubstInPlace(vlines[iline], POCC_AFLOWIN_tag + "STRUCTURE_GROUP", POCC_AFLOWIN_tag + "STRUCTURES_GROUP");
            aurostd::StringSubstInPlace(vlines[iline], POCC_AFLOWIN_tag + "STRUCTURE GROUP", POCC_AFLOWIN_tag + "STRUCTURES_GROUP");
            aurostd::StringSubstInPlace(vlines[iline], aurostd::RemoveWhiteSpacesFromTheBack(vlines[iline]), aurostd::RemoveWhiteSpacesFromTheBack(vlines[iline]) + "/" + aurostd::utype2string(l_supercell_sets_size));
          }
        }
        if (found_POccStructure == true && aurostd::substring2bool(vlines[iline], poscar_stop_tag)) {
          if (pocc_addendum_ss.str().empty()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "STOP tag found before START", _FILE_CORRUPT_);
          }
          if (aurostd::substring2bool(vlines[iline], POSCAR_STOP_tag)) {
            aurostd::StringSubstInPlace(vlines[iline], POSCAR_STOP_tag, POSCAR_series_STOP_tag + pocc_addendum_ss.str());
          } else {
            vlines.insert(vlines.begin() + iline++, POSCAR_series_STOP_tag + pocc_addendum_ss.str());
          }
        ...
        if (aurostd::substring2bool(vlines[iline], poscar_start_tag)) {
          ...
          if (i_l_supercell_sets_size >= l_supercell_sets_size) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Found more unique supercells than expected", _INDEX_MISMATCH_);
          }
          if (aurostd::substring2bool(vlines[iline], POSCAR_START_tag)) {
            if (iline + 1 > vlines.size() - 1) {
              throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot grab title at iline=" + aurostd::utype2string(iline), _FILE_CORRUPT_);
            } // see if title is accessible
            parsePOccHashFromXStructureTitle(vlines[iline + 1], pocc_hash);
          } else {
            parsePOccHashFromXStructureTitle(vlines[iline], pocc_hash);
          } // SHACHAR OLD FORMAT
          if (pocc_hash.empty()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Could not find pocc_hash at iline=" + aurostd::utype2string(iline + 1), _FILE_CORRUPT_);
          }
          ...
          if ((uint) i_l_supercell_sets_size > pscs_psc_set_sizes.size() - 1) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "i_l_supercell_sets_size>pscs_psc_set_sizes.size()-1", _INDEX_MISMATCH_);
          }
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " pocc_addendum=" << pocc_addendum_ss.str() << endl;
          }
          // everything checks out, patch the line
          if (aurostd::substring2bool(vlines[iline], POSCAR_START_tag)) {
            aurostd::StringSubstInPlace(vlines[iline], POSCAR_START_tag, POSCAR_series_START_tag + pocc_addendum_ss.str());
          } else {
            vlines.insert(vlines.begin() + iline++, POSCAR_series_START_tag + pocc_addendum_ss.str());
          } //++ so we don't find the same line
          if (!aurostd::substring2bool(vlines[iline], pocc_hash)) {
            aurostd::StringSubstInPlace(vlines[iline], POccStructure_title_tag, pocc_hash + " " + POccStructure_title_tag);
          } // SHACHAR OLD FORMAT
          if (!aurostd::substring2bool(vlines[iline], DOI_POCC)) {
            vlines[iline] = aurostd::RemoveWhiteSpacesFromTheBack(vlines[iline]) + DOI_POCC;
          }  // SHACHAR OLD FORMAT
        }
        aurostd::StringSubstInPlace(vlines[iline], "#", ""); // NO continue
      }
      // stringstream structures_file_ss_new;
      structures_file_ss.str("");
      for (size_t iline = 0; iline < vlines.size(); iline++) {
        structures_file_ss << vlines[iline] << endl;
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " patched file=" << endl;
        cerr << structures_file_ss.str() << endl;
      }
      return true;
    }
    return false;
  ...
  bool patchStructuresUniqueFile(const _aflags& aflags, string& structures_file, stringstream& structures_file_ss, ofstream& FileMESSAGE, ostream& oss) {
    bool found_file = false;
    if (!found_file && aurostd::CompressFileExist(aflags.Directory + "/" + POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE, structures_file)) {
      aurostd::compressfile2stringstream(structures_file, structures_file_ss);
      found_file = true;
    }
    if (!found_file && aurostd::FileExist(aflags.Directory + "/" + POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE, structures_file)) {
      aurostd::file2stringstream(structures_file, structures_file_ss);
      found_file = true;
    }
    if (!found_file) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, aflags.Directory + "/" + POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE + " does not exist", _FILE_NOT_FOUND_);
    }
    if (aurostd::substring2bool(structures_file_ss, POSCAR_series_START_tag)) {
      return false;
    }
    ...
    stringstream message;
    message << "Patching " << POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_WARNING_);
  ...
  bool patchStructuresUniqueFile(stringstream& structures_file_ss) {
    ...
    if (!aurostd::substring2bool(structures_file_str, POSCAR_series_START_tag)) {
      ...
      aurostd::string2vectorstring(structures_file_str, vlines); // aurostd::compressfile2vectorstring(structures_file,vlines);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " looking for l_supercell_sets_size" << endl;
      }
      for (size_t iline = 0; iline < vlines.size(); iline++) {
        if (aurostd::substring2bool(vlines[iline], POSCAR_START_tag)) {
          l_supercell_sets_size++;
        }
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " l_supercell_sets_size=" << l_supercell_sets_size << endl;
      ...
      for (size_t iline = 0; iline < vlines.size(); iline++) {
        ...
        if (aurostd::substring2bool(vlines[iline], POSCAR_START_tag)) {
          ...
          // everything checks out, patch the line
          aurostd::StringSubstInPlace(vlines[iline], POSCAR_START_tag, POSCAR_series_START_tag + pocc_addendum_ss.str());
          if (!aurostd::substring2bool(vlines[iline + 1], pocc_hash)) {
            aurostd::StringSubstInPlace(vlines[iline + 1], POccStructure_title_tag, pocc_hash + " " + POccStructure_title_tag);
          } // SHACHAR OLD FORMAT  //iline+1 for title line
          if (!aurostd::substring2bool(vlines[iline + 1], DOI_POCC)) {
            vlines[iline + 1] = aurostd::RemoveWhiteSpacesFromTheBack(vlines[iline + 1]) + DOI_POCC;
          }  // SHACHAR OLD FORMAT  //iline+1 for title line
        }
        aurostd::StringSubstInPlace(vlines[iline], "#", ""); // NO continue
      }
  ...
  void patchStructuresFile(const _aflags& aflags, ofstream& FileMESSAGE, ostream& oss) {
    ...
    if (patchStructuresAllFile(aflags, structures_file, structures_file_ss, FileMESSAGE, oss)) {
      message << "Writing out patched " << POCC_FILE_PREFIX + POCC_ALL_SUPERCELLS_FILE;
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
      string structures_file_new = structures_file;
      aurostd::StringSubstInPlace(structures_file_new, POCC_FILE_PREFIX + POCC_ALL_SUPERCELLS_FILE, POCC_FILE_PREFIX + POCC_ALL_SUPERCELLS_FILE + ".OLD");
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " structures_file=" << structures_file << endl;
        cerr << __AFLOW_FUNC__ << " structures_file_new=" << structures_file_new << endl;
      }
      if (!aurostd::file2file(structures_file, structures_file_new)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Could not move structures_file to structures_file_new=" + structures_file_new, _RUNTIME_ERROR_);
      }
      aurostd::stringstream2file(structures_file_ss, aflags.Directory + "/" + POCC_FILE_PREFIX + POCC_ALL_SUPERCELLS_FILE);
      structures_file_ss.str("");
    }

    // POCC_UNIQUE_SUPERCELLS_FILE
    if (patchStructuresUniqueFile(aflags, structures_file, structures_file_ss, FileMESSAGE, oss)) {
      message << "Writing out patched " << POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE;
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
      string structures_file_new = structures_file;
      aurostd::StringSubstInPlace(structures_file_new, POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE, POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE + ".OLD");
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " structures_file=" << structures_file << endl;
        cerr << __AFLOW_FUNC__ << " structures_file_new=" << structures_file_new << endl;
      }
      ...
      aurostd::stringstream2file(structures_file_ss, aflags.Directory + "/" + POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE);
      structures_file_ss.str("");
    }

    if (!(aurostd::CompressFileExist(aflags.Directory + "/PARTCAR") || aurostd::FileExist(aflags.Directory + "/PARTCAR"))) { // patch: write out PARTCAR, should really only happen during generateStructures()
      message << "Writing out PARTCAR";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
      string AflowIn_file;
      string AflowIn;
      KBIN::getAflowInFromAFlags(aflags, AflowIn_file, AflowIn, FileMESSAGE, oss);
...
namespace pocc {
  vector<string> getElementsList() {
    ...
    for (uint i = 0; i < skip_every + 1; i++) {
      for (size_t j = i; j < VSTD_ELEMENTS_LIST.size(); j += skip_every + 1) {
        elements.push_back(VSTD_ELEMENTS_LIST[j]);
      }
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " elements_list=" << aurostd::joinWDelimiter(elements, ",") << endl;
    }
    return elements;
  }
} // namespace pocc
...
namespace pocc {
  bool sortPSCSetsUFFEnergy(const POccSuperCellSet& a, const POccSuperCellSet& b) {
    return a.getUFFEnergy() < b.getUFFEnergy();
  }
} // namespace pocc
...
namespace pocc {
  double getHmix(const xvector<double>& xv_energies, const xvector<double>& xv_dgs) {
    double dg_total;
    return getHmix(xv_energies, xv_dgs, dg_total);
  }
  double getHmix(const xvector<double>& xv_energies, const xvector<double>& xv_dgs, double& dg_total) {
    return aurostd::meanWeighted(xv_energies, xv_dgs, dg_total);
  }
  double getEFA(const xvector<double>& xv_energies, const xvector<double>& xv_dgs) {
    const bool LDEBUG = (false || _DEBUG_POCC_ || XHOST.DEBUG);
    double dg_total = 0.0;
    const double Hmix = getHmix(xv_energies, xv_dgs, dg_total);
    double sigma = 0.0;
    for (int i = xv_dgs.lrows; i <= xv_dgs.urows; i++) {
      sigma += xv_dgs[i] * pow(xv_energies[i] - Hmix, 2.0);
    }
    sigma /= (dg_total - 1);
    sigma = sqrt(sigma);
    if (LDEBUG) {
    ...
    const double efa = 1.0 / sigma;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " efa=" << efa << endl;
    }
    return efa;
  }
  ...
  void POccCalculator::StructuresAllFile2SupercellSets() {
    ...
    string arun_directory;
    string pocc_directory_abs;
    const xQMVASP qmvasp;
    m_ARUN_directories.clear();
    for (size_t iline = 0; iline < vlines.size(); iline++) {
      if (aurostd::substring2bool(vlines[iline], POCC_AFLOWIN_tag + "STRUCTURES_GROUP")) {
        l_supercell_sets.emplace_back();
        continue;
      }
      if (aurostd::substring2bool(vlines[iline], POCC_AFLOWIN_tag + "UFF_ENERGY=")) {
        aurostd::string2tokens(vlines[iline], vtokens, "=");
        if (vtokens.size() != 2) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown UFF_ENERGY line format", _FILE_CORRUPT_);
        }
        if (!aurostd::isfloat(vtokens[1])) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "UFF_ENERGY is not a double", _VALUE_ERROR_);
        }
        l_supercell_sets.back().m_psc_set.emplace_back(); // this comes before structure
      ...
      if (aurostd::substring2bool(vlines[iline], POSCAR_POCC_series_START_tag)) {
        ...
        if (vtokens.size() <= 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown START.POCC_ (H1) line", _FILE_CORRUPT_);
        }
        aurostd::string2tokens(vtokens.back(), vtokens2, "C");
        if (vtokens.size() <= 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown START.POCC_ (H2) line", _FILE_CORRUPT_);
        }
        if (!aurostd::isfloat(vtokens2[0])) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "hnf_index is not an integer", _VALUE_ILLEGAL_);
        }
        ...
        if (vtokens.size() <= 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown START.POCC_ (C1) line", _FILE_CORRUPT_);
        }
        if (!aurostd::isfloat(aurostd::RemoveWhiteSpaces(vtokens.back()))) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "site_config_index is not an integer", _VALUE_ERROR_);
        }
        ...
        if (!aurostd::substring2bool(vlines[iline + 1], "DG=")) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "DG= is not present in POSCAR title line", _FILE_CORRUPT_);
        }
        ...
        aurostd::StringSubstInPlace(arun_directory, POSCAR_series_START_tag, "");
        ...
        if (!aurostd::isfloat(vtokens[2])) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot determine POcc structure group index", _VALUE_ILLEGAL_);
        }
        if (aurostd::string2utype<int>(vtokens[2]) == 1) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " pocc_directory_abs=" << pocc_directory_abs << endl;
          }
          if (!aurostd::IsDirectory(pocc_directory_abs)) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "POcc directory [dir=" + pocc_directory_abs + "] not found", _FILE_NOT_FOUND_);
          }
  ...
  void POccCalculator::StructuresUniqueFile2SupercellSets() {
    ...
    for (size_t iline = 0; iline < vlines.size(); iline++) {
      if (aurostd::substring2bool(vlines[iline], POSCAR_POCC_series_START_tag)) {
        l_supercell_sets.emplace_back();
        l_supercell_sets.back().m_psc_set.emplace_back();
        // hnf_matrix
        aurostd::string2tokens(vlines[iline], vtokens, "H");
        if (vtokens.size() <= 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown START.POCC_ (H1) line", _FILE_CORRUPT_);
        ...
        if (!aurostd::isfloat(aurostd::RemoveWhiteSpaces(vtokens.back()))) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "site_config_index is not an integer", _VALUE_ILLEGAL_);
        }
        l_supercell_sets.back().m_psc_set.back().m_site_config_index = aurostd::string2utype<unsigned long long int>(aurostd::RemoveWhiteSpaces(vtokens.back()));
        // degeneracy
        l_supercell_sets.back().m_psc_set.back().m_degeneracy = 1;
        ...
        // directory
        arun_directory = vlines[iline];
        aurostd::StringSubstInPlace(arun_directory, POSCAR_series_START_tag, ""); // POCC_01_H0C0
        ...
        if (!aurostd::IsDirectory(pocc_directory_abs)) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "POcc directory [dir=" + pocc_directory_abs + "] not found", _FILE_CORRUPT_);
        }
        // m_ARUN_directories.push_back(pocc_directory_abs); //do not save full path
        // debug
        if (LDEBUG) {
  ...
  void POccCalculator::setDFTEnergies() {
    ...
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " look at pocc_directory_abs=" << pocc_directory_abs << endl;
      }
      // energy_dft
      qmvasp_filename = pocc_directory_abs + "/" + DEFAULT_AFLOW_QMVASP_OUT;
      if (!aurostd::CompressFileExist(qmvasp_filename, qmvasp_filename)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No qmvasp file found [dir=" + pocc_directory_abs + "]", _FILE_NOT_FOUND_);
      }
      ...
      if ((*it).m_energy_dft == AUROSTD_NAN) {
        (*it).m_energy_dft = qmvasp.H_atom_relax;
      }
      if ((*it).m_energy_dft == AUROSTD_NAN) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No H_atom found in qmvasp [dir=" + pocc_directory_abs + "]", _FILE_CORRUPT_);
      }
      // energy_dft_ground
      ...
      if (LDEBUG) {
        cerr << " H_atom[isupercell=" << isupercell << "]=" << (*it).m_energy_dft << endl;
      }
    }
    if (LDEBUG) {
      cerr << " H_atom_ground=" << m_energy_dft_ground << endl;
    }
  ...
  void POccCalculator::setPOccStructureProbabilities(double temperature) {
    ...
    if (m_energy_dft_ground == AUROSTD_MAX_DOUBLE) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_energy_dft_ground not set", _RUNTIME_ERROR_);
    }

    if (std::signbit(temperature)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Negative temperature found", _VALUE_ILLEGAL_);
    }
    if (aurostd::isequal(temperature, 0.0)) {
      temperature = _ZERO_TOL_; // 1e-6;
      message << "Converting T=0 -> T=" << temperature << " for Boltzmann probabilities";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_); // WARNING
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " denom=" << denom << endl;
    }
    ...
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      isupercell = std::distance(l_supercell_sets.begin(), it);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " degeneracy[isupercell=" << isupercell << "]=" << (*it).getDegeneracy() << endl;
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " m_energy_dft[isupercell=" << isupercell << "]=" << (*it).m_energy_dft << endl;
      }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " prob[isupercell=" << isupercell << "]=" << (*it).m_probability << endl;
      }
    }
    if (!aurostd::isequal(prob_total, 1.0)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "prob_total != 1.0", _RUNTIME_ERROR_);
    }
  }
  ...
  void getTemperatureStringParameters(int& temperature_precision, bool& temperatures_int, int& zero_padding_temperature) {
    const vector<double> v_temperatures = getVTemperatures(DEFAULT_POCC_TEMPERATURE_STRING);
  ...
  void getTemperatureStringParameters(const vector<double>& v_temperatures, int& temperature_precision, bool& temperatures_int, int& zero_padding_temperature) {
    ...
    if (!v_temperatures.empty()) {
      ...
      for (size_t itemp = 0; itemp < v_temperatures.size(); itemp++) {
        if (!aurostd::isinteger(v_temperatures[itemp])) {
          temperatures_int = false;
          break;
        }
  ...
  string getTemperatureString(double temperature, int precision, bool temperatures_int, int zero_padding_temperature) {
    const bool LDEBUG = (false || _DEBUG_POCC_ || XHOST.DEBUG);
    stringstream t_ss;
    if (temperatures_int == false) {
      t_ss.setf(std::ios::fixed, std::ios::floatfield);
      t_ss.precision(precision);
    }
    t_ss.width(zero_padding_temperature);
    t_ss.fill('0');
    t_ss << temperature;
  ...
  void POccCalculator::setAvgRDF(double temperature) {
    const bool LDEBUG = (true || _DEBUG_POCC_ || XHOST.DEBUG);
    stringstream message;
    ...
    uint i = 0;
    const bool raw_counts = false; // make options for these later
    const double sigma = 0; // make options for these later
    const int window_gaussian = 0; // make options for these later
    ...
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end() && perform_rdf == true; ++it) {
      ...
      message << "Processing CONTCAR.relax of " << m_ARUN_directories[isupercell] << " for the RDF (takes some time)";
      ...
      if (xstr.species.empty() || (!xstr.species.empty() && xstr.species[0].empty())) {
        xstr.SetSpecies(aurostd::vector2deque(_vspecies));
      }
      if (isupercell == 0) {
        vspecies.clear();
        for (i = 0; i < _vspecies.size(); i++) {
          vspecies.push_back(_vspecies[i]);
        }
        num_each_type.clear();
        for (i = 0; i < xstr.num_each_type.size(); i++) {
          num_each_type.push_back(xstr.num_each_type[i]);
        }
        //
        ...
        if (perform_rdf && (vspecies != _vspecies)) {
          message << "Found a mismatch in vspecies in " << m_ARUN_directories[isupercell] << ", not performing pdos averaging";
          pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
          perform_rdf = false;
        }
        if (perform_rdf && (num_each_type != xstr.num_each_type)) {
          message << "Found a mismatch in num_each_type in " << m_ARUN_directories[isupercell] << ", not performing pdos averaging";
          pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
          perform_rdf = false;
        }
        //
    ...
    if (perform_rdf == false) {
      m_rdf_all.clear();
      return;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_rdf_all=" << endl << m_rdf_all << endl;
    }
  }
  ...
  void POccCalculator::setAvgDOSCAR(double temperature) {
    ...
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      ...
      if (!aurostd::CompressFileExist(getARUNDirectoryPath(isupercell) + "/POSCAR.static", POSCAR_file)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "POSCAR.static not found in " + m_ARUN_directories[isupercell], _FILE_NOT_FOUND_);
      }
      xstr.clear();
      xstr.initialize(POSCAR_file, IOVASP_POSCAR);
      if (LDEBUG) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vnatoms=" << aurostd::joinWDelimiter(vnatoms, ",") << endl;
    }
    ...
    bool insulator_found = false;
    uint ispin = 0;
    uint iatom = 0;
    uint iorbital = 0;
    uint vDOS_avg_size = 0;
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      isupercell = std::distance(l_supercell_sets.begin(), it);
      if (!aurostd::CompressFileExist(getARUNDirectoryPath(isupercell) + "/DOSCAR.static", DOSCAR_file)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "DOSCAR.static not found in " + m_ARUN_directories[isupercell], _FILE_NOT_FOUND_);
      }
      message << "Processing xDOSCAR of " << m_ARUN_directories[isupercell];
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
      xdoscar.GetPropertiesFile(DOSCAR_file);
      message << "xDOSCAR read";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
      xdoscar.GetBandGap();
      for (ispin = 0; ispin < xdoscar.Egap.size(); ispin++) {
        message << "ISPIN=" << ispin << ", Egap_DOS=" << xdoscar.Egap[ispin];
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
      }
      if (isupercell == 0) { // set all to 0
        ...
        for (ispin = 0; ispin < m_xdoscar.viDOS.size(); ispin++) {
          std::fill(m_xdoscar.viDOS[ispin].begin(), m_xdoscar.viDOS[ispin].end(), 0.0);
        }

        if (m_xdoscar.vDOS.empty()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.vDOS.size()==0", _INDEX_MISMATCH_);
        }
        if (perform_pdos_avg) {
          vDOS_avg_size = m_xdoscar.vDOS.size();
        } else {
        ...
        if (m_xdoscar.energy_max != xdoscar.energy_max) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.energy_max!=xdoscar.energy_max", _INDEX_MISMATCH_);
        }
        if (m_xdoscar.energy_min != xdoscar.energy_min) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.energy_min!=xdoscar.energy_min", _INDEX_MISMATCH_);
        }
        if (m_xdoscar.number_energies != xdoscar.number_energies) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.number_energies!=xdoscar.number_energies", _INDEX_MISMATCH_);
        }

        if (m_xdoscar.venergy.size() != xdoscar.venergy.size()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.venergy.size()!=xdoscar.venergy.size()", _INDEX_MISMATCH_);
        }
        if (m_xdoscar.venergyEf.size() != xdoscar.venergyEf.size()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.venergyEf.size()!=xdoscar.venergyEf.size()", _INDEX_MISMATCH_);
        }
        if (m_xdoscar.viDOS.size() != xdoscar.viDOS.size()) {
          message << "Mismatch SPIN-ON/SPIN-OFF settings, attempting to rectify";
          ...
          } else if (xdoscar.viDOS.size() == 1) {
            xdoscar.convertSpinOFF2ON();
          }
          if (m_xdoscar.viDOS.size() != xdoscar.viDOS.size()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__,
                                  "m_xdoscar.viDOS.size()!=xdoscar.viDOS.size() [" + aurostd::utype2string(m_xdoscar.viDOS.size()) + "!=" + aurostd::utype2string(xdoscar.viDOS.size()) + "]", _INDEX_MISMATCH_);
          }
        }
        for (ispin = 0; ispin < m_xdoscar.viDOS.size(); ispin++) {
          if (m_xdoscar.viDOS[ispin].size() != xdoscar.viDOS[ispin].size()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.viDOS[ispin].size()!=xdoscar.viDOS[ispin].size()", _INDEX_MISMATCH_);
          }
        }
        if (perform_pdos_avg) {
          if (m_xdoscar.vDOS.size() != xdoscar.vDOS.size()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.vDOS.size()!=xdoscar.vDOS.size()", _INDEX_MISMATCH_);
          }
        } else {
          if (xdoscar.vDOS.empty()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "xdoscar.vDOS.size()==0", _INDEX_MISMATCH_);
          }
        }
        for (iatom = 0; iatom < vDOS_avg_size; iatom++) {
          if (m_xdoscar.vDOS[iatom].size() != xdoscar.vDOS[iatom].size()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.vDOS[iatom].size()!=xdoscar.vDOS[iatom].size()", _INDEX_MISMATCH_);
          }
          for (iorbital = 0; iorbital < m_xdoscar.vDOS[iatom].size(); iorbital++) {
            if (m_xdoscar.vDOS[iatom][iorbital].size() != xdoscar.vDOS[iatom][iorbital].size()) {
              throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.vDOS[iatom][iorbital].size()!=xdoscar.vDOS[iatom][iorbital].size()", _INDEX_MISMATCH_);
            }
            for (ispin = 0; ispin < m_xdoscar.vDOS[iatom][iorbital].size(); ispin++) {
              if (m_xdoscar.vDOS[iatom][iorbital][ispin].size() != xdoscar.vDOS[iatom][iorbital][ispin].size()) {
                throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_xdoscar.vDOS[iatom][iorbital][ispin].size()!=xdoscar.vDOS[iatom][iorbital][ispin].size()", _INDEX_MISMATCH_);
              }
            }
          }
        }
        if (m_Egap_DOS.size() != xdoscar.Egap.size()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_Egap_DOS.size()!=xdoscar.Egap.size()", _INDEX_MISMATCH_);
        }
      }
      ...
      uint ienergy = 0;
      for (ienergy = 0; ienergy < m_xdoscar.venergyEf.size(); ienergy++) {
        m_xdoscar.venergyEf[ienergy] += ((*it).m_probability * xdoscar.venergyEf[ienergy]);
      } // NOT the same across all ARUNS
      for (ispin = 0; ispin < m_xdoscar.viDOS.size(); ispin++) {
      ...
      if (!perform_pdos_avg) {
        for (iatom = 1; iatom < xdoscar.vDOS.size(); iatom++) { // iatom==1 starts first atom, sum from here onward
          while (xdoscar.vDOS[iatom].size() > m_xdoscar.vDOS[iatom].size()) {
            m_xdoscar.addOrbitalChannel();
          }
      ...
      for (ispin = 0; ispin < m_Egap_DOS.size(); ispin++) {
        ...
        if (xdoscar.Egap[ispin] == _METALGAP_) {
          metal_found = true;
        } // just be safe, check spin-polarized
        else {
          insulator_found = true;
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " xdoscar.Egap[ispin=" << ispin << "]=" << xdoscar.Egap[ispin] << endl;
        }
        m_Egap_DOS[ispin] += ((*it).m_probability * xdoscar.Egap[ispin]);
      }
      if (xdoscar.Egap_net == _METALGAP_) {
        metal_found = true;
      } // just be safe, check _net
      else {
        insulator_found = true;
      }
    ...
    if (metal_found && insulator_found) {
      message << "Mixed metal and insulator states found, averaging to a metallic gap";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
    }
    if (metal_found) {
      for (ispin = 0; ispin < m_Egap_DOS.size(); ispin++) {
        m_Egap_DOS[ispin] = _METALGAP_;
      }
      m_Egap_DOS_net = _METALGAP_;
    }

    // set some attributes
    if (m_vflags.AFLOW_SYSTEM.isentry && !m_vflags.AFLOW_SYSTEM.content_string.empty()) {
      m_xdoscar.title = m_vflags.AFLOW_SYSTEM.content_string;
    }
    ...
    for (ispin = 0; ispin < m_xdoscar.Egap.size(); ispin++) {
      message << "ISPIN=" << ispin << ", Egap_DOS=" << m_xdoscar.Egap[ispin];
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    }
    message << "Egap_DOS_net=" << xdoscar.Egap_net;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);

    // from POccCalculator
    message << "Average of Egap_DOSs";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    for (ispin = 0; ispin < xdoscar.Egap.size(); ispin++) {
      message << "ISPIN=" << ispin << ", Egap_DOS=" << m_Egap_DOS[ispin];
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    }
    message << "Egap_DOS_net=" << m_Egap_DOS_net;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    ...
    for (int i = 1; i <= 3; i++) {
      m_xdoscar.lattice[i] *= 1e-10;
    } // convert Angstroms to meters

    // XDOSCAR.OUT
    // string xdoscar_filename=POCC_DOSCAR_FILE+"_T"+aurostd::utype2string(temperature,TEMPERATURE_PRECISION)+"K";
    const string xdoscar_filename = POCC_DOSCAR_FILE + "_T" + (*this).getTemperatureString(temperature) + "K";
    message << "Writing out " << xdoscar_filename;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
  ...
  void POccCalculator::calculateRELAXProperties(double temperature) {
    ...
    for (i = 1; i <= max; i++) { // i=max;i<=max;i--
      if (aurostd::CompressFileExist(getARUNDirectoryPath(0) + "/OUTCAR.relax" + aurostd::utype2string(i))) {
        m_relaxation_max = i;
      } else {
        break;
      }
    }
    if (m_relaxation_max == AUROSTD_MAX_UINT) {
      return;
    } // no runs completed

    const string OUTCAR_relax = "OUTCAR.relax" + aurostd::utype2string(m_relaxation_max);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " OUTCAR_relax=" << OUTCAR_relax << endl;
    }
    ...
    for (unsigned long long int isupercell = 0; isupercell < m_ARUN_directories.size() && found_all_OUTCARs; isupercell++) {
      if (!aurostd::CompressFileExist(getARUNDirectoryPath(isupercell) + "/" + OUTCAR_relax)) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " " << OUTCAR_relax << " not found in " + m_ARUN_directories[isupercell] << endl;
        }
        found_all_OUTCARs = false;
      }
    }
    if (!found_all_OUTCARs) {
      return;
    }
  ...
  void POccCalculator::calculateSTATICProperties(double temperature) {
    ...
    for (unsigned long long int isupercell = 0; isupercell < m_ARUN_directories.size() && found_all_OUTCARs == true; isupercell++) {
      if (!aurostd::CompressFileExist(getARUNDirectoryPath(isupercell) + "/OUTCAR.static")) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " OUTCAR.static not found in " + m_ARUN_directories[isupercell] << endl;
        }
        found_all_OUTCARs = false;
      }
    }
  ...
  void POccCalculator::plotAvgDOSCAR(double temperature) const {
    return plotAvgDOSCAR(m_xdoscar, temperature, m_aflags.Directory);
  }
  void POccCalculator::plotAvgDOSCAR(const string& doscar_path, const string& directory) const {
    ...
    const xDOSCAR xdos(doscar_path, *p_FileMESSAGE, true, *p_oss);
    const double temperature = poccDOSCAR2temperature(doscar_path);
  ...
  void POccCalculator::plotAvgDOSCAR(const xDOSCAR& xdos, double temperature, const string& directory) const {
    ...
    aurostd::xoption cmdline_opts;
    aurostd::xoption plot_opts;
    ...
    if (xdos.carstring == "POCC") { // turn off for ME - POCC+APL
      ...
      const xstructure xstr = plotter::getStructureWithNames(plot_opts, *p_FileMESSAGE, xdos.carstring, *p_oss);
      if (xdos.vDOS.size() == xstr.atoms.size()) {
        plotter::PLOT_DOS(plot_opts, xdos, *p_FileMESSAGE, *p_oss);
      } else {
      ...
      if (false) { // turn on for SG - plot orbitals for each species near fermi energy (dos_species_T0K_Cs_1)
        plot_opts.pop_attached("PROJECTION");
        plot_opts.push_attached("PROJECTION", "ORBITALS");
        plot_opts.push_attached("DATATYPE", "SPECIES");
        plot_opts.pop_attached("XMIN");
        plot_opts.push_attached("XMIN", "-1.5");
        plot_opts.pop_attached("XMAX");
        plot_opts.push_attached("XMAX", "1.5");
        for (size_t ispecies = 0; ispecies < xstr_pocc.species.size(); ispecies++) {
...
#define pocc_roundoff_tol 5.0 * pow(10, -((int) pocc_precision) - 1)
  ...
  void POccCalculator::writeResults(double temperature) const { // TEMPERATURE-DEPENDENT
    ...
    double Egap_net = 0.0;
    for (size_t ispin = 0; ispin < m_Egap.size(); ispin++) {
      if (m_Egap[ispin] != _METALGAP_) {
        Egap[ispin] = m_Egap[ispin];
      }
    }
    if (m_Egap_net != _METALGAP_) {
      Egap_net = m_Egap_net;
    }
  ...
  bool POccCalculator::QMVASPsFound() const {
    ...
    for (isupercell = 0; isupercell < m_ARUN_directories.size() && found_all_QMVASPs == true; isupercell++) {
      ...
      if (!aurostd::CompressFileExist(qmvasp_file)) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " " << DEFAULT_AFLOW_QMVASP_OUT << " not found in " + m_ARUN_directories[isupercell] << endl;
        }
        found_all_QMVASPs = false;
      }
    }
    if (!found_all_QMVASPs) {
      message << "Waiting for complete VASP calculations (" << qmvasp_file << " is missing)";
  ...
  vector<double> getVTemperatures(const string& temp_string) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " temp_string=" << temp_string << endl;
    }
    if (temp_string.empty()) {
      vector<double> tmp;
      return tmp; // return something, it will try again
      // throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"temp_string.empty()",_INPUT_ILLEGAL_);
    }

    vector<string> tokens;
    if (temp_string.find(":") != string::npos) {
      aurostd::string2tokens(temp_string, tokens, ":");
    } else if (temp_string.find("-") != string::npos) {
      aurostd::string2tokens(temp_string, tokens, "-");
    } else if (temp_string.find(",") != string::npos) {
      aurostd::string2tokens(temp_string, tokens, ",");
    } else {
    ...
    if (tokens.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown temp_string format", _INPUT_ILLEGAL_);
    }
    ...
    for (size_t i = 0; i < tokens.size(); i++) {
      if (!aurostd::isfloat(tokens[i])) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "temp_string token[i=" + tokens[i] + "] is not a float", _INPUT_ILLEGAL_);
      } // check that all doubles
      dtokens.push_back(aurostd::string2utype<double>(tokens[i]));
      if (std::signbit(dtokens.back())) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Negative temperature found in temp_string: " + tokens[i], _INPUT_ILLEGAL_);
      }
    }

    // simple comma-separated list
    if (dtokens.size() == 1 || temp_string.find(",") != string::npos) {
      return dtokens;
    }
    ...
    if (dtokens.size() == 3) {
      interval = dtokens.back();
    }
    vector<double> vtemperatures;
    for (double temp = dtokens[0]; temp <= dtokens[1]; temp += interval) {
      vtemperatures.push_back(temp);
    }
    if (vtemperatures.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No temperatures extracted from temp_string", _INPUT_ILLEGAL_);
    }

    if (LDEBUG) {
      for (size_t i = 0; i < vtemperatures.size(); i++) {
  ...
  void POccCalculator::CleanPostProcessing() {
    ...
    for (size_t i = 0; i < vfiles.size(); i++) {
      ...
      if (m_kflags.KBIN_PHONONS_CALCULATION_APL) {
        if (vfiles[i].find(POCC_PHDOSCAR_FILE) != string::npos) {
          message << "Removing old postprocessing file: " << vfiles[i];
          pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
          aurostd::RemoveFile(aurostd::CleanFileName(getOutputPath() + "/" + vfiles[i]));
          continue;
        } // PHDOSCAR.pocc_T0000K.xz
        if (vfiles[i].find(DEFAULT_APL_PHPOSCAR_FILE) != string::npos) {
          message << "Removing old postprocessing file: " << vfiles[i];
          pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
          aurostd::RemoveFile(aurostd::CleanFileName(getOutputPath() + "/" + vfiles[i]));
          continue;
        } // PHDOSCAR.xz
        if (vfiles[i].find(POCC_FILE_PREFIX + POCC_APL_OUT_FILE) != string::npos) {
          message << "Removing old postprocessing file: " << vfiles[i];
          pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
          aurostd::RemoveFile(aurostd::CleanFileName(getOutputPath() + "/" + vfiles[i]));
          continue;
  ...
  void POccCalculator::convolution() {
    ...
    const string inputs = m_p_flags.getattachedscheme("POCC::CONVOLUTION");
    ...
    if (vinputs.size() < 2) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "vinputs.size()<2", _INPUT_NUMBER_);
    }
    ...
    for (idir = 0; idir < vinputs.size(); idir++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " analyzing directory=" << aflags.Directory << endl;
      }
      if (!pocc::structuresGenerated(aflags.Directory)) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " system_name=" << vsystems.back() << endl;
      }
      ...
      if (idir == 0) {
        xstr_pocc = pcalc.xstr_pocc;
      }
      pcalc.loadDataIntoCalculator();
      if (!pcalc.QMVASPsFound()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "QMVASP files not found");
      }
      // add to pcalc
      for (std::list<POccSuperCellSet>::const_iterator it = pcalc.l_supercell_sets.begin(); it != pcalc.l_supercell_sets.end(); ++it) {
      ...
      for (i = 0; i < pcalc.m_ARUN_directories.size(); i++) {
        ...
        message << "adding m_ARUN_directories[" << m_ARUN_directories.size() - 1 << "]=" << m_ARUN_directories.back();
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
      }
    }
    if (m_ARUN_directories.size() != l_supercell_sets.size()) {
    ...
    string proto0;
    string pocc_params0;
    const uint natoms0 = pcalc.xstr_pocc.atoms.size();
    ...
    for (i = 0; i < vsystems.size(); i++) {
      ...
      if (!found_pocc) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unable to process system name: " + vsystems[i]);
      }
      ...
      if (pps != pps0 || proto0 != proto || natoms0 != xstr.atoms.size()) {
        same_pps_proto = false;
      }
    }
    if (same_pps_proto == false) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "found mismatch in pps or proto or natoms");
    }
    ...
    for (iatom = 0; iatom < natoms0; iatom++) {
      datoms.push_back(xstr_pocc.atoms[iatom]);
      datoms.back().partial_occupation_value = 0.0;
    } // add and reset
    for (iatom = 0; iatom < natoms0; iatom++) {
      for (i = 0; i < vxstr.size(); i++) {
        datoms[iatom].partial_occupation_value += (1.0 / vxstr.size()) * vxstr[i].atoms[iatom].partial_occupation_value;
      }
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " xstr_nopocc=" << endl << xstr_nopocc;
    }
    pflow::FIX_POCC_PARAMS(xstr_nopocc, pocc_params_new);

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " pocc_params_new(post)=" << pocc_params_new << endl;
    }
    aurostd::StringSubstInPlace(xstr_pocc.title, pocc_params0, pocc_params_new);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " xstr_pocc=" << endl << xstr_pocc;
    }

    m_vflags.AFLOW_SYSTEM.isentry = true;
    m_vflags.AFLOW_SYSTEM.content_string = vsystems.front();
    aurostd::StringSubstInPlace(m_vflags.AFLOW_SYSTEM.content_string, pocc_params0, pocc_params_new);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_vflags.AFLOW_SYSTEM.content_string=" << m_vflags.AFLOW_SYSTEM.content_string << endl;
    }
  ...
  void POccCalculator::loadDataIntoCalculator() {
    ...
    if (!m_kflags.KBIN_POCC_ARUNS2SKIP_STRING.empty()) {
      aurostd::string2tokens(m_kflags.KBIN_POCC_ARUNS2SKIP_STRING, v_aruns2skip, ",");
    }
    if (XHOST.vflag_control.flag("ARUNS2SKIP")) {
      aurostd::string2tokens(XHOST.vflag_control.getattachedscheme("ARUNS2SKIP"), v_aruns2skip, ",");
    }
    if (!v_aruns2skip.empty()) {
      uint index = 0;
      ...
      for (i = 0; i < v_aruns2skip.size(); i++) {
        ...
        if (!aurostd::isfloat(index_str)) { // input looks likes ARUN.POCC_..., convert to 1,2,3...
          aurostd::StringSubstInPlace(index_str, "ARUN.POCC_", "");
          vector<string> tokens;
      ...
      if (!v_indices.empty()) {
        ...
        for (i = 0; i < v_indices.size(); i++) {
          ...
          // erase
          it = l_supercell_sets.begin();
          std::advance(it, v_indices[i]);
  ...
  void POccCalculator::setTemperatureStringParameters() {
    vector<double> v_temperatures;
    return setTemperatureStringParameters(v_temperatures);
  }
  void POccCalculator::setTemperatureStringParameters(vector<double>& v_temperatures) {
    ...
    if (XHOST.vflag_control.flag("CALCULATION_TEMPERATURE")) {
      v_temperatures.clear();
      v_temperatures = getVTemperatures(XHOST.vflag_control.getattachedscheme("CALCULATION_TEMPERATURE"));
    } // command line input
    if (v_temperatures.empty()) {
      v_temperatures.clear();
      v_temperatures = getVTemperatures(DEFAULT_POCC_TEMPERATURE_STRING);
    } // user aflow.rc
    if (v_temperatures.empty()) {
      v_temperatures.clear();
      v_temperatures = getVTemperatures(AFLOWRC_DEFAULT_POCC_TEMPERATURE_STRING);
    } // internal aflow.rc, will always work
    if (v_temperatures.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "v_temperatures.empty()", _INPUT_ILLEGAL_);
    }
  ...
  void POccCalculator::postProcessing() {
    ...
    message << "Performing POCC post-processing for these temperatures: " << aurostd::joinWDelimiter(aurostd::vecDouble2vecString(v_temperatures, 5), ",");
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
...
namespace pocc {
  ...
  bool sortPSCsUFFEnergy(const POccSuperCell& a, const POccSuperCell& b) {
    return a.m_energy_uff < b.m_energy_uff;
  }
} // namespace pocc
...
namespace pocc {
  ...
  bool POccSuperCellSet::operator<(const POccSuperCellSet& other) const {
    if (getHNFIndex() != other.getHNFIndex()) {
      return getHNFIndex() < other.getHNFIndex();
    }
  ...
  unsigned long long int POccSuperCellSet::getDegeneracy() const {
    ...
    for (size_t i = 0; i < m_psc_set.size(); i++) {
      degeneracy += m_psc_set[i].m_degeneracy;
    }
    if (degeneracy == 0) {
      throw aurostd::xerror(__AFLOW_FILE__, "POccSuperCellSet::getDegeneracy():", "degeneracy==0", _RUNTIME_ERROR_);
    }
    return degeneracy;
  ...
  double POccSuperCellSet::getHNFIndex() const {
    return getSuperCell().m_hnf_index;
  } // fix me eventually to do robust matrix comparison
  double POccSuperCellSet::getSiteConfigIndex() const {
    return getSuperCell().m_site_config_index;
  } // fix me eventually to do robust matrix comparison
  double POccSuperCellSet::getUFFEnergy() const {
    return getSuperCell().m_energy_uff;
  }
} // namespace pocc
...
namespace pocc {
  ...
  void POccCalculator::loadFromAFlags(const aurostd::xoption& loader) { // grabs from m_aflags
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " loaded aflow.in" << endl;
    }
    if (loader.flag("LOAD::KFLAGS")) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " loaded kflags" << endl;
      }
    }
    if (loader.flag("LOAD::VFLAGS")) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " loaded vflags" << endl;
      }
    }
  ...
  string POccCalculator::getARUNDirectoryPath(uint isupercell) const {
    if (isupercell > m_ARUN_directories.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, XPID + "getARUNDirectoryPath():", "isupercell>m_ARUN_directories.size()-1", _INDEX_BOUNDS_);
    }
    if (m_convolution) {
      return m_ARUN_directories[isupercell];
    }
  ...
  bool sortPOccSites(const POccUnit& p1, const POccUnit& p2) {
    ...
    if (p1.equivalent != p2.equivalent) {
      return p1.equivalent < p2.equivalent;
    }
            ...
            { // CO20200106 - patching for auto-indenting
              ...
              { // CO20200106 - patching for auto-indenting
                ...
                { // CO20200106 - patching for auto-indenting
                  ...
                  duplicate_H = false;
                  for (size_t i = 0; i < v_unique_superlattices.size() && !duplicate_H; i++) {
                    if (LDEBUG) {
                      cerr << "SUPERLATTICES OLD " << endl;
                      cerr << v_unique_superlattices[i] << endl;
                    }
                    for (size_t pg = 0; pg < (eliminate_by_pgroup == true ? pgroup.size() : 1) && !duplicate_H; pg++) { // identity is ALWAYS the first pgroup by construction (see TRICK)
                      if (LDEBUG) {
  ...
  vector<uint> getConfigOrder(vector<vector<int>>& v_types_config) {
    ...
    for (size_t site = 0; site < v_types_config.size(); site++) {
      ...
      for (size_t pos = 0; pos < v_types_config[site].size(); pos++) {
        if (v_types_config[site][pos] == -1) {
          v_svc.back().vacancy_count++;
        }
      }
    }
    std::sort(v_svc.begin(), v_svc.end()); // most vacancies should go last
    vector<uint> v_config_order;
    for (size_t i = 0; i < v_svc.size(); i++) {
      v_config_order.push_back(v_svc[i].site);
    }
  ...
  bool POccCalculator::getNextSiteConfiguration() {
    ...
    if (v_types_config.empty()) {
      ...
      for (size_t site = 0; site < v_str_configs[config_iterator].site_configs.size(); site++) {
    ...
    if (!getNextSiteConfiguration(v_config_order, v_types_config)) {
      ...
      if (config_iterator >= v_str_configs.size() - 1) {
        return false;
      }
      config_iterator++;
      // const vector<StructureConfiguration>& v_str_configs=p_str.v_str_configs;
      for (size_t site = 0; site < v_types_config.size(); site++) {
        v_types_config[site].clear();
      }
      v_types_config.clear();
      for (size_t site = 0; site < v_str_configs[config_iterator].site_configs.size(); site++) {
  ...
  bool getNextSiteConfiguration(vector<vector<POccSiteConfiguration>>& vv_count_configs, vector<int>& v_config_iterators, vector<vector<int>>& v_types_config) {
    ...
    for (size_t site = 0; site < v_config_iterators.size(); site++) {
      if (v_config_iterators[site] >= (int) vv_count_configs[site].size() - 1) { // greater than not necessary but safe
        if (site == v_config_iterators.size() - 1) {
          break;
        } // stop condition
  ...
  bool getNextSiteConfiguration(vector<uint>& v_config_order, vector<vector<int>>& v_types_config) {
    uint site;
    for (size_t index = 0; index < v_config_order.size(); index++) {
      ...
      if (!getNextSiteConfiguration(v_types_config[site])) {
        ...
        if (index == v_types_config.size() - 1) {
          return false;
        } // stop condition
        continue;
      }
  ...
  bool getNextSiteConfiguration(vector<int>& types_config) {
    ...
    size_t _i = 0;
    size_t _j = 0;
    for (size_t i = 1; i < types_config.size(); i++) {
      if (types_config[i - 1] < types_config[i]) {
        _i = i;
      }
    }
    if (_i == 0) {
    ...
    for (size_t j = 0; j < types_config.size(); j++) {
      if (types_config[_i - 1] < types_config[j]) {
        _j = j;
      }
    }
    // cerr << "i=" << _i << "  j=" << _j << " ";
  ...
  void POccCalculator::getTotalPermutationsCount() {
    ...
    for (size_t str_config = 0; str_config < v_str_configs.size(); str_config++) {
      ...
      for (size_t site = 0; site < v_str_configs[str_config].site_configs.size(); site++) {
        // for(size_t config=0;config<v_str_configs[site].size();config++){
        ...
        while (getNextSiteConfiguration(current_config)) {
          config_permutations_count++;
        }
    ...
    if (m_p_flags.flag("POCC_COUNT_TOTAL")) {
  ...
  string getUFFParameterString(const string& atom) {
    if (atom == "H") {
      return "H   0.354   180.000 2.886   0.044   12.000  0.712   0.000   0.000   4.528   6.945   0.371   ";
    }
    if (atom == "He") {
      return "He  0.849   90.000  2.362   0.056   15.240  0.098   0.000   0.000   9.660   14.920  1.300   ";
    }
    if (atom == "Li") {
      return "Li  1.336   180.000 2.451   0.025   12.000  1.026   0.000   2.000   3.006   2.386   1.557   ";
    }
    if (atom == "Be") {
      return "Be  1.074   109.470 2.745   0.085   12.000  1.565   0.000   2.000   4.877   4.443   1.240   ";
    }
    if (atom == "B") {
      return "B   0.838   109.470 4.083   0.180   12.052  1.755   0.000   2.000   5.110   4.750   0.822   ";
    }
    if (atom == "C") {
      return "C   0.706   180.000 3.851   0.105   12.730  1.912   0.000   2.000   5.343   5.063   0.759   ";
    }
    if (atom == "N") {
      return "N   0.656   180.000 3.660   0.069   13.407  2.544   0.000   2.000   6.899   5.880   0.715   ";
    }
    if (atom == "O") {
      return "O   0.639   180.000 3.500   0.060   14.085  2.300   0.000   2.000   8.741   6.682   0.669   ";
    }
    if (atom == "F") {
      return "F   0.668   180.000 3.364   0.050   14.762  1.735   0.000   2.000   10.874  7.474   0.706   ";
    }
    if (atom == "Ne") {
      return "Ne  0.920   90.000  3.243   0.042   15.440  0.194   0.000   2.000   11.040  10.550  1.768   ";
    }
    if (atom == "Na") {
      return "Na  1.539   180.000 2.983   0.030   12.000  1.081   0.000   1.250   2.843   2.296   2.085   ";
    }
    if (atom == "Mg") {
      return "Mg  1.421   109.470 3.021   0.111   12.000  1.787   0.000   1.250   3.951   3.693   1.500   ";
    }
    if (atom == "Al") {
      return "Al  1.244   109.470 4.499   0.505   11.278  1.792   0.000   1.250   4.060   3.590   1.201   ";
    }
    if (atom == "Si") {
      return "Si  1.117   109.470 4.295   0.402   12.175  2.323   1.225   1.250   4.168   3.487   1.176   ";
    }
    if (atom == "P") {
      return "P   1.101   93.800  4.147   0.305   13.072  2.863   2.400   1.250   5.463   4.000   1.102   ";
    }
    if (atom == "S") {
      return "S   1.064   92.1000 4.035   0.274   13.969  2.703   0.000   1.250   6.928   4.486   1.047   ";
    }
    if (atom == "Cl") {
      return "Cl  1.044   180.000 3.947   0.227   14.866  2.348   0.000   1.250   8.564   4.946   0.994   ";
    }
    if (atom == "Ar") {
      return "Ar  1.032   90.000  3.868   0.185   15.763  0.300   0.000   1.250   9.465   6.355   2.108   ";
    }
    if (atom == "K") {
      return "K   1.953   180.000 3.812   0.035   12.000  1.165   0.000   0.700   2.421   1.920   2.586   ";
    }
    if (atom == "Ca") {
      return "Ca  1.761   90.000  3.399   0.238   12.000  2.141   0.000   0.700   3.231   2.880   2.000   ";
    }
    if (atom == "Sc") {
      return "Sc  1.513   109.470 3.295   0.019   12.000  2.592   0.000   0.700   3.395   3.080   1.750   ";
    }
    if (atom == "Ti") {
      return "Ti  1.412   109.470 3.175   0.017   12.000  2.659   0.000   0.700   3.470   3.380   1.607   ";
    }
    if (atom == "V") {
      return "V   1.402   109.470 3.144   0.016   12.000  2.679   0.000   0.700   3.650   3.410   1.470   ";
    }
    if (atom == "Cr") {
      return "Cr  1.345   90.000  3.023   0.015   12.000  2.463   0.000   0.700   3.415   3.865   1.402   ";
    }
    if (atom == "Mn") {
      return "Mn  1.382   90.000  2.961   0.013   12.000  2.430   0.000   0.700   3.325   4.105   1.533   ";
    }
    if (atom == "Fe") {
      return "Fe  1.270   109.470 2.912   0.013   12.000  2.430   0.000   0.700   3.760   4.140   1.393   ";
    }
    if (atom == "Co") {
      return "Co  1.241   90.000  2.872   0.014   12.000  2.430   0.000   0.700   4.105   4.175   1.406   ";
    }
    if (atom == "Ni") {
      return "Ni  1.164   90.000  2.834   0.015   12.000  2.430   0.000   0.700   4.465   4.205   1.398   ";
    }
    if (atom == "Cu") {
      return "Cu  1.302   109.470 3.495   0.005   12.000  1.756   0.000   0.700   4.200   4.220   1.434   ";
    }
    if (atom == "Zn") {
      return "Zn  1.193   109.470 2.763   0.124   12.000  1.308   0.000   0.700   5.106   4.285   1.400   ";
    }
    if (atom == "Ga") {
      return "Ga  1.260   109.470 4.383   0.415   11.000  1.821   0.000   0.700   3.641   3.160   1.211   ";
    }
    if (atom == "Ge") {
      return "Ge  1.197   109.470 4.280   0.379   12.000  2.789   0.701   0.700   4.051   3.438   1.189   ";
    }
    if (atom == "As") {
      return "As  1.211   92.100  4.230   0.309   13.000  2.864   1.500   0.700   5.188   3.809   1.204   ";
    }
    if (atom == "Se") {
      return "Se  1.190   90.600  4.205   0.291   14.000  2.764   0.335   0.700   6.428   4.131   1.224   ";
    }
    if (atom == "Br") {
      return "Br  1.192   180.000 4.189   0.251   15.000  2.519   0.000   0.700   7.790   4.425   1.141   ";
    }
    if (atom == "Kr") {
      return "Kr  1.147   90.000  4.141   0.220   16.000  0.452   0.000   0.700   8.505   5.715   2.270   ";
    }
    if (atom == "Rb") {
      return "Rb  2.260   180.000 4.114   0.040   12.000  1.592   0.000   0.200   2.331   1.846   2.770   ";
    }
    if (atom == "Sr") {
      return "Sr  2.052   90.000  3.641   0.235   12.000  2.449   0.000   0.200   3.024   2.440   2.415   ";
    }
    if (atom == "Y") {
      return "Y   1.698   109.470 3.345   0.072   12.000  3.257   0.000   0.200   3.830   2.810   1.998   ";
    }
    if (atom == "Zr") {
      return "Zr  1.564   109.470 3.124   0.069   12.000  3.667   0.000   0.200   3.400   3.550   1.758   ";
    }
    if (atom == "Nb") {
      return "Nb  1.473   109.470 3.165   0.059   12.000  3.618   0.000   0.200   3.550   3.380   1.603   ";
    }
    if (atom == "Mo") {
      return "Mo  1.467   90.000  3.052   0.056   12.000  3.400   0.000   0.200   3.465   3.755   1.530   ";
    }
    if (atom == "Tc") {
      return "Tc  1.322   90.000  2.998   0.048   12.000  3.400   0.000   0.200   3.290   3.990   1.500   ";
    }
    if (atom == "Ru") {
      return "Ru  1.478   90.000  2.963   0.056   12.000  3.400   0.000   0.200   3.575   4.015   1.500   ";
    }
    if (atom == "Rh") {
      return "Rh  1.332   90.000  2.929   0.053   12.000  3.500   0.000   0.200   3.975   4.005   1.509   ";
    }
    if (atom == "Pd") {
      return "Pd  1.338   90.000  2.899   0.048   12.000  3.210   0.000   0.200   4.320   4.000   1.544   ";
    }
    if (atom == "Ag") {
      return "Ag  1.386   180.000 3.148   0.036   12.000  1.956   0.000   0.200   4.436   3.134   1.622   ";
    }
    if (atom == "Cd") {
      return "Cd  1.403   109.470 2.848   0.228   12.000  1.650   0.000   0.200   5.034   3.957   1.600   ";
    }
    if (atom == "In") {
      return "In  1.459   109.470 4.463   0.599   11.000  2.070   0.000   0.200   3.506   2.896   1.404   ";
    }
    if (atom == "Sn") {
      return "Sn  1.398   109.470 4.392   0.567   12.000  2.961   0.199   0.200   3.987   3.124   1.354   ";
    }
    if (atom == "Sb") {
      return "Sb  1.407   91.600  4.420   0.449   13.000  2.704   1.100   0.200   4.899   3.342   1.404   ";
    }
    if (atom == "Te") {
      return "Te  1.386   90.250  4.470   0.398   14.000  2.882   0.300   0.200   5.816   3.526   1.380   ";
    }
    if (atom == "I") {
      return "I   1.382   180.000 4.500   0.339   15.000  2.650   0.000   0.200   6.822   3.762   1.333   ";
    }
    if (atom == "Xe") {
      return "Xe  1.267   90.000  4.404   0.332   12.000  0.556   0.000   0.200   7.595   4.975   2.459   ";
    }
    if (atom == "Cs") {
      return "Cs  2.570   180.000 4.517   0.045   12.000  1.573   0.000   0.100   2.183   1.711   2.984   ";
    }
    if (atom == "Ba") {
      return "Ba  2.277   90.000  3.703   0.364   12.000  2.727   0.000   0.100   2.814   2.396   2.442   ";
    }
    if (atom == "La") {
      return "La  1.943   109.470 3.522   0.017   12.000  3.300   0.000   0.100   2.836   2.742   2.071   ";
    }
    if (atom == "Ce") {
      return "Ce  1.841   90.000  3.556   0.013   12.000  3.300   0.000   0.100   2.774   2.692   1.925   ";
    }
    if (atom == "Pr") {
      return "Pr  1.823   90.000  3.606   0.010   12.000  3.300   0.000   0.100   2.858   2.564   2.007   ";
    }
    if (atom == "Nd") {
      return "Nd  1.816   90.000  3.575   0.010   12.000  3.300   0.000   0.100   2.869   2.621   2.007   ";
    }
    if (atom == "Pm") {
      return "Pm  1.801   90.000  3.547   0.009   12.000  3.300   0.000   0.100   2.881   2.673   2.000   ";
    }
    if (atom == "Sm") {
      return "Sm  1.780   90.000  3.520   0.008   12.000  3.300   0.000   0.100   2.912   2.720   1.978   ";
    }
    if (atom == "Eu") {
      return "Eu  1.771   90.000  3.493   0.008   12.000  3.300   0.000   0.100   2.879   2.788   2.227   ";
    }
    if (atom == "Gd") {
      return "Gd  1.735   90.000  3.368   0.009   12.000  3.300   0.000   0.100   3.167   2.975   1.968   ";
    }
    if (atom == "Tb") {
      return "Tb  1.732   90.000  3.451   0.007   12.000  3.300   0.000   0.100   3.018   2.834   1.954   ";
    }
    if (atom == "Dy") {
      return "Dy  1.710   90.000  3.428   0.007   12.000  3.300   0.000   0.100   3.056   2.872   1.934   ";
    }
    if (atom == "Ho") {
      return "Ho  1.696   90.000  3.409   0.007   12.000  3.416   0.000   0.100   3.127   2.891   1.925   ";
    }
    if (atom == "Er") {
      return "Er  1.673   90.000  3.391   0.007   12.000  3.300   0.000   0.100   3.187   2.915   1.915   ";
    }
    if (atom == "Tm") {
      return "Tm  1.660   90.000  3.374   0.006   12.000  3.300   0.000   0.100   3.251   2.933   2.000   ";
    }
    if (atom == "Yb") {
      return "Yb  1.637   90.000  3.355   0.228   12.000  2.618   0.000   0.100   3.289   2.965   2.158   ";
    }
    if (atom == "Lu") {
      return "Lu  1.671   90.000  3.640   0.041   12.000  3.271   0.000   0.100   2.963   2.463   1.896   ";
    }
    if (atom == "Hf") {
      return "Hf  1.611   109.470 3.141   0.072   12.000  3.921   0.000   0.100   3.700   3.400   1.759   ";
    }
    if (atom == "Ta") {
      return "Ta  1.511   109.470 3.170   0.081   12.000  4.075   0.000   0.100   5.100   2.850   1.605   ";
    }
    if (atom == "W") {
      return "W   1.392   90.000  3.069   0.067   12.000  3.700   0.000   0.100   4.630   3.310   1.538   ";
    }
    if (atom == "Re") {
      return "Re  1.372   90.000  2.954   0.066   12.000  3.700   0.000   0.100   3.960   3.920   1.600   ";
    }
    if (atom == "Os") {
      return "Os  1.372   90.000  3.120   0.037   12.000  3.700   0.000   0.100   5.140   3.630   1.700   ";
    }
    if (atom == "Ir") {
      return "Ir  1.371   90.000  2.840   0.073   12.000  3.731   0.000   0.100   5.000   4.000   1.866   ";
    }
    if (atom == "Pt") {
      return "Pt  1.364   90.000  2.754   0.080   12.000  3.382   0.000   0.100   4.790   4.430   1.557   ";
    }
    if (atom == "Au") {
      return "Au  1.262   90.000  3.293   0.039   12.000  2.625   0.000   0.100   4.894   2.586   1.618   ";
    }
    if (atom == "Hg") {
      return "Hg  1.340   180.000 2.705   0.385   12.000  1.750   0.000   0.100   6.270   4.160   1.600   ";
    }
    if (atom == "Tl") {
      return "Tl  1.518   120.000 4.347   0.680   11.000  2.068   0.000   0.100   3.200   2.900   1.530   ";
    }
    if (atom == "Pb") {
      return "Pb  1.459   109.470 4.297   0.663   12.000  2.846   0.100   0.100   3.900   3.530   1.444   ";
    }
    if (atom == "Bi") {
      return "Bi  1.512   90.000  4.370   0.518   13.000  2.470   1.000   0.100   4.690   3.740   1.514   ";
    }
    if (atom == "Po") {
      return "Po  1.500   90.000  4.709   0.325   14.000  2.330   0.300   0.100   4.210   4.210   1.480   ";
    }
    if (atom == "At") {
      return "At  1.545   180.000 4.750   0.284   15.000  2.240   0.000   0.100   4.750   4.750   1.470   ";
    }
    if (atom == "Rn") {
      return "Rn  1.420   90.000  4.765   0.248   16.000  0.583   0.000   0.100   5.370   5.370   2.200   ";
    }
    if (atom == "Fr") {
      return "Fr  2.880   180.000 4.900   0.050   12.000  1.847   0.000   0.000   2.000   2.000   2.300   ";
    }
    if (atom == "Ra") {
      return "Ra  2.512   90.000  3.677   0.404   12.000  2.920   0.000   0.000   2.843   2.434   2.200   ";
    }
    if (atom == "Ac") {
      return "Ac  1.983   90.000  3.478   0.033   12.000  3.900   0.000   0.000   2.835   2.835   2.108   ";
    }
    if (atom == "Th") {
      return "Th  1.721   90.000  3.396   0.026   12.000  4.202   0.000   0.000   3.175   2.905   2.018   ";
    }
    if (atom == "Pa") {
      return "Pa  1.711   90.000  3.424   0.022   12.000  3.900   0.000   0.000   2.985   2.905   1.800   ";
    }
    if (atom == "U") {
      return "U   1.684   90.000  3.395   0.022   12.000  3.900   0.000   0.000   3.341   2.853   1.713   ";
    }
    if (atom == "Np") {
      return "Np  1.666   90.000  3.424   0.019   12.000  3.900   0.000   0.000   3.549   2.717   1.800   ";
    }
    if (atom == "Pu") {
      return "Pu  1.657   90.000  3.424   0.016   12.000  3.900   0.000   0.000   3.243   2.819   1.840   ";
    }
    if (atom == "Am") {
      return "Am  1.660   90.000  3.381   0.014   12.000  3.900   0.000   0.000   2.990   3.004   1.942   ";
    }
    if (atom == "Cm") {
      return "Cm  1.801   90.000  3.326   0.013   12.000  3.900   0.000   0.000   2.832   3.190   1.900   ";
    }
    if (atom == "Bk") {
      return "Bk  1.761   90.000  3.339   0.013   12.000  3.900   0.000   0.000   3.194   3.036   1.900   ";
    }
    if (atom == "Cf") {
      return "Cf  1.750   90.000  3.313   0.013   12.000  3.900   0.000   0.000   3.197   3.101   1.900   ";
    }
    if (atom == "Es") {
      return "Es  1.724   90.000  3.299   0.012   12.000  3.900   0.000   0.000   3.333   3.089   1.900   ";
    }
    if (atom == "Fm") {
      return "Fm  1.712   90.000  3.286   0.012   12.000  3.900   0.000   0.000   3.400   3.100   1.900   ";
    }
    if (atom == "Md") {
      return "Md  1.689   90.000  3.274   0.011   12.000  3.900   0.000   0.000   3.470   3.110   1.900   ";
    }
    if (atom == "No") {
      return "No  1.679   90.000  3.248   0.011   12.000  3.900   0.000   0.000   3.475   3.175   1.900   ";
    }
    if (atom == "Lw") {
      return "Lw  1.698   90.000  3.236   0.011   12.000  3.900   0.000   0.000   3.500   3.200   1.900   ";
    } else {
      return "";
    }
  }
  ...
  vector<UFFParamAtom> getTypes2UFFParamsMap(const vector<string>& elements) {
    ...
    const UFFParamAtom uffp;
    vector<string> tokens;
    for (size_t i = 0; i < elements.size(); i++) {
      const string& element = elements[i];

      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " element[i=" << i << "]=" << element << endl;
      }
      uff_parameters_string = getUFFParameterString(element);
      if (uff_parameters_string.empty()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unable to fetch UFF parameters (requested " + element + ")");
      }
      aurostd::string2tokens(uff_parameters_string, tokens, " ");
      if (tokens.size() != 12) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unexpected UFF Parameters size", _VALUE_ILLEGAL_);
      }
  ...
  void POccCalculator::getHNFMatSiteConfig(const POccSuperCell& psc, xmatrix<double>& _hnf_mat, vector<vector<int>>& _v_types_config) {
    const bool LDEBUG = (false || _DEBUG_POCC_ || XHOST.DEBUG);
    if (LDEBUG) {
    ...
    while (iterateHNFMatrix()) { // sets hnf_mat
      ...
      while (getNextSiteConfiguration()) { // sets v_types_config
        if (site_config_index != psc.m_site_config_index) {
          site_config_index++;
          continue;
        }
  ...
  bool POccCalculator::areEquivalentStructures(const POccSuperCell& psc_a, const POccSuperCell& psc_b) {
    ...
    const xstructure b = createXStructure(psc_b, n_hnf, hnf_count, types_config_permutations_count, true,
                                          false); // PRIMITIVIZE==false, in general it is faster to find whether two structures are equivalent than it is to find primitive cell
  ...
  unsigned long long int POccCalculator::runRobustStructureComparison(std::list<POccSuperCellSet>::iterator it_pscs) {
    ...
    const unsigned long long int i_pscs = std::distance(l_supercell_sets.begin(), it_pscs);
    ...
    const bool test_iterator_insertion = false; // short circuit
    xstructure a;
    xstructure b;
    const uint starting_index = 1;
    pflow::updateProgressBar(0, vpsc.size() - starting_index, *p_oss);
    for (size_t i = starting_index; i < vpsc.size(); i++) {
      ...
      for (size_t j = 0; j < unique_structure_bins.size() && !found_equivalent; j++) {
        if (unique_structure_bins[j].empty()) {
          message << "No structure groups found";
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_RANGE_);
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " comparing structure[" << i << "] with structure[bin=" << j << ",i=" << 0 << "] (max=" << vpsc.size() << ")" << endl;
        }
        ...
        if (are_equivalent) {
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " found equivalent structure" << endl;
          }
    ...
    for (size_t i = 0; i < unique_structure_bins.size(); i++) {
      count_structures += unique_structure_bins[i].size();
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " unique_structure_bins[" << i << "]=" << unique_structure_bins[i].size() << endl;
      }
    }
    ...
    if (unique_structure_bins.size() == 1) {
      return unique_structure_bins.size();
    } // they are all equivalent as dictated by UFF
    ...
    for (size_t i = 1; i < unique_structure_bins.size(); i++) {
      POccSuperCellSet pscs;
      for (size_t j = 0; j < unique_structure_bins[i].size(); j++) {
        pscs.m_psc_set.push_back(vpsc[unique_structure_bins[i][j]]);
        vi_psc_to_remove.push_back(unique_structure_bins[i][j]);
      }
      if (!pscs.m_psc_set.empty()) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " pre-insertion it=" << std::distance(l_supercell_sets.begin(), it) << endl;
        }
        l_supercell_sets.insert(it, pscs);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " post-insertion it=" << std::distance(l_supercell_sets.begin(), it) << endl;
        }
        if (LDEBUG) {
    ...
    for (size_t i = 0; i < vi_psc_to_remove.size(); i++) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " removing structure " << vi_psc_to_remove[i] << " from bin[" << std::distance(l_supercell_sets.begin(), it_pscs) + 1 << "]" << endl;
      }
      (*it_pscs).m_psc_set.erase((*it_pscs).m_psc_set.begin() + vi_psc_to_remove[i]); // remove!
    }

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " new bin size=" << (*it_pscs).m_psc_set.size() << endl;
    }
  ...
  void POccCalculator::calculatePOccSuperCellUFF(
    ...
    while (true) {
      {
        const std::lock_guard<std::mutex> lg_job(m_job);
        if (npsc_queue <= 0) {
          break;
        }
        npsc_queue--;
        ipsc = npsc_queue;
        if (npsc_queue % step_size == 0) {
          pflow::updateProgressBar(vpsc.size() - npsc_queue, vpsc.size(), *p_oss);
        } // use a step size to avoid updating the progress bar too often
      }
      ...
      const double uff_energy = aurostd::round(local_ea[vpsc[ipsc].m_hnf_index].getUFFEnergy(), 12);
      const std::lock_guard<std::mutex> lg_save(m_save);
  ...
  void POccCalculator::countUniquePOccSuperCellUFF(int thread_id,
                                                   std::map<unsigned long long int, std::unordered_map<long long int, unsigned long int>>& map_unique,
                                                   const vector<POccSuperCell>& vpsc,
    ...
    size_t ipsc = 0;
    std::map<uint, POccUFFEnergyAnalyzer> local_ea;
    std::map<unsigned long long int, std::unordered_map<long long int, unsigned long int>> local_mu;
    const size_t step_size = std::max((size_t) 1, vpsc.size() / 100);
    while (true) {
      {
        const std::lock_guard<std::mutex> lg_job(m_job);
      ...
      local_ea[vpsc[ipsc].m_hnf_index].setBonds(vv_types_config[ipsc]);
      // cerr << local_ea[vpsc[ipsc].m_hnf_index].getUFFEnergy() <<  " | " << uff_tol<< "  |  " << local_ea[vpsc[ipsc].m_hnf_index].getUFFEnergy() / uff_tol << "\n\n";
      const long long int uff_energy = std::ceil(local_ea[vpsc[ipsc].m_hnf_index].getUFFEnergy() / uff_tol);
      local_mu[vpsc[ipsc].m_hnf_index][uff_energy]++;
    }
    const std::lock_guard<std::mutex> lg_save(m_save);
    for (std::map<unsigned long long int, std::unordered_map<long long int, unsigned long int>>::iterator it_out = local_mu.begin(); it_out != local_mu.end(); it_out++) {
      for (std::unordered_map<long long int, unsigned long int>::iterator it = it_out->second.begin(); it != it_out->second.end(); it++) {
  ...
  void POccCalculator::calculate() {
    ...
    const string struct_gen_algo = DEFAULT_POCC_STRUCTURE_GENERATION_ALGO;
    ...
    if (struct_gen_algo == "UFF") {
      ...
      if (m_count_unique_fast) {
        ...
        const double uff_tol = DEFAULT_UFF_ENERGY_TOLERANCE;
        std::map<unsigned long long int, std::unordered_map<long long int, unsigned long int>> map_unique;
        std::function<void(int, std::map<unsigned long long int, std::unordered_map<long long int, unsigned long int>>&, const vector<POccSuperCell>&, const vector<POccUFFEnergyAnalyzer>&,
                           const vector<vector<vector<int>>>&, size_t&, std::mutex&, std::mutex&)>
            fn = std::bind(&POccCalculator::countUniquePOccSuperCellUFF, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5,
        ...
        unsigned long long int count_tot = 0;
        unsigned long long int count_bin = 0;
        for (std::map<unsigned long long int, std::unordered_map<long long int, unsigned long int>>::iterator it_out = map_unique.begin(); it_out != map_unique.end(); it_out++) {
        ...
        for (size_t i = 0; i < vpsc.size(); i++) {
          add2DerivativeStructuresList(vpsc[i]);
        }
      }
      ...
      resetHNFMatrices();
      while (iterateHNFMatrix()) {
        vhnf_mats.push_back(hnf_mat);
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " vhnf_mats.size()=" << vhnf_mats.size() << endl;
      }

      xmatrix<long long int> H;
      xmatrix<long long int> U;
      xmatrix<long long int> V;
      xmatrix<long long int> S;
      ...
      for (i = 0; i < vhnf_mats.size(); i++) {
        ...
        for (j = 0; j < vS_uniq.size() && match == false; j++) {
          if (aurostd::isequal(S, vS_uniq[j])) {
            match = true;
          }
        }
        if (match == false) {
      ...
      if (LDEBUG) {
        ...
        for (i = 0; i < vS_uniq.size(); i++) {
          cerr << __AFLOW_FUNC__ << " viH_uniqSmith[i=" << i << "]=" << endl;
          cerr << viH_uniqSmith[i] << endl;
          cerr << __AFLOW_FUNC__ << " vS_uniq[i=" << i << "]=" << endl;
          cerr << vS_uniq[i] << endl;
        }
      }
      ...
      for (i = 0; i < vhnf_mats.size(); i++) {
        ...
        if (LDEBUG) {
          ...
          cerr << vhnf_mats[i] << endl;
          cerr << __AFLOW_FUNC__ << " H=" << endl;
          cerr << H << endl;
          cerr << __AFLOW_FUNC__ << " U=" << endl;
          cerr << U << endl;
          cerr << __AFLOW_FUNC__ << " V=" << endl;
          cerr << V << endl;
        }
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " xstr_ss.lattice=" << endl;
      ...
      vector<int> types_config0;
      vector<int> types_config1;
      vector<int> types_config2;
      const uint natoms_super = (uint) n_hnf * xstr_nopocc.atoms.size();
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " natoms_super=" << natoms_super << endl;
      }
      if (natoms_super == 0) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "natoms_super==0", _RUNTIME_ERROR_);
      }
      types_config0.resize(natoms_super);
      types_config1.resize(natoms_super);
      types_config2.resize(natoms_super);
      ...
      for (i = 0; i < vv_types_config.size(); i++) { // do NOT use a constant vv_types_config.size(), it will change within loop
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " types_config0=" << aurostd::joinWDelimiter(types_config0, ",") << endl;
        }
        for (j = vv_types_config.size() - 1; j > i; j--) { // do NOT use a constant vv_types_config.size(), it will change within loop
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " comparing i=" << i << " with j=" << j << endl;
          }
          // build types_config2
          k = 0;
          ...
          found = false;
          // build types_config1
          for (k = 0; k < types_config0.size(); k++) {
            types_config1[k] = types_config0[k];
          }
          for (permut = 0; permut < natoms_super; permut++) {
            ...
            if (types_config1 == types_config2) {
              found = true;
              break;
            } // vector<int> can be compared exactly
          }
          if (found == true) {
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " erasing j=" << j << endl;
            }
    ...
    if (DEFAULT_POCC_PERFORM_ROBUST_STRUCTURE_COMPARISON) {
      ...
      for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
        if (DEBUG_RSC || LDEBUG) {
          cerr << __AFLOW_FUNC__ << " current it=" << std::distance(l_supercell_sets.begin(), it) + 1 << "/" << std::distance(l_supercell_sets.begin(), l_supercell_sets.end()) << " [1]" << endl;
        }
        ...
        if (DEBUG_RSC || LDEBUG) {
          cerr << __AFLOW_FUNC__ << " advancing by " << count_new_groups - 1 << endl;
        }
        std::advance(it, count_new_groups - 1); // adjust iterator appropriately
        if (DEBUG_RSC || LDEBUG) {
          cerr << __AFLOW_FUNC__ << " current it=" << std::distance(l_supercell_sets.begin(), it) + 1 << "/" << std::distance(l_supercell_sets.begin(), l_supercell_sets.end()) << " [2]" << endl;
        }
        if (DEBUG_RSC || LDEBUG) {
          cerr << __AFLOW_FUNC__ << " it=" << std::distance(l_supercell_sets.begin(), it) << " vs. l_supercell_sets.end()=" << std::distance(l_supercell_sets.begin(), l_supercell_sets.end()) << endl;
        }
      }
    }
    ...
    if (m_p_flags.flag("POCC_COUNT_UNIQUE")) {
      return;
    } // so we don't bombard user with too much verbosity
    ...
    if (true && !m_aflags.Directory.empty() && !m_p_flags.flag("POCC_SKIP_WRITING_FILES")) { // move me so it runs whenever pocc runs (even post-processing)
      message << "Writing out " << POCC_ALL_HNF_MATRICES_FILE;
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    ...
    if (true && !m_aflags.Directory.empty() && !m_p_flags.flag("POCC_SKIP_WRITING_FILES")) { // move me so it runs whenever pocc runs (even post-processing)
      message << "Writing out " << POCC_ALL_SITE_CONFIGURATIONS_FILE;
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
      ...
      while (getNextSiteConfiguration()) {
        ...
        for (size_t site = 0; site < v_types_config.size(); site++) {
          for (size_t atom = 0; atom < v_types_config[site].size(); atom++) {
        ...
        for (size_t site = 0; site < v_types_config.size(); site++) {
          for (size_t atom = 0; atom < v_types_config[site].size(); atom++) {
            if (v_types_config[site][atom] >= 0) {
              if (v_types_config[site][atom] < (int) species.size()) {
                all_site_configs_ss << (site == 0 && atom == 0 ? "" : " ") << species[v_types_config[site][atom]];
              } else {
                all_site_configs_ss << (site == 0 && atom == 0 ? "" : " ") << "XX";
              }
            } else {
              all_site_configs_ss << (site == 0 && atom == 0 ? "" : " ") << "VAC";
            }
          }
          all_site_configs_ss << ((site != v_types_config.size() - 1) ? ";" : "");
        }
    ...
    if (DEFAULT_POCC_WRITE_OUT_ALL_SUPERCELLS && !m_aflags.Directory.empty() && !m_p_flags.flag("POCC_SKIP_WRITING_FILES")) {
      ...
      for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
        isupercell = std::distance(l_supercell_sets.begin(), it);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " isupercell=" << isupercell << endl;
        }
        ...
        all_supercells_ss << AFLOWIN_SEPARATION_LINE << endl;
        for (size_t i = 0; i < pscs.m_psc_set.size(); i++) {
          if (LDEBUG) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " structure created" << endl;
          }
  ...
  string POccCalculator::getARUNString(unsigned long long int i) const {
    return pocc::getARUNString(l_supercell_sets, i);
  }
  ...
  string getARUNString(unsigned long long int index_structure_group,
    ...
    if (include_strgrp) {
      ARUN << std::setfill('0') << std::setw(aurostd::getZeroPadding(vstructures_size)) << index_structure + 1 << "_";
    }
    ARUN << "H";
  ...
  vector<uint> POccCalculator::getMapToPARTCAR(unsigned long long int isupercell, const xstructure& xstr_in) {
    ...
    vector<string> vlines;
    const string structures_file = aurostd::CleanFileName(m_aflags.Directory + "/" + POCC_FILE_PREFIX + POCC_UNIQUE_SUPERCELLS_FILE);
    if (!aurostd::CompressFileExist(structures_file)) {
      message =
          "Could not load derivative structure."
          " Unique structures file " +
          structures_file + " not found.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_NOT_FOUND_);
    }
    aurostd::compressfile2vectorstring(structures_file, vlines);
    stringstream poscar;
    bool found_POSCAR_START_tag = false;
    bool found_POSCAR_STOP_tag = false;
    ...
    const xstructure xstr_derivative(poscar);
    if (LDEBUG) {
    ...
    const bool scale_volume = true;
    const bool optimize_match = false;
    double misfit = 0.0;
    structure_mapping_info mapping_info = compare::initialize_misfit_struct();
    bool matched = false;
    const uint natoms_in = xstr_in.atoms.size();
    const uint natoms_derivative = xstr_derivative.atoms.size();
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << "atom_map = " << aurostd::joinWDelimiter(mapping_info.atom_map, " ") << std::endl;
    }
    ...
    if (natoms_in < natoms_derivative) {
      ...
      for (size_t i = 0; i < mapping_info.atom_map.size(); i++) {
        map_to_derivative[i] = mapping_info.atom_map[i] / atom_ratio;
      }
    }

    if (LDEBUG) {
      std::cerr << "map_to_derivative = " << aurostd::joinWDelimiter(map_to_derivative, " ") << std::endl;
    }
    ...
    for (size_t i = 0; !tokens.empty(); i++) {
      if (tokens[i].find("site_config(") != string::npos) {
        aurostd::string2tokens(tokens[i], tokens2, "/");
        if (tokens2.size() != 2) {
          continue; // not the right site_config string
        }
        aurostd::StringSubstInPlace(tokens2[0], "site_config(", "");
        aurostd::StringSubstInPlace(tokens2[0], "#", "");
    ...
    const uint nsites = m_pocc_sites.size();
    const uint ntypes = xstr_pocc.species.size();
    ...
    for (size_t site = 0; site < m_pocc_sites.size(); site++) {
      for (size_t t = 0; t < m_pocc_sites[site].v_types.size(); t++) {
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0; i < type2occMap.size(); i++) {
    ...
    for (size_t site = 0; site < config.size(); site++) {
      std::sort(config[site].begin(), config[site].end()); // Site configurations are not sorted, but structures always are
      for (size_t at = 0; at < config[site].size(); at++) {
        type = config[site][at];
        if (type < 0) {
          continue; // type can be -1 when sites are not used
        }
    ...
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " derivative_partcar_map = " << aurostd::joinWDelimiter(derivative_partcar_map, " ") << std::endl;
    }

    // Finally, combine the maps to map the input structure to PARTCAR
    vector<uint> map_to_partcar;
    for (size_t i = 0; i < map_to_derivative.size(); i++) {
      map_to_partcar.push_back(derivative_partcar_map[map_to_derivative[i]]);
    }
    if (LDEBUG) {
      std::cerr << __AFLOW_FUNC__ << " map_to_partcar = " << aurostd::joinWDelimiter(map_to_partcar, " ") << std::endl;
    }
...
namespace pocc {
  ...
  void POccSiteConfiguration::prepareNoPOccConfig() {
    ...
    const int _site = site;
    const int _i_hnf = i_hnf;
    const vector<POccUnit> pocc_groups = m_pocc_groups;
    clear();
    site = _site;
    i_hnf = _i_hnf;
    m_pocc_groups = pocc_groups;
  ...
  void POccSiteConfiguration::preparePOccConfig() {
    ...
    const int pocc_groups_count = m_pocc_groups.size();
    const xvector<int> xvi_dummy(pocc_groups_count - 1, 0);
    const xvector<double> xvd_dummy(pocc_groups_count - 1, 0);
    ...
    for (int i = 0; i < xv_occupation_count_input.rows; i++) {
      xv_occupation_count_input[i] = m_pocc_groups[i].v_occupants.size();
    }
    // debug
    if (LDEBUG) {
  ...
  int POccSiteConfiguration::getNextOccupationMultiple(int i_hnf, xvector<int>& xv_next_occupation_multiple) {
    ...
    for (int i = 0; i < xv_next_occupation_multiple.rows; i++) {
      if (xv_next_occupation_multiple(i) >= i_hnf) { // greater than not necessary, but safe
        if (i == xv_next_occupation_multiple.rows - 1) {
          break;
        } // STOP condition, safe
        xv_next_occupation_multiple(i) = 0;
        continue;
      }
  ...
  int POccSiteConfiguration::calculateOccupationCountTotal(xvector<int>& xv_next_occupation_multiple) const {
  ...
  void POccSiteConfiguration::calculateError() {
    ...
    max_site_error = 0.0;
    for (size_t pocc_group = 0; pocc_group < m_pocc_groups.size(); pocc_group++) {
  ...
  vector<int> POccSiteConfiguration::getStartingTypesConfiguration() const {
    ...
    site_count += vacancy_count;
    // for(int i=0;i<vacancy_count;i++){starting_config[site_count]=-1; site_count++;}
    for (size_t group = 0; group < m_pocc_groups.size(); group++) {
      for (size_t occ = 0; occ < m_pocc_groups[group].v_types.size(); occ++) {
        for (int i = 0; i < xv_occupation_multiple[group]; i++) {
          starting_config[site_count] = m_pocc_groups[group].v_types[occ];
          site_count++;
        }
      }
    }
...
namespace pocc {
  vector<uint> getVacanciesSuperCell(const vector<int>& pc2sc_map, const vector<vector<int>>& v_types_config) {
    ...
    int type;
    uint starting_supercell_atom_index;
    uint supercell_atom_index;
    for (size_t site = 0; site < v_types_config.size(); site++) {
      ...
      for (size_t i = 0; i < v_types_config[site].size(); i++) {
        ...
        if (type < 0) {
          supercell_atom_index = starting_supercell_atom_index + i;
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " supercell_atom_index=" << supercell_atom_index << endl;
          }
    ...
    for (size_t site = 0; site < v_types_config.size(); site++) {
      ...
      for (size_t i = 0; i < v_types_config[site].size(); i++) {
        ...
        if (type >= 0) { // not a vacancy
          ...
          cpos = supercell_atom.cpos;
          supercell_atom = pocc_atom;
          supercell_atom.fpos = fpos;
          supercell_atom.cpos = cpos;
  ...
  void rebuildSuperCell(const xstructure& xstr_pocc, const vector<uint>& v_vacancies, xstructure& supercell) {
    ...
    const vector<uint> new_species;
    bool found;
    for (size_t atom = 0; atom < atoms.size(); atom++) {
      found = false;
      for (size_t type = 0; type < new_types.size() && !found; type++) {
        if (atoms[atom].type == (int) new_types[type]) {
          found = true;
        }
      }
      if (!found) {
    ...
    for (size_t type = 1; type < new_types.size(); type++) {
      if (new_types[type - 1] > new_types[type]) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Error in atom types reassignment, atoms are out of order");
      }
    }
    ...
    for (size_t atom = 0; atom < atoms.size(); atom++) {
      supercell_clean.AddAtom(atoms[atom], false);
    ...
    uint atom;
    uint type;
    for (size_t it = 0; it < new_types.size(); it++) {
      type = new_types[it];
      ...
      if ((uint) atoms[atom].type < xstr_pocc.species_pp_type.size()) {
        supercell_clean.species_pp_type[it] = xstr_pocc.species_pp_type[type];
      } // species_pp_type
      if ((uint) atoms[atom].type < xstr_pocc.species_pp_version.size()) {
        supercell_clean.species_pp_version[it] = xstr_pocc.species_pp_version[type];
      } // species_pp_version
      if ((uint) atoms[atom].type < xstr_pocc.species_pp_ZVAL.size()) {
        supercell_clean.species_pp_ZVAL[it] = xstr_pocc.species_pp_ZVAL[type];
      } // species_pp_ZVAL
      if ((uint) atoms[atom].type < xstr_pocc.species_pp_vLDAU.size()) {
        supercell_clean.species_pp_vLDAU[it] = xstr_pocc.species_pp_vLDAU[type];
      } // species_pp_vLDAU
    }
    ...
    if (supercell_clean.title.empty()) {
      supercell_clean.buildGenericTitle(true, false);
    }
    ...
    if (LDEBUG) {
      ...
      cerr << __AFLOW_FUNC__ << " supercell_clean.lattice=" << endl;
      cerr << supercell_clean.lattice << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " final supercell=" << endl;
      cerr << supercell << endl;
    }
  }
...
namespace pocc {
  ...
  void POccCalculatorTemplate::setPOccStructure(const xstructure& _xstr_pocc) {
    ...
    const xvector<double> _stoich_each_type(xstr_pocc.stoich_each_type.size() - 1, 0);
    stoich_each_type = _stoich_each_type;
    for (size_t i = 0; i < xstr_pocc.stoich_each_type.size(); i++) {
      stoich_each_type[i] = xstr_pocc.stoich_each_type[i];
    }
  }

  void POccCalculatorTemplate::setNonPOccStructure(const xstructure& _xstr_nopocc) {
    xstr_nopocc = _xstr_nopocc;
  }

  void POccCalculatorTemplate::setSpeciesRedecoration(const vector<string>& species_redecoration) {
    m_species_redecoration = species_redecoration;
  }
...
namespace pocc {
  ...
  */
  ...
  void POccCalculator::generateStructures(const _xvasp& in_xvasp) {
    ...
    message << "Writing out " << POCC_UNIQUE_SUPERCELLS_FILE;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);

    stringstream unique_derivative_structures_ss;
    stringstream new_AflowIn_ss;
    const bool original_quiet_flag = XHOST.QUIET;
    unsigned long long int isupercell = 0;

    // for(unsigned long long int i=0;i<getUniqueSuperCellsCount();i++)
    for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) { // CO20200106 - patching for auto-indenting
      isupercell = std::distance(l_supercell_sets.begin(), it);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " starting structure[" << isupercell << "]" << endl;
      }
      ...
      unique_derivative_structures_ss << AFLOWIN_SEPARATION_LINE << endl;
      unique_derivative_structures_ss << AFLOW_POCC_TAG << "UFF_ENERGY=" << std::fixed << std::setprecision(15) << (*it).getUFFEnergy() << endl;
      unique_derivative_structures_ss.unsetf(std::ios_base::floatfield);
      unique_derivative_structures_ss << AFLOWIN_SEPARATION_LINE << endl;
      ...
      xvasp.AVASP_INCAR_EXPLICIT_START_STOP.str(""); // reset to avoid build-up //HE20241017
      XHOST.QUIET = true;
  ...
  void POccCalculator::createModuleAflowIns(const _xvasp& xvasp_in, const string& MODULE) {
    ...
    if (!QMVASPsFound()) {
      message =
          "Not all POCC calculations have finished."
  ...
  vector<uint> getTypes2PCMap(const xstructure& xstr) {
    ...
    for (size_t i = 0; i < types2pc_map.size(); i++) {
      for (size_t j = i + 1; j < types2pc_map.size(); j++) {
        if (xstr.atoms[types2pc_map[i]].name == xstr.atoms[types2pc_map[j]].name) {
          if (xstr.atoms[types2pc_map[i]].name.empty()) {
  ...
  void POccCalculator::initializePOccStructure() {
    ...
    message << "Input PARTCAR" << endl;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    message << xstr_pocc;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_RAW_);
    ...
    message << "Clean PARTCAR" << endl;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    message << xstr_pocc;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_RAW_);

    message << "Parent structure" << endl;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    message << xstr_nopocc;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_RAW_);
    ...
    message << "Standardized (re-decorated) parent structure" << endl;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
    message << xstr_nopocc;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_RAW_);
  ...
  const xmatrix<double>& POccCalculator::getLattice() const {
    return xstr_sym.lattice;
  }
  const vector<_sym_op>& POccCalculator::getPGroup() const {
    return xstr_sym.pgroup;
  }
  ...
  vector<POccUnit> getPOccSites(const xstructure& xstr_pocc, const _aflags& aflags, ostream& oss) {
    ofstream FileMESSAGE;
  ...
  vector<POccUnit> getPOccSites(const xstructure& xstr_pocc, const _aflags& aflags, ofstream& FileMESSAGE, ostream& oss) {
    ...
    bool new_site = true;
    for (size_t atom = 0; atom < xstr_pocc.atoms.size(); atom++) {
      new_site = true;
      const _atom& pocc_atom = xstr_pocc.atoms[atom];
      for (size_t site = 0; site < pocc_sites.size() && new_site; site++) {
        POccUnit& pocc_site = pocc_sites[site];
        if (!pocc_site.v_occupants.empty() && aurostd::modulus(pocc_atom.cpos - xstr_pocc.atoms[pocc_site.v_occupants[0]].cpos) < _AFLOW_POCC_ZERO_TOL_) {
      ...
      if (new_site) {
        pocc_sites.emplace_back(aflags, FileMESSAGE, oss);
    ...
    for (size_t site = 0; site < pocc_sites.size(); site++) {
      pocc_sites[site].partial_occupation_flag = (pocc_sites[site].partial_occupation_flag || pocc_sites[site].v_occupants.size() > 1); // if the only other occupant is a vacancy
    } // is it a pocc site?
    ...
    for (size_t site = 0; site < pocc_sites.size(); site++) {
      POccUnit& pocc_site = pocc_sites[site];
      for (size_t occ = 0; occ < pocc_site.v_occupants.size(); occ++) {
        new_group = true;
        const _atom& pocc_atom = xstr_pocc.atoms[pocc_site.v_occupants[occ]];
        for (size_t group = 0; group < pocc_site.m_pocc_groups.size() && new_group; group++) {
        ...
        if (new_group) {
          pocc_site.m_pocc_groups.emplace_back(aflags, FileMESSAGE, oss);
    ...
    if (LDEBUG) {
      ...
      for (size_t site = 0; site < pocc_sites.size(); site++) {
        POccUnit& pocc_site = pocc_sites[site];
        for (size_t group = 0; group < pocc_site.m_pocc_groups.size(); group++) {
          const POccUnit& pocc_group = pocc_site.m_pocc_groups[group];
  ...
  void POccCalculator::partitionPOccSites() {
    ...
    for (size_t site = 0; site < m_pocc_sites.size(); site++) {
      if (m_pocc_sites[site].partial_occupation_flag) {
        pocc_atoms_total += m_pocc_sites[site].v_occupants.size();
      }
    }
  }

  void POccCalculator::getOptimizedSiteCountConfigurations(int site, int i_hnf, vector<POccSiteConfiguration>& v_site_configs) {
    const bool LDEBUG = (false || _DEBUG_POCC_ || XHOST.DEBUG);
    stringstream message;
    // reset
    ...
    // test of stupidity
    if (m_pocc_sites[site].m_pocc_groups.empty()) {
    ...
    while (pocc_group_iterator != -1) {
      ...
      if (pocc_group_to_skip != pocc_group_iterator) { // slight optimizer
        ...
        if (site_config.calculateOccupationCountTotal(xv_next_occupation_multiple) > i_hnf) { // start ignoring
          if (LDEBUG) {
            ...
            cerr << __AFLOW_FUNC__ << " site_config.calculateOccupationCountTotal(xv_next_occupation_multiple)=" << site_config.calculateOccupationCountTotal(xv_next_occupation_multiple) << " > " << i_hnf
                 << "=i_hnf" << endl;
          }
          pocc_group_to_skip = pocc_group_iterator;
          skipping = true;
          continue;
        }
      ...
      if (skipping && pocc_group_iterator != pocc_group_to_skip) {
  ...
  xvector<double> POccCalculator::calculateStoichEachType(vector<vector<int>>& v_types_config) {
    ...
    for (size_t site = 0; site < v_types_config.size(); site++) {
      for (size_t atom = 0; atom < v_types_config[site].size(); atom++) {
        if (v_types_config[site][atom] != -1) {
          stoich_each_type[v_types_config[site][atom]] += 1.0;
          total_atoms += 1.0;
        }
      }
    }
    if (total_atoms > 0.0) {
      for (int type = 0; type < stoich_each_type.rows; type++) {
        stoich_each_type[type] /= total_atoms;
      }
    }
  ...
  void POccCalculator::getSiteCountConfigurations(int i_hnf) { // CO20200106 - patching for auto-indenting
    ...
    if (LDEBUG) {
      for (size_t site = 0; site < vv_count_configs.size(); site++) {
        ...
        for (size_t config = 0; config < vv_count_configs[site].size(); config++) {
    ...
    while (pocc::getNextSiteConfiguration(vv_count_configs, v_config_iterators, v_types_config)) { // CO20200106 - patching for auto-indenting
      ...
      if (aurostd::isequal(stoich_error, eps, _AFLOW_POCC_ZERO_TOL_) || stoich_error < eps) {
        ...
        if (!aurostd::isequal(stoich_error, eps, _AFLOW_POCC_ZERO_TOL_)) {
          eps = stoich_error;
          // for(size_t str_config=0;str_config<v_str_configs.size();str_config++){v_str_configs[str_config].clear();}
          v_str_configs.clear();
        }
        v_str_configs.emplace_back();
  ...
  xstructure createNonPOccStructure(const xstructure& xstr_pocc, const vector<POccUnit>& pocc_sites, bool use_automatic_volumes_in) {
    ...
    for (size_t site = 0; site < pocc_sites.size(); site++) {
      if (pocc_sites[site].partial_occupation_flag) {
        ...
        for (size_t atom = 1; atom < pocc_sites[site].v_occupants.size(); atom++) {
  ...
  void POccCalculator::redecorateXStructures() {
    ...
    for (size_t i = 0; i < types2pc_map.size(); i++) {
      species.push_back(xstr_pocc.atoms[types2pc_map[i]].cleanname);
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " species             =" << aurostd::joinWDelimiter(species, ",") << endl;
    }

    if (use_standard_elements_set == true) {
      species.clear();
      for (size_t i = 0; i < types2pc_map.size(); i++) {
        species.push_back(elements_std[i]);
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " species_std         =" << aurostd::joinWDelimiter(species, ",") << endl;
      }
      ...
      const bool resort_standard_elements_set = true; // the approach here is WRONG, we need structure comparison
      if (resort_standard_elements_set == true) {
        ...
        for (size_t i = 0; i < species.size(); i++) {
          species_orig.push_back(species[i]);
        }
        vector<uint> occupant2species_map; // maps occupants (m_pocc_sites) to species (types) in order of m_pocc_sites, NOT NEEDED
        species.clear();
        species.resize(species_orig.size()); // make room for species before you fill, need occupant2species_map to dictate where new species go
        vector<POccUnit> m_pocc_sites_resorted; // make copy of m_pocc_sites for reshuffling
        for (site = 0; site < m_pocc_sites.size(); site++) {
          for (occ = 0; occ < m_pocc_sites[site].v_occupants.size(); occ++) {
            occupant2species_map.push_back(m_pocc_sites[site].v_types[occ]);
          }
        ...
        for (site = 0; site < m_pocc_sites_resorted.size(); site++) {
          ...
          sublattice_found = (!m_pocc_sites_resorted[site].v_types.empty()); // set to true first
          for (itype = 0; itype < m_pocc_sites_resorted[site].v_types.size() && sublattice_found; itype++) {
            ...
            if (!aurostd::WithinList(vanions, occupant)) {
              sublattice_found = false;
            }
          }
          if (sublattice_found) {
            ...
            if (!aurostd::WithinList(types_added, type)) {
              const string& occupant = xstr_pocc.atoms[m_pocc_sites_resorted[site].v_occupants[itype]].name;
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " mapping " << occupant << " to " << species_orig[iocc2spec] << " [sublattice]" << endl;
              }
        ...
        for (site = 0; site < m_pocc_sites_resorted.size(); site++) {
          ...
          if (!aurostd::WithinList(types_added, type)) {
            const string& occupant = xstr_pocc.atoms[m_pocc_sites_resorted[site].v_occupants[itype]].name;
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " mapping " << occupant << " to " << species_orig[iocc2spec] << " [first-occupant]" << endl;
            }
            species[type] = species_orig[iocc2spec++];
            types_added.push_back(type);
          }
        ...
        for (site = 0; site < m_pocc_sites_resorted.size(); site++) {
          for (itype = 0; itype < m_pocc_sites_resorted[site].v_types.size(); itype++) {
            ...
            if (!aurostd::WithinList(types_added, type)) {
              const string& occupant = xstr_pocc.atoms[m_pocc_sites_resorted[site].v_occupants[itype]].name;
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " mapping " << occupant << " to " << species_orig[iocc2spec] << " [remaining]" << endl;
              }
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " species_std_resorted=" << aurostd::joinWDelimiter(species, ",") << endl;
        }
      }
      ...
      const vector<POccUnit> pocc_sites_tmp = getPOccSites(xstr_pocc_tmp);
  ...
  void POccCalculator::calculateSymNonPOccStructure(bool verbose) {
    ...
    const bool badbit_set = (*p_oss_sym).bad(); //((*p_oss_sym).rdstate()==std::ios_base::badbit);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " badbit_set=" << badbit_set << endl;
    }
    ofstream* p_FileMESSAGE_sym = p_FileMESSAGE;
    ofstream devNull("/dev/null"); // must be outside if statement (scope)
    // verbose=true;
    const bool osswrite = verbose;
    if (!verbose) {
      pflow::defaultKFlags4SymWrite(kflags_sym, false); // setting to false is always ok
      if (!badbit_set) {
        (*p_oss_sym).setstate(std::ios_base::badbit);
      } // like NULL
    ...
    const bool sym_calculated = pflow::PerformFullSymmetry(xstr_sym, *p_FileMESSAGE_sym, m_aflags, kflags_sym, osswrite, *p_oss_sym);
    if (!badbit_set) {
      (*p_oss_sym).clear();
    } // clear NULL
    if (sym_calculated == false) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "AFLOW-SYM failed to calculate the symmetry of the clean (non-pocc'd) structure");
    }
  ...
  void POccCalculator::propagateEquivalentAtoms2POccStructure() {
    ...
    for (size_t site = 0; site < m_pocc_sites.size(); site++) {
      for (size_t atom = 0; atom < m_pocc_sites[site].v_occupants.size(); atom++) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " mapping iatoms info of xstr_sym[" << site << "] to xstr_pocc[" << m_pocc_sites[site].v_occupants[atom] << "]" << endl;
        }
  ...
  uint POccCalculator::getHNFTableIterationPadding() const {
    return 4;
  }
  uint POccCalculator::getHNFTableErrorPadding() const {
    return getHNFTabelPOCCPrecision() + 2;
  }
  uint POccCalculator::getHNFTableColumnPadding() const {
    return 2 * getHNFTableGeneralPrecision() + 2 * getHNFTableIterationPadding() + 15;
  }
  string POccCalculator::getHeaderMaxSiteError() const {
    return "site_error";
  }
  string POccCalculator::getHeaderMaxStoichError() const {
    return "stoich_error";
  }
  string POccCalculator::getHeaderStoichiometry() const {
  ...
  string POccCalculator::hnfTableHeader() {
    ...
    for (size_t site = 0; site < m_pocc_sites.size(); site++) {
      if (m_pocc_sites[site].partial_occupation_flag) {
        for (size_t occ = 0; occ < m_pocc_sites[site].v_occupants.size(); occ++) {
          ...
          // output - START
          ss_header << "| ";
          ss_header << aurostd::PaddedCENTER("pocc_atom = " + aurostd::utype2string(pocc_atom) + "/" + aurostd::utype2string(pocc_atoms_total) + (!atom_name.empty() ? " [" + atom_name + "]" : ""),
                                             getHNFTableColumnPadding() + 2); // need to +2, bug in PaddedCENTER()
  ...
  string POccCalculator::hnfTableLineOutput(int i_hnf, int str_config) {
    ...
    stringstream tmp_ss;
    tmp_ss.precision(getHNFTableGeneralPrecision());
    tmp_ss.setf(std::ios::fixed, std::ios::floatfield);

    double stoich_error;
    double site_error = 1.0 / i_hnf;
  ...
  void POccCalculator::writeHNFTableOutput(int i_hnf, double& stoich_error, double& site_error) {
    ...
    for (size_t str_config = 0; str_config < v_str_configs.size(); str_config++) {
      if (v_str_configs[str_config].site_configs.empty()) {
  ...
  void POccCalculator::calculateHNF() {
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message_raw, m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_RAW_);
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, hnfTableHeader(), m_aflags, *p_FileMESSAGE, *p_oss, _LOGGER_RAW_);
...
namespace pocc {
  ...
  void POccUFFEnergyAnalyzer::calculateNNDistances(xstructure& xstr, vector<uint>& v_vacancies) {
    const xmatrix<double> _distance_matrix(xstr.atoms.size() - 1, xstr.atoms.size() - 1, 0, 0);
    distance_matrix = _distance_matrix;

    // get distance matrix first, then find nearest-neighbor distances

    // get distance matrix first
    for (size_t atom1 = 0; atom1 < xstr.atoms.size(); atom1++) {
      if (isVacancy(v_vacancies, atom1)) {
        continue;
      }
      for (size_t atom2 = atom1 + 1; atom2 < xstr.atoms.size(); atom2++) {
    ...
    for (size_t i = 0; i < xstr.atoms.size(); i++) {
      v_dist_nn.push_back(std::numeric_limits<double>::max());
    } // initialize with big number
    for (size_t atom1 = 0; atom1 < xstr.atoms.size(); atom1++) {
      if (isVacancy(v_vacancies, atom1)) {
        continue;
      }
      for (size_t atom2 = 0; atom2 < xstr.atoms.size(); atom2++) {
        if (isVacancy(v_vacancies, atom2)) {
          continue;
        }
        if (atom1 != atom2 && distance_matrix(atom1, atom2) < v_dist_nn[atom1]) {
          v_dist_nn[atom1] = distance_matrix(atom1, atom2);
        }
      }
    }
  }
  ...
  void POccUFFEnergyAnalyzer::setExplorationRadius() {
    ...
    if (COMPARE_WITH_KESONG) {
      m_exploration_radius = ENERGY_RADIUS;
    }
  ...
  void POccUFFEnergyAnalyzer::getCluster(xmatrix<double>& _hnf_mat) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " xstr_ss=" << endl;
      cerr << xstr_ss << endl;
    }
  ...
  uint POccUFFEnergyAnalyzer::NNDistancesMapPC(uint atom) {
    return sc2pc_map[xstr_cluster.grid_atoms_sc2pcMap[atom]];
  }
  uint POccUFFEnergyAnalyzer::NNDistancesMapSC(uint atom) {
    return xstr_cluster.grid_atoms_sc2pcMap[atom];
  }
  ...
  void POccUFFEnergyAnalyzer::setBonds(const vector<vector<int>>& v_types_config) {
    ...
    has_vacancies = (!v_vacancies.empty());
    if (LDEBUG) {
    ...
    if (has_vacancies) {
      if (bonding_set) {
        bonding_set = (v_vacancies == m_vacancies);
      } // bonding_set==false when cluster is generated, check to recalculate only if has been calculated before
    ...
    if (!bonding_set) {
      ...
      v_nonbonded_atom_indices.clear();
      uint atom1;
      const double uff_bonding_distance = DEFAULT_UFF_BONDING_DISTANCE; // CO20190208 - remember, this is not a raw distance, but a distance to within the nn distance
      for (size_t sc_atom1 = 0; sc_atom1 < xstr_ss.atoms.size(); sc_atom1++) {
        if (isVacancy(m_vacancies, sc_atom1)) {
          continue;
        }
        ...
        for (uint atom2 = 0; atom2 < (uint) xstr_cluster.grid_atoms_number; atom2++) {
          if (atom1 != atom2) {
            ...
            if (distij <= m_exploration_radius) { // KY (via SC) used <= as well (GetNeighData)
              ...
              if (std::abs(distij - v_dist_nn[(this->*NNDistancesMap)(atom1)]) <= uff_bonding_distance) { // KY only looked at <, not <= (ExtractBonds)
                v_bonded_atom_indices.emplace_back(0);
  ...
  bool POccUFFEnergyAnalyzer::isVacancy(vector<uint>& v_vacancies, uint atom) { // this function must remain fast
    for (size_t vi = 0, v_vacancies_size = v_vacancies.size(); vi < v_vacancies_size; vi++) {
      if (atom == v_vacancies[vi]) {
        return true;
      }
    }
    return false;
  ...
  double POccUFFEnergyAnalyzer::bondEnergyBond(const UFFParamBond& uffb) {
    return 0.5 * uffb.Kij * uffb.delta * uffb.delta;
  }
  double POccUFFEnergyAnalyzer::bondEnergyNoBond(const UFFParamBond& uffb) {
    return uffb.Dij * (uffb.X12 - 2.0 * uffb.X6);
  }
  ...
  double POccUFFEnergyAnalyzer::getUFFBondEnergy(xstructure& xstr, vector<vector<uint>>& v_bonded_atom_indices, uint MODE) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " bonding mode = " << (MODE == BOND_MODE ? "bonding" : "non-bonding") << endl;
    }

    // pointer's for bond/non-bonded energy contributions
    double (POccUFFEnergyAnalyzer::*BondEnergy)(const UFFParamBond& uffb) = nullptr;
    if (MODE == BOND_MODE) {
      BondEnergy = &POccUFFEnergyAnalyzer::bondEnergyBond;
    } // spring potential
    else if (MODE == NONBOND_MODE) {
      BondEnergy = &POccUFFEnergyAnalyzer::bondEnergyNoBond;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " v_bonded_atom_indices.size()=" << v_bonded_atom_indices.size() << endl;
    }

    if (types2uffparams_map.empty()) {
    ...
    for (size_t i = 0; i < v_bonded_atom_indices.size(); i++) {
      ...
      if (isVacancy(m_vacancies, atom1)) {
        continue;
      }
  ...
  xstructure POccCalculator::createXStructure(const POccSuperCell& psc, int n_hnf, unsigned long long int hnf_count, unsigned long long int types_config_permutations_count, bool clean_structure, bool primitivize) {
    const bool LDEBUG = (false || _DEBUG_POCC_ || XHOST.DEBUG);
    vector<int> sc2pc_map;
    vector<int> pc2sc_map;
    ...
    if (LDEBUG) {
      ...
      vstmp.clear();
      for (size_t site = 0; site < v_types_config.size(); site++) {
        vstmp.push_back(aurostd::joinWDelimiter(v_types_config[site], ","));
      }
      cerr << __AFLOW_FUNC__ << " v_types_config=" << aurostd::joinWDelimiter(vstmp, ";") << endl;
    }

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " creating superstructure" << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " resolving vacancies (if any)" << endl;
    }
    const vector<uint> v_vacancies = getVacanciesSuperCell(pc2sc_map, v_types_config); // get vacancies
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vacancies=" << aurostd::joinWDelimiter(v_vacancies, ",") << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " rebuilding structure" << endl;
    }
    rebuildSuperCell(xstr_pocc, v_vacancies, supercell); // remove vacancies and clean all xstructure properties
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " structure built!" << endl;
    }

    if (clean_structure) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " structure pre-cleaning" << endl;
        cerr << supercell << endl;
      }
      if (primitivize) {
        supercell.GetStandardPrimitive();
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " structure post-primitivization" << endl;
          cerr << supercell << endl;
        }
      }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " structure post-cleaning" << endl;
        cerr << supercell << endl;
      }
    }
    //[HNF(4)=7/7= 1 0 0; 1 2 0; 1 0 2]
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " rewriting title" << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " final structure" << endl;
      cerr << supercell << endl;
    }
    return supercell;
  }
  ...
  double POccUFFEnergyAnalyzer::getUFFEnergy() {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " supercell(start)=" << endl;
      cerr << supercell << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " supercell(random-sites-only)=" << endl;
      cerr << supercell << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " structure energy=" << m_energy_uff << endl;
    }
    ...
    if (COMPARE_WITH_KESONG) {
      ...
      const double uff_energy_kesong = pocc::CalculateUFFEnergy(supercell);
      cout << "KESONG " << std::fixed << std::setprecision(15) << uff_energy_kesong << endl;
      const double diff = std::abs(m_energy_uff - uff_energy_kesong);
...
namespace pocc {
  ...
  // constructor
  //--------------------------------------------------------------------------------
  POccStructuresFile::POccStructuresFile(ostream& oss) : xStream(oss), m_initialized(false) {
    initialize();
  }
  POccStructuresFile::POccStructuresFile(ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize();
  }
  POccStructuresFile::POccStructuresFile(const string& fileIN, ostream& oss) : xStream(oss), m_initialized(false) {
    initialize(fileIN);
  }
  POccStructuresFile::POccStructuresFile(const string& fileIN, ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(fileIN);
  }
  POccStructuresFile::POccStructuresFile(const _aflags& aflags, ostream& oss) : xStream(oss), m_initialized(false) {
    initialize(aflags);
  }
  POccStructuresFile::POccStructuresFile(const _aflags& aflags, ofstream& FileMESSAGE, ostream& oss) : xStream(FileMESSAGE, oss), m_initialized(false) {
    initialize(aflags);
  }
  POccStructuresFile::POccStructuresFile(const string& fileIN, const _aflags& aflags, ostream& oss) : xStream(oss), m_initialized(false) {
  ...
  void POccStructuresFile::setAFlags(const _aflags& aflags) {
    m_aflags = aflags;
  }
  ...
  void POccStructuresFile::readFile(const string& fileIN) {
    ...
    bool found_file = false;
    if (!found_file && aurostd::CompressFileExist(m_aflags.Directory + "/" + fileIN, m_filename)) {
      aurostd::compressfile2stringstream(m_filename, structures_file_ss);
      found_file = true;
    }
    if (!found_file && aurostd::FileExist(m_aflags.Directory + "/" + fileIN, m_filename)) {
      aurostd::file2stringstream(m_filename, structures_file_ss);
      found_file = true;
    }
    if (!found_file) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, m_aflags.Directory + "/" + fileIN + " does not exist", _FILE_NOT_FOUND_);
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_filename=" << m_filename << endl;
    }

    m_content = structures_file_ss.str();
    aurostd::string2vectorstring(m_content, m_vcontent);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_vcontent.size()=" << m_vcontent.size() << endl;
    }
  }
  ...
  void POccStructuresFile::processFile() {
    ...
    const bool found_POSCAR_START_tag = aurostd::substring2bool(m_content, poscar_start_tag);
    if (!found_POSCAR_START_tag) {
      poscar_start_tag = POccStructure_title_tag;
    }
    if (!aurostd::substring2bool(m_content, poscar_start_tag)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot find POSCAR start tag", _FILE_CORRUPT_);
    }
    string poscar_stop_tag = POSCAR_STOP_tag;
    const bool found_POSCAR_STOP_tag = aurostd::substring2bool(m_content, poscar_stop_tag);
    if (!found_POSCAR_STOP_tag) {
      poscar_stop_tag = "[AFLOW] **";
    }
    if (!aurostd::substring2bool(m_content, poscar_stop_tag)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot find POSCAR stop tag", _FILE_CORRUPT_);
    }
    if (LDEBUG) {
    ...
    if (found_tag_sg == false) {
      ...
      if (aurostd::substring2bool(m_content, structure_group_tag)) {
        ...
        found_tag_sg = true;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " \"STRUCTURES_GROUP\" tag found" << endl;
        }
      }
    }
    // STRUCTURE_GROUP
    if (found_tag_sg == false) {
      ...
      if (aurostd::substring2bool(m_content, structure_group_tag)) {
        ...
        found_tag_sg = true;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " \"STRUCTURE_GROUP\" tag found" << endl;
        }
      }
    }
    // STRUCTURE GROUP
    if (found_tag_sg == false) {
      ...
      if (aurostd::substring2bool(m_content, structure_group_tag)) {
        ...
        found_tag_sg = true;
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " \"STRUCTURE GROUP\" tag found" << endl;
        }
      }
    }
    ...
    if (!(m_fileoptions.flag("TYPE::ALL_SUPERCELLS") || m_fileoptions.flag("TYPE::UNIQUE_SUPERCELLS"))) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown structures file type (none set)", _FILE_CORRUPT_);
    }
    if (m_fileoptions.flag("TYPE::ALL_SUPERCELLS") && m_fileoptions.flag("TYPE::UNIQUE_SUPERCELLS")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown structures file type (all set)", _FILE_CORRUPT_);
    }
    ...
    for (iline = 0; iline < m_vcontent.size(); iline++) {
      ...
      if (aurostd::substring2bool(m_vcontent[iline], poscar_start_tag)) { // do NOT use "else if" as structure_group_tag might equal poscar_start_tag
        reading_POSCAR = true;
        if (l_supercell_sets.empty()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Found POSCAR before structures tag", _FILE_CORRUPT_);
        }
        l_supercell_sets.back().m_psc_set.emplace_back();
        if (m_vPOSCAR_lines.empty()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Found POSCAR stop tag before start tag", _FILE_CORRUPT_);
        }
        m_vPOSCAR_lines.back().emplace_back(0);
        ititle = (found_POSCAR_START_tag ? iline + 1 : iline);
        if (ititle >= m_vcontent.size()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot grab title at iline=" + aurostd::utype2string(ititle), _FILE_CORRUPT_);
        }
      ...
      if (reading_POSCAR && aurostd::substring2bool(m_vcontent[iline], poscar_stop_tag)) {
        ...
        if (m_vPOSCAR_lines.empty() || m_vPOSCAR_lines.back().empty() || m_vPOSCAR_lines.back().back().size() != 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Found POSCAR stop tag before start tag", _FILE_CORRUPT_);
        }
    ...
    for (iline = 0; iline < m_vcontent.size(); iline++) {
      ...
      if (aurostd::substring2bool(m_vcontent[iline], POCC_AFLOWIN_tag + "UFF_ENERGY=")) {
        if (index_str >= (*it).m_psc_set.size()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "index_str>=(*it).m_psc_set.size()", _INDEX_BOUNDS_);
        }
  ...
  bool POccStructuresFile::getARUNDirectories(vector<string>& ARUN_directories, bool tryDirectoryLS) {
    const bool LDEBUG = (false || _DEBUG_POCC_ || XHOST.DEBUG);
    if (!m_initialized) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "POccStructuresFile not initialized", _RUNTIME_ERROR_);
    }
    ...
    for (i = 0; i < m_vPOSCAR_lines.size() && missing_tag == false; i++) {
      if (m_vPOSCAR_lines[i].empty()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_vPOSCAR_lines[" + aurostd::utype2string(i) + "].size()==0", _INDEX_ILLEGAL_);
      }
      if (m_vPOSCAR_lines[i][0].empty()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "m_vPOSCAR_lines[" + aurostd::utype2string(i) + "][0].size()==0", _INDEX_ILLEGAL_);
      }
      iline = m_vPOSCAR_lines[i][0][0];
      iline -= 1; // the line we are looking for is just above the POSCAR title
      if (iline >= m_vcontent.size()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "iline>=m_vcontent.size()", _INDEX_ILLEGAL_);
      }
      if (!aurostd::substring2bool(m_vcontent[iline], POSCAR_POCC_series_START_tag)) {
        missing_tag = true;
        break;
      ...
      if (vtokens.size() == 2) { // POCC_01
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " POCC_NN setting found (old POCC type)" << endl;
        }
        if (!aurostd::isfloat(vtokens[1])) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot determine POcc structure group index", _VALUE_ILLEGAL_);
        }
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " POCC_NN_HASH setting found (unique_supercells filetype)" << endl;
        }
        if (m_fileoptions.flag("TYPE::ALL_SUPERCELLS")) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "ALL_STRUCTURES setting but detected UNIQUE_STRUCTURES format", _FILE_WRONG_FORMAT_);
        }
        if (!aurostd::isfloat(vtokens[1])) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot determine POcc structure group index", _VALUE_ILLEGAL_);
        }
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " POCC_NN_NN_HASH setting found (all_supercells filetype)" << endl;
        }
        if (m_fileoptions.flag("TYPE::UNIQUE_SUPERCELLS")) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "UNIQUE_STRUCTURES setting but detected ALL_STRUCTURES format", _FILE_WRONG_FORMAT_);
        }
        if (!aurostd::isfloat(vtokens[1])) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot determine POcc structure group index", _VALUE_ILLEGAL_);
        }
        if (!aurostd::isfloat(vtokens[2])) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot determine POcc structure index", _VALUE_ILLEGAL_);
        }
        arun_directory = "ARUN." + vtokens[0] + "_" + vtokens[1] + "_" + vtokens[3];
      } else {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown POcc hash format (_)", _FILE_CORRUPT_);
    ...
    if (missing_tag == false && ARUN_directories.size() == l_supercell_sets.size()) {
      return true;
    }

    if (m_fileoptions.flag("TYPE::ALL_SUPERCELLS")) { // build titles, but we need vstructures_size, which requires TYPE::ALL_SUPERCELLS
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " trying to build directories from information in ALL_STRUCTURES file" << endl;
      }
      unsigned long long int isupercell = 0;
      for (std::list<POccSuperCellSet>::iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
        isupercell = std::distance(l_supercell_sets.begin(), it);
    ...
    if (ARUN_directories.size() == l_supercell_sets.size()) {
      return true;
    }

    // last attempt - DirectoryLS()
    if (tryDirectoryLS == false) {
      return false;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " trying DirectoryLS() (last attempt)" << endl;
    }
    aurostd::DirectoryLS(m_aflags.Directory, vtokens);
    for (size_t i = 0; i < vtokens.size(); i++) {
      if (aurostd::substring2bool(vtokens[i], "ARUN.POCC_") && aurostd::IsDirectory(m_aflags.Directory + "/" + vtokens[i])) {
        ARUN_directories.push_back(vtokens[i]);
      }
    }
  ...
  bool POccStructuresFile::loadDataIntoCalculator(POccCalculator& pcalc, bool tryDirectoryLS) {
    ...
    pcalc.l_supercell_sets.clear();
    for (std::list<POccSuperCellSet>::const_iterator it = l_supercell_sets.begin(); it != l_supercell_sets.end(); ++it) {
      pcalc.l_supercell_sets.push_back(*it);
    }
    if (!getARUNDirectories(pcalc.m_ARUN_directories, tryDirectoryLS)) {
      return false;
    }
    if (LDEBUG) {
...
// JSON Serialization implementation classes
namespace pocc {

  // Json serialization of POccUnit

  aurostd::JSON::object POccUnit::serialize() const {
    return aurostd::JSON::object({AST_JSON_GETTER(JSON_POccUnit_MEMBERS)});
  }

  POccUnit POccUnit::deserialize(const aurostd::JSON::object& jo) {
    AST_JSON_SETTER(JSON_POccUnit_MEMBERS)
    return *this;
  }

  // Json serialization of POccCalculator

  aurostd::JSON::object POccCalculator::serialize() const {
    return aurostd::JSON::object({AST_JSON_GETTER(JSON_POccCalculator_MEMBERS)});
  }

  POccCalculator POccCalculator::deserialize(const aurostd::JSON::object& jo) {
    AST_JSON_SETTER(JSON_POccCalculator_MEMBERS)
    return *this;
  }

  // Json serialization of POccSiteConfiguration

  aurostd::JSON::object POccSiteConfiguration::serialize() const {
    return aurostd::JSON::object({AST_JSON_GETTER(JSON_POccSiteConfiguration_MEMBERS)});
  }

  POccSiteConfiguration POccSiteConfiguration::deserialize(const aurostd::JSON::object& jo) {
    AST_JSON_SETTER(JSON_POccSiteConfiguration_MEMBERS)
    return *this;
  }

  // Json serialization of POccUFFEnergyAnalyzer

  aurostd::JSON::object POccUFFEnergyAnalyzer::serialize() const {
    return aurostd::JSON::object({AST_JSON_GETTER(JSON_POccUFFEnergyAnalyzer_MEMBERS)});
  }

  POccUFFEnergyAnalyzer POccUFFEnergyAnalyzer::deserialize(const aurostd::JSON::object& jo) {
    AST_JSON_SETTER(JSON_POccUFFEnergyAnalyzer_MEMBERS)
    return *this;
  }

  // Json serialization of POccStructuresFile

  aurostd::JSON::object POccStructuresFile::serialize() const {
    return aurostd::JSON::object({AST_JSON_GETTER(JSON_POccStructuresFile_MEMBERS)});
  }

  POccStructuresFile POccStructuresFile::deserialize(const aurostd::JSON::object& jo) {
    AST_JSON_SETTER(JSON_POccStructuresFile_MEMBERS)
    return *this;
  }

  // Json serialization of POccSuperCellSet

  aurostd::JSON::object POccSuperCellSet::serialize() const {
    return aurostd::JSON::object({AST_JSON_GETTER(JSON_POccSuperCellSet_MEMBERS)});
  }

  POccSuperCellSet POccSuperCellSet::deserialize(const aurostd::JSON::object& jo) {
    AST_JSON_SETTER(JSON_POccSuperCellSet_MEMBERS)
    return *this;
  }

  // Json serialization of UFFParamAtom

  aurostd::JSON::object UFFParamAtom::serialize() const {
    return aurostd::JSON::object({AST_JSON_GETTER(JSON_UFFParamAtom_MEMBERS)});
  }

  UFFParamAtom UFFParamAtom::deserialize(const aurostd::JSON::object& jo) {
    AST_JSON_SETTER(JSON_UFFParamAtom_MEMBERS)
    return *this;
  }
  ...
  aurostd::JSON::object POccSuperCell::serialize() const {
    return aurostd::JSON::object({AST_JSON_GETTER(JSON_POccSuperCell_MEMBERS)});
  }

  POccSuperCell POccSuperCell::deserialize(const aurostd::JSON::object& jo) {
    AST_JSON_SETTER(JSON_POccSuperCell_MEMBERS)
    return *this;
  }

  // Json serialization of StructureConfiguration

  aurostd::JSON::object StructureConfiguration::serialize() const {
    return aurostd::JSON::object({AST_JSON_GETTER(JSON_StructureConfiguration_MEMBERS)});
  }

  StructureConfiguration StructureConfiguration::deserialize(const aurostd::JSON::object& jo) {
    AST_JSON_SETTER(JSON_StructureConfiguration_MEMBERS)
    return *this;
  }
