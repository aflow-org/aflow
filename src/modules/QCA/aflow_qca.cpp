...
namespace qca {
  ...
  bool QuasiChemApproxCalculator::initialize(const aurostd::xoption& qca_opts) {
    free();
  ...
  /// @authors
  /// @mod{SD,20220718,created function}
  void QuasiChemApproxCalculator::readQCAOptions(const aurostd::xoption& qca_opts) {
    if (qca_opts.getattachedscheme("QCA::PLATTICE").empty() || qca_opts.getattachedscheme("QCA::ELEMENTS").empty()) {
      const string message = "QCA::PLATTICE and QCA::ELEMENTS flags must be defined";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_MISSING_);
    }
    plattice = qca_opts.getattachedscheme("QCA::PLATTICE");
    aurostd::string2tokens(qca_opts.getattachedscheme("QCA::ELEMENTS"), elements, ",");
    if (!qca_opts.getattachedscheme("QCA::DIRECTORY").empty()) {
      rootdirpath = qca_opts.getattachedscheme("QCA::DIRECTORY");
    }
    if (!qca_opts.getattachedscheme("QCA::AFLOWLIB_DIRECTORY").empty()) {
      aflowlibpath = qca_opts.getattachedscheme("QCA::AFLOWLIB_DIRECTORY");
    }
    if (!qca_opts.getattachedscheme("QCA::AFLOW_MAX_NUM_ATOMS").empty()) {
      aflow_max_num_atoms = aurostd::string2utype<int>(qca_opts.getattachedscheme("QCA::AFLOW_MAX_NUM_ATOMS"));
    }
    if (!qca_opts.getattachedscheme("QCA::CV_CUTOFF").empty()) {
      cv_cut = aurostd::string2utype<double>(qca_opts.getattachedscheme("QCA::CV_CUTOFF"));
    }
    if (!qca_opts.getattachedscheme("QCA::CONC_NPTS").empty()) {
      conc_npts = aurostd::string2utype<int>(qca_opts.getattachedscheme("QCA::CONC_NPTS"));
    }
    if (!qca_opts.getattachedscheme("QCA::TEMP_RANGE").empty()) {
      aurostd::string2tokens(qca_opts.getattachedscheme("QCA::TEMP_RANGE"), temp_range, ",");
    }
    if (!qca_opts.getattachedscheme("QCA::TEMP_NPTS").empty()) {
      temp_npts = aurostd::string2utype<int>(qca_opts.getattachedscheme("QCA::TEMP_NPTS"));
    }
    if (!qca_opts.getattachedscheme("QCA::PRINT").empty()) {
      print = qca_opts.getattachedscheme("QCA::PRINT");
    }
    if (qca_opts.flag("QCA::SCREEN_ONLY")) {
      screen_only = true;
    }
    if (qca_opts.flag("QCA::IMAGE_ONLY")) {
      image_only = true;
    }
    if (qca_opts.flag("QCA::BINODAL")) {
      calc_binodal = true;
    }
    if (qca_opts.flag("QCA::USE_SG")) {
      use_sg = true;
    }
  }
  ...
  void QuasiChemApproxCalculator::errorFix() {
    ...
    if (aflow_max_num_atoms < 1 || max_num_atoms < 1) {
      const string message = "Maximum number of atoms per cluster must be at least 1";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ERROR_);
    }
    // Check if directory is writable
    if (!aurostd::DirectoryMake(rootdirpath)) {
      const string message = "Cannot create directory";
    ...
    if (conc_npts < 2) {
      const string message = "Number of points for the concentration range must be at least 2";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ERROR_);
    }
    // Check if concentration curve format is valid
    if (conc_curve) {
      if (conc_curve_range.size() != 2 * nelem) {
        const string message = "Concentration curve must have format [X1_start, X1_end, X2_start, X2_end,...X(K)_start, X(K)_end]";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_);
      }
      // Check if concentration curve is within [0,1] and sums to 1
      double totconc_init = 0.0;
      double totconc_final = 0.0;
      vector<double> vconc_init;
      vector<double> vconc_final;
      vector<size_t> vzeros_init;
      vector<size_t> vzeros_final;
      for (size_t i = 0; i < nelem && totconc_init <= 1.0 && totconc_final <= 1.0; i++) {
        if (conc_curve_range[2 * i] < 0 || conc_curve_range[2 * i] > 1 || conc_curve_range[2 * i + 1] < 0 || conc_curve_range[2 * i + 1] > 1) {
          const string message = "Concentration curve must be defined on [0,1]";
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ERROR_);
        }
        totconc_init += conc_curve_range[2 * i];
        totconc_final += conc_curve_range[2 * i + 1];
        vconc_init.push_back(conc_curve_range[2 * i]);
        vconc_final.push_back(conc_curve_range[2 * i + 1]);
      }
      if (!aurostd::isequal(totconc_init, 1.0) || !aurostd::isequal(totconc_final, 1.0)) {
        const string message = "Total concentration must sum to 1";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ERROR_);
      }
      // Redefine concentration curve from [0,1] to (0,1)
      aurostd::WithinList(vconc_init, 0.0, vzeros_init);
      aurostd::WithinList(vconc_final, 0.0, vzeros_final);
      const double cdelta_init = CONC_SHIFT * (double) vzeros_init.size() / ((double) vconc_init.size() - (double) vzeros_init.size());
      const double cdelta_final = CONC_SHIFT * (double) vzeros_final.size() / ((double) vconc_final.size() - (double) vzeros_final.size());
      for (size_t i = 0; i < conc_curve_range.size(); i++) {
    ...
    if (temp_range.size() != 2) {
      const string message = "Temperature range must have format [T_start T_end]";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_);
    }
    // Check if temperature range has enough points
    if (temp_npts < 2) {
      const string message = "Number of points for the temperature range must be at least 2";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ERROR_);
    }
    // Check if temperature values are valid
    if (temp_range[0] < 0 || temp_range[1] < 0) {
      const string message = "Temperature cannot be below 0K";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ERROR_);
    }
    // Check if output format is valid
    if (print != "txt" && print != "json" && print != "pdf" && print != "eps" && print != "png") {
      const string message = "Format \"" + print + "\" is invalid";
    ...
    for (uint ialloy = 2; ialloy <= 3; ialloy++) { // AFLOW database only contains data for binaries and ternaries
      ...
      while (xc.increment()) {
        ...
        for (size_t i = 0; i < index_xc.size(); i++) {
          if (index_xc[i] == 1) {
            elements_xc.push_back(elements[i]);
          }
        }
        aurostd::sort(elements_xc);
        alloyname_PBE = "";
        for (size_t i = 0; i < elements_xc.size(); i++) {
          alloyname_PBE += AVASP_Get_PseudoPotential_PAW_PBE(elements_xc[i]);
        }
  ...
  string QuasiChemApproxCalculator::getLatForATAT(bool scale) const {
    ...
    for (size_t i = 0; i < nelem; i++) {
      alat += GetAtomRadiusCovalent(elements[i]);
    }
    alat /= nelem;
  ...
  vector<xstructure> QuasiChemApproxCalculator::getATATXstructures(const int max_num_atoms, bool fromfile) const {
    ...
    if (fromfile) {
      ...
      for (size_t i = 0; i < files.size(); i++) {
        if (aurostd::FileExist(rundirpath + "/" + files[i] + "/str.out")) {
          ...
          vstr_tmp.emplace_back(oss, IOATAT_STR);
      ...
      for (size_t i = 0; i < index.size(); i++) {
        vstr[index[i]] = vstr_tmp[i];
      }
      return vstr;
    }
    double scale = 0.0;
    for (size_t i = 0; i < nelem; i++) {
      scale += GetAtomRadiusCovalent(elements[i]);
    }
    scale /= nelem;
    if (plattice == "fcc") {
    ...
    const string tmpfile = aurostd::TmpStrCreate();
    aurostd::string2file(lat_atat, tmpfile);
    const std::pair<std::string, std::string> sstr = aurostd::execute2OutErrPair("genstr -n " + aurostd::utype2string<int>(max_num_atoms) + " -l " + tmpfile);
    aurostd::RemoveFile(tmpfile);
    if (sstr.first.empty() || aurostd::substring2bool(sstr.second, "Unable to open lattice file")) {
      message << "Invalid lat.in file";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_CORRUPT_);
    } else if (aurostd::substring2bool(sstr.second, "command not found")) {
      message << "Missing genstr program";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    aurostd::string2vectorstring(sstr.first, vinput);
    ...
    for (size_t line = 0; line < vinput.size(); line++) {
      if (aurostd::substring2bool(vinput[line], "end")) {
        vstr.emplace_back(oss, IOATAT_STR);
  ...
  void QuasiChemApproxCalculator::runATAT() {
    const bool LDEBUG = (false || XHOST.DEBUG);
    stringstream message;
    const string cdirpath = aurostd::getPWD();
    uint iter = 0;
    if (aurostd::substring2bool(aurostd::execute2OutErrPair("mmaps").second, "command not found")) {
    ...
    while (!aurostd::substring2bool(logstring, "true and predicted ground states agree") && !aurostd::substring2bool(logstring, "No other ground states")) {
      iter++;
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Sleeping, iter=" << iter << endl;
      }
  ...
  void QuasiChemApproxCalculator::readCVCluster() {
    ...
    if (tokens[0] != "Crossvalidation") {
      const string message = "ATAT run did not complete";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
  ...
  void QuasiChemApproxCalculator::calculateNumAtomCluster() {
    ...
    for (size_t i = 0; i < vstr_ce.size(); i++) {
      natom = 0;
      for (size_t j = 0; j < vstr_ce[i].num_each_type.size(); j++) {
        natom += vstr_ce[i].num_each_type[j];
      }
    ...
    for (size_t i = 0; i < vstr_ce.size(); i++) {
      if (nelem != vstr_ce[i].stoich_each_type.size()) {
        ...
        for (size_t j = 0; j < nelem; j++) {
          if (aurostd::WithinList(str_elements, elements[j], ie)) {
            stoich(j + 1) = vstr_ce[i].stoich_each_type[ie];
          }
        }
      } else {
      ...
      for (size_t j = 0; j < nelem; j++) {
        conc_cluster(i + 1, j + 1) = stoich(j + 1);
      }
    }
  ...
  void QuasiChemApproxCalculator::readExcessEnergyCluster() {
    ...
    const xvector<double> nrg_ref = aurostd::vector2xvector(aurostd::vectorstring2vectorutype<double>(vinput));
    ...
    if (nelem > tokens.size()) {
      const string message = "fit.out has the wrong output format";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    for (size_t line = 0; line < vinput.size(); line++) {
    ...
    aurostd::string2tokens(vinput[0], tokens, " ");
    if (nelem > tokens.size()) {
      const string message = "predstr.out has the wrong output format";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
    for (size_t line = 0; line < vinput.size(); line++) {
      aurostd::string2tokens(vinput[line], tokens, " ");
  ...
  void QuasiChemApproxCalculator::setCongruentClusters() {
    ...
    for (int i = num_atom_cluster.lrows; i <= num_atom_cluster.urows; i++) {
      if (!(max_num_atoms % num_atom_cluster(i))) {
        index_cluster.push_back(i);
      }
    }
  ...
  void QuasiChemApproxCalculator::calculateDegeneracyCluster() {
    ...
    if (!rundirpath.empty()) {
      ...
      for (int i = tokens.size() - 2; i >= 0; i--) {
        filepath = "/" + tokens[i] + filepath;
      } // well defined path due to how we define rundirpath
    ...
    if (plattice == "fcc") {
      ...
      lattice(1, 3) = 0.0;
      lattice(2, 1) = -0.5;
      lattice(2, 2) = 0.5 * sqrt3;
      lattice(2, 3) = 0.0;
      lattice(3, 1) = 0.0;
      lattice(3, 2) = 0.0;
      lattice(3, 3) = sqrt8 / sqrt3;
      atom.clear();
      atom.fpos(1) = 0.0;
      atom.fpos(2) = 0.0;
      atom.fpos(3) = 0.0;
    ...
    while (xcpocc.increment()) {
      if (aurostd::sum(xcpocc.getCombo()) == max_num_atoms) {
        vpocc.push_back(aurostd::xvector2utype<double>(aurostd::vector2xvector(xcpocc.getCombo())) / (double) max_num_atoms);
      }
    }
    vector<xstructure> vstr_ds;
    vector<xstructure> vstr_u;
    long int sum_calculated = 0;
    long int sum_accepted = 0;
    for (size_t i = 0; i < vpocc.size(); i++) {
      ...
      for (size_t j = 1; j < atoms.size(); j++) {
        if (atoms[j].name != atoms[j - 1].name) {
          itype++;
        }
        atoms[j].type = itype;
      }
      ...
      for (vector<xstructure>::iterator it = vstr_u.begin(); it != vstr_u.end(); it++) {
        vstr_ds.push_back(*it);
      }
    ...
    if (!filepath.empty()) {
      stringstream ss;
      for (int i = degeneracy_cluster.lrows; i <= degeneracy_cluster.urows; i++) {
        ss << degeneracy_cluster(i) << endl;
      }
  ...
  void QuasiChemApproxCalculator::calculateProbabilityIdealCluster() {
    ...
    for (int ix = conc_macro.lrows; ix <= conc_macro.urows; ix++) {
      ...
      prob_ideal_cluster.setmat(prob_ideal_cluster.getxmat(ix, ix, conc_cluster.lrows, conc_cluster.urows) / aurostd::sum(prob_ideal_cluster.getxmat(ix, ix, conc_cluster.lrows, conc_cluster.urows)), ix, 1); // normalize sum to 1
    }
  }
  ...
  void QuasiChemApproxCalculator::calculateProbabilityCluster1D(int iix, const int it) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " it=" << it << " ix=" << ix << " | coeff=" << coeff << endl;
    }
    ...
    for (int isol = rootr.lrows; isol <= rootr.urows; isol++) {
      if (rootr(isol) > soln(1) && aurostd::isequal(rooti(isol), 0.0) && max_num_atoms * std::pow(rootr(isol), max_num_atoms) != INFINITY) { // solution must be positive, real, and finite
        soln(1) = rootr(isol);
        found_soln = true;
      }
    }
  ...
  void QuasiChemApproxCalculator::calculateProbabilityClusterND(int iix, const int it) {
    ...
    vector<std::function<double(xvector<double>)>> vpoly;
    vector<std::function<double(xvector<double>)>> vdpoly;
    vector<vector<std::function<double(xvector<double>)>>> jac;
    bool found_soln = false;
    for (int ieq = 1; ieq <= (int) nelem - 1; ieq++) {
      vpoly.emplace_back([this, it, ix, ieq](xvector<double> xvar) { return getProbabilityConstraint(it, ix, ieq, 0, xvar); });
      for (int ideq = 1; ideq <= (int) nelem - 1; ideq++) {
        vdpoly.emplace_back([this, it, ix, ieq, ideq](xvector<double> xvar) { return getProbabilityConstraint(it, ix, ieq, ideq, xvar); });
      }
      jac.push_back(vdpoly);
      vdpoly.clear();
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " it=" << it << " ix=" << ix << endl;
    }
    found_soln = aurostd::findZeroDeflation(soln0.getcol(ix), vpoly, jac, msoln);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << "   exist=" << found_soln << " | real roots=" << endl << msoln << endl;
    }
...
#else
        for (int iix = 0; iix < prob_ideal_cluster.rows; iix++) {
          calculateProbabilityCluster1D(iix, it);
        }
#endif
...
#else
        for (int iix = 0; iix < prob_ideal_cluster.rows; iix++) {
          calculateProbabilityClusterND(iix, it);
        }
#endif
  ...
  void QuasiChemApproxCalculator::checkProbabilityEquilibrium() const {
    const bool LDEBUG = (false || XHOST.DEBUG);
    double diff = AUROSTD_MAX_DOUBLE;
    double diff_old;
    for (int it = temp.lrows; it <= temp.urows; it++) {
      for (int ix = prob_ideal_cluster.lrows; ix <= prob_ideal_cluster.urows; ix++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " |P - P0| = " << diff << endl;
      }
  ...
  void QuasiChemApproxCalculator::calculateRelativeEntropyEC() {
    ...
    const int n_fit = 8;
    nconc = 1;
    const xmatrix<double> conc_macro_orig = conc_macro; // store original macroscopic concentration
    ...
    const double dtemp = temp(2) - temp(1);
    ...
    const xvector<double> temp_orig = temp; // store original temperature
    ...
    if (order_param.rows > 3) {
      const double d1 = (order_param(3) - order_param(1)) / (temp(3) - temp(1));
      const double d2 = (order_param(4) - order_param(2)) / (temp(4) - temp(2));
      if (std::abs(d1) > std::abs(d2)) {
        low_t_div = true;
      }
    }
    double temp_mean = aurostd::mean(temp);
    double temp_std = aurostd::stddev(temp);
    ...
    const xvector<double> coeff = aurostd::polynomialCurveFit(temp_scaled, order_param, n_fit, wts);
    xvector<double> coeff1 = aurostd::evalPolynomialCoeff(coeff, 1);
    xvector<double> coeff2 = aurostd::evalPolynomialCoeff(coeff, 2);
    ...
    if (low_t_div) {
      ...
      for (int i = dadt.lrows; i <= dadt.urows; i++) {
        dadt_interp(i + 1) = dadt(i);
      } // add Da/DT(0K) = 0
    ...
    xvector<double> rootr(n_fit - 2);
    xvector<double> rooti(n_fit - 2);
  ...
  void QuasiChemApproxCalculator::calculateBinodalCurve() {
    ...
    xvector<double> coeff;
    xvector<double> rootr(n_fit);
    xvector<double> rooti(n_fit);
    const xvector<double> wts = aurostd::ones_xv<double>(temp_npts);
    const double temp_mean = aurostd::mean(temp);
    const double temp_std = aurostd::stddev(temp);
    const xvector<double> temp_scaled = (temp - temp_mean) / temp_std; // scale for numerical stability
  ...
  void QuasiChemApproxCalculator::writeData() {
    const string filepath = rundirpath + "/" + QCA_FILE_PREFIX + "output." + print;
    stringstream output;
    if (print == "txt") {
  ...
  void QuasiChemApproxCalculator::readData() {
    const string filepath = rundirpath + "/" + QCA_FILE_PREFIX + "output.json";
  ...
  void QuasiChemApproxCalculator::plotData() {
    ...
    if (conc_curve) { // generic x-axis for the concentration curve
      ...
      for (int ix = conc_macro.lrows; ix <= conc_macro.urows; ix++) {
        data.push_back({conc_macro(ix, 1), binodal_curve(ix)});
      }
      // START - custom
      gpfile << "# Custom" << endl;
...
namespace qca {
  ...
  ///
  /// @param qca_opts command line options
  ///
  /// @authors
  /// @mod{SD,20220718,created function}
  void quasiChemicalApprox(const aurostd::xoption& qca_opts) {
    if (qca_opts.flag("QCA::USAGE")) {
      if (!qca_opts.flag("QCA::SCREEN_ONLY")) {
        displayUsage();
      }
      return;
  ...
  void displayUsage() {
    ...
    usage_options.emplace_back("aflow --quasi_chem_approx|--qca --plattice=|--plat=fcc --elements=|--elem=Au,Pt[,Zn] [qca_options] [--directory=[DIRECTORY]]");
    usage_options.emplace_back(" ");
    usage_options.emplace_back("qca_options:");
    usage_options.emplace_back(" ");
    usage_options.emplace_back("GENERAL OPTIONS:");
    usage_options.emplace_back("--usage");
    usage_options.emplace_back("--screen_only");
    usage_options.emplace_back("--image_only|--image");
    usage_options.emplace_back("--aflowlib_directory=|--aflowlib_dir=...");
    usage_options.emplace_back("--print=|--p=|--output=|--o=txt");
    usage_options.emplace_back(" ");
    usage_options.emplace_back("BINODAL OPTIONS:");
    usage_options.emplace_back("--binodal");
    usage_options.emplace_back("--use_sg");
    usage_options.emplace_back("--aflow_max_num_atoms=4");
    usage_options.emplace_back("--max_num_atoms=|--mna=8");
    usage_options.emplace_back("--cv_cutoff=|--cv_cut=0.05");
    usage_options.emplace_back("--conc_curve_range=|--conc_curve=0,1,1,0");
    usage_options.emplace_back("--conc_npts=20");
    usage_options.emplace_back("--temp_range=|--temp=300,5000");
    usage_options.emplace_back("--temp_npts=150");
    usage_options.emplace_back(" ");
