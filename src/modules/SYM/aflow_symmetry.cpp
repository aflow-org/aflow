...
namespace SYM {
  double minimumDistance(const xstructure& xstr) {
    ...
    const uint natoms = atoms.size(); // DX20210502
    for (uint i = 0; i < natoms; i++) {
      BringInCellInPlace(atoms[i], xstr.lattice);
    } // DX20210502
...
namespace SYM {
  ...
  double minimumDistance(const deque<_atom>& atoms, const xmatrix<double>& lattice, double scale) {
    ...
    for (int i = 1; i <= 3; i++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " lattice_vector[" << i << "]: min_dist=" << min_dist << endl;
      }
    }
    //}
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " LATTICE VECTORS [1] " << min_dist << endl;
    }
    ...
    vector<int> index;
    for (int a = 1; a <= dim; a++) {
      latt.push_back(a * lattice(1));
      index.push_back(a);
    } // DX calc once and store
    lattice_lengths.push_back(latt);
    latt.clear();
    lattice_indices.push_back(index);
    index.clear();
    for (int b = 1; b <= dim; b++) {
      latt.push_back(b * lattice(2));
      index.push_back(b);
    } // DX calc once and store
    lattice_lengths.push_back(latt);
    latt.clear();
    ...
    for (size_t i = 0; i < lattice_lengths.size() - 1; i++) { // CO20190808 - .size()-1
      for (size_t j = i + 1; j < lattice_lengths.size(); j++) {
        for (size_t a = 0; a < lattice_lengths[i].size(); a++) {
          for (size_t b = 0; b < lattice_lengths[j].size(); b++) {
            min_dist = aurostd::min(min_dist, aurostd::modulus(lattice_lengths[i][a] + lattice_lengths[j][b]));
            min_dist = aurostd::min(min_dist, aurostd::modulus(lattice_lengths[i][a] - lattice_lengths[j][b]));
#if DEBUG_MINIMUM_DISTANCE
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " lattice_vectors: a=" << lattice_indices[i][a] << ",b=" << lattice_indices[j][b] << "; min_dist=" << min_dist << endl;
            }
#endif
          }
        }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " COMBOS OF 2 LATTICE VECTORS [2] " << min_dist << endl;
    }
    ...
    //         min_dist = aurostd::min(min_dist,aurostd::modulus(a*lattice(i)-b*lattice(j)));
    //         if(LDEBUG) {cerr << __AFLOW_FUNC__ << " lattice_vectors: a=" << a << ",b=" << b << "; min_dist=" << min_dist << endl;}
    //       }
    //     }
    //   }
    // }
    // if(LDEBUG) cerr << __AFLOW_FUNC__ << " COMBOS OF 2 LATTICE VECTORS [2] " << min_dist << endl;
    ...
    for (size_t i = 0; i < lattice_lengths.size() - 1; i++) { // CO20190808 - .size()-1
      for (size_t j = i + 1; j < lattice_lengths.size(); j++) {
        for (size_t a = 0; a < lattice_lengths[i].size(); a++) {
          for (size_t b = 0; b < lattice_lengths[j].size(); b++) {
            const xvector<double> added = lattice_lengths[i][a] + lattice_lengths[j][b];
            const xvector<double> subtracted = lattice_lengths[i][a] + lattice_lengths[j][b];
            for (size_t k = j + 1; k < lattice_lengths.size(); k++) {
              for (size_t c = 0; c < lattice_lengths[k].size(); c++) {
...
#if DEBUG_MINIMUM_DISTANCE
                if (LDEBUG) {
                  cerr << __AFLOW_FUNC__ << " lattice_vectors: a=" << lattice_indices[i][a] << ",b=" << lattice_indices[j][b] << ",c=" << lattice_indices[k][c] << "; min_dist=" << min_dist << endl;
                }
#endif
              }
            }
    ...
    //             min_dist = aurostd::min(min_dist,aurostd::modulus(a*lattice(i)+b*lattice(j)-c*lattice(k)));
    //             min_dist = aurostd::min(min_dist,aurostd::modulus(a*lattice(i)-b*lattice(j)+c*lattice(k)));
    //             if(LDEBUG) {cerr << __AFLOW_FUNC__ << " lattice_vectors: a=" << a << ",b=" << b << ",c=" << c << "; min_dist=" << min_dist << endl;}
    ...
    // }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " COMBOS OF 3 LATTICE VECTORS [3] " << min_dist << endl;
    }
    ...
    for (size_t i = 0; i < atoms.size() - 1; i++) { // CO20190808 - .size()-1
      ...
      for (size_t k = i + 1; k < atoms.size(); k++) {
        const xvector<double> incell_dist = atoms[k].cpos - atoms[i].cpos;
...
#if DEBUG_MINIMUM_DISTANCE
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " atoms[" << i << "," << k << "]: a=" << a_index[m] << ",b=" << b_index[n] << ",c=" << c_index[p] << "; min_dist=" << min_dist << "; this_dist=" << aurostd::modulus(tmp) << endl;
              }
#endif
            }
          }
    ...
    //           if(LDEBUG) {cerr << __AFLOW_FUNC__ << " atoms[" << i << "," << k << "]: a=" << a << ",b=" << b << ",c=" << c << "; min_dist=" << min_dist << endl;}
    //         }
    //       }
    //     }
    //   }
    // }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " DIST BETWEEN ATOMS [4] " << min_dist << endl;
    }

    // rescale
    min_dist *= scale;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " RESCALED [5] " << min_dist << endl;
    }
...
namespace SYM {
  double defaultTolerance(const xstructure& xstr) {
    ...
    if (min_dist == AUROSTD_NAN || min_dist == AUROSTD_MAX_DOUBLE) {
      min_dist = SYM::minimumDistance(xstr);
    } // CO20180409 //DX20210615 - add AUROSTD_MAX_DOUBLE
    // if(xstr.dist_nn_min == AUROSTD_NAN){xstr.MinDist();}  //CO20180409
    // min_dist = xstr.dist_nn_min; //CO20180409
    const double tolerance = min_dist / 100.0;
...
namespace SYM {
  bool checkAngle(xvector<double>& v1, xvector<double>& v2, double input_angle, bool& is_deg, double tolerance) { // CO20190520 - removed pointers for bools and doubles, added const where possible
    const double mod_v1 = aurostd::modulus(v1);
    const double mod_v2 = aurostd::modulus(v2);
    const double avg_vec_mod = (mod_v1 + mod_v2) / (2.0);
...
namespace SYM {
  ...
  { // CO20200106 - patching for auto-indenting
    ...
    const bool LDEBUG = (false || XHOST.DEBUG);
    const string directory = xstr.directory; // DX20180426 - added current working directory
    uint cycle_count = 0; // keeps track of number of full cycle iteration
    const double step = 0.05; // logarithmic step size
    const double max_range = 1.0; // scan an order of magnitude in each direction
    double sign = 1.0;
    double orig_range = 0.0;
    double range = 0.0;
    const double change_count_max = 41; // DX20210406

    const double incomming_tolerance = tolerance; // store current tolerance
    ...
    if (orig_range <= max_range && cycle_count <= 20) {
      ...
      if (!no_scan && xstr.sym_eps_change_count <= change_count_max) { // DX20210330 - check cycle count
        if (range <= max_range) {
          ...
          if (LDEBUG) {
            cerr << "SYM::change_tolerance: Inconsistent symmetry at this tolerance " << incomming_tolerance << ", changing tolerance to: " << tolerance << " [str.title=" << xstr.title << "] [dir=" << directory
                 << "]." << endl; // DX20220926
          }
          return true;
        } else {
          cerr << "SYM::change_tolerance WARNING: Inconsistent symmetry, tolerance range (" << std::pow(10.0, (std::log10(orig_tolerance) - (max_range))) << " to "
               << std::pow(10.0, (std::log10(orig_tolerance) + (max_range))) << ") tested [str.title=" << xstr.title << "] [dir=" << directory << "]." << endl; // DX20180426 - changed xstr.directory to directory (pwd)
          // count -=1;
      ...
      // count -=1;
      no_scan = true;
      cerr << "SYM::change_tolerance WARNING: Inconsistent symmetry, but the tolerance scan is suppressed. [str.title=" << xstr.title << "] [dir=" << directory << "]"
           << endl; // DX20180426 - changed xstr.directory to directory (pwd)
      tolerance = orig_tolerance; // DX20170906
      return false;
    }
    // return true;
...
namespace SYM {
  vector<vector<_atom>> break_up_by_type(vector<_atom> expanded_crystal) {
    ...
    while (count < expanded_crystal.size()) {
      for (size_t i = 0; i < expanded_crystal.size(); i++) {
        NEW = true;
        for (size_t j = 0; j < used_types.size(); j++) {
        ...
        if (NEW == true) {
          ...
          for (size_t ii = 0; ii < expanded_crystal.size(); ii++) {
...
namespace SYM {
  ...
  bool AtomsMapped(const _atom& a, const _atom& b, const xmatrix<double>& lattice, const xmatrix<double>& f2c, bool skew, double tol) { // CO20190520 - removed pointers for bools and doubles, added const where possible //DX20190619 - lattice and f2c as input
    if (a.spin_is_given) { // DX20170921 - magnetic symmetry
      const bool spins_match = (aurostd::abs(a.spin - b.spin) <= tol);
      ...
      return a.type == b.type && (aurostd::isequal(a.partial_occupation_value, b.partial_occupation_value, _AFLOW_POCC_ZERO_TOL_)) && spins_match &&
             FPOSMatch(a, b, lattice, f2c, skew, tol); // DX20190619 - lattice and f2c as input, remove "Atom" prefix from name
    } else if (a.noncoll_spin_is_given) { // DX20171205 - magnetic sym (non-collinear)
      const bool spins_match = (aurostd::abs(a.noncoll_spin(1) - b.noncoll_spin(1)) <= tol && aurostd::abs(a.noncoll_spin(2) - b.noncoll_spin(2)) <= tol && aurostd::abs(a.noncoll_spin(3) - b.noncoll_spin(3)) <= tol);
      // return a.type==b.type&&spins_match&&FPOSMatch(a,b,lattice,f2c,skew,tol); //DX20190619 - lattice and f2c as input, remove "Atom" prefix from name
      return a.type == b.type && (aurostd::isequal(a.partial_occupation_value, b.partial_occupation_value, _AFLOW_POCC_ZERO_TOL_)) && spins_match &&
             FPOSMatch(a, b, lattice, f2c, skew, tol); // DX20190619 - lattice and f2c as input, remove "Atom" prefix from name
    } else {
      // return FPOSMatch(a,b,lattice,f2c,skew,tol)&&a.type==b.type; //DX20190619 - lattice and f2c as input, remove "Atom" prefix from name
      return FPOSMatch(a, b, lattice, f2c, skew, tol) && a.type == b.type &&
             (aurostd::isequal(a.partial_occupation_value, b.partial_occupation_value, _AFLOW_POCC_ZERO_TOL_)); // DX20190619 - lattice and f2c as input, remove "Atom" prefix from name
    }
  }
} // namespace SYM
...
namespace SYM {
  bool isLatticeSkewed(const xmatrix<double>& lattice, double& min_dist, double tol) { // CO20190520 - removed pointers for bools and doubles, added const where possible
    const double max_skew = aurostd::max(aurostd::abs(aurostd::scalar_product(lattice(1), lattice(2)) / (aurostd::modulus(lattice(1)) * aurostd::modulus(lattice(2)))),
    ...
    const double skew_tol = (1.0 - aurostd::abs(max_skew)) * min_dist;
...
namespace SYM {
  xvector<double> minimizeDistanceCartesianMethod(const xvector<double>& cpos1, const xvector<double>& cpos2, const xmatrix<double>& lattice, xvector<int>& ijk) {
    ...
    const xvector<double> incell_dist = cpos1 - cpos2;
    xvector<double> min_vec = incell_dist;
    double min_mod = aurostd::modulus(min_vec);

    if (min_mod < _ZERO_TOL_) {
      return min_vec;
    } // DX20210316 - return right away

    // ---------------------------------------------------------------------------
    // determine dimensions to produce uniform sphere; account for skewness
    const double radius = min_mod; // DX20210315 - use distance between atoms, significant speed up for skewed cells (used to be RadiusSphereLattice(lattice); slow)
    xvector<int> dims(3);
    ...
    xvector<double> tmp; // DX20210315 - define outside loop
...
namespace SYM {
  xvector<double> minimizeDistanceFractionalMethod(const xvector<double>& fpos1, const xvector<double>& fpos2) {
    xvector<int> ijk;
    const xvector<double> fdiff = fpos1 - fpos2;
...
namespace SYM {
  ...
  bool FPOSMatch(const deque<_atom>& atom_set, const _atom& atom2, uint& match_type, const xmatrix<double>& lattice, const xmatrix<double>& f2c, bool skew, double tol) { // CO20190520 - removed pointers for bools and doubles, added const where possible //DX20190620 - lattice and f2c as input and remove "Atom" prefix in name
...
namespace SYM {
  ...
  bool FPOSMatch(const _atom& atom1, const _atom& atom2, const xmatrix<double>& lattice, const xmatrix<double>& f2c, bool skew, double tol) { // CO20190520 - removed pointers for bools and doubles, added const where possible //DX20190620 - lattice and f2c as input and remove "Atom" prefix in name
...
namespace SYM {
  ...
  bool FPOSMatch(const xvector<double>& fpos1, const xvector<double>& fpos2, const xmatrix<double>& lattice, const xmatrix<double>& f2c, bool skew, double tol) { // CO20190520 - removed pointers for bools and doubles, added const where possible //DX20190620 - lattice and f2c as input and remove "Atom" prefix in name

    if (XHOST.SKEW_TEST) {
      const xvector<double> min_fdiff = minimizeDistanceFractionalMethod(fpos1, fpos2); // DX20190613
      const double min_fdiff_dist = aurostd::modulus(f2c * min_fdiff);
      const xvector<double> cpos1 = f2c * fpos1;
      const xvector<double> cpos2 = f2c * fpos2;
      const xvector<double> min_cdiff = minimizeDistanceCartesianMethod(cpos1, cpos2, lattice); // DX20190613
      const double min_cdiff_dist = aurostd::modulus(min_cdiff); // DX20190613 - changed variable names
      stringstream message;
      if ((min_cdiff_dist <= tol) == (min_fdiff_dist <= tol) && aurostd::abs(min_cdiff_dist - min_fdiff_dist) < _ZERO_TOL_) {
        message << __AFLOW_FUNC__ << " minimum distances equal, and mappings same -- globally optimized: " << min_cdiff_dist << " | bring-in-cell: " << min_fdiff_dist << " || tol: " << tol
                << " || skew_tol: " << XHOST.SKEW_TOL;
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, cerr, _LOGGER_MESSAGE_);
      }
      if ((min_cdiff_dist <= tol) == (min_fdiff_dist <= tol) && min_cdiff_dist <= tol && aurostd::abs(min_cdiff_dist - min_fdiff_dist) > _ZERO_TOL_) {
        message << __AFLOW_FUNC__ << " WARNING-MAP: minimum distances unequal, but mapping outcome same -- globally optimized: " << min_cdiff_dist << " | bring-in-cell: " << min_fdiff_dist << " || tol: " << tol
                << " || skew_tol: " << XHOST.SKEW_TOL;
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, cerr, _LOGGER_WARNING_);
      }
      if ((min_cdiff_dist <= tol) == (min_fdiff_dist <= tol) && min_cdiff_dist > tol && aurostd::abs(min_cdiff_dist - min_fdiff_dist) > _ZERO_TOL_) {
        message << __AFLOW_FUNC__ << " WARNING-NOMAP: minimum distances unequal, but mapping outcome same -- globally optimized: " << min_cdiff_dist << " | bring-in-cell: " << min_fdiff_dist
                << " || tol: " << tol << " || skew_tol: " << XHOST.SKEW_TOL;
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, cerr, _LOGGER_WARNING_);
      }
      if ((min_cdiff_dist <= tol) != (min_fdiff_dist <= tol) && aurostd::abs(min_cdiff_dist - min_fdiff_dist) > _ZERO_TOL_) {
        message << __AFLOW_FUNC__ << "ERROR: minimum distances unequal, and mappings unequal -- globally optimized: " << min_cdiff_dist << " | bring-in-cell: " << min_fdiff_dist << " || tol: " << tol
                << " || skew_tol: " << XHOST.SKEW_TOL;
      ...
      if ((min_cdiff_dist - XHOST.SKEW_TOL < _ZERO_TOL_) != (min_fdiff_dist - XHOST.SKEW_TOL < _ZERO_TOL_)) {
        message << __AFLOW_FUNC__ << "THRESHOLD ERROR: minimum distances unequal, and mappings unequal -- globally optimized: " << min_cdiff_dist << " | bring-in-cell: " << min_fdiff_dist << " || tol: " << tol
                << " || skew_tol: " << XHOST.SKEW_TOL;
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _GENERIC_ERROR_);
      }
    }
    const double min_dist = aurostd::modulus(CPOSDistFromFPOS(fpos1, fpos2, lattice, f2c, skew)); // DX20190620
...
// ******************************************************************************
namespace SYM {
  xvector<double> FPOSDistFromFPOS(const xvector<double>& fpos1, const xvector<double>& fpos2, const xmatrix<double>& lattice, bool skew) {
    const xmatrix<double> f2c = trasp(lattice);
    const xmatrix<double> c2f = inverse(trasp(lattice));
    return FPOSDistFromFPOS(fpos1, fpos2, lattice, c2f, f2c, skew);
  }
  xvector<double> FPOSDistFromFPOS(const xvector<double>& fpos1, const xvector<double>& fpos2, const xmatrix<double>& lattice, const xmatrix<double>& c2f, const xmatrix<double>& f2c, bool skew) { // CO20190525
    const bool VERBOSE = false; // DX20201210
...
namespace SYM {
  xvector<double> CPOSDistFromFPOS(const xvector<double>& fpos1, const xvector<double>& fpos2, const xmatrix<double>& lattice, bool skew) {
    const xmatrix<double> f2c = trasp(lattice);
    return CPOSDistFromFPOS(fpos1, fpos2, lattice, f2c, skew);
  }
  xvector<double> CPOSDistFromFPOS(const xvector<double>& fpos1, const xvector<double>& fpos2, const xmatrix<double>& lattice, const xmatrix<double>& f2c, bool skew) { // CO20190525
    const bool VERBOSE = false; // using LDEBUG would pollute output
...
namespace SYM {
  ...
  bool validateAtomPosition(const xvector<double>& cpos, const xvector<double>& fpos, const xmatrix<double>& c2f, const xmatrix<double>& f2c, bool skew, double& _eps_) { // CO20190520 - removed pointers for bools and doubles, added const where possible
    double min_dist = 1e9;
    if (skew) {
      const xvector<double> tmp_cpos = f2c * fpos;
      const xmatrix<double> lattice = trasp(f2c); // DX20190613
      const xvector<double> min_cdiff = minimizeDistanceCartesianMethod(cpos, tmp_cpos, lattice); // DX20190613
      min_dist = aurostd::modulus(min_cdiff); // DX20190613
    } else {
      const xvector<double> tmp_fpos = c2f * cpos; // DX20190613
      const xvector<double> min_fdiff = minimizeDistanceFractionalMethod(tmp_fpos, fpos); // DX20190613
...
namespace SYM {
  // overload //DX20190620
  bool MapAtomWithBasis(const vector<_atom>& vec, const _atom& a, bool map_types, deque<uint>& index_to_check, const xmatrix<double>& lattice, bool skew, double tol, uint& mapped_index, bool fast) {
    const xmatrix<double> f2c = trasp(lattice);
    return MapAtomWithBasis(vec, a, map_types, index_to_check, lattice, f2c, skew, tol, mapped_index, fast);
  }

  bool MapAtomWithBasis(const vector<_atom>& vec, const _atom& a, bool map_types, deque<uint>& index_to_check, const xmatrix<double>& lattice, const xmatrix<double>& f2c, bool skew, double tol, uint& mapped_index, bool fast) { // CO20190520 - removed pointers for bools and doubles, added const where possible //DX20190619 - lattice and f2c as input, remove "Atom" prefix from name
    int count = 0;

    if (map_types) {
      for (size_t i = 0; i < index_to_check.size(); i++) {
        ...
        if (a.type == vec[index_to_check[i]].type) { // DX20170731 - Speed increase
          if (AtomsMapped(a, vec[index_to_check[i]], lattice, f2c, skew, tol)) { // type specific //DX20190619 - lattice and f2c as input
            ...
            if (fast) {
              return true;
            } // DX20170731 - Speed increase, check one-to-one after
          }
        }
      }
    } else {
...
namespace SYM {
  // overload //DX20190620
  bool MapAtomWithBasis(const deque<_atom>& deq, const _atom& a, bool map_types, deque<uint>& index_to_check, const xmatrix<double>& lattice, bool skew, double tol, uint& mapped_index, bool fast) {
    const xmatrix<double> f2c = trasp(lattice);
    return MapAtomWithBasis(deq, a, map_types, index_to_check, lattice, f2c, skew, tol, mapped_index, fast);
  }

  bool MapAtomWithBasis(const deque<_atom>& deq, const _atom& a, bool map_types, deque<uint>& index_to_check, const xmatrix<double>& lattice, const xmatrix<double>& f2c, bool skew, double tol, uint& mapped_index, bool fast) { // CO20190520 - removed pointers for bools and doubles, added const where possible //DX20190619 - lattice and f2c as input
    int count = 0;
...
namespace SYM {
  bool CheckForIdentity(const xstructure& xstr) {
    ...
    for (size_t i = 0; i < xstr.pgroup.size(); i++) {
...
namespace SYM {
  bool checkSuperCellLatticePoints(xstructure& xstr, int& num_lattice_points, char& centering, uint& expand_size) {
    const bool skew = isLatticeSkewed(xstr.lattice, xstr.dist_nn_min, xstr.sym_eps);
    ...
    if (num_lattice_points == 4) { // Face centered
      ...
      tmp(1) = 2.0 / 3.0;
      tmp(2) = 1.0 / 3.0;
      tmp(3) = 1.0 / 3.0; // (2/3,1/3,1/3)
      conv_lattice_points.push_back(tmp);
      tmp(1) = 1.0 / 3.0;
      tmp(2) = 2.0 / 3.0;
      tmp(3) = 2.0 / 3.0; // (1/3,2/3,2/3)
      conv_lattice_points.push_back(tmp);
    } else if (num_lattice_points == 2 && centering == 'I') { // Body-centered
      tmp(1) = 0.5;
      tmp(2) = 0.5;
      tmp(3) = 0.5; // (1/2,1/2,1/2)
      conv_lattice_points.push_back(tmp);
    ...
    for (size_t i = 0; i < conv_lattice_points.size(); i++) {
    ...
    for (size_t j = 0; j < compressed_lattice_points.size(); j++) {
      bool matched = false;
      for (size_t ii = 0; ii < xstr.fgroup.size(); ii++) {
        if (aurostd::identical(xstr.fgroup[ii].Uf, xstr.fgroup[0].Uf)) { // DX20171207 use default xmatrix identical tol
          double min_dist = 1e9;
          if (skew) {
            const xvector<double> tmp1 = xstr.f2c * xstr.fgroup[ii].ftau;
            const xvector<double> tmp2 = xstr.f2c * compressed_lattice_points[j];
            const xvector<double> min_cdiff = minimizeDistanceCartesianMethod(tmp1, tmp2, xstr.lattice); // DX20190613
            min_dist = aurostd::modulus(min_cdiff); // DX20190613
          } else {
            const xvector<double> fdiff = xstr.fgroup[ii].ftau - compressed_lattice_points[j]; // DX20190613
            const xvector<double> min_fdiff = minimizeDistanceFractionalMethod(fdiff); // DX20190613
      ...
      if (matched == false) {
        if (num_lattice_points == 3 && !reverse_tested) { // Need to test reverse setting before returning
          ...
          tmp(1) = 1.0 / 3.0;
          tmp(2) = 2.0 / 3.0;
          tmp(3) = 1.0 / 3.0; // (1/3,2/3,1/3)
          conv_lattice_points.push_back(tmp);
          tmp(1) = 2.0 / 3.0;
          tmp(2) = 1.0 / 3.0;
          tmp(3) = 2.0 / 3.0; // (2/3,1/3,2/3)
          conv_lattice_points.push_back(tmp);
          reverse_tested = true;
        } else {
...
namespace SYM {
  bool ComparePointGroupAndSpaceGroupString(xstructure& xstr, int& multiplicity_of_primitive, bool& derivative_structure) {
    ...
    for (size_t i = 0; i < space_group_string.size(); i++) {
      if (i > 0) { // Do not need lattice type
        // Reduce glide to mirror
        if (space_group_string[i] == 'a' || space_group_string[i] == 'b' || space_group_string[i] == 'c' || space_group_string[i] == 'd' || space_group_string[i] == 'e' || space_group_string[i] == 'n') {
          tmp_point_group += "m";
        }
    ...
    if (tmp_point_group == xstr.point_group_Hermann_Mauguin) {
      ...
      for (size_t i = 0; i < tmp_point_group.size(); i++) {
...
namespace SYM {
  // DX+CO START
  bool ApplyAtomValidate(const _atom& atom_in, _atom& atom_out, const _sym_op& symop, const xstructure& a) {
    const bool _incell_ = false;
    const bool roff = true; // CO, hate this, but it was the default, NEVER fudge positions
    return ApplyAtomValidate(atom_in, atom_out, symop, a, _incell_, roff);
  }
  bool ApplyAtomValidate(const _atom& atom_in, _atom& atom_out, const _sym_op& symop, const xstructure& a, bool _incell_, bool roff) {
    ...
    if (min_dist == AUROSTD_NAN) {
      min_dist = SYM::minimumDistance(a);
    }
    // if(a.dist_nn_min == AUROSTD_NAN){a.MinDist();}  //CO20180409
    //   min_dist = a.dist_nn_min; //CO20180409
    const bool skew = isLatticeSkewed(a.lattice, min_dist, _eps_);
    return ApplyAtomValidate(atom_in, atom_out, symop, a.lattice, a.c2f, a.f2c, skew, _incell_, roff, _eps_);
  }
  bool ApplyAtomValidate(const _atom& atom_in, _atom& atom_out, const _sym_op& symop, const xstructure& a, bool skew, bool _incell_, bool roff, double _eps_) {
    return ApplyAtomValidate(atom_in, atom_out, symop, a.lattice, a.c2f, a.f2c, skew, _incell_, roff, _eps_);
  }
  bool ApplyAtomValidate(const _atom& atom_in, _atom& atom_out, const _sym_op& symop, const xmatrix<double>& lattice, const xmatrix<double>& c2f, const xmatrix<double>& f2c, bool skew, bool _incell_, bool roff, double _eps_) {
    const bool LDEBUG = (false || XHOST.DEBUG);
    const bool in_valid = validateAtomPosition(atom_in, c2f, f2c, skew, _eps_);
    if (!in_valid) {
      if (LDEBUG) {
        ...
        cerr << "c2f " << c2f << endl;
        cerr << "f2c " << f2c << endl;
        const xvector<double> fdiff = (c2f * atom_in.cpos) - atom_in.fpos;
    ...
    const bool out_valid = validateAtomPosition(atom_out, c2f, f2c, skew, _eps_);
    if (!out_valid) {
      return false;
    }
    return true;
...
namespace SYM {
  ...
  _atom ApplyAtom(const _atom& atom_in, const _sym_op& symop, const xstructure& str, bool _incell_, bool roff) {
    const bool validatePosition = true;
  ...
  _atom ApplyAtom(const _atom& atom_in, const _sym_op& symop, const xstructure& str, bool _incell_, bool roff, bool validatePosition) {
    ...
    const xmatrix<double> lattice = str.lattice;
    xmatrix<double> f2c(3, 3);
    f2c = str.f2c; // trasp(str.lattice); //  cpos=f2c*fpos;
    xmatrix<double> c2f(3, 3);
    c2f = str.c2f; // inverse(trasp(str.lattice));
    ...
    if (min_dist == AUROSTD_NAN) {
      min_dist = SYM::minimumDistance(str);
    }
    const bool skew = isLatticeSkewed(lattice, min_dist, _eps_);
  ...
  _atom ApplyAtom(const _atom& atom_in, const _sym_op& symop, const xmatrix<double>& lattice, const xmatrix<double>& c2f, const xmatrix<double>& f2c, bool skew, bool _incell_, bool roff, bool validatePosition, double tolerance) {
    ...
    const int check = int(symop.is_pgroup) + int(symop.is_pgroupk) + int(symop.is_fgroup) + int(symop.is_pgroup_xtal) + int(symop.is_pgroupk_xtal) + int(symop.is_sgroup) +
                      int(symop.is_agroup); // DX+CO //DX20191207 - added pgroupk_xtal
    ...
    if (validatePosition) {
      if (!validateAtomPosition(atom, c2f, f2c, skew, tolerance)) {
    ...
    if (symop.is_pgroup == true || symop.is_pgroup_xtal == true || symop.is_pgroupk == true || symop.is_pgroupk_xtal == true || symop.is_agroup == true) { // DX20191207 - added pgroupk_xtal
      ...
      //}
      if (_incell_) {
        atom.ijk = xint(floor(atom.fpos));
      }
      atom.basis = atom_in.basis;
    } else if (symop.is_fgroup == true) {
      // cerr << "factor group symmetry" << endl;
      // if(symop.str_Hermann_Mauguin != "1")
      if (!isidentity(symop.Uf)) { //[CO20200106 - close bracket for indenting]}
      ...
      if (isidentity(symop.Uf) && symop.ftau[1] <= _ZERO_TOL_ && symop.ftau[2] <= _ZERO_TOL_ && symop.ftau[3] <= _ZERO_TOL_) { //[CO20200106 - close bracket for indenting]}
        identity = true;
      }
      ...
      // if(symop.str_Hermann_Mauguin != "1")
      if (!isidentity(symop.Uf)) { // CO20200106 - patching for auto-indenting
        atom.cpos = symop.Uc * atom_in.cpos; // symop.ctau+symop.Uc*atom_in.cpos;if(roff) roundoff(atom.cpos,_EPS_roundoff_);
        atom.fpos = symop.Uf * atom_in.fpos; // symop.ftau+symop.Uf*atom_in.fpos;if(roff) roundoff(atom.fpos,_EPS_roundoff_);
      }
      if (!(symop.ftau[1] <= _ZERO_TOL_ && symop.ftau[2] <= _ZERO_TOL_ && symop.ftau[3] <= _ZERO_TOL_)) {
      ...
      // if(symop.str_Hermann_Mauguin == "1" && symop.ftau[1]<=_ZERO_TOL_ && symop.ftau[2]<=_ZERO_TOL_ && symop.ftau[3]<=_ZERO_TOL_ &&
      if (isidentity(symop.Uf) && symop.ftau[1] <= _ZERO_TOL_ && symop.ftau[2] <= _ZERO_TOL_ && symop.ftau[3] <= _ZERO_TOL_ && symop.ftrasl[1] <= _ZERO_TOL_ && symop.ftrasl[2] <= _ZERO_TOL_ && symop.ftrasl[3] <= _ZERO_TOL_) {
        identity = true;
      }

      if (false && validatePosition) {
        if (!validateAtomPosition(atom, c2f, f2c, skew, tolerance)) {
    ...
    if (roff) {
      roundoff(atom.cpos, _EPS_roundoff_);
    }
    if (roff) {
      roundoff(atom.fpos, _EPS_roundoff_);
    }
...
namespace SYM {
  xvector<double> ApplyCpos(const xvector<double>& cpos_in, const _sym_op& symop, const xstructure& str, bool _incell_) {
    _atom atom;
    atom.cpos = cpos_in;
    atom.fpos = C2F(str.lattice, atom.cpos);
...
namespace SYM {
  xvector<int> ApplyIJK(const xvector<int>& ijk_in, const _sym_op& symop, const xstructure& str) {
    stringstream message;
    xvector<double> fijk(3);
    xvector<double> cijk(3);
    xvector<int> ijk(3);
    const bool roff = true;
...
namespace SYM {
  int ApplyL(const int& l, const _sym_op& symop, const xstructure& str) {
    if (str.lijk_calculated == false) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "str.lijk_calculated must be calculated", _RUNTIME_ERROR_);
    }
...
namespace SYM {
  xstructure ApplyXstructure(const _sym_op& symop, const xstructure& str, bool _incell_) {
    ...
    for (size_t iat = 0; iat < str.atoms.size(); iat++) {
      str_out.atoms.at(iat) = SYM::ApplyAtom(str.atoms[iat], symop, str, _incell_);
    }
    return str_out;
  }
} // namespace SYM
...
namespace SYM {
  bool AtomsEquivalent(xstructure& str, _atom& atom1, _atom& atom2) {
    return AtomsEquivalent(str, atom1, atom2, str.sym_eps);
  }
  ...
  bool AtomsEquivalent(xstructure& str, _atom& atom1, _atom& atom2, double& eps) {
    ...
    if (str.dist_nn_min == AUROSTD_NAN) {
      str.MinDist();
    }
    min_dist = str.dist_nn_min;
    const bool skew = isLatticeSkewed(str.lattice, min_dist, eps);
  ...
  bool AtomsEquivalent(xstructure& str, _atom& a, _atom& b, bool skew, double tol) { // CO20190520 - removed pointers for bools and doubles, added const where possible
    if (!str.pgroup_calculated || !str.fgroup_calculated) { // DX20210111 - removed sgroup
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Symmetry must have been calculated", _RUNTIME_ERROR_);
    }
    _atom tatom;
    for (size_t fg = 0; fg < str.fgroup.size(); fg++) {
...
namespace SYM {
  bool CposEquivalent(const xstructure& str, const xvector<double>& cpos1, const xvector<double>& cpos2, const double& eps) {
    ...
    _atom tatom;
    iatom1.cpos = cpos1;
    iatom1.fpos = C2F(str.lattice, iatom1.cpos);
    iatom2.cpos = cpos2;
    iatom2.fpos = C2F(str.lattice, iatom2.cpos);
    for (size_t sg = 0; sg < str.sgroup.size(); sg++) {
      tatom = SYM::ApplyAtom(iatom2, str.sgroup[sg], str);
      if (modulus(iatom1.cpos - tatom.cpos) < eps) {
        return true;
      }
    }
    return false;
...
namespace SYM {
  bool FposEquivalent(const xstructure& str, const xvector<double>& fpos1, const xvector<double>& fpos2, const double& eps) {
    ...
    _atom tatom;
    iatom1.fpos = fpos1;
    iatom1.cpos = F2C(str.lattice, iatom1.fpos);
    iatom2.fpos = fpos2;
    iatom2.cpos = F2C(str.lattice, iatom2.fpos);
    for (size_t sg = 0; sg < str.sgroup.size(); sg++) {
      tatom = SYM::ApplyAtom(iatom2, str.sgroup[sg], str);
      if (modulus(iatom1.fpos - tatom.fpos) < eps) {
        return true;
      }
    }
    return false;
...
namespace SYM {
  ...
  bool TypePointGroupOperation(const xmatrix<double>& Uc,
                               xmatrix<double>& Uf,
                               string& _string,
                               bool& _inversion,
                               double& _angle,
                               ...
                               xvector<xcomplex<double>>& _su2_coefficients,
                               double _eps_) // DX20180117 - added SU(2) and su(2) coefficients
  { // CO20200106 - patching for auto-indenting
    ...
    const bool _roundoff_U = true;
    const bool _roundoff_rrr = true;
    xmatrix<double> A(3, 3);
    xmatrix<double> U(3, 3);
    xmatrix<double> I(3, 3);
    xmatrix<double> II(3, 3);
    ...
    xvector<double> r(3);
    const xvector<double> ro(3);
    double theta;
    I = aurostd::identity(I);
    II = -I;
    if (_roundoff_U) {
      U = roundoff(U, _EPS_roundoff_);
    }
    //  cerr << " U= " << endl << U << endl;
    // DX20171207 Use Uf, elements are -1,0,1 : if(identical(U,I,_eps_))                                           // check unity  // fix the problem at theta=0.0
    if (identical(Uf, I)) { // CO20200106 - patching for auto-indenting
      theta = 0.0;
      r(1) = 0.0;
      r(2) = 0.0;
      r(3) = 0.0;
      clear(A); // ANGLE IS ZERO and axis is not defined
      inversion = false;
      aus << "unity I       ";
      _inversion = inversion;
      _angle = theta * rad2deg;
      _axis(1) = r(1);
      _axis(2) = r(2);
      _axis(3) = r(3);
      _string = aus.str();
      _generator = A;
      _generator_coefficients(1) = r(1);
      _generator_coefficients(2) = r(2);
      _generator_coefficients(3) = r(3); // DX20171206 - added generator coefficents
    ...
    if (identical(Uf, II)) { // CO20200106 - patching for auto-indenting
      theta = 0.0;
      r(1) = 0.0;
      r(2) = 0.0;
      r(3) = 0.0;
    ...
    if (identical(Uf * Uf, I)) { // CO20200106 - patching for auto-indenting
      double x;
      double y;
      double z;
      x = (U(1, 1) + 1.0) / 2.0; // definition of x
      if (x > 0.0) {
        x = sqrt(x); // can only take square root of positive number, always true for orthogonal matrix
      } else {
        x = 0.0; // in case matrix has become de-orthogonalised
      }
      y = (U(2, 2) + 1.0) / 2.0; // definition of y
      if (y > 0.0) {
        y = sqrt(y); // can only take square root of positive number, always true for orthogonal matrix
      } else {
        y = 0.0; // in case matrix has become de-orthogonalised
      }
      z = (U(3, 3) + 1.0) / 2.0; // definition of z
      if (z > 0.0) {
        z = sqrt(z); // can only take square root of positive number, always true for orthogonal matrix
      } else {
        z = 0.0; // in case matrix has become de-orthogonalised
      }
      if ((std::abs(x) < _eps_) && !(std::abs(y) < _eps_) && !(std::abs(z) < _eps_)) { // implements  last 6 rows of above table
        if (!(U(2, 3) > 0.0)) {
          y = -y;
        }
      } else if ((std::abs(y) < _eps_) && !(std::abs(z) < _eps_)) {
        if (!(U(1, 3) > 0.0)) {
          z = -z;
        }
      } else if ((std::abs(z) < _eps_)) {
        if (!(U(1, 2) > 0.0)) {
          x = -x;
        }
      }
      ...
      if (r(3) < 0.0) {
        r = -r;
        theta = 2 * pi - theta;
      } // theta along r is equivalent to 2pi-theta along -r... try to plot.  CHOICE
      if (_roundoff_rrr) {
        roundoff(r, _EPS_roundoff_);
      }
      if (inversion == false) {
        aus << "rotation      "; // theta = " << (theta*rad2deg<100?" ":"") << theta*rad2deg << "  around r = [" << r << "]";
      }
      if (inversion == true) {
        aus << "rotoinversion "; // theta = " << (theta*rad2deg<100?" ":"") << theta*rad2deg << "  around r = [" << r << "]";
      }
      A(1, 1) = 0;
      A(1, 2) = -z;
      A(1, 3) = y;
...
namespace SYM {
  ...
  //					    xvector<double>& _axis,const xmatrix<double>& _generator)
  bool TypePointGroupOperationInternational(const xmatrix<double>& Uc,
                                            string& _stringHM,
                                            string& _stringSC,
                                            ...
                                            xmatrix<xcomplex<double>>& _SU2_matrix,
                                            xvector<xcomplex<double>>& _su2_coefficients,
                                            double _eps_) // DX20171206 - added generator coefficents //DX20180117 - added SU(2) and su(2) coefficients
  { // CO20200106 - patching for auto-indenting
    ...
    if (isdiagonal(Uc, _eps_)) {
      if (isequal(Uc[1][1], 1.0, _eps_) && isequal(Uc[2][2], 1.0, _eps_) && isequal(Uc[3][3], 1.0, _eps_)) {
        _stringHM = "1";
        _stringSC = "1";
        return true;
      }
    ...
    if (det(Uc) > 0.0) {
      if (isequal(_angle, 60.0, _eps_)) {
        _stringHM = "6";
        _stringSC = "C6";
        return true;
      }
      if (isequal(_angle, 90.0, _eps_)) {
        _stringHM = "4";
        _stringSC = "C4";
        return true;
      }
      if (isequal(_angle, 120.0, _eps_)) {
        _stringHM = "3";
        _stringSC = "C3";
        return true;
      }
      if (isequal(_angle, 180.0, _eps_)) {
        _stringHM = "2";
        _stringSC = "C2";
        return true;
      }
      if (isequal(_angle, 240.0, _eps_)) {
        _stringHM = "3";
        _stringSC = "C3";
        return true;
      }
      if (isequal(_angle, 270.0, _eps_)) {
        _stringHM = "4";
        _stringSC = "C4";
        return true;
      }
      if (isequal(_angle, 300.0, _eps_)) {
        _stringHM = "6";
        _stringSC = "C6";
        return true;
      }
...
bool MapOperations(string s1, string s2) {
  ...
  uint maps = 0;
  for (size_t i1 = 0; i1 < tokens1.size(); i1++) {
    for (size_t i2 = 0; i2 < tokens2.size(); i2++) {
      if (tokens1[i1] != "X" && tokens2[i2] != "X") {
        if (tokens1[i1] == tokens2[i2]) { // nail
          tokens1[i1] = "X";
          tokens2[i2] = "X";
          maps++;
        }
  ...
  //  for(size_t i1=0;i1<tokens1.size();i1++) cerr << tokens1.at(i1) << endl;
  //   for(size_t i1=0;i1<tokens1.size();i1++) cerr << tokens2.at(i1) << endl;
  //   cerr << maps << endl;
  if (maps == tokens1.size()) {
    return true;
  }
  return false;
}
...
namespace SYM {
  bool PointGroupsIdentical(const vector<_sym_op>& vpg1, const vector<_sym_op>& vpg2, double eps, bool is_same_lattice) { // DX20171207 - added is_same_lattice
    ...
    if (is_same_lattice) {
      for (size_t i = 0; i < vpg1.size(); i++) {
        found = false;
        for (size_t j = 0; j < vpg2.size() && !found; j++) {
...
namespace SYM {
  bool CalculateQuaternion(_sym_op& a) {
    ...
    a.quaternion_vector.clear();
    a.quaternion_matrix.clear(); // CO20170706 - clear first
    const xvector<double> quaternion_vector(4); // creating the vector for 4x1
    const xmatrix<double> quaternion_matrix(4, 4); // creating the matrix for 4x4
    ...
    double fourthcomponent; // declaring the variables of the components
...
namespace SYM {
  bool ComplexSU2Rotations(xmatrix<xcomplex<double>>& _SU2_matrix, xvector<xcomplex<double>>& _su2_coefficients, double& theta, xvector<double>& _axis) {
    const bool LDEBUG = (false || XHOST.DEBUG);
    const xcomplex<double> q11(1.0 * cos(theta / 2.0), _axis(3) * sin(theta / 2.0)); // DX20180117 - added SU(2) matrix
    const xcomplex<double> q12(_axis(2) * sin(theta / 2.0), _axis(1) * sin(theta / 2.0)); // DX20180117 - added SU(2) matrix
    const xcomplex<double> q21(-_axis(2) * sin(theta / 2.0), _axis(1) * sin(theta / 2.0)); // DX20180117 - added SU(2) matrix
    const xcomplex<double> q22(1.0 * cos(theta / 2.0), -_axis(3) * sin(theta / 2.0)); // DX20180117 - added SU(2) matrix
    _SU2_matrix(1, 1) = q11;
    _SU2_matrix(1, 2) = q12;
    _SU2_matrix(2, 1) = q21;
    _SU2_matrix(2, 2) = q22; // DX20180117 - added SU(2) matrix
    const xcomplex<double> coefficient1(0.0, _axis(1) / 2.0);
    _su2_coefficients(1) = coefficient1; // DX20180117 - added su(2) coefficients
    const xcomplex<double> coefficient2(0.0, _axis(2) / 2.0);
    _su2_coefficients(2) = coefficient2; // DX20180117 - added su(2) coefficients
    const xcomplex<double> coefficient3(0.0, _axis(3) / 2.0);
    _su2_coefficients(3) = coefficient3; // DX20180117 - added su(2) coefficients

    if (LDEBUG) {
      xmatrix<xcomplex<double>> complex_matrix(2, 2);
      const xcomplex<double> tmp(0.0, _axis(3) / 2.0);
      complex_matrix(1, 1) = theta * tmp;
      const xcomplex<double> tmp1(_axis(2) / 2.0, _axis(1) / 2.0);
      complex_matrix(1, 2) = theta * tmp1;
      const xcomplex<double> tmp2(-_axis(2) / 2.0, _axis(1) / 2.0);
      complex_matrix(2, 1) = theta * tmp2;
      const xcomplex<double> tmp3(0.0, -_axis(3) / 2.0);
      complex_matrix(2, 2) = theta * tmp3;
      const xmatrix<xcomplex<double>> exp_matrix = aurostd::exp(complex_matrix);
...
namespace SYM {
  bool CalculatePointGroupCrystal(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, double _eps_, string format) {
    ...
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUP_CRYSTAL Symmetry: BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);

    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: SYM::CalculatePointGroupCrystal [0]" << endl;
    }
    // this function returns the PointGroup of the Crystal by searching through the Hermann-Mauguin combinations

    bool Krun = true;
    xvector<double> ctau(3);
    xvector<double> ftau(3);
    xvector<double> ctrasl(3);
    xvector<double> ftrasl(3); // translation
    xmatrix<double> Uf(3, 3);
    xmatrix<double> Uc(3, 3); // matrices
    // ---------------------------------------------------------------------------
    ...
    for (size_t i = 0; i < a.atoms.size(); i++) {
      basis_atoms_map[i] = i; // identically map each over each
    }
    std::vector<int> basis_types_map(a.atoms.size()); // will map each on each
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: SYM::CalculatePointGroupCrystal [0d]" << endl;
    }
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: SYM::CalculatePointGroupCrystal a.atoms.size()=" << a.atoms.size() << endl;
    }
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: SYM::CalculatePointGroupCrystal a.species.size()=" << a.species.size() << endl;
    }
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: SYM::CalculatePointGroupCrystal basis_types_map.size()=" << basis_types_map.size() << endl;
    }
    for (size_t i = 0; i < a.atoms.size(); i++) {
      basis_types_map[i] = a.atoms[i].type; // identically map each over each
    }
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: SYM::CalculatePointGroupCrystal [0e]" << endl;
    }
    const string message = "PGROUP_XTAL";

    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: SYM::CalculatePointGroupCrystal [1]" << endl;
    }

    if (a.pgroup_calculated == false) {
      Krun = Krun && SYM::CalculatePointGroup(FileMESSAGE, a, aflags, _write_, osswrite, oss); // NEED POINT GROUP
    }
    if (a.fgroup_calculated == false) {
      Krun = Krun && SYM::CalculateFactorGroup(FileMESSAGE, a, aflags, _write_, osswrite, oss); // NEED FACTOR GROUP
    }

    // this one removes the double matrices so it takes care of non primitive systems
    for (size_t ip = 0; ip < a.fgroup.size(); ip++) {
      clear(ctau);
      clear(ftau);
      clear(ctrasl);
      clear(ftrasl);
      Uf = a.fgroup[ip].Uf;
      Uc = a.fgroup[ip].Uc;
      ftau = a.fgroup[ip].ftau;
      ctau = a.fgroup[ip].ctau;
      bool sym_found = false;
      for (size_t ii = 0; ii < a.pgroup_xtal.size() && !sym_found; ii++) {
    ...
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: SYM::CalculatePointGroupCrystal [2]" << endl;
    }
    ...
    for (size_t kk = 1; kk <= a.pgroup_xtal.size(); kk++) { // shift by 1 to use [kk-1]...
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " ";
      aus << a.pgroup_xtal[kk - 1].str_type << " theta=";
      if (a.pgroup_xtal[kk - 1].angle < 100) {
        aus << " ";
      }
      if (a.pgroup_xtal[kk - 1].angle < 10) {
        aus << " ";
      }
...
namespace SYM {
  bool CalculatePointGroupKPatterson(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, double _eps_, string format) {
    ...
    ostringstream aus;
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUPK_PATTERSON Symmetry: BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    ...
    std::vector<int> basis_types_map(a.atoms.size()); // will map each on each
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: " << __AFLOW_FUNC__ << " [0d]" << endl;
    }
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: " << __AFLOW_FUNC__ << " a.atoms.size()=" << a.atoms.size() << endl;
    }
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: " << __AFLOW_FUNC__ << " a.species.size()=" << a.species.size() << endl;
    }
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: " << __AFLOW_FUNC__ << " basis_types_map.size()=" << basis_types_map.size() << endl;
    }
    for (size_t i = 0; i < a.atoms.size(); i++) {
      basis_types_map[i] = a.atoms[i].type; // identically map each over each
    }
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: " << __AFLOW_FUNC__ << " [0e]" << endl;
    }
    const string message = "PGROUPK_PATTERSON";
    ...
    if (a.pgroup_xtal_calculated == false) {
      Krun = Krun && SYM::CalculatePointGroupCrystal(FileMESSAGE, a, aflags, _write_, osswrite, oss); // NEED POINT GROUP CRYSTAL
    }
    if (a.pgroupk_xtal_calculated == false) {
      Krun = Krun && SYM::CalculatePointGroupKCrystal(FileMESSAGE, a, aflags, _write_, osswrite, oss); // NEED POINT GROUP KCRYSTAL
    }

    const xmatrix<double> inversion_symmetry_matrix = -aurostd::identity((double) 0, 3);

    // ---------------------------------------------------------------------------
    // check if pgroup_xtal contains inversion symmetry already
    if (DEBUG_SYMMETRY) {
      cerr << "DEBUG: " << __AFLOW_FUNC__ << " [2a] Check if pgroup_xtal contains inverison symmetry (i.e., centrosymmetric) " << endl;
    }
    bool contains_inversion = false;
    for (size_t ip = 0; ip < a.pgroupk_xtal.size() && !contains_inversion; ip++) {
    ...
    if (contains_inversion) {
      if (DEBUG_SYMMETRY) {
        cerr << "DEBUG: " << __AFLOW_FUNC__ << " [3a] point group is already centrosymmetric, same as POINT GROUP CRYSTAL " << endl;
      }
      const _sym_op symop;
      for (size_t k = 0; k < a.pgroupk_xtal.size(); k++) {
        SYM::AddSymmetryToStructure(a, a.pgroupk_xtal[k].Uc, a.pgroupk_xtal[k].Uf, a.pgroupk_xtal[k].ctau, a.pgroupk_xtal[k].ftau, a.pgroupk_xtal[k].ctrasl, a.pgroupk_xtal[k].ftrasl,
                                    a.pgroupk_xtal[k].basis_atoms_map, a.pgroupk_xtal[k].basis_types_map, false, _PGROUPK_PATTERSON_);
      }
    } else {
      // ---------------------------------------------------------------------------
      ...
      const uint calculation_mode = 1;

      // ---------------------------------------------------------------------------
      // add inversion symmetry to pgroup_xtal
      if (calculation_mode == 1) {
        if (DEBUG_SYMMETRY) {
          cerr << "DEBUG: " << __AFLOW_FUNC__ << " [3b] calculation_mode: add inversion symmetry to pgroup_xtal (fast, default) " << endl;
        }
        const std::vector<_sym_op> pgroupk_Patterson; // rotations/inversions operations

        // ---------------------------------------------------------------------------
        // create Uc and Uf versions of inverse
        const xmatrix<double> Uc_inv = inversion_symmetry_matrix;
        const xmatrix<double> Uf_inv = inverse(trasp(a.klattice)) * Uc_inv * trasp(a.klattice); // i.e., c2f*Uc_inv*f2c for klattice

        for (size_t ip = 0; ip < a.pgroupk_xtal.size(); ip++) {
          Uf = a.pgroupk_xtal[ip].Uf;
          Uc = a.pgroupk_xtal[ip].Uc;
          bool sym_found = false;
          for (size_t ii = 0; ii < a.pgroupk_Patterson.size() && !sym_found; ii++) {
          ...
          Uf = Uf_inv * a.pgroupk_xtal[ip].Uf;
          Uc = Uc_inv * a.pgroupk_xtal[ip].Uc; // multiply by inversion matrix
          sym_found = false;
          for (size_t ii = 0; ii < a.pgroupk_Patterson.size() && !sym_found; ii++) {
            sym_found = (identical(Uf, a.pgroupk_Patterson[ii].Uf)); // look in all the list of operations  //DX20171207 - Use xmatrix identical eps
          }
      ...
      // calculate symmetry of vector set [TO-DO]
        ...
        if (DEBUG_SYMMETRY) {
          cerr << "DEBUG: " << __AFLOW_FUNC__ << " [3c] calculation_mode: explicitly calculate symmetry of vector set " << endl;
        }
        // TO-DO
      }
    }
    ...
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " " << SEPARATION_LINE_DASH_SHORT << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    for (size_t kk = 1; kk <= a.pgroupk_Patterson.size(); kk++) { // shift by 1 to use [kk-1]...
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " ";
      aus << a.pgroupk_Patterson[kk - 1].str_type << " theta=";
      if (a.pgroupk_Patterson[kk - 1].angle < 100) {
        aus << " ";
      }
      if (a.pgroupk_Patterson[kk - 1].angle < 10) {
        aus << " ";
      }
    ...
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " Symmetry: unique point group operations " << a.pgroupk_Patterson.size() << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " Symmetry: END " << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    if (_write_) {
      Krun = Krun && KBIN_SymmetryWrite(FileMESSAGE, a, aflags, _PGROUPK_PATTERSON_, osswrite, oss, format);
    }
    string pgname;
    string operations;
    const bool point_group_valid = PointGroupMap(a, pgname, operations, _PGROUPK_PATTERSON_);

    // ---------------------------------------------------------------------------
    // check Patterson point group label against Laue label (should be the same)
    const string Laue_symbol = GetLaueLabel(a.point_group_Hermann_Mauguin);
...
namespace SYM {
  bool PointGroupMap(xstructure& a, string& pgname, string& operations, char group) {
    ...
    vops.clear();
    if (group == _PGROUP_) {
      for (size_t kk = 1; kk <= a.pgroup.size(); kk++) { // shift by 1 to use [kk-1]...
    ...
    if (group == _PGROUPK_PATTERSON_) {
      for (size_t kk = 1; kk <= a.pgroupk_Patterson.size(); kk++) { // shift by 1 to use [kk-1]...
        ...
        if (kk < a.pgroupk_Patterson.size()) {
          operations = operations + " ";
        }
      }
    }
    ...
    // TRICLINIC
    pgf.emplace_back("1");
    pgn.emplace_back("1"); // 1
    pgf.emplace_back("1 -1");
    pgn.emplace_back("-1"); // 2
    // MONOCLINIC
    pgf.emplace_back("1 2");
    pgn.emplace_back("2"); // 3-5
    pgf.emplace_back("1 m");
    pgn.emplace_back("m"); // 6-9
    pgf.emplace_back("1 2 m -1");
    pgn.emplace_back("2/m"); // 10-15
    // ORTHORHOMBIC
    pgf.emplace_back("1 2 2 2");
    pgn.emplace_back("222"); // 16-24
    pgf.emplace_back("1 m m 2");
    pgn.emplace_back("mm2"); // 25-46
    pgf.emplace_back("1 m m 2 m 2 2 -1");
    pgn.emplace_back("mmm"); // 47-74
    // TETRAGONAL
    pgf.emplace_back("1 4 2 4");
    pgn.emplace_back("4"); // 75-80
    pgf.emplace_back("1 2 -4 -4");
    pgn.emplace_back("-4"); // 81-82
    pgf.emplace_back("1 4 2 4 m -4 -1 -4");
    pgn.emplace_back("4/m"); // 83-88
    pgf.emplace_back("1 4 2 4 2 2 2 2");
    pgn.emplace_back("422"); // 89-98
    pgf.emplace_back("1 m -2 4 m 2 4 -2");
    pgn.emplace_back("4mm"); // 99-110
    pgf.emplace_back("1 m 2 -4 m 2 -4 2");
    pgn.emplace_back("-42m"); // 111-122
    pgf.emplace_back("1 m -2 4 m 2 4 -2 m 2 2 -4 2 -1 -4 2");
    pgn.emplace_back("4/mmm"); // 123-142
    // HEXAGONAL-TRIGONAL
    pgf.emplace_back("1 3 3");
    pgn.emplace_back("3"); // 143-146
    pgf.emplace_back("1 3 3 -1 -3 -3");
    pgn.emplace_back("-3"); // 147-148
    pgf.emplace_back("1 2 2 3 3 2");
    pgn.emplace_back("32"); // 149-155
    pgf.emplace_back("1 3 3 m -2 -2");
    pgn.emplace_back("3m"); // 156-161
    pgf.emplace_back("1 m -2 3 3 -2 -1 2 2 -3 -3 2");
    pgn.emplace_back("-3m"); // 162-167

    // HEXAGONAL-HEXAGONAL
    pgf.emplace_back("1 3 3 2 6 6");
    pgn.emplace_back("6"); // 168-173
    pgf.emplace_back("1 3 3 m -6 -6");
    pgn.emplace_back("-6"); // 174
    pgf.emplace_back("1 3 3 2 6 6 m -6 -6 -1 -3 -3");
    pgn.emplace_back("6/m"); // 175-176
    pgf.emplace_back("1 3 3 2 6 6 2 2 2 2 2 2");
    pgn.emplace_back("622"); // 177-182
    pgf.emplace_back("1 -2 m 3 3 -2 2 -2 m 6 6 -2");
    pgn.emplace_back("6mm"); // 183-186
    pgf.emplace_back("1 m -2 2 m 2 3 -6 3 -6 -2 2");
    pgn.emplace_back("-6m2"); // 187-190
    pgf.emplace_back("1 m -2 2 m 2 3 -6 3 -6 -2 2 2 -1 -2 2 m 2 6 -3 6 -3 -2 2");
    pgn.emplace_back("6/mmm"); // 	191-194
    // CUBIC
    pgf.emplace_back("1 2 3 3 3 3 2 2 3 3 3 3");
    pgn.emplace_back("23"); //  195-199
    pgf.emplace_back("1 3 3 2 3 3 3 3 2 3 3 2 -1 -3 -3 m -3 -3 -3 -3 m -3 -3 m");
    pgn.emplace_back("m-3"); // 200-206
    pgf.emplace_back("1 4 2 4 2 3 4 3 3 4 3 2 2 2 2 2 4 3 2 3 3 4 3 2");
    pgn.emplace_back("432"); // 207-214
    pgf.emplace_back("1 -2 -2 3 3 -2 2 -2 -4 3 3 -4 -4 3 2 -4 3 -4 -4 2 -2 3 3 -2");
    pgn.emplace_back("-43m"); // 215-220
    pgf.emplace_back("1 -2 2 m -2 3 -3 4 3 -2 4 -3 2 -2 -1 2 -3 -3 -2 3 4 4 3 -2 -3 -4 3 2 2 3 -4 -3 2 -4 -3 3 4 m 2 -4 -3 2 3 -4 m 4 -4 2");
    pgn.emplace_back("m-3m"); // 221-230
    ...
    // done
    for (size_t i = 0; i < pgf.size(); i++) {
      aurostd::StringSubstInPlace(pgf[i], "-2", "m"); // -2 are mirrors orthogonal to the axis....
    }

    // -------------------------------------------------------------------- scanning  ORDER=48
    // find point group
    aurostd::StringSubstInPlace(operations, "-2", "m");

    for (size_t i = 0; i < pgf.size() && !pg_found; i++) {
      if (MapOperations(operations, pgf[i])) {
        pg_found = true;
        pgname = pgn.at(i);
      }
    }
...
namespace SYM {
  bool PointGroupLookUpTable(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, string format) {
    ostringstream aus;
    const bool LDEBUG = (false || XHOST.DEBUG || DEBUG_SYMMETRY);
    if (LDEBUG) {
    ...
    a.crystal_family = "";
    a.crystal_system = "";
    a.point_group_crystal_class = "";
    a.point_group_Shoenflies = "";
    a.point_group_Hermann_Mauguin = "";
    a.point_group_orbifold = "";
    a.point_group_type = "";
    a.point_group_order = "";
    a.point_group_structure = "";
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " point group symbol: " << pgname << endl;
    } // DX20210327
    // -------------------------------------------------------------------- scanning  ORDER=48
    if (pgname == "m-3m") {
      pg_found = true;
      a.crystal_family = "cubic";
      a.crystal_system = "cubic";
      a.point_group_crystal_class = "hexoctahedral";
      a.point_group_Shoenflies = "O_h";
      a.point_group_Hermann_Mauguin = "m-3m";
      a.point_group_orbifold = "*432";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "48";
      a.point_group_structure = "2 x symmetric";
    }
    // -------------------------------------------------------------------- scanning  ORDER=24
    if (pgname == "-43m") {
      pg_found = true;
      a.crystal_family = "cubic";
      a.crystal_system = "cubic";
      a.point_group_crystal_class = "tetrahedral";
      a.point_group_Shoenflies = "T_d";
      a.point_group_Hermann_Mauguin = "-43m";
      a.point_group_orbifold = "*332";
      a.point_group_type = "none";
      a.point_group_order = "24";
      a.point_group_structure = "symmetric"; // DX20180827 - point group type changed from - to none
    }
    if (pgname == "432") {
      pg_found = true;
      a.crystal_family = "cubic";
      a.crystal_system = "cubic";
      a.point_group_crystal_class = "gyroidal";
      a.point_group_Shoenflies = "O";
      a.point_group_Hermann_Mauguin = "432";
      a.point_group_orbifold = "432";
      a.point_group_type = "enantiomorphic";
      a.point_group_order = "24";
      a.point_group_structure = "symmetric";
    }
    if (pgname == "m-3") {
      pg_found = true;
      a.crystal_family = "cubic";
      a.crystal_system = "cubic";
      a.point_group_crystal_class = "diploidal";
      a.point_group_Shoenflies = "T_h";
      a.point_group_Hermann_Mauguin = "m-3";
      a.point_group_orbifold = "3*2";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "24";
      a.point_group_structure = "2 x alternating";
    }
    if (pgname == "6/mmm") {
      pg_found = true;
      a.crystal_family = "hexagonal";
      a.crystal_system = "hexagonal";
      a.point_group_crystal_class = "dihexagonal-dipyramidal";
      a.point_group_Shoenflies = "D_6h";
      a.point_group_Hermann_Mauguin = "6/mmm";
      a.point_group_orbifold = "*622";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "24";
      a.point_group_structure = "2 x dihedral";
    }
    // -------------------------------------------------------------------- scanning  ORDER=16
    if (pgname == "4/mmm") {
      pg_found = true;
      a.crystal_family = "tetragonal";
      a.crystal_system = "tetragonal";
      a.point_group_crystal_class = "ditetragonal-dipyramidal";
      a.point_group_Shoenflies = "D_4h";
      a.point_group_Hermann_Mauguin = "4/mmm";
      a.point_group_orbifold = "*422";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "16";
      a.point_group_structure = "2 x dihedral";
    }
    // -------------------------------------------------------------------- scanning  ORDER=12
    if (pgname == "23") {
      ...
      a.crystal_system = "cubic";
      a.point_group_crystal_class = "tetrahedral";
      a.point_group_Shoenflies = "T";
      a.point_group_Hermann_Mauguin = "23";
      a.point_group_orbifold = "332";
      a.point_group_type = "enantiomorphic";
      a.point_group_order = "12";
      a.point_group_structure = "alternating";
    }
    if (pgname == "-6m2" || pgname == "-62m") {
      pg_found = true;
      a.crystal_family = "hexagonal";
      a.crystal_system = "hexagonal";
      a.point_group_crystal_class = "ditrigonal-dipyramidal";
      a.point_group_Shoenflies = "D_3h";
      a.point_group_Hermann_Mauguin = "-6m2";
      a.point_group_orbifold = "*322";
      a.point_group_type = "none";
      a.point_group_order = "12";
      a.point_group_structure = "dihedral"; // DX20180827 - point group type changed from - to none
    }
    if (pgname == "6mm") {
      pg_found = true;
      a.crystal_family = "hexagonal";
      a.crystal_system = "hexagonal";
      a.point_group_crystal_class = "dihexagonal-pyramidal";
      a.point_group_Shoenflies = "C_6v";
      a.point_group_Hermann_Mauguin = "6mm";
      a.point_group_orbifold = "*66";
      a.point_group_type = "polar";
      a.point_group_order = "12";
      a.point_group_structure = "dihedral";
    }
    if (pgname == "622") {
      pg_found = true;
      a.crystal_family = "hexagonal";
      a.crystal_system = "hexagonal";
      a.point_group_crystal_class = "hexagonal-trapezoidal";
      a.point_group_Shoenflies = "D_6";
      a.point_group_Hermann_Mauguin = "622";
      a.point_group_orbifold = "622";
      a.point_group_type = "enantiomorphic";
      a.point_group_order = "12";
      a.point_group_structure = "dihedral";
    ...
    if (pgname == "6/m") {
      ...
      a.crystal_family = "hexagonal";
      a.crystal_system = "hexagonal";
      a.point_group_crystal_class = "hexagonal-dipyramidal";
      a.point_group_Shoenflies = "C_6h";
      a.point_group_Hermann_Mauguin = "6/m";
      a.point_group_orbifold = "6*";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "12";
      a.point_group_structure = "2 × cyclic";
    }
    if (pgname == "-3m" || pgname == "-3m1" || pgname == "-31m") {
      pg_found = true;
      a.crystal_family = "hexagonal";
      a.crystal_system = "trigonal";
      a.point_group_crystal_class = "ditrigonal-scalahedral";
      a.point_group_Shoenflies = "D_3d";
      a.point_group_Hermann_Mauguin = "-3m";
      a.point_group_orbifold = "2*3";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "12";
      a.point_group_structure = "dihedral";
    }
    ...
    if (pgname == "-42m" || pgname == "-4m2") {
      ...
      a.crystal_family = "tetragonal";
      a.crystal_system = "tetragonal";
      a.point_group_crystal_class = "tetragonal-scalenoidal";
      a.point_group_Shoenflies = "D_2d";
      a.point_group_Hermann_Mauguin = "-42m";
      a.point_group_orbifold = "2*2";
      a.point_group_type = "none";
      a.point_group_order = "8";
      a.point_group_structure = "dihedral"; // DX20180827 - point group type changed from - to none
    }
    if (pgname == "4mm") {
      pg_found = true;
      a.crystal_family = "tetragonal";
      a.crystal_system = "tetragonal";
      a.point_group_crystal_class = "ditetragonal-pyramidal";
      a.point_group_Shoenflies = "C_4v";
      a.point_group_Hermann_Mauguin = "4mm";
      a.point_group_orbifold = "*44";
      a.point_group_type = "polar";
      a.point_group_order = "8";
      a.point_group_structure = "dihedral";
    }
    if (pgname == "422") {
      ...
      a.crystal_family = "tetragonal";
      a.crystal_system = "tetragonal";
      a.point_group_crystal_class = "tetragonal-trapezoidal";
      a.point_group_Shoenflies = "D_4";
      a.point_group_Hermann_Mauguin = "422";
      a.point_group_orbifold = "422";
      a.point_group_type = "enantiomorphic";
      a.point_group_order = "8";
      a.point_group_structure = "dihedral";
    ...
    if (pgname == "4/m") {
      ...
      a.crystal_family = "tetragonal";
      a.crystal_system = "tetragonal";
      a.point_group_crystal_class = "tetragonal-dipyramidal";
      a.point_group_Shoenflies = "C_4h";
      a.point_group_Hermann_Mauguin = "4/m";
      a.point_group_orbifold = "4*";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "8";
      a.point_group_structure = "2 x cyclic";
    }
    if (pgname == "mmm") { //
      pg_found = true;
      a.crystal_family = "orthorhombic";
      a.crystal_system = "orthorhombic";
      a.point_group_crystal_class = "orthorhombic-bipyramidal";
      a.point_group_Shoenflies = "D_2h";
      a.point_group_Hermann_Mauguin = "mmm";
      a.point_group_orbifold = "*222";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "8";
      a.point_group_structure = "2 x dihedral";
    }
    ...
    if (pgname == "-6") {
      ...
      a.crystal_family = "hexagonal";
      a.crystal_system = "hexagonal";
      a.point_group_crystal_class = "trigonal-dipyramidal";
      a.point_group_Shoenflies = "C_3h";
      a.point_group_Hermann_Mauguin = "-6";
      a.point_group_orbifold = "3*";
      a.point_group_type = "none";
      a.point_group_order = "6";
      a.point_group_structure = "cyclic"; // DX20180827 - point group type changed from - to none
    }
    if (pgname == "6") {
      pg_found = true;
      a.crystal_family = "hexagonal";
      a.crystal_system = "hexagonal";
      a.point_group_crystal_class = "hexagonal-pyramidal";
      a.point_group_Shoenflies = "C_6";
      a.point_group_Hermann_Mauguin = "6";
      a.point_group_orbifold = "66";
      a.point_group_type = "enantiomorphic polar";
      a.point_group_order = "6";
      a.point_group_structure = "cyclic";
    }
    if (pgname == "3m" || pgname == "3m1" || pgname == "31m") {
      pg_found = true;
      a.crystal_family = "hexagonal";
      a.crystal_system = "trigonal";
      a.point_group_crystal_class = "ditrigonal-pyramidal";
      a.point_group_Shoenflies = "C_3v";
      a.point_group_Hermann_Mauguin = "3m";
      a.point_group_orbifold = "*33";
      a.point_group_type = "polar";
      a.point_group_order = "6";
      a.point_group_structure = "dihedral";
    }
    if (pgname == "32" || pgname == "321" || pgname == "312") {
      ...
      a.crystal_family = "hexagonal";
      a.crystal_system = "trigonal";
      a.point_group_crystal_class = "trigonal-trapezoidal";
      a.point_group_Shoenflies = "D_3";
      a.point_group_Hermann_Mauguin = "32";
      a.point_group_orbifold = "322";
      a.point_group_type = "enantiomorphic";
      a.point_group_order = "6";
      a.point_group_structure = "dihedral";
    ...
    if (pgname == "-3") {
      ...
      a.crystal_family = "hexagonal";
      a.crystal_system = "trigonal";
      a.point_group_crystal_class = "rhombohedral";
      a.point_group_Shoenflies = "S_6";
      a.point_group_Hermann_Mauguin = "-3";
      a.point_group_orbifold = "3x";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "6";
      a.point_group_structure = "cyclic";
    }
    ...
    if (pgname == "-4") {
      ...
      a.crystal_family = "tetragonal";
      a.crystal_system = "tetragonal";
      a.point_group_crystal_class = "tetragonal-disphenoidal";
      a.point_group_Shoenflies = "S_4";
      a.point_group_Hermann_Mauguin = "-4";
      a.point_group_orbifold = "2x";
      a.point_group_type = "none";
      a.point_group_order = "4";
      a.point_group_structure = "cyclic"; // DX20180827 - point group type changed from - to none
    }
    if (pgname == "4") {
      pg_found = true;
      a.crystal_family = "tetragonal";
      a.crystal_system = "tetragonal";
      a.point_group_crystal_class = "tetragonal-pyramidal";
      a.point_group_Shoenflies = "C_4";
      a.point_group_Hermann_Mauguin = "4";
      a.point_group_orbifold = "44";
      a.point_group_type = "enantiomorphic polar";
      a.point_group_order = "4";
    ...
    if (pgname == "mm2") {
      ...
      a.crystal_family = "orthorhombic";
      a.crystal_system = "orthorhombic";
      a.point_group_crystal_class = "orthorhombic-pyramidal";
      a.point_group_Shoenflies = "C_2v";
      a.point_group_Hermann_Mauguin = "mm2";
      a.point_group_orbifold = "*22";
      a.point_group_type = "polar";
      a.point_group_order = "4";
      a.point_group_structure = "dihedral";
    }
    if (pgname == "222") {
      ...
      a.crystal_family = "orthorhombic";
      a.crystal_system = "orthorhombic";
      a.point_group_crystal_class = "orthorhombic-sphenoidal";
      a.point_group_Shoenflies = "D_2";
      a.point_group_Hermann_Mauguin = "222";
      a.point_group_orbifold = "222";
      a.point_group_type = "enantiomorphic";
      a.point_group_order = "4";
      a.point_group_structure = "dihedral";
    ...
    if (pgname == "2/m") {
      pg_found = true;
      a.crystal_family = "monoclinic";
      a.crystal_system = "monoclinic";
      a.point_group_crystal_class = "monoclinic-prismatic";
      a.point_group_Shoenflies = "C_2h";
      a.point_group_Hermann_Mauguin = "2/m";
      a.point_group_orbifold = "2*";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "4";
      a.point_group_structure = "2 x cyclic";
    }
    ...
    if (pgname == "3") {
      ...
      a.crystal_family = "hexagonal";
      a.crystal_system = "trigonal";
      a.point_group_crystal_class = "trigonal-pyramidal";
      a.point_group_Shoenflies = "C_3";
      a.point_group_Hermann_Mauguin = "3";
      a.point_group_orbifold = "33";
      a.point_group_type = "enantiomorphic polar";
      a.point_group_order = "3";
      a.point_group_structure = "cyclic";
    }
    // -------------------------------------------------------------------- scanning  ORDER=2
    if (pgname == "m") {
      ...
      a.crystal_family = "monoclinic";
      a.crystal_system = "monoclinic";
      a.point_group_crystal_class = "monoclinic-domatic";
      a.point_group_Shoenflies = "C_S";
      a.point_group_Hermann_Mauguin = "m";
      a.point_group_orbifold = "1*";
      a.point_group_type = "polar";
      a.point_group_order = "2";
      a.point_group_structure = "cyclic";
    }
    if (pgname == "2") {
      pg_found = true;
      a.crystal_family = "monoclinic";
      a.crystal_system = "monoclinic";
      a.point_group_crystal_class = "monoclinic-sphenoidal";
      a.point_group_Shoenflies = "C_2";
      a.point_group_Hermann_Mauguin = "2";
      a.point_group_orbifold = "22";
      a.point_group_type = "enantiomorphic polar";
      a.point_group_order = "2";
      a.point_group_structure = "cyclic";
    ...
    if (pgname == "-1") {
      pg_found = true;
      a.crystal_family = "triclinic";
      a.crystal_system = "triclinic";
      a.point_group_crystal_class = "triclinic-pinacoidal";
      a.point_group_Shoenflies = "C_i";
      a.point_group_Hermann_Mauguin = "-1";
      a.point_group_orbifold = "x";
      a.point_group_type = "centrosymmetric";
      a.point_group_order = "2";
      a.point_group_structure = "cyclic";
    }
    ...
    if (pgname == "1") {
      ...
      a.crystal_family = "triclinic";
      a.crystal_system = "triclinic";
      a.point_group_crystal_class = "triclinic-pedial";
      a.point_group_Shoenflies = "C_1";
      a.point_group_Hermann_Mauguin = "1";
      a.point_group_orbifold = "11";
      a.point_group_type = "enantiomorphic polar";
      a.point_group_order = "1";
      a.point_group_structure = "trivial";
    }

    // ------------------------------------------------------------------------------------------- PRINTING AND LEAVING
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " " << SEPARATION_LINE_DASH_SHORT << endl;
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " Symmetry: unique point group operations " << a.pgroup_xtal.size() << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    ...
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    if (_write_) {
      Krun = Krun && KBIN_SymmetryWrite(FileMESSAGE, a, aflags, _PGROUP_XTAL_, osswrite, oss, format);
    }
    return pg_found;
  }
} // namespace SYM
...
namespace SYM {
  uint AddSymmetryToStructure(xstructure& a,
                              const uint& iat,
                              const xmatrix<double>& Uc,
                              const xmatrix<double>& Uf,
                              const xvector<double>& ctau,
                              const xvector<double>& ftau,
                              const xvector<double>& ctrasl,
  ...
  uint AddSymmetryToStructure(xstructure& a,
                              ...
                              bool roff) {
    // DX+CO END
    xmatrix<double> _Uc(3, 3);
    _Uc = Uc; // here we make copies so we do not mess up input
    xmatrix<double> _Uf(3, 3);
    _Uf = Uf; // here we make copies so we do not mess up input
    string _string;
    string _stringHM;
    string _stringSC;
    bool _inversion;
    ...
    for (size_t i = 0; i < basis_atoms_map.size(); i++) {
      symop.basis_atoms_map.push_back(basis_atoms_map[i]);
    }
    symop.basis_types_map.clear(); // empty
    for (size_t i = 0; i < basis_types_map.size(); i++) {
      symop.basis_types_map.push_back(basis_types_map[i]);
    }
    // DX+CO START
    symop.basis_map_calculated = basis_map_calculated;
    // DX+CO END
    if (group == _PGROUP_ || group == _PGROUP_XTAL_ || group == _FGROUP_ || group == _SGROUP_ || group == _AGROUP_ || group == _PGROUPK_ || group == _PGROUPK_XTAL_ ||
        group == _PGROUPK_PATTERSON_) { // DX20171205 - Added pgroupk_xtal //DX20200129 - added Patterson symmetry
      // DX+CO START
      if (roff) {
        _Uc = roundoff(_Uc, _EPS_roundoff_);
        _Uf = roundoff(_Uf, _EPS_roundoff_);
      } // Uc cleanup from roundoff errors
      if (roff) {
        roundoff(ctau, _EPS_roundoff_);
        roundoff(ftau, _EPS_roundoff_);
      }
      if (roff) {
        roundoff(ctrasl, _EPS_roundoff_);
        roundoff(ftrasl, _EPS_roundoff_);
      }
      // DX+CO END
      symop.Uc = _Uc;
      symop.Uf = _Uf;
      ...
      symop.ctrasl = ctrasl;
      symop.ftrasl = ftrasl;
      SYM::TypePointGroupOperation(_Uc, _Uf, _string, _inversion, _angle, _axis, _generator, _generator_coefficients, _SU2_matrix, _su2_coefficients,
                                   a.sym_eps); // extract information in //DX added eps //DX20171206 - generator coefficients //DX20171207 - added Uf //DX20180117 - add SU(2) and su(2) coefficients
      SYM::TypePointGroupOperationInternational(_Uc, _stringHM, _stringSC, _inversion, _angle, _axis, _generator, _generator_coefficients, _SU2_matrix, _su2_coefficients,
                                                a.sym_eps); // extract information //DX added eps //DX20171206 - generator coefficients //DX20180117 - add SU(2) and su(2) coefficients
      symop.generator = _generator;
      symop.generator_coefficients = _generator_coefficients;
      symop.angle = _angle;
      symop.axis = _axis; // DX20171206 - generator coefficients
      symop.SU2_matrix = _SU2_matrix;
      symop.su2_coefficients = _su2_coefficients; // DX20180117 - add SU(2) and su(2) coefficients
      symop.str_type = _string;
      symop.flag_inversion = _inversion;
    ...
    if (group == _PGROUP_) {
      ...
      clear(symop.ftau);
      clear(symop.ctrasl);
      clear(symop.ftrasl); // no translation on point group
      symop.is_pgroup = true;
      symop.is_fgroup = false;
      symop.is_sgroup = false;
      symop.is_agroup = false;
      symop.is_pgroupk = false;
      symop.is_pgroup_xtal = false;
      symop.is_pgroupk_xtal = false;
      symop.is_pgroupk_Patterson = false;
    ...
    if (group == _PGROUP_XTAL_) {
      ...
      clear(symop.ctrasl);
      clear(symop.ftrasl); // no translation on point group
      symop.is_pgroup = false;
      symop.is_fgroup = false;
      symop.is_sgroup = false;
      symop.is_agroup = false;
      symop.is_pgroupk = false;
      symop.is_pgroup_xtal = true;
      symop.is_pgroupk_xtal = false;
      symop.is_pgroupk_Patterson = false;
    ...
    if (group == _PGROUPK_PATTERSON_) { // DX20200129
      ...
      symop.is_pgroup_xtal = false;
      symop.is_pgroupk_xtal = false;
      symop.is_pgroupk_Patterson = true;
    ...
    if (group == _AGROUP_) {
      ...
      clear(symop.ftau);
      clear(symop.ctrasl);
      clear(symop.ftrasl); // no translation on site point group
      symop.is_pgroup = false;
      symop.is_fgroup = false;
      symop.is_sgroup = false;
      symop.is_agroup = true;
      symop.is_pgroupk = false;
      symop.is_pgroup_xtal = false;
      symop.is_pgroupk_xtal = false;
      symop.is_pgroupk_Patterson = false;
...
namespace SYM {
  ...
  uint AddSymmetryToStructure(
      xstructure& a, const uint& iat, const xmatrix<double>& Uc, const xmatrix<double>& Uf, const std::vector<int>& basis_atoms_map, const std::vector<int>& basis_types_map, bool basis_map_calculated, char group, bool roff) {
    xvector<double> xxx(3);
    xxx.clear();
...
namespace SYM {
  bool CalculatePointGroup(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, double _eps_, string format) {
    ...
    if (DEBUG_SYMMETRY) {
      cerr << __AFLOW_FUNC__ << " DEBUG" << endl;
    }
    // ------------------------------------------------------------------------------
    ...
    if (a.title == "KLATTICE") {
      pgroup_type = "PGROUP_KLATTICE";
    }
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << pgroup_type << " Symmetry: BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    // generic variables
    ...
    xvector<double> ddd(3); // for printing
    // ---------------------------------------------------------------------------
    // create placeholder for atoms_map for AddSymmetryToStructure() (no atom mappings for pgroups)
    std::vector<int> basis_atoms_map(a.atoms.size()); // will map each on each
    for (size_t i = 0; i < a.atoms.size(); i++) {
    ...
    xmatrix<double> temp_clattice(3, 3);
    xmatrix<double> temp_flattice(3, 3);
...
#endif
    xmatrix<double> Act(3, 3);
    xmatrix<double> Bct(3, 3);
    xmatrix<double> Uc(3, 3); // for generation of pgroup
    xmatrix<double> Adt(3, 3);
    xmatrix<double> Bdt(3, 3);
    xmatrix<double> Uf(3, 3); // for generation of pgroup_ijk
    xvector<double> a1(3);
    xvector<double> a2(3);
    xvector<double> a3(3); // lattice vectors
    xvector<double> ctau(3);
    xvector<double> ftau(3);
    xvector<double> ctrasl(3);
    xvector<double> ftrasl(3); // translation
    ...
    xvector<double> clatticedata(6);
    xvector<double> temp_clatticedata(6); // triplet position
    clatticedata = _Getabc_angles(lattice, DEGREES);
    vector<xvector<double>*> grid_clattice;
    xvector<double>* grid_clattice_ptr; // grid for rrr points
    vector<xvector<double>*> grid_flattice;
    xvector<double>* grid_flattice_ptr; // grid for ijk points
    double na1;
    double na2;
    double na3;
    double amin;
    double latticevol;
    double temp_clatticevol;
    ...
    //}
    // DX ===
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << pgroup_type << " Symmetry: inside sphere, dimensions = [" << dims[1] << "," << dims[2] << "," << dims[3] << "]   radius=" << radius << "  "
          ...
          if (!(i == 0 && j == 0 && k == 0)) {
            if ((!(i == 1 && j == 0 && k == 0)) && (!(i == 0 && j == 1 && k == 0)) && (!(i == 0 && j == 0 && k == 1))) { // to avoid plugging a1,a2,a2 back
              ...
              if (modulus(rrr) < radius &&
                  ...
                  (aurostd::abs(modulus(rrr) - na1) < _eps_ || aurostd::abs(modulus(rrr) - na2) < _eps_ || aurostd::abs(modulus(rrr) - na3) < _eps_)) { // CO20200106 - patching for auto-indenting
    ...
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << pgroup_type << " Symmetry: found " << grid_clattice.size() << " vectors inside radius" << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    // ------------------------------------------------------------------------------
    ...
    uint jj = 0;
    uint kk = 0;
    for (size_t i = 0; i < grid_clattice.size(); i++) {
      for (ii = 1; ii <= 3; ii++) {
        temp_clattice(1, ii) = (*grid_clattice[i])(ii);
      }
      for (ii = 1; ii <= 3; ii++) {
        temp_flattice(1, ii) = (*grid_flattice[i])(ii);
      }
      for (size_t j = 0; j < grid_clattice.size(); j++) {
        for (ii = 1; ii <= 3; ii++) {
          temp_clattice(2, ii) = (*grid_clattice[j])(ii);
        }
        for (ii = 1; ii <= 3; ii++) {
          temp_flattice(2, ii) = (*grid_flattice[j])(ii);
        }
        for (size_t k = 0; k < grid_clattice.size(); k++) {
          if (i != j && i != k && j != k) {
            for (ii = 1; ii <= 3; ii++) {
              temp_clattice(3, ii) = (*grid_clattice[k])(ii);
            }
            for (ii = 1; ii <= 3; ii++) {
              temp_flattice(3, ii) = (*grid_flattice[k])(ii);
            }
            temp_clatticevol = std::abs(det(temp_clattice));
            if (std::abs(temp_clatticevol - latticevol) <= _eps_vol_ && temp_clatticevol > 0.0) { // check volume !!! FAST !
              ...
              if (std::abs(clatticedata[1] - temp_clatticedata[1]) < _eps_) { // check lattices
                if (std::abs(clatticedata[2] - temp_clatticedata[2]) < _eps_) { // check lattices
                  if (std::abs(clatticedata[3] - temp_clatticedata[3]) < _eps_) { // check lattices
                    // DX =======
                    if (checkAngle(clatticedata[2], clatticedata[3], clatticedata[4], temp_clatticedata[4], is_deg, _eps_)) { // check angles //DX
                      if (checkAngle(clatticedata[3], clatticedata[1], clatticedata[5], temp_clatticedata[5], is_deg, _eps_)) { // check angles //DX
                        if (checkAngle(clatticedata[1], clatticedata[2], clatticedata[6], temp_clatticedata[6], is_deg, _eps_)) { // check angles //DX
                          ...
                          Bct = temp_clattice;
                          Uc = trasp(Act) * inverse(trasp(Bct)); // Act=lattice;
                          Bdt = temp_flattice;
                          Uf = trasp(Adt) * inverse(trasp(Bdt)); // Adt=I3... could remove !
                          ...
                          if (sym_found == false) { // new operation, generate and save it
                            ...
                            aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << pgroup_type << " " << a.pgroup[kk - 1].str_type << " theta=";
                            if (a.pgroup[kk - 1].angle < 100) {
                              aus << " ";
                            }
                            if (a.pgroup[kk - 1].angle < 10) {
                              aus << " ";
                            }
    ...
    a.pgroup_calculated = true;
    // ------------------------------------------------------------------------------
    // for(size_t k=0;k<a.pgroup.size();k++) {
    //  Uc=a.pgroup[k].Uc;
    //  Uf=a.pgroup[k].Uf;
    ...
    grid_clattice.clear();
    for (size_t i = 0; i < grid_flattice.size(); i++) {
      delete grid_flattice[i];
    }
    grid_flattice.clear();
    // ------------------------------------------------------------------------------
    // Printing and leaving
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << pgroup_type << " Symmetry: unique point group operations " << a.pgroup.size() << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << pgroup_type << " Symmetry: END " << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    if (_write_) {
      Krun = Krun && KBIN_SymmetryWrite(FileMESSAGE, a, aflags, _PGROUP_, osswrite, oss, format);
    }
    string pgname; // DX20170906
    string operations; // DX20170906
...
int PointGroup_HITS(xstructure& a, double _eps_) {
  ...
  if (a.title == "KLATTICE") {
    message = "PGROUP_KLATTICE";
  }
  const double _eps_angle_ = 10.0 * _eps_;
  double _eps_abc_;
  double _eps_vol_;
  xvector<double> rrr(3);
  xvector<double> ddd(3);
  ...
  for (size_t i = 0; i < a.atoms.size(); i++) {
    basis_types_map[i] = a.atoms[i].type;
  }
  a.pgroup.clear();
  ...
  const xmatrix<double> Uc(3, 3);
  xmatrix<double> Adt(3, 3);
  const xmatrix<double> Bdt(3, 3);
  const xmatrix<double> Uf(3, 3);
  xvector<double> a1(3);
  xvector<double> a2(3);
  xvector<double> a3(3);
  const xvector<double> ctau(3);
  const xvector<double> ftau(3);
  const xvector<double> ctrasl(3);
  const xvector<double> ftrasl(3);
  a1 = lattice(1);
  a2 = lattice(2);
  a3 = lattice(3);
  Act = lattice;
  Adt = aurostd::identity((double) 0, 3); // ME20200123 - new identity format
  xvector<double> clatticedata(6);
  xvector<double> temp_clatticedata(6);
  clatticedata = _Getabc_angles(lattice, DEGREES);
  vector<xvector<double>*> grid_clattice;
  xvector<double>* grid_clattice_ptr;
  vector<xvector<double>*> grid_flattice;
  xvector<double>* grid_flattice_ptr;
  double na1;
  double na2;
  double na3;
  double amin;
    ...
    for (size_t j = 0; j < grid_clattice.size(); j++) {
      ...
      for (size_t k = 0; k < grid_clattice.size(); k++) {
        if (i != j && i != k && j != k) {
          ...
          temp_clatticevol = std::abs(det(temp_clattice));
          if (std::abs(temp_clatticevol - latticevol) <= _eps_vol_) {
            temp_clatticedata = _Getabc_angles(temp_clattice, DEGREES);
            jj++;
            if (std::abs(clatticedata[1] - temp_clatticedata[1]) < _eps_abc_) {
              if (std::abs(clatticedata[2] - temp_clatticedata[2]) < _eps_abc_) {
                if (std::abs(clatticedata[3] - temp_clatticedata[3]) < _eps_abc_) {
                  if (std::abs(clatticedata[4] - temp_clatticedata[4]) < _eps_angle_) {
                    if (std::abs(clatticedata[5] - temp_clatticedata[5]) < _eps_angle_) {
                      if (std::abs(clatticedata[6] - temp_clatticedata[6]) < _eps_angle_) {
                        count++;
                      }
                    }
                  }
...
vector<double> PointGroupHistogramCheck(xstructure& a) {
  ...
  result.clear();
  result.push_back(0.001);
  result.push_back(0.0025);
  result.push_back(0.005);
  result.push_back(0.01);
  result.push_back(0.025);
  return result;
}
...
namespace SYM {
  bool CalculatePointGroupKLattice(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, string format) { // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    xstructure aa;
    aa.ReScale(1.0); // contains FixLattices()
    aa.lattice = a.klattice;
    aa.ReScale(1.0); // contains FixLattices()
    ...
    // ------------------------------------------------------------------------------
    _sym_op symop;
    for (size_t k = 0; k < aa.pgroup.size(); k++) {
      symop = aa.pgroup[k];
      // cerr << symop << endl;
    ...
    if (_write_) {
      Krun = Krun && KBIN_SymmetryWrite(FileMESSAGE, a, aflags, _PGROUPK_, osswrite, oss, format);
    }
    if (LDEBUG) {
      cerr << "DEBUG: SYM::CalculatePointGroupKLattice: a.pgroupk.size()=" << a.pgroupk.size() << endl;
    }
    return Krun;
  }
} // namespace SYM
...
namespace SYM {
  bool CalculatePointGroupKCrystal(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, string format) { // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    //  cerr << aa << endl;
    if (a.pgroup_xtal_calculated == false) {
      Krun = Krun && SYM::CalculatePointGroupCrystal(FileMESSAGE, a, aflags, _write_, osswrite, oss); // NEED POINT GROUP CRYSTAL //HE20220912
    }
    Krun = Krun && SYM::TransformSymmetryFromRealToReciprocal(FileMESSAGE, a, aa, aflags, osswrite, oss, pgroup_type); // DX20170808 - New klattice routine
    // ------------------------------------------------------------------------------
    a.pgroupk_xtal_calculated = true;
    // ------------------------------------------------------------------------------
    _sym_op symop;
    for (size_t k = 0; k < aa.pgroup_xtal.size(); k++) {
      symop = aa.pgroup_xtal[k];
      // cerr << symop << endl;
      //      SYM::AddSymmetryToStructure(a,symop.Uc,symop.Uf,symop.ctau,symop.ftau,symop.ctrasl,symop.ftrasl,symop.basis_atoms_map,symop.basis_types_map,_PGROUPK_);
    ...
    if (_write_) {
      Krun = Krun && KBIN_SymmetryWrite(FileMESSAGE, a, aflags, _PGROUPK_XTAL_, osswrite, oss, format);
    }
    if (LDEBUG) {
      cerr << "DEBUG: SYM::CalculatePointGroupKCrystal: a.pgroupk_xtal.size()=" << a.pgroupk_xtal.size() << endl;
    }
    return Krun;
  }
} // namespace SYM
...
//
namespace SYM {
  bool TransformSymmetryFromRealToReciprocal(ofstream& FileMESSAGE, xstructure& real_xstr, xstructure& reciprocal_xstr, _aflags& aflags, const bool& osswrite, ostream& oss, string& pgroup_type) {
    ostringstream aus;
    xvector<double> ctau(3);
    xvector<double> ftau(3);
    xvector<double> ctrasl(3);
    ...
    for (size_t i = 0; i < reciprocal_xstr.atoms.size(); i++) {
      basis_atoms_map[i] = i; // identically map each over each
    }
    std::vector<int> basis_types_map(reciprocal_xstr.atoms.size()); // will map each on each
    for (size_t i = 0; i < reciprocal_xstr.atoms.size(); i++) {
      basis_types_map[i] = reciprocal_xstr.atoms[i].type; // identically map each over each
    }
    ...
    if (pgroup_type == "pgroupk" && real_xstr.pgroup_calculated) {
      const string message = "PGROUP_KLATTICE";
      // clean up pgroupk                                                     // just initialize
      reciprocal_xstr.pgroup.clear();
      reciprocal_xstr.pgroup_calculated = false; // just initialize
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " Symmetry: BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
      uint kk = 0;
      xmatrix<double> Uf;
      xmatrix<double> Uc;

      const xmatrix<double> trasp_real_f2c = trasp(real_xstr.f2c);
      const xmatrix<double> trasp_real_c2f = trasp(real_xstr.c2f);
      for (size_t i = 0; i < real_xstr.pgroup.size(); i++) {
        ...
        aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " " << reciprocal_xstr.pgroup[kk - 1].str_type << " theta=";
        if (reciprocal_xstr.pgroup[kk - 1].angle < 100) {
          aus << " ";
        }
        if (reciprocal_xstr.pgroup[kk - 1].angle < 10) {
          aus << " ";
        }
      ...
      for (size_t k = 0; k < reciprocal_xstr.pgroup.size(); k++) {
        Uc = reciprocal_xstr.pgroup[k].Uc;
        Uf = reciprocal_xstr.pgroup[k].Uf;
        reciprocal_xstr.pgroup[k].Uc = roundoff(reciprocal_xstr.pgroup[k].Uc, _EPS_roundoff_);
      }
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " Symmetry: unique point group operations " << reciprocal_xstr.pgroup.size() << endl; // Message(__AFLOW_FILE__,aflags) << endl;
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " Symmetry: END " << endl; // Message(__AFLOW_FILE__,aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
      return true;
    }
    // pgroupk_xtal version
      ...
      const string message = "PGROUPK_XTAL";
      ...
      const xmatrix<double> trasp_real_f2c = trasp(real_xstr.f2c);
      const xmatrix<double> trasp_real_c2f = trasp(real_xstr.c2f);
      for (size_t i = 0; i < real_xstr.pgroup_xtal.size(); i++) {
        ...
        aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " " << reciprocal_xstr.pgroup_xtal[kk - 1].str_type << " theta=";
        if (reciprocal_xstr.pgroup_xtal[kk - 1].angle < 100) {
          aus << " ";
        }
        if (reciprocal_xstr.pgroup_xtal[kk - 1].angle < 10) {
          aus << " ";
        }
      ...
      for (size_t k = 0; k < reciprocal_xstr.pgroup_xtal.size(); k++) {
        Uc = reciprocal_xstr.pgroup_xtal[k].Uc;
        Uf = reciprocal_xstr.pgroup_xtal[k].Uf;
        reciprocal_xstr.pgroup_xtal[k].Uc = roundoff(reciprocal_xstr.pgroup_xtal[k].Uc, _EPS_roundoff_);
      }
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " Symmetry: unique point group operations " << reciprocal_xstr.pgroup_xtal.size() << endl; // Message(__AFLOW_FILE__,aflags) << endl;
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << message << " Symmetry: END " << endl; // Message(__AFLOW_FILE__,aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
      return true;
    }
...
namespace SYM {
  ...
  bool getFullSymBasis(const xstructure& a, _sym_op& symOp, bool map_types, double tolerance, vector<int>& basis_atoms_map, vector<int>& basis_types_map) { // CO20190520 - removed pointers for bools and doubles, added const where possible
    ...
    if (min_dist == AUROSTD_NAN) {
      min_dist = minimumDistance(a);
    } // CO20180409
    const bool skew = isLatticeSkewed(a.lattice, min_dist, tolerance); // CO20180409
  ...
  bool getFullSymBasis(const deque<_atom>& atoms,
                       const xmatrix<double>& lattice,
                       const xmatrix<double>& c2f,
                       const xmatrix<double>& f2c,
                       _sym_op& symOp,
                       bool map_types,
                       bool skew,
                       double tolerance,
                       vector<int>& basis_atoms_map,
                       vector<int>& basis_types_map) { // MAIN FUNCTION //CO20190520 - removed pointers for bools and doubles, added const where possible
    const bool LDEBUG = (false || XHOST.DEBUG);

    const bool fast = true; // DX COME BACK HERE, MAKE THIS AN INPUT //CO, no need, if fast doesn't work, we have bigger problems

    // CO20180420 - fixing for ME
    const bool basis_map_calculated_orig = symOp.basis_map_calculated;
    ...
    // bool cont = true;
    uint count = 0;
    const deque<_atom> transformedcrystal;
...
namespace SYM {
  bool CalculateFactorGroup(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, double _eps_, string format) {
    ...
    // Obtain the structure tolerance
    a.sym_eps = _eps_;
    const bool skew = isLatticeSkewed(a.lattice, a.dist_nn_min, _eps_);
    ...
    if (a.pgroup_calculated == false) {
      Krun = Krun && SYM::CalculatePointGroup(FileMESSAGE, a, aflags, _write_, osswrite, oss, format); // NEED POINT GROUP
    }

    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "FGROUP Symmetry: BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    a.fgroup.clear();
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " DEBUG grabbed index for smallest group" << endl;
    }
    ...
    for (size_t pg = 0; pg < a.pgroup.size(); pg++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " DEBUG starting pg=" << pg << endl;
      }
      // symOp.str_Hermann_Mauguin=a.pgroup[pg].str_Hermann_Mauguin; //no longer necessary
      // CO END
      for (size_t j = 0; j < atoms_by_type[index_for_smallest_group].size(); j++) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " DEBUG j=" << j << endl;
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " DEBUG index_for_smallest_group=" << index_for_smallest_group << endl;
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " DEBUG atoms_by_type.size()=" << atoms_by_type.size() << endl;
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " DEBUG atoms_by_type[index_for_smallest_group].size()=" << atoms_by_type[index_for_smallest_group].size() << endl;
        }
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " DEBUG about to test symop" << endl;
        }
        // CO END
        ...
        if (getFullSymBasis(a.atoms, a.lattice, a.c2f, a.f2c, symOp, true, skew, _eps_, basis_atoms_map, basis_types_map)) { // CO20200106 - patching for auto-indenting
          ...
          clear(symOp.ftrasl);
          sym_found = false;
          for (size_t ii = 0; ii < a.fgroup.size() && !sym_found; ii++) {
          ...
          if (sym_found == false) { // new operation, generate and save it
            ...
            aus << " theta=";
            if (a.fgroup[kk - 1].angle < 100) {
              aus << " ";
            }
            if (a.fgroup[kk - 1].angle < 10) {
              aus << " ";
            }
            ...
            aus << " basis_atoms_map = [ ";
            for (size_t n = 0; n < a.fgroup[kk - 1].basis_atoms_map.size(); n++) {
              aus << a.fgroup[kk - 1].basis_atoms_map[n] << " ";
            }
            aus << "] ";
            aus << endl; // remember vectors start from 0
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    ...
    uint unities = 0;
    for (size_t ii = 0; ii < a.fgroup.size(); ii++) {
      if (aurostd::identical(a.fgroup[ii].Uf, a.fgroup[0].Uf)) {
        unities++;
      } // check unity 0 is always unity //DX used to be _eps_/10.0  //DX20171207 - Use xmatrix identical eps
    }
    if (unities > 1) {
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "FGROUP Symmetry: Cell not primitive: as big as " << unities << endl; // Message(__AFLOW_FILE__,aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    }
    for (size_t ii = 0 + 1; ii < a.fgroup.size(); ii++) { // avoid origin
    ...
    // Printing and leaving
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "FGROUP Symmetry: unique factor group operations " << a.fgroup.size() << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "FGROUP Symmetry: END " << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);

    if (_write_) {
      Krun = Krun && KBIN_SymmetryWrite(FileMESSAGE, a, aflags, _FGROUP_, osswrite, oss, format);
    }
    return Krun;
  }
} // namespace SYM
...
namespace SYM {
  // DX+CO START
  bool CalculateSpaceGroup(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, string format) { // AFLOW_FUNCTION_IMPLEMENTATION
    // ------------------------------------------------------------------------------
    ...
    a1 = a.lattice(1);
    a2 = a.lattice(2);
    a3 = a.lattice(3); // a1,a2,a3 are the rows of the lattice matrix
    xvector<double> ctau(3);
    xvector<double> ftau(3);
    xvector<double> ctrasl(3);
    xvector<double> ftrasl(3); // translations
    xmatrix<double> Uf(3, 3);
    ...
    if (a.fgroup_calculated == false) {
      Krun = Krun && SYM::CalculateFactorGroup(FileMESSAGE, a, aflags, _write_, osswrite, oss, format); // NEED FACTOR GROUP
    }

    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "SGROUP Symmetry: BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    // clean up sgroup                                // just initialize
    a.sgroup.clear();
    a.sgroup_calculated = false;
    if (a.sgroup_radius <= 0.1) {
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "SGROUP Symmetry: picking default normalized radius = " << _calculate_symmetry_default_sgroup_radius_ << "  " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    ...
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "SGROUP Symmetry: inside sphere (radius=" << a.sgroup_radius << ", dims = [" << dims[1] << "," << dims[2] << "," << dims[3] << "]) "
        << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    ...
    for (int i = -dims[1]; i <= dims[1]; i++) {
      for (int j = -dims[2]; j <= dims[2]; j++) {
        for (int k = -dims[3]; k <= dims[3]; k++) {
          ...
          ftrasl(2) = (double) j;
          ftrasl(3) = (double) k;
          for (size_t fg = 0; fg < a.fgroup.size(); fg++) {
            Uc = a.fgroup[fg].Uc;
            Uf = a.fgroup[fg].Uf;
            ...
            // 	  for(size_t ii=0;ii<a.sgroup.size()&&!sym_found;ii++)
    ...
    // ------------------------------------------------------------------------------
    // Printing and leaving
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "SGROUP Symmetry: unique space group operations " << a.sgroup.size() << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "SGROUP Symmetry: END " << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    // ------------------------------------------------------------------------------
    if (_write_) {
      Krun = Krun && KBIN_SymmetryWrite(FileMESSAGE, a, aflags, _SGROUP_, osswrite, oss, format);
    }
    return Krun;
  }
} // namespace SYM
...
namespace SYM {
  ...
  bool CalculateInequivalentAtoms(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, string format) { // AFLOW_FUNCTION_IMPLEMENTATION
    const bool rely_on_basis = true; // HUGE SPEED UP
...
namespace SYM {
  bool CalculateInequivalentAtoms(ofstream& FileMESSAGE, xstructure& a, bool rely_on_basis, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, double _eps_, string format) { // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "IATOMS ATOMS:  BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    ...
    for (size_t iat = 0; iat < a.atoms.size(); iat++) {
      a.atoms[iat].equivalent = iat;
      a.atoms[iat].is_inequivalent = true;
    }
    // ------------------------------------------------------------------------------
    // clear things
    ...
    if (rely_on_basis) {
      for (size_t iat1 = 0; iat1 < a.atoms.size(); iat1++) {
        for (size_t iat2 = iat1 + 1; iat2 < a.atoms.size(); iat2++) {
          ...
          if (a.atoms[iat1].type == a.atoms[iat2].type) {
            if (AtomsEquivalent_Basis(a, iat1, iat2)) {
              a.atoms[iat2].equivalent = a.atoms[iat1].equivalent;
              a.atoms[iat2].is_inequivalent = false;
              iat2 = a.atoms.size();
            }
          }
        }
      ...
      for (size_t iat1 = 0; iat1 < a.atoms.size(); iat1++) {
        for (size_t iat2 = iat1 + 1; iat2 < a.atoms.size(); iat2++) {
          // if(iat2!=iat1) {  //ALREADY true  //[CO20200106 - close bracket for indenting]}
          if (a.atoms[iat1].type == a.atoms[iat2].type) {
            if (AtomsEquivalent(a, a.atoms[iat1], a.atoms[iat2], skew, _eps_)) {
              a.atoms[iat2].equivalent = a.atoms[iat1].equivalent;
              a.atoms[iat2].is_inequivalent = false;
              iat2 = a.atoms.size();
            }
    ...
    for (size_t iat = 0; iat < a.atoms.size(); iat++) {
      if (a.atoms[iat].is_inequivalent) {
        a.iatoms.emplace_back(1, iat);
      }
    }
    // create the list of inequivalent (cols)
    for (size_t iat1 = 0; iat1 < a.atoms.size(); iat1++) {
      if (!a.atoms[iat1].is_inequivalent) {
        for (size_t iat2 = 0; iat2 < a.iatoms.size(); iat2++) {
          if (a.atoms[iat1].equivalent == a.iatoms[iat2].at(0)) {
            a.iatoms[iat2].push_back(iat1);
    ...
    for (size_t iat = 0; iat < a.atoms.size(); iat++) {
      if (a.atoms[iat].is_inequivalent) {
        a.atoms[iat].num_equivalents = a.iatoms.at(iequivalent).size();
        a.atoms[iat].index_iatoms = iequivalent;
        iequivalent++;
      }
    }
    // ------------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------------
    // Printing and leaving
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "IATOMS ATOMS: number of inequivalent atoms = " << a.iatoms.size() << "   " << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    for (size_t iat1 = 0; iat1 < a.iatoms.size(); iat1++) {
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "IATOMS ATOMS: [";
      if (a.iatoms[iat1].at(0) < 10) {
        aus << "0";
      }
      aus << a.iatoms[iat1].at(0) << "] ";
      for (size_t iat2 = 0; iat2 < a.iatoms[iat1].size(); iat2++) {
        aus << a.iatoms[iat1][iat2] << " ";
      }
      aus << endl;
    }
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "IATOMS ATOMS: END " << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    // ------------------------------------------------------------------------------
    if (_write_) {
      Krun = Krun && KBIN_SymmetryWrite(FileMESSAGE, a, aflags, _IATOMS_, osswrite, oss, format);
    }
    return Krun;
  }
} // namespace SYM
...
namespace SYM {
  bool CalculateSitePointGroup(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, string format) {
    ...
    const int CALCULATION_MODE = 0; // default
  ...
  bool CalculateSitePointGroup(ofstream& FileMESSAGE, xstructure& a, int CALCULATION_MODE, _aflags& aflags, bool _write_, const bool& osswrite, ostream& oss, string format) {
    ...
    if (a.pgroup_xtal_calculated == false) {
      Krun = Krun && SYM::CalculatePointGroupCrystal(FileMESSAGE, a, aflags, _write_, osswrite, oss, format); // NEED POINT GROUP CRYSTAL
    }
    // if(a.sgroup_calculated==false) Krun=Krun && SYM::CalculateSpaceGroup(FileMESSAGE,a,aflags,false,osswrite,oss);          // NEED SPACE GROUP
    if (a.iatoms_calculated == false) {
      Krun = Krun && SYM::CalculateInequivalentAtoms(FileMESSAGE, a, aflags, _write_, osswrite, oss, format); // NEED EQUIV ATOMS
    }
    // a.write_inequivalent_flag=true;

    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "AGROUP Symmetry: BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);

    // ------------------------------------------------------------------------------
    // clear a little and create the framework
    for (size_t i = 0; i < a.agroup.size(); i++) {
      a.agroup[i].clear();
    }
    a.agroup.clear();
    for (size_t i = 0; i < a.atoms.size(); i++) {
      a.agroup.emplace_back(0);
    }
    // ------------------------------------------------------------------------------
    // seeking for the
    uint iat;
    uint kk = 0;
    bool sym_found;
    const _sym_op aSymOp;
    const _sym_op fSymOp;
    const _atom tatom;

    const bool iatoms_only = (CALCULATION_MODE == 0 || CALCULATION_MODE == 1);
    const uint atoms_range = (iatoms_only ? a.iatoms.size() : a.atoms.size()); // CO, if iatoms_only, only go over iatoms, all atoms otherwise
    xvector<double> origin(3);
    xvector<double> frigin(3);
    const string ATOM_INDICATOR = (iatoms_only ? "ineq atom #=" : "atom #=");
    for (uint iiat = 0; iiat < atoms_range; iiat++) {
      ...
      aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "AGROUP " << "[site=";
      if (iat < 10) {
        aus << "0";
      }
      aus << iat << "] ";
      ...
      for (size_t ii = 0; ii < b_atoms.size(); ii++) {
      ...
      for (size_t pg = 0; pg < a.pgroup_xtal.size(); pg++) { // DX+CO
        if (getFullSymBasis(b_atoms, a.lattice, a.c2f, a.f2c, a.pgroup_xtal[pg], true, skew, _eps_, basis_atoms_map, basis_types_map)) { // DX+CO
          ...
          sym_found = false;
          for (size_t ii = 0; ii < a.agroup.at(iat).size() && !sym_found; ii++) {
          ...
          if (sym_found == false) { // new operation, generate and save it
            ...
            if (a.agroup[iat][kk - 1].angle < 100) {
              aus << " "; // remember vectors start from 0
            }
            if (a.agroup[iat][kk - 1].angle < 10) {
              aus << " "; // remember vectors start from 0
            }
      ...
      aus << iat << "] ";
      // aus << "unique site point group operations " << a.agroup.at(iat).size() << " (#pg=" << pgroup.size() << ")  " << endl;// Message(__AFLOW_FILE__,aflags) << endl;
      aus << "unique site point group operations " << a.agroup.at(iat).size() << " (#pg_xtal=" << a.pgroup_xtal.size() << ")  " << endl; // Message(__AFLOW_FILE__,aflags) << endl;  //DX+CO
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    }
    ...
    if (CALCULATION_MODE == 2) {
      ...
      for (size_t i = 0; i < a.iatoms.size(); i++) {
        ...
        for (size_t j = 1; j < a.iatoms[i].size(); j++) {
    ...
    // ------------------------------------------------------------------------------
    // Printing and leaving
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "AGROUP number of inequivalent atoms = " << a.iatoms.size() << "   " << endl; // Message(__AFLOW_FILE__,aflags) << endl;
    for (size_t iat1 = 0; iat1 < a.iatoms.size(); iat1++) {
      ...
      aus << a.iatoms[iat1].at(0) << "] ";
      for (size_t iat2 = 0; iat2 < a.iatoms[iat1].size(); iat2++) {
        aus << a.iatoms[iat1].at(iat2) << "(" << a.agroup.at(a.iatoms[iat1][iat2]).size() << ")" << " ";
      }
      aus << endl;
    }
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "AGROUP Symmetry: END " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
    if (_write_) {
      Krun = Krun && KBIN_SymmetryWrite(FileMESSAGE, a, aflags, _AGROUP_, osswrite, oss, format);
    }
    return Krun;
  }
} // namespace SYM
...
namespace SYM {
  ...
  bool CalculateSitePointGroup_EquivalentSites(xstructure& a, double _eps_) {
    const bool get_full_basis = true;
  ...
  bool CalculateSitePointGroup_EquivalentSites(xstructure& a, bool get_full_basis, double _eps_) {
    const bool LDEBUG = (false || XHOST.DEBUG);
    DEBUG_SYMMETRY = DEBUG_SYMMETRY || LDEBUG;
    const string directory = aurostd::getPWD(); // DX20180426 - added current working directory
    ...
    if (a.pgroup_calculated == false) {
      return false; // NEED FACTOR GROUP
    }
    if (a.fgroup_calculated == false) {
      return false; // NEED FACTOR GROUP
    }
    ...
    if (a.iatoms_calculated == false) {
      return false; // NEED IATOMS
    }
    if ((a.fgroup.empty()) || (a.iatoms.empty()) || (a.agroup.empty())) {
      return false;
    }
    for (size_t iiat = 0; iiat < a.iatoms.size(); iiat++) {
      if (a.agroup.at(a.iatoms[iiat].at(0)).empty()) {
        return false;
      } // NEED AGROUP FOR INEQUIVALENT ATOMS
      for (size_t ii = 1; ii < a.iatoms[iiat].size(); ii++) {
        a.agroup.at(a.iatoms[iiat][ii]).clear(); // clear otherwise
      }
    }
    ...
    for (size_t iiat = 0; iiat < a.iatoms.size(); iiat++) {
      iat = a.iatoms[iiat][0];
      for (size_t ieat = 1; ieat < a.iatoms[iiat].size(); ieat++) {
        found_fgroup = false;
        eat = a.iatoms[iiat][ieat];
        for (size_t fg = 0; fg < a.fgroup.size() && !found_fgroup; fg++) {
        ...
        if (found_fgroup) {
          for (size_t ag = 0; ag < a.agroup[iat].size(); ag++) {
            ...
            if (get_full_basis) {
              ...
              for (size_t iii = 0; iii < a.atoms.size(); iii++) {
            ...
            SYM::AddSymmetryToStructure(a, eat, aSymOp.Uc, aSymOp.Uf, aSymOp.ctau, aSymOp.ftau, aSymOp.ctrasl, aSymOp.ftrasl, aSymOp.basis_atoms_map, aSymOp.basis_types_map, aSymOp.basis_map_calculated,
                                        _AGROUP_, false); // CO20170706 - make sure quaternion is updated
          }
        } else {
    ...
    // for(size_t ii=1;ii<a.iatoms.at(iiat).size();ii++){
    // found_fgroup=false;
    ...
    //}
    // if(found_fgroup){
    // for(size_t ag=0;ag<a.agroup.at(iat).size();ag++){
...
namespace SYM {
  ...
  void CalculateSitePointGroup2(xstructure& a, bool ComMidss) { // CO20200106 - patching for auto-indenting
    ...
    dims = LatticeDimensionSphere(lattice, radius);
    flog << "radius = " << radius << endl << "dims: " << dims << endl << "tolerance eps, epsabc: " << _eps_ << ", " << _eps_abc_ << endl;
    grid_clattice.clear();
    grid_flattice.clear();
    for (uint i = 1; i <= 3; i++) {
      grid_clattice_ptr = new xvector<double>(3);
      ...
      for (size_t j = 0; j < grid_clattice.size(); j++) {
        ...
        for (size_t k = 0; k < grid_clattice.size(); k++) {
          if (i != j && i != k && j != k) {
            ...
            temp_clatticevol = std::abs(det(temp_clattice));
            if (std::abs(temp_clatticevol - latticevol) <= _eps_vol_) { // check volume !!! FAST !
              temp_clatticedata = _Getabc_angles(temp_clattice, DEGREES);
              jj++; // if(!mod(jj,100000)) cerr << temp_clatticedata << endl;
              // compare the temp_clattice... and lattice... to see if they are the same lattice
              // that is do the lattice vectors have the same lengths and the same angles
              if (std::abs(clatticedata[1] - temp_clatticedata[1]) < _eps_abc_) { // check lattices
                if (std::abs(clatticedata[2] - temp_clatticedata[2]) < _eps_abc_) { // check lattices
                  if (std::abs(clatticedata[3] - temp_clatticedata[3]) < _eps_abc_) { // check lattices
                    if (std::abs(clatticedata[4] - temp_clatticedata[4]) < _eps_angle_) { // check angles
                      if (std::abs(clatticedata[5] - temp_clatticedata[5]) < _eps_angle_) { // check angles
                        if (std::abs(clatticedata[6] - temp_clatticedata[6]) < _eps_angle_) { // check angles
                          // SYMMETRY OPERATION POINT_GROUP and FACTOR_GROUP
                          // get the matrix that relates the two lattice vectors
                          // A=U*B but in A and B we plug vectors as columns
                          // watch lattice is per row Uc=A*inv(B)
                          ...
                          if (sym_found == false) { // new operation, generate and save it
                            ...
                            aus << a.pgroup[kk - 1].str_type << " theta=";
                            if (a.pgroup[kk - 1].angle < 100) {
                              aus << " ";
                            }
                            if (a.pgroup[kk - 1].angle < 10) {
                            ...
                            aus << a.pgroup[kk - 1].angle << " " << " r=(" << a.pgroup[kk - 1].axis << ")" << endl; // remember vectors start from 0
    ...
    for (size_t k = 0; k < a.pgroup.size(); k++) {
      Uc = a.pgroup[k].Uc;
      Uf = a.pgroup[k].Uf;
      a.pgroup[k].Uc = roundoff(a.pgroup[k].Uc, _EPS_roundoff_);
      // for(i=1;i<=3;i++)
      //   for(j=1;j<=3;j++)
      //   if(abs((*a.pgroup[k])(i,j))<_EPS_roundoff_) (*a.pgroup[k])(i,j)=0.0;
      // for(i=0;i<9;i++) if(abs((*a.pgroup[k])(int(i/3)+1,mod(i,3)+1))<1.0e-10) (*a.pgroup[k])(int(i/3)+1,mod(i,3)+1)=0.0;
    ...
    for (i = 0; i < a.pgroup.size(); i++) {
      cout << "pointsym_op " << i << "/" << a.pgroup.size() << ": " << a.pgroup[i].str_type << ", " << a.pgroup[i].angle << " about " << a.pgroup[i].axis << endl << a.pgroup[i].Uc << endl;
      flog << "pointsym_op " << i << "/" << a.pgroup.size() << ": " << a.pgroup[i].str_type << ", " << a.pgroup[i].angle << " about " << a.pgroup[i].axis << endl << a.pgroup[i].Uc << endl;
    }
    ...
    double n3;
    Nbasis = a.atoms.size();
    xmatrix<double> rlatt(3, 3);
    xmatrix<double> rlattb4Uc(3, 3);
    xmatrix<double> rlatttmp(3, 3);
    xmatrix<double> mG(3, 3);
    xmatrix<double> mGb4Uc(3, 3);
    ...
    for (ib = 0; ib < Nbasis; ib++) {
      ...
      for (it = 1; it < 4; it++) {
        rlattb4Uc(1, it) = lattice(1, it) - basis[ib][it];
      }
      for (it = 1; it < 4; it++) {
        rlattb4Uc(2, it) = lattice(2, it) - basis[ib][it];
      }
      for (it = 1; it < 4; it++) {
        rlattb4Uc(3, it) = lattice(3, it) - basis[ib][it];
      }
      for (it = 1; it < 4; it++) {
        mGb4Uc(1, it) = -basis[ib][it];
      }
      mGb4Uc(3) = mGb4Uc(2) = mGb4Uc(1);
      flog << "rlatt r=r-basis[" << ib << "]:" << endl << rlattb4Uc << endl << "gamma g=g-basis[" << ib << "]: " << mGb4Uc(1) << endl;
      for (jp = 0; jp < a.pgroup.size(); jp++) {
        ...
        flog << "rlatt r=Ur:" << endl << rlatt << endl << "gamma  g=U*g: " << mG(1) << endl;
        // r=r+A
        for (it = 1; it < 4; it++) {
          rlatt(1, it) = rlatt(1, it) + basis[ib][it];
        }
        for (it = 1; it < 4; it++) {
          rlatt(2, it) = rlatt(2, it) + basis[ib][it];
        }
        for (it = 1; it < 4; it++) {
          rlatt(3, it) = rlatt(3, it) + basis[ib][it];
        }
        for (it = 1; it < 4; it++) {
          mG(1, it) = mG(1, it) + basis[ib][it];
        }
        flog << "rlatt r=r+basis[" << ib << "]:" << endl << rlatt << endl << "gamma  g=g+basis[" << ib << "]: " << mG(1) << endl;
        // bring inside radius
        ...
        mG(2, 3) = 0.0;
        for (it = 1; it < 4; it++) {
          mG(2, 1) = mG(2, 1) + mG(1, it) * irlatt(it, 1);
        }
        for (it = 1; it < 4; it++) {
          mG(2, 2) = mG(2, 2) + mG(1, it) * irlatt(it, 2);
        }
        for (it = 1; it < 4; it++) {
          mG(2, 3) = mG(2, 3) + mG(1, it) * irlatt(it, 3);
        }
        n1 = (int) (round(mG(2, 1)));
        n2 = (int) (round(mG(2, 2)));
        n3 = (int) (round(mG(2, 3)));
        ...
        for (it = 1; it < 4; it++) {
          rlatt(1, it) = rlatt(1, it) - (n1 * a1(it) + n2 * a2(it) + n3 * a3(it));
        }
        for (it = 1; it < 4; it++) {
          rlatt(2, it) = rlatt(2, it) - (n1 * a1(it) + n2 * a2(it) + n3 * a3(it));
        }
        for (it = 1; it < 4; it++) {
          rlatt(3, it) = rlatt(3, it) - (n1 * a1(it) + n2 * a2(it) + n3 * a3(it));
        }
        for (it = 1; it < 4; it++) {
          mG(1, it) = mG(1, it) - (n1 * a1(it) + n2 * a2(it) + n3 * a3(it));
        }
        if (modulus(rlatt(1)) > (radius + 3 * _eps_abc_) || modulus(rlatt(2)) > (radius + 3 * _eps_abc_) || modulus(rlatt(3)) > (radius + 3 * _eps_abc_)) {
        ...
        flog << "rlatt row-lattice vectors new coord: " << endl << rlatt << endl << "Gamma-point new cart coord: " << mG(1) << endl;
        ...
        for (i = 0; i < grid_clattice.size(); i++) {
          ...
          if (!foundG && (std::abs(mG(1, 1) - (*grid_clattice[i])(1)) < _eps_abc_) && (std::abs(mG(1, 2) - (*grid_clattice[i])(2)) < _eps_abc_) && (std::abs(mG(1, 3) - (*grid_clattice[i])(3)) < _eps_abc_)) {
            foundG = true;
            flog << "**foundG**" << endl;
          }
          ...
          if (!founda1 && (std::abs(rlatt(1, 1) - (*grid_clattice[i])(1)) < _eps_abc_) && (std::abs(rlatt(1, 2) - (*grid_clattice[i])(2)) < _eps_abc_) && (std::abs(rlatt(1, 3) - (*grid_clattice[i])(3)) < _eps_abc_)) {
            founda1 = true;
            flog << "**founda1**" << endl;
          }
          ...
          if (!founda2 && (std::abs(rlatt(2, 1) - (*grid_clattice[i])(1)) < _eps_abc_) && (std::abs(rlatt(2, 2) - (*grid_clattice[i])(2)) < _eps_abc_) && (std::abs(rlatt(2, 3) - (*grid_clattice[i])(3)) < _eps_abc_)) {
            founda2 = true;
            flog << "**founda2**" << endl;
          }
          ...
          if (!founda3 && (std::abs(rlatt(3, 1) - (*grid_clattice[i])(1)) < _eps_abc_) && (std::abs(rlatt(3, 2) - (*grid_clattice[i])(2)) < _eps_abc_) && (std::abs(rlatt(3, 3) - (*grid_clattice[i])(3)) < _eps_abc_)) {
            founda3 = true;
            flog << "**founda3**" << endl;
          }
    ...
    for (i = 0; i < Nbasis; i++) {
      ...
      for (j = 0; j < Bsitesym[i].size(); j++) {
        cout << "sitesym_op " << j << "/" << Bsitesym[i].size() << ": " << a.pgroup[Bsitesym[i][j]].str_type << ", " << a.pgroup[Bsitesym[i][j]].angle << " about " << a.pgroup[Bsitesym[i][j]].axis << endl
          ...
          for (iB = iA + 1; iB < Nbasis; iB++) {
            for (jpB = 0; jpB < Bsitesym[iB].size(); jpB++) {
              if (Bsitesym[iA][jpA] == Bsitesym[iB][jpB]) {
                ...
                for (it = 1; it < 4; it++) {
                  AMB[it] = (basis[iA][it] + basis[iB][it]) / 2.0;
                }
                // r=r-A
                for (it = 1; it < 4; it++) {
                  rlattb4Uc(1, it) = lattice(1, it) - AMB[it];
                }
                for (it = 1; it < 4; it++) {
                  rlattb4Uc(2, it) = lattice(2, it) - AMB[it];
                }
                for (it = 1; it < 4; it++) {
                  rlattb4Uc(3, it) = lattice(3, it) - AMB[it];
                }
                for (it = 1; it < 4; it++) {
                  mGb4Uc(1, it) = -AMB[it];
                }
                ...
                // r=r+A
                for (it = 1; it < 4; it++) {
                  rlatt(1, it) = rlatt(1, it) + AMB[it];
                }
                for (it = 1; it < 4; it++) {
                  rlatt(2, it) = rlatt(2, it) + AMB[it];
                }
                for (it = 1; it < 4; it++) {
                  rlatt(3, it) = rlatt(3, it) + AMB[it];
                }
                for (it = 1; it < 4; it++) {
                  mG(1, it) = mG(1, it) + AMB[it];
                }
                // bring inside radius
                irlatt = inverse(lattice);
                mG(2, 1) = 0.0;
    ...
    for (iA = 0; iA < Nbasis - 1; iA++) {
      for (iB = iA + 1; iB < Nbasis; iB++) {
        ...
        for (jp = 0; jp < AMBsitesym[iA][iB].size(); jp++) {
          cout << " " << AMBsitesym[iA][iB][jp];
        }
        cout << endl;
      }
    }
...
string SymmetryToJson(vector<_sym_op>& group, char& mode) {
  ...
  if (mode == _PGROUP_XTAL_) {
    group_str = "pgroup_xtal";
  }
  if (mode == _PGROUPK_XTAL_) {
    group_str = "pgroupk_xtal";
  }
  if (mode == _PGROUPK_PATTERSON_) {
    group_str = "pgroupk_Patterson";
  }
  ...
  for (size_t i = 0; i < group.size(); i++) {
    // group
    if (!group_str.empty()) {
      sscontent_json << R"("group":")" << group_str << "\"" << eendl;
    } else {
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"group\":null" << eendl;
      }
    }
    ...
    if (group[i].ctau.rows && group_str == "agroup") {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"site\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    // type
    if (!group[i].str_type.empty()) {
      sscontent_json << R"("type":")" << aurostd::RemoveWhiteSpacesFromTheBack(group[i].str_type) << "\"" << eendl;
    } else {
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"type\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    // Hermann-Mauguin
    if (!group[i].str_Hermann_Mauguin.empty()) {
      sscontent_json << R"("Hermann_Mauguin":")" << group[i].str_Hermann_Mauguin << "\"" << eendl;
    } else {
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"Hermann_Mauguin\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    // Schoenflies
    if (!group[i].str_Schoenflies.empty()) {
      sscontent_json << R"("Schoenflies":")" << group[i].str_Schoenflies << "\"" << eendl;
    } else {
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"Schoenflies\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].generator.lrows) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"generator\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].generator.lrows) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"generator_coefficients\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].SU2_matrix.lrows) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"SU2_matrix\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].su2_coefficients.lrows) {
      sscontent_json << "\"su2_coefficients\":[" << aurostd::xcomplex2json(group[i].su2_coefficients(1)) << "," << aurostd::xcomplex2json(group[i].su2_coefficients(2)) << ","
                     << aurostd::xcomplex2json(group[i].su2_coefficients(3)) << "]" << eendl;
    } else {
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"su2_coefficients\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].angle != AUROSTD_NAN) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"angle\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].axis.lrows) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"axis\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].quaternion_vector.lrows) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"quaternion_vector\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].quaternion_matrix.lrows) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"quaternion_matrix\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].flag_inversion == true || group[i].flag_inversion == false) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"inversion\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].ctau.rows && (group_str == "fgroup" || group_str == "sgroup")) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"ctau\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].ftau.rows && (group_str == "fgroup" || group_str == "sgroup")) {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"ftau\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    // basis_atoms_map
    if (!group[i].basis_atoms_map.empty() && (group_str == "fgroup" || group_str == "sgroup")) {
      sscontent_json << "\"basis_atoms_map\":[" << aurostd::joinWDelimiter(group[i].basis_atoms_map, ",") << "]" << eendl;
    } else if (group_str == "fgroup" || group_str == "sgroup") {
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"basis_atoms_map\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    // basis_types_map
    if (!group[i].basis_types_map.empty() && (group_str == "fgroup" || group_str == "sgroup")) {
      sscontent_json << "\"basis_types_map\":[" << aurostd::joinWDelimiter(group[i].basis_types_map, ",") << "]" << eendl;
    } else if (group_str == "fgroup" || group_str == "sgroup") {
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"basis_types_map\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].ctrasl.rows && group_str == "sgroup") {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"ctrasl\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
    ...
    if (group[i].ftrasl.rows && group_str == "sgroup") {
      ...
      if (PRINT_NULL_JSON) {
        sscontent_json << "\"ftrasl\":null" << eendl;
      }
    }
    vcontent_json.push_back(sscontent_json.str());
    sscontent_json.str("");
...
string EquivalentAtomsToJson(vector<vector<int>>& iatoms) {
  ...
  if (!iatoms.empty()) {
    ...
    for (size_t iat1 = 0; iat1 < iatoms.size(); iat1++) {
    ...
    if (PRINT_NULL_JSON) {
      sscontent_json << "\"inequivalent_atoms\":null" << eendl;
    }
  }
  vcontent_json.push_back(sscontent_json.str());
  sscontent_json.str("");
  ...
  if (!iatoms.empty()) {
    ...
    for (size_t iat1 = 0; iat1 < iatoms.size(); iat1++) {
      sscontent_json << "[" << eendl;
      for (size_t iat2 = 0; iat2 < iatoms[iat1].size(); iat2++) {
    ...
    if (PRINT_NULL_JSON) {
      sscontent_json << "\"equivalent_sets\":null" << eendl;
    }
  }
  vcontent_json.push_back(sscontent_json.str());
  sscontent_json.str("");
...
bool KBIN_SymmetryWrite(ofstream& FileMESSAGE, xstructure& a, _aflags& aflags, char mode, const bool& osswrite, ostream& oss, const string& format) { // DX20170802
  stringstream message;
  ostringstream aus;
  xvector<double> aux_rrr(9);
  xvector<double> aux_ijk(9);
  ...
  if (mode == _PGROUP_) {
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUP Symmetry: writing BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _PGROUPK_) {
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUPK Symmetry: writing BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _FGROUP_) {
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "FGROUP Symmetry: writing BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _SGROUP_) {
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "SGROUP Symmetry: writing BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _AGROUP_) {
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "AGROUP Symmetry: writing BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _IATOMS_) {
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "IATOMS Symmetry: writing BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _PGROUP_XTAL_) {
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUP_XTAL Symmetry: writing BEGIN " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _PGROUPK_XTAL_) {
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUPK_XTAL Symmetry: writing BEGIN " << Message(__AFLOW_FILE__, aflags) << endl; // DX20171205 - Added pgroupk_xtal
  }
  if (mode == _PGROUPK_PATTERSON_) {
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUPK_PATTERSON Symmetry: writing BEGIN " << Message(__AFLOW_FILE__, aflags) << endl; // DX20200129
  }
  aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
  if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
    if (mode == _PGROUP_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUP_OUT;
    }
    if (mode == _PGROUPK_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUPK_OUT;
    }
    if (mode == _FGROUP_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_FGROUP_OUT;
    }
    if (mode == _SGROUP_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_SGROUP_OUT;
    }
    if (mode == _AGROUP_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_AGROUP_OUT;
    }
    if (mode == _IATOMS_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_IATOMS_OUT;
    }
    if (mode == _PGROUP_XTAL_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUP_XTAL_OUT;
    }
    if (mode == _PGROUPK_XTAL_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUPK_XTAL_OUT; // DX20171205 - Added pgroupk_xtal
    }
    if (mode == _PGROUPK_PATTERSON_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUPK_PATTERSON_OUT; // DX20200129
    }
  }
  if (aurostd::toupper(format) == "JSON") { // DX20200206
    if (mode == _PGROUP_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUP_JSON;
    }
    if (mode == _PGROUPK_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUPK_JSON;
    }
    if (mode == _FGROUP_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_FGROUP_JSON;
    }
    if (mode == _SGROUP_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_SGROUP_JSON;
    }
    if (mode == _AGROUP_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_AGROUP_JSON;
    }
    if (mode == _IATOMS_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_IATOMS_JSON;
    }
    if (mode == _PGROUP_XTAL_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUP_XTAL_JSON;
    }
    if (mode == _PGROUPK_XTAL_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUPK_XTAL_JSON; // DX20171205 - Added pgroupk_xtal
    }
    if (mode == _PGROUPK_PATTERSON_) {
      FileNameOUTPUT = aflags.Directory + "/" + DEFAULT_AFLOW_PGROUPK_PATTERSON_JSON; // DX20200129
    }
  }
  ...
  if (mode == _PGROUP_) {
    ...
    if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
      ...
      for (size_t k = 0; k < a.pgroup.size(); k++) {
        for (int i = 0; i < 9; i++) {
          aux_rrr(i + 1) = a.pgroup[k].Uc((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        for (int i = 0; i < 9; i++) {
          aux_ijk(i + 1) = a.pgroup[k].Uf((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        if (k == 0) {
          FileOUTPUT << SEPARATION_LINE_DASH << endl;
        }
    ...
    FileOUTPUT.clear();
    FileOUTPUT.close();
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUP Symmetry: writing END " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _PGROUP_XTAL_) {
    ...
    if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
      ...
      for (size_t k = 0; k < a.pgroup_xtal.size(); k++) {
        for (int i = 0; i < 9; i++) {
          aux_rrr(i + 1) = a.pgroup_xtal[k].Uc((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        for (int i = 0; i < 9; i++) {
          aux_ijk(i + 1) = a.pgroup_xtal[k].Uf((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        if (k == 0) {
          FileOUTPUT << SEPARATION_LINE_DASH << endl;
        }
    ...
    FileOUTPUT.clear();
    FileOUTPUT.close();
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUP_XTAL Symmetry: writing END " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  // DX20171205 - Added pgroupk_xtal - START
  if (mode == _PGROUPK_XTAL_) {
    ...
    if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
      ...
      for (size_t k = 0; k < a.pgroupk_xtal.size(); k++) {
        for (int i = 0; i < 9; i++) {
          aux_rrr(i + 1) = a.pgroupk_xtal[k].Uc((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        for (int i = 0; i < 9; i++) {
          aux_ijk(i + 1) = a.pgroupk_xtal[k].Uf((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        if (k == 0) {
          FileOUTPUT << SEPARATION_LINE_DASH << endl;
        }
    ...
    FileOUTPUT.clear();
    FileOUTPUT.close();
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUPK_XTAL Symmetry: writing END " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  // DX20171205 - Added pgroupk_xtal - END
  if (mode == _PGROUPK_) {
    ...
    if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
      ...
      for (size_t k = 0; k < a.pgroupk.size(); k++) {
        for (int i = 0; i < 9; i++) {
          aux_rrr(i + 1) = a.pgroupk[k].Uc((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        for (int i = 0; i < 9; i++) {
          aux_ijk(i + 1) = a.pgroupk[k].Uf((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        if (k == 0) {
          FileOUTPUT << SEPARATION_LINE_DASH << endl;
        }
    ...
    FileOUTPUT.clear();
    FileOUTPUT.close();
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUPK Symmetry: writing END " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _PGROUPK_PATTERSON_) { // DX20200129
    ...
    if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
      ...
      for (size_t k = 0; k < a.pgroupk_Patterson.size(); k++) {
        for (int i = 0; i < 9; i++) {
          aux_rrr(i + 1) = a.pgroupk_Patterson[k].Uc((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        for (int i = 0; i < 9; i++) {
          aux_ijk(i + 1) = a.pgroupk_Patterson[k].Uf((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        if (k == 0) {
          FileOUTPUT << SEPARATION_LINE_DASH << endl;
        }
    ...
    FileOUTPUT.clear();
    FileOUTPUT.close();
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "PGROUPK_PATTERSON Symmetry: writing END " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _FGROUP_) {
    ...
    if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
      ...
      for (size_t k = 0; k < a.fgroup.size(); k++) {
        for (int i = 0; i < 9; i++) {
          aux_rrr(i + 1) = a.fgroup[k].Uc((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        for (int i = 0; i < 9; i++) {
          aux_ijk(i + 1) = a.fgroup[k].Uf((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        if (k == 0) {
          FileOUTPUT << SEPARATION_LINE_DASH << endl;
        }
    ...
    FileOUTPUT.clear();
    FileOUTPUT.close();
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "FGROUP Symmetry: writing END " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _SGROUP_) {
    ...
    if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
      ...
      for (size_t k = 0; k < a.sgroup.size(); k++) {
        for (int i = 0; i < 9; i++) {
          aux_rrr(i + 1) = a.sgroup[k].Uc((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        for (int i = 0; i < 9; i++) {
          aux_ijk(i + 1) = a.sgroup[k].Uf((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
        }
        if (k == 0) {
          FileOUTPUT << SEPARATION_LINE_DASH << endl;
        }
    ...
    FileOUTPUT.clear();
    FileOUTPUT.close();
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "SGROUP Symmetry: writing END " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _AGROUP_) {
    ...
    if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
      ...
      for (size_t iat = 0; iat < a.atoms.size(); iat++) {
        ...
        for (size_t k = 0; k < a.agroup.at(iat).size(); k++) {
          for (int i = 0; i < 9; i++) {
            aux_rrr(i + 1) = a.agroup.at(iat)[k].Uc((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
          }
          for (int i = 0; i < 9; i++) {
            aux_ijk(i + 1) = a.agroup.at(iat)[k].Uf((i / 3) + 1, aurostd::mod(i, 3) + 1); // put in rows
          }
          if (k == 0) {
            FileOUTPUT << SEPARATION_LINE_DASH << endl;
          }
    ...
    FileOUTPUT.clear();
    FileOUTPUT.close();
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "AGROUP Symmetry: writing END " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  if (mode == _IATOMS_) {
    ...
    if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
      ...
      FileOUTPUT << SEPARATION_LINE_DASH << endl;
      for (size_t iat1 = 0; iat1 < a.iatoms.size(); iat1++) {
        FileOUTPUT << " [" << a.iatoms[iat1].at(0) << "]  ";
        for (size_t iat2 = 0; iat2 < a.iatoms[iat1].size(); iat2++) {
          FileOUTPUT << a.iatoms[iat1][iat2] << " ";
        }
        FileOUTPUT << endl;
      }
      FileOUTPUT << SEPARATION_LINE_DASH << endl;
      const bool temp = a.write_inequivalent_flag;
      a.write_inequivalent_flag = true;
      FileOUTPUT << a;
    ...
    FileOUTPUT.clear();
    FileOUTPUT.close();
    aus << XPID << (aflags.QUIET ? "" : "00000  MESSAGE ") << "IATOMS Symmetry: writing END " << Message(__AFLOW_FILE__, aflags) << endl;
  }
  aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET, osswrite, oss);
  return Krun;
}
...
bool KBIN_SymmetryToScreen(xstructure& a, const string& format, ostream& oss, char mode) {
  ...
  if (aurostd::toupper(format) == "TXT" || aurostd::toupper(format) == "TEXT") { // DX20200206
    ...
    if (mode == '\0' || mode == _IATOMS_) {
      ...
      oss << SEPARATION_LINE_DASH << endl;
      for (size_t iat1 = 0; iat1 < a.iatoms.size(); iat1++) {
        oss << " [" << a.iatoms[iat1].at(0) << "]  ";
        for (size_t iat2 = 0; iat2 < a.iatoms[iat1].size(); iat2++) {
          oss << a.iatoms[iat1][iat2] << " ";
        }
        oss << endl;
      }
      oss << SEPARATION_LINE_DASH << endl;
  ...
  if (aurostd::toupper(format) == "JSON") { // DX20200206
    ...
    if (mode == '\0' || mode == _PGROUPK_XTAL_) { // DX20171205 - Added pgroupk_xtal
      ...
      vcontent_json.push_back(sscontent_json.str());
      sscontent_json.str(""); // DX20171205 - Added pgroupk_xtal
  ...
  switch (mode) {
    ...
    case _AGROUP_:
      ...
      for (size_t iat = 0; iat < a.atoms.size(); iat++) {
        ...
        sscontent_txt << SEPARATION_LINE_DASH << endl;
        for (size_t k = 0; k < a.agroup[iat].size(); k++) {
...
bool KBIN_StepSymmetryPerform(xstructure& a, string AflowIn, ofstream& FileMESSAGE, _aflags& aflags, _kflags& kflags, const bool& osswrite, ostream& oss) {
  ...
  if (kflags.KBIN_SYMMETRY_CALCULATION == true) {
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_); // DX20210703
...
namespace SYM {
  void writePythonScript(ostream& oss) {
    ...
    const string sym_python_subdir = "AFLOW_SYM_PYTHON";
    const string python_dir = directory + "/" + sym_python_subdir;
    ...
    aurostd::EmbData::save_to_file("aflow_sym_python.py", "SCRIPTS", python_dir + "/" + "aflow_sym_python.py");
  }
} // namespace SYM
