...
namespace SYM {
  string spacegroup2latticeAndCentering(uint sg_number) {
    ...
    if (sg_number >= 3 && sg_number <= 15) {
      // simple monoclinic
      if (sg_number == 3 || sg_number == 4 || sg_number == 6 || sg_number == 7 || sg_number == 10 || sg_number == 11 || sg_number == 13 || sg_number == 14) {
        return "mP";
      }
    ...
    if (sg_number >= 75 && sg_number <= 142) {
      // simple tetragonal
      if ((sg_number >= 75 && sg_number <= 78) || (sg_number == 81) || (sg_number >= 83 && sg_number <= 86) || (sg_number >= 89 && sg_number <= 96) || (sg_number >= 99 && sg_number <= 106) ||
          (sg_number >= 111 && sg_number <= 118) || (sg_number >= 123 && sg_number <= 138)) {
        return "tP";
      }
      // body-centered tetragonal
      else if ((sg_number >= 79 && sg_number <= 80) || (sg_number == 82) || (sg_number >= 87 && sg_number <= 88) || (sg_number >= 97 && sg_number <= 98) || (sg_number >= 107 && sg_number <= 110) ||
               (sg_number >= 119 && sg_number <= 122) || (sg_number >= 139 && sg_number <= 142)) {
        return "tI";
      }
    }
    ...
    if (sg_number >= 143 && sg_number <= 167) {
      // trigonal
      if ((sg_number >= 143 && sg_number <= 145) || (sg_number == 147) || (sg_number >= 149 && sg_number <= 154) || (sg_number >= 156 && sg_number <= 159) || (sg_number >= 162 && sg_number <= 165)) {
        return "hP";
      }
      // rhombohedral
      else if ((sg_number == 146) || (sg_number == 148) || (sg_number == 155) || (sg_number >= 160 && sg_number <= 161) || (sg_number >= 166 && sg_number <= 167)) {
        return "hR";
      }
    }
    ...
    if (sg_number >= 195 && sg_number <= 230) {
      // simple cubic
      if (sg_number == 195 || sg_number == 198 || sg_number == 200 || sg_number == 201 || sg_number == 205 || sg_number == 207 || sg_number == 208 || sg_number == 212 || sg_number == 213 || sg_number == 215 ||
          sg_number == 218 || sg_number == 221 || sg_number == 222 || sg_number == 223 || sg_number == 224) {
        return "cP";
      }
      // face-centered cubic
      else if (sg_number == 196 || sg_number == 202 || sg_number == 203 || sg_number == 209 || sg_number == 210 || sg_number == 216 || sg_number == 219 || sg_number == 225 || sg_number == 226 ||
               sg_number == 227 || sg_number == 228) {
        return "cF";
      }
      // body-centered cubic
      else if (sg_number == 197 || sg_number == 199 || sg_number == 204 || sg_number == 206 || sg_number == 211 || sg_number == 214 || sg_number == 217 || sg_number == 220 || sg_number == 229 || sg_number == 230) {
        return "cI";
      }
    }
...
namespace SYM {
  bool getAtomGCD(deque<_atom>& atomic_basis, deque<deque<_atom>>& split_atom_types, int& gcd_num) {
    ...
    if (split_atom_types.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, "SYM::getAtomGCD():", "split_atom_types.size()==0", _INPUT_ERROR_);
    }
    ...
    if (split_atom_types.size() > 1) { // If only one type of atom, set to 1
      ...
      for (size_t p = 0; p < split_atom_types.size() - 1; p++) {
...
namespace SYM {
  bool orientVectorsPositiveQuadrant(vector<xvector<double>> lattice_vectors, double& tol) {
    for (size_t i = 0; i < lattice_vectors.size(); i++) {
  ...
  vector<double> ExtractLatticeParametersFromWyccar(const vector<string>& wyccar_ITC) { // DX20191030 - added const
    ...
    if (wyccar_ITC.size() > 3) {
      ...
      for (size_t i = 0; i < tokens.size(); i++) {
        if (i < 6) {
          lattice_parameters.push_back(aurostd::string2utype<double>(tokens[i]));
        }
      }
    } else {
...
namespace SYM {
  string ExtractWyckoffAttributeString(const vector<string>& wyccar_ITC, uint attribute_index) { // DX20191030 - added const
    ...
    string prev_atom_name;
    string wyckoff_letter;
    for (size_t i = 0; i < wyccar_ITC.size(); i++) {
...
//  ******************************************************************************
namespace SYM {
  vector<vector<vector<string>>> getWyckoffLettersWithSameMultiplcityAndSiteSymmetry(uint& space_group_number, vector<GroupedWyckoffPosition>& grouped_Wyckoff_positions, uint& cell_choice) {
    ...
    if (LDEBUG) {
      cerr << "SYM::getWyckoffLettersWithSameMultiplcityAndSiteSymmetry(): Lattice type (unique axis if specified) and cell choice  : " << axis_cell.str() << endl;
    }
    SymmetryInformationITC ITC_sym_info; // DX20190215
    ITC_sym_info.initsgs(axis_cell.str()); // DX20190215

    const string spacegroupstring = ITC_sym_info.gl_sgs[space_group_number - 1];
    vector<vector<vector<string>>> grouped_possible_Wyckoff_letters;
    for (size_t i = 0; i < grouped_Wyckoff_positions.size(); i++) {
      vector<vector<string>> possible_Wyckoff_letters;
      for (size_t j = 0; j < grouped_Wyckoff_positions[i].site_symmetries.size(); j++) {
...
namespace SYM {
  vector<string> splitSiteSymmetry(const string& site_symmetry) { // DX20190730 - added const
    ...
    stringstream ss_primary;
    stringstream ss_secondary;
    stringstream ss_tertiary;
    ...
    if (site_symmetry.size() == 2) {
      ...
      if (site_symmetry[0] == '-') {
        ...
        ss_primary << site_symmetry[0];
        ss_secondary << site_symmetry[1];
    ...
    if (site_symmetry.size() == 3 && aurostd::substring2bool(site_symmetry, "-")) {
      if (site_symmetry[0] == '-') {
        ss_primary << site_symmetry[0] << site_symmetry[1];
        ss_secondary << site_symmetry[2];
        split_site_symmetry.push_back(ss_primary.str());
        split_site_symmetry.push_back(ss_secondary.str());
        ss_primary.str("");
        ss_secondary.str("");
        ...
        ss_primary << site_symmetry[0];
        ss_secondary << site_symmetry[1] << site_symmetry[2];
        split_site_symmetry.push_back(ss_primary.str());
        split_site_symmetry.push_back(ss_secondary.str());
        ss_primary.str("");
        ss_secondary.str("");
    ...
    if (aurostd::substring2bool(site_symmetry, ".")) {
      for (size_t i = 0; i < site_symmetry.size(); i++) {
        if (site_symmetry[i] == '.') {
          dot_index.push_back(i);
          if (i == 0) {
            dot_direction.emplace_back("primary");
          } else if (i < site_symmetry.size() - 1) {
            dot_direction.emplace_back("secondary");
          } else if (i == site_symmetry.size() - 1) {
            dot_direction.emplace_back("tertiary");
          }
        }
      }
...
//    for(size_t j=0;j<expanded_basis.size();j++){
...
//  //bubble sort
//
//  for(size_t i=0;i<all_dist_vec.size();i++){
//    for(size_t j=0;j<all_dist_vec[i].size();j++){
//      for(size_t k=0;k<all_dist_vec[i].size();k++){
...
namespace SYM {
  double orthogonality_defect(xmatrix<double> lattice) {
    const double a = sqrt(lattice(1, 1) * lattice(1, 1) + lattice(1, 2) * lattice(1, 2) + lattice(1, 3) * lattice(1, 3));
    const double b = sqrt(lattice(2, 1) * lattice(2, 1) + lattice(2, 2) * lattice(2, 2) + lattice(2, 3) * lattice(2, 3));
    const double c = sqrt(lattice(3, 1) * lattice(3, 1) + lattice(3, 2) * lattice(3, 2) + lattice(3, 3) * lattice(3, 3));
    const double det = aurostd::abs(aurostd::determinant(lattice));
    return a * b * c / det;
  }
} // namespace SYM
  ...
  void ReducedRowEchelonForm(xmatrix<double>& M, double& tol) { // DX20190215 -added tol
    ...
    for (uint r = 0; r < rowCount; r++) {
      ...
      if (lead != columnCount) {
        ...
        const double divisor = M(r + 1, lead);
...
namespace SYM {
  bool checkLinearSystem(vector<xvector<double>>& LHS, vector<double>& RHS, xmatrix<double>& lattice, double& tol) { // DX20190215 -added tol
    ...
    for (uint i = 0; i < numRows; i++) {
      ...
      if (allzero == true) {
        ...
        if (aurostd::abs(RHS[i]) > tol) {
          ...
          if (aurostd::abs(aurostd::abs(RHS[i]) - aurostd::abs(lattice(1)[xyz])) < tol || aurostd::abs(aurostd::abs(RHS[i]) - aurostd::abs(lattice(2)[xyz])) < tol ||
    ...
    for (size_t v = 0; v < LHS.size(); v++) {
      const int xyz_v = v % 3 + 1;
      for (size_t u = v + 1; u < LHS.size(); u++) {
        const int xyz_u = u % 3 + 1;
        if (aurostd::abs(LHS[v][1] - LHS[u][1]) < tol && aurostd::abs(LHS[v][2] - LHS[u][2]) < tol && aurostd::abs(LHS[v][3] - LHS[u][3]) < tol) {
          if (aurostd::abs(RHS[v] - RHS[u]) > tol) {
            if (aurostd::abs(aurostd::abs(RHS[v] - RHS[u]) - aurostd::abs(lattice(1)[xyz_v])) < tol || aurostd::abs(aurostd::abs(RHS[v] - RHS[u]) - aurostd::abs(lattice(2)[xyz_v])) < tol ||
                aurostd::abs(aurostd::abs(RHS[v] - RHS[u]) - aurostd::abs(lattice(3)[xyz_v])) < tol || aurostd::abs(aurostd::abs(RHS[v] - RHS[u]) - aurostd::abs(lattice(1)[xyz_u])) < tol ||
                aurostd::abs(aurostd::abs(RHS[v] - RHS[u]) - aurostd::abs(lattice(2)[xyz_u])) < tol || aurostd::abs(aurostd::abs(RHS[v] - RHS[u]) - aurostd::abs(lattice(3)[xyz_u])) < tol) {
              continue;
            }
            return false;
          }
...
namespace SYM {
  bool solve_overdetermined_system(vector<xvector<double>>& LHS, vector<double>& RHS, xvector<double>& SOL, xmatrix<double>& lattice, double& min_dist, double& tol) { // DX20190215 - added tol
    ...
    for (size_t i = 0; i < LHS.size(); i += 3) {
      ...
      b = LHS[i + 1];
      c = LHS[i + 2];
      const xmatrix<double> lhs_tmp = xvec2xmat(a, b, c);
      const xmatrix<double> lhs_cart_mat = f2c * lhs_tmp * inverse(f2c);
      ...
      if (skew) {
        ...
        const xvector<double> rhs_frac_xvec = SYM::minimizeDistanceFractionalMethod(rhs_tmp); // DX20190613
    ...
    for (size_t i = 0; i < LHS.size(); i++) {
      for (size_t j = 1; j < LHS.size(); j++) {
        for (size_t k = 2; k < LHS.size(); k++) {
      ...
      for (uint j = 0; j < 3; j++) {
        for (uint k = 0; k < 3; k++) {
          ...
          for (size_t ii = 0; ii < LHS_cart.size(); ii += 3) {
            ...
            if (skew) {
              const xvector<double> origin;
              const xvector<double> coord_cart = f2c * coord;
              const xvector<double> min_coord_cart = SYM::minimizeDistanceCartesianMethod(origin, coord_cart, lattice); // DX20190613
              min_dist = aurostd::modulus(min_coord_cart); // DX20190613
            } else {
              const xvector<double> min_frac_coord = SYM::minimizeDistanceFractionalMethod(coord); // DX20190613
...
namespace SYM {
  vector<string> get_minimum_enumerated_Wyckoff_letters(string spacegroupstring, vector<int>& multiplicities, vector<string> site_symmetries) {
    ...
    for (size_t i = 0; i < multiplicities.size(); i++) {
      ...
      const vector<int> enumerated_letters;
      int min_enumerated_letter = 1e9;
      string min_letter;
      for (size_t j = 0; j < letters.size(); j++) {
        const int enumerated_letter = SYM::enumerate_wyckoff_letter(letters[j]);
        if (enumerated_letter < min_enumerated_letter) {
          bool contains_variable = false;
          if (aurostd::substring2bool(all_positions[j][0], "x") || aurostd::substring2bool(all_positions[j][0], "y") || aurostd::substring2bool(all_positions[j][0], "z")) {
          ...
          for (size_t k = 0; k < minimum_enumerated_Wyckoff_letters.size(); k++) {
    ...
    for (uint i = 0; i < spaceg.length(); i++) {
      ...
      if (line > 1) {
        ...
        if (spaceg[i] == 0x0a) {
          ...
          temp >> letter;
          temp >> site_symm;
          const int tmp_mult = atoi(multiplicity.c_str());
...
// DX20191029 END
namespace SYM {
  void get_Wyckoff_from_letter(uint space_group_number, string& space_group_setting, string& Wyckoff_letter, uint& Wyckoff_multiplicity, string& site_symmetry, vector<string>& positions) {
...
// DX20190128 START
namespace SYM {
  void get_Wyckoff_from_letter(string& spaceg, string& Wyckoff_letter, uint& Wyckoff_multiplicity, string& site_symmetry, vector<string>& positions) { // DX20190128 - int to uint
    stringstream temp;
    int line = 0;
    for (uint i = 0; i < spaceg.length(); i++) {
      if (spaceg[i] == 0x0a) {
...
namespace SYM {
  xvector<double> Wyckoff_position_string2xvector(string& string_position) {
    ...
    for (size_t i = 0; i < tokens.size(); i++) {
      ...
      if (aurostd::substring2bool(tokens[i], "/")) {
        ...
        if (frac_tokens.size() == 2) {
          const double numerator = aurostd::string2utype<double>(frac_tokens[0]);
          const double denominator = aurostd::string2utype<double>(frac_tokens[1]);
    ...
    // string tmp_num;
    // int coord = 1;
    // for (size_t i=0; i<string_postition.size(); i++) {
...
namespace SYM {
  vector<xvector<double>> get_possible_origin_shifts(string spacegroupstring, int multiplicity, string site_symmetry) {
    ...
    if (LDEBUG) {
      ...
      for (size_t s = 0; s < same_site_symmetry_positions.size(); s++) {
        print(same_site_symmetry_positions[s]);
        xb();
      }
    }
    for (size_t s = 0; s < same_site_symmetry_positions.size(); s++) {  // DX20170911 - Check all shifts not just minimum (minimum may not work)
      for (size_t t = 0; t < same_site_symmetry_positions[s].size(); t++) {  // DX20170911 - Check all shifts not just minimum (minimum may not work)
        const xvector<double> candidate_shift = SYM::Wyckoff_position_string2xvector(same_site_symmetry_positions[s][t]);
        bool stored_shift = false;
        for (size_t p = 0; p < possible_shifts.size(); p++) {
        ...
        if (!stored_shift) {
          ...
          for (size_t p = 0; p < possible_shifts.size(); p++) {
            for (size_t q = 0; q < possible_shifts.size(); q++) {
...
namespace SYM {
  void getWyckoffAutomorphismSets(const string& spaceg, int mult, string site_symmetry, vector<string>& site_symmetries, vector<string>& letters, vector<vector<string>>& all_positions, bool origin_shifts_only) {
    ...
    string site_symmetry_base = site_symmetry;
    string site_symmetry_ITC_base;
    ...
    vector<string> all_Wyckoff_strings;
    ...
    for (size_t i = 2; i < all_Wyckoff_strings.size(); i++) { // DX 20191107 starting at 2 since the first two lines are header and centering, repectively
      ...
      if (Wyckoff_tokens.size() > 3) {
        ...
        if (multiplicity == mult) {
          ...
          if (site_symmetry_ITC_base == site_symmetry_base) {
            ...
            letters.push_back(letter);
            vector<string> positions;
            for (size_t p = 3; p < Wyckoff_tokens.size(); p++) {
...
namespace SYM {
  vector<string> get_wyckoff_equation(string spaceg, int mult) {
    ...
    if (!aurostd::WithinList(mult_vec, mult)) { // DX20210422 - SYM::invec() -> aurostd::WithinList()
      if (LDEBUG) {
        cerr << "SYM::get_wyckoff_equation: WARNING: no wyckoff position with multiplicity " << mult << "." << endl;
      }
      vector<string> none;
      return none;
    }
    vector<int> index;
    for (size_t i = 0; i < mult_vec.size(); i++) {
      // if(mult_vec[i]*get_centering(spaceg).size()==mult)
      if (mult_vec[i] == mult) { // CO20200106 - patching for auto-indenting
        // cerr <<"match: " << i << endl;
        index.push_back(i);
      }
    }
    ...
    for (size_t l = index[0]; l < index[0] + index.size(); l++) { // CO20200106 - patching for auto-indenting
...
namespace SYM {
  vector<vector<string>> get_wyckoff_pos(string spaceg, int Wyckoff_multiplicity, string Wyckoff_letter) {
    const bool LDEBUG = (false || XHOST.DEBUG);
    const vector<int> mult_vec = get_multiplicities(spaceg);
    // Error if mult is not contained in mult_vec (i.e., a wyckoff position with multiplicity mult does not exist for the space group spaceg)
    if (!aurostd::WithinList(mult_vec, Wyckoff_multiplicity)) { // DX20210422 - SYM::invec() -> aurostd::WithinList()
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << ": WARNING: no wyckoff position with multiplicity " << Wyckoff_multiplicity << "." << endl;
      }
    ...
    for (size_t i = 2; i < all_Wyckoff_strings.size(); i++) { // DX20191107 starting at 2 since the first two lines are header and centering, repectively
      aurostd::string2tokens(all_Wyckoff_strings[i], Wyckoff_tokens, " ");
      ...
      if (Wyckoff_tokens.size() > 3) {
        letter = Wyckoff_tokens[1];
        if (letter == Wyckoff_letter) {
          multiplicity = Wyckoff_tokens[0];
          site_symmetry = Wyckoff_tokens[2];
          for (size_t t = 3; t < Wyckoff_tokens.size(); t++) { // DX20191030 - need loop to get all positions
            string position = aurostd::RemoveWhiteSpaces(Wyckoff_tokens[t]);
            position = aurostd::RemoveCharacter(position, '\n');
            if (!position.empty()) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << ": Wyckoff positions without centering(s): " << aurostd::joinWDelimiter(all_positions, " ") << endl;
    }
    ...
    for (size_t i = 0; i < all_positions.size(); i++) {
      ...
      for (size_t t = 0; t < tokens.size(); t++) {
        string pos = aurostd::StringSubst(tokens[t], "(", "");
        aurostd::StringSubstInPlace(pos, ")", "");
    ...
    for (size_t i = 0; i < get_centering(spaceg).size(); i++) {
      for (size_t j = 0; j < non_shifted_Wyckoff_positions.size(); j++) {
        vector<string> vec_pos;
        for (size_t k = 0; k < non_shifted_Wyckoff_positions[j].size(); k++) {
    ...
    if (LDEBUG) {
      vector<string> tmp;
      for (size_t i = 0; i < all_Wyckoff_positions.size(); i++) {
        tmp.push_back("(" + aurostd::joinWDelimiter(all_Wyckoff_positions[i], ",") + ")");
      }
...
namespace SYM {
  ...
  vector<vector<string>> getWyckoffEquations(const uint space_group_number, const string& space_group_setting, const string& Wyckoff_letter) {
    ...
    SymmetryInformationITC ITC_sym_info;
    ITC_sym_info.initsgs(space_group_setting);
    const string Wyckoff_string = ITC_sym_info.gl_sgs[space_group_number - 1];
...
namespace SYM {
  // ---------------------------------------------------------------------------
  // input: space group number + setting
  void getWyckoffInformation(const uint space_group_number, const string& space_group_setting, const string& Wyckoff_letter, uint& Wyckoff_multiplicity, string& site_symmetry, vector<vector<string>>& all_positions) {
    // get Wyckoff information based on space group and setting
    SymmetryInformationITC ITC_sym_info;
    ITC_sym_info.initsgs(space_group_setting);
    const string Wyckoff_string = ITC_sym_info.gl_sgs[space_group_number - 1];
  ...
  // ---------------------------------------------------------------------------
  // input: space group number + setting
  void getWyckoffInformation(const string& Wyckoff_string, const string& Wyckoff_letter, uint& Wyckoff_multiplicity, string& site_symmetry, vector<vector<string>>& all_positions) {
    const bool LDEBUG = (false || XHOST.DEBUG);
    const bool reduce = true; // DEBUGGING variable: simplify/reduce Wyckoff coordinates (e.g., 0.25+0.5 -> 0.75)
    vector<string> split_Wyckoff_strings;
    vector<string> Wyckoff_tokens;
    vector<string> positions;
    ...
    for (size_t i = 2; i < split_Wyckoff_strings.size(); i++) { // DX20191107 starting at 2 since the first two lines are header and centering, repectively
      aurostd::string2tokens(split_Wyckoff_strings[i], Wyckoff_tokens, " ");

      // expected sequence: "24 h ..2 (x, y, z) (x, 0, z) ..."
      if (Wyckoff_tokens.size() > 3) {
        const string letter = Wyckoff_tokens[1];
        if (letter == Wyckoff_letter) {
          Wyckoff_multiplicity = aurostd::string2utype<uint>(Wyckoff_tokens[0]);
          site_symmetry = Wyckoff_tokens[2];
          for (size_t t = 3; t < Wyckoff_tokens.size(); t++) { // DX20191030 - need loop to get all positions
            string position = aurostd::RemoveWhiteSpaces(Wyckoff_tokens[t]);
            position = aurostd::RemoveCharacter(position, '\n');
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << ": Wyckoff positions without centering(s): " << aurostd::joinWDelimiter(positions, " ") << endl;
    }
    ...
    for (size_t i = 0; i < positions.size(); i++) {
      ...
      for (size_t t = 0; t < tokens.size(); t++) {
        ...
        if (reduce) {
          const vector<sdouble> sd_coordinate = simplify(pos);
    ...
    tmp_Wyckoff_equation.str(std::string());
    tmp_Wyckoff_equation.clear();
    for (size_t i = 0; i < get_centering(Wyckoff_string).size(); i++) {
      for (size_t j = 0; j < non_shifted_Wyckoff_positions.size(); j++) {
        vector<string> vec_pos;
        for (size_t k = 0; k < non_shifted_Wyckoff_positions[j].size(); k++) {
          tmp_Wyckoff_equation << non_shifted_Wyckoff_positions[j][k];
          ...
          // split into equation entities (number, variable)
          if (reduce) {
            const vector<sdouble> sd_coordinate = simplify(tmp_Wyckoff_equation.str());
            vec_pos.push_back(SYM::formatWyckoffPosition(sd_coordinate));
          } else {
            vec_pos.push_back(tmp_Wyckoff_equation.str());
          }
    ...
    if (LDEBUG) {
      ...
      for (size_t i = 0; i < all_positions.size(); i++) {
        tmp_vposition.push_back("(" + aurostd::joinWDelimiter(all_positions[i], ",") + ")");
      }
...
namespace SYM {
  vector<vector<vector<string>>> get_wyckoff_pos(string spaceg, int mult) {
    ...
    // DISPLAY CENTRING
    // for(size_t ii=0;ii<get_centering(spaceg).size();ii++){
    //   for(size_t j=0;j<get_centering(spaceg)[ii].size();j++){
    ...
    for (size_t w = 0; w < all_wyckoff_sets.size(); w++) {
      for (size_t ii = 0; ii < get_centering(spaceg).size(); ii++) {
        for (size_t k = 0; k < all_wyckoff_sets[w].size(); k++) {
          for (size_t j = 0; j < all_wyckoff_sets[w][k].size(); j++) // for(int j=0;j<3;j++)
          { // CO20200106 - patching for auto-indenting
            for (size_t i = 0; i < all_wyckoff_sets[w][k][j].size(); i++) {
    ...
    // for (size_t l=0;l<outvec.size(); l++){
    //	cerr << "outvec[l].size(): " << outvec[l].size() << endl;
    //   for (size_t m=0;m<outvec[l].size(); m++){
    //     cerr << "outvec[l][m].size(): " << outvec[l][m].size() << endl;
    //     for (size_t n=0;n<outvec[l][m].size(); n++){
...
namespace SYM {
  vector<vector<vector<string>>> get_wyckoff_pos(string spaceg, int mult, bool getcentering) {
    ...
    for (size_t l = index[0]; l < index[0] + index.size(); l++) {
      for (uint i = 0; i < sg_lines[l].length(); i++) {
        if (sg_lines[l][i] == '(') {
          start = i;
          break;
    ...
    int reg = 0; // Counter for tuplets
    int breg = 0; // Counter for multiplicity blocks
    for (size_t k = 0; k < wyckoff_positions.size(); k++) {
      // for(size_t ii=0;ii<get_centering(nocent).size();ii++)
      // for(uint ii=0;ii<C;ii++)
      //{ //CO20200106 - patching for auto-indenting
      for (size_t j = 0; j < wyckoff_positions[k].size(); j++) // for(int j=0;j<3;j++)
      { // CO20200106 - patching for auto-indenting
        for (size_t i = 0; i < wyckoff_positions[k][j].size(); i++) {
...
namespace SYM {
  vector<string> findGeneralWyckoffPosition(string& spacegroupstring, int& general_wyckoff_multiplicity) {
    ...
    const vector<vector<vector<string>>> testvvvstring = get_wyckoff_pos(spacegroupstring, general_wyckoff_multiplicity);

    // DEBUG for(size_t i=0;i<testvvvstring.size();i++){
    // DEBUG  for(size_t j=0;j<testvvvstring[i].size();j++){
    // DEBUG   ::print(testvvvstring[i][j]);
    // DEBUG  }
    //}
    const vector<vector<string>> centering = get_centering(spacegroupstring);
    vector<vector<vector<vector<sdouble>>>> testvvvsd = convert_wyckoff_pos_sd(testvvvstring);
    for (size_t j = 0; j < testvvvsd.size(); j++) {
      for (size_t m = 0; m < testvvvsd[j].size(); m++) {
        string equation;
        vector<string> eqn;
        for (uint k = 0; k < 3; k++) {
...
void wyckoffsite_ITC::getWyckoffFromLetter(uint space_group_number, const string& Wyckoff_letter, int setting) {
  SymmetryInformationITC ITC_sym_info;
  const string space_group_setting = aurostd::utype2string<int>(setting);
  ITC_sym_info.initsgs(space_group_setting);
  const string space_group_string = ITC_sym_info.gl_sgs[space_group_number - 1];
...
void wyckoffsite_ITC::getWyckoffFromLetter(const string& space_group_string, const string& Wyckoff_letter) {
  ...
  const vector<string> all_positions;
  vector<string> all_Wyckoff_strings;
  vector<string> Wyckoff_tokens;
  ...
  for (size_t i = 2; i < all_Wyckoff_strings.size(); i++) { // DX 20191107 starting at 2 since the first two lines are header and centering, repectively
    ...
    if (Wyckoff_tokens.size() > 3) {
      ...
      if (letter == Wyckoff_letter) {
        ...
        //}
        site_symmetry = Wyckoff_tokens[2];
        for (size_t t = 3; t < Wyckoff_tokens.size(); t++) { // DX 20191030 - need loop to get all positions
          string position = aurostd::RemoveWhiteSpaces(Wyckoff_tokens[t]);
          position = aurostd::RemoveCharacter(position, '\n');
          if (!position.empty()) {
            components.clear();
            ...
            for (size_t t = 0; t < components.size(); t++) {
              string component = aurostd::StringSubst(components[t], "(", "");
              aurostd::StringSubstInPlace(component, ")", "");
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << ": Wyckoff positions without centering(s): ";
    for (size_t i = 0; i < equations_no_centering.size(); i++) {
      cerr << aurostd::joinWDelimiter(equations_no_centering[i], ",") << endl;
    }
  }

  stringstream tmp_Wyckoff_equation;
  for (size_t i = 0; i < SYM::get_centering(space_group_string).size(); i++) {
    for (size_t j = 0; j < equations_no_centering.size(); j++) {
      vector<string> vec_pos;
      for (size_t k = 0; k < equations_no_centering[j].size(); k++) {
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << ": Wyckoff positions with centering(s): ";
...
namespace SYM {
  uint numberOccupiedSitesInConventionalCell(const vector<wyckoffsite_ITC>& Wyckoff_sites) {
    ...
    uint natoms = 0;
    for (size_t i = 0; i < Wyckoff_sites.size(); i++) {
      natoms += Wyckoff_sites[i].multiplicity;
    }
    return natoms;
  }
} // namespace SYM
...
namespace SYM {
  vector<uint> numberEachTypeFromWyckoff(const vector<wyckoffsite_ITC>& Wyckoff_sites) {
    ...
    for (size_t i = 0; i < grouped_Wyckoff_positions.size(); i++) {
      ...
      for (size_t j = 0; j < grouped_Wyckoff_positions[i].multiplicities.size(); j++) {
...
//  ******************************************************************************
namespace SYM {
  vector<string> findWyckoffEquations(uint space_group_number, string& space_group_setting, string& Wyckoff_letter, uint Wyckoff_multiplicity, bool convert2frac) { // DX20200423 - add convert2frac
...
namespace SYM {
  vector<string> findWyckoffEquations(string& spacegroupstring, string& Wyckoff_letter, uint Wyckoff_multiplicity, bool convert2frac, bool keep_multiplication_symbol) { // DX20200423 - add convert2frac
    vector<string> positions;
    const vector<vector<string>> testvvstring = get_wyckoff_pos(spacegroupstring, Wyckoff_multiplicity, Wyckoff_letter);
    vector<vector<vector<string>>> testvvvstring;
    testvvvstring.push_back(testvvstring); // quick fix, should probably fix (DX)
    vector<vector<vector<vector<sdouble>>>> testvvvsd = convert_wyckoff_pos_sd(testvvvstring);
    for (size_t j = 0; j < testvvvsd.size(); j++) {
      for (size_t m = 0; m < testvvvsd[j].size(); m++) {
        string equation;
...
namespace SYM {
  string formatWyckoffPosition(const vector<sdouble>& sd_coordinate, bool convert2frac, bool keep_multiplication_symbol, int precision) { // CO20220607 - added precision
    ...
    stringstream ss_eqn;
    ss_eqn.precision(precision); // CO20220607 - added precision
    string coordinate;
    ...
    for (size_t j = 0; j < sd_coordinate.size(); j++) {
      const sdouble sd_num = sd_coordinate[j];
      // ---------------------------------------------------------------------------
    ...
    if (double_only) {
      if (convert2frac) { // DX20190419
        const string running_frac = aurostd::dbl2frac(running_double, false);
        ss_eqn << running_frac;
      } // DX20190419
      // DX20190419 - START
      else {
      ...
      if (!aurostd::isequal(running_double, 0.0, _ZERO_TOL_)) { // DX20190718 - don't add +0 to the end
        const string running_frac = aurostd::dbl2frac(running_double, false); // DX20190724 - now namespace aurostd
        ss_eqn << running_frac;
        vec_coord.push_back(ss_eqn.str());
        ss_eqn.str("");
      }
    ...
    aurostd::StringSubstInPlace(coordinate, "-+", "-");
    aurostd::StringSubstInPlace(coordinate, "+-", "-");
...
namespace SYM {
  string reorderWyckoffPosition(const string& orig_position) {
    ...
    for (size_t i = 0; i < tokens.size(); i++) {
      // ---------------------------------------------------------------------------
      // split into equation entities (number, variable)
      const vector<sdouble> sd_coordinate = simplify(tokens[i]);
...
namespace SYM {
  bool shiftWyckoffPositions(deque<deque<_atom>>& equivalent_atoms_shifted, xvector<double>& previous_shift, xvector<double>& new_shift) {
    ...
    for (size_t i = 0; i < equivalent_atoms_shifted.size(); i++) {
      for (size_t j = 0; j < equivalent_atoms_shifted[i].size(); j++) {
...
namespace SYM {
  bool findWyckoffPositions(xstructure& CCell,
    ...
    for (size_t i = 0; i < equivalent_atoms_shifted.size(); i++) {
      const int mtmp = equivalent_atoms_shifted[i].size();
      ...
      for (size_t ixx = 0; ixx < wyckoffmult.size(); ixx++) {
      ...
      if (tmpvvvstring.empty()) {
        if (LDEBUG) {
          cerr << "SYM::findWyckoffPositions: Could not find wyckoff position based on multiplicity provided [1]. Return string = " << tmpvvvstring.size() << endl;
        }
      ...
      for (size_t j = 0; j < tmpvvvsd.size(); j++) { // LOOP OF WYCKOFF POSITIONS WITH MULTIPLICITY EQUAL TO EQUIVALENT ATOM SET:
        ...
        for (int m = 0; m < (int) wyckoff_set.size(); m++) {
          ...
          for (size_t ix = 0; ix < tmp_equivalent_atoms_shifted.size(); ix++) { // PERMUTE THROUGH ATOMS IN SET (first may not work)
            ...
            for (int k = 0; k < 3; k++) { // TAKE FIRST SYMMETRY-RELATED SITE TO ASSIGN VARIABLES (HENCE: [j][0])
              ...
              for (size_t l = 0; l < wyckoff_set[m][k].size(); l++) {
            ...
            for (uint i = 0; i < 3; i++) {
              for (uint j = 0; j < 3; j++) {
                for (uint k = 0; k < 3; k++) {
                  ...
                  // cerr << W_tmp << endl; //DEBUG MODE
                  xvector<double> solution;
                  solution(1) = (W_tmp(1, 4));
                  solution(2) = (W_tmp(2, 4));
                  solution(3) = (W_tmp(3, 4));
                  solution = SYM::minimizeDistanceFractionalMethod(solution); // DX20190613
                  const xvector<double> cart_solution = trasp(CCell.lattice) * solution;
                  ...
                  const xvector<double> ideal_cart_position = trasp(CCell.lattice) * ideal_wyckoff;
              ...
              for (int ixx = 1; ixx < 4; ixx++) {
                ...
                if (first_wyckoff == true) {
                  ...
                  if (CCell.partial_occupation_flag) {
                    ...
                    tmp_Wyckoff_site.site_occupation = 1.0;
                  }
                  ...
                  if (aurostd::string2tokens(tmp_Wyckoff_site.wyckoffSymbol, tokens, " ") == 3) {
                    ...
                    vector<string> positions = SYM::findWyckoffEquations(spacegroupstring, tmp_Wyckoff_site.letter, tmp_Wyckoff_site.multiplicity);
        ...
        if (found_count == equivalent_atoms_shifted[i].size()) {
          ...
          woss << setprecision(14) << fixed << "   " << wyckoff_atom.fpos(1) << "   " << wyckoff_atom.fpos(2) << "   " << wyckoff_atom.fpos(3) << "   " << wyckoff_atom.name << tmp_Wyckoff_site.wyckoffSymbol
               << endl; // GET WYCKOFF SITE SYMBOL FOR APPROPRIATE MULT
      ...
      if (found_count != equivalent_atoms_shifted[i].size() && orig_origin_shift == true) {
        ...
        if (CCell.lattice_label_ITC == 'R' && obverse_force_transformed == false) {
          ...
          for (size_t k = 0; k < equivalent_atoms.size(); k++) {
            for (size_t l = 0; l < equivalent_atoms[k].size(); l++) {
      ...
      if (tmpvvvstring.empty()) {
        if (LDEBUG) {
          cerr << "SYM::findWyckoffPositions: Could not find wyckoff position based on multiplicity provided [2]. Return string = " << tmpvvvstring.size() << endl;
        }
        break;
      }
    }
...
namespace SYM {
  vector<vector<vector<string>>> GetSameSymmetryWyckoffLetters(uint space_group_number, vector<GroupedWyckoffPosition>& grouped_Wyckoff_positions, uint setting) {
    ...
    for (size_t i = 0; i < grouped_Wyckoff_positions.size(); i++) {
      vector<vector<string>> possible_Wyckoff_letters;
      const vector<vector<string>> grouped_possible_Wyckoff_site_symmetries;
      for (size_t j = 0; j < grouped_Wyckoff_positions[i].site_symmetries.size(); j++) {
        vector<string> site_syms;
        vector<string> letters;
        vector<string> positions;
...
// for(size_t j=0;j<per_atom_tokens.size();j++){
...
namespace SYM {
  void convert_wyckoff_pos(vector<vector<vector<string>>> wyckoff_positions) {
    ...
    //  atof(tmpstr.c_str())
    for (size_t k = 0; k < wyckoff_positions.size(); k++) {
      for (size_t j = 0; j < wyckoff_positions[k].size(); j++) {
        for (size_t i = 0; i < wyckoff_positions[k][j].size(); i++) {
          oss << wyckoff_positions[k][j][i];
        }
...
namespace SYM {
  vector<string> splitstring(string str) {
    ...
    for (uint i = 0; i < str.length(); i++) {
      ...
      if (!blank(oss.str())) { // CO20200106 - patching for auto-indenting
        // DX20200924 - START
...
namespace SYM {
  vector<string> splitstring_2(string str) {
    ...
    for (uint i = 0; i < str.length(); i++) {
      oss << str[i];
      // if(str[i+1]=='-' || str[i+1]=='+' || str[i+1]=='\0' || str[i+1]==' ')
      if (str[i + 1] == '\0' || str[i + 1] == ' ') { // CO20200106 - patching for auto-indenting
        temp.push_back(oss.str());
        // cerr << oss.str();// << endl;
        oss.str("");
      }
...
namespace SYM {
  vector<sdouble> simplify(const string& _str) {
    ...
    if (aurostd::substring2bool(str, "--")) {
      aurostd::StringSubstInPlace(str, "--", "+");
    }
    sdouble out;
    vector<sdouble> out_vec;
    double num = 0;
    vector<string> splitstr = splitstring(str);
    for (size_t S = 0; S < splitstr.size(); S++) {
      ...
      if (!containschar(oss.str())) {
        if (havechar(oss.str(), '/')) {
          ...
          vector<string> tokens;
          aurostd::string2tokens(oss.str(), tokens, "*");
          if (tokens.size() == 2) {
            num += atof(tokens[0].c_str()) * atof(tokens[1].c_str());
          } else {
            throw aurostd::xerror(__AFLOW_FILE__, "SYM::simplify", "Expected two fields surrounding '*': " + oss.str(), _RUNTIME_ERROR_);
          }
        } else {
          num += atof(oss.str().c_str());
        }
...
namespace SYM {
  void print_wyckoff_pos(vector<vector<vector<string>>> wyckoff_positions) {
    ...
    for (size_t k = 0; k < wyckoff_positions.size(); k++) {
      for (size_t j = 0; j < wyckoff_positions[k].size(); j++) {
        for (size_t i = 0; i < wyckoff_positions[k][j].size(); i++) {
          for (size_t l = 0; l < simplify(wyckoff_positions[k][j][i]).size(); l++) {
    ...
    for (size_t k = 0; k < wyckoff_positions.size(); k++) {
      ...
      for (size_t j = 0; j < wyckoff_positions[k].size(); j++) {
        ...
        for (size_t i = 0; i < wyckoff_positions[k][j].size(); i++) {
          ...
          const vector<sdouble> tmpsd = simplify(wyckoff_positions[k][j][i]);
          // xb();
          // for(size_t ix=0;ix<tmpsd.size();ix++){
...
namespace SYM {
  vector<xvector<double>> getLatticeVectorsFromOriginalRotationOperations(
      vector<Screw>& old_rotations_twofold, vector<Screw>& old_rotations_rot, vector<Screw>& new_rotations, vector<xvector<double>>& twofold_lattice_vectors, vector<xvector<double>>& rot_lattice_vectors, bool& all_matched) {
    vector<xvector<double>> new_lattice_vectors;
...
namespace SYM {
  bool MapAtomsInNewCell(_atom& a, _atom& b, xmatrix<double>& lattice_new, bool& skew, double& tol) { // DX20190619 - changed c2f_orig and f2c_new to lattice_new
    double min_dist = 1e9;
    if (skew) {
      const xvector<double> min_cdiff = SYM::minimizeDistanceCartesianMethod(a.cpos, b.cpos, lattice_new); // DX20190613
      min_dist = aurostd::modulus(min_cdiff); // DX20190613
    } else {
      const xmatrix<double> f2c_new = trasp(lattice_new); // DX20190613
      const xvector<double> fdiff = a.fpos - b.fpos; // DX20190613
      const xvector<double> min_fdiff = SYM::minimizeDistanceFractionalMethod(fdiff); // DX20190613
...
namespace SYM {
  bool MapAtomsInNewCell(xvector<double>& a, xvector<double>& b, xmatrix<double>& lattice_new, bool& skew, double& tol) { // DX20190619 - changed c2f_orig and f2c_new to lattice_new
    ...
    const xmatrix<double> f2c_new = trasp(lattice_new); // DX20190613
    if (skew) {
      const xvector<double> cpos1 = f2c_new * a; // DX20190613 - coord1 to cpos1
      const xvector<double> cpos2 = f2c_new * b; // DX20190613 - coord2 to cpos2
      const xvector<double> min_cdiff = SYM::minimizeDistanceCartesianMethod(cpos1, cpos2, lattice_new); // DX20190613
      min_dist = aurostd::modulus(min_cdiff);
    } else {
      const xvector<double> fdiff = a - b; // DX20190613
      const xvector<double> min_fdiff = SYM::minimizeDistanceFractionalMethod(fdiff); // DX20190613
      min_dist = aurostd::modulus(f2c_new * min_fdiff); // DX20190613
    }
    return (min_dist < tol); // DX20190613
...
namespace SYM {
  deque<deque<_atom>> groupSymmetryEquivalentAtoms(deque<_atom>& atoms, xmatrix<double>& lattice, vector<xmatrix<double>>& sym_ops, vector<xvector<double>>& translations, double& min_dist, double& tol) { // DX20190215 - added tol
    const bool LDEBUG = (false || XHOST.DEBUG);
    deque<deque<_atom>> equivalent_atoms;
    const xmatrix<double> f2c = trasp(lattice);
    const bool skew = SYM::isLatticeSkewed(lattice, min_dist, tol); // DX20190215
    for (size_t k = 0; k < atoms.size(); k++) {
      ...
      for (size_t i = 0; i < sym_ops.size(); i++) {
        ...
        for (size_t j = 0; j < equivalent_atoms.size(); j++) {
  ...
  deque<deque<_atom>> shiftSymmetryEquivalentAtoms(deque<deque<_atom>>& equivalent_atoms, xmatrix<double>& lattice, xvector<double>& translation, double& min_dist, double& tol) { // DX20190215
    ...
    for (size_t i = 0; i < equivalent_atoms.size(); i++) {
      ...
      for (size_t j = 0; j < equivalent_atoms[i].size(); j++) {
        ...
        tmp.fpos = tmpxvec;
        tmp.cpos = f2c * tmp.fpos;
        if (one_shifted_group.empty()) {
          one_shifted_group.push_back(tmp);
        } else {
          bool duplicate_atom = false;
          for (size_t b = 0; b < one_shifted_group.size(); b++) {
...
namespace SYM {
  bool GCD_conventional_atomic_basis(deque<_atom>& conventional_basis_atoms, deque<deque<_atom>>& prim_split_atom_types, int& prim_GCD) {
    ...
    if (split_atom_types.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, "SYM::GCD_conventional_atomic_basis():", "split_atom_types.size()==0", _INPUT_ERROR_);
    }
    ...
    if (split_atom_types.size() > 1) { // If only one type of atom, set to 1
      ...
      for (size_t p = 0; p < prim_split_atom_types.size(); p++) {
        for (size_t s = 0; s < split_atom_types.size(); s++) {
    ...
    } else if (split_atom_types.empty()) { // Conventional basis atoms == 0
      if (LDEBUG) {
        cerr << "SYM::GCD_conventional_atomic_basis::WARNING: Conventional Basis atoms = 0" << endl;
      }
      consistent_ratio = false;
    }
...
namespace SYM {
  vector<xvector<double>> find_vectors_inplane(const vector<xvector<double>>& big_expanded, const xvector<double>& perp_to_vec, double& tol) { // DX20190215
    ...
    vector<int> min_vecs;
    for (size_t i = 0; i < indicies_vecs_in_plane.size(); i++) {
      min_vecs.push_back(i);
    }
    for (size_t i = 0; i < min_vecs.size(); i++) {
...
namespace SYM {
  vector<int> countWyckoffTypes(const vector<wyckoffsite_ITC>& Wyckoff_positions) {
    ...
    for (size_t i = 0; i < Wyckoff_positions.size(); i++) {
      if (!aurostd::WithinList(types, (int) Wyckoff_positions[i].index, (int) index)) {
...
namespace SYM {
  vector<Glide> mirror_operations(
      vector<xvector<double>> expanded_lattice, vector<xvector<double>> expanded_cell, xmatrix<double> L, xmatrix<double> Linv, vector<xvector<double>>& lattice_vectors, double& radius, bool& skew, double& tol) { // DX20190215
    // GET MIRROR PLANES
    //*********************************************************************
    ...
    const uint tsize = expanded_lattice.size();
    const uint doubles_size = CombinationNr(2, tsize);
    ...
    for (size_t i = 0; i < doubles_size; i++) {
      ...
      if (cont_dbl == true) {
        ...
        for (size_t i = 0; i < mirror_ops_vec.size(); i++) {
...
// **********************************************************************************************************************
namespace SYM {
  vector<Screw> twofold_operations(
      vector<xvector<double>> expanded_lattice, vector<xvector<double>> expanded_cell, xmatrix<double> L, xmatrix<double> Linv, vector<xvector<double>>& lattice_vectors, double& radius, bool& skew, double& tol) { // DX20190215 - added tol
    ...
    const uint tsize = expanded_lattice.size();
    const xvector<double> Nullvec;
    const int doubles_size = CombinationNr(2, tsize);
    vector<int> cmb_dbl = AllCombination41(2, tsize, 1);
    vector<xvector<double>> tmp_dbl;
    for (int i = 0; i < doubles_size; i++) {
      ...
      const xvector<double> point_on_axis = Nullvec;
...
// **********************************************************************************************************************
namespace SYM {
  vector<Screw> triplet_operations(
      vector<xvector<double>> expanded_lattice, vector<xvector<double>> expanded_cell, xmatrix<double> L, xmatrix<double> Linv, vector<xvector<double>>& lattice_vectors, double& radius, bool& skew, double& tol) { // DX20190215
    int q = 3;
    ...
    const int triplets_size = CombinationNr(3, tsize);
    vector<int> cmb = AllCombination41(3, tsize, 1);
    vector<xvector<double>> tmp_trip;
    const vector<vector<xvector<double>>> point_sets;
    char tmpchar;
    int count = 0;
    int rotation_ops = 0;
    for (int i = 0; i < triplets_size; i++) {
      ...
      if (tmpchar != 'z') {
        ...
        for (size_t j = 0; j < big_expanded.size(); j++) {
          for (size_t k = j + 1; k < big_expanded.size(); k++) {
        ...
        if (cont == true) {
          ...
          // Check if operation is included in list
          bool contained_in_list = false;
          for (size_t i = 0; i < rotation_ops_vec.size(); i++) {
...
namespace SYM {
  vector<xvector<double>> ReturnITCGenShift(int spacegroupnum, string axis_cell) {
    ...
    const vector<int> genlocations = generatorindex(spacegroupnum + 1);
    vector<xvector<double>> out;
    const int s = 0;
    vector<int> wint = get_multiplicities(ITC_sym_info.gl_sgs[spacegroupnum]); // DX20190215
    const vector<vector<vector<string>>> tmpvvvstring = get_wyckoff_pos(ITC_sym_info.gl_sgs[spacegroupnum], wint[1], false); // false = do not get centered points
    vector<vector<vector<vector<sdouble>>>> tmpvvvsd = convert_wyckoff_pos_sd(tmpvvvstring);
    ...
    for (size_t k = 0; k < tmpvvvsd[s].size(); k++) {
      if (aurostd::WithinList(genlocations, (int) k + 1)) { // DX20210421
        xvector<double> oneshift;
        for (size_t j = 0; j < tmpvvvsd[s][k].size(); j++) {
          double filler = 0;
          for (size_t i = 1; i < tmpvvvsd[s][k][j].size(); i++) {
            // cerr <<"dbl: " <<  tmpvvvsd[s][k][j][i].dbl << endl;
            // cerr << "chr: " <<  tmpvvvsd[s][k][j][i].chr << endl;
            const char tmpchr = tmpvvvsd[s][k][j][i].chr;
            if (tmpchr == '\0') {
              const double tmpdbl = tmpvvvsd[s][k][j][i].dbl;
...
namespace SYM {
  string ReverseSpaceGroup(vector<string> num) {
    stringstream oss;
    const int spacegroupnum = atoi(num[2].c_str()) - 1;
    const int m = atoi(num[3].c_str()); // multiplicity of wyckoff site (eg, 48 j 1 (TWICE))
    int n = 1;
    int l = 1;
    ...
    const string axis_cell;
    SymmetryInformationITC ITC_sym_info; // DX20190215
    ITC_sym_info.initsgs(axis_cell); // DX20190215
    const xmatrix<double> L = spacegroup_to_lattice(spacegroupnum + 1);
    ...
    for (size_t i = 0; i < wint_all.size(); i++) { // DX20210422
      if (!aurostd::WithinList(wint, wint_all[i])) {
        wint.push_back(wint_all[i]);
      } // DX20210422
    } // DX20210422
    // TITLE
    ...
    oss << "1" << endl;
    oss << L << endl;
    const vector<vector<vector<string>>> tmpvvvstring = get_wyckoff_pos(ITC_sym_info.gl_sgs[spacegroupnum], wint[n]);
    ...
    for (int d = 0; d < m; d++) {
      ...
      stringstream ss;
      ss << atomlabel;
      const string typestring = ss.str();
      ...
      for (size_t k = 0; k < tmpvvvsd[s].size(); k++) {
        ...
        for (size_t j = 0; j < tmpvvvsd[s][k].size(); j++) {
          // xb();
          for (size_t i = 0; i < tmpvvvsd[s][k][j].size(); i++) {
            // cerr <<"dbl: " <<  tmpvvvsd[s][k][j][i].dbl << endl;
            // cerr << "chr: " <<  tmpvvvsd[s][k][j][i].chr << endl;
            const char tmpchr = tmpvvvsd[s][k][j][i].chr;
            const double tmpdbl = tmpvvvsd[s][k][j][i].dbl;
...
namespace SYM {
  bool points_equivalent(xmatrix<double>& c2f, xmatrix<double>& f2c, xvector<double> P1, xvector<double> P2, xvector<double>& lattice_vector, double& radius, bool& skew, double& tol) { // DX20190215
    const xvector<double> P1_fpos = c2f * P1;
    const xvector<double> P2_fpos = c2f * P2;

    xvector<double> fdiff = P1_fpos - P2_fpos;
    const double orig_cdiff = aurostd::modulus(f2c * fdiff);
    xvector<int> ijk;
    double min_dist = 1e9; // DX20190613

    if (skew) {
      const xmatrix<double> lattice = trasp(f2c); // DX20190613
      const xvector<double> min_cdiff = SYM::minimizeDistanceCartesianMethod(P1, P2, lattice, ijk); // DX20190613
...
namespace SYM {
  bool is_lattice_point(xmatrix<double> L, xvector<double> point, xvector<double>& lattice_vector, double& radius, bool& skew, double& tol) { // DX20190215
    ...
    const double orig_cdiff = aurostd::modulus(point);
    xvector<int> ijk;
    double min_dist = 1e9; // DX20190613
    if (skew) {
      const xvector<double> min_cdiff = SYM::minimizeDistanceCartesianMethod(origin, point, L, ijk); // DX20190613
      fdiff = c2f * min_cdiff; // DX20190613
      min_dist = aurostd::modulus(min_cdiff); // DX20190613
    } else {
      fdiff = SYM::minimizeDistanceFractionalMethod(fdiff, ijk); // DX20190613
...
namespace SYM {
  char discern_rot_sym(const xmatrix<double>& m, double& tol) { // m is metric_tensor //DX20190215
    ...
    const double six_fold_60 = (Pi_r) / (6.0);
    char out = 'z';
    // Check 3-fold
