...
namespace SYM {
  void orient(xstructure& xstr, bool update_atom_positions) {
    ...
    const char lattice_label = xstr.lattice_label_ITC;
    ...
    const xmatrix<double> Linv = aurostd::inverse(xvec2xmat(lattice_basis[0], lattice_basis[1], lattice_basis[2]));
    ...
    if (true == true) {
      ...
      if (lattice_label == 'b') {
        ...
        const double angle = aurostd::angle(lattice_basis[1], b);
        ...
        if (det(xvec2xmat(lattice_basis[0], lattice_basis[1], lattice_basis[2])) < -tol) {
          const xvector<double> lattice_tmp = lattice_basis[0];
      ...
      if (lattice_label == 'm') {
        ...
        const double angle = aurostd::angle(lattice_basis[2], c);
        if (aurostd::abs(angle - Pi_r) > tol && aurostd::abs(angle) > tol) {
      ...
      if (lattice_label == 'o') {
        ...
        const double angle = aurostd::angle(lattice_basis[0], a);
        if (aurostd::abs(angle - Pi_r) > tol && aurostd::abs(angle) > tol) {
        ...
        // GET ANGLE BETWEEN lattice_basis[2] AND (0 0 1)
        const double angle1 = aurostd::angle(lattice_basis[2], c);
        ...
        const double angle2 = aurostd::angle(lattice_basis[1], b);
      ...
      if (lattice_label == 'h') {
        ...
        const xvector<double> tmpc = c * aurostd::modulus(lattice_basis[2]);
        ...
        const xvector<double> tmpb = -b * aurostd::modulus(lattice_basis[1]);
      ...
      if (lattice_label == 'X') {  //
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " RHOMBOHEDRAL IN HEXAGONAL SETTING" << endl;
        }
        double angle;
        xvector<double> rotaxis;
        Screw S;
        ...
        // print(lattice_basis);
        lattice_basis_xmat = xvec2xmat(lattice_basis[0], lattice_basis[1], lattice_basis[2]);
        const xmatrix<double> f2c = trasp(lattice_basis_xmat);
        const xmatrix<double> c2f = inverse(trasp(lattice_basis_xmat));
        const bool skew = SYM::isLatticeSkewed(lattice_basis_xmat, xstr.dist_nn_min, sym_tol); // DX20190215 - _SYM_TOL_ to sym_tol
...
namespace SYM {
  bool isObverseSetting(xmatrix<double>& lattice, deque<_atom>& atomic_basis_, double& dist_nn_min, double& tolerance) {
    ...
    const bool skew = SYM::isLatticeSkewed(lattice, dist_nn_min, tolerance);
    ...
    if (!obverse) {
      if (LDEBUG) {
        cerr << "SYM::isObverseSetting: RHL: In reversed setting, rotating to obverse setting" << endl;
      }
      // cerr << 1 << endl;
      ...
      if (LDEBUG) {
        cerr << "SYM::isObverseSetting: TRIGONAL? Yes, but origin choice may prevent from finding orientation" << endl;
      }
      return true;
    }
  }
...
namespace SYM {
  bool transformToObverse(xmatrix<double>& lattice, deque<_atom>& atoms) {
    ...
    R_60(3, 2) = 0.0;
    R_60(3, 3) = 1.0;
    const xvector<double> lb1 = R_60 * lattice_basis[0];
    const xvector<double> lb2 = R_60 * lattice_basis[1];
    const xvector<double> lb3 = R_60 * lattice_basis[2];
    const xmatrix<double> lb = xvec2xmat(lattice_basis[0], lattice_basis[1], lattice_basis[2]);
    const xmatrix<double> lb_r60_inv = aurostd::inverse(xvec2xmat(lb1, lb2, lb3));
...
namespace SYM {
  bool transformToObverse(xmatrix<double>& lattice, deque<deque<_atom>>& equivalent_atoms) {
    ...
    lattice_basis.push_back(lattice(2));
    lattice_basis.push_back(lattice(3));
    const xvector<double> tmpvec = lattice_basis[0];
    lattice_basis[0] = lattice_basis[1];
    lattice_basis[1] = tmpvec;
    for (size_t i = 0; i < equivalent_atoms.size(); i++) {
...
// **************************************************************************************************************************************************
uint xstructure::GetPrimitiveCell() {
  const bool LDEBUG = (false || XHOST.DEBUG);
  if (LDEBUG) {
  ...
  const double prim_det = aurostd::det(prim_lattice);
  ...
  const vector<xvector<double>> diff_vectors;
  ...
  const int atom_count = (*this).atoms.size();
  ...
  if (foundbasis == false) {
    for (size_t i = 0; i < atomic_basis_.size(); i++) {
    ...
    for (size_t i = 0; i < atoms_by_type[index_for_smallest_group].size(); i++) {
    ...
    for (size_t d = 1; d < diff_vectors.size(); d++) { // use d=0 if we do double for loop
    ...
    const uint num_of_candidates = lattice_vector_candidates.size();
    ...
    if (num_of_candidates == 3) {
      ...
      const double tol_vol = (*this).sym_eps * amin * amin * amin; // DX20190215 - _SYM_TOL_ to (*this).sym_eps
      ...
      for (size_t a = 0; a < lattice_vector_candidates.size(); a++) {
        const xvector<double> tmp = lattice_vector_candidates[a] * lattice_basis_xmat;
        candidates_cpos.push_back(tmp);
        candidates_mods.push_back(aurostd::modulus(tmp));
      }
      // DX20180503 - FASTER MIN CART DISTANCE CALCULATOR - END

      for (size_t i = 0; i < candidates_cpos.size(); i++) {
        for (size_t j = i + 1; j < candidates_cpos.size(); j++) {
          for (size_t k = j + 1; k < candidates_cpos.size(); k++) {
            ...
            if (aurostd::abs(det) > tol_vol) { // DX20180613 - can be left handed; Minkowski/Niggli will fix; necessary since we aren't checking permutations of vectors
              ...
              const double atom_number_ratio = (double) smallest_group / aurostd::nint(prim_det / det); // atom ratio before and after reducing //DX20180723
              if (det > tol_vol && det < prim_det && aurostd::isinteger(prim_det / det, 0.05) && aurostd::isinteger(atom_number_ratio, 0.05) &&
                  (atom_number_ratio - 1.0) > -_ZERO_TOL_) // DX20180614 - now it should be right-handed // check atom ratio after reducing //DX20180723
              { // CO20200106 - patching for auto-indenting
                // double sum_candidate_mods = candidates_mods[i] + candidates_mods[j] + candidates_mods[k];
                const double sum_candidate_mods = aurostd::modulus(candidate_lattice(1)) + aurostd::modulus(candidate_lattice(2)) +
                                                  aurostd::modulus(candidate_lattice(3)); // DX20180613 - need to recalculate mods, they may have changed during Minkowski/Niggli
                // candidate_lattice=::MinkowskiBasisReduction(candidate_lattice);      // Minkowski first  //DX20180526
                // candidate_lattice=::NiggliUnitCellForm(candidate_lattice);           // Niggli Second //DX20180526
      ...
      const xmatrix<double> transformation_matrix = GetBasisTransformation((*this).lattice, lattice_basis_xmat);
      ...
      // for(size_t n=0;n<newbasis.size();n++){
  ...
  if (!newbasis.empty()) { // IF A NEW BASIS EXISTS (CELL WAS NOT PRIMITIVE) THEN OVERWRITE a.atoms
...
namespace SYM {
  deque<_atom> add_basis(vector<xvector<double>>& expanded_lattice_points, xmatrix<double>& L, xstructure& xstr) {
    ...
    for (size_t i = 0; i < expanded_lattice_points.size(); i++) {
...
namespace SYM {
  xstructure ConventionalCell(xstructure& xstr,
    ...
    for (size_t i = 0; i < primitive_atomic_basis.size(); i++) {
      for (size_t j = i + 1; j < primitive_atomic_basis.size(); j++) {
        if (SYM::MapAtom(primitive_atomic_basis[i], primitive_atomic_basis[j], false, LPRIM, f2c, skew, sym_tol)) { // DX20190215 - added sym_tol //DX20190619 - LPRIM and f2c as input
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: WARNING: Atoms occupying the same space (given tolerance " << sym_tol << "). Check structure [dir=" << xstr.directory << "]." << endl;
          } // DX20190215 - _SYM_TOL_ to sym_tol
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: atom #" << i << " symbol: " << primitive_atomic_basis[i].name << " coord: " << primitive_atomic_basis[i].fpos << endl;
          }
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: atom #" << j << " symbol: " << primitive_atomic_basis[j].name << " coord: " << primitive_atomic_basis[j].fpos << endl;
          }
    ...
    if (!lattice_vectors_found) {
      ...
      for (size_t m = 0; m < mirror_ops_vec.size(); m++) {
        Screw candidate_rotation;
        const xvector<double> axis_direction = mirror_ops_vec[m].return_direction();
        const xvector<double> point_on_axis; // Null by default
      ...
      if (LDEBUG) {
        ...
        for (size_t m = 0; m < twofold_ops_vec.size(); m++) {
        ...
        for (size_t m = 0; m < rot_ops_vec.size(); m++) {
    ...
    while (reform == true) {
      ...
      const deque<_atom> expanded_basis = add_basis(big_expanded, L, xstr);

      // ===== Conventional cell variables  ===== //
      const vector<xvector<double>> bravais_basis;
      ...
      for (size_t i = 0; i < rot_ops_vec.size(); i++) {
      ...
      const uint totalcount = mcount;
      const int totalnum = CombinationNr(3, totalcount);
      vector<int> combiset;
      combiset = AllCombination41(3, totalcount, 1);
      for (int i = 0; i < totalnum; i++) {
        if (aurostd::abs(orthogonality_defect(xvec2xmat(mirror_ops_vec[combiset[0]].return_direction(), mirror_ops_vec[combiset[1]].return_direction(), mirror_ops_vec[combiset[2]].return_direction())) - 1.0) <
            sym_tol) { // DX20190215 - _SYM_TOL_ to sym_tol
          continue_ortho = true;
          break;
        }
      ...
      if (lattice_vectors_found == false) {
        ...
        if ((mcount == 9 && fourcount == 3) || crystalsystem == "CUBIC") {
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: CUBIC [dir=" << xstr.directory << "]." << endl;
          }
          symmetryfound = true;
          crystalsystem = "CUBIC";
        ...
        // =============== HEXAGONAL =============== //
          ...
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: HEX [dir=" << xstr.directory << "]." << endl;
          }
          ...
          if (cell_choice == SG_SETTING_1 || cell_choice == SG_SETTING_ANRL) { // DX20190131 - add ANRL setting
            ...
            if (bravais_count == 3) { // if three lattice points in the cell, then the system is rhl
              ...
              if (LDEBUG) {
                cerr << "SYM::ConventionalCell: Crystal system is trigonal with three lattice points and setting=1, using rhombohedral setting [dir=" << xstr.directory << "]." << endl;
              }
        ...
        else if (((mcount >= 3 && mcount <= 9) && fourcount != 0) || crystalsystem == "TETRAGONAL") { // CO20200106 - patching for auto-indenting
          symmetryfound = true;
          crystalsystem = "TETRAGONAL";
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: TET [dir=" << xstr.directory << "]." << endl;
          }

          if (!findTetragonalLattice(rot_lattice_vectors, twofold_lattice_vectors, rot_ops_vec, twofold_ops_vec, big_expanded, candidate_lattice_vectors, candidate_lattice_chars, sym_tol)) { // DX20190215 - added sym_tol
            symmetryfound = false;
            return xstr_out;
          }
        }
        ...
        // =============== ORTHORHOMBIC/RHOMBOHEDRAL =============== //
          ...
          symmetryfound = true;
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: ORTHO/RHOMB [dir=" << xstr.directory << "]." << endl;
          }
          ...
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: ORTHO true: [dir=" << xstr.directory << "]." << endl;
          }
        ...
        //  else {  //[CO20200106 - close bracket for indenting]}
          ...
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: RHOMB [dir=" << xstr.directory << "]." << endl;
          }
        ...
        { // CO20200106 - patching for auto-indenting
          ...
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: MONO [dir=" << xstr.directory << "]." << endl;
          }

          if (!findMonoclinicLattice(mirror_lattice_vectors, twofold_lattice_vectors, big_expanded, candidate_lattice_vectors, candidate_lattice_chars, cell_choice, sym_tol)) { // DX20180816 - added cell choice //DX20190215 - added sym_tol
            symmetryfound = false;
            return xstr_out;
          }
        }
        ...
        // =============== TRICLINIC =============== //
          ...
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: TRI [dir=" << xstr.directory << "]." << endl;
          }

          if (!findTriclinicLattice(L, candidate_lattice_vectors, candidate_lattice_chars)) {
            symmetryfound = false;
            return xstr_out;
          }
        } else {
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: Could not identify crystal system, i.e., symmetry operation counts do not match with any of the crystal systems [dir=" << xstr.directory << "]." << endl;
          }
        }
      ...
      for (size_t i = start_index; i < candidate_lattice_vectors.size(); i++) {
        const xmatrix<double> CL = candidate_lattice_vectors[i];
        if (LDEBUG) {
        ...
        // *****************************************************************************************************************************
        const xmatrix<double> transformation_matrix = GetBasisTransformation(prim.lattice, CL);
        const xmatrix<double> rotation_matrix = aurostd::eye<double>(3, 3);
        ...
        const string pearson_symbol = getPearsonSymbol(xstr_out.bravais_label_ITC, lattice_char, xstr_out.atoms);

        if (LDEBUG) {
          cerr << "SYM::ConventionalCell: Pearson: " << pearson_symbol << " [dir=" << xstr.directory << "]." << endl;
        }
        ...
        if (checkops.latticesystem == "REDO") {
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: Symmetry check failed... [dir=" << xstr.directory << "]." << endl;
          }
        }
        ...
        if (i == candidate_lattice_vectors.size() - 1 && (checkops.commensurate == false || checkops.latticesystem == "REDO")) { // added redo as test
          ...
          if (LDEBUG) {
            cerr << "SYM::ConventionalCell: All permutations of cell checked. Need to reform the lattice based on the crystal symmetry [dir=" << xstr.directory << "]." << endl;
          }
          // cerr << "i: " << i << endl;
          ...
          for (size_t m = 0; m < mirror_ops_vec_new.size(); m++) {
            Screw candidate_rotation;
            const xvector<double> axis_direction = mirror_ops_vec_new[m].return_direction();
            const xvector<double> point_on_axis; // Null by default
            candidate_rotation.get_screw_direct(axis_direction, point_on_axis, 2);
          ...
          mirror_lattice_vectors = getLatticeVectorsFromOriginalMirrorOperations(mirror_ops_vec_new, checkops.mirror_ops, tmp_mirror_lattice_vectors, all_matched_mirror);
          twofold_lattice_vectors = getLatticeVectorsFromOriginalRotationOperations(twofold_ops_vec_new, rot_ops_vec_new, checkops.twofold_ops, tmp_twofold_lattice_vectors, tmp_rot_lattice_vectors, all_matched_twofold);
          if (mirror_ops_vec_new.size() > 2) {
            rot_lattice_vectors = getLatticeVectorsFromOriginalRotationOperations(twofold_ops_vec_new, rot_ops_vec_new, checkops.rot_ops, tmp_twofold_lattice_vectors, tmp_rot_lattice_vectors, all_matched_rot);
          }
      ...
      if (latticesystem == "REDO") {
        ...
        if (LDEBUG) {
          cerr << "SYM::ConventionalCell: Could not find lattice consistent with crystal symmetry [dir=" << xstr.directory << "]." << endl;
        }
        symmetryfound = false;
        return xstr_out;
      }
      ...
      // **********************************************************************************************************************
        ...
        if (LDEBUG) {
          cerr << "SYM::ConventionalCell: Conventional cell consistent with crystal symmetry is found [dir=" << xstr.directory << "]." << endl;
        }
        return xstr_out;
      }
    ...
    // for(size_t l=0;l<lattice_vector_choices.size(); l++){
...
// ***********************************************************************************************************************
namespace SYM {
  bool findCubicLattice(vector<xvector<double>>& rot_lattice_vectors, vector<Screw>& rot_ops_vec, vector<xmatrix<double>>& candidate_lattice_vectors, vector<char>& candidate_lattice_chars, double& tol) { // DX20190215 - added tol
    // CUBIC lattice vector criteria: Parallel to 3 equivalent fourfold axes.
    const bool LDEBUG = (false || XHOST.DEBUG);

    xmatrix<double> CL;
    const xvector<double> conv_lattice_vec_a;
    const xvector<double> conv_lattice_vec_b;
    const xvector<double> conv_lattice_vec_c;
    ...
    if (latticeVectorsSame(conven[0], conven[1], conven[2], tol)) { // DX20190215 - _SYM_TOL_ to tol
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Two or more of conventional basis vectors are the same..." << endl;
      }
      return false;
    }
    ...
    if (aurostd::abs(aurostd::modulus(conven[0]) - aurostd::modulus(conven[1])) > tol && // DX20190215 - _SYM_TOL_ to tol
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Lattice vectors are not the same length..." << endl;
      }
      return false;
    }
...
// ***********************************************************************************************************************
namespace SYM {
  bool findTrigonalLattice(vector<xvector<double>>& rot_lattice_vectors,
                           vector<Screw>& rot_ops_vec,
                           vector<xvector<double>>& big_expanded,
                           vector<xmatrix<double>>& candidate_lattice_vectors,
                           vector<char>& candidate_lattice_chars,
    ...
    if (six_or_threefold_lattice_vectors.empty()) {
    ...
    if (six_or_threefold_lattice_vectors.empty()) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " c-axis not found. Tolerances may be too tight." << endl;
      }
      return false;
    }
    ...
    if (conv_lattice_vec_c == null) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " c-axis is null." << endl;
      }
      return false;
    }
    ...
    for (size_t i = 0; i < possible_latt_a_b.size(); i++) {
      for (size_t j = 0; j < possible_latt_a_b.size(); j++) {
    ...
    if (anyNullVectors(abc_vecs, tol)) { // DX20190215 - _SYM_TOL_ to tol
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " One or more of the lattice vectors is null" << endl;
      }
      return false;
    }
...
// ***********************************************************************************************************************
namespace SYM {
  bool findTetragonalLattice(vector<xvector<double>>& rot_lattice_vectors,
                             vector<xvector<double>>& twofold_lattice_vectors,
                             vector<Screw>& rot_ops_vec,
                             vector<Screw>& twofold_ops_vec,
                             vector<xvector<double>>& big_expanded,
    ...
    for (size_t f = 0; f < fourfold_lattice_vectors.size(); f++) {
      ...
      // to the c axis
      for (size_t i = 0; i < twofold_ops_vec.size(); i++) {
      ...
      if (anyNullVectors(ab_vecs, tol)) { // DX20190215 - _SYM_TOL_ to tol
        ...
        if (conv_lattice_vec_b(1) > tol && conv_lattice_vec_b(2) < tol && conv_lattice_vec_b(3) < tol) { // DX20190215 - _SYM_TOL_ to tol
          const xvector<double> tmp = conv_lattice_vec_a;
...
// ***********************************************************************************************************************
namespace SYM {
  bool findMonoclinicLattice(vector<xvector<double>>& mirror_lattice_vectors,
                             vector<xvector<double>>& twofold_lattice_vectors,
                             vector<xvector<double>>& big_expanded,
                             vector<xmatrix<double>>& candidate_lattice_vectors,
                             vector<char>& candidate_lattice_chars,
                             int& cell_choice,
                             double& tol) { // DX20180816 - added setting_choice //DX20190215 - added tol
    ...
    if (!mirror_lattice_vectors.empty()) {
    ...
    } else if (!twofold_lattice_vectors.empty()) {
    ...
    if (possible_h_lats.empty()) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " No unique axis found" << endl;
      }
      return false;
    }
    if (possible_h_lats[0] == null) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " Unique axis is null" << endl;
      }
      return false;
    }
    ...
    for (size_t h = 0; h < possible_h_lats.size(); h++) {
      ...
      for (size_t i = 0; i < possible_efg_vectors.size(); i++) {
        for (size_t j = i + 1; j < possible_efg_vectors.size(); j++) {
          for (size_t k = j + 1; k < possible_efg_vectors.size(); k++) {
            int e = -1;
            int f = -1;
            int g = -1;
            ...
            if ((aurostd::abs(tranvec_i[1]) >= _ZERO_TOL_ || aurostd::abs(tranvec_i[2]) >= _ZERO_TOL_ || aurostd::abs(tranvec_i[3]) >= _ZERO_TOL_) &&
              ...
              if (nullVector(closed_triangle, tol)) { // DX20190215 - _SYM_TOL_ to tol
                ...
                if (!SYM::checkAngle(tranvec_e, tranvec_f, Pi_r, tol) && // DX20190215 - _SYM_TOL_ to tol
                  ...
                  int consistent = 0;
                  if ((cell_choice == 0 || cell_choice == SG_SETTING_1 || cell_choice == SG_SETTING_ANRL) && aurostd::det(xvec2xmat(tranvec_f, h_lat, tranvec_e)) > 0.0 &&
                      aurostd::det(xvec2xmat(tranvec_g, h_lat, tranvec_f)) > 0.0 && // DX20190131 - add ANRL setting choice
      ...
      if (index.size() <= 1 && h == possible_h_lats.size() - 1) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " No lattice vectors found in the monoclinic plane" << endl;
        }
        return false;
      } else if (index.size() <= 1 && h < possible_h_lats.size() - 1) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " Not enought lattice vectors found in monoclinic plane. Try the other unique axis." << endl;
        }
      ...
      const int e = index[smallest_gt_min_index(0, -1, -1, total_mod)][0];
      tranvec_e = possible_efg_vectors[e];
      const int f = index[smallest_gt_min_index(0, -1, -1, total_mod)][1];
      tranvec_f = possible_efg_vectors[f];
      const int g = index[smallest_gt_min_index(0, -1, -1, total_mod)][2];
      tranvec_g = possible_efg_vectors[g];
      const double mod_e = aurostd::modulus(tranvec_e);
      const double mod_f = aurostd::modulus(tranvec_f);
      const double mod_g = aurostd::modulus(tranvec_g);
      ...
      if (mod_e <= _ZERO_TOL_ || mod_f <= _ZERO_TOL_ || mod_g <= _ZERO_TOL_) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " One or more lattice vectors in the monoclinic plane is null" << endl;
        }
        return false;
      }
...
// ***********************************************************************************************************************
namespace SYM {
  bool findTriclinicLattice(xmatrix<double>& lattice, vector<xmatrix<double>>& candidate_lattice_vectors, vector<char>& candidate_lattice_chars) {
...
// ***********************************************************************************************************************
namespace SYM {
  bool findOrthorhombicLattice(vector<xvector<double>>& twofold_lattice_vectors, vector<xvector<double>>& mirror_lattice_vectors, vector<xmatrix<double>>& candidate_lattice_vectors, vector<char>& candidate_lattice_chars, double& tol) { // DX20190215 - added tol
    ...
    const uint totalcount = twofold_lattice_vectors.size() + mirror_lattice_vectors.size();
    const int totalnum = CombinationNr(3, totalcount);
    vector<int> combiset;
    combiset = AllCombination41(3, totalcount, 1);
    for (int i = 0; i < totalnum; i++) {
    ...
    if (conven.empty()) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " No conventional basis vectors found...." << endl;
      }
      return false;
    }
...
// ***********************************************************************************************************************
namespace SYM {
  bool findRhombohedralLattice(vector<xvector<double>>& rot_lattice_vectors,
                               vector<Screw>& rot_ops_vec,
                               vector<xvector<double>>& big_expanded,
                               vector<xmatrix<double>>& candidate_lattice_vectors,
                               vector<char>& candidate_lattice_chars,
    ...
    if (six_or_threefold_lattice_vectors.empty()) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " No 3- or 6-fold axis found." << endl;
      }
      return false;
    }
    ...
    int count = 0;
    for (size_t i = 0; i < possible_latt_a_b.size(); i++) {
      for (size_t j = i; j < possible_latt_a_b.size(); j++) {
        tmpa = possible_latt_a_b[i];
        tmpb = possible_latt_a_b[j];
        if (aurostd::abs(aurostd::modulus(tmpa) - aurostd::modulus(tmpb)) < tol && // DX20190215 - _SYM_TOL_ to tol
            SYM::checkAngle(tmpa, tmpb, ((2.0 * Pi_r) / 3.0), tol)) { // DX20190215 - _SYM_TOL_ to tol
...
// ***********************************************************************************************************************
namespace SYM {
  bool findRhombohedralSetting(vector<xvector<double>>& big_expanded, vector<xmatrix<double>>& candidate_lattice_vectors, vector<char>& candidate_lattice_chars, double& tol) { // DX20190215 - added tol
    ...
    const bool LDEBUG = (false || XHOST.DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Finding rhombohedral setting of structure ..." << endl;
    }
    ...
    const xvector<double> hex_c = candidate_lattice_vectors[0](3);

    // use moduli of hex a and c to determine rhl a and alpha
    double mod_hex_a = 0.0;
    double mod_hex_c = 0.0;
    double mod_aprime = 0.0;
    ...
    const double alpha_prime = std::acos(((2.0 * mod_hex_c * mod_hex_c) - (3.0 * mod_hex_a * mod_hex_a)) / (2.0 * (mod_hex_c * mod_hex_c + 3.0 * mod_hex_a * mod_hex_a)));

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Modulus of rhombohedral lattice vectors (a prime) is " << mod_aprime << "." << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " Angle between all rhombohedral lattice vectors (alpha prime) is " << alpha_prime << "." << endl;
    }

    // find lattice vectors that are of size mod_aprime
    vector<xvector<double>> candidate_rhl_setting_vectors;
    for (size_t i = 0; i < big_expanded.size(); i++) {
      if (aurostd::abs(aurostd::modulus(big_expanded[i]) - mod_aprime) < tol) { // DX20190215 - _SYM_TOL_ to tol
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " Found candidate vector: " << big_expanded[i] << "." << endl;
        }
    ...
    double min_norm = 1e9; // DX20180514 - added initialization
    int count = 0;
    for (size_t i = 0; i < candidate_rhl_setting_vectors.size(); i++) {
      tmpa = candidate_rhl_setting_vectors[i];
      moda = aurostd::modulus(tmpa);
      for (size_t j = 0; j < candidate_rhl_setting_vectors.size(); j++) {
        if (j != i) {
          tmpb = candidate_rhl_setting_vectors[j];
          modb = aurostd::modulus(tmpb);
          for (size_t k = 0; k < candidate_rhl_setting_vectors.size(); k++) {
            if (k != j && k != i) {
              tmpc = candidate_rhl_setting_vectors[k];
              modc = aurostd::modulus(tmpc);
...
// ***********************************************************************************************************************
namespace SYM {
  bool determineLatticeCentering(vector<xvector<double>>& bravais_basis,
                                 int& bravais_count,
                                 xmatrix<double>& c2f,
                                 xmatrix<double>& f2c,
                                 bool& skew,
                                 ...
                                 string& crystalsystem,
                                 vector<char>& candidate_lattice_chars,
                                 double& tol) { // DX20190215 - _SYM_TOL_ to tol
    // Find number of lattice points in cell (lattice centering)

    const xmatrix<double> lattice = trasp(f2c); // DX20190619
    for (size_t j = 0; j < big_expanded.size(); j++) {
      if (bravais_basis.empty()) {
        bravais_basis.push_back(big_expanded[j]);
      } else {
        bool duplicate_lattice_point = false;
        for (size_t a = 0; a < bravais_basis.size(); a++) {
          const xvector<double> bravais_fpos = BringInCell(c2f * bravais_basis[a]);
          const xvector<double> tmp = BringInCell(c2f * big_expanded[j]);
    ...
    // ===== If ORTHORHOMBIC, could be P-,F-,C-,or I-centered ===== //
      ...
      if (bravais_count == 2 && (aurostd::abs(zs[0] - zs[1]) < tol || // DX20190215 - _SYM_TOL_ to tol
    ...
    // ===== If RHOMBOHEDRAL and the bravais_count is only 1, then it is actually HEXAGONAL ===== //
      ...
      crystalsystem = "HEXAGONAL";
      for (size_t i = 0; i < candidate_lattice_chars.size(); i++) {
...
namespace SYM {
  symfolder check_ccell(xstructure& xstr, SymmetryInformationITC& ITC_sym_info) {
    ...
    const double tol = xstr.sym_eps; // DX20190215 - added tol
    ...
    const xmatrix<double> L = xstr_CCell.lattice;
    const xmatrix<double> f2c = trasp(L);
    const xmatrix<double> c2f = inverse(trasp(L));
    const bool skew = SYM::isLatticeSkewed(L, xstr_CCell.dist_nn_min, tol); // DX20190215 - _SYM_TOL_ to tol
    const xmatrix<double> Linv = aurostd::inverse(L);
    const char latticetypechar = xstr_CCell.lattice_label_ITC;
    vector<int> SYMINDEX;
    const string latticesystem;
    ...
    for (size_t i = 0; i < atomicbasis.size(); i++) {
    ...
    while (pointgroupmap_success == false) {
      ...
      while (ident_trans == 0) {
        ...
        for (size_t i = 0; i < SYMINDEX.size(); i++) {
          symcount++;
          const xmatrix<double> Rf = ITC_sym_info.sym_mats[SYMINDEX[i]]; // Uf  //CO
          const string sym_symbol = ITC_sym_info.symbol[SYMINDEX[i]];
        ...
        if (ident_trans == 0) {
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " ERROR: Could not find identity." << endl;
          }
      ...
      if (pointgroup_crystalsystem[0] == "REDO") {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " WARNING: Point group mapping failed (number of symmetry elements does not match with a given point group) [dir=" << xstr.directory << "]." << endl;
        }
        SOps.latticesystem = "REDO";
        SOps.commensurate = false;
        return SOps;
...
namespace SYM {
  void calculateSpaceGroups(vector<xstructure>& vxstrs, uint start_index, uint end_index, uint setting) { // DX20191230 add setting option
    ...
    if (end_index == AUROSTD_MAX_UINT) {
      end_index = vxstrs.size();
    }
...
// ********************************************************************************************************************************
string xstructure::aflow2sg() {
  double use_tol = SYM::defaultTolerance((*this));
  bool no_scan = false;
  const uint sgroup = (*this).SpaceGroup_ITC(use_tol, -1, 0, no_scan); // DX20180806 - added setting; 0 means default
...
string xstructure::aflow2sg(double& use_tol, const int& manual_it) {
  bool no_scan = false;
  const uint sgroup = (*this).SpaceGroup_ITC(use_tol, manual_it, 0, no_scan); // DX20180806 - added setting; 0 means default
  return GetSpaceGroupName(sgroup, (*this).directory) + " #" + aurostd::utype2string(sgroup); // DX20180526 - add directory
}
...
// ********************************************************************************************************************************
uint xstructure::SpaceGroup_ITC() {
  double use_tol = SYM::defaultTolerance((*this));
  bool no_scan = false;
...
{ // CO20200106 - patching for auto-indenting
  ...
  if ((*this).iomode == IOVASP_WYCKCAR) {
    iomode_tmp = (*this).iomode;
  ...
  if (generatefakenames == true) {
    if (LDEBUG) {
      cerr << "SYM::SpaceGroup_ITC: Atoms in POSCAR not labeled ... assigning fake names [dir=" << (*this).directory << "]." << endl;
    }
  ...
  // ===== Declare variables ===== //
  ostringstream woss;
  ostringstream woss_min; // stringstream for wyccar //DX+KC20221115 - need to store minimum wyccar stringstream for aflow --wyckoff --print=json
  int spacegroup = 0;
  int spacegroup_global = 0; // DX20220920
  bool foundspacegroup = false;
  bool foundspacegroup_global = false; // DX20220827 - added foundspacegroup_global
  ...
  xvector<double> OriginShift_Wyckoff_min;
  xstructure CCell;
  xstructure CCell_Wyckoff_min; // DX20220827 - added Wyckoff_min
  ...
  int min_wyckoff_sum = 1e9;
  int minimum_scheme_sum = 1e9;
  int global_min_wyckoff_sum = 1e9; // DX20220827 - track minimum Wyckoff sum
  vector<int> min_Wyckoff_numbers;
  vector<int> global_min_Wyckoff_numbers; // DX20221103
  vector<int> wyckoffmult;
  vector<int> wyckoffmult_min; // Wyckoff multiplicity //DX20220827 - added min variant
  vector<string> general_wyckoff_position;
  vector<string> wyckoffsymbols;
  vector<string> wyckoffsymbols_min; // Wyckoff symbol //DX20220827 - added min variant
  deque<_atom> wyckoffPositionsVector;
  deque<_atom> wyckoffPositionsVector_min; // Stored Wyckoff coordinate //DX20220827 - added min variant
  vector<string> wyckoffSymbols;
  vector<string> wyckoffSymbols_min; // Stored Wyckoff symbol //DX20220827 - added min variant
  vector<wyckoffsite_ITC> wyckoffVariables;
  vector<wyckoffsite_ITC> wyckoffVariables_min; // variable values + atom chemical label + wyckoff symbol //DX20220827 - added min variant
  vector<vector<vector<string>>> tmpvvvstring;
  vector<vector<vector<string>>> tmpvvvstring_min; // DX20220827 - added min variant
  ...
  while (foundspacegroup == false) {
    ...
    while (!tested_all_lattices) {
      if (LDEBUG) {
        cerr << "SYM::SpaceGroup_ITC: [BEGINNING OF LOOP] iterate=" << iterate << " : min_wyckoff_sum=" << min_wyckoff_sum << " vs global=" << global_min_wyckoff_sum << endl;
      }
      ...
      if (last_orientation == true) {
        ...
        if (foundspacegroup_global) {
          if (LDEBUG) {
            cerr << "SYM::SpaceGroup_ITC: Explored all iterations, use the lattice and origin choice that found a valid space group." << endl;
          }
          ...
          woss.str("");
          woss << woss_min.str(); // DX+KC20221103 - added woss_min for aflow --wyckoff --print=json
          // update shifts
      ...
      if (MAN_IT != -1) {
        ...
        CCell = SYM::ConventionalCell(xstr, IT, cell_choice, last_orientation, crystalsystem_prev, CrystOut_prev, candidate_lattice_vectors_prev, candidate_lattice_chars_prev, checkops, ITC_sym_info,
                                      lattice_reformed, lattice_pgroups, lattice_sym_mats, crystal_sym_mats, symmetryfound, foundspacegroup_global); // DX20220927 - added foundspacegroup_global
        ...
        if (symmetryfound == false) {
          if (foundspacegroup_global && last_orientation) {
            if (LDEBUG) {
              cerr << "SYM::SpaceGroup_ITC: Last lattice choice/orientation was explored. Consistent symmetry was found with a different choice/orientation, so we will use that conventional cell." << endl;
            }
            continue;
          }
          lattice_reformed = false;
        ...
        iterate = MAN_IT;
        // ===== Conventional Cell Routine ===== //
        CCell = SYM::ConventionalCell(xstr, iterate, cell_choice, last_orientation, crystalsystem_prev, CrystOut_prev, candidate_lattice_vectors_prev, candidate_lattice_chars_prev, checkops, ITC_sym_info,
                                      lattice_reformed, lattice_pgroups, lattice_sym_mats, crystal_sym_mats, symmetryfound, foundspacegroup_global); // DX20220927 - added foundspacegroup_global
        // ===== If the conventional cell was not found, change the tolerance ===== //
        if (symmetryfound == false) {
          lattice_reformed = false;
      ...
      const xmatrix<double> f2c = trasp(CCell.lattice);
      const xmatrix<double> c2f = inverse(trasp(CCell.lattice));
      ...
      const vector<xvector<double>> centeringops = checkops.centeringops;
      ...
      if (LDEBUG) {
        ...
        for (size_t s = 0; s < sg_search.size(); s++) {
          cerr << sg_search[s] << " ";
        }
      ...
      // DEBUG
      // for(size_t i=0;i<pointgroupops.size();i++){
      ...
      for (size_t j = 0; j < ITC_sym_info.sgindex.size(); j++) {
        ...
        if (aurostd::WithinList(sg_search, ITC_sym_info.sgindex[j])) { // DX20210422 - SYM::invec() -> aurostd::WithinList()
          ...
          for (size_t i = 0; i < ITC_sym_info.generators[j].size(); i++) {
            // cerr << ITC_sym_info.generators[j][i].direction << endl;
            vector<int> sub;
            for (size_t k = 0; k < FCG.size(); k++) {
              string symboltmp;
              ...
              if (ITC_sym_info.generators[j][i].symbol == "c" || ITC_sym_info.generators[j][i].symbol == "b" || ITC_sym_info.generators[j][i].symbol == "a" || ITC_sym_info.generators[j][i].symbol == "n" ||
          ...
          const int numofgens = ITC_sym_info.generators[j].size();
          if (numofgens == 5) {
            for (size_t ix = 0; ix < op_ind[0].size(); ix++) {
              for (size_t jx = 0; jx < op_ind[1].size(); jx++) {
                for (size_t kx = 0; kx < op_ind[2].size(); kx++) {
                  for (size_t lx = 0; lx < op_ind[3].size(); lx++) {
                    for (size_t mx = 0; mx < op_ind[4].size(); mx++) {
          ...
          if (match_count == ITC_sym_info.generators[j].size() * (centeringgroups)) {
            ...
            for (size_t ix = 0; ix < CG.size(); ix++) {
              ...
              const xvector<double> SOL;
              for (size_t l = 0; l < oneshiftset.size(); l++) {
                for (int k = 1; k < 4; k++) {
              ...
              if (foundspacegroup == true) {
                ...
                if (CCell.lattice_label_ITC == 'R' && !SYM::isObverseSetting(CCell.lattice, equivalent_atoms_shifted, CCell.dist_nn_min, CCell.sym_eps)) { // DX20190215 - added sym_eps
                  ...
                  for (size_t i = 0; i < equivalent_atoms.size(); i++) {
                    for (size_t j = 0; j < equivalent_atoms[i].size(); j++) {
                ...
                axis_cell << cell_choice; // DX20180806 - use setting
                if (LDEBUG) {
                  cerr << __AFLOW_FUNC__ << " Lattice type (unique axis if specified) and cell choice  : " << axis_cell.str() << endl;
                }
                ...
                while (orig_origin_shift == true || final_shift == false) {
                  ...
                  if (other_shifts_explored == false) {
                    ...
                    if (!possible_shifts.empty()) {
                      if (LDEBUG) {
                        cerr << __AFLOW_FUNC__ << " Exploring possible shifts to find minimum enumerated Wyckoff set. Testing shift " << origin_shift_index << " (" << origin_shift_index + 1 << " of "
                             << possible_shifts.size() << "): " << possible_shifts[origin_shift_index] << "." << endl;
                      }
                    ...
                    for (size_t s = 0; s < sum_wyckoff_letters.size(); s++) {
                      if (LDEBUG) {
                        cerr << __AFLOW_FUNC__ << "shift_valid[s]=" << shift_valid[s] << " sum_wyckoff_letters[s]=" << aurostd::joinWDelimiter(all_enumerated_wyckoff_letters[s], ",") << endl;
                      }
                      if ((min_wyckoff_sum > sum_wyckoff_letters[s] && shift_valid[s]) || (min_wyckoff_sum == sum_wyckoff_letters[s] && all_enumerated_wyckoff_letters[s] < min_Wyckoff_numbers)) // DX20221110 - added tie-breaker
                      {
                  ...
                  found_all_wyckoff = SYM::findWyckoffPositions(CCell, atomicbasis, tmpvvvstring, equivalent_atoms, equivalent_atoms_shifted, foundspacegroup, spacegroupstring, orig_origin_shift, OriginShift,
                                                                wyckoffmult, wyckoffsymbols, wyckoffVariables, wyckoffPositionsVector, wyckoffSymbols, woss, obverse_force_transformed);
                  shift_valid.push_back(found_all_wyckoff);
                  if (LDEBUG) {
                    cerr << __AFLOW_FUNC__ << " Shift is valid ? : " << found_all_wyckoff << endl;
                  }
                  ...
                  for (size_t w = 0; w < grouped_Wyckoff_positions_structure.size(); w++) {
                    for (size_t x = 0; x < grouped_Wyckoff_positions_structure[w].multiplicities.size(); x++) {
                  ...
                  const vector<int> enumerated_wyckoff_letters = SYM::enumerate_wyckoff_letters(wyckoff_letter); // DX20221103
                  ...
                  if (orig_origin_shift == true) {
                    ...
                    string min_letter;
                    string min_site_sym;
                    for (size_t w = 0; w < wyckoff_mult.size(); w++) {
                      const int enumerated_letter = SYM::enumerate_wyckoff_letter(wyckoff_letter[w]);
                    ...
                    const vector<int> found_enumerated_Wyckoff_numbers = SYM::enumerate_wyckoff_letters(wyckoff_letter);
                    const int sum_enumerated_found_letters = aurostd::sum(found_enumerated_Wyckoff_numbers);
                    ...
                    if (minimum_scheme_sum == 1e9) {
                      ...
                      const vector<int> minimum_enumerated_Wyckoff_numbers = SYM::enumerate_wyckoff_letters(minimum_enumerated_Wyckoff_letters);
                    ...
                    if (LDEBUG) {
                      cerr << __AFLOW_FUNC__ << "SYM::SpaceGroup_ITC: minimum_scheme_sum=" << minimum_scheme_sum << " vs current=" << sum_enumerated_found_letters << endl;
                    }
      ...
      if (foundspacegroup == true) {
        if (LDEBUG) {
          cerr << "SYM::SpaceGroup_ITC: spacegroup=" << spacegroup << " iterate=" << iterate << " : min_wyckoff_sum=" << min_wyckoff_sum << " vs global=" << global_min_wyckoff_sum
               << " (spacegroup_global=" << spacegroup_global << ")" << endl;
        ...
        if (spacegroup_global == 0) {
          spacegroup_global = spacegroup;
        } // DX20221229
        foundspacegroup = false;
        // if(min_wyckoff_sum<global_min_wyckoff_sum ||
        if (spacegroup >= spacegroup_global &&
            (min_wyckoff_sum < global_min_wyckoff_sum || (min_wyckoff_sum == global_min_wyckoff_sum && !global_min_Wyckoff_numbers.empty() && min_Wyckoff_numbers < global_min_Wyckoff_numbers))) { // DX20221103 - added tie-breaker
          ...
          woss_min.str("");
          woss_min << woss.str(); // DX+KC20221103 - added woss_min for aflow --wyckoff --print=json
          // update shifts
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " Tolerance used to obtain this space group: " << CCell.sym_eps << " [dir=" << xstr.directory << "]." << endl;
  } // DX20190215 - _SYM_TOL_ to CCell.sym_eps
  ...
  const xmatrix<double> latmat = CCell.lattice;
  const xvector<double> latmat1 = SYM::extract_row(latmat, 1);
  const xvector<double> latmat2 = SYM::extract_row(latmat, 2);
  const xvector<double> latmat3 = SYM::extract_row(latmat, 3);

  const double alpha = SYM::get_angle(latmat2, latmat3, "D");
  const double beta = SYM::get_angle(latmat3, latmat1, "D");
  const double gamma = SYM::get_angle(latmat1, latmat2, "D");

  const double amag = aurostd::modulus(latmat1);
  const double bmag = aurostd::modulus(latmat2);
  const double cmag = aurostd::modulus(latmat3);
  ...
  if (axis_cell.str().find("b") != std::string::npos) {
    ...
    wss << setprecision(4) << fixed << amag << " " << bmag << " " << cmag << " " << alpha << " " << beta << " " << gamma << " " << spacegroup << " " << setting_choice
        << endl; // CODE PREFERS OPTION 1 (ie, first ITC origin choice)
  }
  // === Add number of types of Wyckoff positions === ..
  for (size_t i = 0; i < wyckofftypescount.size(); i++) {
    wss << wyckofftypescount[i] << " ";
  }
  // === Add Wyckoff positions === //
  wss << endl << "Direct(WYCCAR)" << endl;
  wss << woss.str(); //<< endl;
  ...
  //  rhombohedral fixes
  if (latticetypechar == 'r' && CCell.bravais_label_ITC == 'P') {
    CCell.bravais_label_ITC = 'R';
  }
  if (latticetypechar == 'R' || latticetypechar == 'r') {
    latticetypechar = 'h';
  }
  // monoclinic fixes
  if (latticetypechar == 'b') {
    latticetypechar = 'm';
  } // DX20190311 - turns b to m for standard monoclinic designation
  // DX20190212 END
...
namespace SYM {
  string OrthoDefect(istream& cin) {
    ...
    const xmatrix<double> latticemat = xvec2xmat(a.lattice(1), a.lattice(2), a.lattice(3));
...
namespace SYM {
  stringstream* latex_plot_lattice(xmatrix<double> L, string color, int a, int b, int c) {
    ...
    *ss << "\\coordinate (O) at (" << L(1, 1) << "*\\x+" << L(2, 1) << "*\\y+" << L(3, 1) << "*\\z," << L(1, 2) << "*\\x+" << L(2, 2) << "*\\y+" << L(3, 2) << "*\\z," << L(1, 3) << "*\\x+" << L(2, 3) << "*\\y+"
        << L(3, 3) << "*\\z);" << endl;
    *ss << "\\coordinate (P1) at (" << L(1, 1) << "+" << L(1, 1) << "*\\x+" << L(2, 1) << "*\\y+" << L(3, 1) << "*\\z," << L(1, 2) << "+" << L(1, 2) << "*\\x+" << L(2, 2) << "*\\y+" << L(3, 2) << "*\\z,"
        << L(1, 3) << "+" << L(1, 3) << "*\\x+" << L(2, 3) << "*\\y+" << L(3, 3) << "*\\z);" << endl;
    *ss << "\\coordinate (P2) at (" << L(2, 1) << "+" << L(1, 1) << "*\\x+" << L(2, 1) << "*\\y+" << L(3, 1) << "*\\z," << L(2, 2) << "+" << L(1, 2) << "*\\x+" << L(2, 2) << "*\\y+" << L(3, 2) << "*\\z,"
        << L(2, 3) << "+" << L(1, 3) << "*\\x+" << L(2, 3) << "*\\y+" << L(3, 3) << "*\\z);" << endl;
    *ss << "\\coordinate (P3) at (" << L(3, 1) << "+" << L(1, 1) << "*\\x+" << L(2, 1) << "*\\y+" << L(3, 1) << "*\\z," << L(3, 2) << "+" << L(1, 2) << "*\\x+" << L(2, 2) << "*\\y+" << L(3, 2) << "*\\z,"
        << L(3, 3) << "+" << L(1, 3) << "*\\x+" << L(2, 3) << "*\\y+" << L(3, 3) << "*\\z);" << endl;
    *ss << "\\coordinate (P12) at (" << L(1, 1) + L(2, 1) << "+" << L(1, 1) << "*\\x+" << L(2, 1) << "*\\y+" << L(3, 1) << "*\\z," << L(1, 2) + L(2, 2) << "+" << L(1, 2) << "*\\x+" << L(2, 2) << "*\\y+"
        << L(3, 2) << "*\\z," << L(1, 3) + L(2, 3) << "+" << L(1, 3) << "*\\x+" << L(2, 3) << "*\\y+" << L(3, 3) << "*\\z);" << endl;
    *ss << "\\coordinate (P13) at (" << L(1, 1) + L(3, 1) << "+" << L(1, 1) << "*\\x+" << L(2, 1) << "*\\y+" << L(3, 1) << "*\\z," << L(1, 2) + L(3, 2) << "+" << L(1, 2) << "*\\x+" << L(2, 2) << "*\\y+"
        << L(3, 2) << "*\\z," << L(1, 3) + L(3, 3) << "+" << L(1, 3) << "*\\x+" << L(2, 3) << "*\\y+" << L(3, 3) << "*\\z);" << endl;
    *ss << "\\coordinate (P23) at (" << L(2, 1) + L(3, 1) << "+" << L(1, 1) << "*\\x+" << L(2, 1) << "*\\y+" << L(3, 1) << "*\\z," << L(2, 2) + L(3, 2) << "+" << L(1, 2) << "*\\x+" << L(2, 2) << "*\\y+"
        << L(3, 2) << "*\\z," << L(2, 3) + L(3, 3) << "+" << L(1, 3) << "*\\x+" << L(2, 3) << "*\\y+" << L(3, 3) << "*\\z);" << endl;
    *ss << "\\coordinate (P123) at (" << L(1, 1) + L(2, 1) + L(3, 1) << "+" << L(1, 1) << "*\\x+" << L(2, 1) << "*\\y+" << L(3, 1) << "*\\z," << L(1, 2) + L(2, 2) + L(3, 2) << "+" << L(1, 2) << "*\\x+"
        << L(2, 2) << "*\\y+" << L(3, 2) << "*\\z," << L(1, 3) + L(2, 3) + L(3, 3) << "+" << L(1, 3) << "*\\x+" << L(2, 3) << "*\\y+" << L(3, 3) << "*\\z);" << endl;

    *ss << R"(\if{\x = 0 \AND  \y = 0 \AND \z = 0}{)" << endl;
