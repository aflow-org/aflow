...
namespace SYM {
  ...
  template <class dmmy> bool allsame(vector<dmmy> vec) {
    bool all = true;
...
namespace SYM {
  ...
  void get_Wyckoff_from_letter(uint space_group_number, string& space_group_setting, string& Wyckoff_letter, uint& Wyckoff_multiplicity, string& site_symmetry, vector<string>& positions); // DX20191029
  void get_Wyckoff_from_letter(string& spaceg, string& Wyckoff_letter, uint& Wyckoff_multiplicity, string& site_symmetry, vector<string>& positions);
  ...
  void getWyckoffAutomorphismSets(const string& spaceg, int mult, string site_symmetry, vector<string>& site_symmetries, vector<string>& letters, vector<vector<string>>& all_positions, bool origin_shifts_only = false); // DX20220828
  ...
  vector<string> findWyckoffEquations(uint space_group_number, string& space_group_setting, string& Wyckoff_letter, uint Wyckoff_multiplicity, bool convert2frac = true); // DX 20191029 //DX20200423 - add convert2frac
  ...
  void getWyckoffInformation(const uint space_group_number, const string& space_group_setting, const string& Wyckoff_letter, uint& Wyckoff_multiplicity, string& site_symmetry, vector<vector<string>>& all_positions); // DX20191030
  void getWyckoffInformation(const string& Wyckoff_string, const string& Wyckoff_letter, uint& Wyckoff_multiplicity, string& site_symmetry, vector<vector<string>>& all_positions); // DX20191030
  ...
  vector<vector<vector<string>>> getWyckoffLettersWithSameMultiplcityAndSiteSymmetry(uint& space_group_number, vector<GroupedWyckoffPosition>& grouped_Wyckoff_positions, uint& cell_choice); // DX20190201
  ...
  char discern_rot_sym(const xmatrix<double>& m, double& tol); // DX20190215
  ...
  deque<deque<_atom>> groupSymmetryEquivalentAtoms(deque<_atom>& atoms, xmatrix<double>& lattice, vector<xmatrix<double>>& sym_ops, vector<xvector<double>>& translations, double& min_dist, double& tol); // DX20190215
  ...
  vector<xvector<double>> getLatticeVectorsFromOriginalRotationOperations(
      vector<Screw>& old_rotations_twofold, vector<Screw>& old_rotations_higher, vector<Screw>& new_rotations, vector<xvector<double>>& twofold_lattice_vectors, vector<xvector<double>>& rot_lattice_vectors, bool& all_matched);
...
template <class d> void print(vector<d> vec) {
  for (uint i = 0; i < vec.size(); i++) {
    cout << vec[i] << endl;
  }
}
