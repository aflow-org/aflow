# Imports
from aflow_nhull_python import NHull
import json
import re
import numpy as np
...
class Plotter:
    def __init__(self, aflow_executable = 'aflow'):
        ...
        # Load NHull in case a new json must be created
        self.nhull = NHull(aflow_executable=aflow_executable)
    ...
    def plot(self, alloy):
        ...
        # ----- Load NHull json -----
        hull = self.nhull.get_hull(alloy)
        if not em3:
            # 2D Cartesian plot
            _Two_Species_Plotter().plot_NHull(hull, em1, em2)
        else:
            # Ternary plot
            _Three_Species_Plotter().plot_NHull(hull, em1, em2, em3)


# For plotting 2 species alloys (2D cartesian)
class _Two_Species_Plotter:
    def plot_NHull(self, hull, em1, em2):
...
class _Three_Species_Plotter:
    def plot_NHull(self, hull, em1, em2, em3):
