...
namespace estructure {
  string PEDOS_GENERATE_GNUPLOTSCRIPT(const string& plot_pdosname,
                                      const string& PEDOStitle,
                                      const double& DOS_Emin,
                                      const double& DOS_Emax,
                                      const double& DOSMAX,
                                      const double& DOSSCALE,
                                      const int& Natom_pdos,
                                      const vector<vector<vector<double>>>& PDOS,
                                      const string& pdosdatafile) {
...
namespace estructure {
  void PLOT_DOS(string options) {
    ...
    double DOS_Emin = DEFAULT_DOS_EMIN;
    double DOS_Emax = DEFAULT_DOS_EMAX;
    double DOSSCALE = DEFAULT_DOS_SCALE; // some defaults
    ...
    if (LDEBUG) {
      for (size_t i = 0; i < tokens.size(); i++) {
        cerr << __AFLOW_FUNC__ << " tokens.at(i)=" << tokens.at(i) << endl;
      }
    }
    ...
    stringstream ss_doscar; //, stringstreamtmp, strline;
    ...
    if (tokens.size() >= 2) {
      DOS_Emin = aurostd::string2utype<double>(tokens.at(1));
    }
    if (tokens.size() >= 3) {
      DOS_Emax = aurostd::string2utype<double>(tokens.at(2));
    }
    if (tokens.size() >= 4) {
      DOSSCALE = aurostd::string2utype<double>(tokens.at(3));
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " DOS_Emin=" << DOS_Emin << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " DOS_Emax=" << DOS_Emax << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " DOSSCALE=" << DOSSCALE << endl;
    }
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    xOUTCAR pedos_outcar;
    pedos_outcar.GetPropertiesFile(aflowlib::vaspfile2stringstream(directory, "OUTCAR"));
    ...
    const string str_dos = aurostd::vector2string(vvDOS);
    const string dosdatafile = ICSDName + "_DOS.dat";
    ofstream fin;
    ...
    const string gnuplotscript = aurostd::CleanFileName("GNUPLOT_" + ICSDName + "_DOS.gp");
    //////ofstream fin;
    ...
    if (aurostd::FileExist(ICSDName + "_DOS.eps")) {
      aurostd::execute(DEFAULT_KZIP_BIN + " -9f " + ICSDName + "_DOS.eps"); // [CORMAC]
    }
    if (LDEBUG) {
...
namespace estructure {
  void PLOT_DOSWEB(string options) {
    ...
    double DOSMAX = 0.0;
    string directory = "./";
    const string ICSDName = "AFLOW_DOS_WEB";
    string stmp;
    string line;
    stringstream strline;
    stringstream dosfile; //, stringstreamtmp, strline;
    ...
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/DOSCAR.static" + XHOST.vext[iext])) {
        aurostd::compressfile2stringstream(directory + "/DOSCAR.static" + XHOST.vext[iext], dosfile);
      }
      found = !dosfile.str().empty();
    }
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/DOSCAR" + XHOST.vext[iext])) {
        aurostd::compressfile2stringstream(directory + "/DOSCAR" + XHOST.vext[iext], dosfile);
      }
      found = !dosfile.str().empty();
    }
    ...
    const int TotalNumberDOSCAR = aurostd::GetNLinesString(dosfile);
    ...
    dosfile >> stmp;
    getline(dosfile, stmp);
    dosfile >> Emax >> Emin >> DOSGRID >> Efermi >> stmp;
    getline(dosfile, stmp);
    ...
    NIONS = ((TotalNumberDOSCAR - 5) / (DOSGRID + 1)) - 1;
    if (NIONS > 1) {
      FLAG_PDOS = true;
    }
    ...
    if (FLAG_PDOS) {
      ...
      // Wringting into file
      const string dosdatafile = ICSDName + "_TDOS.dat";
      ofstream fin;
      fin.open(dosdatafile.c_str());
      fin.setf(ios_base::fixed, ios_base::floatfield);
      fin.precision(6);
...
namespace estructure {
  void PLOT_PEDOSALL_AFLOWLIB(string options, _aflags& aflags) {
    ...
    if (LDEBUG) {
      for (size_t i = 0; i < tokens.size(); i++) {
        cerr << __AFLOW_FUNC__ << " tokens.at(i)=" << tokens[i] << endl;
      }
    }
    ...
    for (unsigned int iat = 0; iat < xstr_bands.atoms.size(); iat++) {
      InequivalentAtomType.push_back(xstr_bands.atoms[iat].equivalent);
    }

    vector<int> InequiAtomTypePlot;
    const int n = xstr_bands.atoms.size();
    for (int i = 0; i < n; i++) {
      bool matching = false;
      for (int j = 0; (j < i) && (matching == false); j++) {
        if (InequivalentAtomType[i] == InequivalentAtomType[j]) {
          matching = true;
        }
      }
      if (!matching) {
        InequiAtomTypePlot.push_back(InequivalentAtomType[i]);
      }
    }
    ...
    const bool isLSCOUPLING = pedos_outcar.isLSCOUPLING;
    if (isLSCOUPLING) {
    ...
    for (unsigned int i = 0; i < InequiAtomTypePlot.size(); i++) {
      ...
      const string str_out = aurostd::PaddedPRE(aurostd::utype2string(Natom), (uint) (1 + log10(NIONS)), "0") + "/" + aurostd::PaddedPRE(aurostd::utype2string(NIONS), (uint) (1 + log10(NIONS)), "0") + "/" +
      ...
      if (Natom > 0 && Natom <= NIONS) {
        ...
        const string PDOSNAME = "PEDOS_" + aurostd::PaddedPRE(aurostd::utype2string(Natom), (uint) (1 + log10(NIONS)), "0") + "_" +
                                aurostd::PaddedPRE(aurostd::utype2string(NIONS), (uint) (1 + log10(NIONS)), "0") + "_" + AtomicName[Natom - 1];

        //************************************************************************************************************************
        //************************************************************************************************************************
        // Finding the maxmium value of PDOS
        const int Natom_pdos = Natom - 1; // Make the Nth atom corespond to the (Nth-1) data in PDOS data array.
        DOSMAX = aurostd::FindMaxIn2DvectorExcept1stColumn(PDOS[Natom_pdos], DOS_Emin, DOS_Emax);
        //************************************************************************************************************************

        // Writing PEDOS data into file
        const string pdosdatafile = ICSDName + "_PEDOS_" + aurostd::PaddedPRE(aurostd::utype2string(Natom), (uint) (1 + log10(NIONS)), "0") + "_" +
                                    aurostd::PaddedPRE(aurostd::utype2string(NIONS), (uint) (1 + log10(NIONS)), "0") + "_" + AtomicName[Natom - 1] + ".dat";

        ofstream fin;
        fin.open(pdosdatafile.c_str());
        fin.setf(ios_base::fixed, ios_base::floatfield);
        fin.precision(6);
        const string str_pdos = aurostd::vector2string(PDOS[Natom_pdos]);
        fin << str_pdos;
        fin.close();
        ...
        //***********************************GENERATING GNUPLOT SCRIPT************************************
        // Writing Gnuplot Script
        const string gnuplotscript = aurostd::CleanFileName("GNUPLOT_" + ICSDName + "_PEDOS_" + aurostd::PaddedPRE(aurostd::utype2string(Natom), (uint) (1 + log10(NIONS)), "0") + "_" +
                                                            aurostd::PaddedPRE(aurostd::utype2string(NIONS), (uint) (1 + log10(NIONS)), "0") + "_" + AtomicName[Natom - 1] + ".gp");

        const string plot_pdosname = ICSDName + "_" + PDOSNAME;

        const string PEDOStitle = estructure::changeICSDNameGunplot(ICSDName) + "\\\\_" + "PEDOS\\\\_" + aurostd::PaddedPRE(aurostd::utype2string(Natom), (uint) (1 + log10(NIONS)), "0") + "\\\\_" +
                                  aurostd::PaddedPRE(aurostd::utype2string(NIONS), (uint) (1 + log10(NIONS)), "0") + "\\\\_" + AtomicName[Natom - 1] + "(" +
                                  aurostd::utype2string(InequivalentAtomType[Natom - 1]) + ") \"";

        ofstream gscript_fin;
        gscript_fin.open(gnuplotscript.c_str());
        const string gscript = estructure::PEDOS_GENERATE_GNUPLOTSCRIPT(plot_pdosname, PEDOStitle, DOS_Emin, DOS_Emax, DOSMAX, DOSSCALE, Natom_pdos, PDOS, pdosdatafile);
        gscript_fin << gscript;
        gscript_fin.close();
        ...
        if (aurostd::FileExist(plot_pdosname + ".eps")) {
          aurostd::execute(DEFAULT_KZIP_BIN + " -9f " + plot_pdosname + ".eps"); // [CORMAC]
        }
        // Postprocess
        if (!XHOST.vflag_control.flag("KEEP::GPL")) {
...
namespace estructure {
  void PLOT_PEDOSALL(string options) {
    ...
    if (tokens.size() >= 2) {
      command_postfix += "," + tokens.at(1);
    }
    if (tokens.size() >= 3) {
      command_postfix += "," + tokens.at(2);
    }
    if (tokens.size() >= 4) {
      command_postfix += "," + tokens.at(3);
    }
    ...
    for (int i = 1; i <= NIONS; i++) {
      ...
      cout << __AFLOW_FUNC__ << " working on " << aurostd::PaddedPRE(aurostd::utype2string(i), (uint) (1 + log10(NIONS)), "0") << "/"
...
namespace estructure {
  void PLOT_PEDOS(string options) {
    ...
    string numatom = "1";
    string directory = "./"; // some defaults
    vector<string> tokens;
    aurostd::string2tokens(options, tokens, ",");
    double DOS_Emin = DEFAULT_DOS_EMIN;
    double DOS_Emax = DEFAULT_DOS_EMAX;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " numatom=[" << numatom << "]" << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " DOS_Emin=" << DOS_Emin << endl;
    }
...
namespace estructure {
  bool isSpecialKPOINT(string kpoint) {
    if (kpoint.compare("\\Gamma") == 0) {
      return true;
    }
    if (kpoint.compare("Gamma") == 0) {
      return true;
    }
    if (kpoint.compare("\\Sigma_1") == 0) {
      return true;
    }
    if (kpoint.compare("Sigma_1") == 0) {
      return true;
    }
    if (kpoint.compare("\\Sigma") == 0) {
      return true;
    }
    if (kpoint.compare("Sigma") == 0) {
      return true;
  ...
  string fixSpecialKPOINT_GNUPLOT(string kpoint, bool json) {
    if (kpoint.compare("\\Gamma") == 0) {
      return "{" + (json ? string("\\") : "") + "/Symbol G}";
    } // NO ITALICS, Gamma is from GOD -- says SC
    if (kpoint.compare("Gamma") == 0) {
      return "{" + (json ? string("\\") : "") + "/Symbol G}";
    } // NO ITALICS, Gamma is from GOD -- says SC
    if (kpoint.compare("\\Sigma_1") == 0) {
      return "{" + (json ? string("\\") : "") + "/Symbol-Oblique S}_1";
    }
    if (kpoint.compare("Sigma_1") == 0) {
      return "{" + (json ? string("\\") : "") + "/Symbol-Oblique S}_1";
    }
    if (kpoint.compare("\\Sigma") == 0) {
      return "{" + (json ? string("\\") : "") + "/Symbol-Oblique S}";
    }
    if (kpoint.compare("Sigma") == 0) {
      return "{" + (json ? string("\\") : "") + "/Symbol-Oblique S}";
    }
  ...
  string fixKPOINT_GNUPLOT(string kpoint, bool json) {
    if (isSpecialKPOINT(kpoint)) {
      return fixSpecialKPOINT_GNUPLOT(kpoint, json);
    } else {
      if (aurostd::substring2bool(kpoint, "_")) {
        ...
        if (parts.size() != 2) {
          return kpoint;
        } // not expected, code this exception
        kpoint = "{" + (json ? string("\\") : "") + "/Helvetica-Oblique " + parts[0] + "}_" + parts[1];
      } else {
        kpoint = "{" + (json ? string("\\") : "") + "/Helvetica-Oblique " + kpoint + "}";
      }
      return kpoint;
    }
  }
  string fixKPOINT_HTML(string kpoint) {
    if (isSpecialKPOINT(kpoint)) {
      return fixSpecialKPOINT_HTML(kpoint);
    } else {
      if (aurostd::substring2bool(kpoint, "_")) {
        vector<string> parts;
        aurostd::string2tokens(kpoint, parts, "_");
  ...
  string fixKPOINT_LATEX(string kpoint) {
    if (isSpecialKPOINT(kpoint)) {
      return fixSpecialKPOINT_LATEX(kpoint);
    } else {
      if (aurostd::substring2bool(kpoint, "_")) {
        vector<string> parts;
        aurostd::string2tokens(kpoint, parts, "_");
  ...
  string fixKPOINT_SPECIALONLY(string kpoint) {
    if (aurostd::substring2bool(kpoint, "\\")) {
      return aurostd::StringSubst(kpoint, "\\", "\\\\");
    }
    return kpoint;
  }
} // namespace estructure
...
namespace estructure {
  void PLOT_BAND(string options) {
    ...
    stringstream ss_outcar;
    stringstream ss_doscar;
    stringstream ss_engenval;
    stringstream ss_kpoints;
    ...
    while (getline(ss_outcar, line)) {
      ...
      if (line.find(anchor_word_vc) != string::npos) {
        getline(ss_outcar, line);
        ss_outcar >> a1[1] >> a1[2] >> a1[3] >> b1[1] >> b1[2] >> b1[3];
        getline(ss_outcar, line);
        ss_outcar >> a2[1] >> a2[2] >> a2[3] >> b2[1] >> b2[2] >> b2[3];
        getline(ss_outcar, line);
        ss_outcar >> a3[1] >> a3[2] >> a3[3] >> b3[1] >> b3[2] >> b3[3];
      }
    }
    ...
    // Writing into file
    const string str_PDOS = aurostd::vector2string(vvDOS); // TOTALPDOS);  //CO20170830 - plot both tdos and pdos!
    const string dosdatafile = ICSDName + "_DOS.dat";
    ofstream dosfin;
    ...
    for (int i = 0; i < NKPOINTS; i++) {
      dataup[i].resize(Nbands);
      datadn[i].resize(Nbands);
    }
    ...
    ss_kpoints >> LattName >> stmp;
    getline(ss_kpoints, stmp);
    ss_kpoints >> KPOINTSGRID >> stmp >> stmp >> stmp;
    getline(ss_kpoints, stmp);
    getline(ss_kpoints, stmp);
    getline(ss_kpoints, stmp);
    ...
    for (int i = 0; i < count; i++) {
      kpointslabel[i] = fixKPOINT_GNUPLOT(kpointslabel[i]);
    } // CO20170831 - fix all at once
    // Combine the Labels of special kpoints
    for (int i = 1; i < (count - 1); i = i + 2) {
      if (kpointslabel[i].compare(kpointslabel[i + 1]) != 0) {
        const bool need_space = !aurostd::substring2bool(kpointslabel[i], "_"); // CO20170830 - if subscript, extra space looks ODD!
        const string newlabel = aurostd::attach(kpointslabel[i] + (need_space ? " " : ""), kpointslabel[i + 1]); // CO20170830 - italics and make space for first italics
    ...
    // NO SHIFT - CO20170830
    for (int i = 0; i < count; i = i + 2) {
      SPKPOINTScart_LABEL[i] = SPKPOINTScart[i];
    }
    SPKPOINTScart_LABEL[count - 1] = SPKPOINTScart[count - 1];

    const string bandsdatafile = ICSDName + "_BANDS.dat";
    ofstream bandsfin;
    ...
    if (ISPIN == 1) {
      ...
      for (int ik = 0; ik < NKPOINTS; ik++) {
        if (ik && !(ik % KPOINTSGRID)) {
          bandsfin << endl << endl;
        } // CO20170831 - make sure there is NO continuation between KPOINTS, not valid assumption
    ...
    //***********************************GENERATING GNUPLOT SCRIPT**************************************************************
    // Writing Gnuplot Script
    const string gnuplotscript = aurostd::CleanFileName("GNUPLOT_" + ICSDName + ".gp");
    ofstream fin;
    fin.open(gnuplotscript.c_str());
    fin << "#Generated by AFLOW (Kesong Yang [kesong.yang@gmail.com], 2011, Duke University)" << endl;
    fin << "set term postscript eps enhanced color font \"Helvetica, 40\" size 18, 10.125" << endl;
    ...
    if (aurostd::FileExist("bz_.eps")) {
      aurostd::file2file("bz_.eps", ICSDName + "_BZ.eps");
    }
    if (aurostd::FileExist("bz_.png")) {
      aurostd::file2file("bz_.png", ICSDName + "_BZ.png");
    }
    if (aurostd::FileExist("bz_" + ICSDName + ".eps")) {
      aurostd::file2file("bz_" + ICSDName + ".eps", ICSDName + "_BZ.eps");
    }
    if (aurostd::FileExist("bz_" + ICSDName + ".png")) {
      aurostd::file2file("bz_" + ICSDName + ".png", ICSDName + "_BZ.png");
    }
    ...
    if (aurostd::FileExist(ICSDName + "_BZ.eps")) {
      aurostd::execute(DEFAULT_KZIP_BIN + " -9f \"" + ICSDName + "_BZ.eps\"");
    }
    ...
    if (aurostd::FileExist(ICSDName + ".eps")) {
      aurostd::execute(DEFAULT_KZIP_BIN + " -9f \"" + ICSDName + ".eps\""); // [CORMAC]
    }
  }
} // namespace estructure
...
namespace estructure {
  void PLOT_BAND2(string options) {
    ...
    // CO20180216 - this function needs PDOS
    if (!(estructure::GET_DOS_DATA(ss_doscar, ss_outcar, Efermi, TDOS, TOTALPDOS_SPIN) && !TOTALPDOS_SPIN.empty())) {
      message << "DOSCAR extraction failed, perhaps there is no PDOS" << endl;
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_MISSING_);
    }

    vector<vector<double>> TOTALPDOS;
    if (TOTALPDOS_SPIN.at(0).size() == 4) {
      TOTALPDOS = TOTALPDOS_SPIN;
    } else if (TOTALPDOS_SPIN.at(0).size() == 5) {
      TOTALPDOS = TOTALPDOS_SPIN;
    } else if (TOTALPDOS_SPIN.at(0).size() == 7) {
      for (size_t i = 0; i < TOTALPDOS_SPIN.size(); i++) {
        vector<double> vtmp;
        vtmp.clear();
    ...
    const string str_PDOS = aurostd::vector2string(TOTALPDOS);
    // Wringting into file
    const string dosdatafile = ICSDName + "_DOS.dat";
    ofstream dosfin;
    dosfin.open(dosdatafile.c_str());
...
namespace estructure {
  void PLOT_BAND_SPINSPLIT(string options) {
    ...
    stringstream straus;
    stringstream strline; //, dosfile; //, stringstreamtmp, strline;
    ...
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/OUTCAR.bands" + XHOST.vext[iext])) {
        aurostd::compressfile2stringstream(directory + "/OUTCAR.bands" + XHOST.vext[iext], straus);
      }
      found = !straus.str().empty();
    }
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/OUTCAR" + XHOST.vext[iext])) {
        aurostd::compressfile2stringstream(directory + "/OUTCAR" + XHOST.vext[iext], straus);
      }
      found = !straus.str().empty();
    }
    if (!found) {
    ...
    while (getline(straus, line)) {
      ...
      if (line.find(anchor_word_vc) != string::npos) {
        getline(straus, line);
        straus >> a1[1] >> a1[2] >> a1[3] >> b1[1] >> b1[2] >> b1[3];
        getline(straus, line);
        straus >> a2[1] >> a2[2] >> a2[3] >> b2[1] >> b2[2] >> b2[3];
        getline(straus, line);
        straus >> a3[1] >> a3[2] >> a3[3] >> b3[1] >> b3[2] >> b3[3];
      }
    }
    ...
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/EIGENVAL.bands" + XHOST.vext[iext])) {
        aurostd::compressfile2stringstream(directory + "/EIGENVAL.bands" + XHOST.vext[iext], straus);
      }
      found = !straus.str().empty();
    }
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/EIGENVAL" + XHOST.vext[iext])) {
        aurostd::compressfile2stringstream(directory + "/EIGENVAL" + XHOST.vext[iext], straus);
      }
      found = !straus.str().empty();
    }
    if (!found) {
    ...
    found = false;
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/KPOINTS.bands" + XHOST.vext[iext])) {
        aurostd::compressfile2stringstream(directory + "/KPOINTS.bands" + XHOST.vext[iext], straus);
      }
      found = !straus.str().empty();
    }
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/KPOINTS" + XHOST.vext[iext])) {
        aurostd::compressfile2stringstream(directory + "/KPOINTS" + XHOST.vext[iext], straus);
      }
      found = !straus.str().empty();
    }
    if (!found) {
    ...
    straus >> LattName >> stmp;
    getline(straus, stmp);
    straus >> KPOINTSGRID >> stmp >> stmp >> stmp;
    getline(straus, stmp);
    getline(straus, stmp);
    getline(straus, stmp);
    ...
    if (ISPIN == 2) {
      const string gnuplotscriptup = aurostd::CleanFileName("GNUPLOT_" + ICSDName + "_BANDS_UP.gp");
      ofstream finup;
      ...
      // spin down
      const string gnuplotscriptdn = aurostd::CleanFileName("GNUPLOT_" + ICSDName + "_BANDS_DN.gp");
      ofstream findn;
...
namespace estructure {
  string changeICSDNameGunplot(string ICSDName) {
    string ICSDNameNew;
    for (size_t i = 0; i < ICSDName.size(); i++) {
      const char ch = ICSDName[i];
      if (ch != '_') {
...
namespace estructure {
  void CombineTDOSAndTOTALPDOS(const vector<vector<double>>& TDOS, const vector<vector<double>>& TOTALPDOS, vector<vector<double>>& vvDOS) {
    ...
    vvDOS.clear();
    const int Ncolumns = TOTALPDOS.at(0).size();
    for (size_t i = 0; i < TOTALPDOS.size(); i++) {
      vector<double> vtmp;
      for (int j = 0; j < Ncolumns; j++) {
        vtmp.push_back(TOTALPDOS[i].at(j));
      }
      if ((Ncolumns == 4) || (Ncolumns == 5)) {
...
namespace estructure {
  double GET_PDOSDATA(const string& str_dir, vector<vector<vector<double>>>& PDOS) {
    ...
    vector<vector<double>> TOTALPDOS;
    // CO20180216 - this function needs PDOS
    if (!(estructure::GET_DOS_DATA(str_dir, Efermi, TDOS, TOTALPDOS, PDOS) && !TOTALPDOS.empty())) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "DOSCAR extraction failed, perhaps there is no PDOS", _INPUT_MISSING_);
    }
    return Efermi;
  }
...
namespace estructure {
  double GET_PDOSDATA(stringstream& ss_dosfile, stringstream& ss_outfile, vector<vector<vector<double>>>& PDOS) {
    ...
    vector<vector<double>> TOTALPDOS;
    // CO20180216 - this function needs PDOS
    if (!(estructure::GET_DOS_DATA(ss_dosfile, ss_outfile, Efermi, TDOS, TOTALPDOS, PDOS) && !TOTALPDOS.empty())) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "DOSCAR extraction failed, perhaps there is no PDOS", _INPUT_MISSING_);
    }
    return Efermi;
  }
...
namespace estructure {
  bool GET_DOS_DATA(const string& str_dir, double& Efermi, vector<vector<double>>& TDOS, vector<vector<double>>& TOTALPDOS, vector<vector<vector<double>>>& PDOS) {
    stringstream ss_dosfile;
    stringstream ss_outfile;
    if (aflowlib::VaspFileExist(str_dir, "OUTCAR")) {
      aflowlib::vaspfile2stringstream(str_dir, "OUTCAR", ss_outfile); // Not return even OUTCAR does not exist
    }
...
namespace estructure {
  bool GET_DOS_DATA(stringstream& ss_dosfile, stringstream& ss_outfile, double& Efermi, vector<vector<double>>& TDOS, vector<vector<double>>& TOTALPDOS, vector<vector<vector<double>>>& PDOS) {
    ...
    for (size_t i = 0; i < PDOS.size(); i++) {
      for (size_t j = 0; j < PDOS[i].size(); j++) {
        PDOS[i][j].clear();
      }
      PDOS[i].clear();
    }
    ...
    ss_dosfile >> Emax >> Emin >> DOSGRID >> Efermi >> strtmp;
    getline(ss_dosfile, strtmp);

    const int TotNoDOSCAR = aurostd::GetNLinesString(ss_dosfile);
    ...
    if (TotNoDOSCAR - 6 < DOSGRID) {
      cerr << "DOSCAR is cut-off, likely an IO problem" << endl;
      return false;
    } //"DOSCAR is not intergrated!" << endl; return false;  //CO20180216
    bool FLAG_PDOS = false;
    if (NIONS == 0) {
      NIONS = ((TotNoDOSCAR - 5) / (DOSGRID + 1)) - 1; // if OUTCAR does not exist
    }
    // CO20180217
    ...
    if (TotNoDOSCAR - 6 > 2 * DOSGRID) {
      FLAG_PDOS = true; // CO20180217
    }
    ...
    if (aurostd::CountWordsinString(line) == 5) {
      ...
      ss_line >> TDOS[0][1] >> TDOS[0][2];
      TDOS[0][2] *= (-1); // Spin_up & Spin_dn
      ss_line >> TDOS[0][3] >> TDOS[0][4];
      TDOS[0][4] *= (-1); // Sum_up & Sum_dn
      for (int j = 1; j < DOSGRID; j++) {
        ss_dosfile >> TDOS[j][0]; // Energy
        ss_dosfile >> TDOS[j][1] >> TDOS[j][2];
        TDOS[j][2] *= (-1); // Spin_up & Spin_dn
        ss_dosfile >> TDOS[j][3] >> TDOS[j][4];
        TDOS[j][4] *= (-1); // Sum_up & Sum_dn
        getline(ss_dosfile, strtmp);
      }
    }
...
namespace estructure {
  void FormatSpinofPDOS(vector<vector<vector<double>>>& vvva) {
    const vector<vector<double>> vvtmp;
    const int DOSGRID = vvva.at(0).size();
...
namespace estructure {
  ...
  bool DOSDATA_JSON(aurostd::xoption& vpflow, string directory, stringstream& json, bool wrapping_brackets) { // CO20171025
    ...
    // double DOSMAX=0.0;
    // string directory="./";
    const string ICSDName = KBIN::ExtractSystemName(directory); // Get SystemName
    string stmp; //, line;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " options=[" << vpflow.getattachedscheme("DOSDATA2JSON::PARAMS") << "]" << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " tokens.size()=" << tokens.size() << endl;
    }
    ...
    if (!tokens.empty()) {
      DOS_Emin = aurostd::string2utype<double>(tokens.at(0));
    }
    if (tokens.size() >= 2) {
      DOS_Emax = aurostd::string2utype<double>(tokens.at(1));
    }
    if (tokens.size() >= 3) {
      DOSSCALE = aurostd::string2utype<double>(tokens.at(2));
    }
    ...
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (POSCAR_file.empty() && aurostd::FileExist(directory + "/POSCAR.static" + XHOST.vext[iext])) {
        POSCAR_file = directory + "/POSCAR.static" + XHOST.vext[iext];
      }
    }
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (POSCAR_file.empty() && aurostd::FileExist(directory + "/POSCAR" + XHOST.vext[iext])) {
        POSCAR_file = directory + "/POSCAR" + XHOST.vext[iext];
      }
    }
    ...
    // CO20180216
    const bool found_TDOS = (!TDOS.empty() && !TDOS[0].empty());
    const bool found_PDOS = (!PDOS.empty() && !PDOS[0].empty() && !PDOS[0][0].empty());
    ...
    if (wrapping_brackets) {
      json << "{" << std::endl;
    } // CO20171025
    json << "     \"name\": " << "\"" << name << "\"," << endl;
    json << "     \"species\": [" << "\"" << vspecies[0] << "\"";
    for (size_t i = 1; i < vspecies.size(); i++) {
      json << ", \"" << vspecies[i] << "\"";
    }
    json << "]," << endl;
    ...
    if (found_PDOS) {
      if (PDOS[0][0].size() == 4) {
        ...
        json << R"(          "orbitals": ["s", "p", "d"],)" << std::endl;
      ...
      if (PDOS[0][0].size() == 5) {
        ...
        json << R"(          "orbitals": ["s", "p", "d", "f"],)" << std::endl;
        json << "          \"spin_polarized\": false," << std::endl;
  ...
  uint inequivalentAtomsJSON(vector<vector<vector<double>>>& PDOS, vector<int>& iatoms, vector<double>& numbers, vector<string>& vspecies, ostream& json) {
    ...
    for (size_t i = 0; i < vspecies.size(); i++) {
      json << "          \"" << vspecies[i] << "\": {" << endl;
      int comma = false;
      for (size_t j = 0; j < iatoms.size(); j++) {
        if (i) {
          if (iatoms[j] >= sum_numbers.at(i - 1) && iatoms[j] < sum_numbers.at(i)) {
            if (comma) {
              json << "," << endl;
            }
            json << "               \"" << iatoms[j] << "\": {" << endl;
            constructInequivalentAtomPDOSJSON(PDOS, iatoms[j], json);
            json << "               }";
            comma = true;
          }
        } else {
          if (iatoms[j] < sum_numbers.at(i)) {
  ...
  uint constructInequivalentAtomPDOSJSON(vector<vector<vector<double>>>& PDOS, int iatom, ostream& json) {
    ...
    if (PDOS[0][0].size() == 4) {
      ...
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][1];
      }
      json << "]," << endl;
      ...
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][1] + PDOS[iatom][k][2] + PDOS[iatom][k][3];
      }
      json << "]" << endl;
    }
    //--------------------s, p , d, f (non-polarized)-----------------------
    if (PDOS[0][0].size() == 5) {
      ...
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][1] + PDOS[iatom][k][2] + PDOS[iatom][k][3] + PDOS[iatom][k][4];
      }
      json << "]" << endl;
    }
    //--------------------s, p , d (polarized)-----------------------
    if (PDOS[0][0].size() == 7) {
      ...
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][1] + PDOS[iatom][k][3] + PDOS[iatom][k][5];
      }
      json << "]," << endl;
      json << "                    \"total_minority\": [" << PDOS[iatom][0][2] + PDOS[iatom][0][4] + PDOS[iatom][0][6];
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][2] + PDOS[iatom][k][4] + PDOS[iatom][k][6];
      }
      json << "]" << endl;
    }
    //--------------------s, p, d, f (polarized)-----------------------
    if (PDOS[0][0].size() == 9) {
      ...
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][1] + PDOS[iatom][k][3] + PDOS[iatom][k][5] + PDOS[iatom][k][7];
      }
      json << "]," << endl;
      json << "                    \"total_minority\": [" << PDOS[iatom][0][2] + PDOS[iatom][0][4] + PDOS[iatom][0][6] + PDOS[iatom][0][8];
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][2] + PDOS[iatom][k][4] + PDOS[iatom][k][6] + PDOS[iatom][k][8];
      }
      json << "]" << endl;
    }
    //--------------------s, py, pz, px, dxy, dyz, dz2, dxz, dx2-y2 (non-spin-polarized)-----
    if (PDOS[0][0].size() == 10) {
      ...
      json << "]," << endl;
      json << "                    \"total\": ["
           << PDOS[iatom][0][1] + PDOS[iatom][0][2] + PDOS[iatom][0][3] + PDOS[iatom][0][4] + PDOS[iatom][0][5] + PDOS[iatom][0][6] + PDOS[iatom][0][7] + PDOS[iatom][0][8] + PDOS[iatom][0][9];
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][1] + PDOS[iatom][k][2] + PDOS[iatom][k][3] + PDOS[iatom][k][4] + PDOS[iatom][k][5] + PDOS[iatom][k][6] + PDOS[iatom][k][7] + PDOS[iatom][k][8] + PDOS[iatom][k][9];
      }
      json << "]" << endl;
    }
    //--------------------stotal, sx, sy, xz, ptotal, px, py, pz ... (LS COUPLINT)------------
    if (PDOS[0][0].size() == 13) {
      ...
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][10];
      }
      json << "]," << endl;
      json << "                    \"dy\": [" << PDOS[iatom][0][11];
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][11];
      }
      json << "]," << endl;
      json << "                    \"dz\": [" << PDOS[iatom][0][12];
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][12];
      }
      json << "]," << endl;
      json << "                    \"total\": [" << PDOS[iatom][0][1] + PDOS[iatom][0][5] + PDOS[iatom][0][9];
      for (size_t k = 1; k < PDOS[0].size(); k++) {
        json << "," << PDOS[iatom][k][1] + PDOS[iatom][k][5] + PDOS[iatom][k][9];
      }
      json << "]" << endl;
    }
    //--------------------s, py, pz, px, dxy, dyz, dz2, dxz, dx2-y2, f1 to f7 (non-spin-polarized)-----
...
namespace estructure {
  ...
  bool BANDSDATA_JSON(aurostd::xoption& vpflow, string directory, stringstream& json, bool wrapping_brackets) { // CO20171025
    ...
    vector<vector<double>> kpoints;
    vector<string> kpointslabel;
    vector<string> kpointslabel_html;
    vector<string> kpointslabel_gnuplot;
    vector<string> kpointslabel_latex; // CO20171025
    int count = 0;
    int j = 0; // count is the number of rows of kpoints
    // cout << ss_kpoints.str() << endl;
    while (getline(ss_kpoints, line)) {
    ...
    //*******************************Redefine the symbols of special kpoints
    for (int i = 0; i < count; i++) {
      kpointslabel[i] = fixKPOINT_SPECIALONLY(kpointslabel[i]);
    } // fixKPOINT(kpointslabel[i]);  //CO20170831 - fix all at once
    for (int i = 0; i < count; i++) {
      kpointslabel_html[i] = fixKPOINT_HTML(kpointslabel_html[i]);
    } // fixKPOINT(kpointslabel[i]);  //CO20170831 - fix all at once
    for (int i = 0; i < count; i++) {
      kpointslabel_gnuplot[i] = fixKPOINT_SPECIALONLY(fixKPOINT_GNUPLOT(kpointslabel_gnuplot[i], true));
    } // fixKPOINT(kpointslabel[i]);  //CO20170831 - fix all at once
    for (int i = 0; i < count; i++) {
      kpointslabel_latex[i] = fixKPOINT_SPECIALONLY(fixKPOINT_LATEX(kpointslabel_latex[i]));
    } // fixKPOINT(kpointslabel[i]);  //CO20170831 - fix all at once
    // Combine the Labels of special kpoints
    string newlabel;
    for (int i = 1; i < (count - 1); i = i + 2) {
    ...
    if (ISPIN == 1) {
      ...
      json << "," << endl;
      json << R"(     "title": ")" << ICSDName << " (" << LattName << ")\"," << endl; // CO20171025
      ...
      //   if(kpointslabel[0]=="G") json << "\"&Gamma;\""; else if(kpointslabel[0]=="Sigm") json << "\"&Sigma;\""; else if(kpointslabel[0]=="Sigm_1") json << "\"&Sigma;<sub>1</sub>\""; else
      //   if(kpointslabel[0]=="Y_1") json << "\"Y<sub>1</sub>\""; else if(kpointslabel[0]=="Z_1") json << "\"Z<sub>1</sub>\""; else json << "\"" << kpointslabel[0] << "\""; for(int i=2;i<count;i=i+2)
      //   if(kpointslabel[i]=="G") json << ",\"&Gamma;\""; else if(kpointslabel[i]=="Sigm") json << ",\"&Sigma;\""; else if(kpointslabel[i]=="Sigm_1") json << ",\"&Sigma;<sub>1</sub>\""; else
      //   if(kpointslabel[i]=="Y_1") json << ",\"Y<sub>1</sub>\""; else if(kpointslabel[i]=="Z_1") json << ",\"Z<sub>1</sub>\""; else json << ",\"" << kpointslabel[i] << "\""; json << "]," << endl;

      json << "     \"kpoint_labels_latex\": ["; // CO20171025
      json << "\"" << kpointslabel_latex[0] << "\""; // CO20171025
      for (int i = 2; i < count; i = i + 2) {
        json << ",\"" << kpointslabel_latex[i] << "\""; // CO20171025
      }
    ...
    if (ISPIN == 2) {
      ...
      for (int i = 2; i < count; i = i + 2) {
        json << ",\"" << kpointslabel_gnuplot[i] << "\""; // CO20171025
      }
      ...
      for (int i = 2; i < count; i = i + 2) {
        json << ",\"" << kpointslabel_html[i] << "\""; // CO20171025
      }
