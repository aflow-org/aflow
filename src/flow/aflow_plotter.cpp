...
namespace plotter {
  ...
  xoption getPlotOptions(const aurostd::xoption& xopt, const string& key, bool datasets) {
    ...
    const uint ntokens = aurostd::string2tokens(scheme, tokens, ",", true); // Keep empty arguments
    ...
    if (ntokens >= 2 + shift) {
      plotoptions.push_attached("XMIN", tokens[1 + shift]);
    }
    if (ntokens >= 3 + shift) {
      plotoptions.push_attached("XMAX", tokens[2 + shift]);
    }
    if (ntokens >= 4 + shift) {
      plotoptions.push_attached("YSCALE", tokens[3 + shift]);
    }

    // Get title if present
    scheme = xopt.getattachedscheme("PLOTTER::TITLE");
    if (!scheme.empty()) {
      plotoptions.push_attached("TITLE", scheme);
    }

    // Get image format
    scheme = xopt.getattachedscheme("PLOTTER::PRINT");
    if (!scheme.empty()) {
      plotoptions.push_attached("IMAGE_FORMAT", aurostd::tolower(scheme));
    }
    ...
    if (!outfile.empty()) {
      if (scheme.empty()) {
        scheme = "." + DEFAULT_IMAGE_FORMAT;
      } else {
        scheme = "." + aurostd::tolower(scheme);
      }
      const uint nchar_scheme = scheme.size();
      const uint nchar_outfile = outfile.size();
      if (nchar_outfile > nchar_scheme) {
        uint i = 0;
        for (i = 0; i < nchar_scheme; i++) {
          if (scheme[i] != aurostd::tolower(outfile)[nchar_outfile - nchar_scheme + i]) {
            break;
          }
        }
        if (i == nchar_scheme) {
          outfile = outfile.substr(0, nchar_outfile - nchar_scheme);
        }
      }
  ...
  xoption getPlotOptionsEStructure(const aurostd::xoption& xopt, const string& key, bool datasets) {
    const bool LDEBUG = (false || _DEBUG_PLOTTER_ || XHOST.DEBUG);
    ...
    // Projection
    const string scheme = xopt.getattachedscheme("PLOTTER::PROJECTION");
    if (scheme.empty()) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " projection=" << plotoptions.getattachedscheme("PROJECTION") << endl;
    }
  ...
  xoption getPlotOptionsQHAthermo(const aurostd::xoption& xopt, const string& key) {
    ...
    const string scheme = xopt.getattachedscheme("PLOTTER::EOSMODEL");
    if (scheme.empty()) {
  ...
  void generateHeader(stringstream& out, const aurostd::xoption& plotoptions, bool multiplot) {
    const string plottitle = plotoptions.getattachedscheme("PLOT_TITLE");
    const string outformat = plotoptions.getattachedscheme("OUTPUT_FORMAT");
    if (outformat == "GNUPLOT") {
      ...
      if (plotoptions.flag("NOBORDER")) {
        out << "unset border" << std::endl;
      }
      ...
      out << "set grid back lt " << plotoptions.getattachedscheme("GRID_LINE_TYPE") << " lc rgb '" << plotoptions.getattachedscheme("GRID_COLOR") << "'"
          << " lw " << plotoptions.getattachedscheme("GRID_WIDTH") << std::endl;
      if (!plotoptions.flag("NOWATERMARK")) {
        out << "set label right '" << (plotoptions.flag("BANDDOS") ? "" : "\\scriptsize") << " " << AFLOWLIB_CONSORTIUM_STRING << "' at screen 0.98, 0.025" << std::endl;
      }
      out << std::endl;
    }
  }
  ...
  void savePlotGNUPLOT(const xoption& plotoptions, const stringstream& gpfile) {
    ...
    if (XHOST.is_command("gnuplot")) {
      const string versionstring = aurostd::execute2string(XHOST.command("gnuplot") + " --version");
      vector<string> tokens;
      aurostd::string2tokens(versionstring, tokens, " ");
      const double version = aurostd::string2utype<double>(tokens[1]);
      if (version < 5.0) {
        const string message = "Gnuplot needs to be version 5 or newer (found " + tokens[1] + ").";
    ...
    vector<string> required_binaries = {"gnuplot", "convert"};
    // ME20200609 - old pdfatex versions cannot process eps files
    if (pdflatex_version >= 2010) {
      required_binaries.emplace_back("pdflatex");
      required_binaries.emplace_back("repstopdf");
    } else {
      required_binaries.emplace_back("latex");
      required_binaries.emplace_back("dvips");
      required_binaries.emplace_back("ps2pdf");
    }
    vector<string> missing_binaries;
    for (size_t i = 0; i < required_binaries.size(); i++) {
      if (!XHOST.is_command(required_binaries[i])) {
        missing_binaries.push_back(required_binaries[i]);
      }
    }

    if (missing_binaries.empty()) {
      string directory_work = plotoptions.getattachedscheme("DIRECTORY");
      if (directory_work.empty()) {
        directory_work = aurostd::getPWD();
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " directory_work=" << directory_work << endl;
      }
      const string filename = plotoptions.getattachedscheme("FILE_NAME");
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " filename=" << filename << endl;
      }
      const string filename_latex = plotoptions.getattachedscheme("FILE_NAME_LATEX");
      // PDF is default since we use pdflatex to compile
      string format = plotoptions.getattachedscheme("IMAGE_FORMAT");
      if (format.empty()) {
        format = "pdf";
      }
      const string current_dir = aurostd::getPWD();
      // Create temp directory
      const string directory_tmp = aurostd::TmpDirectoryCreate("plotLATEX") + "/";
      fs::current_path(directory_tmp.c_str());
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " executing command: \"" << command << "\"" << endl;
      }
      aurostd::execute(command);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " directory_tmp = " << directory_tmp << endl;
      }
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << aurostd::execute("ls -las " + directory_tmp) << endl;
      }
      // ME20200609 - old pdfatex versions cannot process eps files
      if (pdflatex_version >= 2010) {
      ...
      if (format != "pdf") {
        command = XHOST.command("convert") + " -quiet -density 300 -background white \"" + filename_latex + ".pdf\" convert_output." + format +
                  " 1>/dev/null 2>&1";   // to avoid C: ... Carbon:PBE = C: in window //CO20210701 - io redirection
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " executing command: \"" << command << "\"" << endl;
        }
        output = aurostd::execute2string(command);
        if (!aurostd::FileExist("convert_output." + format)) {
          const string message = "The ImageMagick policy file disables ghostscript formats. Please see here and update the policy file: https://bugs.archlinux.org/task/60580";
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << aurostd::execute("ls -las " + directory_tmp) << endl;
        }
        aurostd::file2file("convert_output." + format, filename_latex + "." + format);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << aurostd::execute("ls -las " + directory_tmp) << endl;
        }
      }
      fs::current_path(current_dir.c_str());
      aurostd::CopyFile(directory_tmp + filename_latex + "." + format, directory_work + "/" + filename + "." + format);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " moving file to: " << directory_work + "/" + filename + "." + format << endl;
      }
      ...
      const string message = "The following binaries are missing: " + aurostd::joinWDelimiter(missing_binaries, " ") + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }
  }
  ...
  void setFileName(xoption& plotoptions, string filename) {
    const bool LDEBUG = (false || _DEBUG_PLOTTER_ || XHOST.DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " filename_in=" << filename << endl;
    }
    if (filename.empty()) {
      ...
      if (filename.empty()) {
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " default_title=" << default_title << endl;
        }
        // ME20200228 - Remove ANRL parameters
        const string::size_type t = default_title.find(":ANRL=");
        if (t != string::npos) {
          default_title = default_title.substr(0, t);
          if (LDEBUG) {
            std::cerr << __AFLOW_FUNC__ << " default_title (post ANRL)=" << default_title << std::endl;
          }
        }
        filename = default_title;
        // Get filename
        const string ext = plotoptions.getattachedscheme("EXTENSION");
        if (!ext.empty()) {
          if (filename.empty()) {
            filename = ext;
          } else {
        ...
        aurostd::StringSubstInPlace(filename, ' ', '_');
        const string set = plotoptions.getattachedscheme("DATASET");
    ...
    aurostd::StringSubstInPlace(filename_latex, ".", "_"); // The .tex file created by gnuplot cannot have . or includegraphics will break
    aurostd::StringSubstInPlace(filename_latex, ":", "_"); // ME20200409 - Some terminals do not handle : well, which can break convert
  ...
  // setTitle//////////////////////////////////////////////////////////////////
  //  Sets the plot title.
  void setTitle(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return setTitle(plotoptions, FileMESSAGE, oss);
  } // CO20200404
  void setTitle(xoption& plotoptions, ofstream& FileMESSAGE, ostream& oss) { // CO20200404
    ...
    // Format title
    if (title.empty()) {
      title = formatDefaultPlotTitle(plotoptions, FileMESSAGE, oss);
    }
  ...
  //  appropriately.
  string formatDefaultPlotTitle(const xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return formatDefaultPlotTitle(plotoptions, FileMESSAGE, oss);
  } // CO20200404
  string formatDefaultPlotTitle(const xoption& plotoptions, ofstream& FileMESSAGE, ostream& oss) { // CO20200404
    ...
    if (default_title.find("_ICSD_") != string::npos) {  // Check if AFLOW ICSD format
      ...
      if (tokens.size() == 3) {
        ...
        const string lattice = plotoptions.getattachedscheme("LATTICE");
        if (lattice.empty()) {
          title += ")";
        } else {
          title += ", " + lattice + ")";
      ...
      if ((tokens.size() > 2) && aurostd::substring2bool(tokens[2], "ANRL")) {
        const string::size_type t = tokens[2].find_first_of(":");
        if (t != string::npos) {
      ...
      const string::size_type loc = comp_str.find(":"); // remove LIB1 pp junk
      ...
      if ((tokens.size() == 2) || (tokens.size() == 3)) {
        ...
        std::string uid = anrl::getPrototypeUID(proto, false);
        if (!uid.empty()) {
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " found proto in - https://aflow.org/p/" << uid << endl;
          }
          if (tokens.size() == 3) {
            proto += "." + tokens[2];
          }
          const vector<string> elements = aurostd::getElements(comp_str);
          const vector<double> composition = getCompositionFromANRLPrototype(proto);
          if (LDEBUG) {
          ...
          vector<string> comp;
          uid = anrl::getPrototypeUID(proto, true);
          const int index = 0;
          if (!uid.empty()) {
            proto = aurostd::fixStringLatex(proto, false, false); // Prevent LaTeX errors
            const vector<string> elements = aurostd::getElements(comp_str);
            const vector<double> composition = getCompositionFromHTQCPrototype(proto, comp[index]);
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " elements=" << aurostd::joinWDelimiter(elements, ",") << endl;
              cerr << __AFLOW_FUNC__ << " composition=" << aurostd::joinWDelimiter(aurostd::vecDouble2vecString(composition, 5), ",") << endl;
            }
            ...
            try {
              title = pflow::prettyPrintCompound(elements, composition, no_vrt, true, latex_ft) + " (" + proto;
            } //_none_ //_latex_   //CO20190629 //DX+ME20210729 - for unaries in LIB2 errors
            catch (aurostd::xerror& excpt) {
              return aurostd::fixStringLatex(default_title, false, false);
            } // DX+ME20210729 - for unaries in LIB2 errors
  ...
  vector<double> getCompositionFromHTQCPrototype(const string& htqc_prototype, const string& composition) {
    ...
    const string::size_type t = htqc_prototype.find(".");
  ...
  vector<double> getCompositionFromANRLPrototype(const string& prototype) {
    ...
    if (t != string::npos) {
      seq = prototype.substr(t + 1, string::npos);
    }
    t = prototype.find("_");
  ...
  string formatDefaultTitlePOCC(const xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return formatDefaultTitlePOCC(plotoptions, FileMESSAGE, oss);
  } // CO20191110  //CO20200404
  string formatDefaultTitlePOCC(const xoption& plotoptions, ofstream& FileMESSAGE, ostream& oss) {  // CO version //CO20191110  //CO20200404
    const bool LDEBUG = (false || _DEBUG_PLOTTER_ || XHOST.DEBUG);
    const string default_title = plotoptions.getattachedscheme("DEFAULT_TITLE");
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " default_title=" << default_title << endl;
    }
    ...
    xstructure xstr;
    const bool found_pocc = pflow::POccInputs2Xstr(default_title, pocc_settings, xstr, FileMESSAGE, oss);
    if (!found_pocc) {
      return aurostd::fixStringLatex(default_title, false, false);
    }

    const string pps = pocc_settings.getattachedscheme("PPS");
    const string proto = pocc_settings.getattachedscheme("PROTO");
    const string pocc_params = pocc_settings.getattachedscheme("POCC_PARAMS");
    const string pocc_tol = pocc_settings.getattachedscheme("POCC_TOL");
    const string pocc_arun = pocc_settings.getattachedscheme("POCC_ARUN");
    const string module_arun = pocc_settings.getattachedscheme("MODULE_ARUN");
    ...
    if (!pocc_arun.empty()) {
      ...
      string pocc_hash;
      if (tokens.size() > 1) {
        pocc_hash = tokens.back();
      }
      if (!pocc_hash.empty()) {
        new_title += ":" + aurostd::fixStringLatex(pocc_hash, false, false);
      }
    }
    ...
    if (!module_arun.empty()) {
      ...
      if (!module_hash.empty()) {
        new_title += ":" + aurostd::fixStringLatex(module_hash, false, false);
      }
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " new_title=" << new_title << endl;
    }
...
static const string LM_ORBITALS_LATEX[16] = {"s",       "p_y",      "p_z",     "p_x",      "d_{xy}",         "d_{yz}",         "d_{z^2}", "d_{xz}", "d_{x^2-y^2}", "f_{y(3x^2-y^2)}",
                                             "f_{xyz}", "f_{yz^2}", "f_{z^3}", "f_{xz^2}", "f_{z(x^2-y^2)}", "f_{x(x^2-3y^2)}"};
// AS20201110 as used by E.G. in BANDSDATA_JSON procedure:
static const string LM_ORBITALS[16] = {"s", "py", "pz", "px", "dxy", "dyz", "dz2", "dxz", "dx2-y2", "f1", "f2", "f3", "f4", "f5", "f6", "f7"};
static const string SPIN_LABEL[2] = {"majority", "minority"};// AS20201102
static const string LS_ORBITALS[16] = {"s_total", "sx", "sy", "sz", "p_total", "px", "py", "pz", "d_total", "dx", "dy", "dz", "f_total", "fx", "fy", "fz"};// AS20201105
...
namespace plotter {
  ...
  void PLOT_DOS(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_DOS(plotoptions, FileMESSAGE, oss);
  } // CO20200404
  ...
  void PLOT_DOS(xoption& plotoptions, const xDOSCAR& xdos, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_DOS(plotoptions, xdos, FileMESSAGE, oss);
  } // CO20191110  //CO20200404
  ...
  void PLOT_DOS(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_DOS(plotoptions, out, FileMESSAGE, oss);
  } // CO20191110 //CO20200404
  void PLOT_DOS(xoption& plotoptions, stringstream& out, ofstream& FileMESSAGE, ostream& oss) { // CO20191110  //CO20200404
    ...
    vector<string> vpocc_doscars;
    aurostd::DirectoryLS(directory, vfiles);
    uint i = 0;
    for (i = 0; i < vfiles.size(); i++) {
      if (vfiles[i].find(POCC_DOSCAR_PREFIX) != string::npos) {
        vpocc_doscars.push_back(aurostd::CleanFileName(directory + "/" + vfiles[i]));
      }
    }
    if (!vpocc_doscars.empty()) {
      std::sort(vpocc_doscars.begin(), vpocc_doscars.end());
      const string pscheme = plotoptions.getattachedscheme("PROJECTION");
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " pscheme=" << pscheme << endl;
      }
      if (pscheme.empty()) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No projection scheme provided", _INPUT_MISSING_);
      }
      ...
      int temperature_precision = 0;
      int zero_padding_temperature = 0;
      ...
      if (aurostd::FileExist(aflags.Directory + "/" + _AFLOWIN_) && pocc::structuresGenerated(aflags.Directory)) {
        ...
        pcalc.setTemperatureStringParameters(v_temperatures); // needed for DOSCAR plots
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " found POCC directory" << endl;
      }
      for (i = 0; i < vpocc_doscars.size(); i++) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " looking at POCC DOSCAR: " << vpocc_doscars[i] << endl;
        }
  ...
  void patchDefaultTitleAFLOWIN(xoption& plotoptions) { // CO20191110
    ...
    const string& directory = plotoptions.getattachedscheme("DIRECTORY");
    if (!aurostd::FileExist(directory + "/" + _AFLOWIN_)) {
      return;  // ME20200922
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " directory=" << directory << endl;
    }
    const string SYSTEM = KBIN::ExtractSystemName(directory); // CO20200731
    if (!SYSTEM.empty()) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " DEFAULT_TITLE(OLD)=" << plotoptions.getattachedscheme("DEFAULT_TITLE") << endl;
      }
      plotoptions.pop_attached("DEFAULT_TITLE");
      plotoptions.push_attached("DEFAULT_TITLE", SYSTEM);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " DEFAULT_TITLE(NEW)=" << plotoptions.getattachedscheme("DEFAULT_TITLE") << endl;
      }
    }
  }

  void PLOT_DOS(xoption& plotoptions, stringstream& out, const xDOSCAR& xdos, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_DOS(plotoptions, out, xdos, FileMESSAGE, oss);
  }  // CO20200404
  void PLOT_DOS(xoption& plotoptions, stringstream& out, const xDOSCAR& xdos, ofstream& FileMESSAGE, ostream& oss) {  // CO20200404
    const bool LDEBUG = (false || _DEBUG_PLOTTER_ || XHOST.DEBUG);
    const string extension = plotoptions.getattachedscheme("EXTENSION");
    if (extension.empty()) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " EFERMI set" << endl;
    }
  ...
  void PLOT_PDOS(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PDOS(plotoptions, FileMESSAGE, oss);
  } // CO20200404  //CO20200404
  ...
  void PLOT_PDOS(xoption& plotoptions, const xDOSCAR& xdos, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PDOS(plotoptions, xdos, FileMESSAGE, oss);
  }  // CO20191110  //CO20200404
  ...
  void PLOT_PDOS(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PDOS(plotoptions, out, FileMESSAGE, oss);
  }  // CO20191110  //CO20200404
  ...
  void PLOT_PDOS(xoption& plotoptions, stringstream& out, const xDOSCAR& xdos, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PDOS(plotoptions, out, xdos, FileMESSAGE, oss);
  } // CO20200404
  void PLOT_PDOS(xoption& plotoptions, stringstream& out, const xDOSCAR& xdos, ofstream& FileMESSAGE, ostream& oss) { // CO20200404
    const string set = plotoptions.getattachedscheme("DATASET");
    const string datatype = plotoptions.getattachedscheme("DATATYPE");  // CO20191010 - which table to look at, "atoms-projected" by default
    const string outformat = plotoptions.getattachedscheme("OUTPUT_FORMAT");
    const bool plot_all_atoms = plotoptions.flag("PLOT_ALL_ATOMS"); // CO20191010
    int pdos = -1;
    if (!set.empty()) {
      pdos = aurostd::string2utype<int>(set);
    }
    ...
    if (pdos == 0) {  // Plot total DOS
      ...
      if (datatype == "SPECIES") {  // CO20191010 - plot "species-projected"
        if (pdos == -1) {  // Plot partial DOS of all species
          const uint nspecies = xstr.num_each_type.size();
  ...
  void PLOT_BAND(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_BAND(plotoptions, FileMESSAGE, oss);
  }  // CO20200404
  ...
  void PLOT_BAND(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_BAND(plotoptions, out, FileMESSAGE, oss);
  }  // CO20200404
  ...
  void PLOT_BANDDOS(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_BANDDOS(plotoptions, FileMESSAGE, oss);
  } // CO20200404
  ...
  void PLOT_BANDDOS(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_BANDDOS(plotoptions, out, FileMESSAGE, oss);
  }  // CO20200404
  ...
  xstructure getStructureWithNames(const xoption& plotoptions, const string& carstring, ostream& oss) {
    ofstream FileMESSAGE;
    return getStructureWithNames(plotoptions, FileMESSAGE, carstring, oss);
  } // CO20200404
  xstructure getStructureWithNames(const xoption& plotoptions, ofstream& FileMESSAGE, const string& carstring, ostream& oss) { // CO20200404
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " directory(input)=" << directory << endl;
    }
    string poscar_file = "POSCAR"; // CO20200404
    std::stringstream poscar;
    // if (plotoptions.getattachedscheme("EXTENSION") == "phdos")
    if (carstring == "PHON") { // CO20200106 - patching for auto-indenting
      poscar_file = DEFAULT_APL_PHPOSCAR_FILE; // CO20200404
      if (aurostd::CompressFileExist(directory + "/" + poscar_file, poscar_file)) {
        aurostd::compressfile2stringstream(poscar_file, poscar);
      }  // CO20200404
    } else {
      if (carstring == "POCC") { // CO20191110 //CO20200404
        //[do NOT load in PARTCAR, we need an example ARUN POSCAR, they all have the same num_each_type]aurostd::compressfile2stringstream(directory+"/PARTCAR", poscar);
        string arun;
        arun = plotoptions.getattachedscheme("ARUN_DIRECTORY");  // relative path
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " grabbing POSCAR from " << arun << endl;
        }
        directory += "/" + arun;
        if (!aurostd::FileExist(directory)) {   // SG20240131
          aurostd::StringSubstInPlace(directory, "/RAW/", "/LIB/");  // read POSCAR from LIB, not RAW (may not have been processed yet)
          if (!aurostd::FileExist(directory)) { // this will happen if --lib2raw=[dir] is done with --local
            aurostd::StringSubstInPlace(directory, "/LIB/", "/");
            if (!aurostd::FileExist(directory)) {
              const string message = "Cannot find directory to read POSCAR from: " + directory;
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " directory(POSCAR)=" << directory << endl;
      }
      aflowlib::vaspfile2stringstream(directory + "", poscar_file, poscar);  // CO20200404
    }

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " poscar=" << endl << poscar.str() << endl;
    }
    ...
    if (xstr.is_vasp4_poscar_format) {  // PARTCAR has species and it is NOT vasp4 format  //CO20191110
      ...
      for (size_t i = 0; i < atom_names.size() && not_all_names_given == false; i++) { // CO20200404
        if (atom_names[i].empty()) {
          not_all_names_given = true;
        }
      }
      if (not_all_names_given) { // CO20200404
        stringstream message;
        message << "Species CANNOT be extracted from dir=" << directory << ", no labels can be supplied";
        pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, FileMESSAGE, oss, _LOGGER_WARNING_);  // CO20200404
      } else { // CO20200404
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " patching names of POSCAR with: " << aurostd::joinWDelimiter(atom_names, ", ") << endl;
        }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " final xstr=" << endl;
      cerr << xstr << endl;
    }  // CO20200404
    return xstr;
  }
  ...
  string getLatticeFromKpointsTitle(const string& title) {
    ...
    if (tokens.size() >= 3) {  // AFLOW-formatted KPOINTS titles have at least three columns
      if (tokens[0] == "CUB") {
        return tokens[0];
      } else if (tokens[0] == "FCC") {
        return tokens[0];
      } else if (tokens[0] == "BCC") {
        return tokens[0];
      } else if (tokens[0] == "TET") {
        return tokens[0];
      } else if (tokens[0] == "BCT") {
        return tokens[0];
      } else if (tokens[0] == "BCT1") {
        return tokens[0];
      } else if (tokens[0] == "BCT2") {
        return tokens[0];
      } else if (tokens[0] == "ORC") {
        return tokens[0];
      } else if (tokens[0] == "ORCF") {
        return tokens[0];
      } else if (tokens[0] == "ORCF1") {
        return tokens[0];
      } else if (tokens[0] == "ORCF2") {
        return tokens[0];
      } else if (tokens[0] == "ORCF3") {
        return tokens[0];
      } else if (tokens[0] == "ORCI") {
        return tokens[0];
      } else if (tokens[0] == "ORCC") {
        return tokens[0];
      } else if (tokens[0] == "HEX") {
        return tokens[0];
      } else if (tokens[0] == "RHL") {
        return tokens[0];
      } else if (tokens[0] == "RHL1") {
        return tokens[0];
      } else if (tokens[0] == "RHL2") {
        return tokens[0];
      } else if (tokens[0] == "MCL") {
        return tokens[0];
      } else if (tokens[0] == "MCLC") {
        return tokens[0];
      } else if (tokens[0] == "MCLC1") {
        return tokens[0];
      } else if (tokens[0] == "MCLC2") {
        return tokens[0];
      } else if (tokens[0] == "MCLC3") {
        return tokens[0];
      } else if (tokens[0] == "MCLC4") {
        return tokens[0];
      } else if (tokens[0] == "MCLC5") {
        return tokens[0];
      } else if (tokens[0] == "TRI") {
        return tokens[0];
      } else if (aurostd::toupper(tokens[0]) == "TRI1A") {
        return tokens[0];
      } else if (aurostd::toupper(tokens[0]) == "TRI1B") {
        return tokens[0];
      } else if (aurostd::toupper(tokens[0]) == "TRI2A") {
        return tokens[0];
      } else if (aurostd::toupper(tokens[0]) == "TRI2B") {
        return tokens[0];
      } else {
        return "";
      }
...
#define BANDS_DOS_JSON_VERSION 1.0
  ...
  /// @brief Converts DOS data from xDOSCAR to the file in json format.
  ///
  ...
  aurostd::JSON::object DOS2JSON(xoption& xopt, const xDOSCAR& xdos, ofstream& FileMESSAGE, ostream& oss) {
    ...
    const string name = KBIN::ExtractSystemName(directory);
    aurostd::JSON::object dos_json(aurostd::JSON::object_types::DICTIONARY);
    // TDOS header begin
    dos_json["version"] = BANDS_DOS_JSON_VERSION;
    dos_json["name"] = name;
    dos_json["species"] = xstr.species;
    dos_json["composition"] = xstr.num_each_type;
    dos_json["Emin"] = xdos.energy_min;
    dos_json["Emax"] = xdos.energy_max;
    dos_json["Efermi"] = xdos.Efermi;
    dos_json["DOS_grid"] = xdos.number_energies;
    // TDOS header end
    dos_json["tDOS_data"] = aurostd::JSON::object(aurostd::JSON::object_types::DICTIONARY);
    dos_json["tDOS_data"]["energies_shifted"] = !xopt.flag("NOSHIFT");
    dos_json["tDOS_data"]["energy"] = xopt.flag("NOSHIFT") ? xdos.venergy : xdos.venergyEf;
    if (aurostd::substring2bool(xdos.carstring, "CAR")) {
      dos_json["tDOS_data"]["x_unit"] = "EV";
      dos_json["tDOS_data"]["y_unit"] = "";
    } else if (aurostd::substring2bool(xdos.carstring, "PHON")) {
      dos_json["tDOS_data"]["x_unit"] = "EV";  // ME20211014 - units are always eV
      dos_json["tDOS_data"]["y_unit"] = "";
    }

    if (xdos.spin) {
      dos_json["tDOS_data"]["spin_majority"] = xdos.vDOS[0][0][0];
      // negative for minority spin
      deque<double> minority = xdos.vDOS[0][0][1];
      for (double& i : minority) {
        i *= -1;
      }
      dos_json["tDOS_data"]["spin_minority"] = minority;

      dos_json["tDOS_data"]["sum_spin_majority"] = xdos.viDOS[0];
      // negative for minority spin
      minority = xdos.viDOS[1];
      for (double& i : minority) {
        i *= -1;
      }
      dos_json["tDOS_data"]["sum_spin_minority"] = minority;
    } else {
      dos_json["tDOS_data"]["tDOS"] = xdos.vDOS[0][0][0];
      dos_json["tDOS_data"]["sum"] = xdos.viDOS[0];
    }
    ...
    if (xdos.partial && aurostd::substring2bool(xdos.carstring, "CAR")) {
      ...
      if (xdos.isLSCOUPLING) {
        for (uint i = 0; i < 16; i++) {
          orb_labels[i] = LS_ORBITALS[i];
        }
      } else if (xdos.lmResolved) {
        for (uint i = 0; i < 16; i++) {
          orb_labels[i] = LM_ORBITALS[i];
        }
      } else {
        for (uint i = 0; i < 4; i++) {
          orb_labels[i] = ORBITALS[i];
        }
      }
      ...
      for (size_t i = 0; i < orb_labels_out.size(); i++) {
        orb_labels_out[i] = ORBITALS[i];
      }

      // header of partial DOS
      dos_json["pDOS_data"] = aurostd::JSON::object(aurostd::JSON::object_types::DICTIONARY);
      dos_json["pDOS_data"]["orbitals"] = orb_labels_out;
      dos_json["pDOS_data"]["spin_polarized"] = xdos.spin;
      dos_json["pDOS_data"]["energies_shifted"] = !xopt.flag("NOSHIFT");
      dos_json["pDOS_data"]["energy"] = xopt.flag("NOSHIFT") ? xdos.venergy : xdos.venergyEf;
      dos_json["pDOS_data"]["x_unit"] = "EV";
      dos_json["pDOS_data"]["y_unit"] = "";
      ...
      for (size_t species_id = 0; species_id < xstr.species.size(); species_id++) {
        for (size_t iatom = 0; iatom < xstr.iatoms.size(); iatom++) {
      ...
      for (uint species_id = 0; species_id < xstr.species.size(); species_id++) {
        ...
        for (uint iatom = 0; iatom < map_species_to_iatoms[species_id].size(); iatom++) {
          ...
          if (xdos.isLSCOUPLING) {
            for (size_t orb = 1; orb < xdos.vDOS[atom_id].size(); orb++) {
              // there are 4 spin channels
              for (size_t spin = 0; spin < xdos.vDOS[atom_id][orb].size(); spin++) {
                label = orb_labels[4 * (orb - 1) + spin];
                atom_json[label] = xdos.vDOS[atom_id][orb][spin];
              }
            }

            atom_json["total"] = xdos.vDOS[atom_id][0][0];
          } else {
            for (uint spin = 0; spin <= xdos.spin; spin++) {
              if (!xdos.vDOS[atom_id].empty()) {
                label = "total" + (xdos.spin ? "_" + SPIN_LABEL[spin] : "");
                if (spin) {
                  // negative for minority spin
                  deque<double> minority = xdos.vDOS[atom_id][0][spin];
                  for (uint i = 0; i < minority.size(); i++) {
                    minority[i] *= -1;
                  }
                  atom_json[label] = minority;
                } else {
                  atom_json[label] = xdos.vDOS[atom_id][0][spin];
                }
              }
          ...
          dos_json["pDOS_data"][xstr.species[species_id]][aurostd::utype2string<int>(atom_id - 1)] = atom_json;
        }
      }
      ...
      if (xdos.lmResolved) {
        ...
        for (int l = 0, norb = 1; norb <= (int) xdos.vDOS[0].size() - 1; l++, norb += (2 * l + 1)) {
          for (uint spin = 0; spin <= xdos.spin; spin++) {
            for (size_t en = 0; en < orb_dos[spin].size(); en++) {
              orb_dos[spin][en] = 0.0;
            }
          }
          ...
          for (uint spin = 0; spin <= xdos.spin; spin++) {
            ...
            dos_json["pDOS_data"][label] = orb_dos[spin];
          }
        }
      } else {
        for (size_t orb = 1; orb < xdos.vDOS[0].size(); orb++) {
          for (uint spin = 0; spin <= xdos.spin; spin++) {
            if (xdos.isLSCOUPLING) {
            ...
            if (spin) {
              ...
              dos_json["pDOS_data"][label] = minority;
            } else {
              dos_json["pDOS_data"][label] = xdos.vDOS[0][orb][spin];
            }
          }
        }
      }
    ...
    if (xdos.partial && aurostd::substring2bool(xdos.carstring, "PHON")) {
      ...
      if (xdos.vDOS.size() >= 2) {
        for (size_t j = 1; j < xdos.vDOS[1].size(); j++) {
          projections.push_back("projection_" + aurostd::utype2string(j));
        }
      }
      dos_json["tDOS_data"]["orbitals"] = projections;
      dos_json["tDOS_data"]["spin_polarized"] = xdos.spin;
      dos_json["tDOS_data"]["energies_shifted"] = !xopt.flag("NOSHIFT");
      dos_json["tDOS_data"]["energy"] = xopt.flag("NOSHIFT") ? xdos.venergy : xdos.venergyEf;
      dos_json["tDOS_data"]["x_unit"] = "EV";  // ME20211014 - units are always eV
      dos_json["tDOS_data"]["y_unit"] = "";
      ...
      for (uint species_id = 0; species_id < xstr.species.size(); species_id++) {
        ...
        for (uint iatom = 0; iatom < map_species_to_iatoms[species_id].size(); iatom++) {
          ...
          dos_json["tDOS_data"][xstr.species[species_id]][aurostd::utype2string<int>(atom_id - 1)] = atom_json;
        }
      }
    }
  ...
  aurostd::JSON::object bands2JSON(const xEIGENVAL& xeigen, const xKPOINTS& xkpts, const vector<double>& distances, const vector<double>& segment_points, const xoption& plotoptions) {
    ...
    const string LattName = tokens[0];

    // write header
    aurostd::JSON::object json(aurostd::JSON::object_types::DICTIONARY);
    json["title"] = name + " (" + LattName + ")";
    json["n_kpoints"] = xeigen.number_kpoints;
    json["n_bands"] = xeigen.number_bands;
    if (aurostd::substring2bool(xeigen.carstring, "CAR")) {
      json["x_unit"] = "";
      json["y_unit"] = "EV";
    } else if (aurostd::substring2bool(xeigen.carstring, "PHON")) {
      json["x_unit"] = "";
      json["y_unit"] = "EV";  // ME20211014 - units are always eV
    }

    static const uint num = 4;
    const string tags[num] = {"kpoint_labels", "kpoint_labels_latex", "kpoint_labels_gnuplot", "kpoint_labels_html"};
    const string formats[num] = {"", "LATEX", "GNUPLOT", "HTML"};

    // we need clean labels: to be consistent with E.G. format of the JSON file
    // they will be converted into all possible formats
    const uint nsegments = xkpts.vpath.size() / 2;
    ...
    for (uint f = 0; f < num; f++) {
      ...
      for (size_t j = 0; j < labels_formated.size(); j++) {
        aurostd::StringSubstInPlace(labels_formated[j], "\\", "\\\\");
      }

      json[tags[f]] = labels_formated;
    }

    json["kpoint_positions"] = segment_points;
    json["energies_shifted"] = !plotoptions.flag("NOSHIFT");

    // write bands data
    string bandslabel;
    const double Efermi = aurostd::string2utype<double>(plotoptions.getattachedscheme("EFERMI"));
  ...
  /// to JSON object
  aurostd::JSON::object bandsDOS2JSON(const xDOSCAR& xdos, const xEIGENVAL& xeigen, const xKPOINTS& xkpts, xoption& xopt, ofstream& FileMESSAGE, ostream& oss) {
    // get DOS part of JSON
    aurostd::JSON::object json = DOS2JSON(xopt, xdos, FileMESSAGE, oss);

    // get BANDS part of JSON
    const xstructure xstr = getStructureWithNames(xopt, FileMESSAGE, xdos.carstring, oss);
    ...
    aurostd::JSON::object json_bands(aurostd::JSON::object_types::DICTIONARY);
    generateBandPlot(json_stream, json_bands, xeigen, xkpts, xstr, xopt);
    const string bands = json_stream.str();
    json.join(json_bands);
    return json;
  }
  ...
  void generateDosPlot(stringstream& out, const xDOSCAR& xdos, xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return generateDosPlot(out, xdos, plotoptions, FileMESSAGE, oss);
  } // CO20200404
  void generateDosPlot(stringstream& out, const xDOSCAR& xdos, xoption& plotoptions, ofstream& FileMESSAGE, ostream& oss) {  // CO20200404
    ...
    const string dataset = plotoptions.getattachedscheme("DATASET");
    const int pdos = aurostd::string2utype<int>(dataset);
    vector<string> labels;
    labels.emplace_back("total");  // There is always a total DOS
    if (pdos > 0) {
      labels.front() += " " + plotoptions.getattachedscheme("DATALABEL");
    } // CO20191010
    const string projection = plotoptions.getattachedscheme("PROJECTION");
    const string datatype = plotoptions.getattachedscheme("DATATYPE");  // CO20191010 - which table to look at, "atoms-projected" by default
    if (LDEBUG) {
    ...
    if (projection == "ORBITALS") {
      ...
      int norbitals = 0;
      if (datatype == "SPECIES") {  // CO20191010 - plot "species-projected"
        const xstructure xstr = getStructureWithNames(plotoptions, FileMESSAGE, xdos.carstring, oss);  // CO20200404
        deque<deque<deque<deque<double>>>> vDOS_species = xdos.GetVDOSSpecies(xstr);
        if (!dosDataAvailable(vDOS_species, pdos)) {
          const string message = "DOS data not available for pdos = " + aurostd::utype2string<int>(pdos);
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
        }
        ...
        if (xdos.lmResolved) {
          norbitals = (int) std::sqrt(xdos.vDOS[pdos].size());
        }  // size is either 17 or 10
        else {
          norbitals = (int) xdos.vDOS[pdos].size() - 1;
        }
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " norbitals=" << norbitals << endl;
        }
        if (xdos.lmResolved) {
      ...
      // Safety check
      if (!xdos.lmResolved) {
        const string message = "Projection scheme LM chosen, but DOSCAR is not lm-resolved.";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
      }
      if (!dosDataAvailable(xdos.vDOS, pdos)) {
        const string message = "DOS data not available for pdos = " + aurostd::utype2string<int>(pdos);
      ...
      for (size_t i = 1; i < xdos.vDOS[pdos].size(); i++) {
      ...
      if (!dosDataAvailable(xdos.vDOS, 0)) {
        const string message = "Total DOS not available";
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
      }
      ...
      if (plotoptions.flag("PLOT_ALL_ATOMS")) { // CO20191010 - special mode - ALL atoms
        if (xdos.vDOS.size() > 1) {
          for (size_t i = 0; i < xstr.atoms.size(); i++) {
            if (!dosDataAvailable(xdos.vDOS, i + 1)) {
              const string message = "DOS data not available for atom " + aurostd::utype2string<uint>(i + 1);
              throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
            }
        ...
        if (pdos == 0) {
          if (xdos.vDOS.size() > 1) {
            pflow::PerformFullSymmetry(xstr);
            int iat = 0;
            for (size_t i = 0; i < xstr.iatoms.size(); i++) {
              iat = xstr.iatoms[i][0];
              if (!dosDataAvailable(xdos.vDOS, iat + 1)) {
                const string message = "DOS data not available for iatom " + aurostd::utype2string<int>(iat + 1);
                throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
              }
      ...
      if (pdos == 0) {
        ...
        if (vDOS_species.size() > 1) {
          for (size_t i = 0; i < xstr.species.size(); i++) {
            if (!dosDataAvailable(vDOS_species, i + 1)) {
              const string message = "DOS data not available for species " + aurostd::utype2string<uint>(i + 1);
              throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
            }
        ...
        const int dos_index = xstr.atoms[pdos - 1].type + 1;
        if (!dosDataAvailable(vDOS_species, dos_index)) {
          const string message = "DOS data not available for dos_index = " + aurostd::utype2string<int>(dos_index);
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
        }
      ...
      const string message = "Unknown projection scheme " + projection + ".";
  ...
  void generateBandPlot(stringstream& out, const xEIGENVAL& xeigen, const xKPOINTS& xkpts, const xstructure& xstr, const xoption& plotoptions) {
    aurostd::JSON::object json(aurostd::JSON::object_types::DICTIONARY);
    generateBandPlot(out, json, xeigen, xkpts, xstr, plotoptions);
  }

  void generateBandPlot(stringstream& out, aurostd::JSON::object& json, const xEIGENVAL& xeigen, const xKPOINTS& xkpts, const xstructure& xstr, const xoption& plotoptions) {
    // Create segments
    const uint nsegments = xkpts.vpath.size() / 2;
    // Make sure that the number of k-points is consistent with EIGENVAL
    if (xeigen.number_kpoints != nsegments * xkpts.path_grid) {
      const string message = "Number of k-points in EIGENVAL and KPOINTS files do not match.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _RUNTIME_ERROR_);
    }

    // Labels
    const string format = plotoptions.getattachedscheme("KPOINT_FORMAT");
    ...
    if (outformat == "GNUPLOT") {
      ...
      json = bands2JSON(xeigen, xkpts, distances, segment_points, plotoptions).toString();
      out << json.toString();
    }
  }
  ...
  string convertKPointLetter(string letter, const string& format) {
    if (format == "LATEX") {
      ...
      if (aurostd::substring2bool(letter, "\\")) {
        aurostd::StringSubstInPlace(letter, "\\", "&");
        letter += ";";
      }
    }
    // AS20201103 BEGIN
  ...
  void generateDosPlotGNUPLOT(stringstream& out, const xDOSCAR& xdos, const deque<double>& energies, const deque<deque<deque<double>>>& dos, const vector<string>& labels, const xoption& plotoptions) {
    const bool LDEBUG = (false || _DEBUG_PLOTTER_ || XHOST.DEBUG);
    // Initialize variables
    const double Efermi = aurostd::string2utype<double>(plotoptions.getattachedscheme("EFERMI"));
    const double Emin = aurostd::string2utype<double>(plotoptions.getattachedscheme("XMIN"));
    const double Emax = aurostd::string2utype<double>(plotoptions.getattachedscheme("XMAX"));
    const bool banddos = plotoptions.flag("BANDDOS");
    const bool swap = (banddos || plotoptions.flag("SWAP_AXES"));
    const uint ndos = dos.size();
    ...
    if (aurostd::isequal(dosmax, 0.0)) {
      dosmax = 1.0;
    } // CO+ME20210729 - issues with LIB0 pDOS plots, might need to rerun with NEDOS=5000, EMIN=-45, EMAX=30
    const string maxdos = aurostd::utype2string<double>(dosmax);
    string mindos;
    if (xdos.spin == 0) {
      mindos = "0";
    } else {
    ...
    string energyLabel;
    if (aurostd::substring2bool(unit, "EV")) {
      energyLabel = "energy";
    } else {
    ...
    if (dos.size() == xdos.spin + 1) { // no need for key when only total DOS is plotted, NO-SPIN: xdos.spin==0, SPIN: xdos.spin==1  //CO20200404
      ...
      if (plotoptions.flag("LEGEND_HORIZONTAL")) {  // CO20200404
        int maxcols = 3;
        const string maxcols_str = plotoptions.getattachedscheme("LEGEND_MAXCOLS");
        if (!maxcols_str.empty()) {
          maxcols = aurostd::string2utype<int>(maxcols_str);
        }
    ...
    const string normalization = aurostd::tolower(plotoptions.getattachedscheme("NORMALIZATION"));
    if (banddos) {
    ...
    for (uint i = 0; i < ndos; i++) {
      ...
      out << "'$dos_data' u " << xcol << ":" << ycol << " w l lt -1 "
          << "lc rgb '" << ESTRUCTURE_COLORS[i % ESTRUCTURE_NCOLORS] << "' lw 2 title '" << labels[i] << "'" << (((xdos.spin == 1) || (i < ndos - 1)) ? ",\\" : "") << std::endl;
    }
    // Minority spin
    if (xdos.spin == 1) {
      for (uint i = 0; i < ndos; i++) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " gnuplot file:" << endl << out.str() << endl;
    }
  }
  ...
  //  plots.
  double getDosLimits(const xoption& plotoptions, const xDOSCAR& xdos, const deque<deque<deque<double>>>& dos, const deque<double>& energies) {
    const bool LDEBUG = (false || _DEBUG_PLOTTER_ || XHOST.DEBUG);

    const double Emin = aurostd::string2utype<double>(plotoptions.getattachedscheme("XMIN"));
    const double Emax = aurostd::string2utype<double>(plotoptions.getattachedscheme("XMAX"));
    const string dosscale = plotoptions.getattachedscheme("YSCALE");
    const uint ndos = dos.size(); // orbital
    ...
    int e2 = -1;
    for (uint e = 0; e < xdos.number_energies; e++) {
      if ((e1 < 0) && (energies[e] >= Emin)) {
        e1 = (int) e;
      }
      if ((e2 < 0) && (energies[e] > Emax)) {
        e2 = (int) e;
      }
      if ((e1 > 0) && (e2 > 0)) {
        break;
    ...
    if (e2 < 0) {
      e2 = (int) xdos.number_energies; // Emax not found
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " dosmax(FOUND)=" << dosmax << endl;
    }

    if (!dosscale.empty()) {
      dosmax *= aurostd::string2utype<double>(dosscale);
    }

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " dosmax(SCALED)=" << dosmax << endl;
    }

    // l = order of magnitude
    // x = scalar multiple
    // Now round up the numbers to give good tic values.
    const int l = (int) log10(dosmax);
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " x(new2)=" << x << endl;
    }
  ...
  void generateBandPlotGNUPLOT(stringstream& out, const xEIGENVAL& xeigen, const vector<double>& xvals, const vector<double>& ticvals, const vector<string>& ticlabels, const xoption& plotoptions) {
    const bool LDEBUG = (false || _DEBUG_PLOTTER_ || XHOST.DEBUG);
    ...
    const uint kpts_per_segment = xeigen.number_kpoints / (ticvals.size() - 1);
    // Create data block
    ...
    if (Efermi > Emin) {
      out << std::endl << "# Fermi level" << std::endl;
      out << "set arrow from 0, " << Efermi << " to graph 1, first " << Efermi << " nohead lt 1 lc rgb '" << EFERMI_COLOR << "' lw 3" << std::endl;
    }
    ...
    for (uint b = 0; b < xeigen.number_bands; b++) {
      ...
      out << "'$band_data' u 1:" << (b + 2) << " w l lt -1 lc rgb '" << ISPIN_COLORS[0] << "' lw 2" << (((xeigen.spin == 1) || (b < xeigen.number_bands - 1)) ? ",\\" : "") << std::endl;
    }
    ...
    if (xeigen.spin == 1) {
      for (uint b = 0; b < xeigen.number_bands; b++) {
        out << "      ";
        out << "'$band_data' u 1:" << (b + xeigen.number_bands + 2) << " w l lt 1 lc rgb '" << ISPIN_COLORS[1] << "' lw 2" << ((b < xeigen.number_bands - 1) ? ",\\" : "") << std::endl;
      }
    }
  }
...
namespace plotter {
  ...
  // PLOT_PHDOS////////////////////////////////////////////////////////////////
  //  Plots phonon DOS.
  void PLOT_PHDOS(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PHDOS(plotoptions, FileMESSAGE, oss);
  } // CO20200404
  ...
  void PLOT_PHDOS(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PHDOS(plotoptions, out, FileMESSAGE, oss);
  }  // CO20200404
  void PLOT_PHDOS(xoption& plotoptions, stringstream& out, ofstream& FileMESSAGE, ostream& oss) {  // CO20200404
    ...
    for (size_t i = 0; i < vfiles.size(); i++) {
      ...
      if (vfiles[i].find(POCC_PHDOSCAR_PREFIX) == 0) {
        vpocc_doscars.push_back(vfiles[i]);
      }
    }
    xDOSCAR xdos;
    if (!vpocc_doscars.empty()) {
      ...
      for (size_t i = 0; i < vpocc_doscars.size(); i++) {
  ...
  // ME20210927
  void PLOT_PHDOS(xoption& plotoptions, const xDOSCAR& xdos, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PHDOS(plotoptions, xdos, FileMESSAGE, oss);
  }
  ...
  void PLOT_PHDOS(xoption& plotoptions, stringstream& out, xDOSCAR xdos, ofstream& FileMESSAGE, ostream& oss) {
    ...
    const string unit = plotoptions.getattachedscheme("UNIT");
  ...
  void PLOT_PHDISP(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PHDISP(plotoptions, FileMESSAGE, oss);
  }  // CO20200404
  ...
  void PLOT_PHDISP(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PHDISP(plotoptions, out, FileMESSAGE, oss);
  } // CO20200404
  void PLOT_PHDISP(xoption& plotoptions, stringstream& out, ofstream& FileMESSAGE, ostream& oss) { // CO20200404
    ...
    string poscar_file = DEFAULT_APL_PHPOSCAR_FILE;
    stringstream poscar;
    if (aurostd::CompressFileExist(directory + "/" + poscar_file, poscar_file)) {
      aurostd::compressfile2stringstream(poscar_file, poscar);
    }  // CO20191110
  ...
  void PLOT_PHDISPDOS(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PHDISPDOS(plotoptions, FileMESSAGE, oss);
  } // CO20200404
  ...
  void PLOT_PHDISPDOS(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_PHDISPDOS(plotoptions, out, FileMESSAGE, oss);
  }  // CO20200404
  ...
  void convertEnergies(xEIGENVAL& xeigen, const string& unit) {
    const double conversion_factor = getEnergyConversionFactor(unit);
    for (uint k = 0; k < xeigen.number_kpoints; k++) {
      for (uint b = 0; b < xeigen.number_bands; b++) {
        for (uint s = 0; s < xeigen.spin + 1; s++) {
...
namespace plotter {
  ...
  void PLOT_THERMO(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_THERMO(plotoptions, FileMESSAGE, oss);
  }  // CO20200404
  ...
  void PLOT_THERMO(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_THERMO(plotoptions, out, FileMESSAGE, oss);
  } // CO20200404
  void PLOT_THERMO(xoption& plotoptions, stringstream& out, ofstream& FileMESSAGE, ostream& oss) { // CO20200404
    ...
    const string ylabels[nprops] = {"U", "F", "S", "c_V"};
    const string extensions[nprops] = {"vib_internal_energy", "vib_free_energy", "vib_entropy", "apl_cV"};
    const string yunits[nprops] = {"meV/cell", "meV/cell", "$k_B$/cell", "$k_B$/cell"};
    const string ymin[nprops] = {"", "", "0", "0"};

    // Get data
    const string directory = plotoptions.getattachedscheme("DIRECTORY");
    const string thermo_file = directory + "/" + DEFAULT_APL_FILE_PREFIX + DEFAULT_APL_THERMO_FILE;
    // ME20200413 - Since multiple data files are plotted, the user file
    //  name functions as base file name.
    const string user_file_name = plotoptions.getattachedscheme("FILE_NAME_USER");
    plotoptions.pop_attached("FILE_NAME_USER");
    if (aurostd::CompressFileExist(thermo_file)) {
      const string outformat = plotoptions.getattachedscheme("OUTPUT_FORMAT");
      plotoptions.push_attached("DATA_FILE", thermo_file);
      plotoptions.push_attached("KEYWORD", "APL_THERMO");
      const vector<vector<double>> data = readAflowDataFile(plotoptions);
      if (!user_file_name.empty()) {
        plotoptions.push_attached("DEFAULT_TITLE", user_file_name);  // ME20200413
      }
      for (int i = 0; i < nprops; i++) {
        plotoptions.pop_attached("YMIN");
        if (!ymin[i].empty()) {
          plotoptions.push_attached("YMIN", ymin[i]);
        }
      ...
      const string message = "Could not find file " + thermo_file + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_NOT_FOUND_);
    }
  }
  ...
  void PLOT_THERMO_QHA(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    PLOT_THERMO_QHA(plotoptions, FileMESSAGE, oss);
  }  // CO20200404
  ...
  void PLOT_THERMO_QHA(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    PLOT_THERMO_QHA(plotoptions, out, FileMESSAGE, oss);
  } // CO20200404
  void PLOT_THERMO_QHA(xoption& plotoptions, stringstream& out, ofstream& FileMESSAGE, ostream& oss) {
    ...
    const string ylabels[nprops] = {"V", "F", "B", "\\beta", "c_V", "c_P", "\\gamma"};
    const string extensions[nprops] = {"volume_equilibrium_qha", "energy_free_qha", "modulus_bulk_qha", "thermal_expansion_qha", "cV_qha", "cP_qha", "gruneisen_parameter_qha"};
    const string yunits[nprops] = {"\\AA$^{3}$/atom", "eV/atom", "GPa", "$10^{-5}K^{-1}$", "$k_B$/atom", "$k_B$/atom", ""};
    const string ymin[nprops] = {"", "", "", "", "0", "0", ""};

    const string eos_model = plotoptions.getattachedscheme("EOSMODEL");
    if (eos_model != "SJ" && eos_model != "BM2" && eos_model != "BM3" && eos_model != "BM4" && eos_model != "M") {
    ...
    const string keyword = "QHA_" + eos_model + "_THERMO";

    // Get data
    const string directory = plotoptions.getattachedscheme("DIRECTORY");
    const string thermo_file = directory + "/" + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_THERMO_FILE;
    // ME20200413 - Since multiple data files are plotted, the user file
    //  name functions as base file name.
    const string user_file_name = plotoptions.getattachedscheme("FILE_NAME_USER");
    plotoptions.pop_attached("FILE_NAME_USER");
  ...
  void PLOT_GRUENEISEN_DISPERSION(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_GRUENEISEN_DISPERSION(plotoptions, FileMESSAGE, oss);
  }  // CO20200404
  ...
  void PLOT_GRUENEISEN_DISPERSION(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_GRUENEISEN_DISPERSION(plotoptions, out, FileMESSAGE, oss);
  } // CO20200404
  void PLOT_GRUENEISEN_DISPERSION(xoption& plotoptions, stringstream& out, ofstream& FileMESSAGE, ostream& oss) { // CO20200404
    ...
    const string directory = plotoptions.getattachedscheme("DIRECTORY");
    xEIGENVAL xeigen;
    xeigen.GetPropertiesFile(directory + "/" + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_GP_PATH_FILE);
    xKPOINTS xkpts;
    xkpts.GetPropertiesFile(directory + "/" + DEFAULT_QHA_FILE_PREFIX + DEFAULT_QHA_KPOINTS_FILE);
    string poscar_file = DEFAULT_APL_PHPOSCAR_FILE;
    stringstream poscar;
    if (aurostd::CompressFileExist(directory + "/" + poscar_file, poscar_file)) {
      aurostd::compressfile2stringstream(poscar_file, poscar);
    ...
    for (uint k = 0; k < xeigen.number_kpoints; k++) {
      for (uint b = 0; b < xeigen.number_bands; b++) {
        ...
        if (eigval < xeigen.energy_min) {
          xeigen.energy_min = eigval;
        }
        if (eigval > xeigen.energy_max) {
          xeigen.energy_max = eigval;
        }
      }
    }
  ...
  void PLOT_TCOND(xoption& plotoptions, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_TCOND(plotoptions, FileMESSAGE, oss);
  } // CO20200404
  ...
  void PLOT_TCOND(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return PLOT_TCOND(plotoptions, out, FileMESSAGE, oss);
  }  // CO20200404
  void PLOT_TCOND(xoption& plotoptions, stringstream& out, ofstream& FileMESSAGE, ostream& oss) {  // CO20200404
    ...
    const string directory = plotoptions.getattachedscheme("DIRECTORY");
    const string tcond_file = directory + "/" + DEFAULT_AAPL_FILE_PREFIX + DEFAULT_AAPL_TCOND_FILE;
    if (aurostd::CompressFileExist(tcond_file)) {
      ...
      const string message = "Could not find file " + tcond_file + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_NOT_FOUND_);
    }
  }
...
namespace plotter {
  ...
  static const string MATRIX_LABELS[9] = {"xx", "yx", "zx", "xy", "yy", "zy", "xz", "yz", "zz"};

  // plotSingleFromSet/////////////////////////////////////////////////////////
  //  Plots a single column from a dataset.
  void plotSingleFromSet(xoption& plotoptions, stringstream& out, const vector<vector<double>>& data_set, int col, ostream& oss) {
    ofstream FileMESSAGE;
    return plotSingleFromSet(plotoptions, out, data_set, col, FileMESSAGE, oss);
  } // CO20200404
  ...
  void plotMatrix(xoption& plotoptions, stringstream& out, ostream& oss) {
    ofstream FileMESSAGE;
    return plotMatrix(plotoptions, out, FileMESSAGE, oss);
  }  // CO20200404
  void plotMatrix(xoption& plotoptions, stringstream& out, ofstream& FileMESSAGE, ostream& oss) {  // CO20200404
    ...
    const bool contravariant = plotoptions.flag("CONTRAVARIANT");
    const string title_base = plotoptions.getattachedscheme("YLABEL");
    vector<string> titles(9);
    const vector<string> colors(9);
    const vector<int> point_styles(9);
    for (int i = 0; i < 9; i++) {
      if (contravariant) {
        titles[i] = "$" + title_base + "^{" + MATRIX_LABELS[i] + "}$";
      } else {
        titles[i] = "$" + title_base + "_{" + MATRIX_LABELS[i] + "}$";
      }
    }
  ...
  void setPlotLabels(xoption& plotoptions, const string& xlabel, const string& xunit, const string& ylabel, const string& yunit) {
  ...
  vector<vector<double>> readAflowDataFile(xoption& plotoptions) {
    ...
    const string keyword = plotoptions.getattachedscheme("KEYWORD");
    const string path_to_file = plotoptions.getattachedscheme("DATA_FILE");
    const string startstring = "[" + keyword + "]START";
    const string stopstring = "[" + keyword + "]STOP";
    const string systemstring = "[" + keyword + "]SYSTEM=";
    aurostd::compressfile2vectorstring(path_to_file, vcontent);
    string line;
    const uint nlines = vcontent.size();
    for (uint iline = 0; iline < nlines; iline++) {
      ...
      if (aurostd::substring2bool(line, systemstring)) {
        ...
        if (tokens.size() == 2) {
          plotoptions.push_attached("DEFAULT_TITLE", tokens.back());
        }
      }
  ...
  void generatePlotGNUPLOT(stringstream& out, const xoption& plotoptions, const vector<vector<double>>& data) {
    const bool LDEBUG = (false || _DEBUG_PLOTTER_ || XHOST.DEBUG);
    const uint ndata = data[0].size() - 1;

    // Axes settings
    const string xmax = plotoptions.getattachedscheme("XMAX");
    const string xmin = plotoptions.getattachedscheme("XMIN");
    const string ymax = plotoptions.getattachedscheme("YMAX");
    const string ymin = plotoptions.getattachedscheme("YMIN");
    const string xlabel = plotoptions.getattachedscheme("XLABEL");
    string xunit = plotoptions.getattachedscheme("XUNIT");
    if (!xunit.empty()) {
      xunit = " (" + xunit + ")";
    }
    const string ylabel = plotoptions.getattachedscheme("YLABEL");
    ...
    const bool linespoints = plotoptions.flag("LINESPOINTS");
    const bool points = linespoints || plotoptions.flag("NOLINES");
    const bool lines = (linespoints || !points);
    string plotstyle;
    if (lines) {
      plotstyle += "l";
    }
    ...
    vector<int> point_styles;
    vector<int> line_types;
    ...
    if ((ndata > 1) && (colors.size() == 1)) {
      colors.assign(ndata, colors[0]);
    }
    if ((ndata > 1) && (point_styles.size() == 1)) {
      point_styles.assign(ndata, point_styles[0]);
    }
    if ((ndata > 1) && (line_types.size() == 1)) {
      line_types.assign(ndata, line_types[0]);
    }
    const bool colors_set = (colors.size() == ndata);
    const bool points_set = ((linespoints || points) && (point_styles.size() == ndata));
    const bool lines_set = (lines && (line_types.size() == ndata));
    ...
    if (titles.size() < ndata) {  // Fill up with empty titles if there aren't enough
      for (size_t i = titles.size(); i < ndata; i++) {
        titles.emplace_back("");
      }
    }
