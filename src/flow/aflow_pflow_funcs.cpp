...
// (see Warren,eq.3.24).
double DebyeWallerFactor(double theta, double temp, double debye_temp, double mass, double lambda) {
  const double st = sin(theta);
  const double h = PLANCKSCONSTANT_h; // ME20181020
  const double twoB = h * h * temp * 12.0 / (mass * KBOLTZ * debye_temp * debye_temp);
  const double twoM = twoB * st * st / (lambda * lambda);
  const double DWfact = exp(-1.0 * twoM / 2.0); // Use e^-M, not e^-2M.
  return DWfact;
}
...
string getGenericTitleXStructure(const xstructure& xstr, bool latex) { // CO20190520
  ...
  for (size_t i = 0; i < xstr.atoms.size() && atom_names; i++) {
    if (xstr.atoms[i].cleanname.empty()) {
      atom_names = false;
    }
  }
  for (size_t itype = 0; itype < xstr.num_each_type.size(); itype++) {
    for (uint j = 0; j < (uint) xstr.num_each_type[itype]; j++) {
      if (j == 0) {
        if (atom_names) {
          title += xstr.atoms[iat].cleanname;
        } // CO20200624 - never use pp names, never mix pp with composition
        else {
          title += char('A' + itype);
        }
        if (latex) {
          title += "$_{" + aurostd::utype2string(xstr.comp_each_type[itype], comp_prec) + "}$";
        } else {
          title += aurostd::utype2string(xstr.comp_each_type[itype], comp_prec);
        }
      }
      iat++;
    }
...
// balanceEquation()
// ***************************************************************************
xvector<double> balanceChemicalEquation(const vector<xvector<double>>& lhs, const vector<xvector<double>>& rhs, bool normalize, double tol) { // CO20180801
  const bool LDEBUG = (false || XHOST.DEBUG);
  const xvector<double> dummy;
  if (lhs.empty() || rhs.empty()) {
    return dummy;
  }
  const int dim = lhs[0].rows;
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " lhs=";
    for (size_t i = 0; i < lhs.size(); i++) {
      cerr << lhs[i] << (i != lhs.size() - 1 ? ", " : "");
    }
    cerr << endl;
    cerr << __AFLOW_FUNC__ << " rhs=";
    for (size_t i = 0; i < rhs.size(); i++) {
  ...
  for (size_t i = 0; i < lhs.size(); i++) {
    counter = 1;
    for (int j = lhs[i].lrows; j <= lhs[i].urows; j++) {
      composition_matrix(i + 1, counter++) = lhs[i][j];
    }
  }             // lhs
  for (size_t i = 0; i < rhs.size(); i++) {
    counter = 1;
    for (int j = rhs[i].lrows; j <= rhs[i].urows; j++) {
      composition_matrix(i + 1 + lhs.size(), counter++) = -rhs[i][j];
    }
  } // rhs

  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " composition matrix:" << endl;
...
xvector<double> balanceChemicalEquation(const xmatrix<double>& composition_matrix, bool normalize, double tol) { // CO20191110
  ...
  if (composition_matrix.rows <= composition_matrix.cols) {
    ...
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _INPUT_ILLEGAL_); // CO20190226
  }
  xmatrix<double> Q;
  xmatrix<double> R;
  ...
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " PRE-coefficients =" << coef << endl;
  }
  if (normalize) {
    if (std::abs(coef[1]) < tol) {
      return coef;
    } // avoid divide by 0
    coef /= coef[1];
  }
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " POST-coefficients=" << coef << endl;
  }
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " checking: all coef's are >0" << endl;
  }
  ...
  for (int i = 1; i < coef.rows + 1; i++) {
    if (coef[i] < -_ZERO_TOL_) {
      ...
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_RANGE_); // CO20190226
    }
  }
  if (LDEBUG) {
    cerr << __AFLOW_FUNC__ << " checking: all scalar products should yield 0" << endl;
  }
  double sum;
  for (int i = 1; i < composition_matrix.cols + 1; i++) {
    if (LDEBUG) {
      cerr << "component-" << i << ": sum=";
    }
    sum = 0.0;
    for (int j = 1; j < composition_matrix.rows + 1; j++) {
      sum += coef(j) * composition_matrix(j, i);
    }
    if (LDEBUG) {
      cerr << sum << endl;
    }
    if (std::abs(sum) > _ZERO_TOL_) {
...
#define DEFAULT_SURFACE_LAYERS 3  // See W. Sun and G. Ceder, Surface Science 617 (2013) 53-59, fix/relax these layers of the slab
  ...
  void GeneralizedStackingFaultEnergyCalculation(const aurostd::xoption& vpflow, const xstructure& xstr_in, const _aflags& aflags, const _kflags& kflags, const _vflags& vflags, ofstream& FileMESSAGE, ostream& oss) {
    ...
    const bool convert_sconv = true;
    if (convert_sconv) {
      xstr_bulk = Standard_Conventional_UnitCellForm(xstr_bulk);
      xstr_bulk.clean();
    }  // best to work with standard conventional unitcell //DX20191220 - uppercase to lowercase clean
    if (check_min_dist) { // sanity check as we rotate structure/atoms
      min_dist = xstr_bulk.MinDist();
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " mindist[" << count_check_min_dist++ << "]=" << min_dist << endl;
      ...
      if (!aurostd::isequal(min_dist_orig, min_dist)) {
        ...
        // throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"Minimum distance changed",_VALUE_ERROR_);
        message << "Minimum distance changed (sprim -> sconv)";
    ...
    const bool skew = SYM::isLatticeSkewed(xstr_bulk.lattice, xstr_bulk.dist_nn_min, xstr_bulk.sym_eps);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " xstr_bulk.dist_nn_min=" << xstr_bulk.dist_nn_min << endl;
      cerr << __AFLOW_FUNC__ << " xstr_bulk.sym_eps=" << xstr_bulk.sym_eps << endl;
    ...
    int h_s = 1;
    int k_s = 1;
    int l_s = 0;  // hkl of shear
    xvector<int> hkl_s;
    xvector<int> hkl_s_ORIG;
    ...
    hkl_s[2] = k_s;
    hkl_s[3] = l_s;
    if (hkl_s[1] == 0 && hkl_s[2] == 0 && hkl_s[3] == 0) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "hkl_s=(0,0,0)", _INPUT_ERROR_);
    }
    hkl_s_ORIG = hkl_s;
    const string step_size_string = vpflow.getattachedscheme("GENERALIZED_STACKING_FAULT_ENERGY::STEP_SIZE"); // step size
    if (aurostd::isfloat(step_size_string)) {
      const double _step_size = aurostd::string2utype<double>(step_size_string);
      if (_step_size > 0.0 && _step_size < 1.0) {
        step_size = _step_size;
      }
    }
    const string steps_string = vpflow.getattachedscheme("GENERALIZED_STACKING_FAULT_ENERGY::STEPS"); // step size
    if (aurostd::isfloat(steps_string)) {
      const int _steps = aurostd::string2utype<int>(steps_string);
      if (_steps > 0) {
        step_size = 1.0 / _steps;
      }
    }
    const string fixed_layers_string = vpflow.getattachedscheme("GENERALIZED_STACKING_FAULT_ENERGY::FIXED_LAYERS");
    if (aurostd::isfloat(fixed_layers_string)) {
      const int _fixed_layers = aurostd::string2utype<int>(fixed_layers_string);
      if (_fixed_layers > 0) {
    ...
    message << "shear_direction" << hkl_s;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    message << "step_size=" << step_size;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    message << "fixed_layers=" << fixed_layers;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    message << "spin=" << (spin_off ? "OFF" : "ON");
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    message << "partial_dissociation=" << (partial_dissociation ? "ON" : "OFF");
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);

    message.precision(prec_original); // set back
    message.flags(ff_original); // set back

    const aurostd::xoption slab_flags = vpflow; // may modify based on how many layers in unit cell
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " xstr_bulk.lattice=" << endl;
      cerr << xstr_bulk.lattice << endl;
    ...
    if (total_layers % 2 != 0) {
      message << "total_layers is odd, it is better to pick an even number (top vs. bottom)";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_WARNING_);
    }
    ...
    const xvector<double> n_i_ORIG = n_i;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " n_i[hkl=" << hkl_i << "]=" << n_i << endl;
    }
    ...
    //[CO20190515 - not necessarily true that n_i and n_s are perpendicular, n_s must have no 0 component AFTER rotation]  throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,message,_VALUE_ERROR_);
    ...
    xvector<double> n_s = xstr_bulk.f2c * aurostd::xvector2utype<double>(hkl_s);
    n_s /= aurostd::modulus(n_s); // f2c=trasp(xstr_bulk.lattice)
    const xvector<double> n_s_ORIG = n_s;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " n_s[hkl=" << hkl_s << "]=" << n_s << endl;
    }

    const double angle_ns_unrotated = aurostd::angle(n_i, n_s);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " angle_ns(unrotated)=" << rad2deg * angle_ns_unrotated << endl;
    }
    ...
    // rotate n_i and n_s
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " n_i[hkl=" << hkl_i << "](unrotated)=" << n_i << endl;
    }
    xvector<double> n_i_rotated = rotation * n_i;
    n_i_rotated /= aurostd::modulus(n_i_rotated); // rotate to match structure rotation
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " n_i[hkl=" << hkl_i << "](rotated)  =" << n_i_rotated << endl;
    }

    if (!aurostd::isequal(aurostd::abs(n_i_rotated[3]), 1.0)) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " n_s[hkl=" << hkl_s << "](unrotated)=" << n_s << endl;
    }
    xvector<double> n_s_rotated = rotation * n_s;
    n_s_rotated /= aurostd::modulus(n_s_rotated); // rotate to match structure rotation
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " n_s[hkl=" << hkl_s << "](rotated)  =" << n_s_rotated << endl;
    }

    const double angle_ns_rotated = aurostd::angle(n_i_rotated, n_s_rotated);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " angle_ns(unrotated)=" << rad2deg * angle_ns_unrotated << endl;
      cerr << __AFLOW_FUNC__ << " angle_ns(rotated)  =" << rad2deg * angle_ns_rotated << endl;
    }
    ...
    if (!partial_dissociation) {
      if (!aurostd::isequal(rad2deg * angle_ns_unrotated, 90.0) || aurostd::isequal(n_s_rotated[3], 0.0)) { // not sure about partial dissociation
        ...
        vector<xvector<double>> v_n_s_sym;
        vector<xvector<double>> v_n_s_sym_rotated;
        vector<uint> v_pgs;
        xvector<double> n_s_sym;
        xvector<double> n_s_sym_rotated;
        double angle_nssym_unrotated = 0.0;
        for (size_t pg = 0; pg < v_pgroups.size(); pg++) {
          ...
          if (aurostd::isequal(rad2deg * angle_nssym_unrotated, 90.0)) {
            ...
            if (aurostd::isequal(n_s_sym_rotated[3], 0.0)) {
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " found viable symmetrically equivalent n_s=" << n_s_sym << ", n_s_sym_rotated=" << n_s_sym_rotated << ", pg=" << pg << endl;
              }
        ...
        if (v_n_s_sym.empty()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Cannot find any viable symmetrically equivalent n_s", _RUNTIME_ERROR_);
        }
        ...
        // it's also cleaner
        xvector<double> xvtmp;
        xvector<double> xvtmp_rotated;
        uint uitmp = 0;
        bool swap = false;
        ...
        const int lrows = n_s.lrows;
        for (size_t i = 0; i < v_n_s_sym_rotated.size() - 1; i++) {
          for (size_t j = i + 1; j < v_n_s_sym_rotated.size(); j++) {
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " comparing " << v_n_s_sym_rotated[i] << " with " << v_n_s_sym_rotated[j] << endl;
            }
            ...
            if (swap == false) {
              ...
              for (int k = v_n_s_sym_rotated[i].lrows; k <= v_n_s_sym_rotated[i].urows; k++) {
                if (aurostd::isequal(aurostd::abs(v_n_s_sym_rotated[i][k]), 1.0)) {
                  if (LDEBUG) {
                    cerr << __AFLOW_FUNC__ << " [i] is single direction" << endl;
                  }
                  sing_dir_vec_i = true;
                }
                if (aurostd::isequal(aurostd::abs(v_n_s_sym_rotated[j][k]), 1.0)) {
                  if (LDEBUG) {
                    cerr << __AFLOW_FUNC__ << " [j] is single direction" << endl;
                  }
                  sing_dir_vec_j = true;
                }
              }
              if (sing_dir_vec_i != sing_dir_vec_j) {
                if (sing_dir_vec_j == true && sing_dir_vec_i == false) {
                  swap = true;
                } else {
                  continue;
                }
            ...
            if (swap == false) {
              ...
              if (aurostd::isequal(std::abs(v_n_s_sym_rotated[i][lrows]), std::abs(v_n_s_sym_rotated[i][lrows + 1]))) {
                equ_comp_vec_i = true;
              }
              if (aurostd::isequal(std::abs(v_n_s_sym_rotated[j][lrows]), std::abs(v_n_s_sym_rotated[j][lrows + 1]))) {
                equ_comp_vec_j = true;
              }
              if (equ_comp_vec_i != equ_comp_vec_j) {
                if (equ_comp_vec_j == true && equ_comp_vec_i == false) {
                  swap = true;
                }
            ...
            if (swap == false) {
              for (int k = v_n_s_sym_rotated[i].lrows; k <= v_n_s_sym_rotated[i].urows && swap == false; k++) {
                ...
                if (LDEBUG) {
                  cerr << __AFLOW_FUNC__ << " [i][k=" << k << "] ?== [j][k=" << k << "] == " << components_equ << endl;
                }
                if (components_equ == false) {
                  components_equ_abs = aurostd::isequal(std::abs(v_n_s_sym_rotated[i][k]), std::abs(v_n_s_sym_rotated[j][k]));
                  j_gt_i_abs = bool(std::abs(v_n_s_sym_rotated[j][k]) > std::abs(v_n_s_sym_rotated[i][k]));
                  j_gt_i = bool(v_n_s_sym_rotated[j][k] > v_n_s_sym_rotated[i][k]);
                  if (LDEBUG) {
                    cerr << __AFLOW_FUNC__ << " abs([i][k=" << k << "]) ?== abs([j][k=" << k << "]) == " << components_equ_abs << endl;
                    cerr << __AFLOW_FUNC__ << " abs([j][k=" << k << "]) ?> abs([i][k=" << k << "]) == " << j_gt_i_abs << endl;
                    cerr << __AFLOW_FUNC__ << " [j][k=" << k << "] ?> [i][k=" << k << "] == " << j_gt_i << endl;
                  }
                  if ((components_equ_abs == false && j_gt_i_abs == true) || (components_equ_abs == true && components_equ == false && j_gt_i == true)) {
                    swap = true;
                  }
            ...
            if (swap) {
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " swapping " << v_n_s_sym_rotated[i] << " and " << v_n_s_sym_rotated[j] << endl;
              }
              xvtmp = v_n_s_sym[i];
        ...
        if (LDEBUG) {
          for (size_t i = 0; i < v_n_s_sym.size(); i++) {
            cerr << __AFLOW_FUNC__ << " v_n_s_sym[i=" << i << "]=" << v_n_s_sym[i] << ", v_n_s_sym_rotated[i=" << i << "]=" << v_n_s_sym_rotated[i] << ", v_pgs[i=" << i << "]=" << v_pgs[i] << endl;
          }
        }
        ...
        xvector<double> n_s_tmp = xstr_bulk.f2c * aurostd::xvector2utype<double>(hkl_s); // do not normalize
        n_s_tmp = v_pgroups[v_pgs[0]].Uc * n_s_tmp; // rotate
        hkl_s = aurostd::xvector2utype<int>(xstr_bulk.c2f * n_s_tmp); // convert to fractional
    ...
    const double d_spacing_i = slab::getSpacingHKLPlane(xstr_bulk, hkl_i); // aurostd::modulus(xstr_slab.lattice(1))/sqrt(h_i*h_i+k_i*k_i+l_i*l_i);
    const double d_layers_i = slab::getDistanceBetweenImages(xstr_bulk, n_i, false); // this depends on UN-ROTATED lattice
    const double d_cells_i = slab::getDistanceBetweenImages(xstr_bulk, n_i, true); // go outside cell
    const int layers_per_cell_i = (int) (d_cells_i / d_layers_i); // floor
    const int supercell_layers_i = (total_layers + layers_per_cell_i - 1) / layers_per_cell_i; // ceil //(double)total_layers;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " n_i[hkl=" << hkl_i << "]=" << n_i << endl;
      cerr << __AFLOW_FUNC__ << " d_spacing_i=" << d_spacing_i << endl;
      cerr << __AFLOW_FUNC__ << " d_layers_i=" << d_layers_i << endl;
      cerr << __AFLOW_FUNC__ << " d_cells_i=" << d_cells_i << endl;
      cerr << __AFLOW_FUNC__ << " layers_per_cell_i=" << layers_per_cell_i << endl;
      cerr << __AFLOW_FUNC__ << " supercell_layers_i=" << supercell_layers_i << endl;
    }
    const double d_spacing_s = slab::getSpacingHKLPlane(xstr_bulk, hkl_s); // aurostd::modulus(xstr_slab.lattice(1))/sqrt(h_s*h_s+k_s*k_s+l_s*l_s);
    const double d_layers_s = slab::getDistanceBetweenImages(xstr_bulk, n_s, false); // this depends on UN-ROTATED lattice
    const double d_cells_s = slab::getDistanceBetweenImages(xstr_bulk, n_s, true); // go outside cell
    const int layers_per_cell_s = (int) (d_cells_s / d_layers_s); // floor
    const int supercell_layers_s = (total_layers + layers_per_cell_s - 1) / layers_per_cell_s; // ceil //(double)total_layers;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " n_s[hkl=" << hkl_s << "]=" << n_s << endl;
      cerr << __AFLOW_FUNC__ << " d_spacing_s=" << d_spacing_s << endl;
      cerr << __AFLOW_FUNC__ << " d_layers_s=" << d_layers_s << endl;
      cerr << __AFLOW_FUNC__ << " d_cells_s=" << d_cells_s << endl;
      cerr << __AFLOW_FUNC__ << " layers_per_cell_s=" << layers_per_cell_s << endl;
      cerr << __AFLOW_FUNC__ << " supercell_layers_s=" << supercell_layers_s << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " identifying selective dynamics" << endl;
    }
    ...
    int k = 0;
    int k_min = xstr_slab.atoms.size();
    int k_max = -xstr_slab.atoms.size();
    for (size_t i = 0; i < xstr_slab.atoms.size(); i++) {
      const _atom& atom = xstr_slab.atoms[i];
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " atom.fpos=" << atom.fpos << ", atom.ijk=" << atom.ijk << endl;
      }
      k = atom.ijk[3];
      if (k < k_min) {
        k_min = k;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " k_min=" << k_min << ", k_max=" << k_max << endl;
    }
    ...
    uint count_total_fixed = 0;
    uint count_bottom_fixed = 0;
    uint count_top_fixed = 0; // tests of stupidity
    ...
    for (size_t i = 0; i < xstr_slab.atoms.size(); i++) {
      ...
      if (fixed_bottom || fixed_top) { // keep fixed
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " atom.ijk=" << atom.ijk << " is FIXED" << endl;
        }
      } else {
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " atom.ijk=" << atom.ijk << " will RELAX" << endl;
        }
      }
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " count_bottom_fixed=" << count_bottom_fixed << endl;
      cerr << __AFLOW_FUNC__ << " count_top_fixed=" << count_top_fixed << endl;
      cerr << __AFLOW_FUNC__ << " count_bottom_fixed+count_top_fixed=" << count_bottom_fixed + count_top_fixed << endl;
      cerr << __AFLOW_FUNC__ << " count_total_fixed=" << count_total_fixed << endl;
    }
    if (count_total_fixed != count_bottom_fixed + count_top_fixed) {
      ...
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _VALUE_ERROR_); // CO20190226
    }
    ...
    const bool create_final_duplicate_directory = (true || LDEBUG); // debug really, we don't need the 1.0 shear fraction directory
    double shear_fraction = 0.0;
    const double shear_fraction_final = (1.0 + (create_final_duplicate_directory ? _ZERO_TOL_ : -_ZERO_TOL_)); // it's just a LITTLE bigger than 1.0 for the while loop
    ...
    if (partial_dissociation) {
      ...
      if (LDEBUG) {
        ...
        cerr << __AFLOW_FUNC__ << " total_shift_fpos=" << total_shift_fpos << endl;
        cerr << __AFLOW_FUNC__ << " z_shift_fpos=" << z_shift_fpos << endl;
      }
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " z_shift_fpos=" << z_shift_fpos << endl;
    ...
    //_kflags kflags;
    //_vflags vflags;
    //[CO20190405 - does not work, could be multiple layers within this spacing]double d_layers_s=getSpacingHKLPlane(xstr_slab.lattice,h_s,k_s,l_s);
    ////aurostd::modulus(xstr_slab.lattice(1))/sqrt(h_s*h_s+k_s*k_s+l_s*l_s); get dir_count_total
    dir_count = 0;
    shear_fraction = 0.0;
    ...
    while (shear_fraction < shear_fraction_final) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " looking for: " << FileName << endl;
      }
    ...
    if (!all_vasp_done) {
      message << "Creating sheared VASP runs";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    }

    // create directories if vasp is not done
    const int half_k = supercell_layers_i / 2; // floor
    // half_k=(supercell_layers_i+2-1)/2;  //ceil
    uint count_total = 0;
    uint count_bottom = 0;
    uint count_top = 0; // tests of stupidity
    while (shear_fraction < shear_fraction_final) {
      if (all_vasp_done == false) {
        xvasp.clear();
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " shear_fraction=" << shear_fraction << endl;
        }

        // apply shear
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " applying shear" << endl;
        }
        ...
        count_bottom = 0;
        count_top = 0;
        for (size_t i = 0; i < xstr_shear.atoms.size(); i++) {
          _atom& atom = xstr_shear.atoms[i];
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " atom.fpos=" << atom.fpos << endl;
          }
          k = atom.ijk[3];
          if (k < (k_min + half_k)) { // not shearing
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " atom.ijk=" << atom.ijk << " NOT shearing" << endl;
            }
            count_bottom++;
          } else { // shearing
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " atom.ijk=" << atom.ijk << " shearing" << endl;
            }
            atom.cpos += (shear_fraction * d_layers_s) * n_s;
            if (partial_dissociation) {
              atom.cpos -= (shear_fraction * z_shift_fpos) * n_z;
            }
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " count_bottom=" << count_bottom << endl;
          cerr << __AFLOW_FUNC__ << " count_top=" << count_top << endl;
          cerr << __AFLOW_FUNC__ << " count_bottom+count_top=" << count_bottom + count_top << endl;
          cerr << __AFLOW_FUNC__ << " count_total=" << count_total << endl;
          cerr << __AFLOW_FUNC__ << " xstr_shear.atoms.size()=" << xstr_shear.atoms.size() << endl;
        }
        ...
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " xstr_shear=" << endl;
          cerr << xstr_shear << endl;
        }

        if (check_min_dist) { // sanity check as we rotate structure/atoms
          min_dist = xstr_shear.MinDist();
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " mindist[" << (count_check_min_dist++) + dir_count << "]=" << min_dist << endl;
          }
          if (!aurostd::isequal(min_dist_orig, min_dist)) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Minimum distance changed", _VALUE_ERROR_);
          }
        ...
        xvasp.aopts.push_attached("AFLOWIN_FLAG::RELAX_TYPE", "IONS");
        xvasp.aopts.flag("FLAG::VOLUME_PRESERVED", true); // no volume changes +/*
        if (spin_off) {
          xvasp.aopts.flag("FLAG::AVASP_SPIN", false);
        }
    ...
    if (!all_vasp_done) {
      message << "Now waiting for sheared VASP runs";
  ...
  void CleavageEnergyCalculation(const aurostd::xoption& vpflow, const xstructure& xstr_in, const _aflags& aflags, const _kflags& kflags, const _vflags& vflags, ofstream& FileMESSAGE, ostream& oss) {
    ...
    int h_i = 1;
    int k_i = 1;
    int l_i = 1; // hkl of interest //needed for dirname
    ...
    hkl_i[2] = k_i;
    hkl_i[3] = l_i;
    if (hkl_i[1] == 0 && hkl_i[2] == 0 && hkl_i[3] == 0) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "hkl_i=(0,0,0)", _INPUT_ERROR_);
    }
    const string relaxation_layers_string = vpflow.getattachedscheme("CLEAVAGE_ENERGY::RELAXATION_LAYERS"); // step size
    if (aurostd::isfloat(relaxation_layers_string)) {
      const int _relaxation_layers = aurostd::string2utype<int>(relaxation_layers_string);
      if (_relaxation_layers > 0) {
        relaxation_layers = _relaxation_layers;
      }
    }
    ...
    message << "relaxation_layers=" << relaxation_layers;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    message << "spin=" << (spin_off ? "OFF" : "ON");
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    ...
    const xvector<double> n_i = slab::HKLPlane2Normal(xstr_bulk.lattice, hkl_i);
    if (LDEBUG) {
    ...
    int dir_count = 0;
    const int dir_count_total = 2; // two total dirs, bulk and slab
    bool all_vasp_done = true;
    string FileName;
    ...
    const bool convert_sprim = true;
    if (!all_vasp_done) {
      message << "Creating bulk VASP run";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
      ...
      if (convert_sprim) {
        ...
        xvasp.str.ShiftOriginToAtom(0);
        xvasp.str.origin = 0.0; // reset origin
      ...
      // relax ions ONLY
      xvasp.aopts.flag("AFLOWIN_FLAG::RELAX_TYPE", true);
      xvasp.aopts.push_attached("AFLOWIN_FLAG::RELAX_TYPE", "ALL");
      if (spin_off) {
        xvasp.aopts.flag("FLAG::AVASP_SPIN", false);
      }
    ...
    if (!all_vasp_done) {
      message << "Now waiting for the bulk VASP run";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_COMPLETE_);
      return;
    }
    ...
    const xstructure xstr_relaxed = KBIN::GetMostRelaxedStructure(xvasp.Directory);
    message << "structure(bulk_relaxed)=";
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    message << xstr_relaxed << endl;
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_RAW_);
    ...
    uint count_total_relaxing = 0;
    uint count_bottom_relaxing = 0;
    uint count_top_relaxing = 0; // tests of stupidity
    bool relax_bottom = false;
    bool relax_top = false;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " count_bottom_relaxing=" << count_bottom_relaxing << endl;
      cerr << __AFLOW_FUNC__ << " count_top_relaxing=" << count_top_relaxing << endl;
      cerr << __AFLOW_FUNC__ << " count_bottom_relaxing+count_top_relaxing=" << count_bottom_relaxing + count_top_relaxing << endl;
      cerr << __AFLOW_FUNC__ << " count_total_relaxing=" << count_total_relaxing << endl;
    }
    ...
    if (!all_vasp_done) {
      message << "Creating slab VASP run";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_MESSAGE_);
    ...
    if (!all_vasp_done) {
      message << "Now waiting for the slab VASP run";
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_COMPLETE_);
      return;
    }
...
namespace pflow {
  bool findClosedPackingPlane(istream& input) {
    const xstructure xstr_in(input, IOAFLOW_AUTO);
    return findClosedPackingPlane(xstr_in);
  } // CO20191110
  bool findClosedPackingPlane(const xstructure& xstr_in) { // CO20191110
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " grid_atoms=" << endl;
      for (size_t i = 0; i < grid_atoms.size(); i++) {
        cerr << "i=" << i << ": cpos=" << grid_atoms[i].cpos << endl;
      }
    }
    ...
    while (xc.increment()) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " checking coplanarity of:" << endl;
        cerr << a1.cpos << endl;
        cerr << a2.cpos << endl;
        cerr << a3.cpos << endl;
        cerr << a4.cpos << endl;
      ...
      if (aurostd::isequal(aurostd::scalar_product(aurostd::vector_product(a2.cpos - a1.cpos, a4.cpos - a1.cpos), a3.cpos - a1.cpos), 0.0)) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " found coplanar set" << endl;
        }
      }
    }
    ...
    if (ChemFormula.size() == nchar) { // last atom with no concentration
      ...
      if ((ChemFormula[0] > '0' && ChemFormula[0] < '9') || ChemFormula[0] == '.') {
        ...
        for (i = 0; i < (int) ChemFormula.size(); i++) {
          if (!((ChemFormula[i] > '0' && ChemFormula[i] < '9') || ChemFormula[i] == '.')) {
            break;
          }
        }
        sC = ChemFormula.substr(0, i);
        if (sC[0] == '.') {
          sC = "0" + sC; // append 0 in case the first character is '.'
        }
        if (sC[sC.size() - 1] == '.') {
          sC = sC + "0";
        }
        AtomConc = aurostd::string2utype<float>(sC);
        if (i == (int) ChemFormula.size()) {
          ChemFormula = "";
        } else {
          ChemFormula = ChemFormula.substr(i, ChemFormula.size());
        }
      } else {
        AtomConc = 1.0; // set concentration to 1.0 if atom symbol is valid but concentration is not given
      }
    }
...
// Corey Oses 20190620
namespace pflow {
  void GetXray2ThetaIntensity(const xstructure& str, vector<double>& v_twotheta, vector<double>& v_intensity,
    ...
    const double tol = XRAY_THETA_TOL;
  ...
  vector<uint> GetXrayPeaks(const xstructure& str,
    ...
    const bool LDEBUG = (false || XHOST.DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " input str=" << endl;
      cerr << str << endl;
    }
  ...
  vector<uint> GetXrayPeaks(const vector<double>& v_twotheta, const vector<double>& v_intensity, vector<double>& v_intensity_smooth) { // CO20190520 //CO20190620 - v_peaks_amplitude not needed
    ...
    if (v_twotheta.size() < 2) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "v_twotheta.size()<2", _VALUE_ILLEGAL_);
    }
    ...
    const xvector<double> xv_intensity = aurostd::vector2xvector<double>(v_intensity);
    xvector<double> xv_intensity_smooth;
    const uint smoothing_iterations = 4;
    uint avg_window = 4;
    const int width_maximum = 1;
    const double significance_multiplier = 1.0; // defaults
    ...
    if (LDEBUG) {
      ...
      cerr << __AFLOW_FUNC__ << " davg_window=" << davg_window << endl;
    ...
    vector<uint> peak_indices;
    for (size_t i = 0; i < _peak_indices.size(); i++) {
      peak_indices.push_back(_peak_indices[i] - xv_intensity.lrows);
    } // shift indices for xvector -> vector conversion

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " _peak_indices=" << aurostd::joinWDelimiter(_peak_indices, ",") << endl;
      cerr << __AFLOW_FUNC__ << "  peak_indices=" << aurostd::joinWDelimiter(peak_indices, ",") << endl;
    }
    ...
    for (int i0 = -kmx; i0 <= kmx; i0++) {
      for (int i1 = -kmx; i1 <= kmx; i1++) {
        for (int i2 = -kmx; i2 <= kmx; i2++) {
          const int ii0 = i0 + kmx;
          const int ii1 = i1 + kmx;
          const int ii2 = i2 + kmx;
          const int id = ii2 + ii1 * len + ii0 * len * len;
          ids[id][0] = dist[id];
          ids[id][1] = id;
          ids[id][2] = i0;
          ids[id][3] = i1;
    ...
    for (int i0 = -kmx; i0 <= kmx; i0++) {
      for (int i1 = -kmx; i1 <= kmx; i1++) {
        for (int i2 = -kmx; i2 <= kmx; i2++) {
          ...
          ii1 = (int) ids[id1][3];
          ii2 = (int) ids[id1][4];
          const double pdist = dist[id]; // Get present distance.
...
namespace pflow {
  void GetXray(const xstructure& str,
               vector<double>& dist,
               vector<double>& sf,
    ...
    rlat = sstr.klattice;
    const int num_atoms = sstr.atoms.size();
...
namespace pflow {
  vector<vector<int>> getvvitypesRDF(const xstructure& xstr) { // CO20220627
    const bool LDEBUG = (false || XHOST.DEBUG);
    vector<vector<int>> _vvitypes;
    vector<vector<int>> vvitypes;
    vector<int> vitypes;
    vector<int> vsizes;
    uint i = 0;
    uint j = 0;
    for (i = 0; i < 2; i++) {
      _vvitypes.emplace_back(0);
      for (j = 0; j < xstr.num_each_type.size(); j++) {
        _vvitypes.back().push_back(j);
      } // initial set, not all of these are viable options
      vsizes.push_back(xstr.num_each_type.size());
    }
    aurostd::xcombos xc(vsizes, 'E');
    while (xc.increment()) {
      vitypes = xc.applyCombo(_vvitypes);
      if (vitypes[1] < vitypes[0]) {
        continue;
      } // filter so we don't get duplicates
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " vitypes=" << aurostd::joinWDelimiter(vitypes, ",") << endl;
      }
  ...
  void GetRDF(const xstructure& xstr_in, aurostd::xmatrix<double>& rdf_all, const double rmax, const int nbins, bool raw_counts, const double sigma, const int _window_gaussian) { // CO20220624 - new procedure
    ...
    if (xstr.species.empty()) {
      xstr.species = aurostd::vector2deque(pflow::getFakeElements(xstr.num_each_type.size()));
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " species=" << aurostd::joinWDelimiter(xstr.species, ",") << endl;
    }
    ...
    for (i = 0; i < atoms_cell.size(); i++) {
      vincrements.push_back(1.0 / xstr.num_each_type[atoms_cell[i].type]);
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vincrements=" << aurostd::joinWDelimiter(aurostd::vecDouble2vecString(vincrements), ",") << endl;
    }
    int itype = 0;
    int ibin = 0;
    double dist = 0.0;
    double rad = 0.0;
    const double drad = rmax / (double) nbins; // sphere shell volume=4*pi*rad^2*drad
    vector<int> vitypes;
    uint j = 0;
    uint k = 0;
    for (i = 0; i < i_neighbors.size(); i++) {
      for (j = 0; j < i_neighbors[i].size(); j++) {
        ...
        if (dist < rmax) {
          ...
          vitypes.clear();
          vitypes.push_back(atoms_cell[i].type);
          vitypes.push_back(xstr.grid_atoms[i_neighbors[i][j]].type);
          ...
          for (k = 0; k < vvitypes.size() && itype == AUROSTD_MAX_INT; k++) {
            if (vitypes == vvitypes[k]) {
              itype = k;
            }
          }
          if (itype == AUROSTD_MAX_INT) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "could not find itype", _INDEX_MISMATCH_);
          }
    ...
    if (raw_counts == false) {
      for (ibin = rdf_all.lrows; ibin <= rdf_all.urows; ibin++) {
        ...
        if (ibin == rdf_all.lrows && !aurostd::iszero(rdf_all[ibin][rdf_all.ucols], _ZERO_TOL_)) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "first bin cannot be normalized (rad==0), increase nbins", _VALUE_ILLEGAL_);
        }
        for (itype = rdf_all.lcols; itype <= rdf_all.ucols; itype++) {
          if (aurostd::nonZeroWithinTol(rad, _ZERO_TOL_)) {
            rdf_all[ibin][itype] /= (4.0 * PI * std::pow(rad, 2.0) * drad);
          }
        }
      }
    }
    if (!aurostd::iszero(sigma)) { // smooth
      aurostd::xvector<double> _xcol;
      aurostd::xvector<double> xcol;
      aurostd::xvector<double> filter_gaussian;
      int window_gaussian = _window_gaussian;
      if (window_gaussian == 0) {
        window_gaussian = aurostd::gaussian_filter_get_window(sigma);
      }
      filter_gaussian = aurostd::gaussian_filter_xv(sigma, window_gaussian, rdf_all.lrows);
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " filter_gaussian[sigma=" << sigma << ",window_gaussian=" << window_gaussian << "]=" << filter_gaussian << endl;
      }
      for (itype = rdf_all.lcols; itype <= rdf_all.ucols; itype++) {
        ...
        for (ibin = rdf_all.lrows; ibin <= rdf_all.urows; ibin++) {
          rdf_all[ibin][itype] = xcol[ibin];
        } // plug back in
      }
    }
    if (LDEBUG) {
      PrintRDF(xstr, rmax, nbins, rdf_all, cerr);
    }
    if (LDEBUG) {
...
//  >=0 again.
namespace pflow {
  void GetRDFShells(const xstructure& str,
                    const double& rmax,
                    const int& nbins,
                    const int& smooth_width,
                    const aurostd::matrix<double>& rdf, // CO20200404 pflow::matrix()->aurostd::matrix()
                    aurostd::matrix<double>& rdfsh,
    ...
    if (smooth_width) {
      ;
    } // phony just to keep smooth_width busy
    if (!str.atoms.empty()) {
      ;
    } // phony just to keep str busy

    const int _rdi = (int) rdf.size();
    rdfsh = aurostd::matrix<double>(_rdi, 0);
    pflow::VVset(rdfsh, 0); // CO20200404 pflow::matrix()->aurostd::matrix()
    rdfsh_loc = aurostd::matrix<double>(_rdi, 0);
    pflow::VVset(rdfsh_loc, 0); // CO20200404 pflow::matrix()->aurostd::matrix()
    const double dr = (rmax / (double) nbins);
    for (int i = 0; i < _rdi; i++) {
      ...
      */
      ...
      for (int ib = 1; ib < nbins - 1; ib++) {
        // If drdf==0
        if (drdf[ib] == 0 && ddrdf[ib] < 0) {
          drdf_zeros[ib] = -1;
        }
        if (drdf[ib] == 0 && ddrdf[ib] == 0) {
          drdf_zeros[ib] = 0;
        }
        if (drdf[ib] == 0 && ddrdf[ib] == 1) {
          drdf_zeros[ib] = 1;
        }
        // If drdf== is passing through 0 from +->- or -->+
        if (drdf[ib] != 0) {
          if (drdf[ib - 1] > 0 && drdf[ib + 1] < 0) {
            drdf_zeros[ib] = -1;
          }
          if (drdf[ib - 1] < 0 && drdf[ib + 1] > 0) {
            drdf_zeros[ib] = +1;
      ...
      for (int ib = 0; ib < nbins - 2; ib++) {
        ...
        const double rad = (dr * (double) ib) + dr / 2.0;
...
namespace pflow {
  void CmpRDFShells(const xstructure& str_A,
    ...
    const std::deque<int> netype_A = str_A.num_each_type;
    const std::deque<int> netype_B = str_B.num_each_type;
    ...
    if (!pflow::VVequal(netype_A, netype_B) || netype_A.size() != netype_B.size()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "structures A and B do not have the same number of each type of atom", _INPUT_ILLEGAL_); // CO20200624
    }
    const int nat = str_A.atoms.size();
    const int nt = netype_A.size();

    // Get rms_mat
    rms_mat = aurostd::matrix<double>(nat, nat);
    pflow::VVset(rms_mat, -1.0); // CO20200404 pflow::matrix()->aurostd::matrix()
    ...
    for (int iaA = 0; iaA < (int) rms_mat.size(); iaA++) {
      ...
      double trms = -10;
      int best_at_id = -1;
      const int typeA = str_A.atoms.at(iaA).type;
      // Check each B atom and if it is the right type and lowest rms
      // then track its id.
      for (int ipB = 0; ipB < (int) cand_Batoms.size(); ipB++) {
        const int iaB = cand_Batoms[ipB];
        const int typeB = str_B.atoms.at(iaB).type;
        if (typeA == typeB) {
...
namespace pflow {
  void CmpStrDist(xstructure str1, xstructure str2, const double& cutoff, aurostd::matrix<double>& dist1, aurostd::matrix<double>& dist2, aurostd::matrix<double>& dist_diff, aurostd::matrix<double>& dist_diff_n) { // CO20200404 pflow::matrix()->aurostd::matrix()
    ...
    const int ntypes1 = (int) str1.num_each_type.size();
    const int ntypes2 = (int) str2.num_each_type.size();
    const int nprs1 = (ntypes1 * (ntypes1 + 1)) / 2;
    const int nprs2 = (ntypes2 * (ntypes2 + 1)) / 2;
    const vector<double> tmpvec;
    ...
    for (int ia = 0; ia < (int) neigh_mat1.size(); ia++) {
      const _atom a = neigh_mat1[ia][0];
      for (int in = 1; in < (int) neigh_mat1[ia].size(); in++) {
        const _atom an = neigh_mat1[ia][in];
        const int ta = a.type; // CONVASP_MODE
    ...
    for (int ia = 0; ia < (int) neigh_mat2.size(); ia++) {
      const _atom a = neigh_mat2[ia][0];
      for (int in = 1; in < (int) neigh_mat2[ia].size(); in++) {
        const _atom an = neigh_mat2[ia][in];
        const int ta = a.type; // CONVASP_MODE
        const int tna = an.type; // CONVASP_MODE
        const int i = std::min(ta, tna);
        const int j = std::max(ta, tna);
    ...
    const int ntypes_min = std::min(ntypes1, ntypes2);
    const int nprs_min = ntypes_min * (ntypes_min + 1) / 2;
...
namespace pflow {
  ...
  void pdosdata::PrintPDOS(ostream& outf, const int& sp) {
    ...
    if (sp == 0) {
      outf << "# Energy         Up            Cumulative_Up" << endl;
    }
    if (sp == 1) {
      outf << "# Energy         Up             Dn             Up-Dn        Cumulative_Up   Cumulative_Dn   Cumulative_Up-Dn" << endl;
    }
...
namespace pflow {
  ...
  rtparams::rtparams() {
    ...
    const vector<double> color(3, 0.0);
    ...
    shading = "fullshade";
    outfile = "POSCAR_RT";
    sc = aurostd::matrix<double>(3, 3);
    pflow::VVset(sc, 0.0); // CO20200404 pflow::matrix()->aurostd::matrix()
    sc[0][0] = 1;
    sc[1][1] = 1;
    sc[2][2] = 1;
    sc_s = 0;
  ...
  rtparams::rtparams(const rtparams& b) {
    copy(b);
  }
  ...
  void rtparams::Print() const {
    ...
    cout << "ROTATION = " << rotation[0] << " " << rotation[1] << " " << rotation[2] << " " << rotation[3] << " " << rotation[4] << " " << rotation[5] << endl;
  }
} // namespace pflow
  ...
  void GetDatFromOutcar(vector<aurostd::matrix<double>>& lat_vec, // CO20200404 pflow::matrix()->aurostd::matrix()
    ...
    while (outcar_inf >> a[2]) {
      ...
      if (key == "ions per type") {
        ...
        while (id < (int) s.size()) {
          const int n = atoi(aurostd::GetNextVal(s, id).c_str());
...
namespace pflow {
  vector<xstructure> GetStrVecFromOUTCAR_XDATCAR(ifstream& outcar_inf, ifstream& xdatcar_inf) {
    ...
    if (lat_vec.size() > fpos_vec.size()) {
      lat_vec.resize(fpos_vec.size());
    }
    if (lat_vec.size() < fpos_vec.size()) {
      const aurostd::matrix<double> tlat = lat_vec[lat_vec.size() - 1]; // CO20200404 pflow::matrix()->aurostd::matrix()
    ...
    xstructure str;
    const int nstr = (int) lat_vec.size();
    for (int is = 0; is < nstr; is++) {
      const int nat = fpos_vec[0].size();
      const vector<string> names(nat, "H");
      const vector<int> names_were_given(nat, false);
...
namespace pflow {
  void ReadInRTParams(ifstream& rtinfile, pflow::rtparams& rtp) {
    ...
    while (!rtinfile.eof()) {
      ...
      if (!s_ns.empty()) { // Make sure line was not blank
        if (s_ns[0] != '#') { // Exclude comment lines
          string token;
          string sval;
          const int id = s.find('=');
          if (id >= (int) s.length()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "the following token is incorrectly formatted: " + s, _INPUT_ILLEGAL_); // CO20200624
          }
          token = s.substr(0, id);
          token = aurostd::RemoveSpaces(token);
          const int i_f = std::min(s.length(), s.find('#')); // End of string
    ...
    for (int i = 0; i < (int) token_vec.size(); i++) {
      ...
      if (tok == "ROTATION") {
        int id = 0;
        for (int ic = 0; ic < 6; ic++) {
          const string s = aurostd::GetNextVal(val_vec[i], id);
      ...
      if (!found_token) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "token is not recognized: " + tok, _INPUT_ILLEGAL_); // CO20200624
      }
...
namespace pflow {
  void ReadInStrVec(vector<xstructure>& vstr, ifstream& strlist_inf) {
    ...
    while (iline < vline.size()) {
      if (aurostd::RemoveWhiteSpaces(vline.at(iline)).empty()) {
        const xstructure str(sss);
        vstr.push_back(str);
        sss.clear();
        sss.str("");
      } else {
...
namespace pflow {
  void JoinStrVec(vector<xstructure> vstr1, vector<xstructure> vstr2, vector<xstructure>& vstrtot) {
    // cout << "In JOIN" << endl;
    // cout << "strlist1 size " << vstr1.size() << endl;
    // cout << "strlist2 size " << vstr2.size() << endl;
    const int size1 = vstr1.size();
    const int size2 = vstr2.size();
    if (size1 < size2) { // Pad vstr1
      const xstructure str = vstr1[size1 - 1];
      for (int is = size1; is < size2; is++) {
        vstr1.push_back(str);
      }
    }
    if (size2 < size1) { // Pad vstr2
      const xstructure str = vstr2[size2 - 1];
    ...
    for (int is = 0; is < size1; is++) {
      ...
      const xstructure str2 = vstr2[is];
      const aurostd::matrix<double> lat1 = pflow::GetLat(str1); // CO20200404 pflow::matrix()->aurostd::matrix()
      xmatrix<double> xlat1(3, 3);
      xlat1 = str1.lattice;
      aurostd::matrix<double> cpos2 = pflow::GetCpos(str2); // CO20200404 pflow::matrix()->aurostd::matrix()
      const deque<int> num_each_type_1 = str1.num_each_type; // DX20210119 pflow::GetNumEachType() -> xstructure.num_each_type
      const int num_types_1 = num_each_type_1.size();
      deque<int> num_each_type_2 = str2.num_each_type; // DX20210119 pflow::GetNumEachType() -> xstructure.num_each_type
      const int num_types_2 = num_each_type_2.size();
...
namespace pflow {
  void SetRTParams(xstructure& str, pflow::rtparams& rtp) {
    ...
    str = ReScale(str, 1.0);
    const aurostd::matrix<double> lat = pflow::GetLat(str); // CO20200404 pflow::matrix()->aurostd::matrix()
    aurostd::matrix<double> cpos = pflow::GetCpos(str); // CO20200404 pflow::matrix()->aurostd::matrix()
    const int nat = cpos.size();
    ...
    if (rtp.first_set) {
      const int ntypes = str.num_each_type.size(); // DX20210119 pflow::GetNumEachType() -> xstructure.num_each_type
      ...
      for (int i = 0; i < (int) rtp.sphtex_tex.size(); i++) {
        const int id = (int) rtp.sphtex_tex[i][0] - 1;
        for (int j = 1; j < size + 1; j++) {
          if (id >= 0 && id < ntypes) {
            tmp[id][j - 1] = rtp.sphtex_tex[i][j];
          }
        }
      }
      rtp.sphtex_tex = tmp;
      ...
      for (int i = 0; i < (int) rtp.sphtex_color.size(); i++) {
        const int id = (int) rtp.sphtex_color[i][0] - 1;
        for (int j = 1; j < size + 1; j++) {
          if (id >= 0 && id < ntypes) {
            tmp[id][j - 1] = rtp.sphtex_color[i][j];
          }
        }
      }
      ...
      for (int i = 0; i < (int) rtp.sphtex_phong.size(); i++) {
        const int id = (int) rtp.sphtex_phong[i][0] - 1;
        for (int j = 1; j < size + 1; j++) {
          if (id >= 0 && id < ntypes) {
            tmp[id][j - 1] = rtp.sphtex_phong[i][j];
          }
        }
      }
      ...
      for (int i = 0; i < (int) rtp.sph_rad.size() / 2; i++) {
        const int id = (int) rtp.sph_rad[2 * i] - 1;
        if (id >= 0 && id < ntypes) {
          vtmp[id] = rtp.sph_rad[2 * i + 1];
        }
      }
      rtp.sph_rad = vtmp;
      // Set sphtex_names
    ...
    if (!rtp.struct_origin_s) { // If struct_orig was not set by user
      ...
      const vector<double> mom1 = xvector2vector(GetMom1(str));
    ...
    if (!rtp.center_s) { // If center was not set by user
      const vector<double> mom1 = rtp.struct_origin;
      const vector<double> vd = pflow::SVprod(1.0 / pflow::norm(rtp.viewdir), rtp.viewdir);
      const vector<double> up = pflow::SVprod(1.0 / pflow::norm(rtp.updir), rtp.updir);
      vector<double> upp;
      upp = pflow::VVsum(up, pflow::SVprod(-pflow::VVprod(up, vd), vd));
      upp = pflow::SVprod(1.0 / pflow::norm(upp), upp);
      const vector<double> vdp = pflow::VVcross(vd, upp);
      double proj = 0;
      //      double max_iat=0;
      for (int iat = 0; iat < nat; iat++) {
        const double p1 = pflow::VVprod(pflow::VVdiff(cpos[iat], mom1), vdp);
        const double p2 = pflow::VVprod(pflow::VVdiff(cpos[iat], mom1), upp);
        const double nproj = sqrt(p1 * p1 + p2 * p2);
        if (nproj > proj) {
          proj = nproj;
          //	  max_iat=iat;
        }
      }
      const double displ = -2.0 * proj;
...
namespace pflow {
  void GetRTDatFile(xstructure str, const pflow::rtparams& rtp, ostringstream& rtdat_file) {
    ...
    const int ntype = (int) num_each_type.size();
    int cnt = 0;
...
namespace pflow {
  string CreateRTtgaFile(const string& datfile, const pflow::rtparams& rt_params) {
    ...
    aurostd::file2file("outfile.tga", tgafile);
    return tgafile;
  }
} // namespace pflow
...
namespace pflow {
  string CreateRTjpgFile(const string& tgafile, const pflow::rtparams& rt_params) {
    ...
    // aurostd::execute(convert_stream.str().c_str());
    return jpgfile;
  }
} // namespace pflow
...
namespace pflow {
  string CreateRTmpgFile(const pflow::rtparams& rt_params, const string& encfile) {
    ...
    /*  char* encoder_char = encoder_stream.str().c_str();
        aurostd::execute(encoder_char);*/
    aurostd::execute(encoder_stream.str().c_str());
  ...
  void RayTraceManager(vector<string> argv) {
    ...
    switch (rtp.calc_type) {
      case 0: { // A single file of structures.
        ...
        if (rtp.input_files.empty()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "must specify a structure list file to open with the token INFILE (e.g., INFILE = STRLIST)", _FILE_CORRUPT_); // CO20200624
        }
        ...
        for (int is = 0; is < nstr; is++) {
          ...
          const string num = aurostd::PaddedNumString(is, 5);
          const string datname = (rtp.outfile + "_" + num + ".dat");
          const string tganame = (rtp.outfile + "_" + num + ".tga");
          const string jpgname = (rtp.outfile + "_" + num + ".jpg");
          aurostd::file2file(datfile, datname);
          aurostd::file2file(tgafile, tganame);
          aurostd::file2file(jpgfile, jpgname);
          if (nstr > 1) { // Remove dat,tga files if there are a lot of them.
            aurostd::RemoveFile(datname);
          }
        }
      ...
      default: {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "invalid CALCTYPE = " + aurostd::utype2string(rtp.calc_type), _INPUT_ILLEGAL_); // CO20200624
      }
    } // switch
  }
} // namespace pflow
...
namespace pflow {
  ...
  void projdata::Print(ostream& outf) {
    ...
    for (int ik = 0; ik < (int) wkpt.size(); ik++) {
      outf << "  " << ik + 1 << "  " << kpts[ik][0] << "  " << kpts[ik][1] << "  " << kpts[ik][2] << "      " << wkpt[ik] << endl;
    }
    ...
    for (int ik = 0; ik < (int) ener_k_b_u.size(); ik++) {
      for (int ib = 0; ib < (int) ener_k_b_u[ik].size(); ib++) {
        ...
        if (sp) {
          outf << "  " << setprecision(5) << ener_k_b_d[ik][ib];
        }
        outf << endl;
      }
    }
        ...
        for (int ik = 0; ik < (int) pdat_u[it].size(); ik++) {
          for (int ib = 0; ib < (int) pdat_u[it][ik][iat].size(); ib++) {
            for (int ilm = 0; ilm < (int) pdat_u[it][ik][iat][ib].size(); ilm++) {
              ...
              if (sp) {
                outf << "  " << setw(w2) << pdat_d[it][ik][iat][ib][ilm];
              }
              outf << endl;
            }
          }
        ...
        for (int iat = 0; iat < pd.num_each_type.at(it); iat++) {
          for (int ib = 0; ib < pd.nbands; ib++) {
            for (int ilm = 0; ilm < pd.nlm; ilm++) {
              ...
              const std::complex<double> ctmp(rval[ilm], ival[ilm]);
        ...
        for (int ib = 0; ib < pd.nbands; ib++) {
          for (int it = 0; it < pd.ntypes; it++) {
            for (int iat = 0; iat < pd.num_each_type.at(it); iat++) {
              ...
              for (int ilm = 0; ilm < pd.nlm; ilm++) {
                pd.pdat_u[it][ik][iat][ib][ilm] = pd.pdat_u[it][ik][iat][ib][ilm] + rval[ilm];
              }
            }
          }
        }
    ...
    for (int ik = 0; ik < pd.nkpts; ik++) {
      ...
      for (int ib = 0; ib < pd.nbands; ib++) {
        ...
        for (int ii = 0; ii < pd.nions + skip; ii++) {
          ...
          getline(infile, s);
          if (aurostd::RemoveSpaces(s).empty()) {
          ...
          for (int ib = 0; ib < pd.nbands; ib++) {
            for (int it = 0; it < pd.ntypes; it++) {
              for (int iat = 0; iat < pd.num_each_type.at(it); iat++) {
                ...
                for (int ilm = 0; ilm < pd.nlm; ilm++) {
                  pd.pdat_d[it][ik][iat][ib][ilm] = pd.pdat_d[it][ik][iat][ib][ilm] + rval[ilm];
                }
              }
            }
          }
...
namespace pflow {
  void CalcNeatProj(projdata& pd, int only_occ) {
    ...
    const aurostd::matrix<double> m2(pd.nkpts, pd.nlm_max, 0.0); // CO20200404 pflow::matrix()->aurostd::matrix()
    const aurostd::matrix<double> m3(pd.nbands, pd.nlm_max, 0.0); // CO20200404 pflow::matrix()->aurostd::matrix()
        ...
        for (int ik = 0; ik < pd.nkpts; ik++) {
          for (int ib = 0; ib < pd.nbands; ib++) {
            for (int ilm = 0; ilm < pd.nlm; ilm++) {
              ...
              pd.occ_vs_ion_kpt_lm_u[ioncnt][ik][ilm] = pd.occ_vs_ion_kpt_lm_u[ioncnt][ik][ilm] + pd.occ_vs_ion_kpt_bnd_lm_u[ioncnt][ik][ib][ilm];
              pd.occ_vs_ion_bnd_lm_u[ioncnt][ib][ilm] = pd.occ_vs_ion_bnd_lm_u[ioncnt][ib][ilm] + pd.wkpt[ik] * pd.occ_vs_ion_kpt_bnd_lm_u[ioncnt][ik][ib][ilm];
              pd.occ_vs_ion_lm_u[ioncnt][ilm] = pd.occ_vs_ion_lm_u[ioncnt][ilm] + pd.wkpt[ik] * pd.occ_vs_ion_kpt_bnd_lm_u[ioncnt][ik][ib][ilm];
              if (pd.sp) {
                ...
                pd.occ_vs_ion_kpt_lm_d[ioncnt][ik][ilm] = pd.occ_vs_ion_kpt_lm_d[ioncnt][ik][ilm] + pd.occ_vs_ion_kpt_bnd_lm_d[ioncnt][ik][ib][ilm];
                pd.occ_vs_ion_bnd_lm_d[ioncnt][ib][ilm] = pd.occ_vs_ion_bnd_lm_d[ioncnt][ib][ilm] + pd.wkpt[ik] * pd.occ_vs_ion_kpt_bnd_lm_d[ioncnt][ik][ib][ilm];
                pd.occ_vs_ion_lm_d[ioncnt][ilm] = pd.occ_vs_ion_lm_d[ioncnt][ilm] + pd.wkpt[ik] * pd.occ_vs_ion_kpt_bnd_lm_d[ioncnt][ik][ib][ilm];
              }
            }
          }
        }
    ...
    const aurostd::matrix<double> n1(pd.nbands, pd.nl_max + 1, 0.0); // CO20200404 pflow::matrix()->aurostd::matrix()
    const aurostd::matrix<double> n2(pd.nkpts, pd.nl_max + 1, 0.0); // CO20200404 pflow::matrix()->aurostd::matrix()
    const aurostd::matrix<double> n3(pd.nbands, pd.nl_max + 1, 0.0); // CO20200404 pflow::matrix()->aurostd::matrix()
    ...
    for (int it = 0; it < pd.ntypes; it++) {
      for (int iat = 0; iat < pd.num_each_type.at(it); iat++) {
        ...
        for (int il = 0; il < pd.nl; il++) {
          ...
          if (pd.sp) {
            pd.occ_vs_ion_l_d[ioncnt][il] /= (pd.nbands * pd.nkpts);
          }
          for (int ik = 0; ik < pd.nkpts; ik++) {
            pd.occ_vs_ion_kpt_l_u[ioncnt][ik][il] /= (pd.nbands);

            if (pd.sp) {
              pd.occ_vs_ion_kpt_l_d[ioncnt][ik][il] /= (pd.nbands);
            }
          }
          for (int ib = 0; ib < pd.nbands; ib++) {
            pd.occ_vs_ion_bnd_l_u[ioncnt][ib][il] /= (pd.nkpts);
            if (pd.sp) {
              pd.occ_vs_ion_bnd_l_d[ioncnt][ib][il] /= (pd.nkpts);
            }
          }
        }
        ...
        for (int id = 0; id < pd.nl_max; id++) {
          ...
          for (int ik = 0; ik < pd.nkpts; ik++) {
            for (int ib = 0; ib < pd.nbands; ib++) {
              ...
              if (pd.sp) {
                pd.occ_vs_ion_kpt_bnd_l_d[ioncnt][ik][ib][iid] += pd.occ_vs_ion_kpt_bnd_l_d[ioncnt][ik][ib][id];
              }
            }
          }
        }
    ...
    if (pd.sp) {
      pd.occ_vs_ion_kpt_bnd_lmtot_d = aurostd::matrix<aurostd::matrix<double>>(pd.nions, pd.nkpts, m1); // CO20200404 pflow::matrix()->aurostd::matrix()
    }
...
namespace pflow {
  void ReadInPDOSData(const pflow::projdata& prd, pflow::pdosdata& pdd) {
    ...
    while (!infile.eof()) {
      ...
      if (!s_ns.empty()) { // Make sure line was not blank
        if (s_ns[0] != '#') { // Exclude comment lines
          ...
          token = s.substr(0, id);
          token = aurostd::RemoveSpaces(token);
          const int i_f = std::min((int) s.length(), (int) s.find('#')); // End of string
          sval = s.substr(id + 1, i_f - id - 1);
          token_vec.push_back(token);
          val_vec.push_back(sval);
        } // if s_ns[0]!=#
    ...
    for (int i = 0; i < (int) token_vec.size(); i++) {
      ...
      if (tok == "KPOINTS") {
        if ((int) pdd.pdos_k.size() != atom_cnt || atom_cnt == 0) {
          AtomCntError(tok, (int) pdd.pdos_k.size(), atom_cnt);
        }
        id = 0;
        tmp.clear();
        // set up atoms matrix
      ...
      if (tok == "LMVALUES") {
        if ((int) pdd.pdos_lm.size() != atom_cnt || atom_cnt == 0) {
          AtomCntError(tok, (int) pdd.pdos_lm.size(), atom_cnt);
        }
        id = 0;
        tmp.clear();
        // set up atoms matrix
    ...
    vv[2] = prd.nbands;
    vv[3] = prd.nlmtot;
    const int mx = *max_element(vv.begin(), vv.end());
    vector<int> cnt(mx);
    for (int i = 1; i <= mx; i++) {
      cnt[i - 1] = i;
    }
    //  int mx = max(prd.nions,prd.nkpts,prd.nbands,prd.nlmtot);
    const vector<int> atv(prd.nions);
    for (int i = 0; i < (int) pdd.pdos_at.size(); i++) {
      if (pdd.pdos_at[i].empty()) {
        pdd.pdos_at[i] = vector<int>(cnt.begin(), cnt.begin() + prd.nions);
      }
      if (pdd.pdos_k[i].empty()) {
        pdd.pdos_k[i] = vector<int>(cnt.begin(), cnt.begin() + prd.nkpts);
      }
      if (pdd.pdos_b[i].empty()) {
        pdd.pdos_b[i] = vector<int>(cnt.begin(), cnt.begin() + prd.nbands);
      }
      if (pdd.pdos_lm[i].empty()) {
        pdd.pdos_lm[i] = vector<int>(1, prd.nlmtot);
      }
    }
    ...
    for (int i = 0; i < (int) pdd.pdos_at.size(); i++) { // cases
      for (int ia = 0; ia < (int) pdd.pdos_at[i].size(); ia++) {
        const int atp = pdd.pdos_at[i][ia];
      ...
      for (int ilm = 0; ilm < (int) pdd.pdos_lm[i].size(); ilm++) {
        const int lmp = pdd.pdos_lm[i][ilm];
...
namespace pflow {
  void CalcPDOS(const projdata& prd, pdosdata& pdd) {
    const double de = (pdd.emax - pdd.emin) / pdd.nbins;
    double e;
    int ibin;
    int sp_size = 1;
    if (prd.sp) {
    ...
    for (int i = 0; i < (int) pdd.pdos_at.size(); i++) { // cases
      for (int ia = 0; ia < (int) pdd.pdos_at[i].size(); ia++) {
        const int at = pdd.pdos_at[i][ia] - 1;
        for (int ik = 0; ik < (int) pdd.pdos_k[i].size(); ik++) {
          const int kp = pdd.pdos_k[i][ik] - 1;
          for (int ib = 0; ib < (int) pdd.pdos_b[i].size(); ib++) {
            const int bp = pdd.pdos_b[i][ib] - 1;
            for (int ilm = 0; ilm < (int) pdd.pdos_lm[i].size(); ilm++) {
              const int lmp = pdd.pdos_lm[i][ilm] - 1;
...
namespace pflow {
  void SmoothPDOS(const projdata& prd, pdosdata& pdd) {
    ...
    for (int is = 1; is <= sp_size; is++) {
      ...
      const double sig = pdd.smooth_sigma / de;
...
namespace pflow {
  void AtomCntError(const string& tok, const int tokcnt, const int atom_cnt) {
    ...
    throw aurostd::xerror(__AFLOW_FILE__, "pflow::AtomCntError():", message, _INPUT_ILLEGAL_); // CO20200624
  }
} // namespace pflow
...
namespace pflow {
  void ReadInPDOSData(aurostd::matrix<aurostd::matrix<double>>& allpdos,
                      pflow::pdosdata& pdd, // CO20200404 pflow::matrix()->aurostd::matrix()
                      ifstream& infile) {
    string sdum;
    // Get natoms
    infile >> pdd.natoms;
    ...
    getline(infile, sdum);
    if (std::abs(pdd.efermi + 999) < 1e-6) { // Reset efermi if it has special -999 value
      pdd.efermi = tmpe;
    }
    // Skip total DOS
    ...
    const aurostd::matrix<double> tmp(pdd.nlm + 1, pdd.nbins); // CO20200404 pflow::matrix()->aurostd::matrix()
  ...
  void SumPDOS(const aurostd::matrix<aurostd::matrix<double>>& allpdos, pflow::pdosdata& pdd) { // CO20200404 pflow::matrix()->aurostd::matrix()
    ...
    for (int ic = 0; ic < (int) pdd.pdos_at.size(); ic++) {
      for (int ida = 0; ida < (int) pdd.pdos_at[ic].size(); ida++) {
        const int ia = pdd.pdos_at[ic][ida] - 1;
        // Error check
        ...
        for (int idlm = 0; idlm < (int) pdd.pdos_lm[ic].size(); idlm++) {
          const int ilm = pdd.pdos_lm[ic][idlm];
          // Error check
    ...
    const double dE = pdd.pdos[1][0] - pdd.pdos[0][0];
    if (pdd.spin == 1) {
      pdd.pdos[0][2] = 0;
      for (int ib = 1; ib < pdd.nbins; ib++) {
...
namespace pflow {
  double TotalAtomDist(xstructure str, xstructure str00, const string& path_flag) {
    ...
    const aurostd::matrix<double> lat = pflow::GetLat(str); // CO20200404 pflow::matrix()->aurostd::matrix()
    const int nat = std::min(cpos.size(), cpos00.size());
    double dtot = 0;
    for (int iat = 0; iat < nat; iat++) {
...
namespace pflow {
  vector<double> GetRBEner(const int& nim) {
    ...
    const std::string tmpfile = aurostd::TmpFileCreate();
    vector<string> rbdir = GetRBDir(nim);
    for (int im = 0; im < nim + 2; im++) {
      ostringstream cmd;
      cmd << "cd " << rbdir[im] << ";";
      cmd << R"(tail -2 OSZICAR | head -1 | awk '{if($1=="CG") {print $4;}} {if($1=="DAV:") {print $3;}} {if($1=="RMM:") {print $3;}}' >> )" << tmpfile << ";" << "cd ..;" << ends;
      aurostd::execute(cmd.str().c_str());
...
namespace pflow {
  vector<double> GetRBDistFromStrI(const vector<xstructure>& vstr, const xstructure& strI, const string& path_flag) {
    const int nim = vstr.size() - 2;
    vector<double> d((nim + 2), 0.0);
    xstructure diffstr;
    aurostd::matrix<double> cm; // CO20200404 pflow::matrix()->aurostd::matrix()
...
namespace pflow {
  void RBPoscarDisp(const xstructure& str1in,
                    const xstructure& str2in,
                    xstructure& diffstr,
                    double& totdist,
                    aurostd::matrix<double>& cm, // CO20200404 pflow::matrix()->aurostd::matrix()
...
namespace pflow {
  void GetChgInt(vector<aurostd::matrix<double>>& rad_chg_int, aurostd::matrix<double>& vor_chg_int) { // CO20200404 pflow::matrix()->aurostd::matrix()
    ...
    string ds;
    const int natoms = pflow::GetNumAtoms(str);
    cin >> ds;
    const string keyword = "augmentation";
    if (ds == keyword) {
    ...
    const double DR = 0.05; // width of radial bins.
    const double RMAX = 3; // Max distance to bin to.
    const int NRBIN = (int) (RMAX / DR); // First bin is 0->DR, last bin is RMAX-DR->RMAX
    const aurostd::matrix<double> dum_mat(NRBIN, 5, 0.0); // CO20200404 pflow::matrix()->aurostd::matrix()
    ...
    vector<int> imax(3);
    const double scale = pflow::GetScale(str);
        ...
        for (ig[1] = -imax[1]; ig[1] < imax[1]; ig[1]++) {
          for (ig[2] = -imax[2]; ig[2] < imax[2]; ig[2]++) {
            ...
            for (int ic = 0; ic < 3; ic++) {
              ...
              if (true_ig[ic] < 0) {
                true_ig[ic] = true_ig[ic] + ngrid[ic];
              }
            } // ic
            // Get charge values for each grid position
            const int id = true_ig[0] + true_ig[1] * ngrid[0] + true_ig[2] * ngrid[0] * ngrid[1];
            const double chgtot = chg_tot[id];
            const double chgdiff = chg_diff[id];
            const double chgup = 0.5 * (chg_tot[id] + chg_diff[id]);
            const double chgdn = 0.5 * (chg_tot[id] - chg_diff[id]);
            const vector<double> fpos(3);
            // Get distance from iat to true grid point position.
            const double dist = pflow::GetDistToAtomDir(str, chg_fpos, iat);
            // Get bin for this atom and increment radial charge density.
            const int ibin = (int) (dist / DR);
            if (ibin < NRBIN) {
    ...
    for (ig[0] = 0; ig[0] < ngrid[0]; ig[0]++) {
      for (ig[1] = 0; ig[1] < ngrid[1]; ig[1]++) {
        for (ig[2] = 0; ig[2] < ngrid[2]; ig[2]++) {
          ...
          const int id = ig[0] + ig[1] * ngrid[0] + ig[2] * ngrid[0] * ngrid[1];
          const double chgtot = chg_tot[id];
          const double chgdiff = chg_diff[id];
          const double chgup = 0.5 * (chg_tot[id] + chg_diff[id]);
          const double chgdn = 0.5 * (chg_tot[id] - chg_diff[id]);
    ...
    for (int iat = 0; iat < natoms; iat++) {
      ...
      for (int i = 1; i < 5; i++) {
        for (int ib = 0; ib < NRBIN; ib++) {
          ...
          if (ib > 0) {
            rad_chg_int[iat][ib][i] = (rad_chg_int[iat][ib][i] + rad_chg_int[iat][ib - 1][i]);
          }
        } // ib
...
// ***************************************************************************
namespace pflow {
  bool ReadCHGCAR(xstructure& str, stringstream& chgcar_header, vector<int>& ngrid, vector<int>& format_dim, vector<double>& chg_tot, vector<double>& chg_diff, stringstream& chgcar_ss, ostream& oss) {
    ...
    if (LDEBUG) {
      oss << __AFLOW_FUNC__ << "CHECKING FORMAT OF SYSTEM NAME AND SCALING FACTOR" << endl;
    }
    ...
    if (line_tokens.size() != 1) {
      oss << endl;
      oss << __AFLOW_FUNC__ << "ERROR: Unrecognized CHGCAR header format (scaling factor)." << endl;
      oss << endl;
      return false;
    }
    line_tokens.clear();
    if (LDEBUG) {
      oss << __AFLOW_FUNC__ << "FORMAT FOR SCALING FACTOR LOOKS OK" << endl;
    }

    // next three lines should contain lattice vectors
    if (LDEBUG) {
      oss << __AFLOW_FUNC__ << "CHECKING FORMAT FOR LATTICE VECTORS" << endl;
    }
    for (uint i = 2; i < 5; i++) {
      aurostd::string2tokens(vcontent.at(i), line_tokens, " ");
      if (line_tokens.size() != 3) {
        oss << endl;
        oss << __AFLOW_FUNC__ << "ERROR: Unrecognized CHGCAR header format (lattice vectors)." << endl;
        oss << endl;
        return false;
      }
    }
    line_tokens.clear();
    if (LDEBUG) {
      oss << __AFLOW_FUNC__ << "FORMAT FOR LATTICE VECTORS LOOKS OK" << endl;
    }
    ...
    line_tokens.clear();
    if (LDEBUG) {
      oss << __AFLOW_FUNC__ << "IS VASP4 FORMAT = " << is_vasp_4_poscar << endl;
    }
    ...
    if (LDEBUG) {
      oss << __AFLOW_FUNC__ << "READING POSCAR" << endl;
    }
    ...
    for (size_t i = 0; i < sum_tokens.size(); i++) {
      natoms += aurostd::string2utype<uint>(sum_tokens[i]);
    }
    ...
    // define xstruture
    if (LDEBUG) {
      oss << __AFLOW_FUNC__ << "DEFINE XSTRUCTURE" << endl;
    }
    str = poscar;
    if (LDEBUG) {
      oss << str << endl;
    ...
    if (LDEBUG) {
      oss << __AFLOW_FUNC__ << "READ CHG VALUES" << endl;
    }
    while (vcontent.size() > linecount) {
      ...
      if (vcontent.at(linecount).compare(0, 12, keyword) == 0) {
        if (LDEBUG) {
          oss << __AFLOW_FUNC__ << "GET FORMAT FOR AUGMENTATION OCCUPANCIES" << endl;
        }
        for (uint i = 0; i < natoms; i++) {
          ...
          if (line_tokens.size() != 4) {
            oss << endl;
            oss << __AFLOW_FUNC__ << "ERROR: Unrecognized CHGCAR format (augmentation occupancies)." << endl;
            oss << endl;
            return false;
          }
          ...
          if (LDEBUG) {
            oss << __AFLOW_FUNC__ << "SUCCESSFULLY GATHERED FORMAT FOR AUGMENTATION OCCUPANCIES" << endl;
          }
        }
      } else { // must be chg_tot or chg_diff
        npts = 1;
        ngrid.clear(); // clear for next round
        line_tokens.clear();
        ...
        if (sum_tokens.size() != 3) {
          oss << endl;
          oss << __AFLOW_FUNC__ << "ERROR: Unrecognized CHGCAR format (number of grid points)." << endl;
          oss << endl;
          return false;
        }
        for (size_t i = 0; i < sum_tokens.size(); i++) {
          grid_point = aurostd::string2utype<int>(sum_tokens[i]);
        ...
        if (!got_chgtot) {
          if (LDEBUG) {
            oss << __AFLOW_FUNC__ << "READING CHG_DIFF" << endl;
          }
          ...
          if (LDEBUG) {
            oss << __AFLOW_FUNC__ << "READING CHG_TOT" << endl;
          }
          chg_diff = vector<double>(npts, 1.0);
          if (npts != npts_compare) { // chg_tot and chg_diff have different number of points
            oss << endl;
            oss << __AFLOW_FUNC__ << "ERROR: Number of grid points for CHG_tot and CHG_diff do not match. " << endl;
            oss << __AFLOW_FUNC__ << "ERROR: npts_total: " << npts << endl;
            oss << __AFLOW_FUNC__ << "ERROR: npts_diff: " << npts_compare << endl;
          ...
          if (ngrid != ngrid_compare) { // chg_tot and chg_diff have different grid
            oss << endl;
            oss << __AFLOW_FUNC__ << "ERROR: Grids for CHG_tot and CHG_diff do not match. " << endl;
            oss << __AFLOW_FUNC__ << "ERROR: ngrid_total: " << ngrid.at(0) << " " << ngrid.at(1) << " " << ngrid.at(2) << endl;
            oss << __AFLOW_FUNC__ << "ERROR: ngrid_diff: " << ngrid_compare.at(0) << " " << ngrid_compare.at(1) << " " << ngrid_compare.at(2) << endl;
            oss << __AFLOW_FUNC__ << "ERROR: This will give nonsense!! " << endl;
            oss << endl;
            return false;
          }
        ...
        for (uint i = 0; i < npts; i += numcolumns) {
          line_tokens.clear();
          aurostd::string2tokens(vcontent.at(linecount), line_tokens, " ");
        ...
        line_tokens.clear();
        if (LDEBUG) {
          oss << __AFLOW_FUNC__ << "SUCCESSFULLY READ CHG VALUES" << endl;
        }
      }
    }
    if (LDEBUG) {
...
// ***************************************************************************
namespace pflow {
  bool ReadChg(xstructure& str, vector<int>& ngrid, vector<double>& chg_tot, vector<double>& chg_diff, istream& chgfile) {
...
namespace pflow {
  void GetChgInt(vector<aurostd::matrix<double>>& rad_chg_int, // CO20200404 pflow::matrix()->aurostd::matrix()
    ...
    const aurostd::matrix<double> dum_mat(NRBIN, 5, 0.0); // CO20200404 pflow::matrix()->aurostd::matrix()
    const int natoms = pflow::GetNumAtoms(str);
    const int npts = ngrid[0] * ngrid[1] * ngrid[2];
    rad_chg_int = vector<aurostd::matrix<double>>(natoms, dum_mat); // CO20200404 pflow::matrix()->aurostd::matrix()
    vector<int> ig(3);
    // Determine the number of images along each lattice vector which must be considered.
    // You will center the charge density on each atom so the max number of images along a
...
namespace pflow {
  ...
  void ReadPlaneDensParams(const xstructure& str, pd_params& pdp, istream& infile) {
    ...
    for (int ic = 0; ic < 3; ic++) {
      ...
      if (pdp.type[0] == 'D' || pdp.type[0] == 'd') {
        pdp.pts[ic] = pflow::vecF2C(pflow::GetLat(str), pdp.pts[ic]);
      }
    }
    ...
    if (pdp.ortho[0] == 'O' || pdp.ortho[0] == 'o') {
      const double norm_orig = norm(pdp.dpts[2]);
      pdp.dpts[2] = pflow::CompAperpB(pdp.dpts[2], pdp.dpts[1]);
      const double norm_new = norm(pdp.dpts[2]);
      if (norm_new < TOL) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "two axis are parallel and no plane can be determined", _INPUT_ILLEGAL_); // CO20200624
      }
    ...
    if (pdp.orig_loc[0] == 'M' || pdp.orig_loc[0] == 'm') {
      const vector<double> shift = pflow::SVprod(0.5, VVsum(pdp.dpts[1], pdp.dpts[2]));
    ...
    for (int ic = 0; ic < 3; ic++) {
      ...
      // rescale coordinates
      if (ic > 0) {
        pdp.dpts[ic] = pflow::SVprod(pdp.scale, pdp.dpts[ic]);
      }
    }
  }
} // namespace pflow
...
namespace pflow {
  void GetPlaneDens(const pd_params& pdp, vector<double>& dens2d_tot, vector<double>& dens2d_diff, const xstructure& str, const vector<int>& ngrid, const vector<double>& chg_tot, const vector<double>& chg_diff) {
    if (!str.atoms.empty()) {
      ;
    } // phony just to keep str busy
    const int Nx = pdp.Nx;
    ...
    for (int i = 0; i < Nx; i++) {
      for (int j = 0; j < Ny; j++) {
        ...
        const int id3d = ii + ngrid[0] * jj + ngrid[0] * ngrid[1] * kk;
...
// ***************************************************************************
namespace pflow {
  void PrintPlaneDens(const pd_params& pdp, const vector<double>& dens2d_tot, const vector<double>& dens2d_diff, const xstructure& str) {
    if (!str.atoms.empty()) {
      ;
    } // phony just to keep str busy
...
namespace pflow {
  void Ewald(const xstructure& in_str, double& epoint, double& ereal, double& erecip, double& eewald, double& eta, const double& SUMTOL) {
    ...
    str = PutInCell(str);
    const aurostd::matrix<double> lat = pflow::GetScaledLat(str); // CO20200404 pflow::matrix()->aurostd::matrix()
    const aurostd::matrix<double> rlat = pflow::RecipLat(lat); // CO20200404 pflow::matrix()->aurostd::matrix()
    const int natoms = pflow::GetNumAtoms(str);
    const double vol = pflow::GetVol(lat);
    const aurostd::matrix<double> atfpos = pflow::GetFpos(str); // CO20200404 pflow::matrix()->aurostd::matrix()
    const vector<int> attyp = GetTypes(str);
    ...
    if (eta <= 0) {
      eta = GetEta(natoms, vol);
    }
    // Constants
    const double rteta = sqrt(eta);
    // Point energy
...
namespace pflow {
  double GetEta(const int& natoms, const double& vol) {
    const double term = std::pow((double) (5.5 * natoms / vol / vol), 1.0 / 6.0);
...
namespace pflow {
  double GetRecipEner(const double& eta,
                      const vector<double>& atchg,
                      const double& vol,
                      const aurostd::matrix<double>& rlat, // CO20200404 pflow::matrix()->aurostd::matrix()
                      const aurostd::matrix<double>& atfpos,
                      const double& SUMTOL) { // CO20200404 pflow::matrix()->aurostd::matrix()
    const double TOL = 1e-16;
    const double log_eps = -30; // This assumes sf<
    double arg = 0;
    double erecip = 0;
    int gcnt = 1;
      ...
      for (ig[0] = -gcnt; ig[0] <= gcnt; ig[0]++) {
        for (ig[1] = -gcnt; ig[1] <= gcnt; ig[1]++) {
          for (ig[2] = -gcnt; ig[2] <= gcnt; ig[2]++) {
            if ((std::abs(ig[0]) == gcnt) || (std::abs(ig[1]) == gcnt) || (std::abs(ig[2]) == gcnt) || gcnt == 1) { // Just does new shells
              const vector<double> gvec = pflow::VMmult(ig, rlat);
              const double gsq = pflow::VVdot(gvec, gvec);
              if (gsq > TOL) { // Avoid gsq=0.
                // get structure factor
                double sfr = 0;
                double sfi = 0;
                for (int ia = 0; ia < (int) atfpos.size(); ia++) {
                  const double exparg = TWOPI * pflow::VVprod(atfpos[ia], ig);
                  sfr = sfr + atchg[ia] * cos(exparg);
                  sfi = sfi + atchg[ia] * sin(exparg);
                }
                if (aurostd::abs(sfr) < 1e-16) {
                  sfr = 0.0;
                }
                if (aurostd::abs(sfi) < 1e-16) {
                  sfi = 0.0;
                }
                const double sf = sfr * sfr + sfi * sfi;
                ...
                if (aurostd::abs(term) > maxterm) {
                  maxterm = aurostd::abs(term);
                }
                if (expval > maxterm) {
                  maxterm = expval;
                }
...
namespace pflow {
  double GetRealEner(const double& eta,
                     const vector<double>& atchg,
                     const double& vol,
                     const aurostd::matrix<double>& lat, // CO20200404 pflow::matrix()->aurostd::matrix()
                     const aurostd::matrix<double>& atfpos,
                     const double& SUMTOL) { // CO20200404 pflow::matrix()->aurostd::matrix()
    if (vol) {
      ;
    } // phony just to keep vol busy

    double ereal = 0;
    const double TOL = 1e-5;
    const double log_eps = -30;
    double erfcarg = 0;
    const double rteta = sqrt(eta);
    const int nat = atfpos.size();
    int rcnt = 1;
    int rcont = 1;
    while (rcont) {
      double maxterm = 0;
      ...
      for (ir[0] = -rcnt; ir[0] <= rcnt; ir[0]++) {
        for (ir[1] = -rcnt; ir[1] <= rcnt; ir[1]++) {
          for (ir[2] = -rcnt; ir[2] <= rcnt; ir[2]++) {
            if ((std::abs(ir[0]) == rcnt) || (std::abs(ir[1]) == rcnt) || (std::abs(ir[2]) == rcnt) || rcnt == 1) { // Just does new shells
              for (int ia = 0; ia < nat; ia++) { // All atoms in unit cell
                for (int ja = 0; ja < nat; ja++) { // All neighbors in cell given by ir
                  ...
                  const double dist = pflow::norm(disp);
                  if (dist > TOL) { // Avoid atom dist to itself.
                    ...
                    // for large x.
                    double erfcval;
                    const double term1 = atchg[ia] * atchg[ja] / dist;
                    double term2 = 0;
                    if (std::abs(term1) > 0) {
                      term2 = log(std::abs(term1)) - erfcarg * erfcarg - log(erfcarg) - 0.5 * log(PI);
                    }
                    ...
                    const double term = term1 * erfcval;
                    ereal = ereal + term;
                    if (aurostd::abs(term) > maxterm) {
                      maxterm = aurostd::abs(term);
                    }
            ...
            if ((std::abs(ir[0]) == rcnt) || (std::abs(ir[1]) == rcnt) || (std::abs(ir[2]) == rcnt) || rcnt == 1) { // Just does new shells
              for (int ia = 0; ia < nat; ia++) { // All atoms in unit cell
                for (int ja = 0; ja < nat; ja++) { // All neighbors in cell given by ir
                  ...
                  const double dist = pflow::norm(disp);
                  if (dist > TOL) { // Avoid atom dist to itself.
                    const double term = exp(-Ks * dist) * atchg[ia] * atchg[ja] / dist;
                    ereal = ereal + term;
                    if (aurostd::abs(term) > maxterm) {
                      maxterm = aurostd::abs(term);
                    }
...
namespace pflow {
  void BZMAX(istream& input) {
    ...
    b1[1] = str_sp.klattice(1, 1);
    b1[2] = str_sp.klattice(1, 2);
    b1[3] = str_sp.klattice(1, 3);
    b2[1] = str_sp.klattice(2, 1);
    b2[2] = str_sp.klattice(2, 2);
    b2[3] = str_sp.klattice(2, 3);
    b3[1] = str_sp.klattice(3, 1);
    b3[2] = str_sp.klattice(3, 2);
    b3[3] = str_sp.klattice(3, 3);
    ...
    int count = 0;
    int j = 0; // count is the number of rows of kpoints
...
namespace pflow {
  ...
  string prettyPrintCompound(const string& compound, vector_reduction_type vred, bool exclude1, filetype ftype) { // char mode  //CO20190629
    ...
    const vector<string> vspecies = aurostd::getElements(compound, vcomposition);
  ...
  string prettyPrintCompound(const vector<string>& vspecies, const vector<uint>& vcomposition, vector_reduction_type vred, bool exclude1, filetype ftype) { // overload //char mode //DX20200727
    ...
    for (size_t i = 0; i < vcomposition.size(); i++) {
      vcomposition_dbl.push_back((double) vcomposition[i]);
    }
  ...
  string prettyPrintCompound(const vector<string>& vspecies, const xvector<double>& vcomposition, vector_reduction_type vred, bool exclude1, filetype ftype) { // main function //char mode //CO20190629
    ...
    if (LDEBUG) {
      ...
      cerr << __AFLOW_FUNC__ << " vcomposition=" << aurostd::joinWDelimiter(aurostd::xvecDouble2vecString(vcomposition), ",") << endl;
    }
    ...
    if (vspecies.size() == 1) {
      output << vspecies[0];
      if (!exclude1) {
        output << (vred == gcd_vrt ? 1 : vcomposition[vcomposition.lrows]);
      }
      return output.str();
    }
    const xvector<double> comp = vcomposition;
    xvector<double> final_comp = comp; // DX20191125
    if (vred == gcd_vrt) {
      aurostd::reduceByGCD(comp, final_comp, ZERO_TOL);
    } // DX20191125 - new function form
    else if (vred == frac_vrt) {
      final_comp = aurostd::normalizeSumToOne(comp, ZERO_TOL);
    } else if (vred == no_vrt) {
      ;
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown reduce mode", _INPUT_UNKNOWN_);
    }
    if (std::abs(aurostd::sum(final_comp)) < ZERO_TOL) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Empty composition");
    }
    for (size_t i = 0, fl_size_i = vspecies.size(); i < fl_size_i; i++) {
      output << vspecies[i];
      if (!(exclude1 && aurostd::identical(final_comp[i + final_comp.lrows], 1.0, ZERO_TOL))) {
        if (ftype == latex_ft) {
          output << "$_{"; // mode==_latex_ //CO20190629
        } else if (ftype == gnuplot_ft) {
          output << "_{";
        } // mode==_gnuplot_ //CO20190629
        output << final_comp[i + final_comp.lrows];
        if (ftype == latex_ft) {
          output << "}$";
        } // mode==_latex_ //CO20190629
        else if (ftype == gnuplot_ft) {
          output << "}";
        } // mode==_gnuplot_ //CO20190629
      }
...
namespace pflow {
  ...
  bool ANode::isStatus(const node_status& status) const {
    ...
    //[keep silent]  //status
    //[keep silent]  if(status==NODE_FREE){cerr << __AFLOW_FUNC__ << " status==NODE_FREE" << endl;}
    //[keep silent]  else if(status==NODE_OCCUPIED){cerr << __AFLOW_FUNC__ << " status==NODE_OCCUPIED" << endl;}
    //[keep silent]  else if(status==NODE_FULL){cerr << __AFLOW_FUNC__ << " status==NODE_FULL" << endl;}
    //[keep silent]  else if(status==NODE_DOWN){cerr << __AFLOW_FUNC__ << " status==NODE_DOWN" << endl;}
    //[keep silent]  else if(status==NODE_OFFLINE){cerr << __AFLOW_FUNC__ << " status==NODE_OFFLINE" << endl;}
    //[keep silent]  else if(status==NODE_OPERATIONAL){cerr << __AFLOW_FUNC__ << " status==NODE_OPERATIONAL" << endl;}
    //[keep silent]  else if(status==NODE_NONOPERATIONAL){cerr << __AFLOW_FUNC__ << " status==NODE_NONOPERATIONAL" << endl;}
    //[keep silent]  //m_status
    //[keep silent]  if(m_status==NODE_FREE){cerr << __AFLOW_FUNC__ << " m_status==NODE_FREE" << endl;}
    //[keep silent]  else if(m_status==NODE_OCCUPIED){cerr << __AFLOW_FUNC__ << " m_status==NODE_OCCUPIED" << endl;}
    //[keep silent]  else if(m_status==NODE_FULL){cerr << __AFLOW_FUNC__ << " m_status==NODE_FULL" << endl;}
    //[keep silent]  else if(m_status==NODE_DOWN){cerr << __AFLOW_FUNC__ << " m_status==NODE_DOWN" << endl;}
    //[keep silent]  else if(m_status==NODE_OFFLINE){cerr << __AFLOW_FUNC__ << " m_status==NODE_OFFLINE" << endl;}
    //[keep silent]}
    if (status == NODE_OPERATIONAL && (m_status == NODE_FREE || m_status == NODE_OCCUPIED || m_status == NODE_FULL)) {
      return true;
    } else if (status == NODE_NONOPERATIONAL && (m_status == NODE_DOWN || m_status == NODE_OFFLINE)) {
      return true;
    }
...
namespace pflow {
  ...
  uint AQueue::getNNodes() const {
    return m_nodes.size();
  }
  uint AQueue::getNCPUS() const {
    uint ncpus = 0;
    for (size_t i = 0; i < m_nodes.size(); i++) {
      ncpus += m_nodes[i].m_ncpus;
    }
    return ncpus;
  }
  uint AQueue::getNNodes(const APartition& partition) const {
    return partition.m_inodes.size();
  }
  uint AQueue::getNCPUS(const APartition& partition) const {
    uint ncpus = 0;
    for (size_t i = 0; i < partition.m_inodes.size(); i++) {
  ...
  uint AQueue::getNNodes(const APartition& partition, const node_status& status) const {
    uint nnodes = 0;
    for (size_t i = 0; i < partition.m_inodes.size(); i++) {
      if (m_nodes[partition.m_inodes[i]].isStatus(status)) {
        nnodes += 1;
      }
    }
    return nnodes;
  ...
  uint AQueue::getNCPUS(const string& user, const string& partition, const job_status& status) const {
    const uint ipartition = partitionName2Index(partition);
    if (ipartition > m_partitions.size() - 1) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "ipartition>m_partitions.size()-1", _INDEX_BOUNDS_);
    }
  ...
  uint AQueue::getNCPUS(const string& user, const APartition& partition, const job_status& status) const {
    const bool LDEBUG = (false || _AQUEUE_DEBUG_ || XHOST.DEBUG);
    uint ncpus = 0;
    uint ijob = 0;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " partition=" << partition.m_name << endl;
    }
    for (size_t i = 0; i < partition.m_vijobs.size(); i++) {
      ijob = partition.m_vijobs[i];
      if (ijob > m_jobs.size() - 1) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "ijob>m_jobs.size()-1", _INDEX_BOUNDS_);
      }
      const AJob& job = m_jobs[ijob];
      if (job.m_user == user && job.m_status == status) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " job=" << job.m_id << " ncpus=" << job.m_ncpus << endl;
        }
        ncpus += job.m_ncpus;
      }
    }
  ...
  double AQueue::getPercentage(const string& user, const APartition& partition, const job_status& status) const {
    const uint denom = getNCPUS(partition, NODE_OPERATIONAL);
    if (denom == 0) {
      return 0.0;
    }
  ...
  uint AQueue::nodeName2Index(const string& name) const {
    for (size_t inode = 0; inode < m_nodes.size(); inode++) {
      if (m_nodes[inode].m_name == name) {
        return inode;
      }
    }
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No node found with name=" + name, _INPUT_UNKNOWN_);
  }
  uint AQueue::partitionName2Index(const string& name) const {
    for (size_t ipartition = 0; ipartition < m_partitions.size(); ipartition++) {
      if (m_partitions[ipartition].m_name == name) {
        return ipartition;
      }
    }
    throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No partition found with name=" + name, _INPUT_UNKNOWN_);
  }
  ...
  uint getTORQUEIDFromString(const string& torqueid_str) {
    ...
    if (tokens.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "tokens.size()==0 [1]", _RUNTIME_ERROR_);
    }
    tmp = tokens[0];
    ...
    if (tokens.size() == 1) {
      ...
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "tokens.size()!=1||2", _RUNTIME_ERROR_);
    }
    if (!aurostd::isfloat(tmp)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "!aurostd::isfloat(tmp)", _RUNTIME_ERROR_);
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " id=" << tmp << endl;
    }
  ...
  bool AQueue::addJob(const AJob& _job) { // add job to m_jobs
    ...
    for (i = 0; i < m_jobs.size() && found_job == false; i++) {
      if (m_jobs[i].m_id == _job.m_id) {
        // job found
        AJob& job = m_jobs[i];
        if (job.m_status != _job.m_status) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "job.m_status!=_job.m_status", _RUNTIME_ERROR_);
        } // mismatch in statuses
        const job_status& jstat = job.m_status;
        if (jstat == JOB_RUNNING && _job.m_vinodes.empty()) { // then where is it running? we should have node information (m_vinodes and m_vncpus)
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "RUNNING job=" + aurostd::utype2string(_job.m_id) + " has not been assigned to a node", _RUNTIME_ERROR_);
        }
        if (!_job.m_vinodes.empty()) { // node information is available for mapping to a node
          // to add a job, its record should have been taken from one node (at a time)
          if (_job.m_vinodes.size() != 1) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "_job.m_vinodes.size()!=1", _RUNTIME_ERROR_);
          }
          if (_job.m_vncpus.size() != 1) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "_job.m_vncpus.size()!=1", _RUNTIME_ERROR_);
          }
          if (_job.m_vinodes.size() != _job.m_vncpus.size()) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "_job.m_vinodes.size()!=_job.m_vncpus.size()", _RUNTIME_ERROR_);
          }
          ...
          for (j = 0; j < job.m_vinodes.size() && found_node == false; j++) {
            if (job.m_vinodes[j] == inode) {
              if (LDEBUG) {
                cerr << __AFLOW_FUNC__ << " adding " << ncpus << " cpus to job=" << job.m_id << ",inode=" << job.m_vinodes[j] << endl;
              }
              job.m_ncpus += ncpus;
          ...
          if (!found_node) {
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " job=" << job.m_id << " is also running on inode=" << inode << " with " << ncpus << " cpus" << endl;
            }
            job.m_ncpus += ncpus;
    ...
    if (!found_job) {
      ...
      m_jobs.back().m_index = m_jobs.size() - 1;
      job_added = true;
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " adding NEW job=" << m_jobs.back().m_id << endl;
      }
    }
    return job_added;
  }
  ...
  bool AQueue::addPartition(const APartition& _partition) {
    ...
    for (i = 0; i < m_partitions.size(); i++) {
      if (m_partitions[i].m_name == _partition.m_name) {
        return false;
      }
    }
    bool partition_added = false;
    m_partitions.push_back(_partition);
    m_partitions.back().m_index = m_partitions.size() - 1;
    partition_added = true;
    APartition& partition = m_partitions.back();
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " adding partition=" << partition.m_name << endl;
    }
    if (partition.m_properties_node.empty()) { // only available if user is root
      ...
      //[resort to default]throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"Not sure how to define partitions for HOST="+XHOST.hostname,_RUNTIME_ERROR_);
      if (XHOST.hostname == "qrats.materials.duke.edu") { // protection, ideally the aflow user would be "root"
        // hack
        if (partition.m_name == "priority") {
          partition.m_properties_node = "sharedCompute";
        } else if (partition.m_name == "debug") {
          partition.m_properties_node = "debug";
        } else if (partition.m_name == "batch") {
          partition.m_properties_node = "sharedCompute";
        } else if (partition.m_name == "research") {
          partition.m_properties_node = "sharedCompute";
        } else {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown queue=" + partition.m_name, _RUNTIME_ERROR_);
        }
      }
    }
...
#define DELIM_PROPERTIES_NODE ","
  ...
  bool AQueue::addNode(const ANode& _node) {
    ...
    for (inode = 0; inode < m_nodes.size(); inode++) {
      if (m_nodes[inode].m_name == _node.m_name) {
        ...
        if (node.m_properties != _node.m_properties &&
            node.m_properties.find(_node.m_properties) == string::npos) { // QUSER patch: a comma-separated string is important for quser where each node is listed N times for each partition it is associated with
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " adding properties=\"" << _node.m_properties << "\" to node=" << node.m_name << " properties (NEW properties=" << node.m_properties << ")" << endl;
          }
        }
        return false;
      }
    ...
    if (node.m_status == NODE_FREE && node.m_ncpus_occupied > 0) {
      node.m_status = NODE_OCCUPIED;
    }
    if (node.m_status == NODE_OCCUPIED && node.m_ncpus_occupied >= node.m_ncpus) {
      node.m_status = NODE_FULL;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " adding ";
      if (node.m_status == NODE_FREE) {
        cerr << "FREE";
      } else if (node.m_status == NODE_OCCUPIED) {
        cerr << "OCCUPIED";
      } else if (node.m_status == NODE_FULL) {
        cerr << "FULL";
      } else if (node.m_status == NODE_DOWN) {
        cerr << "DOWN";
      } else if (node.m_status == NODE_OFFLINE) {
        cerr << "OFFLINE";
      } else {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown node.status", _RUNTIME_ERROR_);
      }
    ...
    for (ipartition = 0; ipartition < m_partitions.size(); ipartition++) {
      ...
      for (iproperty = 0; iproperty < vproperties_node.size(); iproperty++) {
        if (vproperties_node[iproperty] == partition.m_properties_node) {
          ...
          found = true;
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " mapping node=" << node.m_name << " to partition=" << partition.m_name << endl;
          }
        }
      }
    }
    if (!found) {
      APartition _partition;
      _partition.free();
      _partition.m_name = node.m_properties;
      // set m_properties_node inside addPartition()
      if (!addPartition(_partition)) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Partition already exists", _RUNTIME_ERROR_);
      }
  ...
  void AQueue::jobMapping(AJob& job) {
    ...
    if (job.m_vipartitions.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "job=" + aurostd::utype2string(job.m_id) + " has not been assigned a partition", _INPUT_ERROR_);
    }
    for (i = 0; i < job.m_vipartitions.size(); i++) {
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " mapping job=" << job.m_id << " to partition=" << partition.m_name << endl;
      }
    }
    // map RUNNING job to node
    if (job.m_status == JOB_RUNNING && job.m_vinodes.empty()) { // then where is it running? we should have node information (m_vinodes and m_vncpus)
      ...
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "RUNNING job=" + aurostd::utype2string(job.m_id) + " has not been assigned a node", _RUNTIME_EXTERNAL_FAIL_); // this will issue a re-run
    }
    if (!job.m_vinodes.empty()) {
      uint inode = 0;
      for (i = 0; i < job.m_vinodes.size(); i++) {
        inode = job.m_vinodes[i];
        if (inode > m_nodes.size() - 1) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "inode>m_nodes.size()-1", _INDEX_BOUNDS_);
        }
        ANode& node = m_nodes[inode];
        node.m_vijobs.push_back(job.m_index);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " mapping job=" << job.m_id << " to node=" << node.m_name << endl;
        }
        ...
        //[WRONG - job maps to node and node to partitions, but job maps to 1 (usually) partition]  if(ipartition>m_partitions.size()-1){throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"ipartition>m_partitions.size()-1",_INDEX_BOUNDS_);}
        ...
        //[WRONG - job maps to node and node to partitions, but job maps to 1 (usually) partition]  if(LDEBUG){cerr << __AFLOW_FUNC__ << " mapping job=" << job.m_id << " to partition=" << partition.m_name << endl;}
...
#define ATTEMPTS_GETQUEUE_MAX 10
  void AQueue::getQueue() {
    const bool LDEBUG = (true || _AQUEUE_DEBUG_ || XHOST.DEBUG);
    ...
    while (attempts++ < ATTEMPTS_GETQUEUE_MAX && SUCCESS == false) {
      try {
        ...
        SUCCESS = false;
        if (err.whatCode() != _RUNTIME_EXTERNAL_FAIL_) {
          attempts = ATTEMPTS_GETQUEUE_MAX;
        }
        if (attempts < ATTEMPTS_GETQUEUE_MAX) {
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " failed attempt, trying to process queue again" << endl;
          }
          aurostd::Sleep(2);
        } else {
          if (SUCCESS == false) {
  ...
  void AQueue::readNodesPartitionsSLURM() {
    ...
    if (!aurostd::IsCommandAvailable("sinfo")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "no sinfo command found for SLURM configuration", _RUNTIME_EXTERNAL_MISS_);
    }
    ...
    for (iline = 0; iline < lines.size(); iline++) {
      if (lines[iline].find("NODELIST") != string::npos) {
        continue;
      } // skip date and header
      ...
      if (tokens.size() < 11) {
        continue;
      } // skip date and header //throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"tokens.size()<11",_RUNTIME_ERROR_);
      _node.m_name = tokens[0];
      // tokens[2] is partition, addPartition() takes care of checking that it's not already in the list
      _partition.free();
      _partition.m_name = tokens[2];
      aurostd::StringSubstInPlace(_partition.m_name, "*", ""); // this signifies default queue, we don't care
      addPartition(_partition);
      _node.m_properties = _partition.m_name; // for node-partition mapping() later
      // tokens[3] is status
      if (tokens[3].find('*') != string::npos) {
        _node.m_status = NODE_DOWN;
      } // catch first, doesn't matter what status it is in, IT'S NOT RESPONDING
      else if (tokens[3] == "allocated+" || tokens[3] == "allocated" || tokens[3] == "alloc" || tokens[3] == "completing" || tokens[3] == "comp" || false) {
        _node.m_status = NODE_FULL;
      } // most likely to appear first, quicker to appear at the top
      else if (tokens[3] == "idle") {
        _node.m_status = NODE_FREE;
      } else if (tokens[3] == "mixed" || tokens[3] == "mix") {
        _node.m_status = NODE_OCCUPIED;
      } else if (tokens[3] == "reserved" || tokens[3] == "resv" || tokens[3] == "unknown" || tokens[3] == "unk") {
        _node.m_status = NODE_OFFLINE;
      } else if (tokens[3] == "down" || tokens[3] == "drained" || tokens[3] == "drain" || tokens[3] == "draining" || tokens[3] == "drng" || tokens[3] == "fail" || tokens[3] == "failing" ||
                 tokens[3] == "failg" || tokens[3] == "future" || tokens[3] == "futr" || tokens[3] == "maint" || tokens[3] == "reboot" || tokens[3] == "perfctrs" || tokens[3] == "npc" ||
                 tokens[3] == "power_down" || tokens[3] == "powering_down" || tokens[3] == "pow_dn" || tokens[3] == "power_up" || tokens[3] == "powering_up" || tokens[3] == "pow_up" ||
                 tokens[3] == "no_respond" || false) {
        _node.m_status = NODE_DOWN;
      } // leave for last, many variants to consider
      // tokens[4] is ncpus
      if (!aurostd::isfloat(tokens[4])) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "tokens[4] is NOT a float (ncpus)", _RUNTIME_ERROR_);
      }
      _node.m_ncpus = aurostd::string2utype<double>(tokens[4]);
      addNode(_node);
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " found " << m_partitions.size() << " partitions" << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " found " << m_nodes.size() << " nodes" << endl;
    }
  }
  ...
  void AQueue::readJobsSLURM() {
    ...
    if (!aurostd::IsCommandAvailable("squeue")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "no squeue command found for SLURM configuration", _RUNTIME_EXTERNAL_MISS_);
    }
    ...
    _job.free();
    for (iline = 0; iline < lines.size(); iline++) {
      if (lines[iline].find("JOBID") != string::npos) {
        continue;
      } // skip date and header
      line = aurostd::RemoveWhiteSpacesFromTheFrontAndBack(lines[iline]);
      if (line.empty()) {
      ...
      if (!aurostd::isfloat(tokens[0])) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "!aurostd::isfloat(tokens[0])", _RUNTIME_ERROR_);
      }
      _job.free();
      ...
      if (tokens[4] == "PD") {
        _job.m_status = JOB_QUEUED;
      } else if (tokens[4] == "R" || tokens[4] == "ST") {
        _job.m_status = JOB_RUNNING;
      } // ST because it retains cores
      else if (tokens[4] == "S") {
        _job.m_status = JOB_HELD;
      } // S because cores are given up for other jobs (transition state)
      else if (tokens[4] == "CD" || tokens[4] == "CG" || tokens[4] == "F" || tokens[4] == "PR" || false) {
        _job.m_status = JOB_DONE;
      } else {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown job status=" + tokens[4], _RUNTIME_ERROR_);
      }
      // tokens[7] is ncpus
      if (!aurostd::isfloat(tokens[7])) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "!aurostd::isfloat(tokens[7])", _RUNTIME_ERROR_);
      }
  ...
  void AQueue::readPartitionsTORQUE() {
    ...
    if (!aurostd::IsCommandAvailable("qstat")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "no qstat command found for TORQUE configuration", _RUNTIME_EXTERNAL_MISS_);
    }
    ...
    for (iline = 0; iline < lines.size(); iline++) {
      ...
      if (line.find("Queue:") != string::npos) {
        if (!_partition.m_name.empty()) {
          addPartition(_partition);
        }
        aurostd::string2tokens(line, tokens, ":");
        if (tokens.size() != 2) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "tokens.size()!=2", _RUNTIME_ERROR_);
        }
        ...
        if (tokens.size() != 2) {
          continue;
        } // sometimes we get garbage lines like 'lete:0' //throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"!tokens.size()",_RUNTIME_ERROR_);
        key = aurostd::RemoveWhiteSpacesFromTheFrontAndBack(tokens[0]);
        value = aurostd::RemoveWhiteSpacesFromTheFrontAndBack(tokens[1]);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " key=" << key << endl;
        }
        if (key == "resources_default.neednodes") {
          _partition.m_properties_node = value;
        } // really only works for root, but we leave here for now
      }
    }
    if (!_partition.m_name.empty()) {
      addPartition(_partition);
  ...
  void AQueue::readNodesJobsTORQUE() {
    ...
    if (!aurostd::IsCommandAvailable("pbsnodes")) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "no pbsnodes command found for TORQUE configuration", _RUNTIME_EXTERNAL_MISS_);
    }
    ...
    for (iline = 0; iline < lines.size(); iline++) {
      ...
      if (line.find('=') == string::npos) {
        if (!_node.m_properties.empty()) {
          addNode(_node);
        } // we have node information loaded up
        ...
        if (key == "state") {
          ...
          if (value.find("down") != string::npos) {
            _node.m_status = NODE_DOWN;
          } // down first so we can rectify
          else if (value.find("offline") != string::npos) {
            _node.m_status = NODE_OFFLINE;
          } // offline next
          else if (value.find("job-exclusive") != string::npos) {
            _node.m_status = NODE_OCCUPIED;
          } else if (value.find("free") != string::npos) {
            _node.m_status = NODE_FREE;
          } else {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "unknown state from pbsnodes: state=\"" + value + "\"", _RUNTIME_ERROR_); // more states to account for
          }
        } else if (key == "np") {
          _node.m_ncpus = aurostd::string2utype<uint>(value);
        } else if (key == "properties") {
          _node.m_properties = value;
        } // needed to match with queue
        else if (key == "jobs") {
  ...
  void AQueue::readJobsTORQUE() {
    ...
    for (iline = 0; iline < lines.size(); iline++) {
      if (lines[iline].find("Job ID") != string::npos) {
        continue;
      } // skip header lines
      line = aurostd::RemoveWhiteSpacesFromTheFrontAndBack(lines[iline]);
      if (line.empty()) {
      ...
      if (tmp.find("---") != string::npos) {
        continue;
      } // skip header //must be longer than 2 --, see 'Req'd Memory'
      ...
      for (i = 0; i < m_jobs.size() && found == false; i++) {
        if (m_jobs[i].m_id == id) {
          ...
          if (LDEBUG) {
            cerr << __AFLOW_FUNC__ << " adding user=" << m_jobs[i].m_user << " to job=" << m_jobs[i].m_id << endl;
          }
      ...
      if (found == false) { // add job
        ...
        // tokens[6] has ncpus
        if (!aurostd::isfloat(tokens[6])) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "!aurostd::isfloat(tokens[6])", _RUNTIME_ERROR_);
        }
        ...
        if (tokens[9] == "Q") {
          ...
          _job.m_status = JOB_RUNNING;
        } else if (tokens[9] == "H" || tokens[9] == "S") {
          _job.m_status = JOB_HELD;
        } else if (tokens[9] == "C" || tokens[9] == "E" || tokens[9] == "W") {
          _job.m_status = JOB_DONE;
        } // not sure about W...
        else {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown job status=" + tokens[9], _RUNTIME_ERROR_);
        } // T?
        addJob(_job);
      }
    }
  ...
  void AQueue::processQueue() {
    ...
    if (aurostd::IsCommandAvailable("squeue")) {
      m_qsys = QUEUE_SLURM;
    } else if (aurostd::IsCommandAvailable("pbsnodes")) {
      m_qsys = QUEUE_TORQUE;
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "cannot identify queuing system", _RUNTIME_ERROR_);
    }

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " m_qsys=";
      if (m_qsys == QUEUE_SLURM) {
        cerr << "SLURM";
      }
      if (m_qsys == QUEUE_TORQUE) {
        cerr << "TORQUE";
      }
      cerr << endl;
    }
    ...
    if (m_qsys == QUEUE_SLURM) {
      ...
      // if(LDEBUG){cerr << __AFLOW_FUNC__ << " XHOST.command(\"showq\")=" << XHOST.command("showq") << endl;}
    }
    ...
    // get job mappings
    for (size_t ijob = 0; ijob < m_jobs.size(); ijob++) {
    ...
    if (LDEBUG) {
      ...
      for (size_t inode = 0; inode < m_nodes.size(); inode++) {
        ANode& node = m_nodes[inode];
        cerr << __AFLOW_FUNC__ << " node=" << node.m_name << " has jobs=";
        for (size_t i = 0; i < node.m_vijobs.size(); i++) {
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " " << XHOST.hostname << " has " << getNNodes() << " nodes (" << getNCPUS() << " cpus)" << endl;
      bool add_comma = false;
      for (size_t i = 0; i < m_partitions.size(); i++) {
        add_comma = false;
        cerr << __AFLOW_FUNC__ << " " << m_partitions[i].m_name << ":";
        if (getNCPUS(m_partitions[i], NODE_FREE) > 0) {
          cerr << (add_comma ? "," : "") << " " << getNNodes(m_partitions[i], NODE_FREE) << " free nodes (" << getNCPUS(m_partitions[i], NODE_FREE) << " cpus)";
          if (add_comma == false) {
            add_comma = true;
          }
        }
        if (getNCPUS(m_partitions[i], NODE_OCCUPIED) > 0) {
          cerr << (add_comma ? "," : "") << " " << getNNodes(m_partitions[i], NODE_OCCUPIED) << " occupied nodes (" << getNCPUS(m_partitions[i], NODE_OCCUPIED, CPUS_FREE) << " cpus free, "
               << getNCPUS(m_partitions[i], NODE_OCCUPIED, CPUS_OCCUPIED) << " cpus occupied)";
          if (add_comma == false) {
            add_comma = true;
          }
        }
        if (getNCPUS(m_partitions[i], NODE_FULL) > 0) {
          cerr << (add_comma ? "," : "") << " " << getNNodes(m_partitions[i], NODE_FULL) << " full nodes (" << getNCPUS(m_partitions[i], NODE_FULL) << " cpus)";
          if (add_comma == false) {
            add_comma = true;
          }
        }
        if (getNCPUS(m_partitions[i], NODE_DOWN) > 0) {
          cerr << (add_comma ? "," : "") << " " << getNNodes(m_partitions[i], NODE_DOWN) << " down nodes (" << getNCPUS(m_partitions[i], NODE_DOWN) << " cpus)";
          if (add_comma == false) {
            add_comma = true;
          }
        }
        if (getNCPUS(m_partitions[i], NODE_OFFLINE) > 0) {
          cerr << (add_comma ? "," : "") << " " << getNNodes(m_partitions[i], NODE_OFFLINE) << " offline nodes (" << getNCPUS(m_partitions[i], NODE_OFFLINE) << " cpus)";
          if (add_comma == false) {
            add_comma = true;
          }
        }
        if (getNCPUS(m_partitions[i]) > 0) {
          cerr << (add_comma ? "," : "") << " " << getNNodes(m_partitions[i]) << " TOTAL nodes (" << getNCPUS(m_partitions[i]) << " cpus)";
          if (add_comma == false) {
            add_comma = true;
          }
        }
...
namespace pflow {
  vector<string> getFakeElements(uint nspecies) {
    ...
    if (nspecies > 26) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "There are more than 26 species, this function must be modified to include more fake elements.", _RUNTIME_ERROR_);
    }

    vector<string> elements;
    for (uint i = 0; i < nspecies; i++) {
      stringstream ss_letter;
      ss_letter << char('A' + i); // cannot type cast char to string directly //DX20200907 - use ASCII
...
namespace pflow {
  bool hasRealElements(const xstructure& xstr) {
    ...
    if (!xstr.species.empty()) {
      ...
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "The species are empty.", _INPUT_NUMBER_);
    }
...
namespace pflow {
  vector<double> getSymmetryToleranceSpectrum(const string& tolerance_range_string) {
    ...
    if (aurostd::string2tokens(tolerance_range_string, tokens, ":") == 3) {
      const double start = aurostd::string2utype<double>(tokens[0]);
      const double end = aurostd::string2utype<double>(tokens[1]);
      const uint nsteps = aurostd::string2utype<uint>(tokens[2]) - 1;
      if (end < start) {
      ...
      for (uint i = 0; i <= nsteps; i++) {
        tolerance_spectrum.push_back(start + ((double) i * interval));
      }
    } else {
...
namespace pflow {
  ...
  void outputAtomicEnvironment(const string& auid, uint aeMode, double radius) {
    ...
    if (!auid.empty() && aurl.empty()) {
      cerr << __AFLOW_FUNC__ << " Quick search failed! Trying standard methode." << endl;
    ...
    if (!aurl.empty()) {
      ...
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "could not load a structure", _INPUT_ERROR_);
    }
  ...
  uint getSpaceGroupSetting(const string& setting_string, uint mode_default) {
    ...
    if (!setting_string.empty()) {
      if (aurostd::tolower(setting_string) == "aflow" || aurostd::tolower(setting_string) == "anrl") {
        ...
        const int setting_num = aurostd::string2utype<int>(setting_string);
        if (setting_num == 1) {
          setting = SG_SETTING_1;
        } else if (setting_num == 2) {
          setting = SG_SETTING_2;
        }
        if (setting_num != SG_SETTING_1 && setting_num != SG_SETTING_2) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__,
                                "Setting must be 1, 2, or \"aflow\" (for rhombohedral systems: 1=rhl setting and 2=hex setting; for monoclinic systems: 1=unique axis-b and 2=unique axis-c).", _INPUT_ILLEGAL_);
        }
      }
    }
