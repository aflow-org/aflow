...
namespace KBIN {
  bool VASP_Produce_INPUT(_xvasp& xvasp, const string& AflowIn, ofstream& FileMESSAGE, _aflags& aflags, _kflags& kflags, _vflags& vflags, bool load_POSCAR_from_xvasp) {
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    if (AflowIn.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, "KBIN::VASP_Produce_INPUT():", "empty AflowIn", _FILE_CORRUPT_);
    } // CO20200624
    bool Krun = true;
    if (load_POSCAR_from_xvasp) {
      if (Krun) {
        Krun = (Krun && KBIN::VASP_Produce_POSCAR(xvasp));     // produce POSCAR before KPOINTS  //CO20180420 - good for POCC
      }
    } else {
      if (Krun) {
        Krun = (Krun && KBIN::VASP_Produce_POSCAR(xvasp, AflowIn, FileMESSAGE, aflags, kflags, vflags));     // produce POSCAR before KPOINTS
      }
    }
    if (Krun) {
      Krun = (Krun && KBIN::VASP_Produce_INCAR(xvasp, AflowIn, FileMESSAGE, aflags, kflags, vflags));
    }
    if (LDEBUG) {
      cerr << "VASP_Produce_INPUT: Checking if generate aflow.in only before calling VASP_Produce_POTCAR" << endl;  // CT20180719
    }
    if (!XHOST.GENERATE_AFLOWIN_ONLY) { // CT20180719
      if (LDEBUG) {
        cerr << "VASP_Produce_INPUT: Calling VASP_Produce_POTCAR" << endl;  // CT20180719
      }
      if (Krun) {
        Krun = (Krun && KBIN::VASP_Produce_POTCAR(xvasp, AflowIn, FileMESSAGE, aflags, kflags, vflags));
      }
    ...
    if (Krun) {
      Krun = (Krun && KBIN::VASP_Produce_KPOINTS(xvasp, AflowIn, FileMESSAGE, aflags, kflags, vflags));
    }
    return Krun;
  }
} // namespace KBIN
...
namespace KBIN {
  bool VASP_Modify_INPUT(_xvasp& xvasp, ofstream& FileMESSAGE, _aflags& aflags, _kflags& kflags, _vflags& vflags) {
    ...
    if (Krun) {
      Krun = (Krun && KBIN::VASP_Modify_INCAR(xvasp, FileMESSAGE, aflags, kflags, vflags));
    }
    if (LDEBUG) {
      cerr << "VASP_Modify_INPUT: Checking if generate aflow.in only before calling VASP_Produce_POTCAR" << endl; // CT20180719
    }
    if (!XHOST.GENERATE_AFLOWIN_ONLY) { // CT20180719
      if (LDEBUG) {
        cerr << "VASP_Modify_INPUT: Calling VASP_Modify_POTCAR" << endl;  // CT20180719
      }
      if (Krun) {
        Krun = (Krun && KBIN::VASP_Modify_POTCAR(xvasp, FileMESSAGE, aflags, vflags));
      }
    } // CT20180719
    if (Krun) {
      Krun = (Krun && KBIN::VASP_Modify_KPOINTS(xvasp, FileMESSAGE, aflags, vflags));
    }
    return Krun;
  }
} // namespace KBIN
...
namespace KBIN {
  bool VASP_Write_ppAUID_FILE(const string& directory, const vector<string>& vppAUIDs, const vector<string>& species) {
    ...
    for (size_t i = 0; i < vppAUIDs.size(); i++) {
      vWRITE.push_back(vppAUIDs[i] + " " + aurostd::PaddedPOST(aurostd::VASP_PseudoPotential_CleanName(species.at(i)), 3));
    }
  ...
  bool VASP_Write_ppAUID_AFLOWIN(const string& directory, const vector<string>& vppAUIDs, const vector<string>& species) {
    ...
    const string WRITE = "[VASP_POTCAR_AUID]" + aurostd::joinWDelimiter(vppAUIDs, ",");
    if (!aurostd::substring2bool(aurostd::file2string(directory + "/" + _AFLOWIN_), WRITE)) {
...
namespace KBIN {
  bool VASP_Write_INPUT(_xvasp& xvasp, _vflags& vflags, const string& ext_module) {
    ...
    if (Krun) {
      Krun = (Krun && aurostd::stringstream2file(xvasp.INCAR, string(xvasp.Directory + "/INCAR")));
    }
    if (Krun) {
      Krun = (Krun && aurostd::stringstream2file(xvasp.KPOINTS, string(xvasp.Directory + "/KPOINTS")));
    }
    if (Krun) {
      Krun = (Krun && aurostd::stringstream2file(xvasp.POTCAR, string(xvasp.Directory + "/POTCAR")));
    }
    if (Krun) {
      Krun = (Krun && VASP_Write_ppAUID_FILE(xvasp.Directory, xvasp.POTCAR_AUID, xvasp.str.species));
    }
    if (Krun) {
      Krun = (Krun && VASP_Write_ppAUID_AFLOWIN(xvasp.Directory, xvasp.POTCAR_AUID, xvasp.str.species));
    }

    // VASP BACKUP VASP WRITE
    if (Krun && xvasp.aopts.flag("FLAG::XVASP_POSCAR_changed")) {
      Krun = (Krun && aurostd::stringstream2file(xvasp.POSCAR_orig, string(xvasp.Directory + "/POSCAR.orig" + ext_module)));
    }
    if (Krun && xvasp.aopts.flag("FLAG::XVASP_INCAR_changed")) {
      Krun = (Krun && aurostd::stringstream2file(xvasp.INCAR_orig, string(xvasp.Directory + "/INCAR.orig" + ext_module)));
    }
    if (Krun && xvasp.aopts.flag("FLAG::XVASP_KPOINTS_changed")) {
      Krun = (Krun && aurostd::stringstream2file(xvasp.KPOINTS_orig, string(xvasp.Directory + "/KPOINTS.orig" + ext_module)));
    }
    if (Krun && xvasp.aopts.flag("FLAG::XVASP_POTCAR_changed")) {
      Krun = (Krun && aurostd::stringstream2file(xvasp.POTCAR_orig, string(xvasp.Directory + "/POTCAR.orig" + ext_module)));
    }
...
namespace KBIN {
  bool VASP_PseudoPotential_CleanName_TEST() { // CO20190712
    const bool LDEBUG = (true || XHOST.DEBUG);
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " compound_to_test=\"" << compound_to_test << "\", velements=" << aurostd::joinWDelimiter(aurostd::wrapVecEntries(velements, "\""), ",") << endl;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " compound_to_test=\"" << compound_to_test << "\", velements=" << aurostd::joinWDelimiter(aurostd::wrapVecEntries(velements, "\""), ",")
           << ", vcomposition=" << aurostd::joinWDelimiter(aurostd::vecDouble2vecString(vcomposition, 5), ",") << endl;
    }
    ...
    vector<string> element_parts;
    vector<string> _element_parts;
    string element_raw;
    string element_clean;
    string element;
    string suffix;
    aurostd::string2tokens(DEFAULT_VASP_POTCAR_DIRECTORIES, directories_2_search, ",");
    for (size_t i = 0; i < directories_2_search.size(); i++) {
      if (aurostd::IsDirectory(directories_2_search[i])) {
        const string& dir = directories_2_search[i];
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " searching dir=" << dir << endl;
        }
        aurostd::string2vectorstring(aurostd::execute2string("find " + dir + " -name POTCAR*"), potcar_paths);
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << " potcar_paths=" << aurostd::joinWDelimiter(potcar_paths, " ") << endl;
        }
        for (size_t j = 0; j < potcar_paths.size(); j++) {
          if (!aurostd::RemoveWhiteSpacesFromTheBack(potcar_paths[j]).empty()) {
            ...
            if (aurostd::WithinList(path_parts, "TESTS")) {
              continue;
            }  // skip VASP/TESTS
            if (aurostd::WithinList(path_parts, "BENCHS")) {
              continue;
            }  // skip VASP/BENCHS
            ...
            if (aurostd::substring2bool(element_raw, "runelements")) {
              continue;
            } // garbage pp
            if (aurostd::substring2bool(element_raw, "Free")) {
              continue;
            } // garbage pp
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " element_raw=" << element_raw << endl;
            }
            aurostd::string2tokens(element_raw, element_parts, "_");
            if (element_parts.empty()) {
              continue;
            }
            element = suffix = "";
            if (element_parts.size() > 1) {
              _element_parts.clear();
              for (size_t k = 1; k < element_parts.size(); k++) {
                _element_parts.push_back(element_parts[k]);
              } // skip element
              suffix = "_" + aurostd::joinWDelimiter(_element_parts, "_");
            }
            element = element_parts[0];
            element = aurostd::VASP_PseudoPotential_CleanName(element);  // clean H1.25
            if (LDEBUG) {
              cerr << __AFLOW_FUNC__ << " element=\"" << element << "\", suffix=\"" << suffix << "\"" << endl;
            }
            element_clean = aurostd::VASP_PseudoPotential_CleanName(element_raw);
            if (element != element_clean) { // idempotent
              message << "\"" << element << "\" != \"" << element_clean << "\" == aurostd::VASP_PseudoPotential_CleanName(\"" << element_raw << "\")";
              pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, message, aflags, FileMESSAGE, oss, _LOGGER_ERROR_);
              return false;
            }
...
namespace KBIN {
  bool VASP_SplitAlloySpecies(string alloy_in, string& speciesA, string& speciesB) {
    string alloy = aurostd::VASP_PseudoPotential_CleanName(aurostd::VASP_PseudoPotential_CleanName(alloy_in));
    speciesA = "";
    speciesB = "";
    int speciesN = 0;
...
namespace KBIN {
  bool VASP_SplitAlloySpecies(vector<string> alloy, vector<string>& speciesA, vector<string>& speciesB) {
    for (size_t k = 0; k < alloy.size(); k++) {
      speciesA.emplace_back("");
      speciesB.emplace_back("");
      KBIN::VASP_SplitAlloySpecies(alloy[k], speciesA.at(k), speciesB.at(k));
    }
    return true;
  }
} // namespace KBIN
...
namespace KBIN {
  bool VASP_SplitAlloySpecies(vector<string> alloy, vector<string>& speciesA, vector<string>& speciesB, vector<string>& speciesC) {
    for (size_t k = 0; k < alloy.size(); k++) {
      speciesA.emplace_back("");
      speciesB.emplace_back("");
      KBIN::VASP_SplitAlloySpecies(alloy[k], speciesA.at(k), speciesB.at(k), speciesC.at(k));
    }
    return true;
  }
} // namespace KBIN
...
namespace KBIN {
  bool VASP_SplitAlloyPseudoPotentials(vector<string> alloy, vector<string>& pseudosA, vector<string>& pseudosB) {
    for (size_t k = 0; k < alloy.size(); k++) {
      pseudosA.emplace_back("");
      pseudosB.emplace_back("");
      KBIN::VASP_SplitAlloyPseudoPotentials(alloy[k], pseudosA.at(k), pseudosB.at(k));
    }
    return true;
  }
} // namespace KBIN
...
namespace KBIN {
  bool VASP_SplitAlloyPseudoPotentials(vector<string> alloy, vector<string>& pseudosA, vector<string>& pseudosB, vector<string>& pseudosC) {
    for (size_t k = 0; k < alloy.size(); k++) {
      pseudosA.emplace_back("");
      pseudosB.emplace_back("");
      KBIN::VASP_SplitAlloyPseudoPotentials(alloy[k], pseudosA.at(k), pseudosB.at(k), pseudosC.at(k));
    }
    return true;
  }
} // namespace KBIN
...
namespace KBIN {
  bool VASP_Produce_INCAR(_xvasp& xvasp, const string& AflowIn, ofstream& FileMESSAGE, _aflags& aflags, _kflags& kflags, _vflags& vflags) { // AFLOW_FUNCTION_IMPLEMENTATION
    if (AflowIn.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "empty AflowIn", _FILE_NOT_FOUND_);
    }
    if (!kflags.AFLOW_MODE_VASP) {
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "kflags.AFLOW_MODE_VASP NOT set", std::cout, _LOGGER_WARNING_);
    }
    ostringstream aus;
    bool Krun = true;
    ...
    aus << "00000  MESSAGE INCAR   generation in " << xvasp.Directory << "  " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);

    const bool KBIN_VASP_INCAR_MODE_EMPTY = !vflags.KBIN_VASP_INCAR_MODE.flag("IMPLICIT") && !vflags.KBIN_VASP_INCAR_MODE.flag("EXPLICIT") && !vflags.KBIN_VASP_INCAR_MODE.flag("EXTERNAL");

    // IMPLICIT or EXPLICIT or EXTERNAL for INCAR
    Krun = (Krun && (vflags.KBIN_VASP_INCAR_MODE.flag("IMPLICIT") || vflags.KBIN_VASP_INCAR_MODE.flag("EXPLICIT") || vflags.KBIN_VASP_INCAR_MODE.flag("EXTERNAL") || KBIN_VASP_INCAR_MODE_EMPTY));
    if (!Krun) {
      aurostd::StringstreamClean(aus);
      aus << "EEEEE  [VASP_INCAR_MODE_IMPLICIT] or [VASP_INCAR_MODE_EXPLICIT] or [VASP_INCAR_MODE_EXTERNAL] must be specified " << Message(__AFLOW_FILE__, aflags) << endl; // ME20181113
    ...
    if (Krun && KBIN_VASP_INCAR_MODE_EMPTY) {  // [VASP_INCAR_MODE_EMPTY] construction
      aus << "00000  MESSAGE INCAR   generation EMPTY file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_INCAR_MODE.flag("IMPLICIT")) {  // [VASP_INCAR_MODE_IMPLICIT] construction
      aus << "00000  MESSAGE INCAR   generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_INCAR_MODE.flag("EXPLICIT")) {  // [VASP_INCAR_MODE_EXPLICIT] construction
      if (vflags.KBIN_VASP_INCAR_FILE.flag("KEYWORD") && !vflags.KBIN_VASP_INCAR_MODE.flag("EXPLICIT_START_STOP")) {
        aus << "00000  MESSAGE INCAR   generation EXPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        ...
        aus << "00000  MESSAGE INCAR   generation EXPLICIT file from " << _AFLOWIN_ << " with START/STOP  " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        ...
        aus << "EEEEE  [VASP_INCAR_MODE_EXPLICIT] do not confuse aflow !!" << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  [VASP_INCAR_MODE_EXPLICIT] Possible modes " << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "----------------------------------------------------------------------------------------------------" << endl;
        ...
        aus << "[AFLOW]" << endl;
        aus << "----------------------------------------------------------------------------------------------------" << endl;
        aus << "EEEEE  [VASP_INCAR_MODE_EXPLICIT] Note " << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  [VASP_INCAR_MODE_EXPLICIT]START must be present and no [VASP_INCAR_FILE]" << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  [VASP_INCAR_MODE_EXPLICIT]STOP  must be present and no [VASP_INCAR_FILE]" << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  or [VASP_INCAR_FILE] present and NO START/STOP" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      }
    ...
    if (Krun && vflags.KBIN_VASP_INCAR_MODE.flag("EXTERNAL")) {  // [VASP_INCAR_MODE_EXTERNAL] construction
      string file;
      aus << "00000  MESSAGE INCAR   generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      ...
      if (vflags.KBIN_VASP_INCAR_FILE.flag("COMMAND") && vflags.KBIN_VASP_INCAR_FILE.flag("FILE")) {
        aus << "EEEEE   [VASP_INCAR_MODE]FILE=  and  [VASP_INCAR_MODE]COMMAND=  can not be used together " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      }
      if (!vflags.KBIN_VASP_INCAR_FILE.flag("COMMAND") && (vflags.KBIN_VASP_INCAR_FILE.flag("FILE") || !vflags.KBIN_VASP_INCAR_FILE.flag("FILE"))) {
        if (vflags.KBIN_VASP_INCAR_FILE.flag("FILE")) {
          ...
          aus << "00000  MESSAGE INCAR   generation from DEFAULT file=" << DEFAULT_VASP_EXTERNAL_INCAR << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (!aurostd::FileExist(file)) {
          aus << "EEEEE  ERROR INCAR file=" << file << " does not exist! " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
          Krun = false;
          return Krun;
        }
        if (aurostd::FileEmpty(file)) {
          aus << "EEEEE  ERROR INCAR file=" << file << " is empty! " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
          Krun = false;
          return Krun;
        }
...
namespace KBIN {
  bool VASP_Modify_INCAR(_xvasp& xvasp, ofstream& FileMESSAGE, _aflags& aflags, _kflags& kflags, _vflags& vflags) {
    ostringstream aus;
    ...
    if (Krun && kflags.KBIN_MPI) {
      ...
      if (kflags.KBIN_MPI_AUTOTUNE) {
        aus << "00000  MESSAGE INCAR-MPI: found AUTOTUNE option " << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "00000  MESSAGE INCAR-MPI: input files WILL be auto-tuned for PARALLEL execution with " << kflags.KBIN_MPI_NCPUS << " CPUs " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        KBIN::XVASP_MPI_Autotune(xvasp, aflags, vflags.KBIN_VASP_INCAR_VERBOSE);
        xvasp.aopts.flag("FLAG::XVASP_INCAR_changed", true);
      } else {
        aus << "00000  MESSAGE INCAR-MPI: AUTOTUNE option NOT found! (aflow_ivasp.cpp) " << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "00000  MESSAGE INCAR-MPI: input files MUST be appropriate for PARALLEL execution with " << kflags.KBIN_MPI_NCPUS << " CPUs " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_SYSTEM_AUTO.isentry) {                                                        /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]SYSTEM_AUTO - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isentry &&
        (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("ALL") || vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS") || vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_SHAPE") ||
         vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_VOLUME") || vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS_CELL_VOLUME"))) {      /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("ALL")) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_ALL - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS")) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_IONS - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_SHAPE")) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_CELL_SHAPE - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_VOLUME")) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_CELL_VOLUME - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS_CELL_VOLUME")) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_IONS_CELL_VOLUME - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.isentry) {
        if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.xscheme == "ENERGY") {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_MODE=ENERGY (default) - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.xscheme == "FORCES") {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_MODE=FORCES - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.xscheme == "ENERGY_FORCES") {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_MODE=ENERGY_FORCES - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.xscheme == "FORCES_ENERGY") {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_MODE=FORCES_ENERGY - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
      } else {
        aus << "00000  MESSAGE-DEFAULT RELAX_MODE=" << "ENERGY" << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_EDIFFG_EQUAL.isentry) {      /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]EDIFFG=" << vflags.KBIN_VASP_FORCE_OPTION_EDIFFG_EQUAL.content_double << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_NBANDS_AUTO_isentry) {             /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]NBANDS - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_NBANDS_EQUAL.isentry) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]NBANDS=" << vflags.KBIN_VASP_FORCE_OPTION_NBANDS_EQUAL.content_string << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (aurostd::toupper(vflags.KBIN_VASP_FORCE_OPTION_NBANDS_EQUAL.content_string) == "VASP") {
        // use the vasp value
        KBIN::XVASP_INCAR_PREPARE_GENERIC("NBANDS", xvasp, vflags, "", -1, 0.0, false);
      } else if (aurostd::toupper(vflags.KBIN_VASP_FORCE_OPTION_NBANDS_EQUAL.content_string) == "AFLOW3") {
        // use the old value
        KBIN::XVASP_INCAR_PREPARE_GENERIC("NBANDS", xvasp, vflags, "", KBIN::XVASP_INCAR_GetNBANDS_AFLOW3(xvasp, aflags, vflags.KBIN_VASP_FORCE_OPTION_SPIN.option), 0.0, false);
      } else {
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_PSTRESS_EQUAL.isentry) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]PSTRESS=" << vflags.KBIN_VASP_FORCE_OPTION_PSTRESS_EQUAL.content_double << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_POTIM_EQUAL.isentry && vflags.KBIN_VASP_FORCE_OPTION_POTIM_EQUAL.content_double > 0) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]POTIM=" << vflags.KBIN_VASP_FORCE_OPTION_POTIM_EQUAL.content_double << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_SPIN.isentry) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]SPIN=" << (vflags.KBIN_VASP_FORCE_OPTION_SPIN.option ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      } else {
        //      aus << "00000  MESSAGE-DEFAULT SPIN=" << (DEFAULT_VASP_FORCE_OPTION_SPIN?"ON":"OFF") << " - " << Message(__AFLOW_FILE__,aflags) << endl;
        aus << "00000  MESSAGE-DEFAULT SPIN=" << "NEGLECT" << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (vflags.KBIN_VASP_FORCE_OPTION_SPIN.isentry) {
      ...
      if (vflags.KBIN_VASP_FORCE_OPTION_SPIN_REMOVE_RELAX_1) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]SPIN=REMOVE_RELAX_1 - " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_LSCOUPLING.isentry) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LSCOUPLING=" << (vflags.KBIN_VASP_FORCE_OPTION_LSCOUPLING.option ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      } else {
        aus << "00000  MESSAGE-DEFAULT LSCOUPLING=" << (DEFAULT_VASP_FORCE_OPTION_LSCOUPLING ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_SPIN.isentry && vflags.KBIN_VASP_FORCE_OPTION_SPIN.option) { // CO, MAGMOM should only be on if SPIN is SPECIFIED and ON (as default is to neglect)
        if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_MAGMOM.isentry) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]AUTO_MAGMOM=" << (vflags.KBIN_VASP_FORCE_OPTION_AUTO_MAGMOM.option ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
        } else {
          aus << "00000  MESSAGE-DEFAULT AUTO_MAGMOM=" << (DEFAULT_VASP_FORCE_OPTION_AUTO_MAGMOM ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]BADER=" << (vflags.KBIN_VASP_FORCE_OPTION_BADER.option ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      } else {
        aus << "00000  MESSAGE-DEFAULT BADER=" << (DEFAULT_VASP_FORCE_OPTION_BADER ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ELF=" << (vflags.KBIN_VASP_FORCE_OPTION_ELF.option ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      } else {
        aus << "00000  MESSAGE-DEFAULT ELF=" << (DEFAULT_VASP_FORCE_OPTION_ELF ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_NELM_EQUAL.isentry && xvasp.NRELAXING <= xvasp.NRELAX) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]NELM=" << vflags.KBIN_VASP_FORCE_OPTION_NELM_EQUAL.content_int << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_NELM_STATIC_EQUAL.isentry && xvasp.NRELAXING > xvasp.NRELAX) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]NELM_STATIC=" << vflags.KBIN_VASP_FORCE_OPTION_NELM_STATIC_EQUAL.content_int << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_NSW_EQUAL) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_NSW_EQUAL_VALUE > 0) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]NSW=" << vflags.KBIN_VASP_FORCE_OPTION_NSW_EQUAL_VALUE << " " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_SYM.isentry) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]SYM=" << (vflags.KBIN_VASP_FORCE_OPTION_SYM.option ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      } else {
        aus << "00000  MESSAGE-DEFAULT SYM=" << (DEFAULT_VASP_FORCE_OPTION_SYM ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_WAVECAR.isentry) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]WAVECAR=" << (vflags.KBIN_VASP_FORCE_OPTION_WAVECAR.option ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      } else {
        aus << "00000  MESSAGE-DEFAULT WAVECAR=" << (DEFAULT_VASP_FORCE_OPTION_WAVECAR ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_CHGCAR.isentry) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CHGCAR=" << (vflags.KBIN_VASP_FORCE_OPTION_CHGCAR.option ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      } else {
        aus << "00000  MESSAGE-DEFAULT CHGCAR=" << (DEFAULT_VASP_FORCE_OPTION_CHGCAR ? "ON" : "OFF") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CHGCAR_FILE.isentry) {
      ...
      if (chgcar[0] != '/') {
        chgcar = aurostd::CleanFileName(aflags.Directory + "/" + chgcar); // relative path
      }
      if (aurostd::FileExist(chgcar)) {
        if (aurostd::IsCompressed(chgcar)) {
          const string ext = aurostd::GetFileExtension(chgcar);
          aurostd::CopyFile(chgcar, aflags.Directory + "/CHGCAR" + ext); // relative path
          aurostd::DecompressFile(aflags.Directory + "/CHGCAR" + ext);
        } else {
        ...
        const string message = "Cannot use CHGCAR file " + chgcar + ". File not found.";
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_LDAU0.isentry) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LDAU=OFF - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_LDAU1.isentry) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LDAU1=ON - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (!vflags.KBIN_VASP_LDAU_SPECIES.empty()) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LDAU_SPECIES=\"" << vflags.KBIN_VASP_LDAU_SPECIES << "\" - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      if (!vflags.KBIN_VASP_LDAU_PARAMETERS.empty()) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LDAU_PARAMETERS=\"" << vflags.KBIN_VASP_LDAU_PARAMETERS << "\" - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LDAU_AFLOW_AUTO_flag=\"" << vflags.KBIN_VASP_LDAU_AFLOW_AUTO_flag << "\" - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_LDAU2.isentry) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LDAU2=ON - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (!vflags.KBIN_VASP_LDAU_SPECIES.empty()) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LDAU_SPECIES=\"" << vflags.KBIN_VASP_LDAU_SPECIES << "\" - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_LDAU_ADIABATIC.isentry) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LDAU=ADIABATIC (steps=" << vflags.KBIN_VASP_FORCE_OPTION_LDAU_ADIABATIC.content_int << ") - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_LDAU_CUTOFF.isentry) { /*************** INCAR **************/
      ...
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]LDAU=CUTOFF - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_PREC.isentry == true) { /*************** INCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]PREC=" << vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme << " - " << Message(__AFLOW_FILE__, aflags) << endl;
        if (vflags.KBIN_VASP_FORCE_OPTION_PREC.preserved) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]PREC_preserved - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
      } else {
        aus << "00000  MESSAGE-DEFAULT PREC=" << DEFAULT_VASP_FORCE_OPTION_PREC_SCHEME << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_ENMAX_MULTIPLY_EQUAL.isentry) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ENMAX_MULTIPLY=" << vflags.KBIN_VASP_FORCE_OPTION_ENMAX_MULTIPLY_EQUAL.content_double << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      ...
      if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "PHONONS") {
        aus << "00000  MESSAGE-OPTION ALGO = NORMAL (override by PREC=PHONONS) - " << Message(__AFLOW_FILE__, aflags) << endl;
        ...
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ALGO=" << vflags.KBIN_VASP_FORCE_OPTION_ALGO.xscheme << " - " << Message(__AFLOW_FILE__, aflags) << endl;
        if (vflags.KBIN_VASP_FORCE_OPTION_ALGO.preserved) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ALGO_PRESERVED - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
      } else {
        aus << "00000  MESSAGE-DEFAULT ALGO=" << DEFAULT_VASP_FORCE_OPTION_ALGO_SCHEME << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.isentry) { /*************** INCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]METAGGA=" << vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme << " - " << Message(__AFLOW_FILE__, aflags) << endl;
        ;
      } else {
        aus << "00000  MESSAGE-DEFAULT METAGGA=" << DEFAULT_VASP_FORCE_OPTION_METAGGA_SCHEME << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_IVDW.isentry) { /*************** INCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]IVDW=" << vflags.KBIN_VASP_FORCE_OPTION_IVDW.xscheme << " - " << Message(__AFLOW_FILE__, aflags) << endl;
        ;
      } else {
        aus << "00000  MESSAGE-DEFAULT IVDW=" << DEFAULT_VASP_FORCE_OPTION_IVDW_SCHEME << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_ABMIX.isentry) { /*************** INCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ABMIX=" << vflags.KBIN_VASP_FORCE_OPTION_ABMIX.xscheme << " - " << Message(__AFLOW_FILE__, aflags) << endl;
        // the rest is neglected... no AMIX BMIX AMIX_MAG BMIX_MAG
      } else {
        // neglected     aus << "00000  MESSAGE-DEFAULT ABMIX=" << DEFAULT_VASP_FORCE_OPTION_ABMIX_SCHEME << " - " << Message(__AFLOW_FILE__,aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_TYPE.isentry) { /*************** INCAR **************/
        if (vflags.KBIN_VASP_FORCE_OPTION_TYPE.xscheme.at(0) == 'D') {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]TYPE=" << vflags.KBIN_VASP_FORCE_OPTION_TYPE.xscheme << "  - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_TYPE.xscheme.at(0) == 'M') {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]TYPE=" << vflags.KBIN_VASP_FORCE_OPTION_TYPE.xscheme << "  - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_TYPE.xscheme.at(0) == 'S') {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]TYPE=" << vflags.KBIN_VASP_FORCE_OPTION_TYPE.xscheme << "  - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_TYPE.xscheme.at(0) == 'I') {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]TYPE=" << vflags.KBIN_VASP_FORCE_OPTION_TYPE.xscheme << "  - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
      } else {
        aus << "00000  MESSAGE-DEFAULT TYPE=" << DEFAULT_VASP_FORCE_OPTION_TYPE_SCHEME << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_ISMEAR_EQUAL.isentry && xvasp.NRELAXING <= xvasp.NRELAX) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ISMEAR=" << vflags.KBIN_VASP_FORCE_OPTION_ISMEAR_EQUAL.content_int << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_ISMEAR_STATIC_EQUAL.isentry && xvasp.NRELAXING == xvasp.NRELAX + 1) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ISMEAR_STATIC=" << vflags.KBIN_VASP_FORCE_OPTION_ISMEAR_STATIC_EQUAL.content_int << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_ISMEAR_BANDS_EQUAL.isentry && xvasp.NRELAXING > xvasp.NRELAX + 1) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ISMEAR_BANDS=" << vflags.KBIN_VASP_FORCE_OPTION_ISMEAR_BANDS_EQUAL.content_int << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_SIGMA_EQUAL.isentry && xvasp.NRELAXING <= xvasp.NRELAX) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]SIGMA=" << aurostd::utype2string(vflags.KBIN_VASP_FORCE_OPTION_SIGMA_EQUAL.content_double, _IVASP_DOUBLE2STRING_PRECISION_) << " - "
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_SIGMA_STATIC_EQUAL.isentry && xvasp.NRELAXING == xvasp.NRELAX + 1) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]SIGMA_STATIC=" << aurostd::utype2string(vflags.KBIN_VASP_FORCE_OPTION_SIGMA_STATIC_EQUAL.content_double, _IVASP_DOUBLE2STRING_PRECISION_) << " - "
          << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_SIGMA_BANDS_EQUAL.isentry && xvasp.NRELAXING > xvasp.NRELAX + 1) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]SIGMA_BANDS=" << aurostd::utype2string(vflags.KBIN_VASP_FORCE_OPTION_SIGMA_BANDS_EQUAL.content_double, _IVASP_DOUBLE2STRING_PRECISION_) << " - "
          << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false) { /*************** INCAR **************/
      if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isentry) { /*************** INCAR **************/
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("STANDARD_PRIMITIVE")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=STANDARD_PRIMITIVE - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("STANDARD_CONVENTIONAL")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=STANDARD_CONVENTIONAL - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("NIGGLI")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=NIGGLI - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("MINKOWSKI")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=MINKOWSKI - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("INCELL")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=INCELL - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("COMPACT")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=COMPACT - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("WIGNERSEITZ")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=WIGNERSEITZ - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("CARTESIAN")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=CARTESIAN - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("FRACTIONAL")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=FRACTIONAL - " << Message(__AFLOW_FILE__, aflags) << endl;
        }
        if (vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("PRESERVE")) {
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=PRESERVE - " << Message(__AFLOW_FILE__, aflags) << endl; // CO
        }
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.isentry) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]IGNORE_AFIX=" << vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.content_string << "  - " << Message(__AFLOW_FILE__, aflags) << endl;
      for (size_t i = 0; i < vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.vxscheme.size(); i++) {
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]IGNORE_AFIX=" << vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.vxscheme[i] << "  - " << Message(__AFLOW_FILE__, aflags) << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }

    // DO THE PAW CORRECTIONS
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && false) { /*************** INCAR **************/
      if (xvasp.POTCAR_PAW == true) {
        aus << "00000  MESSAGE-DEFAULT PAW_CORRECTIONS" << " - " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        KBIN::XVASP_INCAR_PREPARE_GENERIC("LASPH", xvasp, vflags, "", 0, 0.0, true);
        xvasp.aopts.flag("FLAG::XVASP_INCAR_changed", true);
      }
    }
    ...
    if (Krun && vflags.KBIN_VASP_RUN.flag("STATIC")) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_RUN_STATIC] - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      KBIN::XVASP_INCAR_Static_ON(xvasp, vflags);
      xvasp.aopts.flag("FLAG::XVASP_INCAR_changed", true);
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("STATIC")) { /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]STATIC - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      KBIN::XVASP_INCAR_Static_ON(xvasp, vflags);
      xvasp.aopts.flag("FLAG::XVASP_INCAR_changed", true);
    }
    ...
    if (Krun && aurostd::substring2bool(xvasp.INCAR, "LEPSILON=.true.", true)) { /*************** INCAR **************/ // CO20210315
      ...
      aus << "00000  MESSAGE REMOVE ENTRIES NPAR and NCORE because of LEPSILON - " << Message(__AFLOW_FILE__, aflags) << endl; // ME20191205
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && aurostd::substring2bool(xvasp.INCAR, "LCALCEPS=.true.", true)) { /*************** INCAR **************/ // CO20210315
      // ME20191205 - also remove NCORE
      aus << "00000  MESSAGE REMOVE ENTRIES NPAR and NCORE because of LCALCEPS - " << Message(__AFLOW_FILE__, aflags) << endl; // ME20191205
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && aurostd::substring2bool(xvasp.INCAR, "IBRION=", true)) { /*************** INCAR **************/ // CO20210315
      const uint IBRION = aurostd::kvpair2utype<uint>(xvasp.INCAR, "IBRION", "="); // CO20210315
      if (IBRION == 8) {
        aus << "00000  MESSAGE REMOVE ENTRIES NPAR and NCORE because of IBRION=" << IBRION << "  - " << Message(__AFLOW_FILE__, aflags) << endl; // ME20191205
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
...
namespace KBIN {
  void VASP_CleanUp_INCAR(_xvasp& xvasp) { // CO20200624
    ...
    vector<string> vlines_orig;
    vector<string> vlines;
    aurostd::stream2vectorstring(xvasp.INCAR, vlines_orig);
    bool found = false;
    for (size_t iline1 = 0; iline1 < vlines_orig.size(); iline1++) {
      found = false;
      for (size_t iline2 = 0; iline2 < vlines.size() && found == false; iline2++) {
        if (vlines_orig[iline1] == vlines[iline2]) {
          found = true;
        } // find STRICT duplicate lines first
      }
      if (found) {
        continue;
      }
    ...
    for (size_t iline = 0; iline < vlines.size(); iline++) {
      if (iline == vlines.size() - 1 && aurostd::RemoveWhiteSpacesFromTheFrontAndBack(vlines[iline]).empty()) {
        continue;
      } // remove last extra line, without this line, clean will add newlines everytime (growing INCAR unnecessarily)
...
namespace KBIN {
  ...
  bool VASP_Reread_INCAR(_xvasp& xvasp, ofstream& FileMESSAGE, _aflags& aflags) { // AFLOW_FUNCTION_IMPLEMENTATION
    if (!aurostd::FileExist(xvasp.Directory + "/INCAR")) {
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "INCAR not present in directory: " + xvasp.Directory, aflags, FileMESSAGE, std::cout, _LOGGER_ERROR_);
      return false;
    }
...
// POSCAR
namespace KBIN {
  bool VASP_Produce_POSCAR(_xvasp& xvasp, const string& AflowIn, ofstream& FileMESSAGE, _aflags& aflags, _kflags& kflags, _vflags& vflags) {
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    if (AflowIn.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, "KBIN::VASP_Produce_POSCAR():", "empty AflowIn", _FILE_CORRUPT_);
    } // CO20200624
    if (!kflags.AFLOW_MODE_VASP) {
      cerr << XPID << "KBIN::VASP_Produce_POSCAR: should kflags.AFLOW_MODE_VASP be set ??" << endl;
    }
    ostringstream aus;
    bool Krun = true;
    ...
    Krun = (Krun && (vflags.KBIN_VASP_POSCAR_MODE.flag("IMPLICIT") || vflags.KBIN_VASP_POSCAR_MODE.flag("EXPLICIT") || vflags.KBIN_VASP_POSCAR_MODE.flag("EXTERNAL")));
    if (!Krun) {
      aurostd::StringstreamClean(aus);
      aus << "EEEEE  [VASP_POSCAR_MODE_IMPLICIT] or [VASP_POSCAR_MODE_EXPLICIT] or [VASP_POSCAR_MODE_EXPLICIT] must be specified " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
      Krun = false;
      return Krun;
    }
    ...
    if (Krun && vflags.KBIN_VASP_POSCAR_MODE.flag("IMPLICIT")) { // [VASP_POSCAR_MODE_IMPLICIT] construction
      aus << "00000  MESSAGE POSCAR  generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (!vflags.KBIN_VASP_POSCAR_FILE.flag("PROTOTYPE")) {
        aus << "EEEEE  [VASP_POSCAR_FILE] In POSCAR_MODE_IMPLICIT you must specify PROTOTYPE " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      } else {
        ...
        // for(size_t j=0;j<atomABC.size();j++) cerr << atomABC.at(j) << endl;
        // for(size_t j=0;j<volumeABC.size();j++) cerr << volumeABC.at(j) << endl;
        bool done = false;
        if (atomABC.size() == 2 && volumeABC.empty()) {
          done = true;
          deque<string> atomX;
          deque<double> volumeX;
          for (uint isp = 0; isp <= 1; isp++) {
            atomX.push_back(atomABC[isp]);
            volumeX.push_back(GetAtomVolume(atomABC[isp]));
          } // CO20181129
          //[CO20181106]for(uint isp=0;isp<=1;isp++) {atomX.push_back(atomABC[isp]);volumeX.push_back(GetAtomVolume(aurostd::VASP_PseudoPotential_CleanName(atomABC[isp])));}
          xvasp.str = aflowlib::PrototypeLibraries(FileMESSAGE, label, "", atomX, volumeX, -1.0, LIBRARY_MODE_HTQC);
          // xvasp.str=aflowlib::PrototypeLibraries(FileMESSAGE,label,parameters,atomABC[0],GetAtomVolume(aurostd::VASP_PseudoPotential_CleanName(atomABC[0])),atomABC[1],GetAtomVolume(aurostd::VASP_PseudoPotential_CleanName(atomABC[1])),-1.0); // OLD WAY
        }
        if (atomABC.size() == 2 && volumeABC.size() == 1) {
          ...
          for (uint isp = 0; isp <= 1; isp++) {
            atomX.push_back(atomABC[isp]);
            volumeX.push_back(0.0);
          }
          // xvasp.str=aflowlib::PrototypeLibraries(FileMESSAGE,label,parameters,atomABC[0],0.0,atomABC[1],0.0,volumeABC[0]); // OLD WAY
          xvasp.str = aflowlib::PrototypeLibraries(FileMESSAGE, label, "", atomX, volumeX, volumeABC[0], LIBRARY_MODE_HTQC);
        }
        if (atomABC.size() == 2 && volumeABC.size() == 2) {
          ...
          for (uint isp = 0; isp <= 1; isp++) {
            atomX.push_back(atomABC[isp]);
            volumeX.push_back(volumeABC[isp]);
          };
        ...
        if (done == false) {
          aus << "EEEEE  POSCAR_MODE_IMPLICIT error in the PROTOTYPE definition" << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "EEEEE  [VASP_POSCAR_FILE]PROTOTYPE=" << structure << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
          Krun = false;
          return Krun;
        }
    ...
    if (Krun && vflags.KBIN_VASP_POSCAR_MODE.flag("EXPLICIT")) { // [VASP_POSCAR_MODE_EXPLICIT] construction
      if (vflags.KBIN_VASP_POSCAR_FILE.flag("KEYWORD") && !vflags.KBIN_VASP_POSCAR_MODE.flag("EXPLICIT_START_STOP") && !vflags.KBIN_VASP_POSCAR_MODE.flag("EXPLICIT_START_STOP_POINT")) {
        aus << "00000  MESSAGE POSCAR  generation EXPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        ...
        if (!xvasp.POSCAR.str().empty()) {
          xvasp.str = xstructure(xvasp.POSCAR, IOVASP_AUTO);
        } // load structure
        // get ONE of MANY
        ...
        aus << "EEEEE  [VASP_POSCAR_MODE_EXPLICIT] do not confuse aflow !!" << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  [VASP_POSCAR_MODE_EXPLICIT] Possible modes " << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "----------------------------------------------------------------------------------------------------" << endl;
        ...
        aus << "[AFLOW]" << endl;
        aus << "----------------------------------------------------------------------------------------------------" << endl;
        aus << "EEEEE  [VASP_POSCAR_MODE_EXPLICIT] Note " << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  [VASP_POSCAR_MODE_EXPLICIT]START must be present and no [VASP_POSCAR_FILE]" << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  [VASP_POSCAR_MODE_EXPLICIT]STOP  must be present and no [VASP_POSCAR_FILE]" << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  or [VASP_POSCAR_FILE] present and NO START/STOP" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      }
    ...
    if (Krun && vflags.KBIN_VASP_POSCAR_MODE.flag("EXTERNAL")) { // [VASP_POSCAR_MODE_EXTERNAL] construction
      string file;
      aus << "00000  MESSAGE POSCAR  generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (vflags.KBIN_VASP_POSCAR_FILE.flag("COMMAND") && vflags.KBIN_VASP_POSCAR_FILE.flag("FILE")) {
        aus << "EEEEE   [VASP_POSCAR_MODE]FILE=  and  [VASP_POSCAR_MODE]COMMAND=  can not be used together " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      }
      if (!vflags.KBIN_VASP_POSCAR_FILE.flag("COMMAND") && (vflags.KBIN_VASP_POSCAR_FILE.flag("FILE") || !vflags.KBIN_VASP_POSCAR_FILE.flag("FILE"))) {
        if (vflags.KBIN_VASP_POSCAR_FILE.flag("FILE")) {
          ...
          aus << "00000  MESSAGE POSCAR  generation from DEFAULT file=" << DEFAULT_VASP_EXTERNAL_POSCAR << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (!aurostd::FileExist(file)) {
          aus << "EEEEE  ERROR POSCAR file=" << file << " does not exist! " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
          Krun = false;
          return Krun;
        }
        if (aurostd::FileEmpty(file)) {
          aus << "EEEEE  ERROR POSCAR file=" << file << " is empty! " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
          Krun = false;
          return Krun;
        }
    ...
    if (!kflags.KBIN_POCC) { // CO20180420 - VERY SLOW AND UNNECESSARY - let's do these mods when we read-in/run in the ARUNS
      if (Krun) {
        Krun = (Krun && KBIN::VASP_Modify_POSCAR(xvasp, AflowIn, FileMESSAGE, aflags, vflags));
      }
    }

    // CHECK for negative determinant
    if (det(xvasp.str.scale * xvasp.str.lattice) < 0.0) {
      aus << "EEEEE  POSCAR ERROR: the triple product of the basis vectors is negative                      " << Message(__AFLOW_FILE__, aflags) << endl;
      aus << "EEEEE  POSCAR ERROR: exchange two basis vectors and adjust the atomic positions accordingly   " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
      Krun = false;
      return Krun;
    }
...
namespace KBIN {
  bool VASP_Modify_POSCAR(_xvasp& xvasp, const string& AflowIn, ofstream& FileMESSAGE, _aflags& aflags, _vflags& vflags) {
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    ostringstream aus;
    bool Krun = true;
    if (xvasp.aopts.flag("FLAG::XVASP_POSCAR_generated") == false) {
      aus << "EEEEE  KBIN::VASP_Modify_POSCAR: can`t modify POSCAR if it does not exist ! " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
      Krun = false;
      return Krun;
    }
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_POSCAR_FILE_VOLUME.flag(\"EQUAL_EQUAL\")=" << vflags.KBIN_VASP_POSCAR_FILE_VOLUME.flag("EQUAL_EQUAL") << endl;
    }
    if (Krun && vflags.KBIN_VASP_POSCAR_MODE.flag("IMPLICIT") && vflags.KBIN_VASP_POSCAR_FILE_VOLUME.flag("EQUAL_EQUAL")) { // [VASP_POSCAR_FILE]VOLUME=                       /*************** POSCAR **************/
      const double factor = aurostd::substring2utype<double>(AflowIn, "[VASP_POSCAR_FILE]VOLUME=", false);
      aus << "00000  MESSAGE POSCAR  IMPLICIT Volume = " << factor << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (factor <= 0.0) {
        aus << "EEEEE  KBIN::VASP_Modify_POSCAR Volume can not be <=0 (factor=" << factor << ")" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      }
      aus << "00000  MESSAGE POSCAR  IMPLITIC Old Volume= " << xvasp.str.Volume() << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      xvasp.str.SetVolume(factor);
      aus << "00000  MESSAGE POSCAR  IMPLITIC New Volume= " << xvasp.str.Volume() << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_VOLUME.flag(\"EQUAL_EQUAL\")=" << vflags.KBIN_VASP_FORCE_OPTION_VOLUME.flag("EQUAL_EQUAL") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_VOLUME.flag("EQUAL_EQUAL")) { // [VASP_FORCE_OPTION]VOLUME=                                                    /*************** POSCAR **************/
      const double factor1 = aurostd::substring2utype<double>(AflowIn, "[VASP_FORCE_OPTION]VOLUME=", false);
      const double factor = vflags.KBIN_VASP_FORCE_OPTION_VOLUME.getattachedutype<double>("EQUAL_EQUAL");
      aus << "00000  MESSAGE POSCAR  FORCE Volume = " << factor << " (factor1=" << factor1 << ")  " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (factor <= 0.0) {
        aus << "EEEEE  KBIN::VASP_Modify_POSCAR Volume can not be <=0 (factor=" << factor << ")" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
      ...
      aus << "00000  MESSAGE POSCAR  FORCE Old Volume= " << xvasp.str.Volume() << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      xvasp.str.SetVolume(factor);
      aus << "00000  MESSAGE POSCAR  FORCE New Volume= " << xvasp.str.Volume() << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      xvasp.str.title = xvasp.str.title + " [Forced Volume =]";
      aurostd::StringstreamClean(xvasp.POSCAR);
      xvasp.POSCAR << xvasp.str;
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_POSCAR_FILE_VOLUME.flag(\"MULTIPLY_EQUAL\")=" << vflags.KBIN_VASP_POSCAR_FILE_VOLUME.flag("MULTIPLY_EQUAL") << endl;
    }
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_VOLUME.flag(\"MULTIPLY_EQUAL\")=" << vflags.KBIN_VASP_FORCE_OPTION_VOLUME.flag("MULTIPLY_EQUAL") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_VOLUME.flag("MULTIPLY_EQUAL")) { // [VASP_FORCE_OPTION]VOLUME*=                                                    /*************** POSCAR **************/
      const double factor1 = aurostd::substring2utype<double>(AflowIn, "[VASP_FORCE_OPTION]VOLUME*=", false);
      const double factor = vflags.KBIN_VASP_FORCE_OPTION_VOLUME.getattachedutype<double>("MULTIPLY_EQUAL");
      aus << "00000  MESSAGE POSCAR  FORCE Volume *= " << factor << " (factor1=" << factor1 << ")  " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (factor <= 0.0) {
        aus << "EEEEE  KBIN::VASP_Modify_POSCAR Volume can not be <=0 (factor=" << factor << ")" << Message(__AFLOW_FILE__, aflags) << endl;
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_POSCAR_FILE_VOLUME.flag(PLUS_EQUAL)=" << vflags.KBIN_VASP_POSCAR_FILE_VOLUME.flag("PLUS_EQUAL") << endl;
    }
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_VOLUME.flag(PLUS_EQUAL)=" << vflags.KBIN_VASP_FORCE_OPTION_VOLUME.flag("PLUS_EQUAL") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_VOLUME.flag("PLUS_EQUAL")) { // [VASP_FORCE_OPTION]VOLUME+=                                                    /*************** POSCAR **************/
      const double factor1 = aurostd::substring2utype<double>(AflowIn, "[VASP_FORCE_OPTION]VOLUME+=", false);
      const double factor = vflags.KBIN_VASP_FORCE_OPTION_VOLUME.getattachedutype<double>("PLUS_EQUAL");
      aus << "00000  MESSAGE POSCAR  FORCE Volume += " << factor << " (factor1=" << factor1 << ")  " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      aus << "00000  MESSAGE POSCAR  FORCE Old Volume= " << xvasp.str.Volume() << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
...
namespace KBIN {
  ...
  ///
  /// @authors
  /// @mod{SD,20250218,updated for any VASP version using `vasp --version`}
  ...
  void convertPOSCARFormat(_xvasp& xvasp, const _kflags& kflags) {
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    const string vasp_path_full = kflags.KBIN_BIN;
    const double vaspVersion = aurostd::VersionString2Double(KBIN::BIN2VASPVersion(vasp_path_full));
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vasp_path_full=\"" << vasp_path_full << "\"" << endl;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vaspVersion=" << vaspVersion << endl;
    }
    ...
    if ((vaspVersion >= 5.0 || vaspVersion == 0.0) && !xvasp.str.is_vasp5_poscar_format) { // CO20210713
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " converting POSCAR to vasp5 format" << endl;
      }
      ...
      xvasp.POSCAR << xvasp.str;
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " xvasp.POSCAR=" << endl << xvasp.POSCAR.str() << endl;
      }
    } else if ((vaspVersion < 5.0) && !xvasp.str.is_vasp4_poscar_format) { // CO20210713
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " converting POSCAR to vasp4 format" << endl;
      }
  ...
  bool VASP_Convert_Unit_Cell(_xvasp& xvasp, _vflags& vflags, _aflags& aflags, ofstream& FileMESSAGE, ostringstream& aus) {
    ...
    if (LDEBUG) {
      aus << "00000  MESSAGE-OPTION  KBIN::VASP_Convert_Unit_Cell: BEGIN" << endl; // SC20200410
    }
    if (LDEBUG) {
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET); // SC20200410
    }

    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isentry) { /*************** POSCAR **************/
      //    aus << "00000  MESSAGE POSCAR  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.content_string << Message(__AFLOW_FILE__,aflags) << endl;
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]CONVERT_UNIT_CELL=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.content_string << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }

    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"PRESERVE\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("PRESERVE") << endl;
    }
    // POSCAR must be modified before doing the KPOINTS
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("PRESERVE")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_PRESERVE construction /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  PRESERVE Unit Cell " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }

    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"STANDARD_PRIMITIVE\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("STANDARD_PRIMITIVE") << endl;
    }
    // POSCAR must be modified before doing the KPOINTS
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("STANDARD_PRIMITIVE")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_STANDARD_PRIMITIVE construction /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  STANDARD_PRIMITIVE Unit Cell " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      ...
      const bool write_inequivalent_flag = xvasp.str.write_inequivalent_flag;
      // CO END
      const string bravais_lattice_type = xvasp.str.bravais_lattice_type;
      const string bravais_lattice_variation_type = xvasp.str.bravais_lattice_variation_type;
      const string pearson_symbol = xvasp.str.pearson_symbol;
      ...
      xvasp.POSCAR << xvasp.str;
      xvasp.str.clear();
      xvasp.POSCAR >> xvasp.str; // CO, this is important, clear all symmetry stuff as the whole lattice has changed //DX20191220 - uppercase to lowercase clear
      ...
      // CO END
      xvasp.str.bravais_lattice_type = bravais_lattice_type;
      xvasp.str.bravais_lattice_variation_type = bravais_lattice_variation_type;
      xvasp.str.pearson_symbol = pearson_symbol;
      xvasp.aopts.flag("FLAG::XVASP_POSCAR_generated", true);
      xvasp.aopts.flag("FLAG::XVASP_POSCAR_changed", true);
      aus << "00000  MESSAGE POSCAR  STANDARD_PRIMITIVE Unit Cell Lattice = [" + xvasp.str.bravais_lattice_type << "," << xvasp.str.bravais_lattice_variation_type << "," << xvasp.str.pearson_symbol << "]"
          << "  " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }

    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"STANDARD_CONVENTIONAL\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("STANDARD_CONVENTIONAL") << endl;
    }
    // POSCAR must be modified before doing the KPOINTS
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("STANDARD_CONVENTIONAL")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_STANDARD_CONVENTIONAL construction /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  STANDARD_CONVENTIONAL Unit Cell " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      ...
      xvasp.POSCAR << xvasp.str;
      xvasp.str.clear();
      xvasp.POSCAR >> xvasp.str; // CO, this is important, clear all symmetry stuff as the whole lattice has change //DX20191220 - uppercase to lowercase clear
      // CO add these flags to prevent recalculation and wasted effort
      xvasp.str.Standard_Lattice_calculated = true;
      ...
      xvasp.aopts.flag("FLAG::XVASP_POSCAR_generated", true);
      xvasp.aopts.flag("FLAG::XVASP_POSCAR_changed", true);
      aus << "00000  MESSAGE POSCAR  STANDARD_CONVENTIONAL Unit Cell Lattice = [" + xvasp.str.bravais_lattice_type << "," << xvasp.str.bravais_lattice_variation_type << "," << xvasp.str.pearson_symbol << "]"
          << "  " << Message(__AFLOW_FILE__, aflags) << endl; // CO
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET); // CO
      // cerr << det(xvasp.str.lattice) << endl;
    }

    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"NIGGLI\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("NIGGLI") << endl;
    }
    // POSCAR must be modified before doing the KPOINTS
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("NIGGLI")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_NIGGLI construction                     /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  NIGGLI Unit Cell Reduction " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(MINKOWSKI)=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("MINKOWSKI") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("MINKOWSKI")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_MINKOWSKI construction               /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  MINKOWSKI Basis Reduction " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"INCELL\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("INCELL") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("INCELL")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_INCELL construction                     /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  INCELL Unit Cell Basis " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"COMPACT\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("COMPACT") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("COMPACT")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_COMPACT construction                   /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  COMPACT Unit Cell Basis " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"WIGNERSEITZ\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("WIGNERSEITZ") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("WIGNERSEITZ")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_WIGNERSEITZ construction           /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  WIGNERSEITZ Unit Cell Basis " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"CARTESIAN\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("CARTESIAN") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("CARTESIAN")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_CARTESIAN construction               /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  CARTESIAN Basis Coordinates" << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      xvasp.str.SetCoordinates(_COORDS_CARTESIAN_);
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"FRACTIONAL\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("FRACTIONAL") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("FRACTIONAL")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_FRACTIONAL construction            /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  FRACTIONAL Basis Coordinate" << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      xvasp.str.SetCoordinates(_COORDS_FRACTIONAL_);
      aurostd::StringstreamClean(xvasp.POSCAR);
      xvasp.POSCAR << xvasp.str;
    ...
    if (LDEBUG) {
      cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag(\"DIRECT\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("DIRECT") << endl;
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.flag("DIRECT")) { // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_DIRECT construction                    /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  DIRECT Basis Coordinate" << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      xvasp.str.SetCoordinates(_COORDS_FRACTIONAL_);
      aurostd::StringstreamClean(xvasp.POSCAR);
      xvasp.POSCAR << xvasp.str;
    ...
    if (LDEBUG) {
      aus << "00000  MESSAGE-OPTION  KBIN::VASP_Convert_Unit_Cell: END" << endl; // SC20200410
    }
    if (LDEBUG) {
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET); // SC20200410
    }
...
namespace KBIN {
  ...
  bool VASP_Reread_POSCAR(_xvasp& xvasp, ofstream& FileMESSAGE, _aflags& aflags) { // AFLOW_FUNCTION_IMPLEMENTATION
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " reading POSCAR from: " << xvasp.Directory << endl;
    }
    if (!aurostd::FileExist(xvasp.Directory + "/POSCAR")) {
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "POSCAR not present in directory: " + xvasp.Directory, aflags, FileMESSAGE, std::cout, _LOGGER_ERROR_);
      return false;
    }
    if (aurostd::FileEmpty(xvasp.Directory + "/POSCAR")) {
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "POSCAR empty in directory: " + xvasp.Directory, aflags, FileMESSAGE, std::cout, _LOGGER_ERROR_);
      return false;
    }
    aurostd::StringstreamClean(xvasp.POSCAR_orig);
    xvasp.POSCAR_orig << xvasp.POSCAR.str();
    aurostd::StringstreamClean(xvasp.POSCAR);
    xvasp.POSCAR << aurostd::file2string(xvasp.Directory + "/POSCAR"); // DID REREAD
    ...
    if (aurostd::RemoveWhiteSpacesFromTheFrontAndBack(xvasp.POSCAR.str()).empty()) {
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "POSCAR file contents empty in directory: " + xvasp.Directory, aflags, FileMESSAGE, std::cout, _LOGGER_ERROR_);
      return false;
    }
    return true;
  }
...
// KPOINTS
namespace KBIN {
  bool VASP_Produce_KPOINTS(_xvasp& xvasp, const string& AflowIn, ofstream& FileMESSAGE, _aflags& aflags, _kflags& kflags, _vflags& vflags) {
    if (AflowIn.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, "KBIN::VASP_Produce_KPOINTS():", "empty AflowIn", _FILE_CORRUPT_);
    } // CO20200624
    if (!kflags.AFLOW_MODE_VASP) {
      cerr << XPID << "KBIN::VASP_Produce_KPOINTS: should kflags.AFLOW_MODE_VASP be set ??" << endl;
    }
    ostringstream aus;
    bool Krun = true;
    bool IMPLICIT = false;
    aurostd::StringstreamClean(xvasp.KPOINTS);
    xvasp.str.kpoints_k1 = 0;
    xvasp.str.kpoints_k2 = 0;
    xvasp.str.kpoints_k3 = 0; // RESET KPOINTS
    xvasp.str.kpoints_s1 = 0.0;
    xvasp.str.kpoints_s2 = 0.0;
    xvasp.str.kpoints_s3 = 0.0; // RESET SHIFTS
    ...
    Krun = (Krun && (vflags.KBIN_VASP_KPOINTS_MODE.flag("IMPLICIT") || vflags.KBIN_VASP_KPOINTS_MODE.flag("EXPLICIT") || vflags.KBIN_VASP_KPOINTS_MODE.flag("EXTERNAL")));
    if (!Krun) {
      aurostd::StringstreamClean(aus);
      aus << "EEEEE  [VASP_KPOINTS_MODE_IMPLICIT] or [VASP_KPOINTS_MODE_EXPLICIT] or [VASP_KPOINTS_MODE_EXTERNAL] must be specified " << Message(__AFLOW_FILE__, aflags) << endl; // ME20181113
      aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
      Krun = false;
      return Krun;
    }
    if (Krun && xvasp.aopts.flag("FLAG::XVASP_POSCAR_generated") == false) {
      aus << "EEEEE  INCAR   generation: POSCAR must be called before " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
      Krun = false;
      return Krun;
    }
    ...
    if (Krun && vflags.KBIN_VASP_KPOINTS_MODE.flag("EXPLICIT")) { // [VASP_KPOINTS_MODE_EXPLICIT] construction
      ...
      if (vflags.KBIN_VASP_KPOINTS_FILE.flag("KEYWORD") && !vflags.KBIN_VASP_KPOINTS_MODE.flag("EXPLICIT_START_STOP")) {
        aus << "00000  MESSAGE KPOINTS generation EXPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        ...
        const string START = "[VASP_KPOINTS_MODE_EXPLICIT]START";
        const string STOP = "[VASP_KPOINTS_MODE_EXPLICIT]STOP";
        aus << "00000  MESSAGE KPOINTS generation EXPLICIT file from " << _AFLOWIN_ << " with START/STOP  " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        xvasp.KPOINTS << vflags.KBIN_VASP_KPOINTS_EXPLICIT_START_STOP.str(); // ME20181113
        KBIN::XVASP_KPOINTS_string2numbers(xvasp);
      } else {
        aus << "EEEEE  [VASP_KPOINTS_MODE_EXPLICIT] do not confuse aflow !!" << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  [VASP_KPOINTS_MODE_EXPLICIT] Possible modes " << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "----------------------------------------------------------------------------------------------------" << endl;
        ...
        aus << "[AFLOW]" << endl;
        aus << "----------------------------------------------------------------------------------------------------" << endl;
        aus << "EEEEE  [VASP_KPOINTS_MODE_EXPLICIT] Note " << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  [VASP_KPOINTS_MODE_EXPLICIT]START must be present and no [VASP_KPOINTS_FILE]" << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  [VASP_KPOINTS_MODE_EXPLICIT]STOP  must be present and no [VASP_KPOINTS_FILE]" << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "EEEEE  or [VASP_KPOINTS_FILE] present and NO START/STOP" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      }
    ...
    { // CO20200106 - patching for auto-indenting
      IMPLICIT = true;
      aus << "00000  MESSAGE KPOINTS generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      string stringKPPRA;
      int NK = 1;
      xvasp.str.kpoints_k1 = 1;
      xvasp.str.kpoints_k2 = 1;
      xvasp.str.kpoints_k3 = 1;
      ...
      if (vflags.KBIN_VASP_RUN.flag("DIELECTRIC")) {
        STRING_KPOINTS_TO_SHOW = "KPOINTS_DIELECTRIC";
        LOCAL_KBIN_VASP_KPOINTS_KSCHEME_isentry = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSCHEME.isentry; // overrides kscheme
        LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSCHEME.content_string; // overrides kscheme
        if (vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KMODE.isentry) {
          LOCAL_KBIN_VASP_KPOINTS_KMODE_isentry = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KMODE.isentry;
          LOCAL_KBIN_VASP_KPOINTS_KMODE_content_int = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KMODE.content_int;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KMODE overrides KPOINTS_KMODE generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KMODE: LOCAL_KBIN_VASP_KPOINTS_KMODE_isentry=" << vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KMODE.isentry << " - " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KMODE: LOCAL_KBIN_VASP_KPOINTS_KMODE_content_int=" << vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KMODE.content_int << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KPPRA.isentry) {
          LOCAL_KBIN_VASP_KPOINTS_KPPRA_isentry = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KPPRA.isentry;
          LOCAL_KBIN_VASP_KPOINTS_KPPRA_content_int = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KPPRA.content_uint;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KPPRA overrides KPOINTS_KPPRA generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KPPRA: LOCAL_KBIN_VASP_KPOINTS_KPPRA_isentry=" << vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KPPRA.isentry << " - " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KPPRA: LOCAL_KBIN_VASP_KPOINTS_KPPRA_content_int=" << vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KPPRA.content_uint << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSCHEME.isentry) {
          LOCAL_KBIN_VASP_KPOINTS_KSCHEME_isentry = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSCHEME.isentry;
          LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSCHEME.content_string;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSCHEME overrides KPOINTS_KSCHEME generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSCHEME: LOCAL_KBIN_VASP_KPOINTS_KSCHEME_isentry=" << vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSCHEME.isentry << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSCHEME: LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string=" << vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSCHEME.content_string << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSHIFT.isentry) {
          LOCAL_KBIN_VASP_KPOINTS_KSHIFT_isentry = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSHIFT.isentry;
          LOCAL_KBIN_VASP_KPOINTS_KSHIFT_content_string = vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSHIFT.content_string;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSHIFT overrides KPOINTS_KSHIFT generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSHIFT: LOCAL_KBIN_VASP_KPOINTS_KSHIFT_isentry=" << vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSHIFT.isentry << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSHIFT: LOCAL_KBIN_VASP_KPOINTS_KSHIFT_content_string=" << vflags.KBIN_VASP_KPOINTS_DIELECTRIC_KSHIFT.content_string << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
      } else if (vflags.KBIN_VASP_RUN.flag("STATIC")) {
        ...
        if (vflags.KBIN_VASP_KPOINTS_STATIC_KMODE.isentry) {
          ...
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KMODE overrides KPOINTS_KMODE generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KMODE: LOCAL_KBIN_VASP_KPOINTS_KMODE_isentry=" << vflags.KBIN_VASP_KPOINTS_STATIC_KMODE.isentry << " - " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KMODE: LOCAL_KBIN_VASP_KPOINTS_KMODE_content_int=" << vflags.KBIN_VASP_KPOINTS_STATIC_KMODE.content_int << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (vflags.KBIN_VASP_KPOINTS_STATIC_KPPRA.isentry) {
          ...
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KPPRA overrides KPOINTS_KPPRA generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KPPRA: LOCAL_KBIN_VASP_KPOINTS_KPPRA_isentry=" << vflags.KBIN_VASP_KPOINTS_STATIC_KPPRA.isentry << " - " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KPPRA: LOCAL_KBIN_VASP_KPOINTS_KPPRA_content_int=" << vflags.KBIN_VASP_KPOINTS_STATIC_KPPRA.content_uint << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (vflags.KBIN_VASP_KPOINTS_STATIC_KSCHEME.isentry) {
          ...
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSCHEME overrides KPOINTS_KSCHEME generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSCHEME: LOCAL_KBIN_VASP_KPOINTS_KSCHEME_isentry=" << vflags.KBIN_VASP_KPOINTS_STATIC_KSCHEME.isentry << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSCHEME: LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string=" << vflags.KBIN_VASP_KPOINTS_STATIC_KSCHEME.content_string << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (vflags.KBIN_VASP_KPOINTS_STATIC_KSHIFT.isentry) {
          ...
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSHIFT overrides KPOINTS_KSHIFT generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSHIFT: LOCAL_KBIN_VASP_KPOINTS_KSHIFT_isentry=" << vflags.KBIN_VASP_KPOINTS_STATIC_KSHIFT.isentry << " - " << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSHIFT: LOCAL_KBIN_VASP_KPOINTS_KSHIFT_content_string=" << vflags.KBIN_VASP_KPOINTS_STATIC_KSHIFT.content_string << " - "
              << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
      }
      ...
      if (LOCAL_KBIN_VASP_KPOINTS_KMODE_content_int == 0 || LOCAL_KBIN_VASP_KPOINTS_KMODE_isentry == false) {
        ...
        if (LOCAL_KBIN_VASP_KPOINTS_KSCHEME_isentry == false) {
          ...
          aus << "00000  MESSAGE-DEFAULT " << STRING_KPOINTS_TO_SHOW << "_KSCHEME=" << xvasp.str.kpoints_kscheme << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        } else {
          xvasp.str.kpoints_kscheme = "Monkhorst-Pack"; // DEFAULT FIX
          xvasp.str.kpoints_kscheme = DEFAULT_KSCHEME; // DEFAULT FIX
          if (LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'M' || LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'm') {
            xvasp.str.kpoints_kscheme = "Monkhorst-Pack";
          }
          if (LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'G' || LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'g') {
            xvasp.str.kpoints_kscheme = "Gamma";
          }
          if (LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'A' || LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'a') {
            xvasp.str.kpoints_kscheme = DEFAULT_KSCHEME;
          }
          if (LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'C' || LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'c') {
            xvasp.str.kpoints_kscheme = "Cartesian";
          }
          if (LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'K' || LOCAL_KBIN_VASP_KPOINTS_KSCHEME_content_string.at(0) == 'k') {
            xvasp.str.kpoints_kscheme = "Cartesian";
          }
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSCHEME=" << xvasp.str.kpoints_kscheme << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (xvasp.str.kpoints_kscheme == DEFAULT_KSCHEME) {
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << " Calculating structure lattice" << " - " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          xvasp.str.GetLatticeType();
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << " Found Lattice=" << xvasp.str.bravais_lattice_variation_type << " - " << Message(__AFLOW_FILE__, aflags) << endl;
          xvasp.str.kpoints_kscheme = "Monkhorst-Pack";
          if (xvasp.str.bravais_lattice_variation_type == "HEX" || xvasp.str.bravais_lattice_variation_type == "FCC" || xvasp.str.bravais_lattice_variation_type == "RHL") {
            xvasp.str.kpoints_kscheme = "Gamma"; // CO+DX20200404 - 3-fold symmetries REQUIRE Gamma-centered
          }
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSCHEME=\"" << xvasp.str.kpoints_kscheme << "\" " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        ...
        if (LOCAL_KBIN_VASP_KPOINTS_KPPRA_isentry == false) {
          NK = 1; // DEFAULT FIX
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KPPRA=NNNN is missing, taking NNNN=" << NK << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        } else {
          NK = LOCAL_KBIN_VASP_KPOINTS_KPPRA_content_int;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KPPRA=" << NK << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        stringKPPRA = KPPRA(xvasp.str, NK);
        // VERBOSE on LOCK /SCREEN
        if (true) {
          FileMESSAGE << stringKPPRA;
        }
        if (true) {
          aus.precision(5);
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KPPRA routine [" << xvasp.str.kpoints_k1 << "," << xvasp.str.kpoints_k2 << "," << xvasp.str.kpoints_k3
              << "]=" << xvasp.str.kpoints_k1 * xvasp.str.kpoints_k2 * xvasp.str.kpoints_k3 << "=[" << modulus(xvasp.str.klattice(1) / ((double) xvasp.str.kpoints_k1)) << ","
              << modulus(xvasp.str.klattice(2) / ((double) xvasp.str.kpoints_k2)) << "," << modulus(xvasp.str.klattice(3) / ((double) xvasp.str.kpoints_k3)) << "]" << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        // KSHIFT ******************************
        if (LOCAL_KBIN_VASP_KPOINTS_KSHIFT_isentry == false) {
          xvasp.str.kpoints_s1 = xvasp.str.kpoints_s2 = xvasp.str.kpoints_s3 = 0.0;
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSHIFT= X X X  is missing, taking X X X = 0.0 0.0 0.0 " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        } else {
          xvasp.str.kpoints_s1 = xvasp.str.kpoints_s2 = xvasp.str.kpoints_s3 = 0.0;
          stringstream oaus;
          ...
          aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << "_KSHIFT= " << xvasp.str.kpoints_s1 << " " << xvasp.str.kpoints_s2 << " " << xvasp.str.kpoints_s3 << " " << " " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        // create KPOINTS ******************************
        aurostd::StringstreamClean(xvasp.KPOINTS);
        ...
        aurostd::StringstreamClean(aus);
        aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << " K=[" << xvasp.str.kpoints_k1 << " " << xvasp.str.kpoints_k2 << " " << xvasp.str.kpoints_k3 << "]" << " " << Message(__AFLOW_FILE__, aflags) << endl;
        aus << "00000  MESSAGE " << STRING_KPOINTS_TO_SHOW << " Kpoints=" << xvasp.str.kpoints_kppra << "  [KPPRA=" << NK << "]    with " << xvasp.str.kpoints_kscheme << "   " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      } else {
        aurostd::StringstreamClean(aus);
        aus << "EEEEE             Only [VASP_KPOINTS_FILE]KMODE=0 is supported ! " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      }
    ...
    if (Krun && vflags.KBIN_VASP_KPOINTS_MODE.flag("EXTERNAL")) { // [VASP_KPOINTS_MODE_EXTERNAL] construction
      string file;
      aus << "00000  MESSAGE KPOINTS  generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (vflags.KBIN_VASP_KPOINTS_FILE.flag("COMMAND") && vflags.KBIN_VASP_KPOINTS_FILE.flag("FILE")) {
        aus << "EEEEE   [VASP_KPOINTS_MODE]FILE=  and  [VASP_KPOINTS_MODE]COMMAND=  can not be used together " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      }
      if (!vflags.KBIN_VASP_KPOINTS_FILE.flag("COMMAND") && (vflags.KBIN_VASP_KPOINTS_FILE.flag("FILE") || !vflags.KBIN_VASP_KPOINTS_FILE.flag("FILE"))) {
        if (vflags.KBIN_VASP_KPOINTS_FILE.flag("FILE")) {
          ...
          aus << "00000  MESSAGE KPOINTS  generation from DEFAULT file=" << DEFAULT_VASP_EXTERNAL_KPOINTS << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (!aurostd::FileExist(file)) {
          aus << "EEEEE  ERROR KPOINTS file=" << file << " does not exist! " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
          Krun = false;
          return Krun;
        }
        if (aurostd::FileEmpty(file)) {
          aus << "EEEEE  ERROR KPOINTS file=" << file << " is empty! " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
          Krun = false;
          return Krun;
        }
...
namespace KBIN {
  bool VASP_Modify_KPOINTS(_xvasp& xvasp, ofstream& FileMESSAGE, _aflags& aflags, _vflags& vflags) {
    ostringstream aus;
    const bool Krun = true;

    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("KEEPK") == false && false) { /*************** KPOINTS **************/
      vflags.KBIN_VASP_WRITE_KPOINTS = true;
      if (aurostd::_isodd(xvasp.str.kpoints_k1)) {
        xvasp.str.kpoints_k1++;
      }
      if (aurostd::_isodd(xvasp.str.kpoints_k2)) {
        xvasp.str.kpoints_k2++;
      }
      if (aurostd::_isodd(xvasp.str.kpoints_k3)) {
      ...
      aus << "00000  MESSAGE KPOINTS Option Tune - K=[" << xvasp.str.kpoints_k1 << " " << xvasp.str.kpoints_k2 << " " << xvasp.str.kpoints_k3 << " " << xvasp.str.kpoints_kmax << "] - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }

    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isentry) { /*************** KPOINTS **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=" << vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.content_string << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("KEEPK")) { /*************** KPOINTS **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=KEEPK - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("EVEN")) { /*************** KPOINTS **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=EVEN - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("ODD")) { /*************** KPOINTS **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=ODD - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      KBIN::XVASP_KPOINTS_OPERATION(xvasp, "Xodd,Yodd,Zodd"); // KBIN::XVASP_KPOINTS_ODD(xvasp);
      xvasp.aopts.flag("FLAG::XVASP_KPOINTS_changed", true);
    }
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("KSHIFT_GAMMA_EVEN")) {
      /*************** KPOINTS **************/ // CO20210315 - the 'GAMMA' here is not appropriate unless 'Gamma' is set below, shifting the even by 0.5 puts it on the same 'grid' as Gamma, see https://www.vasp.at/wiki/index.php/KPOINTS#Monkhorst-Pack, not changing for legacy
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=KSHIFT_GAMMA_EVEN - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("KSHIFT_GAMMA_ODD")) {
      ...
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=KSHIFT_GAMMA_ODD - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("KSCHEME_MONKHORST_PACK")) { /*************** KPOINTS **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=KSCHEME_MONKHORST_PACK - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("KSCHEME_GAMMA")) { /*************** KPOINTS **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=KSCHEME_GAMMA - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && (vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry == false && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("KSCHEME_AUTO"))) { /*************** KPOINTS **************/
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=KSCHEME_AUTO  Calculating structure lattice" << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      xvasp.str.GetLatticeType();
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=KSCHEME_AUTO  Found Lattice=" << xvasp.str.bravais_lattice_variation_type << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      xvasp.str.kpoints_kscheme = "Monkhorst-Pack";
      ...
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=KSCHEME_" << xvasp.str.kpoints_kscheme << " - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("GAMMA")) {
      ...
      xvasp.aopts.flag("FLAG::XVASP_KPOINTS_changed", true);
      aus
          << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=GAMMA - K=["
          << xvasp.str.kpoints_k1
          << " "
          << xvasp.str.kpoints_k2
          << " "
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("IBZKPT")) {
      aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]KPOINTS=IBZKPT - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }
    // ------------------------------------
    // end
...
namespace KBIN {
  ...
  bool VASP_Reread_KPOINTS(_xvasp& xvasp, ofstream& FileMESSAGE, _aflags& aflags) { // AFLOW_FUNCTION_IMPLEMENTATION
    if (!aurostd::FileExist(xvasp.Directory + "/KPOINTS")) {
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "KPOINTS not present in directory: " + xvasp.Directory, aflags, FileMESSAGE, std::cout, _LOGGER_ERROR_);
      return false;
    }
    aurostd::StringstreamClean(xvasp.KPOINTS_orig);
    xvasp.KPOINTS_orig << xvasp.KPOINTS.str();
...
namespace KBIN {
  bool VASP_Find_FILE_POTCAR(const string& species_pp, string& FilePotcar, string& DataPotcar, string& AUIDPotcar) {
    ...
    if (LDEBUG) {
      cerr << "[VASP_Find_FILE_POTCAR]: species_pp=" << species_pp << endl;
    }
    if (LDEBUG) {
      cerr << "[VASP_Find_FILE_POTCAR]: pseudopotential=" << pseudopotential << endl;
    }

    bool found = false;
    FilePotcar = "";
    for (size_t j = 0; j < vVASP_POTCAR_DIRECTORIES.size() && !found; j++) { // cycle through possible directories
      for (uint k = 0; k <= 9 && !found; k++) { // cycle through fixing current before/after
        string FilePotcark;
        if (k == 0) {
          FilePotcark = species_pp; // k==0 dont touch
        }
        if (k > 0) {
          FilePotcark = vVASP_POTCAR_DIRECTORIES[j] + "/" + species_pp;
        }
        // if(k==1) FilePotcark=FilePotcark; // nothing done //CO20190329 - compiler doesn't like FilePotcark=FilePotcark
        if (k == 2) {
          aurostd::StringSubstInPlace(FilePotcark, pseudopotential, pseudopotential + "/" + DEFAULT_VASP_POTCAR_DATE); // current POST
        }
        if (k == 3) {
          aurostd::StringSubstInPlace(FilePotcark, pseudopotential, DEFAULT_VASP_POTCAR_DATE + "/" + pseudopotential); // current PRE
        }
        if (k == 4) {
          aurostd::StringSubstInPlace(FilePotcark, pseudopotential, pseudopotential + "/dateP1"); // dateP1 POST
        }
        if (k == 5) {
          aurostd::StringSubstInPlace(FilePotcark, pseudopotential, "dateP1/" + pseudopotential); // dateP1 PRE
        }
        if (k == 6) {
          aurostd::StringSubstInPlace(FilePotcark, pseudopotential, pseudopotential + "/dateP2"); // dateP2 POST
        }
        if (k == 7) {
          aurostd::StringSubstInPlace(FilePotcark, pseudopotential, "dateP2/" + pseudopotential); // dateP2 PRE
        }
        if (k == 8) {
          aurostd::StringSubstInPlace(FilePotcark, pseudopotential, pseudopotential + "/dateP3"); // dateP3 POST
        }
        if (k == 9) {
          aurostd::StringSubstInPlace(FilePotcark, pseudopotential, "dateP3/" + pseudopotential); // dateP3 PRE
        }
        for (uint l = 0; l <= 1 && !found; l++) { // cycle through POTCAR as postfix
          FilePotcar = FilePotcark; // default
          if (l == 0) {
            FilePotcar = FilePotcark; // current NO
          }
          if (l == 1) {
            FilePotcar = FilePotcark + "/POTCAR"; // POTCAR after
          }
          // clean up to avoid comments //
          // TESTING POTCARS
          if (!aurostd::substring2bool(FilePotcar, "POTCAR")) {
            FilePotcar = FilePotcar + "/POTCAR"; // some fix, _AFLOWIN_ might have some personality issues
          }
          // DEBUG=true;
          FilePotcar = aurostd::CleanFileName(FilePotcar);
          if (aurostd::FileExist(FilePotcar) && !aurostd::FileEmpty(FilePotcar)) {
            found = true;
          }
          if (LDEBUG) {
            cout << "DDDDD  POTCAR  (" << species_pp << ",j=" << j << ",k=" << k << ",l=" << l << ")  [FilePotcar=" << FilePotcar << "] found=" << found
                 << " aurostd::FileExist(FilePotcar)=" << aurostd::FileExist(FilePotcar) << endl;
          }
    ...
    if (found) {
      ...
      if (LDEBUG) {
        cerr << "[VASP_Find_FILE_POTCAR]: AUIDPotcar=" << AUIDPotcar << endl;
      }
    }
...
namespace KBIN {
  bool VASP_Produce_POTCAR(_xvasp& xvasp, const string& AflowIn, ofstream& FileMESSAGE, _aflags& aflags, _kflags& kflags, _vflags& vflags) {
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    if (AflowIn.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, "KBIN::VASP_Produce_POTCAR():", "empty AflowIn", _FILE_CORRUPT_);
    } // CO20200624
    if (!kflags.AFLOW_MODE_VASP) {
      cerr << XPID << "KBIN::VASP_Produce_POTCAR: should kflags.AFLOW_MODE_VASP be set ??" << endl;
    }
    string::size_type sub_size1;
    string::size_type sub_size2;
    string subS;
    string subS1;
    string subS2;
    const string subSDIR = "$POTCARDIR";
    ostringstream aus;
    bool Krun = true;
    ...
    if (!Krun) {
      aurostd::StringstreamClean(aus);
      aus << "EEEEE  [VASP_POTCAR_MODE_IMPLICIT] or [VASP_POTCAR_MODE_EXPLICIT] or [VASP_POTCAR_MODE_EXPLICIT] must be specified " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
      Krun = false;
      return Krun;
    }
    ...
    if (Krun && vflags.KBIN_VASP_POTCAR_MODE.flag("IMPLICIT") && vflags.KBIN_VASP_POTCAR_FILE.flag("KEYWORD")) { // [VASP_POTCAR_MODE_IMPLICIT] construction
      // Prepare POTCAR
      aus << "00000  MESSAGE POTCAR  generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.isentry) { /*************** POTCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]AUTO_PSEUDOPOTENTIALS - " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]AUTO_PSEUDOPOTENTIALS=\"" << vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme << "\" - " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      // Prepare POTCAR
      const ifstream FileINPUT;
      ...
      if (!vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.isentry) { // CO20181226
        ...
        for (size_t i = 0; i < FilePotcars.size() && i < xvasp.str.species_pp.size(); i++) {
          aurostd::string2tokens(FilePotcars[i], tokens, "/"); // ME20190313
          xvasp.str.species_pp[i] = tokens.back(); // ME20190313
          // xvasp.str.species_pp[i]=FilePotcars[i];
        }
      }
      // IF AUTO_PSEUDOPOTENTIALS
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.isentry=" << vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.isentry << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.isentry) {
        for (size_t i = 0; i < FilePotcars.size(); i++) {
          string cleanname = aurostd::VASP_PseudoPotential_CleanName(FilePotcars[i]);
          aurostd::StringSubstInPlace(cleanname, "/POTCAR", "");
          vector<string> tokens;
          aurostd::string2tokens(cleanname, tokens, "/");
          if (!tokens.empty()) {
            cleanname = tokens.at(tokens.size() - 1);
          }
          if (LDEBUG) {
            cerr << "[VASP_Produce_POTCAR]: vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme=" << vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme << endl;
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme == "potpaw_PBE_KIN") {
            FilePotcars[i] = DEFAULT_VASP_POTCAR_DIR_POTPAW_PBE_KIN + "/" + AVASP_Get_PseudoPotential_PAW_PBE_KIN(cleanname);
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme == "potpaw_LDA_KIN") {
            FilePotcars[i] = DEFAULT_VASP_POTCAR_DIR_POTPAW_LDA_KIN + "/" + AVASP_Get_PseudoPotential_PAW_LDA_KIN(cleanname);
          }

          if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme == "potpaw_PBE") {
            FilePotcars[i] = DEFAULT_VASP_POTCAR_DIR_POTPAW_PBE + "/" + AVASP_Get_PseudoPotential_PAW_PBE(cleanname);
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme == "potpaw_GGA") {
            FilePotcars[i] = DEFAULT_VASP_POTCAR_DIR_POTPAW_GGA + "/" + AVASP_Get_PseudoPotential_PAW_GGA(cleanname);
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme == "potpaw_LDA") {
            FilePotcars[i] = DEFAULT_VASP_POTCAR_DIR_POTPAW_LDA + "/" + AVASP_Get_PseudoPotential_PAW_LDA(cleanname);
          }

          if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme == "pot_PBE") {
            FilePotcars[i] = DEFAULT_VASP_POTCAR_DIR_POT_PBE + "/" + AVASP_Get_PseudoPotential_PBE(cleanname);
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme == "pot_GGA") {
            FilePotcars[i] = DEFAULT_VASP_POTCAR_DIR_POT_GGA + "/" + AVASP_Get_PseudoPotential_GGA(cleanname);
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.xscheme == "pot_LDA") {
            FilePotcars[i] = DEFAULT_VASP_POTCAR_DIR_POT_LDA + "/" + AVASP_Get_PseudoPotential_LDA(cleanname);
          }

          // ME20190211 - Occupy species_pp
          aurostd::string2tokens(FilePotcars[i], tokens, "/");
          if (!tokens.empty()) {
            xvasp.str.species_pp[i] = tokens.back();
          }

          if (LDEBUG) {
            cerr << "[VASP_Produce_POTCAR]: cleanname=" << cleanname << endl;
          }
          if (LDEBUG) {
            cerr << "[VASP_Produce_POTCAR]: FilePotcars.at(" << i << ")=" << FilePotcars[i] << endl;
          }
        }
      }
      ...
      // PRINT POTCARS
      for (size_t i = 0; i < FilePotcars.size(); i++) {
        aus << "00000  MESSAGE POTCAR  Loading potcar file = [" << FilePotcars[i] << "]" << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      ...
      for (size_t i = 0; i < FilePotcars.size(); i++) { // cycle through all POTCARS
        aurostd::StringSubstInPlace(FilePotcars[i], "//", "/");
        // strip everything
        tokens.clear();
        FilePotcarNaked = FilePotcars[i];
        for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
          FilePotcarNaked = aurostd::RemoveSubStringFirst(FilePotcarNaked, "/POTCAR" + XHOST.vext[iext]);
        }
        aurostd::string2tokens(FilePotcarNaked, tokens, "/");
        xvasp.POTCAR_POTENTIALS << tokens.at(tokens.size() - 1);
        // now build it

        if (KBIN::VASP_Find_FILE_POTCAR(FilePotcars[i], FilePotcar, DataPotcar, AUIDPotcar)) {
          ...
          aus << "EEEEE  POTCAR [" << FilePotcars[i].c_str() << "] not found! (aflow_ivasp.cpp) " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    if (Krun && vflags.KBIN_VASP_POTCAR_MODE.flag("EXPLICIT")) { // [VASP_POTCAR_MODE_EXPLICIT] construction
      // Prepare POTCAR
      aus << "00000  MESSAGE POTCAR  generation EXPLICIT " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      {
        ifstream FileINPUT;
        const string FileNameINPUT = xvasp.Directory + "/" + _AFLOWIN_;
        string line;
        bool lflag = false;
        FileINPUT.open(FileNameINPUT.c_str(), std::ios::in);
        while (getline(FileINPUT, line)) {
          if (lflag == true) {
            xvasp.POTCAR << line << endl;
          }
          //	if(lflag==true)  xvasp.POTCAR+=line+"\n";
          if (aurostd::substring2bool(line, "[VASP_POTCAR_MODE_EXPLICIT]")) {
            lflag = true;
          }
        }
        FileINPUT.clear();
    ...
    if (Krun && vflags.KBIN_VASP_POTCAR_MODE.flag("EXTERNAL")) { // [VASP_POTCAR_MODE_EXTERNAL] construction
      string file;
      aus << "00000  MESSAGE POTCAR  generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      if (vflags.KBIN_VASP_POTCAR_FILE.flag("COMMAND") && vflags.KBIN_VASP_POTCAR_FILE.flag("FILE")) {
        aus << "EEEEE   [VASP_POTCAR_MODE]FILE=  and  [VASP_POTCAR_MODE]COMMAND=  can not be used together " << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
        Krun = false;
        return Krun;
      }
      if (!vflags.KBIN_VASP_POTCAR_FILE.flag("COMMAND") && (vflags.KBIN_VASP_POTCAR_FILE.flag("FILE") || !vflags.KBIN_VASP_POTCAR_FILE.flag("FILE"))) {
        if (vflags.KBIN_VASP_POTCAR_FILE.flag("FILE")) {
          ...
          aus << "00000  MESSAGE POTCAR  generation from DEFAULT file=" << DEFAULT_VASP_EXTERNAL_POTCAR << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        if (!aurostd::FileExist(file)) {
          aus << "EEEEE  ERROR POTCAR file=" << file << " does not exist! " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
          Krun = false;
          return Krun;
        }
        if (aurostd::FileEmpty(file)) {
          aus << "EEEEE  ERROR POTCAR file=" << file << " is empty! " << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintErrorStream(FileMESSAGE, aus, XHOST.QUIET);
          Krun = false;
          return Krun;
        }
    ...
    xPOTCAR potcar;
    potcar.GetProperties(xvasp.POTCAR);
    xvasp.POTCAR_ENMAX = potcar.ENMAX;
    xvasp.POTCAR_ENMIN = potcar.ENMIN;
    aus << "00000  MESSAGE POTCAR  ENMAX max   = " << xvasp.POTCAR_ENMAX << " - " << Message(__AFLOW_FILE__, aflags) << endl;
    aus << "00000  MESSAGE POTCAR  ENMIN min   = " << xvasp.POTCAR_ENMIN << " - " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);

    // get xvasp.POTCAR_PAW **************************************************
    xvasp.POTCAR_PAW = potcar.POTCAR_PAW;
    xvasp.POTCAR_TYPE = potcar.POTCAR_TYPE;
    aus << "00000  MESSAGE POTCAR  PAW = " << (xvasp.POTCAR_PAW ? "true" : "false") << " - " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    aus << "00000  MESSAGE POTCAR  POTCAR_TYPE=" << xvasp.POTCAR_TYPE << " - " << Message(__AFLOW_FILE__, aflags) << endl;
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
...
namespace KBIN {
  bool VASP_Modify_POTCAR(_xvasp& xvasp, ofstream& FileMESSAGE, _aflags& aflags, _vflags& vflags) {
...
namespace KBIN {
  bool VASP_Reread_POTCAR(_xvasp& xvasp, ofstream& FileMESSAGE, _aflags& aflags) { // AFLOW_FUNCTION_IMPLEMENTATION
    if (!aurostd::FileExist(xvasp.Directory + "/POTCAR")) {
      pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "POTCAR not present in directory: " + xvasp.Directory, aflags, FileMESSAGE, std::cout, _LOGGER_ERROR_);
      return false;
    }
    aurostd::StringstreamClean(xvasp.POTCAR_orig);
    xvasp.POTCAR_orig << xvasp.POTCAR.str();
    aurostd::StringstreamClean(xvasp.POTCAR);
    xvasp.POTCAR << aurostd::file2string(xvasp.Directory + "/POTCAR"); // DID REREAD
...
namespace KBIN {
  void XVASP_Get_NPAR_NCORE(const _xvasp& xvasp, const _aflags& aflags, int& NPAR, int& NCORE) { // CO20210315
    ...
    if (xvasp.NCPUS > 0 && xvasp.NCPUS <= 32) {
      NPAR = 2;
      NCORE = xvasp.NCPUS / NPAR;
    }
    if (xvasp.NCPUS > 32 && xvasp.NCPUS <= 64) {
      NPAR = 4;
      NCORE = xvasp.NCPUS / NPAR;
    }
    if (xvasp.NCPUS > 64 && xvasp.NCPUS <= 128) {
      NPAR = 8;
      NCORE = xvasp.NCPUS / NPAR;
    }
    if (xvasp.NCPUS > 128 && xvasp.NCPUS <= 256) {
      NPAR = 16;
      NCORE = xvasp.NCPUS / NPAR;
    }
    if (xvasp.NCPUS > 256 && xvasp.NCPUS <= 512) {
      NPAR = 32;
      NCORE = xvasp.NCPUS / NPAR;
    }
    if (xvasp.NCPUS > 512 && xvasp.NCPUS <= 1024) {
      NPAR = 64;
      NCORE = xvasp.NCPUS / NPAR;
    }
    if (xvasp.NCPUS > 1024) {
    ...
    if (xvasp.NCPUS == 4) { // best 4 times
      ...
      NPAR = 1;
      NCORE = 4; // trying to make NPAR*NCORE=NCPUS
    }
    if (xvasp.NCPUS == 6) { // best 4 times
      ...
      NPAR = 3;
      NCORE = 2; // trying to make NPAR*NCORE=NCPUS
    }
    if (xvasp.NCPUS == 8) { // best 4 times
      ...
      NPAR = 2;
      NCORE = 4; // trying to make NPAR*NCORE=NCPUS if possible
    }
    if (xvasp.NCPUS == 12) { // best times
      ...
      NPAR = 3;
      NCORE = 2; // trying to make NPAR*NCORE=NCPUS if possible
    }
    if (xvasp.NCPUS == 14) { // best times
      ...
      NPAR = 2;
      NCORE = 7; // trying to make NPAR*NCORE=NCPUS if possible
    }
    if (xvasp.NCPUS == 16) { // best times
      ...
      NPAR = 4;
      NCORE = 8; // trying to make NPAR*NCORE=NCPUS if possible
    }
    if (xvasp.NCPUS == 24) { // best times
      ...
      NPAR = 3;
      NCORE = 12; // trying to make NPAR*NCORE=NCPUS if possible
    }
    ...
    if (xvasp.NCPUS == 28) { // best times  (delay %)
      ...
      NPAR = 4;
      NCORE = 6; // trying to make NPAR*NCORE=NCPUS if possible
    }
...
namespace KBIN {
  void XVASP_MPI_Autotune(_xvasp& xvasp, _aflags& aflags, bool VERBOSE) {
    ...
    for (size_t i = 1; i <= imax; i++) {
      ...
      if (aurostd::substring2bool(strline, "IALGO", true) || aurostd::substring2bool(strline, "#IALGO", true) || aurostd::substring2bool(strline, "LPLANE", true) ||
          aurostd::substring2bool(strline, "#LPLANE", true) || aurostd::substring2bool(strline, "NPAR", true) || aurostd::substring2bool(strline, "#NPAR", true) || aurostd::substring2bool(strline, "LSCALU", true) ||
          aurostd::substring2bool(strline, "#LSCALU", true) || aurostd::substring2bool(strline, "NSIM", true) || aurostd::substring2bool(strline, "#NSIM", true)) {
        if (VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::VASP_MPI_Autotune)" << endl;
        }
      } else {
        if (!VERBOSE && !strline.empty()) {
    ...
    // xvasp.INCAR << endl;
    if (VERBOSE) {
      xvasp.INCAR << "# Performing MPI-AUTOTUNE [AFLOW] begin" << endl;
    }
    if (VERBOSE) {
      xvasp.INCAR << "# MPI tuning for Linux cluster with CPUS=" << xvasp.NCPUS << endl;
    }
    if (VERBOSE) {
      xvasp.INCAR << "# look at the VASP manual" << endl;
    }
    ...
    if (VERBOSE) {
      xvasp.INCAR << "# Performing MPI-AUTOTUNE [AFLOW] end" << endl;
    }
  }
} // namespace KBIN
...
namespace KBIN {
  void XVASP_INCAR_System_Auto(_xvasp& xvasp, bool VERBOSE) {
    ...
    aurostd::StringstreamClean(xvasp.INCAR);
    xvasp.aopts.flag("FLAG::XVASP_INCAR_changed", true);
    const size_t imax = aurostd::GetNLinesString(FileContent);
    for (size_t i = 1; i <= imax; i++) {
      strline = aurostd::GetLineString(FileContent, i);
      if (aurostd::substring2bool(strline, "SYSTEM", true) || aurostd::substring2bool(strline, "#SYSTEM", true) || aurostd::substring2bool(strline, "INFO", true) ||
          aurostd::substring2bool(strline, "#INFO", true) || aurostd::substring2bool(strline, "PROTOTYPE", true) || aurostd::substring2bool(strline, "#PROTOTYPE", true)) {
        if (VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_System_Auto)" << endl;
        }
      } else {
        if (!VERBOSE && !strline.empty()) {
          xvasp.INCAR << strline << endl;
...
// ***************************************************************************
namespace KBIN {
  void XVASP_INCAR_Relax_ON(_xvasp& xvasp, _vflags& vflags, int number) {
    string FileContent;
    string strline;
    size_t imax;
    size_t isif = 3;
    if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS_CELL_VOLUME") == false) { // whatever is the number
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS") == true && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_SHAPE") == false && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_VOLUME") == false) {
        isif = 2; // 0,1,2 FIX
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS") == true && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_SHAPE") == true && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_VOLUME") == true) {
        isif = 3;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS") == true && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_SHAPE") == true && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_VOLUME") == false) {
        isif = 4;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS") == false && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_SHAPE") == true && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_VOLUME") == false) {
        isif = 5;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS") == false && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_SHAPE") == true && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_VOLUME") == true) {
        isif = 6;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS") == false && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_SHAPE") == false && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("CELL_VOLUME") == true) {
        isif = 7;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("ALL")) {
        isif = 3;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS_CELL_SHAPE")) {
        isif = 4; // AS20201208
      }
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.flag("IONS_CELL_VOLUME")) { // whatever is the number
      if (aurostd::_isodd(number)) {
        isif = 7; // VOLUME
      }
      if (aurostd::_iseven(number)) {
        isif = 2; // IONS
      }
    }
    ...
    const bool nelm_patch = (aurostd::kvpair2bool(FileContent, "NELM", "=") == false);
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.xscheme == "ENERGY") {
      for (size_t i = 1; i <= imax; i++) {
        strline = aurostd::GetLineString(FileContent, i);
        if (aurostd::substring2bool(strline, "IBRION", true) || aurostd::substring2bool(strline, "#IBRION", true) || aurostd::substring2bool(strline, "NSW", true) ||
            aurostd::substring2bool(strline, "#NSW", true) || aurostd::substring2bool(strline, "LORBIT", true) || aurostd::substring2bool(strline, "#LORBIT", true) || // CO20180130 get spinD
            aurostd::substring2bool(strline, "ISIF", true) || aurostd::substring2bool(strline, "#ISIF", true) ||
            (nelm_patch && (aurostd::substring2bool(strline, "NELM", true) || aurostd::substring2bool(strline, "#NELM", true)) &&
             (!(aurostd::substring2bool(strline, "NELMIN", true) || aurostd::substring2bool(strline, "#NELMIN", true) || aurostd::substring2bool(strline, "NELMDL", true) ||
                aurostd::substring2bool(strline, "#NELMDL", true)))) || // CO20200624
            false) {
          if (vflags.KBIN_VASP_INCAR_VERBOSE) {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_Relax_ON)" << endl;
          }
        } else {
          if (!vflags.KBIN_VASP_INCAR_VERBOSE && !strline.empty()) {
            xvasp.INCAR << strline << endl;
          }
          if (vflags.KBIN_VASP_INCAR_VERBOSE) {
            xvasp.INCAR << strline << endl;
      ...
      if (vflags.KBIN_VASP_INCAR_VERBOSE) {
        xvasp.INCAR << "# Performing RELAX=ENERGY [AFLOW] begin" << endl;
      }
      ...
      if (vflags.KBIN_VASP_INCAR_VERBOSE) {
        xvasp.INCAR << "# Performing RELAX=ENERGY [AFLOW] end" << endl;
      }
    }
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.xscheme == "FORCES") {
      for (size_t i = 1; i <= imax; i++) {
        strline = aurostd::GetLineString(FileContent, i);
        if (aurostd::substring2bool(strline, "IBRION", true) || aurostd::substring2bool(strline, "#IBRION", true) || aurostd::substring2bool(strline, "NELMIN", true) ||
            aurostd::substring2bool(strline, "#NELMIN", true) || aurostd::substring2bool(strline, "ADDGRID", true) || aurostd::substring2bool(strline, "#ADDGRID", true) ||
            aurostd::substring2bool(strline, "EDIFFG", true) || aurostd::substring2bool(strline, "#EDIFFG", true) || aurostd::substring2bool(strline, "LORBIT", true) ||
            aurostd::substring2bool(strline, "#LORBIT", true) || // CO20180130 get spinD
            aurostd::substring2bool(strline, "NSW", true) || aurostd::substring2bool(strline, "#NSW", true) || aurostd::substring2bool(strline, "ISIF", true) || aurostd::substring2bool(strline, "#ISIF", true) ||
            (nelm_patch && (aurostd::substring2bool(strline, "NELM", true) || aurostd::substring2bool(strline, "#NELM", true)) &&
             (!(aurostd::substring2bool(strline, "NELMIN", true) || aurostd::substring2bool(strline, "#NELMIN", true) || aurostd::substring2bool(strline, "NELMDL", true) ||
                aurostd::substring2bool(strline, "#NELMDL", true)))) || // CO20200624
            false) {
      ...
      if (vflags.KBIN_VASP_INCAR_VERBOSE) {
        xvasp.INCAR << "# Performing RELAX_MODE=FORCES [AFLOW] begin" << endl;
      }
      ...
      if (vflags.KBIN_VASP_INCAR_VERBOSE) {
        xvasp.INCAR << "# Performing RELAX_MODE=FORCES [AFLOW] end" << endl;
      }
    }
  ...
  void XVASP_INCAR_Static_ON(_xvasp& xvasp, _vflags& vflags) {
    ...
    for (size_t i = 1; i <= imax; i++) {
      ...
      if (aurostd::substring2bool(strline, "IBRION=0", true) || aurostd::substring2bool(strline, "#IBRION=0", true) || // std md
          ...
          aurostd::substring2bool(strline, "NSW", true) || aurostd::substring2bool(strline, "#NSW", true) || aurostd::substring2bool(strline, "LORBIT", true) ||
          aurostd::substring2bool(strline, "#LORBIT", true) || // CO20180130 get spinD
          ...
          false) {
        if (vflags.KBIN_VASP_INCAR_VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_Static_ON)" << endl;
        }
      } else {
        if (!vflags.KBIN_VASP_INCAR_VERBOSE && !strline.empty()) {
          xvasp.INCAR << strline << endl;
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry && vflags.KBIN_VASP_FORCE_OPTION_BADER.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LAECHG=.true.", _incarpad_) << " # Performing STATIC  (Bader ON)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry && !vflags.KBIN_VASP_FORCE_OPTION_BADER.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LAECHG=.false.", _incarpad_) << " # Performing STATIC  (Bader OFF)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry && vflags.KBIN_VASP_FORCE_OPTION_ELF.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LELF=.true.", _incarpad_) << " # Performing STATIC  (Elf ON)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry && !vflags.KBIN_VASP_FORCE_OPTION_ELF.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LELF=.false.", _incarpad_) << " # Performing STATIC  (Elf OFF)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_ALGO.preserved == false) {
      xvasp.INCAR << aurostd::PaddedPOST("ALGO=Normal", _incarpad_) << " # Performing STATIC" << endl;
    }
    ...
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "#removing IBRION, NSW, ISIF" << endl;
    }
  }
} // namespace KBIN
...
namespace KBIN {
  void XVASP_INCAR_Relax_Static_ON(_xvasp& xvasp, _vflags& vflags) {
    ...
    for (size_t i = 1; i <= imax; i++) {
      strline = aurostd::GetLineString(FileContent, i);
      if (((aurostd::substring2bool(strline, "PREC", true) && !aurostd::substring2bool(strline, "SYMPREC", true)) && vflags.KBIN_VASP_FORCE_OPTION_PREC.preserved == false) || // CO20171003 - don't confuse PREC and SYMPREC
          ...
          (aurostd::substring2bool(strline, "ALGO", true) && vflags.KBIN_VASP_FORCE_OPTION_ALGO.preserved == false) || // SC from ICSD
          (aurostd::substring2bool(strline, "#ALGO", true) && vflags.KBIN_VASP_FORCE_OPTION_ALGO.preserved == false) || // SC from ICSD
          aurostd::substring2bool(strline, "ISMEAR", true) || aurostd::substring2bool(strline, "#ISMEAR", true) || aurostd::substring2bool(strline, "SIGMA", true) ||
          aurostd::substring2bool(strline, "#SIGMA", true) || aurostd::substring2bool(strline, "ISIF", true) || aurostd::substring2bool(strline, "#ISIF", true) ||
          aurostd::substring2bool(strline, "IBRION", true) || aurostd::substring2bool(strline, "#IBRION", true) || aurostd::substring2bool(strline, "NSW", true) || aurostd::substring2bool(strline, "#NSW", true) ||
          ((aurostd::substring2bool(strline, "NELM", true) || aurostd::substring2bool(strline, "#NELM", true)) &&
           (!(aurostd::substring2bool(strline, "NELMIN", true) || aurostd::substring2bool(strline, "#NELMIN", true) || aurostd::substring2bool(strline, "NELMDL", true) ||
              aurostd::substring2bool(strline, "#NELMDL", true)))) || // CO20200624
          aurostd::substring2bool(strline, "NELMIN", true) ||
          aurostd::substring2bool(strline, "#NELMIN", true) || aurostd::substring2bool(strline, "LORBIT", true) || aurostd::substring2bool(strline, "#LORBIT", true) ||
          aurostd::substring2bool(strline, "RWIGS", true) || aurostd::substring2bool(strline, "#RWIGS", true) || aurostd::substring2bool(strline, "LCHARG", true) || aurostd::substring2bool(strline, "#LCHARG", true) ||
          (aurostd::substring2bool(strline, "LAECHG", true) && vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry) || (aurostd::substring2bool(strline, "#LAECHG", true) && vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry) ||
          aurostd::substring2bool(strline, "LWAVE", true) || aurostd::substring2bool(strline, "#LWAVE", true) || (aurostd::substring2bool(strline, "LELF", true) && vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry) ||
          (aurostd::substring2bool(strline, "#LELF", true) && vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry) || aurostd::substring2bool(strline, "LPARD", true) || aurostd::substring2bool(strline, "#LPARD", true) ||
          aurostd::substring2bool(strline, "IBAND", true) || aurostd::substring2bool(strline, "#IBAND", true) || aurostd::substring2bool(strline, "LSEPB", true) || aurostd::substring2bool(strline, "#LSEPB", true)) {
        if (vflags.KBIN_VASP_INCAR_VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_Relax_Static_ON)" << endl;
        }
      } else {
        if (!vflags.KBIN_VASP_INCAR_VERBOSE && !strline.empty()) {
          xvasp.INCAR << strline << endl;
    ...
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "# Performing RELAX_STATIC [AFLOW]" << endl;
    }
    xvasp.INCAR << aurostd::PaddedPOST("ISMEAR=" + aurostd::utype2string(vflags.KBIN_VASP_FORCE_OPTION_ISMEAR_STATIC_EQUAL.content_int), _incarpad_)
                << " # Performing RELAX_STATIC (tetrahedron method with Blochl corrections : wahyu/stefano 2009/10/09)" << endl; // CO20200624
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << aurostd::PaddedPOST("", _incarpad_) << " # with -5 forces are not correct in metals, so it is good" << endl;
    }
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << aurostd::PaddedPOST("", _incarpad_) << " # for semiconductors/insulator or for NO relaxations" << endl;
    }
    xvasp.INCAR << aurostd::PaddedPOST("SIGMA=" + aurostd::utype2string(vflags.KBIN_VASP_FORCE_OPTION_SIGMA_STATIC_EQUAL.content_double, _IVASP_DOUBLE2STRING_PRECISION_), _incarpad_)
                << " # Performing RELAX_STATIC (so the DOS will not spill too much from band edges)" << endl; // CO20200624
    if (vflags.KBIN_VASP_FORCE_OPTION_ALGO.preserved == false) {
      xvasp.INCAR << aurostd::PaddedPOST("ALGO=Normal", _incarpad_) << " # Performing RELAX_STATIC (stefano from ICSD)" << endl;
    }
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry && vflags.KBIN_VASP_FORCE_OPTION_BADER.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LAECHG=.true.", _incarpad_) << " # Performing RELAX_STATIC  (Bader ON)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry && !vflags.KBIN_VASP_FORCE_OPTION_BADER.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LAECHG=.false.", _incarpad_) << " # Performing RELAX_STATIC  (Bader OFF)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry && vflags.KBIN_VASP_FORCE_OPTION_ELF.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LELF=.true.", _incarpad_) << " # Performing RELAX_STATIC  (Elf ON)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry && !vflags.KBIN_VASP_FORCE_OPTION_ELF.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LELF=.false.", _incarpad_) << " # Performing RELAX_STATIC  (Elf OFF)" << endl;
    }
    xvasp.INCAR << aurostd::PaddedPOST("LWAVE=.false.", _incarpad_) << " # Performing RELAX_STATIC" << endl;
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << aurostd::PaddedPOST("#LPARD=.true.", _incarpad_) << " # Performing RELAX_STATIC" << endl;
    }
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << aurostd::PaddedPOST("#IBAND=8 9", _incarpad_) << " # Performing RELAX_STATIC" << endl;
    }
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << aurostd::PaddedPOST("#LSEPB=.true.", _incarpad_) << " # Performing RELAX_STATIC" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_PREC.preserved == false) {
      xvasp.INCAR << aurostd::PaddedPOST("PREC=Accurate", _incarpad_) << " # Performing RELAX_STATIC (aleksey)" << endl;
    }
    ...
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "#adjusting ISMEAR,SIGMA,ISIF,IBRION,NSW,NELM,NELMIN,LORBIT,LCHARG,LAECHG,LWAVE,LELF,LPARD,IBAND,LSEPB" << endl;
    }
    ...
    if (xvasp.POTCAR_TYPE == "LDA" || xvasp.POTCAR_TYPE == "GGA") {
      ...
      ofstream oss;
      KBIN::XVASP_INCAR_RWIGS_Static(xvasp, vflags, oss, true);
    }
  }
} // namespace KBIN
  ...
  void XVASP_INCAR_Bands_ON(_xvasp& xvasp, _vflags& vflags) {
    ...
    for (size_t i = 1; i <= imax; i++) {
      ...
      if (((aurostd::substring2bool(strline, "PREC", true) && !aurostd::substring2bool(strline, "SYMPREC", true)) && vflags.KBIN_VASP_FORCE_OPTION_PREC.preserved == false) || // CO20171003 - don't confuse PREC and SYMPREC
          (aurostd::substring2bool(strline, "#PREC", true) && vflags.KBIN_VASP_FORCE_OPTION_PREC.preserved == false) || // Aleksey
          aurostd::substring2bool(strline, "ISMEAR", true) || aurostd::substring2bool(strline, "#ISMEAR", true) || aurostd::substring2bool(strline, "SIGMA", true) ||
          aurostd::substring2bool(strline, "#SIGMA", true) || aurostd::substring2bool(strline, "IALGO", true) || aurostd::substring2bool(strline, "#IALGO", true) || // SC from ICSD
          (aurostd::substring2bool(strline, "ALGO", true) && vflags.KBIN_VASP_FORCE_OPTION_ALGO.preserved == false) || // SC from ICSD
          (aurostd::substring2bool(strline, "#ALGO", true) && vflags.KBIN_VASP_FORCE_OPTION_ALGO.preserved == false) || // SC from ICSD
          aurostd::substring2bool(strline, "ISIF", true) || aurostd::substring2bool(strline, "#ISIF", true) || aurostd::substring2bool(strline, "IBRION", true) ||
          aurostd::substring2bool(strline, "#IBRION", true) || aurostd::substring2bool(strline, "NSW", true) || aurostd::substring2bool(strline, "#NSW", true) ||
          ((aurostd::substring2bool(strline, "NELM", true) || aurostd::substring2bool(strline, "#NELM", true)) &&
           (!(aurostd::substring2bool(strline, "NELMIN", true) || aurostd::substring2bool(strline, "#NELMIN", true) || aurostd::substring2bool(strline, "NELMDL", true) ||
              aurostd::substring2bool(strline, "#NELMDL", true)))) || // CO20200624
          aurostd::substring2bool(strline, "NELMIN", true) ||
          aurostd::substring2bool(strline, "#NELMIN", true) || aurostd::substring2bool(strline, "EMIN", true) || aurostd::substring2bool(strline, "#EMIN", true) || // Aleksey
          aurostd::substring2bool(strline, "EMAX", true) || aurostd::substring2bool(strline, "#EMAX", true) || // Aleksey
          aurostd::substring2bool(strline, "NEDOS", true) || aurostd::substring2bool(strline, "#NEDOS", true) || // Aleksey
          aurostd::substring2bool(strline, "LORBIT", true) || aurostd::substring2bool(strline, "#LORBIT (Wahyu REMOVED, CORRECT)", true) || aurostd::substring2bool(strline, "RWIGS", true) ||
          aurostd::substring2bool(strline, "#RWIGS", true) || aurostd::substring2bool(strline, "LELF", true) || aurostd::substring2bool(strline, "#LELF (Wahyu REMOVED, CORRECT)", true) ||
          aurostd::substring2bool(strline, "ICHARG", true) || aurostd::substring2bool(strline, "#ICHARG", true) || aurostd::substring2bool(strline, "LCHARG", true) ||
          aurostd::substring2bool(strline, "#LCHARG", true) || (aurostd::substring2bool(strline, "LAECHG", true) && vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry) ||
          (aurostd::substring2bool(strline, "#LAECHG", true) && vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry) || (aurostd::substring2bool(strline, "LELF", true) && vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry) ||
          (aurostd::substring2bool(strline, "#LELF", true) && vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry) || aurostd::substring2bool(strline, "LWAVE", true) || aurostd::substring2bool(strline, "#LWAVE", true)) {
        if (vflags.KBIN_VASP_INCAR_VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_Bands_ON)" << endl;
        }
      } else {
        if (vflags.KBIN_VASP_INCAR_VERBOSE || !strline.empty()) {
          xvasp.INCAR << strline << endl;
    ...
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "# Performing RELAX_STATIC_BANDS [AFLOW]" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_PREC.preserved == false) {
      xvasp.INCAR << aurostd::PaddedPOST("PREC=Accurate", _incarpad_) << " # Performing RELAX_STATIC_BANDS (aleksey)" << endl;
    }
    xvasp.INCAR << aurostd::PaddedPOST("ISMEAR=" + aurostd::utype2string(vflags.KBIN_VASP_FORCE_OPTION_ISMEAR_BANDS_EQUAL.content_int), _incarpad_)
                << " # Performing RELAX_STATIC_BANDS (put back Gauss, use 1 if metals)" << endl; // CO20200624
    xvasp.INCAR << aurostd::PaddedPOST("SIGMA=" + aurostd::utype2string(vflags.KBIN_VASP_FORCE_OPTION_SIGMA_BANDS_EQUAL.content_double, _IVASP_DOUBLE2STRING_PRECISION_), _incarpad_)
                << " # Performing RELAX_STATIC_BANDS (so the DOS will not spill too much from band edges)" << endl; // CO20200624
    if (vflags.KBIN_VASP_FORCE_OPTION_ALGO.preserved == false) {
      xvasp.INCAR << aurostd::PaddedPOST("ALGO=Normal", _incarpad_) << " # Performing RELAX_STATIC_BANDS (stefano from ICSD)" << endl;
    }
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry && vflags.KBIN_VASP_FORCE_OPTION_BADER.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LAECHG=.true.", _incarpad_) << " # Performing RELAX_STATIC_BANDS (Bader ON)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_BADER.isentry && !vflags.KBIN_VASP_FORCE_OPTION_BADER.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LAECHG=.false.", _incarpad_) << " # Performing RELAX_STATIC_BANDS (Bader OFF)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry && vflags.KBIN_VASP_FORCE_OPTION_ELF.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LELF=.true.", _incarpad_) << " # Performing RELAX_STATIC_BANDS (Elf ON)" << endl;
    }
    if (vflags.KBIN_VASP_FORCE_OPTION_ELF.isentry && !vflags.KBIN_VASP_FORCE_OPTION_ELF.option) {
      xvasp.INCAR << aurostd::PaddedPOST("LELF=.false.", _incarpad_) << " # Performing RELAX_STATIC_BANDS (Elf OFF)" << endl;
    }

    xvasp.INCAR << aurostd::PaddedPOST("LWAVE=.false.", _incarpad_) << " # Performing RELAX_STATIC_BANDS" << endl;
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "#adjusting ISMEAR,SIGMA,ISIF,IBRION,NSW,NELM,NELMIN,ICHARG,LCHARG,LAECHG,LWAVE" << endl;
    }
...
namespace KBIN {
  void XVASP_INCAR_Dielectric_ON(_xvasp& xvasp, _vflags& vflags, _aflags& aflags) {
    ...
    for (size_t i = 1; i <= imax; i++) {
      strline = aurostd::GetLineString(FileContent, i);
      if (aurostd::substring2bool(strline, "ALGO", true) || aurostd::substring2bool(strline, "#ALGO", true) || aurostd::substring2bool(strline, "LOPTICS", true) ||
          aurostd::substring2bool(strline, "#LOPTICS", true) || aurostd::substring2bool(strline, "NEDOS", true) || aurostd::substring2bool(strline, "#NEDOS", true) ||
          aurostd::substring2bool(strline, "ISMEAR", true) || aurostd::substring2bool(strline, "#ISMEAR", true) || aurostd::substring2bool(strline, "SIGMA", true) ||
          aurostd::substring2bool(strline, "#SIGMA", true) || aurostd::substring2bool(strline, "EDIFF", true) || aurostd::substring2bool(strline, "#EDIFF", true) ||
          aurostd::substring2bool(strline, "NBANDS", true) || aurostd::substring2bool(strline, "#NBANDS", true)) {
        if (vflags.KBIN_VASP_INCAR_VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_Dielectric_ON)" << endl;
        }
      } else {
        if (vflags.KBIN_VASP_INCAR_VERBOSE || !strline.empty()) {
          xvasp.INCAR << strline << endl;
    ...
    xvasp.INCAR << aurostd::PaddedPOST("ALGO=EXACT", _incarpad_) << " # Performing DIELECTRIC" << endl;
    xvasp.INCAR << aurostd::PaddedPOST("LOPTICS=.true.", _incarpad_) << " # Performing DIELECTRIC" << endl;
    xvasp.INCAR << aurostd::PaddedPOST("CSHIFT=0.20", _incarpad_) << " # Performing DIELECTRIC" << endl;
    xvasp.INCAR << aurostd::PaddedPOST("OMEGAMAX=40", _incarpad_) << " # Performing DIELECTRIC" << endl;
    xvasp.INCAR << aurostd::PaddedPOST("NEDOS=40000", _incarpad_) << " # Performing DIELECTRIC" << endl;
    xvasp.INCAR << aurostd::PaddedPOST("NBANDS=" + aurostd::utype2string<int>(XVASP_INCAR_GetNBANDS(xvasp, aflags) * dielectric_step_modifier), _incarpad_) << " # Performing DIELECTRIC" << endl;
    xvasp.INCAR << aurostd::PaddedPOST("ISMEAR=0", _incarpad_) << " # Performing DIELECTRIC" << endl;
    xvasp.INCAR << aurostd::PaddedPOST("SIGMA=0.01", _incarpad_) << " # Performing DIELECTRIC" << endl;
    xvasp.INCAR << aurostd::PaddedPOST("EDIFF=1E-8", _incarpad_) << " # Performing DIELECTRIC" << endl;

    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "#adjusting ALGO,LOPTICS,CSHIFT,OMEGAMAX,NEDOS,NBANDS,ISMEAR,SIGMA,EDIFF" << endl;
    }
  }
} // namespace KBIN
...
namespace KBIN {
  void XVASP_INCAR_RWIGS_Static(_xvasp& xvasp, _vflags& vflags, ofstream& FileMESSAGE, bool OPERATION) {
    ...
    size_t imax;
    if (OPERATION == true) {
      aus << "00000  MESSAGE FORCE RWIGS_STATIC  ON : [VASP_FORCE_OPTION]RWIGS_STATIC " << " - " << Message(__AFLOW_FILE__) << endl;
    }
    if (OPERATION == false) {
      aus << "00000  MESSAGE FORCE RWIGS_STATIC  OFF: [VASP_FORCE_OPTION]RWIGS_STATIC " << " - " << Message(__AFLOW_FILE__) << endl;
    }
    aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    ...
    for (size_t i = 1; i <= imax; i++) {
      strline = aurostd::GetLineString(FileContent, i);
      if (aurostd::substring2bool(strline, "RWIGS", true) || aurostd::substring2bool(strline, "#RWIGS", true) || aurostd::substring2bool(strline, "LORBIT", true) || aurostd::substring2bool(strline, "#LORBIT", true)) {
        if (vflags.KBIN_VASP_INCAR_VERBOSE && OPERATION == true) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_RWIGS_Static_ON)" << endl;
        }
        if (vflags.KBIN_VASP_INCAR_VERBOSE && OPERATION == false) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_RWIGS_Static_OFF)" << endl;
        }
      } else {
        if (!vflags.KBIN_VASP_INCAR_VERBOSE && !strline.empty()) {
          xvasp.INCAR << strline << endl;
    ...
    // xvasp.INCAR << endl;
    if (vflags.KBIN_VASP_INCAR_VERBOSE && OPERATION == true) {
      xvasp.INCAR << "# Performing RWIGS_STATIC=ON [AFLOW]" << endl;
    }
    if (vflags.KBIN_VASP_INCAR_VERBOSE && OPERATION == false) {
      xvasp.INCAR << "# Performing RWIGS_STATIC=OFF [AFLOW]" << endl;
    }
    if (OPERATION == true) {
      xvasp.INCAR << "RWIGS=";
      vector<string> tokens;
      ...
      for (size_t i = 0; i < tokens.size(); i++) {
        if (aurostd::substring2bool(tokens[i], "RWIGS")) {
    ...
    if (vflags.KBIN_VASP_INCAR_VERBOSE && OPERATION == true) {
      xvasp.INCAR << "# adjusting RWIGS/LORBIT" << endl;
    }
    if (vflags.KBIN_VASP_INCAR_VERBOSE && OPERATION == false) {
      xvasp.INCAR << "# removing RWIGS/LORBIT" << endl;
    }
  ...
  void XVASP_INCAR_Precision(_xvasp& xvasp, _vflags& vflags) {
    ...
    for (size_t i = 1; i <= imax; i++) {
        ...
        if ((aurostd::substring2bool(strline, "PREC", true) && !aurostd::substring2bool(strline, "SYMPREC", true)) || aurostd::substring2bool(strline, "#PREC", true) || // CO20171003 - don't confuse PREC and SYMPREC
          ...
          if (vflags.KBIN_VASP_INCAR_VERBOSE) {
            if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "LOW") {
              xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=LOW)" << endl;
            }
            if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "MEDIUM") {
              xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=MEDIUM)" << endl;
            }
            if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "NORMAL") {
              xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=NORMAL)" << endl;
            }
            if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "HIGH") {
              xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=HIGH)" << endl;
            }
            if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "ACCURATE") {
              xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=ACCURATE)" << endl;
            }
          }
        } else {
          if (!vflags.KBIN_VASP_INCAR_VERBOSE && !strline.empty()) {
      ...
      if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "ACCURATE" || vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "HIGH") {
        if (((aurostd::substring2bool(strline, "PREC", true) && !aurostd::substring2bool(strline, "SYMPREC", true)) || aurostd::substring2bool(strline, "#PREC", true) || // CO20171003 - don't confuse PREC and SYMPREC
             aurostd::substring2bool(strline, "ENMAX", true) || aurostd::substring2bool(strline, "#ENMAX", true) || aurostd::substring2bool(strline, "LREAL", true) || aurostd::substring2bool(strline, "#LREAL", true) ||
             aurostd::substring2bool(strline, "EDIFF", true) || aurostd::substring2bool(strline, "#EDIFF", true) || aurostd::substring2bool(strline, "ALGO", true) || aurostd::substring2bool(strline, "#ALGO", true)) &&
      ...
      if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "PHONONS") {
        if ((aurostd::substring2bool(strline, "PREC", true) && !aurostd::substring2bool(strline, "SYMPREC", true)) || aurostd::substring2bool(strline, "#PREC", true) || // CO20171003 - don't confuse PREC and SYMPREC
            aurostd::substring2bool(strline, "ENMAX", true) || aurostd::substring2bool(strline, "#ENMAX", true) || aurostd::substring2bool(strline, "LREAL", true) ||
            aurostd::substring2bool(strline, "#LREAL", true) || aurostd::substring2bool(strline, "EDIFF", true) || aurostd::substring2bool(strline, "#EDIFF", true) ||
            aurostd::substring2bool(strline, "EDIFFG", true) || aurostd::substring2bool(strline, "#EDIFFG", true) || // CO20210315
            ((aurostd::substring2bool(strline, "NELM", true) || aurostd::substring2bool(strline, "#NELM", true)) &&
             (!(aurostd::substring2bool(strline, "NELMIN", true) || aurostd::substring2bool(strline, "#NELMIN", true) || aurostd::substring2bool(strline, "NELMDL", true) ||
                aurostd::substring2bool(strline, "#NELMDL", true)))) || // CO20200624
            aurostd::substring2bool(strline, "NELMIN", true) ||
            aurostd::substring2bool(strline, "#NELMIN", true) || // CO20210315
          ...
          if (vflags.KBIN_VASP_INCAR_VERBOSE) {
            if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "PHONONS") {
              xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=PHONONS)" << endl;
            }
          }
        } else {
          if (!vflags.KBIN_VASP_INCAR_VERBOSE && !strline.empty()) {
    ...
    // xvasp.INCAR << endl;
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "# Performing Precision PREC=" << vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme << " [AFLOW] begin" << endl;
    }

    if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "LOW") {
      xvasp.INCAR << aurostd::PaddedPOST("PREC=Low", _incarpad_) << " # low and fast" << endl;
      xvasp.INCAR << aurostd::PaddedPOST("ENMAX=" + aurostd::utype2string(xvasp.POTCAR_ENMAX * DEFAULT_VASP_PREC_ENMAX_LOW, _IVASP_DOUBLE2STRING_PRECISION_), _incarpad_) << " # " << DEFAULT_VASP_PREC_ENMAX_LOW
                  << "*ENMAX (" << xvasp.POTCAR_ENMAX << ") of pseudopotentials" << endl;
    };
    if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "MEDIUM") {
      xvasp.INCAR << aurostd::PaddedPOST("PREC=Medium", _incarpad_) << " # medium reasonable" << endl;
      xvasp.INCAR << aurostd::PaddedPOST("ENMAX=" + aurostd::utype2string(xvasp.POTCAR_ENMAX * DEFAULT_VASP_PREC_ENMAX_MEDIUM, _IVASP_DOUBLE2STRING_PRECISION_), _incarpad_) << " # "
                  << DEFAULT_VASP_PREC_ENMAX_MEDIUM << "*ENMAX (" << xvasp.POTCAR_ENMAX << ") of pseudopotentials" << endl;
    };
    if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "NORMAL") {
      xvasp.INCAR << aurostd::PaddedPOST("PREC=Normal", _incarpad_) << " # avoid wrap around errors" << endl;
      xvasp.INCAR << aurostd::PaddedPOST("ENMAX=" + aurostd::utype2string(xvasp.POTCAR_ENMAX * DEFAULT_VASP_PREC_ENMAX_NORMAL, _IVASP_DOUBLE2STRING_PRECISION_), _incarpad_) << " # "
                  << DEFAULT_VASP_PREC_ENMAX_NORMAL << "*ENMAX (" << xvasp.POTCAR_ENMAX << ") of pseudopotentials" << endl;
    };
    if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "HIGH") {
      xvasp.INCAR << aurostd::PaddedPOST("PREC=High", _incarpad_) << " # avoid wrap around errors" << endl;
      xvasp.INCAR << aurostd::PaddedPOST("ENMAX=" + aurostd::utype2string(xvasp.POTCAR_ENMAX * DEFAULT_VASP_PREC_ENMAX_HIGH, _IVASP_DOUBLE2STRING_PRECISION_), _incarpad_) << " # "
                  << DEFAULT_VASP_PREC_ENMAX_HIGH << "*ENMAX (" << xvasp.POTCAR_ENMAX << ") of pseudopotentials" << endl;
    };
    if (vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme == "ACCURATE") // || vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme=="HIGH")
    { // CO20200106 - patching for auto-indenting
      xvasp.INCAR << aurostd::PaddedPOST("PREC=Accurate", _incarpad_) << " # avoid wrap around errors" << endl;
      xvasp.INCAR << aurostd::PaddedPOST("ENMAX=" + aurostd::utype2string(xvasp.POTCAR_ENMAX * DEFAULT_VASP_PREC_ENMAX_ACCURATE, _IVASP_DOUBLE2STRING_PRECISION_), _incarpad_) << " # "
                  << DEFAULT_VASP_PREC_ENMAX_ACCURATE << "*ENMAX (" << xvasp.POTCAR_ENMAX << ") of pseudopotentials" << endl;
    ...
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "# Performing Precision PREC=" << vflags.KBIN_VASP_FORCE_OPTION_PREC.xscheme << " [AFLOW] end" << endl;
    }
    //  cerr << xvasp.INCAR.str() << endl;
  }
} // namespace KBIN
...
namespace KBIN {
  void XVASP_INCAR_Metagga(_xvasp& xvasp, _vflags& vflags) {
    // TPSS | RTPSS | M06L | MBJL | SCAN | MS0 | MS1 | MS2 | NONE
    string FileContent;
    string strline;
    size_t imax;
    // cerr << xvasp.INCAR.str() << endl;
    ...
    for (size_t i = 1; i <= imax; i++) {
      ...
      if (aurostd::substring2bool(strline, "METAGGA", true) || aurostd::substring2bool(strline, "#METAGGA", true)) {
        if (vflags.KBIN_VASP_INCAR_VERBOSE) {
          if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "TPSS") {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=TPSS)" << endl;
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "RTPSS") {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=RTPSS)" << endl;
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "M06L") {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=M06L)" << endl;
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "MBJL") {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=MBJL)" << endl;
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "SCAN") {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=SCAN)" << endl;
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "MS0") {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=MS0)" << endl;
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "MS1") {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=MS1)" << endl;
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "MS2") {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=MS2)" << endl;
          }
          if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "NONE") {
            xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=NONE)" << endl;
          }
        }
      } else {
        if (!vflags.KBIN_VASP_INCAR_VERBOSE && !strline.empty()) {
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme != "NONE") {
      if (vflags.KBIN_VASP_INCAR_VERBOSE) {
        xvasp.INCAR << "# Performing Metagga METAGGA=" << vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme << " [AFLOW] begin" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "TPSS") {
        xvasp.INCAR << aurostd::PaddedPOST("METAGGA=TPSS", _incarpad_) << " # METAGGA = TPSS" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "RTPSS") {
        xvasp.INCAR << aurostd::PaddedPOST("METAGGA=RTPSS", _incarpad_) << " # METAGGA = RTPSS" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "M06L") {
        xvasp.INCAR << aurostd::PaddedPOST("METAGGA=M06L", _incarpad_) << " # METAGGA = M06L" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "MBJL") {
        xvasp.INCAR << aurostd::PaddedPOST("METAGGA=MBJL", _incarpad_) << " # METAGGA = MBJL" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "SCAN") {
        xvasp.INCAR << aurostd::PaddedPOST("METAGGA=SCAN", _incarpad_) << " # METAGGA = SCAN" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "MS0") {
        xvasp.INCAR << aurostd::PaddedPOST("METAGGA=MS0", _incarpad_) << " # METAGGA = MS0" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "MS1") {
        xvasp.INCAR << aurostd::PaddedPOST("METAGGA=MS1", _incarpad_) << " # METAGGA = MS1" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "MS2") {
        xvasp.INCAR << aurostd::PaddedPOST("METAGGA=MS2", _incarpad_) << " # METAGGA = MS2" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme == "NONE") {
        xvasp.INCAR << aurostd::PaddedPOST("METAGGA=NONE", _incarpad_) << " # METAGGA = NONE" << endl;
      }
      if (vflags.KBIN_VASP_INCAR_VERBOSE) {
        xvasp.INCAR << "# Performing Metagga METAGGA=" << vflags.KBIN_VASP_FORCE_OPTION_METAGGA.xscheme << " [AFLOW] end" << endl;
      }
    }
  }
} // namespace KBIN
...
namespace KBIN {
  void XVASP_INCAR_Ivdw(_xvasp& xvasp, _vflags& vflags) {
    // number_for_VASP_see_manual_for_IVDW | 0
    string FileContent;
    string strline;
    size_t imax;
    FileContent = xvasp.INCAR.str();
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_IVDW.xscheme != "0") {
      if (vflags.KBIN_VASP_INCAR_VERBOSE) {
        xvasp.INCAR << "# Performing Ivdw IVDW=" << vflags.KBIN_VASP_FORCE_OPTION_IVDW.xscheme << " [AFLOW] begin" << endl;
      }
      if (vflags.KBIN_VASP_FORCE_OPTION_IVDW.xscheme == "0" || vflags.KBIN_VASP_FORCE_OPTION_IVDW.xscheme == "OFF" || vflags.KBIN_VASP_FORCE_OPTION_IVDW.xscheme == "NONE" ||
          vflags.KBIN_VASP_FORCE_OPTION_IVDW.xscheme.empty()) {
      ...
      if (vflags.KBIN_VASP_INCAR_VERBOSE) {
        xvasp.INCAR << "# Performing Ivdw IVDW=" << vflags.KBIN_VASP_FORCE_OPTION_IVDW.xscheme << " [AFLOW] end" << endl;
      }
    }
  }
} // namespace KBIN
...
namespace KBIN {
  void XVASP_INCAR_ABMIX(_xvasp& xvasp, _vflags& vflags) {
    ...
    for (size_t i = 1; i <= imax; i++) {
      strline = aurostd::GetLineString(FileContent, i);
      if (aurostd::substring2bool(strline, "AMIX", true) || aurostd::substring2bool(strline, "#AMIX", true) || aurostd::substring2bool(strline, "BMIX", true) || aurostd::substring2bool(strline, "#BMIX", true)) {
        if (vflags.KBIN_VASP_INCAR_VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_ABMIX)" << endl;
        }
      } else {
        if (!vflags.KBIN_VASP_INCAR_VERBOSE && !strline.empty()) {
          xvasp.INCAR << strline << endl;
    ...
    // xvasp.INCAR << endl;
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "# Performing KBIN::XVASP_INCAR_ABMIX [AFLOW]" << endl;
    }
    uint isUS = false;
    uint isPAW = false;
    if (vflags.KBIN_VASP_FORCE_OPTION_ABMIX.xscheme.at(0) == 'A') { // AUTO
      ...
      if (xvasp.POTCAR_TYPE == "LDA") {
        isUS = true;
      }
      if (xvasp.POTCAR_TYPE == "GGA") {
        isUS = true;
      }
      if (xvasp.POTCAR_TYPE == "PAW_LDA") {
        isPAW = true;
      }
      if (xvasp.POTCAR_TYPE == "PAW_GGA") {
        isPAW = true;
      }
      if (xvasp.POTCAR_TYPE == "PAW_PBE") {
        isPAW = true;
      }
      if (xvasp.POTCAR_TYPE == "PAW_LDA_KIN") {
        isPAW = true;
      }
      if (xvasp.POTCAR_TYPE == "PAW_PBE_KIN") {
        isPAW = true;
      }
      if (xvasp.POTCAR_TYPE == "PAW_RPBE") {
        isPAW = true;
      }
      if (!isUS && !isPAW) {
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_ABMIX.xscheme == "US" || isUS) { // US
      if (isUS) {
        xvasp.INCAR << "# ABMIX=AUTO => Found US pseudopotentials" << endl;
      }
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_ABMIX.xscheme == "PAW" || isPAW) { // PAW
      if (isPAW) {
        xvasp.INCAR << "# ABMIX=AUTO => Found PAW pseudopotentials" << endl;
      }
...
namespace KBIN {
  int XVASP_INCAR_GetNBANDS_AFLOW3(const _xvasp& xvasp, const _aflags& aflags, bool ispin) {
    vector<double> vZVAL;
    const double ZVAL = GetZVAL(xvasp.POTCAR, vZVAL);
    int nbands = 0;
    ispin = true; // CO20210315 - override for safety
    int NPAR = 0;
    int NCORE = 1;
    KBIN::XVASP_Get_NPAR_NCORE(xvasp, aflags, NPAR, NCORE); // CO20210315
    nbands = GetNBANDS_AFLOW3((int) ZVAL, (int) xvasp.str.atoms.size(), 5, ispin, NPAR);
    if (!XHOST.QUIET) {
      cout << XPID << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]NBANDS  = " << nbands << endl;
    }
    return nbands;
  }
  ...
  int XVASP_INCAR_GetNBANDS(const _xvasp& xvasp, const _aflags& aflags, bool ispin) {
    vector<double> vZVAL;
    GetZVAL(xvasp.POTCAR, vZVAL);
    xstructure str_(xvasp.str);
    int electrons = str_.GetZVAL(vZVAL);
    int NPAR = 0;
    int NCORE = 1;
    KBIN::XVASP_Get_NPAR_NCORE(xvasp, aflags, NPAR, NCORE);
    return GetNBANDS_VASP_SERIAL_spineach(electrons, xvasp.str.atoms.size(), 5, ispin);
  }
} // namespace KBIN
...
namespace KBIN {
  bool XVASP_INCAR_Read_MAGMOM(_xvasp& xvasp) { // CO20210315
    ...
    if (aurostd::kvpair2bool(xvasp.INCAR, "LNONCOLLINEAR", "=") && aurostd::kvpair2string(xvasp.INCAR, "LNONCOLLINEAR", "=") == ".true.") {
      return false;
    }
    if (!aurostd::kvpair2bool(xvasp.INCAR, "MAGMOM", "=")) {
      return false;
    }
    const string value = aurostd::kvpair2string(xvasp.INCAR, "MAGMOM", "=");
    vector<string> vtokens;
    vector<string> vtokens2;
    vector<int> vmags;
    ...
    for (i = 0; i < vtokens.size(); i++) {
      if (vtokens[i].find("*") == string::npos) {
        ...
        if ((!aurostd::isfloat(vtokens2[0])) || (!aurostd::isfloat(vtokens2[1]))) {
          return false;
        }
        ...
        for (j = 0; j < (uint) natoms; j++) {
          vmags.push_back(aurostd::string2utype<int>(vtokens2[1]));
        }
      }
    }
    if (vmags.size() != xvasp.str.atoms.size()) {
      return false;
...
namespace KBIN {
  bool XVASP_INCAR_PREPARE_GENERIC(const string& command, _xvasp& xvasp, const _vflags& vflags, const string& svalue, int ivalue, double dvalue, bool OPTION) {
    ...
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    const string function = "KBIN::XVASP_INCAR_PREPARE_GENERIC";
    const string operation = function + "(" + command + ")";
    const string operation_svalue = function + "(" + command + "=" + svalue + ")";
    const string operation_ivalue = function + "(" + command + "=" + aurostd::utype2string(ivalue) + ")";
    const string operation_dvalue = function + "(" + command + "=" + aurostd::utype2string(dvalue, _IVASP_DOUBLE2STRING_PRECISION_) + ")";
    const string operation_option = function + "(" + command + "=" + (OPTION == true ? string("ON") : string("OFF")) + ")";
    ...
    string keyword;
    string incar_input;
    string incar_input_old;
    string incar_comment;
    ...
    if (command == "ALGO") {
      ...
      if (Krun) {
        if (svalue.empty()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "svalue empty (command=" + command + ")", _INPUT_ILLEGAL_);
        }
        if (svalue == "NORMAL") {
          incar_input = keyword + "=Normal";
          incar_comment = "blocked Davidson block iteration scheme (IALGO=38)";
        } else if (svalue == "VERYFAST") {
          incar_input = keyword + "=Veryfast";
          incar_comment = "RMM-DIIS (IALGO=48)";
        } else if (svalue == "FAST") {
          incar_input = keyword + "=Fast";
          incar_comment = "IALGO=38 is used for the initial phase, then VASP switches to IALGO=48";
        } else if (svalue == "ALL") {
          incar_input = keyword + "=All";
          incar_comment = "all band simultaneous update of orbitals (IALGO=58)";
        } else if (svalue == "DAMPED") {
          incar_input = keyword + "=Damped";
          incar_comment = "a damped velocity friction algorithm (IALGO=53)";
        } else {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "svalue=" + svalue + " unknown (command=" + command + ")", _INPUT_ILLEGAL_);
        }

        if (aurostd::kvpair2bool(xvasp.INCAR, keyword, "=")) {
          incar_input_old = keyword + "=" + aurostd::kvpair2string(xvasp.INCAR, keyword, "=");
          if (incar_input == incar_input_old) {
            Krun = false;
          }
        }
      }
      ...
      if (Krun) {
        ...
        // ADD LINES
        if (VERBOSE) {
          xvasp.INCAR << "# Performing " << operation_svalue << " [AFLOW] begin" << endl;
        }
        xvasp.INCAR << aurostd::PaddedPOST(incar_input, _incarpad_) << " # " << operation_svalue << " " << incar_comment << endl;
        if (VERBOSE) {
          xvasp.INCAR << "# Performing " << operation_svalue << " [AFLOW] end" << endl;
        }
      }
    }
    // ***************************************************************************
    ...
    // AMIN AMIN AMIN AMIN AMIN AMIN
      ...
      if (Krun) {
        ...
        if (std::signbit(dvalue) == false) { // use dvalue<0 to remove the key only
          if (VERBOSE) {
            xvasp.INCAR << "# Performing " << operation_dvalue << " [AFLOW] begin" << endl;
          }
          xvasp.INCAR << aurostd::PaddedPOST(incar_input, _incarpad_) << " # " << operation_dvalue << endl;
          if (VERBOSE) {
            xvasp.INCAR << "# Performing " << operation_dvalue << " [AFLOW] end" << endl;
          }
        }
      }
    }
      ...
      if (Krun) {
        ...
        if (ivalue >= 0) {
          ...
          if (Krun) {
            ...
            if (VERBOSE) {
              xvasp.INCAR << "# Performing " << operation_svalue << " CHGCAR_FILE=" << chgcar << " [AFLOW] begin" << std::endl;
            }
            xvasp.INCAR << aurostd::PaddedPOST(incar_input, _incarpad_) << " # " << operation_svalue << std::endl;
            if (VERBOSE) {
              xvasp.INCAR << "# Performing " << operation_svalue << " CHGCAR_FILE=" << chgcar << " [AFLOW] end" << std::endl;
            }
          }
        }
      }
    ...
    // ISMEAR ISMEAR ISMEAR ISMEAR ISMEAR ISMEAR
      ...
      if (Krun) {
        if (ivalue == 0) {
          incar_comment = "Gaussian smearing";
        } else if (ivalue == 1) {
          incar_comment = "Methfessel-Paxton smearing order 1";
        } else if (ivalue == 2) {
          incar_comment = "Methfessel-Paxton smearing order 2";
        } else if (ivalue == -1) {
          incar_comment = "Fermi smearing";
        } else if (ivalue == -4) {
          incar_comment = "tetrahedron smearing";
        } else if (ivalue == -5) {
          incar_comment = "tetrahedron smearing with Bloechl corrections";
        } else {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "ivalue=" + aurostd::utype2string(ivalue) + " unknown (command=" + command + ")", _INPUT_ILLEGAL_);
        }
        ...
        // ADD LINES
        if (VERBOSE) {
          xvasp.INCAR << "# Performing " << operation_ivalue << " [AFLOW] begin" << endl;
        }
        xvasp.INCAR << aurostd::PaddedPOST(incar_input, _incarpad_) << " # " << operation_ivalue << " " << incar_comment << endl;
        if (VERBOSE) {
          xvasp.INCAR << "# Performing " << operation_ivalue << " [AFLOW] end" << endl;
        }
      }
    }
    // ***************************************************************************
    ...
    // LASPH LASPH LASPH LASPH LASPH LASPH
      ...
      if (Krun) {
        incar_input = keyword + "=" + (OPTION == true ? string(".true.") : string(".false."));
        if (aurostd::kvpair2bool(xvasp.INCAR, keyword, "=")) {
          incar_input_old = keyword + "=" + aurostd::kvpair2string(xvasp.INCAR, keyword, "=");
          if (incar_input == incar_input_old) {
            Krun = false;
      ...
      if (Krun) {
        ...
        // ADD LINES
        if (VERBOSE) {
          xvasp.INCAR << "# Performing " << operation_option << " [AFLOW] begin" << endl;
        }
        xvasp.INCAR << aurostd::PaddedPOST(incar_input, _incarpad_) << " # " << operation_option << endl;
        if (VERBOSE) {
          xvasp.INCAR << "# Performing " << operation_option << " [AFLOW] end" << endl;
        }
      }
    }
    // ***************************************************************************
    ...
    // LS_COUPLING LS_COUPLING LS_COUPLING LS_COUPLING LS_COUPLING LS_COUPLING
      ...
      if (Krun) {
        incar_input = keyword + "=" + (OPTION == true ? ".true." : ".false.");
        if (aurostd::kvpair2bool(xvasp.INCAR, keyword, "=")) {
          incar_input_old = keyword + "=" + aurostd::kvpair2string(xvasp.INCAR, keyword, "=");
          if (incar_input == incar_input_old) {
            Krun = false;
    ...
    // NELM NELM NELM NELM NELM NELM
      ...
      if (Krun) {
        const bool nelm_fix = true; //[CO20210315 - always patch, you might have NELM in INCAR section of aflow.in](ivalue!=AFLOWRC_DEFAULT_VASP_FORCE_OPTION_NELM); //CO20200624 - default for VASP is 60, don't add the line if unnecessary
        if (nelm_fix == false) {
          Krun = false;
        }
      }
    ...
    // SPIN SPIN SPIN SPIN SPIN SPIN
      ...
      if (Krun) {
        incar_input = keyword + "=" + (OPTION == true ? "2" : "1");
        if (aurostd::kvpair2bool(xvasp.INCAR, keyword, "=")) {
          incar_input_old = keyword + "=" + aurostd::kvpair2string(xvasp.INCAR, keyword, "=");
          if (incar_input == incar_input_old) {
            Krun = false;
      ...
      if (Krun) {
        ...
        if (remove_magmom) {
          XVASP_INCAR_REMOVE_ENTRY(xvasp, "MAGMOM", operation_option, VERBOSE);
        } // CO20200624
        // otherwise, check if has already been specified (e.g., in INCAR section of _AFLOWIN_)
        const bool magmom_already_specified = aurostd::kvpair2bool(xvasp.INCAR, "MAGMOM", "="); // CO20210315 - must use kvpair2bool instead of substring2bool() since magmom values have spaces
        // ADD LINES
        if (VERBOSE) {
          xvasp.INCAR << "# Performing " << operation_option << " [AFLOW] begin" << endl;
        }
        ...
        if (OPTION == true && magmom_already_specified == false && vflags.KBIN_VASP_FORCE_OPTION_LSCOUPLING.option == false) { // CO
          if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_MAGMOM.isentry && vflags.KBIN_VASP_FORCE_OPTION_AUTO_MAGMOM.option) { // with spin, make the mag mom +5
            xvasp.INCAR << aurostd::PaddedPOST("MAGMOM=" + aurostd::utype2string(xvasp.str.atoms.size()) + "*5", _incarpad_) << " # " << operation_option << " " << xvasp.str.atoms.size() << " atom"
                        << (xvasp.str.atoms.size() > 1 ? "s" : "") << endl;
          } else { // otherwise, make the magmom +1
            xvasp.INCAR << aurostd::PaddedPOST("MAGMOM=" + aurostd::utype2string(xvasp.str.atoms.size()) + "*1", _incarpad_) << " # " << operation_option << " " << xvasp.str.atoms.size() << " atom"
                        << (xvasp.str.atoms.size() > 1 ? "s" : "") << " (default)" << endl;
          }
        }
        if (VERBOSE) {
          xvasp.INCAR << "# Performing " << operation_option << " [AFLOW] end" << endl;
    ...
    // SYM SYM SYM SYM SYM SYM
      ...
      if (Krun) {
        if (OPTION == true) {
          incar_input = keyword + "=2";
          incar_comment = "SYMMETRY=ON";
        } else { // OPTION==false
          // expand this list as needed in the future:
          if (vflags.KBIN_VASP_FORCE_OPTION_SPIN.isentry && vflags.KBIN_VASP_FORCE_OPTION_SPIN.option) {
            incar_input = keyword + "=-1";
            incar_comment = "SYMMETRY=OFF (MAGNETIC)";
          } // SPIN==true
          else {
            incar_input = keyword + "=0";
            incar_comment = "SYMMETRY=OFF";
          } // SPIN==false
        }
        if (aurostd::kvpair2bool(xvasp.INCAR, keyword, "=")) {
          incar_input_old = keyword + "=" + aurostd::kvpair2string(xvasp.INCAR, keyword, "=");
    ...
    // TYPE TYPE TYPE TYPE TYPE TYPE
      ...
      keyword = "ISMEAR";
      const string keyword2 = "SIGMA";
      string incar_input2;

      if (Krun) {
        if (svalue == "DEFAULT") {
          incar_input = keyword + "=1";
          incar_input2 = keyword2 + "=0.1";
          incar_comment = "for default (as metal)";
        } else if (svalue == "METAL") {
          incar_input = keyword + "=1";
          incar_input2 = keyword2 + "=0.1";
          incar_comment = "as metal";
        } else if (svalue == "SEMICONDUCTOR" || svalue == "INSULATOR") {
          incar_input = keyword + "=0";
          incar_input2 = keyword2 + "=0.05";
          incar_comment = "for insulators/semiconductors";
        } else {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "svalue=" + svalue + " unknown (command=" + command + ")", _INPUT_ILLEGAL_);
        }
        bool Krun1 = true;
        ...
        if (aurostd::kvpair2bool(xvasp.INCAR, keyword2, "=")) {
          ...
          if (incar_input2 == incar_input_old) {
            Krun2 = false;
          }
        }
        if (Krun1 == false && Krun2 == false) {
          Krun = false;
        }
      }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " RETURNING CORRECTLY" << endl;
      cerr << __AFLOW_FUNC__ << " command=" << command << endl;
      cerr << __AFLOW_FUNC__ << " svalue=" << svalue << endl;
      cerr << __AFLOW_FUNC__ << " ivalue=" << ivalue << endl;
      cerr << __AFLOW_FUNC__ << " dvalue=" << dvalue << endl;
      cerr << __AFLOW_FUNC__ << " OPTION=" << OPTION << endl;
    }
...
namespace KBIN {
  ...
  void XVASP_INCAR_ADJUST_ICHARG(_xvasp& xvasp, _vflags& vflags, _aflags& aflags, int step, bool write_incar, ofstream& FileMESSAGE) { // CO20210315 - write_input
    const string function = "KBIN::XVASP_INCAR_ADJUST_ICHARG";
    if ((step == 1) && vflags.KBIN_VASP_FORCE_OPTION_CHGCAR_FILE.isentry) {
      ...
      if (!vflags.KBIN_VASP_FORCE_OPTION_CHGCAR.option) {
        ostringstream aus;
        aus << "00000  MESSAGE ICHARG: Removing ICHARG - " << Message(__AFLOW_FILE__, aflags) << std::endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
...
namespace KBIN {
  void XVASP_INCAR_SPIN_REMOVE_RELAX(_xvasp& xvasp, _aflags& aflags, _vflags& vflags, int step, bool write_incar, ofstream& FileMESSAGE) { // CO20210315 - write_input
    const string function = "KBIN::XVASP_INCAR_SPIN_REMOVE_RELAX";
    ostringstream aus;
    bool fix_aflowin = false;
    const _kflags kflags;
    if (step == 1 && vflags.KBIN_VASP_FORCE_OPTION_SPIN_REMOVE_RELAX_1 && std::abs(xvasp.str.qm_mag_atom) < DEFAULT_VASP_SPIN_REMOVE_CUTOFF) {
      ...
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      KBIN::XVASP_INCAR_PREPARE_GENERIC("SPIN", xvasp, vflags, "", 0, 0.0, false);
      xvasp.aopts.flag("FLAG::XVASP_INCAR_changed", true);
      fix_aflowin = true;
    }
    if (step == 2 && vflags.KBIN_VASP_FORCE_OPTION_SPIN_REMOVE_RELAX_2 && std::abs(xvasp.str.qm_mag_atom) < DEFAULT_VASP_SPIN_REMOVE_CUTOFF) {
...
namespace KBIN {
  void XVASP_KPOINTS_IBZKPT_UPDATE(_xvasp& xvasp, _aflags& aflags, _vflags& vflags, int step, bool write_incar, ofstream& FileMESSAGE) { // CO20210315 - write_input
    ostringstream aus;
    // aus << "00000  MESSAGE IBZKPT.relax1 - step=" << step << " " << Message(__AFLOW_FILE__,aflags) << endl;
    // aus << "00000  MESSAGE IBZKPT.relax1 - vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("IBZKPT")=" << vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("IBZKPT") << " " << Message(__AFLOW_FILE__,aflags) << endl;
    // aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    if (step >= 1 && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.flag("IBZKPT")) {
      aus << "00000  MESSAGE FORCE KPOINTS from IBZKPT.relax1 - " << Message(__AFLOW_FILE__, aflags) << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      aurostd::StringstreamClean(xvasp.KPOINTS);
      xvasp.aopts.flag("FLAG::XVASP_KPOINTS_changed", true);
  ...
  void XVASP_INCAR_LDAU_OFF(_xvasp& xvasp, bool VERBOSE) {
    ...
    for (size_t i = 1; i <= imax; i++) {
      ...
      if (!aurostd::substring2bool(strline, "SYSTEM", true) &&
          (aurostd::substring2bool(strline, "LDAU", true) || aurostd::substring2bool(strline, "#LDAU", true) || aurostd::substring2bool(strline, "LMAXMIX", true) || aurostd::substring2bool(strline, "#LMAXMIX", true))) {
        if (VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_LDAU_OFF)" << endl;
        }
      } else {
        if (!VERBOSE && !strline.empty()) {
          xvasp.INCAR << strline << endl;
    ...
    if (VERBOSE) {
      xvasp.INCAR << "# Performing LDAU=OFF [AFLOW] begin" << endl;
    }
    xvasp.INCAR << aurostd::PaddedPOST("LDAU=.false.", _incarpad_) << " # LDAU=OFF" << endl;
    if (VERBOSE) {
      xvasp.INCAR << "# Performing LDAU=OFF [AFLOW] end" << endl;
    }
  }
} // namespace KBIN
...
namespace KBIN {
  void XVASP_INCAR_LDAU_ON(_xvasp& xvasp, _vflags& vflags, uint type) {
    ...
    for (size_t i = 1; i <= imax; i++) {
      strline = aurostd::GetLineString(FileContent, i);
      if (!aurostd::substring2bool(strline, "SYSTEM", true) && !aurostd::substring2bool(strline, "LDAUL", true) && !aurostd::substring2bool(strline, "#LDAUL", true) &&
          !aurostd::substring2bool(strline, "LDAUU", true) && !aurostd::substring2bool(strline, "#LDAUU", true) && !aurostd::substring2bool(strline, "LDAUJ", true) && !aurostd::substring2bool(strline, "#LDAUJ", true) &&
          (aurostd::substring2bool(strline, "LDAU", true) || aurostd::substring2bool(strline, "#LDAU", true) || aurostd::substring2bool(strline, "LMAXMIX", true) || aurostd::substring2bool(strline, "#LMAXMIX", true))) {
        if (vflags.KBIN_VASP_INCAR_VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_LDAU" << type << "_ON)" << endl;
        }
      } else {
        if (!vflags.KBIN_VASP_INCAR_VERBOSE && !strline.empty()) {
          xvasp.INCAR << strline << endl;
    ...
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "# Performing LDAU" << type << "=ON [AFLOW] begin" << endl;
    }
    int LMAXMIX = 6;
    ...
    vLDAUJ.clear();
    uint nspecies = 0;
    vector<string> tokens_group;
    vector<string> tokens;

    if (vflags.KBIN_VASP_LDAU_AFLOW_AUTO_flag == true) { // get them from the names
      // cerr << __AFLOW_FUNC__ << " vflags.KBIN_VASP_LDAU_AFLOW_AUTO_flag==true => get them from the names " << endl;
      if (vflags.KBIN_VASP_LDAU_SPECIES.length() > 1) {
        ...
        if (xvasp.str.species.size() != nspecies) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "xvasp.str.species.size() != nspecies=tokens.size()", _INDEX_MISMATCH_); // CO20200624
        }
        for (size_t i = 0; i < xvasp.str.species.size(); i++) {
          AVASP_Get_LDAU_Parameters(aurostd::CleanStringASCII(aurostd::VASP_PseudoPotential_CleanName(tokens.at(i))), LDAU, vLDAUspecies, vLDAUtype, vLDAUL, vLDAUU, vLDAUJ); // parameters for LDAU2
        }
      }
    } else { // get them from PARAMETERS
      // cerr << __AFLOW_FUNC__ << " vflags.KBIN_VASP_LDAU_AFLOW_AUTO_flag==false => get them from the parameters:= " << vflags.KBIN_VASP_LDAU_PARAMETERS << endl;
      LDAU = true;
      aurostd::string2tokens(vflags.KBIN_VASP_LDAU_PARAMETERS, tokens_group, ";");
      if (tokens_group.size() < 3) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "must specify 4 entries in " + vflags.KBIN_VASP_LDAU_PARAMETERS, _INPUT_NUMBER_); // CO20200624
      }
      aurostd::string2tokens(tokens_group.at(0), tokens, ",");
      nspecies = tokens.size();
      for (uint j = 0; j < nspecies; j++) {
        vLDAUspecies.emplace_back("");
        vLDAUtype.push_back(2);
        vLDAUL.push_back(-1);
        vLDAUU.push_back(0.0);
        vLDAUJ.push_back(0.0);
      } // make space
      for (size_t i = 0; i < tokens_group.size(); i++) {
        // cerr << i << " " << tokens_group[i] << endl;
        aurostd::string2tokens(tokens_group[i], tokens, ",");
        if (nspecies != tokens.size()) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "must specify " + aurostd::utype2string(nspecies) + " entries in " + tokens_group[i], _INPUT_NUMBER_); // CO20200624
        }
        //	for(uint j=0;j<nspecies;j++) cerr << tokens.at(j) << " "; cerr << endl;
        if (i == 0) {
          for (uint j = 0; j < nspecies; j++) {
            vLDAUspecies.at(j) = tokens.at(j);
          }
        }
        if (i == 1) {
          for (uint j = 0; j < nspecies; j++) {
            vLDAUL.at(j) = aurostd::string2utype<int>(tokens.at(j));
          }
        }
        if (i == 2) {
          for (uint j = 0; j < nspecies; j++) {
            vLDAUU.at(j) = aurostd::string2utype<double>(tokens.at(j));
          }
        }
        if (i == 3) {
          for (uint j = 0; j < nspecies; j++) {
            vLDAUJ.at(j) = aurostd::string2utype<double>(tokens.at(j));
          }
        }
      }
    }
    // get type
    if (xvasp.str.species.size() != nspecies) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "xvasp.str.species.size() != nspecies=tokens.size()", _INPUT_NUMBER_); // CO20200624
    }
    ...
    if (type1 == false && type2 == false) { // no LDAU
      cerr << __AFLOW_FUNC__ << " no need for LDAU, all species do not need or are not in the table : " << xvasp.str.title << endl;
    }
    if (type1 == true && type2 == false && type != 1) { // LDAU type 1
      cerr << __AFLOW_FUNC__ << " your type=" << type << " is incompatible with type1 of table : " << xvasp.str.title << endl;
    }
    if (type1 == false && type2 == true && type != 2) { // LDAU type 2
      cerr << __AFLOW_FUNC__ << " your type=" << type << " is incompatible with type2 of table : " << xvasp.str.title << endl;
    }
    ...
    for (size_t i = 0; i < vLDAUL.size(); i++) {
      if (2 * vLDAUL[i] > LMAXMIX) {
        LMAXMIX = 2 * vLDAUL[i]; // get the MAX MIX for the orbitals
      }
    }
    // FIX NEGATIVES
    for (size_t i = 0; i < vLDAUL.size(); i++) {
    ...
    stringstream straus;
    straus.clear();
    straus.str(std::string());
    straus << "#LDAU_SPECIES=";
    for (uint i = 0; i < nspecies; i++) {
      straus << vLDAUspecies.at(i) << " ";
    }
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_LDAU_ADIABATIC.content_int == 0) {
      ...
      for (int j = 1; j <= vflags.KBIN_VASP_FORCE_OPTION_LDAU_ADIABATIC.content_int; j++) {
        for (uint i = 0; i < nspecies; i++) {
          double cff = 0.0;
          if (vLDAUL.at(i) == 0) {
            cff = 0.0; // s orbital no U
          }
          if (vLDAUL.at(i) == 1) {
            cff = 0.0; // p orbital no U
          }
          if (vLDAUL.at(i) == 2) {
            cff = ((j + 1.5) / (double) (vflags.KBIN_VASP_FORCE_OPTION_LDAU_ADIABATIC.content_int)); // d orbital U
          }
          if (vLDAUL.at(i) == 3) {
            cff = ((j + 1.5) / (double) (vflags.KBIN_VASP_FORCE_OPTION_LDAU_ADIABATIC.content_int)); // f orbital U
          }
          if (cff < 0.0) {
            cff = 0.0;
          }
          ...
          if (j < vflags.KBIN_VASP_FORCE_OPTION_LDAU_ADIABATIC.content_int || i < nspecies - 1) {
            straus << ",";
          }
        }
      }
    ...
    xvasp.INCAR << aurostd::PaddedPOST("LMAXMIX=" + aurostd::utype2string(LMAXMIX), _incarpad_)
                << " # Controls up to which l-quantum number the onsite PAW charge densities are passed through the charge density mixer." << endl;
    ...
    // cerr << xvasp.INCAR.str() << endl;
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "# Performing LDAU" << type << "=ON [AFLOW] end" << endl;
    }
  }
} // namespace KBIN
...
namespace KBIN {
  void XVASP_INCAR_LDAU_ADIABATIC(_xvasp& xvasp, int step) {
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    // reload incar
    xvasp.INCAR_orig.clear();
    xvasp.INCAR_orig.str(xvasp.INCAR.str());
    xvasp.INCAR.clear();
    xvasp.INCAR.str(aurostd::file2string(xvasp.Directory + "/INCAR"));
    //  xvasp.aopts.flag("FLAG::XVASP_INCAR_changed",true);
    // operate
    const string FileContent = xvasp.INCAR.str();
    string strline;
    const size_t imax = aurostd::GetNLinesString(FileContent);
    stringstream straus;
    straus.clear();
    ...
    for (int iline = 1; iline <= imax; iline++) {
      ...
      if (!aurostd::substring2bool(strline, "LDAUL=", true) && !aurostd::substring2bool(strline, "LDAUU=", true) && !aurostd::substring2bool(strline, "LDAUJ=", true)) {
        ...
        if (strline.find("#ADIABATIC_LDAU=") != string::npos) {
          ...
          if (LDEBUG) {
            cerr << "[KBIN::XVASP_INCAR_LDAU_ADIABATIC] LDAU_ADIABATIC" << endl;
          }
          ...
          const uint nsteps = aurostd::string2utype<int>(tokens.at(t++));
          const uint nspecies = aurostd::string2utype<int>(tokens.at(t++));
          if (LDEBUG) {
            cerr << "[KBIN::XVASP_INCAR_LDAU_ADIABATIC] LDAU_STEP=" << step << endl;
          }
          if (LDEBUG) {
            cerr << "[KBIN::XVASP_INCAR_LDAU_ADIABATIC] LDAU_ADIABATIC_NSTEPS=" << nsteps << endl;
          }
          if (LDEBUG) {
            cerr << "[KBIN::XVASP_INCAR_LDAU_ADIABATIC] LDAU_ADIABATIC_NSPECIES=" << nspecies << endl;
          }
          if (tokens.size() != nsteps * nspecies * 3 + 2) {
            throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "wrong number of ADIABATIC_LDAU entries (" + aurostd::utype2string(tokens.size()) + ")", _INPUT_NUMBER_); // CO20200624
          }
          vector<int> vLDAUL;
          vector<double> vLDAUU;
          vector<double> vLDAUJ;
          ...
          xvasp.INCAR << aurostd::PaddedPOST(straus.str(), _incarpad_) << " # l-quantum number for which the on site interaction is added (Default 2) automatic LDAUL table" << endl;
          if (LDEBUG) {
            cerr << "[KBIN::XVASP_INCAR_LDAU_ADIABATIC] " << aurostd::PaddedPOST(straus.str(), _incarpad_) << endl;
          }
          straus.clear();
          straus.str(std::string());
          straus << "LDAUU=";
...
namespace KBIN {
  void XVASP_INCAR_LDAU_CUTOFF(_xvasp& xvasp, bool VERBOSE) {
    ...
    for (size_t i = 1; i <= imax; i++) {
      strline = aurostd::GetLineString(FileContent, i);
      if (!aurostd::substring2bool(strline, "SYSTEM", true) && (aurostd::substring2bool(strline, "LDAU", true) || aurostd::substring2bool(strline, "#LDAU", true))) {
        if (VERBOSE) {
          xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR_LDAU_CUTOFF)" << endl;
        }
      } else {
        if (!VERBOSE && !strline.empty()) {
          xvasp.INCAR << strline << endl;
    ...
    // xvasp.INCAR << endl;
    if (VERBOSE) {
      xvasp.INCAR << "# Performing LDAU=CUTOFF [AFLOW] begin" << endl;
    }
    xvasp.INCAR << aurostd::PaddedPOST("LDAU=.false.", _incarpad_) << " # LDAU=CUTOFF" << endl;
    if (VERBOSE) {
      xvasp.INCAR << "# Performing LDAU=CUTOFF [AFLOW] end" << endl;
    }
  }
} // namespace KBIN
...
namespace KBIN {
  void XVASP_INCAR_REMOVE_ENTRY(_xvasp& xvasp, const string& keyword, const string& comment, bool VERBOSE) {
  ...
  void XVASP_INCAR_REMOVE_ENTRY(_xvasp& xvasp, const vector<string>& vkeywords, const string& comment, bool VERBOSE) {
    ...
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    const string function = "KBIN::XVASP_INCAR_REMOVE_ENTRY";
    ...
    for (iline = 0; iline < vlines.size(); iline++) {
      ...
      for (i = 0; i < vkeywords.size() && !found; i++) {
        if (aurostd::kvpair2bool(strline, vkeywords[i], "=")) {
          found = true;
        }
      }
      if (found) {
        if (VERBOSE) {
          xvasp.INCAR << patch << strline << endl;
        } // aurostd::PaddedPOST("# "+aurostd::RemoveWhiteSpacesFromTheFrontAndBack(strline),_incarpad_) << " # AFLOW REMOVED (" << function << ") " << comment << endl;
      } else {
        if (VERBOSE || !strline.empty()) {
          xvasp.INCAR << strline << endl;
        }
...
namespace KBIN {
  ...
  bool AFLOWIN_REMOVE(const string& aflowin_file, const vector<string>& vkeywords, const string& comment) { // CO20210313
    const vector<string> vkeywords2ignore;
  ...
  bool AFLOWIN_REMOVE(const string& aflowin_file, const string& keyword, const string& keyword2avoid, const string& comment) { // CO20210313
    ...
    aurostd::string2tokens(keyword, vkeywords, ",");
    aurostd::string2tokens(keyword2avoid, vkeywords2ignore, ",");
    return AFLOWIN_REMOVE(aflowin_file, vkeywords, vkeywords2ignore, comment);
  }
  bool AFLOWIN_REMOVE(const string& aflowin_file, const vector<string>& vkeywords, const vector<string>& vkeywords2ignore, const string& comment) { // CO20210313
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    const string function = "KBIN::AFLOWIN_REMOVE";
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vkeywords=" << aurostd::joinWDelimiter(vkeywords, ",") << endl;
      cerr << __AFLOW_FUNC__ << " vkeywords2ignore=" << aurostd::joinWDelimiter(vkeywords2ignore, ",") << endl;
    }

    if (!aurostd::FileExist(aflowin_file)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, aflowin_file + " not found", _FILE_NOT_FOUND_);
    }

    vector<string> vlines;
    vector<string> vlines_new;
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " FILE(orig)=" << aflowin_file << endl;
      for (iline = 0; iline < vlines.size(); iline++) {
    ...
    for (iline = 0; iline < vlines.size(); iline++) {
      ...
      for (i = 0; i < vkeywords.size() && !found; i++) {
        if (line.find(vkeywords[i]) != string::npos) {
          ...
          for (j = 0; j < vkeywords2ignore.size() && !found_ignore; j++) {
            if (line.find(vkeywords2ignore[j]) != string::npos) {
              found_ignore = true;
            }
          }
          if (found_ignore) {
      ...
      if (found == false) {
        vlines_new.push_back(vlines[iline]);
      } else { // found==true
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " FILE(new)=" << aflowin_file << endl;
      for (iline = 0; iline < vlines.size(); iline++) {
        cerr << "[iline=" << iline << "]=\"" << vlines[iline] << "\"" << endl;
      }
    }
    ...
    if (!aurostd::vectorstring2file(vlines_new, aflowin_file)) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "issues writing new " + aflowin_file, _FILE_CORRUPT_);
    }

    return mod_made;
  }
  void AFLOWIN_ADD(const string& aflowin_file, const stringstream& streamin, const string& comment) { // CO20210313
    const string lines = streamin.str();
  ...
  void AFLOWIN_ADD(const string& aflowin_file, const vector<string>& vlines2add, const string& comment) { // CO20210313
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " vlines2add=" << endl;
      for (iline = 0; iline < vlines2add.size(); iline++) {
        cerr << "\"" << vlines2add[iline] << "\"" << endl;
      }
    }
    ...
    if (!comment.empty()) {
      ...
      vlines.insert(vlines.end(), vlines2add.begin(), vlines2add.end());
    }
...
namespace KBIN {
  bool XVASP_KPOINTS_IncludesGamma(const _xvasp& xvasp) { // CO20210315
    ...
    if (!xvasp.str.kpoints_kscheme.empty()
        && (aurostd::toupper(xvasp.str.kpoints_kscheme[0]) == 'G' || aurostd::toupper(xvasp.str.kpoints_kscheme[0]) == 'M')
        && (aurostd::_isodd(xvasp.str.kpoints_k1) && aurostd::_isodd(xvasp.str.kpoints_k2) && aurostd::_isodd(xvasp.str.kpoints_k3))
        && (aurostd::isequal(xvasp.str.kpoints_s1, 0.0) && aurostd::isequal(xvasp.str.kpoints_s2, 0.0) && aurostd::isequal(xvasp.str.kpoints_s3, 0.0))) {
      return true;
    }
    return false;
...
namespace KBIN {
  bool XVASP_KPOINTS_OPERATION(_xvasp& xvasp, const string& _operation) {
    ...
    // the schemes below check if the modification needs to be made (has it already been made?)
    // maintain this feedback system to ensure aflow doesn't keep spinning its wheels on the same fixes
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG); // true;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " operation=" << _operation << endl;
      cerr << __AFLOW_FUNC__ << " xvasp.str.kpoints_kscheme=(" << xvasp.str.kpoints_kscheme << ")" << endl;
      cerr << __AFLOW_FUNC__ << " xvasp.str.kpoints_k*=(" << xvasp.str.kpoints_k1 << "," << xvasp.str.kpoints_k2 << "," << xvasp.str.kpoints_k3 << ")" << endl;
      cerr << __AFLOW_FUNC__ << " xvasp.str.kpoints_s*=(" << xvasp.str.kpoints_s1 << "," << xvasp.str.kpoints_s2 << "," << xvasp.str.kpoints_s3 << ")" << endl;
    }
    if (_operation.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "empty operation", _INPUT_ILLEGAL_);
    } // CO20210315
    string operation = aurostd::toupper(_operation);
    aurostd::StringSubstInPlace(operation, "+=1", "++"); // backwards compatibility
    aurostd::StringSubstInPlace(operation, "-=1", "--"); // backwards compatibility
    aurostd::StringSubstInPlace(operation, "KMAX", "XMAX,YMAX,ZMAX"); // expand
    aurostd::StringSubstInPlace(operation, "GAMMA_SHIFT", "GAMMA,XZEROSHIFT,YZEROSHIFT,ZZEROSHIFT"); // expand  //repetita iuvant  //can also do XODD,YODD,ZODD
    ...
    if (operation.find("X--") != string::npos) {
      if (xvasp.str.kpoints_k1 >= 2) {
        xvasp.str.kpoints_k1--;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "X-- k1=" << xvasp.str.kpoints_k1 << endl;
        }
      }
    }
    if (operation.find("Y--") != string::npos) {
      if (xvasp.str.kpoints_k2 >= 2) {
        xvasp.str.kpoints_k2--;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Y-- k2=" << xvasp.str.kpoints_k2 << endl;
        }
      }
    }
    if (operation.find("Z--") != string::npos) {
      if (xvasp.str.kpoints_k3 >= 2) {
        xvasp.str.kpoints_k3--;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Z-- k3=" << xvasp.str.kpoints_k3 << endl;
        }
      }
    }
    ...
    if (operation.find("X-=2") != string::npos) {
      if (xvasp.str.kpoints_k1 >= 3) {
        xvasp.str.kpoints_k1 -= 2;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "X-=2 k1=" << xvasp.str.kpoints_k1 << endl;
        }
      }
    } // CO20210315
    if (operation.find("Y-=2") != string::npos) {
      if (xvasp.str.kpoints_k2 >= 3) {
        xvasp.str.kpoints_k2 -= 2;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Y-=2 k2=" << xvasp.str.kpoints_k2 << endl;
        }
      }
    } // CO20210315
    if (operation.find("Z-=2") != string::npos) {
      if (xvasp.str.kpoints_k3 >= 3) {
        xvasp.str.kpoints_k3 -= 2;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Z-=2 k3=" << xvasp.str.kpoints_k3 << endl;
        }
      }
    } // CO20210315
    ...
    if (operation.find("XEVEN") != string::npos) {
      if (aurostd::_isodd(xvasp.str.kpoints_k1)) {
        ...
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Xeven k1=" << xvasp.str.kpoints_k1 << endl;
        }
      }
      if (!aurostd::isequal(xvasp.str.kpoints_s1, 0.0)) {
        xvasp.str.kpoints_s1 = 0.0;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Xeven s1=" << xvasp.str.kpoints_s1 << endl;
        }
      }
    }
    if (operation.find("YEVEN") != string::npos) {
      if (aurostd::_isodd(xvasp.str.kpoints_k2)) {
        ...
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Yeven k2=" << xvasp.str.kpoints_k2 << endl;
        }
      }
      if (!aurostd::isequal(xvasp.str.kpoints_s2, 0.0)) {
        xvasp.str.kpoints_s2 = 0.0;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Yeven s2=" << xvasp.str.kpoints_s2 << endl;
        }
      }
    }
    if (operation.find("ZEVEN") != string::npos) {
      if (aurostd::_isodd(xvasp.str.kpoints_k3)) {
        ...
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Zeven k3=" << xvasp.str.kpoints_k3 << endl;
        }
      }
      if (!aurostd::isequal(xvasp.str.kpoints_s3, 0.0)) {
        xvasp.str.kpoints_s3 = 0.0;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Zeven s3=" << xvasp.str.kpoints_s3 << endl;
        }
      }
    }
    // iodd: make ki odd (if not already) AND si=0 (if not already)
    if (operation.find("XODD") != string::npos) {
      if (aurostd::_iseven(xvasp.str.kpoints_k1)) {
        xvasp.str.kpoints_k1++;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Xodd k1=" << xvasp.str.kpoints_k1 << endl;
        }
      }
      if (!aurostd::isequal(xvasp.str.kpoints_s1, 0.0)) {
        xvasp.str.kpoints_s1 = 0.0;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Xodd s1=" << xvasp.str.kpoints_s1 << endl;
        }
      }
    }
    if (operation.find("YODD") != string::npos) {
      if (aurostd::_iseven(xvasp.str.kpoints_k2)) {
        xvasp.str.kpoints_k2++;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Yodd k2=" << xvasp.str.kpoints_k2 << endl;
        }
      }
      if (!aurostd::isequal(xvasp.str.kpoints_s2, 0.0)) {
        xvasp.str.kpoints_s2 = 0.0;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Yodd s2=" << xvasp.str.kpoints_s2 << endl;
        }
      }
    }
    if (operation.find("ZODD") != string::npos) {
      if (aurostd::_iseven(xvasp.str.kpoints_k3)) {
        xvasp.str.kpoints_k3++;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Zodd k3=" << xvasp.str.kpoints_k3 << endl;
        }
      }
      if (!aurostd::isequal(xvasp.str.kpoints_s3, 0.0)) {
        xvasp.str.kpoints_s3 = 0.0;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Zodd s3=" << xvasp.str.kpoints_s3 << endl;
        }
      }
    }
    //
    ...
    if (operation.find("XMAX") != string::npos) {
      if (xvasp.str.kpoints_k1 != xvasp.str.kpoints_kmax) {
        xvasp.str.kpoints_k1 = xvasp.str.kpoints_kmax;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Xmax k1=" << xvasp.str.kpoints_k1 << endl;
        }
      }
    }
    if (operation.find("YMAX") != string::npos) {
      if (xvasp.str.kpoints_k2 != xvasp.str.kpoints_kmax) {
        xvasp.str.kpoints_k2 = xvasp.str.kpoints_kmax;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Ymax k2=" << xvasp.str.kpoints_k2 << endl;
        }
      }
    }
    if (operation.find("ZMAX") != string::npos) {
      if (xvasp.str.kpoints_k3 != xvasp.str.kpoints_kmax) {
        xvasp.str.kpoints_k3 = xvasp.str.kpoints_kmax;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Zmax k3=" << xvasp.str.kpoints_k3 << endl;
        }
      }
    }
    ...
    if (operation.find("XEVENSHIFT") != string::npos) {
      if (aurostd::_iseven(xvasp.str.kpoints_k1)) {
        if (!aurostd::isequal(xvasp.str.kpoints_s1, 0.5)) {
          xvasp.str.kpoints_s1 = 0.5;
          nchanges_made++;
          if (LDEBUG) {
            cerr << "Xevenshift s1=" << xvasp.str.kpoints_s1 << endl;
          }
        }
      } else { //_isodd()
        if (!aurostd::isequal(xvasp.str.kpoints_s1, 0.0)) {
    ...
    if (operation.find("YEVENSHIFT") != string::npos) {
      if (aurostd::_iseven(xvasp.str.kpoints_k2)) {
        if (!aurostd::isequal(xvasp.str.kpoints_s2, 0.5)) {
          xvasp.str.kpoints_s2 = 0.5;
          nchanges_made++;
          if (LDEBUG) {
            cerr << "Yevenshift s2=" << xvasp.str.kpoints_s2 << endl;
          }
        }
      } else { //_isodd()
        if (!aurostd::isequal(xvasp.str.kpoints_s2, 0.0)) {
    ...
    if (operation.find("ZEVENSHIFT") != string::npos) {
      if (aurostd::_iseven(xvasp.str.kpoints_k3)) {
        if (!aurostd::isequal(xvasp.str.kpoints_s3, 0.5)) {
          xvasp.str.kpoints_s3 = 0.5;
          nchanges_made++;
          if (LDEBUG) {
            cerr << "Zevenshift s3=" << xvasp.str.kpoints_s3 << endl;
          }
        }
      } else { //_isodd()
        if (!aurostd::isequal(xvasp.str.kpoints_s3, 0.0)) {
    ...
    if (operation.find("XODDSHIFT") != string::npos) {
      if (aurostd::_isodd(xvasp.str.kpoints_k1)) {
        if (!aurostd::isequal(xvasp.str.kpoints_s1, 0.5)) {
          ...
          nchanges_made++;
          if (LDEBUG) {
            cerr << "Xoddshift s1=" << xvasp.str.kpoints_s1 << endl;
          }
        }
      } else { //_iseven()
        if (!aurostd::isequal(xvasp.str.kpoints_s1, 0.0)) {
    ...
    if (operation.find("YODDSHIFT") != string::npos) {
      if (aurostd::_isodd(xvasp.str.kpoints_k2)) {
        if (!aurostd::isequal(xvasp.str.kpoints_s2, 0.5)) {
          ...
          nchanges_made++;
          if (LDEBUG) {
            cerr << "Yoddshift s2=" << xvasp.str.kpoints_s2 << endl;
          }
        }
      } else { //_iseven()
        if (!aurostd::isequal(xvasp.str.kpoints_s2, 0.0)) {
    ...
    if (operation.find("ZODDSHIFT") != string::npos) {
      if (aurostd::_isodd(xvasp.str.kpoints_k3)) {
        if (!aurostd::isequal(xvasp.str.kpoints_s3, 0.5)) {
          ...
          nchanges_made++;
          if (LDEBUG) {
            cerr << "Zoddshift s3=" << xvasp.str.kpoints_s3 << endl;
          }
        }
      } else { //_iseven()
        if (!aurostd::isequal(xvasp.str.kpoints_s3, 0.0)) {
    ...
    if (operation.find("XZEROSHIFT") != string::npos) {
      if (!aurostd::isequal(xvasp.str.kpoints_s1, 0.0)) {
        xvasp.str.kpoints_s1 = 0.0;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Xzeroshift s1=" << xvasp.str.kpoints_s1 << endl;
        }
      }
    }
    if (operation.find("YZEROSHIFT") != string::npos) {
      if (!aurostd::isequal(xvasp.str.kpoints_s2, 0.0)) {
        xvasp.str.kpoints_s2 = 0.0;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Yzeroshift s2=" << xvasp.str.kpoints_s2 << endl;
        }
      }
    }
    if (operation.find("ZZEROSHIFT") != string::npos) {
      if (!aurostd::isequal(xvasp.str.kpoints_s3, 0.0)) {
        xvasp.str.kpoints_s3 = 0.0;
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Zzeroshift s3=" << xvasp.str.kpoints_s3 << endl;
        }
      }
    }
    // M vs. G: set kscheme accordingly (if not already)
    if (operation.at(0) == 'M' || operation.find("MONKHORST-PACK") != string::npos) {
      if (xvasp.str.kpoints_kscheme != "Monkhorst-Pack") {
        xvasp.str.kpoints_kscheme = "Monkhorst-Pack";
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Monkhorst-Pack" << endl;
        }
      }
    } // "Monkhorst-Pack";
    if (operation.at(0) == 'G' || operation.find("GAMMA") != string::npos) {
      if (xvasp.str.kpoints_kscheme != "Gamma") {
        xvasp.str.kpoints_kscheme = "Gamma";
        nchanges_made++;
        if (LDEBUG) {
          cerr << "Gamma" << endl;
    ...
    } // "Gamma";  //CO20210315 - do not check if !KBIN::XVASP_KPOINTS_IncludesGamma(xvasp) here, changing to 'G' if 'G' is not set is an entirely new scheme: https://www.vasp.at/wiki/index.php/KPOINTS#Monkhorst-Pack
    if (xvasp.str.kpoints_k1 < 1) {
      xvasp.str.kpoints_k1 = 1;
    }
    if (xvasp.str.kpoints_k2 < 1) {
      xvasp.str.kpoints_k2 = 1;
    }
    if (xvasp.str.kpoints_k3 < 1) {
      xvasp.str.kpoints_k3 = 1;
    }
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " nchanges_made=" << nchanges_made << endl;
      cerr << __AFLOW_FUNC__ << " xvasp.str.kpoints_kscheme=(" << xvasp.str.kpoints_kscheme << ")" << endl;
      cerr << __AFLOW_FUNC__ << " xvasp.str.kpoints_k*=(" << xvasp.str.kpoints_k1 << "," << xvasp.str.kpoints_k2 << "," << xvasp.str.kpoints_k3 << ")" << endl;
      cerr << __AFLOW_FUNC__ << " xvasp.str.kpoints_s*=(" << xvasp.str.kpoints_s1 << "," << xvasp.str.kpoints_s2 << "," << xvasp.str.kpoints_s3 << ")" << endl;
    }
...
namespace KBIN {
  bool XVASP_KPOINTS_isAutoMesh(const _xvasp& xvasp) { // CO20210315
    ...
    const bool LDEBUG = (false || VERBOSE_MONITOR_VASP || _DEBUG_IVASP_ || XHOST.DEBUG);

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " xvasp.KPOINTS=" << endl;
      cerr << xvasp.KPOINTS.str() << endl;
    }
    ...
    const string nkpoints = aurostd::RemoveWhiteSpacesFromTheFrontAndBack(vlines[1]);
    if (aurostd::isfloat(nkpoints) && aurostd::string2utype<uint>(nkpoints) == 0) {
      return true;
    } // is an auto-mesh
...
namespace KBIN {
  bool XVASP_KPOINTS_string2numbers(_xvasp& xvasp) { // CO20210315 - cleaned up
    ...
    if (scheme.empty() || (aurostd::toupper(scheme[0]) != 'M' && aurostd::toupper(scheme[0]) != 'G')) {
      return false;
    }
...
namespace KBIN {
  bool XVASP_KPOINTS_numbers2string(_xvasp& xvasp) { // CO20210315 - cleaned up
    // CO20210315 - can only read auto-meshes of MP or G
    // https://www.vasp.at/wiki/index.php/KPOINTS
    const bool LDEBUG = (false || VERBOSE_MONITOR_VASP || XHOST.DEBUG);

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " xvasp.KPOINTS(pre)=" << endl;
      cerr << xvasp.KPOINTS.str() << endl;
    }

    if (xvasp.str.kpoints_kscheme.empty() || (aurostd::toupper(xvasp.str.kpoints_kscheme[0]) != 'M' && aurostd::toupper(xvasp.str.kpoints_kscheme[0]) != 'G')) {
      return false;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " xvasp.KPOINTS(post)=" << endl;
      cerr << xvasp.KPOINTS.str() << endl;
    }
    return true;
  }
...
namespace KBIN {
  void XVASP_Afix_Clean(const _xvasp& xvasp, const string& preserve_name) {
    ...
    const vector<string> vrem{"aflow.tmp", "CHG", "CONTCAR", "DOSCAR", "EIGENVAL", "IBZKPT", "OUTCAR", "OSZICAR", "PCDAT", "WAVECAR", "aflow.qsub*", "XDATCAR", "vasprun.xml", DEFAULT_VASP_OUT, "core*"};
    for (size_t i = 0; i < vrem.size(); i++) {
      aurostd::RemoveFile(xvasp.Directory + "/" + vrem[i]);
    }
    if (!xvasp.aopts.flag("FLAG::CHGCAR_PRESERVED")) {
      aurostd::RemoveFile(xvasp.Directory + "/CHGCAR");
    }
  }
} // namespace KBIN
...
namespace KBIN {
  ...
  bool XVASP_Afix_NBANDS(_xvasp& xvasp, const _aflags& aflags, const _vflags& vflags, int& nbands, bool increase) {
    ...
    const bool LDEBUG = (false || VERBOSE_MONITOR_VASP || _DEBUG_IVASP_ || XHOST.DEBUG);
    const string function = "KBIN::XVASP_Afix_NBANDS";
    ...
    if (aurostd::FileExist(xvasp.Directory + "/OUTCAR") && aurostd::FileNotEmpty(xvasp.Directory + "/OUTCAR")) {
      const xOUTCAR OUTCAR_NBANDS(xvasp.Directory + "/OUTCAR", true); // quiet, there might be issues with halfway-written OUTCARs
    ...
    if (nbands == 0 && aurostd::kvpair2bool(xvasp.INCAR, "NBANDS", "=")) { // GET BANDS FROM INCAR //CO20210315 - the kvpair2bool() is more robust than substring2bool
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " GET NBANDS FROM INCAR" << endl;
      }
    ...
    if (aurostd::kvpair2bool(xvasp.INCAR, "NPAR", "=")) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " GET NPAR FROM INCAR" << endl;
      }
      ...
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " GET NCORE FROM INCAR" << endl;
      }
    ...
    if (LDEBUG) {
      ...
      cerr << __AFLOW_FUNC__ << " nelectrons=" << nelectrons << endl;
      cerr << __AFLOW_FUNC__ << " npar=" << npar << endl;
    }
    if (nbands == 0) {
      nbands = KBIN::XVASP_INCAR_GetNBANDS(xvasp, aflags, true);
    } else {
      ...
      if (increase) {
        nbands += (int) (20 + (double) nbands * 0.2);
      } else {
        nbands = (int) ((double) nbands * 0.8);
      } // CO20210315 //CO20210713 - increasing rate of decrease for NBANDS to 20%, 10% is too slow, issues with walltime
    }
    // make sure nbands is divisible by npar (default =1)
    const int increment = (increase ? +1 : -1);
    while ((nbands % npar) != 0) {
      nbands += increment;
    }
    ...
    if (nelectrons != 0 && nbands < (nelectrons + 1) / 2) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " nbands < " << (nelectrons + 1) / 2 << " = (nelectrons+1)/2 (too small)" << endl;
      }
      return false;
    }

    if (!KBIN::XVASP_INCAR_PREPARE_GENERIC("NBANDS", xvasp, vflags, "", nbands, 0.0, false)) {
      return false;
    }
...
namespace KBIN {
  ...
  bool XVASP_Afix_POTIM(_xvasp& xvasp, const _vflags& vflags, double& potim) {
    ...
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    const string function = "KBIN::XVASP_Afix_POTIM";
    ...
    if (aurostd::isequal(potim, 0.0) && aurostd::FileExist(xvasp.Directory + "/OUTCAR") && aurostd::FileNotEmpty(xvasp.Directory + "/OUTCAR")) {
      const xOUTCAR OUTCAR_POTIM(xvasp.Directory + "/OUTCAR", true); // quiet, there might be issues with halfway-written OUTCARs
    ...
    if (aurostd::isequal(potim, 0.0) && aurostd::kvpair2bool(xvasp.INCAR, "POTIM", "=")) { // GET POTIM FROM INCAR  //CO20210315 - the kvpair2bool() is more robust than substring2bool
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " GET POTIM FROM INCAR" << endl;
      }
      potim = aurostd::kvpair2utype<double>(xvasp.INCAR, "POTIM", "="); // CO20210315
    }
    if (aurostd::isequal(potim, 0.0)) {
      potim = 0.5;
    } // default - https://www.vasp.at/wiki/index.php/POTIM
    if (LDEBUG) {
    ...
    if (!KBIN::XVASP_INCAR_PREPARE_GENERIC("POTIM", xvasp, vflags, "", 0, potim, false)) {
      return false;
    }
...
namespace KBIN {
  ...
  bool XVASP_Afix_NELM(_xvasp& xvasp, const _vflags& vflags, int& nelm) { // CO20200624
    ...
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    const string function = "KBIN::XVASP_Afix_NELM";

    // get NELM from OUTCAR
    aurostd::RemoveFile(xvasp.Directory + "/aflow.tmp");
    nelm = 0;
    if (aurostd::FileExist(xvasp.Directory + "/OUTCAR") && aurostd::FileNotEmpty(xvasp.Directory + "/OUTCAR")) {
      const xOUTCAR OUTCAR_NELM(xvasp.Directory + "/OUTCAR", true); // quiet, there might be issues with halfway-written OUTCARs
    ...
    if (nelm == 0 && aurostd::kvpair2bool(xvasp.INCAR, "NELM", "=")) { // GET NELM FROM INCAR  //CO20210315 - the kvpair2bool() is more robust than substring2bool
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " GET NELM FROM INCAR" << endl;
      }
      nelm = aurostd::kvpair2utype<int>(xvasp.INCAR, "NELM", "="); // CO20210315
    }
    if (nelm == 0) {
      nelm = 60;
    } // default - https://www.vasp.at/wiki/index.php/NELM
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " nelm=" << nelm << endl;
    }
    //[CO20200624 - no point in setting over and over again, just do in one shot]nelm=nelm*2.0;
    if (nelm >= MAX_VASP_NELM) {
      return false;
    } // too high as set by user
    nelm = MAX_VASP_NELM;
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " nelm=" << nelm << endl;
    }

    if (!KBIN::XVASP_INCAR_PREPARE_GENERIC("NELM", xvasp, vflags, "", nelm, 0.0, false)) {
      return false;
    }
...
namespace KBIN {
  bool XVASP_Afix_EFIELD_PEAD(_xvasp& xvasp, const _vflags& vflags) {
    xvector<double> E;
    return XVASP_Afix_EFIELD_PEAD(xvasp, vflags, E);
  }
  bool XVASP_Afix_EFIELD_PEAD(_xvasp& xvasp, const _vflags& vflags, xvector<double>& E) {
    // note, this function is part of the XVASP_Afix_* series
    // it will assume xvasp.INCAR has been pre-loaded and will NOT rewrite the INCAR
    // this is all done inside the main XVASP_Afix() function
    // BE CAREFUL not to overwrite xvasp.INCAR
    const bool LDEBUG = (false || _DEBUG_IVASP_ || XHOST.DEBUG);
    constexpr double DEFAULT_EFIELD_PEAD = 0.001;
    ...
    if (aurostd::FileExist(xvasp.Directory + "/OUTCAR") && aurostd::FileNotEmpty(xvasp.Directory + "/OUTCAR")) {
      const xOUTCAR OUTCAR_EP(xvasp.Directory + "/OUTCAR", true); // quiet, there might be issues with halfway-written OUTCARs
      E = OUTCAR_EP.efield_pead;
    }
    if (E.lrows != 1 || E.urows != 3) {
      E.clear();
      E.resize(3);
    } // safety
    for (int i = E.lrows; i <= E.urows; i++) {
      if (E[i] <= 1.0E-6) {
        E[i] = DEFAULT_EFIELD_PEAD;
      }
    }
    E *= 0.2; // dvalue used to be an input, not used right now, maybe make an aflowrc value in the future

    const string incar_input = "EFIELD_PEAD=" + aurostd::utype2string(E[1], 6) + " " + aurostd::utype2string(E[2], 6) + " " + aurostd::utype2string(E[3], 6);
    if (aurostd::kvpair2bool(xvasp.INCAR, "EFIELD_PEAD", "=")) {
      const string incar_input_old = "EFIELD_PEAD=" + aurostd::kvpair2string(xvasp.INCAR, "EFIELD_PEAD", "=");
      if (incar_input == incar_input_old) {
        return false;
      }
    }
    ...
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "# Performing " << __AFLOW_FUNC__ << " [AFLOW] start" << endl;
    }
    xvasp.INCAR << aurostd::PaddedPOST(incar_input, _incarpad_) << " # " << __AFLOW_FUNC__ << endl;
    if (vflags.KBIN_VASP_INCAR_VERBOSE) {
      xvasp.INCAR << "# Performing " << __AFLOW_FUNC__ << " [AFLOW] end" << endl;
    }
...
namespace KBIN {
  bool XVASP_Afix_ApplyFix(const string& _fix, aurostd::xoption& xfixed, _xvasp& xvasp, _kflags& kflags, _vflags& vflags, _aflags& aflags, ofstream& FileMESSAGE) { // CO20200624
    ...
    // the schemes below check if the modification needs to be made (has it already been made?)
    // maintain this feedback system to ensure aflow doesn't keep spinning its wheels on the same fixes
    const string function = "KBIN::XVASP_Afix_ApplyFix";
    const string fix = aurostd::toupper(_fix);
    const string fix_str = "(" + fix + ")";
    const string operation = function + fix_str;
    string incar_input;
    stringstream aus;
    const bool VERBOSE = (false || VERBOSE_MONITOR_VASP || XHOST.vflag_control.flag("MONITOR_VASP") == false);
    ...
    if (fix == "EFIELD_PEAD") {
      ...
      apply_once = false;
    } else if (fix.find("KPOINTS+") != string::npos || fix.find("KPOINTS-") != string::npos) {
      apply_once = false;
    } else if (fix.find("NBANDS+") != string::npos || fix.find("NBANDS-") != string::npos) {
      apply_once = false;
    } else if (fix == "NBANDS") {
      apply_once = false;
    } else if (fix == "POTIM") {
    ...
    if (apply_once == true && xfixed.flag(fix)) {
      if (VERBOSE) {
        aus << "MMMMM  MESSAGE ignoring FIX=\"" << fix << "\"" << ": already applied" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      return false;
    }
    ...
    if (relaxing) {
      ...
      if (fix.find("ISMEAR=") != string::npos) { // specific smearing selected is very important for static/bands
        if (VERBOSE) {
          aus << "MMMMM  MESSAGE ignoring FIX=\"" << fix << "\"" << ": does not apply for static/bands calculations" << Message(__AFLOW_FILE__, aflags) << endl;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        }
        Krun = false;
      }
      ...
      //[CO20210315 - can change KPOINTS for static]  if(VERBOSE){aus << "MMMMM  MESSAGE ignoring FIX=\"" << fix << "\"" << ": does not apply for static/bands calculations" << Message(__AFLOW_FILE__,aflags) <<
      // endl;aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);} [CO20210315 - can change KPOINTS for static]  Krun=false; [CO20210315 - can change KPOINTS for static]}
    ...
    XVASP_KPOINTS_string2numbers(xvasp); // preload kpoints, load into xvasp.str
    if (fix.find("KPOINTS") != string::npos && XVASP_KPOINTS_isAutoMesh(xvasp) == false) { // cannot change non-auto-mesh (bands)
      if (VERBOSE) {
        aus << "MMMMM  MESSAGE ignoring FIX=\"" << fix << "\"" << ": cannot change KPOINTS for non-auto-mesh" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      Krun = false;
    }
    ...
    if (found_up_down) {
      xfixed.flag("KPOINTS_EXHAUSTED", true);
    }
    if (Krun == false) {
      if (VERBOSE) {
        aus << "MMMMM  MESSAGE ignoring FIX=\"" << fix << "\"" << ": avoiding KPOINTS++ and KPOINTS--" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      return false;
    }
    ...
    for (size_t i = 0; i < xfixed.vxscheme.size() && Krun; i++) {
      ...
      if (scheme.find("NBANDS+") != string::npos && fix.find("NBANDS-") != string::npos) {
        if (xfixed.flag("NBANDS_EXHAUSTED")) {
          Krun = false;
        } else {
          found_up_down = true;
        }
    ...
    if (found_up_down) {
      xfixed.flag("NBANDS_EXHAUSTED", true);
    }
    if (Krun == false) {
      if (VERBOSE) {
        aus << "MMMMM  MESSAGE ignoring FIX=\"" << fix << "\"" << ": avoiding NBANDS++ and NBANDS--" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      return false;
    }
    ...
    const _xvasp xvasp_orig(xvasp); // keep original, restore later if necessary
    const _kflags kflags_orig(kflags); // keep original, restore later if necessary
    const _vflags vflags_orig(vflags); // keep original, restore later if necessary
    ...
    if (fix.find("ALGO=") != string::npos && fix.find("IALGO=") == string::npos) {
      ...
      if (Krun) {
        loc = fix.find('=');
        if (loc == fix.size() - 1 || loc == string::npos) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "ALGO mode not found", _INPUT_ILLEGAL_);
        }
        param_string = fix.substr(loc + 1); // get everything after the '='
      }

      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE attempting FIX=\"" << fix << "\"" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      ...
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE applied FIX=\"" << fix << "\"" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      ...
      loc = fix.find('=');
      if (loc == fix.size() - 1 || loc == string::npos) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "AMIN mode not found", _INPUT_ILLEGAL_);
      }
      ...
      // START - load INCAR into xvasp, modify, then write out new INCAR
      Krun = (Krun && VASP_Reread_INCAR(xvasp)); // preload incar
      Krun = (Krun && KBIN::XVASP_INCAR_PREPARE_GENERIC("AMIN", xvasp, vflags, "", 0, param_double, true));
      if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
        Krun = (Krun && aurostd::stringstream2file(xvasp.INCAR, string(xvasp.Directory + "/INCAR"))); // write out incar
      }
      // END - load INCAR into xvasp, modify, then write out new INCAR
      ...
      loc = fix.find('=');
      if (loc == fix.size() - 1 || loc == string::npos) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "AMIX mode not found", _INPUT_ILLEGAL_);
      }
      param_double = aurostd::string2utype<double>(fix.substr(loc + 1)); // get everything after the '='
      ...
      // START - load INCAR into xvasp, modify, then write out new INCAR
      Krun = (Krun && VASP_Reread_INCAR(xvasp)); // preload incar
      Krun = (Krun && KBIN::XVASP_INCAR_PREPARE_GENERIC("AMIX", xvasp, vflags, "", 0, param_double, true));
      if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
        Krun = (Krun && aurostd::stringstream2file(xvasp.INCAR, string(xvasp.Directory + "/INCAR"))); // write out incar
      }
      // END - load INCAR into xvasp, modify, then write out new INCAR
      ...
      loc = fix.find('=');
      if (loc == fix.size() - 1 || loc == string::npos) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "BMIX mode not found", _INPUT_ILLEGAL_);
      }
      param_double = aurostd::string2utype<double>(fix.substr(loc + 1)); // get everything after the '='
      ...
      // START - load INCAR into xvasp, modify, then write out new INCAR
      Krun = (Krun && VASP_Reread_INCAR(xvasp)); // preload incar
      Krun = (Krun && KBIN::XVASP_INCAR_PREPARE_GENERIC("BMIX", xvasp, vflags, "", 0, param_double, true));
      if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
        Krun = (Krun && aurostd::stringstream2file(xvasp.INCAR, string(xvasp.Directory + "/INCAR"))); // write out incar
      }
      // END - load INCAR into xvasp, modify, then write out new INCAR
      ...
      if (Krun) {
        ...
        if (param_double == AUROSTD_MAX_DOUBLE && aurostd::FileExist(xvasp.Directory + "/OUTCAR") && aurostd::FileNotEmpty(xvasp.Directory + "/OUTCAR")) {
          const xOUTCAR OUTCAR(xvasp.Directory + "/OUTCAR", true); // quiet, there might be issues with halfway-written OUTCARs
        ...
        if (param_double == AUROSTD_MAX_DOUBLE) {
          param_double = 1e-4;
        } // default for vasp
      ...
      // END - load INCAR into xvasp, modify, then write out new INCAR
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE applied FIX=\"" << fix << "\"=" << param_double << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      ...
      // END - load INCAR into xvasp, modify, then write out new INCAR
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE applied FIX=\"" << fix << "\"=[" << aurostd::joinWDelimiter(aurostd::xvecDouble2vecString(param_xvd, 6), " ") << "]" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
    } else if (fix == "ENMAX") {
      if (Krun && VERBOSE) {
      ...
      if (Krun) {
        param_double = AUROSTD_MAX_DOUBLE;
        if (aurostd::kvpair2bool(xvasp.INCAR, "ENMAX", "=")) {
          param_double = aurostd::kvpair2utype<int>(xvasp.INCAR, "ENMAX", "=");
        } // CO20210315 - the kvpair2bool check makes sure we don't get enmax=0 for lack of input
        else {
          param_double = DEFAULT_VASP_PREC_ENMAX_ACCURATE * xvasp.POTCAR_ENMAX;
        }
        if (param_double == AUROSTD_MAX_DOUBLE || aurostd::isequal(param_double, 0.0)) {
          param_double = 500.0;
        } // CO20210315
        param_double *= 0.97; // reduce 3%.... if enough // param_double*=0.99; // reduce 1%.... if enough
        if (aurostd::isequal(xvasp.POTCAR_ENMAX, 0.0) == false) {
          if (param_double < DEFAULT_VASP_ENMAX_MINIMUM * xvasp.POTCAR_ENMAX) {
            Krun = false;
          } // so it doesn't run forever
        }
        KBIN::XVASP_INCAR_PREPARE_GENERIC("LREAL", xvasp, vflags, "", 0, 0.0, true); // no Krun here, if it's already there, we're set
      }
      ...
      loc = fix.find('=');
      if (loc == fix.size() - 1 || loc == string::npos) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "ISMEAR mode not found", _INPUT_ILLEGAL_);
      }
      ...
      if (xvasp.aopts.flag("FLAG::KPOINTS_PRESERVED")) {
        Krun = false;
      } // don't touch kpoints
      if (Krun) {
        const string key = "KPOINTS";
        loc = fix.find(key);
        if (loc == fix.size() - 1 || loc == string::npos) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "KPOINTS mode not found", _INPUT_ILLEGAL_);
        }
        string value;
        value = fix.substr(loc + key.length()); // get everything after 'KPOINTS', including '='
        if (value == "=GAMMA") {
          param_string = "Gamma_Shift";
        } else if (value == "=GAMMA_ODD") {
          param_string = "Gamma,Xodd,Yodd,Zodd";
        } else if (value == "=GAMMA_EVEN") {
          param_string = "Gamma,Xeven,Yeven,Zeven";
        } else if (value == "=KMAX") {
          param_string = "Kmax";
        } else if (value == "++") {
          param_string = "X++,Y++,Z++";
        } else if (value == "+=2") {
          param_string = "X+=2,Y+=2,Z+=2";
        } else if (value == "--") {
          param_string = "X--,Y--,Z--";
        } else if (value == "-=2") {
          param_string = "X-=2,Y-=2,Z-=2";
        } else {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "KPOINTS unknown mode: \"" + value + "\"", _INPUT_ILLEGAL_);
        }

        Krun = (Krun && VASP_Reread_KPOINTS(xvasp) && XVASP_KPOINTS_string2numbers(xvasp)); // preload kpoints, load into xvasp.str
        if (Krun && value == "=GAMMA" && KBIN::XVASP_KPOINTS_IncludesGamma(xvasp)) {
          Krun = false;
        } // already done
      }
      ...
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE attempting FIX=\"" << fix << "\"" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        aus << "MMMMM  MESSAGE KPOINTS(pre )=[" << xvasp.str.kpoints_kscheme << ";" << xvasp.str.kpoints_k1 << "," << xvasp.str.kpoints_k2 << "," << xvasp.str.kpoints_k3 << ";" << xvasp.str.kpoints_s1 << ","
            << xvasp.str.kpoints_s2 << "," << xvasp.str.kpoints_s3 << "]" << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      ...
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE applied FIX=\"" << fix << "\"" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        aus << "MMMMM  MESSAGE KPOINTS(post)=[" << xvasp.str.kpoints_kscheme << ";" << xvasp.str.kpoints_k1 << "," << xvasp.str.kpoints_k2 << "," << xvasp.str.kpoints_k3 << ";" << xvasp.str.kpoints_s1 << ","
            << xvasp.str.kpoints_s2 << "," << xvasp.str.kpoints_s3 << "]" << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
    } else if (fix == "LREAL") {
      ...
      // START - load INCAR into xvasp, modify, then write out new INCAR
      Krun = (Krun && VASP_Reread_INCAR(xvasp)); // preload incar
      Krun = (Krun && KBIN::XVASP_INCAR_PREPARE_GENERIC("LREAL", xvasp, vflags, "", 0, 0.0, true));
      if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
        Krun = (Krun && aurostd::stringstream2file(xvasp.INCAR, string(xvasp.Directory + "/INCAR"))); // write out incar
      }
      // END - load INCAR into xvasp, modify, then write out new INCAR
      ...
      const bool increase = (fix.find("--") == string::npos); // increase if "--" NOT found, so NBANDS or NBANDS++ works
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE attempting FIX=\"" << fix << "\"" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      ...
      // END - load INCAR into xvasp, modify, then write out new INCAR
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE applied FIX=\"" << fix << "\"=" << param_int << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
    } else if (fix == "NCPUS") {
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE attempting FIX=\"" << fix << "\" kflags.KBIN_MPI_NCPUS(pre)=" << kflags.KBIN_MPI_NCPUS << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      ...
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE applied FIX=\"" << fix << "\" kflags.KBIN_MPI_NCPUS(post)=" << kflags.KBIN_MPI_NCPUS << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      ...
      // END - load INCAR into xvasp, modify, then write out new INCAR
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE attempting FIX=" << fix << "=" << param_int << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
      ...
      loc = fix.find('=');
      if (loc == fix.size() - 1 || loc == string::npos) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "NPAR mode not found", _INPUT_ILLEGAL_);
      }
      param_int = aurostd::string2utype<int>(fix.substr(loc + 1)); // get everything after the '='
      ...
      if (xvasp.aopts.flag("FLAG::POSCAR_PRESERVED")) {
        Krun = false;
      } // don't touch poscar
      if (Krun) {
        const string key = "POSCAR_SCALE";
        loc = fix.find(key);
        if (loc == fix.size() - 1 || loc == string::npos) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "POSCAR_SCALE mode not found", _INPUT_ILLEGAL_);
        }
        string value;
        value = fix.substr(loc + key.length()); // get everything after 'POSCAR_SCALE', including '='
        if (value.find("*=") != string::npos) {
          param_string = value;
          aurostd::StringSubstInPlace(param_string, "*=", "");
          param_double = aurostd::string2utype<double>(param_string);
        } else {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "POSCAR_SCALE unknown mode: \"" + value + "\"", _INPUT_ILLEGAL_);
        }
      }
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE attempting FIX=\"" << fix << "\"" << Message(__AFLOW_FILE__, aflags) << endl;
      ...
      if (Krun) {
        xvasp.str = xstructure(xvasp.POSCAR, IOVASP_POSCAR);
      } // plug inside xvasp.str (WARNING: this overwrites xvasp.str_kpoints*)
      if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
      ...
      if (Krun) {
        if (VERBOSE) {
          aus << "00000  MESSAGE WARNING changing scale" << Message(__AFLOW_FILE__, aflags) << endl;
      ...
      if (Krun) {
        const string key = "POSCAR_VOLUME";
        loc = fix.find(key);
        if (loc == fix.size() - 1 || loc == string::npos) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "POSCAR_VOLUME mode not found", _INPUT_ILLEGAL_);
        }
        string value;
        ...
        if (value.find("*=") != string::npos) {
          ...
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "POSCAR_VOLUME unknown mode: \"" + value + "\"", _INPUT_ILLEGAL_);
        }
      }
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE attempting FIX=\"" << fix << "\"" << Message(__AFLOW_FILE__, aflags) << endl;
      ...
      if (Krun) {
        if (VERBOSE) {
          aus << "00000  MESSAGE WARNING changing volume" << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE BEFORE: volume=" << xvasp.str.GetVolume() << endl;
          aus << "00000  MESSAGE BEFORE: structure: " << endl;
          aus << xvasp.str;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      ...
      if (Krun) {
        xvasp.str.scale *= std::pow(param_double, (1.0 / 3.0));
      }
      if (Krun) {
        if (VERBOSE) {
          aus << "00000  MESSAGE AFTER: volume=" << xvasp.str.GetVolume() << endl;
      ...
      if (Krun && xvasp.str.Standard_Lattice_conventional) {
        Krun = false;
      } // DX is this right?  //CO20210315 - obsolete as we read in fresh POSCAR above, so this setting will not be set, we need a function here
      if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
        Krun = (Krun && aurostd::stringstream2file(xvasp.POSCAR, string(xvasp.Directory + "/POSCAR.orig"))); // CO20210315 - POSCAR.orig here is NOT the original structure, but a saved state, come back later
      }
      if (Krun) {
        ...
        const bool remove_magmom = aurostd::kvpair2bool(xvasp.INCAR, "MAGMOM", "=");
        const bool write_magmom = XVASP_INCAR_Read_MAGMOM(xvasp);
        if (VERBOSE) {
          aus << "00000  MESSAGE WARNING changing to the conventional cell" << Message(__AFLOW_FILE__, aflags) << endl;
          ...
          aus << "00000  MESSAGE BEFORE: volume=" << xvasp.str.GetVolume() << endl;
          if (write_magmom) {
            aus << "00000  MESSAGE BEFORE: MAGMOM=" << aurostd::kvpair2string(xvasp.INCAR, "MAGMOM", "=") << endl;
          }
          aus << "00000  MESSAGE BEFORE: structure: " << endl;
          aus << xvasp.str;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
        ...
        if (remove_magmom) {
          KBIN::XVASP_INCAR_REMOVE_ENTRY(xvasp, "MAGMOM", operation, vflags.KBIN_VASP_INCAR_VERBOSE);
        } // CO20200624
        if (write_magmom) {
          KBIN::XVASP_INCAR_PREPARE_GENERIC("AUTO_MAGMOM", xvasp, vflags, "", 0, 0.0, false);
        }
        if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
          if (remove_magmom || write_magmom) {
            aurostd::stringstream2file(xvasp.INCAR, string(xvasp.Directory + "/INCAR"));
          } // write out incar
        }
        //
        if (VERBOSE) {
          ...
          aus << "00000  MESSAGE AFTER: volume=" << xvasp.str.GetVolume() << endl;
          if (write_magmom) {
            aus << "00000  MESSAGE AFTER: MAGMOM=" << aurostd::kvpair2string(xvasp.INCAR, "MAGMOM", "=") << endl;
          }
          aus << "00000  MESSAGE AFTER: structure: " << endl;
          aus << xvasp.str;
          aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      ...
      if (Krun) {
        if (VERBOSE) {
          aus << "00000  MESSAGE WARNING recycling CONTCAR" << Message(__AFLOW_FILE__, aflags) << endl;
          aus << "00000  MESSAGE BEFORE: a,b,c,alpha,beta,gamma=" << xvasp.str.a << "," << xvasp.str.b << "," << xvasp.str.c << "," << xvasp.str.alpha << "," << xvasp.str.beta << "," << xvasp.str.gamma << endl;
          aus << "00000  MESSAGE BEFORE: volume=" << xvasp.str.GetVolume() << endl;
          aus << "00000  MESSAGE BEFORE: structure: " << endl;
          aus << xvasp.str;
      ...
      if (Krun) {
        xvasp.str = xstructure(param_string, IOVASP_POSCAR);
      }
      if (Krun) {
        if (VERBOSE) {
          aus << "00000  MESSAGE AFTER: a,b,c,alpha,beta,gamma=" << xvasp.str.a << "," << xvasp.str.b << "," << xvasp.str.c << "," << xvasp.str.alpha << "," << xvasp.str.beta << "," << xvasp.str.gamma << endl;
      ...
      if (Krun) {
        ...
        if (aurostd::kvpair2bool(xvasp.INCAR, "IBRION", "=")) {
          const string incar_input_old = "IBRION=" + aurostd::kvpair2string(xvasp.INCAR, "IBRION", "=");
          if (incar_input == incar_input_old) {
            Krun = false;
          }
        }
      ...
      if (Krun) {
        ...
        // ADD LINES
        if (vflags.KBIN_VASP_INCAR_VERBOSE) {
          xvasp.INCAR << "# Performing " << operation << " [AFLOW] start" << endl;
        }
      ...
      // START - load INCAR into xvasp, modify, then write out new INCAR
      Krun = (Krun && VASP_Reread_INCAR(xvasp)); // preload incar
      Krun = (Krun && KBIN::XVASP_INCAR_PREPARE_GENERIC("SYMPREC", xvasp, vflags, "", 0, 0.0, false));
      if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
        Krun = (Krun && aurostd::stringstream2file(xvasp.INCAR, string(xvasp.Directory + "/INCAR"))); // write out incar
      }
      // END - load INCAR into xvasp, modify, then write out new INCAR
      ...
      // START - load INCAR into xvasp, modify, then write out new INCAR
      Krun = (Krun && VASP_Reread_INCAR(xvasp)); // preload incar
      Krun = (Krun && KBIN::XVASP_INCAR_PREPARE_GENERIC("SYM", xvasp, vflags, "", 0, 0.0, false));
      if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
        Krun = (Krun && aurostd::stringstream2file(xvasp.INCAR, string(xvasp.Directory + "/INCAR"))); // write out incar
      }
      // END - load INCAR into xvasp, modify, then write out new INCAR
      ...
      if (Krun) {
        kflags.KBIN_MPI_OPTIONS = string("ulimit -s unlimited");
      } //+string(" && ")+kflags.KBIN_MPI_OPTIONS;
      if (Krun && VERBOSE) {
        aus << "MMMMM  MESSAGE applied FIX=\"" << fix << "\"" << Message(__AFLOW_FILE__, aflags) << endl;
        aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
      }
    } else {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "Unknown fix: \"" + fix + "\"", _INPUT_ILLEGAL_); // CO20210315
    }
    ...
    if (fix.find("KPOINTS") != string::npos) {
      reset_xfixes = true;
    } else if (fix.find("POSCAR_SCALE") != string::npos) {
      reset_xfixes = true;
    } else if (fix.find("POSCAR_VOLUME") != string::npos) {
      reset_xfixes = true;
    } else if (fix.find("RECYCLE_CONTCAR") != string::npos) {
      reset_xfixes = true;
    }
    // add others here
    if (reset_xfixes) {
      for (size_t i = xfixed.vxscheme.size() - 1; i < xfixed.vxscheme.size(); i--) { // go backwards
        const string& scheme = xfixed.vxscheme[i];
        if (scheme.find("ALGO=") != string::npos) {
          if (VERBOSE) {
            aus << "MMMMM  MESSAGE clearing \"" << scheme << "\" from xfixed (FIX=\"" << fix << "\")" << Message(__AFLOW_FILE__, aflags) << endl;
            aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
          }
...
namespace KBIN {
  bool XVASP_Afix_IgnoreFix(const string& _fix, const _vflags& vflags) {
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.flag("FIX:ALL")) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.flag(\"FIX:ALL\")=" << vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.flag("FIX:" + _fix) << endl;
      }
      return true;
    }
    ...
    if (vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.flag("FIX:" + _fix)) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.flag(\"FIX:" + _fix + "\")=" << vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.flag("FIX:" + _fix) << endl;
      }
      return true;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " generic_fix_string[1]=" << fix << endl;
    }

    if (vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.flag("FIX:" + fix)) {
      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.flag(\"FIX:" + fix + "\")=" << vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.flag("FIX:" + fix) << endl;
      }
      return true;
    }
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " generic_fix_string[2]=" << fix << endl;
    }
    ...
    // nparc and nparn
    aurostd::StringSubstInPlace(fix, "NPARC", "NPAR");
    aurostd::StringSubstInPlace(fix, "NPARN", "NPAR");

    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " generic_fix_string[3]=" << fix << endl;
    }
...
namespace KBIN {
  bool XVASP_Afix(const string& mode, int& submode, bool try_last_ditch_efforts, aurostd::xoption& xfixed, _xvasp& xvasp, _kflags& kflags, _vflags& vflags, _aflags& aflags, ofstream& FileMESSAGE) { // CO20200624 - adding submode
    ...
    // maintain this feedback system to ensure aflow doesn't keep spinning its wheels on the same fixes
    const bool LDEBUG = (false || VERBOSE_MONITOR_VASP || _DEBUG_IVASP_ || XHOST.DEBUG);
    const string function = "KBIN::XVASP_Afix";
    string file_error;
    stringstream aus;

    const int submode_orig = submode; // keep original, restore later if necessary
    const _xvasp xvasp_orig(xvasp); // keep original, restore later if necessary
    const _kflags kflags_orig(kflags); // keep original, restore later if necessary
    const _vflags vflags_orig(vflags); // keep original, restore later if necessary
    ...
    if (mode == "BRMIX") {
      ...
      fix = "ALGO=VERYFAST";
      if (XVASP_Afix_IgnoreFix(fix, vflags)) {
        Krun = false;
      }
      Krun = (Krun && XVASP_Afix_ApplyFix(fix, xfixed, xvasp, kflags, vflags, aflags, FileMESSAGE));
    } else if (mode == "CSLOSHING") {
      if (submode < 0) {
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "no submode set: \"" + mode + "\"", _INPUT_ILLEGAL_);
      } // CO20210315
      if (submode == 0) { // try ALGO=NORMAL //CO20200624
        ...
        if (!Krun) {
          Krun = true;
          submode++;
        } // reset and go to the next solution
      }
      ...
      submode += submode_increment;
      submode_increment = 1; // increment and reset
      ...
      if (submode == 0) { // lower NCPUS
        ...
        bool Krun2 = true;
        fix = "NCPUS";
        const bool ignorefix2 = XVASP_Afix_IgnoreFix(fix, vflags);
        if (ignorefix2) {
          Krun2 = false;
        }
        ...
        bool Krun3 = true;
        fix = "RECYCLE_CONTCAR"; // recycle contcar if possible
        if (XVASP_Afix_IgnoreFix(fix, vflags)) {
          Krun3 = false;
        }
        ...
        if (!Krun) { // remove fixes before going to next submode
          if (!ignorefix2) {
            fix = "NCPUS_RESTORE";
            XVASP_Afix_ApplyFix(fix, xfixed, xvasp, kflags, vflags, aflags, FileMESSAGE);
            xfixed.flag(fix, false);
          }
        }
        if (!Krun) {
          Krun = true;
      ...
      if (Krun) {
        xfixed.flag("NBANDS_EXHAUSTED", false);
      } // if the NBANDS error comes up, the ONLY solution is to increase NBANDS //let it continue resetting "NBANDS_EXHAUSTED", no problem... //remember, there are other reasons for fix=="NBANDS++"
      Krun = (Krun && XVASP_Afix_ApplyFix(fix, xfixed, xvasp, kflags, vflags, aflags, FileMESSAGE));
      ...
      if (submode == 0) { // AMIX/BMIX fixes, helps here: ICSD/LIB/CUB/Pd1Tm1_ICSD_649071
        bool Krun1 = true;
        fix = "AMIX=0.1";
        const bool ignorefix1 = XVASP_Afix_IgnoreFix(fix, vflags);
        if (ignorefix1) {
          Krun1 = false;
        }
        Krun1 = (Krun1 && XVASP_Afix_ApplyFix(fix, xfixed, xvasp, kflags, vflags, aflags, FileMESSAGE));
      ...
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "unknown mode: \"" + mode + "\"", _INPUT_ILLEGAL_); // CO20210315
    }
    ...
    if (LDEBUG) {
      aus << __AFLOW_FUNC__ << " Krun=" << Krun << " [1]" << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }

    try_last_ditch_effort = try_last_ditch_efforts;
    if (mode == "MEMORY") {
      try_last_ditch_effort = false;
    } // changing POSCAR doesn't help
    else if (mode == "MPICH11") {
      try_last_ditch_effort = false;
    } // changing POSCAR doesn't help
    else if (mode == "MPICH139") {
    ...
    if (LDEBUG) {
      aus << __AFLOW_FUNC__ << " Krun=" << Krun << " [2]" << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }
    ...
    if (LDEBUG) {
      aus << __AFLOW_FUNC__ << " Krun=" << Krun << " [3]" << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }
    ...
    if (LDEBUG) {
      aus << __AFLOW_FUNC__ << " Krun=" << Krun << " [4]" << endl;
      aurostd::PrintMessageStream(FileMESSAGE, aus, XHOST.QUIET);
    }
    ...
    if (file_error.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No file_error specified", _INPUT_ILLEGAL_);
    } // CO20200624
    if (xvasp.aopts.flag("FLAG::AFIX_DRYRUN") == false) {
...
namespace KBIN {
  xstructure GetMostRelaxedStructure(string directory) {
    ...
    if (XHOST.vext.size() != XHOST.vcat.size()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "XHOST.vext.size()!=XHOST.vcat.size(), aborting", _RUNTIME_ERROR_); // CO20200404
    }

    ///////////////////////////////////////////////////////////////////////////////////////////////
    // READ POSCAR.orig
    const string file_poscar_tmp = aurostd::TmpFileCreate("POSCAR.tmp");
    ...
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found_POSCAR && aurostd::FileExist(directory + "/POSCAR.bands" + XHOST.vext[iext])) {
        found_POSCAR = true;
        aurostd::execute(XHOST.vcat.at(iext) + " " + directory + "/POSCAR.bands" + XHOST.vext[iext] + " > " + file_poscar_tmp);
        POSCARfile = file_poscar_tmp;
      }
    }
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found_POSCAR && aurostd::FileExist(directory + "/POSCAR.static" + XHOST.vext[iext])) {
        found_POSCAR = true;
        aurostd::execute(XHOST.vcat.at(iext) + " " + directory + "/POSCAR.static" + XHOST.vext[iext] + " > " + file_poscar_tmp);
        POSCARfile = file_poscar_tmp;
      }
    }
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found_POSCAR && aurostd::FileExist(directory + "/POSCAR.relax1" + XHOST.vext[iext])) {
        found_POSCAR = true;
        aurostd::execute(XHOST.vcat.at(iext) + " " + directory + "/POSCAR.relax1" + XHOST.vext[iext] + " > " + file_poscar_tmp);
        POSCARfile = file_poscar_tmp;
      }
    }
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found_POSCAR && aurostd::FileExist(directory + "/POSCAR.relax2" + XHOST.vext[iext])) {
        found_POSCAR = true;
        aurostd::execute(XHOST.vcat.at(iext) + " " + directory + "/POSCAR.relax2" + XHOST.vext[iext] + " > " + file_poscar_tmp);
        POSCARfile = file_poscar_tmp;
      }
    }
    if (!found_POSCAR) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No POSCAR[.bands|.static|.relax2|.relax1][.EXT] found in the directory, aborting", _INPUT_MISSING_); // CO20200404
    }

    const xstructure xstr_name(POSCARfile, IOVASP_POSCAR); // import xstructure from filename
    if (aurostd::FileExist(file_poscar_tmp)) {
      aurostd::RemoveFile(file_poscar_tmp);
    }
    return xstr_name;
  }
} // namespace KBIN
...
// ***************************************************************************//
namespace KBIN {
  vector<string> ExtractAtomicSpecies(const string& directory) {
    ofstream FileMESSAGE;
    return ExtractAtomicSpecies(directory, FileMESSAGE);
  } // CO20200404 - added ofstream
  vector<string> ExtractAtomicSpecies(const string& directory, ofstream& FileMESSAGE) { // CO20200404 - added ofstream
    ...
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " most relaxed structure:" << endl << xstr_name << endl;
    }
    ...
    if (AtomNames.empty()) {
      for (i = 0; i < xstr_name.species.size(); i++) {
        if (!aurostd::RemoveWhiteSpacesFromTheFrontAndBack(xstr_name.species[i]).empty()) {
          ...
          for (j = 0; j < xstr_name.num_each_type[i]; j++) {
            AtomNames.push_back(xstr_name.species[i]);
          }
        } else {
          AtomNames.clear();
          break;
        } // go on to next extraction method
      }
    }
    // ********************************************************************************
    // READ OUTCAR
    if (AtomNames.empty()) {
      ...
      if (aurostd::FileExist(directory + "./OUTCARfile.tmp")) {
        aurostd::RemoveFile(directory + "./OUTCARfile.tmp"); // keep for legacy
      }
      string OUTCARfile;
      // "" look for just OUTCAR
      const vector<string> vstages{"", ".bands", ".static", ".relax", ".relax2", ".relax1", ".orig"};
      uint istage = 0;
      uint iext = 0;
      for (istage = 0; istage < vstages.size() && OUTCARfile.empty(); istage++) {
        if (aurostd::FileExist(directory + "/OUTCAR" + vstages[istage])) {
          OUTCARfile = directory + "/OUTCAR" + vstages[istage];
        }
      }
      ...
      for (istage = 0; istage < vstages.size() && OUTCARfile.empty(); istage++) {
        for (iext = 0; iext < XHOST.vext.size() && OUTCARfile.empty(); iext++) {
          if (aurostd::FileExist(directory + "/OUTCAR" + vstages[istage] + XHOST.vext[iext])) {
            aurostd::compressfile2tempfile(directory + "/OUTCAR" + vstages[istage] + XHOST.vext[iext], OUTCARfile, delete_OUTCARfile);
          }
        }
      }
      if (OUTCARfile.empty()) {
        if (delete_OUTCARfile) {
          aurostd::RemoveFile(OUTCARfile);
        }
        throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No OUTCAR[.static][.EXT] found in the directory, aborting", _INPUT_MISSING_); // CO20200404
      }

      if (LDEBUG) {
        cerr << __AFLOW_FUNC__ << " OUTCARfile=" << OUTCARfile << endl;
      }
      ...
      for (i = 0; i < vpotcars.size(); i++) {
        if (LDEBUG) {
          cerr << __AFLOW_FUNC__ << R"( found "TITEL" line=")" << vpotcars[i] << "\"" << endl;
        }
        aurostd::string2tokens(vpotcars[i], vpp, " ");
        if (vpp.size() < 4) {
          throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, OUTCARfile + R"( has weird "TITEL" line: ")" + vpotcars[i] + "\"", _FILE_CORRUPT_); // CO20200404
        }
        AtomSpeciesName.push_back(vpp[3]);
        aurostd::VASP_PseudoPotential_CleanName_InPlace(AtomSpeciesName.back());
        for (j = 0; j < xstr_name.num_each_type[i]; j++) {
          AtomNames.push_back(AtomSpeciesName.back());
        }
      }
    }
    // ********************************************************************************
    // ERROR
    if (AtomNames.empty()) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "AtomNames cannot be extracted", _RUNTIME_ERROR_);
    } // CO20200404

    for (i = 0; i < AtomNames.size(); i++) {
      aurostd::VASP_PseudoPotential_CleanName_InPlace(AtomNames[i]);
    } // repetita iuvant
    if (LDEBUG) {
      cerr << __AFLOW_FUNC__ << " AtomNames=" << aurostd::joinWDelimiter(AtomNames, ",") << endl;
    }
...
namespace KBIN {
  double ExtractEfermiOUTCAR(string directory) {
    ...
    const string file_outcar_tmp = aurostd::TmpFileCreate("OUTCAR.tmp");
    if (aurostd::FileExist(directory + "./OUTCARfile.tmp")) {
      aurostd::RemoveFile(directory + "./OUTCARfile.tmp");
    }
    ...
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found_OUTCAR && aurostd::FileExist(directory + "/OUTCAR.static" + XHOST.vext[iext])) {
        found_OUTCAR = true;
        aurostd::execute(XHOST.vcat.at(iext) + " " + directory + "/OUTCAR.static" + XHOST.vext[iext] + " > " + file_outcar_tmp);
    ...
    for (unsigned int i = 0; i < vlines.size(); i++) {
      if (vlines[i].find(anchor_word_Efermi) != string::npos) {
        strline.clear();
        strline.str(vlines[i]);
        strline >> stmp >> stmp >> Efermi;
      }
    }
    if (aurostd::FileExist(file_outcar_tmp)) {
      aurostd::RemoveFile(file_outcar_tmp);
    }
...
namespace KBIN {
  ...
  string ExtractSystemName(const string& directory) {
    ...
    if (system_name.empty()) {
      const string message = "Could not extract system.";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_CORRUPT_);
    }
    aurostd::StringSubstInPlace(system_name, ":.", "."); // CO20200731 - patching bug in system name generation
    aurostd::StringSubstInPlace(system_name, "@ARUN.A", ":ARUN.A"); // CO20200731 - patching for AEL/AGL ARUNs - @ in filenames are NOT good
    return system_name;
  }
  ...
  string ExtractSystemNameFromAFLOWIN(const string& _directory) {
    ...
    string system_name;
    const string aflowin_path = directory + "/" + _AFLOWIN_;
    if (!aurostd::FileExist(aflowin_path)) {
      const string message = "Could not find file " + aflowin_path + ".";
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_NOT_FOUND_);
    }
    // ME20200525 - Need to take potential white space between [AFLOW],
    // SYSTEM, and = into account.
    const string AflowIn = aurostd::RemoveComments(aurostd::file2string(aflowin_path)); // for safety //CO20180502
    vector<string> vAflowIn;
    aurostd::string2vectorstring(AflowIn, vAflowIn);
    const uint nlines = vAflowIn.size();
    for (uint iline = 0; iline < nlines && system_name.empty(); iline++) {
      if (vAflowIn[iline].find("SYSTEM") != string::npos) {
        ...
        const string::size_type t = line.find_first_of("=");
  ...
  string ExtractSystemNameFromVASP(const string& _directory) {
    ...
    if (LDEBUG) {
      cerr << XPID << "KBIN::ExtractSystemName: directory=" << directory << endl;
    }
    ...
    if (LDEBUG) {
      cerr << XPID << "KBIN::ExtractSystemName: [1]" << endl;
    }
    const string doscarfile_tmp = aurostd::TmpFileCreate("DOSCAR.tmp");
    if (aurostd::FileExist(directory + "/DOSCARfile.tmp")) {
      aurostd::RemoveFile(directory + "/DOSCARfile.tmp");
    }
    ...
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/DOSCAR.static" + XHOST.vext[iext])) {
        found = true;
        aurostd::execute(XHOST.vcat.at(iext) + " \"" + directory + "/DOSCAR.static" + XHOST.vext[iext] + "\"" + " > " + doscarfile_tmp);
        DOSCARfile = doscarfile_tmp;
        found = true;
      }
    }
    if (found) {
      ...
      if (aurostd::FileExist(doscarfile_tmp)) {
        aurostd::RemoveFile(doscarfile_tmp);
      }
      ...
      fs::current_path(directory.c_str()); // Jump into the data directory
      const string data_dir = aurostd::getPWD(); // CO20191112
      fs::current_path(work_dir.c_str()); // Jump back into the work directory  //CO20191112

      // Check whether it is MAGNETIC DATABASE
      bool FLAG_MAGNETIC = false;
      const string abs_dir(data_dir); // convert the char type of data_dir into string type
      if (abs_dir.find("LIB3") != string::npos) {
        FLAG_MAGNETIC = true;
      }
          ...
          if (datasize >= 2) {
            if (FLAG_LAST_ENTRY_BANDS) {
              if (FLAG_SYSTEMNAME_UNKNOWN && FLAG_DIRECTORY_ICSD) {
                ...
                SystemName = data.at(datasize - 3) + "." + data.at(datasize - 2);
              }
            } else {
              if (FLAG_SYSTEMNAME_UNKNOWN && FLAG_DIRECTORY_ICSD) {
                SystemName = Last_Entry;
              } else {
                SystemName = data.at(datasize - 2) + "." + data.back();
              }
            }
          } else {
    ...
    // TRY OUTCAR.relax2
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/OUTCAR.relax2" + XHOST.vext[iext])) {
        found = true;
        const xOUTCAR outcar(directory + "/OUTCAR.relax2" + XHOST.vext[iext]);
    ...
    // TRY OUTCAR.relax1
    for (size_t iext = 0; iext < XHOST.vext.size(); iext++) {
      if (!found && aurostd::FileExist(directory + "/OUTCAR.relax1" + XHOST.vext[iext])) {
        found = true;
        const xOUTCAR outcar(directory + "/OUTCAR.relax1" + XHOST.vext[iext]);
        SystemName = outcar.SYSTEM;
        return SystemName;
      }
    }
    // NOTHING FOUND !!!
    if (!found) {
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, "No DOSCAR[.static][.EXT], OUTCAR[.relax1|.relax2]][.EXT] found in the directory, aborting", _FILE_CORRUPT_); // CO20200624
    }
    return "";
  }
} // namespace KBIN
...
namespace KBIN {
  ...
  xstructure GetPOSCARXStructureFromDirectory(const string& directory, const int iomode, const int index) {
    ...
    if (!ExtractPOSCARStringStreamFromDirectory(directory, poscar, index)) {
      const string message = "Could not find valid " + _AFLOWIN_ + " in " + directory;
      throw aurostd::xerror(__AFLOW_FILE__, __AFLOW_FUNC__, message, _FILE_NOT_FOUND_);
    }
  ...
  bool ExtractPOSCARStringStreamFromAFLOWIN(const string& AflowIn, stringstream& poscar, const int index) {
    if (index == 0) {
      const string message = "Index cannot be 0";
  ...
  xstructure GetPOSCARXStructureFromAFLOWIN(const string& AflowIn, const int iomode, const int index) {
    ...
    if (!ExtractPOSCARStringStreamFromAFLOWIN(AflowIn, poscar, index)) {
      const string message = "Invalid " + _AFLOWIN_;
